"""Timezone-aware clock for Cryozen.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``CRYOZEN_TIMEZONE`` env var, then ``timezone`` in ``~/.cryozen-agent/config.yaml``, else server-local
...
# CRYOZEN_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``CRYOZEN_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("CRYOZEN_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without cryozen_cli importable.
        try:
