"""Process bootstrap for Cryozen entry points: Windows UTF-8 stdio, import-path
...
point (``cryozen``, ``cryozen-agent``, ``cryozen-acp``, ``gateway.run``, ``batch_runner``,
...
here may pull in a Cryozen package that a project-local directory could shadow.
"""
...
def _patch_urllib3_create_connection(module) -> None:
    ...
    if getattr(module.create_connection, "_cryozen_happy_eyeballs", False):
        return
    ...
    _urllib3_racer._cryozen_happy_eyeballs = True  # type: ignore[attr-defined]
...
class _Urllib3ConnectionPatcher(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    ...
    Importing urllib3 eagerly costs ~50 ms on every CLI start, and ``cryozen`` / the TUI
...
def install_happy_eyeballs_socket_connect() -> None:
    ...
    """
    if getattr(socket.create_connection, "_cryozen_happy_eyeballs", False):
        return
    ...
    _socket_racer._cryozen_happy_eyeballs = True  # type: ignore[attr-defined]
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``cryozen_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``cryozen_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Cryozen modules.

    Cryozen ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the Cryozen root is *relocated* to the front, not merely inserted when
    ...
    """
    root = src_root or os.environ.get("CRYOZEN_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``CRYOZEN_LAZY_INSTALL_TARGET``) on ``sys.path``.
    ...
    """
    if not os.environ.get("CRYOZEN_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
def export_scratch_tmp_env() -> None:
    """Point ``TMPDIR``/``TMP``/``TEMP`` at ``CRYOZEN_HOME/cache/scratch`` unless the user set them.

    System temp is tmpfs on most Linux hosts and containers; Cryozen' browser profiles, PTY
    probes and every ``tempfile`` default a child script makes would eat RAM there. Runs at
    import so every entry point and every child they spawn inherits it; ``cryozen_cli.main``
    re-runs it after ``--profile`` re-homes the process. Never raises.
    """
    try:
        from cryozen_constants import export_scratch_tmp_env as _export
        _export()
    except Exception:
        pass  # a missing/unwritable home just leaves the system temp dir in place


# Apply on import — entry points only need ``import cryozen_bootstrap`` first.
