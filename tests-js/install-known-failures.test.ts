...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/cryozen.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/cryozen.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'cryozen-update',
  error: 'E2E ASSERTION FAILED: cryozen update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('cryozen.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'cryozen-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured cryozen desktop spec; update completed',
      logs: { desktop: '[cryozen] [updates] no staged updater; surfacing manual `cryozen update` for CLI install at C:/install\n[cryozen] [updates] manual: cryozen update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: cryozen update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> cryozen-update (v2026.3.12 -> HEAD)'
