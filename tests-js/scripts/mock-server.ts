 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `cryozen serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Cryozen should add
...
export const APPROVAL_COMMAND = 'rm -rf /tmp/cryozen-e2e-approval-probe'
 ...
 * `E2E_SAY(code-farmer)[{at}cryozen Reply with B.] E2E_SAY(cryozen)[B]`.
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `cryozen-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated CRYOZEN_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `cryozen-dev-mock-${Date.now()}`))
  const cryozenHome = nodePath.join(root, 'cryozen-home')
...
function writeMockConfig(cryozenHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(cryozenHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(cryozenHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.cryozenHome, mock.url)
  console.log(`  CRYOZEN_HOME: ${sandbox.cryozenHome}`)
  ...
  const env: Record<string, string> = {
    ...
    CRYOZEN_DESKTOP_CRYOZEN_ROOT: repoRoot,
    CRYOZEN_DESKTOP_APP_NAME: `CryozenDevMock-${Date.now()}`,
  }
