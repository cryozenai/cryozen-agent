...
from cryozen_state_errors import STORAGE_RECOVERY_DOCS_URL, classify_persistence_error, is_disk_full_error
...
_DOCTOR = "Run `cryozen {profile_arg}doctor --fix` to diagnose and repair."

# cause -> (code, gloss, action). "disk" is split by is_disk_full_error at lookup time.
_STORAGE_FAILURES: dict[str, tuple[str, str, str]] = {
    "locked": (
        "storage_locked",
        "the session database is locked by another Cryozen process",
        "Wait a moment and try again; if it persists, stop the other Cryozen process "
        "(`cryozen {profile_arg}gateway stop`).",
    ),
    "disk_full": (
        "disk_full",
        "the disk is full",
    ...
    "corrupt": (
        ...
        _DOCTOR + " Recovery: `cryozen {profile_arg}sessions recover --source <state.db> --inspect-only`.",
    ),
    "fts_index": (
        "storage_index_corrupt",
        "the session search index is damaged (the messages themselves are intact)",
        "Run `cryozen {profile_arg}doctor --fix` (or `cryozen {profile_arg}sessions repair`) to rebuild it.",
    ),
    "replaced": (
        "storage_replaced",
        "the session database file was replaced while Cryozen was running",
        "Stop Cryozen (`cryozen {profile_arg}gateway stop`), run `cryozen {profile_arg}doctor`, then start it again.",
    ),
    ...
    "deleted_wal": (
        ...
        "Nothing is lost. Quit every Cryozen process on this profile (Desktop app, "
        "`cryozen {profile_arg}gateway stop`, dashboard, cron), run `cryozen {profile_arg}doctor` — it names "
        "any process still holding the log — then start Cryozen again. Do not run `doctor --fix` or delete "
    ...
    "turn_lease": (
        "storage_busy",
        "another Cryozen process took over this session",
