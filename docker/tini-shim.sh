...
# Cryozen WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: CRYOZEN_TINI_SHIM_TARGET overrides the `/init` path (and
# CRYOZEN_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${CRYOZEN_TINI_SHIM_TARGET:-/init}"
WRAPPER="${CRYOZEN_TINI_SHIM_WRAPPER:-/opt/cryozen/docker/main-wrapper.sh}"
