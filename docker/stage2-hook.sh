...
#
# Wired into the image as /etc/cont-init.d/01-cryozen-setup by the
...
CRYOZEN_HOME="${CRYOZEN_HOME:-/opt/data}"
INSTALL_DIR="/opt/cryozen"

# Drop to cryozen via s6-setuidgid, but skip it when already non-root.
as_cryozen() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid cryozen "$@"; }
...
# starts non-root. The baked install tree under /opt/cryozen is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $CRYOZEN_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass CRYOZEN_UID/CRYOZEN_GID — or the PUID/PGID aliases — which the
...
# `id -u cryozen` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# cryozen UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u cryozen)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-cryozen UID).
...
and the baked /opt/cryozen install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e CRYOZEN_UID=\$(id -u) -e CRYOZEN_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the cryozen user to that UID/GID at boot and chowns the data
...
# --- Bootstrap CRYOZEN_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid cryozen mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom CRYOZEN_HOME paths whose parents only
# root can create (e.g. `CRYOZEN_HOME=/home/cryozen/.cryozen-agent` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for CRYOZEN_UID/CRYOZEN_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  CRYOZEN_UID/
# CRYOZEN_GID still win when both are set.  See #15290, salvages #25872.
CRYOZEN_UID="${CRYOZEN_UID:-${PUID:-}}"
CRYOZEN_GID="${CRYOZEN_GID:-${PGID:-}}"

if [ -n "${CRYOZEN_UID:-}" ] && validate_uid_gid "$CRYOZEN_UID" && [ "$CRYOZEN_UID" != "$(id -u cryozen)" ]; then
    echo "[stage2] Changing cryozen UID to $CRYOZEN_UID"
    usermod -u "$CRYOZEN_UID" cryozen
fi
if [ -n "${CRYOZEN_GID:-}" ] && validate_uid_gid "$CRYOZEN_GID" && [ "$CRYOZEN_GID" != "$(id -g cryozen)" ]; then
    echo "[stage2] Changing cryozen GID to $CRYOZEN_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$CRYOZEN_GID" cryozen 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised cryozen user (UID 10000)
...
# cryozen process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes cryozen. Idempotent across container
...
#   - cryozen is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G cryozen 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] cryozen already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" cryozen 2>/dev/null; then
        echo "[stage2] Added cryozen to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group cryozen failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When CRYOZEN_UID is remapped or the top-level $CRYOZEN_HOME isn't owned by
# the runtime cryozen UID, restore ownership to cryozen — but ONLY for the
# directories cryozen actually writes to. The full $CRYOZEN_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of cryozen-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_cryozen_uid=$(id -u cryozen)
...
chown_cryozen_tree() {
    ...
    chown -R cryozen:cryozen "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_cryozen_owner() {
    target="$1"
    find "$target" \( ! -user cryozen -o ! -group cryozen \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$CRYOZEN_HOME" 2>/dev/null)" != "$actual_cryozen_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $CRYOZEN_HOME (targeted) to cryozen ($actual_cryozen_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $CRYOZEN_HOME: chown the directory itself (not its contents)
    # so cryozen can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$CRYOZEN_HOME"; then
        :
    else
        chown cryozen:cryozen "$CRYOZEN_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $CRYOZEN_HOME failed (rootless container?) — continuing"
    fi
    # Cryozen-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by cryozen (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$CRYOZEN_HOME/$sub" ] && tree_has_non_cryozen_owner "$CRYOZEN_HOME/$sub"; then
            chown_cryozen_tree "$CRYOZEN_HOME/$sub"
        fi
    done
fi
...
# cryozen user. Hosted/container instances keep mutable state under
# $CRYOZEN_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# CRYOZEN_DISABLE_LAZY_INSTALLS=1. Keeping /opt/cryozen root-owned and
...
# $CRYOZEN_HOME/lazy-packages dir on the data volume (Dockerfile sets
# CRYOZEN_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to cryozen
...
# Always reset ownership of $CRYOZEN_HOME/profiles to cryozen on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> cryozen …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as cryozen and walks
...
if [ -d "$CRYOZEN_HOME/profiles" ] && tree_has_non_cryozen_owner "$CRYOZEN_HOME/profiles"; then
    chown_cryozen_tree "$CRYOZEN_HOME/profiles"
fi

# Always reset ownership of $CRYOZEN_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged cryozen runtime even
...
# gate as profiles/).
if [ -d "$CRYOZEN_HOME/cron" ] && tree_has_non_cryozen_owner "$CRYOZEN_HOME/cron"; then
    chown_cryozen_tree "$CRYOZEN_HOME/cron"
fi

# Always ensure logs/gateways is cryozen-owned (#45258). Formerly healed by
...
# top-level $CRYOZEN_HOME is mis-owned, so a warm volume with cryozen-owned
# CRYOZEN_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid cryozen mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as cryozen.
if [ -d "$CRYOZEN_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$CRYOZEN_HOME/logs/gateways"; then
        :
    else
        chown cryozen:cryozen "$CRYOZEN_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# cryozen pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged cryozen gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $CRYOZEN_HOME is
...
# and cron/.
if [ -d "$CRYOZEN_HOME/platforms/pairing" ] && tree_has_non_cryozen_owner "$CRYOZEN_HOME/platforms/pairing"; then
    chown_cryozen_tree "$CRYOZEN_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$CRYOZEN_HOME/pairing" ] && tree_has_non_cryozen_owner "$CRYOZEN_HOME/pairing"; then
    chown_cryozen_tree "$CRYOZEN_HOME/pairing"
fi

# Reset ownership of cryozen-owned top-level state files on every boot.
# The targeted data-volume chown above only covers cryozen-owned
# *subdirectories*; loose state files living directly under $CRYOZEN_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> cryozen …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged cryozen runtime then hits
...
# sweep so host-owned files in a bind-mounted $CRYOZEN_HOME are never
...
# entries of cryozen_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$CRYOZEN_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$CRYOZEN_HOME/$f"; then
            :
        else
            chown cryozen:cryozen "$CRYOZEN_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the cryozen runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$CRYOZEN_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$CRYOZEN_HOME/config.yaml"; then
        :
    else
        chown cryozen:cryozen "$CRYOZEN_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$CRYOZEN_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as cryozen user ---
# Run as cryozen via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $CRYOZEN_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_cryozen mkdir -p \
    "$CRYOZEN_HOME/backups" \
    "$CRYOZEN_HOME/cron" \
    "$CRYOZEN_HOME/sessions" \
    "$CRYOZEN_HOME/logs" \
    "$CRYOZEN_HOME/logs/gateways" \
    "$CRYOZEN_HOME/hooks" \
    "$CRYOZEN_HOME/memories" \
    "$CRYOZEN_HOME/skills" \
    "$CRYOZEN_HOME/skins" \
    "$CRYOZEN_HOME/plans" \
    "$CRYOZEN_HOME/workspace" \
    "$CRYOZEN_HOME/home" \
    "$CRYOZEN_HOME/pairing" \
    "$CRYOZEN_HOME/platforms/pairing" \
    "$CRYOZEN_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/cryozen/.install_method (see Dockerfile), NOT written here into
# $CRYOZEN_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $CRYOZEN_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.cryozen-agent:/opt/data) and sometimes shared with a
...
# 'cryozen update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $CRYOZEN_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/cryozen stamp, so deleting the data-dir copy is safe).
if [ -f "$CRYOZEN_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$CRYOZEN_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$CRYOZEN_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$CRYOZEN_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$CRYOZEN_HOME/$dest"; then
            :
        else
            as_cryozen cp "$INSTALL_DIR/$src" "$CRYOZEN_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/cryozen/.env.example being present
...
# not generate one. Cryozen loads $CRYOZEN_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$CRYOZEN_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$CRYOZEN_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $CRYOZEN_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$CRYOZEN_HOME/.env" ] && ! refuse_symlinked_path "clean" "$CRYOZEN_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$CRYOZEN_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$CRYOZEN_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$CRYOZEN_HOME/.env"; then
        :
    else
        if [ ! -f "$CRYOZEN_HOME/.env" ]; then
            ...
            (umask 077 && as_cryozen touch "$CRYOZEN_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$CRYOZEN_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$CRYOZEN_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$CRYOZEN_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $CRYOZEN_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $CRYOZEN_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$CRYOZEN_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$CRYOZEN_HOME/.env"; then
        :
    else
        chown cryozen:cryozen "$CRYOZEN_HOME/.env" 2>/dev/null || true
        chmod 600 "$CRYOZEN_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `cryozen` user — without this it gets EACCES on every
...
# cryozen group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp cryozen /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted cryozen group access to the Fly Machines API socket"
    else
...
# $CRYOZEN_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `cryozen update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set CRYOZEN_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$CRYOZEN_HOME/config.yaml" ]; then
    s6-setuidgid cryozen "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$CRYOZEN_HOME/auth.json" ] && [ -n "${CRYOZEN_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$CRYOZEN_HOME/auth.json"; then
        :
    else
        printf '%s' "$CRYOZEN_AUTH_JSON_BOOTSTRAP" > "$CRYOZEN_HOME/auth.json"
        chown cryozen:cryozen "$CRYOZEN_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$CRYOZEN_HOME/auth.json"
    fi
fi
...
# can set CRYOZEN_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# CRYOZEN_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$CRYOZEN_HOME/gateway_state.json" ] && \
        [ "${CRYOZEN_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$CRYOZEN_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$CRYOZEN_HOME/gateway_state.json"
        chown cryozen:cryozen "$CRYOZEN_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$CRYOZEN_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_cryozen "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/cryozen/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Cryozen spawns for the
...
# main-wrapper.sh propagates it into the supervised ``cryozen`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-cryozen, dashboard, etc.).
