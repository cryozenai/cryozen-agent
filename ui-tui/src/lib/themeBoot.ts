 ...
 * `cryozen-boot-background` / `cryozen-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports CRYOZEN_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.cryozen-agent matches get_cryozen_home()'s default.
const bootFilePath = () => join(process.env.CRYOZEN_HOME ?? join(homedir(), '.cryozen-agent'), 'tui-theme-boot.json')

// Never touch the user's real ~/.cryozen-agent from test runs (the TS suite has no
// CRYOZEN_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into CRYOZEN_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.CRYOZEN_TUI_THEME || env.CRYOZEN_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.CRYOZEN_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.CRYOZEN_TUI_BACKGROUND
  ) {
    env.CRYOZEN_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.CRYOZEN_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.CRYOZEN_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into CRYOZEN_TUI_THEME.
