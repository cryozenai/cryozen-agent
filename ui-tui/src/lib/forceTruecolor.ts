...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CRYOZEN_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Cryozen on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CRYOZEN_TUI_TRUECOLOR=1.
