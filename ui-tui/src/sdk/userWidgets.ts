...
import { Box, Text } from '@cryozen/ink'
...
/**
 * User widget apps — Cryozen authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$CRYOZEN_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.cryozen-agent/plugins/`: files under CRYOZEN_HOME execute
...
const widgetsDir = () => join(process.env.CRYOZEN_HOME?.trim() || join(homedir(), '.cryozen-agent'), 'tui-widgets')
 ...
 *  every change, so a widget Cryozen writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Cryozen) ever writes must hot-load too — a 10s poll here read
