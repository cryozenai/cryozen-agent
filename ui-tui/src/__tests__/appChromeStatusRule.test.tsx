...
describe('StatusRule session count click target', () => {
  ...
  it('keeps status + model and drops the low-value tail on a narrow terminal', () => {
    const element = StatusRule({
      ...
      busy: false,
      cols: 44,
      cwdLabel: '~/src/cryozen-agent/apps/desktop (bb/tui-statusbar-responsive)',
      liveSessionCount: 3,
      model: 'opus-4.8',
...
describe('StatusRule notice render priority', () => {
  it('replaces the idle status with the notice text and keeps model + context', () => {
    const element = StatusRule({
      ...baseProps,
      notice: { key: 'notice.blocked', kind: 'sticky', level: 'error', text: '✕ access paused' }
    })

    const rendered = textContent(element)

    // Notice replaces the status verb slot …
    expect(rendered).toContain('✕ access paused')
  ...
  it('busy wins: the FaceTicker shows, the notice is hidden mid-turn', () => {
    const element = StatusRule({
      ...baseProps,
      busy: true,
      notice: { key: 'notice.warn', kind: 'sticky', level: 'warn', text: '⚠ 90% used' },
      turnStartedAt: Date.now()
    })
  ...
  it('colours the notice by level (error → theme error, success → statusGood)', () => {
    const errEl = StatusRule({
      ...baseProps,
      notice: { key: 'notice.blocked', kind: 'sticky', level: 'error', text: '✕ exhausted' }
    })
    ...
    const okEl = StatusRule({
      ...baseProps,
      notice: { key: 'notice.restored', kind: 'ttl', level: 'success', text: '✓ restored', ttl_ms: 8000 }
    })
  ...
  it('the notice text is the shrinkable element (flexShrink=1 + truncate-end) so a long notice ellipsizes', () => {
    ...
    const element = StatusRule({
      ...baseProps,
      cols: 50,
      notice: { key: 'notice.warn', kind: 'sticky', level: 'warn', text: longText }
    })
