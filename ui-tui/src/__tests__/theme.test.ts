...
// CRYOZEN_TUI_THEME=light (or CRYOZEN_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('Cryozen Agent')
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#E0F7FF')
...
describe('detectLightMode', () => {
  ...
  it('honors CRYOZEN_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ CRYOZEN_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ CRYOZEN_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ CRYOZEN_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ CRYOZEN_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ CRYOZEN_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets CRYOZEN_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', CRYOZEN_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors CRYOZEN_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ CRYOZEN_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ CRYOZEN_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', CRYOZEN_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', CRYOZEN_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses CRYOZEN_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ CRYOZEN_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // CRYOZEN_TUI_THEME=light would flip the base palette and make these
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ CRYOZEN_TUI_BACKGROUND: '#ffffff' })
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ CRYOZEN_TUI_BACKGROUND: '#ffffff' })
...
describe('derived tone ladder', () => {
  it('reproduces the original hand-tuned tones from seeds (reverse-engineered knobs)', async () => {
    // The ladder's knobs were grid-search fitted so the MATH lands on a set of
    // hand-tuned literals. Contract: every derived tone stays within a few RGB
    // units of its reference (imperceptible), so knob edits that drift the
    // ladder fail here instead of shipping as vibes. The seeds below are the
    // fixed calibration inputs the knobs were fitted against (test data, not
    // the shipped defaults), so the contract survives default-palette changes.
    const { buildPalette } = await importThemeWithCleanEnv()

    const calibrationDark = {
      accent: '#FFBF00',
      activeRow: '#333355',
      bg: '#101014',
      border: '#CD7F32',
    ...
    const dark = buildPalette(calibrationDark, false)
    const light = buildPalette(calibrationLight, true)

    const cases: Array<[string, string, string]> = [
      [dark.muted, '#CC9B1F', 'dark muted'],
      [dark.label, '#DAA520', 'dark label'],
      [dark.statusFg, '#C0C0C0', 'dark statusFg'],
      [dark.completionBg, '#1a1a2e', 'dark surface'],
      [dark.completionCurrentBg, '#333355', 'dark chip'],
      [dark.selectionBg, '#3a3a55', 'dark selection'],
      // Light references = liftForContrast(dark literal, white, 4.5): the
      // colors xterm's minimumContrastRatio rendered on light hosts.
      [light.muted, '#946C08', 'light muted'],
      [light.statusFg, '#6F6F6F', 'light statusFg'],
      [light.completionBg, '#F5F5F5', 'light surface'],
      [light.completionCurrentBg, '#e0d1bf', 'light chip'],
      [light.selectionBg, '#D4E4F7', 'light selection']
    ]
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  it('renders a dark-authored skin on light like minimumContrastRatio hosts do (the standardized look)', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ CRYOZEN_TUI_BACKGROUND: '#ffffff' })
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ CRYOZEN_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ CRYOZEN_TUI_BACKGROUND: '#ffffff' })
  ...
  it('base palettes are fixed points of the adaptation', async () => {
    ...
    const light = await importThemeWithEnv({ CRYOZEN_TUI_BACKGROUND: '#ffffff' })

    expect(light.fromSkin({}, {}).color).toEqual(light.LIGHT_THEME.color)
  })

  it('defaultThemeForCurrentBackground follows a late CRYOZEN_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ CRYOZEN_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
