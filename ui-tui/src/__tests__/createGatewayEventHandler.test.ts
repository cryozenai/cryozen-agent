import type { ConnectionOperationTarget } from '@cryozen/shared/gateway-events'
...
// Mock the external-URL opener so the billing-wall tests can
...
describe('createGatewayEventHandler', () => {
  ...
  it('falls back to /model from the confirm dialog when the provider has no billing URL', () => {
    const appended: Msg[] = []
    ...
    expect(confirm?.title).toContain('DeepSeek')
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'cryozen-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith("💾 Self-improvement review: Skill 'cryozen-release' patched")
  })
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.cryozen-agent/config.yaml'
      },
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33mCryozen here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `cryozen model` to select a provider, or run `cryozen setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
  ...
  it('picks the polarity-matching paired palette from gateway.ready skins', async () => {
    ...
    // Dark terminal (clean env): the dark-authored `colors` block wins. Pin the
    // host hints too: Apple_Terminal defaults to light polarity, so an
    // inherited TERM_PROGRAM would otherwise flip this on developer machines.
    vi.stubEnv('TERM_PROGRAM', '')
    vi.stubEnv('COLORFGBG', '')
    vi.stubEnv('CRYOZEN_TUI_BACKGROUND', '')
    ...
    vi.stubEnv('CRYOZEN_TUI_BACKGROUND', '#ffffff')
    createGatewayEventHandler(buildCtx(appended))({ payload: skin, type: 'skin.changed' } as any)
  ...
  it('opens a fresh session before starting voice after wake detection', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey cryozen', start_new_session: true },
      type: 'wake.detected'
    } as any)
  ...
  it('keeps the current session when wake detection disables session creation', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey cryozen', start_new_session: false },
      type: 'wake.detected'
    } as any)
  ...
  it('keeps gateway noise informational and approval out of Activity', async () => {
    ...
    onEvent({ payload: { line: 'INFO cryozen.mcp: 3 servers discovered' }, type: 'gateway.stderr' } as any)
  ...
  // ── Status notice ──────────────────────────────────────
  describe('status notice', () => {
    it('shows a notice immediately when idle (no turn in flight)', () => {
      const onEvent = createGatewayEventHandler(buildCtx([]))

      onEvent({
        payload: { key: 'notice.blocked', kind: 'sticky', level: 'error', text: '✕ access paused' },
    ...
    it('holds a notice arriving mid-turn (busy) and flushes it at message.complete', () => {
      ...
      onEvent({
        payload: { key: 'notice.warn', kind: 'sticky', level: 'warn', text: '⚠ 90% used' },
        type: 'notification.show'
      } as any)
      ...
      expect(getUiState().notice).toMatchObject({ key: 'notice.warn', text: '⚠ 90% used' })
    })
    ...
    it('flushes a held notice at interruptTurn (turn-end via ctrl-c)', () => {
      ...
      try {
        ...
        onEvent({ payload: {}, type: 'message.start' } as any)
        onEvent({
          payload: { key: 'notice.blocked', kind: 'sticky', level: 'error', text: '✕ out' },
          type: 'notification.show'
        } as any)
        expect(getUiState().notice).toBeNull()
        ...
        expect(getUiState().notice).toMatchObject({ key: 'notice.blocked', text: '✕ out' })
      } finally {
        vi.runAllTimers()
        vi.useRealTimers()
      }
    ...
    it('latest-wins: a second mid-turn notice replaces the first held one', () => {
      ...
      onEvent({
        payload: { key: 'notice.blocked', kind: 'sticky', level: 'error', text: '✕ exhausted' },
        type: 'notification.show'
      } as any)
      ...
      expect(getUiState().notice).toMatchObject({ key: 'notice.blocked', text: '✕ exhausted' })
    })
    ...
    it('clears a visible notice only when the clear key matches (no-op otherwise)', () => {
      ...
      onEvent({
        payload: { key: 'notice.info', kind: 'sticky', level: 'warn', text: '• heads-up' },
        type: 'notification.show'
      } as any)
      expect(getUiState().notice).not.toBeNull()

      // Stale/late clear for a DIFFERENT key must not wipe the newer notice.
      onEvent({ payload: { key: 'notice.other' }, type: 'notification.clear' } as any)
      expect(getUiState().notice).toMatchObject({ key: 'notice.info' })

      // Matching key clears.
      onEvent({ payload: { key: 'notice.info' }, type: 'notification.clear' } as any)
      expect(getUiState().notice).toBeNull()
    })
    ...
    it('a ttl notice self-expires after ttl_ms when applied while idle', () => {
      ...
      try {
        ...
        onEvent({
          payload: { key: 'notice.restored', kind: 'ttl', level: 'success', text: '✓ access restored', ttl_ms: 8000 },
          type: 'notification.show'
        } as any)
        expect(getUiState().notice).toMatchObject({ key: 'notice.restored' })
    ...
    it('R3-C2: a ttl notice self-expires even when statusTimer is also armed (timer isolation)', () => {
      ...
      try {
        ...
        onEvent({
          payload: { key: 'notice.restored', kind: 'ttl', level: 'success', text: '✓ restored', ttl_ms: 8000 },
          type: 'notification.show'
        } as any)
        expect(getUiState().notice).toMatchObject({ key: 'notice.restored' })
    ...
    it('latest-wins cancels a prior ttl timer so it cannot wipe the newer notice', () => {
      ...
      try {
        ...
        onEvent({
          payload: { id: 'a', key: 'notice.restored', kind: 'ttl', level: 'success', text: '✓ a', ttl_ms: 5000 },
          type: 'notification.show'
        } as any)
        ...
        onEvent({
          payload: { id: 'b', key: 'notice.blocked', kind: 'sticky', level: 'error', text: '✕ b' },
          type: 'notification.show'
        } as any)
    ...
    it('sticky survives a turn: applied with no pending notice does not clear it', () => {
      ...
      expect(getUiState().notice).toMatchObject({ key: 'notice.blocked' })
    ...
    it('reset()/fullReset() clears pending + timer + visible notice (no cross-session leak)', () => {
      ...
      try {
        ...
        onEvent({
          payload: { key: 'notice.blocked', kind: 'sticky', level: 'error', text: '✕ A cut' },
          type: 'notification.show'
        } as any)
        onEvent({ payload: {}, type: 'message.start' } as any)
        onEvent({
          payload: { key: 'notice.warn', kind: 'sticky', level: 'warn', text: '⚠ A 90%' },
          type: 'notification.show'
        } as any)
        expect(getUiState().notice).toMatchObject({ key: 'notice.blocked' })
    ...
    it('ignores a notification.show with no text', () => {
      ...
      onEvent({ payload: { key: 'notice.warn', level: 'warn' }, type: 'notification.show' } as any)
      expect(getUiState().notice).toBeNull()
    })
  })
