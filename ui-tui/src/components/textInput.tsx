import type { InputEvent, Key } from '@cryozen/ink'
import * as Ink from '@cryozen/ink'
 ...
 * but that accepts `key.meta` on macOS — and cryozen-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.CRYOZEN_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which cryozen-ink decodes as meta+'d'; without this branch it
