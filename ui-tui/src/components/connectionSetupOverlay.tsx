import { Box, Text, useInput } from '@cryozen/ink'
import type {
...
} from '@cryozen/shared/gateway-events'
...
const FAILURE_LINE = 'That did not work.'

const hasFailed = (target: ConnectionOperationTarget): boolean => target.state === 'failed'
...
function RetryPhase({ action, more, notice, sending, t, target }: RetryPhaseProps) {
  return (
    <Box flexDirection="column">
      <Header more={more} t={t} target={target} />
      <Text color={t.color.muted}>{FAILURE_LINE}</Text>
      <DetailLine t={t} text={target.detail} />
...
function FormPhase(p: FormPhaseProps) {
  ...
  return (
    <Box flexDirection="column">
      ...
      {reopened ? <Text color={t.color.muted}>{FAILURE_LINE}</Text> : null}
...
export function ConnectionSetupOverlay({ cols, t }: ConnectionSetupOverlayProps) {
  ...
  const respond = (result: ConnectionRespondParams['result'], answeredAt: AnsweredRow | null) => {
    ...
    send(
      ...
      'That answer did not reach Cryozen. Try again.'
    )
  }
  ...
  const tryAgain = () => {
    ...
    send(
      ...
      'Cryozen could not start that again. Try again.'
    )
  }
