import { Box, Text, useStdout } from '@cryozen/ink'
import { mix } from '@cryozen/shared/color'
...
const TAG_FULL = 'Cryozen · Your personal AI agent'
const TAG_MID = 'Your personal AI agent'
const TAG_TINY = 'Cryozen'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  // cryozen_cli/banner.py) and the "connected" label on the collapse toggle.
  ...
  const heroColumn = wide ? (
    <Box flexDirection="column" width="100%">
      ...
      <Text color={t.color.accent}>
        ...
        <Text color={t.color.muted}> · Cryozen</Text>
      </Text>
    ...
    <Box flexDirection="column" width="100%">
      ...
      {typeof info.update_behind === 'number' && info.update_behind > 0 && (
        <Text bold color={t.color.warn}>
          ...
          <Text bold color={t.color.warn}>
            {info.update_command || 'cryozen update'}
          </Text>
          <Text bold={false} color={t.color.warn} dimColor>
            {' '}
            to update
