import { contrastRatio, ensureContrast, mix, parseColor, relativeLuminance, toHex } from '@cryozen/shared/color'
import type { SkinBranding, SkinColors } from '@cryozen/shared/skin'
...
// ── Color math ───────────────────────────────────────────────────────
//
// Generic color computation lives in @cryozen/shared/color (the primitives,
...
// Mirrors @cryozen/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @cryozen/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#7DD3FC',
  // The classic Cryozen navy surfaces are IDENTITY, not derivation drift —
...
// Light-terminal seeds: the default skin's `light_colors` overlay
// (cryozen_cli/skin_engine.py). On white the pale ice tones wash out, so the
 ...
 *  answer when known (cached in CRYOZEN_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.CRYOZEN_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit CRYOZEN_TUI_THEME / COLORFGBG signals above still win,
...
// `CRYOZEN_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `CRYOZEN_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `CRYOZEN_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `CRYOZEN_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Cryozen palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.CRYOZEN_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.CRYOZEN_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.CRYOZEN_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into CRYOZEN_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into CRYOZEN_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Cryozen navy
  ...
  // the derived value already carries the identity seeds (e.g. Cryozen navy).
  const activeRow =
