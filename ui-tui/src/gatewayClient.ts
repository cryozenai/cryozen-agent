...
import type { GatewayEvent } from '@cryozen/shared/gateway-events'
import {
  ...
  type ServerRequest,
  wireFrameText
} from '@cryozen/shared/json-rpc-channel'
import { reconnectBackoffDelayMs } from '@cryozen/shared/reconnect-backoff'
...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.CRYOZEN_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.CRYOZEN_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
// @cryozen/shared's JsonRpcRequestChannel; these re-exports keep the TUI's
...
const resolveGatewayAttachUrl = () => {
  const raw = process.env.CRYOZEN_TUI_GATEWAY_URL?.trim()

  return raw ? raw : null
}

const resolveSidecarUrl = () => {
  const raw = process.env.CRYOZEN_TUI_SIDECAR_URL?.trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = process.env.CRYOZEN_PYTHON?.trim() || process.env.PYTHON?.trim()
...
const redactUrl = (raw: string): string => {
  ...
  try {
    ...
    const withoutUserInfo = raw.replace(_USERINFO_FALLBACK_RE, '$1***@')
    const queryIdx = withoutUserInfo.indexOf('?')

    return queryIdx >= 0 ? `${withoutUserInfo.slice(0, queryIdx)}?***` : withoutUserInfo
  }
}
...
export class GatewayClient extends EventEmitter {
  ...
  private startSpawnedGateway(root: string) {
    ...
    const cwd = process.env.CRYOZEN_CWD || root
    const env = { ...process.env }
    const pyPath = env.PYTHONPATH?.trim()

    env.PYTHONPATH = pyPath ? `${root}${delimiter}${pyPath}` : root
    // Tell the gateway child where the Cryozen source root is so its import
    // guard can force it ahead of any same-named package in the launch cwd.
    env.CRYOZEN_PYTHON_SRC_ROOT = root
  ...
  start() {
    ...
    const root = process.env.CRYOZEN_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
