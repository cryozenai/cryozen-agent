...
import { terminalBackgroundHex } from '@cryozen/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $CRYOZEN_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see CRYOZEN_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['CRYOZEN_TUI_BACKGROUND', process.env.CRYOZEN_TUI_BACKGROUND ?? '(unset)'],
            ['CRYOZEN_TUI_THEME', process.env.CRYOZEN_TUI_THEME ?? '(unset)'],
