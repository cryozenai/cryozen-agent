...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@cryozen/ink'
import { stripAnsi } from '@cryozen/shared/ansi'
import { relativeLuminance } from '@cryozen/shared/color'
import type { StreamDeltaPayload, SubagentStatus, Usage } from '@cryozen/shared/gateway-events'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.CRYOZEN_TUI_THEME : undefined

  writeBootTheme(theme, process.env.CRYOZEN_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to CRYOZEN_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit CRYOZEN_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the CRYOZEN_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.CRYOZEN_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.CRYOZEN_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a CRYOZEN_TUI_THEME the user
    ...
    delete process.env.CRYOZEN_TUI_THEME
  }
 ...
 * is cached into CRYOZEN_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * CRYOZEN_TUI_LIGHT / CRYOZEN_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.CRYOZEN_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.CRYOZEN_TUI_THEME || process.env.CRYOZEN_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.CRYOZEN_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.CRYOZEN_TUI_BACKGROUND ||
      process.env.CRYOZEN_TUI_THEME ||
      process.env.CRYOZEN_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.CRYOZEN_TUI_BACKGROUND || process.env.CRYOZEN_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.CRYOZEN_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: AnyGatewayEvent) => void {
  ...
  const scheduleStartupPrompt = () => {
    ...
    setTimeout(async () => {
      ...
      // launches, `cryozen --tui -q "…"`) — submit LITERALLY, bypassing the
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey Cryozen" if this surface owns it (server gates on config).
    ...
    // forging a brand-new one.  Mirrors classic CLI's `cryozen -c` /
    // `cryozen --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: AnyGatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'notification.show': {
        // Status notice from the gateway. Payload is snake_case on the
      ...
      case 'wake.detected': {
        // "Hey Cryozen": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: cryozen -p ${wakeProfile} --tui`)
