import type { RunExternalProcess } from '@cryozen/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`cryozen ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching cryozen: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`cryozen ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
