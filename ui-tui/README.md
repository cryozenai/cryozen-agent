...
React + Ink terminal UI for Cryozen. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CRYOZEN_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `cryozen setup`, suspends Ink while it runs, opens a new session on success
...
- `registry.ts` — assembles `SLASH_COMMANDS` from all command files in registration order (core → session → ops → wake → setup → debug) and exposes `findSlashCommand(name)` for case-insensitive lookup
...
- Input history is stored in `~/.cryozen-agent/.cryozen_history` or under `CRYOZEN_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `cryozen setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/cryozen-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external cryozen setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.cryozen-agent/tui-parent.log
    ...
    types/
      cryozen-ink.d.ts            type declarations for @cryozen/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent CryozenCLI subprocess for slash commands
```
