# Cryozen Plugin Catalog

Curated, maintainer-approved Cryozen plugins. Each YAML file in this directory
(except `removed.yaml`) is one catalog entry, discoverable via
`cryozen plugins catalog` / `cryozen plugins search` and installable with
...
1. **Human-merged gate.** Entries are added *only* via a PR to the
   `cryozen-agent` repository, reviewed and merged by a maintainer. There is
...
3. **No self-updating code.** A listed plugin must not fetch and replace
   ...
   `cryozen plugins update <name>`. Keep the updater in the standalone
...
5. **Owner-or-major-contributor submissions, or a maintainer-curated sweep.**
   ...
   Cryozen maintainers may also add entries in batches from a reviewed sweep
...
7. **The install scanner runs at admission.** `cryozen plugins validate` includes
...
8. **Desktop plugins stay inside the SDK surface.** A `desktop/plugin.js` runs
   ...
   no `eval`/`new Function`, no `import()` of anything but `@cryozen/plugin-sdk`
   / `react` (app bundle chunks, blob or http URLs included), no script-tag
   injection, no reaching into the app's internal stores. `cryozen plugins
...
requires_cryozen: ">=0.19"   # optional
docs_url: ""                # optional
...
Every entry gets a page at `https://cryozenai.github.io/cryozen-agent/docs/plugins/<name>`
