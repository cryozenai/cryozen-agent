...
from cryozen_cli.config_providers import _canonical_api_mode
...
from cryozen_constants import OPENROUTER_BASE_URL, cryozen_home_key
from utils import base_url_host_matches, base_url_hostname, base_url_origin, is_truthy_value, model_forces_max_completion_tokens, normalize_proxy_env_vars
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_verify``, ``CRYOZEN_CA_BUNDLE`` / ``SSL_CERT_FILE``); any failure → httpx default (``True``)."""
    try:
        from agent.ssl_verify import resolve_httpx_verify
        from cryozen_cli.config import get_custom_provider_tls_settings, load_config_readonly
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `cryozen update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Cryozen owns aux retry/fallback policy; the SDK default (max_retries=2) would triple
    # wall time on a hung endpoint before Cryozen sees one failure.
    # Cryozen owns auxiliary retry + provider/model fallback policy (the same-provider transient retry in
    ...
    # so a 120s timeout can stall ~360s before Cryozen sees a single failure (issue #54465). Disable
    # SDK-internal retries by default and let Cryozen control the budget; explicit callers can still override
    # via kwargs.
...
def _run_protected_sync_provider_call(callback: Callable[[dict[str, Any]], Any], kwargs: dict[str, Any]) -> Any:
    ...
    threading.Thread(
        target=provider_context.run, args=(_provider_worker,), name="cryozen-protected-aux-provider",
        daemon=True).start()
    while True:
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    not isinstance, so out-of-tree clients can opt out of wrappers unimported (cf. SUPPORTS_CRYOZEN_TOOL_CALLS)."""
    try:
...
# provider — mirrors cryozen_cli.auth._PROVIDER_ALIASES. Without this group an explicit
...
def _provider_alias_table() -> Dict[str, str]:
    """The same alias table the main provider path resolves against (cryozen_cli.auth).
    ...
    if _ALIAS_TABLE is None:
        ...
        with contextlib.suppress(Exception):
            from cryozen_cli.auth import _PROVIDER_ALIASES as _auth_table
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from cryozen_cli.auth import resolve_api_key_provider_credentials
        from cryozen_cli.models_pricing import fetch_models_with_pricing
        ...
        if base_url.endswith("/v1"):  # fetch_models_with_pricing appends /v1/models
            base_url = base_url[:-3]
        catalog = fetch_models_with_pricing(api_key=api_key or None, base_url=base_url, timeout=3.0) or {}
    except Exception:
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://cryozenai.github.io/cryozen-agent",
    "X-Title": "Cryozen Agent",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    try:
        from cryozen_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Precedence env > config > default: ``CRYOZEN_OPENROUTER_CACHE`` overrides
    ``openrouter.response_cache``; ``CRYOZEN_OPENROUTER_CACHE_TTL`` (1-86400 s) overrides
    ...
    if or_config is None:
        try:
            ...
            or_config = {}
    env_cache = os.environ.get("CRYOZEN_OPENROUTER_CACHE", "").strip().lower()
    if not (env_cache in {"1", "true", "yes", "on"} if env_cache else or_config.get("response_cache", False)):
        return headers
    headers["X-OpenRouter-Cache"] = "true"
    env_ttl = os.environ.get("CRYOZEN_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
...
_NVIDIA_NIM_CLOUD_HEADERS = {"X-BILLING-INVOKE-ORIGIN": "CryozenAgent"}
...
_AUTH_JSON_PATH = get_cryozen_home() / "auth.json"
...
def _auth_json_path():
    ...
    from cryozen_cli.auth import _auth_file_path
...
def _to_openai_base_url(base_url: str) -> str:
    ...
    if base_url_hostname(url) == "api.actual.inc":
        from cryozen_cli.auth import normalize_actual_base_url
...
def _pool_runtime_api_key(entry: Any) -> str:
    # runtime_api_key handles provider-specific fallback; None entry → "".
...
class _CodexCompletionsAdapter:
    ...
    def _build_responses_kwargs(self, kwargs: Dict[str, Any]) -> Tuple[Dict[str, Any], str, Any]:
        ...
        wire_tools, wire_aliases = _alias_wire_tools(
            ...
            {"provider": getattr(self._client, "_cryozen_aux_effective_provider", "") or None, "base_url": host},
            is_xai,
        )
        ...
        resp_kwargs: Dict[str, Any] = {
            # Codex only knows the base slug; strip the Cryozen ``-900k`` picker suffix.
    ...
    def create(self, **kwargs) -> Any:
        from cryozen_cli.providers import is_actual_route

        if is_actual_route(
            getattr(self._client, "_cryozen_aux_effective_provider", ""),
        ...
        try:
            ...
            # Undo only the aliases THIS request emitted, before the call reaches Cryozen dispatch.
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(self, real_client: Any, model: str, base_url: str | None = None):
        self._client = real_client
        self._model = model
        # Caller URL only: falling back to the SDK client's host would flip MiniMax/Zhipu aux
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (already TRANSLATED to native fields — raw would 400 on strict gateways) and ``_`` Cryozen plumbing.
        ...
        _nr = get_transport("anthropic_messages").normalize_response(response)
        usage = None
...
class AnthropicAuxiliaryClient:
    ...
    def __init__(self, real_client: Any, model: str, api_key: str, base_url: str):
        self._real_client = real_client
        self.chat = _ChatShim(_AnthropicCompletionsAdapter(real_client, model, base_url=base_url))
        self.api_key = api_key
        self.base_url = base_url
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``cryozen_cli.runtime_provider._detect_api_mode_for_url`` so aux and main agree: any
...
def _maybe_wrap_anthropic(
    ...
    """
    # Anthropic/Bedrock/Codex wrappers, plus any client declaring CRYOZEN_SKIP_TRANSPORT_WRAP
    ...
    if (
        ...
        or _client_declares(client_obj, "CRYOZEN_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
    ...
    return AnthropicAuxiliaryClient(real_client, model, api_key, base_url)
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from cryozen_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(_scoped_key_env("CRYOZEN_XAI_BASE_URL"))
    ...
    try:
        from cryozen_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    if pool_present:
        ...
        if token:
            return token
    try:
        from cryozen_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from cryozen_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            # Explicit-config gate: an ambient ANTHROPIC_API_KEY must not silently become aux fallback.
            with contextlib.suppress(ImportError):
                from cryozen_cli.auth import is_provider_explicitly_configured
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            from cryozen_cli.models import copilot_default_headers
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        from cryozen_cli.copilot_auth import copilot_request_headers
        ...
        from tools.xai_http import cryozen_xai_default_headers
        headers = dict(cryozen_xai_default_headers())
    else:
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    try:
        from cryozen_cli.config import cfg_get, load_config_readonly
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from cryozen_cli import config as _cfg_mod
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        from cryozen_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `cryozen setup` or `cryozen model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key, base_url=base_url, default_headers=cryozen_xai_default_headers()
    )
    return CodexAuxiliaryClient(real_client, model), model


def _codex_base_url_override() -> str:
    """Profile-scoped ``CRYOZEN_CODEX_BASE_URL`` (same read as the API-key env vars: under a
    multiplexer the routed profile's .env decides the endpoint, never a sibling's process env)."""
    return _scoped_key_env("CRYOZEN_CODEX_BASE_URL").rstrip("/")
...
def _try_azure_foundry(
    *, model: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    try:
        from cryozen_cli.runtime_provider import _resolve_azure_foundry_runtime
        from cryozen_cli.auth import AuthError
        from cryozen_cli.config import load_config_readonly
    except ImportError:
        return None, None
...
def _is_model_incompatible_error(exc: Exception) -> bool:
    ...
    if _contains_any(err_lower, (
        ...
        "not available on the free tier", "quota",
    )):
        return False
    return _contains_any(err_lower, (
...
def _evict_cached_clients(provider: str) -> None:
    ...
    Scoped to the calling profile (``cryozen_home_key()`` is the first key slot): a rotation in
...
_POOL_PROVIDER_BY_HOST = (
    ...
    ("api.anthropic.com", "anthropic"), ("githubcopilot.com", "copilot"), ("api.kimi.com", "kimi-coding"), ("api.x.ai", "xai-oauth"),
)
...
def _refresh_copilot_credentials() -> bool:
    from cryozen_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_codex_credentials() -> bool:
    from cryozen_cli.auth import resolve_codex_runtime_credentials
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_cryozen_fallback_destination", None)
...
def _call_fallback_candidate_sync(
    ...
    try:
        ...
        fb_provider, retry = _plan_fallback_auth_retry(
            destination, rebuild, async_mode=False)
...
async def _call_fallback_candidate_async(
    ...
    try:
        ...
        if not _is_auth_error(fb_err):
            ...
            return None
        fb_provider, retry = _plan_fallback_auth_retry(
            destination, rebuild, async_mode=True)
        failed_destination = destination
        if retry is not None:
            failed_destination = retry[2]
            try:
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    from cryozen_cli.fallback_config import resolve_entry_api_key
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        with contextlib.suppress(Exception):
            client._cryozen_fallback_destination = _fallback_destination_from_entry(entry, client, resolved_model)
...
def _try_main_fallback_chain(
    ...
    try:
        from cryozen_cli.config import load_config_readonly
        from cryozen_cli.fallback_config import get_fallback_chain
...
def _warn_stale_openai_base_url(runtime_provider: str) -> None:
    ...
    ~/.cryozen-agent/.env value after `cryozen model` poisons routing)."""
    ...
    if (_env_base and _cfg_provider and _cfg_provider != "custom" and not _cfg_provider.startswith("custom:")):
        logger.warning(
            ...
            "Run: cryozen model to reconfigure, or remove "
            "OPENAI_BASE_URL from ~/.cryozen-agent/.env",
...
def _try_main_provider_route(
    ...
    if runtime_base_url and main_provider == "custom":
        ...
        with contextlib.suppress(ImportError):
            from cryozen_cli.runtime_provider import _get_named_custom_provider
...
def _discovery_chain_allowed(main_provider: str, task: Optional[str] = None) -> bool:
    ...
    compression silently charged to another account's balance)."""
    ...
    logger.warning(
        ...
        "configured — refusing to guess another logged-in provider. Re-authenticate (`cryozen model`) "
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_cryozen_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "CRYOZEN_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    # Cryozen owns the auxiliary retry/timeout budget; disable SDK-internal retries.
    # See #54465.
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from cryozen_cli.model_normalize import normalize_model_for_provider
...
def _is_actual_auxiliary_route(req: _ResolveRequest, base_url: str) -> bool:
    from cryozen_cli.auth import normalize_actual_base_url
    from cryozen_cli.providers import is_actual_route
    from cryozen_cli.route_identity import normalize_route_base_url
...
def _wrap_transport(req: _ResolveRequest, client_obj: Any, final_model_str: str,
    ...
    if _is_actual_auxiliary_route(req, base_url_str):
        ...
        client._cryozen_aux_effective_provider = "actual"
        return client
...
def _resolve_auto_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if routed_client is not None and effective_provider:
        try:
            setattr(routed_client, "_cryozen_aux_effective_provider", effective_provider)
...
def _resolve_openai_codex_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if not model:
        ...
        return None, None
    no_token_msg = "resolve_provider_client: openai-codex requested but no Codex OAuth token found (run: cryozen model)"
    if req.raw_codex:
...
def _resolve_xai_oauth_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default,
                          ...
                          "OAuth token found (run: cryozen model -> xAI Grok OAuth — SuperGrok / Premium+)")
...
def _resolve_named_custom_branch(req: _ResolveRequest) -> Optional[_ResolveResult]:
    ...
    # canonical name (e.g. ``openrouter``) still defer to the built-in.
    custom_entry = None
    ...
    if not custom_base:
        ...
        return None, None
    from cryozen_cli.config import normalize_extra_headers
    ...
    if entry_api_mode == "anthropic_messages":
        ...
        return _route_client(
            req, AnthropicAuxiliaryClient(real_client, final_model, custom_key, custom_base), final_model)
...
def _resolve_azure_foundry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default_model,
                          ...
                          "runtime resolution failed (run: cryozen doctor for diagnostics)")
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    if provider == "copilot" and final_model and not req.raw_codex:
        with contextlib.suppress(ImportError):
            from cryozen_cli.models import _should_use_copilot_responses_api
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    try:
        ...
        logger.debug("cryozen_cli.auth not available for provider %s", provider)
        return None, None
...
def resolve_provider_client(
    ...
    # Excluded: ``auto`` (a stale main slug could pair with any picked provider).
...
# Strict (explicitly requested) vision backends by normalized provider name. openai-codex has no
# safe default model; callers set auxiliary.<task>.model.
...
def _client_cache_key(
    ...
    # Profile home leads the key: callers that omit api_key (pool / auth.json paths) would
    # otherwise share one client across multiplex profiles holding different credentials.
    return (cryozen_home_key(), provider, async_mode, base_url or "", api_key_key, api_mode or "", runtime_key, is_vision, task_key, pool_hint, model_key)
...
def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
    ...
    try:
        ...
        return normalized in {
            "anthropic", "copilot", "copilot-acp", "minimax-oauth", "openai-codex", "qwen-oauth", "xai-oauth",
        }
...
def _resolve_task_provider_model(
    ...
    from cryozen_cli.runtime_provider_custom import expand_direct_api_alias
    if provider:
...
def _fast_lane_config_fields(config: Dict[str, Any]) -> tuple[str, str, bool]:
    ...
    from cryozen_constants import parse_reasoning_effort
...
def _acquire_sync_aux_semaphore(task: Optional[str]) -> Optional[threading.BoundedSemaphore]:
    ...
    return _cached_semaphore(_aux_sync_semaphores, (cryozen_home_key(), task), limit, threading.BoundedSemaphore)
...
def _acquire_async_aux_semaphore(task: Optional[str]):
    ...
    try:
        ...
        return None
    from cryozen_constants import cryozen_home_key
    return _cached_semaphore(_aux_async_semaphores, (cryozen_home_key(), task, id(loop)), limit, asyncio.Semaphore)
...
def _merge_aux_extra_body(
    ...
    # _get_task_extra_body) takes the same wire clamp: Cryozen-only ``ultra`` never reaches the
...
def _build_call_kwargs(
    ...
    # ``extra_body.reasoning`` fallback. Clamp Cryozen-internal levels (``ultra``) to the
...
def _managed_local_netloc() -> str:
    ...
    if now - ts < _MANAGED_LOCAL_STATE_TTL_S:
        return cached
    try:
        from cryozen_cli.local_runtime.supervisor import state_path
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Cryozen manages."""
    if not base_url:
        return False
...
def _resolve_call_client(
    ...
    if client is None:
        raise AuxiliaryClientUnavailable(f"No LLM provider configured for task={task} "
                                         f"provider={resolved_provider}. Run: cryozen setup")
...
def _parameter_rungs(client: Any, max_tokens: Optional[int]) -> tuple:
    ...
    return (
        ...
        # The endpoint refuses the *disable* rather than the field (e.g. gpt-6-astra behind a relay: "Reasoning is
...
def _aux_recovery_ladder(
    ...
    strips → credential refresh/pool rotation → provider fallback.
