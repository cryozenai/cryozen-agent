"""Lightweight i18n for Cryozen' static user-facing strings (approval prompts, a few gateway replies).
...
Language resolution: explicit ``lang=`` > ``CRYOZEN_LANGUAGE`` > ``display.language`` > ``en``.
"""
...
# Natural aliases so "japanese" / "ja-JP" / "jp" hit the right catalog instead of
# silently falling back to English. pt-br shares the pt catalog (no separate br one).
# Chinese is not shipped: "zh", "zh-CN", "zh-TW" and the like resolve to English.
...
def _locales_dir() -> Path:
    """Locale dir: ``CRYOZEN_BUNDLED_LOCALES`` (sealed packaging, e.g. Nix) if it exists, else ``<repo-root>/locales``.

    The source path is returned even when missing so ``_load_catalog`` can log
    the path it looked at rather than raise.
    """
    override = os.getenv("CRYOZEN_BUNDLED_LOCALES", "").strip()
    if override and Path(override).is_dir():
        return Path(override)
    if override:
        logger.warning(
            "CRYOZEN_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _normalize_lang(value: Any) -> str:
    """Map a user-supplied value (code, alias, or regional tag like ``pt-BR``) to a supported code, else default."""
...
@lru_cache(maxsize=8)
def _config_language_cached(cryozen_home: str) -> str | None:
...
def _config_language() -> str | None:
    ...
    return _config_language_cached(str(get_cryozen_home()))
...
def get_language() -> str:
    """Resolve the active language using env > config > default order. ``CRYOZEN_LANGUAGE`` is a
    ...
    try:
        env_lang = get_secret("CRYOZEN_LANGUAGE")
    except UnscopedSecretError:
        env_lang = os.environ.get("CRYOZEN_LANGUAGE")  # unscoped default-profile path: environ IS its own value
