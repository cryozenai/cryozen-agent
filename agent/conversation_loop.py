...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in cryozen_state.py; kept local so importing
# cryozen_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Cryozen tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Cryozen "
        ...
        "again. A known-good starting point is 65,536 tokens. In Cryozen config, set "
...
def _billing_or_entitlement_message(*, capability: str, provider: str, base_url: str, model: str) -> str:
...
def _print_billing_or_entitlement_guidance(
    agent, *, capability: str, provider: str, base_url: str, model: str
) -> bool:
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if not getattr(agent, "_persist_disabled", False):
        try:
            from cryozen_cli.lifecycle import invoke_hook as _invoke_hook
            _invoke_hook(
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from cryozen_cli.moa_config import decode_moa_turn
...
def _run_conversation_turn(
    ...
    # Adopt ~/.cryozen-agent/.env credential/base-url edits made since the last turn — a
...
def _close_durable_failed_turn(agent, result: Any) -> None:
    """Append a Cryozen-authored assistant boundary when a failed turn left ``user`` as the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('cryozen_constants', 'PARTIAL_STREAM_STUB_ID'),
