...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("cryozen-verify-", "cryozen-ad-hoc-")
...
def _db_path() -> Path:
    return get_cryozen_home() / "verification_evidence.db"
...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = "cryozen verify",
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``cryozen verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command="cryozen verify", kind="verify",
