...
_BILLING_ERROR_CODES = frozenset({
    ...
    "no_usable_credits", "balance_depleted",
...
# Parameters Cryozen sends on SOME routes only → hosts where that is deliberate.
...
def is_reasoning_required_rejection(error_msg: str) -> bool:
    ...
    this endpoint and cannot be disabled", e.g. gpt-6-astra behind a relay). The opposite of
...
# 404: some aggregators surface credit depletion as a paid model vanishing
# (billing, not missing model); policy block before model_not_found.
_404_RULES = (
...
def _plugin_verdict(c: _Ctx) -> Optional[Verdict]:
    ...
    try:
        from cryozen_cli.plugins import get_plugin_error_classification
...
def classify_api_error(
    ...
    ``base_url`` (optional) is the route the call went to; an auth refusal from a non-stock
    route names that host."""
...
def _off_route_host(c: _Ctx) -> str:
    ...
    from cryozen_cli.route_identity import provider_owns_route
