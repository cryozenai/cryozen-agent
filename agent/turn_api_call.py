"""The provider call for the conversation turn's retry loop: ``perform_api_call`` (streaming
decision, MoA prepared-request handshake, LLM execution middleware wrapper, the redirect
``_model_request_active`` bracket and the response-vs-redirect crossing check) and
...
def perform_api_call(
    ...
    from cryozen_cli.middleware import run_llm_execution_middleware
