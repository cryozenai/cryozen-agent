...
def _detect_kanban() -> bool:
    ...
    if os.getenv("CRYOZEN_KANBAN_TASK") or os.getenv("CRYOZEN_KANBAN_BOARD"):
        try:
...
def skill_matches_apps(frontmatter: Dict[str, Any]) -> bool:
    ...
    Names resolve through ``cryozen_platform.declaration`` (registered by whoever owns the server,
...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no cryozen_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at CRYOZEN_HOME; absolute paths pass through."""
    from cryozen_constants import get_cryozen_home
    return p if p.is_absolute() else get_cryozen_home() / p


# Never disableable: `cryozen-agent` is the agent's own operating manual and the
# system prompt points at it unconditionally.
ESSENTIAL_SKILLS: frozenset = frozenset({"cryozen-agent"})


def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    """Disabled skill names from config.yaml: global list ∪ platform list
    (*platform* defaults to ``CRYOZEN_PLATFORM`` / ``CRYOZEN_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("CRYOZEN_PLATFORM") or get_session_env("CRYOZEN_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``cryozen config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to CRYOZEN_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to CRYOZEN_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_cryozen_home()}/skills/"
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.cryozen-agent/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.cryozen-agent/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".cryozen-agent", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (CRYOZEN_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under CRYOZEN_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`cryozen skills trust`)
...
# high-confidence findings only. The scan cache lives under CRYOZEN_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from cryozen_constants import get_cryozen_home
        cache_dir = get_cryozen_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped CRYOZEN_HOME), so normalization
    ...
    # ~/.cryozen-agent/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _cryozen_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.cryozen`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    cryozen = metadata.get("cryozen") if isinstance(metadata, dict) else None
    return cryozen if isinstance(cryozen, dict) else {}
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    cryozen = _cryozen_metadata(frontmatter)
    return {key: cryozen.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.cryozen.config`` declarations (key/description/default/prompt).
    Entries missing ``key`` or ``description`` are skipped; ``prompt`` defaults to the description."""
    raw = _cryozen_metadata(frontmatter).get("config")
...
def _expand_skill_config_path(value: str) -> str:
    """Expand ``~`` / ``$HOME`` against the HOME Cryozen injects into tool subprocesses.

    Skill config defaults describe paths the agent hands to tools, so in a container where the
    control process HOME (``/opt/data``) differs from the tool HOME (``{CRYOZEN_HOME}/home``) a
