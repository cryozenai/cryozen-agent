...
# The Cryozen-authored assistant row that closes a durable turn which ended without one. A
...
def provider_label_for(provider: Any) -> str:
    """Human-friendly provider name for chat copy (``"OpenRouter"``, ``"Anthropic"``…)."""
...
_NEXT_STEPS_LOOP = (
    ...
    "If it happens again, run `cryozen doctor` and share the error details."
)
...
_NONRETRYABLE_COPY: Dict[str, str] = {
    FailoverReason.model_not_found.value: (
        ...
        "(or `cryozen model` in a terminal).{prefix_hint}"
    ),
    FailoverReason.format_error.value: (
        "{label} rejected this request as malformed, so the model didn't answer. Start a clean "
        "session with /new or switch models with /model; if it keeps happening, run `cryozen doctor`."
    ),
    ...
    FailoverReason.ssl_cert_verification.value: (
        "Cryozen couldn't verify {label}'s security certificate, so the connection was refused. "
...
_AUTH_COPY: Dict[str, str] = {
    ...
    "api_key": (
        ...
        "Settings → Providers, or run `cryozen setup` in a terminal."
    ),
}
...
_FAILURE_CODE_COPY: Dict[str, str] = {
    "context_overflow": (
        "This conversation has grown too long for {model} to read, and Cryozen couldn't shrink "
    ...
    "truncated": (
        ...
        "Cryozen didn't run the incomplete action. Nothing was changed. Send `continue`, ask for "
    ...
    "loop_error": (
        "Cryozen hit repeated errors and stopped this turn so it wouldn't keep retrying. "
        + _NEXT_STEPS_LOOP + "\n\nDetails: {detail}"
    ),
    "interpreter_shutdown": (
        "Cryozen was shutting down and stopped this turn. Your conversation is saved — reopen "
...
_ONE_OFF_COPY: Dict[str, str] = {
    "payload_too_large": (
        ...
        "Cryozen couldn't shrink it enough automatically. Start a new session with /new (your "
    ...
    "server_context_rejection": (
        ...
        "about {tokens:,} tokens — well under the {window:,}-token window Cryozen knows for "
        ...
        "capacity, or the server runs {model} with a smaller window than Cryozen assumes. Wait a "
    ...
    "local_processing_error": (
        "Cryozen hit an internal error while handling the model's reply and stopped this turn. "
        + _NEXT_STEPS_LOOP + "\n\nDetails: {detail}"
    ),
    "reasoning_only": (
...
def site_copy(code: str, **fields: Any) -> str:
    ...
    fields.setdefault("home", display_cryozen_home())
...
def exhausted_copy(reason: str, *, label: str, attempts: int, summary: str, reset_seconds: Optional[float] = None) -> str:
    ...
    return (
        ...
        f"add a backup provider with `cryozen fallback add`.\n\nProvider said: {summary}"
    )
...
def oauth_relogin_command(provider: Any) -> str:
    ...
    named profile: a profile's credentials are its own (93889b770da), so a bare ``cryozen auth`` from
    ...
    return f"cryozen {profile_cli_selector()}auth add {slug} --type oauth"
...
def relogin_command_hint(provider: Any) -> str:
    ...
    the exact OAuth command for a known OAuth slug, ``cryozen auth add <slug>`` for a known API-key
    ...
    slug = str(provider or "").strip().lower()
    if not slug:
        return f"cryozen {profile_cli_selector()}auth add <provider>"
    from agent.error_surface import auth_kind

    if auth_kind(slug) == "oauth":
        return oauth_relogin_command(slug)
    return f"cryozen {profile_cli_selector()}auth add {slug}"
...
def nonretryable_copy(
    ...
    body = template.format(label=label, model=model, home=display_cryozen_home(), prefix_hint=prefix_hint,
