...
Installs go to a Cryozen-owned staging dir, ``<CRYOZEN_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``cryozen lsp status`` reports it).
...
# ``bin`` in ``<CRYOZEN_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `cryozen lsp status` reports its presence.
...
def cryozen_lsp_bin_dir() -> Path:
    """Return the Cryozen-owned bin staging dir for LSP servers."""
    from cryozen_constants import get_cryozen_home

    p = get_cryozen_home() / "lsp" / "bin"
...
def _npm_bin_dir() -> Path:
    ...
    return cryozen_lsp_bin_dir().parent / "node_modules" / ".bin"
...
def _existing_binary(name: str, *, is_windows: Optional[bool] = None) -> Optional[str]:
    ...
    bases = [cryozen_lsp_bin_dir() / name] + ([_npm_bin_dir() / name] if win else [])
...
def _link_into_bin(target: Path) -> str:
    ...
    link = cryozen_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    if pm is None:
        return None
    # Managed Node first: $CRYOZEN_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Cryozen installed.
    ...
    if pm_bin is None:
        ...
        return None
    staging = cryozen_lsp_bin_dir().parent  # <CRYOZEN_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = cryozen_lsp_bin_dir().parent / "python-packages"
    pip_target.mkdir(parents=True, exist_ok=True)
    try:
        logger.info("[install] pip install --target %s %s", pip_target, pkg)
        from cryozen_cli.tools_config import _pip_install
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``cryozen lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "cryozen_lsp_bin_dir"]
