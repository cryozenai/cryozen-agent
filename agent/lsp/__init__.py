"""Language Server Protocol (LSP) integration for Cryozen Agent.
...
# Routed multiplex profiles (CRYOZEN_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``cryozen chat`` exit leaks pyright processes for a few seconds
