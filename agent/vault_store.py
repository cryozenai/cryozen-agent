...
# browser fill targets (agent/vault_login_classifier.py); the Desktop Add dialog and `cryozen vault add`
...
class VaultStore:
    """Encrypted, profile-scoped vault under ``<CRYOZEN_HOME>/vault/``."""

    def __init__(self, base_dir: Optional[Path] = None):
        self._base = Path(base_dir) if base_dir is not None else (
            Path(get_cryozen_home()) / "vault"
        )
    ...
    def _ensure_dir(self) -> None:
        ...
        # group-share mode and CRYOZEN_UID/GID ownership) rather than a
    ...
    def _locked(self):
        ...
        `cryozen vault add` and a TUI slash worker all write the same ``vault.json.enc``; two unlocked
