...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    "turn_lease": (
        "the turn was stopped because another Cryozen process "
    ...
    "locked": (
        ...
        "(another Cryozen process was writing to the state "
    ...
    # logger.error at cryozen_state.py::_raise_if_db_replaced — never in the chat reply.
    "replaced": (
        "the session database file was replaced while Cryozen was running, so this "
        "message was not saved (a copy is kept in {home}/sessions/). Stop Cryozen "
        "(`cryozen {profile_arg}gateway stop`), run `cryozen {profile_arg}doctor` — not "
        "`cryozen {profile_arg}doctor --fix`, which would repair the wrong file in place — "
        "then start it again and send your message once more. Advanced recovery steps are "
        "in the log."
    ),
    "deleted_wal": (
        "another Cryozen process still holds an old copy of the session database's write-ahead "
        "log, so Cryozen stopped writing to keep the file safe and this message was not saved (a "
        "copy is kept in {home}/sessions/). Nothing is lost. Quit every Cryozen process on this "
        "profile (Desktop app, `cryozen {profile_arg}gateway stop`, dashboard, cron), run "
        "`cryozen {profile_arg}doctor` — it names any process still holding the log — then start "
        "Cryozen again and send your message once more. Do not run `doctor --fix` or delete "
    ...
    "corrupt": (
        ...
        "1. Run `cryozen {profile_arg}doctor --fix`\n"
        "2. Stop the gateway, then recover with:\n"
        "   cryozen {profile_arg}sessions recover --source {db_path} --inspect-only\n"
        "   (if it reports recoverable) cryozen {profile_arg}sessions recover "
    ...
    "fts_index": (
        ...
        "recovery tools or restore a backup. Run `cryozen {profile_arg}doctor --fix` "
        "(or restart Cryozen, which repairs the index on open), then "
        "send your message again."
    ),
    "disk": (
        "Cryozen couldn't save this conversation to disk, so it stopped rather than lose "
...
_PERSISTENCE_DEFAULT_EXPLANATION = (
    "Cryozen couldn't save this conversation, so it stopped rather than lose your messages. "
    "Possible causes: the drive is out of room, or another Cryozen process is holding the "
    "database. Close other Cryozen windows, run `cryozen {profile_arg}doctor` to check "
...
def _display_flag_enabled(agent, *, env_var: str, config_key: str, cache_attr: str) -> bool:
    ...
    and tests patch it at ``cryozen_cli.config``). Any failure → True (safe default: on)."""
    try:
        ...
        if cached is not None:
            return cached
        try:
            from cryozen_cli.config import load_config as _load_config
...
class TurnExplainersMixin:
    ...
    def _record_file_mutation_result(
        ...
        *, task_id: Optional[str] = None,
        ...
        if landed:
            ...
            # Cryozen-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        """``display.file_mutation_verifier`` / ``CRYOZEN_FILE_MUTATION_VERIFIER`` (a patchable seam)."""
        return _display_flag_enabled(
            self, env_var="CRYOZEN_FILE_MUTATION_VERIFIER", config_key="file_mutation_verifier",
            cache_attr="_file_mutation_verifier_enabled_cache",
        )

    def _turn_completion_explainer_enabled(self) -> bool:
        """``display.turn_completion_explainer`` / ``CRYOZEN_TURN_COMPLETION_EXPLAINER``."""
        return _display_flag_enabled(
            self, env_var="CRYOZEN_TURN_COMPLETION_EXPLAINER", config_key="turn_completion_explainer",
    ...
    def _format_turn_completion_explanation(
        ...
        if body is None and reason == "session_persistence_failed":
            from cryozen_constants import display_cryozen_home, profile_cli_selector
            from cryozen_state_errors import STORAGE_RECOVERY_DOCS_URL

            # Copy-pasteable, so pin every `cryozen` command to the profile whose store failed:
            # a multi-profile backend (Desktop serve) hosts sessions whose state.db is NOT the
            # process default, and a bare `cryozen` follows active_profile (#105887).
            body = (
                _PERSISTENCE_CAUSE_EXPLANATIONS.get(
                    persistence_cause or "unknown", _PERSISTENCE_DEFAULT_EXPLANATION
                )
                .replace("{home}", display_cryozen_home())
            ...
            if persistence_cause in ("corrupt", "fts_index"):
                ...
                body = body.replace(
                    "{backups_dir}", str(get_default_cryozen_root() / "backups")
                )
