...
class ReasoningParamsMixin:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        some routes 400 on ``reasoning``, so gate to known reasoning-capable families."""
        url = self._base_url_lower
        if base_url_host_matches(url, "ai-gateway.vercel.sh"):
            return True
        ...
        try:
            from cryozen_cli.models_reasoning_caps import openrouter_model_reasoning_capabilities, warm_openrouter_reasoning_caps_async
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        try:
            from cryozen_cli.models_local import lmstudio_model_reasoning_options
        except Exception:
            return []
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from cryozen_cli.models_local import ollama_model_supports_thinking
        except Exception:
            return False
