...
from cryozen_cli.config import DEFAULT_CONFIG, cfg_get
from cryozen_cli.route_identity import normalize_route_base_url
from cryozen_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'cryozen memory status'. Note: "
        "systemd/gateway services do not inherit ~/.cryozen-agent/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from cryozen_cli.providers import CRYOZEN_OVERLAYS, get_provider
        overlay = CRYOZEN_OVERLAYS.get(provider)
    ...
    with suppress(Exception):
        ...
        from cryozen_cli.models import normalize_provider as normalize_model_provider
        from cryozen_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: cryozen config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$CRYOZEN_HOME`` is profile-scoped; not a config key)."""
    return get_cryozen_home() / ".codex_gpt55_autoraise_notice"
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if is_actual_route(agent.provider, base_url):
        ...
        try:
            ...
            from cryozen_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    with suppress(Exception):
        from cryozen_cli.model_normalize import (
    ...
    from cryozen_cli.runtime_provider_backends import _is_external_process_provider
...
# Per-turn bookkeeping: budgets, activity tracking, rate-limit telemetry.
...
def _init_prompt_cache_config(agent):
    ...
    # (1h when a person paces the session, 5m when a machine does); unknown values keep "5m". A falsy/off value disables caching entirely (plans
    ...
    # amortizes across long sessions with >5-minute pauses between turns (#14971), or for third-party
    # proxies that inject their own cache_control markers (#13477).
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from cryozen_cli.config import load_config_readonly as _load_pc_cfg
...
def _setup_logging(agent):
    ...
    from cryozen_logging import setup_logging, setup_verbose_logging
    setup_logging(cryozen_home=_ra()._cryozen_home)
    ...
    # would starve the root file handlers. Noise reduction belongs in cryozen_logging.
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "bedrock":
        ...
        return
    # ANTHROPIC_API_KEY fallback only for native Anthropic — other anthropic_messages providers
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from cryozen_cli.auth import build_minimax_oauth_token_provider
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Optional[Dict[str, Any]]:
    ...
    if _routed_client is not None:
        from cryozen_cli.providers import is_actual_route, normalize_provider
        effective_provider = getattr(_routed_client, "_cryozen_aux_effective_provider", "")
    ...
    raise RuntimeError(
        "No LLM provider configured. Run `cryozen model` to "
        "select a provider, or run `cryozen setup` for first-time "
        "configuration."
    )
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("cryozen_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "cryozen_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched CRYOZEN_HOME since model_tools was imported;
...
def _publish_session_id(session_id: str) -> None:
    ...
    try:
        ...
        if not delegated_child:
            os.environ["CRYOZEN_SESSION_ID"] = session_id
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # ~/.cryozen-agent/sessions/ — kept unconditionally for request_dump_*.json debug breadcrumbs.
    agent.logs_dir = get_cryozen_home() / "sessions"
    ...
    # Process-scoped --yolo is persisted so `cryozen --resume` restores the bypass
...
def _memory_provider_init_kwargs(agent, platform) -> Dict[str, Any]:
    ...
    with suppress(Exception):
        ...
        kwargs["agent_workspace"] = "cryozen"
    return kwargs
        ...
        try:
            ...
            if not is_core_memory_provider(_mem_provider_name):
                ...
                if _mp is None:
                    ...
                    from cryozen_cli.memory_provider_migration import recover_at_startup
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    if app_server_auto not in {"native", "cryozen", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, cryozen, off.",
            app_server_auto,
        )
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from cryozen_cli.auth import resolve_provider as resolve_auth_provider
...
def _scope_context_length_to_default_runtime(
    ...
    if isinstance(_default, dict):
        from cryozen_cli.config import split_model_config_default
...
def config_context_length_for_runtime(agent, config=None) -> Optional[int]:
    ...
    """
    try:
        from cryozen_cli.config import get_compatible_custom_providers, load_config
...
def _select_context_engine(_agent_cfg):
    ...
    if _selected_engine is None:
        try:
            from cryozen_cli.plugins import get_plugin_context_engine
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, cryozen_home=str(get_cryozen_home()),
...
def init_agent(
    ...
    try:
        from cryozen_cli.config import load_config_readonly as _load_agent_config
