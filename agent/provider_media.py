"""``$CRYOZEN_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$CRYOZEN_HOME/cache/<kind>/``, creating parents as needed."""
    from cryozen_constants import get_cryozen_home
    path = get_cryozen_home() / "cache" / kind
