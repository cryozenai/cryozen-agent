"""Unified removal contract for every credential source Cryozen reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``cryozen auth remove <provider> <N>`` must make the entry stay gone across
...
``~/.codex/auth.json``): ``auth.adopt_external_logins`` in config.yaml.
"""
...
EXTERNAL_LOGINS_NOT_ADOPTED_NOTICE = (
    "External CLI logins (Codex CLI) are not adopted: auth.adopt_external_logins is false. "
    "Cryozen uses only its own logins; run `cryozen auth add <provider>` to add one."
)
...
def adopt_external_logins_enabled() -> bool:
    ...
    Codex OAuth refresh tokens are single-use and rotate, so once Cryozen borrows the CLI's
    token pair the two programs hold one token family and whichever refreshes first logs the other
    out. When the user opts out, Cryozen never reads or refreshes those files and says so once per
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.cryozen-agent/.env; hint when the shell exports it."""
    from cryozen_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like cryozen_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            f"(not in ~/.cryozen-agent/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Cryozen.",
            f"  The pool entry is now suppressed — Cryozen will ignore "
            f"{env_var} until you run `cryozen auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_auth_store_oauth(provider: str, removed) -> RemovalResult:
    """Clear auth.json ``providers.<provider>`` (minimax-oauth, xai-oauth, openai-codex).
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `cryozen model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `cryozen auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `cryozen auth add copilot` to re-enable if needed.",
    ])
...
def _suppress_only(*hints: str) -> Callable[..., RemovalResult]:
    ...
    (Qwen CLI, config.yaml): never delete it, just suppress and
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `cryozen auth add qwen-oauth` to re-enable if needed.",
        ),
