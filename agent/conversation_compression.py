...
_COMPRESSOR_ATTEMPT_GENERATION: contextvars.ContextVar[Any] = contextvars.ContextVar(
    "cryozen_compressor_attempt_generation", default=None
)
...
# Defaults for the in-agent progress-aware wrap; mirror cryozen_cli.config.DEFAULT_CONFIG["compression"] keys.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    Only the exact old ``cryozen_state.SessionDB`` class (hot-reload skew) may fail open; proxies, lookalikes,
...
def _rebind_session_context(session_id: str) -> None:
    ...
    with contextlib.suppress(Exception):
        from cryozen_logging import set_session_context
...
def _lower_threshold_to_aux_context(
    ...
    if threshold_suggestion_viable:
        ...
        msg += (
            ...
            f"Cryozen's small-context floor and output reservation would recompute the trigger to "
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    "⚠ No auxiliary LLM provider configured: Cryozen has no helper model for summarising "
                    "long chats, so older messages will be cut without a summary. Run `cryozen setup` to add one."
                )
            _emit_feasibility_notice(agent, msg)
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Cryozen "
        ...
        if _lock_lookup_error is not None:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "in-memory module after an update; restart the process (or `cryozen update`) to resync.",
                    _lock_sid,
                )
...
def _parent_deliberately_ended(session_db: Any, session_id: str) -> bool:
    ...
    if not callable(reader):
        return False
    try:
        from cryozen_state_common import is_automatic_end_reason
...
def _carry_session_state_to_child(agent: Any, old_session_id: str, old_title: Any) -> None:
    ...
    with _swallow('Could not migrate goal on compression: %s'):
        ...
        from cryozen_cli.goals import migrate_goal_to_session
        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
    with _swallow('Could not migrate heartbeat on compression: %s'):
        from cryozen_cli.heartbeat import migrate_heartbeat_to_session
...
def _publish_rotated_compaction(
    ...
    # publish also COALESCEs from the parent row for threads lacking CRYOZEN_HOME.
...
def _warn_summary_or_aux_fallback(agent: Any) -> None:
    ...
    if summary_error:
        ...
        if _aux_fail_model and getattr(agent, "_last_aux_fallback_warning_key", None) != _aux_key:
            ...
            agent._emit_warning(
                f"ℹ Configured compression model '{_aux_fail_model}' failed, so Cryozen summarised "
...
def compress_context(
    ...
    # compression.codex_app_server_auto). Memory handoff is Cryozen-only: no native
...
def _compress_context_via_codex_app_server(
    ...
    Cryozen' transcript is left unchanged."""
    ...
    if auto_mode not in {"native", "cryozen", "off"}:
        auto_mode = "native"
    skip_reason = None
    if not force and auto_mode != "cryozen":
...
def _shrink_data_url(url: str, *, max_dimension: int, resize_fn: Any) -> tuple:
    ...
    try:
        ...
        tmp = tempfile.NamedTemporaryFile(
            prefix="cryozen_shrink_", suffix=_IMAGE_SUFFIX_BY_MIME.get(mime, ".jpg"), delete=False
        )
        try:
            tmp.write(raw)
            tmp.close()
