...
_SUMMARY_ROUTE_PIN: contextvars.ContextVar[Optional[Dict[str, Any]]] = (
    contextvars.ContextVar("cryozen_summary_route_pin", default=None)
)
...
# (cryozen_state._rows_to_conversation), so any code that mutates a loaded or flushed dict's content in place
...
_COMPRESSION_MARKER_PREFIX = "⟪CRYOZEN-CONTEXT-COMPRESSION:"
_COMPRESSION_MARKER_TEMPLATE = (
    _COMPRESSION_MARKER_PREFIX
    + " {omitted:,} of {total:,} chars omitted here by Cryozen's context compressor. "
...
def _today_for_prompt() -> str:
    ...
    try:
        ...
        # via cryozen_time.now(). The compaction summary is a mid-conversation message that is NOT part of
        ...
        return _cryozen_now().strftime("%Y-%m-%d")
