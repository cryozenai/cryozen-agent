...
YAML files in ``<CRYOZEN_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``CRYOZEN_BUNDLES_DIR`` override (tests) or ``<CRYOZEN_HOME>/skill-bundles``."""
    override = os.environ.get("CRYOZEN_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_cryozen_home() / "skill-bundles"
...
# File-level CRUD — used by `cryozen bundles`.
