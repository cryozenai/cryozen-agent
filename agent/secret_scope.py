...
def serves_routed_profile() -> bool:
    ...
    multiplexing, else when a CRYOZEN_HOME override names another home (dashboard/desktop backend,
    ...
    if is_multiplex_active():
        return True
    from cryozen_constants import get_cryozen_home_override, get_process_cryozen_home, cryozen_home_key
    override = get_cryozen_home_override()
    return override is not None and cryozen_home_key(override) != cryozen_home_key(get_process_cryozen_home())
...
class UnscopedSecretError(RuntimeError):
    ...
    def __init__(self, secret_name: str = "", developer_detail: str = ""):
        ...
        super().__init__(
            f"Cryozen could not read {what} (an internal profile-scoping bug on the multiplexed "
            "gateway, not your configuration). Run `cryozen gateway restart`; if it keeps happening, "
            "report it with `cryozen debug share`."
        )
...
_GLOBAL_ENV_EXACT = frozenset({
    # Cryozen runtime / deployment
    "CRYOZEN_HOME", "CRYOZEN_PROFILE", "CRYOZEN_GATEWAY_LOCK_DIR",
    "CRYOZEN_MAX_ITERATIONS", "CRYOZEN_API_TIMEOUT",
    ...
    "CRYOZEN_KANBAN_DB", "CRYOZEN_KANBAN_WORKSPACES_ROOT", "CRYOZEN_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "CRYOZEN_KANBAN_",
    "CRYOZEN_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Cryozen writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
# ``invalidate_env_file_cache()`` is the explicit knob; ``cryozen_cli.config.invalidate_env_cache()``
# calls it for Cryozen's own .env writers.
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    """THE ``.env`` tokenizer: every reader (profile scope, ``cryozen_cli.config.load_env``, the dashboard
...
def build_profile_secret_scope(cryozen_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(cryozen_home) / ".env")
    try:
        from cryozen_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(cryozen_home))
    except Exception:
        external_secrets = {}
    ...
    if bridged is not None and _is_process_home(cryozen_home):
        secrets.setdefault("GATEWAY_ALLOW_ALL_USERS", bridged)
    return secrets


def _is_process_home(cryozen_home: Path) -> bool:
    from cryozen_constants import get_process_cryozen_home
    try:
        return Path(cryozen_home).resolve() == get_process_cryozen_home().resolve()
    except OSError:
        return False
