...
INSPECTION_STAMP_ATTR = "data-cryozen-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-cryozen-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-cryozen-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-cryozen-vault-slot]").forEach((n) => n.removeAttribute("data-cryozen-vault-slot"));
