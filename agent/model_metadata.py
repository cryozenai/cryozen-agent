...
from cryozen_constants import OPENROUTER_MODELS_URL, openrouter_variant_base
...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    for env_var in ("CRYOZEN_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _cache_file(name: str) -> Path:
    ...
    return get_cryozen_home() / "cache" / name
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    runs (``cryozen -q``, cron) start cold, so without this every launch paid the live probe. Local endpoints are never memoized."""
...
def _strip_openrouter_routing_variant(
    ...
    :func:`cryozen_cli.models.validate_requested_model` applies. Sharing the
...
def _is_codex_route(provider: str, base_url: str, custom_providers: list | None) -> bool:
    ...
    ``openai-codex`` provider (also behind a ``CRYOZEN_CODEX_BASE_URL`` / ``model.base_url`` proxy)
    ...
    with contextlib.suppress(Exception):  # config unreadable → not a known Codex route
        from cryozen_cli.config import get_custom_provider_api_mode
...
def _get_context_cache_path() -> Path:
    ...
    from cryozen_constants import get_cryozen_home
    return get_cryozen_home() / "context_length_cache.yaml"
...
def _invalidate_cached_context_length(model: str, base_url: str) -> None:
    ...
    for memo_key in list(_BEDROCK_PROBE_FAILURE_CACHE):  # snapshot: another thread may be memoising
        if memo_key[:2] == (cryozen_home_key(), stripped) and memo_key[2] in (model, bare):
...
def _normalize_model_version(model: str) -> str:
    """Dots -> dashes so dashed ids (claude-opus-4-6) compare with OpenRouter's (claude-opus-4.6)."""
...
def _resolve_codex_oauth_context_length_with_source(model: str, access_token: str = "") -> Tuple[Optional[int], str]:
    ...
    # ``-900k`` variants are Cryozen picker aliases — the Codex catalog only knows the base slug, so resolve
...
def _resolve_bedrock_context_length(model: str, base_url: str) -> Optional[int]:
    ...
    from cryozen_constants import cryozen_home_key
    memo_key = (cryozen_home_key(), cache_key_url.rstrip('/'), model, region)
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        with contextlib.suppress(Exception):  # fall through to models.dev
            from cryozen_cli.models import get_copilot_model_context
            ctx = get_copilot_model_context(model, api_key=api_key)
            if ctx:
                return ctx
    # 5b. Codex OAuth (lower limits than the direct API for the same slug; its own /models is
    # authoritative). Persist ONLY the authoritative source ("live"): a static-table value cached
    # on a blip would be frozen in by step 1 forever.
    sourced = {
...
def get_model_context_length(
    ...
    # in cryozen_cli.models.validate_requested_model.
    ...
    # (CRYOZEN_CODEX_BASE_URL, model.base_url, custom api_mode: codex_responses) the URL looks
    ...
    if not effective_provider:
        ...
        if model in metadata:
            ...
            if or_ctx == 32768 and _model_name_suggests_stale_32k_underreport(model):
                ...
                return or_ctx
    # 7. Local server before hardcoded defaults — ``Llama-3.1-70B-Instruct`` matches ``llama``
