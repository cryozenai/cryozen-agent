"""OpenAI-shape bridge shared by Cryozen' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Cryozen' tool schemas travel INTO the
...
read/edit/execute tools forwards only Cryozen' agent-level tools, since re-offering overlapping ones
makes Cryozen redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``cryozen_projected_messages``)
    Cryozen reads off the ``create`` result; a plain list would drop them on the stream path."""
