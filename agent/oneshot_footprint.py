"""What a finite one-shot session (``cryozen chat -q`` / ``--oneshot``, ``cryozen -z``) does NOT do.

A one-shot run has no later session in its CRYOZEN_HOME to learn for: the process answers one query and
...
delegation dispatcher already read (``CRYOZEN_SINGLE_QUERY_SESSION``), so interactive sessions are untouched:
...
def is_single_query_session() -> bool:
    ...
    return str(get_session_env("CRYOZEN_SINGLE_QUERY_SESSION", "") or "") == "1"
