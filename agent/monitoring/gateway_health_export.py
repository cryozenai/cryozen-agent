...
_OBSERVABLE_METRIC_NAMES = (
    "cryozen.gateway.up", "cryozen.gateway.state", "cryozen.gateway.active_agents", "cryozen.gateway.busy",
    "cryozen.gateway.drainable", "cryozen.gateway.restart_requested", "cryozen.gateway.background_work",
    "cryozen.gateway.background_delegations", "cryozen.platform.up", "cryozen.platform.degraded",
    "cryozen.cron.scheduler.heartbeat_age_seconds", "cryozen.cron.scheduler.last_success_age_seconds",
    "cryozen.cron.scheduler.catch_up_occurrences", "cryozen.cron.jobs.enabled", "cryozen.cron.jobs.running",
    "cryozen.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="cryozen-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("cryozen.gateway.background_work", _read_background_work_count),
            ("cryozen.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("cryozen.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("cryozen.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'cryozen-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="cryozen-gateway-health-export", daemon=True)
            thread.start()
