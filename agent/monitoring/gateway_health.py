...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "cryozen.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("cryozen.gateway.up", int(bool(gateway_running))),
        metric("cryozen.gateway.active_agents", active_agents),
        metric("cryozen.gateway.busy", int(bool(busy))),
        metric("cryozen.gateway.drainable", int(bool(drainable))),
        metric("cryozen.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("cryozen.gateway.state", 1, **{"cryozen.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"cryozen.platform": str(platform), "cryozen.platform.state": state}
        metrics.append(metric("cryozen.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("cryozen.platform.degraded", int(is_degraded), **pattrs, **{"cryozen.error_code": error_code}))
        if is_degraded:
