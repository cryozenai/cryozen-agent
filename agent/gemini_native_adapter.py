...
try:
    import cryozen_cli as _cryozen_cli

    _CRYOZEN_VERSION = str(_cryozen_cli.__version__)
except Exception:
    _CRYOZEN_VERSION = "0.0.0"
_API_CLIENT = f"cryozen-agent/{_CRYOZEN_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Cryozen typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.cryozen-agent/.env, "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.cryozen-agent/.env (get one at https://aistudio.google.com/app/apikey), or run `cryozen setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Cryozen retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No CRYOZEN_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
