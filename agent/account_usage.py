...
from cryozen_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
class AccountUsageSnapshot:
    ...
    # fields Cryozen does not normalize yet. Only populated by providers that fetch a JSON body.
...
def _usage_windows(
    ...
    """Build windows from ``source[key][used_key]`` (already a percentage)."""
...
def _codex_reset_outcome(body: dict, available: int) -> CodexResetRedeemResult:
    ...
    if code == "reset":
        ...
        try:
            from cryozen_cli.auth import clear_codex_pool_quota_cooldowns
...
def redeem_codex_reset_credit(
    *, base_url: Optional[str] = None, api_key: Optional[str] = None, force: bool = False,
    ...
    try:
        ...
        return _unavailable("No Codex credentials available. Run `cryozen auth` to sign in with your ChatGPT account.")
    ...
    try:
        ...
        if code in (401, 403):
            return _unavailable(f"Codex backend rejected the request (HTTP {code}). Reset credits require ChatGPT-account "
                                "(OAuth) auth — run `cryozen auth` and sign in with your ChatGPT account.")
...
_USAGE_FETCHERS: dict[str, Callable[[Optional[str], Optional[str]], Optional[AccountUsageSnapshot]]] = {
    "openai-codex": _fetch_codex_account_usage, "openrouter": _fetch_openrouter_account_usage,
}
