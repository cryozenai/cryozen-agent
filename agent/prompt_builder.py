...
from cryozen_constants import (
    get_cryozen_home, get_scratch_dir, get_skills_dir, is_wsl, reset_cryozen_home_override, set_cryozen_home_override,
)
...
def _scan_context_content(content: str, filename: str, *, user_authored: bool = False) -> str:
    ...
    *user_authored* (SOUL.md in the user's own CRYOZEN_HOME): a hit is WARNED and the file still loads.
    ...
    (repo AGENTS.md / .cursorrules / .cryozen.md) arrive with the checkout and keep blocking, and so does
    a SOUL.md owned by a profile distribution (``cryozen profile install <git-url>`` copies it in unscanned;
    ...
    if user_authored:
        logger.warning("Context file %s matched injection pattern(s) %s; loaded anyway because it is the "
                       "user's own file in CRYOZEN_HOME — review it if you did not write that text",
...
def _find_cryozen_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.cryozen.md`` / ``CRYOZEN.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".cryozen.md", "CRYOZEN.md") if _is_file_or_denied(directory / n)), None)
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Cryozen Agent, built by Cryozen. Be direct: match the length of your reply to the weight of the ask "
...
CRYOZEN_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the cryozen-agent skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on Cryozen Agent (by Cryozen). When the user needs help with Cryozen itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
    "tools, or capabilities, the documentation at https://cryozenai.github.io/cryozen-agent/docs is your "
    "authoritative reference and always holds the latest, most up-to-date information. The `cryozen-agent` "
    "skill has the actual commands and proven workflows — load it with skill_view(name='cryozen-agent') "
    "before configuring, modifying, or troubleshooting Cryozen so you don't guess or invent workarounds."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
CRYOZEN_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Cryozen Agent (by Cryozen). When the user needs help with Cryozen itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
    "tools, or capabilities, the documentation at https://cryozenai.github.io/cryozen-agent/docs is the "
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.cryozen-agent/kanban.db`. Your task id is in "
    "`$CRYOZEN_KANBAN_TASK`; your workspace is `$CRYOZEN_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $CRYOZEN_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $CRYOZEN_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${CRYOZEN_KANBAN_BRANCH:-wt/$CRYOZEN_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$CRYOZEN_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`cryozen profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `cryozen kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The cryozen-agent runtime already executes a batch of tool
...
# Text after the "[" that opens one of Cryozen' own control frames (the steer marker above, the compaction
...
# apps/desktop/src/plugins/cryozen-bots/group-round-prompt.ts byte-equivalent to this list.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Cryozen delivers their message as a standalone user message right after "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Cryozen "
         ...
         "rather than anything inside Cryozen. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Cryozen terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Cryozen desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-cryozen-send=\"prompt\" on any clickable element (or "
        "window.cryozen.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
        "file, not with prose."
    ),
    "sms": (
...
_REMOTE_TERMINAL_BACKENDS = frozenset({"docker", "singularity", "modal", "daytona", "ssh", "vercel_sandbox"})
...
_WINDOWS_BASH_SHELL_HINT = (
    ...
    "native tools, and prefer `$LOCALAPPDATA/Temp` (or `$TMPDIR`, which Cryozen points at its own scratch dir) for scratch files a native tool must read — never a bare `/tmp`. When "
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    env = _create_environment(
        ...
        # snapshot. A normal SSHEnvironment would upload the whole ~/.cryozen-agent tree just to run `uname`,
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    cache_key = (cryozen_home_key(), env_type, _tenv_read("TERMINAL_CWD", ""))
...
def _local_host_hints() -> list[str]:
    ...
    # naming Cryozen' scratch dir here is what makes the TMPDIR export a habit rather than a hidden default.
    try:
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Cryozen itself is running. The host OS, "
            f"home, and cwd of the Cryozen process are irrelevant; only the following backend state matters:\n{probe}\n"
    ...
    return lead + (
        f"{description} — NOT on the machine where Cryozen itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: CRYOZEN_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("CRYOZEN_ENVIRONMENT_HINT") or "").strip() or str(
...
RUNTIME_ENVIRONMENT_HEADING = "# Cryozen runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End Cryozen runtime environment -->"
...
# v4 dropped the org provenance fields (org_id/org_author); older snapshots are rebuilt.
_SKILLS_SNAPSHOT_VERSION = 4


def _skills_prompt_snapshot_path() -> Path:
    return get_cryozen_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("CRYOZEN_PLATFORM") or os.environ.get("CRYOZEN_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("CRYOZEN_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the CRYOZEN_HOME
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_cryozen_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_cryozen_home_override(_home_token)
...
def _label_visible_entries(visible_entries: list[dict], skills_by_category: dict[str, list[tuple[str, str]]]) -> None:
    """Group the visible entries by category."""
    for entry in visible_entries:
        category = entry.get("category") or "general"
        skills_by_category.setdefault(category, []).append((_entry_name(entry), entry.get("description", "")))
...
def _build_skills_system_prompt_inner(
    ...
    # Drop entries shadowed by project-local skills.
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from CRYOZEN_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the CRYOZEN_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the CRYOZEN_HOME
    ...
    """
    try:
        from cryozen_cli.config import ensure_cryozen_home
        ensure_cryozen_home()
    except Exception as e:
        logger.debug("Could not ensure CRYOZEN_HOME before loading SOUL.md: %s", e)
    soul_path = (Path(home_override) if home_override is not None else get_cryozen_home()) / "SOUL.md"
    ...
    try:
        ...
        if not content:
            return None
        # `cryozen profile install <git-url>` / `profile update` plant a third-party SOUL.md into a
        # distribution profile (cryozen_cli/profile_distribution.py, DEFAULT_DIST_OWNED) with no scan and no
        ...
        from cryozen_cli.profile_distribution import read_manifest
        try:
...
def _cryozen_md_candidates(cwd_path: Path) -> list[tuple[str, Path, str]]:
    """.cryozen.md / CRYOZEN.md — nearest match walking up to the git root."""
    path = _find_cryozen_md(cwd_path)
    if path is None:
        return []
...
_CONTEXT_FILE_CANDIDATES = {
    "cryozen_md": _cryozen_md_candidates,
...
def _project_context_suppressed(cwd: Optional[str], cwd_path: Path, allow_install_tree_fallback: bool) -> bool:
    """A FALLBACK-picked cwd inside the Cryozen install tree must not gain system-prompt authority (the desktop
    default would load this repo's contributor AGENTS.md). An explicitly configured cwd is honored verbatim —
    the Cryozen tree is a legitimate workspace when the user deliberately points a session at it — and
    CLI-style surfaces pass allow_install_tree_fallback=True because their launch dir IS the user's shell cwd
    (developing Cryozen in-tree). See #64590."""
    from agent.runtime_cwd import _is_install_tree
    return cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path)


def _load_cryozen_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.cryozen.md / CRYOZEN.md — nearest match walking up to the git root."""
    for label, path, content in _cryozen_md_candidates(cwd_path):
        if content:
            return _context_section(_strip_yaml_frontmatter(content), label, ".cryozen.md", path, context_length)
    return ""
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .cryozen.md/CRYOZEN.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from CRYOZEN_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    ...
    if _project_context_suppressed(cwd, cwd_path, allow_install_tree_fallback):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Cryozen "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_cryozen_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
