"""Rate-limit and response-cache header capture for ``AIAgent``.

Parses provider response headers into ``_rate_limit_state`` and logs OpenRouter cache status.
...
class RateLimitHeadersMixin:
    """Rate-limit header capture and OpenRouter cache logging (see module docstring)."""
    ...
    def _capture_anthropic_response_headers(self, http_response: Any) -> None:
        """Capture rate-limit state from Anthropic Messages response headers (the SDK's aggregated
        ``Message`` drops them). Fail-open."""
