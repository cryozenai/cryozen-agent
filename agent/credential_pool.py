...
from cryozen_cli.auth_plugin_providers import plugin_refresh_hook
...
import cryozen_cli.auth as auth_mod
...
def _is_source_suppressed_fn() -> Callable[[str, str], bool]:
    """``cryozen_cli.auth.is_source_suppressed`` (late-bound), or an always-False stub."""
    try:
        from cryozen_cli.auth import is_source_suppressed
...
# singleton to re-seed from and the user can re-add via ``cryozen auth add``.
...
# selection. On Windows several Cryozen processes share one rotating log guarded by concurrent-log-handler's
...
# readiness handshake ("Timed out connecting to Cryozen backend after 15000ms"). Logging the condition at
...
_EXTRA_KEYS = frozenset({
    "token_type", "scope", "client_id", "obtained_at",
    "expires_in", "tls", "secret_source", "secret_fingerprint",
...
class PooledCredential:
    ...
    # Epoch of the last deliberate ``cryozen auth reset`` of this entry. Sticky: a later exhaustion
    ...
    def runtime_base_url(self) -> Optional[str]:
        if self.provider == "openai-codex":
            ...
            return get_secret_str("CRYOZEN_CODEX_BASE_URL", "").strip().rstrip("/") or self.base_url
...
def _codex_principal_identity(access_token: Any) -> Optional[Tuple[str, str]]:
    ...
    Decoded without signature verification: this only decides whether two credentials Cryozen
...
def _codex_entry_tracks_singleton(entry: PooledCredential, singleton_tokens: Dict[str, Any]) -> bool:
    ...
    ``cryozen auth add openai-codex`` (must never be overwritten — adopting turned two logins into
...
def custom_provider_pool_key_candidates(
    ...
    ``cryozen auth add <key>`` stores ``providers.<key>`` credentials under the
...
def _guarded_global_root(global_path: Optional[Path]) -> Optional[Path]:
    ...
    never write the real user's ``~/.cryozen-agent/auth.json`` even when CRYOZEN_HOME
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".cryozen-agent" / "auth.json"
            try:
...
def _write_through_provider_state_to_global_root(
    ...
    ``cryozen_cli.auth._write_through_xai_oauth_to_global_root``.
...
def _update_root_pool_rows(
    ...
    *, status_cleared_ids: Optional[Iterable[str]] = None,
    ...
    profile's singleton-prune must not delete the root grant, so
    ``removed_ids`` is ignored by callers.
    """
...
def persist_pool_entries(
    ...
    *,
    ...
    A named profile that sees a single-use-refresh provider (Codex,
    xAI OAuth) only through the global-root fallback must not
...
# ``cryozen_cli.auth`` refresh function and terminal-error predicate (looked
...
# ``cryozen_cli.auth_plugin_providers.is_refreshable_oauth_provider``); any other provider is returned
# unchanged by that path, so callers must not report a refresh for them.
REFRESHABLE_OAUTH_PROVIDERS = frozenset(_TOKENS_SINGLETON_PROVIDERS)
...
_REFRESH_TIMEOUT_ENV_VARS = {
    "openai-codex": "CRYOZEN_CODEX_REFRESH_TIMEOUT_SECONDS",
    "xai-oauth": "CRYOZEN_XAI_REFRESH_TIMEOUT_SECONDS",
}
...
class CredentialPool(CredentialPoolAdminMixin, CredentialPoolModelCooldownMixin):
    ...
    def _sync_entry_from_pool_store(self, entry: PooledCredential) -> PooledCredential:
        """Adopt a token pair rotated by another pool instance (xai-oauth, plugin providers).
        ...
        protect a single-use refresh token across requests or processes. A row
        with no token material at all is refused: adopting it would blank a
        usable credential.
        """
        if self.provider != "xai-oauth" and plugin_refresh_hook(self.provider) is None:
            return entry
        is_xai = self.provider == "xai-oauth"
        display = "xAI" if is_xai else self.provider
        try:
            persisted = next(
            ...
            # No token material at all is never a "rotation" (a plugin row a peer blanked
            # mid-write): adopting it would replace a usable credential with nothing.
    ...
    def _sync_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        A fresh ``cryozen model`` / ``cryozen auth`` login writes new tokens
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        token. Applies to the OpenAI Codex and xAI OAuth singletons.
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # across Cryozen processes, or two processes adopt the same on-disk
        # token, both POST it, and the loser gets ``refresh_token_reused`` /
        # ``invalid_grant``. Serialize through the shared cross-process
        # auth-store flock; a waiter's in-lock re-sync picks up the winner's
        # rotated token and skips the POST.
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        env_var = _REFRESH_TIMEOUT_ENV_VARS.get(self.provider)
    ...
    def _recover_failed_refresh(self, entry: PooledCredential, exc: Exception) -> Optional[PooledCredential]:
        ...
        if self.provider in _TOKENS_SINGLETON_PROVIDERS:
            ...
            if getattr(auth_mod, terminal_fn_name)(exc):
                ...
                # silent quarantine looked like "I logged in once and Cryozen keeps failing" (#113023).
                logger.warning(
                    "%s OAuth refresh token is terminally invalid (%s); clearing local token state. "
                    "Re-run 'cryozen auth add %s' to sign in again.", display, exc, self.provider)
    ...
    def _mark_dead_refresh_grant(self, entry: PooledCredential, exc: Exception) -> None:
        ...
        (``cryozen auth add``) survives, and an unmarked survivor re-enters rotation and re-fires the
    ...
    def _reset_cleared_after(self, entry: PooledCredential) -> Optional[float]:
        """Epoch of a ``cryozen auth reset`` persisted by another process AFTER *entry*'s status, else None."""
        try:
    ...
    def _resync_stale_entry(self, entry: PooledCredential) -> PooledCredential:
        ...
        The user may have re-authed (``cryozen model`` / ``cryozen auth``,
        another profile) leaving fresh tokens on disk while
        the pool entry is frozen behind ``last_error_reset_at``. A ``cryozen auth
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `cryozen auth add %s`",
...
def _upsert_entry(entries: List[PooledCredential], provider: str, source: str, payload: Dict[str, Any]) -> bool:
    ...
    if token_changed and not existing.access_token:
        # Borrowed sources (env-backed rows) are written to
...
def _normalize_pool_priorities(provider: str, entries: List[PooledCredential]) -> bool:
    """Anthropic: manually added keys rank ahead of the env-seeded ``ANTHROPIC_API_KEY`` row."""
...
class _Seeder:
    ...
    def upsert(self, source: str, payload: Dict[str, Any]) -> bool:
        """Upsert unless suppressed (``cryozen auth remove`` must stay stable across loads)."""
...
def _drop_anthropic_oauth_entries(seed: _Seeder) -> None:
    """Remove Anthropic subscription-OAuth rows persisted by older releases.

    Cryozen accepts only Anthropic API keys. Rows from the retired Claude Code / Claude
    subscription logins (``claude_code``, ``cryozen_pkce``, ``manual:*pkce``, ``sk-ant-oat``
    tokens) are dropped on load, and the change is persisted, instead of being rotated into
    requests."""
    retained = [entry for entry in seed.entries if entry.auth_type != AUTH_TYPE_OAUTH]
    if len(retained) != len(seed.entries):
        seed.entries[:] = retained
        seed.changed = True
...
def _seed_copilot_singleton(seed: _Seeder) -> None:
    ...
    try:
        from cryozen_cli.copilot_auth import (
            COPILOT_ENV_VARS,
...
def _seed_qwen_singleton(seed: _Seeder) -> None:
    ...
    try:
        from cryozen_cli.auth import resolve_qwen_runtime_credentials
...
def _seed_tokens_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    Cryozen owns its own Codex auth state and does NOT auto-import
    ...
    explicit one-time prompt via `cryozen auth openai-codex`.
    """
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    auth_store = _load_auth_store()
    if provider == "anthropic":
        _drop_anthropic_oauth_entries(seed)
        ...
        # `cryozen auth remove openai-codex` suppresses device_code; without
...
def get_env_prefer_dotenv(key: str) -> str:
    """Resolve a credential env var, preferring ~/.cryozen-agent/.env over os.environ.
...
def _warn_env_ingestion_once(provider: str, env_var: str) -> None:
    ...
    logger.warning(
        ...
        "cryozen auth remove %s <n> to suppress.",
        env_var,
        provider,
...
def _env_payload(*, env_var: str, token: str, base_url: str) -> Dict[str, Any]:
    ...
    try:
        from cryozen_cli.env_loader import get_secret_source
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    for env_var in env_vars:
        token = get_env_prefer_dotenv(env_var)
        if not token or (provider == "anthropic" and token.strip().startswith("sk-ant-oat")):
            # A Claude subscription OAuth token is never an Anthropic credential for Cryozen.
            continue
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # requested (an `cryozen auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons disappear when their backing file is gone.
        return is_borrowed_credential_source(entry.source, entry.provider)
