...
class AgentNotice:
    ...
    renders its own way. ``kind``/``ttl_ms`` let a producer choose sticky or TTL presentation."""
...
class StatusOutputMixin:
    ...
    def _vprint(self, *args, force: bool = False, diagnostic: bool = False, **kwargs):
        ...
        the main response; ``force=True`` bypasses both. ``suppress_status_output`` (``cryozen chat -q``) wins."""
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``suppress_status_output`` (the strict machine-readable mode used by ``cryozen chat -Q``) always
