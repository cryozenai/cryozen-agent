...
``cryozen pause`` writes a sentinel at ``$CRYOZEN_HOME/ESTOP``; ``cryozen resume``
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.cryozen-agent).
from agent.file_safety import _cryozen_home_path as _cryozen_home, _cryozen_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `cryozen pause`."""
    return _cryozen_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (CRYOZEN_HOME=~/.cryozen-agent/profiles/<n>) must still honor an operator's ~/.cryozen-agent/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Cryozen is paused{tag}. New work is on hold; run `cryozen resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `cryozen resume` (%s)",
