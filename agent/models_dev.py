...
thread refreshes) → disk cache (~/.cryozen-agent/models_dev_cache.json, any age) →
...
from cryozen_constants import openrouter_variant_base
...
# Cryozen provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # Cryozen provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → Cryozen ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_cryozen_ids(mdev_id: str) -> List[str]:
    """Return the Cryozen provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for cryozen_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(cryozen_id)
...
def _models_dev_id(provider: str) -> Optional[str]:
    """models.dev provider id for a Cryozen provider id, or None. A custom provider reaches the
    ...
    if mdev_id is None:
        ...
        if mdev_id is not None and mdev_id not in PROVIDER_TO_MODELS_DEV.values() \
            ...
            if (key, alias) not in _UNKNOWN_CATALOG_PROVIDER_WARNED:
                ...
                logger.warning("providers.%s: catalog_provider %r is neither a Cryozen provider id nor a "
...
def _cryozen_path(name: str) -> Path:
    from cryozen_constants import get_cryozen_home
    return get_cryozen_home() / name


def _get_cache_path() -> Path:
    return _cryozen_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _cryozen_path("models_dev_cache.etag")
...
def _save_etag(etag: str) -> None:
    def write() -> None:
        ...
        mkdir_under_cryozen_home(etag_path.parent)
...
def _get_provider_models(provider: str, *, allow_network: bool = False) -> Optional[Dict[str, Any]]:
    """Resolve a Cryozen provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the Cryozen
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by Cryozen OR models.dev id), or None."""
    overrides = _load_model_overrides()
    provider_key = (provider or "").strip()
    if not overrides or not provider_key:
        return None
    # Forward (Cryozen → models.dev id) and reverse (caller passed a models.dev id, config keyed by Cryozen id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_cryozen_ids(provider_key)]
...
def _builtin_model_metadata(provider: str, model: str) -> Optional[Dict[str, Any]]:
    """Built-in metadata for a provider/model pair, if Cryozen has a vendor-specific entry."""
...
def get_provider_info(provider_id: str, *, allow_network: bool = True) -> Optional[ProviderInfo]:
    """Provider metadata by Cryozen or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(provider_id: str, model_id: str, *, allow_network: bool = False) -> Optional[ModelInfo]:
    """Full model metadata by Cryozen or models.dev provider ID (exact match, then case-insensitive), or
