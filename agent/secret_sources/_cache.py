...
a cache problem must never block Cryozen startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_cryozen_dotenv()``, else ``$CRYOZEN_HOME``/``~/.cryozen-agent``."""
    if home_path is None:
        from cryozen_constants import get_cryozen_home

        home_path = get_cryozen_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<cryozen_home>/cache/<basename>``::
