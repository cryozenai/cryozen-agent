...
# applied by cryozen_cli/commands_platforms.py, not here.
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    try:
        ...
        resolved_platform = os.getenv("CRYOZEN_PLATFORM") or get_session_env("CRYOZEN_SESSION_PLATFORM")
    except Exception:
        resolved_platform = os.getenv("CRYOZEN_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Effective Cryozen home the scan is scoped to (profiles carry their own
    ...
    ``set_cryozen_home_override``), but the module-level scan only tracked
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    """Append a ``[Skill config: ...]`` block with resolved ``metadata.cryozen.config``
    ...
    try:
        ...
        if not resolved:
            return
        parts.append("")
        parts.append(f"[Skill config (from {display_cryozen_home()}/config.yaml):")
...
def _scan_skill_md(skill_md: Path, disabled: set, seen_names: set, commands: Dict[str, Dict[str, Any]]) -> None:
    ...
    if skill_command_collision_note(name) is not None:
        logger.warning("Skill %r generates slash command '/%s' which collides with a core Cryozen command; "
...
def build_preloaded_skills_prompt(
    ...
    this path bypasses the scan-time filter, and ``cryozen -s <skill>`` must not
...
def build_auto_load_prompt(
    ...
    *home_override* makes home resolution EXPLICIT (same seam as ``build_skills_system_prompt``): the config,
    ...
    that lost the CRYOZEN_HOME ContextVar cannot pin the launch profile's skills into another profile's prompt.
    """
    from cryozen_constants import reset_cryozen_home_override, set_cryozen_home_override
    home_token = set_cryozen_home_override(str(home_override)) if home_override is not None else None
    try:
