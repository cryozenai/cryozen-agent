...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `cryozen doctor` for credential-chain diagnostics, or",
    ...
    _dhh = display_cryozen_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_API_KEY in {_dhh}/.env",
        "     • Verify the key at https://platform.claude.com/settings/keys",
        "     • Inspect what Cryozen holds: cryozen auth list anthropic",
        "     • Clear stale keys: cryozen config set ANTHROPIC_API_KEY \"\"",
    )
...
def recover_after_classification(
    ...
    Codex stale-reasoning strip on 401 ``token_expired`` →
    credential-pool rotation → image shrink → multimodal-tool-content strip → corrupt-image
    strip → per-provider 401 credential refresh →
    ...
    # Route rejecting a reasoning disable: a reasoning-mandatory route (OpenRouter,
    ...
    if (
        ...
        try:
            from cryozen_cli.models_reasoning_caps import refresh_reasoning_caps_async
    ...
    if provider in {"openai-codex", "xai-oauth"} and status_code == 401:
        if provider == "openai-codex":
            ...
            _vlines(
                ...
                "      refreshed by another client (Codex CLI, VS Code) or another Cryozen profile.",
            ...
            _vlines(
                ...
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `cryozen model`.",
            )
        return
    _vlines(
        agent,
        "   💡 Your API key was rejected by the provider. Check:",
        "      • Is the key valid? Run: cryozen setup",
...
def _missing_vendor_prefix_suggestion(api_error: Exception, provider: Any, model: Any) -> Optional[str]:
    ...
    if getattr(api_error, "status_code", None) != 404:
        return None
    try:
        from cryozen_cli.model_normalize import suggest_prefixed_model_id
...
def nonretryable_client_error_result(
    ...
    if classified.is_auth or classified.reason == FailoverReason.billing:
        ...
        _vlines(agent, f"   💡 Fix: pick another model (/model), or check `{display_cryozen_home()}/logs/agent.log`.")
    ...
    if classified.reason == FailoverReason.upstream_blocked:
        _vlines(
            ...
            "      set `extra_headers: {User-Agent: CryozenAgent/1.0}` on the custom_providers entry,",
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "      To route future blocks to another provider automatically: cryozen fallback add",
        )
    ...
    if classified.reason == FailoverReason.ssl_cert_verification:
        _vlines(
            agent,
            "   💡 Cryozen couldn't verify the provider's security certificate. This fails the same",
            ...
            "      • Missing/stale system CA store? Refresh it (in Cryozen's venv: `uv pip install",
...
def max_retries_exhausted_result(
    ...
    result.update({
        ...
        # Present only for billing walls: (provider, billing_url, message).
...
class ClassifiedErrorVerdict:
    ...
    ``"break"`` (restart armed on ``_retry``) or ``"fallthrough"`` (proceed to overflow / client-error / backoff
...
def route_classified_error(
    ...
    error_context: Any, base_url: Any, model: Any,
