...
def _routermint_headers() -> dict:
    ...
    return {"User-Agent": f"CryozenAgent/{_CRYOZEN_VERSION}"}
...
_ROUTE_DEFAULT_HEADERS = (
    ...
    ("githubcopilot.com", lambda self, url: _lazy_attr("cryozen_cli.models", "copilot_default_headers")()),
    ...
    ("x.ai", lambda self, url: _lazy_attr("tools.xai_http", "cryozen_xai_default_headers")()),
)
...
class ClientLifecycleMixin:
    ...
    def _request_anthropic_client_key(self) -> tuple:
        """Cache key over everything forcing a fresh client: credential, base URL/region, timeout."""
    ...
    def _build_anthropic_client_for_key(self, key: tuple) -> Any:
        ...
        return build_anthropic_client(key[1], key[2], timeout=key[3])
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from cryozen_cli import auth as _auth
            resolve = (
    ...
    def _resolve_env_credentials(self) -> Optional[tuple]:
        ...
        if pconfig and getattr(pconfig, "auth_type", "") == "api_key" and getattr(pconfig, "api_key_env_vars", ()):
            ...
            if self.provider == "actual":
                ...
                from cryozen_cli.runtime_provider import _config_base_url_for_provider, _get_model_config
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ``~/.cryozen-agent/.env`` credential/base-url edits at the turn boundary (a Settings save updates ``.env``
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        if not self._is_copilot_provider():
            return False
        try:
            from cryozen_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token, evict_cached_exchanged_token
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        # Only native Anthropic re-reads ANTHROPIC_API_KEY; other anthropic_messages providers (MiniMax,
        # Alibaba, ...) and Azure use their own keys, which must never be swapped for an Anthropic one.
        if (
    ...
    def _apply_client_headers_for_base_url(self, base_url: str, *, apply_user_headers: bool = True) -> None:
        ...
        if self.api_mode not in ("anthropic_messages", "bedrock_converse"):
            try:
                from cryozen_cli.config import apply_custom_provider_extra_headers_to_client_kwargs
