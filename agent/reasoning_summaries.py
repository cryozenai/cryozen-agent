...
The chat wire lacks the Responses API's ``summary_index`` delimiter (verified live on
``openai/gpt-5.6-sol`` over OpenRouter), so plain concatenation glues ``**One****Two**`` into one
half-bold paragraph. The boundary is re-derived from a delta opening a bold heading, matching
the blank-line join Cryozen' own Responses adapter does.
"""
