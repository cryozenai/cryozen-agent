...
``## label`` section, so a .cryozen.md whose YAML frontmatter the builder strips can read a few chars larger
...
_STATUS_DISPLAY = {
    ...
    "suppressed": ("○", "not loaded — cwd fell back to the Cryozen install tree"),
}
...
def list_context_file_sources(
    ...
    """One dict per context file Cryozen considered, in the builder's priority order.
    ...
    if not skip_soul:
        home = Path(home_override) if home_override is not None else _pb.get_cryozen_home()
