...
def ctx_bound(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    is a ContextVar-scoped CRYOZEN_HOME override plus the per-turn secret scope; a worker started
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include ``cryozen_home`` (profile-scoped storage; never hardcode
        ``~/.cryozen-agent``) and ``platform``; may include ``agent_context`` ("primary" |
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        """Setup fields for ``cryozen memory setup`` ([] if none): ``key``, ``description``,
    ...
    def backup_paths(self) -> List[str]:
        """Absolute paths of provider state OUTSIDE CRYOZEN_HOME for ``cryozen backup``/``import``
