# Cryozen Agent - Development Guide

Instructions for AI coding assistants and developers working on the cryozen-agent codebase.
...
Cryozen is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Cryozen ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`cryozen tools`, `cryozen setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `CRYOZEN_HOME` — two of them (A→B→A) when the
...
- **New `CRYOZEN_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `cryozen tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.cryozen-agent/plugins/` or pip
  entry point), promoted in Cryozen GitHub Discussions. This is a
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `cryozen <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`cryozen webhook`, `cryozen cron`, `cryozen tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.cryozen-agent/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, or one behind URL + token (a self-hosted server),
and only the first two carry `CRYOZEN_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Cryozen desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_CRYOZEN_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `CRYOZEN_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`cryozen --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.cryozen-agent/cryozen-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _CRYOZEN_CORE_TOOLS
├── cli.py                # CryozenCLI (REPL, slash dispatch) + cryozen_cli/cli_*_mixin.py
├── cryozen_state.py       # SessionDB facade; cryozen_state_*.py siblings
├── cryozen_constants.py   # get_cryozen_home(), display_cryozen_home() — profile-aware paths
├── cryozen_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── cryozen_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
├── ui-tui/               # Ink (React) terminal UI — `cryozen --tui`
...
**User state:** `~/.cryozen-agent/config.yaml` (settings), `~/.cryozen-agent/.env` (secrets only),
`~/.cryozen-agent/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_cryozen_home()`. Browse logs with `cryozen logs [--follow] [--level] [--session]`.
...
families: `cryozen_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`cryozen_cli/kanban.py` (14), `cryozen_cli/web_server.py` (13 + 24 routers), `cryozen_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `cryozen_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::cryozen_cli.plugin_compat.CryozenPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `cryozen_cli.update_cmd._cryozen_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `cryozen_cli/AGENTS.md`.
- **Never hardcode `~/.cryozen-agent`.** `get_cryozen_home()` for code paths, `display_cryozen_home()`
  for user-facing text (both from `cryozen_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.cryozen-agent/profiles`) so `cryozen -p x profile list`
...
- **One process may serve many profiles; code that runs outside a turn binds the owning
  ...
  module constants are a bug class — key slots by `cryozen_home_key()` or resolve at call time.
  ...
  `CRYOZEN_HOME`. Advisory lint: `scripts/check_profile_scope_patterns.py`.
- **Machine facts and resource lookup go through `cryozen_platform`.** `cryozen_platform.host` is the
  ...
  hand-written known-path table outside `cryozen_platform/` fails
  `tests/test_managed_runtime_resolution.py` unless allowlisted with a reason; resolvers land in
  `cryozen_platform/resolver/`. Lookup never installs, downloads, or starts anything.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `cryozen webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `CRYOZEN_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `CRYOZEN_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `CRYOZEN_TEST_FILE_RETRIES=0` disables); a worker killed by signal or the file timeout is never
...
- **Placement mirrors the source tree.** A test lives in `tests/<top-level source dir>/` (`tests/cryozen_cli/`,
  `tests/agent/`, `tests/cryozen_state/`, `tests/gateway/relay/`, ...); installer/updater script tests
  under `tests/scripts/{install,desktop_update}/`. Only tests of root-level modules (`batch_runner`,
  `utils`, `cryozen_constants`, packaging) sit directly in `tests/`. No issue numbers in filenames —
...
- **Tests must not write to `~/.cryozen-agent/`.** The autouse `_isolate_cryozen_home` fixture in
  `tests/conftest.py` redirects `CRYOZEN_HOME`; never hardcode `~/.cryozen-agent/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_cryozen_home()` stay
  in the temp dir (pattern: `tests/cryozen_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".cryozen-agent"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("CRYOZEN_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `CRYOZEN_HOME` — code reads the
  env var, not `Path.home()/.cryozen-agent`.
...
`tests/cryozen_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `cryozen_cli/`, `main.py` | `cryozen_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `cryozen update` pipeline, profiles / multiplex |
...
| `plugins/`, `cryozen_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `cryozen_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`cryozen_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
...
`cryozen-agent-dev` skill, not here.
