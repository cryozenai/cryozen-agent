...
schedule via the `cronjob` tool; users via `cryozen cron list|add|edit|pause|resume|run|remove` or
...
- **Inactivity watchdog** on cron agent sessions (`_cron_inactivity_seconds()`): default 600s idle,
  `CRYOZEN_CRON_TIMEOUT` overrides, `0` = unlimited. It is idle time, not wall-clock — a stalled
...
- Per-home tick lock `<home>/cron/.tick.lock` prevents duplicate ticks across processes for
  that profile's store; never a `~/.cryozen-agent/...` literal.
...
- **A claim is released under the key it was registered with.** The cron scope is a ContextVar:
  ...
  sees phantom work. Never rebuild a home from a key half (`Path(key[0])`): `cryozen_home_key`
...
- **The host gateway stands down for a profile that runs its OWN gateway.** `run.py::
  _cron_profile_gate` (the same gate `cryozen_cli/web_server.py` passes) keeps the launch process
...
- The cron ticker runs in the desktop-spawned backend when `CRYOZEN_DESKTOP=1` — that env var means
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `cryozen kanban
...
- **CLI:** `cryozen_cli/kanban.py` facade + 14 `kanban_*.py` siblings (`boards`, `db`, `db_connect`,
...
- **Toolset:** `tools/kanban_tools.py` — `kanban_show, kanban_complete, kanban_request_review,
  ...
  `kanban` (`cryozen tools enable kanban --platform <p>`; default-off, in `CONFIGURABLE_TOOLSETS`) get
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/cryozen-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `CRYOZEN_KANBAN_BOARD` pinned in their env and
...
- **Notifications leave through the task's owning profile.** `cryozen_cli/kanban_db_notify.py`
  ...
  Dispatched workers get `CRYOZEN_KANBAN_BOARD` and the assignee's `CRYOZEN_HOME` pinned in a
...
- **Prompt injection sites gate on ownership, not tool access.** Tool access (`kanban_show` visible
  via a profile's toolset) and an inherited `CRYOZEN_KANBAN_TASK` (delegate children, cron runs beside
...
- **Descendant fence is a path, not a flag.** A delegated child's Kanban marker
  ...
  `kanban_path_is_fenced(path)` denies mutations only on the dispatcher-pinned `CRYOZEN_KANBAN_DB`
  or under that root, so a child working against a scratch `CRYOZEN_HOME` keeps a writable board.

## Tests

`tests/cron/`, `tests/cryozen_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
