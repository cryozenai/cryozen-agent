...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    from cryozen_cli.auth import AuthError, is_rate_limited_auth_error
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `cryozen setup`) for that home, or pin a "
            "working provider via `cryozen cron edit "
...
def _credential_store_scope_label() -> str:
    """``[profile '<name>', CRYOZEN_HOME <path>]`` for the home this preflight read credentials from.

    The verdict must name the store it judged: a scheduler process whose home differs from the
    shell where "the same credential works" (Docker HOME vs CRYOZEN_HOME, a multiplexed satellite
    ...
    return f"[profile '{get_active_profile_name() or 'default'}', CRYOZEN_HOME {get_cryozen_home()}]"
...
def _primary_profile_routes_for_current_home() -> list:
    ...
    try:
        ...
        primary_home = get_default_cryozen_root()
        current_home = _sched.Path(get_cryozen_home())
        if (
    ...
    for platform_name in platform_parts:
        ...
        if (
            ...
            return (
                ...
                "`cryozen setup` or change the job's `deliver` target."
            )
    return None
