...
``AuthError`` from ``cryozen_cli.auth_codex._codex_quota_exhausted_error``). When the whole
...
def hold_active(job: Dict[str, Any], now: Optional[datetime] = None) -> bool:
    ...
    return until is not None and until > (now or _cryozen_now())
...
def plan_hold(job: Dict[str, Any], hold_seconds: float) -> bool:
    ...
    if schedule.get("kind") not in {"cron", "interval"} or job.get("state") == "paused":
        clear_state(job)
        return False
    window_end = _cryozen_now() + timedelta(seconds=float(hold_seconds) + HOLD_SLACK_SECONDS)
