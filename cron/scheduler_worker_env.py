...
``cron.scheduler``, not ``cryozen_cli.main``, so nothing bootstraps the gateway's checkout
...
The shared subprocess sanitizer strips Cryozen-owned PYTHONPATH entries because user
children must not see our tree. This child IS Cryozen, so the pin is applied *after* the
...
def pin_cryozen_tree_on_pythonpath(worker_env: dict, repo_root: Path) -> dict:
