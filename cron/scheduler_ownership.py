...
``is_gateway_runtime_lock_active`` goes through ``get_process_cryozen_home()``, which ignores the
...
def register_ticked_homes(homes) -> None:
    ...
    for home in homes:
        path = Path(home)
        resolved[cryozen_home_key(path)] = path
    with _ticked_lock:
...
def serves_profile(home: Optional[Union[Path, str]] = None) -> bool:
    ...
    key = cryozen_home_key(home if home is not None else get_cryozen_home())
    with _ticked_lock:
...
def record_serves_profile(record: Any, home: Optional[Union[Path, str]] = None) -> bool:
    ...
    from gateway.status import (
        _get_process_cryozen_home, _profile_label_for_home, _same_cryozen_home)

    try:
        target = Path(home) if home is not None else get_cryozen_home()
        record_home = record.get("cryozen_home")
        if not isinstance(record_home, str) or not record_home.strip():
            record_home = _get_process_cryozen_home()
        if _same_cryozen_home(record_home, target):
            return True
    except Exception:
        return False
