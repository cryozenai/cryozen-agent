...
_PROBE_INITIALIZE_BODY = (  # JSON-RPC ``initialize`` body for the content-type preflight POST
    ...
    '"capabilities":{},"clientInfo":{"name":"cryozen-probe","version":"0.1"}}}')
...
def _live_endpoint(server_name: str) -> Optional[tuple[str, dict]]:
    ...
    from cryozen_platform.resolver.app import AppResolver
...
class MCPServerTransportMixin:
    ...
    async def _complete_handshake_at_offered_version(self, session):
        ...
        client_info = getattr(session, "_client_info", None) or types.Implementation(name="cryozen-agent", version="0")
    ...
    def _track_spawned_children(self, new_pids: Set[int]) -> None:
        ...
        for _pid in new_pids:
            try:
                from cryozen_cli.process_identity import register_child
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _core._ensure_mcp_sdk():
            raise ImportError(f"MCP server '{self.name}' requires the 'mcp' Python SDK, but "
                              "it is not installed. Run `cryozen setup` to install MCP support, then retry.")
        ...
        new_pids: set = set()
        # Subprocess stderr goes to ~/.cryozen-agent/logs/mcp-stderr.log so banners can't corrupt the TUI.
