...
``metadata.cryozen.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.cryozen.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.cryozen.blueprint`` is a mapping with a
    ...
    cryozen = meta.get("cryozen") if isinstance(meta, dict) else None
    blueprint = cryozen.get("blueprint") if isinstance(cryozen, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.cryozen.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.cryozen.blueprint`` block) ready for ``cryozen skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"cryozen": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
