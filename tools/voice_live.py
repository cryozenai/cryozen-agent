"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to Cryozen.
...
delegates every real request to Cryozen as its *client-delegation* backend. Cryozen stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — Cryozen' answer is what the voice speaks.
...
# backend (Cryozen) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are Cryozen, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- Cryozen agent: a full AI agent with tools — it can run commands, read and edit files, "
