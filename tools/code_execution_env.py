...
# prefix, the exact-name CRYOZEN_ allowlist, or (Windows) an OS-essential name.
# The broad "CRYOZEN_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (CRYOZEN_BASE_URL, CRYOZEN_KANBAN_DB, *_WEBHOOK).
# CRYOZEN_RPC_SOCKET / CRYOZEN_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. CRYOZEN_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Cryozen code loses the Kanban mutation guard
# while still inheriting CRYOZEN_HOME.
_CRYOZEN_CHILD_ALLOWED = frozenset({
    "CRYOZEN_HOME", "CRYOZEN_PROFILE", "CRYOZEN_CONFIG", "CRYOZEN_ENV", "CRYOZEN_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational CRYOZEN_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret CRYOZEN_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _CRYOZEN_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("CRYOZEN_"):
            _dropped_cryozen.append(k)
    ...
    if _dropped_cryozen:
        logger.debug(
            "execute_code: dropped %d non-allowlisted CRYOZEN_* var(s) from the "
            ...
            len(_dropped_cryozen), ", ".join(sorted(_dropped_cryozen)),
        )
    ...
    # non-allowlisted CRYOZEN_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "CRYOZEN_KANBAN_DB", "CRYOZEN_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from cryozen_constants import apply_scratch_tmp_env, apply_subprocess_home_env, get_cryozen_home_override
    child_env = _scrub_child_env(os.environ)
    child_env["CRYOZEN_RPC_SOCKET"] = rpc_endpoint
    child_env["CRYOZEN_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; CRYOZEN_TIMEZONE is an internal setting (and under the multiplexed
    # gateway holds only the default profile's value — cryozen_time resolves the routed profile's).
    from cryozen_time import get_timezone_name
    ...
    child_env.pop("CRYOZEN_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # Multiplexed gateway/Desktop (#110303): the server process env carries the machine-default
    # CRYOZEN_HOME, but this turn runs under a per-profile override (ContextVar bound per turn).
    ...
    _home_override = get_cryozen_home_override()
    if _home_override:
        child_env["CRYOZEN_HOME"] = _home_override
        apply_scratch_tmp_env(child_env)  # TMPDIR follows the routed home, like HOME does
    # PYTHONPATH: the staging dir (cryozen_tools.py) must always be importable even when project
    # mode changes CWD. Cryozen's root is added ONLY when the child runs in Cryozen's Python env —
    # exposing Cryozen's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Cryozen-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Cryozen-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Cryozen-
    ...
    _strip_cryozen_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_cryozen_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import cryozen_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Cryozen "
                    "environment; cryozen root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the Cryozen environment.
...
def _uses_cryozen_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Cryozen's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the cryozen root."""
