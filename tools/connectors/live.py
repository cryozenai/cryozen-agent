...
session's profile scope) resolves it through ``cryozen_home_key``."""
...
def _profile_key(profile_home: Optional[str]) -> str:
    ...
    return cryozen_home_key(profile_home or get_process_cryozen_home())
...
def open(operation: ConnectionOperation) -> None:  # noqa: A001 - the verb is the API
    operation.profile_key = cryozen_home_key()
