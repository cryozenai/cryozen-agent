...
def target_scope(profile: str):
    ...
    scope = (launch_profile_runtime_scope(server._cryozen_home) if home is None
...
class HostInstaller:
    ...
    def refuse(self, entry: Any) -> None:
        ...
        from cryozen_cli.plugins_cmd_catalog import _refuse_unsupported_catalog_platform, raise_if_removed
    ...
    def skill_meta(self, identifier: str) -> Optional[Dict[str, Any]]:
        ...
        from cryozen_cli.skills_hub import _sources
...
class _Runner:
    ...
    def _resolve(self, target: Target) -> None:
        if target.kind == "plugin":
            ...
            if entry is None:
                raise LookupError(f"'{target.name}' is not in the Cryozen plugin catalog")
...
def _plugin_row(entry: Any) -> Dict[str, Any]:
    requirements = [f"Cryozen {entry.requires_cryozen}"] if entry.requires_cryozen else []
...
def _save_credentials(env: Dict[str, str]) -> None:
    from cryozen_cli.config import save_env_value, validate_env_var_name_for_write
