...
def probe_with_rollback(
        server_name: str, cfg: dict, cryozen_home: str, flow, reconnect_live: bool, *,
    ...
    from cryozen_cli.mcp_config import _oauth_tokens_present, _probe_single_server
    from tools.mcp_dashboard_oauth import exception_message
    from tools.mcp_oauth import CryozenTokenStorage, login_connect_timeout
    from tools.mcp_oauth_manager import get_manager
    manager = get_manager()
    storage = CryozenTokenStorage(server_name)
    ...
    def undo() -> None:
        ...
        if flow is not None and _ACTIVE.get((cryozen_home, server_name)) not in (None, flow):
            return
        storage.restore(backup)
        manager.restore_entry(server_name, previous_entry, cryozen_home=cryozen_home)

    try:
        previous_entry = manager.remove(server_name, cryozen_home=cryozen_home)
...
# (cryozen home, server) -> the newest card attempt. A retry or a new operation replaces an attempt
...
def run_worker(
        cryozen_home: str, server_name: str, cfg: dict, reconnect_live: bool, *,
    ...
    try:
        ...
        home_token = set_cryozen_home_override(cryozen_home)
        secret_token = set_secret_scope({**build_profile_secret_scope(Path(cryozen_home)), **(env or {})})
        try:
            if not (reuse_saved and flow is not None
                ...
                with force_interactive_oauth(), dashboard_oauth_flow(flow):
                    probe_with_rollback(
                        server_name, cfg, cryozen_home, flow, reconnect_live, on_commit=on_commit)
        finally:
        ...
        if flow is not None:
            ...
            with _COMMIT_GUARD:
                if _ACTIVE.get((cryozen_home, server_name)) is flow:
                    _ACTIVE.pop((cryozen_home, server_name), None)
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802
            ...
            if parsed.path.rstrip("/") not in ("/callback", ""):
                ...
                self.end_headers()
                return
            body = b"<h1>Authorization received</h1><p>You can close this tab and return to Cryozen.</p>"
            status = 200
            try:
...
def start(
    ...
    cfg["auth"] = "oauth"
    cryozen_home = str(get_cryozen_home().expanduser().resolve(strict=False))
    flow = DashboardOAuthFlow(
        flow_id=secrets.token_urlsafe(24), server_name=server_name, profile=None,
        cryozen_home=cryozen_home, redirect_uri="", reconnect_live=False)
    with _COMMIT_GUARD:
        older = _ACTIVE.get((cryozen_home, server_name))
        _ACTIVE[(cryozen_home, server_name)] = flow
    ...
    threading.Thread(
        target=run_worker, args=(cryozen_home, server_name, cfg, False),
