...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("CRYOZEN_YOLO_MODE", ""))
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from Cryozen YOLO. Lazy import so approval-only
...
def _permanent_set() -> set:
    ...
    profile (CRYOZEN_HOME override) → its own set, lazily loaded from ITS ``command_allowlist``: the
...
def _unattended_contexts() -> list[_Unattended]:
    """Active unattended contexts in evaluation order: single-query first (``cryozen chat -q``
    exports CRYOZEN_INTERACTIVE=1 but nobody answers); cron beats a platform marker because
...
def _presence(approval_callback=None) -> tuple:
    ...
    Single-query ``-q`` and cron clear the presence trio: ``cryozen chat -q`` exports
    CRYOZEN_INTERACTIVE=1 for sudo prompts, and a gateway sets CRYOZEN_EXEC_ASK=1 at startup and
    ...
    is_ask = env_var_enabled("CRYOZEN_EXEC_ASK")
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               "BLOCKED (fail-closed). Set CRYOZEN_INTERACTIVE or "
                               "CRYOZEN_GATEWAY_SESSION to answer the prompt.", *log_args)
                return _blocked(no_human_block_message or (
                    ...
                    pattern_key=pattern_key, description=description)
        logger.warning("%s (pattern: %s): %s — set CRYOZEN_INTERACTIVE or "
                       "CRYOZEN_GATEWAY_SESSION to require approval.", *log_args)
        return _approved()
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'reset_cryozen_interactive_context': ('tools.approval_context', 'reset_cryozen_interactive_context'),
    ...
    'set_cryozen_interactive_context': ('tools.approval_context', 'set_cryozen_interactive_context'),
}
