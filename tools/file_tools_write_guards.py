...
# resolve per call in production (below) because ``get_cryozen_home()`` /
...
_cryozen_config_resolved: str | None = None
_cryozen_config_resolved_loaded = False
_real_cryozen_home_cached: str | None = None
_real_cryozen_home_loaded = False
...
def _cryozen_home_real() -> str:
    from cryozen_constants import get_cryozen_home
    return os.path.realpath(str(get_cryozen_home()))


def _get_cryozen_config_resolved() -> str | None:
    """Resolved absolute path of the Cryozen config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``CRYOZEN_HOME`` scope (#107327);
    a test may pin it via ``_cryozen_config_resolved`` + ``_cryozen_config_resolved_loaded``."""
    if _cryozen_config_resolved_loaded:
        return _cryozen_config_resolved
    try:
        ...
        # home rather than the active multiplex ``CRYOZEN_HOME`` — comparing
        ...
        # ``get_cryozen_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from cryozen_constants import get_cryozen_home
            return str((Path(str(get_cryozen_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_cryozen_home() -> str | None:
    """Realpath of the authoritative Cryozen home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``CRYOZEN_HOME`` scope (#107327);
    a test may pin it via ``_real_cryozen_home_cached`` + ``_real_cryozen_home_loaded``.
    Consumers exempting a whole TREE want ``_cryozen_exempt_homes()``: under a named
    profile this home is ``<root>/profiles/<name>`` and the root is exempt too."""
    if _real_cryozen_home_loaded:
        return _real_cryozen_home_cached
    try:
        return _cryozen_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_cryozen_config_resolved``): re-derive from ``get_cryozen_home()``
        # rather than ``_expand_tilde("~/.cryozen-agent")`` so the protected-instruction
        ...
        # the consumer: the ``~/.cryozen-agent`` exemption is skipped, so the gate runs.
        try:
            from cryozen_constants import get_cryozen_home
            return os.path.realpath(str(get_cryozen_home()))
        except Exception:
...
def _cryozen_exempt_homes() -> tuple[str, ...]:
    """Realpaths of the Cryozen home tree(s) the protected-instruction gate must stay out of:
    the ACTIVE profile's home, plus the Cryozen ROOT when that home is a named profile
    (``<root>/profiles/<name>``). Exempting only the profile dir left the root's DIRECT files
    (LEDGER.md / MEMORY.md / SOUL.md / AGENTS.md ...) to the ``.cryozen-agent`` component rule, which
    gated them like a project-local ``<repo>/.cryozen-agent/config.yaml`` — fail-closed headless
    (#110630). They are the agent's own store, governed by their own guards, exactly like
    ``~/.cryozen-agent`` under the default profile. The root is added only when the shape really is a
    named profile (``named_profile_home``), so a coincidental ``profiles/`` dir elsewhere never
    exempts its parent; the home comes from the ACTIVE scope, never ``CRYOZEN_HOME`` alone."""
    home = _get_real_cryozen_home()
    if not home:
        return ()
    try:
        from cryozen_constants import named_profile_home
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    cryozen_config = _get_cryozen_config_resolved()
    if cryozen_config and cryozen_config in candidates:
        return (
            f"Refusing to write to Cryozen config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.cryozen-agent/config.yaml directly or use 'cryozen config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .cryozen-agent tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from cryozen_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.cryozen-agent itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.cryozen-agent`` component rule, which would match the home.
    # ``_cryozen_exempt_homes`` also covers the ROOT when the active home is a named
    # profile, so ~/.cryozen-agent/<file> cannot read as project-local ``.cryozen-agent`` config.
    for real_home in _cryozen_exempt_homes():
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .cryozen-agent config dirs (<repo>/.cryozen-agent/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.cryozen-agent.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".cryozen-agent":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Cryozen mirror prefix for persistent Docker file tools."""
    try:
