...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_cryozen_home() / "auth.json"
...
def cryozen_xai_user_agent() -> str:
    """Return a stable Cryozen-specific User-Agent for xAI HTTP calls."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Cryozen home, then mark it seen."""
    notice = xai_storage_notice_text(section_name)
    if not notice:
        return None
    try:
        from cryozen_constants import get_cryozen_home
        marker_dir = get_cryozen_home() / "state"
...
def _xai_base_url_override() -> str:
    """``CRYOZEN_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    from cryozen_cli.config import get_env_value
    return str(get_env_value("CRYOZEN_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: Cryozen-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.cryozen-agent/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``CRYOZEN_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers Cryozen-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``cryozen_cli.config.get_env_value`` so keys stored in ``~/.cryozen-agent/.env`` (the standard Cryozen
    ...
    scoping is identical to the fallback branch, and the base URL honors ``CRYOZEN_XAI_BASE_URL`` /
