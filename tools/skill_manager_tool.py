...
broad, declarative). New skills land in ~/.cryozen-agent/skills/ (or ``skills.create_dir``);
...
from tools.skill_manager_guards import (
    ...
    _containing_skills_root, _curator_consolidation_delete_guard, _pinned_guard, _validate_delete_target,
...
# All skills live in ~/.cryozen-agent/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    An explicitly patched module-level ``SKILLS_DIR`` (tests) wins over the live CRYOZEN_HOME.

    Long-lived multi-profile runtimes (Dashboard/TUI/Desktop backend, cron, kanban workers) import this
    module once under the launch CRYOZEN_HOME and later bind a different profile per session (#40677).
    """
    configured = Path(SKILLS_DIR)
    return configured if configured != _SKILLS_DIR_AT_IMPORT else get_cryozen_home() / "skills"
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    if len(others) == 1:
        ...
        base += (
            ...
            f"it, switch profiles (`cryozen -p {other_profile}`) or edit the file directly "
            f"(file tools / terminal).")
    elif others:
        names = ", ".join(f"'{p}'" for p, _ in others)
        base += (
            f" Skills by that name exist in other profiles: {names}. Switch profiles (`cryozen -p "
...
def _locate_for_write(name: str, action: str, not_found_suffix: str = ""):
    """Find the skill and run the background-review write guard -> ``(skill_dir, None)`` |
    ``(None, error_dict)``."""
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path, before: Optional[str] = None) -> None:
    ...
    result["lint_hint"] = (
        ...
        "— fix them with skill_manage(action='patch') to match Cryozen skill standards.")
...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    from cryozen_constants import mkdir_under_cryozen_home
    mkdir_under_cryozen_home(skill_dir)
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    if unsafe := _validate_delete_target(skill_dir):  # defense-in-depth before rmtree
        return _err(unsafe)
    # Curator consolidations must be RECOVERABLE (`cryozen curator restore`): archive instead
...
def _record_success(action, name, result, *, file_path, absorbed_into, task_id,
    ...
    # record as STATE_ARCHIVED (`cryozen curator status`/`restore`); only a hard delete forgets.
    with suppress(Exception):
        ...
        # ~/.cryozen-agent/skills/.archive/ is documented as the maximum destructive action the curator may take,
        # and `cryozen curator restore` promises the skill can be brought back. Route through the recoverable
