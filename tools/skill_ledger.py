...
``~/.cryozen-agent/skills/.curator_ledger.jsonl`` with before/after file manifests whose
contents are stored content-addressed (sha256-deduped) under
``~/.cryozen-agent/.curator_backups/blobs/``. JSONL, not the state DB: durable, greppable,
...
def blobs_dir() -> Path:
    return get_cryozen_home() / ".curator_backups" / "blobs"
...
def _skills_cfg(key: str, default):
    ...
    try:
        from cryozen_cli.config import cfg_get, load_config_readonly  # read-only hot path: no deepcopy
...
def fill_snapshot_from_curator_backup(
    ...
    for rel, data in extra.items():
        ...
        if not _is_within(skills, dest) or not _is_within(get_cryozen_home(), dest):
            continue
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """Every entry path must be under CRYOZEN_HOME — a hand-edited ledger must not
