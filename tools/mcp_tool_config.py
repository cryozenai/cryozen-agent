...
def _get_mcp_stderr_log() -> Any:
    ...
    from cryozen_constants import get_cryozen_home, cryozen_home_key, mkdir_under_cryozen_home
    home_key = cryozen_home_key()
    with _mcp_stderr_log_lock:
        fh = _mcp_stderr_log_fh.get(home_key)
        if fh is None or fh.closed:
            try:
                log_dir = get_cryozen_home() / "logs"
                mkdir_under_cryozen_home(log_dir)
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    from cryozen_cli.env_loader import secret_source_names
    env = {
    ...
    for key in secret_source_names():
        ...
        if value is not None:
            env[key] = value
    for key in ("CRYOZEN_KANBAN_DB", "CRYOZEN_KANBAN_BOARD"):
...
def _node_fallback(command: str, *, windows: Optional[bool] = None) -> str:
    ...
    The managed tree comes from ``iter_cryozen_node_dirs`` (Windows unpacks into ``<home>\\node``, POSIX into
    ``<home>/node/bin``) under the active profile's ``get_cryozen_home()``; on Windows the real files are
    ...
    # /usr/local/bin: canonical Node location (from-source Linux, Cryozen Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    directories = [*map(str, iter_cryozen_node_dirs(get_cryozen_home())), os.path.join(home, ".local", "bin"),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``cryozen_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); Cryozen already supervises the
