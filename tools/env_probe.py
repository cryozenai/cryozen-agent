...
_REMOTE_BACKENDS = frozenset({
    "docker", "singularity", "modal", "daytona", "ssh", "vercel_sandbox",
})
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here (unlike Cryozen's own uv call sites): this reports
    # the environment *the model will see* in the terminal tool, whose PATH includes
    # the Cryozen-managed $CRYOZEN_HOME/bin via local.py.
