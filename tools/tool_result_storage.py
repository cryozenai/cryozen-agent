...
is ALWAYS host-side ``$CRYOZEN_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
STORAGE_DIR = os.path.join(tempfile.gettempdir(), "cryozen-results")
...
def get_spillover_dir():
    """Return $CRYOZEN_HOME/cache/spillover as a Path (not created)."""
    from cryozen_constants import get_cryozen_home
    return get_cryozen_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $CRYOZEN_HOME/cache/spillover; returns path str or None.
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/cryozen-results" if temp_dir else STORAGE_DIR
...
def _write_to_sandbox(content: str, remote_path: str, env) -> bool:
    ...
    if persisted_size == expected:
        return True
    # Only heredoc mode may be +1; a pipe-mode backend delivers stdin verbatim, so it is expected
    # byte-exact and any drift there is a real loss.
...
HEREDOC_MARKER = "CRYOZEN_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
