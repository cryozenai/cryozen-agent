...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Cryozen logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Cryozen' logging
        (agent.log via cryozen_logging), tagged with the server name. Without this, the SDK's default
