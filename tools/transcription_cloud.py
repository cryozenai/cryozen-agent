...
local server > env). Facade-owned state and helpers
...
def _transcribe_openai(
    ...
    def _run(client):
        ...
        with tempfile.TemporaryDirectory(prefix="cryozen-stt-") as work_dir:
            try:
...
def _transcribe_xai(
    ...
    if not api_key:
        return _error_result("No xAI credentials found. Configure xAI OAuth in `cryozen model` or set XAI_API_KEY")
    ...
    def _post() -> Any:
        from tools.xai_http import cryozen_xai_user_agent
        ...
        def _post_transcription(bearer: str, endpoint_base_url: str):
            headers = {"Authorization": f"Bearer {bearer}", "User-Agent": cryozen_xai_user_agent()}
...
def _transcribe_deepinfra(
    ...
    """Resolve DeepInfra credentials/model (shared ``cryozen_cli.models`` helpers), then delegate to :func:`_transcribe_openai`."""
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    """``(api_key, base_url)`` for the OpenAI STT client from direct credentials. A stored ``stt``
    selection with no key gets the selection-naming error. Failures raise ValueError."""
    from tools.transcription_tools import _load_stt_config
    ...
    if direct is not None:
        return direct
    missing = "neither stt.openai.api_key in config nor VOICE_TOOLS_OPENAI_KEY/OPENAI_API_KEY is set"
    selected = read_selection("stt")
    if selected is not None:
        raise ValueError(selection_error("stt", selected, missing))
