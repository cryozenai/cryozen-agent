...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Cryozen tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``cryozen gateway restart|stop|uninstall``
    targeting cryozen-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_CRYOZEN_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (cryozen
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``cryozen gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of Cryozen. Perform authorized "
    ...
    if unsafe:
        ...
        return _blocked_json(
            ...
            "to child processes). Run `cryozen gateway restart` from a "
