"""Shared ``OAuthClientProvider`` customizations for Cryozen MCP OAuth.
...
if TYPE_CHECKING:
    from tools.mcp_oauth import CryozenTokenStorage
...
def _default_auth_request_user_agent() -> str:
    """``Cryozen-Agent/<version>`` for SDK-built OAuth requests that would otherwise carry no User-Agent at
    ...
    return f"Cryozen-Agent/{__version__}"
...
class CryozenProviderMixin:
    """Token-endpoint fixes layered over the SDK's ``OAuthClientProvider`` (must precede it in
    the MRO; subclasses set ``_cryozen_logger`` to keep their own logger name).
    ...
    def _request_google_offline_access(self) -> None:
        ...
        if inner is None or getattr(inner, "_cryozen_offline_access", False):
            return
        ...
        _with_offline_access._cryozen_offline_access = True  # type: ignore[attr-defined]
        self.context.redirect_handler = _with_offline_access

    async def _cryozen_accept_origin_issued_metadata(self, response):
        ...
        runs), while the RFC 9207 ``iss`` check and Cryozen' refresh-token binding use the document's issuer.
    ...
    def _prepare_token_request(self, request):
        ...
        else the shared ``Cryozen-Agent/<version>`` default. These requests are built by hand — the
        ...
        ua = getattr(self, "_cryozen_token_user_agent", None)  # tests build via __new__
        if ua:
            request.headers["User-Agent"] = ua
        return stamp_default_user_agent(request)

    def _coerce_client_secret_post(self) -> None:
        """Same rule as ``CryozenTokenStorage._coerce_secret_auth_method``, applied to the
        ...
        if CryozenTokenStorage._coerce_secret_auth_method(data):
    ...
    # _cryozen_release_refresh_fence. Never shared across instances.
    _cryozen_fence: int | None = None
                ...
                while True:
                    ...
                    try:
                        ...
                        if failure:
                            ...
                            sent = await self._cryozen_accept_origin_issued_metadata(sent)
            finally:
                await self._cryozen_release_refresh_fence()
    ...
    async def _refresh_token(self):
        ...
        """
        self._coerce_client_secret_post()
        await self._cryozen_acquire_refresh_fence()
        try:
            ...
            candidate = await self._cryozen_rotated_candidate()
            if candidate is not None:
                if not self._cryozen_install_disk_pair(candidate):
                ...
                if self._cryozen_live_ttl() and self.context.is_token_valid():
    ...
    async def _cryozen_acquire_refresh_fence(self) -> None:
        ...
        if tokens_path is None:  # pragma: no cover - non-Cryozen storage
            return
        self._cryozen_fence = await acquire_refresh_fence(tokens_path())

    async def _cryozen_release_refresh_fence(self) -> None:
        """Release the fence if held. Idempotent and never raises."""
        from tools.mcp_oauth import release_refresh_fence

        fd, self._cryozen_fence = self._cryozen_fence, None
        if fd is not None:
            release_refresh_fence(fd)

    async def _cryozen_rotated_candidate(self):
    ...
    def _cryozen_install_disk_pair(self, tokens) -> bool:
    ...
    async def _initialize(self) -> None:
        ...
        storage = self.context.storage
        from tools.mcp_oauth import CryozenTokenStorage
        if isinstance(storage, CryozenTokenStorage) and self.context.oauth_metadata is None:
    ...
    async def _handle_refresh_response(self, response) -> bool:
        ...
        """
        try:
            return await self._cryozen_handle_refresh_response(response)
        finally:
            await self._cryozen_release_refresh_fence()

    async def _cryozen_handle_refresh_response(self, response) -> bool:
        if not (200 <= response.status_code < 300):
            self._cryozen_logger.warning("Token refresh failed: %s", response.status_code)
            # A writer outside the fence (interactive `cryozen mcp login`, or a
            # pre-fence Cryozen sharing this CRYOZEN_HOME) may have rotated the
            ...
            if await self._cryozen_reload_tokens_after_refresh_failure():
                self._cryozen_logger.info(
        ...
        try:
            ...
            self._cryozen_logger.warning("Invalid refresh response: %s", response.status_code)
            self.context.clear_tokens()
            return False
    ...
    async def _cryozen_reload_tokens_after_refresh_failure(self) -> bool:
        ...
        a writer outside the fence (an interactive ``cryozen mcp login`` or a
        pre-fence Cryozen) having rotated the grant between our read and our
        ...
        if (
            self._cryozen_install_disk_pair(candidate)
            and self._cryozen_live_ttl()
...
def bind_issuer_from_context(context: Any) -> None:
    ...
    it. No-op when metadata is not discovered yet or storage is not Cryozen'."""
    from tools.mcp_oauth import CryozenTokenStorage
    storage = getattr(context, "storage", None)
    issuer = _metadata_issuer(context)
    if isinstance(storage, CryozenTokenStorage) and issuer:
...
def enforce_refresh_token_issuer(context: Any) -> None:
    ...
    if not isinstance(storage, CryozenTokenStorage) or tokens is None or not getattr(tokens, "refresh_token", None):
        return
...
def prepare_oauth_config(server_name: str, server_url: str, oauth_config: dict | None) -> tuple[dict, "CryozenTokenStorage"]:
    ...
    return cfg, mo.CryozenTokenStorage(server_name)


def build_provider_kwargs(cfg: dict, storage: "CryozenTokenStorage", *, ssh_proxy_hint: bool) -> dict[str, Any]:
