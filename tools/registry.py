"""Central registry for all cryozen-agent tools: each tool file calls ``registry.register()``
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_cryozen_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales, so results are cached ~30 s: env-var flips via ``cryozen tools`` still land
...
_BROWSER_IDENTITY_KEYS = (
    "CRYOZEN_SESSION_ID",
    "CRYOZEN_BROWSER_CONTROL_PRINCIPAL",
    "CRYOZEN_BROWSER_CONTROL_TRANSPORT_FAMILY")
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    processes keep the process-wide cache; a multiplex gateway installs a Cryozen-home override
...
def _core_tools_gated_by(fn: Callable) -> Set[str]:
    """Names of ``_CRYOZEN_CORE_TOOLS`` members whose registered ``check_fn`` is *fn*."""
    try:
        from toolsets import _CRYOZEN_CORE_TOOLS
        core = frozenset(_CRYOZEN_CORE_TOOLS)
    except Exception:
        return set()
...
def invalidate_check_fn_cache() -> None:
    """Drop all cached ``check_fn`` results (after config changes like ``cryozen tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin overlays keyed by resolved CRYOZEN_HOME; a profile sees its overlay first.
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> Optional[str]:
        ...
        if module_namespace.startswith("cryozen_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # Ownership binds to the plugin package root (``cryozen_plugins.{name}``), not
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        absent). Probes use the ~30 s TTL cache so ``cryozen tools enable`` lands quickly."""
        result = []
