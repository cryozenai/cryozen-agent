...
# dependency could read every Cryozen secret from process.env.
...
# than a concrete path (also compared in cryozen_cli/tools_config.py and doctor.py).
...
# The config-derived ones are keyed by profile home (``cryozen_home_key()``): the multiplexed
...
_REAL_PROFILE_SESSION = "cryozen-real-profile"
...
def _socket_safe_tmpdir() -> str:
    ...
    (``cryozen_constants.socket_safe_tmpdir``)."""
...
# Owner Cryozen home per session: the janitor is one process-global thread, so each
...
def _url_policy_error(url: str, *, auto_local: bool = False) -> Optional[dict]:
    ...
    URL protects nothing. Cryozen' own secrets leaking into a URL are caught by ``_secret_url_error``."""
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = cryozen_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_cryozen_dir("cache/screenshots", "browser_screenshots")
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_cryozen_home_override': ('cryozen_constants', 'get_cryozen_home_override'),
    'cryozen_home_key': ('cryozen_constants', 'cryozen_home_key'),
    ...
    'node_tool_runnable': ('cryozen_constants', 'node_tool_runnable'),
