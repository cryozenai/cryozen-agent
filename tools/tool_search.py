...
tool_search / tool_describe / tool_call. Invariants: core tools (``toolsets._CRYOZEN_CORE_TOOLS``)
...
# Bound the work one bridge call requests.
...
def _config_from_loader(loader_name: str) -> ToolSearchConfig:
    """Tool-search config via ``cryozen_cli.config.<loader_name>`` (defaults on any failure)."""
    try:
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_CRYOZEN_CORE_TOOLS)
    except Exception:
        return frozenset()


# Session-gated GUI toolsets: off ``_CRYOZEN_CORE_TOOLS`` so non-GUI clients never pay
...
def bridge_tool_schemas(deferred_count: int, listing: Optional[str] = None,
    ...
    return [
        ...
        _bridge_schema(
            ...
            "— one entry per invocation; exactly one entry per tool_call. "
            ...
            "Policy, hooks, and approvals run as for directly-listed tools.",
            {
                "calls": {
                    ...
                    "description": "Exactly one invocation.",
                },
            },
            ["calls"],
        ),
...
def _clip_description(text: str, cap: int = 500) -> str:
    ...
    500 keeps most tool descriptions whole along with their first sentence.
    """
...
def dispatch_tool_search(args: Dict[str, Any], *, current_tool_defs: List[Dict[str, Any]],
    ...
    payload: Dict[str, Any] = {"queries": queries, "total_available": len(catalog),
