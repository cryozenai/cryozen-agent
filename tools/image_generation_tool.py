#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``cryozen tools``, persisted to ``image_gen.model``).
...
def _check_fal_selection() -> None:
    """Raise the selection-naming error when `cryozen tools` stored a provider but FAL_KEY is
    missing; a never-configured category is left to the caller's "no backend" message."""
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    """Submit a FAL request with the direct FAL_KEY credentials."""
    _load_fal_client()
    _check_fal_selection()
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.cryozen-agent", "singularity": "/root/.cryozen-agent",
                          "modal": "/root/.cryozen-agent", "ssh": "~/.cryozen-agent"}
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.cryozen-agent"
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    # _check_fal_selection(); only never-configured reports "no backend at all".
    _check_fal_selection()
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `cryozen tools` → Image Generation.")
...
def check_fal_api_key() -> bool:
    """True when FAL_KEY is configured (the only FAL backend)."""
    return fal_key_is_configured()


def _build_no_backend_setup_message() -> str:
    """Actionable no-backend error: FAL_KEY signup, plugin alternative."""
    ...
    lines.append("  2. Configure a different image_gen provider via `cryozen tools` → Image Generation "
                 "(run `cryozen plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `cryozen plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `cryozen tools` → "
...
def _handle_image_generate(args, **kw):
    ...
    # Order matters: explicit plugin provider first, then the in-tree FAL pipeline.
    ...
    raw = None
    for route in (_dispatch_to_plugin_provider, image_generate_tool):
...
def _active_image_capabilities() -> Dict[str, Any]:
    ...
    Mirrors runtime dispatch: a set ``image_gen.provider`` asks that plugin, else the FAL catalog.
    Fail-closed: an undeclared capability is advertised as absent.
    """
    info: Dict[str, Any] = dict(_NO_CAPABILITIES)
    configured_provider = _read_configured_image_provider()
    plugin_name = configured_provider if configured_provider and configured_provider != "fal" else None
    if plugin_name:
        try:
