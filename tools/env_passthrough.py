...
# Config-based allowlist, keyed by Cryozen home: under gateway.multiplex_profiles one process serves
...
def _is_cryozen_provider_credential(name: str) -> bool:
    """True if ``name`` is a Cryozen-managed provider credential per
    ``_CRYOZEN_PROVIDER_ENV_BLOCKLIST`` or a dynamic Cryozen-internal secret
    ...
    in the ``execute_code`` child); non-Cryozen keys (TENOR_API_KEY, …) stay
    registerable. Fails closed when the blocklist cannot be imported."""
    try:
        from tools.environments.local_env_policy import (
            _is_cryozen_internal_secret, _is_provider_env_blocklisted)
    ...
    return _is_cryozen_internal_secret(name) or _is_provider_env_blocklisted(name)


def register_env_passthrough(var_names: Iterable[str]) -> None:
    """Register env var names as allowed in sandboxed environments (typically a
    skill's ``required_environment_variables``). Cryozen-managed provider credentials
    ...
    for name in _accepted((n.strip() for n in var_names), (
        "env passthrough: refusing to register Cryozen provider "
        "credential %r (blocked by _CRYOZEN_PROVIDER_ENV_BLOCKLIST). "
...
def _accepted(names, refusal_msg: str):
    """Yield non-empty *names* that are not Cryozen provider credentials; refused
    names are logged with *refusal_msg* (``%r`` = name)."""
    for name in names:
        if not name:
            continue
        if _is_cryozen_provider_credential(name):
...
def _load_config_passthrough() -> frozenset[str]:
    ...
    try:
        ...
        result.update(_accepted((i.strip() for i in items if isinstance(i, str)), (
            "env passthrough: refusing to register Cryozen "
            "provider credential %r from config.yaml (blocked "
            "by _CRYOZEN_PROVIDER_ENV_BLOCKLIST). Operator "
...
def scoped_passthrough_additions(present: Iterable[str]) -> dict[str, str]:
    ...
    ``os.environ`` (``load_cryozen_dotenv`` skips the process-global load for a routed home), so a
