...
_CRYOZEN_CONFIG_FILES = r'\.cryozen-agent/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.cryozen-agent/\.env|\~/\.cryozen-agent/\.env',
     "cryozen_env_access", "critical", "exfiltration", "directly references Cryozen secrets file"),
    ...
    # invocation: any client plugin that relays Cryozen' secure prompts has to name them, and a bare
    ...
    # quarantine only acts on "dangerous") but high for Cryozen/other config (setup docs routinely say
    ...
    (_prose_modify_re(_CRYOZEN_CONFIG_FILES),
     "cryozen_config_mod", "high", "persistence", "modification language aimed at Cryozen configuration files (verify intent)"),
    (_shell_write_re(_CRYOZEN_CONFIG_FILES),
     "cryozen_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Cryozen configuration files"),
    (r'\.cryozen-agent/config\.yaml|\.cryozen-agent/SOUL\.md',
     "cryozen_config_ref", "low", "persistence", "references Cryozen configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Cryozen-native; `.clawhubignore` is honored for skills published through ClawHub.
