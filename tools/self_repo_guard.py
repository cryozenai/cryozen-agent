...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(cryozen_home).expanduser() if cryozen_home else Path.home() / ".cryozen-agent") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Cryozen's live source checkout "
        ...
        "is pushed. To change this checkout, stop Cryozen, run the command externally, then restart "
        "Cryozen.")
