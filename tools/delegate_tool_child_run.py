...
def _dump_subagent_timeout_diagnostic(
    *, child: Any, task_index: int, timeout_seconds: float, duration_seconds: float,
    ...
    response", 0 API calls, nothing to inspect): ``~/.cryozen-agent/logs/subagent-timeout-<sid>-<ts>.log`` with the
    ...
    try:
        ...
        logs_dir = get_cryozen_home() / "logs"
        try:
...
def _register_child(
    ...
    if owner_session_id is None:
        with _quiet(None):
            ...
            owner_session_id = get_session_env("CRYOZEN_UI_SESSION_ID", "") or None
...
def _validate_child_output_schema(
    ...
    try:
        ...
        # unmarked turn is misread as the dispatcher-owned worker by every CRYOZEN_KANBAN_* gate.
