...
mistral, xai, elevenlabs, deepinfra; plus
...
def _resolve_stt_language(
    ...
    (plus *extra_keys* aliases, e.g. ``language_code``) > ``stt.language`` > ``CRYOZEN_LOCAL_STT_LANGUAGE``
...
def _openai_audio_unavailable_reason() -> Optional[str]:
    """None when OpenAI audio has usable credentials (config or env); else the reason."""
    try:
        # Resolve directly instead of via the boolean probe: the probe flattens
        # _resolve_openai_audio_client_config's selection-specific ValueError into False (#93045).
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Whether *provider* is exempt from Cryozen's remote upload cap."""
...
def _resolve_explicit_local() -> str:
    ...
    if not backend:
        logger.warning("STT provider 'local' configured but unavailable "
                       "(install faster-whisper or set CRYOZEN_LOCAL_STT_COMMAND)")
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(target=_watch, name="cryozen-stt-idle-unload", daemon=True)
        ...
        if not _is_local_stt_provider(provider, stt_config):
            ...
            if Path(file_path).suffix.lower() == ".caf":
                work_dir = cleanup.enter_context(
                    TemporaryDirectory(prefix="cryozen-caf-", ignore_cleanup_errors=True)
                )
...
def _no_provider_error(provider: str, stt_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Surface it — with its `cryozen tools` remediation — instead of the all-provider setup hint (#93045).
