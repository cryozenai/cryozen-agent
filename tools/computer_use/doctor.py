"""`cryozen computer-use doctor` — thin client for cua-driver's `health_report` MCP tool. cua-driver owns the health
...
def _build_identity(binary: str, report: Report) -> Report:
    """Cryozen-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(driver_cmd: Optional[str] = None, *, include: Sequence[str] = (), skip: Sequence[str] = (), json_output: bool = False,
    ...
    try:  # prefer real health_report; on denial/non-schema, synthesize via probes
        ...
        print(f"cua-driver could not be started from {binary!r}: {e}\n"
              "  The Cryozen runtime interpreter cannot execute this binary; the tool may still work because the\n"
              "  shell resolves a different copy on PATH. Fix: install cua-driver outside the protected directory\n"
              "  (e.g. the upstream installer's default under your user profile) or point CRYOZEN_CUA_DRIVER_CMD at\n"
              "  a copy the runtime can execute, then re-run `cryozen computer-use doctor`.", file=sys.stderr)
        return 2
    except RuntimeError as e:
    ...
    if json_output:
        # Additive envelope: upstream keys preserved, identity under cryozen_identity (and environment under
        # cryozen_environment when present) so overall/checks parsers keep working.
        payload = {**report, "cryozen_identity": identity}
        if environment:
            payload["cryozen_environment"] = environment
