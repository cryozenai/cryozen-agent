...
_FORCE_SYNC_ENV = "CRYOZEN_FORCE_FILE_SYNC"
...
_SYNC_BACK_TEMP_PREFIX = "cryozen-sync-back-"
...
def iter_sync_files(container_base: str = "/root/.cryozen-agent") -> list[tuple[str, str]]:
    """Enumerate all (host_path, remote_path) pairs to sync to a remote. Credential paths are
    remapped from the hardcoded /root/.cryozen-agent to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.cryozen-agent", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``CRYOZEN_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def _sync_transaction(self, *, force: bool = False) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="cryozen-sync-push-") as staging:
                staged_files = []
                pushed_hashes = {}
    ...
    def sync_back(self, cryozen_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.cryozen-agent/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(cryozen_home=cryozen_home)

    def _sync_back_transaction(self, cryozen_home: Path | None = None) -> None:
        ...
        # to avoid retry storms against an uninitialized remote .cryozen-agent/.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (cryozen_home or get_cryozen_home()) / ".sync.lock"
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.cryozen-agent/skills/b.md`` -> ``~/.cryozen-agent/skills/b.md``)."""
