...
from tools.environments.local_env_policy import _is_cryozen_internal_secret, _is_provider_env_blocklisted


def load_cryozen_env_vars() -> dict[str, str]:
    """``~/.cryozen-agent/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            cryozen_env_loader: Callable[[], dict[str, str]] = load_cryozen_env_vars,
    ...
    is filtered through the Cryozen provider-credential blocklist and the dynamic internal-secret
    ...
    try:
        ...
        pass
    implicit_forward = {k for k in passthrough_keys if not _is_cryozen_internal_secret(k)}
    forward_keys = set(explicit_forward) | {
        k for k in implicit_forward if not _is_provider_env_blocklisted(k)}
    cryozen_env = cryozen_env_loader() if forward_keys else {}
    exec_env: dict[str, str] = {}
    unset_names: set[str] = set()
    for key in sorted(forward_keys):
        value = os.getenv(key) or cryozen_env.get(key)
