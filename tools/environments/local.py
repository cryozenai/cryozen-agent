...
from tools.environments.local_env_policy import (  # noqa: F401 — _CRYOZEN_PROVIDER_ENV_BLOCKLIST stays importable from here
    _ALWAYS_STRIP_KEYS, _CRYOZEN_PROVIDER_ENV_BLOCKLIST, _CRYOZEN_PROVIDER_ENV_FORCE_PREFIX,
    _is_cryozen_internal_secret, _is_provider_env_blocklisted, _is_terminal_first_party_env,
...
from tools.environments.local_pythonpath import (
    _build_cryozen_repo_root_aliases, _strip_cryozen_owned_pythonpath_and_runtime_markers)
...
# get_temp_dir() defaults to CRYOZEN_HOME/cache/terminal (real storage, not tmpfs), so
...
# Background artifacts come in triplets (cryozen_bg_<id>.log/.pid/.exit). A live
...
_BG_GROUP_RE = re.compile(r"^(cryozen_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")


def _default_terminal_temp_dir() -> "Path | None":
    """Return CRYOZEN_HOME/cache/terminal, or None if unresolvable."""
    try:
        from cryozen_constants import get_cryozen_home
        return get_cryozen_home() / "cache" / "terminal"
    except Exception:
        return None
...
def cleanup_terminal_temp_cache(max_age_hours: float = TERMINAL_TEMP_MAX_IDLE_HOURS) -> int:
    ...
    from cryozen_constants_scratch import subtree_touched_since
...
def _apply_profile_home(env: dict) -> None:
    """Bridge the context-local CRYOZEN_HOME override, then the subprocess HOME contract."""
    from cryozen_constants import apply_subprocess_home_env, get_cryozen_home_override
    try:
        if value := get_cryozen_home_override():
            env["CRYOZEN_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    """Bridge gateway session ContextVars (CRYOZEN_SESSION_*) into a child env.
...
def _filter_secret_env(
    ...
    """Copy *items* into *out*, dropping Cryozen-managed secrets. ``_CRYOZEN_FORCE_<NAME>``
    ...
    for key, value in items.items():
        if key.startswith(_CRYOZEN_PROVIDER_ENV_FORCE_PREFIX):
            if not unwrap_force:
                continue
            key = key[len(_CRYOZEN_PROVIDER_ENV_FORCE_PREFIX):]
            if not _is_cryozen_internal_secret(key):
                out[key] = value
            continue
        if _is_cryozen_internal_secret(key) or key.upper() in plugin_strip_folded:
            continue
...
def _finalize_child_env(env: dict) -> dict:
    ...
    bridging, Cryozen-owned PYTHONPATH + venv-marker strip, MSYS defaults, delegate_task
    Kanban scrub. Returns the (possibly new) dict."""
    _apply_profile_home(env)
    _inject_session_context_env(env)
    _strip_cryozen_owned_pythonpath_and_runtime_markers(env)
...
def _scrubbed_env(parts, plugin_strip: frozenset, fix_path) -> dict:
    ...
    *fix_path* (always prepending the cryozen install dir so bare ``cryozen`` resolves
    ...
    # Keep bare ``cryozen`` invocations available to child jobs even when the gateway was launched by a
    ...
    if path_key is not None:
        out[path_key] = _prepend_cryozen_bin_dir(fix_path(out.get(path_key, "")))
    return _finalize_child_env(out)


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Cryozen-managed secrets from a subprocess environment (background/PTY
...
def cryozen_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
...
def _scrub_credentials(env: dict, *, inherit_credentials: bool) -> dict:
    ...
    for key in list(env):
        if (key.upper() in strip_folded
                ...
                or key.startswith(_CRYOZEN_PROVIDER_ENV_FORCE_PREFIX)
                or _is_cryozen_internal_secret(key)):
            del env[key]
    return env
...
def build_subprocess_env(
    ...
    bridges CRYOZEN_HOME + HOME and ``extra`` is applied last so caller overrides win.
...
def served_profile_child_env(
    ...
    """Child env for a process that acts FOR the active (possibly served) profile: ``cryozen -p X``
    ...
    (its ``.env`` + hydrated sources, what a standalone ``cryozen -p X`` loads itself) are overlaid — never
    ...
    ``cryozen_subprocess_env`` snapshot."""
    from agent.secret_scope import (
        UnscopedSecretError, build_profile_secret_scope, current_secret_scope, is_multiplex_active)
    from cryozen_constants import apply_scratch_tmp_env, get_cryozen_home_override
    env = dict(base) if base is not None else cryozen_subprocess_env(inherit_credentials=inherit_credentials)
    target = str(target_home or get_cryozen_home_override() or "")
    if target:
        env["CRYOZEN_HOME"] = target
...
def _is_routed_home(target_home: "str | Path") -> bool:
    ...
    from cryozen_constants import get_process_cryozen_home
    try:
        return Path(target_home).resolve() != get_process_cryozen_home().resolve()
    except OSError:
        return True
...
def strip_launch_profile_env(env: dict, target_home: "str | Path | None" = None) -> dict:
    ...
    the secret scrub removes credentials but not settings (``CRYOZEN_MODEL``, ``TERMINAL_ENV``,
    ``CRYOZEN_LANGUAGE``...), so a standalone ``cryozen -p X`` worker and a served one saw different
    ...
    installing a CRYOZEN_HOME override without that flag."""
    from agent.secret_scope import _is_global_env, load_env_file
    from cryozen_constants import get_cryozen_home_override, get_process_cryozen_home
    target = target_home or get_cryozen_home_override()
    if not target or not _is_routed_home(target):
        return env
    launch_home = get_process_cryozen_home()
    from cryozen_cli.config import TERMINAL_CONFIG_ENV_MAP
...
def _windows_bash_candidates(custom: "str | None") -> list[str]:
    """Ordered bash.exe candidates on Windows: CRYOZEN_GIT_BASH_PATH, our portable Git
    under %LOCALAPPDATA%\\cryozen\\git (PortableGit ``bin`` and MinGit ``usr\\bin``),
    ...
    roots = [
        lad and os.path.join(lad, "cryozen", "git", "bin"),
        lad and os.path.join(lad, "cryozen", "git", "usr", "bin"),
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("CRYOZEN_GIT_BASH_PATH")
    candidates = _windows_bash_candidates(custom)
    # First candidate that can actually start wins: a stale CRYOZEN_GIT_BASH_PATH
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "CRYOZEN_GIT_BASH_PATH=%s fails to start; using %s instead", custom, candidate)
            return candidate
    if candidates:
        probe_details = "\n".join(
        ...
        return candidates[0]
    raise RuntimeError(
        "Git Bash not found. Cryozen Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set CRYOZEN_GIT_BASH_PATH to your bash.exe location.")
...
# Cached directory containing the ``cryozen`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_CRYOZEN_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_cryozen_bin_dir() -> str | None:
    """Directory holding the ``cryozen`` console-script, or None (cached). A gateway
    launched by systemd/cron/a desktop launcher lacks the install dir on PATH and bare
    ``cryozen`` exits 127. Order: ``which``; absolute ``sys.argv[0]`` naming a real
    cryozen executable; ``sys.executable``'s dir if it holds the shim."""
    global _CRYOZEN_BIN_DIR
    if _CRYOZEN_BIN_DIR is not _SENTINEL:
        return _CRYOZEN_BIN_DIR  # type: ignore[return-value]
    which = shutil.which("cryozen")
    argv0 = sys.argv[0] if sys.argv else ""
    base = os.path.basename(argv0).lower()
    exe_dir = os.path.dirname(sys.executable) if sys.executable else ""
    shim = "cryozen.exe" if _IS_WINDOWS else "cryozen"
    if which:
        candidate = os.path.dirname(which)
    elif (os.path.isabs(argv0) and (base == "cryozen" or base.startswith("cryozen."))
    ...
    _CRYOZEN_BIN_DIR = candidate if candidate and os.path.isdir(candidate) else None
    return _CRYOZEN_BIN_DIR


def _prepend_cryozen_bin_dir(existing_path: str) -> str:
    """Prepend the cryozen install dir to ``existing_path`` if missing."""
    bin_dir = _resolve_cryozen_bin_dir()
    return _prepend_missing_path_entries(existing_path, [bin_dir] if bin_dir else [])


def _managed_runtime_path_entries() -> list[str]:
    """Existing Cryozen-managed runtime dirs: ``$CRYOZEN_HOME/node`` (+``/bin``) and
    ``$CRYOZEN_HOME/bin`` (managed ``uv``). Per call, not cached: home is
    profile-scoped and a managed tree can appear mid-process."""
    try:
        from cryozen_constants import get_cryozen_home, iter_cryozen_node_dirs
        return [str(d) for d in (*iter_cryozen_node_dirs(), get_cryozen_home() / "bin") if d.is_dir()]
    except Exception:
        return []
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    and ``wmic``. Cryozen runs terminal commands through bash on Windows, so set the standard MSYS opt-out by
    default. Refs #56700.
...
# --- Cryozen venv / repo-root detection (module-level, computed once) ---
...
# spellings — the Windows gateway launcher renders Cryozen-owned paths under the
# configured CRYOZEN_HOME spelling (possibly a junction to another drive).
_cryozen_repo_root: Path = Path(__file__).resolve().parents[2]
_cryozen_repo_root_aliases: tuple[Path, ...] = _build_cryozen_repo_root_aliases(
    _cryozen_repo_root, Path(__file__).absolute().parents[2], get_process_cryozen_home())
_in_venv: bool = (getattr(sys, "base_prefix", sys.prefix) != sys.prefix
                  or hasattr(sys, "real_prefix"))  # real_prefix: virtualenv<20
_cryozen_site_packages: list[Path] | None = None  # lazily cached by local_pythonpath
...
def _kill_process_group_posix(proc) -> None:
    ...
    try:
        ...
        if (pgid := getattr(proc, "_cryozen_pgid", None)) is None:
            raise
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Cryozen host itself — controller-side platform behavior
    ...
    def get_temp_dir(self) -> str:
        ...
        (Termux has no system temp dir), ``CRYOZEN_HOME/cache/terminal`` (real storage: a
        tmpfs system temp dir fills under Cryozen load; pruned by ``cleanup_terminal_temp_cache``),
        ...
        so always the CRYOZEN_HOME cache dir with forward slashes (bash- and Python-valid)."""
        if _IS_WINDOWS:
            cache_dir = (_default_terminal_temp_dir()
                         or Path(tempfile.gettempdir()) / "cryozen_terminal")
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False, timeout: int = 120,
        ...
        if not _IS_WINDOWS:
            with contextlib.suppress(ProcessLookupError):
                proc._cryozen_pgid = os.getpgid(proc.pid)
