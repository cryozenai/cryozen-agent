"""Secret-scrub policy for Cryozen child processes: pure data + predicates for which env
names are Cryozen-managed credentials. The env *builders* applying it (``_make_run_env``,
``_sanitize_subprocess_env``, ``cryozen_subprocess_env``) live in ``tools.environments.local``."""

import os

# Prefix a caller uses in ``extra_env`` to force a blocklisted var through.
_CRYOZEN_PROVIDER_ENV_FORCE_PREFIX = "_CRYOZEN_FORCE_"

# Cryozen-managed AWS *inference* credentials for ``auth_type="aws_sdk"`` (Bedrock):
...
_STATIC_PROVIDER_ENV_BLOCKLIST = frozenset({
    ...
    "EMAIL_HOME_ADDRESS", "EMAIL_HOME_ADDRESS_NAME", "CRYOZEN_DASHBOARD_SESSION_TOKEN",
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        ...
        pass
    # BUZZ_* is deliberately NOT discarded: this list feeds every scrub surface, so an
    # import-time discard would leak BUZZ_PRIVATE_KEY into non-terminal children; the Buzz
    # carve-out is terminal-only and context-gated (``_is_terminal_first_party_env``).
...
_CRYOZEN_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _is_provider_env_blocklisted(name: str) -> bool:
    ...
    ``OPENAI_API_KEY``; consistent with ``_is_cryozen_internal_secret``, which
    already folds (``key.upper()``)."""
    return (name in _CRYOZEN_PROVIDER_ENV_BLOCKLIST
            or name.upper() in _CRYOZEN_PROVIDER_ENV_BLOCKLIST)
...
# ``_sanitize_subprocess_env``); execute_code, cryozen_subprocess_env, docker and
...
# ``buzz`` (the gateway's ``CRYOZEN_SESSION_PLATFORM`` ContextVar; concurrency safe under a multi-session
...
# scope. Every other surface stays sealed — execute_code scrubbing, :func:`cryozen_subprocess_env` (browser /
...
# terminal snapshot across profiles. These ARE Cryozen-managed first-party platform credentials, so they stay IN the blocklist for every non-terminal surface. See issue #78026 (Buzz
...
def _buzz_terminal_context_active() -> bool:
    ...
    try:
        ...
        return get_session_env("CRYOZEN_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# ANOTHER project's deps into the Cryozen venv (still reachable via PATH, so stripping
# is safe); PYTHONHOME redirects a child interpreter's stdlib to the Cryozen venv
# (version-mismatch crashes). PYTHONPATH is handled separately (Cryozen-owned entries only).
...
# dependencies into the Cryozen venv path instead of the project's own ``.venv`` — silently clobbering the
# Cryozen environment (e.g. a project pinned to a different Python version overwrites it and breaks the
# gateway). PYTHONHOME is included because a gateway-inherited value redirects the standard-library search
# of ANY child interpreter — including unrelated system/venv Pythons — to the Cryozen venv's stdlib, which
# crashes with version-mismatch errors before a child script even imports a package (#75018). Cryozen itself
...
# _strip_cryozen_owned_pythonpath() which removes only Cryozen-owned entries, preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_cryozen_internal_secret(key: str) -> bool:
    """True for Cryozen-internal secrets injected under *dynamic* names the static
...
# adapter is covered without a second edit; ``CRYOZEN_*`` never counts (``CRYOZEN_MEDIA_ALLOW_DIRS``,
# ``CRYOZEN_ALLOW_PRIVATE_URLS`` are process settings, not adapter gates).
...
def is_profile_gate_env(name: str) -> bool:
    ...
    upper = name.upper()
    if upper.startswith("CRYOZEN_") or upper.startswith("_"):
        return False
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_cryozen_internal_secret, but _ID has no secret suffix, so it must be
    # enumerated here to stay stripped on the inherit_credentials=True path.
    "GATEWAY_RELAY_ID", "GATEWAY_RELAY_SECRET", "GATEWAY_RELAY_DELIVERY_KEY",
    "HASS_TOKEN", "EMAIL_PASSWORD", "CRYOZEN_DASHBOARD_SESSION_TOKEN",
