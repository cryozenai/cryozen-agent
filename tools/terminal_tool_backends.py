...
# startup notice and `cryozen doctor` read it through terminal_backend_unavailable_reason() so the user
...
_CONTAINER_KEYS = (
    ...
    ("container_persistent", True), ("vercel_runtime", ""),
...
_MODAL_NO_CREDENTIALS = (
    "Modal backend selected but no Modal credentials/config were found. Set MODAL_TOKEN_ID and "
    "MODAL_TOKEN_SECRET (or run `modal setup`), or choose a different TERMINAL_ENV.")
...
def _build_docker_env(*, image, cwd, timeout, cc, task_id, host_cwd, **_):
    ...
    # One-shot reaper for labeled containers orphaned by prior Cryozen processes that died before
...
def _build_modal_env(*, image, cwd, timeout, cc, task_id, **_):
    ...
    if not has_direct_modal_credentials():
        raise ValueError(_MODAL_NO_CREDENTIALS)
...
def _build_plugin_env(*, env_type, image, cwd, timeout, cc, task_id, **_):
    ...
    if provider is not None:
        ...
        try:
            env_obj._cryozen_backend_name = provider.name.strip().lower()
        except AttributeError:
            pass
        return env_obj
    try:
...
def _modal_pre(config: Dict[str, Any]) -> Optional[bool]:
    if not has_direct_modal_credentials():
        return _reject(_MODAL_NO_CREDENTIALS)
    return None
...
def _ssh_pre(config: Dict[str, Any]) -> bool:
    ...
    return _reject("the SSH host and user are not configured (TERMINAL_SSH_HOST / TERMINAL_SSH_USER); "
                   "run `cryozen setup terminal` to enter them or pick the 'local' backend")
