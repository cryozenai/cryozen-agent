...
# ladders, parked-server self-probes and repeated `cryozen mcp test` runs re-run the preflight
...
# The cache is also persisted under the Cryozen home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``cryozen_constants.get_cryozen_home()`` so the cache follows the
    active profile and is isolated across Cryozen homes. The cache directory
    is created on demand. Returns ``None`` when Cryozen home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "cryozen-agent-osv-check/1.0"},
        method="POST")
