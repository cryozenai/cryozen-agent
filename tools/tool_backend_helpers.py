...
def _dotenv_value(env_var: str) -> str:
    """``.env`` value via ``cryozen_cli.config.get_env_value`` (``""`` when unavailable)."""
    try:
...
def _env_source_suppressed(provider_id: str, env_var: str) -> bool:
    """``cryozen auth remove <provider>`` records ``env:<VAR>`` in ``suppressed_sources`` and promises
    the variable is ignored until ``cryozen auth add``; a value still living in a long-running
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    ``cryozen_cli.config.get_env_value``) -> credential pool for ``provider_id``. Under an
    ...
    Resolution order (fixes #68003 — keys added via ``cryozen auth add <provider>`` were invisible to the
...
def resolve_openai_audio_api_key() -> str:
    ...
    Outside a multiplexed turn, ``OPENAI_API_KEY`` additionally falls back to the credential pool (``cryozen
...
def _raw_section(section: str) -> Dict[str, Any] | None:
    ...
    try:
        from cryozen_cli.config import read_raw_config_readonly
...
def read_selection(section: str) -> str | None:
    """THE single runtime read of the persisted `cryozen tools` selection: a vendor name (own
    credentials) or ``None`` (never configured -> legacy autodetect allowed). Reads the RAW
    config.yaml so key presence means "actually written", not "schema default"; a raw ``local``
    is therefore a real user selection. Never raises."""
...
# (cryozen_cli.config.validate_config_structure) and selection_error() consult this map so the user learns
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (f"{section} is configured to use {selection_name} (set via cryozen "
            f"tools), but {failure}. Run 'cryozen tools' to change it.")
