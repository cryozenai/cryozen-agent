...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    cryozen_meta = meta.get("cryozen") if isinstance(meta, dict) else None
    if not isinstance(cryozen_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.cryozen.{tags, related_skills}.")
    elif "tags" not in cryozen_meta:
        yield _warn("missing-metadata", "metadata.cryozen.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author != "Cryozen" and " ".join(author.lower().split()) in ("cryozen", "cryozen agent", "agent"):
        yield _warn("author-value", f"author '{author}' should be exactly 'Cryozen'.")
