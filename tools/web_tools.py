...
Backend is selected during ``cryozen tools`` (``web.backend`` in config.yaml; per
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via the config-aware env layer (``cryozen config set`` values), then process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set through Cryozen' config/.env layer
    (``cryozen config set``, ``cryozen tools``) are honored here too — not just raw process-env exports.
...
def _get_backend() -> str:
    ...
    # Never-configured install: explicit user credentials first, free tiers trail paid.
    backend_candidates = (
...
def _web_requires_env() -> list[str]:
    """Tool-registry metadata env vars for the web backends. Contract: set var -> tool sees it."""
    return [
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            fallback = "No web search provider configured. Run `cryozen tools` to set one up."
...
def _provider_is_ready(provider) -> bool:
    ...
    False (so dispatch can emit a precise error), so readiness gates (tool check_fn, ``cryozen doctor``)
