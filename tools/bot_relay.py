...
from tools.bot_mode_probe import _default_home, _cryozen_root, alias_forms
...
# Mirrors RELAY_DELIVER_TIMEOUT_MS in apps/desktop/src/plugins/cryozen-bots/relay.ts; both test suites pin it.
...
# One turn in a profile's canonical Bot Chat: ``cryozen -p <profile> *BOT_CHAT_TURN_ARGS``.
...
# (``cryozen_cli.quiet_single_query.adopt_unanswered_turn``, which consumes the variable before the turn).
RESUME_UNANSWERED_TURN_ENV = "CRYOZEN_RESUME_UNANSWERED_TURN"
...
def _ensure_dirs(root: Path | str) -> Path:
    ...
    for sub in (OUTBOX_DIR, CLAIMED_DIR, REPLIES_DIR):
        ...
        mkdir_under_cryozen_home(base / sub)
    return base
...
def _bot_mode_cfg(key: str, *, loader: str) -> Any:
    ...
    try:
        import cryozen_cli.config as cfgmod
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    handle = str(row.get("handle") or "").strip().lstrip("@") or ("cryozen" if profile == "default" else profile)
...
def _target_aliases(row: dict) -> set[str]:
    ...
    ``default`` is ``@cryozen`` on every gateway, so its title is the only bare form that can single it out."""
...
def _title_slug(row: dict) -> str:
    ...
    title is empty, reserved (a bot titled "Cryozen") or not a valid handle."""
...
def remote_target_forms(roster: list[dict], local_taken: "set[str] | frozenset[str]" = frozenset()) -> list[str]:
    ...
    ``@cos-bot``, since bare ``@cryozen`` is always this gateway's own default); else
...
def qualify_sender_stamp(message: str, from_handle: Any, from_connection: Any, roster: list[dict],
    ...
    ``remote_target_forms`` renders it, else ``handle@connection``. A relayed ``@cryozen`` is another
...
def cleanup_bot_relay_artifacts(max_age_hours: float | None = None) -> int:
    ...
    del max_age_hours
    try:
        base = relay_root(_cryozen_root(Path(_default_home())))
...
def _cryozen_cli() -> str:
    """cryozen CLI beside this interpreter, then ``shutil.which``, then the bare name
    (service contexts lack PATH, so a bare "cryozen" died with ENOENT).

    The deliver RPC runs on the target gateway, whose process is the venv python — its bin/Scripts directory
    holds the matching ``cryozen`` entrypoint. A bare ``"cryozen"`` relies on PATH, which is exactly what
    ...
    """
    sibling = Path(sys.executable or "").parent / ("cryozen.exe" if sys.platform == "win32" else "cryozen")
    return str(sibling) if sibling.is_file() else shutil.which("cryozen") or "cryozen"
...
def local_delivery_command(profile: str, query_file: str) -> list[str]:
    ...
    return [_cryozen_cli(), "-p", profile, *BOT_CHAT_TURN_ARGS, "--query-file", query_file]
...
def _delivery_child_session_env_names() -> "tuple[str, ...]":
    ...
    ``CRYOZEN_SESSION_*`` prefix match, which would also strip non-identity knobs
    (e.g. ``CRYOZEN_SESSION_STALL_TIMEOUT``)."""
...
def delivery_env(author: Optional[dict], profile_home: "str | Path | None" = None) -> dict[str, str]:
    """Environment for one delivery turn's ``cryozen -p <profile>`` child. The dispatcher's own
    CRYOZEN_TURN_AUTHOR is dropped first so a delivery without an author never inherits the author of the turn
    ...
    ``os.environ``; ``-p`` alone only pinned CRYOZEN_HOME. ``profile_home`` is the target's home when the
...
# Deliveries are separate ``cryozen`` subprocesses, so the lock is a per-profile
...
# target profile must never run their Bot Chat turns concurrently: deliveries spawn separate ``cryozen``
