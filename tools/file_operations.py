...
NOT_REGULAR_SENTINEL = "__cryozen_not_regular__"
...
MISSING_SENTINEL = "__cryozen_missing__"

_READ_SENTINEL_PREFIX = "__CRYOZEN_RF_"
_WRITE_SENTINEL_PREFIX = "__CRYOZEN_WF_"
...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        understand the MSYS ``/c/...`` form and Cryozen disables MSYS argument
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        tmpl = self._escape_shell_arg(".cryozen-tmp.XXXXXX")
        script = (
            "set -e; "
            ...
            '|| mktemp "$d/.cryozen-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.cryozen-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        snippet = (
            ...
            "    if size > MAX:\n"
            "        print('CRYOZEN_UTF16:NO'); sys.exit(0)\n"
            "    with open(p, 'rb') as f:\n"
            "        data = f.read()\n"
            "    sample = data[:SAMPLE]\n"
            "    enc = None\n"
    ...
    def _native_read_enabled(self) -> bool:
        ...
        file_operations holds Git-Bash-style paths there). ``CRYOZEN_NATIVE_FILE_READ=0``
        turns the fast path off."""
        flag = os.environ.get("CRYOZEN_NATIVE_FILE_READ", "1").strip().lower()
