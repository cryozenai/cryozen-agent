...
CRYOZEN_HOME = get_cryozen_home()  # all skills live in ~/.cryozen-agent/skills/ (seeded from bundled)
...
def _skills_dir() -> Path:
    ...
    it, else live profile-scoped CRYOZEN_HOME (long-lived runtimes may import before profile set)."""
...
def load_env() -> Dict[str, str]:
    """Snapshot of CRYOZEN_HOME/.env for the post-skill secret-capture diff (same tokenizer that
    installs the profile scope, so a captured value never differs from the served one)."""
    from agent.secret_scope import load_env_file

    return load_env_file(get_cryozen_home() / ".env")
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    """``~/.cryozen-agent/skills/mlops/axolotl/SKILL.md`` -> ``"mlops"``; active profile dir first
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    """Disabled in config? Platform precedence: explicit arg, ``CRYOZEN_PLATFORM``, session
    ``CRYOZEN_SESSION_PLATFORM``. A globally-disabled skill stays disabled on every platform
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("CRYOZEN_PLATFORM")
        if not resolved_platform:
            with suppress(Exception):
                from gateway.session_context import get_session_env
                resolved_platform = get_session_env("CRYOZEN_SESSION_PLATFORM") or ""
    ...
    if skill_md is not None and project_dirs:
        ...
        if _under_any(skill_md, project_dirs) and is_quarantined_project_skill(skill_md):
            return _fail(
                ...
                "`cryozen skills untrust`."), None, None
...
def _log_security_warnings(name: str, skill_md: Path, content: str, all_dirs, active_skills_dir):
    ...
    if not _under_any(skill_md, trusted_dirs):
        warnings.append(f"skill file is outside the trusted skills directory (~/.cryozen-agent/skills/): {skill_md}")
...
def skill_view(
    ...
    try:
        ...
        if _is_skill_disabled(resolved_name):
            return _fail(f"Skill '{resolved_name}' is disabled. Enable it with `cryozen skills` or inspect the files directly on disk.")
        if file_path and skill_dir:
            return _serve_skill_file(
                skill_dir, file_path, name, list_available=True, mark_read=True,
                hint="Use a relative path within the skill directory")
        # tags/related_skills: metadata.cryozen.* (agentskills.io) first, then top-level.
        metadata = frontmatter.get("metadata")
        cryozen_meta = (metadata.get("cryozen", {}) or {}) if isinstance(metadata, dict) else {}
        tags, related_skills = (
            _parse_tags(cryozen_meta.get(k) or frontmatter.get(k, "")) for k in ("tags", "related_skills"))
