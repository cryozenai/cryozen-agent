...
GitHub releases to $CRYOZEN_HOME/bin/tirith in a background thread -- SHA-256 always verified,
...
from cryozen_constants import get_cryozen_home, get_cryozen_home_override, cryozen_home_key
...
def _cached_path() -> str | None:
    ...
    if get_cryozen_home_override() is not None:
        return _resolved_path_by_home.get(cryozen_home_key())
...
def _store_resolved(path: str) -> None:
    ...
    if get_cryozen_home_override() is not None:
        _resolved_path_by_home[cryozen_home_key()] = path
    else:
        _resolved_path = path
...
def _failure_marker_path() -> str:
    return os.path.join(str(get_cryozen_home()), ".tirith-install-failed")
...
# --- Auto-install ---
def _cryozen_bin_dir() -> str:
    """$CRYOZEN_HOME/bin, created if needed."""
    os.makedirs(d := os.path.join(str(get_cryozen_home()), "bin"), exist_ok=True)
    return d
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CRYOZEN_HOME/bin/tirith -> ``(installed_path,
    ...
    try:
        ...
        if src is None:
            return None, reason
        dest = os.path.join(_cryozen_bin_dir(), "tirith")
        try:
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": PATH, then $CRYOZEN_HOME/bin."""
    cryozen_bin = os.path.join(_cryozen_bin_dir(), "tirith")
    return shutil.which("tirith") or (cryozen_bin if _is_executable(cryozen_bin) else None)
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    $CRYOZEN_HOME/bin/tirith → install; failures cached in-process and on disk for 24h). On a miss
