...
def _cryozen_tags(fm: dict) -> Any:
    """``metadata.cryozen.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    cryozen_meta = metadata.get("cryozen", {}) if isinstance(metadata, dict) else None
    return cryozen_meta.get("tags", []) if isinstance(cryozen_meta, dict) else []
