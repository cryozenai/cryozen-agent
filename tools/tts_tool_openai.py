...
Also owns the credential/base-URL selection that decides where the OpenAI client points. Seams defined on the origin module (``_load_tts_config``,
...
from tools.tool_backend_helpers import read_selection, resolve_openai_audio_api_key, selection_error
...
# DeepInfra base URL is resolved via cryozen_cli.models.deepinfra_base_url (shared).
DEFAULT_DEEPINFRA_TTS_VOICE = "default"


def _resolve_openai_audio_client_config() -> tuple[str, str]:
    """``(api_key, base_url)`` for the OpenAI audio client: ``tts.openai.api_key`` then
    ``VOICE_TOOLS_OPENAI_KEY``/``OPENAI_API_KEY``. A stored ``tts`` selection with no key gets the
    selection-naming error; an unset one the plain missing-key error."""
    origin = _origin()
    openai_cfg = _section(origin._load_tts_config(), "openai")
    direct_api_key = openai_cfg.get("api_key") or resolve_openai_audio_api_key()
    if direct_api_key:
        return direct_api_key, openai_cfg.get("base_url") or DEFAULT_OPENAI_BASE_URL
    missing = "neither tts.openai.api_key in config nor VOICE_TOOLS_OPENAI_KEY/OPENAI_API_KEY is set"
    selected = read_selection("tts")
    if selected is not None:
        raise ValueError(selection_error("tts", selected, missing))
    raise ValueError(missing[0].upper() + missing[1:])
...
def _generate_openai_tts(
    ...
    and skip the credential resolution; otherwise the OpenAI auth chain and ``tts.openai``
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    """Resolve DeepInfra credentials/model (live ``cryozen_cli.models`` catalog, no hardcoded ids), then
    delegate to the OpenAI-compatible handler."""
    api_key = _origin()._resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError("DEEPINFRA_API_KEY not set. Run `cryozen setup` to configure, or set the env var directly.")
    di_config = _section(tts_config, "deepinfra")
    from cryozen_cli.models import deepinfra_base_url, deepinfra_model_ids
