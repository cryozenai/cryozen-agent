...
_CRYOZEN_CORE_TOOLS = [
    ...
    "computer_use",
    # Local MCP server install / enable / authorize (tools.connectors.enabled off switch).
    "manage_connections",
]

# Webhook payloads are untrusted third-party content: no file/system execution.
_CRYOZEN_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `cryozen-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _CRYOZEN_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_CRYOZEN_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _CRYOZEN_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `cryozen tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``cryozen tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _CRYOZEN_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "connections": _ts("Local MCP server install, enable, and authorization", ["manage_connections"]),
    ...
    # (model_tools._select_tool_names). Never configurable, never in `cryozen tools`.
    "setup": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (CRYOZEN_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _CRYOZEN_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in cryozen_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Cryozen toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. cryozen-acp is the
    ...
    "cryozen-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors cryozen-cli; `cryozen tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "cryozen-cron": _bundle("Default cron toolset - same core tools as cryozen-cli; gated by `cryozen tools`"),
    "cryozen-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "cryozen-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "cryozen-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "cryozen-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "cryozen-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "cryozen-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "cryozen-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "cryozen-email": _bundle("Email bot toolset - interact with Cryozen via email (IMAP/SMTP)"),
    "cryozen-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "cryozen-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "cryozen-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "cryozen-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "cryozen-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "cryozen-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "cryozen-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "cryozen-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "cryozen-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _CRYOZEN_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "cryozen-sms": _bundle("SMS bot toolset - interact with Cryozen via SMS (Twilio)"),
    "cryozen-webhook": _ts("Webhook toolset - receive and process external webhook events", _CRYOZEN_WEBHOOK_SAFE_TOOLS),
    "cryozen-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "cryozen-telegram", "cryozen-discord", "cryozen-whatsapp", "cryozen-slack",
            "cryozen-signal", "cryozen-bluebubbles", "cryozen-homeassistant", "cryozen-email",
            "cryozen-sms", "cryozen-mattermost", "cryozen-matrix", "cryozen-dingtalk",
            "cryozen-feishu", "cryozen-wecom", "cryozen-wecom-callback", "cryozen-weixin",
            "cryozen-qqbot", "cryozen-webhook", "cryozen-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _CRYOZEN_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only cryozen-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_CRYOZEN_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `cryozen-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("cryozen-"):
        return []
    platform_name = name[len("cryozen-"):]
    try:
...
def profile_role_toolsets(profile_home: Optional[Path] = None) -> Tuple[Set[str], Set[str]]:
    ...
    from cryozen_cli.profiles import read_profile_meta
    from cryozen_constants import get_cryozen_home
    role = read_profile_meta(Path(profile_home or get_cryozen_home())).get("role")
