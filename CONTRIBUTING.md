# Contributing to Cryozen Agent

Thank you for contributing to Cryozen Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Cryozen to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo cryozenai/cryozen-agent "<your terms>"
  gh search prs --repo cryozenai/cryozen-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/cryozenai/cryozen-agent/issues?q=) · [PRs (all states)](https://github.com/cryozenai/cryozen-agent/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Cryozen itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Cryozen install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `cryozen skills browse` (labeled "official") and install it with `cryozen skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Cryozen GitHub Discussions](https://github.com/cryozenai/cryozen-agent/discussions). Users can install it with `cryozen skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.cryozen-agent/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(cryozen_home, config)` for setup-wizard integration
...
- Integrate with `cryozen memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Cryozen ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.cryozen-agent/plugins/`, project `.cryozen-agent/plugins/`, or a pip entry point) — see [Build a Cryozen Plugin](https://cryozenai.github.io/cryozen-agent/docs/guides/build-a-cryozen-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Cryozen GitHub Discussions](https://github.com/cryozenai/cryozen-agent/discussions) so users can find and install it
...
The installer creates the Cryozen venv, wires the `cryozen` command, stamps the
install method for `cryozen update`, and clones the full git project into
`$CRYOZEN_HOME/cryozen-agent` (usually `~/.cryozen-agent/cryozen-agent`). That keeps your
...
Use this only if you intentionally do not want Cryozen' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `cryozen` entrypoint from this venv; running the
system `python3 -m cryozen_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.cryozen-agent/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.cryozen-agent/config.yaml
touch ~/.cryozen-agent/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.cryozen-agent/.env
```

### Run

```bash
# The standard installer already put `cryozen` on PATH.
cryozen doctor
cryozen chat -q "Hello"
```

If you used the manual clone fallback, run `./cryozen` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/cryozen" ~/.local/bin/cryozen
```
...
├── cli.py                    # CryozenCLI class — interactive CLI orchestrator (~4.6k LOC + cryozen_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (cryozen-cli, cryozen-telegram, etc.)
├── cryozen_state.py           # SessionDB facade (~1.4k LOC); implementation in cryozen_state_*.py (21 siblings) — FTS5 search, session titles
...
│
├── cryozen_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── cli_*_mixin.py            # CryozenCLI mixins (slash commands, display, session, ...)
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, credential pools, OAuth providers (facade + auth_*.py siblings)
...
│
├── skills/                   # Bundled skills (copied to ~/.cryozen-agent/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (cryozenai.github.io/cryozen-agent)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.cryozen-agent/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.cryozen-agent/`)

| Path | Purpose |
|------|---------|
| `~/.cryozen-agent/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.cryozen-agent/.env` | API keys and secrets |
| `~/.cryozen-agent/auth.json` | Provider credentials and credential pools |
| `~/.cryozen-agent/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.cryozen-agent/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.cryozen-agent/state.db` | SQLite session database |
| `~/.cryozen-agent/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written; state.db is canonical. |
| `~/.cryozen-agent/cron/` | Scheduled job data |
| `~/.cryozen-agent/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`cryozen_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `cryozen sessions export` for an explicit export.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (provider API keys such as Anthropic, OpenAI or OpenRouter, OAuth providers, or custom endpoint).
...
(for example `_CRYOZEN_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.cryozen`:
...
The user may skip setup and keep loading the skill. Cryozen only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `cryozen setup` or update `~/.cryozen-agent/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Cryozen tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Cryozen tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `cryozen --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Cryozen uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.cryozen-agent/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `cryozen_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `cryozen_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Cryozen runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the cryozen wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `cryozen_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `cryozen_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `cryozen_cli/gateway_windows.py::
...
Cryozen has terminal access. Security matters.
...
2. **Test manually**: Run `cryozen` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/cryozenai/cryozen-agent/issues)
- Include: OS, Python version, Cryozen version (`cryozen --version`), full error traceback
...
- **GitHub Discussions**: [cryozenai/cryozen-agent discussions](https://github.com/cryozenai/cryozen-agent/discussions) for questions, design proposals and sharing skills
- **Skills Hub**: publish specialized skills to a skills registry instead of bundling them
