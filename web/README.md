# Cryozen Agent — Web UI

Browser-based dashboard for managing Cryozen Agent configuration, API keys, and monitoring active sessions.
...
cd ../
python -m cryozen_cli.main web --no-open
...
`cryozen dashboard` on port 9119 serves the **built** bundle from `cryozen_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../cryozen_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- Use the **semantic text tokens** from the design system's `src/ds/globals.css`:
...
- The dashboard keeps an uppercase chrome aesthetic, but it is **opt-in per element, not global**.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `cryozen-agent/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
Typography is **opt-in per surface**, not global on layout shells — the app shell and page header keep their original theme/expanded fonts; the `font-label` family applies only where explicitly set.

| Tier | Classes | Use for |
|------|---------|---------|
| Brand chrome | `font-label text-display` (or `themedChrome`) | Sidebar nav, card section headers (`CardTitle`), Segmented filter buttons, filter panel headings |
| Themed body | `font-label normal-case` (or `themedBody`) | Card content (`Card`, `CardDescription`), session/platform rows, analytics tables — **scoped to the component** |
| Page chrome | `font-expanded` | Page header h1 (`PageHeaderProvider`) — sentence case, not `text-display` |
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Cryozen Agent” — mixed case, no `font-label`, no `text-display` |
...
- **`Typography`** defaults to `font-sans` unless a font prop is passed.
- Do **not** use raw `font-sans` or `font-display` (theme sans variable) on new dashboard UI — prefer the `font-label` tiers above where appropriate.
