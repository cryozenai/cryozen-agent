# web/ + cryozen_cli/web_routers/ — the dashboard (`cryozen dashboard` → `/chat`)

Applies on top of the root `AGENTS.md`. Backend routers: `cryozen_cli/web_routers/*.py`, one file per
dashboard surface, mounted by `cryozen_cli/web_server.py` (+ `web_server_*.py` siblings). Frontend:
`web/src/`. Shared JSON-RPC/WS client: `apps/shared` (`@cryozen/shared`), also used by the desktop.

## The dashboard embeds the REAL `cryozen --tui` — not a rewrite

`cryozen_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `web_server.py`:
...
- The server spawns exactly what `cryozen --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL
...
`cmd_dashboard` skips `_build_web_ui` and exports `CRYOZEN_SERVE_HEADLESS=1` so `mount_spa()`
...
- Routers are one-file-per-surface; a new surface is a new `web_routers/<surface>.py`, not a growing
  `web_server.py`.
- Tests: Python in `tests/cryozen_cli/` (routers, pty bridge); JS in the `web/` vitest suite. Python
