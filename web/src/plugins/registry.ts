 ...
 *
 * Plugins call window.__CRYOZEN_PLUGINS__.register(name, Component)
...
import { Card, CardHeader, CardTitle, CardContent } from "@/ds/components/card";
 ...
 * Exposed at runtime as ``window.__CRYOZEN_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__CRYOZEN_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__CRYOZEN_PLUGIN_SDK__ = {
    ...
    // window.__CRYOZEN_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components from the dashboard design system where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
