...
import { api, CRYOZEN_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "cryozen:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${CRYOZEN_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${CRYOZEN_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?cryozen_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-cryozen-plugin", manifest.name);
