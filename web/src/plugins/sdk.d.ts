/**
 * Cryozen Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__CRYOZEN_PLUGIN_SDK__`` and
 * ``window.__CRYOZEN_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__CRYOZEN_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, the design system, …). Deriving the type would
 ...
 *
 * Versioning: bump ``CryozenPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@cryozen/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls design-system types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__CRYOZEN_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__CRYOZEN_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__CRYOZEN_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface CryozenPluginSDK {
  ...
  /**
   * Shared UI primitives (design system / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __CRYOZEN_PLUGIN_SDK__?: CryozenPluginSDK;
    __CRYOZEN_PLUGINS__?: PluginRegistry;
  }
}
