...
export const API_UNREACHABLE_MESSAGE =
  "Cryozen dashboard cannot reach the Cryozen service. Is `cryozen dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The Cryozen service hit an internal error.",
  502: "The dashboard proxy could not reach the Cryozen service.",
  503: "The Cryozen service is not ready yet. Try again in a moment.",
  504: "The Cryozen service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The Cryozen service answered with an unexpected error (${status}).`;
}
