...
// (e.g. https://mission-control.tilos.com/cryozen/). The Python backend
// injects ``window.__CRYOZEN_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__CRYOZEN_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const CRYOZEN_BASE_PATH = readBasePath();
const BASE = CRYOZEN_BASE_PATH;
...
declare global {
  interface Window {
    __CRYOZEN_SESSION_TOKEN__?: string;
    __CRYOZEN_BASE_PATH__?: string;
    ...
    __CRYOZEN_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Cryozen-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__CRYOZEN_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``cryozen update``, ``cryozen gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__CRYOZEN_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__CRYOZEN_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__CRYOZEN_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``cryozen_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__CRYOZEN_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Cryozen-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``cryozen_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__CRYOZEN_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildCryozenWebSocketUrl({
...
export const api = {
  ...
  updateCryozen: () =>
    fetchJSON<ActionResponse>("/api/cryozen/update", { method: "POST" }),
  checkCryozenUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/cryozen/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * is attached. ``email`` and ``display_name`` are empty strings when the
 * provider emits no email/name claims; the AuthWidget then surfaces a
...
/** Preflight for `cryozen gateway migrate --multiplex` (mirrors the CLI plan JSON). */
...
export interface SkillHubSource {
  ...
  /** cryozen-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``cryozen update``. */
  can_update_cryozen?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the CRYOZEN_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the CRYOZEN_HOME volume — no staleness dimension, so no
 * sampled_at. */
