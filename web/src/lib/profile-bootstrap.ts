declare global {
  interface Window {
    __CRYOZEN_DASHBOARD_PROFILE__?: string;
    __CRYOZEN_INITIAL_PROFILE__?: string;
  }
}

export function dashboardInitialProfile(): string {
  if (typeof window === "undefined") return "";
  return window.__CRYOZEN_INITIAL_PROFILE__ ?? "";
}

/**
 * The profile this backend process itself serves, injected by the server and
 * empty when it cannot be named unambiguously (custom CRYOZEN_HOME).
 *
...
export function dashboardServingProfile(): string {
  if (typeof window === "undefined") return "";
  return window.__CRYOZEN_DASHBOARD_PROFILE__ ?? "";
}
