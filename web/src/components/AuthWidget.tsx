 ...
 * OAuth gate (Phase 7 of .cryozen-agent/plans/2026-05-21-dashboard-oauth-auth.md).
 *
 * Renders nothing in loopback / --insecure mode. In gated mode, fetches
 * /api/auth/me on mount and surfaces:
 *
 *   - the display_name or email when the provider supplies them, else the
 *     user_id truncated to 14 chars + ellipsis (providers that emit no
 *     email/name claims respond with empty strings for those fields)
...
export function AuthWidget({ className }: AuthWidgetProps) {
  ...
  const gated =
    typeof window !== "undefined" && !!window.__CRYOZEN_AUTH_REQUIRED__;
  ...
  // Prefer display_name, then email, then the truncated user_id; some
  // providers populate only user_id.
