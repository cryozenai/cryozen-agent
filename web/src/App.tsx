...
import { SelectionSwitcher } from "@/ds/components/selection-switcher";
import { Spinner } from "@/ds/components/spinner";
import { Typography } from "@/ds/components/typography/index";
import { ConfirmDialog } from "@/ds/components/confirm-dialog";
import { cn } from "@/lib/utils";
import { SidebarFooter } from "@/components/SidebarFooter";
import { SidebarStatusStrip, gatewayLine } from "@/components/SidebarStatusStrip";
import { useBelowBreakpoint } from "@/ds/hooks/use-below-breakpoint";
...
const SIDEBAR_COLLAPSED_KEY = "cryozen-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="cryozen-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="cryozen-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateCryozen = status?.can_update_cryozen === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'cryozen update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "cryozen update";
    return (
      t.status.updateCryozenConfirmMessage ??
      `This will run 'cryozen update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateCryozenConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateCryozen) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateCryozen,
      runningLabel: t.status.updatingCryozen,
      spin: false,
    });
  }
  ...
  return (
      ...
      description={
        sharedGateway
          ...
          : (t.status.restartGatewayConfirmMessage ??
            "This restarts the Cryozen gateway process. Connected channels and active sessions will reconnect afterward.")
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateCryozenConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateCryozenConfirmTitle ?? `${t.status.updateCryozen}?`}
    />
    </>
  );
}
