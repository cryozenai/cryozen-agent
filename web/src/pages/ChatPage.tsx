/**
 * ChatPage — embeds `cryozen --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (cryozen_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const tokenMissing =
    ...
    !window.__CRYOZEN_SESSION_TOKEN__ &&
    !window.__CRYOZEN_AUTH_REQUIRED__;
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("cryozen-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("cryozen-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__CRYOZEN_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Cryozen TUI has enabled xterm mouse-events
    ...
    //      payloads upload to CRYOZEN_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to CRYOZEN_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[cryozen-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__CRYOZEN_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets CRYOZEN_HOME pointed at the
    ...
    // forwarding them into Cryozen. Keyboard input, paste, and resize still
    // behave normally.
    ...
    createPortal(
      <>
        ...
        <div
          ...
          className={cn(
            "font-label fixed top-0 right-0 z-[60] flex h-dvh max-h-dvh w-64 min-w-0 flex-col antialiased",
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="cryozen-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
