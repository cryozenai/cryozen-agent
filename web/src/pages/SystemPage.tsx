...
import { H2 } from "@/ds/components/typography/h2";
...
import { useConfirmDelete } from "@/ds/hooks/use-confirm-delete";
import { ConfirmDialog } from "@/ds/components/confirm-dialog";
import { useModalBehavior } from "@/hooks/useModalBehavior";
import { DeleteConfirmDialog } from "@/components/DeleteConfirmDialog";
import { CryozenConsoleModal } from "@/components/CryozenConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `cryozen import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_cryozen === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkCryozenUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_cryozen],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_cryozen === false) {
      showToast(
        "Cryozen updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateCryozen();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "cryozen-update");
  ...
  const canUpdateCryozen = status?.can_update_cryozen !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateCryozen && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Cryozen?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'cryozen update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'cryozen update' (${updateInfo?.update_command ?? "cryozen update"}) and restart the gateway when it finishes.`
        }
        ...
        >
          <div className={cn(themedBody, "relative w-full max-w-lg border border-border bg-card shadow-2xl flex flex-col")}>
            ...
            <header className="p-5 pb-3 border-b border-border">
              <h2 className="font-label text-display text-base tracking-wider">
                New shell hook
              </h2>
            </header>
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Cryozen</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.cryozen_version}</span>
                  {canUpdateCryozen &&
                    updateInfo &&
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$CRYOZEN_HOME/backups/cryozen-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Cryozen backup?"
              description={`This will overwrite your current Cryozen configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Cryozen team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
