...
import { ListItem } from "@/ds/components/list-item";
import { Segmented } from "@/ds/components/segmented";
...
const SOURCE_CONFIG: Record<string, { icon: typeof Terminal; color: string }> =
  {
    ...
    cryozen_flow: { icon: Play, color: "text-warning" },
...
const NO_MATCHING_SESSION_SOURCE = "__cryozen_dashboard_no_matching_source__";
...
function SnippetHighlight({ snippet }: { snippet: string }) {
  ...
  return (
    <p className="font-label normal-case mt-0.5 min-w-0 max-w-full truncate text-xs text-text-secondary">
      {parts}
    </p>
  );
}
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-0.5">
              <div className="flex min-w-0 items-center gap-2">
                {renaming ? (
                  ...
                  <span
                    className={`font-label normal-case min-w-0 flex-1 truncate text-sm ${hasTitle ? "font-medium" : "text-muted-foreground italic"}`}
                  >
                    {hasTitle
                      ? session.title
                      : session.preview
        ...
        const res = await fetch(api.exportSessionUrl(id, rowProfile(id)), {
          ...
          headers: {
            "X-Cryozen-Session-Token":
              (window as unknown as { __CRYOZEN_SESSION_TOKEN__?: string })
                .__CRYOZEN_SESSION_TOKEN__ ?? "",
          },
        });
        if (!res.ok) {
  ...
  return (
    <div className="flex min-w-0 w-full max-w-full flex-col gap-4">
        ...
        <div className="border border-border bg-background-base/50">
          <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs font-label tracking-[0.12em] truncate">
                {activeAction === "restart"
                  ? t.status.restartGateway
                  : t.status.updateCryozen}
              </span>
          ...
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-3">
            ...
            {showList && emptyCount > 0 && !isSearching && (
              <Button
                ...
                prefix={<Eraser />}
              >
                <span className="font-label normal-case text-xs">
      ...
      {showList && selectedIds.size > 0 && (
        ...
        >
          <span className="font-label normal-case text-xs text-primary tabular-nums">
                ...
                {recentSessions.map((s) => (
                  ...
                  >
                    <div className="flex min-w-0 flex-1 flex-col gap-1">
                      <span
                        className={`font-label normal-case min-w-0 truncate text-sm ${s.title ? "font-medium" : "text-muted-foreground italic"}`}
                      >
                        {s.title ??
                          (s.preview
                      ...
                      {s.preview && s.title && (
                        <p className="font-label normal-case min-w-0 max-w-full text-xs leading-snug text-text-tertiary [overflow-wrap:anywhere]">
                          {s.preview}
                        </p>
                      )}
                    </div>
