...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, CRYOZEN_HOME=home, PATH="/usr/bin:/bin", CRYOZEN_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["cryozen-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["cryozen-cli"], quiet_mode=True)
