...
The A/B battery behind PR [#81958](https://github.com/cryozenai/cryozen-agent/pull/81958)
(Browser Use CLI 3.0 mode, based on #66476): built-in
...
- **Arms differ only by tree + config.** `base` runs the built-in twelve
  ...
  digest's value). Each cell gets a throwaway `CRYOZEN_HOME`; web-fetch
...
- **Backend matrix.** `orchestrate.py` drives a local headless-Chrome CDP;
  `orchestrate_cloud.py --backend browserbase` provisions a real
