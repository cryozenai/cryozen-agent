...
The run gets a throwaway CRYOZEN_HOME so no local config leaks in, and the
...
hh = os.path.join(home, ".cryozen-agent")
...
os.environ["CRYOZEN_HOME"] = hh
# Strip web-fetch shortcuts: every arm must drive the browser.
os.environ.pop("BROWSER_USE_API_KEY", None)
for k in ("FIRECRAWL_API_KEY", "TAVILY_API_KEY", "SERPER_API_KEY"):
...
# Provider: OpenRouter (the original battery). The key is never printed.
