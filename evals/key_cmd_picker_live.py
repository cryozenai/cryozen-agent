...
Run with the Cryozen venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Cryozen children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def rows_worker():
    ...
    b = _PickerBuild(current_provider="", current_base_url="", current_model="",
                     max_models=None, for_picker=True,
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "cryozen_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="cryozen-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".cryozen-agent"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "CRYOZEN_HOME": str(state),
