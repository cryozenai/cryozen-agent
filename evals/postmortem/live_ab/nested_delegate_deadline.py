...
real result. Uses glm-5.3 via OpenRouter for cost. Deadline shortened via config to keep the run short."""
...
# Temp CRYOZEN_HOME with the real auth + a config that shortens the generic sequential deadline to 40 s, so the
...
home = tempfile.mkdtemp(prefix="dl_home_"); os.environ["CRYOZEN_HOME"] = home
real_home = os.environ.get("CRYOZEN_HOME_SOURCE", os.path.expanduser("~/.cryozen-agent"))  # credentials are copied from here into a temp home
...
ag = AIAgent(model=MODEL, provider="openrouter", base_url=rt.get("base_url"), api_key=rt.get("api_key"), api_mode=rt.get("api_mode"),
