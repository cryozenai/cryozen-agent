...
Runs N independent Cryozen ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Results that motivated cryozen-agent #104284 / #104421 (2026-09-05/06, Fable 5.1, 20 sessions x 6
calls unless noted):
...
Usage:
  python -m evals.postmortem.live_ab.cache_concurrency_probe --repo . --provider openrouter \
      --workers 20 --calls 6 --out probe.jsonl [--wire chat|native] [--model ID] \
      [--pin anthropic] [--settle 2] [--ttl 5m]
  providers: openrouter (OPENROUTER_API_KEY or --api-key),
             anthropic (ANTHROPIC_API_KEY or --api-key)
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real Cryozen request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="cryozen-agent checkout to import from")
    ap.add_argument("--provider", required=True, choices=["openrouter", "anthropic"])
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what Cryozen would pick for the provider/model)")
...
os.environ.setdefault("CRYOZEN_HOME", os.path.expanduser("~/.cryozen-agent"))
import anthropic
...
class _Ctx:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.rec["trace_headers"] = {k: v for k, v in hdrs.items() if k.lower() in ("cf-ray", "x-request-id", "request-id")}
...
json.dump(dict(provider=PROVIDER, wire=API_MODE, model=MODEL, workers=N, calls_per_session=CALLS, ttl=ARGS.ttl, pin=ARGS.pin, settle_s=SETTLE_S,
               ...
               bad_pairs=[dict(kind=k, session=f"cacheprobe-{PROVIDER}-{ARGS.wire or API_MODE}-{N}-{w}", ok_id=p.get("message_id"), ok_cf_ray=(p.get("trace_headers") or {}).get("cf-ray"),
                              bad_id=c.get("message_id"), bad_cf_ray=(c.get("trace_headers") or {}).get("cf-ray"), read_before=p["cache_read"], read_after=c["cache_read"],
