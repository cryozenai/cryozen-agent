...
sys.path.insert(0,sys.argv[1] if len(sys.argv)>1 else os.getcwd());os.environ['CRYOZEN_HOME']=tempfile.mkdtemp(prefix='cache-boundary-')  # usage: <repo_root>
from agent.anthropic_message_convert import convert_messages_to_anthropic
from agent.context_compressor import ContextCompressor
from agent.turn_context import _preflight_request_tokens
from agent.model_metadata import estimate_messages_tokens_rough
model='anthropic/claude-fable-5.1';url='https://openrouter.ai/api/v1'
cc=ContextCompressor(model=model,provider='openrouter',base_url=url,api_mode='anthropic_messages',config_context_length=1000000,threshold_tokens_cap=200000)
...
a=types.SimpleNamespace(api_mode='anthropic_messages',provider='openrouter',model=model,base_url=url,tools=[],_usage_anchor=None)
