...
- **`forensics/`** reads a *copy* of a Cryozen `state.db` (plus rotated `agent.log*` and git) and
...
- A Cryozen checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.cryozen-agent/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while Cryozen is idle) and, optionally, the rotated `~/.cryozen-agent/logs/agent.log*`.
- For `--live` probes: real credentials in `CRYOZEN_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.cryozen-agent/logs/agent.log*"
...
Cryozen recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp CRYOZEN_HOME), main vs a branch or integration checkout:
...
The instrument behind #104284 / #104421. N concurrent real `AIAgent`
...
Reference results (2026-09-05/06): OpenRouter pinned 0/161; OpenRouter unpinned 9.8% collapse.
Use this to clear a new upstream before changing its default wire. `--ttl 1h` reproduces the 2× write price
...
Forensic lanes: five parallel Cryozen subagents (2026-09-04), rewritten here to take `--db`/`--root`
