...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), CRYOZEN_HOME=str(home / "cryozen"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("cryozen-gateway*", "cryozen-serve*")] + [unit + ".service"]
...
unit = f"cryozen-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
