...
measured in the production session DB) through `cryozen chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    CRYOZEN_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Cryozen home with credentials for the
...
(tracker: cryozenai/cryozen-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "CRYOZEN_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "cryozen_cli.main", "chat", "--query", q,
