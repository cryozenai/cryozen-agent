...
batch (tracker: [#77056](https://github.com/cryozenai/cryozen-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Cryozen home, same model, same tasks, same reps.
...
1. Create a dedicated Cryozen home with credentials for the models under test:
   ...
   integration at it; no Cryozen observability plugin needs to be enabled.
