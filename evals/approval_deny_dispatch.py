...
with tempfile.TemporaryDirectory(prefix="cryozen-deny-dispatch-") as directory:
    home = Path(directory)
    os.environ.clear()
    # The terminal's own NOPASSWD probe must also resolve to our inert fixture.
    os.environ.update(PATH=directory + ":/usr/bin:/bin", HOME=directory, CRYOZEN_HOME=directory,
                      CRYOZEN_INTERACTIVE="1", TERMINAL_ENV="local")
