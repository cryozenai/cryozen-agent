#!/usr/bin/env python3
"""Live test harness for Cryozen Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any cryozen imports.
ORIGINAL_HOME = os.environ.get("CRYOZEN_HOME")
ORIGINAL_AUTH = Path.home() / ".cryozen-agent" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.cryozen-agent/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.cryozen-agent/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="cryozen_ts_live_"))
    cryozen_home = home_dir / ".cryozen-agent"
    cryozen_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, cryozen_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".cryozen-agent" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, cryozen_home / ".env")
        ...
        load_cryozen_dotenv(cryozen_home=str(Path.home() / ".cryozen-agent"))
    ...
    (cryozen_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return cryozen_home
...
def reset_module_state():
    """Drop cached modules so the new CRYOZEN_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "cryozen_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original CRYOZEN_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CRYOZEN_HOME"] = ORIGINAL_HOME
    else:
