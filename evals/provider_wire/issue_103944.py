...
os.environ.clear()
os.environ.update(
    HOME=home, CRYOZEN_HOME=home + "/.cryozen-agent", PATH="/usr/bin:/bin", NO_PROXY="*"
)
os.chdir(home)
...
Path(os.environ["CRYOZEN_HOME"]).mkdir(parents=True, exist_ok=True)
# JSON is valid YAML, allowing the real config loader to read the seeded file.
(Path(os.environ["CRYOZEN_HOME"]) / "config.yaml").write_text(json.dumps(config))
from run_agent import AIAgent
from providers import get_provider_profile
from cryozen_cli.config import get_compatible_custom_providers, load_config
from cryozen_cli.config_providers import get_custom_provider_context_length
    ...
    json.dumps(
        {
            ...
            "cryozen_home": os.environ["CRYOZEN_HOME"],
            "captures": captures,
            "rows": rows,
