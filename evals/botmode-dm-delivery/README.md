...
Real Electron, production Python backend and tool execution, disposable HOME/CRYOZEN_HOME,
...
(cd apps/desktop && DISPLAY=:0 XAUTHORITY=/run/user/1000/xauth_cnpsqU \
  ...
  CRYOZEN_DESKTOP_CDP_PORT=off npx playwright test e2e/probe-dm-delivery.spec.ts --reporter=list)
...
fixture's generated cryozen shim pins every child to this checkout, not an installed launcher.
...
custom Cryozen root, and holds the real quiet CLI child at loopback inference.
...
creates a second `.cryozen-agent` root under HOME.
