...
Differences from the TypeScript original are format-only: Cryozen transcripts
...
def apply_decisions(messages, decisions: List[Decision], calls: List[ToolCall], head: int):
    """Rebuild the Cryozen transcript: dropped calls vanish with their result row,
...
def openrouter_asker(api_key: Optional[str] = None, model: str = OPENROUTER_JEV_MODEL,
    ...
    def ask(state, questions):
        ...
        req = urllib.request.Request(url, data=body, headers={
            ...
            "HTTP-Referer": "https://github.com/cryozenai/cryozen-agent", "X-Title": "cryozen compaction eval",
        })
        try:
