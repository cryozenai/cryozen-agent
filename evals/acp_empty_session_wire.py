...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='cryozen-acp-empty-'))
    cryozen = home / '.cryozen-agent'
    cryozen.mkdir()
    (cryozen / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), CRYOZEN_HOME=str(cryozen), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               CRYOZEN_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(cryozen / 'state.db') as db:
