...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["CRYOZEN_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(CryozenTokenStorage("positive", cryozen_home=owner).get_tokens())
        ...
        override = set_cryozen_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from cryozen_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="cryozen-oauth-http-") as temp:
            home = Path(temp)
            (home / ".cryozen-agent").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "CRYOZEN_HOME": str(home / ".cryozen-agent"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
