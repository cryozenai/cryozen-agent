...
Each probe runs in a FRESH subprocess with an isolated CRYOZEN_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "CRYOZEN_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "CRYOZEN_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("CRYOZEN_SESSION", "CRYOZEN_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "cryozen_cli.main", "gateway.run", "tools.registry", "cryozen_state", "tui_gateway.server", "cryozen_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "cryozen-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `cryozen --version`, `cryozen --help`, `cryozen doctor --help`, `cryozen config get model` (no network)
CLI = {"version": ["cryozen_cli/main.py", "--version"], "help": ["cryozen_cli/main.py", "--help"], "config_get": ["cryozen_cli/main.py", "config", "get", "model"], "tools_list": ["cryozen_cli/main.py", "tools", "--help"], "skills_help": ["cryozen_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_cryozen_default_ms", lambda: resolve_toolset("cryozen-default"), 200)
...
try:
    import cryozen_state, tempfile, uuid
    from pathlib import Path
    db=cryozen_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(cryozen_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
