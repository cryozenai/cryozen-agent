"""Run the read-tool eval through the REAL Cryozen AIAgent.

For each task: fresh temp CRYOZEN_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    cryozen_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".cryozen-agent"
    ...
    try:
        ...
        shutil.rmtree(cryozen_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.cryozen-agent/.env; set +a  — then relaunch."
        )
