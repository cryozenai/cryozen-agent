...
Run from the repo with a clean environment and a temporary CRYOZEN_HOME:
...
from cryozen_cli import heartbeat  # noqa: E402
...
async def main(base_poller):
    assert os.environ.get("CRYOZEN_HOME"), "Use a temporary CRYOZEN_HOME"
    ...
    with patch.object(heartbeat, "time", SimpleNamespace(time=lambda: clock.now)):
        ...
        if not base_poller:
            ...
            runner.session_store = SessionStore(
                sessions_dir=Path(os.environ["CRYOZEN_HOME"]) / "sessions", config=runner.config)
