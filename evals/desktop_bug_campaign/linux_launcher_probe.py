...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "CRYOZEN_HOME": str(root / "home/.cryozen-agent"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "CRYOZEN_NONINTERACTIVE": "1"}
    Path(env["CRYOZEN_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from cryozen_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'cryozen')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,cryozen_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,cryozen_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Cryozen\n", b"Name=Cryozen custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["CRYOZEN_HOME"]) / "config.yaml"
