...
env.update(HOME=str(home), CRYOZEN_HOME=str(home), CRYOZEN_DASHBOARD_SESSION_TOKEN='history-fixture-token', PYTHONPATH=repo, CRYOZEN_NONINTERACTIVE='1')
cmd=[sys.executable, '-m', 'cryozen_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(args.port), '--isolated']
...
try:
    for _ in range(120):
        try:
            req=urllib.request.Request(f'http://127.0.0.1:{args.port}/api/status',headers={'X-Cryozen-Token':'history-fixture-token'})
