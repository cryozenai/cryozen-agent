"""
Cryozen MCP Server — expose messaging conversations as MCP tools (`cryozen mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Cryozen-specific
channels_list. Client config: {"mcpServers": {"cryozen": {"command": "cryozen", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("cryozen.mcp_serve")
...
def _cryozen_home() -> Path:
    try:
        ...
        return Path(os.environ.get("CRYOZEN_HOME", Path.home() / ".cryozen-agent"))


def _get_sessions_dir() -> Path:
    return _cryozen_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_cryozen_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_cryozen_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Cryozen analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Cryozen MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("cryozen", instructions=(
        "Cryozen Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Cryozen MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
