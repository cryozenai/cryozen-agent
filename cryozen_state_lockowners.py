...
open-descriptor scan (``cryozen_state_holders``) cannot tell a reader from the one writer: every
Cryozen process has the DB open. SQLite's unix VFS takes ``fcntl`` byte-range locks whose offsets
