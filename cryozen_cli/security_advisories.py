"""Security advisory checker for Cryozen Agent.
...
class Advisory:
    ...
    ``{cryozen_home}``; ``full_remediation_text`` fills it with ``display_cryozen_home()`` at render
    time so a profile / CRYOZEN_HOME user is sent to their own .env.
    """
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including cryozen when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in {cryozen_home}/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: cryozen doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'cryozen doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
def full_remediation_text(hit: AdvisoryHit) -> list[str]:
    ...
    return [
        ...
        "",
        "Remediation:",
        *(f"  {i}. {step.format(cryozen_home=display_cryozen_home())}" for i, step in enumerate(a.remediation, 1)),
    ]


# ─── Startup-banner gating ────────────────────────────────────────────────────
# Once the banner is seen we cache that in ``~/.cryozen-agent/cache/advisory_banner_seen`` (one
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_cryozen_home()) / "cache"
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active (IDs: {', '.join(h.advisory.id for h in fresh)}). "
            "Run `cryozen doctor` on the gateway host for details.")
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``cryozen doctor``.
