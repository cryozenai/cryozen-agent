...
Pure leaf: imports nothing from ``cryozen_cli.auth`` so the per-provider modules
(``auth_codex``, ``auth_xai``, ...) can import it at module scope without cycles."""
...
# httpx is imported lazily (~30ms) because cryozen_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("cryozen_cli.auth.httpx.Client")
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from cryozen_cli import __version__ as _CRYOZEN_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _CRYOZEN_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"cryozen-cli/{_CRYOZEN_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://cryozenai.github.io/cryozen-agent/docs/user-guide/features/spotify"
...
OAUTH_OVER_SSH_DOCS_URL = "https://cryozenai.github.io/cryozen-agent/docs/guides/oauth-over-ssh"
