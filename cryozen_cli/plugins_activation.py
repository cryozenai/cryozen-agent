...
def activate_plugin_now(name: str, *, in_process: bool = True) -> Dict[str, Any]:
    ...
    in another process (``cryozen plugins install``) passes ``in_process=False``; the running Desktop /
    ...
    if not in_process:
        activation = (notify_serve_backend(name, Path(get_cryozen_home())) or {}).get("activation")
    answer = None
    try:
        ...
        if answer is None:
            root = Path(get_default_cryozen_root())
...
def load_and_go_live(name: str) -> Optional[Dict[str, Any]]:
    ...
    if activation is None:
        return None
    from cryozen_cli.plugins_activation_live import connect_plugin_mcp, live_notice, plugin_skills
    ...
    if server is not None and (servers or note):
        from cryozen_constants import get_cryozen_home
        try:
            server.refresh_plugin_sessions(Path(get_cryozen_home()), note)
        except Exception:
...
def notify_serve_backend(name: str, home: Path) -> Optional[Dict[str, Any]]:
    """Ask the running Desktop / dashboard backend (``cryozen serve``, found through its host record) to
    ...
    try:
        ...
        request = urllib.request.Request(
            ...
            headers={"Content-Type": "application/json", "X-Cryozen-Session-Token": token})
