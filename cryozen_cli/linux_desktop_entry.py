"""Install and remove the Linux desktop entry (``cryozen.desktop``).
...
DESKTOP_ENTRY_NAME = "cryozen.desktop"
...
READY_FD_ENV_VAR = "CRYOZEN_DESKTOP_READY_FD"
...
def _can_import_cryozen_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``cryozen_cli.main`` unaided.
    ...
    ok = _run_quiet(
        [key, "-I", "-c", "import cryozen_cli.main"],
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``cryozen_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``cryozen desktop``.

    Prefer the real ``cryozen`` launcher; fall back to ``<python> -m cryozen_cli.main desktop``.
    """
    from cryozen_cli.relaunch import resolve_cryozen_bin

    bin_path = _resolve_cryozen_bin_for_desktop_entry(resolve_cryozen_bin, checkout_root=project_root)
    interpreter = _running_interpreter()
    if not _can_import_cryozen_cli(Path(interpreter)):
    ...
    argv = [interpreter, "-m", "cryozen_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `cryozen` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `cryozen` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m cryozen_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_cryozen_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`cryozen_cli.relaunch.resolve_cryozen_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from cryozen_cli.relaunch import resolve_cryozen_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/cryozen, a venv console script) wins
    ...
    # /opt/.../bin/cryozen from another install method, or a venv console script). It must be evaluated
    ...
    # ``cryozen``) must NOT return None here: that skipped the durable-wrapper probe below and persisted
    # the module form, so the entry's bytes flipped on every alternating launch context — and
    # gnome-shell 50.x crashes when cryozen.desktop changes while its ShellApp is STARTING (#110885).
    ...
    # every flip rewrites cryozen.desktop, which arms the gnome-shell 50.x crash this function's
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``). The wrapper is always named ``cryozen``.
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "cryozen")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/cryozen"))
    candidates.append(Path.home() / ".local" / "bin" / "cryozen")
    return candidates
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Cryozen\n"
        "GenericName=Cryozen Desktop\n"
        "Comment=Launch Cryozen Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Cryozen\n"
    )
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "cryozen.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=cryozen`` resolves without an
...
def _launcher_entry_management_enabled() -> bool:
    ...
    rewrite: a hand-edited ``cryozen.desktop`` is then left alone instead
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    ...
    icon_value = str(icon) if icon.is_file() else "cryozen"
...
class DeferredDesktopEntryInstall:
    ...
    The launcher hands Electron the write end of a pipe (``CRYOZEN_DESKTOP_READY_FD``); Electron
