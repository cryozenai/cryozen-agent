...
Split out of :mod:`cryozen_cli.plugins`. Names that tests patch on the origin
...
from cryozen_cli.plugin_capabilities import VALID_CAPABILITY_IDS
from cryozen_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
from cryozen_cli.plugins_manifest import (
...
ENTRY_POINTS_GROUP = "cryozen_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "cryozen_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    ``cryozen_agent.plugin_capabilities`` group (``<plugin-id>.<capability-id>`` entries pointing at the same
...
def collect_directory_manifests() -> List[PluginManifest]:
    ...
    from cryozen_cli import plugins as _origin  # patched names resolve through the origin
    manifests: List[PluginManifest] = []
    ...
    # which is the key `cryozen plugins enable/disable` and the dashboard write (#27548); the manifest name
    ...
    user_dir = get_cryozen_home() / "plugins"
    logger.debug("Scanning user plugins: %s", user_dir)
    _scan("user", user_dir, "user")
    if _origin._env_enabled("CRYOZEN_ENABLE_PROJECT_PLUGINS"):
        project_dir = Path.cwd() / ".cryozen-agent" / "plugins"
        logger.debug("Scanning project plugins: %s", project_dir)
        _scan("project", project_dir, "project")
    else:
        logger.debug("Project plugins disabled (set CRYOZEN_ENABLE_PROJECT_PLUGINS=1 to enable)")
    return manifests
...
def resolve_manifest_winners(manifests: List[PluginManifest]) -> Dict[str, PluginManifest]:
    ...
    ``~/.cryozen-agent/plugins/<name>`` is the documented way to override a bundled plugin, and is logged. A flat
    ...
    ``cryozen plugins enable kanban`` never activates unrelated code under the bundled name."""
...
def gate_manifest(
    ...
    if names & LEGACY_RELAY_PLUGIN_KEYS:
        error = (
            "removed — Relay lifecycle is owned by Cryozen core; configure "
            f"{RELAY_PLUGINS_CONFIG_ENV} instead"
        )
        return _placeholder(error, logging.WARNING, "Refusing to load removed Cryozen Relay plugin '%s'; %s", error)
    if names & disabled:
    ...
    if manifest.source == "bundled":
        ...
        # Bundled platforms register LAZILY: eagerly importing ~20 heavy SDKs added seconds to every `cryozen`
    ...
    if enabled is None or not names & enabled:
        return _placeholder(
            f"not enabled in config (run `cryozen plugins enable {lookup_key}` to activate)", logging.DEBUG,
    ...
    if manifest.source != "bundled":
        ...
        from cryozen_cli.plugins_cmd_catalog import installed_plugin_removal
        removed = installed_plugin_removal(manifest.name, manifest.path)
        if removed is not None:
            error = f"removed from the Cryozen plugin catalog: {removed.reason or 'no reason recorded'}"
            return _placeholder(error, logging.WARNING, "Refusing to load plugin '%s' — %s; run `cryozen plugins remove`",
                                error)
