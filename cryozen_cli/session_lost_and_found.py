...
from cryozen_cli.session_schema_history import SCHEMA_HISTORY, reachable_physical_layouts

from cryozen_state_ids import SESSION_ID_PATTERN  # timestamp prefix: strongest sentinel for schema-less rows
...
# 3.50.7 and 3.44.6 — the same version gate cryozen_state applies to the
...
# caught exactly this shell converting a live Cryozen state.db into two
# generations. A salvage shell must therefore be version-gated, not just
# capability-gated, before it is pointed at (a copy of) a Cryozen database.
#
# The predicate lives in cryozen_cli.sqlite_runtime (stdlib-only, shared with
# the installer/update gates) so the embedded runtime and the salvage shell
# can never disagree about which versions are safe.
from cryozen_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _wal_reset_vulnerable  # noqa: E502

_WAL_RESET_VULNERABLE_GUIDANCE = (
    "salvage against a Cryozen database with the WAL-reset bug "
...
def _cli_supports_recover(binary: str) -> bool:
    ...
    scratch_dir = tempfile.mkdtemp(prefix="cryozen-recover-probe-")
...
def _cli_recover_attempts(source: Path, lf_path: Path, sqlite3_bin: str, *, timeout: float) -> list[dict[str, Any]]:
    ...
    for command in (".recover --ignore-freelist", ".recover"):
        ...
        with tempfile.TemporaryFile(prefix="cryozen-recover-dump-stderr-") as dump_stderr:
...
def _type_conflicts(value: Any, declared: str) -> bool:
    ...
    non-numeric string in an INTEGER column, but Cryozen never writes one, so
...
# The closed sets SessionDB writes (cryozen_state.py: request/claim/complete/
