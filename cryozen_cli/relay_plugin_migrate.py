"""Migrate legacy ``CRYOZEN_NEMO_RELAY_ATIF_*`` / ``ATOF_*`` exporter vars into a Relay ``plugins.toml``.

The Relay cutover (Aug 2026) stopped honouring the legacy exporter variables: a profile that still
carries them and no ``CRYOZEN_NEMO_RELAY_PLUGINS_TOML`` logs one warning and initialises NO exporters,
...
the file is exactly what Relay validates), points ``CRYOZEN_NEMO_RELAY_PLUGINS_TOML`` at it, and
comments the legacy lines out. It runs from ``cryozen update`` for every profile home and from
``cryozen relay migrate`` for the active one.
"""
...
from cryozen_cli.relay_plugin_cutover import (
...
def relay_plugin_payload_from_legacy_env(env: Mapping[str, Any]) -> dict[str, Any]:
    ...
    atif = atof = None
    if _truthy(env.get("CRYOZEN_NEMO_RELAY_ATIF_ENABLED")):
        kwargs: dict[str, Any] = {
            "enabled": True,
            "agent_name": _s(env, "CRYOZEN_NEMO_RELAY_ATIF_AGENT_NAME", "Cryozen Agent"),
            "model_name": _s(env, "CRYOZEN_NEMO_RELAY_ATIF_MODEL_NAME", "unknown"),
            "filename_template": _s(env, "CRYOZEN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE", "cryozen-atif-{session_id}.json"),
        }
        if _s(env, "CRYOZEN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"):
            kwargs["output_directory"] = _s(env, "CRYOZEN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY")
        if _s(env, "CRYOZEN_NEMO_RELAY_ATIF_AGENT_VERSION"):
            kwargs["agent_version"] = _s(env, "CRYOZEN_NEMO_RELAY_ATIF_AGENT_VERSION")
        atif = obs.AtifConfig(**kwargs)
    if _truthy(env.get("CRYOZEN_NEMO_RELAY_ATOF_ENABLED")):
        mode = _s(env, "CRYOZEN_NEMO_RELAY_ATOF_MODE", "append").lower()
        sink = obs.AtofFileSinkConfig(
            output_directory=_s(env, "CRYOZEN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY") or None,
            filename=_s(env, "CRYOZEN_NEMO_RELAY_ATOF_FILENAME", "cryozen-atof.jsonl"),
...
def migrate_profile_relay_env(home: Path, *, validate: bool = True) -> RelayMigrationResult:
    ...
    from cryozen_cli.config import _env_line_defines_key, _quote_env_value, _read_env_lines, _write_env_lines
    ...
    if env.get(RELAY_PLUGINS_CONFIG_ENV, "").strip():
        ...
        return result
    if not (_truthy(env.get("CRYOZEN_NEMO_RELAY_ATIF_ENABLED")) or _truthy(env.get("CRYOZEN_NEMO_RELAY_ATOF_ENABLED"))):
    ...
    header = (
        "# NeMo Relay plugin configuration for Cryozen (selected via "
        f"{RELAY_PLUGINS_CONFIG_ENV} in .env).\n"
        "# Generated by `cryozen update` from the legacy CRYOZEN_NEMO_RELAY_ATIF_*/ATOF_* variables,\n"
...
def migrate_all_profile_relay_envs(*, validate: bool = True) -> list[RelayMigrationResult]:
    ...
    from cryozen_cli.profiles import _get_default_cryozen_home, _iter_named_profile_dirs
    homes = [_get_default_cryozen_home(), *_iter_named_profile_dirs()]
...
def print_relay_migration_report(results: list[RelayMigrationResult]) -> None:
    """Loud, actionable notice for `cryozen update` / `cryozen relay migrate`."""
    ...
    if migrated:
        ...
        print("   The legacy CRYOZEN_NEMO_RELAY_ATIF_*/ATOF_* variables stopped producing traces after the")
...
def run_relay_migration_after_update() -> None:
    """`cryozen update` hook: migrate every profile home, print the notice. Best-effort by contract."""
    print_relay_migration_report(migrate_all_profile_relay_envs())


RELAY_MIGRATE_COMMAND = "cryozen migrate relay"


def cmd_migrate_relay(args) -> None:
    """``cryozen migrate relay [--all-profiles] [--no-validate]``."""
    ...
    if getattr(args, "all_profiles", False):
        ...
        results = [migrate_profile_relay_env(get_cryozen_home(), validate=validate)]
