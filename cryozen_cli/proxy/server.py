...
from cryozen_cli.proxy.sse_done import DONE_SSE_FRAME, SseDoneTracker, content_type_is_sse
...
def _require_aiohttp() -> None:
    if not AIOHTTP_AVAILABLE:
        raise RuntimeError("aiohttp is required for `cryozen proxy`. Run `cryozen setup` to install it.")
...
def create_app(adapter: UpstreamAdapter) -> "web.Application":
    ...
    Every adapter method is synchronous and blocking (an adapter may take the cross-process
    ``_auth_store_lock()`` and POST a token refresh; xAI rotates its key pool under a lock),
    ...
    async def handle_proxy(request: "web.Request") -> "web.StreamResponse":
        ...
        if upstream_resp.status in {401, 429}:
            # One-shot retry with a refreshed/rotated credential (xAI: pool rotation).
            try:
