...
DeepInfra pricing adapters. Split out of ``cryozen_cli.models``; helpers still defined there are
looked up on ``cryozen_cli.models`` at call time so ``patch("cryozen_cli.models.<name>")`` mocks keep
intercepting.
"""
...
from cryozen_cli.models_reasoning_caps import _seed_reasoning_caps
...
def _pricing_entry(pricing: dict, prompt_key: str = "prompt", completion_key: str = "completion") -> dict[str, Any]:
    ...
    ``pricing`` block whose cache fields already use the cryozen names."""
    entry: dict[str, Any] = {
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    headers = {"Accept": "application/json", "User-Agent": _CRYOZEN_USER_AGENT}
    if api_key:
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            result[mid] = _pricing_entry(pricing)
...
def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from cryozen_cli.models import _pricing_profile_key
...
# Static endpoint identity per provider; dynamic ones (deepinfra) are resolved in pricing_cache_scope.
...
def pricing_cache_scope(provider: str) -> str:
    ...
    from cryozen_cli.models import _deepinfra_catalog_url, normalize_provider
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from cryozen_cli.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
...
def _fetch_novita_pricing(timeout: float = 8.0, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "Accept": "application/json", "User-Agent": _CRYOZEN_USER_AGENT}
