"""``cryozen debug`` debug tools for Cryozen Agent."""
...
_REDACTION_BANNER = (
    "[cryozen debug share: log content redacted at upload time. "
...
_USER_AGENT = "cryozen-agent/debug-share"
...
# ``cryozen debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
# leaked ~20 MB per share.


def _pending_file() -> Path:
    return get_cryozen_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    try:
        ...
        pass  # non-fatal — worst case the user runs ``cryozen debug delete`` manually
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `cryozen debug share` "
...
def _upload_dpaste_com(content: str, expiry_days: int = 1) -> str:
    boundary = "----CryozenDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from cryozen_cli.logs import LOG_FILES
    filename = LOG_FILES.get(log_name)
    return (get_cryozen_home() / "logs" / filename) if filename else None
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Cryozen Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``cryozen dump`` and return its stdout as a string."""
    from cryozen_cli.dump import run_dump
    capture = io.StringIO()
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``cryozen dump`` is run internally.
    """
    buf = io.StringIO()
...
def build_debug_share(
    ...
    Shared by ``cryozen debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
    ...
    if redact:
        logger.info(
            "cryozen debug share: applied force-mode redaction to log snapshots before upload")
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `cryozen debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    if dpaste_urls:
        ...
        print(f"⚠️  {len(dpaste_urls)} of {len(result.urls)} upload(s) fell back to "
              ...
              "deleted with `cryozen debug delete`.\n"
              "\nShare these links with the Cryozen team for support.")
    else:
        print(f"\n⏱  Pastes will auto-delete in {result.auto_delete_seconds // 3600} hours.\n"
              "To delete now:  cryozen debug delete <url>\n"
              "\nShare these links with the Cryozen team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: cryozen debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'cryozen debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
