"""Host-platform checks for cryozen doctor: interpreter, SQLite, certificates, macOS TCC, gateway supervision, command install.
Split out of ``cryozen_cli/doctor.py``, which re-exports every name so ``cryozen_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from cryozen_cli.config import is_nix_install_method, recommended_update_command_for_method
from cryozen_cli.doctor_report import (
    Finding, _fail_and_issue, _section, check_bool, check_fail, check_info, check_ok, check_warn, doctor_check,
    warn_on_error,
)
from cryozen_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from cryozen_cli.doctor import PROJECT_ROOT
    from cryozen_cli.config import detect_install_method
    ...
    action = cmd if is_nix_install_method(method) else {  # nix: prose guidance, not a shell command
        "docker": f"run `{cmd}`, then recreate all Cryozen containers", "apt": f"run `{cmd}`"}.get(method, "run `cryozen update`")
    return f"({action}; fixed versions: 3.51.3+ / 3.50.7 / 3.44.6 — see https://sqlite.org/wal.html#walresetbug)"


def _cryozen_database_paths(cryozen_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for Cryozen-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from cryozen_cli.backup import _QUICK_STATE_FILES
    entries = [(name, cryozen_home / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
    for board_db in sorted((cryozen_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(cryozen_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``cryozen_cli.sqlite_safe_read``), and the dashboard console runs
    ``run_doctor`` in-process with live ``SessionDB`` connections — the helper refuses then (unreadable).
    """
    from cryozen_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
...
def _report_database_holders(name: str, db_path: Path) -> None:
    ...
    from cryozen_state_holders import describe_holder_pid, foreign_state_db_holders
...
def _report_database_journal_modes(cryozen_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    ...
    from cryozen_cli.doctor import CRYOZEN_HOME
    ...
    try:
        databases = _cryozen_database_paths(cryozen_home if cryozen_home is not None else CRYOZEN_HOME)
    except Exception as exc:
        check_warn(f"Could not list Cryozen databases: {exc}")
        return
    exposed = []
    for name, path in databases:
        ...
        if error is None and mode == "wal" and configured == "delete":
            ...
            check_warn(f"{name} is in WAL mode ({size}) despite database.journal_mode=delete",
                       ...
                       + ". Stop every Cryozen process for this profile, then run "
                       f"`cryozen sessions set-journal-mode delete{'' if name == 'state.db' else f' --db {path}'}`)")
            ...
            check_warn(f"{name} is in WAL mode on a cross-VM filesystem (virtiofs/9p, {size})",
                       "(WAL can silently corrupt across the VM boundary; stop every Cryozen process and run "
                       f"`cryozen sessions set-journal-mode delete{'' if name == 'state.db' else f' --db {path}'}`, then "
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs cryozen_cli.__version__ drift (a conflict resolution can revert one but not the
    other). Silent no-op for installed wheels (no pyproject)."""
    try:
        from cryozen_cli import __version__ as init_version
    except Exception:
        return
    ...
    _fail_and_issue("Version mismatch between source files", f"(pyproject.toml {pyproject_version} != cryozen_cli/__init__.py {init_version})",
                    "Re-sync version files (e.g. run 'cryozen update', or set cryozen_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from cryozen_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in ("main-cryozen", "dashboard"):  # s6-rc symlinks under /run/service/, same s6-svstat probe
...
def _report_host_gateway_slot(mgr, issues: list[str]) -> None:
    ...
    if topology is None:
        if not slots:
            return check_info("No gateway registered yet — run `cryozen gateway install`")
        ...
        issues.append("No host gateway owns the gateway role — start the ONE host multiplexer: "
                      "cryozen --profile default gateway start")
    ...
    if legacy_up:
        ...
        issues.append("Fold the legacy per-profile gateways into the host gateway: "
                      "cryozen --profile default gateway migrate --multiplex")
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        issues.append(f"Repair the CA bundle: run `cryozen doctor --fix`, or `{pip_cmd}`")
        return
...
_TCC_CDHASH_DETAIL = (
    ...
    "all permission grants. Run `cryozen update` to get the stable identifier-pinned signing identity, "
    "then re-grant permissions once.")
_TCC_STABLE_DETAIL = {
    True: "(certificate-anchored DR; grants survive rebuilds)",
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `cryozen desktop --setup-tcc-identity`)",
}
...
def check_macos_tcc_grants() -> None:
    ...
    check_info("If macOS still re-prompts for permissions (toggle shows ON): the stored grant is stale — run "
               "`tccutil reset ScreenCapture com.cryozen.cryozen` (repeat per affected service), toggle it ON in "
               "System Settings, then fully quit & relaunch Cryozen once.")


def _desktop_app_bundle() -> Path | None:
    """Locate the locally-built desktop bundle (``apps/desktop/release/mac-<arch>/Cryozen.app``), newest first.

    The only layout whose ad-hoc re-signed bundle can invalidate TCC grants. ``/Applications/Cryozen.app`` is
    deliberately not probed: it is the separately-signed, certificate-anchored Cryozen-Setup launcher.
    """
    release_dir = Path(__file__).resolve().parents[1] / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Cryozen.app") if p.is_dir()]
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from cryozen_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        check_info("One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and Cryozen "
                   ...
                   "    then enable your terminal (and Cryozen.app if you use Desktop), and restart them once. "
                   "With Cryozen' stable signing identities the grant survives every update.")
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from cryozen_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
    ...
    for hit in fresh_hits:
        ...
        _fail_and_issue(f"{hit.advisory.title}", f"({hit.package}=={hit.installed_version})",
                        ...
                        f"and rotate credentials, then run `cryozen doctor --ack {hit.advisory.id}`.", f.manual_issues)
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from cryozen_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        src = sqlite_source_id()
        # Warn-only: Cryozen already refuses WAL on fresh DBs and runtime repair is best-effort.
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next((c for c in (PROJECT_ROOT / n / "bin" / "cryozen" for n in ("venv", ".venv")) if c.exists()), None)
    if venv_bin is None:
        check_warn("Venv entry point not found", "(cryozen not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
    ...
    link = link_dir / "cryozen"
    if link.is_symlink():
        target, expected = link.resolve(), venv_bin.resolve()
        if target == expected:
            return check_ok(f"{display}/cryozen → correct target")
        check_warn(f"{display}/cryozen points to wrong target", f"(→ {target}, expected → {expected})")
        if not should_fix:
            return f.issues.append(f"Broken symlink at {display}/cryozen — run 'cryozen doctor --fix'")
        link.unlink()
        verb = "Fixed"
    elif link.exists():  # regular file (wrapper script), not a symlink
        return check_ok(f"{display}/cryozen exists (non-symlink)")
    else:
        check_fail(f"{display}/cryozen not found", "(cryozen command may not work outside the venv)")
        if not should_fix:
            return f.issues.append(f"Missing {display}/cryozen symlink — run 'cryozen doctor --fix'")
        link_dir.mkdir(parents=True, exist_ok=True)
        verb = "Created"
    link.symlink_to(venv_bin)
    check_ok(f"{verb} symlink: {display}/cryozen → {venv_bin}")
    f.fixed += 1
