...
from cryozen_cli.web_server_memory import _normalize_memory_provider_name
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `cryozen egress setup` "
            "and `cryozen egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "auth.adopt_external_logins": {
        "type": "boolean",
        "description": (
            "Borrow and refresh the Codex CLI login when Cryozen has no usable login of its own. "
            "Off: Cryozen uses only its own logins (`cryozen auth add <provider>`)."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": _select(
        "When the chat app / gateway updates Cryozen (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during cryozen update. "
...
# telegram) currently surface a single schema field each.
...
def _schema_with_dynamic_provider_options() -> Dict[str, Dict[str, Any]]:
    ...
    """
    from cryozen_cli.web_server_profiles import _plugin_terminal_backend_rows
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    1. Vendor-name → Cryozen-provider: when the provider is not a known provider/alias but the
    ...
    from cryozen_cli.models import _AGGREGATOR_PROVIDERS, _KNOWN_PROVIDER_NAMES, normalize_provider
    from cryozen_cli.model_normalize import normalize_model_for_provider
    from cryozen_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and not is_custom_provider_slug and "/" in model_in:
        ...
        if cur_provider and normalize_provider(cur_provider) in _AGGREGATOR_PROVIDERS:
            ...
            from cryozen_cli.models_detect import provider_has_credentials
...
def _validated_main_model_selection(
    ...
    if is_bare_custom and base_url.strip():
        ...
        # over the wrong wire.
        from cryozen_cli.providers import determine_api_mode
        url = base_url.strip()
...
def _apply_main_model_assignment(model_cfg: "Any", result: "ModelSwitchResult", api_key: str = "") -> dict:
    ...
    (``cryozen_cli.model_switch.apply_model_selection``). An explicit key for a custom endpoint is
    ...
    Returns a new dict."""
    from cryozen_cli.model_switch import apply_model_selection
...
# in cryozen_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    Long-lived dashboard / Desktop-owned ``cryozen serve`` processes freeze ``sys.modules``
    at boot; after ``cryozen update`` replaces the checkout, a first-time lazy import can
...
def _dashboard_skew_restart_hint() -> str:
    ...
    dashboard and Desktop-owned ``cryozen serve``; naming a systemd unit would mislead
    ...
    if os.environ.get("CRYOZEN_SERVE_HEADLESS") == "1":
        return (
            ...
            "(use Restart backend in Cryozen Desktop, or quit and reopen the app)"
        )
    return (
        "restart this Cryozen process to load the new code "
        "(cryozen dashboard --port <port>, or the equivalent service restart for this install)"
    )
...
def _register_custom_endpoint(base_url: str, api_key: str, model: str) -> None:
    ...
    ``cryozen model`` custom flow) so the picker gets a proper ready row instead of a "needs
    setup" dead-end. Dedups by base_url; never blocks the already-persisted assignment."""
    try:
        from cryozen_cli.main_provider_setup import _auto_provider_name, _save_custom_provider
...
def _infer_provider_on_model_change(model_val: str, prev_provider: str) -> tuple[str, str]:
    ...
    if not name:
        return "", name
    try:
        from cryozen_cli.models import _AGGREGATOR_PROVIDERS, detect_provider_for_model, normalize_provider
    except Exception:
        return "", name
