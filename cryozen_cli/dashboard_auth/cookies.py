...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``cryozen_session_at``
(access token; Max-Age = token TTL), ``cryozen_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``cryozen_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``cryozen_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``cryozen_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "cryozen_session_at"
SESSION_RT_COOKIE = "cryozen_session_rt"
SESSION_PROVIDER_COOKIE = "cryozen_session_provider"
PKCE_COOKIE = "cryozen_session_pkce"
SSO_ATTEMPT_COOKIE = "cryozen_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/cryozen`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    OIDC providers commonly issue a rotating refresh token (cryozen #37247); a provider that omits it returns
