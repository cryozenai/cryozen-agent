...
- **gateway profiles**, relaunched through the existing per-profile
  ``cryozen_cli.main -p <profile> gateway restart`` command; and
- **``cryozen-serve*`` systemd units**, restarted directly through
  ``systemctl``.  ``cryozen serve`` is not a gateway profile and has no
  ...
  inventory, so a manually launched or Desktop-owned ``cryozen serve`` — which
  has no relaunch authority — can never enter this path.

Serve-unit identity is always ``<scope>/<unit>`` (``user/cryozen-serve``,
``system/cryozen-serve``).  The two managers can each own a unit of the same
...
_RECOVERY_ENV = "CRYOZEN_UPDATE_RESTART_RECOVERY"
_GATEWAY_MARKERS = ("_CRYOZEN_GATEWAY", "CRYOZEN_GATEWAY", "CRYOZEN_GATEWAY_MODE")
_PROFILE_RESTART_TIMEOUT = 90
_VERIFY_TIMEOUT = 15
from cryozen_constants import PROFILE_ID_RE as _PROFILE_ID_RE

_SUPERVISOR_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,31}$")
_UNIT_RE = re.compile(r"^cryozen-serve(-[a-z0-9][a-z0-9_-]{0,63})?\.service$")
_SERVE_UNIT_PATTERN = "cryozen-serve*"
...
def _launch_profile() -> str:
    """Profile this recovery process was launched as: ``<root>/profiles/<name>`` in ``CRYOZEN_HOME`` names it,
    anything else is the default profile. Mirrors ``cryozen_cli.profiles.profile_root_for_env_home`` without
    importing it — this module stays stdlib-only at import time."""
    home = os.environ.get("CRYOZEN_HOME", "").strip()
    if home:
...
def _run_profile_restart(profile: str, *, run: Callable[..., Any]) -> bool:
    ...
    argv = [sys.executable, "-m", "cryozen_cli.main", "-p", profile, "gateway", "restart"]
...
def _systemd_unit_candidates(profile: str) -> tuple[str, ...]:
    ...
    if profile == "default":
        return ("cryozen-gateway.service", "gateway.service", "gateway-default.service")
    return (f"cryozen-gateway-{profile}.service", f"gateway-{profile}.service")
...
def _host_state_dir() -> str:
    ...
    This module imports no Cryozen code at runtime — importing the freshly pulled tree is exactly
    what aborted the phase that calls us — so the rule is duplicated here rather than shared.
    """
    override = os.environ.get("CRYOZEN_GATEWAY_LOCK_DIR")
    if override:
        return override
    ...
    return os.path.join(state_home, "cryozen", "gateway-locks")
...
def _systemctl_scopes() -> list[tuple[str, list[str]]]:
    ...
    ``systemctl`` comes from ``shutil.which`` so this module never imports a Cryozen platform helper —
...
def _listed_serve_units(scope: list[str], *, run: Callable[..., Any]) -> list[str]:
    ...
    if result is None:
        return []
    # The glob is a systemd pattern, not a name gate (`cryozen-serve*` also matches
    # `cryozen-server.service`): require the exact base unit or the profile family.
    units: list[str] = []
...
def _normalized_skips(skip_units: Iterable[Any]) -> tuple[set[tuple[str, str]], set[str]]:
    """Split already-settled units into scope-qualified and legacy entries (a bare ``"cryozen-serve"``
...
def restart_serve_units(
    *, skip_units: Iterable[Any] = (), run: Callable[..., Any] = subprocess.run, sleep: Callable[[float], Any] = time.sleep
    ...
    """Restart every active ``cryozen-serve*`` systemd unit from this process.

    Units are enumerated from systemd, never from the update inventory, so a manually launched or
    Desktop-owned ``cryozen serve`` (no unit) structurally cannot be touched here.
    """
...
def _parse_payload(stream) -> tuple[list[str], dict[str, str], bool, list[str]]:
    ...
    if raw_serve is not None:
        ...
        for entry in raw_skip:
            ...
            # scope (`cryozen-serve.service` can exist in BOTH managers; settling one says nothing about
