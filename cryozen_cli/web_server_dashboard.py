...
from cryozen_cli.config import cfg_get, get_process_cryozen_home
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    ``cryozen_cli.dashboard_auth.prefix`` so gate, OAuth, cookies and SPA mount agree)."""
    from cryozen_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    target palette instead of flashing the bundle's default Cryozen Teal until
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="cryozen-theme-bootstrap">'
                ":root{"
...
_HEADLESS_MSG = (
    "Headless backend (cryozen serve): web UI disabled — use "
    "`cryozen dashboard` for the browser UI."
)
...
def mount_spa(application: FastAPI):
    ...
    proxy (``X-Forwarded-Prefix: /cryozen``) the served index.html is rewritten so absolute
    asset URLs and the runtime ``__CRYOZEN_BASE_PATH__`` honour that prefix without a rebuild.
    ...
    """
    from cryozen_cli.web_server import WEB_DIST, _DASHBOARD_EMBEDDED_CHAT_ENABLED, app
    from cryozen_cli.web_deps import _server

    # `cryozen serve` is the headless backend: it must NEVER serve the browser SPA, even if a
    # dist is lying around, so only the JSON-RPC/WS/API surface is reachable.
    if os.environ.get("CRYOZEN_SERVE_HEADLESS") == "1":

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake: the Electron shell boots by fetching `/` and reading
            # ``window.__CRYOZEN_SESSION_TOKEN__`` for /api/ws auth. When headless 404'd every
            # path, a renderer whose spawn token no longer matched (e.g. after `cryozen update`)
            ...
            if full_path == "" and not gated:
                return HTMLResponse(
                    ...
                    f"window.__CRYOZEN_SESSION_TOKEN__={json.dumps(_server()._SESSION_TOKEN)};"
                    "window.__CRYOZEN_AUTH_REQUIRED__=false;"
    ...
    # A missing WEB_DIST is deliberately NOT a mount-time terminal state (#82614): a long-lived `cryozen
    ...
    def _serve_index(prefix: str = ""):
        ...
        cookie auth; ``__CRYOZEN_AUTH_REQUIRED__`` tells it which scheme to use for /api/pty
        ...
        token_js = "" if gated else f'window.__CRYOZEN_SESSION_TOKEN__="{_server()._SESSION_TOKEN}";'
        ...
        from cryozen_cli.web_server_profiles import serving_profile_name as _serving_profile_name
        serving_profile_js = json.dumps(_serving_profile_name()).replace("</", "<\\/")
        bootstrap_script = (
            f"<script>{token_js}"
            f"window.__CRYOZEN_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__CRYOZEN_BASE_PATH__="{prefix}";'
            f"window.__CRYOZEN_AUTH_REQUIRED__={'true' if gated else 'false'};"
            f"window.__CRYOZEN_INITIAL_PROFILE__={initial_profile_js};"
            f"window.__CRYOZEN_DASHBOARD_PROFILE__={serving_profile_js};"
            f"</script>"
        )
        if prefix:
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Cryozen Teal",         "description": "Classic dark teal — the canonical Cryozen look"},
    {"name": "default-large", "label": "Cryozen Teal (Large)", "description": "Cryozen Teal with bigger fonts and roomier spacing"},
    {"name": "cryozen-blue",  "label": "Cryozen Blue",        "description": "Light mode with vivid blue accents on a cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    ~/.cryozen-agent/, the same trust level as config.yaml. Empty asset values are dropped so a
...
def _discover_user_themes() -> list:
    ...
    Uses the process launch home, not ``get_cryozen_home()``, so a transient profile override
    from embedded chat does not hide themes under the server's own ``CRYOZEN_HOME``.
    """
    themes_dir = get_process_cryozen_home() / "dashboard-themes"
...
def _dashboard_plugin_search_dirs() -> List[tuple]:
    ...
    When the process itself is profile-scoped (``CRYOZEN_HOME=<root>/profiles/<name>``) the
    launch home has no ``plugins/`` — user plugins live in the cryozen root — so the default
    ...
    # context-local CRYOZEN_HOME override (e.g. embedded /chat under --open-profile). #87197: when the
    # process itself is profile-scoped (``--profile <name>`` sets ``CRYOZEN_HOME=<root>/profiles/<name>``),
    # the launch home is the profile directory, which has no ``plugins/`` — user plugins are installed in
    # the cryozen root (``~/.cryozen-agent/plugins``). Scan the default root as well (``get_default_cryozen_root()``
    # unwraps ``<root>/profiles/<name>`` → ``<root>`` and returns a custom ``CRYOZEN_HOME`` unchanged when it
    # *is* the root), mirroring how ``cryozen_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any) authoritative over same-named root
    # plugins.
    user_plugin_roots = [get_process_cryozen_home() / "plugins"]
    root_plugins = get_default_cryozen_root() / "plugins"
    ...
    # ``true`` / ``yes`` / ``on``) so the gate matches ``cryozen_cli/plugins.py`` and the documented user
    # contract.
    if env_var_enabled("CRYOZEN_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".cryozen-agent" / "plugins", "project"))
    return search_dirs
...
def _discover_dashboard_plugins() -> list:
    ...
    project plugin sources — same three sources as ``cryozen_cli.plugins``."""
    plugins = []
    seen_names: set = set()
    ...
    try:
        ...
        for tname in provides_tools:
            ...
            if cached_result is None:
                ...
                return True, f"cryozen auth {name}"
    except Exception:
        pass
    return False, ""
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    """
    from cryozen_cli.web_server_memory import _discover_memory_provider_statuses, _normalize_memory_provider_name
    from cryozen_cli.web_server import _get_dashboard_plugins
    from cryozen_cli.config import get_cryozen_home, load_config
    global _plugins_hub_cache, _plugins_hub_cache_expires_at
    now = time.monotonic()
    ...
    from cryozen_cli.plugins_cmd import (
    ...
    from cryozen_cli.plugins_cmd_catalog import removed_annotation
    from cryozen_cli.plugin_catalog import resolved_removed_entries
    ...
    plugins_root_resolved = (get_cryozen_home() / "plugins").resolve()
    ...
    for name, version, description, source, dir_str, key in _discover_all_plugins():
        # Same verdict as `cryozen plugins list` / the TUI hub: name+key aliases for the lists, bundled
...
def _plugin_api_mount_skip_reason(plugin: Dict[str, Any], enabled_set: set, disabled_set: set) -> Optional[str]:
    ...
    disable; project plugins (``./.cryozen-agent/plugins/``) ship with the CWD and are
...
def _mount_plugin_api_routes():
    ...
    (``./.cryozen-agent/plugins/``) ship with the CWD and are therefore attacker-controlled in any threat model
    ...
    """
    from cryozen_cli.web_server import _get_dashboard_plugins, app
    try:
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.cryozen-agent/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"cryozen_dashboard_plugin_{plugin['name']}"
