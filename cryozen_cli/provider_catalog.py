...
The provider list shown by ``cryozen model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`cryozen_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `cryozen model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``cryozen model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("cryozen_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("cryozen_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    CRYOZEN_OVERLAYS = _safe_import("cryozen_cli.providers", "CRYOZEN_OVERLAYS", {})
    try:
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = CRYOZEN_OVERLAYS.get(slug)
