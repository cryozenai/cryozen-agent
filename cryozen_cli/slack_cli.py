"""``cryozen slack manifest`` — generate the Slack app manifest JSON that registers every gateway
...
def _build_full_manifest(
    ...
    Other sections (OAuth scopes, socket mode) are sensible Cryozen defaults, tweakable in the Slack
    UI after pasting.
    """
    from cryozen_cli.commands_platforms import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Cryozen in threads and DMs."}
        ...
        features["agent_view"] = {"agent_description": "Chat with Cryozen in Slack Messages."}
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Cryozen agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    """Print or write a Slack app manifest JSON (flags documented in ``cryozen_cli/main.py``)."""
    name = getattr(args, "name", None) or "Cryozen"
    description = getattr(args, "description", None) or "Your Cryozen agent on Slack"
    ...
    def fail(msg: str) -> int:
        print(f"cryozen slack manifest: {msg}", file=sys.stderr)
        return 2
    ...
    if isinstance(write_target, bool) and write_target:  # bare --write → default location
        ...
        target = Path(get_cryozen_home()) / "slack-manifest.json"
    else:
    ...
    print(
        "\nNext steps:\n"
        "  1. Open https://api.slack.com/apps and pick your Cryozen app\n"
