...
Wraps the same config layer the CLI uses (cryozen_cli.mcp_config); stdio ``env``
...
def _normalize_mcp_server_create(body: MCPServerCreate) -> tuple[str, Dict[str, Any], Optional[str]]:
    ...
    """
    from cryozen_cli.mcp_config import _bearer_auth_headers, _strip_bearer_prefix
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.cryozen_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    from cryozen_cli.mcp_config import _oauth_tokens_present, _probe_single_server, _save_mcp_server
    try:
        from agent.secret_scope import build_profile_secret_scope, reset_secret_scope, set_secret_scope
        from cryozen_constants import reset_cryozen_home_override, set_cryozen_home_override
        from tools.mcp_dashboard_oauth import dashboard_oauth_flow
        from tools.mcp_oauth import CryozenTokenStorage, force_interactive_oauth, login_connect_timeout
        from tools.mcp_oauth_manager import get_manager

        home_token = set_cryozen_home_override(flow.cryozen_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.cryozen_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = CryozenTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(flow.server_name, cryozen_home=flow.cryozen_home)
                    ...
                    manager.restore_entry(flow.server_name, previous_entry, cryozen_home=flow.cryozen_home)
                    raise
        finally:
