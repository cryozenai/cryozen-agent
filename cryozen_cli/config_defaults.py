"""Default configuration data for Cryozen Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from cryozen_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["cryozen-cli"],
    # journal_mode: SQLite journal mode for every Cryozen DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `cryozen -c`: each CLI session writes a breadcrumb under
        # $CRYOZEN_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `cryozen
        ...
        # Cryozen-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Cryozen (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env CRYOZEN_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `cryozen doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`cryozen status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # CRYOZEN_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else CRYOZEN_HOME/cache/terminal (auto-pruned
        ...
        # (bytes); raise it for a ~/.cryozen-agent tree that legitimately exceeds 2 GiB.
        "sync_back_max_bytes": 2 * 1024 * 1024 * 1024,
        # HOME for host tool subprocesses: "auto" = host keeps the real OS-user HOME, containers use
        # CRYOZEN_HOME/home; "real" = force real HOME; "profile" = force CRYOZEN_HOME/home when it
        ...
        # ["/home/user/.cryozen-agent/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Cryozen image,
        # which drops to `cryozen` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `cryozen tools`, not a ring
        # member). Set by the `cryozen tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Cryozen-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Cryozen' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.cryozen-agent/browser-profile/ so credentials
        ...
        # runs `cryozen browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `cryozen chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.cryozen-agent/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `cryozen checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .cryozen.md,
    ...
    # Same bound for single-query mode (``cryozen -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Cryozen' summarizer cannot shrink it. native = codex decides; cryozen = Cryozen' threshold
        ...
        # scope it to one route ({"openai-codex:astra": 0.85} leaves Astra on OpenRouter at the
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `cryozen model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # `path`/`paths` = CRYOZEN_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/cryozen`.
        ...
        "runtime_footer": {
            ...
            # deployment a routing proxy reported / Cryozen' fallback route).
        ...
        # CLI/TUI/desktop, managed with `cryozen pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_cryozen_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: CRYOZEN_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than CRYOZEN_TUI_SESSION_TTL_S, default 6h) with
        ...
        # OAuth gate (engaged when --host is set and --insecure is not): the dashboard_auth/self_hosted
        # plugin reads oauth.self_hosted.{issuer, client_id, scopes, client_secret} (generic OIDC).
        "oauth": {
            "self_hosted": {"issuer": "", "client_id": ""},
        },
        ...
        # no-op. Env CRYOZEN_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # CRYOZEN_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env CRYOZEN_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/cryozen) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.cryozen-agent/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → Cryozen turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to Cryozen (any model / provider you have
    ...
    # "Hey Cryozen" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_cryozen" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.cryozen-agent/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.cryozen-agent/cache/delegation/ with a head+tail window + read_file offset footer, nothing
        ...
        # Total subagents a finite one-shot run (cryozen chat -q / --oneshot) may spawn; 0 = unlimited.
    ...
    "moa": {
        ...
        # <cryozen_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.cryozen-agent/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # CRYOZEN_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.cryozen-agent/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `cryozen skills trust` / `untrust`.
        ...
        # skip; CRYOZEN_IGNORE_RULES suppresses the list like the other auto-injected context.
        "auto_load": [],
        # Substitute ${CRYOZEN_SKILL_DIR} / ${CRYOZEN_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `cryozen skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.cryozen-agent/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.cryozen-agent/.curator_backups/blobs/); powers `cryozen
    ...
    # daemon. `cryozen curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `cryozen curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `cryozen update` restoring them);
        ...
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `cryozen curator purge`
        ...
        # ~/.cryozen-agent/skills/ to ~/.cryozen-agent/skills/.curator_backups/<utc-iso>/skills.tar.gz (`cryozen curator
    ...
    # peerName, sessions, enabled); cryozen-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "✻ *Cryozen Agent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (CRYOZEN_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export CRYOZEN_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. CRYOZEN_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `cryozen plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `cryozen plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.cryozen-agent/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # CRYOZEN_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "auth": {  # Login policy (credentials themselves live in auth.json / .env).
        # Borrow and refresh the Codex CLI (~/.codex/auth.json) login automatically when Cryozen has no
        # usable login of its own. Its refresh tokens are single-use and rotate, so two programs on one
        # login can log each other out; set false to make Cryozen use only
        # its own logins (`cryozen auth add <provider>`). `cryozen auth add openai-codex` still offers the import
        # interactively.
        "adopt_external_logins": True,
        # How `cryozen auth add openai-codex` / `cryozen model` sign in to OpenAI Codex.
        ...
        # to device code when that port is busy. `cryozen auth add openai-codex --browser` opts in
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .cryozen-agent config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `cryozen doctor --ack <id>`; remove by editing the list. Catalog:
        # cryozen_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $CRYOZEN_HOME/plugins/ <name>/).
        # An unknown or unavailable provider falls back to the built-in so cron never loses its
        # trigger.
        "provider": "",
        ...
        # Env override: CRYOZEN_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: CRYOZEN_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: CRYOZEN_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: CRYOZEN_CRON_MEDIA_SEND_TIMEOUT.
        ...
        # before one reminder ping (the run is still recorded; `cryozen cron incidents` shows it).
        # A green run or a different error alerts again immediately; `cryozen cron incidents ack`
    ...
    # promotes dependency-satisfied todos to ready, and fires `cryozen -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Per-home claim allowlist for boards shared across Cryozen homes (#110995): profile names
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `cryozen kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with cryozen-agent's own python
    ...
    "tools": {
        ...
        # manage_connections (local MCP server install / enable / authorize through the
        # approval card). The flag is the user's off switch.
        "connectors": {"enabled": True},
    },
    "logging": {  # File logging to ~/.cryozen-agent/logs/: agent.log captures INFO+, errors.log WARNING+.
        "level": "INFO",       # minimum level for agent.log: DEBUG, INFO, WARNING
        "max_size_mb": 5,      # max size per log file before rotation
        "backup_count": 3,     # rotated backups to keep
    },
    # Remote model-catalog manifest: curated OpenRouter model lists fetched from this
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://cryozenai.github.io/cryozen-agent/docs/api/model-catalog.json",
        # Disk cache TTL in minutes. The gateway refreshes in the background on this cadence; the
        # CLI refetches on the next /model or `cryozen model` once the cache is older. Network
    ...
    # into "text-only" and hide vision_analyze / reasoning controls (#112649). Provider keys: Cryozen
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "cryozen-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to CRYOZEN_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to CRYOZEN_STARTUP_WATCHDOG / CRYOZEN_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.cryozen-agent/sessions/sessions.json mirror of the routing index
        ...
        # `cryozen gateway migrate --multiplex` folds any per-profile fleet that is left.
        ...
        # adapter is parked; `cryozen profile create --clone` therefore leaves messaging channels
        # behind unless --clone-channels is passed.
        "multiplex_profiles": True,
        # May `cryozen update` fold this install onto a multiplexed default gateway by itself?
        ...
        # AUTOMATIC path reads this: `cryozen gateway migrate --multiplex` is explicit and proceeds.
        ...
        # (CRYOZEN_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # CRYOZEN_GATEWAY_MAX_STARTS / CRYOZEN_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.cryozen-agent/.env, auth.json). True:
        # files must be under the Cryozen cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to CRYOZEN_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Cryozen cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to CRYOZEN_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to CRYOZEN_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # CRYOZEN_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.cryozen-agent/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `cryozen sessions prune`).
        ...
        # stay until `cryozen sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `cryozen update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.cryozen-agent/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to CRYOZEN_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`cryozen sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `cryozen doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `cryozen update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <CRYOZEN_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``cryozen backup`` zip in
        # <CRYOZEN_HOME>/backups/ (``cryozen import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `cryozen update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `cryozen update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        # skipped. 0 = for the process lifetime (until `cryozen lsp restart`); N = retried after N
        ...
        "exclude_roots": [],
        # Missing server binaries: auto = install via npm/go/pip into <CRYOZEN_HOME>/lsp/bin/ on
        ...
        # <CRYOZEN_HOME>/lsp/node_modules; a configured manager that is not installed, or an unknown
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `cryozen tools`.
    "x_search": {
    ...
    # them in ~/.cryozen-agent/.env.
    # Browser credential vault: which login sources browser_vault_list/fill may draw from. The local
    # encrypted vault (`cryozen vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.cryozen-agent/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.cryozen-agent/cache/,
            ...
            # Auto-download bws into ~/.cryozen-agent/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Cryozen sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Cryozen also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.cryozen-agent/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Cryozen Desktop (Electron) launch options; only affect `cryozen desktop`.
        ...
        # Bridged to CRYOZEN_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to CRYOZEN_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "local_runtime": {
        # Off = detection-only (Cryozen still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Cryozen releases after validation.
        "tag": "b10964",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Cryozen mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'cryozen model' for endpoint-specific config)",
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Cryozen' wake word engine (optional; "
    ...
    "CRYOZEN_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "CRYOZEN_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "CRYOZEN_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Cryozen, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @cryozen:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. CRYOZEN_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #cryozen)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: cryozen-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'cryozen-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Cryozen API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Cryozen API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Cryozen API server (proxy mode). Must "
    ...
    # CRYOZEN_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. CRYOZEN_TOOL_PROGRESS is unsupported.
    "CRYOZEN_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "CRYOZEN_EPHEMERAL_SYSTEM_PROMPT": _setting(
