"""ZIP-download fallback for ``cryozen update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``cryozen_cli.update_cmd.<name>`` keeps
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = ".cryozen-update-staging", ".cryozen-update-old"
...
_ZIP_PRESERVED_NESTED = {
    ...
    "cryozen_cli": ("web_dist",),
    "scripts": ("whatsapp-bridge/node_modules",),
    "ui-tui": ("dist", "node_modules", "packages/cryozen-ink/dist"),
    "web": ("node_modules",),
}

_STASH_HINT = "  Stash or commit your changes, then rerun `cryozen update`."
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``cryozen_cli.main`` re-export surface.
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    staging = f"{dst}.cryozen-update-staging"
    backup = f"{dst}.cryozen-update-old"
...
def _zip_overlay_block_reason(
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.cryozen-update-staging`` siblings that git
...
def _is_zip_preserved_entry_status_line(line: str, shipped: Optional[Collection[str]] = None) -> bool:
    ...
    ship (``.bytecode-fingerprint``, ``.cryozen-bootstrap-complete``, ``cryozen_agent.egg-info/`` — the swap
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``cryozen-agent-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
    extracted = os.path.join(tmp_dir, f"cryozen-agent-{branch}")
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, cryozen_constants.py, ...). Check up front
...
def _link_or_copy_artifact(source: str, destination: str) -> None:
    ...
    valid after the swap unlinks the old tree; on Windows a link also succeeds on a locked Cryozen.exe
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    tmp_dir = tempfile.mkdtemp(prefix="cryozen-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"cryozen-agent-{branch}.zip")
        ...
        print("  Re-run `cryozen update` to retry; if the agent won't start, reinstall from https://cryozenai.github.io/cryozen-agent")
        _m().sys.exit(1)
    finally:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False, _windows_gateway_resume=None) -> bool:
    ...
    from cryozen_cli.update_cmd import _hand_off_post_swap, _m, _read_project_version, _resolve_update_options, _sweep_bytecode_after_update
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `cryozen update "
            f"--branch {branch}`, or update against main with `cryozen update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    _download_and_swap_zip(branch, f"https://github.com/cryozenai/cryozen-agent/archive/refs/heads/{branch}.zip")
...
def _finish_zip_update(
    *, active_tool_dependencies, pre_update_version, had_desktop_app_before_update: bool,
    ...
    if not import_ok:
        ...
        print("  Re-run `cryozen update` to complete it.")
        _m().sys.exit(1)
