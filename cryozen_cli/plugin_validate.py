"""``cryozen plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``CRYOZEN_HOME``), so a crashing or
...
from cryozen_cli.plugin_validate_desktop import check_desktop_surface
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "CRYOZEN_VALIDATE_JSON:"
...
def _requires_cryozen_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_cryozen`` spec.

    Unlike :func:`cryozen_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from cryozen_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_cryozen(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_cryozen") or "").strip()
    if not spec:
        report.add("requires_cryozen", True, "not declared")
        return
    if _requires_cryozen_spec_valid(spec):
        report.add("requires_cryozen", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_cryozen",
            False,
            f"requires_cryozen spec {spec!r} does not parse "
...
# from cryozen so a hostile plugin only sees a bare interpreter — the one
...
class RecordingContext:
    ...
    plugin_id = "cryozen_validate_probe_plugin"
...
def _run_capability_probe(plugin_dir: Path, manifest: dict) -> Tuple[Optional[dict], str]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix="cryozen-validate-") as scratch:
        env = dict(os.environ)
        env["CRYOZEN_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_cryozen(report, manifest)
...
def _check_loadable(report: ValidationReport, plugin_dir: Path) -> None:
    """A plugin.yaml with nothing beside it that Cryozen can load (no ``register()`` module, no
...
def _check_python_dependencies(report: ValidationReport, plugin_dir: Path) -> None:
    ...
    from cryozen_cli.plugin_python_deps import read_declaration
    ...
    if decl.external:
        ...
        return
    from cryozen_cli.plugin_python_deps import applicable_specs, unsupported_specs
