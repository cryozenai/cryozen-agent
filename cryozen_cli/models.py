...
``cryozen_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from cryozen_cli.urllib_security import open_credentialed_url
from cryozen_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from cryozen_cli.models_reasoning_caps import (
...
_CRYOZEN_USER_AGENT = f"cryozen-cli/{_CRYOZEN_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from cryozen_cli.config import get_custom_provider_tls_settings
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from cryozen_cli.model_catalog import refresh_interval_seconds
...
def _openrouter_catalog_disk_path() -> Path:
    ...
    return get_cryozen_home() / "cache" / "openrouter_curated_catalog.json"
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from cryozen_cli.model_catalog import get_default_model_from_cache
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (cryozen-agent is tool-calling-first).
    Permissive when the field is absent/malformed: some OpenRouter-compatible gateways (private
    mirrors) don't populate it, and the picker must not silently empty for them.
...
# Reasoning-capability cache slots for the OpenRouter catalog. The logic
...
def fetch_openrouter_models(
    ...
    from cryozen_cli.models_profile_cache import profile_slot_get, profile_slot_set
    ...
    try:
        from cryozen_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — cryozen-agent requires it and surfacing
...
def fetch_ai_gateway_models(
    ...
    from cryozen_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from cryozen_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from cryozen_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``cryozen model`` and ``/model``)."""
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Copilot, etc. all support /models)
...
def _resolve_static_model_alias(
    ...
    try:
        from cryozen_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # Step 0: a bare provider name typed as the model (`/model anthropic`) is a provider switch to that
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if resolved_provider in _PROVIDER_LABELS and default_models and resolved_provider not in current_keys:
            ...
            # so [0] would silently escalate `/model openrouter` to the priciest flagship.
...
def _resolve_provider_prefix(model_name: str) -> Optional[tuple[str, str]]:
    """Route an explicit ``vendor/model`` prefix (``deepinfra/deepseek-v4-pro``, ``ollama/qwen3.5:4b``) to
    a provider the user defined in ``providers:`` (by raw name or alias) instead of the default.

    ``deepinfra/deepseek-v4-pro`` or ``ollama/qwen3.5:4b`` should route to the named provider instead of falling
...
def detect_provider_for_model(
    ...
    NAMED the provider (``/model anthropic``), or there is no current provider yet (``auto``) — then the
    first guess is returned so the credential step fails loudly instead of silently ignoring input."""
    from cryozen_cli.models_detect import (
    ...
    # (Codex early-access ids, Ollama Cloud models absent from _PROVIDER_MODELS)
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``cryozen_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Cryozen should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from cryozen_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from cryozen_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from cryozen_cli.codex_models import get_codex_model_ids
    ...
    try:
        from cryozen_cli.auth import _codex_access_token_is_expiring, resolve_codex_runtime_credentials
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``cryozen model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Cryozen actually ships. (#46850)
    ...
    from cryozen_cli.providers import is_official_openai_host
...
def _azure_foundry_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    """
    try:
        from cryozen_cli.azure_detect import _probe_openai_models
...
def _profile_live_catalog(normalized: str) -> Optional[list[str]]:
    ...
    if profile.auth_type == "external_process":
        ...
        # `cryozen model` offer one list: live ids plus any pinned id the probe omitted.
...
# every authed provider's /v1/models). One JSON file at $CRYOZEN_HOME/provider_models_cache.json;
...
def _spawn_swr_refresh(cache_key: str, refresh_fn=None) -> None:
    ...
    from cryozen_constants import get_cryozen_home_override, cryozen_home_key
    inflight_key = cache_key if get_cryozen_home_override() is None else (cryozen_home_key(), cache_key)
...
def _provider_models_cache_path() -> Path:
    from cryozen_constants import get_cryozen_home
    return get_cryozen_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    if provider == "azure-foundry":
        try:
            from cryozen_cli.runtime_provider import _config_base_url_for_provider
    ...
    if provider == "openai-codex":
        from cryozen_cli.codex_models import codex_catalog_credential_identity
        ...
        try:
            ...
            for rel in ("auth.json", "credentials.json"):
                _mtime_part(rel, get_cryozen_home() / rel)
        except Exception:
            pass
        for rel in ("~/.codex/auth.json",
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``cryozen model --refresh``."""
    try:
...
def _resolve_anthropic_pool_catalog_credentials() -> tuple[str, str]:
    """Read-only API-key pool credential for model discovery (``resolve_anthropic_token()`` reads
...
def _fetch_anthropic_models(
    ...
    ``api_key``, else ``resolve_anthropic_token()`` (``ANTHROPIC_API_KEY``), else a read-only
    ...
    headers: dict[str, str] = {"anthropic-version": "2023-06-01", "x-api-key": token}
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _CRYOZEN_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"cryozen-agent/{_CRYOZEN_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=cryozen"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _CRYOZEN_USER_AGENT}
    try:
...
def cached_fetch_api_models(
    ...
    from cryozen_cli.model_switch_providers import _NativePickerModelList
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('cryozen_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('cryozen_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('cryozen_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'ensure_lmstudio_model_loaded': ('cryozen_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('cryozen_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('cryozen_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('cryozen_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('cryozen_cli.models_local', 'fetch_ollama_local_models'),
    'get_pricing_for_provider': ('cryozen_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('cryozen_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('cryozen_cli.models_local', 'lmstudio_model_reasoning_options'),
    'ollama_model_supports_thinking': ('cryozen_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('cryozen_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('cryozen_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('cryozen_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('cryozen_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('cryozen_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('cryozen_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('cryozen_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('cryozen_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'should_use_ollama_native_catalog': ('cryozen_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('cryozen_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('cryozen_cli.models_validate', 'validate_requested_model'),
    'warm_openrouter_reasoning_caps_async': ('cryozen_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
