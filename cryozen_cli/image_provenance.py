"""Image-authored deployment provenance for immutable Cryozen runtimes.

The image bakes ``/etc/cryozen/image-provenance.json`` outside ``$CRYOZEN_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/cryozen/image-provenance.json")
