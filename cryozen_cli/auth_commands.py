...
from cryozen_cli.auth_plugin_providers import (
...
_OAUTH_CAPABLE_PROVIDERS = {"openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth", "openrouter"}
# ...and default to it when ``--type`` is omitted. OpenRouter stays API-key-first: the documented
# ``cryozen auth add openrouter --api-key sk-or-...`` must keep working with no ``--type``.
_OAUTH_DEFAULT_PROVIDERS = _OAUTH_CAPABLE_PROVIDERS - {"openrouter"}
# Providers whose sibling CLI login Cryozen may borrow (``auth.adopt_external_logins``).
EXTERNAL_LOGIN_PROVIDERS = {"openai-codex"}
...
def _unknown_provider_exit(provider: str) -> SystemExit:
    ...
    return SystemExit(
        f"Unknown provider '{provider}'.{hint} Run `cryozen auth` to see the provider list, or "
        "`cryozen model` to pick one interactively.")
...
def _qwen_oauth_login(args) -> dict:
    from cryozen_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``cryozen auth add <provider> --type oauth`` path."""
...
def _unsuppress_provider_sources(provider: str) -> None:
    ...
    user wants auth re-enabled. Covers env:* (shell-exported vars), gh_cli (copilot),
...
def _add_api_key_credential(args, provider: str, pool) -> PooledCredential:
    ...
    if not token:
        try:
            ...
            # No terminal and nothing piped on stdin (e.g. an editor/agent shell): say how to
            # supply the key without putting it on the command line.
            raise SystemExit(
                "No API key received: stdin is not a terminal and nothing was piped in.\n"
                f"Run this in a terminal, or pipe the key on stdin, e.g.:\n"
                f"  pbpaste | cryozen auth add {provider} --type api-key"
            ) from None
    if not token:
        raise SystemExit("No API key provided.")
    if any(ch.isspace() for ch in token):
        # API keys never contain whitespace; this is almost always the wrong clipboard contents.
        # Refuse it here instead of storing a value that fails later as a 401.
        raise SystemExit(
            "That value contains spaces or line breaks, so it is not an API key. "
            "Copy the key itself and try again.")
    if provider == "anthropic":
        from agent.anthropic_credentials import is_consumer_oauth_token
        if is_consumer_oauth_token(token):
            raise SystemExit(
                "That is a Claude subscription OAuth token (sk-ant-oat...), which Cryozen does not use. "
                "Create an Anthropic API key at https://platform.claude.com/settings/keys.")
...
def _add_credential(args, provider: str, pool, requested_type: str) -> PooledCredential:
    ...
    if spec is None:
        raise SystemExit(f"`cryozen auth add {provider}` is not implemented for auth type {requested_type} yet.")
    ...
    for position, sibling in enumerate(existing, start=1):
        if _codex_principal_identity(sibling.access_token) == identity:
            print(f'warning: this login is the same OpenAI account as openai-codex credential #{position} '
                  ...
                  f"(`cryozen auth remove openai-codex {position}`).", file=sys.stderr)
            return
...
def _report_priority(provider: str, pool, moved, requested: int, verb: str, prep: str) -> None:
    ...
    print(f'{verb} {provider} credential "{moved.label}" {prep} priority {moved.priority} '
          f"(#{moved.priority + 1} in `cryozen auth list {provider}`)")
...
def auth_priority_command(args) -> None:
    """`cryozen auth priority <provider> <target> <priority>`: reorder one pooled credential."""
    provider = _normalize_provider(getattr(args, "provider", ""))
    pool = load_pool(provider)
...
def _print_external_login_notice() -> None:
    """One line telling the user why no Codex CLI login shows up when adoption is off."""
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Cryozen reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_refresh_command(args) -> None:
    """`cryozen auth refresh <provider> [target]`: force one pooled OAuth entry to refresh.
    ...
    if target is None or not str(target).strip():
        if len(entries) != 1:
            raise SystemExit(
                ...
                f"label (see `cryozen auth list {provider}`).")
    ...
    if refreshed is None:
        ...
        raise SystemExit(
            ...
            f"Sign in again with `cryozen auth add {provider} --type oauth`.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `cryozen auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `cryozen auth` is called bare."""
...
def _interactive_add() -> None:
    ...
    auth_add_command(SimpleNamespace(
        ...
        client_id=None, scope=None, no_browser=False, timeout=None, insecure=False, ca_bundle=None))
