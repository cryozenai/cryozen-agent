"""Single source of truth for provider identity in Cryozen Agent."""
...
# -- Cryozen overlay: metadata models.dev doesn't provide ----------------------

@dataclass(frozen=True)
class CryozenOverlay:
    """Cryozen-specific provider metadata layered on top of models.dev."""
...
CRYOZEN_OVERLAYS: Dict[str, CryozenOverlay] = {
    "moa": CryozenOverlay(auth_type="virtual", base_url_override="moa://local"),
    "openrouter": CryozenOverlay(is_aggregator=True, base_url_env_var="OPENROUTER_BASE_URL"),
    "openai-codex": CryozenOverlay(transport="codex_responses", auth_type="oauth_external",
                                  base_url_override="https://chatgpt.com/backend-api/codex"),
    "openai-api": CryozenOverlay(transport="codex_responses", base_url_override="https://api.openai.com/v1",
                                base_url_env_var="OPENAI_BASE_URL"),
    "xai-oauth": CryozenOverlay(transport="codex_responses", auth_type="oauth_external",
                               base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "qwen-oauth": CryozenOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="CRYOZEN_QWEN_BASE_URL"),
    "lmstudio": CryozenOverlay(extra_env_vars=("LM_API_KEY",), base_url_override="http://127.0.0.1:1234/v1",
                              base_url_env_var="LM_BASE_URL"),
    "copilot-acp": CryozenOverlay(transport="codex_responses", auth_type="external_process",
                                 base_url_override="acp://copilot", base_url_env_var="COPILOT_ACP_BASE_URL"),
    "github-copilot": CryozenOverlay(extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN")),
    "anthropic": CryozenOverlay(transport="anthropic_messages"),
    "zai": CryozenOverlay(extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"), base_url_env_var="GLM_BASE_URL"),
    "kimi-for-coding": CryozenOverlay(base_url_env_var="KIMI_BASE_URL"),
    "stepfun": CryozenOverlay(extra_env_vars=("STEPFUN_API_KEY",),
                             base_url_override="https://api.stepfun.ai/step_plan/v1",
                             base_url_env_var="STEPFUN_BASE_URL"),
    "minimax": CryozenOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_BASE_URL"),
    "minimax-oauth": CryozenOverlay(transport="anthropic_messages", auth_type="oauth_external",
                                   base_url_override="https://api.minimax.io/anthropic"),
    "minimax-cn": CryozenOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_CN_BASE_URL"),
    "deepseek": CryozenOverlay(base_url_env_var="DEEPSEEK_BASE_URL"),
    "alibaba": CryozenOverlay(base_url_env_var="DASHSCOPE_BASE_URL"),
    "alibaba-coding-plan": CryozenOverlay(base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL"),
    "vercel": CryozenOverlay(is_aggregator=True),
    "opencode": CryozenOverlay(is_aggregator=True, base_url_env_var="OPENCODE_ZEN_BASE_URL"),
    "opencode-go": CryozenOverlay(is_aggregator=True, base_url_env_var="OPENCODE_GO_BASE_URL"),
    "kilo": CryozenOverlay(is_aggregator=True, base_url_env_var="KILOCODE_BASE_URL"),
    "huggingface": CryozenOverlay(is_aggregator=True, base_url_env_var="HF_BASE_URL"),
    "novita": CryozenOverlay(is_aggregator=True, base_url_env_var="NOVITA_BASE_URL"),
    "xai": CryozenOverlay(transport="codex_responses", base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "nvidia": CryozenOverlay(base_url_override="https://integrate.api.nvidia.com/v1", base_url_env_var="NVIDIA_BASE_URL"),
    "xiaomi": CryozenOverlay(base_url_env_var="XIAOMI_BASE_URL"),
    "tencent-tokenhub": CryozenOverlay(base_url_env_var="TOKENHUB_BASE_URL"),
    "tencent-tokenplan": CryozenOverlay(transport="anthropic_messages",
                                       base_url_override="https://api.lkeap.cloud.tencent.com/plan/anthropic",
                                       base_url_env_var="TOKENPLAN_BASE_URL"),
    "arcee": CryozenOverlay(base_url_override="https://api.arcee.ai/api/v1", base_url_env_var="ARCEE_BASE_URL"),
    "gmi": CryozenOverlay(extra_env_vars=("GMI_API_KEY",), base_url_override="https://api.gmi-serving.com/v1",
                         base_url_env_var="GMI_BASE_URL"),
    "fireworks": CryozenOverlay(extra_env_vars=("FIREWORKS_API_KEY",),
                               base_url_override="https://api.fireworks.ai/inference/v1"),
    "actual": CryozenOverlay(transport="chat_completions", extra_env_vars=("ACTUAL_API_KEY",),
                            base_url_override="https://api.actual.inc/v1", base_url_env_var="ACTUAL_BASE_URL"),
    "upstage": CryozenOverlay(extra_env_vars=("UPSTAGE_API_KEY",), base_url_override="https://api.upstage.ai/v1",
                             base_url_env_var="UPSTAGE_BASE_URL"),
    "nebius-token-factory": CryozenOverlay(extra_env_vars=("NEBIUS_API_KEY", "NEBIUS_TOKEN_FACTORY_API_KEY"),
                                          base_url_override="https://api.tokenfactory.nebius.com/v1",
                                          base_url_env_var="NEBIUS_BASE_URL"),
    "ollama-cloud": CryozenOverlay(base_url_override="https://ollama.com/v1", base_url_env_var="OLLAMA_BASE_URL"),
    # Azure Foundry serves OpenAI- and Anthropic-style endpoints; transport comes from model.api_mode.
    "azure-foundry": CryozenOverlay(base_url_env_var="AZURE_FOUNDRY_BASE_URL"),
    "bedrock": CryozenOverlay(transport="bedrock_converse", auth_type="aws_sdk"),
    ...
    "vertex": CryozenOverlay(auth_type="vertex"),
}
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "cryozen", "user-config"
...
_LABEL_OVERRIDES: Dict[str, str] = {
    "moa": "Mixture of Agents", "openai-codex": "ChatGPT or Codex Subscription",
...
def _overlay_pdef(canonical, ov: CryozenOverlay, name, env_vars, base_url, doc, source) -> ProviderDef:
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    """Look up a built-in provider by id or alias: models.dev catalog merged with the Cryozen overlay;
    Cryozen-only overlay (openai-codex, xai-oauth, …); plugin provider profiles with a concrete endpoint."""
    canonical = normalize_provider(name)
    mdev_info = _models_dev_info(canonical, allow_network)
    overlay = CRYOZEN_OVERLAYS.get(canonical)
    if mdev_info is not None:
        ov = overlay or CryozenOverlay()
    ...
    if overlay is not None:
        return _overlay_pdef(canonical, overlay, _LABEL_OVERRIDES.get(canonical, canonical), overlay.extra_env_vars,
                             overlay.base_url_override, "", "cryozen")
    # Plugin-registered profiles (plugins/model-providers/<name>/) absent from models.dev and
    # CRYOZEN_OVERLAYS would otherwise be "Unknown provider" in /model, --provider and model-switch
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    """API mode (wire protocol) for a provider/endpoint: host-mandated mode, then the known
    provider's transport, then bedrock, else ``chat_completions``."""
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    """Exact Cryozen registry ids win over LOSSY alias collapsing (kimi-coding-cn must stay distinct
    ...
    try:
        from cryozen_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
...
# below and the picker's Local row (``cryozen_cli/inventory.py``) — the two drifting apart is what
...
def _has_staged_local_models() -> bool:
    """True when GGUFs are staged under the Cryozen home's ``models/`` — the model the picker's Local
    row offers, which the runtime seam serves by booting/attaching a server on selection."""
    try:
        from cryozen_cli.local_runtime.bootstrap import staged_model_ids
...
def _llamacpp_pdef() -> Optional[ProviderDef]:
    ...
    try:
        ...
        from cryozen_cli.local_runtime.endpoint import resolve_llamacpp_endpoint
...
def resolve_provider_full(name: str, user_providers: Optional[Dict[str, Any]] = None,
    ...
    # and `cryozen model` must not reject them as unknown. Last rung, so every user-configured
