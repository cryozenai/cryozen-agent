...
Split out of ``cryozen_cli/config.py``; every name is re-imported there, so
``cryozen_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``cryozen_cli.config``
...
logger = logging.getLogger("cryozen_cli.config")
...
_KNOWN_PROVIDER_KEYS = {
    # ``provider`` duplicates the ``providers.<name>`` mapping key and is unused here, but Cryozen'
...
def _normalize_custom_provider_entry(
    ...
    # ``models_discovered`` marks a mapping auto-discovered by Cryozen, not hand-curated.
...
def get_custom_provider_session_affinity_header(
    ...
    Opt-in per provider (default off): Cryozen never ships a session identifier to an endpoint
...
def get_custom_provider_context_length(
    ...
    """
    from cryozen_cli.config import get_compatible_custom_providers, load_config_readonly
