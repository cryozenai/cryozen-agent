...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    """Load prefill messages (JSON array) from *file_path*; relative to ~/.cryozen-agent/; missing/empty -> []."""
    from cli import _cryozen_home
    if not file_path:
        return []
    path = Path(file_path).expanduser()
    if not path.is_absolute():
        path = _cryozen_home / path
    if not path.exists():
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    return (
        os.getenv("CRYOZEN_PREFILL_MESSAGES_FILE", "").strip()
...
def _mirror_config_to_env(defaults, _file_has_terminal_config):
    ...
    _is_gateway = os.environ.get("_CRYOZEN_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["CRYOZEN_REDACT_SECRETS"] = str(redact).lower()

    # Session-search index knobs (cryozen_state reads the env carriers).
    sessions_config = defaults.get("sessions", {})
    if isinstance(sessions_config, dict):
        if "cjk_fts" in sessions_config:
            os.environ["CRYOZEN_CJK_FTS"] = str(sessions_config["cjk_fts"])
        if "search_slow_ms" in sessions_config:
            os.environ["CRYOZEN_SEARCH_SLOW_MS"] = str(sessions_config["search_slow_ms"])
...
def _cli_config_defaults():
    ...
    return {
        ...
        "agent": {
            ...
            "personalities": {},  # user overrides merged by name over cryozen_cli.personality builtins
        },
        "display": {
            "compact": False,
            # /resume recap tuning and show_reasoning: keep in sync with cryozen_cli/config.py DEFAULT_CONFIG
...
def _merge_file_config(defaults: Dict[str, Any], file_config: Dict[str, Any]) -> None:
    ...
    if "model" in file_config:
        if isinstance(file_config["model"], str):
            ...
            # Promote model.model -> model.default (CryozenCLI checks "default" first).
...
def load_cli_config() -> Dict[str, Any]:
    """~/.cryozen-agent/config.yaml (else ./cli-config.yaml) over built-in defaults; env vars win.

    ``CRYOZEN_IGNORE_USER_CONFIG=1`` skips the user config entirely (``.env`` still loads).
    """
    from cli import _cli_config_defaults, _cryozen_home, _merge_file_config, _mirror_config_to_env
    config_path = _cryozen_home / 'config.yaml'
    if not config_path.exists() or os.environ.get("CRYOZEN_IGNORE_USER_CONFIG") == "1":
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from cryozen_cli.config import _normalize_root_model_keys
    ...
    # independently of cryozen_cli.config, so this keeps parity with `cryozen config`. Fail-open.
...
def _init_logging_and_display_from_config() -> None:
    ...
    for step in (
        lambda: _im("cryozen_logging").setup_logging(mode="cli"),
        lambda: _im("cryozen_cli.config").print_config_warnings(),
        lambda: _im("cryozen_cli.skin_engine").init_skin_from_config(_cli().CLI_CONFIG),
