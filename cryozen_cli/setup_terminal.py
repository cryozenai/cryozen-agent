...
setup.py names are resolved through the module object so test patches on ``cryozen_cli.setup.<name>``
...
def _pip_install_vercel(package):
    """uv when Cryozen has one ($CRYOZEN_HOME/bin is never on PATH, so which() misses it and
...
def _setup_backend_local(config: dict) -> None:
    ...
    # Gateway cwd defaults to home; sudo stays off. Both configurable via `cryozen setup terminal`.
...
def _setup_backend_docker(config: dict) -> None:
    ...
    # Image and resource limits use defaults; tune via `cryozen setup terminal`.
    ...
    if _setup.prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
        ...
        _setup.print_info(
            "Run `cryozen egress setup` then `cryozen egress start` to mint tokens and launch the proxy.")
    else:
        _setup.print_info("Skipping egress firewall. You can enable it later with `cryozen egress setup`.")
...
def _setup_backend_vercel(config: dict) -> None:
    ...
    _setup._info("Cloud microVM sandboxes with snapshot-backed filesystem persistence.",
                 "Requires the optional SDK: pip install 'cryozen-agent[vercel]'")
    _ensure_sdk("vercel", "pip install 'cryozen-agent[vercel]'", show_stderr=True, install=_pip_install_vercel)
...
_BACKEND_ENV_MIRROR = {"vercel_sandbox": ("TERMINAL_VERCEL_RUNTIME", "vercel_runtime", "node24")}
...
def setup_terminal_backend(config: dict):
    ...
    _setup._info("Choose where Cryozen runs shell commands and code.",
    ...
    # Plugin-registered backends (~/.cryozen-agent/plugins/). Fail-soft: a broken plugin must not take
    # the wizard down.
