...
class CLIModalMixin:
    ...
    def _build_command_palette_entries(self) -> list:
        ...
        from cryozen_cli.commands import COMMANDS_BY_CATEGORY
    ...
    def _ring_bell(self, prompt: bool = False, context: str = "", detail: str = "") -> None:
        ...
        if not getattr(self, flag, False) or getattr(self, "_terminal_io_broken", False):
            return
        from cryozen_cli.cli_terminal_mixin import _run_on_app_loop, _write_terminal_sequence
        from cryozen_cli.terminal_notify import notification_sequence, write_tty
    ...
    def _connection_retry(self) -> None:
        ...
        if operation is None:
            ...
            return
        target = state["target"]
        if target.get("state") in {"pending", "failed"}:
