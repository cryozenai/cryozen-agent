"""``cryozen plugins`` CLI subcommand — install, update, remove, and list plugins."""
...
from cryozen_cli._subprocess_compat import noninteractive_git_env
from cryozen_cli.cli_output import line_input
from cryozen_cli.config import cfg_get
from cryozen_cli.plugin_capabilities import _child_dict
...
def _plugins_dir() -> Path:
    ...
    plugins = get_cryozen_home() / "plugins"
...
def _read_manifest(plugin_dir: Path) -> dict:
    ...
    if manifest_file is None:
        if not _has_portable_manifest(plugin_dir):
            return {}
        try:
            from cryozen_cli.agent_plugins import read_agent_plugin_manifest
...
def _missing_env_specs(manifest: dict) -> list[dict]:
    ...
    normalised to dicts; nameless entries dropped) whose variable is unset in ``~/.cryozen-agent/.env``."""
    env_specs = [
...
def _refuse_conflicting_python_deps(tmp_target: Path, plugin_name: str) -> None:
    ...
    from cryozen_cli.plugin_python_deps import DependencyConflict, refuse_conflicting_candidate
    try:
        refuse_conflicting_candidate(tmp_target, home=get_cryozen_home())
        ...
        raise PluginOperationError(
            ...
            "its Python packages yourself: cryozen plugins install <identifier> --no-deps") from exc
...
def _install_python_dependencies_quietly(target: Path, warnings: list[str]) -> list[str]:
    ...
    from cryozen_cli.plugin_python_deps import install_for_plugin_dir
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_cryozen_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_cryozen_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_cryozen_home()}/.env later)[/dim]")
...
def _clone_failure_message(git_url: str, git_error: str) -> str:
    ...
    return (f"Could not download the plugin from {git_url}. Check the address (browse the catalog "
            "with `cryozen plugins search`), check your internet connection, or, if the repository "
...
def _unknown_plugin_message(name: str, *, downloaded_only: bool = False) -> str:
    ...
    return (f"[red]No plugin named '{name}'.[/red] Run `cryozen plugins list` to see the exact names "
            f"(nested plugins use their full key, e.g. web/firecrawl).{scope} "
            "To add one: `cryozen plugins install <owner/repo>`.")
...
def _install_metadata_path() -> Path:
    return get_cryozen_home() / "plugins" / ".install-metadata.json"
...
def _check_manifest_version(manifest: dict, plugin_name: str) -> None:
    ...
    from cryozen_cli.plugins_manifest import SUPPORTED_MANIFEST_VERSION
    if mv_int > SUPPORTED_MANIFEST_VERSION:
        ...
        raise PluginOperationError(
            ...
            f"but this Cryozen supports up to {SUPPORTED_MANIFEST_VERSION}. "
            f"Run {recommended_update_command()} to update Cryozen.",
        ) from None
...
def _ensure_tree_readable(root: Path, plugins_dir: Path) -> None:
    """Refuse to ship a tree Cryozen cannot read back. A clone can land unreadable (Windows ACL
...
def _refuse_unavailable_portable_plugin(plugin_name: str, tree: Path) -> None:
    if not (tree / "plugin.json").is_file():
        return
    from cryozen_cli.agent_plugins import load_agent_plugin
    from cryozen_platform.resolver.availability import availability

    try:
        package = load_agent_plugin(tree, tree.parent / ".cryozen-install-data")
    except ValueError as exc:
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `cryozen plugins update {plugin_name}`.")
...
def cmd_install(
    ...
    a ``.cryozen-catalog.json`` sidecar; URLs/shorthand are flagged as custom (unreviewed) sources. Every
    ...
    *enable* None prompts "Enable now? [y/N]"; True/False skip the prompt.
    """
    from cryozen_cli import plugins_cmd_catalog as catalog
    console = _console()
    entry = None
    if catalog.looks_like_catalog_name(identifier):
        ...
        console.print("[yellow]Warning:[/yellow] custom (unreviewed) source — not from the Cryozen catalog.")
    if allow_removed:
        console.print(
    ...
    if not _looks_like_plugin_dir(target):
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Cryozen plugin.")
    ...
    if enable:
        ...
        console.print(
            ...
            f"Run `cryozen plugins enable {installed_name}` to activate.[/dim]")
    ...
    if enable:
        ...
        from cryozen_cli.plugins_activation import activate_plugin_now, activation_hint
...
def cmd_update(name: str) -> None:
    ...
    try:
        output = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"`cryozen plugins install {escape(str(rec.get('source', '<source>')))} --force "
    ...
    if declared_caps:
        from cryozen_cli.plugin_capabilities import declared_set_changed, pending_capabilities
...
def _rescan_after_update(target: Path, name: str, console) -> None:
    ...
    if scan_result.verdict == "dangerous":
        ...
        console.print(
            ...
            f"findings, then re-enable with `cryozen plugins enable {name}` "
            f"if you trust them.")
...
def cmd_remove(name: str) -> None:
    ...
    if result.get("cleared_memory_provider"):
        console.print("[yellow]memory.provider pointed at this plugin and was reset; "
                      "run `cryozen memory setup` to pick another.[/yellow]")
    console.print()
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``cryozen setup`` / ``cryozen plugins disable basic`` can park it there while
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from cryozen_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
    ...
    def _refuse_legacy_relay(plugin: str) -> None:
        if plugin in LEGACY_RELAY_PLUGIN_KEYS:
            _fail(console, (
                ...
                f"by Cryozen core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."))
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from cryozen_cli.plugin_capabilities import parse_declared_capabilities
...
def _declared_capabilities_for_key(key: str) -> list:
    ...
    if entry[3] == "entrypoint":
        from cryozen_cli.plugins import discover_entrypoint_manifests
...
def _run_capability_consent(console, plugin_id: str, declared: list, *, context: str = "install") -> bool:
    ...
    """
    from cryozen_cli.plugin_capabilities import CAPABILITY_REGISTRY, pending_capabilities, record_consent
    ...
    if not _is_tty():
        console.print(
            ...
            f"`cryozen plugins capabilities {plugin_id}` to review and "
            f"`cryozen plugins enable {plugin_id}` to grant interactively.")
        return False
    ...
    console.print(
        ...
        f"`cryozen plugins enable {plugin_id}` to grant later.[/dim]")
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``cryozen plugins capabilities [<id>]`` — declared vs granted."""
    from cryozen_cli.plugin_capabilities import (
        CAPABILITY_REGISTRY,
...
def _resolve_tool_override_grant(console, key: str, allow_tool_override: Optional[bool]) -> None:
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`cryozen plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]")
...
def _discover_all_plugins() -> list:
    ...
    from cryozen_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``cryozen plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] cryozen plugins install owner/repo")
        return
    ...
    for line in removed_lines:
        console.print(line)
    console.print()
    console.print("[dim]Compact view:[/dim] cryozen plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] cryozen plugins")
    console.print("[dim]Enable/disable:[/dim] cryozen plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        ...
        pass
    try:
        from cryozen_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        _fail(console, "[dim]List installed plugins:[/dim] cryozen plugins list")
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    from cryozen_cli.curses_ui import _addnstr, flush_stdin
...
def dashboard_install_plugin(
    ...
    if catalog_name:
        ...
        if entry is None:
            return {"ok": False, "error": f"'{catalog_name}' is not in the Cryozen plugin catalog."}
        identifier = entry.install_identifier
    else:
        warnings.append("Custom (unreviewed) source — not from the Cryozen catalog.")
    try:
    ...
    from cryozen_cli.plugins_activation import activate_plugin_now
...
def _get_plugin_toolset_key(name: str) -> Optional[str]:
    ...
    def _from_manifest_on_disk() -> Optional[str]:
        from cryozen_cli.plugins import get_bundled_plugins_dir
        return next((
...
def _toggle_plugin_toolset(name: str, *, enable: bool) -> None:
    ...
    if not toolset_key:
        return
    from cryozen_cli.config import load_config, save_config
    from cryozen_cli.toolset_validation import parse_platform_toolsets_value
    config = load_config()
    platform_toolsets = _child_dict(config, "platform_toolsets")
    changed = False
    for platform, raw in list(platform_toolsets.items()):
        # A list-literal string (older `cryozen config set`) is the user's real selection; toggling
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.cryozen-agent/plugins/<name>`` if it exists."""
    try:
...
def dashboard_update_user_plugin(name: str, *, accept_capabilities: bool = False) -> dict[str, Any]:
    """``git pull`` inside ``~/.cryozen-agent/plugins/<name>``; catalog installs re-pin instead. A re-pin that
    ...
    try:
        ...
        msg = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"run `cryozen plugins install {rec.get('source', '<source>')} --force "
...
def _run_plugin_git(
    ...
    from cryozen_cli.git_credentials import run_git_with_credential_fallback
...
def _autostash_dirty_tree(git_exe: str, target: Path) -> tuple[str, str]:
    ...
    from cryozen_cli.update_cmd_stash import _intent_to_add_paths
    ...
    push = _run_plugin_git(
        git_exe, target, "stash", "push", "--include-untracked", "-m", "cryozen-plugin-update-autostash")
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    marketplace now succeeds when its checkout has local changes"), and the same autostash approach ``cryozen
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.cryozen-agent/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from cryozen_cli.plugin_dev import doctor_plugin
...
def _catalog():
    from cryozen_cli import plugins_cmd_catalog
    return plugins_cmd_catalog


def _action_pack(args):
    from cryozen_cli.plugin_packs import pack_command
    pack_command(args)


def cmd_compat(args: Any | None = None) -> None:
    """``cryozen plugins compat`` — which installed plugins import paths scheduled for removal, and where."""
    import sys
    from pathlib import Path
    from cryozen_cli.plugin_compat import (
...
def plugins_command(args) -> None:
    """Dispatch cryozen plugins subcommands."""
