"""``cryozen plugins`` subcommand parser."""
...
def build_plugins_parser(subparsers, *, cmd_plugins: Callable) -> None:
    ...
    plugins_parser = subparsers.add_parser(
        ...
        description="Install, update, remove, list, or validate native Cryozen plugins "
    ...
    plugins_install.add_argument(
        "identifier",
        help="Bare plugin catalog entry name (see `cryozen plugins search`), Git URL, or owner/repo "
            "shorthand (e.g. anpicasso/cryozen-plugin-chrome-profiles)")
    plugins_install.add_argument(
    ...
    _install_enable_group.add_argument(
        ...
        help="Install disabled (skip confirmation prompt); enable later with `cryozen plugins enable <name>`",
    )

    plugins_search = plugins_subparsers.add_parser(
        "search", help="Search the curated Cryozen plugin catalog")
    ...
    plugins_pack = plugins_subparsers.add_parser(
        "pack", help="Declarative, shareable plugin sets (cryozen-pack.yaml)",
    ...
    pack_install.add_argument("source", help="Path to a cryozen-pack.yaml file, or an https:// URL")
    ...
    pack_export.add_argument(
        "--name", default="my-cryozen-pack", help="Pack name to embed in the exported YAML")

    pack_show = pack_subparsers.add_parser(
        "show", help="Dry-run: parse and display a pack without installing")
    pack_show.add_argument("source", help="Path to a cryozen-pack.yaml file, or an https:// URL")
