"""``cryozen gateway`` and ``cryozen proxy`` subcommand parsers."""
...
from cryozen_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    _flag(gateway_run, "--no-supervise",
        help="Inside the s6-overlay Docker image, normally `gateway run` is "
            ...
            "if CRYOZEN_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy", help="Remove legacy cryozen.service units from pre-rename installs",
        description="Stop, disable, and remove legacy Cryozen gateway unit files "
            "(e.g. cryozen.service) left over from older installs. Profile "
            "units (cryozen-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser("enroll",
        ...
        description="Redeem a single-use enrollment token with a relay connector. "
            "Authenticates with the identity provider configured under gateway.idp "
            "(the connector derives the authoritative tenant from it), mints this "
            "gateway's per-gateway secret "
            "and per-tenant delivery key, and writes GATEWAY_RELAY_ID / "
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.cryozen-agent/.env. "
            "Requires gateway.idp to be configured. Not available in managed installs.")
    ...
    gateway_enroll.add_argument("--wake-url", dest="wake_url", default=None,
        help="Phase 5 §5.2 wake URL: a reachable URL the connector pokes "
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.cryozen-agent/.env and forwarded at provision. "
    ...
    proxy_start.add_argument("--provider", default="xai",
        help="Upstream provider (default: xai). See `cryozen proxy providers`.")
