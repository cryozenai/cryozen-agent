"""``cryozen verify`` subcommand parser.

Follows the pattern of ``cryozen_cli/subcommands/doctor.py``: parser built
...
# avoid paying an extra module import on every `cryozen` invocation.
...
def build_verify_parser(subparsers, *, cmd_verify: Callable) -> None:
    ...
    verify_parser = subparsers.add_parser(
        ...
        description="Detect how the current project is built, tested, and started "
            "(or load the saved manifest at .cryozen-agent/environment.json), then "
    ...
    verify_parser.add_argument(
        ...
        help="Save the recipe as .cryozen-agent/environment.json in the project")
    verify_parser.add_argument(
