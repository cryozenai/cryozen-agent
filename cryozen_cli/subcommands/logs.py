"""``cryozen logs`` subcommand parser."""
...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Cryozen log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    cryozen logs                    Show last 50 lines of agent.log
    cryozen logs -f                 Follow agent.log in real time
    cryozen logs errors             Show last 50 lines of errors.log
    cryozen logs gateway -n 100     Show last 100 lines of gateway.log
    cryozen logs gui -f             Follow gui.log in real time
    cryozen logs desktop -f         Follow desktop.log (Electron app boot/backend)
    cryozen logs --level WARNING    Only show WARNING and above
    cryozen logs --session abc123   Filter by session ID
    cryozen logs --component tools  Only show tool-related lines
    cryozen logs --since 1h         Lines from the last hour
    cryozen logs --since 30m -f     Follow, starting from 30 min ago
    cryozen logs list               List available log files with sizes
""")
