"""``cryozen pause`` / ``cryozen resume`` — the global emergency stop.
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Cryozen paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `cryozen resume` to lift the pause.")
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Cryozen resumed — dispatch picks up on the next tick.")
    else:
        print(f"Cryozen is not paused (no sentinel at {sentinel_path()}).")
    return 0
