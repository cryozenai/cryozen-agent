"""``cryozen codex-runtime`` — noninteractive counterpart of the ``/codex-runtime`` slash command.

``cryozen codex-runtime migrate [--dry-run] [--json]`` runs the same ``~/.codex/config.toml``
migration the slash command triggers when the codex app-server runtime is enabled, on the
selected profile home (``cryozen -p NAME codex-runtime migrate``), so automation no longer has to
import ``cryozen_cli.codex_runtime_plugin_migration`` directly (issue #79023).
"""
...
def cmd_codex_runtime_migrate(args: argparse.Namespace) -> int:
    """Project Cryozen MCP servers (+ codex plugins) into ~/.codex/config.toml; 1 on any error."""
    from cryozen_cli.codex_runtime_plugin_migration import migrate
...
def build_codex_runtime_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        ...
        description="Noninteractive counterpart of the /codex-runtime slash command. Toggling the "
            ...
            "re-projects Cryozen' mcp_servers + installed codex plugins into the managed block of "
    ...
    migrate_parser = actions.add_parser(
        "migrate", help="Regenerate the cryozen-managed block in codex's config.toml",
        description="Idempotent: replaces the managed block, keeps user text verbatim, skips Cryozen "
    ...
    def _print_help(args):  # noqa: ANN001 — bare `cryozen codex-runtime` lists the actions
        parser.print_help()
        return 0
