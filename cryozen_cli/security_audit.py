"""On-demand supply-chain audit for Cryozen Agent installs.
...
def _discover_plugins(cryozen_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.cryozen-agent/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, cryozen_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = cryozen_home or Path(get_cryozen_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `cryozen security audit`."""
    ...
    components = _discover_components(cryozen_home=home, **skips)
    ...
    try:
        findings = run_audit(cryozen_home=home, components=components)
