"""Gateway subcommand for cryozen CLI.

Handles: cryozen gateway [run|start|stop|restart|status|install|uninstall|setup]
"""

import asyncio
import contextlib
from cryozen_cli.cli_output import line_input  # noqa: F401 — resolved lazily by siblings through the facade
import json
import logging
import os
import shutil
...
from cryozen_cli import setup_platforms  # noqa: F401 — resolved lazily by siblings through the facade
...
from cryozen_cli.config import (  # noqa: F401 — resolved lazily by siblings through the facade
    get_env_value,
    get_cryozen_home,
    is_managed,
    managed_error,
...
# display_cryozen_home is imported lazily: cryozen_constants may be a cached pre-update version.
from cryozen_cli.setup import (  # noqa: F401 — resolved lazily by siblings through the facade
    print_header,
    print_info,
    print_success,
    print_warning,
...
from cryozen_cli.colors import Colors, color  # noqa: F401 — resolved lazily by siblings through the facade
from cryozen_cli.gateway_service_unit import (  # noqa: F401 — resolved lazily by siblings through the facade
    _systemd_env_line,
    _installed_unit_ld_library_path,
    _ld_library_path_line,
    _cryozen_home_for_target_user,
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``all_profiles`` widens the current profile's unit/label to the whole ``cryozen-gateway*`` /
    ``ai.cryozen.gateway*`` fleet so update/reaper never kill a sibling's service gateway as "manual".

    ``all_profiles`` widens the launchd branch to every installed ``ai.cryozen.gateway*`` LaunchAgent — the
    ...
    the ``cryozen-gateway*`` fleet glob.
    """
    pids: set = set()

    # --- systemd (Linux): user and system scopes ---
    if supports_systemd_services():
        pattern = "cryozen-gateway*" if all_profiles else get_service_name()
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                # Belt-and-suspenders for the EXCLUDE use case (#74075): a bare ``launchctl list`` prefix
                # scan also catches ai.cryozen.gateway* agents the label derivation can't map (renamed
    ...
    if is_macos():
        ...
        if all_profiles:
            # Whole fleet, mirroring the systemd ``cryozen-gateway*`` glob above.
            # Every gateway LaunchAgent, not just the invoking profile's — mirrors the systemd branch's
            # ``cryozen-gateway*`` pattern above. The update path restarts the whole fleet, and its
        ...
        for label in sorted(labels):
            ...
            if pid is not None and pid > 0:
                pids.add(pid)
        if all_profiles:
            # Prefix scan also catches ai.cryozen.gateway* agents the label derivation can't map
            ...
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().splitlines():
                        parts = line.split()
                        if len(parts) >= 3 and parts[-1].startswith("ai.cryozen.gateway"):
                            try:
                                pid = int(parts[0])
                                if pid > 0:
                                    pids.add(pid)
...
# its full budget and `cryozen update` can deadlock. Two witnesses classify the loop BEFORE any
...
# fail", and `cryozen update` can deadlock behind it. The loop publishes a liveness signal precisely for this
...
def _get_ancestor_pids() -> set[int]:
    """PIDs of this process and its ancestors, so scans never count the invoking ``cryozen`` CLI as a gateway.

    Walks from the current PID up to PID 1 (init) so that process-table scans never match the calling CLI
    process or any of its parents. This prevents ``cryozen gateway status`` from falsely counting the
    ``cryozen`` CLI that invoked it as a running gateway instance (see #13242).
    """
...
def _scan_gateway_pids(
    ...
    # Exclude the entire ancestor chain so the CLI process that invoked this scan (e.g. ``cryozen gateway
    ...
    from gateway.status import (
        ...
        profile_flag_value,
        cryozen_home_assignments,
        command_line_names_cryozen_home,
    )
    current_home = str(get_cryozen_home().resolve())
    # Forward slashes on both sides of the CRYOZEN_HOME= match (mirrors gateway.status), and no
    # trailing separator: the assignments parser strips one, so the systemd ``Environment=``
    # spelling (``CRYOZEN_HOME=/root/.cryozen-agent/``) compares equal to the resolved home.
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            ...
            if profile_flag_value(command_lc) == current_profile_name_lc:
                return True
            return command_line_names_cryozen_home(command_lc, current_home_lc)
        ...
        # fallback stop could SIGTERM the named gateway). CRYOZEN_HOME may come via env (invisible to
        # wmic/CIM), so only a non-matching explicit CRYOZEN_HOME= disqualifies.
        if profile_flag_value(command_lc) is not None:
            return False
        return (not cryozen_home_assignments(command_lc)
                or command_line_names_cryozen_home(command_lc, current_home_lc))
...
def _windows_process_listing() -> str | None:
    ...
    # failure or timeout (result is None) trips the fallback. ``cryozen update`` hung exactly there on
    ...
    from cryozen_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    """Find running gateway PIDs for the current profile, or every profile with ``all_profiles`` (``cryozen update``)."""
...
def find_profile_gateway_processes(exclude_pids: set | None = None, *, strict: bool = False) -> list[ProfileGatewayProcess]:
    """Return running gateway PIDs mapped to Cryozen profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid, get_running_pid_identity_strict
        from cryozen_cli.profiles import list_profiles
    except Exception:
        if strict:
            raise
        return processes
...
def find_windows_gateway_services(
    *, psutil_module=None, profile_processes: list[ProfileGatewayProcess] | None = None
    ...
    """Profile gateways supervised by real, Cryozen-owned Windows services. Service-logon processes may
    hide their command lines, so identity = Cryozen's own PID file + a parent chain ending at a running
    SCM service PID whose name or binary path is Cryozen's (``gateway_windows.cryozen_owns_windows_service``).
    ...
    try:
        ...
        from cryozen_cli.gateway_windows import cryozen_owns_windows_service, cryozen_service_roots

        cryozen_roots = cryozen_service_roots()
        ...
        for service in psutil_module.win_service_iter():
            try:
                ...
                # PID nor its status may steer the pause. Only Cryozen-owned services reach the guards below.
                # The name alone settles Cryozen-named services; binpath (QueryServiceConfig) is asked only
                # for the rest, and a service that refuses even that to this user is one this user could
                # not `sc stop` either — never Cryozen's, never a reason to abort the enumeration.
                owned = cryozen_owns_windows_service(service_name, "", cryozen_roots)
                if not owned:
                    try:
                        service_binpath = str(_scm_service_field(service, "binpath") or "")
                    except (psutil_module.AccessDenied, OSError):
                        continue
                    owned = cryozen_owns_windows_service(service_name, service_binpath, cryozen_roots)
                if not owned:
                    continue
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "cryozen_cli.main"]
    if profile != "default":
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``cryozen update``: ``--external-supervisor`` gateways
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if old_pid <= 0 or not run_argv:
        return False
    from cryozen_cli._subprocess_compat import windows_detach_flags_without_breakaway, windows_detach_popen_kwargs
    ...
    # normalizes the interpreter and captures a stable cwd + env overlay (CRYOZEN_HOME,
    ...
    if sys.platform == "win32":
        try:
            from cryozen_cli.gateway_windows import windowless_gateway_restart_spec
    ...
    watcher = textwrap.dedent(
        ...
        import sys
        import time
        from cryozen_cli._subprocess_compat import (
        ...
        try:
            ...
            from pathlib import Path
            _log_dir = Path(get_cryozen_home()) / "logs"
        ...
        # CRYOZEN_HOME) the windowless base interpreter needs to import cryozen_cli. Empty on POSIX.
        if _respawn_cwd:
...
def _cryozen_home_pinned_by_unit(unit_path: Path) -> str | None:
    """``CRYOZEN_HOME`` pinned by the unit file at *unit_path*, or None when absent/unreadable."""
    return _unit_environment_value(unit_path, "CRYOZEN_HOME")


def _cryozen_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """``CRYOZEN_HOME`` from the on-disk unit file - what refresh/compare already read, and reliable under ``sudo``."""
    return _cryozen_home_pinned_by_unit(get_systemd_unit_path(system=system))


def _sync_cryozen_home_from_systemd_unit(system: bool) -> None:
    """Adopt a system-scope unit's ``CRYOZEN_HOME``: under ``sudo`` it is stripped and HOME=/root, so
    get_cryozen_home() would pick the wrong profile for runtime-status/PID reads."""
    if not system:
        return
    # On-disk unit first; ``systemctl show`` for units that only exist in the manager.
    unit_home = (_cryozen_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        env_line = _systemctl_show(("Environment",), system=True).get("Environment", "")
        unit_home = _parse_kv_pairs(env_line.split()).get("CRYOZEN_HOME", "").strip()
    if unit_home and os.environ.get("CRYOZEN_HOME", "").strip() != unit_home:
        os.environ["CRYOZEN_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
        ...
        if active_state == "active" and new_pid and (previous_pid is None or new_pid != previous_pid):
            ...
            if gateway_state in ("running", "degraded"):
                ...
                if gateway_state == "degraded":
                    ...
                    print(f"⚠ {scope_label} gateway is DEGRADED — see `cryozen gateway status`")
                return True
    ...
    print(
        ...
        f"  Check status: {sudo}cryozen gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}cryozen gateway restart{scope_flag}")
...
def _s6_gateway_snapshot(gateway_pids: tuple[int, ...]) -> GatewayRuntimeSnapshot | None:
    ...
    from cryozen_cli.service_manager import detect_service_manager, get_service_manager
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from cryozen_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: cryozen gateway stop")
    else:
        print("⚠ Gateway process is running for this profile, but the service is not active")
        print(pids_line)
        print("  This is usually a manual foreground/tmux/nohup run, so `cryozen gateway`")
...
def _print_multiplex_standalone_reason() -> None:
    ...
    if reason:
        ...
        print("  Fold every profile onto this gateway: cryozen gateway migrate --multiplex")
...
def _print_served_ingress_urls(profile: str | None = None) -> None:
    ...
    try:
        from cryozen_cli.gateway_multiplex_served import format_ingress_url_lines, served_profile_ingress_urls
...
def _print_unserved_shared_ingress(profile: str | None) -> None:
    ...
    try:
        from cryozen_cli.gateway_multiplex_served import served_profile_unserved_platforms
...
def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``cryozen gateway status``."""
    try:
        from cryozen_cli.profiles import get_active_profile_name
...
def _print_duplicate_credential_warnings() -> None:
    ...
    with contextlib.suppress(Exception):
        from cryozen_cli.gateway_migrate import duplicate_credential_findings
...
def _gateway_list() -> None:
    ...
    try:
        from cryozen_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            from cryozen_cli.gateway_windows import get_task_name  # profile-aware task name
            _task_name = get_task_name()
        except Exception:
            _task_name = "Cryozen_Gateway"
...
def _reaper_exclusion_pids(extra_exclude: set | None) -> set[int]:
    ...
    with contextlib.suppress(Exception):
        ...
        # cover the whole ai.cryozen.gateway* fleet — not just the current profile's label — or a sibling
...
def stop_profile_gateway() -> bool:
    ...
    if is_windows():
        ...
        # wedged process.
        from gateway.status import get_process_start_time
        from cryozen_cli.gateway_windows import (
            _drain_gateway_pid,
...
from cryozen_constants import is_container, is_termux, is_wsl
...
def _gw_windows():
    """Lazily import :mod:`cryozen_cli.gateway_windows` (Windows-only service backend)."""
    from cryozen_cli import gateway_windows
...
def _gateway_detached_env() -> bool:
    return _truthy_env(os.getenv("CRYOZEN_GATEWAY_DETACHED"))
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    """True for detached Windows gateway runs that should ignore Ctrl+C (``CRYOZEN_GATEWAY_DETACHED=1``
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from cryozen_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "cryozen-gateway"
SERVICE_DESCRIPTION = "Cryozen Agent Gateway - Messaging Platform Integration"
...
def _native_service_homes() -> set[Path]:
    ...
    from cryozen_constants import _get_platform_default_cryozen_home, sudo_invoker_default_home

    homes = {_get_platform_default_cryozen_home().resolve()}
...
def _bare_unit_pinned_home() -> Path | None:
    """Resolved ``CRYOZEN_HOME`` pinned by an installed ``cryozen-gateway.service``, or None. The unit is the
    ...
    if not is_linux() or os.geteuid() != 0:  # windows-footgun: ok — behind is_linux()
        return None
    pinned = _cryozen_home_pinned_by_unit(_SYSTEM_UNIT_DIR / f"{_SERVICE_BASE}.service")
    if not pinned:
        return None
    try:
...
def _profile_suffix() -> str:
    """Service-name suffix for CRYOZEN_HOME: "" for a home that owns the bare name, the profile name for
    ``<root>/profiles/<name>``, else a short hash of the path.

    Bare-name owners: this process's platform-native default (``~/.cryozen-agent``), under sudo the invoking
    user's native default, and the home pinned by an installed ``cryozen-gateway.service``. Under sudo the
    naming basis moves MID-COMMAND — sudo strips CRYOZEN_HOME and sets HOME=/root, then
    ``_sync_cryozen_home_from_systemd_unit()`` adopts the unit's own CRYOZEN_HOME into ``os.environ`` — so a
    basis derived from the process alone names one unit before the adoption and another after it. The
    unit-pinned check must precede the profile branch: ``sudo cryozen gateway install --system`` resolves
    ...
    The bare name is deliberately NOT tied to ``get_default_cryozen_root()``: that helper treats any
    CRYOZEN_HOME outside ``~/.cryozen-agent`` (Docker ``/opt/data``, a temp dir) as "the root itself", which let a
    temp-home harness resolve to the default profile's ``cryozen-gateway`` unit and uninstall the
    ...
    home = get_cryozen_home().resolve()
    if home in _native_service_homes() or home == _bare_unit_pinned_home():
        return ""
    name = _profile_name_from_home(home, get_default_cryozen_root().resolve())
...
def _current_profile_name() -> str:
    ...
    from cryozen_constants import profile_name_for_home
    return profile_name_for_home(get_cryozen_home()) or _profile_suffix()


def _profile_arg(cryozen_home: str | None = None, default_root: str | Path | None = None) -> str:
    """``--profile <name>`` for ``<root>/profiles/<name>``, else "". *cryozen_home*/*default_root* let a
    sudo/root process generate a unit for another user (the defaults would refer to root)."""
    from cryozen_constants import get_default_cryozen_root
    home = Path(cryozen_home or str(get_cryozen_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_cryozen_root().resolve()
    if home == default:
        return ""
...
def get_service_name() -> str:
    """Systemd service name: ``cryozen-gateway`` for default CRYOZEN_HOME, ``cryozen-gateway-<profile>``
    or ``-<hash>`` otherwise."""
...
def _run_systemctl(args: list[str], *, system: bool = False, **kwargs) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        from cryozen_cli.gateway_command_errors import SystemctlUnavailableError
...
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("cryozen.service",)
...
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "cryozen_cli.main gateway",
    "cryozen_cli/main.py gateway",
    "gateway/run.py",
    " cryozen gateway ",
    "/cryozen gateway ",
)
...
def _find_legacy_cryozen_units() -> list[tuple[str, Path, bool]]:
    """``[(unit_name, unit_path, is_system)]`` for legacy gateway units (e.g. ``cryozen.service``), which
    ...
    Detects unit files installed by older Cryozen versions that used a different service name (e.g. When both
    a legacy unit and the current ``cryozen-gateway.service`` are active, they fight over the same bot token
...
def has_legacy_cryozen_units() -> bool:
    """Return True when any legacy Cryozen gateway unit files exist."""
    return bool(_find_legacy_cryozen_units())


def print_legacy_unit_warning() -> None:
    """Warn about installed legacy gateway units; prints nothing when there are none."""
    legacy = _find_legacy_cryozen_units()
    if not legacy:
        return
    print_warning("Legacy Cryozen gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        print_info(f"    {path}  ({_service_scope_label(is_system)} scope)")
    print_info("  These run alongside the current cryozen-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    cryozen gateway migrate-legacy")


def remove_legacy_cryozen_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
    ...
    legacy = _find_legacy_cryozen_units()
    if not legacy:
        print("No legacy Cryozen gateway units found.")
        return 0, []

    print()
    print("Legacy Cryozen gateway unit(s) found:")
    for name, path, is_system in legacy:
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: cryozen gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo cryozen gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    cryozen gateway uninstall")
    print_info("    sudo cryozen gateway uninstall --system")
...
def refuses_container_user_scope_install(system: bool) -> bool:
    ...
    _print_info_lines(
        ...
        "",
        "  cryozen gateway run                                # run as the container's main process",
        ...
        "  sudo cryozen gateway install --system --run-as-user <user>",
    )
    return True
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: cryozen gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``cryozen setup``/``import``).
    ...
    if is_container():
        ...
        print_info("   cryozen gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: cryozen gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: cryozen gateway install")
...
def get_launchd_plist_path() -> Path:
    """``~/Library/LaunchAgents/ai.cryozen.gateway[-<profile>].plist`` under the real account home."""
    import pwd
    suffix = _profile_suffix()
    name = f"ai.cryozen.gateway-{suffix}" if suffix else "ai.cryozen.gateway"
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    the profile layout, NOT by globbing ``~/Library/LaunchAgents``, so a sandboxed CRYOZEN_HOME never
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.cryozen.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.cryozen.gateway-{profile.name}")
...
def legacy_launchd_labels_for_install(exclude=()) -> list[str]:
    ...
    A unit whose label predates the profile-name suffix scheme (``ai.cryozen.gateway-<8hex>`` from the
    ...
    pinned ``CRYOZEN_HOME`` is this install's root or one of its ``profiles/<name>`` homes — ownership
    judged from the plist's content, never from label shape or directory membership — so the
    derivation's boundary holds: a sandboxed CRYOZEN_HOME (tests, side-by-side installs) never
    ...
    try:
        home = Path(pwd.getpwuid(os.getuid()).pw_dir)  # windows-footgun: ok — POSIX launchd (macOS) helper, never invoked on Windows
        root = get_default_cryozen_root().resolve()
    except Exception:
        return []
    ...
    labels: set[str] = set()
    for plist_path in sorted(agents_dir.glob("ai.cryozen.gateway*.plist")):
        try:
            data = plistlib.loads(plist_path.read_bytes())
            label = data["Label"]
            pinned = Path(str(data["EnvironmentVariables"]["CRYOZEN_HOME"])).expanduser().resolve()
        ...
        if not isinstance(label, str) or label in excluded or not label.startswith("ai.cryozen.gateway"):
            continue
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            from cryozen_constants import venv_python_path
        except ImportError:
            # Update-boundary: a gateway restarted mid-update can hold a stale cryozen_constants
            # without this symbol; see _reload_cryozen_constants() in cryozen_cli/managed_uv.py.
            from cryozen_cli.managed_uv import _reload_cryozen_constants
            venv_python_path = _reload_cryozen_constants().venv_python_path
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo cryozen gateway uninstall --system")
    print_info("         cryozen gateway install")
    print_info("         cryozen gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("CRYOZEN_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_cron_drain_timeout() -> float:
    ...
    See #82161.
    """
    return _agent_timeout_setting("CRYOZEN_CRON_DRAIN_TIMEOUT", "cron_drain_timeout", parse_cron_drain_timeout)
...
def _get_restart_exit_wait_budget() -> float:
    ...
    return resolve_restart_exit_wait_budget(
        ...
        _agent_timeout_setting(
            "CRYOZEN_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout", parse_restart_after_turn_timeout
        ),
    )
...
def systemd_install(
    ...
    # Existing system units already pin CRYOZEN_HOME; adopt it before any regenerate.
    if unit_path.exists():
        _sync_cryozen_home_from_systemd_unit(system=system)
    ...
    print()
    print("Next steps:")
    print(f"  {sudo}cryozen gateway start{scope_flag}              # Start the service")
    print(f"  {sudo}cryozen gateway status{scope_flag}             # Check status")
...
def _systemd_unit_belongs_to_current_home(system: bool = False) -> bool:
    """False (with a warning) when the installed unit pins a CRYOZEN_HOME other than this process's: the
    service name then resolved to ANOTHER install's gateway, and stop/disable/unlink would take it down."""
    _sync_cryozen_home_from_systemd_unit(system=system)  # sudo strips CRYOZEN_HOME; adopt the unit's first
    unit_home = _cryozen_home_from_systemd_unit_file(system=system)
    if unit_home is None or Path(unit_home).expanduser().resolve() == get_cryozen_home().resolve():
        return True
    print_warning(
        f"Refusing to remove {get_systemd_unit_path(system=system)}: it runs CRYOZEN_HOME={unit_home}, "
        f"but this process has CRYOZEN_HOME={get_cryozen_home()}"
    )
    return False
...
def _print_service_not_installed(system: bool) -> None:
    ...
    print(f"  Run: {sudo}cryozen gateway install{scope_flag}")
...
def systemd_start(system: bool = False):
    ...
    # CRYOZEN_HOME sync happens in refresh's systemd_unit_is_current gate; the unit is guaranteed to exist here.
    refresh_systemd_unit_if_needed(system=system)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `cryozen gateway status` or logs for final shutdown state."
        )
        return
...
def systemd_restart(system: bool = False):
    ...
    # CRYOZEN_HOME sync happens in refresh's systemd_unit_is_current gate; its os.environ mutation
    ...
    if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
        ...
        print(
            ...
            # graceful shutdown, so waiting the full drain budget only stalls the restart (and `cryozen
...
def _systemd_graceful_restart_action(system: bool, pid: int) -> str | None:
    ...
    from cryozen_cli.update_cmd_drain_report import drain_progress_reporter
...
def _systemd_reset_and_run(action: str, *, system: bool, previous_pid) -> None:
    ...
    try:
        ...
        print(
            ...
            "check `cryozen gateway status` or logs for final state."
        )
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {sudo}cryozen gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if result.stdout.strip() == "active":
        ...
        print(f"  Run: {sudo}cryozen gateway start{scope_flag}")
    ...
    if active_state == "activating" and unit_props.get("SubState", "") == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {sudo}cryozen gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {user_flag}reset-failed {svc} && {sudo}cryozen gateway start{scope_flag}")
...
from cryozen_cli.gateway_launchd import (  # noqa: E402,F401 — facade re-exports; tests patch here
    get_launchd_label,
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    CRYOZEN_HOMEs work) to exit; SIGKILL it after ``force_after`` s of graceful waiting."""
...
def _is_official_docker_checkout() -> bool:
    return str(PROJECT_ROOT) == "/opt/cryozen" and (PROJECT_ROOT / "docker" / "entrypoint.sh").is_file()
...
def _running_under_gateway_supervisor() -> bool:
    ...
    (shells inherit "0"), s6 CRYOZEN_S6_SUPERVISED_CHILD, or ``--external-supervisor``."""
...
def _served_by_another_host_gateway(profile_name: str | None = None):
    ...
    you" would make `cryozen gateway restart` impossible for the profile that launched the host
    ...
    if gateway is None:
        return None
    try:
        from gateway.status import _get_process_cryozen_home, _same_cryozen_home
        if _same_cryozen_home(gateway.home, _get_process_cryozen_home()):
            return None
    except Exception:
...
def named_profile_served_by_running_multiplexer(profile_name: str | None = None) -> bool:
    ...
    try:
        from cryozen_constants import get_default_cryozen_root
        default_root = get_default_cryozen_root()
    except Exception:
        return False

    try:
        from cryozen_cli.gateway_multiplex_served import live_default_gateway_pid, recorded_served_profiles
        if live_default_gateway_pid() is None:
            return False
        from cryozen_cli.profiles import normalize_profile_name
        # The live gateway's own record wins: the CLI process cannot see an env-only opt-in on the
        # default profile (`cryozen -p X` loads X's .env) and a config edit after start is not live yet.
        ...
        from cryozen_cli.gateway_multiplex_mode import explicit_multiplex_flag
...
def _served_profile_needs_no_service() -> bool:
    ...
    Shared by ``cryozen setup gateway`` / ``cryozen setup`` / ``cryozen import`` (``ensure_gateway_service``)
    and the ``cryozen gateway setup`` wizard. See #111958."""
    if not named_profile_served_by_running_multiplexer():
        ...
        # as `gateway install`, so `cryozen -p X setup` cannot grow a fleet member the verb refuses.
        return _named_profile_refused_under_multiplexer()
    from cryozen_cli.profiles import profile_is_standalone
    if profile_is_standalone(get_cryozen_home()):
...
def _named_profile_refused_under_multiplexer(force: bool = False) -> bool:
    ...
    ``--force`` is the one escape (a fleet split across UNIX users or a ``CRYOZEN_HOME`` outside
    ...
    try:
        ...
        standalone = (profile_name_for_home(get_cryozen_home()) not in (None, "default")
                      and profile_is_standalone(get_cryozen_home()))
        ...
        new_standalone = (profile_name_for_home(get_cryozen_home()) not in (None, "default")
    ...
    if served:
        ...
        print()
        print(f"    cryozen -p {owner.profile_label if owner is not None else 'default'} gateway restart")
    else:
        print("  Install or start the host gateway from the default profile; it serves this one too:")
        print()
        print("    cryozen gateway install")
        print()
        print("  Or fold an existing per-profile fleet onto one host gateway:")
        print()
        print("    cryozen gateway migrate --multiplex")
    print()
    print("  A separate per-profile gateway (for a fleet split across UNIX users or a")
    print(f"  CRYOZEN_HOME outside profiles/) needs --force:  cryozen -p {suffix} gateway install --force")
    print()
    from cryozen_constants import display_cryozen_home
    from cryozen_cli.gateway_multiplex_mode import STANDALONE_DEPRECATION_NOTICE
    print("  Temporary compatibility path while multiplexing gaps are closed: set")
    print(f"  gateway.standalone: true in {display_cryozen_home(get_cryozen_home())}/config.yaml,")
...
def _attach_to_host_gateway_or_guard(force: bool = False, replace: bool = False) -> None:
    ...
    try:
        ...
        decision = decide(get_cryozen_home(), replace=replace)
    except Exception:
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    "already running". Same user-facing contract; never scans other CRYOZEN_HOME roots."""
    ...
    try:
        ...
        return
    if pid is None:
        # get_running_pid() filters by the current profile's CRYOZEN_HOME; warn if the PID file
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                logger.warning(
                    "PID file belongs to another profile (cryozen_home=%s). "
                    "The old gateway may still be running under that profile.",
                    stale.get("cryozen_home", "<unknown>"),
                )
        except Exception:
            pass
        return
    ...
    print("  Check with `cryozen gateway status`.")
    print("  To restart it: `cryozen gateway restart`. To stop it: `cryozen gateway stop`.")
    print("  To replace it from here: `cryozen gateway run --replace`.")
    sys.exit(1)


def _guard_official_docker_root_gateway() -> None:
    """Refuse gateway startup when the official Docker privilege drop was bypassed."""
    if not hasattr(os, "geteuid") or os.geteuid() != 0 or _truthy_env(os.getenv("CRYOZEN_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error("Refusing to run the Cryozen gateway as root inside the official Docker image.")
    print(
        "  The image entrypoint normally drops privileges to the 'cryozen' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/cryozen/docker/entrypoint.sh before the Cryozen command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$CRYOZEN_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set CRYOZEN_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def _apply_startup_watchdog_config() -> None:
    ...
    try:
        from cryozen_startup_watchdog import (
        ...
        try:
            from cryozen_cli.config import load_config as _sw_load_config
...
def _make_exit_diag():
    ...
    ``asyncio.run()`` can return, for chasing silent Windows gateway deaths. CRYOZEN_GATEWAY_EXIT_DIAG=0 opts out."""
    from datetime import datetime as _dt, timezone as _tz

    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("CRYOZEN_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from cryozen_constants import get_cryozen_home as _ghh
            log_dir = _ghh() / "logs"
...
def _respawn_storm_backoff() -> None:
    ...
    DEFAULT_CONFIG ``gateway.respawn_storm``; CRYOZEN_GATEWAY_MAX_STARTS / CRYOZEN_GATEWAY_START_WINDOW_S
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            _max_starts = int(os.environ["CRYOZEN_GATEWAY_MAX_STARTS"])
        except (KeyError, ValueError):
            pass
        try:
            _win = float(os.environ["CRYOZEN_GATEWAY_START_WINDOW_S"])
        except (KeyError, ValueError):
            pass
        ...
        if _storm is not None:
            ...
            try:
                from cryozen_startup_watchdog import kick_startup_watchdog
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Detached Windows runs (CRYOZEN_GATEWAY_DETACHED=1, or non-TTY for older wrappers) ignore
    ...
    # exit-code-75 respawn (stale-code or /restart), which bypasses `cryozen gateway restart`.
    if supports_systemd_services():
        try:
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ✻ Cryozen Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
from cryozen_cli.gateway_setup_wizard import (  # noqa: E402,F401 — facade re-exports; tests patch here
    _PLATFORMS,
    _all_platforms,
    _platform_status,
...
def _runtime_health_lines() -> list[str]:
    ...
    if gateway_state == "startup_failed" and exit_reason:
        ...
        if isinstance(work, list) and work:
            from cryozen_cli.update_cmd_drain_report import describe_active_work_unit
...
def _running_under_s6() -> bool:
    from cryozen_cli.service_manager import detect_service_manager
...
def _dispatch_via_service_manager_if_s6(action: str, profile: str | None = None) -> bool:
    ...
    from cryozen_cli.service_manager import (
...
def gateway_command(args):
    ...
    try:
        ...
        from cryozen_cli.gateway_command_errors import explain_service_failure
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    ``CRYOZEN_S6_SUPERVISED_CHILD`` (set by ``S6ServiceManager._render_run_script``) marks the supervised
    child, which must run in foreground or we'd recurse run → start → run; ``--no-supervise`` /
    CRYOZEN_GATEWAY_NO_SUPERVISE=1 opts out (CI smoke, debugging)."""
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("CRYOZEN_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    # CRYOZEN_S6_SUPERVISED_CHILD: we ARE the supervised child; fall through so the gateway starts.
    if no_supervise or os.environ.get("CRYOZEN_S6_SUPERVISED_CHILD"):
        return False
    if not _dispatch_via_service_manager_if_s6("start"):
        return False
    # This process never reaches a GatewayRunner, so the watchdog armed by cryozen_cli.main's argv
    ...
    from cryozen_startup_watchdog import disarm_startup_watchdog
    ...
    print(
        ...
        "  dashboard supervised alongside if CRYOZEN_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or CRYOZEN_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    try:
        ...
        # down the supervised gateway cleanly). Prefer `sleep infinity` (matches the static main-cryozen
        # service's pattern in docker/s6-rc.d/main-cryozen/run, and frees the Python interpreter — the
...
def _refuse_from_inside_gateway(verb: str, reason: str) -> None:
    ...
    if _is_supervised_gateway_process():
        print_error(
            ...
            f"Use `cryozen gateway {verb}` from a shell outside the running gateway."
        )
        sys.exit(1)
...
_WSL_FOREGROUND_HINT = (
    "", "  cryozen gateway run                              # direct foreground",
    "  tmux new -s cryozen 'cryozen gateway run'         # persistent via tmux",
    "  nohup cryozen gateway run > ~/.cryozen-agent/logs/gateway.log 2>&1 &  # background",
)
...
_NO_BACKEND_MESSAGES = {
    ("install", "termux"): (1,
        "Gateway service installation is not supported on Termux.", "Run manually: cryozen gateway"),
    ("install", "wsl"): (1,
    ...
    ("install", "s6"): (None,
        ...
        "  cryozen profile create <name>     # creates the s6 service slot",
        "  cryozen -p <name> gateway start   # bring it up via s6",
        "  cryozen status                    # see currently-supervised gateways"),
    ("install", "container"): (0,
        ...
        "To run the gateway: cryozen gateway run"),
    ("install", "unsupported"): (1,
        "Service installation not supported on this platform.", "Run manually: cryozen gateway run"),
    ("uninstall", "termux"): (1,
        "Gateway service uninstall is not supported on Termux because there is no managed service to remove.",
        "Stop manual runs with: cryozen gateway stop"),
    ("uninstall", "s6"): (None,
        "Per-profile gateways are auto-unregistered when you delete the profile.", "",
        "  cryozen profile delete <name>     # tears down the s6 service slot",
        "  cryozen -p <name> gateway stop    # stop without deleting the profile"),
    ...
    ("start", "container"): (0,
        ...
        "Or run the gateway directly: cryozen gateway run"),
    ("start", "unsupported"): (1,
        "Running the gateway as a background service is not available on this platform "
        "(no systemd, launchd or Scheduled Tasks).",
        "Run it directly with: cryozen gateway run"),
}
...
def _install_systemd_from_cli(args, *, force: bool, system: bool, run_as_user) -> None:
    if is_wsl():
        ...
        _print_info_lines(
            "  Consider running in foreground instead: cryozen gateway run",
            "  Or use tmux/screen for persistence: tmux new -s cryozen 'cryozen gateway run'",
        )
        print()
...
def _host_multiplexer_is_ours(owner) -> bool:
    try:
        from gateway.status import _get_process_cryozen_home, _same_cryozen_home
        return bool(_same_cryozen_home(owner.home, _get_process_cryozen_home()))
    except Exception:
        return False
...
def _print_unfolded_gateway_note(owner) -> None:
    ...
    print("  They were left running; fold them in with: cryozen gateway migrate --multiplex")
...
def _cmd_stop(args):
    ...
    if not stop_all and not find_gateway_pids() and (
        ...
        print(f"    cryozen {owner_flag}gateway stop      # takes every served profile offline")
        print(f"    cryozen {owner_flag}gateway restart")
        sys.exit(GATEWAY_FATAL_CONFIG_EXIT_CODE)
...
def _restart_all(system: bool) -> None:
    ...
    if owner is not None and not _host_multiplexer_is_ours(owner):
        ...
        print()
        print(f"    cryozen -p {owner.profile_label} gateway restart --all")
        sys.exit(GATEWAY_FATAL_CONFIG_EXIT_CODE)
...
def _cmd_restart(args):
    ...
    # detached run below is the restart; bailing here left `cryozen gateway restart` a
    ...
    if kind == "systemd" and supports_systemd_services():
        ...
        if linger_ok is not True:
            ...
            _print_lines(
                ...
                "  Then restart the gateway:", "    cryozen gateway restart",
            )
            return
    ...
    if service_configured:
        _print_lines(
            ...
            "  Fix the service, then retry: cryozen gateway start",
        )
        sys.exit(1)
    ...
    from gateway.status import get_running_pid
    from cryozen_cli.gateway_supervised_restart import (
...
# ``cryozen gateway status`` hints for a manually-run / stopped gateway, keyed by host kind.
_STATUS_RUNNING_HINTS = {
    ...
    "windows": ("To install as a Windows Scheduled Task (auto-start on login):", "  cryozen gateway install"),
    "other": (
        "To install as a service:", "  cryozen gateway install", "  sudo cryozen gateway install --system",
    ),
}
_STATUS_STOPPED_HINTS = {
    "termux": (
        "  nohup cryozen gateway run > ~/.cryozen-agent/logs/gateway.log 2>&1 &  # Best-effort background start",
    ),
    "wsl": (
        "  tmux new -s cryozen 'cryozen gateway run'         # persistent via tmux",
        "  nohup cryozen gateway run > ~/.cryozen-agent/logs/gateway.log 2>&1 &  # background",
    ),
    "windows": ("  cryozen gateway install  # Install as Windows Scheduled Task (auto-start on login)",),
    "other": (
        "  cryozen gateway install  # Install as user service",
        "  sudo cryozen gateway install --system  # Install as boot-time system service",
    ),
}
...
def _cmd_status(args):
    ...
    from cryozen_cli.profiles import get_active_profile_name, profile_is_standalone
    ...
    if not active_standalone and not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        print("  Manage it from the default profile: cryozen gateway status")
        ...
        if pids:
            ...
            print()
            print("To start:")
            print("  cryozen gateway run      # Run in foreground")
...
def _print_standalone_by_config() -> None:
    ...
    from cryozen_cli.profiles import get_active_profile_name, profiles_to_serve
...
def _cmd_migrate_legacy(args):
    """Stop, disable, and remove legacy Cryozen gateway unit files (e.g. cryozen.service)."""
    dry_run = getattr(args, "dry_run", False)
    yes = getattr(args, "yes", False)
    if not supports_systemd_services() and not is_macos():
        print("Legacy unit migration only applies to systemd-based Linux hosts.")
        return
    remove_legacy_cryozen_units(interactive=not yes, dry_run=dry_run)


def _cmd_migrate(args):
    from cryozen_cli.gateway_migrate import cmd_migrate
    cmd_migrate(args)
