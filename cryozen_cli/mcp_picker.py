"""MCP picker — interactive `cryozen mcp picker` (also the default `cryozen mcp`)."""
...
from cryozen_cli.colors import Colors
from cryozen_cli.cli_output import prompt_yes_no
from cryozen_cli.curses_ui import curses_single_select
from cryozen_cli.mcp_catalog import (
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    _say(
        ...
        "Start a new Cryozen session for changes to take effect."
    )
...
def _configure_tools(name: str) -> None:
    ...
    from cryozen_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `cryozen mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: cryozen mcp install <name>    Picker: cryozen mcp", Colors.DIM)
    # Manifest-version warnings: the user's Cryozen is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer Cryozen — run `cryozen update` to install this entry.", Colors.YELLOW)
        print()
    print()


def show_catalog() -> None:
    """`cryozen mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`cryozen mcp picker` (and default `cryozen mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    """`cryozen mcp install <name>` — non-interactive entry-point."""
    from cryozen_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `cryozen mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
...
_PLUGIN_COMPAT_LAZY = {
    'color': ('cryozen_cli.colors', 'color'),
}
