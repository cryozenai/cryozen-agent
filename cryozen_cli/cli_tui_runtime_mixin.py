...
class CLITuiRuntimeMixin:
    ...
    def _tui_print_startup(self):
        ...
        _welcome_text = "Welcome to Cryozen Agent! Type your message or /help for commands."
    ...
    def _tui_startup_prewarm_and_warnings(self, _welcome_skin):
        ...
        with suppress(Exception):
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(CRYOZEN_REDACT_SECRETS={_redact_raw}). "
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from cryozen_cli.config import get_config_path as _get_cfg_path_resid
    ...
    def _tui_startup_background_maintenance(self):
        """Best-effort startup pass: curator skill maintenance.

        Off the main thread: the curator's deterministic pass snapshots and prunes the whole
        skills tree (a due weekly pass held the prompt for 6 minutes on a large library).
        The REPL must never wait on housekeeping."""
        ...
        if self._session_db and self.agent:
            ...
            if not self._delete_session_on_exit:
                ...
                try:
                    ...
                    if self._session_db.delete_session(_sid, sessions_dir=get_cryozen_home() / "sessions"):
