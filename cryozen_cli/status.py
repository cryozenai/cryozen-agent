"""Status command for cryozen CLI."""
...
from cryozen_cli.config import get_env_path, get_env_value, get_cryozen_home, load_config
from cryozen_cli.models import provider_label
from cryozen_cli.runtime_provider import resolve_requested_provider
from cryozen_cli.vercel_auth import describe_vercel_auth
from cryozen_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers)
from cryozen_constants import OPENROUTER_MODELS_URL
...
def _estop_status_line():
    """One-line pause banner for `cryozen status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `cryozen resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'cryozen-agent[vercel]')")
        ...
        # (fail-soft — never break `cryozen status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from cryozen_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if not snapshot.running and named_profile_served_by_running_multiplexer():
            ...
            _kv("Manage with:", "cryozen gateway status   # from the default profile")
            return
        ...
        if snapshot.running and (served := multiplexer_served_secondaries()):
            ...
            from cryozen_cli.gateway_multiplex_served import served_profile_ingress_urls
        ...
        if snapshot.has_process_service_mismatch:
            ...
            _kv("Start with:", "cryozen gateway")
...
def _render_cron(ctx):
    ...
    jobs_file = get_cryozen_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from cryozen_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_cryozen_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from cryozen_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'cryozen doctor' for detailed diagnostics", "  Run 'cryozen setup' to configure"),
            Colors.DIM)
    print()


# Print order of `cryozen status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
    _render_header, _render_environment, _render_api_keys, _render_auth_providers,
...
def show_status(args):
    """Show status of all Cryozen Agent components."""
    # Shared by section renderers: config and --deep.
    ctx = SimpleNamespace(deep=getattr(args, 'deep', False), config={})
