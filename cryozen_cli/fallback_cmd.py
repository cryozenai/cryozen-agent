"""cryozen fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `cryozen model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  cryozen fallback add\n")
        return
    print()
    ...
    print("  Docs: https://cryozenai.github.io/cryozen-agent/docs/user-guide/features/fallback-providers\n")


def cmd_fallback_add(args) -> None:
    """Launch the same picker as `cryozen model`, then append the selection to the chain."""
    from cryozen_cli.main import _require_tty, select_provider_and_model
    from cryozen_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `cryozen model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `cryozen fallback list` to view, or `cryozen fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``cryozen fallback [subcommand]``."""
