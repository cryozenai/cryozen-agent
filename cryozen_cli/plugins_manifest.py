...
Split out of :mod:`cryozen_cli.plugins`; validation warns and never fails a load.
"""
...
# and continue loading. ``capabilities``/``emits``/``listens``/``cryozen``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "cryozen", "depends",
    "requires_cryozen", "python_runtime",
}

# Highest manifest schema version this Cryozen understands.
...
_CONFIG_SCHEMA_TYPES: Dict[str, tuple] = {
    ...
    # ``secret`` values live in ``.env`` (see cryozen_cli.plugins_settings), so a config.yaml copy is
...
def _plugins_debug() -> bool:
    from cryozen_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Cryozen "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `cryozen plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `cryozen plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Cryozen version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_cryozen_error``.
    requires_cryozen: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Cryozen never auto-installs these (isolation design for the install seam
...
# ── requires_cryozen version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_cryozen_version() -> str:
    """Version of the Cryozen code that is running: ``cryozen_cli.__version__``. Distribution metadata is only
    ...
    try:
        ...
        pass
    return importlib.metadata.version("cryozen-agent")
...
def requires_cryozen_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_cryozen`` rejects the running version."""
    if not manifest.requires_cryozen:
        return None
    current = running_cryozen_version()
    if version_satisfies(manifest.requires_cryozen, current):
        return None
    return f"requires cryozen {manifest.requires_cryozen}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_cryozen=str(data.get("requires_cryozen") or "").strip(),
