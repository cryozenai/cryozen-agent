"""Shell completion script generation for cryozen CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_cryozen_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Cryozen Agent bash completion
# Add to ~/.bashrc:
#   eval "$(cryozen completion bash)"

_cryozen_profiles() {{
    local profiles_dir="$HOME/.cryozen-agent/profiles"
...
_cryozen_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_cryozen_profiles)" -- "$cur"))
        return
    fi
...
complete -F _cryozen_completion cryozen
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef cryozen
# Cryozen Agent zsh completion
# Add to ~/.zshrc:
...
_cryozen_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.cryozen-agent/profiles" ]]; then
        profiles+=($HOME/.cryozen-agent/profiles/*(N/:t))
    fi
...
_cryozen() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_cryozen_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'cryozen command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Cryozen Agent fish completion",
        "# Add to your config:",
        "#   cryozen completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __cryozen_profiles",
        "    echo default",
        "    if test -d $HOME/.cryozen-agent/profiles",
        "        for d in $HOME/.cryozen-agent/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c cryozen -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c cryozen -f -s p -l profile"
        " -d 'Profile name' -xa '(__cryozen_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c cryozen -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__cryozen_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
