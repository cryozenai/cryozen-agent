"""Skills configuration for Cryozen Agent. `cryozen skills` enters this module."""
from typing import List, Optional, Set

from cryozen_cli.config import cfg_get, load_config, save_config
from cryozen_cli.colors import Colors, color
from cryozen_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    """Persist disabled skill names to config; essential skills (e.g. ``cryozen-agent``) are
...
def skills_command(args=None):
    """Entry point for `cryozen skills`."""
    from cryozen_cli.curses_ui import curses_checklist
    config = load_config()
