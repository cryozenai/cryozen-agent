"""MCP catalog — curated, maintainer-approved MCP servers shipped with the repo.

Entries are added only by merging a PR into cryozen-agent; presence in ``optional-mcps/`` = maintainer
...
from cryozen_constants import get_cryozen_home, get_optional_mcps_dir
from cryozen_cli._subprocess_compat import noninteractive_git_env
from cryozen_cli.colors import Colors, color
from cryozen_cli.config import load_config, save_config, get_env_value, save_env_value
from cryozen_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # go through auth.env so they are prompted for and land in ~/.cryozen-agent/.env.
...
def _parse_auth(path: Path, raw: Any, name: str, http: bool) -> AuthSpec:
    ...
    if http and a_type == "api_key":
        ...
        from cryozen_cli.mcp_config import _env_key_for_server
...
def _parse_suggest(path: Path, suggest_raw: Any) -> Optional[SuggestSpec]:
    ...
    from cryozen_cli.mcp_app_detection import validate_applications
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Cryozen understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    skipped too but surfaced via :func:`catalog_diagnostics` so UIs can say "update Cryozen".
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    ``kind`` is ``future_manifest`` (newer than this Cryozen) or ``invalid`` (malformed)."""
...
def remove_server(name: str) -> bool:
    ...
    from cryozen_cli.mcp_config import _remove_mcp_server
...
def _install_root() -> Path:
    ...
    root = get_cryozen_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.cryozen-agent/mcp-installs/<name>`` and run bootstrap. Returns the dir."""
...
def _prompt_env_vars(specs: List[EnvVarSpec], preloaded: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    Secrets persist to ~/.cryozen-agent/.env. Non-secrets are only collected and
...
def _build_server_config(entry: CatalogEntry, install_dir: Optional[Path]) -> dict:
    """Translate a manifest into the ``mcp_servers.<name>`` block format used by cryozen_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from cryozen_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    if not server_cfg:
        return None
    try:
        from cryozen_cli.mcp_config import _probe_single_server  # lazy: keep this module cheap
...
def _apply_tool_selection(
    ...
    *,
    ...
    else apply ``default_enabled``, else no filter; point the user at ``cryozen mcp configure``.
    """
    print()
    name = entry.name
    configure_hint = f"`cryozen mcp configure {name}`"
    ...
    if len(chosen_indices) == len(probed):
        ...
        # re-run `cryozen mcp configure <name>` and unselect a tool (switches to include-mode).
...
def install_entry(entry: CatalogEntry, *, enable: bool = True, preloaded_env: Optional[Dict[str, str]] = None) -> None:
    ...
    if entry.auth.type == "oauth" and entry.auth.provider:
        # Provider-mediated OAuth relies on the existing `cryozen auth <provider>` flow; surface
        ...
        _say(
            ...
            f"`cryozen auth {entry.auth.provider}` if you have not "
    ...
    from cryozen_cli.mcp_config import _save_mcp_server
    ...
    _say(
        ...
        f"Start a new Cryozen session to load its tools."
    )
