"""``cryozen sessions repair-profiles`` — the CLI face of :mod:`cryozen_cli.sessions_repair_profiles`.
...
from cryozen_cli.sessions_repair_profiles import (
...
def cmd_repair_profiles(args) -> int:
    ...
    if apply:
        ...
        if live:
            ...
            print(f"A gateway is running for: {names}. It holds the routing index in memory and would "
                  "write it back over this repair. Stop it (`cryozen gateway stop`), then re-run --apply.",
                  file=sys.stderr)
            return 1
    ...
    try:
        ...
        if not getattr(args, "yes", False) and not as_json:
            from cryozen_cli.sessions_cmd import _confirm_prompt
