"""Unified tool configuration for Cryozen Agent."""
...
from cryozen_cli.cli_output import print_info as _print_info
from cryozen_cli.colors import Colors, color
from cryozen_cli.config import cfg_get, load_config, save_config, get_env_value
from cryozen_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
from cryozen_cli.toolset_scope import (
...
# Re-exports: keep ``cryozen_cli.tools_config.X`` callers and test patch targets resolving.
from cryozen_cli.tools_config_cua import (  # noqa: F401
...
from cryozen_cli.tools_config_post_setup import (  # noqa: F401
...
from cryozen_cli.tools_config_providers import (  # noqa: F401
...
from cryozen_cli.tools_config_mcp import (  # noqa: F401
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("connections",     "🔌 Connections",               "install and authorize local MCP servers"),
...
# OFF by default for new installs (still in _CRYOZEN_CORE_TOOLS; the checklist won't pre-select them). x_search
...
# Config-only capabilities: provider setup in `cryozen tools` (TOOL_CATEGORIES) but not model toolsets — zero
...
def _xai_credentials_present() -> bool:
    ...
    try:
        from cryozen_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from cryozen_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        ...
        from cryozen_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Toolset keys the ``cryozen tools`` checklist offers for ``platform`` (mirrors ``_prompt_toolset_checklist``);
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``cryozen-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"cryozen-{platform}"
...
def _row(name: str, badge: str = "", tag: str = "", env_vars: list = (), **markers) -> dict:
    """One TOOL_CATEGORIES provider row; ``markers`` are the ``*_provider`` / ``post_setup`` keys."""
    row = {"name": name}
    if badge:
        row["badge"] = badge
    if tag:
...
_LANGFUSE_PUBLIC = ("CRYOZEN_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("CRYOZEN_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        "icon": "🔍",
        # Provider rows come from plugins.web.<vendor> via _plugin_web_search_providers(). Only the
        # non-provider self-hosted firecrawl setup-flow row lives here.
        "providers": [
    ...
    "video_gen": {
        ...
        # Plugin-backed rows (FAL, xAI, …) are injected at runtime by ``_plugin_video_gen_providers()`` in
        # ``_visible_providers``.
        "providers": [],
    },
    "x_search": {
        ...
        "setup_note": (
            "Cryozen routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        # first so a fresh install's Enter lands on the free local backend (index 0), never on a paid row.
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by Cryozen, text-only (no screenshots)",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; CRYOZEN_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("CRYOZEN_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from cryozen_cli.plugins import get_portable_mcp_server_names_nowait
...
#: "declined": saving ``cryozen tools`` freezes a platform's composite into an explicit list nothing adds to, so
#: a later toolset stays off forever for picker users while ``[cryozen-cli]`` users inherit it.
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (cryozen-acp, cryozen-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``cryozen-cli`` would re-enable disabled toolsets). A mixed list (``[cryozen-cli, spotify]``) still expands the
...
def _coerce_platform_toolsets_value(value, platform: str):
    ...
    The parser is shared with ``cryozen doctor`` and ``cryozen plugins`` (``toolset_validation``
    ...
    if platform not in _warned_invalid_platform_toolsets:
        ...
        logger.warning(
            ...
            "Run `cryozen tools` to reconfigure.", platform, value)
    return value
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly saved list (even a composite like ``cryozen-discord``) is an opt-in to the platform's
    ...
    # Plugin toolsets are first-class on a saved list: ``[cryozen-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[cryozen-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
    ...
    # above. It may arrive as a JSON-array string ("['memory']") from `cryozen config set` or a JSON-mode editor.
...
def _prune_toolsets_stripped_by_disabled(enabled_toolsets: Set[str], disabled_names: List[str]) -> Set[str]:
    ...
    never appear in the list. A name-only subtraction here left inspection surfaces (``cryozen tools
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("cryozen-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``cryozen`` for ``cryozen-cli`` → no
    ...
    if named and not any(validate_toolset(t) for t in named) and platform not in _warned_invalid_platform_toolsets:
        ...
        logger.warning(
            ...
            "name(s): %s) - tools will be unavailable. Run `cryozen tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (cryozen-cli, ...) resolve to ALL tools and would silently override the user's
...
def _toolset_has_keys(
    ts_key: str, config: dict = None, *, force_fresh: bool = False,
) -> bool:
    ...
    if cat:
        return any(_provider_env_ready(p) for p in _visible_providers(cat, config, force_fresh=force_fresh))
...
def _print_tools_summary(config: dict, enabled_platforms: List[str]) -> None:
    """``cryozen tools --summary``: enabled toolsets per platform, non-interactive."""
    total = len(_get_effective_configurable_toolsets())
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `cryozen tools` / `cryozen setup tools`. ``first_install`` skips the menu (checklist + key
    ...
    if getattr(args, "summary", False):
        ...
        return
    print(color("✻ Cryozen Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://cryozenai.github.io/cryozen-agent/docs/user-guide/features/tools", Colors.DIM))
    print()
    if first_install:
    ...
    print(color(f"  Tool configuration saved to {display_cryozen_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'cryozen' or gateway restart.", Colors.DIM))
    print()
