"""Profile distributions — shareable, packaged Cryozen profiles via git.
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "cryozen_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".cryozen_history",
    # User data
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            cryozen_requires=_str(data, "cryozen_requires"), author=_str(data, "author"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional = (
            ("description", self.description), ("cryozen_requires", self.cryozen_requires),
...
def check_cryozen_requires(spec: str, current_version: str) -> None:
    ...
    if not _VERSION_OPS[op](_parse_semver(current_version), _parse_semver(target)):
        raise DistributionError(f"This distribution requires Cryozen {op}{target}, but you have {current_version}.")
    ...
    *workdir* (``.git`` removed); a local directory is used in place."""
    ...
    if _looks_like_git_url(src_str):
        ...
        missing = (
            ...
            "This repository is not a Cryozen profile distribution."
        )
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from cryozen_cli.profiles import _canon_valid, get_profile_dir
    ...
    if manifest is None:
        raise DistributionError(
            f"No {MANIFEST_FILENAME} found at the distribution root — this source is not a Cryozen distribution."
        )
    check_cryozen_requires(manifest.cryozen_requires, cryozen_version)  # fail fast
    canon = _canon_valid(override_name or manifest.name)
    if canon == "default":
        raise DistributionError(
            "Cannot install a distribution as 'default' — that is the built-in "
            "root profile (~/.cryozen-agent).  Pass --name <name> to install under a new profile."
        )
    manifest.name = canon
...
def _copy_dist_payload(staged: Path, target: Path, manifest: DistributionManifest, preserve_config: bool) -> None:
    ...
    if any(rel_parts == ("profile.yaml",) for _, rel_parts in entries):
        from cryozen_cli.profiles import drop_profile_role
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    from cryozen_cli.profiles import _PROFILE_DIRS
...
def install_distribution(
    ...
    from cryozen_cli.profiles import check_alias_collision, create_wrapper_script
    with tempfile.TemporaryDirectory(prefix="cryozen_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `cryozen profile update` to upgrade in place, or pass --force to overwrite."
            )
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from cryozen_cli.profiles import _existing_profile_dir
...
def update_distribution(profile_name: str, force_config: bool = False) -> InstallPlan:
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`cryozen profile install <source> --name {canon} --force`."
        )
    with tempfile.TemporaryDirectory(prefix="cryozen_dist_update_") as tmp:
