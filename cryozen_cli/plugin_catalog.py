"""Plugin catalog — curated, maintainer-approved Cryozen plugins shipped with the repo.
...
an installed Cryozen sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://cryozenai.github.io/cryozen-agent/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requires_cryozen": self.requires_cryozen,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_cryozen=str(data.get("requires_cryozen") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_cryozen_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``CRYOZEN_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
    ...
    try:
        ...
        mkdir_under_cryozen_home(cache.parent)
...
def load_catalog_live() -> List[PluginCatalogEntry]:
    ...
    different pins the NEWER source supplies it — right after ``cryozen update`` bumps an in-tree pin,
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_cryozen:
        bits.append(f"Requires Cryozen {entry.requires_cryozen}.")
    return " ".join(bits)
