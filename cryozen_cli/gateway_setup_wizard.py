"""Gateway platform setup wizard (cryozen gateway setup): platform registry/status table, per-platform setup prompts, service offer.
...
import subprocess
import sys
from cryozen_cli.setup import print_success  # def-time binding (table value)
from cryozen_cli.setup import print_warning  # def-time binding (table value)
...
_PLATFORMS = [
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. cryozen) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL",
             ...
             "password": False, "help": "Channel ID where Cryozen delivers cron results and notifications."},
    ...
    {"key": "weixin", "label": "Weixin / WeChat", "emoji": "💬", "token_var": "WEIXIN_ACCOUNT_ID"},
    {
        ...
        "setup_instructions": [
            ...
            "5. Cryozen connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: cryozen pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Cryozen will connect automatically over WebSocket",
        ],
        "vars": [
...
_UNAUTHORIZED_ACCESS_CHOICES = {
    ...
    False: (1,
        ...
        "Use DM pairing (unknown users request access, you approve with 'cryozen pairing approve')",
        "Politely decline unknown senders (one-time message, then silence)",
...
def _prompt_unauthorized_access(platform_key: str) -> None:
    ...
    if access_idx == 0:
        ...
        _gw().print_info("  Approve with: cryozen pairing approve <platform> <code>")
    elif access_idx == 2:
        ...
        _gw().print_info("  Skipped — configure later with 'cryozen gateway setup'")
...
def _prompt_allowlist_var(var: dict, platform_key: str) -> str | None:
...
def _setup_standard_platform(platform: dict):
    ...
    for var in setup_vars:
        ...
        if var.get("is_allowlist"):
            saved = _prompt_allowlist_var(var, platform.get("key"))
...
def _setup_weixin():
    ...
    print()
    _gw()._print_info_lines(
        "  1. Cryozen will open Tencent iLink QR login in this terminal.",
        "  2. Use WeChat to scan and confirm the QR code.",
        "  3. Cryozen will store the returned account_id/token in ~/.cryozen-agent/.env.",
    ...
    if not check_weixin_requirements():
        ...
        _gw().print_info("  Install them, then rerun `cryozen gateway setup`.")
        return
    ...
    try:
        credentials = _gw().asyncio.run(qr_login(str(_gw().get_cryozen_home())))
    ...
    if access_idx == 2:
        ...
        if access_idx == 0:
            _gw().print_info(
                "  Unknown DM users can request access and you approve them with `cryozen pairing approve`."
            )
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        _gw().print_info("  Unknown users can request access; approve with `cryozen pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        _gw()._print_info_lines(
            ...
            '    signal-cli link -n "CryozenAgent"',
...
def _builtin_setup_fn(key: str):
    """Resolve a built-in platform's setup function; late-bound to dodge the cryozen_cli.setup cycle."""
    from cryozen_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    if required:
        _gw().print_info(f"  Set these env vars in ~/.cryozen-agent/.env: {', '.join(required)}")
    else:
...
def _setup_service_action(
    ...
    try:
        ...
        if backend is not None:
            ...
            _gw().print_info("Start manually: cryozen gateway")
...
_WIZARD_NO_SERVICE_LINES = {
    "wsl": (
        "  WSL detected but systemd is not running.", "  Run in foreground: cryozen gateway run",
        "  For persistence:   tmux new -s cryozen 'cryozen gateway run'",
        "  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'",
    ),
    "termux": (
        "  Termux does not use systemd/launchd services.", "  Run in foreground: cryozen gateway run",
        "  Or start it manually in the background (best effort): nohup cryozen gateway run >{home}/logs/gateway.log 2>&1 &",
    ),
    "unsupported": (
        "  Service install not supported on this platform.", "  Run in foreground: cryozen gateway run",
    ),
}
...
def _wizard_service_status_block() -> None:
    ...
    if _gw().supports_systemd_services() and _gw().has_legacy_cryozen_units():
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        _gw().print_info("  You can install later: cryozen gateway install")
        if _gw().supports_systemd_services():
            _gw().print_info("  Or as a boot-time service: sudo cryozen gateway install --system")
        _gw().print_info("  Or run in foreground:  cryozen gateway run")
        return
    try:
        ...
        _gw().print_info("  You can try manually: cryozen gateway install")
...
def gateway_setup():
    ...
    if any(_is_progress(_gw()._platform_status(p)) for p in _gw()._all_platforms()):
        ...
        print()
        _gw().print_info("No platforms configured. Run 'cryozen gateway setup' when ready.")
