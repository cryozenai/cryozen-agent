...
:func:`gateway.host_rendezvous.host_state_dir` (``$CRYOZEN_GATEWAY_LOCK_DIR`` else
``$XDG_STATE_HOME/cryozen/gateway-locks``) — the one cross-profile, per-OS-user state root the
...
per-profile ``cryozen-gateway-<p>.service`` units on a multiplexed host all point at the same
...
def host_restart_already_completed(sha: Optional[str]) -> bool:
    ...
    ``cryozen update`` must attach to the first restart's outcome, never kill the shared
    multiplexer again.
    """
    record = read_host_obligation()
