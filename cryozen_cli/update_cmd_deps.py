"""Post-``cryozen update`` dependency sync: venv preflight, editable reinstall, lazy refresh,
npm/Desktop rebuilds, self-lock deferral. Names are re-imported by ``update_cmd`` (so
``cryozen_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
from cryozen_constants import project_venv_dir, venv_python_path
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    -e .`` always rewrites console-script shims (Windows: ``cryozen.exe`` quarantine, ``os error 32``
...
_UPDATE_CRITICAL_MODULES = "cryozen_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from cryozen_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from cryozen_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    marker = f"__CRYOZEN_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        "import importlib, json, sys\n"
        # Importing cryozen_cli.main runs the startup dotenv load, which pulls external secret
        ...
        # only our own packages count. Roots come from cryozen_constants so the user hint can't drift.
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('cryozen_') or %r:\n"
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``cryozen tools``."""
    try:
        from cryozen_cli import tools_config
        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Cryozen Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    """Restore allowlisted ``cryozen tools`` dependencies (from a pre-rebuild probe) into a rebuilt
    ...
    try:
        ...
        logger.debug("Cryozen Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Cryozen Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `cryozen update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `cryozen update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not Cryozen extras / ``LAZY_DEPS``), so the core reinstall can strip them;
    ...
    try:
        from cryozen_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _reapply_plugin_python_dependencies() -> None:
    ...
    ``uv sync``/reinstall strips anything Cryozen' own lock does not know). Non-memory plugins whose
    deps no longer resolve are disabled loudly, memory providers last. Never raises."""
    from cryozen_cli.plugin_python_deps import reapply_all
    from cryozen_cli.update_cmd import _m
    ...
    for name, reason in report.dropped:
        print(f"  ✗ Plugin '{name}' DISABLED: {reason}. Fix the plugin's declared dependencies, "
              f"then `cryozen plugins enable {name}`.")
    if report.failed:
...
def _migrate_removed_memory_providers() -> None:
    ...
    try:
        from cryozen_cli.memory_provider_migration import migrate_all_homes
        migrate_all_homes()
...
def _install_psutil_android_compat(
    ...
    from cryozen_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    from cryozen_cli.update_cmd import _m
    from cryozen_cli.managed_uv import resolve_uv
    existing = resolve_uv()
    if existing:
        return existing
...
def _npm_lockfile_changed(cryozen_root: Path) -> bool:
    from cryozen_cli.update_cmd import _m
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    return not _npm_stamp_matches(cryozen_root, current)


def _npm_lock_cache_file(cryozen_root: Path, scope: str = "") -> Path:
    ...
    *scope* separates install closures that share the digest (workspace-scoped vs. full desktop)."""
    ...
    return cryozen_root / f".npm_lock_hash_{cache_key}{scope}"


def _npm_stamp_matches(cryozen_root: Path, current: str, scope: str = "") -> bool:
    """True when the recorded digest for *scope* equals *current*; a missing/unreadable stamp never matches."""
    try:
        return _npm_lock_cache_file(cryozen_root, scope).read_text(encoding="utf-8").strip() == current
    except OSError:
        return False


def _clear_npm_lockfile_hash(cryozen_root: Path, scope: str = "") -> None:
    ...
    with suppress(OSError):
        _npm_lock_cache_file(cryozen_root, scope).unlink()


def _record_npm_lockfile_hash(cryozen_root: Path, scope: str = "") -> None:
    digest = _npm_manifests_digest()
    if digest is None:
        return
    try:
        _npm_lock_cache_file(cryozen_root, scope).write_text(digest, encoding="utf-8")
    except OSError:
...
def _desktop_deps_changed(cryozen_root: Path) -> bool:
    ...
    current = _npm_manifests_digest()
    return current is None or not _npm_stamp_matches(cryozen_root, current, DESKTOP_NPM_SCOPE)
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run cryozen
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `cryozen update`.")
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from cryozen_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `cryozen update`.")
            has_workspace = any(
    ...
    shared_cryozen_root = get_default_cryozen_root()
    ...
    # Runs before the lockfile-unchanged early return below since that's the common `cryozen update` case.
    # See #43564.
    ...
    if not _m()._npm_lockfile_changed(shared_cryozen_root):
    ...
    from cryozen_constants import with_cryozen_node_path
    nixos_env = with_cryozen_node_path(_m()._nixos_build_env())
    ...
    # The chatty npm-deprecation noise during `cryozen update` comes from the *desktop* build, not this step;
    # that one is captured to update.log. See #18840.
    _clear_npm_lockfile_hash(shared_cryozen_root)
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_cryozen_root)
    ...
    print("    deps). Fix npm and re-run `cryozen update`.")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".cryozen-bootstrap-complete", _m()._update_marker_path())
...
def _venv_dependency_set_stale() -> tuple[bool, str]:
    """Whether the venv's ``cryozen-agent`` distribution was installed from an OLDER checkout than
    ...
    from cryozen_cli.update_cmd import _m, _read_project_version
    ...
    try:
        result = subprocess.run(
            [str(venv_python), "-c", "import importlib.metadata as m; print(m.version('cryozen-agent'))"],
    ...
    return stale, f"installed cryozen-agent {installed}, checkout is {expected}" if stale else ""
...
# canonical case: ``cryozen_cli.main`` used to import it at startup while resolving external secret sources;
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``cryozen.exe`` shim
    ...
    if _m()._reexec_dependency_sync_off_windows_shim(gateway_resume):
        ...
        # (and cryozen.exe locked) exactly while the child needs it gone (#101600).
        sys.exit(0)
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `cryozen` launch will complete it")
...
def _rebuild_desktop_after_update(
    ...
    # build stamp under CRYOZEN_HOME: it outlives a swap that lost the artifacts in an earlier run, and
    ...
    desktop_build_cmd = [sys.executable, "-m", "cryozen_cli.main", "desktop", "--build-only"]
    # Capture the loud build output into update.log; retry once on failure (still-settling
    # rebuild window), then surface the tail. Put Cryozen-managed Node on PATH: the desktop
    # updater chain loses shell PATH customizations, so a bare-PATH child hits `node: not found`.
    from cryozen_constants import with_cryozen_node_path
    build_env = with_cryozen_node_path()
    for _attempt in range(2):
        ...
        if build_result.returncode == 0:
            break
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `cryozen desktop` to retry)")
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo cryozen`` dies mid-update with ``venv/bin/cryozen`` already
    ...
    See #83529.
    A later normal ``cryozen update`` then dies mid-mutation inside ``uv pip install -e .`` ("Permission
    denied (os error 13)") with ``venv/bin/cryozen`` already deleted — the CLI is bricked. Same philosophy as
    the contended-venv gate (#87331): a venv we cannot safely mutate is never mutated at all.
    """
    from cryozen_cli.update_cmd import _path_uid
    try:
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Cryozen broken.")
    print("  This usually happens after running cryozen or pip with sudo. Offending paths:")
    for p, uid in foreign:
        print(f"    - {p} (owner uid {uid})")
    print("\n  Fix ownership, then re-run the update:")
    print(f"    sudo chown -R $(id -un): {project_root}")
    print("    cryozen update")
...
def _sync_python_dependencies_after_pull(
    ...
    if _m()._refresh_active_lazy_features(install_prefix, env=lazy_env, features=active_lazy_features):
        ...
        print(
            "  ⚠ Lazy-refresh recovery incomplete — run `cryozen` again "
    ...
    if not import_ok:
        ...
        print("    Run `cryozen update` again — if it persists, reinstall:")
        print("    https://cryozenai.github.io/cryozen-agent")
