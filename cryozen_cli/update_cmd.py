"""Cryozen update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``cryozen_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``cryozen_cli.main`` at call time.
"""
...
from cryozen_cli.config import get_cryozen_home  # noqa: F401  (re-exported; patched via update_cmd)
from cryozen_cli import update_handoff as _update_handoff
from cryozen_cli.update_cmd_common import _best_effort
from cryozen_constants import get_default_cryozen_root, project_venv_dir, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from cryozen_cli.update_abort_recovery import (  # noqa: F401
...
from cryozen_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _cryozen_holder_subcommand, _holder_value_flags,
...
from cryozen_cli.update_cmd_fleet import (  # noqa: F401
...
from cryozen_cli.update_cmd_zip import (  # noqa: F401
...
from cryozen_cli.update_cmd_stash import (  # noqa: F401
...
from cryozen_cli.update_cmd_config import (  # noqa: F401
...
from cryozen_cli.update_cmd_deps import (  # noqa: F401
...
from cryozen_cli.update_cmd_git import (  # noqa: F401
...
from cryozen_cli.update_cmd_maint import (  # noqa: F401
...
def _m():
    """Lazy ``cryozen_cli.main`` handle: keeps main-side test patches effective, import one-way."""
    from cryozen_cli import main
    return main
...
_UPDATE_CRITICAL_FILES = (
    "cryozen_cli/main.py", "cryozen_cli/config.py", "cryozen_cli/__init__.py",
    "cryozen_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "cryozen_constants.py")
...
def _record_update_step(step: str, ok: bool, detail: str = "") -> None:
    ...
    with suppress(Exception):
        from cryozen_cli.update_receipt import record_step
        record_step(step, ok, detail)


# A fetch whose transport dead-stalls (HTTP/2 to GitHub on some networks, a black-holed proxy)
# otherwise leaves `cryozen update` on "Fetching updates..." forever (#93759, #95777). Five
...
def _validate_python_files_syntax(root, relpaths) -> tuple[bool, str | None, str | None]:
    ...
    import tempfile
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="cryozen-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from cryozen_constants import get_cryozen_home  # noqa: F811  (deliberate: constants variant)
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    See #87331.
    """
    print("✗ Cannot continue the update: live Cryozen launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["cryozen.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Cryozen")
    print("  Desktop, a gateway, or another cryozen REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `cryozen` command to finish it automatically.")
...
def _invalidate_update_cache():
    ...
    default_home = get_default_cryozen_root()
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-cryozen.exe gate."""
    shim = scripts_dir / "cryozen.exe"
    lines = [
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from cryozen_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _log_only_write(text: str) -> None:
    ...
    with suppress(Exception):
        if log_file is None:
            log_path = get_cryozen_home() / "logs" / "update.log"
...
def _run_logged_subprocess(cmd, *, cwd=None, env=None):
    ...
    import codecs
    import io
    from cryozen_cli._subprocess_compat import kill_process_tree, windows_hide_flags
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """``cryozen update --check``: fetch and report without installing. ``branch_explicit`` is
    ...
    from cryozen_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    from cryozen_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        from cryozen_cli.gitlock import repair_broken_shallow_boundaries, prune_stale_shallow_grafts
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            return
        from cryozen_cli.banner import _github_compare_behind
...
def _repair_venv_on_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    _write_update_incomplete_marker()
    from cryozen_cli.managed_uv import ensure_uv
    repair_uv = ensure_uv()
    ...
    if not healthy_after:
        ...
        print("  Close all Cryozen windows/gateways and re-run: cryozen update")
        return False
...
def _pip_install_prefix(uv_bin) -> tuple[list[str], dict | None]:
    ...
    if uv_bin:
        ...
        # resolves here.
        # See #83914.
        from cryozen_cli.managed_uv import managed_python_env
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    from cryozen_cli.managed_uv import ensure_uv, update_managed_uv
    runtime_repairs = []
    ...
    if handed_off_sync:
        print("→ Finishing the dependency install handed off by cryozen.exe...")
    elif not healthy:
    ...
    if handed_off_sync or not healthy or stale:
        ...
        if runtime_repaired is not None:
            ...
            # imports pass while lazily-installed backends and ``cryozen tools`` dependencies
            ...
            # must leave the breadcrumb so the next `cryozen` run finishes the repair.
            ...
            if _m()._refresh_active_lazy_features(
                ...
                print("  ⚠ Lazy-refresh recovery incomplete — run `cryozen` again "
    ...
    if runtime_repaired is not None and not _m()._is_windows():
        ...
        print(
            "  Any running Cryozen gateways, Desktop backends, or other "
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if not has_common_ancestor and pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/cryozen-update-backups/orphan-{branch}-"
...
def _rollback_if_pulled_syntax_error(git_cmd, pre_pull_sha) -> None:
    ...
    if pre_pull_sha:
        ...
        if rollback_result.returncode == 0:
            ...
            print("  Try ``cryozen update`` again later once a fix lands.")
        else:
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in cryozen_cli/config.py bricked
    # every user who ran ``cryozen update`` for the 7 minutes between the bad commit and the fix landing).
...
class _UpdateOptions:
    """Resolved ``cryozen update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway cryozen.exe
    holds the venv shim."""
    ...
    with _best_effort('Update receipt unavailable: %s'):
        ...
        from cryozen_cli.update_receipt import begin_update_receipt
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running Cryozen runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from cryozen_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another cryozen.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_cryozen_instances(scripts_dir) if scripts_dir is not None else []
...
def _prepare_git_command() -> tuple[bool, list, bool]:
    ...
    if use_zip_update and sys.platform != "win32":
        ...
        print("  curl -fsSL https://cryozenai.github.io/cryozen-agent/install.sh | bash")
        sys.exit(1)
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``cryozen update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``cryozen
    ...
    if pre_pull_sha and post_pull_sha == pre_pull_sha:
        ...
        print(
            ...
            f"git -C {_m().PROJECT_ROOT} checkout {branch} && cryozen update")
        _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
        sys.exit(1)
...
def _handle_update_called_process_error(
    ...
    if _should_zip_fallback_on_update_error(e):
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            print()
            print("  Cryozen may not start until the dependencies are installed. Fix the error above")
            print("  (usually network or disk space), then run `cryozen update` again.")
            if _m()._is_windows():
                print("  If `cryozen update` itself will not start, retry through the venv interpreter:")
                print(
                    '    venv\\Scripts\\python.exe -c '
                    '"from cryozen_cli.main import main; main()" update --yes')
        else:
            print(f"✗ {stage}.")
            print(f"  Details: {e}")
            _print_called_process_error_tail(e)
...
def _post_swap_payload(
    *, swap: str, branch: str, opts, gateway_mode: bool, had_desktop_app_before_update: bool,
    ...
    from cryozen_cli.update_receipt import detach_update_receipt
...
def _sibling_snapshots_module():
    ...
    import cryozen_cli.update_cmd_config as _cfg
    return _cfg


def _hand_off_post_swap(args, **payload_kwargs) -> None:
    """Re-execute ``cryozen update --post-swap`` on the pulled tree and exit with its code.

    The parent detaches from the receipt and its Windows resume hook — the child owns both —
    and only relays the exit code (``cryozen_cli/update_handoff.py``).
    """
    from cryozen_cli.update_receipt import resume_update_receipt
...
def _execute_post_swap(payload: dict, args, gateway_mode: bool) -> None:
    ...
    import cryozen_cli.update_cmd_config as _cfg
    from cryozen_cli.update_inventory import UpdatePlan
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # A child spawned off cryozen.exe already outwaited its parent in ``cmd_update`` (before the
    # update lock, so the lock it now holds is its own — the parent's marker left with it).
    from cryozen_cli.update_handoff import adopt_handed_off_gateway_resume
    ...
    # Any venv python still running (typically the Desktop `cryozen serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `cryozen
