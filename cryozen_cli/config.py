"""Configuration management for Cryozen Agent: config.yaml / .env loading, saving,
validation, migration, and the ``cryozen config`` command."""
...
# dies with ImportError before any later heal point is reached. See cryozen_cli.stale_modules.
...
from cryozen_cli.colors import Colors, color
from cryozen_cli import managed_scope
from cryozen_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from cryozen_cli.secret_prompt import masked_secret_prompt
# Managed-mode, container and CRYOZEN_UID/GID policy live in cryozen_constants (import-safe);
# re-exported here so existing callers/patch targets keep working.
from cryozen_constants import (  # noqa: F401
    _IGNORED_MANAGED_VALUES, _LEGACY_MANAGED_SYSTEM, _MANAGED_FALSE_VALUES, _MANAGED_TRUE_VALUES,
    _chown_to_cryozen_uid, _container_or_chmod_skipped, _resolve_cryozen_uid_gid,
    apply_secure_dir_policy, get_managed_system)
# Re-export from cryozen_constants — canonical definition lives there.
from cryozen_constants import get_cryozen_home, get_process_cryozen_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    "last-known-good": "Cryozen is running on the settings it loaded before the edit until it is fixed, so recent changes are not applied.",
    "last-known-good-backup": "Cryozen is running on your last good settings until it is fixed, so recent changes are not applied.",
    "refuse-write": "Nothing was written, so the existing file is preserved."}
_PARSE_FAILURE_DEFAULTS_MSG = (
    "Cryozen is running on default settings until it is fixed, so none of your saved settings are applied.")
_PARSE_FAILURE_REPAIR_MSG = "Open it with `cryozen config edit`, fix {where}, then run `cryozen config check`."
...
def format_config_parse_failure(config_path: Path, exc: Exception, *, fallback: str = "defaults") -> str:
    """User copy for an unparseable config.yaml: what happened, what Cryozen is doing, how to fix.
...
def _warn_config_parse_failure(
    ...
    from cryozen_cli.config_backups import backup_config
    ...
    try:
        sys.stderr.write(f"⚠️  cryozen config: {msg}\n    Details: {_yaml_error_details(exc)}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Cryozen restarts through them), PATH (fix tool lookup
...
# and Cryozen runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``CRYOZEN_*`` overall is NOT blocked — many integration credentials use that prefix
# (CRYOZEN_LANGFUSE_PUBLIC_KEY, CRYOZEN_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Cryozen runtime location
    "CRYOZEN_HOME", "CRYOZEN_PROFILE", "CRYOZEN_CONFIG", "CRYOZEN_ENV",
    "CRYOZEN_CONFIG_PATH", "CRYOZEN_ENV_PATH",
    # MCP catalog trust root; package-manager wrappers may still set it in the process env.
    "CRYOZEN_OPTIONAL_MCPS",
    # Local ACP subprocess selection (executable/argv authority).
    "CRYOZEN_COPILOT_ACP_COMMAND", "CRYOZEN_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "CRYOZEN_YOLO_MODE", "CRYOZEN_ACCEPT_HOOKS", "CRYOZEN_REDACT_SECRETS",
    "CRYOZEN_INTERACTIVE", "CRYOZEN_EXEC_ASK", "CRYOZEN_GATEWAY_SESSION",
    "CRYOZEN_CRON_SESSION", "CRYOZEN_SINGLE_QUERY_SESSION",
    "CRYOZEN_SESSION_KEY", "CRYOZEN_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if policy_name in _ENV_VAR_NAME_DENYLIST or policy_name.startswith(_ENV_VAR_NAME_DENY_PREFIXES):
        raise ValueError(
            ...
            "or Cryozen runtime location and security policy (CRYOZEN_HOME, CRYOZEN_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.cryozen-agent/.env "
            "directly.")
...
# acquires it again (cryozen_cli/plugins.py: `with ..., config_mod._CONFIG_LOCK:` then
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean CRYOZEN_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "CRYOZEN_TOOL_PROGRESS_MODE",
    ...
    "CRYOZEN_LANGFUSE_ENV", "CRYOZEN_LANGFUSE_RELEASE", "CRYOZEN_LANGFUSE_SAMPLE_RATE",
    "CRYOZEN_LANGFUSE_MAX_CHARS", "CRYOZEN_LANGFUSE_CAPTURE", "CRYOZEN_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "CRYOZEN_ACP_AUTH_METHOD", "CRYOZEN_ACP_AUTO_APPROVE", "CRYOZEN_COPILOT_ACP_COMMAND",
    "CRYOZEN_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# CRYOZEN_MANAGED). Module-level so tests can patch it without touching /nix/store.
_NIX_STORE = Path("/nix/store")


def is_managed() -> bool:
    """Check if Cryozen is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Cryozen through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Cryozen was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$CRYOZEN_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because CRYOZEN_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``cryozen update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``cryozenai/cryozen-agent`` image bakes a ``docker`` stamp into
    ``/opt/cryozen`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of cryozen_cli/), NOT of $CRYOZEN_HOME,
    ...
    method = _install_method_stamp(get_cryozen_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``cryozen_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull cryozenai/cryozen-agent:latest",
    "apt": "pkg upgrade cryozen-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "cryozen update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (cryozen_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Cryozen.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``cryozen update`` doesn't apply inside the Docker container.

Cryozen Agent runs as a published image (cryozenai/cryozen-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull cryozenai/cryozen-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate cryozen-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm cryozenai/cryozen-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/cryozenai/cryozen-agent/tags
  • Your config and session history live under ``$CRYOZEN_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``cryozen update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Cryozen installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Cryozen installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Cryozen.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CRYOZEN_HOME/.container-mode.
    ...
    already inside the container, or when CRYOZEN_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("CRYOZEN_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_cryozen_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "cryozen-agent"),
        "exec_user": info.get("exec_user", "cryozen"),
        "cryozen_bin": info.get("cryozen_bin", "/data/current-package/bin/cryozen")}


# ---- Config paths / CRYOZEN_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_cryozen_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / CRYOZEN_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("CRYOZEN_IGNORE_USER_CONFIG") == "1":
        return
    ...
    message = (
        f"Cryozen stopped because your settings file ({config_path}) has a formatting error"
        f"{f' at {where}' if where else ''}. Fix it with `cryozen config edit` and check with "
        "`cryozen config check`, or add --ignore-user-config to run once with default settings.")
    if backup_path is not None:
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply CRYOZEN_UID/GID ownership. No-op when managed;
    in a container only an explicit CRYOZEN_HOME_MODE is applied. CRYOZEN_HOME_MODE (e.g. 0701)
    overrides the mode so a web server can traverse CRYOZEN_HOME to a served subdirectory without
    directory listings.

    Also applies ``CRYOZEN_UID``/``CRYOZEN_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    Delegates to the canonical import-safe primitive ``cryozen_constants.apply_secure_dir_policy``
...
_CRYOZEN_HOME_ENSURED: set = set()
_CRYOZEN_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_cryozen_home():
    """Ensure the ~/.cryozen-agent directory skeleton exists with secure permissions.
    ...
    from cryozen_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _CRYOZEN_HOME_ENSURED and home.is_dir():
        return
    from cryozen_cli.config_home import initialize_home
    initialize_home(home, _CRYOZEN_HOME_SUBDIRS, _CRYOZEN_HOME_ENSURED)


# ---- Config loading/saving ----

from cryozen_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from cryozen_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use cryozen_cli.config.<name>)
...
# Back-compat re-exports — :mod:`cryozen_cli.personality` owns personality/overlay semantics.
from cryozen_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``cryozen config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
_ENV_CONFIG_KEYS = frozenset({
    ...
    'TAVILY_API_KEY', 'PERPLEXITY_API_KEY', 'API_SERVER_KEY',
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `cryozen config set` routes this credential-shaped key to .env through the
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `cryozen update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (cryozen_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # cryozen_cli/tools_config.py toolset-save flow
...
def _validate_timezone(config: Dict[str, Any], issues: List[ConfigIssue]) -> None:
    ...
    ``cryozen_time._get_zoneinfo()`` swallows an invalid name behind a single WARNING in the
    ...
    hint = ("Use an IANA zone name such as America/New_York or Asia/Tokyo (see "
            ...
            "schedules silently fall back to server-local time. CRYOZEN_TIMEZONE overrides "
            "this key when set.")
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'cryozen tools' and pick a different Web Search & Extract provider")
...
def _validate_quoted_containers(config: Dict[str, Any], issues: List[ConfigIssue]) -> None:
    ...
    for key, kind in _container_slots().items():
        ...
        if isinstance(parsed, (list, dict)):
            _issue(issues, "warning",
                   f"{key} is the quoted string {value!r} — Cryozen expects a YAML {kind} here "
                   "and every reader ignores the string",
                   f"Run: cryozen config set {key} {shlex.quote(value)}  (stores a real {kind}), "
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from cryozen_cli.config_home import config_load_issue
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Cryozen won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'cryozen doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        ...
        hint_path = display_cryozen_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from cryozen_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ cryozen config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from cryozen_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from cryozen_cli.toolset_validation import validate_platform_toolsets
        from cryozen_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: cryozen config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.cryozen-agent/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_cryozen_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``cryozen config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``cryozen status``/``dump``) read ``name`` and
...
# the CRYOZEN_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _backups_dir_display() -> str:
    from cryozen_constants import display_cryozen_home
    return f"{display_cryozen_home()}/backups/config/"


_FIX_PERMS = "Fix the file permissions or move it aside first."
_FIX_YAML = (
    "Fix it with `cryozen config edit` and check with `cryozen config check`, or copy the newest good "
...
TERMINAL_CONFIG_ENV_MAP = {
    ...
    **{
        ...
        for key in (
            "degraded_mode", "cwd", "temp_dir", "timeout", "lifetime_seconds",
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Cryozen host
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        # Fresh process (CLI restart, `cryozen config get`): nothing loaded yet in this process, so
        ...
        from cryozen_cli.config_backups import load_newest_good_backup
...
def _load_config_cache_hit(path_key: str, cache_sig: Any) -> Optional[Dict[str, Any]]:
    ...
    ``None``. Signatures matching is not enough: a load before load_cryozen_dotenv() would otherwise
...
#   openai-codex (OAuth — cryozen auth) — OpenAI Codex
...
def save_config(
    ...
    """Save configuration to ~/.cryozen-agent/config.yaml.
...
def load_env() -> Dict[str, str]:
    """Load ~/.cryozen-agent/.env as a dict. Memoised inside ``load_env_file`` (``get_env_value()`` runs
...
def sanitize_env_file() -> int:
    """Rewrite ~/.cryozen-agent/.env with normalized line formatting; returns the number of changed lines."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.cryozen-agent/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``CRYOZEN_CUSTOM_`` prefix keeps the
    ...
    return f"CRYOZEN_CUSTOM_{slug}_API_KEY" if slug else "CRYOZEN_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.cryozen-agent/.env and os.environ; True if it was found and removed."""
...
def save_anthropic_api_key(value: str, save_fn=None):
    """Persist an Anthropic API key and clear the retired ``ANTHROPIC_TOKEN`` setup-token slot."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from cryozen_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.cryozen-agent/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Cryozen (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.cryozen-agent/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Cryozen-managed credential preferring ``~/.cryozen-agent/.env`` over ``os.environ``, so a
...
# (``TERMINAL_SSH_HOST``, ``FIRECRAWL_API_URL``, ``BROWSERBASE_PROJECT_ID``).
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale CRYOZEN_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("CRYOZEN_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale CRYOZEN_MAX_ITERATIONS={env_ghost} "
                    f"(run 'cryozen doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from cryozen_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ✻ Cryozen Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from cryozen_cli.auth import get_anthropic_key
    ...
    print(color("  cryozen config edit     # Edit config file", Colors.DIM))
    print(color("  cryozen config set <key> <value>", Colors.DIM))
    print(color("  cryozen setup           # Run setup wizard", Colors.DIM))
    print()
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``cryozen config set`` string to bool/None/int/float/list/dict.
...
def _refuse_container_type_mismatch(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> Any:
    ...
    _exit_invalid(
        ...
        f"  Pass a YAML/JSON literal, e.g.:\n    cryozen config set {key} '{literal}'\n"
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``cryozen config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    cryozen config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    cryozen config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_cryozen_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Cryozen may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if _is_env_config_key(key):
        ...
        return
    from cryozen_cli.config_env_routing import is_env_setting_key, save_env_setting
    ...
    if is_env_setting_key(key):
        ...
        # here, --force included (#111848). The env writer's denylist (CRYOZEN_YOLO_MODE, PATH, ...)
    ...
    if key == "model.provider" and _old_provider.lower() != str(value).strip().lower():
        from cryozen_cli.route_identity import drop_stale_model_route
        ...
        if _popped:
            ...
            _route_notice = color(
                ...
                "`cryozen config unset model.base_url` (and model.api_mode).", Colors.YELLOW)
    # api_base -> base_url alias at set-time too (mirrors _normalize_root_model_keys).
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `cryozen config
...
def get_config_value(key: str, *, as_json: bool = False, raw: bool = False):
    ...
    from cryozen_cli.config_env_routing import is_env_setting_key, read_env_setting
    ...
    if _split_key_path(key)[0] in _known_top_level_keys():
        ...
        if not is_known:
            print(color(
                f"⚠ '{key}' is not a recognized config key — Cryozen may not read it; the value "
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from cryozen_cli.credential_lifecycle import remove_provider_env_credential

        if not remove_provider_env_credential(key.upper()).get("found"):
            _exit_invalid(f"Config key not set: {key}")
        print(f"✓ Unset {key} from {get_env_path()}")
        return
    from cryozen_cli.config_env_routing import is_env_setting_key, remove_env_setting
...
_USAGE_GET = ("Usage: cryozen config get <key> [--json] [--raw]", [
    "cryozen config get model", "cryozen config get terminal.backend",
    "cryozen config get skills.config --json"], None)
_USAGE_SET = ("Usage: cryozen config set [--force] <key> <value>", [
    "cryozen config set model anthropic/claude-sonnet-4", "cryozen config set terminal.backend docker",
    "cryozen config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: cryozen config unset <key>", [
    "cryozen config unset model", "cryozen config unset terminal.backend",
    "cryozen config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'cryozen config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  cryozen config           Show current configuration
  cryozen config edit      Open config in editor
  cryozen config get <key>          Print a resolved config value
  cryozen config set <key> <value>   Set a config value
  cryozen config unset <key>        Remove a config value
  cryozen config check     Check for missing/outdated config
  cryozen config migrate   Update config with new options
  cryozen config path      Show config file path
  cryozen config env-path  Show .env file path"""
...
def _platform_plugin_manifests():
    ...
    ``plugins/platforms/*``, the user's ``<CRYOZEN_HOME>/plugins/platforms/*`` category dir, and flat
    user installs ``<CRYOZEN_HOME>/plugins/*`` that declare ``kind: platform`` (#46600)."""
    user_plugins = get_cryozen_home() / "plugins"
    roots = (
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Teams / IRC / Google Chat and third-party platforms are configurable in the ``cryozen config`` /
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``cryozen_cli/`` — i.e. the git checkout for source
    installs, ``/opt/cryozen`` inside the published image. It is a property of
    the running interpreter, NOT of ``$CRYOZEN_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$CRYOZEN_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/cryozen`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('cryozen_cli.route_identity', 'normalize_route_base_url'),
}
