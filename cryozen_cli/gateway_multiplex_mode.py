...
``cryozen gateway migrate --multiplex`` runs and multiplexes only when the fold would have been
...
default flipped, and the log names the blocker plus ``cryozen gateway migrate --multiplex``.
"""
...
RETIRED_OPT_OUT_REASON = (
    ...
    "every profile. Remove the key; `cryozen gateway migrate --multiplex` folds any leftover "
    "per-profile gateway.")
...
def explicit_multiplex_flag(default_home: Path) -> Optional[bool]:
    ...
    other processes (``cryozen -p X ...`` has X's config loaded) asking about the default's file."""
...
def default_gateway_multiplexes(default_home: Optional[Path] = None) -> bool:
    ...
    root = Path(default_home) if default_home is not None else get_default_cryozen_root()
...
def implicit_multiplex_blocker() -> Optional[str]:
    ...
    Mirrors what makes ``cryozen gateway migrate --multiplex`` refuse or leave a per-profile gateway
    ...
    from cryozen_cli.gateway_migrate import MIGRATE_COMMAND, _host_supports_migration, build_migration_plan
...
def resolve_multiplex_mode(config) -> MultiplexDecision:
    ...
    says why, and it converges by itself once ``cryozen gateway migrate --multiplex`` has run.
    """
...
def record_multiplex_decision(decision: MultiplexDecision) -> None:
    """Persist a guard refusal into ``gateway_state.json`` so `cryozen gateway status` can show why this
