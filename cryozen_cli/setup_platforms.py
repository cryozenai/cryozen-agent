"""Messaging-platform setup wizards (Telegram, BlueBubbles, webhooks) and the ``cryozen setup
gateway`` flow. setup.py re-exports the public names, and tests monkeypatch prompt/print/env
helpers on cryozen_cli.setup, so those are imported lazily per function."""
...
def declines_reconfigure(label: str, question: str, *env_vars: str) -> bool:
    ...
    """
    from cryozen_cli.setup import get_env_value, print_info, prompt_yes_no
...
def save_prompted(env_var: str, question: str, *, password: bool = False, success_msg: str | None = None,
    ...
    """
    from cryozen_cli.setup import print_success, print_warning, prompt, save_env_value
...
def _save_allowlist(env_var: str, users: str, success_msg: str) -> None:
    ...
    from cryozen_cli.setup import print_success, save_env_value
    save_env_value(env_var, users.replace(" ", ""))
    print_success(success_msg)


def _prompt_allowlist(env_var: str, question: str, success_msg: str, open_msg: str) -> str:
    """Persist the prompted answer as an allowlist, warning when it stays open."""
    from cryozen_cli.setup import print_info, prompt
    users = prompt(question)
    if users:
...
def _save_port(env_var: str, value: str, default: str) -> None:
    ...
    from cryozen_cli.setup import print_success, print_warning, save_env_value
    if not value:
        return
    try:
...
def _prompt_telegram_bot_token() -> str | None:
    from cryozen_cli.setup import print_error, print_info, prompt
...
def _telegram_allowlist_nudge() -> None:
    ...
    from cryozen_cli.setup import get_env_value, print_info, prompt, prompt_yes_no
...
def _setup_telegram():
    ...
    from cryozen_cli.setup import _info, print_info, print_header, print_success, prompt_yes_no, save_env_value
    ...
    _info(None, "📬 Home Channel: where Cryozen delivers cron job results,",
...
def _setup_bluebubbles():
    ...
    from cryozen_cli.setup import _info, print_header, print_success, prompt, prompt_yes_no
    print_header("BlueBubbles (iMessage)")
    if declines_reconfigure("BlueBubbles", "Reconfigure BlueBubbles?", "BLUEBUBBLES_SERVER_URL"):
        return
    _info("Connects Cryozen to iMessage via BlueBubbles — a free, open-source",
...
def _setup_webhooks():
    ...
    from cryozen_cli.setup import _info, print_header, print_success, print_warning, prompt, save_env_value
    ...
    print()
    _info("   Full guide: https://cryozenai.github.io/cryozen-agent/docs/user-guide/messaging/webhooks/", None)
    ...
    _info(f"   1. Define webhook routes in {_dhh()}/config.yaml",
          ...
          "   https://cryozenai.github.io/cryozen-agent/docs/user-guide/messaging/webhooks/#configuring-routes",
          None,
          # Printed twice upstream; kept verbatim for output parity.
          "   Open config in your editor:  cryozen config edit",
          "   Open config in your editor:  cryozen config edit")
...
def _warn_missing_home_channels() -> None:
    ...
    from cryozen_cli.setup import get_env_value, _info, print_warning
    missing_home = [
    ...
    _info("   Without a home channel, cron jobs and cross-platform",
          ...
          *(f"     cryozen config set {plat.upper()}_HOME_CHANNEL <channel_id>" for plat in missing_home))
...
def _restart_running_gateway(any_messaging: bool, supports_systemd: bool) -> None:
    ...
    from cryozen_cli.setup import print_error, prompt_yes_no
...
def setup_gateway(config: dict):
    ...
    from cryozen_cli.setup import _info, print_header, print_info, print_success, prompt_checklist
    from cryozen_cli.gateway import _all_platforms, _platform_status, _configure_platform
    print_header("Messaging Platforms")
    _info("Connect to messaging platforms to chat with Cryozen from anywhere.",
    ...
    if not selected:
        print_info("No platforms selected. Run 'cryozen setup gateway' later to configure.")
    ...
    # mode (cron keeps running; adapters come up once tokens are added via `cryozen import` /
    # `cryozen setup gateway`). Gating it on messaging config left install-then-import machines
    # with cron jobs and bot tokens but no process to serve them.
    from cryozen_cli.gateway import _is_service_running, supports_systemd_services, ensure_gateway_service
