"""``cryozen doctor`` — diagnose (and with --fix, repair) a Cryozen install.
...
from cryozen_cli.config import get_env_path, get_cryozen_home, get_project_root
...
_DHH = display_cryozen_home()  # user-facing display path (e.g. ~/.cryozen-agent or ~/.cryozen-agent/profiles/coder)

# Load environment variables from ~/.cryozen-agent/.env so API key checks work
_env_path = get_env_path()
load_cryozen_dotenv(cryozen_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from cryozen_cli.colors import Colors, color
from cryozen_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from cryozen_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from cryozen_cli.doctor_tools import _safe_which

from cryozen_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from cryozen_cli.doctor_platform import (
    _check_certificates,
...
from cryozen_cli.doctor_tools import (
    _check_git_and_rg,
...
from cryozen_cli.doctor_state import (
...
_PROVIDER_ENV_HINTS = (
    ...
    "OPENAI_BASE_URL", "GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY", "KIMI_API_KEY",
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from cryozen_cli.auth import get_codex_auth_status, get_minimax_oauth_auth_status
        # Native OAuth is Cryozen' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
...
def _ack_advisory(ack_target: str) -> None:
    """`cryozen doctor --ack <id>`: persist the ack and return without running diagnostics."""
    from cryozen_cli.security_advisories import ADVISORIES, ack_advisory
    ...
    if ack_advisory(ack_target):
        ...
        print(color(f"  ✗ Could not save the acknowledgement for {ack_target}. Make sure {_DHH}/config.yaml is "
                    f"writable (`cryozen config path` prints the exact file), then re-run "
                    f"`cryozen doctor --ack {ack_target}`.", Colors.RED))
        sys.exit(1)
...
def _print_summary(should_fix: bool, total: Finding) -> None:
    ...
    if should_fix and total.fixed > 0:
        ...
        print(numbered)
        if not should_fix:
            print(color("  Tip: run 'cryozen doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
...
def run_doctor(args):
    ...
    os.environ.setdefault("CRYOZEN_INTERACTIVE", "1")
    ...
    with warn_on_error(""):
        from cryozen_cli.doctor_live import maybe_run_live_checks
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('cryozen_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('cryozen_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('cryozen_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('cryozen_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('cryozen_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('cryozen_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('cryozen_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('cryozen_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('cryozen_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('cryozen_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('cryozen_cli.vercel_auth', 'describe_vercel_auth'),
    ...
    'managed_scope_check': ('cryozen_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('cryozen_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('cryozen_cli.doctor_config', 'report_deprecated_config_and_env'),
}
