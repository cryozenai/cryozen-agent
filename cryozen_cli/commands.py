"""Slash command registry for the Cryozen CLI and gateway.
...
from cryozen_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``cryozen_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Cryozen config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Cryozen works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .cryozen-agent/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Cryozen' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Cryozen is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.cryozen-agent/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Cryozen Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Cryozen Agent version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``cryozen skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('cryozen_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('cryozen_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('cryozen_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('cryozen_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('cryozen_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('cryozen_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('cryozen_cli.commands_platforms', 'telegram_menu_max_commands'),
}
