...
def get_launchd_label() -> str:
    ...
    return f"ai.cryozen.gateway-{suffix}" if suffix else "ai.cryozen.gateway"
...
def _launchd_domain() -> str:
    ...
    See #40831, #23387.
    """
    # The cache lives on the facade: tests and callers reset ``cryozen_cli.gateway._resolved_launchd_domain``.
    gw = _gw()
...
# at all and we degrade to a detached background process (the `nohup cryozen gateway run` workaround). See
# #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return _gw().get_cryozen_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return _gw().get_cryozen_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m cryozen_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [_gw().get_python_path(), "-m", "cryozen_cli.main", *_gw()._profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    (launchd ProgramArguments only) adds ``--external-supervisor`` so ``cryozen update`` hands back to
    ...
    ``--external-supervisor`` so ``cryozen update`` sees the flag on the live grandchild argv and hands the
    ...
    return [_gw().get_python_path(), "-m", "cryozen_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _spawn_detached_gateway() -> bool:
    ...
    the `nohup cryozen gateway run --replace` workaround but keeps it CLI-managed: stdout goes to
    ...
    log_dir = _gw().get_cryozen_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(f"  Try manually: nohup cryozen gateway run --replace > {_dhh()}/logs/gateway.log 2>&1 &")
    if exit_on_failure:
        sys.exit(1)
    return False
...
def _launchd_degrade_or_raise(exc: subprocess.CalledProcessError, what: str) -> None:
    ...
    if _gw()._launchctl_label_supervising_process(label):
        ...
        print("  Apply the definition with: cryozen gateway stop && cryozen gateway install --force")
        raise exc
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from cryozen_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <string>{cryozen_home}</string>
        <key>CRYOZEN_SUPERVISED_CHILD</key>
        <string>1</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if not _gw()._retry_launchctl_bootstrap_until_registered(domain, plist_path, label, deadline=_deadline):
        ...
        return False
    print("↻ Updated gateway launchd service definition to match the current Cryozen install")
    return True
...
def launchd_install(force: bool = False):
        ...
        if not _gw().launchd_plist_is_current():
            ...
            if _gw().refresh_launchd_plist_if_needed():
                ...
                print(
                    ...
                    "Run 'cryozen gateway install --force' or check "
                    f"{display_cryozen_home()}/logs/launchd-reload.log for details."
                )
            return
    ...
    print()
    print("Next steps:")
    print("  cryozen gateway status             # Check status")
    from cryozen_constants import display_cryozen_home as _dhh
...
def launchd_stop():
    ...
    # bootout unloads the definition so KeepAlive doesn't respawn; `cryozen gateway start` re-bootstraps.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and _gw().probe_gateway_loop_liveness(pid) == _gw().GATEWAY_LOOP_WEDGED:
            ...
            # only stalls the restart (and `cryozen update`). Bounded SIGTERM → SIGKILL, ~10s.
...
def launchd_status(deep: bool = False):
    ...
    # Cryozen PID may be a detached fallback process; when launchd IS supervising both PIDs match — don't double-count.
    from gateway.status import get_running_pid
    ...
    if _gw().launchd_plist_is_current():
        print("✓ Service definition matches the current Cryozen install")
    else:
        print("⚠ Service definition is stale relative to the current Cryozen install")
        print("  Run: cryozen gateway start")
    ...
    if not service_listed:
        ...
        if fallback_pid:
            ...
            print("  Cron jobs will fire. Stop with: cryozen gateway stop")
        else:
    ...
    if deep:
        log_file = _gw().get_cryozen_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
