"""PTY bridge for `cryozen dashboard` chat tab.
...
``cryozen_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/cryozen-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "CRYOZEN_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Cryozen Agent supports Windows only via WSL.")
