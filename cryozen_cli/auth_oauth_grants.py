...
Split out of ``cryozen_cli/auth.py`` and re-exported there; origin helpers are imported lazily
inside each function so ``cryozen_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from cryozen_cli.auth_constants import _decode_jwt_claims
...
SINGLE_USE_REFRESH_POOL_PROVIDERS = frozenset({"openai-codex", "xai-oauth"})
...
def _is_oauth_pool_payload(entry: Any) -> bool:
    if not isinstance(entry, dict):
        return False
    # Legacy rows predating ``auth_type``: any row carrying a refresh token is an OAuth grant.
    return (
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    Called after any path that copies credential files between profiles (``cryozen profile create
    ...
    of what was stripped. Never raises: a clone must not fail because hygiene could not run — the
    caller logs the summary.
    """
    from cryozen_cli.auth import _same_path, _save_auth_store
    stripped: Dict[str, Any] = {"pool": [], "providers": []}
    ...
    try:
        ...
        root_auth_path = get_default_cryozen_root() / "auth.json"
...
# the heal for the life of ONE process, so every fresh `cryozen` invocation and every new worker
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    ``cryozen auth list`` / ``cryozen auth status`` print them so the user sees the consolidation.
    """
    from cryozen_cli.auth import _oauth_heal_notices
...
def _oauth_identity(entry: Dict[str, Any]) -> Optional[str]:
    ...
    an opaque token carries none (None → lineage rests on id / token material).
    """
...
def heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    row with no root counterpart (an independent ``cryozen -p <p> auth add`` grant, or the only
    surviving copy); reads only the two auth.json files the root fallback already reads. Returns
    ``{"adopted", "stripped_ids", "providers_block"}`` when something healed, else None.
    Never raises.
    """
    if provider_id not in SINGLE_USE_REFRESH_POOL_PROVIDERS:
        return None
...
class _HealPass:
    ...
    def __init__(self, profile_store, root_store, provider_id):
        self.profile_store, self.root_store = profile_store, root_store
        self.provider_id = provider_id
        self.summary: Dict[str, Any] = {"adopted": False, "stripped_ids": [], "providers_block": False}
    ...
    def notice(self, profile_name: str) -> str:
        ...
        log_bits = [bit for bit, present in (
            ...
            (f"providers.{self.provider_id} block", summary["providers_block"])) if present]
        verdict = (
...
def _heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        # ~/.cryozen-agent/auth.json from a test that forgot to isolate HOME.
        real_home_env = os.environ.get("HOME", "")
        if real_home_env and _same_path(root_path, Path(real_home_env) / ".cryozen-agent" / "auth.json"):
            return None
    ...
    fingerprint = (str(profile_path), _stat_sig(profile_path), str(root_path), _stat_sig(root_path))
    ...
    with _auth_store_lock():
        ...
        with _auth_store_lock(target_path=root_path):
            ...
            run = _HealPass(profile_store, root_store, provider_id)
            run.heal_pool_rows()
