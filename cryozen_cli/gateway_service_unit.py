...
Extracted from ``cryozen_cli/gateway.py``. Bodies read facade helpers through ``_gw()`` (late
binding on ``cryozen_cli.gateway``) so the seams tests and callers patch on the facade keep
...
def _gw():
    from cryozen_cli import gateway  # late: the facade imports this module
    return gateway
...
def _ld_library_path_line(system: bool, target_home_dir: str | None = None) -> str:
    ...
    ~/.cryozen-agent/.env is too late for CUDA libs — #14613); system units remap caller-home components.
...
def _cryozen_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current CRYOZEN_HOME (root's, under sudo) to the target user's equivalent:
    ``/root/.cryozen[/profiles/x]`` → ``/home/alice/.cryozen[/profiles/x]``; custom paths kept as-is."""
    current_cryozen_raw = os.environ.get("CRYOZEN_HOME", "").strip()
    current_cryozen = Path(current_cryozen_raw).expanduser() if current_cryozen_raw else _gw().get_cryozen_home()
    # Keep paths lexical: resolving a non-existent path can bake a different CRYOZEN_HOME into the unit.
    current_default = Path.home() / ".cryozen-agent"
    target_default = Path(target_home_dir) / ".cryozen-agent"
    try:
        # Default ~/.cryozen-agent or a profile/subdir of it → preserve the relative structure under the target.
        return str(target_default / current_cryozen.relative_to(current_default))
    except ValueError:
        return str(current_cryozen)  # Completely custom path (not under ~/.cryozen-agent) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    cryozen_home = _gw().get_cryozen_home()
    extras = (project_root / "node_modules" / ".bin", cryozen_home / "node" / "bin", cryozen_home / "node_modules" / ".bin")
    for extra in extras:
        if _is_dir(extra):
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (CRYOZEN_HOME, else _gw().PROJECT_ROOT). cwd is
...
def _systemd_watchdog_seconds(cryozen_home: str | Path | None = None) -> int:
    ...
    if cryozen_home is not None:
        ...
        override_token = set_cryozen_home_override(cryozen_home)
        reset_home_override = reset_cryozen_home_override
    try:
...
def _append_node_dir_for_service(path_entries: list[str], cryozen_root: Path | None = None) -> None:
    """Append the Node dir a service unit should use: managed ``<cryozen_root>/node`` (profile-scoped)
    first — a unit survives reboots, so baking a shell-PATH Node is permanent breakage — else PATH lookup."""
    from cryozen_constants import (cryozen_managed_node_tree_present, iter_cryozen_node_dirs)
    managed_node_present = cryozen_managed_node_tree_present(cryozen_root)
    for directory in iter_cryozen_node_dirs(cryozen_root) if managed_node_present else ():
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        cryozen_home = _gw()._cryozen_home_for_target_user(home_dir)
        # Profile arg relative to the TARGET user's ~/.cryozen-agent when cryozen_home lives under it.
        target_root = Path(home_dir) / ".cryozen-agent"
        try:
            Path(cryozen_home).resolve().relative_to(target_root.resolve())
            profile_arg = _gw()._profile_arg(cryozen_home, default_root=target_root)
        except ValueError:
            profile_arg = _gw()._profile_arg(cryozen_home)
        ...
        working_dir = str(cryozen_home) if cryozen_home else _gw()._remap_path_for_user(working_dir, home_dir)
        ...
        _gw()._append_node_dir_for_service(_target_node_entries, Path(cryozen_home) if cryozen_home else None)
        ...
        cryozen_home = str(_gw().get_cryozen_home().resolve())
        profile_arg = _gw()._profile_arg(cryozen_home)
        user_home = Path.home()
    ...
    watchdog_seconds = _gw()._systemd_watchdog_seconds(cryozen_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m cryozen_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
{env_lines}Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="CRYOZEN_HOME={cryozen_home}"
Environment="CRYOZEN_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    import re
    return re.sub(
        r"(<key>PATH</key>\s*<string>)(.*?)(</string>)", r"\1__CRYOZEN_PATH__\3",
        _gw()._normalize_service_definition(text), flags=re.S,
    )


def systemd_unit_is_current(system: bool = False) -> bool:
    # CRYOZEN_HOME sync chokepoint for every compare/regenerate path: under `sudo … --system` it is often
    # stripped to /root/.cryozen-agent, so refresh would rewrite a correct unit and status warn forever.
    # Idempotent; the os.environ mutation persists for later runtime reads (restart's PID/drain).
    _gw()._sync_cryozen_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Temp-dir CRYOZEN_HOME baked into a systemd unit / launchd plist, or None. A temp home means a
    test/E2E harness generated it; installing it leaves the gateway "running" but deaf to every platform."""
    import re
    import tempfile
    candidates = re.findall(r'CRYOZEN_HOME=([^"\n]+)', definition)
    candidates += re.findall(r"<key>CRYOZEN_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S)
    temp_roots = {
        Path(tempfile.gettempdir()).resolve(),
        Path("/tmp"), Path("/var/tmp"), Path("/private/tmp"), Path("/private/var/tmp"),  # no-tmp: ok — detects a temp CRYOZEN_HOME in service definitions
    }
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp CRYOZEN_HOME."""
    temp_home = _gw()._temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(f"✗ Refusing to write the gateway {kind}: CRYOZEN_HOME resolves to a temporary directory ({temp_home}).")
    print(
        "  This usually means a test/E2E environment exported CRYOZEN_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # _gw().systemd_unit_is_current is the CRYOZEN_HOME-sync chokepoint; its env mutation persists for the regenerate below.
    ...
    # pytest-tmp CRYOZEN_HOME baked into the developer's real unit breaks their gateway on next reboot.
    if not system and any(m in new_unit for m in ("/pytest-of-", '/cryozen_test"', "/cryozen_test/")):
        return False

    # Structural variant: refuse ANY temp-dir CRYOZEN_HOME (manual E2E homes lack the pytest markers).
    ...
    print(f"↻ Updated gateway {_gw()._service_scope_label(system)} service definition to match the current Cryozen install")
    return True
