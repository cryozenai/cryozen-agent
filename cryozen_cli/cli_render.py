...
def _detect_light_mode_uncached() -> bool:
    ...
    for var in ("CRYOZEN_LIGHT", "CRYOZEN_TUI_LIGHT"):
        v = (os.environ.get(var) or "").strip().lower()
        if _TRUE_RE.match(v):
            return True
        if _FALSE_RE.match(v):
            return False
    theme = (os.environ.get("CRYOZEN_TUI_THEME") or "").strip().lower()
    if theme == "light":
        return True
    if theme == "dark":
        return False
    bg_lum = _luminance_from_hex(os.environ.get("CRYOZEN_TUI_BACKGROUND") or "")
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from cryozen_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_cryozen_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._cryozen_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _build_compact_banner() -> str:
    ...
    if (getattr(_skin, "name", "default") if _skin else "default") == "default":
        ...
        tiny_line = _skin.get_branding("agent_name", "Cryozen Agent") if _skin else "Cryozen Agent"
    line1 = f"{tiny_line} - AI Agent Framework"

    if os.environ.get("CRYOZEN_FAST_STARTUP_BANNER") == "1":
        from cryozen_cli import __release_date__ as _release_date
        from cryozen_cli import __version__ as _version

        version_line = f"Cryozen Agent v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Cryozen[/]\n"
