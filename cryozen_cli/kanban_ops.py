"""Dispatcher and maintenance verbs for ``cryozen kanban``: ``dispatch``,
...
from cryozen_cli.kanban_output import _err, _fmt_ts, _print_json
...
_DAEMON_DEPRECATED = (
    "cryozen kanban daemon: DEPRECATED — the dispatcher now runs\ninside the gateway. To use "
    "kanban:\n\n    cryozen gateway start       # starts the gateway + embedded dispatcher\n\nReady "
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                ...
                print(
                    ...
                    f"successfully.{held} Check profile health (venv, PATH, credentials) and `cryozen "
                    f"kanban list --status ready` / `cryozen kanban list --status blocked` for "
