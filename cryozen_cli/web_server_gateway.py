...
from cryozen_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from cryozen_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from cryozen_cli.profiles import _check_gateway_running, profiles_to_serve
...
# ~/.cryozen-agent/logs/<action>.log which the dashboard tails.

_ACTION_LOG_DIR: Path = get_cryozen_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "cryozen-update": "cryozen-update.log",
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from cryozen_cli.web_server import PROJECT_ROOT
...
def _profile_action_environment(
    ...
    """Environment for a detached ``cryozen <subcommand>`` action.

    The dashboard loads its own profile's ``.env`` into process-global ``os.environ``. Copying
    that mapping verbatim into ``cryozen -p <other> ...`` lets the named child see the dashboard
    profile's platform credentials and ports *before* its own dotenv loads (``load_cryozen_dotenv``
    ...
    Named-profile actions therefore start from Cryozen' standard scrubbed subprocess env, then drop
    the profile-managed keys plus every key declared by the dashboard/default profile dotenv files
    and their hydrated secret sources, and pin ``CRYOZEN_HOME`` to the target profile. The child's
    ...
    if profile is None:
        ...
        from cryozen_cli.env_loader import (
            _PROFILE_MANAGED_ENV_KEYS, _env_keys_defined_in_dotenv, get_secret_source_values,
        )
        from cryozen_cli.web_server_profiles import _resolve_profile_dir
        from cryozen_constants import apply_subprocess_home_env, get_default_cryozen_root
        ...
        try:
            source_homes = {str(get_default_cryozen_root()), str(get_cryozen_home())}
        except Exception:
            source_homes = set()
        ...
        action_env["CRYOZEN_HOME"] = str(target_home)
        apply_subprocess_home_env(action_env)

    action_env["CRYOZEN_NONINTERACTIVE"] = "1"
    ...
    # The dashboard runs inside the gateway process, so os.environ carries _CRYOZEN_GATEWAY=1;
    ...
    action_env.pop("_CRYOZEN_GATEWAY", None)
    if env_overrides:
...
def _action_targets_system_gateway(subcommand: List[str]) -> bool:
    ...
    ``cryozen update`` fleet restart (``update_cmd_fleet._needs_sudo`` / ``_sudo_noninteractive_ok``).
    """
    from cryozen_cli.update_cmd_fleet import _needs_sudo
    ...
    from cryozen_cli.gateway import _select_systemd_scope
    from cryozen_constants import reset_cryozen_home_override, set_cryozen_home_override

    profile = _named_profile_from_action(subcommand)
    if profile is None:
        return _select_systemd_scope(False)
    # Unit names are derived from CRYOZEN_HOME, so a selector-bearing action must be resolved
    ...
    from cryozen_cli.web_server_profiles import _resolve_profile_dir
    token = set_cryozen_home_override(_resolve_profile_dir(profile))
    try:
        return _select_systemd_scope(False)
    finally:
...
def _spawn_cryozen_action(
    subcommand: List[str], name: str, *, env_overrides: Optional[Dict[str, str]] = None
) -> subprocess.Popen:
    """Spawn ``cryozen <subcommand>`` detached (via ``cryozen_cli.main``) and record the handle."""
    from cryozen_cli.web_server import PROJECT_ROOT
    _ACTION_LOG_DIR.mkdir(parents=True, exist_ok=True)
    log_file = open(_ACTION_LOG_DIR / _ACTION_LOG_FILES[name], "ab", buffering=0)
    log_file.write(f"\n=== {name} started {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n".encode())

    cmd = [_dashboard_spawn_executable(), "-m", "cryozen_cli.main", *subcommand]
    if _action_targets_system_gateway(subcommand):
        ...
        # CRYOZEN_HOME past sudo's env_reset and reads SUDO_USER for the service identity.
        ...
        # two-step gate as the ``cryozen update`` fleet restart: a refused blanket probe falls
        # back to ``sudo -l`` on the exact argv, so a command-scoped NOPASSWD entry qualifies.
        from cryozen_cli.update_cmd_fleet import _sudo_noninteractive_ok
        ...
        if not _sudo_noninteractive_ok(["-l", "--", *cmd]):
            message = (
                ...
                f"'sudo cryozen {' '.join(subcommand)}' on the host, or grant that user NOPASSWD sudo."
            )
    ...
    # dashboard profile's credentials; see _profile_action_environment (also drops _CRYOZEN_GATEWAY).
    ...
    action_id = (env_overrides or {}).get("CRYOZEN_ACTION_ID")
    if action_id:
...
def _own_profile_selector(profile: Optional[str]) -> Optional[str]:
    ...
    profile (a pooled Desktop ``cryozen --profile X serve`` answers ``/api/gateway/*`` without
    ...
    own = profile_name_for_home(get_process_cryozen_home())
    return own if own and own != "default" else None


def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    """``cryozen [-p X] gateway <verb>`` argv for a dashboard lifecycle action. A profile served by the
    ...
    pooled ``--profile X serve`` would inherit X's ``CRYOZEN_HOME`` and hit the same exit-78 refusal."""
    from cryozen_cli.web_server_profiles import _profile_cli_args
    ...
    if profile and verb == "restart" and multiplexed_profile_refusal(profile, verb) is not None:
        from cryozen_constants import get_process_cryozen_home, profile_name_for_home
        args = [] if profile_name_for_home(get_process_cryozen_home()) == "default" else ["-p", "default"]
...
def multiplexed_profile_refusal(profile: Optional[str], verb: str) -> Optional[str]:
    ...
    from cryozen_cli.gateway_migrate import _installed_services
    ...
    return (f"Profile '{requested}' does not get a gateway of its own: one host gateway serves every "
            f"profile. Install or start it from the default profile (cryozen gateway install), or fold an "
            f"existing per-profile fleet with `cryozen gateway migrate --multiplex`; "
            f"`cryozen -p {requested} gateway install --force` starts a separate one anyway.")
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from cryozen_cli.web_server import _spawn_gateway_restart
    try:
