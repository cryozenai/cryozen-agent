...
Split out of ``cryozen_cli.kanban_db``; origin-resident helpers are reached
...
from cryozen_cli.quiet_single_query import KANBAN_WORKER_EXIT_TRAILER

if TYPE_CHECKING:
    from cryozen_cli.kanban_db import Task
...
# ``CRYOZEN_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Cryozen profile. Expected steady-state
...
def describe_suppression(results: Iterable[Optional["DispatchResult"]]) -> str:
    ...
    bare zero-spawn count while ``cryozen kanban tail`` is the only place the
...
# children): a per-tick ``cryozen kanban dispatch`` process finds it empty, so
...
def _worker_log_exit_code(task_id: str, board: Optional[str] = None) -> Optional[int]:
    ...
    (``cryozen_cli.quiet_single_query.exit_single_query``), so it is there whether
...
_LOG_CHROME = re.compile(r"[─━═╭╮╰╯│┃┌┐└┘]+|✻\s*Cryozen")
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``cryozen kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``cryozen_cli.profiles.profile_exists``, or ``None`` when it cannot be
    ...
    """
    try:
        from cryozen_cli.profiles import normalize_profile_name, profile_exists
    except Exception:
        return None
...
def _dispatch_profile_allowlist(normalize_profile_name) -> Optional[frozenset]:
    ...
    On a shared board (one ``kanban.db`` mounted across several Cryozen homes),
...
def dispatch_profile_allowlist_summary() -> str:
    ...
    Surfaced by ``cryozen kanban diagnostics`` so an operator on a shared board
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Cryozen profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Cryozen ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``CRYOZEN_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``cryozen -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``cryozen kanban tail`` shows why the task looks stuck.
...
def _module_cryozen_argv() -> list[str]:
    """Interpreter-bound Cryozen CLI invocation (``cryozen_cli.main`` is the
    console-script target — there is no top-level ``cryozen`` package)."""
    return [sys.executable, "-m", "cryozen_cli.main"]


def _absolute_cryozen_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Cryozen shim."""
...
def _cryozen_path_argv(path: str) -> list[str]:
    """argv for a resolved Cryozen executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_cryozen_argv()
    return [_absolute_cryozen_path(path)]


def _resolve_cryozen_argv() -> list[str]:
    """Resolve the ``cryozen`` invocation as argv for ``Popen``: ``$CRYOZEN_BIN``
    ...
    cryozen_cli.main`` (exactly this install; also covers shim-less cron,
    systemd ``User=``, launchd), then ``which("cryozen")`` (Windows: safe PATH
    search, batch shims fall back to the module form) only when ``cryozen_cli``
    is not importable. The module argv must win over PATH: a PATH-first lookup
    lets an attacker-planted ``cryozen`` shadow the running install (#111569).
    Mirrors ``gateway.run._resolve_cryozen_bin``; local because ``cryozen_cli``
    sits below ``gateway`` in the dependency order.
    """
    import importlib.util
    import shutil

    env_bin = os.environ.get("CRYOZEN_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _cryozen_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _cryozen_path_argv(resolved_env_bin)
        return _module_cryozen_argv()
    ...
    cryozen_bin = _safe_which_no_cwd("cryozen") if _kb._IS_WINDOWS else shutil.which("cryozen")
    if cryozen_bin:
        return _cryozen_path_argv(cryozen_bin)
    return _module_cryozen_argv()
...
def _worker_profile_scope(cryozen_home: str, *, bind_home: bool = True):
    ...
    from cryozen_constants import get_process_cryozen_home, reset_cryozen_home_override, set_cryozen_home_override
    ...
    home = Path(cryozen_home)
    is_launch_home = str(home.resolve()) == str(Path(get_process_cryozen_home()).resolve())
    home_token = set_cryozen_home_override(str(home)) if bind_home else None
...
def _resolve_worker_cli_toolsets(cryozen_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``CRYOZEN_KANBAN_TASK`` is set.
    """
    if not cryozen_home:
        return None
    try:
        ...
        with _worker_profile_scope(cryozen_home):
            cfg = load_config()
            toolsets = sorted(_get_platform_tools(cfg, "cli"))
        return toolsets or None
    except Exception as exc:
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for CRYOZEN_HOME=%r (%s)",
            cryozen_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, cryozen_home: Optional[str]) -> list[str]:
    """Build the ``cryozen -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_cryozen_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped CRYOZEN_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(cryozen_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `cryozen kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``cryozen -p <profile> chat -q ...`` subprocess.
    ...
    ``CRYOZEN_KANBAN_DB`` / ``CRYOZEN_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from cryozen_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    try:
        ...
        # CRYOZEN_PROFILE (set below) instead.
        profile_home = None
    ...
    # Inject CRYOZEN_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_cryozen_home() falls back to the DEFAULT
    # profile root because `cryozen -p` applies its override before
    # cryozen_constants is imported.
    if profile_home:
        env["CRYOZEN_HOME"] = profile_home
    ...
    if task.tenant:
        env["CRYOZEN_TENANT"] = task.tenant
    env["CRYOZEN_KANBAN_TASK"] = task.id
    env["CRYOZEN_KANBAN_WORKSPACE"] = workspace
    ...
    env["CRYOZEN_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["CRYOZEN_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["CRYOZEN_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["CRYOZEN_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["CRYOZEN_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["CRYOZEN_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `cryozen -p` rewrites CRYOZEN_HOME (symlink / Docker layouts).
    env["CRYOZEN_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["CRYOZEN_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["CRYOZEN_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["CRYOZEN_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads CRYOZEN_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["CRYOZEN_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping CRYOZEN_TUI covers
    # older cryozen builds on PATH that predate the flag's precedence.
    env.pop("CRYOZEN_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("CRYOZEN_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`cryozen` executable not found on PATH. "
            "Install Cryozen Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``cryozen kanban dispatch`` — the standalone
...
from cryozen_cli import kanban_db as _kb  # noqa: E402
from cryozen_cli import kanban_db_connect as _kbc  # noqa: E402
from cryozen_cli import kanban_db_workspace as _kbw  # noqa: E402
