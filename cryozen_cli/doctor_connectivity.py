"""API connectivity probes for ``cryozen doctor`` (split out of ``doctor.py``).
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from cryozen_cli.providers import normalize_provider as _normalize_provider
...
_OPENROUTER_STATUS = {
    ...
    402: ("(out of credits — payment required)",
          ...
          "Fix: run 'cryozen config set model.provider <provider>' "
...
def _probe_anthropic() -> ProbeResult:
    ...
    try:
        ...
        r = httpx.get("https://api.anthropic.com/v1/models",
                      headers={"anthropic-version": "2023-06-01", "x-api-key": key}, timeout=10)
    except Exception as e:
...
def _anthropic_messages_probe(base: str, key: str):
    ...
    headers = {"anthropic-version": "2023-06-01", "User-Agent": _CRYOZEN_USER_AGENT, **auth}
...
def _apikey_request(key: str, base_env, default_url) -> tuple:
    ...
    headers = {"Authorization": f"Bearer {key}", "User-Agent": _CRYOZEN_USER_AGENT}
...
def _probe_github_token() -> ProbeResult:
    ...
    """
    name = "GitHub token"
    from cryozen_cli.config import get_env_value, load_env
    ...
    try:
        ...
        r = httpx.get(GITHUB_API_PROBE_URL, timeout=10, headers={
            "Authorization": f"Bearer {get_env_value(var)}", "User-Agent": _CRYOZEN_USER_AGENT,
