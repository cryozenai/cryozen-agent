"""CRYOZEN_HOME state checks for cryozen doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
from cryozen_cli.sizefmt import format_bytes as _human_bytes
from cryozen_state_common import FTS_STORAGE_VERSION
...
def _doctor_memory_config(cryozen_home: Path | None = None) -> dict:
    ...
    try:
        ...
        config_path = (cryozen_home if cryozen_home is not None else CRYOZEN_HOME) / "config.yaml"
...
def _render_state_db_stats(stats: dict, holders=None, host_note: str = "") -> list:
    ...
    if isinstance(deferral, dict):
        ...
        if deferral.get("futile"):
            ...
            lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                          ...
                          "'cryozen sessions optimize-storage' with every holder stopped)"))
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run 'cryozen sessions optimize-storage' offline (with the host gateway stopped) to compact FTS storage"
...
def _memory_store_flags(cryozen_home: Path) -> tuple:
    from tools.memory_tool import get_builtin_memory_store_flags
    return get_builtin_memory_store_flags({"memory": _doctor_memory_config(cryozen_home)})


@doctor_check()
def _check_directory_structure(should_fix: bool, f: Finding) -> None:
    """CRYOZEN_HOME, expected subdirs, SOUL.md, and the enabled built-in memory files."""
    from cryozen_cli.doctor import CRYOZEN_HOME, _DHH
    cryozen_home = CRYOZEN_HOME
    ensure_dir(f, should_fix, cryozen_home, f"{_DHH} directory exists", f"Created {_DHH} directory", f"{_DHH} not found")
    _memory_enabled, _user_profile_enabled = _memory_store_flags(cryozen_home)
    ...
    for subdir_name in ["cron", "sessions", "logs", "skills"] + (["memories"] if memory_on else []):
        ensure_dir(f, should_fix, cryozen_home / subdir_name, f"{_DHH}/{subdir_name}/ exists",
                   f"Created {_DHH}/{subdir_name}/", f"{_DHH}/{subdir_name}/ not found")
    _check_scratch_dir(cryozen_home, _DHH)
    # SOUL.md persona file
    soul_path = cryozen_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Cryozen a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text("# Cryozen Agent Persona\n\n<!-- Edit this file to customize how Cryozen communicates. -->\n\n"
                                 "You are Cryozen, a helpful AI assistant.\n", encoding="utf-8")
    ...
    memories_dir = cryozen_home / "memories"
    if not memory_on:
...
def unpruned_cache_hogs(cryozen_home: Path, min_bytes: int = _UNPRUNED_CACHE_WARN_BYTES) -> list[tuple[str, int]]:
    ...
    from cryozen_constants import scratch_dir_usage_bytes

    cache = cryozen_home / "cache"
...
def _check_scratch_dir(cryozen_home: Path, _DHH: str) -> None:
    ...
    scratch = get_scratch_dir(cryozen_home, prune=False)
    ...
    for name, nbytes in unpruned_cache_hogs(cryozen_home):
        check_warn(
    ...
    if tmpdir and tmpdir != os.environ.get(SCRATCH_DIR_MARKER_ENV, ""):
        check_info(f"TMPDIR={tmpdir} is set by you or the OS, so Cryozen leaves it alone")
...
def _write_health_reason(state_db_path: Path, *, should_fix: bool):
    ...
    from cryozen_state_repair import _db_opens_cleanly, _live_writer_holds_db
    ...
    if not should_fix and state_db_path.stat().st_size > _WRITE_PROBE_SNAPSHOT_MAX_BYTES:
        check_info("state.db write-health probe skipped: store is held by a live writer and larger than 1 GB "
                   "(run 'cryozen doctor --fix' to probe it)")
        return None
    import sqlite3
    import tempfile
...
_STATE_DB_REPAIRS = {
    "fts": ("Repaired state.db FTS write health",
            ...
            "state.db FTS write corruption — run 'cryozen doctor --fix' (or 'cryozen sessions repair') to rebuild the FTS index"),
    "schema": ("Repaired state.db schema ({count} sessions recovered)",
               ...
               "state.db schema malformed — run 'cryozen doctor --fix' (or 'cryozen sessions repair') to recover hidden sessions"),
}
_STATE_DB_STRUCTURAL_ISSUE = (
    ...
    "cannot repair it. Stop the gateway, then run 'cryozen {profile_arg}sessions recover --source {db_path} "
    "--inspect-only' and, if it reports recoverable, 'cryozen {profile_arg}sessions recover --source {db_path} "
...
def _repair_state_db(f: Finding, should_fix: bool, state_db_path: Path, kind: str) -> None:
    ...
    from cryozen_state_repair import repair_state_db_schema
...
def _report_structural_damage(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str, reason) -> bool:
    ...
    from cryozen_state_repair import state_db_has_structural_damage
...
def _classify_unreadable_state_db(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str, exc: Exception) -> None:
    ...
    from cryozen_state import is_malformed_db_error
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        from cryozen_state_dbfile import collect_state_db_stats, count_db_holders
        ...
        for _kind, _text, _detail in rows:
            ...
            if "auto_prune" in _detail:
                issues.append("state.db is large — enable sessions.auto_prune in config.yaml"
                              + (" and run 'cryozen sessions optimize-storage' offline (gateway stopped)" if "optimize-storage" in _detail else ""))
...
def _state_db_wal(f: Finding, should_fix: bool, state_db_path: Path) -> None:
    ...
    with warn_on_error(""):
        ...
        if size > 50 * 1024 * 1024:  # 50 MB
            ...
            from cryozen_state_repair import _exclusive_repair_db_guard, _live_writer_holds_db
            ...
            _SKIP = ("Large WAL file — cannot prove state.db is quiet (stop the profile's gateway first, then "
                     "run 'cryozen doctor --fix' to checkpoint)")
            ...
            if not should_fix:
                return f.issues.append(
                    "Large WAL file — stop the profile's gateway, then run 'cryozen doctor --fix' to checkpoint")
            with _exclusive_repair_db_guard(state_db_path) as (guard, guard_error):
                if guard is None:
                    check_warn("WAL checkpoint skipped: could not take exclusive ownership of state.db",
                               f"({guard_error}; stop the profile's gateway and re-run 'cryozen doctor --fix')")
                    return f.issues.append(_SKIP)
...
def _retired_wal_holders(f: Finding, state_db_path: Path, _DHH: str) -> bool:
    ...
    from cryozen_constants import profile_cli_selector
    from cryozen_state_dbfile import iter_deleted_sqlite_sidecar_holders
    from cryozen_state_holders import describe_holder_pid
    ...
    f.issues.append(f"state.db retired WAL generation held by {rendered}{host_gateway_note()} — stop the host "
                    f"gateway, dashboard and cron writers among them ('cryozen {profile_cli_selector()}gateway "
                    "stop' stops the ONE host process serving every profile, quit the Desktop app), do not "
                    "delete the WAL yourself, then rerun 'cryozen doctor'")
    return True
...
def _check_state_db(should_fix: bool, f: Finding) -> None:
    ...
    from cryozen_cli.doctor import CRYOZEN_HOME, _DHH
    state_db_path = CRYOZEN_HOME / "state.db"
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    from cryozen_cli.doctor import CRYOZEN_HOME, _DHH
    hub_dir = CRYOZEN_HOME / "skills" / ".hub"
    if check_bool(hub_dir.exists(), "Skills Hub directory exists", ("Skills Hub directory not initialized", "(run: cryozen skills list)")):
...
def _memory_provider_honcho(issues: list) -> None:
    ...
    if not cfg_path.exists():
        ...
        check_bool(hcfg.api_key or hcfg.base_url,
                   ...
                   ("Honcho config not found", "run: cryozen memory setup"))
    elif not hcfg.enabled:
        check_info(f"Honcho disabled (set enabled: true in {cfg_path} to activate)")
    elif not (hcfg.api_key or hcfg.base_url):
        _fail_and_issue("Honcho API key or base URL not set", "run: cryozen memory setup",
                        "No Honcho API key — run 'cryozen memory setup'", issues)
    else:
...
def _memory_provider_mem0(issues: list) -> None:
    ...
    if mem0_cfg.get("api_key", ""):
        ...
        _fail_and_issue("Mem0 API key not set", "(set MEM0_API_KEY in .env or run cryozen memory setup)",
...
def _memory_provider_generic(name: str) -> None:
    ...
    if _provider and _provider.is_available():
        ...
        check_warn(f"{name} configured but not available", "run: cryozen memory status")
    else:
        check_warn(f"{name} plugin not found", "run: cryozen memory setup")
...
def _check_memory_provider(should_fix: bool, f: Finding) -> None:
    ...
    name = _doctor_memory_config(CRYOZEN_HOME).get("provider", "")
...
def _check_profiles(should_fix: bool, f: Finding) -> None:
    from cryozen_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
    import re as _re
    ...
    if wrapper_dir.is_dir():
        for wrapper in wrapper_dir.iterdir():
            if not wrapper.is_file():
                continue
            with warn_on_error(""):
                _m = _re.search(r"cryozen -p (\S+)", wrapper.read_text(encoding="utf-8"))
                if _m and not profile_exists(_m.group(1)):
                    check_warn(f"Orphan alias: {wrapper.name} → profile '{_m.group(1)}' no longer exists")
    # Same helper as the multiplex migration preflight, so doctor names the duplicates that make
    # `cryozen gateway migrate --multiplex` refuse (and made pre-multiplex standalone gateways race).
