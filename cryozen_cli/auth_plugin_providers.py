"""Model-provider plugins as first-class citizens of ``cryozen auth`` and the credential pool.

A ``plugins/model-providers/<name>/`` profile (``providers.base.ProviderProfile``) is mirrored into
``cryozen_cli.auth.PROVIDER_REGISTRY`` with the ``auth_type`` it declares, so ``resolve_provider()``
...
- ``auth_handler(action, args) -> bool`` for ``cryozen auth add|status|logout|refresh <name>``
...
Every function here late-imports ``cryozen_cli.auth`` names: this module is imported by the auth facade
...
# can be "OAuth-shaped with nobody to log them in": a bundled OAuth provider (openai-codex, xai-oauth …)
...
def register_plugin_provider(pp: Any) -> None:
    ...
    """
    from cryozen_cli.auth import PROVIDER_REGISTRY, ProviderConfig, _api_key_provider
...
def _user_owns_alias(pp: Any, alias: str) -> bool:
    """True when *alias* resolves to the ``$CRYOZEN_HOME`` plugin *pp* in the ``providers`` layer."""
    try:
...
def mirror_aliases(pconfig: Any, pp: Any) -> None:
    ...
    A bundled plugin never steals an alias another row already holds; a ``$CRYOZEN_HOME`` plugin
...
def override_registry_row(pconfig: Any, pp: Any) -> None:
    """A ``$CRYOZEN_HOME`` plugin re-registering a name that already has a row wins for the fields
...
def sync_plugin_provider_registry() -> int:
    ...
    whose own imports pull ``cryozen_cli.auth`` in mid-``_discover_providers()`` sees only what was
    registered so far, and every later plugin would otherwise fail with "Unknown provider" (#102123).
    """
    from cryozen_cli.auth import BUILTIN_PROVIDER_IDS, PROVIDER_REGISTRY
...
def dispatch_plugin_auth(action: str, args: Any, provider: str) -> bool:
    """Offer ``cryozen auth <action> <provider>`` to the provider's ``auth_handler``.
...
def plugin_missing_auth_handler_error(provider: str, action: str) -> Optional[SystemExit]:
    ...
    return SystemExit(
        ...
        f"auth_handler, so `cryozen auth {action} {provider}` cannot be handled. Add "
...
def _pool_entry_expired(entry: Any) -> bool:
    ...
    import time
    from cryozen_cli.auth import _parse_iso_timestamp
...
def get_plugin_oauth_auth_status(provider_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "hint": "" if live else f"Run `cryozen auth add {provider_id}` to sign in."}
