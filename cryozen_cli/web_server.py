"""Cryozen Agent — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m cryozen_cli.main web [--port 8080]``.
"""
...
from cryozen_cli.install_identity import get_install_id as _shared_get_install_id
from cryozen_cli.pty_session import run_reaper
...
from cryozen_cli import __version__
...
try:
    ...
    # running `cryozen dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["CRYOZEN_WEB_DIST"]) if "CRYOZEN_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)


from cryozen_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``cryozen dashboard`` backend, not a gateway, so without
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from cryozen_cli.profiles import (
            ...
            if initial_profile_homes:
                ...
                from cryozen_logging import enable_profile_log_routing
...
async def _lifespan(app: "FastAPI"):
    ...
    # (#79531/#80037): a store left behind by `cryozen update` otherwise 500s
    ...
    # Import cryozen_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    # refuse with "restart required" after `cryozen update` replaced the code
    ...
    # Desktop-spawned backends (CRYOZEN_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `cryozen
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("CRYOZEN_DESKTOP") == "1":
        ...
        try:
            from cryozen_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            from cryozen_cli.config import load_config
            from cryozen_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    try:
        ...
        try:
            from cryozen_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="Cryozen Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from cryozen_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402

app.include_router(_memory_oauth_router)

# Session token for sensitive endpoints. The desktop shell mints it via
# CRYOZEN_DASHBOARD_SESSION_TOKEN; otherwise fresh per server start. It dies with
# the process and is injected into the SPA HTML so only the web UI can use it.
def _resolve_session_token() -> str:
    return os.environ.get("CRYOZEN_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Cryozen-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".cryozen-ssh-runtime-{nonce}")
...
# drift (/api/status once 401'd under the OAuth gate, breaking the liveness probe).
from cryozen_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from cryozen_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    old launch scripts but IGNORED since the June 2026 cryozen-0day campaign.
    """
...
def _desktop_loopback_auth_exempt(
    ...
    ``CRYOZEN_DESKTOP=1``, and an operator-minted credential (env token, SSH
    session token, or owner nonce).
    """
    return (
        host in _LOOPBACK_HOST_VALUES
        and os.environ.get("CRYOZEN_DESKTOP") == "1"
        and bool(os.environ.get("CRYOZEN_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if plugin_name and (
        ...
        try:
            ...
            # (#46435)
            from cryozen_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    """
    from cryozen_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    """
    from cryozen_cli.dashboard_auth.token_auth import token_auth_middleware
...
from cryozen_cli import web_server_gateway as _gateway_mod  # noqa: E402
from cryozen_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from cryozen_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from cryozen_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from cryozen_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT Cryozen home (not the profile CRYOZEN_HOME) so every
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``cryozen gateway restart``, reusing an in-flight or recent restart.
    ...
    proc = _gateway_mod._spawn_cryozen_action(subcommand, "gateway-restart")
...
from cryozen_cli.web_routers import (  # noqa: E402
...
from cryozen_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    for _name in ("basic", "self_hosted"):
        try:
            _reason = importlib.import_module(f"plugins.dashboard_auth.{_name}").LAST_SKIP_REASON
        except Exception:
            continue
        if _reason:
            skip_reasons.append(f"  • {_name}: {_reason}")
    ...
    if host in _LOOPBACK_HOST_VALUES:
        ...
        fix_hint = (
            ...
            "(and unset CRYOZEN_DASHBOARD_PUBLIC_URL) to restore the "
    ...
    fix_hint += (
        ...
        "  • OIDC: set dashboard.oauth.self_hosted.issuer + client_id in config.yaml, or "
    ...
    try:
        from cryozen_cli.config import load_config as _load_cfg
        from cryozen_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
        ...
        if has_creds and (set(disabled) & _BASIC_AUTH_PLUGIN_KEYS):
            fix_hint = (
                ...
                "`cryozen plugins enable basic`), then restart the "
                "dashboard.\n\n"
            ) + fix_hint
    except Exception:
        pass
...
def _configure_auth_gate(
    ...
    # ``--insecure`` no longer disables the gate (June 2026 cryozen-0day
    ...
    if app.state.auth_required:
        ...
        from cryozen_cli.dashboard_auth import list_providers
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from cryozen_cli.web_server_idle_exit import (
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from cryozen_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from cryozen_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    # No-op for standalone `cryozen serve` (no CRYOZEN_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from cryozen_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    # Published by /api/host/identity: an attaching `cryozen dashboard` must never be routed to a
    ...
    # ACTUAL port — what lets `cryozen update` relaunch a manually-started serve
    # on its real endpoint (#63206).
    def _register_identity() -> None:
        from cryozen_cli.process_identity import attach_self_to_kill_on_close_job, register_self
    ...
    # Host rendezvous (multiplex-only): the host lock + record that let a SECOND `cryozen serve`
    ...
    ready_token = "CRYOZEN_BACKEND_READY" if headless else "CRYOZEN_DASHBOARD_READY"
    ...
    if headless:
        ...
        print(f"  Cryozen backend listening on {host}:{actual_port}", flush=True)
    else:
        print(f"  Cryozen Web UI → http://{host}:{actual_port}")
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from cryozen_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    JSON-RPC/WS backend, no UI build, no SPA mount (``CRYOZEN_SERVE_HEADLESS``).
    ...
    from cryozen_cli.resource_limits import apply_nofile_soft_limit
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('cryozen_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('cryozen_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('cryozen_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('cryozen_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('cryozen_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('cryozen_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('cryozen_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('cryozen_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('cryozen_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('cryozen_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('cryozen_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('cryozen_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('cryozen_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('cryozen_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('cryozen_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('cryozen_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('cryozen_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('cryozen_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('cryozen_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('cryozen_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('cryozen_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('cryozen_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('cryozen_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('cryozen_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('cryozen_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('cryozen_cli.web_models', 'HookCreate'),
    'HookDelete': ('cryozen_cli.web_models', 'HookDelete'),
    'ImportRequest': ('cryozen_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('cryozen_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('cryozen_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('cryozen_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('cryozen_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('cryozen_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('cryozen_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('cryozen_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('cryozen_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('cryozen_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('cryozen_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('cryozen_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('cryozen_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('cryozen_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('cryozen_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('cryozen_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('cryozen_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('cryozen_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('cryozen_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('cryozen_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('cryozen_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('cryozen_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('cryozen_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('cryozen_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('cryozen_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('cryozen_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('cryozen_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('cryozen_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('cryozen_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('cryozen_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('cryozen_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('cryozen_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('cryozen_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('cryozen_cli.pty_bridge', 'PtyUnavailableError'),
    'RawConfigUpdate': ('cryozen_cli.web_models', 'RawConfigUpdate'),
    'RegistryFull': ('cryozen_cli.pty_session', 'RegistryFull'),
    'STORAGE_HONCHO_HOST_BLOCK': ('plugins.memory.config_schema', 'STORAGE_HONCHO_HOST_BLOCK'),
    'SessionImport': ('cryozen_cli.web_models', 'SessionImport'),
    'SessionPrune': ('cryozen_cli.web_models', 'SessionPrune'),
    'SessionRename': ('cryozen_cli.web_models', 'SessionRename'),
    'SkillContentUpdate': ('cryozen_cli.web_models', 'SkillContentUpdate'),
    'SkillCreate': ('cryozen_cli.web_models', 'SkillCreate'),
    'SkillInstallRequest': ('cryozen_cli.web_models', 'SkillInstallRequest'),
    'SkillToggle': ('cryozen_cli.web_models', 'SkillToggle'),
    'SkillUninstallRequest': ('cryozen_cli.web_models', 'SkillUninstallRequest'),
    'SkillsUpdateRequest': ('cryozen_cli.web_models', 'SkillsUpdateRequest'),
    'TTSLeaseRequest': ('cryozen_cli.web_models', 'TTSLeaseRequest'),
    'TTSSpeakRequest': ('cryozen_cli.web_models', 'TTSSpeakRequest'),
    'TerminalBackendSelect': ('cryozen_cli.web_models', 'TerminalBackendSelect'),
    'ThemeSetBody': ('cryozen_cli.web_models', 'ThemeSetBody'),
    'ToolsetEnvUpdate': ('cryozen_cli.web_models', 'ToolsetEnvUpdate'),
    'ToolsetModelSelect': ('cryozen_cli.web_models', 'ToolsetModelSelect'),
    'ToolsetPostSetup': ('cryozen_cli.web_models', 'ToolsetPostSetup'),
    'ToolsetProviderSelect': ('cryozen_cli.web_models', 'ToolsetProviderSelect'),
    'ToolsetToggle': ('cryozen_cli.web_models', 'ToolsetToggle'),
    'WebhookCreate': ('cryozen_cli.web_models', 'WebhookCreate'),
    'WebhookEnabledToggle': ('cryozen_cli.web_models', 'WebhookEnabledToggle'),
    'WhatsAppOnboardingApply': ('cryozen_cli.web_models', 'WhatsAppOnboardingApply'),
    'WhatsAppOnboardingStart': ('cryozen_cli.web_models', 'WhatsAppOnboardingStart'),
    'activate_custom_endpoint': ('cryozen_cli.web_routers.config_env', 'activate_custom_endpoint'),
    'add_credential_pool_entry': ('cryozen_cli.web_routers.ops', 'add_credential_pool_entry'),
    'add_mcp_server': ('cryozen_cli.web_routers.mcp', 'add_mcp_server'),
    'apply_whatsapp_onboarding': ('cryozen_cli.web_routers.messaging', 'apply_whatsapp_onboarding'),
    'approve_pairing': ('cryozen_cli.web_routers.ops', 'approve_pairing'),
    'auth_mcp_server': ('cryozen_cli.web_routers.mcp', 'auth_mcp_server'),
    'bulk_delete_sessions_endpoint': ('cryozen_cli.web_routers.sessions', 'bulk_delete_sessions_endpoint'),
    'cancel_oauth_session': ('cryozen_cli.web_routers.oauth', 'cancel_oauth_session'),
    'cancel_whatsapp_onboarding': ('cryozen_cli.web_routers.messaging', 'cancel_whatsapp_onboarding'),
    'cfg_get': ('cryozen_cli.config', 'cfg_get'),
    'check_config_version': ('cryozen_cli.config', 'check_config_version'),
    'check_cryozen_update': ('cryozen_cli.web_routers.actions', 'check_cryozen_update'),
    'clear_model_endpoint_credentials': ('cryozen_cli.config', 'clear_model_endpoint_credentials'),
    'clear_pending_pairing': ('cryozen_cli.web_routers.ops', 'clear_pending_pairing'),
    'coerce_provider_id': ('cryozen_cli.config', 'coerce_provider_id'),
    'console_ws': ('cryozen_cli.web_routers.chat_ws', 'console_ws'),
    'count_empty_sessions_endpoint': ('cryozen_cli.web_routers.sessions', 'count_empty_sessions_endpoint'),
    'create_cron_job': ('cryozen_cli.web_routers.cron', 'create_cron_job'),
    'create_hook': ('cryozen_cli.web_routers.ops', 'create_hook'),
    'create_managed_directory': ('cryozen_cli.web_routers.files', 'create_managed_directory'),
    'create_profile_endpoint': ('cryozen_cli.web_routers.profiles', 'create_profile_endpoint'),
    'create_skill': ('cryozen_cli.web_routers.skills', 'create_skill'),
    'create_webhook': ('cryozen_cli.web_routers.ops', 'create_webhook'),
    'custom_endpoint_key_env': ('cryozen_cli.config', 'custom_endpoint_key_env'),
    'delete_agent_plugin': ('cryozen_cli.web_routers.dashboard_ui', 'delete_agent_plugin'),
    'delete_cron_job': ('cryozen_cli.web_routers.cron', 'delete_cron_job'),
    'delete_custom_endpoint': ('cryozen_cli.web_routers.config_env', 'delete_custom_endpoint'),
    'delete_empty_sessions_endpoint': ('cryozen_cli.web_routers.sessions', 'delete_empty_sessions_endpoint'),
    'delete_hook': ('cryozen_cli.web_routers.ops', 'delete_hook'),
    'delete_learning_node': ('cryozen_cli.web_routers.status', 'delete_learning_node'),
    'delete_managed_file': ('cryozen_cli.web_routers.files', 'delete_managed_file'),
    'delete_profile_endpoint': ('cryozen_cli.web_routers.profiles', 'delete_profile_endpoint'),
    'delete_session_endpoint': ('cryozen_cli.web_routers.sessions', 'delete_session_endpoint'),
    'delete_webhook': ('cryozen_cli.web_routers.ops', 'delete_webhook'),
    'derive_gateway_busy': ('gateway.status', 'derive_gateway_busy'),
    'derive_gateway_drainable': ('gateway.status', 'derive_gateway_drainable'),
    'describe_profile_auto_endpoint': ('cryozen_cli.web_routers.profiles', 'describe_profile_auto_endpoint'),
    'detect_install_method': ('cryozen_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('cryozen_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('cryozen_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('cryozen_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('cryozen_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('cryozen_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('cryozen_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('cryozen_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('cryozen_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('cryozen_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('cryozen_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('cryozen_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('cryozen_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('cryozen_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('cryozen_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('cryozen_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('cryozen_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('cryozen_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('cryozen_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('cryozen_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('cryozen_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('cryozen_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('cryozen_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('cryozen_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('cryozen_cli.config', 'get_config_path'),
    'get_config_raw': ('cryozen_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('cryozen_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('cryozen_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('cryozen_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('cryozen_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('cryozen_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('cryozen_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('cryozen_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('cryozen_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('cryozen_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('cryozen_cli.config', 'get_env_path'),
    'get_env_vars': ('cryozen_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('cryozen_cli.web_routers.status', 'get_health'),
    'get_cryozen_home': ('cryozen_cli.config', 'get_cryozen_home'),
    'get_learning_graph': ('cryozen_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('cryozen_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('cryozen_cli.web_routers.status', 'get_logs'),
    'get_media': ('cryozen_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('cryozen_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('cryozen_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('cryozen_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('cryozen_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('cryozen_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('cryozen_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('cryozen_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('cryozen_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_process_cryozen_home': ('cryozen_cli.config', 'get_process_cryozen_home'),
    'get_profile_setup_command': ('cryozen_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('cryozen_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('cryozen_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('cryozen_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('cryozen_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('cryozen_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('cryozen_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('cryozen_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('cryozen_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('cryozen_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('cryozen_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('cryozen_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('cryozen_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('cryozen_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('cryozen_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('cryozen_cli.web_routers.status', 'get_system_stats'),
    'get_terminal_backends': ('cryozen_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('cryozen_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('cryozen_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('cryozen_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('cryozen_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('cryozen_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('cryozen_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('cryozen_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('cryozen_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('cryozen_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('cryozen_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('cryozen_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('cryozen_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('cryozen_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('cryozen_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('cryozen_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('cryozen_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('cryozen_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('cryozen_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('cryozen_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('cryozen_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('cryozen_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('cryozen_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('cryozen_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('cryozen_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('cryozen_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('cryozen_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('cryozen_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('cryozen_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('cryozen_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('cryozen_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('cryozen_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('cryozen_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('cryozen_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('cryozen_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('cryozen_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('cryozen_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('cryozen_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('cryozen_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('cryozen_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('cryozen_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('cryozen_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('cryozen_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('cryozen_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('cryozen_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('cryozen_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('cryozen_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('cryozen_cli.config', 'load_env'),
    'mcp_oauth_callback': ('cryozen_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('cryozen_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('cryozen_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('cryozen_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('cryozen_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('cryozen_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('cryozen_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('cryozen_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('cryozen_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('cryozen_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('cryozen_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('cryozen_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('cryozen_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('cryozen_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('cryozen_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('cryozen_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    'read_managed_file': ('cryozen_cli.web_routers.files', 'read_managed_file'),
    'read_raw_config': ('cryozen_cli.config', 'read_raw_config'),
    'read_runtime_status': ('gateway.status', 'read_runtime_status'),
    'recommended_update_command_for_method': ('cryozen_cli.config', 'recommended_update_command_for_method'),
    'redact_key': ('cryozen_cli.config', 'redact_key'),
    'remove_credential_pool_entry': ('cryozen_cli.web_routers.ops', 'remove_credential_pool_entry'),
    'remove_env_value': ('cryozen_cli.config', 'remove_env_value'),
    'remove_env_var': ('cryozen_cli.web_routers.config_env', 'remove_env_var'),
    'remove_mcp_server': ('cryozen_cli.web_routers.mcp', 'remove_mcp_server'),
    'rename_profile_endpoint': ('cryozen_cli.web_routers.profiles', 'rename_profile_endpoint'),
    'rename_session_endpoint': ('cryozen_cli.web_routers.sessions', 'rename_session_endpoint'),
    'replace_mcp_servers': ('cryozen_cli.web_routers.mcp', 'replace_mcp_servers'),
    'rescan_dashboard_plugins': ('cryozen_cli.web_routers.dashboard_ui', 'rescan_dashboard_plugins'),
    'reset_memory': ('cryozen_cli.web_routers.ops', 'reset_memory'),
    'resolve_gateway_liveness': ('gateway.status', 'resolve_gateway_liveness'),
    'restart_gateway': ('cryozen_cli.web_routers.actions', 'restart_gateway'),
    'resume_cron_job': ('cryozen_cli.web_routers.cron', 'resume_cron_job'),
    'reveal_env_var': ('cryozen_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('cryozen_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('cryozen_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('cryozen_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('cryozen_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('cryozen_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('cryozen_cli.doctor', 'run_doctor'),
    'run_dump': ('cryozen_cli.dump', 'run_dump'),
    'run_import': ('cryozen_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('cryozen_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('cryozen_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('cryozen_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('cryozen_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('cryozen_cli.config', 'save_config'),
    'save_env_value': ('cryozen_cli.config', 'save_env_value'),
    'save_toolset_env': ('cryozen_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('cryozen_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('cryozen_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('cryozen_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('cryozen_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('cryozen_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('cryozen_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('cryozen_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('cryozen_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('cryozen_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('cryozen_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('cryozen_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('cryozen_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('cryozen_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('cryozen_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('cryozen_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('cryozen_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('cryozen_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('cryozen_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('cryozen_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('cryozen_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('cryozen_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('cryozen_cli.web_routers.oauth', 'start_oauth_login'),
    'start_whatsapp_onboarding': ('cryozen_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('cryozen_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('cryozen_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('cryozen_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('cryozen_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('cryozen_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('cryozen_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('cryozen_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('cryozen_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('cryozen_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('cryozen_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('cryozen_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('cryozen_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('cryozen_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('cryozen_cli.web_routers.cron', 'update_cron_job'),
    'update_cryozen': ('cryozen_cli.web_routers.actions', 'update_cryozen'),
    'update_learning_node': ('cryozen_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('cryozen_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('cryozen_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('cryozen_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('cryozen_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('cryozen_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('cryozen_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('cryozen_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('cryozen_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('cryozen_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('cryozen_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('cryozen_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('cryozen_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('cryozen_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('cryozen_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('cryozen_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('cryozen_cli.config', 'write_platform_config_field'),
}
