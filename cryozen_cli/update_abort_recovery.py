...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``cryozen-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``cryozen-serve.service`` can exist in BOTH the user and system manager (two processes)."""
    rows: list[dict] = []
...
def _run_fresh_recovery_process(
    ...
    """Spawn ``cryozen_cli.update_restart_recovery --stdin`` detached from this process; None when it
    could not run (no systemd-run in gateway mode, OSError, timeout) — the caller fails closed."""
    command = [sys.executable, "-m", "cryozen_cli.update_restart_recovery", "--stdin"]
    env = os.environ.copy()
    env["CRYOZEN_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_CRYOZEN_GATEWAY", "CRYOZEN_GATEWAY", "CRYOZEN_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _recover_gateway_restart_after_abort(
    ...
    part of the identity, not decoration: ``cryozen-serve.service`` can exist in both the user and the system
    ...
    """
    from cryozen_cli.update_cmd import _gateway_recovery_partition
...
def _warn_stale_serve_runtimes(rows) -> None:
    """Name the serve/dashboard processes still on pre-update code: ``cryozen serve`` hosts
    ...
    print("    Ask their owner to relaunch `cryozen serve` / `cryozen dashboard`, or reconnect Desktop for an SSH backend.")
    if sys.platform == "linux" and any(row.get("supervisor") == "systemd" for row in rows):
        print("    For unit-managed backends: `systemctl --user restart cryozen-serve.service`.")
