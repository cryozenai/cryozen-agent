...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``cryozen_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``cryozen_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from cryozen_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (cryozen_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``cryozen acp`` speaks JSON-RPC on stdout; an inherited-fd install child writing there would
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (cryozen_constants is stdlib-only)."""
    # cryozen_constants is stdlib-only, so the canonical layout helpers are safe to use from this
...
#: Cryozen root's ``bin``, next to uv.exe). Keep in lockstep with scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("cryozen", "cryozen-acp")
...
def _default_cryozen_root() -> Path | None:
    """The DEFAULT Cryozen root (not ``get_cryozen_home()``, which under ``cryozen -p <name>`` is
    ...
    try:
        return Path(get_default_cryozen_root())
    except Exception:
        return None
...
def ensure_windows_bin_launchers(
    ...
    r"""Re-stage the Windows ``cryozen`` launchers when they vanish.

    On Windows, ``cryozen`` resolves through launchers derived from the venv console scripts — never
    ...
    The canonical launcher home is the managed binary dir — the default Cryozen root's ``bin``
    (``%LOCALAPPDATA%\\cryozen\\bin``, next to the managed uv) — which lives OUTSIDE the git checkout so no
    git operation can ever touch it. It is a per-machine dir shared by every profile: ``get_cryozen_home()``
    would point inside ``profiles\\<name>`` under ``cryozen -p``, so the anchor here is
    :func:`cryozen_constants.get_default_cryozen_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
        return []
    root = Path(root)
    home = _default_cryozen_root()
    if home is None:
        return []

    targets: list[Path] = []
    # Runs at every cryozen_cli.main process start, so the healthy path must stay a few stat calls.
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print("  ✓ Restored cryozen launcher(s): " + ", ".join(restored), file=sys.stderr)
    return restored
...
def migrate_windows_bin_path(
    ...
    """One-time PATH migration to the ``CRYOZEN_HOME\\bin`` launcher layout (``cryozen update`` tail).
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    if kept != entries:
        try:
            ...
            return False
        with contextlib.suppress(OSError, ValueError):
            print(f"  ✓ cryozen launchers now resolve from {home_bin} "
...
def _quarantine_running_cryozen_exe(
    ...
    """Rename live cryozen*.exe shims aside so the installer can rewrite them.
    ...
    names = set(_load_console_script_names(scripts_dir.parent.parent)) or {
        "cryozen", "cryozen-agent", "cryozen-acp"}
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    (#75584). Warnings land on stderr — this module runs in the early-recovery path and ``cryozen acp``
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = _quarantine_running_cryozen_exe(scripts_dir, failed_out=failed) if scripts_dir else []
    if failed:
    ...
    try:
        ...
        # the shims renamed aside and `cryozen` gone from PATH. Restore only renames back when the
