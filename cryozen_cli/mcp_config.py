"""MCP Server Management CLI — ``cryozen mcp`` subcommand."""
...
from cryozen_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_cryozen_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.cryozen-agent/.env`` into ``os.environ`` and
...
def _probe_single_server(
    ...
    # an OAuth browser flow (e.g. `cryozen mcp login`'s 315s) left that inner bound at the
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return CryozenTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see cryozen_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  cryozen mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  cryozen mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  cryozen mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: cryozen mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_cryozen_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `cryozen mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  cryozen mcp add <name> --url <endpoint>')
        _info('  cryozen mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _probe_failure_next_step(name: str, exc: BaseException) -> str:
    ...
    if _is_auth_error(root) or getattr(getattr(root, "response", None), "status_code", None) in (401, 403):
        return f"The server rejected the sign-in. Run: cryozen mcp login {name}"
    if "missing executable" in _format_connect_error(exc):
        return (f"Install that command, or set mcp_servers.{name}.command in {display_cryozen_home()}/config.yaml "
                "to its full path.")
    return f"Check the server is running and the URL/command in its config, then run: cryozen mcp test {name}"
...
def _reauth_oauth_server(name: str, server_config: dict, *, flow: str | None = None) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `cryozen mcp remove` + `cryozen mcp add` to reconfigure auth.")
        return False
    ...
    try:
        ...
        if selected_flow == "browser":
            ...
            CryozenTokenStorage(name).remove(keep_metadata=True)
    except Exception as exc:
    ...
    # web_server_mcp.py and tui_gateway/mcp_oauth_sessions.py. force_interactive_oauth: `cryozen mcp
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `cryozen mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: cryozen mcp reauth <name>   |   cryozen mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'cryozen mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    "cryozen mcp                                    Open the catalog picker (default)",
    "cryozen mcp catalog                            List curated MCPs",
    "cryozen mcp install <name>                     Install a catalog MCP",
    "cryozen mcp serve                              Run as MCP server",
    "cryozen mcp add <name> --url <endpoint>        Add a custom MCP server",
    "cryozen mcp add <name> --command <cmd>         Add a stdio server",
    "cryozen mcp add <name> --preset <preset>       Add from a known preset",
    "cryozen mcp remove <name>                      Remove a server",
    "cryozen mcp list                               List configured servers",
    "cryozen mcp test <name>                        Test connection",
    "cryozen mcp configure <name>                   Toggle tools",
    "cryozen mcp login <name>                       Re-authenticate OAuth",
    "cryozen mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)


def mcp_command(args):
    """Main dispatcher for ``cryozen mcp`` subcommands."""
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from cryozen_cli import mcp_picker
    ...
    if handler:
        ...
        return handler(args)
    # No subcommand — drop the user into the catalog picker (same UX as `cryozen plugin`).
    from cryozen_cli.mcp_picker import run_picker
    run_picker()
