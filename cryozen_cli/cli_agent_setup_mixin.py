"""Agent construction + session-resume display for ``CryozenCLI``: credential resolution,
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    """Headless clarify answer for ``cryozen chat -q``.
...
def _credential_pool_notice(provider: str) -> tuple:
    ...
    if dead:
        ...
        lines.append(f"The {provider} sign-in was lost ({reason}); run `cryozen auth add {provider}` "
                     "to sign in again.")
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``CryozenCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        try:
            # target_model: the ladder's model-keyed rungs (Zen/Go api_mode, Copilot
            # api_mode) must see the model this CLI will actually send, not config's `default`,
            # or `cryozen -m mimo-v2.5 --provider opencode-go` resolves an api_mode/base_url the
        ...
        if not callable(api_key) and not (isinstance(api_key, str) and api_key):
            if _keyless_custom_base(base_url):
                ...
                print("   Run 'cryozen model' to choose a provider, or "
                      "'cryozen setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: cryozen setup")
            return False
        ...
        # is just the provider slug/display name (`cryozen chat --model <provider-name>`
        ...
        # Still empty (e.g. `cryozen auth add` without `cryozen model`): fall back to the
        ...
        if self.model != _model_at_entry and getattr(self, "_explicit_reasoning_config", None) is None:
            from cryozen_cli.cli_model_switch_mixin import _resolve_cli_reasoning
    ...
    def _explain_unusable_credentials(self, error) -> bool:
        ...
        from cryozen_cli.auth import format_auth_error
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        startup, TTY). Runs the same flow as ``cryozen model`` so onboarding has a single
        source of truth. True when a provider was configured."""
        from cli import _cprint, logger
        _cprint("")
        _cprint("✻ No inference provider is configured yet — let's fix that.")
        _cprint("  You'll pick a provider, add its API key (or sign in) and choose a model.")
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'cryozen model' or 'cryozen setup' any time.")
            return False
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'cryozen model' any time.")
            return False
        except Exception as exc:
            logger.debug("first-run provider setup failed: %s", exc)
            _cprint(f"  ⚠️  Provider setup failed: {exc}")
            _cprint("  Run 'cryozen model' to try again.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'cryozen model' to retry.")
        return False
    ...
    def _load_resumed_history_late(self) -> bool:
        ...
        # stderr so stdout stays machine-readable for `$(cryozen chat -Q --resume ...)`.
        ...
        if not session_meta:
            hint = "Use a session ID from a previous CLI run (cryozen sessions list)."
            if _quiet_mode:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if not self._ensure_runtime_credentials():
            return False
        from cryozen_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
        ...
        try:
            ...
            console = ChatConsole()
            from cryozen_cli.cli_chat_error_copy import agent_init_failure_message
            console.print(f"[bold red]{_escape(agent_init_failure_message(e))}[/]")
            from cryozen_constants import partial_update_hint
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if not session_meta:
            ...
            self._console_print("[dim]Use a session ID from a previous CLI run (cryozen sessions list).[/]")
            return False
        session_meta = self._follow_compression_chain(
            session_meta,
    ...
    def _display_resumed_history(self):
        ...
        role_styles = {
            ...
            "assistant": ("  ◆ Cryozen: ", f"dim bold {_assistant_label_c}", "dim", " " * 12),
            "assistant_last": ("  ◆ Cryozen: ", f"bold {_assistant_label_c}", "", " " * 12),  # full, non-dim
        }
        lines = Text()
        if skipped:
