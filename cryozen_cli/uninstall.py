...
def _strip_cryozen_path_lines(content: str) -> str:
    """Drop the ``# Cryozen Agent`` marker (+ its PATH line) and any cryozen PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Cryozen Agent' in line or '# cryozen-agent' in line:
            skip_next = True
            continue
        if skip_next and ('cryozen' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'cryozen' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Cryozen PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_cryozen_path_lines(content)
...
def remove_wrapper_script():
    """Remove the cryozen wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a cryozen_cli / cryozen-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('cryozen_cli' in content or 'cryozen-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("cryozen", "cryozen-acp", "cryozen-agent"))
...
def remove_node_symlinks(cryozen_home: Path) -> list:
    ...
    node_dir = (cryozen_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from cryozen_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from cryozen_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from cryozen_cli import gateway_windows as gw
...
# CRYOZEN_HOME / CRYOZEN_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\cryozen\git\{cmd,bin,usr\bin}, ...\cryozen\node), PortableGit + Node copies
...
def _cryozen_path_markers(cryozen_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Cryozen-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(cryozen_home).rstrip("\\/")
    subs = ("cryozen-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(cryozen_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Cryozen-owned entries from User-scope PATH in the registry (see ``_cryozen_path_markers``)."""
    markers = tuple(m.lower() for m in _cryozen_path_markers(cryozen_home, include_managed_bin=include_managed_bin))
...
def remove_cryozen_env_vars_windows() -> list[str]:
    """Delete CRYOZEN_HOME and CRYOZEN_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("CRYOZEN_HOME", "CRYOZEN_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(cryozen_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``cryozen_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (cryozen_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``cryozen`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from cryozen_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from cryozen_constants import get_default_cryozen_root
        bin_dir = get_default_cryozen_root() / "bin"
    except Exception as e:
...
def _is_default_cryozen_home(cryozen_home: Path) -> bool:
    """Return True when ``cryozen_home`` points at the default (non-profile) root."""
    try:
        from cryozen_constants import get_default_cryozen_root
        return cryozen_home.resolve() == get_default_cryozen_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``cryozen -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current CRYOZEN_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m cryozen_cli.main` (the `cryozen` wrapper may be half-gone).
    cryozen_invocation = [sys.executable, "-m", "cryozen_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                cryozen_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's CRYOZEN_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``cryozen uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from cryozen_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    cryozen_home = get_cryozen_home()
    summary = gui_install_summary(cryozen_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ✻ Cryozen Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Cryozen Chat GUI installation was found.")
        print(f"  Checked: {cryozen_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Cryozen agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(cryozen_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Cryozen agent at {cryozen_home / 'cryozen-agent'}")
        print(f"  • Your config, sessions, and secrets under {cryozen_home}")
        print()
    ...
    print()
    uninstall_gui(cryozen_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Cryozen agent is still installed. Run 'cryozen' to use the CLI,")
    print("or 'cryozen uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.cryozen-agent/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, cryozen_home=cryozen_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie CRYOZEN_HOMEs and systemd units behind.
    is_default_profile = _is_default_cryozen_home(cryozen_home)
    ...
    print()
    _print_box("│            ✻ Cryozen Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {cryozen_home / 'config.yaml'}")
    print(f"  Secrets: {cryozen_home / '.env'}")
    print(f"  Data:    {cryozen_home / 'cron/'}, {cryozen_home / 'sessions/'}, {cryozen_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, CRYOZEN_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Cryozen data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Cryozen code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, cryozen_home=cryozen_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, cryozen_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Cryozen PATH entries from shell configs / Windows User PATH")
    print("  • Cryozen wrapper scripts and Cryozen-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Cryozen config/data: {cryozen_home}")
    else:
        print(f"  • Cryozen config/data: {cryozen_home}")
        profiles = _discover_named_profiles() if _is_default_cryozen_home(cryozen_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$CRYOZEN_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: cryozen_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; cryozen\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_cryozen_home(cryozen_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(cryozen_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Cryozen-owned PATH entries in User environment"),
        (windows, "Removing CRYOZEN_HOME / CRYOZEN_GIT_BASH_PATH User env vars...",
         remove_cryozen_env_vars_windows, "Removed User env var: {}", "No Cryozen-set User env vars to remove"),
        (True, "Removing cryozen command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows cryozen launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows cryozen launchers found"),
        (True, "Removing Cryozen-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(cryozen_home), "Removed {}", "No Cryozen-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE CRYOZEN_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from cryozen_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(cryozen_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(cryozen_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.cryozen-agent/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(cryozen_home)
    else:
        log_info(f"Keeping configuration and data in {cryozen_home}")
    ...
    print()
    print("Thank you for using Cryozen Agent! ✻")
    print()


_REINSTALL_HINT = {
    True: "  iex (irm https://cryozenai.github.io/cryozen-agent/install.ps1)",
    False: "  curl -fsSL https://cryozenai.github.io/cryozen-agent/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m cryozen_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``cryozen_constants`` + ``cryozen_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m cryozen_cli.uninstall")
    parser.add_argument(
