...
Custom-provider lookup lives in :mod:`cryozen_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`cryozen_cli.runtime_provider_backends` — both re-exported here so
``cryozen_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from cryozen_cli import auth as auth_mod
...
from cryozen_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
    ACTUAL_LOCAL_NOAUTH_PLACEHOLDER, AuthError, DEFAULT_CODEX_BASE_URL, DEFAULT_QWEN_BASE_URL, DEFAULT_XAI_OAUTH_BASE_URL,
    PROVIDER_REGISTRY, format_auth_error, resolve_provider,
...
from cryozen_cli import config as _config_mod
from cryozen_cli import models as _models  # attribute access keeps ``cryozen_cli.models.<name>`` patches effective
from cryozen_constants import OPENROUTER_BASE_URL
from cryozen_cli.providers import determine_api_mode, get_provider, is_actual_route, is_official_openai_host
from utils import base_url_host_matches, base_url_hostname, base_url_path


# Late-bound delegates, deliberately NOT module-level from-imports: this module is often imported
# lazily, so its first import can happen while a test has ``cryozen_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in cryozen_cli/providers.py
...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    - Direct api.anthropic.com endpoints must use the native Messages API (``/v1/messages``); the
    OpenAI-compat ``/chat/completions`` shim on the same host lacks prompt caching and thinking controls.
    - Third-party Anthropic-compatible gateways (MiniMax, Zhipu GLM, LiteLLM proxies, etc.)
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider == "xai":
        ...
        # the host_mandated_api_mode clause in cryozen_cli/providers.py so the runtime resolver stays in
...
_NO_ANTHROPIC_CREDENTIALS_MSG = ("No Anthropic API key found. Set ANTHROPIC_API_KEY or run "
    ...
    if not token:
        ...
        if model and resolve_anthropic_token():
            raise AuthError(f"Anthropic credentials are rate-limited for {model}; "
                            "other Claude models remain available (see `cryozen auth list`).")
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    """Provider request from explicit arg, then config, then ``CRYOZEN_INFERENCE_PROVIDER``, else
    ...
    return get_secret_str("CRYOZEN_INFERENCE_PROVIDER", "").strip().lower() or "auto"


# ── extracted collaborators (re-exported; see module docstring) ────────────────────────────

from cryozen_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from cryozen_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _pool_entry_mode_and_url(provider, entry, model_cfg, effective_model, base_url) -> tuple:
    ...
    if provider in _POOL_ENTRY_SIMPLE_MODES:
        ...
        if provider == "openai-codex":
            ...
            # CRYOZEN_CODEX_BASE_URL override must apply consistently to every
            # credential source, including pooled OAuth credentials.
            override_url = get_secret_str("CRYOZEN_CODEX_BASE_URL", "").strip().rstrip("/")
            if override_url:
...
def _openrouter_should_use_pool(requested_provider, model_cfg, explicit_api_key, explicit_base_url) -> bool:
    ...
    # OpenRouter host — `cryozen setup` persists https://openrouter.ai/api/v1 for plain installs,
...
def _exchange_copilot_pool_entry(entry: Any, pool_api_key: str) -> str:
    ...
    from cryozen_cli.copilot_auth import get_copilot_api_token, validate_copilot_token
...
_EXPLICIT_RESOLVERS: Dict[str, Callable[..., Dict[str, Any]]] = {
    "anthropic": _explicit_anthropic, "openai-codex": _explicit_codex,
...
_OAUTH_RUNTIME_PROVIDERS: Dict[str, _OAuthRuntimeSpec] = {
    "openai-codex": _OAuthRuntimeSpec(lambda: resolve_codex_runtime_credentials(), "codex_responses", "cryozen-auth-store",
                                      "last_refresh", "Auto-detected Codex provider but credentials failed"),
    "xai-oauth": _OAuthRuntimeSpec(lambda: resolve_xai_oauth_runtime_credentials(), "codex_responses", "cryozen-auth-store",
...
def _anthropic_env_runtime(requested_provider: str, model_cfg: Dict[str, Any], target_model: str | None = None) -> Dict[str, Any]:
    ...
    # Microsoft Foundry endpoints take their own Azure key, never ANTHROPIC_API_KEY's resolver path.
...
def _resolve_vertex_runtime(requested_provider: str) -> Dict[str, Any]:
    ...
    if not token or not base_url:
        raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a "
                        "service-account JSON via GOOGLE_APPLICATION_CREDENTIALS (or VERTEX_CREDENTIALS_PATH) in ~/.cryozen-agent/.env, "
                        "or run 'gcloud auth application-default login' for ADC. Set the GCP project/region under vertex: in "
                        "config.yaml if they aren't embedded in the credentials. Run `cryozen setup` to install Vertex support.")
...
def resolve_runtime_provider(*, requested: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    rung returns or raises, else falls to the next):
      ...
      7. OAuth specs (codex/xai/qwen; "auto" swallows AuthError, logs, and stamps it on a
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('cryozen_cli.providers', 'custom_provider_aliases'),
...
def resolve_runtime_with_fallback(config: Optional[Dict[str, Any]], *, requested: Optional[str] = None,
    ...
    """
    from cryozen_cli.auth import AuthError, primary_failure_wording
    try:
        ...
        from cryozen_cli.fallback_config import effective_runtime_provider, get_fallback_chain, resolve_entry_api_key
