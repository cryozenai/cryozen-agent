...
disk cache). Split out of ``cryozen_cli.models``; helpers still defined there are looked up on
``cryozen_cli.models`` at call time so ``patch("cryozen_cli.models.<name>")`` mocks keep intercepting.
"""
...
from cryozen_cli.urllib_security import url_origin
...
def probe_ollama_local_models(
    ...
    from cryozen_cli.models import _CRYOZEN_USER_AGENT, _get_ollama_base_url, _urlopen_model_catalog_request
    ...
    if failed_at is not None:
        ...
        _OLLAMA_LOCAL_PROBE_FAILURE_CACHE.pop(failure_key, None)
    try:
        request_headers = {"User-Agent": _CRYOZEN_USER_AGENT, **(headers or {})}
...
_NEVER_OLLAMA_PROVIDERS = frozenset({"openrouter", "anthropic", "openai", "openai-codex", "gemini", "ollama-cloud"})
...
def _ollama_local_catalog(force_refresh: bool) -> list[str]:
    ...
    Cloud)."""
    from cryozen_cli.models import _get_provider_config_dict, fetch_api_models
    if force_refresh:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    from cryozen_cli.models import _CRYOZEN_USER_AGENT
    from agent.command_token_source import materialize_probe_api_key
    token = materialize_probe_api_key(api_key)
    return {"User-Agent": _CRYOZEN_USER_AGENT, **({"Authorization": f"Bearer {token}"} if token else {})}
...
def _lmstudio_fetch_raw_models(
    ...
    from cryozen_cli.models import _urlopen_model_catalog_request
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_cryozen_home() / "ollama_cloud_models_cache.json"
