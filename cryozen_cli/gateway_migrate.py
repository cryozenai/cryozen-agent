...
what the multiplexer would do at startup. ``cryozen update`` calls :func:`maybe_auto_migrate_after_update`.
"""
...
MIGRATE_COMMAND = "cryozen gateway migrate --multiplex"
...
class ProfileGateway:
    ...
    runtime_home: Optional[Path] = None  # CRYOZEN_HOME the installed unit pins, when it differs from ``home``
...
class MigrationPlan:
    ...
    def eligible_for_migration(self) -> bool:
        ...
        This is the AUTO-migration (``cryozen update``) bar; the explicit command also proceeds with
...
def _home_env(home: Path) -> Iterator[None]:
    """Run service-manager helpers as if ``home`` were the active CRYOZEN_HOME. Both the contextvar
    override (``get_cryozen_home``) and ``os.environ`` (``gateway.status`` identity files, unit
    ...
    previous = os.environ.get("CRYOZEN_HOME")
    token = set_cryozen_home_override(str(home))
    os.environ["CRYOZEN_HOME"] = str(home)
    cryozen_constants._default_cryozen_root_memo = None
    try:
        yield
    finally:
        ...
        if previous is None:
            ...
            os.environ["CRYOZEN_HOME"] = previous
        cryozen_constants._default_cryozen_root_memo = None
...
def _own_gateway_pid(home: Path, owner) -> Optional[int]:
    ...
    PID, so the plan called the host half-migrated and each `cryozen update` SIGTERMed the only
    ...
    if pid is None or owner is None or pid != owner.pid:
        return pid
    with contextlib.suppress(Exception):
        from gateway.status import _same_cryozen_home
        return pid if _same_cryozen_home(Path(owner.home), Path(home)) else None
    return None


def _gateway_identity(home: Path, pid: Optional[int], services: list[tuple[str, bool]]) -> tuple[Optional[int], Path]:
    from cryozen_cli.gateway_migrate_guards import gateway_identity
...
def _installed_services(home: Path) -> list[tuple[str, bool]]:
    ...
    from cryozen_cli import gateway as gw
...
def _windows_task_installed() -> bool:
    """Is a per-profile Windows gateway installed for the ACTIVE ``CRYOZEN_HOME``?

    ``get_task_name()`` is home-suffixed, so this answers per profile exactly the way the systemd
    unit path does. Either half counts: ``cryozen gateway install`` falls back to a Startup-folder
    ...
    """
    from cryozen_cli import gateway_windows as gww
    with contextlib.suppress(Exception):
...
def _stop_gateway_process(home: Path) -> None:
    from cryozen_cli.profiles import _stop_gateway_process
...
def _write_multiplex_flag(default_home: Path, value: bool) -> None:
    ...
    (same read-guard + nested-set + atomic write ``cryozen config set`` uses; no raw YAML edits)."""
    from cryozen_cli.config import _set_nested, _write_user_config, require_readable_config_before_write
...
def _credential_key_names(platform_value: str) -> str:
    ...
    from cryozen_cli.profile_channels import credential_env_keys
...
def duplicate_credential_lines(configs: list[tuple[str, object]]) -> list[str]:
    ...
    source for the migrate preflight, ``cryozen doctor`` and ``cryozen gateway status``, so all three
...
def build_migration_plan() -> MigrationPlan:
    ...
    for check in _PREFLIGHT_CHECKS:
        check(plan, configs)
    from cryozen_cli.gateway_migrate_guards import auto_migration_blockers
    ...
    plan.notices.extend(f"Not migrated automatically by `cryozen update`: {b}" for b in auto_migration_blockers(plan))
    plan.notices.append(
...
def format_update_warning(plan: MigrationPlan, auto_blockers: list[str]) -> list[str]:
    return [
        ...
        *[f"    • {b}" for b in (*plan.blockers, *auto_blockers)],
        ...
        "  (`cryozen update` will migrate automatically once nothing blocks it.)",
    ]
...
def _preflight_apply(plan: MigrationPlan, target: Optional[tuple[str, bool]], run_as_user: Optional[str]) -> Optional[str]:
    ...
    from cryozen_cli import gateway as gw
    from cryozen_cli.config import require_readable_config_before_write
    try:
...
def apply_migration(plan: MigrationPlan, *, served_wait: float = _SERVED_WAIT_SECONDS) -> bool:
    ...
    **Resume beats preflight.** The manifest is read BEFORE the blocker gate: a manifest means an
    ...
    _print(["", f"⚠ Migration applied, but the default gateway has not confirmed serving: {', '.join(missing)}",
            "  Check `cryozen gateway status` and the gateway log; the flag and manifest are in place.",
...
def rollback_migration(default_home: Optional[Path] = None) -> bool:
    ...
    the next ``cryozen gateway migrate --multiplex`` resumes.
    """
    ...
    if live is None:
        print(f"  ✗ default: no gateway confirmed serving this host within {_COMPENSATOR_WAIT_SECONDS:.0f}s "
              f"(check `cryozen gateway status` and the gateway log)")
        print(incomplete)
        return False
    _manifest_path(default_home).unlink(missing_ok=True)
...
def _host_supports_migration() -> Optional[str]:
    ...
    (``cryozen_cli/container_boot.py::reconcile_profile_gateways``), so the convergence belongs to
...
def cmd_migrate(args) -> None:
    """``cryozen gateway migrate [--multiplex] [--dry-run] [--yes]``."""
...
def maybe_auto_migrate_after_update() -> None:
    """``cryozen update`` hook: with >= 2 profiles, per-profile gateways present and multiplex off,
    ...
    service-domain / UNIX-user / CRYOZEN_HOME boundary blocks this path only (the explicit command decides)."""
    from cryozen_cli.gateway_migrate_guards import auto_migration_blockers, auto_migration_opted_out
