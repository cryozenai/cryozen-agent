...
Anthropic/Copilot status probes.
"""
...
def _anthropic_api_key_status() -> Dict[str, Any]:
    """Status for the "Anthropic API Key" card: the registry-ordered env vars (process env —
    where Bitwarden-sourced secrets land — then .env)."""
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    store); the CLI may hold its session in an OS keychain Cryozen can't read, so the unverified
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    (``CRYOZEN_COPILOT_ACP_COMMAND`` / ``COPILOT_CLI_PATH``); others get ``default`` untouched."""
    try:
...
# providers appear automatically. Also carries the Anthropic credential-status card the
# Accounts tab needs.
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    {"id": "openai-codex", "name": "ChatGPT or Codex Subscription", "flow": "device_code",
     "cli_command": "cryozen auth add openai-codex", "docs_url": "https://platform.openai.com/docs",
     "status_fn": None},
    {"id": "qwen-oauth", "name": "Qwen (via Qwen CLI)", "flow": "external",
     "cli_command": "cryozen auth add qwen-oauth", "docs_url": "https://github.com/QwenLM/qwen-code",
     "status_fn": None},
    ...
    {"id": "minimax-oauth", "name": "MiniMax (OAuth)", "flow": "device_code",
     "cli_command": "cryozen auth add minimax-oauth", "docs_url": "https://www.minimax.io", "status_fn": None},
    ...
    {"id": "xai-oauth", "name": "xAI Grok OAuth (SuperGrok / Premium+)", "flow": "device_code",
     "cli_command": "cryozen auth add xai-oauth",
     "docs_url": "https://cryozenai.github.io/cryozen-agent/docs/guides/xai-grok-oauth", "status_fn": None},
    ...
    # Anthropic takes an API key only, added from the terminal.
    {"id": "anthropic", "name": "Anthropic API Key", "flow": "external",
     "cli_command": "cryozen auth add anthropic --type api-key",
     "docs_url": "https://docs.claude.com/en/api/getting-started", "status_fn": _anthropic_api_key_status},
)
...
def _minimax_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """MiniMax poller: PKCE-style ``code_verifier`` + ``user_code`` instead of a plain
    ``device_code``. Builds the same auth_state as the CLI's ``_minimax_oauth_login`` and persists
    via ``_minimax_save_auth_state`` so the system ends up as after ``cryozen auth add minimax-oauth``.
    Region is fixed to "global" here; cn-region operators use the CLI's ``--region cn``."""
    from cryozen_cli.web_server_profiles import _profile_scope
...
def _xai_device_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        ...
        # Mirror `cryozen auth add xai-oauth`: first credential may become active; never overwrite.
