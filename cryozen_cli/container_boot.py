...
01-cryozen-setup (the stage2 hook) has chowned the volume and seeded $CRYOZEN_HOME, but
...
def _named_profile_dirs(cryozen_home: Path) -> list[tuple[str, Path]]:
    """Every real named profile under ``$CRYOZEN_HOME/profiles`` (``SOUL.md`` is the marker)."""
    profiles_root = cryozen_home / "profiles"
...
def reconcile_profile_gateways(
    *, cryozen_home: Path, scandir: Path, dry_run: bool = False,
    ...
    the top of ``$CRYOZEN_HOME``): ``cryozen_cli.gateway`` maps an empty profile suffix to it,
    so it is what ``cryozen gateway start`` (no ``-p``) targets.

    Without it, bare ``cryozen gateway start`` inside the container would land on ``s6-svc -u
    /run/service/gateway-default`` → uncaught ``CalledProcessError`` → traceback to the user (PR #30136
    review).
    """
    actions: list[ReconcileAction] = []
    # ONE gateway per container: named slots are registered (so `cryozen -p X gateway start` has a
    ...
    # as a topology switch (cryozen_cli/gateway_multiplex_mode.py), so there is nothing to read.
    named = [(name, entry, _read_desired_state(entry)) for name, entry in _named_profile_dirs(cryozen_home)]
    ...
    legacy_default_state = _maybe_migrate_legacy_gateway_run_state(
        cryozen_home, container_argv=container_argv, dry_run=dry_run)
    default_prior_state = legacy_default_state or _read_desired_state(cryozen_home)
    # The root slot INHERITS every named slot's autostart intent, because it is the one process
    # that serves them. Without this an image only ever driven as `cryozen -p coder gateway start`
    ...
    if not dry_run:
        _cleanup_stale_runtime_files(cryozen_home)
        _register_service(scandir, "default", start=default_should_start)
    actions.append(_slot_action("default", cryozen_home, default_prior_state, default_should_start,
    ...
    if not dry_run:
        _write_reconcile_log(cryozen_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    cryozen_home: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
    ...
    state_file = cryozen_home / "gateway_state.json"
    if state_file.exists():
        return None
    if os.environ.get("CRYOZEN_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the cryozen args.
    ...
    if args and args[0].endswith("entrypoint-dispatch.sh"):  # non-PID-1 dispatch shim
        args = args[1:]
    if args and Path(args[0]).name == "cryozen":  # the wrapper re-execs `cryozen <subcommand>`
        args = args[1:]
    return args
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        if not start:  # `cryozen -p <profile> gateway start` brings it up later (s6-svc -u)
            (tmp_dir / "down").touch()
        # Pre-create supervise/ with cryozen ownership BEFORE publishing so s6-supervise inherits
        # it and runtime s6-svc calls as the cryozen user won't EACCES.
...
def _write_reconcile_log(cryozen_home: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $CRYOZEN_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted CRYOZEN_HOME both
    ...
    actions = reconcile_profile_gateways(cryozen_home=cryozen_home, scandir=scandir)
