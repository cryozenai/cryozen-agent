...
from cryozen_cli.model_setup_flows_common import _say
...
def _whatsapp_choose_mode(get_env_value, save_env_value):
    """Step 1 of ``cryozen whatsapp``: ``"bot"`` / ``"self-chat"``, or None when cancelled."""
    current_mode = get_env_value("WHATSAPP_MODE") or ""
    if current_mode:
        mode_label = "separate bot number" if current_mode == "bot" else "personal number (self-chat)"
        print(f"\n✓ Mode: {mode_label}")
        return current_mode
    _say("", "How will you use WhatsApp with Cryozen?", "",
...
def _whatsapp_allowed_users(wa_mode: str, get_env_value, save_env_value) -> None:
    """Step 3 of ``cryozen whatsapp``: show / set WHATSAPP_ALLOWED_USERS."""
...
def _whatsapp_install_bridge(bridge_dir) -> bool:
    """Step 4 of ``cryozen whatsapp``: ``npm install`` the bridge when needed. False = stop."""
    ...
    try:
        result = subprocess.run(
            ...
            encoding="utf-8", errors="replace", env=with_cryozen_node_path())
...
def cmd_whatsapp(args):
    ...
    from cryozen_cli.main import _require_tty, get_cryozen_home
    ...
    # every `cryozen gateway` would pay a 30s bridge timeout + indefinite retries. Set only after
    ...
    session_dir = get_cryozen_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if _yes_no("\n  Re-pair? This will clear the existing session. [y/N] "):
            ...
            _say("\n✓ WhatsApp is configured and paired!", "  Start the gateway with: cryozen gateway")
            return
    ...
    with contextlib.suppress(KeyboardInterrupt):
        subprocess.run(
            ...
            cwd=str(bridge_dir), env=with_cryozen_node_path())

    print()
    if not (session_dir / "creds.json").exists():
        print("⚠ Pairing may not have completed. Run 'cryozen whatsapp' to try again.")
        return
    # Only enable WhatsApp now that pairing actually succeeded (see above).
    save_env_value("WHATSAPP_ENABLED", "true")
    _say("✓ WhatsApp paired successfully!", "")
    if wa_mode == "bot":
        _say("  Next steps:", "    1. Start the gateway:  cryozen gateway",
             "    2. Send a message to the bot's WhatsApp number",
             "    3. The agent will reply automatically", "",
             "  Tip: Agent responses are prefixed with '✻ Cryozen Agent'")
    else:
        _say("  Next steps:", "    1. Start the gateway:  cryozen gateway",
    ...
    _say("", "  Or install as a service: cryozen gateway install")


def cmd_whatsapp_cloud(args):
    """Set up WhatsApp Business Cloud API (official Meta integration) — complementary to the
    ``cryozen whatsapp`` Baileys bridge wizard. See ``cryozen_cli/setup_whatsapp_cloud.py``."""
    from cryozen_cli.main import _require_tty
    _require_tty("whatsapp-cloud")
    from cryozen_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
    return run_whatsapp_cloud_setup()


def cmd_slack(args):
    """``cryozen slack <subcommand>``; ``manifest`` prints or writes a Slack app manifest with
    every gateway command registered as a first-class slash."""
    sub = getattr(args, "slack_command", None)
    if sub in {None, ""}:
        _err("usage: cryozen slack <subcommand>\n"
             "\n"
             "subcommands:\n"
             "  manifest   Generate a Slack app manifest with every gateway\n"
             "             command registered as a native slash\n"
             "\n"
             "Run `cryozen slack manifest -h` for details.")
        return 1

    if sub == "manifest":
        from cryozen_cli.slack_cli import slack_manifest_command
