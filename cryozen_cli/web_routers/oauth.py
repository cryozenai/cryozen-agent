...
from cryozen_cli.web_server_oauth import (
    _external_process_cli_command, _oauth_profile_name, _oauth_sessions, _oauth_sessions_lock, _truncate_token,
)
from cryozen_cli.web_models import OAuthSubmitBody
from cryozen_cli.web_routers._common import scoped_to_thread
...
_OAUTH_PROVIDER_CATALOG = LateState("_OAUTH_PROVIDER_CATALOG", "cryozen_cli.web_server_oauth")
_minimax_poller = late("_minimax_poller", "cryozen_cli.web_server_oauth")
_xai_device_poller = late("_xai_device_poller", "cryozen_cli.web_server_oauth")
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Cryozen "
...
def _codex_client(httpx) -> Any:
    ...
    """
    from cryozen_cli.auth_codex import _cap_codex_response_body
...
def _codex_post(httpx, url: str, **kwargs: Any) -> Any:
    ...
    """
    from cryozen_cli.auth_codex import _is_transient_transport_error
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from cryozen_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from cryozen_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from cryozen_cli.auth import _save_codex_tokens
...
# xai: source_label is a human-readable origin (auth-store path /
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        if status_fn is not None:
            return status_fn()
        from cryozen_cli import auth as hauth
...
async def _start_xai_device_code(profile: Optional[str]) -> Dict[str, Any]:
    from cryozen_cli.auth import _xai_oauth_request_device_code
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # "anthropic" is flow == "external" (no in-dashboard login) but Cryozen still OWNS its
    # credential-pool entry (written by `cryozen auth add anthropic`), so it is excluded from
    # the "external providers can't be auto-disconnected" rule.
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    in ``cryozen model`` order, so plugin-added OAuth/external providers appear automatically."""
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "cli_command": f"cryozen auth add {d.slug}", "docs_url": d.signup_url or "", "status_fn": None,
            })
    except Exception:
        pass
    return rows
...
# "approved" the poller has already saved creds). Anthropic has no OAuth login
# (API keys only); that card is "external".
