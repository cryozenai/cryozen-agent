...
from cryozen_cli.web_routers._common import http_failure, scoped_to_thread
...
from cryozen_cli.config import DEFAULT_CONFIG, OPTIONAL_ENV_VARS, read_raw_config, custom_endpoint_key_env, coerce_provider_id, find_provider_entry, get_compatible_custom_providers, redact_key, _deep_merge
from cryozen_cli.config_providers import _canonical_api_mode, _custom_provider_entry_to_provider_config
from cryozen_cli.web_models import ConfigUpdate, EnvVarUpdate, EnvVarDelete, EnvVarReveal, CustomEndpointUpdate
...
_channel_managed_env_keys = late("_channel_managed_env_keys", "cryozen_cli.web_server_messaging")
...
async def get_egress_status(profile: Optional[str] = None):
    ...
    from cryozen_cli.proxy_cli import format_status_text
...
async def update_config(
    ...
    def _run():
        ...
        # different CRYOZEN_HOME than this process's gateway sessions.
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (the ``cryozen model`` universe), so the desktop Keys tab renders a card even
    ...
    """
    try:
        from cryozen_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # by the SAME provider identity the CLI `cryozen model` picker uses.
...
def _write_custom_endpoint(cfg: Dict[str, Any], body: CustomEndpointUpdate) -> Tuple[str, Dict[str, Any]]:
    ...
    # ``cryozen model`` does (``api_mode``; the runtime also reads the v12
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    rather than the process-level CRYOZEN_HOME (mirrors ``/api/config``).
    """
