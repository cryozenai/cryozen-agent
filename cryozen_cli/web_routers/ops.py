...
from cryozen_cli.config import redact_key
from cryozen_cli.web_deps import late
from cryozen_cli.web_server_files import _path_is_under
from cryozen_cli.web_server_gateway import _restart_gateway_after
from cryozen_cli.web_server_memory import _normalize_memory_provider_name, _require_memory_provider_ready
...
from cryozen_cli.web_routers._common import (
    config_scoped_to_thread, config_write_scope, destructive_profile, http_failure,
    spawn_profile_action,
)
from cryozen_cli.web_routers.files import stream_upload_to_path

_log = logging.getLogger("cryozen_cli.web_server")
router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_discover_memory_provider_statuses = late("_discover_memory_provider_statuses", "cryozen_cli.web_server_memory")
...
save_config = late("save_config", "cryozen_cli.config")


def _spawn_action(argv: List[str], name: str, *, log_msg: str, prefix: str,
                  profile: Optional[str] = None) -> dict:
    """Spawn a ``cryozen -p <profile> <argv>`` action; spawn failure -> 500.
...
# --- Webhooks: same JSON store as the CLI (cryozen_cli.webhook); the adapter
...
async def list_webhooks(profile: Optional[str] = None):
    def _run():
        import cryozen_cli.webhook as wh
...
# --- Gateway lifecycle: spawn the real `cryozen gateway <verb>` so behaviour
...
async def start_gateway(profile: Optional[str] = None):
    from cryozen_cli.web_server_gateway import multiplexed_profile_refusal
    # The spawned `cryozen -p X gateway start` would refuse with exit 78 into an action log nobody reads;
    ...
    with http_failure("Failed to spawn gateway start", 500, "Failed to start gateway"):
        proc = _spawn_cryozen_action(_gateway_subcommand(profile, "start"), "gateway-start")
...
async def stop_gateway(profile: Optional[str] = None):
    ...
    with http_failure("Failed to spawn gateway stop", 500, "Failed to stop gateway"):
        proc = _spawn_cryozen_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    ...
    def _run():
        try:
            ...
            pool.add_entry(PooledCredential(
                ...
                # every added account into the latest login: a second ``cryozen auth add openai-codex``
            ...
            # (mirrors `cryozen auth add`).
            if not provider.startswith(CUSTOM_POOL_PREFIX):
                try:
                    from cryozen_cli.auth import _load_auth_store, unsuppress_credential_source
...
async def remove_credential_pool_entry(provider: str, index: int, profile: Optional[str] = None):
    ...
    the same RemovalStep registry as ``cryozen auth remove``: each source cleans
...
async def get_memory_status(profile: Optional[str] = None):
    def _run():  # load_config(), stats and discovery are disk reads — off-loop
        ...
        mem_dir = get_cryozen_home() / "memories"
...
def _dashboard_backup_dir(profile: Optional[str] = None) -> Path:
    ...
    with _config_profile_scope(profile):
        return get_cryozen_home() / "backups"
...
async def run_backup(body: BackupRequest, profile: Optional[str] = None):
    ...
    if not output:
        ...
        archive = _dashboard_backup_dir(profile) / f"cryozen-backup-{stamp}-{secrets.token_hex(4)}.zip"
        try:
...
async def list_hooks(profile: Optional[str] = None):
    ...
    def _run():
        ...
        with contextlib.suppress(Exception):
            from cryozen_cli.plugins import VALID_HOOKS
...
async def create_hook(body: HookCreate, profile: Optional[str] = None):
    ...
    valid_hooks = None
    with contextlib.suppress(Exception):
        from cryozen_cli.plugins import VALID_HOOKS as valid_hooks
...
async def list_checkpoints(profile: Optional[str] = None):
    ...
    def _run():
        cp_dir = get_cryozen_home() / "checkpoints"
        sessions = []
        total_bytes = 0
        if cp_dir.is_dir():
