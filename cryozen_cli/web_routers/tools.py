...
from cryozen_cli.web_routers._common import (
    _CONFIG_MUTATION_LOCK, _profile_cli_args, _profile_scope, _spawn_cryozen_action,
...
def _terminal_backend_rows() -> List[Dict[str, str]]:
    ...
    from cryozen_cli.web_server_profiles import _TERMINAL_BACKENDS
...
def _probe_docker_backend(_cfg) -> tuple:
    ...
    ``find_docker()`` honors ``CRYOZEN_DOCKER_BINARY``, then ``docker`` / ``podman``
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    an in-tree image row's ``imagegen_backend`` names its catalog.
    """
    if ts_key == "image_gen":
...
def _toolset_model_catalog(ts_key: str, plugin_name: str, config: dict):
    ...
    ``imagegen_backend`` (``fal``), that backend's catalog."""
    from cryozen_cli.tools_config import IMAGEGEN_BACKENDS, _plugin_image_gen_catalog, _plugin_video_gen_catalog
...
def _category_providers(ts_key: str, config: dict) -> list:
    """Visible provider rows for a toolset's category."""
    from cryozen_cli.tools_config import TOOL_CATEGORIES, _visible_providers
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from cryozen_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # install `cryozen tools` runs — otherwise the toggle "saves" but the tool
            ...
            if cat:
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        # automatically ready (never-run post_setup installs).
                        "status": provider_readiness_status(prov, config, is_active=is_active)}
...
async def select_toolset_provider(
    ...
    ``cryozen tools``, so both write identical keys).
    ...
    """
    from cryozen_cli.tools_config import apply_provider_selection, web_provider_capabilities
...
async def run_toolset_post_setup(
    ...
    """Spawn ``cryozen tools post-setup <key>`` (long-running installs) as a
    background action tailed via ``GET /api/actions/tools-post-setup/status``;
    ``profile`` is threaded so hooks see the drawer's CRYOZEN_HOME."""
    from cryozen_cli.tools_config import valid_post_setup_keys
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``cryozen computer-use permissions grant`` (macOS-only: launches
...
_PLUGIN_COMPAT_LAZY = {
    'LateState': ('cryozen_cli.web_deps', 'LateState'),
}
