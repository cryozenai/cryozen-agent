...
from cryozen_cli.web_server_sessions import _maybe_auto_archive_for_profile, _session_latest_descendant
from cryozen_cli.web_models import (
    BulkDeleteSessions, SessionImport, SessionOwnerBackfill, SessionPrune, SessionRename)
from cryozen_cli.web_routers._common import log as _log, destructive_profile, http_failure
from cryozen_state import is_malformed_db_error
from cryozen_state_errors import is_transient_sqlite_error

list_router = APIRouter()
search_router = APIRouter()
manage_router = APIRouter()

_cron_default_profile = late("_cron_default_profile", "cryozen_cli.web_server_cron")
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `cryozen sessions prune`)."""
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_cryozen_home()
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics (mirrors `cryozen sessions stats`)."""
    def _stats(db):
        out = {
...
async def get_session_timeline(
    ...
    """
    from cryozen_state_timeline import get_session_timeline as read_timeline
...
async def get_session_messages_around(
    ...
    from cryozen_state_timeline import get_session_messages_around as read_around
