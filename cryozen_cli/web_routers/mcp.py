...
from cryozen_cli.web_server_mcp import _mcp_oauth_flows, _mcp_server_summary, _normalize_mcp_server_create
from cryozen_cli.web_models import MCPCatalogInstall, MCPEnabledToggle, MCPServerCreate, MCPServersReplace
from cryozen_cli.web_routers._common import (
    _profile_cli_args, _profile_scope, _spawn_cryozen_action, config_write_scope, http_failure,
...
_run_dashboard_mcp_oauth = late("_run_dashboard_mcp_oauth", "cryozen_cli.web_server_mcp")
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from cryozen_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from cryozen_cli.mcp_config import _get_mcp_servers, _save_bearer_auth_token, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    from cryozen_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from cryozen_cli.mcp_config import _get_mcp_servers, _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from cryozen_cli.mcp_config import _get_mcp_servers, _oauth_tokens_present, _probe_single_server
    ...
    try:  # probe blocks on a dedicated MCP event loop — keep it off the FastAPI loop
        ...
        from cryozen_cli.mcp_config import redact_mcp_probe_text
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    def _home() -> str:
        return str(get_cryozen_home().expanduser().resolve(strict=False))
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(f.server_name == name and f.cryozen_home == flow_home for f in live):
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Cryozen and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Cryozen for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Cryozen.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None, detect_apps: bool = False):
    """Browse the curated MCP catalog (optional-mcps/ manifests), each
    ...
    if detect_apps:
        ...
        try:
            from cryozen_cli.mcp_app_detection import discover_catalog_apps, validate_applications
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    from cryozen_cli import mcp_catalog
    from cryozen_cli.config import validate_env_var_name_for_write
    ...
    if entry.install is not None:
        ...
        try:
            _spawn_cryozen_action(_profile_cli_args(effective_profile) + ["mcp", "install", name], action)
        except HTTPException:
            raise
        except Exception as exc:
