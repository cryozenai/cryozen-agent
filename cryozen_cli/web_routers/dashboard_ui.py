...
from cryozen_cli.web_routers._common import config_scoped_to_thread
from cryozen_cli.web_server_dashboard import (
...
async def get_dashboard_themes(profile: Optional[str] = None):
    ...
    from `~/.cryozen-agent/dashboard-themes/*.yaml` ship their normalised `definition`."""
    def _run():
        config = load_config()
...
async def get_plugins_catalog(request: Request):
    ...
    def _run():
        from cryozen_cli.plugins_cmd import _discover_all_plugins, _get_disabled_set, _get_enabled_set
        from cryozen_cli.plugins_cmd_catalog import installed_catalog_state
        from cryozen_cli.web_server_dashboard import _plugin_runtime_status
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from cryozen_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    from cryozen_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    from cryozen_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    from cryozen_cli.plugins_cmd import dashboard_remove_user_plugin
...
async def post_agent_plugin_activate(request: Request):
    """``cryozen plugins install`` / ``enable`` in another process asks this backend to load the plugin
    for ``home`` and hand its MCP servers and skills to that profile's open chats
    (``cryozen_cli.plugins_activation.load_and_go_live``). ``home`` must be a profile this host serves."""
    _require_token(request)
    try:
        body = await request.json()
    except Exception:
    ...
    from cryozen_constants import get_cryozen_home, cryozen_home_key, profile_name_for_home
    home = Path(str((body or {}).get("home") or "")).expanduser()
    profile = profile_name_for_home(home) if str(home) not in ("", ".") else None
    if profile is None:
        raise HTTPException(status_code=400, detail="Not a Cryozen profile home.")
    from cryozen_cli.plugins_activation import load_and_go_live

    def _run():
        with _config_profile_scope(None if profile == "default" else profile):
            if cryozen_home_key(get_cryozen_home()) != cryozen_home_key(home):
...
async def put_plugin_providers(request: Request, body: _PluginProvidersPutBody,
    ...
    _require_token(request)
    from cryozen_cli.plugins_cmd import _save_context_engine, _save_memory_provider
