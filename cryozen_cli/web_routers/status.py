"""Status dashboard routes: health, /api/status, system stats, curator, learning graph and
diagnostics actions.

Extracted from ``cryozen_cli.web_server``; app state and helpers are late-bound through
:mod:`cryozen_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from cryozen_cli.web_deps import LateState, late
from cryozen_cli.web_server_gateway import _display_system_platform
...
from cryozen_cli.config import get_config_path, get_env_path
from cryozen_constants import get_process_cryozen_home, profile_name_for_home
from cryozen_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from cryozen_cli.web_routers._common import config_scoped_to_thread, destructive_profile, scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "cryozen_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "cryozen_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "cryozen_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "cryozen_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "cryozen_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "cryozen_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "cryozen_cli.web_server_lifecycle")
_spawn_cryozen_action = late("_spawn_cryozen_action", "cryozen_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "cryozen_cli.config")
get_cryozen_home = late("get_cryozen_home", "cryozen_cli.config")
_profile_cli_args = late("_profile_cli_args", "cryozen_cli.web_server_profiles")
...
_open_session_db_for_profile = late("_open_session_db_for_profile", "cryozen_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another Cryozen process uses it."""
    from cryozen_state import _default_db_path
...
async def get_ssh_ownership(request: Request):
    from cryozen_cli.web_server import _SSH_OWNER_NONCE
...
async def get_host_identity(request: Request):
    """Prove to an attaching `cryozen serve`/`dashboard` WHO owns this port.
    ...
    headless ``serve``, so a `cryozen dashboard` user is never routed to a backend with no UI.
    """
    _require_token(request)
    # ``role`` is the host ROLE this process owns (gateway/host_rendezvous.ROLE_SERVE), not the
    # launch mode: `cryozen serve` and `cryozen dashboard` are one host role that differ in SPA.
...
async def get_health_idle(request: Request):
    ...
    from cryozen_cli.web_server_idle_proof import idle_proof
    _require_token(request)
...
# Profile segment mirrors cryozen_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
def _merge_profile_gateway_platforms(gateway_platforms: dict, profile_platforms: dict) -> dict:
    ...
    failures in its own ``gateway_state.json``, invisible to the machine-level probe an external
    orchestrator reads unless folded in under the validated ``<profile>:<platform>`` grammar. The active
...
# status) are written to the per-profile home, not the process-level CRYOZEN_HOME (see issue #69143). Plain
...
async def _resolve_gateway_status(profile_dir: Optional[Path], health_url) -> Dict[str, Any]:
    ...
    if liveness.runtime is not None:
        ...
        # profile is the process's own home (a pooled ``cryozen --profile X serve``).
        served_name = profile_dir.name if profile_dir is not None else profile_name_for_home(get_process_cryozen_home())
    ...
    if runtime:
        ...
        if not gateway_running:
            ...
            # as ``cryozen gateway status`` so the sidebar strip and the CLI agree.
...
def _auth_gate_status() -> Dict[str, Any]:
    ...
    auth_flows: list[str] = []
    try:
        from cryozen_cli.dashboard_auth import (
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from cryozen_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_cryozen_home())
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    advisory, not a liveness verdict, and flipping ``overall`` on it would page an external
    ...
    try:
        from cryozen_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from cryozen_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Busy/drainable (external lifecycle-safety gate) derive from the persisted in-flight turn
        ...
        # Off-loop: on a cold Windows install the first import of cryozen_cli.gateway blocks
        ...
        status = {
            ...
            "can_update_cryozen": not _dashboard_local_update_managed_externally(),
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_cryozen_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (a remote client
        ...
        if not auth["auth_required"]:
            status.update({
                "cryozen_home": str(get_cryozen_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond cryozen home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "cryozen_version": __version__, "cpu_count": os.cpu_count()}

    def _disk():
        du = psutil.disk_usage(str(get_cryozen_home()))
...
async def set_curator_paused(body: CuratorPause, profile: Optional[str] = None):
    from agent import curator
    # ``_state_file()`` is ``get_cryozen_home()/skills/.curator_state`` resolved at call
...
def _spawn_action(argv: list, name: str, prefix: str, profile: Optional[str] = None) -> dict:
    """Spawn a background ``cryozen -p <profile> <argv>`` action; a spawn failure is
    ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_cryozen_action(_profile_cli_args(profile) + argv, name)
    except Exception as exc:
...
async def get_logs(
    ...
    profile: Optional[str] = None):
    from cryozen_cli.logs import _read_tail, LOG_FILES
    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    with _config_profile_scope(profile):
        log_path = get_cryozen_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from cryozen_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
