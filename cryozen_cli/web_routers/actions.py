"""Gateway restart/drain, Cryozen update and background-action status dashboard routes.

Extracted from ``cryozen_cli.web_server``; helpers/state that tests monkeypatch on
...
from cryozen_cli.config import format_docker_update_message, recommended_update_command_for_method
from cryozen_cli.web_deps import LateState, late
from cryozen_cli.web_server_gateway import _ACTION_LOG_FILES
from cryozen_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "cryozen_cli.config")
...
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "cryozen_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "cryozen_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "cryozen_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "cryozen_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import cryozen_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== cryozen-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Cryozen updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== cryozen update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``cryozen gateway restart`` in the background."""
...
async def gateway_migrate_plan():
    ...
    from cryozen_cli.gateway_migrate import build_migration_plan
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("cryozen-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "cryozen-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/cryozen/update")
async def update_cryozen():
    """Kick off ``cryozen update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("cryozen-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "cryozen-update", "already_running": True}
        action_id = _ACTION_IDS.get("cryozen-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn cryozen update", 500, "Failed to start update"):
        proc = _spawn_cryozen_action(["update"], "cryozen-update", env_overrides={"CRYOZEN_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "cryozen-update", "action_id": action_id}


_NON_APPLYABLE_MESSAGES = {
    "docker": format_docker_update_message,
    "apt": lambda: "Cryozen is managed by Termux APT; run `pkg upgrade cryozen-agent`.",
}


@router.get("/api/cryozen/update/check")
async def check_cryozen_update(force: bool = False, profile: Optional[str] = None):
    """Report whether a Cryozen update is available, without applying it.
    ...
    try:
        from cryozen_cli.banner import check_for_updates, upstream_commits_behind
        ...
        if force:
            ...
            with contextlib.suppress(OSError), _config_profile_scope(profile):
                (get_cryozen_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "cryozen-update":
        ...
        if durable_update_action_id:
            marker = f"=== cryozen-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``cryozen update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``cryozen update`` run since #91283, including
...
@status_router.get("/api/cryozen/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `cryozen update` run recorded).")
