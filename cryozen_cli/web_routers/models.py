...
from cryozen_cli.web_models import ModelAssignment, MoaConfigPayload, MoaModelSlot
from cryozen_cli.web_routers._common import _CONFIG_MUTATION_LOCK, config_write_scope, http_failure
...
def get_recommended_default_model(provider: str = "", profile: Optional[str] = None):
    ...
    ``cryozen model``'s curation so GUI onboarding lands on a sensible default.
    A provider gets the preferred silent default when its curated list carries it, else the first curated model —
    ...
    Response: {"provider", "model"}; ``model`` may be empty (caller degrades gracefully)."""
    slug = (provider or "").strip().lower()

    try:
        from cryozen_cli.inventory import build_models_payload, load_picker_context
        from cryozen_cli.models import pick_silent_default_model
        ...
        for row in payload.get("providers", []):
            if str(row.get("slug", "")).lower() == slug:
                ...
                return {"provider": slug, "model": pick_silent_default_model(models, provider=slug)}
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    with http_failure("PUT /api/model/moa failed", 500, detail="Failed to save MoA config"):
        from cryozen_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.cryozen-agent/config.yaml`` — applies to **new** sessions only; a running chat
