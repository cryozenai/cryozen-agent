"""Provider-specific runtime builders for :mod:`cryozen_cli.runtime_provider`: Azure Foundry, the
...
``cryozen_cli.runtime_provider.*`` (``_get_model_config``, ``load_config``, ``has_usable_secret``,
...
def _azure_foundry_api_key(rp, explicit_api_key: str) -> str:
    ...
    if not api_key:
        raise rp.AuthError(
            ...
            "~/.cryozen-agent/.env or run 'cryozen model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'cryozen model')."
        )
    return api_key
...
def _resolve_azure_foundry_runtime(*, requested_provider: str, model_cfg: Dict[str, Any],
    ...
    if not base_url:
        raise rp.AuthError(
            "Azure Foundry requires a base URL. Set it via 'cryozen model' or "
...
def _resolve_openrouter_runtime(
    *, requested_provider: str, explicit_api_key: Optional[str] = None, explicit_base_url: Optional[str] = None
    ...
    if is_openrouter_context:
        ...
        from cryozen_cli.runtime_provider_custom import _model_cfg_key_env_for
...
def _resolve_bedrock_runtime(requested_provider: str, model_cfg: Dict[str, Any], target_model: Optional[str]) -> Dict[str, Any]:
    ...
    from cryozen_cli.config import load_config  # direct (not the origin delegate), as before
    rp = _rp()
