# cryozen_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `CryozenCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`cryozen_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
`cli_tui_mixin.py` (widgets, keybindings, panels), `cli_tui_runtime_mixin.py`
...
`cryozen_cli/gateway.py` is the `cryozen gateway` facade (process discovery, systemd backend, command
...
(`cryozen gateway setup`: `_PLATFORMS` registry, status table, per-platform prompts, service offer),
...
(late binding on `cryozen_cli.gateway`), so monkeypatch on the facade; mutable state such as
...
`CryozenCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.cryozen-agent/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`cryozen_cli/curses_ui.py`; example
`cryozen_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`cryozen_cli/commands.py`)
...
`cryozen_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`cryozen_cli/config.py`)
...
- **.env = SECRETS ONLY** (keys, tokens, passwords): add to `OPTIONAL_ENV_VARS` with
  ...
  in `.env` is deprecated — the loader warns; canonical is `terminal.cwd`. `cryozen config
...
- **One writer.** Every write of a `config.yaml` (main or profile) goes through
  `cryozen_cli.config.atomic_config_write` (→ `utils.atomic_roundtrip_yaml_save`, ruamel
  ...
  (CI lint) rejects it, and `tests/cryozen_cli/test_config_yaml_comment_preservation.py` guards each
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`cryozen tools/setup`, most subcommands, `cryozen_cli/config.py`, merges `DEFAULT_CONFIG`);
  `cryozen_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `cryozen send`, doctor,
  `cryozen_time`/`cryozen_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
- **Every `DEFAULT_CONFIG` key has a runtime reader, and every reader a registry entry.** Both drift
  ...
  key: `rg -n '"<key>"' cryozen_cli/config_defaults.py` AND `rg -n '<key>' --glob '!tests' .` both hit,
...
## Skin engine (`cryozen_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.cryozen-agent/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`cryozen update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `cryozen update --plan`): read-only inventory — install kind, all
...
- **Apply**: git pull, or the Windows ZIP fallback — which fires ONLY when git itself failed
  ...
  root entry the ZIP does not ship (`.bytecode-fingerprint`, `.cryozen-bootstrap-complete`,
  `cryozen_agent.egg-info/`; tracked root entries stand in for the ZIP set before the download, the
  ...
  `cryozen_cli/web_dist`, `ui-tui/{dist,node_modules,packages/cryozen-ink/dist}`, `web/node_modules`,
  ...
  rebuild decision also trusts the build stamp under CRYOZEN_HOME, so an install that already lost
  its artifacts in an earlier update is rebuilt instead of "forgotten" (#90495).
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `cryozen-gateway*` unit /
  `ai.cryozen.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.cryozen-agent/logs/update_receipts/`
...
- **Nothing runs pulled code in the pre-pull interpreter** (`update_handoff.py`). The process that
  started `cryozen update` imported the PRE-pull tree; once git (or the ZIP swap) has replaced the
  ...
  `cryozen update <same flags> --post-swap <file>` under the venv interpreter, which imports only the
...
`_apply_profile_override()` in `cryozen_cli/main.py` sets `CRYOZEN_HOME` before any module import for
single-profile commands (`cryozen -p x <cmd>`), so there `get_cryozen_home()` scopes to the active
profile. The multiplex gateway and the Desktop/dashboard `serve` backend instead bind the active
profile per activity via a contextvar override while `os.environ["CRYOZEN_HOME"]` keeps the launch
...
carries an identity marker (`cryozen_constants.named_profile_has_identity`: `config.yaml`/`.env`/`SOUL.md`/
...
cron all go through `mkdir_under_cryozen_home` / `_ensure_cron_dir`, which refuse a deleted or
missing named profile, #94590). Process-global per-profile slots (MCP discovery in `mcp_startup.py`,
tool registry overlays) key on `cryozen_constants.cryozen_home_key()`, never a single flag.
...
Migration from per-profile gateways: `cryozen_cli/gateway_migrate.py` (`cryozen gateway migrate
...
other service domain / UNIX user / CRYOZEN_HOME outside `profiles/` — notices for the explicit command,
...
`cryozen gateway restart` for a gateway Cryozen did not install (custom launchd agent / unit running
...
sibling and read facade helpers late-bound through `cryozen_cli.gateway`, so patch them on the facade; user unit AND `--system` unit with `User=`; an unresolvable `User=` is a blocker,
...
profile's `CRYOZEN_HOME` (and `HOME` for the service user) explicitly — a supervisor starts with an
