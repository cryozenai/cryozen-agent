...
_TASK_NAME_DEFAULT = "Cryozen_Gateway"
_TASK_DESCRIPTION = "Cryozen Agent Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("CRYOZEN_GATEWAY_DETACHED", "1"), ("CRYOZEN_SUPERVISED_CHILD", "1"))
...
def _cryozen_home() -> Path:
    from cryozen_cli.config import get_cryozen_home

    return Path(get_cryozen_home())


def cryozen_service_roots() -> tuple[str, ...]:
    """Directories a Cryozen-owned SCM service binary lives under: the checkout (its ``venv`` included),
    the running interpreter's ``Scripts`` dir (``cryozen.exe`` shim) and the ``gateway-service`` launcher dir."""
    project_root = Path(__file__).resolve().parent.parent
    return (str(project_root), str(Path(sys.executable).parent), str(_cryozen_home() / "gateway-service"))
...
def cryozen_owns_windows_service(name: str, binpath: str, cryozen_roots: tuple[str, ...]) -> bool:
    """Positive ownership of an SCM service: Cryozen-named (``cryozen*``) or its binary path starts under a
    Cryozen root. Pure so it is testable off-Windows. A Scheduled-Task-launched gateway descends from
    ...
    if normalized_name.startswith("cryozen"):
        return True
    candidate = _normalize_windows_path(binpath)
    return any(candidate.startswith(_normalize_windows_path(root) + "/") for root in cryozen_roots if root)


def _preserve_cryozen_home_path(path: str | Path) -> str:
    r"""Render Cryozen-owned paths under the configured CRYOZEN_HOME spelling.

    ``%LOCALAPPDATA%\cryozen`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under CRYOZEN_HOME.
    """
    candidate = Path(path)
    try:
        home = _cryozen_home()
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Cryozen profile."""
    from cryozen_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "cryozen_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"CRYOZEN_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "CRYOZEN_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "CRYOZEN_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from cryozen_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<CRYOZEN_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _cryozen_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at CRYOZEN_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m cryozen_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "cryozen_cli.main"]
    if profile_arg:
...
def _launcher_settings(home: Path | None = None) -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, cryozen_home, profile_arg) for generated launchers.
    ``home`` targets another profile's CRYOZEN_HOME (per-profile cold-start, #110959)."""
    from cryozen_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    cryozen_home = str(home if home is not None else _cryozen_home())
    return (
        _preserve_cryozen_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        cryozen_home,
        _profile_arg(cryozen_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_cryozen_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_cryozen_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, cryozen_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "CRYOZEN_HOME={cryozen_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_cryozen_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, cryozen_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('CRYOZEN_HOME')}) = {q(cryozen_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_cryozen_home_path(venv_dir))}",
...
def _atomic_write(path: Path, content: str, tmp: Path) -> None:
    ...
    ``Cryozen_Gateway.tmp`` pops up in Notepad after every sign-in (#114093).
    """
    try:
...
def _build_gateway_argv(home: Path | None = None) -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from cryozen_cli.gateway import PROJECT_ROOT

    python_path, working_dir, cryozen_home, profile_arg = _launcher_settings(home)
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"CRYOZEN_HOME": cryozen_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_cryozen_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_cryozen_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (CRYOZEN_HOME,
    ...
    try:
        cryozen_home = str(_cryozen_home().resolve())
    except Exception:
        cryozen_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "CRYOZEN_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if cryozen_home:
        env_overlay["CRYOZEN_HOME"] = cryozen_home
...
def _stdin_is_interactive(*, isatty: bool, console_mode_ok: bool | None) -> bool:
    ...
    every character device — the NUL device included (`cryozen gateway start < NUL`, stdin=DEVNULL) — so
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("CRYOZEN_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("CRYOZEN_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from cryozen_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids or start_now:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"cryozen {profile_arg} gateway start" if profile_arg else "cryozen gateway start"
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated Cryozen gateway install prompt.")
            if start_now:
                print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
            else:
                print("  Approve the Windows UAC prompt, then run: cryozen gateway status")
            return True
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run in the foreground later with: cryozen gateway run")
        return

    task_name = get_task_name()
    script_path = _write_task_script()
    # A pre-fix install that failed its Startup-folder swap left `Cryozen_Gateway.tmp` there, and the
    ...
    try:
        ...
        pass
    if force:
        # Pre-suffix strays (task ``Cryozen_Gateway``, Startup ``Cryozen_Gateway.vbs``) are unreachable by
        # the current names, so a plain reconcile never heals them (#116157).
        from cryozen_cli.gateway_windows_legacy import remove_legacy_launchers
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: cryozen gateway start")
        _print_next_steps()
        return
...
def _start_attestation_path(home: Path | None = None) -> Path:
    ...
    return (home if home is not None else _cryozen_home()).joinpath(*_START_ATTESTATION_RELATIVE)
...
def _write_start_attestation(pids: list[int], via: str, home: Path | None = None) -> None:
    ...
    newer one written by a concurrent ``cryozen gateway start`` (#110020 review)."""
    try:
        path = _start_attestation_path(home)
        path.parent.mkdir(parents=True, exist_ok=True)
        from cryozen_cli.process_identity import _process_create_time
...
# meant to bridge the seconds between a ✓ and the next ``cryozen gateway status``/``update``; a
...
def _attested_pid_exited_cleanly(pid: int, create_time: float | None = None, home: Path | None = None) -> bool:
    ...
    try:
        ...
        sentinel = get_lifecycle_sentinel_path(home if home is not None else _cryozen_home())
...
def attested_death_generation(current_pids: list[int], home: Path | None = None) -> str | None:
    ...
    one-shot marker — ``cryozen update`` consults it to decide whether a Desktop-owned install
...
def _format_attestation_warning(attested: list[int], data: dict) -> str:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `cryozen gateway start` was inside "
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_cryozen_home()}\\logs\\gateway.log\n    type {_cryozen_home()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {_cryozen_home()}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_gateway_command("uninstall"):
                    print("✓ Launched elevated Cryozen gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: cryozen gateway status")
                    return
...
def _print_scheduled_task_drift(task_name: str) -> None:
    ...
    repair runs from ``start()`` / ``cryozen update`` via ``reconcile_scheduled_task``)."""
    drift = scheduled_task_drift(task_name)
    if drift:
        print(f"⚠ Scheduled Task registration predates the current template ({'; '.join(drift)})")
        print("  Repair: cryozen gateway start  (or: cryozen gateway install)")
...
def reconcile_scheduled_task(task_name: str) -> bool:
    ...
    ``schtasks`` refused (typically Access Denied — the elevating ``cryozen gateway install`` is the fallback)."""
    drift = scheduled_task_drift(task_name)
    if not drift:
        return False
    ...
    if not ok:
        print("  Repair manually: cryozen gateway install")
    return ok
...
def status(deep: bool = False) -> None:
    ...
    from cryozen_cli.gateway_windows_legacy import warn_legacy_launchers
...
def start() -> None:
    ...
    if not is_task_registered() and not is_startup_entry_installed():
        ...
        # answer — the CRYOZEN_GATEWAY_INSTALL_START_ON_LOGIN override or a real TTY prompt — never on a
        # non-TTY default (#113977). Declining still starts the gateway; the command is ``start``.
        start_on_login = _install_choice_from_env("CRYOZEN_GATEWAY_INSTALL_START_ON_LOGIN")
        if start_on_login is None:
            from cryozen_cli.setup import is_interactive_stdin, is_noninteractive, prompt_yes_no
        ...
        if start_on_login:
            ...
            return
        print("ℹ Login auto-start not installed; add it later with: cryozen gateway install")
...
def restart() -> None:
    ...
    from cryozen_cli.gateway import _wait_for_api_server_port_free  # avoid circular init
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `cryozen gateway status`."
        )
