"""cryozen import-agent — import Claude Code / Codex CLI setups into Cryozen.
...
# Entry delimiter of the Cryozen memory store (memories/MEMORY.md) and the openclaw script.
ENTRY_DELIMITER = "\n§\n"
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not path.exists():
        return {}
    fix_hint = "Fix it with `cryozen config edit` (or move it aside), then re-run the import."
...
def _translate_mcp_server(name: str, srv: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    """Map one Claude/Codex MCP server entry to Cryozen shape; returns (server, stripped secret paths)."""
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record("slash-commands", commands_dir, None, "skipped",
                        "Claude slash commands have no direct Cryozen equivalent — "
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → CRYOZEN_HOME/skills/<category>/<name>."""
        ...
        if not skill_dirs:
            ...
            return
        from cryozen_cli.agent_import_sync import skill_tree_digest
...
def import_agent_command(args) -> None:
    """Handle ``cryozen import-agent`` (invoked from cryozen_cli.main)."""
    from cryozen_cli.config import get_config_path, load_config, save_config
    from cryozen_constants import get_cryozen_home
    from cryozen_cli.setup import (Colors, color, print_header, print_info, print_success,
                                  print_error, prompt_yes_no)

    if getattr(args, "sync", False):
        from cryozen_cli.agent_import_sync import sync_imported_agents
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: cryozen import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: cryozen import-agent claude-code   or   cryozen import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ✻ Cryozen — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))
    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info(f"Specify a custom path: cryozen import-agent {agent} --source /path/to/{_AGENT_DEFAULT_DIRS[agent]}")
        return
    ...
    print_info(f"Target:      {cryozen_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'cryozen setup' for credentials")
    ...
    def run_import(execute: bool, phase: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            return AgentImporter(agent, source_dir.resolve(), cryozen_home.resolve(),
    ...
    if not args.yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: cryozen import-agent {agent} --yes")
            return
    ...
    from cryozen_cli.agent_import_sync import update_sync_manifest
    try:
        update_sync_manifest(agent, source_dir.resolve(), cryozen_home.resolve(), overwrite, report)
        print_info("Source registered for sync — re-run 'cryozen import-agent --sync' "
                   "any time to pull in changes.")
    except OSError as exc:
        logger.warning("Could not update import sync manifest: %s", exc)
    print()
    print_success("Import complete.")
    print_info("API keys and credentials were NOT imported — run 'cryozen setup' "
               "to configure providers, or add them to ~/.cryozen-agent/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from cryozen_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped := report.get("stripped_secrets"):
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'cryozen setup' or ~/.cryozen-agent/.env.")
        print()
