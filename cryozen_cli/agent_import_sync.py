"""``cryozen import-agent --sync`` — keep previously imported Claude Code / Codex setups current.

Every successful ``cryozen import-agent`` run records its source in ``CRYOZEN_HOME/import-sync.json``
...
def _iter_sync_files(agent: str, source_root: Path) -> Iterator[Path]:
    ...
    :class:`~cryozen_cli.agent_import.AgentImporter` reads for ``agent``."""
...
def sync_imported_agents(args) -> None:
    """Handle ``cryozen import-agent --sync``: re-import every registered source whose digest
    changed. Prompt-free (cron-friendly); ``--dry-run`` previews without touching the manifest."""
    from cryozen_cli.agent_import import AgentImporter, print_import_report
    from cryozen_cli.setup import print_error, print_header, print_info, print_success
    from cryozen_constants import get_cryozen_home

    dry_run = bool(getattr(args, "dry_run", False))
    cryozen_home = get_cryozen_home().resolve()
    agents: Dict[str, Any] = load_sync_manifest(cryozen_home).get("agents", {})
    if not agents:
        print()
        print_info("No import sources registered yet.")
        print_info("Run 'cryozen import-agent' first — successful imports are "
    ...
    for agent_name in sorted(agents):
        ...
        try:
            report = AgentImporter(agent_name, source_dir, cryozen_home, execute=not dry_run,
        ...
        if not dry_run:
            try:
                ...
                update_sync_manifest(agent_name, source_dir, cryozen_home, overwrite, report,
