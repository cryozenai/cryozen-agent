"""Cryozen-managed uv and Python runtime repair.

The Python backing the install is shared by every Cryozen profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.cryozen-runtime/python``. A vulnerable interpreter is never reinstalled in place: a
...
from cryozen_cli.sqlite_runtime import (
    SQLiteRuntimeInfo, isolated_interpreter_env, probe_sqlite_runtime)

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".cryozen-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.cryozen.cryozen.managed-python"

_Provisioned = tuple[Path, Path, SQLiteRuntimeInfo]


def managed_uv_path() -> Path:
    """Path of Cryozen' own uv binary (``$CRYOZEN_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_cryozen_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def pip_install_hint(package: str) -> str:
    ...
    Names Cryozen' own uv when it exists: the installer drops it in ``$CRYOZEN_HOME/bin``
...
def managed_python_env(
    ...
    """Return a sanitized environment for Cryozen-private uv Python commands."""
    target = (
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: Cryozen keeps databases "
              "out of WAL mode on this SQLite build. The next `cryozen update` "
              "will retry.")
        return
...
def _uv_runs(uv_bin: str) -> bool:
    ...
    ``$CRYOZEN_HOME/bin``: it is a file with the executable bit that never runs, so is_file()+X_OK
...
def _record_runtime_repair(repair: RuntimeRepairResult) -> None:
    """Put the repair outcome into the update receipt (no-op outside ``cryozen update``).
    ...
    """
    from cryozen_cli.update_receipt import record_skip, record_step
...
def _uv_self_update_stamp() -> Path:
    from cryozen_constants import get_cryozen_home
    return get_cryozen_home() / "cache" / ".uv_self_update_stamp"


def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    """True when ``uv self update`` ran recently enough to skip.

    uv releases roughly weekly while many users run ``cryozen update`` daily; a blocking network
...
def _reload_cryozen_constants():
    """Re-execute ``cryozen_constants`` from disk (the imported one may predate venv_python_path)."""
    import cryozen_constants
    return importlib.reload(cryozen_constants)
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the Cryozen generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `cryozen update`. The
...
def _stream_sync(argv: list[str], *, cwd: Path, env: dict[str, str]) -> tuple[int, str]:
    ...
    scrollback only, and the rejection carried a bare exit code — "cryozen update says the SQLite
...
def _install_uv(target: Path) -> None:
    ...
    into ``$CRYOZEN_HOME/bin/`` instead of ``~/.local/bin/``.
    """
