...
Split out of ``cryozen_cli.models``. Catalogs share one implementation parametrized by
:class:`_CapsSource`; the per-source module globals (``_openrouter_reasoning_caps_cache``,
``_openrouter_caps_disk_checked``, ...) stay defined on
``cryozen_cli.models`` — tests reset them there — and are read/written by attribute name.
...
logger = logging.getLogger("cryozen_cli.models")

Caps = dict[str, Optional[dict[str, Any]]]


def _origin():
    from cryozen_cli import models
    return models
...
# that must never block on HTTP — so without a disk copy, `cryozen -p`, a cron job, or a freshly
# booted gateway answers "capability unknown" for its whole first turn. One file holds every
# catalog keyed by URL, so two catalogs (or a staging and a production endpoint) never answer for
# each other.
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_cryozen_home() / "cache" / "reasoning_caps.json"


def _read_reasoning_caps_disk() -> dict[str, Any]:
    from cryozen_cli.models import _read_json_cache
...
def _warm_reasoning_caps_async(refresh) -> None:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        return
    # copy_context: the catalog URL and disk mirror are the calling profile's, not the launch home's.
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    empty so callers remember the failure. Sends a User-Agent: some catalogs 403 anonymous reads."""
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._CRYOZEN_USER_AGENT})
...
# ── Per-source cache (OpenRouter) ─────────────────────────

@dataclass(frozen=True)
class _CapsSource:
    """One catalog's cache slots on ``cryozen_cli.models`` plus how to name its URL.
    ...
    def get(self, slot: str):
        ...
        from cryozen_cli.models_profile_cache import profile_slot_get
    ...
    def set(self, slot: str, value) -> None:
        if not self.per_profile:
            ...
            return
        from cryozen_cli.models_profile_cache import profile_slot_set
...
def _caps_cached(src: _CapsSource) -> Optional[Caps]:
    ...
    One disk attempt per process: naming a catalog may mean resolving credentials, which can itself
    reach the network to refresh a token — too expensive for a per-turn caller.
    """
...
def refresh_reasoning_caps_async(provider: Optional[str]) -> None:
    ...
    """
    src = {"openrouter": _OPENROUTER_CAPS}.get(str(provider or "").strip().lower())
    if src is not None:
        _warm_reasoning_caps_async(lambda: _fetch_caps(src, force=True))
