...
in :func:`run_migrations`. Every write goes through ``cryozen_cli.config._persist_migration`` so a
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_cryozen_home()}/config.yaml and run `cryozen setup` to "
        f"regenerate, or manually set _config_version: {SUPPORT_FLOOR_VERSION} "
        "after reviewing the changelog.")


def _cfg():
    """Return the live ``cryozen_cli.config`` module (lazy, cycle-free, monkeypatch-friendly)."""
...
def _installed_user_plugins(disabled: set) -> List[str]:
    """Names of plugins under ``$CRYOZEN_HOME/plugins/`` with a manifest, minus *disabled*."""
    _c = _cfg()
    found: List[str] = []
    try:
        user_plugins_dir = _c.get_cryozen_home() / "plugins"
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    message = (
        ...
        "Use `cryozen plugins enable <name>` to activate.")
    _commit(
        config, results, quiet,
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # section; deep-merge made it work but users could not see/edit it and `cryozen curator status`
    # had no stable logs dir. Only keys the user hasn't set are written.
    _c = _cfg()
    DEFAULT_CONFIG = _c.DEFAULT_CONFIG

    try:
        curator_dir = _c.get_cryozen_home() / "logs" / "curator"
        ...
        for label, added in (("curator", added_curator), ("auxiliary.curator", added_aux)):
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added)}) — edit via `cryozen config set`")
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from cryozen_cli.relay_plugin_cutover import legacy_relay_plugin_keys
    ...
    message = (
        ...
        "CRYOZEN_NEMO_RELAY_PLUGINS_TOML.")
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if changed:
        _commit(
            ...
            "lists — video generation now lives under `cryozen tools` → "
...
def _migrate_to_41(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from tools.bot_mode_probe import _PROTOCOL_HEADING, _cryozen_root, _roster, strip_legacy_protocol

    cleaned: List[str] = []
    for name, profile_dir in _roster(_cryozen_root(get_cryozen_home())):
...
def _migrate_to_45(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from cryozen_cli.tools_config import _configurable_keys, _get_plugin_toolset_keys
    ...
    for platform, toolsets in saved.items():
        ...
        if not toolset_allowed_for_platform("connections", platform):
            continue
        # A composite like [cryozen-cli] already inherits every core tool at read time.
    ...
    _commit(
        ...
        "Uncheck Connections in `cryozen tools` to turn it off.")
...
def _migrate_to_46(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # `disabled: true` wins over an explicit `enabled: true`: `cryozen mcp add` writes that, and the
    # old editor only added `disabled`, so letting `enabled` win would skip nearly every server.
    from cryozen_cli.tools_config import _parse_enabled_flag
...
# Tool sections that could route through the retired hosted tool gateway (``use_gateway: true`` or
# a backend/provider value naming the retired provider).
_RETIRED_GATEWAY_TOOL_SECTIONS = ("web", "image_gen", "video_gen", "tts", "stt", "browser")


def _migrate_to_47(results: Dict[str, Any], quiet: bool) -> None:
    # 46 → 47: the hosted tool gateway, hosted cron provider and hosted telemetry upload are gone.
    # Drop their settings so an old config does not keep a stale endpoint or credential for them.
    config = read_raw_config()
    changed: List[str] = []

    if "tool_gateway_declined_tools" in config:
        del config["tool_gateway_declined_tools"]
        changed.append("removed tool_gateway_declined_tools:")

    for section in _RETIRED_GATEWAY_TOOL_SECTIONS:
        block = config.get(section)
        if not isinstance(block, dict):
            continue
        if "use_gateway" in block:
            del block["use_gateway"]
            changed.append(f"removed {section}.use_gateway")

    terminal = config.get("terminal")
    if isinstance(terminal, dict) and "modal_mode" in terminal:
        del terminal["modal_mode"]
        changed.append("removed terminal.modal_mode")

    telemetry = config.get("telemetry")
    if isinstance(telemetry, dict):
        for key in ("send", "endpoint"):
            if key in telemetry:
                del telemetry[key]
                changed.append(f"removed telemetry.{key}")

    cron = config.get("cron")
    if isinstance(cron, dict):
        if "chronos" in cron:
            del cron["chronos"]
            changed.append("removed cron.chronos")
        if str(cron.get("provider") or "").strip().lower() == "chronos":
            del cron["provider"]
            changed.append("removed cron.provider")
    ...
    if isinstance(dashboard, dict):
        ...
        if isinstance(oauth, dict):
            for key in ("client_id", "portal_url"):
                if key in oauth:
                    del oauth[key]
                    changed.append(f"removed dashboard.oauth.{key}")

    if not changed:
        return
    _commit(
        config, results, quiet,
        "retired hosted-service settings: " + ", ".join(changed),
        "  ✓ Removed settings for hosted services this release no longer offers "
        f"({len(changed)} change(s)). Run `cryozen model` if you need to pick a new provider.")
...
MIGRATIONS: Tuple[Tuple[int, Callable[[Dict[str, Any], bool], None]], ...] = (
    ...
    (44, _rewrite_stale_default(
        ...
        message=(
            ...
            "skills/.archive/ (recoverable with `cryozen curator restore`). Set it back to 90 to keep the old window."))),
    ...
    (46, _migrate_to_46),
    # 46 → 47: drop settings of retired hosted services (see _migrate_to_47).
    (47, _migrate_to_47),
)
