"""Profile management for multiple isolated Cryozen instances."""
...
from cryozen_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
...
_PROFILE_ID_RE = PROFILE_ID_RE  # legacy alias; cryozen_constants.PROFILE_ID_RE is canonical
...
# stay visible; containers persist HOME state here). See cryozen_constants.get_subprocess_home().
...
# (``~/.cryozen-agent``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
# ``cryozen_cli.backup._EXCLUDED_ROOT_DIRS`` so the two lists cannot drift.
_CLONE_ALL_DEFAULT_EXCLUDE_ROOT: frozenset[str] = frozenset({
    "cryozen-agent", ".worktrees", "profiles", "bin", "node_modules",
}) | LOCAL_RUNTIME_ROOT_DIRS

# Per-profile history excluded from --clone-all for ANY source: SQLite session store
# (+wal/shm, can reach many GB), session dirs, `cryozen backup` archives, quick-backup
...
# Marker written by `cryozen profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `cryozen update` all-profile sync, the
...
_PLACEHOLDER_ENV = (
    "# Per-profile secrets for this Cryozen profile.\n"
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_cryozen_home().resolve():
...
# Allow-list for ``export_profile("default")``: when CRYOZEN_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known Cryozen profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``cryozen_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"cryozen", "default", "test", "tmp", "root", "sudo"})

# Cryozen subcommands that cannot be used as profile names/aliases
_CRYOZEN_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the cryozen root (NOT the current CRYOZEN_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_cryozen_home() / "profiles"


def _get_default_cryozen_home() -> Path:
    """Default (pre-profile) CRYOZEN_HOME: ``~/.cryozen-agent``, or CRYOZEN_HOME itself in
    Docker/custom deployments (e.g. ``/opt/data``)."""
    from cryozen_constants import get_default_cryozen_root
    return get_default_cryozen_root()


def _get_active_profile_path() -> Path:
    return _get_default_cryozen_home() / "active_profile"
...
def _is_our_wrapper(path: Path) -> bool:
    """True when *path* reads as a Cryozen-generated wrapper (contains ``cryozen -p``)."""
    try:
        return "cryozen -p" in path.read_text(encoding="utf-8")
    except Exception:
        return False


def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: cryozen profile create {canon}")
...
def _unknown_profile_error(canon: str) -> FileNotFoundError:
    ...
    return FileNotFoundError(f"No profile named '{canon}'. See your profiles with: cryozen profile list")


def _profile_exists_error(canon: str) -> FileExistsError:
    return FileExistsError(
        f"A profile named '{canon}' already exists. Switch to it with `cryozen profile use {canon}`, "
        "see all profiles with `cryozen profile list`, or choose a different name."
    )
...
def _invalid_profile_name_error(name: str) -> ValueError:
    ...
    return ValueError(
        ...
        f"Then run `cryozen profile create {suggestion}`."
    )
...
def validate_profile_name(name: str) -> None:
    ...
    """
    if name == "default":
        return  # special alias for ~/.cryozen-agent
    if not _PROFILE_ID_RE.match(name):
        raise _invalid_profile_name_error(name)
    if name in _RESERVED_NAMES:
        raise ValueError(
            f"Profile name {name!r} is reserved — it collides with either "
            f"the Cryozen installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CRYOZEN_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_cryozen_home()
    ...
    # profiles/cryozen may still exist and must keep resolving.
...
def _iter_named_profile_dirs(*, live_only: bool = True) -> List[Path]:
    ...
    shell into a "real" profile on the next ``cryozen update`` (#95188, #94823, #99392)."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _CRYOZEN_SUBCOMMANDS:
        return f"'{canon}' conflicts with a cryozen subcommand"
    try:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        if sys.platform == "win32":
            ...
            cryozen_exe = shutil.which("cryozen") or "cryozen"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(cryozen_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `cryozen doctor --fix` surfaces
    ...
    with contextlib.suppress(Exception):
        ...
        from cryozen_cli.config import check_config_version, migrate_config
        token = set_cryozen_home_override(str(profile_dir))
        try:
...
# wrappers are a few hundred bytes with the ``cryozen -p X`` needle near the top; the wrapper
...
class ProfileInfo:
    ...
    display_name: str = ""
    # Bot Mode title (``profile.yaml`` ``ui_meta['cryozen-bots'].title``) — the name
    # the Bots roster shows. Presentation-only, like ``display_name``.
    bot_title: str = ""
    # Canonical ids this profile was previously known by (``cryozen profile rename``
    # appends here). Lets Bot Mode group chats re-link persisted member
    # descriptors to the renamed live profile (#110200).
    previous_names: List[str] = field(default_factory=list)
    # Backend-assigned role (``SETUP_ROLE`` or None). Written through ``write_profile_meta``.
...
def _read_config_model(profile_dir: Path) -> tuple:
    ...
    def _read() -> tuple:
        try:
            ...
            from cryozen_cli.config import read_user_config_raw
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    if config_path.exists():
        return
    with contextlib.suppress(Exception):  # creation must not fail over this; `cryozen model` sets it later
        from cryozen_constants import get_cryozen_home
        from cryozen_cli.config import atomic_config_write, read_user_config_raw
        source = get_cryozen_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating CRYOZEN_HOME.
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from cryozen_cli.gateway import named_profile_served_by_running_multiplexer
...
def _count_skills(profile_dir: Path) -> int:
    ...
    synchronously when stale — detail surfaces (``cryozen profile info``, ``profiles.describe``)
...
def _cached_skill_count(profile_dir: Path) -> int:
    ...
    if due:
        threading.Thread(target=_count_skills, args=(profile_dir,),
                         name="cryozen-skill-count", daemon=True).start()
    cached = _SKILL_COUNT_CACHE.get(key)
    return cached[2] if cached is not None else 0


# profile.yaml — per-profile metadata (description, role, etc.)
# Deliberately tiny and separate from ``config.yaml`` (user-facing Cryozen config, ~5000
...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    corrupt file on one profile must not break ``cryozen profile list``."""
    def _read() -> dict:
        data = _load_yaml_dict(profile_dir / "profile.yaml") or {}
        ui_meta = data.get("ui_meta")
        bot_title = ""
        if isinstance(ui_meta, dict):
            cryozen_bots = ui_meta.get("cryozen-bots")
            if isinstance(cryozen_bots, dict):
                bot_title = str(cryozen_bots.get("title") or "").strip()
        return {
...
def list_profiles(*, lazy_skill_count: bool = False) -> List[ProfileInfo]:
    ...
    profiles = []
    default_home = _get_default_cryozen_home()
...
def profile_is_standalone(home: Path) -> bool:
    ...
    if home == _get_default_cryozen_home():
...
def profiles_to_serve(multiplex: bool, *, include_standalone: bool = False) -> List[Tuple[str, Path]]:
    """``(profile_name, cryozen_home)`` pairs a gateway should serve — the single chokepoint
    ...
    serve: List[Tuple[str, Path]] = [("default", _get_default_cryozen_home())]
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    # auth.json copied verbatim forks single-use OAuth grants
    # (Codex / xAI): one credential with two owners, and the first profile to
    ...
    from cryozen_cli.auth import strip_cloned_single_use_oauth_grants
...
def _bootstrap_profile_dir(profile_dir: Path, source_dir: Optional[Path],
    ...
    ``sync_imports`` also copies the source's ``import-sync.json`` (the ``cryozen import-agent``
    manifest) so the clone stays registered against the same external Claude Code / Codex trees
    and ``cryozen -p <clone> import-agent --sync`` keeps pulling from them. The link is to the
    ...
    for relpath in _CLONE_SUBDIR_FILES:
        _clone_file(source_dir, profile_dir, relpath)
    if sync_imports:
        from cryozen_cli.agent_import_sync import SYNC_MANIFEST_NAME  # lazy: keeps yaml/utils off the hot startup path
...
def create_profile(
    ...
    makes two gateways fight over one bot (``cryozen_cli.profile_channels``; callers list what
    was left behind with ``channel_platforms_configured(source_dir)``).
    ``no_skills`` creates an empty profile and writes a marker so ``cryozen update`` skips
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.cryozen-agent).")
    profile_dir = get_profile_dir(canon)
    ...
    if source_dir is not None and clone_channels:
        from cryozen_cli.profile_channels import clone_channels_refusal
    ...
    try:
        ...
        if source_dir is not None and not clone_channels:
            from cryozen_cli.profile_channels import strip_channel_settings
    ...
    # `cryozen -p <profile> gateway start` supervises via `s6-svc -u` instead of a bare
...
def _finish_profile_layout(profile_dir: Path, *, no_skills: bool, clone_all: bool,
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `cryozen -p <name> auth add`)
    ...
    with contextlib.suppress(Exception):  # best-effort — don't fail profile creation over this
        from cryozen_cli.default_soul import DEFAULT_SOUL_MD
        _seed_file_if_missing(profile_dir / "SOUL.md", DEFAULT_SOUL_MD)

    # Opt-out marker read by seed_profile_skills() and `cryozen update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`cryozen profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `cryozen profile describe` works later
...
def _notify_multiplexer(canon: str) -> None:
    from cryozen_cli.gateway_multiplex_served import notify_multiplexer_profiles_changed
...
def _purge_identity(canon: str) -> bool:
    ...
    from cryozen_cli.profile_identity import purge_profile_identity
...
def _live_default_multiplexer() -> bool:
    ...
    try:
        from cryozen_cli.gateway_multiplex_served import recorded_served_profiles
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches CRYOZEN_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "CRYOZEN_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_cryozen_home() / ".env"
...
_CRYOZEN_ARGV_MARKERS = ("cryozen_cli.main", "cryozen-gateway", "tui_gateway")
...
# `findOnPath('cryozen')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "cryozen".
...
# argv[1] is matched against exact names, not ``startswith("cryozen")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("cryozen-notes.py").
_CRYOZEN_CONSOLE_SCRIPT_NAMES = frozenset({"cryozen", "cryozen-agent", "cryozen-acp"})


def _is_cryozen_argv(argv: list) -> bool:
    """True for a Cryozen process: entrypoint marker in argv, executable named ``cryozen*``,
    or a python interpreter directly exec'ing a known ``cryozen`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _CRYOZEN_ARGV_MARKERS) or exe_name.startswith("cryozen"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _CRYOZEN_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Cryozen *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`cryozen -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_cryozen_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by CRYOZEN_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("CRYOZEN_HOME", "")
...
class ProfileIdentitySettlementPending(RuntimeError):
    ...
    settled (``cryozen_cli.profile_identity.purge_profile_identity`` returned False).

    Subclasses ``RuntimeError`` so delete-failure handling that treats the error as fatal (the
    CLI's ``cryozen profile delete``) keeps working unchanged; surfaces that can report a partial
    ...
    def __init__(self, profile: str, path: Path):
        ...
        self.path = path
        self.retry_command = f"cryozen profile purge-identity {profile}"
        super().__init__(
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.cryozen-agent).\nTo remove everything, use: cryozen uninstall")
    ...
    from cryozen_constants import cryozen_home_key
    from tools.mcp_tool_lifecycle import shutdown_mcp_servers
    shutdown_mcp_servers(scope=cryozen_home_key(profile_dir))
    ...
    with contextlib.suppress(Exception):
        from cryozen_state_registry import close_all_under as _close_session_dbs_under
    ...
    with contextlib.suppress(Exception):
        from cryozen_logging import release_profile_log_handlers
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``CRYOZEN_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # CRYOZEN_HOME is set temporarily so _profile_suffix resolves the service name.
    old_home = os.environ.get("CRYOZEN_HOME")
    try:
        os.environ["CRYOZEN_HOME"] = str(profile_dir)
        from cryozen_cli.gateway import get_service_name, get_launchd_plist_path, user_systemd_unit_dir
        ...
        os.environ.pop("CRYOZEN_HOME", None)
        if old_home is not None:
            os.environ["CRYOZEN_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's cryozen_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded CRYOZEN_HOME "
...
def get_active_profile(root: Path | None = None) -> str:
    """Read the sticky active profile name (of *root*, default: this process's Cryozen root)."""
...
def get_active_profile_name() -> str:
    """Profile name inferred from CRYOZEN_HOME: ``"default"`` when unset or ``~/.cryozen-agent``, the
    name under ``~/.cryozen-agent/profiles/<name>``, ``"custom"`` for any other path."""
    from cryozen_constants import get_cryozen_home
    resolved = get_cryozen_home().resolve()
    if resolved == _get_default_cryozen_home().resolve():
        return "default"
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when CRYOZEN_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_cryozen_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point CRYOZEN_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/cryozen-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".cryozen-profile-exports", Path(tempfile.gettempdir()) / f"cryozen-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    CRYOZEN_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-Cryozen file the user may have lying alongside CRYOZEN_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``cryozen
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.cryozen-agent (dir name ".cryozen-agent"), so both paths stage a filtered
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: cryozen profile import <archive> --name <name>"
        )
    ...
    # target ~/.cryozen-agent itself.
    canon = _canon_valid(inferred_name)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.cryozen-agent). "
            "Specify a different name: cryozen profile import <archive> --name <name>"
        )
    profile_dir = get_profile_dir(canon)
    if profile_dir.exists():
        raise _profile_exists_error(canon)
    _get_profiles_root().mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="cryozen_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"cryozen_{old_name}"
    legacy_old_host = f"cryozen.{old_name}"
    new_host = f"cryozen_{new_name}"
    candidates = [
        new_dir / "honcho.json", _get_default_cryozen_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``cryozen_<old>`` or legacy ``cryozen.<old>``
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    # ``mkdir_under_cryozen_home`` does not refuse it) and the periodic reconcile re-adopts the
    ...
    from cryozen_constants import cryozen_home_key
    from tools.mcp_tool_lifecycle import shutdown_mcp_servers
    shutdown_mcp_servers(scope=cryozen_home_key(old_dir))
    ...
    from cryozen_cli.profile_identity import _migrate_profile_identity
...
def profile_root_for_env_home(env_home: str, default_root: Path) -> Path:
    """Cryozen root named by an exported ``CRYOZEN_HOME``: the grandparent of a profile-shaped value
    (``<root>/profiles/<name>``, mirrors ``get_default_cryozen_root()``), the value itself otherwise,
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a CRYOZEN_HOME path string. Called early in the CLI entry
    point, before cryozen modules are imported, to set CRYOZEN_HOME.

    When CRYOZEN_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match Cryozen-owned PYTHONPATH
    ...
    root = profile_root_for_env_home(os.environ.get("CRYOZEN_HOME", ""), _get_default_cryozen_home())
    if canon == "default":
        return str(root)
