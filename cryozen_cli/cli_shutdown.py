...
def _exit_watchdog_timeout() -> float:
    """``CRYOZEN_EXIT_WATCHDOG_S`` as a float (default 30; ``0`` disables)."""
    from cli import _float_env
    return _float_env("CRYOZEN_EXIT_WATCHDOG_S", 30.0)
...
def _arm_exit_watchdog(timeout_s: float | None = None, *, from_signal: bool = False) -> None:
    ...
    timer survives ``Py_FinalizeEx``'s joins. ``CRYOZEN_EXIT_WATCHDOG_S=0`` disables.
