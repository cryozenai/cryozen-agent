"""Multi-provider authentication system for Cryozen Agent.

- ``ProviderConfig`` / ``PROVIDER_REGISTRY`` describe every known inference provider.
- The auth store (``~/.cryozen-agent/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ``auth_codex``/``auth_xai``/``auth_qwen``/``auth_minimax``/``auth_spotify``/``auth_openrouter`` and are
  re-imported here so ``cryozen_cli.auth.<name>`` stays the public/patchable surface."""
...
from cryozen_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
from cryozen_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from cryozen_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from cryozen_cli.auth_device_flow import (  # noqa: F401  re-exported
    _can_open_graphical_browser, _default_verify, _is_remote_session,
    _offer_existing_oauth_credentials, _poll_device_token_generic,
    _print_device_code_instructions, _print_login_success, _print_loopback_ssh_hint,
    _prompt_yes_no, _resolve_verify, _ssh_user_at_host)
from cryozen_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from cryozen_cli.auth_minimax import (  # noqa: F401  re-exported
...
from cryozen_cli.auth_xai import (  # noqa: F401  re-exported
...
from cryozen_cli.auth_codex import (  # noqa: F401  re-exported
...
from cryozen_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from cryozen_cli.auth_openrouter import _openrouter_pkce_login  # noqa: F401  re-exported
from cryozen_cli.auth_qwen import (  # noqa: F401  re-exported
...
from cryozen_cli.auth_constants import (  # noqa: F401  re-exported
    _decode_jwt_claims, AUTH_STORE_VERSION, AUTH_LOCK_TIMEOUT_SECONDS,
    ACCESS_TOKEN_REFRESH_SKEW_SECONDS, DEFAULT_CODEX_BASE_URL,
    ...
    ACTUAL_LOCAL_NOAUTH_PLACEHOLDER, CODEX_RATE_LIMITED_CODE, AuthError, httpx)
...
# ``cryozen_cli.config`` discovers model-provider plugins while importing, and a plugin may read this
...
from cryozen_cli.config import (  # noqa: E402
    atomic_config_write, get_cryozen_home, get_config_path, read_raw_config, require_readable_config_before_write)
...
from cryozen_cli.auth_plugin_providers import (  # noqa: E402
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.cryozen-agent/.env`` so a deliberate
    key rotation isn't shadowed by a stale shell export (matches the api-key resolution path — see #20591).
    """
    from cryozen_cli.config import get_env_value_prefer_dotenv
...
def _model_level_key_env(provider_id: str) -> str:
    ...
    (``model.key_env`` → ``$CRYOZEN_HOME/.env``) instead of the registry's canonical env var,
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from cryozen_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    # Prefer ~/.cryozen-agent/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
    ...
    # model.key_env → $CRYOZEN_HOME/.env (e.g. CRYOZEN_CUSTOM_LMSTUDIO_API_KEY) while keeping
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``cryozen auth``."""
...
# Entitlement failures get a fixed generic message (or the raw error when no generic text exists
# for the code).
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        # Profile-aware: a bare `cryozen model` from a named profile re-signs the ROOT store (#114012).
        from cryozen_constants import profile_cli_selector

        return f"{error} Run `cryozen {profile_cli_selector()}model` to re-authenticate."
...
# ── Auth Store — persistence layer for ~/.cryozen-agent/auth.json ──────────────────────────────────────────

def _auth_file_path() -> Path:
    path = get_cryozen_home() / "auth.json"
    # Seat belt: under pytest, refuse to touch the real user's auth store (tests that forgot to
    # monkeypatch CRYOZEN_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".cryozen-agent" / "auth.json")):
        raise RuntimeError(
            f"Refusing to touch real user auth store during test run: {path}. "
            "Set CRYOZEN_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_cryozen_root()
    except Exception:
        return None
    return None if _same_path(get_cryozen_home(), global_root) else global_root / "auth.json"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST") and os.environ.get("HOME"):
        real_root = Path(os.environ["HOME"]) / ".cryozen-agent" / "auth.json"
        try:
...
def _file_lock(
    ...
    supply their own ``threading.local`` so independent locks (profile store vs global root) track
    reentrancy separately."""
...
def _auth_store_lock(
    ...
    reentrancy tracker and kernel lock."""
...
def _save_private_json(target: Path, data: Any, *, fsync_dir: bool = False, **dump_kwargs: Any) -> None:
    ...
    mkdir_under_cryozen_home(target.parent)
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``cryozen auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    when the profile has zero entries for it (``cryozen auth add`` in the profile shadows global)."""
...
def _merge_disk_cooldown_state(
    ...
    ``cryozen auth reset`` that postdates the snapshot's cooldown (``status_cleared_at`` newer than
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``cryozen auth reset``) and skip the
...
def _config_selects_provider(normalized: str) -> bool:
    ...
    Anthropic credentials for that slot even when the main model is another provider."""
    from cryozen_cli.config import load_config
    cfg = load_config()
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT Cryozen flows (manual add / device-code / PKCE) or live env keys;
    ambient borrowed sources (gh_cli / qwen-cli) are deliberately excluded."""
    return any(_pool_entry_is_explicit(entry) for entry in read_credential_pool(normalized))


_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "manual"})
...
def _env_secret(name: str) -> bool:
    ...
    Must not read raw ``os.getenv``: under ``cryozen serve`` / Desktop multiplex the
    ...
    CRYOZEN_HOME ``.env`` first, then the scope-checked environ) so the gate and the key that
...
def _explicit_env_credentials_present(normalized: str) -> bool:
    ...
    if pconfig is None:
        from cryozen_cli.providers import get_provider
        pconfig = get_provider(normalized)
        if not pconfig:
            return False
    if pconfig.auth_type == "api_key":
        return any(_env_secret(v) for v in pconfig.api_key_env_vars)
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit Cryozen flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when Cryozen-scoped routing config is present.
    ...
    Cryozen-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    Cryozen-initiated flow, or Cryozen-scoped routing config for keyless cloud-SDK providers. Ambient
    borrowed credentials (gh CLI, qwen-cli) never count."""
    normalized = (provider_id or "").strip().lower()
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``cryozen logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    if str(provider_name or "").strip().lower() in {"opencode-free", "free", "opencode_free"}:
        return ("OpenCode discontinued anonymous free-tier access outside its own client "
                ...
                "($10/mo subscription, OPENCODE_GO_API_KEY) via 'cryozen model'.")
    try:
        from cryozen_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""
        lines = ["Config issue detected — run 'cryozen doctor' for full diagnostics:"]
        for ci in issues:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from cryozen_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run cryozen setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `cryozen auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `cryozen auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `cryozen auth list` showing the credential while requests go out
...
def _config_model_provider() -> Tuple[Any, Optional[str]]:
    ...
    endpoint is explicit intent like any registry pin: without this rung the boot inventory read a
    llama.cpp/vLLM install as "nothing configured" and the dashboard's Ink chat parked every session
    on Setup Required while ``cryozen chat`` worked (#108383)."""
    try:
        from cryozen_cli.config import load_config
        model_cfg = (load_config() or {}).get("model")
        ...
        if provider:
            from cryozen_cli.runtime_provider_custom import has_named_custom_provider
        ...
        if base_url:
            from cryozen_cli.runtime_provider import _config_base_url_trustworthy_for_bare_custom
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.cryozen-agent/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.cryozen-agent/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    "openrouter"; 4. OpenRouter pool; 5. provider env keys (GLM, Kimi, MiniMax, ...) -> that
    provider; 6. auth.json ``active_provider``; 7. AWS Bedrock chain;
    8. AuthError(no_provider_configured). See #29285.
    """
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'cryozen model' for available providers, "
                "or run 'cryozen doctor' to diagnose config issues.")
    ...
    # below every rung of explicit user intent.
    try:
    ...
    raise AuthError(
        "Cryozen is not connected to any AI provider yet. Run `cryozen model` or `cryozen setup` "
        "to pick a provider and add its API key, or add a key with "
        f"`cryozen auth add <provider>`. (Advanced: put an API key such as OPENROUTER_API_KEY in "
        f"{display_cryozen_home()}/.env.)",
...
def _quarantine_flat_oauth_state(state: Dict[str, Any], provider: str, exc: "AuthError") -> None:
    ...
    subsequent calls fail fast without a network retry (mirrors the xAI / Codex pattern)."""
...
# mtime-keyed memo for _load_global_auth_store(): (path, mtime_ns, store). Keyed on auth.json path +
# mtime so profile switches don't share a memo and login/logout/add/remove invalidate naturally.
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("cryozen_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``cryozen logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
def _pool_first_oauth_status(
    ...
    Pool first (where `cryozen auth` / `cryozen model` store device_code tokens), then
...
def _provider_env_base_url(pconfig: ProviderConfig) -> str:
    if pconfig.id == "actual":
        from cryozen_cli.providers import normalize_provider
...
def _external_process_auth_evidence(provider_id: str, resolved_command: Optional[str]) -> tuple[bool, Optional[str]]:
    ...
    for any external-process profile is its binary resolving: the subprocess owns real auth and Cryozen
...
def _copilot_acp_auth_evidence() -> tuple[bool, Optional[str]]:
    ...
    try:
        from cryozen_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: CRYOZEN_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / CRYOZEN_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``cryozen_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``cryozen doctor`` runs the live probe). ``api_key`` (default): usable
    ``AZURE_FOUNDRY_API_KEY``."""
    info: Dict[str, Any] = {"provider": "azure-foundry"}
    try:
        from cryozen_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            info.update(
                ...
                hint=(
                    ...
                    "is skipped here. Run `cryozen doctor` to verify token acquisition."
                ) if installed else (
                    "azure-identity not installed. Install with: "
                    "pip install azure-identity  (or rely on Cryozen' "
...
def _update_config_for_provider(
    ...
    *default_model*, when given, is written as ``model.default`` in the same step so the gateway
    ...
    from cryozen_cli.config import clear_model_endpoint_credentials
...
def login_command(args) -> None:
    """Deprecated: use 'cryozen model' or 'cryozen setup' instead."""
    print("The 'cryozen login' command has been removed.\nUse 'cryozen auth' to manage credentials,\n"
          "'cryozen model' to select a provider, or 'cryozen setup' for full setup.")
    raise SystemExit(0)
...
def logout_command(args) -> None:
    ...
    if not should_reset_config:
        ...
        print("Cryozen will use OpenRouter for inference.")
    else:
        print("Run `cryozen model` or configure an API key to use Cryozen.")
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('cryozen_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('cryozen_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('cryozen_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('cryozen_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('cryozen_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),

    'OAUTH_OVER_SSH_DOCS_URL': ('cryozen_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('cryozen_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('cryozen_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('cryozen_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('cryozen_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('cryozen_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('cryozen_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('cryozen_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
