"""Per-provider model-selection wizard flows for ``cryozen setup`` / ``cryozen model``.

main / config / auth / models helpers are imported lazily inside bodies: avoids the main.py import
cycle and lets tests patch ``cryozen_cli.config.load_config`` etc. at call time. The shared skeleton
lives in :mod:`cryozen_cli.model_setup_flows_common`; the custom / Azure / Bedrock flows live in their
...
from cryozen_cli.model_setup_flows_common import (
...
from cryozen_cli.model_setup_flows_custom import _model_flow_custom, _model_flow_named_custom
from cryozen_cli.model_setup_flows_azure import _model_flow_azure_foundry
from cryozen_cli.model_setup_flows_bedrock import _model_flow_bedrock


def _env_base_url(base_url_env: str) -> str:
    """Base-URL override from ``.env`` then the process environment ('' when unset)."""
    from cryozen_cli.config import get_env_value
    if not base_url_env:
        return ""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from cryozen_constants import OPENROUTER_BASE_URL
    from cryozen_cli.auth import ProviderConfig, _prompt_model_selection
    ...
    from cryozen_cli.models import model_ids
    from cryozen_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from cryozen_cli.main_provider_setup import _prompt_api_key
    from cryozen_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection
    from cryozen_cli.config import get_env_value
    ...
    from cryozen_cli.models import ai_gateway_model_ids
    from cryozen_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    from cryozen_cli.auth import _save_model_choice
    from cryozen_cli.moa_config import normalize_moa_config
    moa = normalize_moa_config(config.get("moa") if isinstance(config, dict) else {})
    presets = moa.get("presets") or {}
    if not presets:
        print("No MoA presets configured. Run `cryozen moa configure <name>` first.")
        return
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    # Prefer the credential pool (where `cryozen auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from cryozen_cli.models import provider_model_ids
    ...
    # credentials may live only in the pool (``cryozen auth add xai-oauth``) — fall back to
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from cryozen_cli.main_provider_setup import _DEFAULT_QWEN_PORTAL_MODELS
    from cryozen_cli.auth import (
        get_qwen_auth_status, resolve_qwen_runtime_credentials, _prompt_model_selection, DEFAULT_QWEN_BASE_URL)
    from cryozen_cli.models import fetch_api_models
...
def _copilot_catalog(api_key: str):
    ...
    from cryozen_cli.models import fetch_github_model_catalog, normalize_copilot_model_id
...
def _copilot_obtain_token() -> bool:
    ...
    if choice == "1":
        try:
            from cryozen_cli.copilot_auth import copilot_device_code_login
    ...
    if choice == "2":
        ...
        with contextlib.suppress(ImportError):
            from cryozen_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot(config, current_model=""):
    ...
    from cryozen_cli.models import fetch_api_models, copilot_model_api_mode
    provider_id = "copilot"
    ...
    if not api_key:
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from cryozen_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    _say("  GitHub Copilot ACP delegates Cryozen turns to `copilot --acp`.",
         "  Cryozen currently starts its own ACP subprocess for each request.",
         "  Cryozen uses your selected model as a hint for the Copilot ACP session.",
         f"  Command: {resolved_command}", f"  Backend marker: {effective_base}", "")
    try:
        creds = resolve_external_process_provider_credentials(provider_id)
    except Exception as exc:
        _say(f"  ⚠ {exc}", "  Set CRYOZEN_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from cryozen_cli.auth import PROVIDER_REGISTRY, KIMI_CODE_BASE_URL
    from cryozen_cli.config import get_env_value, save_env_value
    from cryozen_cli.models import _PROVIDER_MODELS
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from cryozen_cli.main_provider_setup import _infer_stepfun_region, _prompt_provider_choice, _stepfun_base_url_for_region
    from cryozen_cli.auth import PROVIDER_REGISTRY
    from cryozen_cli.config import save_env_value
    from cryozen_cli.models import _PROVIDER_MODELS, fetch_api_models
    provider_id = "stepfun"
    pconfig = PROVIDER_REGISTRY[provider_id]
    base_url_env = pconfig.base_url_env_var or ""
...
def _model_flow_vertex(config, current_model=""):
    ...
    from cryozen_cli.auth import _prompt_model_selection
    from cryozen_cli.config import load_config, get_env_value
    from cryozen_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        _say("  Vertex credentials: Application Default Credentials (ADC)",
             ...
             "      • set VERTEX_CREDENTIALS_PATH in ~/.cryozen-agent/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    """Picker for the official Z.AI endpoints (``ZAI_ENDPOINTS`` in ``cryozen_cli.auth``, kept in
    sync with the probe list) plus a custom-proxy option. Returns the selected base URL;
    *current_base* on cancel/error."""
    from cryozen_cli.main_provider_setup import _prompt_provider_choice
    from cryozen_cli.auth import ZAI_ENDPOINTS
...
_GEMINI_FREE_TIER_NOTICE = (
    ...
    "   Cryozen typically makes 3-10 API calls per user turn (tool iterations + auxiliary tasks),",
    "   so the free tier is exhausted after a handful of messages and cannot sustain",
    "   an agent session.", "",
    "   To use Gemini with Cryozen, enable billing on your Google Cloud project and regenerate",
    "   the key in a billing-enabled project: https://aistudio.google.com/apikey", "",
    "   Alternatives with workable free usage: DeepSeek, OpenRouter (free models), Groq.", "",
...
def _api_key_provider_model_list(provider_id: str, pconfig, existing_key: str, key_env: str, effective_base: str) -> list:
    ...
    from cryozen_cli.config import get_env_value
    from cryozen_cli.models import _PROVIDER_MODELS, fetch_api_models, probe_profile_catalog
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from cryozen_cli.auth import PROVIDER_REGISTRY
    from cryozen_cli.config import save_env_value, load_config
    from cryozen_cli.models import opencode_model_api_mode, normalize_opencode_model_id
    pconfig = PROVIDER_REGISTRY[provider_id]
...
def _anthropic_authenticate() -> bool:
    """Interactive Anthropic API-key entry. False = flow must stop."""
    from agent.anthropic_credentials import is_consumer_oauth_token
    from cryozen_cli.config import save_env_value, save_anthropic_api_key
    ...
    if is_consumer_oauth_token(api_key):
        print("  That is a Claude subscription OAuth token, not an API key; Cryozen does not use it.")
        return False
...
def _model_flow_anthropic(config, current_model=""):
    """Flow for Anthropic provider (API key)."""
    from agent.anthropic_credentials import is_consumer_oauth_token
    from cryozen_cli.auth import get_anthropic_key
    from cryozen_cli.models import _PROVIDER_MODELS

    existing_key = get_anthropic_key()
    # A leftover subscription OAuth token counts as missing so the API-key prompt is offered.
    has_creds = bool(existing_key) and not is_consumer_oauth_token(existing_key)
...
# ``_model_flow_api_key_provider``; ``cryozen_cli.main`` routes every registered profile missing
...
def _plugin_flow_external_process(provider_id: str, profile) -> tuple[str, str] | None:
    from cryozen_cli.auth import get_external_process_provider_status, resolve_external_process_provider_credentials
    status = get_external_process_provider_status(provider_id)
    _say(f"  {profile.display_name or provider_id} delegates Cryozen turns to a local `{status.get('command') or profile.process_command}` process.",
...
def _plugin_flow_oauth(provider_id: str, profile) -> tuple[str, str] | None:
    ...
    from cryozen_cli.auth_plugin_providers import plugin_missing_auth_handler_error
    ...
    if not status.get("logged_in"):
        ...
        _say(f"  ⚠ Not signed in to {profile.display_name or provider_id}.",
             f"  {missing.code if missing else status.get('hint') or f'Run `cryozen auth add {provider_id}` to sign in.'}")
        return None
...
def _is_profile_plugin_flow_provider(provider_id: str) -> bool:
    ...
    from cryozen_cli.auth_plugin_providers import plugin_profile
...
def _model_flow_plugin_provider(config, provider_id, current_model=""):
    """Generic ``cryozen model`` flow for a registered plugin profile: credential step by auth_type,
    catalog via ``merge_profile_catalog`` (live ``fetch_models()`` ⊕ ``fallback_models``), persist."""
    from cryozen_cli.auth_plugin_providers import plugin_profile
    from cryozen_cli.models import merge_profile_catalog
    del config
    profile = plugin_profile(provider_id)
...
_PLUGIN_COMPAT_LAZY = {
    'BEDROCK_GEO_PREFIXES': ('cryozen_cli.model_setup_flows_bedrock', 'BEDROCK_GEO_PREFIXES'),
    'bedrock_model_routable_from_region': ('cryozen_cli.model_setup_flows_bedrock', 'bedrock_model_routable_from_region'),
    'bedrock_region_geo_prefix': ('cryozen_cli.model_setup_flows_bedrock', 'bedrock_region_geo_prefix'),
    'custom_provider_slug': ('cryozen_cli.providers', 'custom_provider_slug'),
