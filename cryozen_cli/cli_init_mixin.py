"""CryozenCLI constructor phases: display options, model/provider routing, turn limits, toolsets, checkpoints, prompt/reasoning, runtime state, session store and UI state.

Mixin split out of ``cli.py``; bound onto ``CryozenCLI`` via the MRO.
"""
...
class CLIInitMixin:
    ...
    def _init_display_options(self, verbose, compact):
        ...
        if self._focus_view_enabled:
            from cryozen_cli.focus_view import FOCUS_TOOL_PROGRESS_MODE, normalize_tool_progress_mode
    ...
    def _init_model_and_provider(self, model, provider, api_key, base_url):
        ...
        _cfg_provider = _model_config.get("provider") or os.getenv("CRYOZEN_INFERENCE_PROVIDER")
    ...
    def _init_turn_limits(self, max_turns, run_budget):
        ...
        self.max_turns = _resolve_turn_limit(next(
            ...
            os.getenv("CRYOZEN_MAX_ITERATIONS"),
        ))
    ...
    def _init_checkpoints_and_rules(self, checkpoints, pass_session_id, ignore_rules):
        ...
        self.ignore_rules = ignore_rules or is_truthy_value(os.environ.get("CRYOZEN_IGNORE_RULES"))
    ...
    def _init_prompt_and_reasoning(self, reasoning):
        ...
        # Env var wins, then cryozen_cli.personality (single owner of overlay resolution).
        from cryozen_cli.personality import available_personalities, resolve_ephemeral_system_prompt

        self.system_prompt = os.getenv("CRYOZEN_EPHEMERAL_SYSTEM_PROMPT", "") or resolve_ephemeral_system_prompt(CLI_CONFIG)
        ...
        # — resolved through the shared chokepoint in cryozen_constants (Closes #21256).
    ...
    def _init_runtime_state(self, resume):
        ...
        self._history_file = _cryozen_home / ".cryozen_history"
    ...
    def _init_ui_state(self):
        ...
        try:  # composer placeholder chosen once so it stays stable on screen
            from cryozen_cli.tips import get_random_composer_placeholder
        ...
        from cryozen_cli.prompt_stash import PromptStash as _PromptStash
