...
# sentence per toolset instead. Provider names must exist under plugins/web/.
_MULTI_PROVIDER_NOTICES: dict[str, str] = {
    "web": ("[yellow]⚠ Web search is off[/] — no search provider is set up yet (any one of Exa, "
            "Tavily, Firecrawl, Brave, or free DuckDuckGo works). Run [bold]cryozen setup tools[/] and set one up under "
            "\"Web Search & Scraping\"."),
}

_GENERIC_FOOTER = "[dim]   Run 'cryozen setup tools' to configure[/]"
...
def filter_to_enabled_toolsets(unavailable: list[dict], enabled: Iterable[str],
    ...
    composite bundle such as ``["cryozen-cli"]``, never the individual names ``check_tool_availability``
...
def _terminal_line(backend: str, reason: Optional[str]) -> str:
    ...
    return (f"[yellow]⚠ Terminal tool disabled:[/] the '{backend}' backend is not usable{detail}. "
            "Run [bold]cryozen doctor[/] for details, or [bold]cryozen setup terminal[/] to pick another backend.")
