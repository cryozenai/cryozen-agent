"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (cryozen gui/desktop).

Split out of ``cryozen_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
from cryozen_cli.desktop_console import desktop_console_output, desktop_launch_notice
from cryozen_platform.host import facts
from cryozen_cli.main_tui_launch import _npm_lifecycle_env
from cryozen_cli.main_web_build import (
    _hash_source_tree, _nixos_build_env, _stamp_is_current, _write_build_stamp)

# Log-record parity with the origin module.
logger = logging.getLogger("cryozen_cli.main")
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $CRYOZEN_HOME."""
    from cryozen_constants import get_cryozen_home
    return get_cryozen_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Cryozen.app/Contents/MacOS/Cryozen → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Cryozen.app/Contents/MacOS/Cryozen"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / "Cryozen.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("cryozen", "Cryozen")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Cryozen.exe to the launcher. Prefer
        ...
        # wrong-architecture Cryozen.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``Cryozen.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``cryozen update`` used to report
...
def _swap_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Optional[Path]:
    ...
    try:
        ...
        if moved_aside:
            ...
            # never exited at all (a manual `cryozen update`/`cryozen desktop` run does not
...
# (Desktop → cryozen-setup --update → `cryozen update` → `cryozen desktop --build-only` → relaunch) rebuilds
# Cryozen.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Cryozen.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built Cryozen.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Cryozen.exe from backup.")
        print("    Your existing version was kept and still works. Run `cryozen desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `cryozen desktop --force-build` to rebuild, or re-run the Cryozen")
...
def _redownload_electron_dist(project_root: Path, env: dict, *, mirror: Optional[str] = None) -> bool:
    ...
    if not installer.is_file():
        return False
    from cryozen_constants import find_node_executable, with_cryozen_node_path
    ...
    dl_env = with_cryozen_node_path(env)
    if mirror:
...
def _desktop_macos_local_codesign(app: Path, *, desktop_dir: Path, identity: str = "-") -> bool:
    ...
    #    the outer Cryozen.app component.
...
def _desktop_macos_relaunchable_fixup(
    ...
    "Cryozen is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../Cryozen.app/Contents/MacOS/Cryozen  ->  app bundle = .../Cryozen.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="cryozen-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:cryozenlocal",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "Cryozen Local Signing") -> bool:
    ...
    from cryozen_cli.main import PROJECT_ROOT
    if sys.platform != "darwin":
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.cryozen.cryozen"
    )
    return True
...
def _running_macos_app_bundles() -> set[Path]:
    """``.app`` bundles of every live Cryozen Desktop process. A running bundle is never swapped
    ...
    for proc in psutil.process_iter(["exe"]):
        ...
        if exe.endswith("/Contents/MacOS/Cryozen"):
...
def _install_rebuilt_desktop_app(desktop_dir: Path) -> tuple[list[Path], list[str]]:
    """Copy the rebuilt macOS bundle over every stale installed ``Cryozen.app`` (#52339).

    ``cryozen desktop --build-only`` (what ``cryozen update`` runs) packages into
    ...
    if rebuilt_exe is None:
        return [], []
    from cryozen_cli.gui_uninstall import packaged_gui_app_paths  # noqa: PLC0415
    # .../Cryozen.app/Contents/MacOS/Cryozen -> .../Cryozen.app
...
def _install_rebuilt_macos_bundles(
    ...
    for app in candidates:
        ...
        if app.resolve() in running:
            problems.append(
                f"{app} is running and was not refreshed; quit Cryozen Desktop and run "
                "`cryozen update` again (or update from inside the app)")
            continue
        tmp = app.parent / f"{app.name}.cryozen-update-new"
        old = app.parent / f"{app.name}.cryozen-update-old"
        shutil.rmtree(tmp, ignore_errors=True)
        shutil.rmtree(old, ignore_errors=True)
        try:
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Cryozen Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Cryozen Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry(defer: bool = False):
    """Install the XDG desktop entry for Cryozen Desktop (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``cryozen uninstall --gui`` removes it.
    ...
    from cryozen_cli.main import PROJECT_ROOT
    try:
        from cryozen_cli.linux_desktop_entry import DeferredDesktopEntryInstall, install_desktop_entry, is_supported
        if not is_supported():
            return None
        if defer:
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from cryozen_cli.main import PROJECT_ROOT
    from cryozen_cli.main_web_build import _run_npm_install_deterministic
    from cryozen_cli.update_cmd_deps import (
        DESKTOP_NPM_SCOPE, _clear_npm_lockfile_hash, _desktop_deps_changed, _record_npm_lockfile_hash)
    from cryozen_constants import get_default_cryozen_root, with_cryozen_node_path
    cryozen_root = get_default_cryozen_root()
    if not _desktop_deps_changed(cryozen_root) and (_electron_dir(PROJECT_ROOT) / "package.json").is_file():
        print("→ Desktop workspace dependencies unchanged, skipping install")
        return
    print("→ Installing desktop workspace dependencies...")
    _clear_npm_lockfile_hash(cryozen_root, DESKTOP_NPM_SCOPE)
    ...
    nixos_env = with_cryozen_node_path(_nixos_build_env())
    install_result = _run_npm_install_deterministic(npm, PROJECT_ROOT, capture_output=False, env=nixos_env)
    if install_result.returncode == 0:
        _record_npm_lockfile_hash(cryozen_root, DESKTOP_NPM_SCOPE)
        return
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Cryozen.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # Cryozen.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds Cryozen.exe locked on Windows, so the
    ...
    if build_result.returncode != 0:
        ...
        if sys.platform == "win32":
            print("  If this says \"Access is denied\" on Cryozen.exe, close any")
            print("  running Cryozen desktop window and retry.")
        print("  If the log shows Electron download retries, rebuild via a mirror:")
        print("    ELECTRON_MIRROR=<mirror-base-url> cryozen desktop --force-build")
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    # with_cryozen_node_path() copies os.environ when called with no arg.
    env = with_cryozen_node_path()
    _prefer_wsl_d3d12(env)
    for attr, key in (
        ("fake_boot", "CRYOZEN_DESKTOP_BOOT_FAKE"), ("ignore_existing", "CRYOZEN_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "cryozen_root", None):
        env["CRYOZEN_DESKTOP_CRYOZEN_ROOT"] = str(Path(args.cryozen_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["CRYOZEN_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "CRYOZEN_DESKTOP_DISABLE_GPU" not in os.environ:
        env["CRYOZEN_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "CRYOZEN_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["CRYOZEN_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from cryozen_cli.main import PROJECT_ROOT
    from cryozen_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Cryozen Local Signing"
    ...
    if source_mode or needs_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  cryozen gui")
            sys.exit(1)
    ...
    from cryozen_cli.linux_desktop_entry import launched_from_shell
    ...
    if source_mode:
        print("→ Launching Cryozen Desktop from source build...")
    ...
    if not source_mode:
        desktop_launch_notice(f"→ Launching packaged Cryozen Desktop: {' '.join(launch_command)}")
    pass_fds: tuple[int, ...] = ()
