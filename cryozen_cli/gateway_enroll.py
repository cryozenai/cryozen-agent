"""``cryozen gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            message = (
                "Connector rejected the caller identity (401). The identity token from "
                "gateway.idp could not be verified; check the IdP settings and retry."
            )
        elif exc.code == 403:
...
def cmd_gateway_enroll(args) -> None:
    ...
    from cryozen_cli.config import is_managed, save_env_value

    # Managed installs get GATEWAY_RELAY_* stamped in by the orchestrator; save_env_value refuses
    # to write there anyway.
    if is_managed():
        _fail(
            "✗ `cryozen gateway enroll` is not available in a managed/hosted install.\n"
    ...
    # is set, or an ambient token endpoint.
    try:
    ...
    # Persist idempotently; save_env_value writes the sensitive values to ~/.cryozen-agent/.env and never
    ...
    from cryozen_cli.config import get_env_path
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    try:
        ...
        default_root = Path(get_default_cryozen_root()).resolve()
