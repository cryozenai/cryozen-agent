"""Helpers for loading Cryozen .env files consistently across entrypoints."""
...
# Once-per-process guards: load_cryozen_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `cryozen
...
# CRYOZEN_HOME paths already pulled external secrets for: load_cryozen_dotenv() runs at import time from
...
# a link-style launcher spawns `cryozen dashboard` with a fresh CRYOZEN_DASHBOARD_SESSION_TOKEN and keeps
# the same token for its own /api probes). They are never .env configuration, so a persisted value in
# ~/.cryozen-agent/.env must not replace an injected one — the parent would then 401 against its own child
# (#115955). A value an earlier dotenv pass published is still reloaded normally.
_SPAWN_CREDENTIAL_KEYS: frozenset[str] = frozenset({"CRYOZEN_DASHBOARD_SESSION_TOKEN"})

# Behavioral routing keys a parent Cryozen process injects into child env that silently redirect a profile
...
def get_secret_source_values(cryozen_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``cryozen_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(cryozen_home).resolve()), {}))


def hydrate_profile_secret_sources(cryozen_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(cryozen_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_cryozen_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
        ...
        local_env["CRYOZEN_HOME"] = str(home)
...
def reset_secret_source_cache(cryozen_home: str | os.PathLike | None = None) -> None:
    """Forget applied homes so the next load re-pulls (tests, long-running processes after config edits).

    ``cryozen_home`` limits the reset to ONE home: a multiplex gateway keeps every profile's snapshot in
    ...
    (#102041)."""
    if cryozen_home is None:
        _APPLIED_HOMES.clear()
        _SECRET_SOURCES.clear()
        _SECRET_SOURCE_VALUES_BY_HOME.clear()
        _SECRET_SOURCE_RESTORE_BY_HOME.clear()
        return
    home_key = str(Path(cryozen_home).resolve())
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `cryozen setup` (or edit the "
...
def _load_dotenv_with_fallback(path: Path, *, override: bool, load_pass: int | None = None) -> None:
    ...
    ``load_pass`` groups the layered files of one ``load_cryozen_dotenv`` call: within a pass a later layer
    (project, managed) still sees the earlier layer's output, as it always did; only OTHER passes' output
    is peeled. A bare call (``cryozen send``'s direct reload) is its own pass."""
    raw = path.read_bytes()
    try:
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from cryozen_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_cryozen_dotenv(
    *,
    cryozen_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Cryozen env files: ``~/.cryozen-agent/.env`` overrides stale shell exports; project ``.env`` is a dev
    ...
    home_path = Path(cryozen_home) if cryozen_home else get_process_cryozen_home()
    ...
    if is_multiplex_active() and get_cryozen_home_override() is not None:
        ...
        if load_external_secrets:
            from cryozen_cli import _early_recovery
    ...
    # Windows (#73381, #86735). 2. A fresh ``cryozen update`` retry just completed a deferred dependency
    ...
    # ~/.cryozen-agent/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.cryozen-agent/.env (e.g. written by an older
    # `cryozen setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_cryozen_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_cryozen_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Cryozen reads credentials; failures never block startup. Precedence/conflicts/provenance
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and cryozen_logging reuse one parse instead of 3-4.
    if home_path == _process_cryozen_home():
        try:
...
def _process_cryozen_home() -> Path:
    """The CRYOZEN_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_cryozen_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``cryozen_constants.get_process_cryozen_home()`` is the override-immune
