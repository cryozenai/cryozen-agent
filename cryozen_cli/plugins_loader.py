...
dependency/config-schema warnings. Mixed into :class:`cryozen_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``cryozen_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from cryozen_constants import get_cryozen_home, reset_cryozen_home_override, set_cryozen_home_override
from registration_lifecycle import replacement_coordinator
from cryozen_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from cryozen_cli.plugins_manifest import PluginManifest, manifest_key, portable_mcp_server_name, validate_config_schema
from cryozen_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from cryozen_cli.plugins import LoadedPlugin, PluginContext

logger = logging.getLogger("cryozen_cli.plugins")

_NS_PARENT = "cryozen_plugins"
...
def _reserve_abandoned_loader_slot() -> None:
    ...
    raise PluginLoadTimeout(
        ...
        f"(plugins.load_timeout_seconds); restart Cryozen to retry"
    )
...
def run_with_load_deadline(plugin_key: str, ctx: "PluginContext", fn: Callable[[], Any]) -> Any:
    ...
    The worker inherits the caller's context (the Cryozen-home override is a ContextVar). On timeout the
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Cryozen home."""
...
class PluginLoaderMixin:
    def on_plugin_loaded(self, callback: Callable[[List[Dict[str, Any]]], Any]) -> Callable[[], None]:
        ...
        just the newcomer after a mid-run ``cryozen plugins install/enable``, Desktop / dashboard /
        ...
        :func:`cryozen_cli.plugins_activation.plugin_activation_summary` for the payload: ``activated_now``
    ...
    def _notify_plugin_loaded(self, loaded_before: frozenset) -> None:
        ...
        if not self._plugin_loaded_listeners:
            return
        from cryozen_cli.plugins_activation import activation_summaries
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``cryozen plugins list`` until then."""
        from cryozen_cli.plugins import LoadedPlugin
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``cryozen tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``cryozen tools`` checklist, and even an explicit
        ...
        """
        from cryozen_cli.plugins import PluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            # Tools registered before the raise are live: credit them or `cryozen plugins list` under-reports
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        ``cryozen plugins install``/``enable`` and after ``cryozen update`` (``cryozen_cli.plugin_python_deps``)
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Run `cryozen plugins enable %s` to install them, "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from cryozen_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        if manifest.portable:
            ...
            return
        # requires_cryozen gate: skip cleanly (no import, no traceback) on a version mismatch.
        from cryozen_cli.plugins_manifest import requires_cryozen_error
        reason = requires_cryozen_error(manifest)
        if reason:
            loaded.error = reason
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (cryozen_cli.plugin_compat).
        from cryozen_cli.plugin_compat import disable_reason
        ...
        try:
            if run_with_load_deadline(plugin_key, ctx, _import_and_register):
                ...
                loaded.enabled = True
                from cryozen_cli.plugins_ledger import _hook_source_of
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from cryozen_cli.plugins import PluginContext
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `cryozen plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        try:
            ...
            package = load_agent_plugin(
                Path(manifest.path), get_cryozen_home() / "plugin-data" / manifest.skill_namespace)
            ctx = PluginContext(manifest, self)
            ...
            from cryozen_cli.agent_plugins import _clear_liveness, _set_liveness
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``cryozen_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``cryozen_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another Cryozen home, or an earlier force
