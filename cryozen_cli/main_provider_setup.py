...
routing menu, API-key/reasoning prompts.

Split out of ``cryozen_cli/main.py``. Names that still live in main are imported lazily at call time.
"""

import contextlib

from typing import Optional
from cryozen_cli.model_setup_flows_common import _ask, _ensure_dict_section, _print_numbered, _radiolist, _say
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.cryozen-agent/.env unless the active provider is 'custom' — a
    leftover value routes provider:auto auxiliary clients to the old custom endpoint."""
    from cryozen_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    (:meth:`cryozen_cli.plugins.PluginContext.register_auxiliary_task`)."""
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        _say("", "  Auxiliary models — side-task routing", "",
             ...
             "  Cryozen only falls back to a lightweight backend (OpenRouter)",
             "  if the main model is unavailable.  Override a",
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    with contextlib.suppress(Exception):
        from cryozen_cli.setup import _curses_prompt_choice
...
_CUSTOM_API_MODES = (
    ("", "Auto-detect", "Use Cryozen URL heuristics; best for standard OpenAI-compatible endpoints.",
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    from cryozen_cli.runtime_provider import _detect_api_mode_for_url
...
def _prompt_main_reasoning_effort(model: str, provider: str) -> None:
    """The effort step every ``cryozen model`` flow shares: after a main-model pick, offer the
    ...
    from cryozen_cli.config import load_config, save_config
    from cryozen_cli.setup import _current_reasoning_effort, _set_reasoning_effort
...
def _main_model_reasoning_efforts(model: str, provider: str) -> Optional[list[str]]:
    ...
    if slug == "copilot":
        from cryozen_cli.models import github_model_reasoning_efforts
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "", existing_source: str = "") -> tuple:
    """API-key entry for ``cryozen setup`` / ``cryozen model``: first-time entry, or [K]eep / [R]eplace /
    ...
    from cryozen_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(f"  API key cleared.  Re-run `cryozen setup` to configure {pconfig.name} again.")
        return "", True
...
def _stepfun_base_url_for_region(region: str) -> str:
    from cryozen_cli.auth import STEPFUN_STEP_PLAN_CN_BASE_URL, STEPFUN_STEP_PLAN_INTL_BASE_URL
...
def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
    ...
    from cryozen_cli.config import get_compatible_custom_providers, read_raw_config
...
def _build_provider_picker_rows(config: dict, active: str, provider_labels: dict[str, str],
    ...
    """Rows for the ``cryozen model`` provider picker plus the pre-selected index. Canonical providers
    ...
    from cryozen_cli.models import CANONICAL_PROVIDERS, _PROVIDER_ALIASES
    from cryozen_cli.models_catalog_static import group_providers, provider_group_for_slug
