...
maps. Split out of ``cryozen_cli.models``.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    synthesis) so the gateway /model picker and the CLI ``cryozen model`` flow share one source."""
    from cryozen_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# Mirrors the xAI-direct IDs from $CRYOZEN_HOME/models_dev_cache.json; the cache overrides it on the
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Cryozen-curated xAI models missing from models.dev, right after the pinned headline."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    """Offline curated floor for xAI / xAI OAuth pickers: $CRYOZEN_HOME/models_dev_cache.json
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # (see cryozen_cli/model_setup_flows.py); validated live against a GCP project (global region).
    "vertex": [
...
# lists, displays, or iterates providers (cryozen model, /model, list_authenticated_providers)
# derives from it. slug = internal ID (config.yaml, --provider); label = short display name;
# tui_desc = longer description for the `cryozen model` picker.
# ---------------------------------------------------------------------------
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [ProviderEntry(*row) for row in (
    ...
    ("anthropic", "Anthropic", "Anthropic (Claude models via API key)"),
...
# ...) fold under one top-level row in the INTERACTIVE PICKERS (``cryozen model``, setup wizard,
...
# Offline/fresh-install fallback for the model Cryozen silently lands on when the user never picked
...
# Network-free (cache-only) on purpose — this is the hot resolution path.
...
_AGGREGATOR_PROVIDERS = frozenset({"openrouter", "ai-gateway", "copilot", "kilocode"})
...
# agentic subset of 400+ models — merging would dump everything), and providers with dedicated live-endpoint
...
# OpenRouter-style ids -> Copilot ids. Dash-notation Claude ids are accepted too: Cryozen' default
