"""User-facing copy for ``cryozen gateway start/stop/restart`` failures on systemd hosts.

``cryozen_cli/gateway.py`` is a facade; this sibling owns the small exception -> guidance table so
...
_JOURNAL_HINT = 'journalctl --user -u cryozen-gateway --since "5 min ago"'

_SYSTEMCTL_FAILED_LINES = (
    "Could not {verb} the gateway service; systemd reported an error.",
    "See why with `cryozen gateway status --deep` or `{journal}`.",
    "To reinstall the service run `cryozen gateway install --force`.",
)

_NO_SYSTEMCTL_LINES = (
    "This system has no systemd, so Cryozen cannot install a background service here.",
    "Run the gateway directly with `cryozen gateway run` (keep it alive with tmux or screen).",
)
