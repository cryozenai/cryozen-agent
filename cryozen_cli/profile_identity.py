...
def migrate_profile_identity(old_name: str, new_name: str) -> bool:
    ...
    ``cryozen profile migrate-identity <old> <new>`` for when that attempt failed. The rename
    ...
    """
    from cryozen_cli.profiles import _canon_valid, _live_default_multiplexer, _unknown_profile_error, get_profile_dir
...
def purge_profile_identity(profile: str) -> bool:
    ...
    """
    from cryozen_cli.profiles import _canon_valid, _live_default_multiplexer, profile_exists
...
def _purge_profile_identity(canon: str, live_mux: bool) -> bool:
    ...
    if live_mux:
        ...
        print(
            ...
            f"    cryozen profile purge-identity {canon}",
            file=sys.stderr)
        return False
...
def _migrate_checkpoint_identity(old_canon: str, new_canon: str) -> bool:
    ...
    print(
        ...
        f"    cryozen profile migrate-identity {old_canon} {new_canon}",
        file=sys.stderr,
    )
    return False
