"""Top-level argparse construction for the cryozen CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``cryozen_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``CRYOZEN_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/cryozen_cli/
...
def _cfg_path() -> str:
    """``~/.cryozen-agent/config.yaml`` spelled for the active profile, for help text.

    ``main._apply_profile_override`` builds this parser (via ``top_level_value_flag_sets``) BEFORE
    it re-homes the process to the sticky ``active_profile``; ``get_cryozen_home()`` would emit the
    "[CRYOZEN_HOME fallback] ... wrong profile" warning on every ``cryozen`` command for that
    throwaway help string. Read the process home directly: after the override it IS the profile home.
    """
    from cryozen_constants import display_cryozen_home, get_process_cryozen_home

    return f"{display_cryozen_home(get_process_cryozen_home())}/config.yaml"
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``cryozen --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``cryozen_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    cryozen                        Start interactive chat
    cryozen chat -q "Hello"        Single query mode
    cryozen --tui                  Launch the modern TUI (or set display.interface: tui)
    cryozen --cli                  Force the classic REPL (overrides display.interface: tui)
    cryozen -c                     Resume the most recent session
    cryozen -c "my project"        Resume a session by name (latest in lineage)
    cryozen --resume <session_id>  Resume a specific session by ID
    cryozen --resume latest        Resume the most recent session (same as -c)
    cryozen --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    cryozen setup                  Run setup wizard
    cryozen logout                 Clear stored authentication
    cryozen auth add <provider>    Add a pooled credential
    cryozen auth list              List pooled credentials
    cryozen auth remove <p> <t>    Remove pooled credential by index, id, or label
    cryozen auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    cryozen auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    cryozen auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    cryozen model                  Select default model
    cryozen fallback [list]        Show fallback provider chain
    cryozen fallback add           Add a fallback provider (same picker as `cryozen model`)
    cryozen fallback remove        Remove a fallback provider from the chain
    cryozen config                 View configuration
    cryozen config edit            Edit config in $EDITOR
    cryozen config set model gpt-4 Set a config value
    cryozen gateway                Run messaging gateway
    cryozen gateway install        Install gateway background service
    cryozen gateway start          Start the installed gateway service
    cryozen gateway stop           Stop the gateway service
    cryozen gateway status         Show gateway status
    cryozen -p <profile> <cmd>     Run any command against a named profile's
                                  home (also --profile) — e.g. cryozen -p coder gateway stop
    cryozen -s cryozen-agent-dev,github-auth
    cryozen -w                     Start in isolated git worktree
    cryozen sessions list          List past sessions
    cryozen sessions browse        Interactive session picker
    cryozen sessions rename ID T   Rename/title a session
    cryozen logs                   View agent.log (last 50 lines)
    cryozen logs -f                Follow agent.log in real time
    cryozen logs errors            View errors.log
    cryozen logs --since 1h        Lines from the last hour
    cryozen debug share             Upload debug report for support
    cryozen console                Open the safe Cryozen command console
    cryozen update                 Update to latest version
    cryozen dashboard              Start web UI dashboard (port 9119)
    cryozen dashboard --stop       Stop running dashboard processes
    cryozen dashboard --status     List running dashboard processes

For more help on a command:
    cryozen <command> --help
"""
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via CRYOZEN_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `cryozen setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to CRYOZEN_ACCEPT_HOOKS=1 or "
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``cryozen -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/cryozen_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Cryozen Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also CRYOZEN_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Cryozen itself.")
...
class CryozenArgumentParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        if isinstance(action, argparse._SubParsersAction) and value not in action.choices:
            # ``self.prog`` is "cryozen" at the top level and "cryozen gateway" for a nested group
...
def build_top_level_parser():
    ...
    """
    parser = CryozenArgumentParser(
        prog="cryozen", description="Cryozen Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_EPILOGUE)
    _add_top_level_flags(parser)
    # metavar keeps the usage line to ``cryozen [...] <command>`` instead of the brace list of
    # every subcommand name; ``cryozen --help`` still lists each command with its help row.
