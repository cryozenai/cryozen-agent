...
from cryozen_cli.web_models import CronJobCreate
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from cryozen_cli.web_server_profiles import _fallback_profile_dicts
...
def _cron_default_profile() -> str:
    ...
    profile's job into ~/.cryozen-agent. ``custom`` (CRYOZEN_HOME outside the profiles tree) has no
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, CRYOZEN_HOME)."""
    from cryozen_cli import profiles as profiles_mod
...
def _cron_store_scope(home: Path):
    """Point CRYOZEN_HOME and the cron.jobs store at one profile's home for the block.
...
def _fire_cron_job_for_profile(profile: str, job_id: str, *, force: bool = False) -> bool:
    ...
    Used by the dashboard trigger path and external callers on the web_deps late-binding seam.
    """
