...
Mixin split out of ``cli.py``; bound onto ``CryozenCLI`` via the MRO. cli.py-internal symbols are
...
from cryozen_constants import is_termux as _is_termux_environment
from rich.markup import escape as _escape
from utils import base_url_hostname, file_signature

from cryozen_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        from cryozen_cli.banner import build_welcome_banner
        self.console.clear()
        ctx_len = None
        ...
        if self.compact or shutil.get_terminal_size().columns < 80:
            ...
            from cryozen_cli.banner import (
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Cryozen needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # `cryozen skills trust`. Never raises.
        try:
            ...
            if _proj_dirs:
                ...
                if _untrusted is not None:
                    ...
                    self._console_print(
                        ...
                        f"loaded — run `cryozen skills trust` to enable them.[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from cryozen_cli.models import model_supports_fast_mode
        except Exception:
            return False
    ...
    def show_help(self, arg: str = ""):
        ...
        from cryozen_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
        ...
        try:
            from cryozen_cli.skin_engine import get_active_help_header
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        (tests/cryozen_cli/test_bang_shell_mode.py). Returns False when the text is not a bang command or
        bang mode is disabled for this context (gateway/cron), so the caller routes normally.
        """
        from cli import _rich_text_from_ansi
        from cryozen_cli.bang_shell import (
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_cryozen_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_cryozen_home()}/config.yaml")
            print()

    def _print_random_tip(self) -> None:
        """Best-effort discovery tip (startup + /clear); never raises."""
        try:
            from cryozen_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from cryozen_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if str(self._agent_or_self("provider") or "").strip().lower() != "openai-codex":
            ...
            print("  Switch with `/model` or `cryozen auth` first.")
            return
    ...
    def _show_usage(self):
        """Rate limits + session token usage (when a live agent exists) + provider account limits.

        Account limits are agent-independent, so they render even with no live agent — the TUI's
        /usage slash-worker resumes the session WITHOUT building an agent.
        """
        from cli import datetime, format_duration_compact

        def _limits_or(fallback: str) -> None:
            ...
            if not self._print_account_limits():
                print(fallback)

        if not self.agent:
            _limits_or("(._.) No active agent -- send a message first.")
            return
        agent = self.agent
        calls = agent.session_api_calls
        if calls == 0:
            _limits_or("(._.) No API calls made yet in this session.")
            return
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from cryozen_cli.config import get_config_path as _get_config_path
        ...
        from cryozen_cli.config import _expand_env_vars
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.cryozen-agent/skills/ and queue a note for the next user turn.
