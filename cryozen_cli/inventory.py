...
def build_models_payload(
    ...
    capabilities: bool = False, featured: bool = False,
    ...
    if pricing:
        _apply_pricing(rows, cached_only=pricing_cache_only)
    if capabilities:
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`cryozen_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
...
def _append_unconfigured_rows(
    ...
    from cryozen_cli.models import CANONICAL_PROVIDERS, _model_requires_account_discovery
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else f"Configured provider is not authenticated; run `cryozen model` to reactivate. {tail}"
            )
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    def _is_explicit(row: dict, slug: str) -> bool:
        ...
        return (
            # External-process CLIs (copilot-acp) are deliberate sign-ins that leave no trace in
            # config/env; keep the rows discovery accepted.
            _external_process_signed_in(slug)
...
def _external_process_signed_in(slug: str) -> bool:
    ...
    try:
        from cryozen_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (f"paste {key_env} to activate" if auth_type == "api_key" and key_env
                          else f"run `cryozen model` to configure ({auth_type})")
...
def _apply_pricing(rows: list[dict], *, cached_only: bool = False) -> None:
    """Set ``row["pricing"] = {model_id: {input, output, cache | None, free}}``. ``cached_only`` never
    hits the network; missing pricing is then left unset until the prewarm fills the caches."""
    from cryozen_cli.models_pricing import _format_price_per_mtok, get_pricing_for_provider
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        ...
        from cryozen_cli.providers import LLAMACPP_ALIASES, LLAMACPP_PROVIDER_ID
        ...
        if not current:
            ...
            try:
                from cryozen_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(rows: list[dict]) -> Optional[Thread]:
    ...
    from cryozen_cli.models_pricing import pricing_cache_scope

    slugs = {str(row.get("slug") or "").lower() for row in rows if row.get("slug")}
    endpoint_scope = tuple(sorted(
        (slug, pricing_cache_scope(slug)) for slug in slugs))
    prewarm_key = (cryozen_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="cryozen-picker-pricing-prewarm", daemon=True)
