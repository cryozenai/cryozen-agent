"""Streamlined setup flows: first-time quick setup, Blank Slate setup and the `--quick`
missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``cryozen_cli.setup`` take effect."""
...
def _blank_slate_done(config: dict, cryozen_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from cryozen_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         cryozen skills opt-in --sync",
          "  Add MCP servers:     cryozen mcp add", *extra, "  Tune agent settings: cryozen setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from cryozen_cli.setup import load_config
...
def _run_first_time_quick_setup(config: dict, cryozen_home, is_existing: bool):
    """Streamlined first-time setup: provider + API key and model, terminal & messaging;
    everything else gets defaults."""
    from cryozen_cli.setup import (
        _apply_default_agent_settings, _info, print_info, _print_setup_summary, print_success,
        prompt_choice, save_config, setup_gateway, setup_model_provider, setup_terminal_backend
    )
    # Step 1: pick an inference provider, add its API key and choose the default model (the same
    # flow as `cryozen model`; it saves config itself).
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'cryozen setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `cryozen import`).
        from cryozen_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    cryozen setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  cryozen setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    """
    from cryozen_cli.setup import _info
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Cryozen.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Cryozen update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``cryozen-agent`` skill
    ...
    try:
        ...
        from cryozen_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "cryozen-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("cryozen-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``cryozen setup agent``."""
...
def _run_blank_slate_setup(config: dict, cryozen_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `cryozen-agent` skill is always kept so the agent",
          "can help you drive and configure Cryozen itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, cryozen_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `cryozen update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, cryozen_home, "  Enable tools:        cryozen tools", "  Enable plugins:      cryozen plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, cryozen_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `cryozen-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`cryozen update` runs from re-injecting them. Opt back in any",
              "time with `cryozen skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `cryozen tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `cryozen plugins list` / `cryozen plugins install`.",
         "No plugins enabled. Add later with `cryozen plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `cryozen mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `cryozen mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, cryozen_home, "  Enable more tools:   cryozen tools")


def _run_quick_setup(config: dict, cryozen_home):
    ...
    from cryozen_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'cryozen setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Cryozen to messaging apps to chat from anywhere.",
              "You can configure these later with 'cryozen setup gateway'.")
