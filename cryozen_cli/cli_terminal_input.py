...
def _cryozen_call_output_screen_diff(
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    try:
        ...
        if getattr(_vt100_mod, "_cryozen_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._cryozen_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_cryozen_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._cryozen_bp_start = now
        ...
        _vt100_mod._cryozen_bp_timeout_patched = True
...
def _enable_extended_enter_keys(output=None, env: Optional[Mapping[str, str]] = None) -> bool:
    ...
    ``install_modify_other_keys_aliases()`` (called at CLI startup from ``cryozen_cli.pt_input_extras``) now
...
def _terminal_may_leak_cpr() -> bool:
    ...
    under heavy subagent / status-line load — see ``tests/cryozen_cli/test_cpr_local_leak.py``.
    """
