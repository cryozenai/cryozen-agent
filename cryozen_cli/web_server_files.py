...
_MANAGED_FILES_ROOT_ENV = "CRYOZEN_DASHBOARD_FILES_ROOT"
...
def _default_cryozen_root_is_opt_data() -> bool:
    raw = os.environ.get("CRYOZEN_HOME", "").strip()
    if not raw:
        return False
    try:
        from cryozen_constants import get_default_cryozen_root

        root = get_default_cryozen_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``cryozen update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the cryozen-webui image), where
    ...
    if _default_cryozen_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the Cryozen
    # root actually IS /opt/data or CRYOZEN_DASHBOARD_FILES_ROOT is set.
    if _default_cryozen_root_is_opt_data():
