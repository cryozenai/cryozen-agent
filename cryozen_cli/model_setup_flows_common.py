...
cycle) and tests patch ``cryozen_cli.config.load_config`` etc. at call time.
"""
...
def _ask(prompt: str, *, secret: bool = False, raw: bool = False, cancel_msg: str | None = "\nCancelled.",
    ...
    if secret:
        from cryozen_cli.secret_prompt import masked_secret_prompt as fn
    else:
...
def _commit_model_config(cfg: dict) -> None:
    ...
    from cryozen_cli.auth import deactivate_provider
...
def _activate_provider_model(selected, provider_id: str, base_url: str, done: str,
    ...
    from cryozen_cli.auth import _save_model_choice, _update_config_for_provider
    if not selected:
...
def _login_retry_context(args) -> tuple[str, str]:
    ...
    among its positional args. An OAuth provider is retried with ``cryozen auth add <provider>`` and
    named by its own portal host (``cryozen login`` no longer exists). Falls back to ``cryozen model`` when no provider config is in play."""
    pconfig = next((a for a in args if hasattr(a, "id") and hasattr(a, "portal_base_url")), None)
    if pconfig is None:
        return "cryozen model", "the sign-in service"
    ...
    return (f"cryozen auth add {provider_id}" if provider_id else "cryozen model"), host
...
def _run_login(login_fn, *args, **kwargs) -> bool:
    ...
    provider config passed to the helper, so a MiniMax failure never names another provider's
    retry command or host. Helpers that print their own copy raise ``SystemExit(1)`` with no
