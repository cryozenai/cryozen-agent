...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    values: Dict[str, Any] = {}
    for path in (cryozen_home / f"{name}.json", cryozen_home / name / "config.json"):
    ...
    # Holographic stores under plugins.cryozen-memory-store.
    plugins_cfg = cfg.get("plugins")
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("cryozen-memory-store")
