...
Tokens live in ~/.cryozen-agent/auth.json, NOT ~/.codex/: Cryozen keeps its own Codex OAuth session
separate from the Codex CLI / VS Code extension so one app's refresh-token rotation cannot
invalidate the other's session.

Split out of ``cryozen_cli/auth.py``; origin helpers are imported lazily inside each function
so ``cryozen_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from cryozen_cli.auth_constants import (
...
# Log-record parity with the origin module (caplog tests pin "cryozen_cli.auth").
logger = logging.getLogger("cryozen_cli.auth")

# ``{relogin}`` is filled at raise time with the profile-aware sign-in command: a bare
# ``cryozen auth`` from a named profile re-signs the ROOT store (93889b770da, #114012).
...
def _clear_pool_entry_status(entry: Dict[str, Any]) -> None:
    ...
    from cryozen_cli.auth import _POOL_STATUS_FIELDS
...
def _codex_base_url() -> str:
    return os.getenv("CRYOZEN_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _load_auth_store_maybe_locked(lock: bool) -> Dict[str, Any]:
    ...
    from cryozen_cli.auth import _auth_store_lock, _load_auth_store
    if lock:
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Cryozen auth store (~/.cryozen-agent/auth.json)."""
    from cryozen_cli.auth import _load_provider_state, _nonempty_str
...
def _sync_codex_pool_entries(
    ...
    ``device_code`` (the singleton-seeded entry from ``cryozen setup`` / the model picker) is always
    synced. ``manual:device_code`` (``cryozen auth add openai-codex``) is synced only when its
...
def _recover_codex_tokens_from_cli(
    ...
    """Adopt a valid Codex CLI token pair into Cryozen auth, if available.
    ...
    from cryozen_cli.auth import _import_codex_cli_tokens, _provider_state_transaction, _save_codex_tokens
    ...
    with _provider_state_transaction("openai-codex") as (_store, state, _source):
        ...
        if known and _codex_principal_identity(imported["access_token"]) not in (None, known):
            logger.warning(
                ...
                "workspace than the Cryozen credential. Run `%s` to re-authenticate it.",
...
def _ssl_interop_hint(exc: BaseException) -> str:
    ...
    return (
        ...
        " https://cryozenai.github.io/cryozen-agent/docs/integrations/providers"
    )
...
def _codex_refresh_failure_error(response: "httpx.Response") -> AuthError:
    ...
    from cryozen_cli.auth import _nonempty_str
...
def refresh_codex_oauth_pure(
    ...
    """Refresh Codex OAuth tokens without mutating Cryozen auth state."""
    from cryozen_cli.auth import _nonempty_str, _utc_now_z
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    """
    from cryozen_cli.auth import _provider_state_transaction, _save_codex_tokens, refresh_codex_oauth_pure
    ...
    with _provider_state_transaction("openai-codex", lock_timeout) as (_store, state, _source):
        ...
        try:
            ...
            # (or another Cryozen process) rotates the shared token this frozen copy fails with a
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    from cryozen_cli.auth import _codex_access_token_is_expiring
...
def resolve_codex_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    """Resolve runtime credentials from Cryozen's own Codex token store.
    ...
    try:
        if read_only:
            ...
            with _auth_store_lock():
                ...
                from cryozen_cli.auth import _load_auth_store, _load_provider_state
    ...
    refresh_timeout_seconds = env_float("CRYOZEN_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    if _should_refresh(access_token):
        # Re-read under lock to avoid racing with other Cryozen processes
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout):
            ...
            access_token = _stripped(tokens.get("access_token"))
    return _codex_runtime_result(
        access_token, source="cryozen-auth-store", last_refresh=data.get("last_refresh"))
...
def _probe_codex_quota_restored(
    ...
    this freely.
    """
    from cryozen_cli.auth import _codex_quota_probe_cache, _nonempty_str
...
def _refresh_expired_codex_probe_token(
    ...
    """
    from cryozen_cli.auth import _codex_quota_probe_cache
...
def _probe_codex_pool_entry_quota_restored(entry: Dict[str, Any]) -> Optional[bool]:
    ...
    from cryozen_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
...
def _codex_pool_rate_limit_status() -> Optional[Dict[str, Any]]:
    ...
    from cryozen_cli.auth import _nonempty_str, read_credential_pool
...
def _login_openai_codex(args, pconfig: ProviderConfig, *, force_new_login: bool = False) -> None:
    ...
    ``auth.codex_login_flow``). Tokens stored in ~/.cryozen-agent/auth.json."""
    from cryozen_cli.auth import (
    ...
    from cryozen_cli.auth_codex_browser import codex_oauth_login
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Cryozen will create its own session to avoid conflicts with Codex CLI / VS Code.")
            if _prompt_yes_no(
                ...
                print("Cryozen will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh OAuth flow — Cryozen gets its own session (device code unless the user opted in
    # to the browser flow).
    print()
...
def _codex_login_rate_limited_error(response: "httpx.Response", *, during: str = "") -> AuthError:
    ...
    # callers surface a "retry later" notice instead of a misleading "run cryozen auth" prompt (see issue
    # #32790).
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    from cryozen_cli.auth import _utc_now_z
