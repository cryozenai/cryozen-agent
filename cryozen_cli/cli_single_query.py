...
def _interrupt_agent_for_signal(agent, signum) -> None:
    """Hard-interrupt ``agent`` for a shutdown signal, then sleep ``CRYOZEN_SIGTERM_GRACE`` (1.5 s).
    ...
    try:
        if agent is not None:
            ...
            _grace = _float_env("CRYOZEN_SIGTERM_GRACE", 1.5)
            if _grace > 0:
                time.sleep(_grace)
    except Exception:
...
def _run_kanban_goal_loop_q(cli: "CryozenCLI", first_response: str, run_turn=None, log=None) -> None:
    ...
    task_id = (os.environ.get("CRYOZEN_KANBAN_TASK") or "").strip()
    if not task_id:
        return
    raw_run_id = (os.environ.get("CRYOZEN_KANBAN_RUN_ID") or "").strip()
    worker_run_id = _int_or(raw_run_id, None) if raw_run_id else None
    if raw_run_id and worker_run_id is None:
        logger.warning("invalid CRYOZEN_KANBAN_RUN_ID=%r", raw_run_id)

    from cryozen_cli import kanban_db as _kb
    from cryozen_cli import kanban_db_connect as _kbc
    from cryozen_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def _run_kanban_goal_loop_chat(cli: "CryozenCLI", first_response: str) -> None:
...
def _single_query_exit_code(result, *, credentials_rate_limited: bool = False) -> int:
    ...
    failed, so ``result`` is not a dict). A Kanban worker (``CRYOZEN_KANBAN_TASK`` set) that
    ...
    if not isinstance(result, dict):
        if credentials_rate_limited and os.environ.get("CRYOZEN_KANBAN_TASK"):
            from cryozen_cli.kanban_db import KANBAN_RATE_LIMIT_EXIT_CODE
    ...
    if os.environ.get("CRYOZEN_KANBAN_TASK"):
        ...
        if reason in _TERMINAL_PROVIDER_REASONS:
            from cryozen_cli.kanban_db import KANBAN_TERMINAL_PROVIDER_EXIT_CODE
...
def _run_quiet_single_query(cli, effective_query, emitter=None):
    ...
    CRYOZEN_TURN_AUTHOR (set only by a bot-to-bot dispatcher) is consumed here so tool subprocesses do not inherit it.
    ...
    from cryozen_cli.quiet_single_query import (
    ...
    if os.environ.get("CRYOZEN_KANBAN_GOAL_MODE") == "1":
        try:
...
def _collect_kanban_task_images(single_query_images):
    ...
    _kanban_task_id = os.environ.get("CRYOZEN_KANBAN_TASK", "").strip()
...
def _run_single_query_mode(cli, query, image, quiet, oneshot, stream_json: bool = False):
    ...
    # Mark single-query for the approval gate. cli.py sets CRYOZEN_INTERACTIVE earlier for interactive sudo
    ...
    from cryozen_cli.quiet_single_query import exit_single_query
    ...
    try:
        ...
        if quiet:
            ...
            if stream_json:
                ...
                from cryozen_cli.stream_json import StreamJsonEmitter
