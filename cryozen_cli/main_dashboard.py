...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None,
    ...
    *scope_home*: keep only backends whose resolved Cryozen home (see
    ``_cryozen_home_for_pid``) is this home; unreadable ownership is spared, never guessed.
    ...
    """
    from cryozen_cli.dashboard_procs import (
        _caller_ancestor_pids,
        _is_caller_wrapper_shell,
        _pids_owned_by_cryozen_home,
    ...
    # 'cryozen dashboard --stop'``); killing it takes down the invoking terminal.
    ancestors = _caller_ancestor_pids()
    pids = [pid for pid in pids if not _is_caller_wrapper_shell(pid, ancestors)]
    return _pids_owned_by_cryozen_home(pids, scope_home) if scope_home else pids
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"cryozen {candidate}", f"cryozen_cli.main {candidate}", f"cryozen_cli/main.py {candidate}")
...
_DASHBOARD_SYSTEMD_UNIT = "cryozen-dashboard.service"
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (Cryozen installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``cryozen-serve.service``), or None.
...
# launchd plist directories that can supervise a ``cryozen dashboard`` / ``cryozen serve`` backend on
...
def _loaded_launchd_backend_jobs(
    ...
    ``ProgramArguments`` is a ``cryozen dashboard`` / ``cryozen serve`` backend. macOS only (empty
    ...
    from cryozen_cli.gateway import _launchd_print_service_pid
...
def _restart_launchd_job(domain: str, label: str, old_pid: int | None, *, timeout: float = 15.0) -> bool:
    ...
    from cryozen_cli.gateway import _wait_for_launchd_service_pid
    try:
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``cryozen update``, detached, logging to
    ...
    log_path = get_cryozen_home() / "logs" / "dashboard-restart.log"
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``cryozen update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # cryozen_cli.config.get_cryozen_home to simulate setup failure.
        from cryozen_cli.config import get_cryozen_home as _get_cryozen_home
        logs_dir = _get_cryozen_home() / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        log_file = open(logs_dir / "update.log", "a", buffering=1, encoding="utf-8")

        import datetime as _dt

        stage = "continued on the pulled code" if os.environ.get("CRYOZEN_UPDATE_POST_SWAP") == "1" else "started"
        log_file.write(f"\n=== cryozen update {stage} {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    """
    from cryozen_cli.dashboard_procs import _scan_dashboard_processes
    ...
    if not live:
        print("No cryozen dashboard or serve processes running.")
        return 0

    print(f"{len(live)} cryozen dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from cryozen_cli.web_server import should_require_dashboard_auth
    ...
    print("    [2] OpenID Connect via your own identity provider\n    [3] Cancel\n")
    ...
    if choice == "2":
        print()
        print(
            "  Register a public OIDC client with your identity provider (Authentik, "
            "Keycloak, Zitadel, ...), set dashboard.oauth.self_hosted.issuer and "
            "dashboard.oauth.self_hosted.client_id in config.yaml, then start the "
            "dashboard again.\n"
    ...
    try:
        ...
        from cryozen_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
        cfg = load_config()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from cryozen_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under the account's $HOME/.cryozen-agent/
    # desktop-ssh, independent of CRYOZEN_HOME and the active profile. Anchor
    # validation there, NOT get_cryozen_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_cryozen_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".cryozen-agent" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``cryozen dashboard`` inheriting that ``CRYOZEN_WEB_DIST`` would
...
def _attach_to_host_backend(args, headless_backend: bool) -> None:
    """Multiplex-only: a second `cryozen serve`/`dashboard` attaches to the host backend.
    ...
    Desktop pool backends (CRYOZEN_DESKTOP=1) keep their own lifecycle.
    ...
    * the owner must ANSWER on its recorded port and identify itself (a record alone cannot see a
      ...
      `cryozen update` relaunch landing in that window would otherwise exit 0 with NOTHING
    ...
    * a `cryozen dashboard` user is never handed a headless backend's URL (no SPA behind it).

    Returns normally — leaving the caller to BIND — when no owner answers.
    """
    if getattr(args, "isolated", False) or os.environ.get("CRYOZEN_DESKTOP") == "1":
        return
    ...
    if not headless_backend and not identity.get("servesSpa"):
        print(f"Refusing to start: this host is already served by {hr.describe(record)}, "
              "which is a headless `cryozen serve` backend with no dashboard UI.")
        print("  Stop it and run `cryozen dashboard`, or use --isolated for a dedicated server.")
        sys.exit(1)
    ...
    print(f"Cryozen {kind} already running on this host: PID {record.pid}, port {record.port}.")
...
def _route_named_profile_dashboard(
    ...
    opts out; Desktop pool backends (CRYOZEN_DESKTOP=1) stay per-profile. Returns normally when no
    routing applies.
    """
    try:
        from cryozen_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        or os.environ.get("CRYOZEN_DESKTOP") == "1"
    ):
        return
    ...
    reexec_argv = [
        sys.executable, "-m", "cryozen_cli.main",
        "-p", "default",
    ...
    # CRYOZEN_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping CRYOZEN_HOME: in the Docker layout the root is /opt/data, and an
    # unset CRYOZEN_HOME would fall back to $HOME/.cryozen-agent = /opt/data/.cryozen-agent — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_cryozen_root() strips a trailing profiles/<name> for both layouts.
    try:
        from cryozen_constants import get_default_cryozen_root
        env["CRYOZEN_HOME"] = str(get_default_cryozen_root())
    except Exception:
        env.pop("CRYOZEN_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets CRYOZEN_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless CRYOZEN_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed CRYOZEN_WEB_DIST can't be populated.
    """
    from cryozen_cli.main import PROJECT_ROOT
    from cryozen_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["CRYOZEN_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "CRYOZEN_WEB_DIST" not in os.environ and not skip_build:
        ...
        _dist_root = (
            ...
            Path(os.environ["CRYOZEN_WEB_DIST"])
            if "CRYOZEN_WEB_DIST" in os.environ
            else PROJECT_ROOT / "cryozen_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            # Only the default dist location is recoverable (desktop launches with
            # --build-mode skip after a wipe of web_dist); a custom CRYOZEN_WEB_DIST
            ...
            _recoverable = "CRYOZEN_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # CRYOZEN_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # CRYOZEN_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        # path).
        _dist_root = Path(os.environ["CRYOZEN_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ CRYOZEN_WEB_DIST is set but no web dist found at: {_dist_root}")
            print(_PRE_BUILD_HINT)
            print("  Or unset CRYOZEN_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # web_server reads CRYOZEN_WEB_DIST raw at import (no expanduser), so a
        # validated "~/dist" would otherwise pass here and still 404 there.
        os.environ["CRYOZEN_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from CRYOZEN_WEB_DIST: {_dist_root}")
