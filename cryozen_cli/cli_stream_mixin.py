...
class CLIStreamMixin:
    ...
    def _on_notice(self, notice) -> None:
        ...
        Notices fire mid-turn; printing immediately
        races the stream and buries the line behind the prompt. Flushed by _flush_agent_notices()
        ...
        try:
            ...
            def queue_notice():
                if not hasattr(self, "_pending_agent_notices"):
                    self._pending_agent_notices = []
                self._pending_agent_notices.append((level, text))
            render_notification(queue_notice, platform="cli", diagnostic=is_diagnostic_notice(notice),
                                user_config=getattr(getattr(self, "agent", None), "_notification_config", None))
        except Exception:
            pass

    def _flush_agent_notices(self) -> None:
        ...
        try:
            pending = getattr(self, "_pending_agent_notices", None)
            if not pending:
                return
            self._pending_agent_notices = []
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        from cli import (
            CryozenCLI, _ACCENT, _RST, _STREAM_PARTIAL_PREVIEW_LEN, _cprint, _strip_markdown_syntax, datetime)
        if not text:
            return
        ...
        if not self._stream_box_opened:
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - CryozenCLI._status_bar_display_width(label)
    ...
    def _on_tool_progress(self, event_type: str, function_name: str = None, preview: str = None, function_args: dict = None, **kwargs):
        ...
        """
        from cli import CLI_CONFIG, _DIM, _RST, _cprint, _cryozen_home
