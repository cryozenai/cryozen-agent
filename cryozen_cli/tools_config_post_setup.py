"""Post-setup install hooks and installed-state predicates for `cryozen tools` provider rows."""
...
from cryozen_cli.tools_config_cua import (
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Cryozen-managed ``$CRYOZEN_HOME/bin`` copy does."""
...
def _post_setup_lightpanda() -> None:
    ...
    if lightpanda_bin:
        ...
        if os.name == "nt":
            _print_info("    Lightpanda has no native Windows build; run Cryozen under WSL2.")
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    global/Cryozen-managed install), so there is no ``npm install`` step here."""
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung cryozen_cli.doctor uses so this probe can't diverge
        ...
        # agent-browser is no longer a root package.json dependency (#43564) — it resolves lazily via npx
        # (or a global/Cryozen-managed install) instead of a local `npm install`, so there's no node_modules/
    ...
    # Reuse the runtime resolution cascade (PATH -> Homebrew/Cryozen-managed node -> npx) rather than
    # a bare shutil.which — Cryozen-managed-Node-only setups resolve agent-browser/npx only that way.
    try:
    ...
    if _running_in_docker():
        ...
        _info_lines("Pull the latest image to get the bundled Chromium:",
                    "  docker pull ghcr.io/cryozenai/cryozen-agent:latest")
        return
...
_PIP_POST_SETUP_HOOKS: dict = {
    "faster_whisper": _pip_hook(
        ...
        on_install=("Model sizes: tiny, base (default), small, medium, large-v3",
                    "Change via stt.local.model in ~/.cryozen-agent/config.yaml")),
    "kittentts": _pip_hook(
    ...
    "piper": _pip_hook(
        ...
        always=("Default voice: en_US-lessac-medium (downloaded on first TTS call)",
                ...
                "Switch voices by setting tts.piper.voice in ~/.cryozen-agent/config.yaml")),
    "ddgs": _pip_hook(
...
def _post_setup_spotify() -> None:
    # Full `cryozen auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.cryozen-agent/.env)
    # then PKCE; existing app → OAuth only.
    from types import SimpleNamespace
    try:
        from cryozen_cli.auth import login_spotify_command
    except Exception as exc:
        _print_warning(f"    Could not load Spotify auth: {exc}")
        _info_lines("Run manually: cryozen auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: cryozen auth spotify")
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        from cryozen_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        _info_lines("Run manually: cryozen plugins enable observability/langfuse")
    _info_lines("Restart Cryozen for tracing to take effect.", "Verify: cryozen plugins list")
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from cryozen_cli.auth import get_xai_oauth_auth_status
    ...
    try:
        from cryozen_cli.setup import prompt_choice, prompt as _setup_prompt
        from cryozen_cli.setup_tts import _run_xai_oauth_login_from_setup
        from cryozen_cli.config import save_env_value
    except Exception as exc:
        _print_warning(f"    Could not load setup helpers: {exc}")
        _info_lines("Run later: cryozen auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    idx = prompt_choice(
        ...
        choices=["Sign in with xAI Grok OAuth (SuperGrok / Premium+) — browser login",
                 ...
                 "Skip — configure later via `cryozen auth add xai-oauth`"])
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            _print_success("    Logged in — xAI will use these OAuth credentials")
        else:
            _print_warning("    xAI Grok OAuth login did not complete. Run later: cryozen auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: cryozen auth add xai-oauth")
    else:
...
def _post_setup_openai_codex() -> None:
    ...
    relogin = "cryozen auth add openai-codex"
    _print_info("    OpenAI (Codex auth) needs credentials.")
    try:
        from cryozen_cli.auth import _codex_device_code_login, _save_codex_tokens
        from cryozen_cli.setup import is_noninteractive, prompt_choice
    except Exception as exc:
        _print_warning(f"    Could not load setup helpers: {exc}")
...
def _xai_credentials_ready() -> bool:
    from cryozen_cli.tools_config import _xai_credentials_present  # facade binding: tests patch it there
...
def run_post_setup_command(args) -> int:
    """``cryozen tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
    backend setup without re-implementing install logic. Exit code: 0 ok, 2 unknown key."""
    key = getattr(args, "post_setup_key", None)
    if not key:
        _print_error("Usage: cryozen tools post-setup <key>")
        return 2
...
# Python deps installed via ``cryozen tools`` aren't in the managed runtime's locked ``all`` sync, so a
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``cryozen tools`` Python dependencies present in this runtime."""
    return [
...
def _has_agent_browser() -> bool:
    ...
    from cryozen_constants import agent_browser_runnable
    ...
    try:
        ...
        # Cryozen-managed Node dirs ($CRYOZEN_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_cryozen_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
