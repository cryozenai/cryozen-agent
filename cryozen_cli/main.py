#!/usr/bin/env python3
"""Cryozen CLI - Main entry point.

Usage:
    cryozen                     # Interactive chat (default)
    cryozen chat / gateway / setup / status / cron / doctor / update / ...
    cryozen --version           # Show version and update status
    cryozen <cmd> --help        # Per-command help
"""

# cryozen_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``cryozen update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``cryozen update``.
try:
    import cryozen_bootstrap  # noqa: F401
...
from cryozen_cli._subprocess_compat import suppress_platform_ver_console
...
# Inline path math so ``python cryozen_cli/main.py`` (script mode: sys.path[0]
# is cryozen_cli/, not the repo root) can import cryozen_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from cryozen_cli import _startup_fast  # noqa: E402

# A literal ``~``/``$VAR`` in CRYOZEN_HOME (fish, or any quoted value) must become absolute
# before the first reader — otherwise it resolves against cwd and scaffolds <cwd>/~/.cryozen-agent.
_startup_fast.normalize_cryozen_home_env()

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``cryozen update`` may have left a recovery marker with a core package wiped;
# the cryozen_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``cryozen_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from cryozen_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from cryozen_cli.oneshot import run_oneshot
...
def _warn_if_unsupervised_pid1(pid: "int | None" = None) -> None:
    ...
    children). A Compose service that overrides ``entrypoint:`` to invoke cryozen directly makes
    cryozen itself PID 1 — nothing then ``wait()``s on those orphans and they accumulate as
    ...
    if (pid if pid is not None else os.getpid()) != 1:
        return
    print(
        "[cryozen] WARNING: this process is PID 1 with no init above it "
...
def _set_process_title() -> None:
    """Cosmetic: show 'cryozen' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``cryozen.exe``). Never fatal.
    """
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("cryozen")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"cryozen", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"cryozen")
    except Exception:
        pass
...
# cryozen_cli.config is importable. Cached per config path so early callers
...
# sets CRYOZEN_HOME, and a cache keyed on nothing pinned every later caller to
...
def _early_interface_config_path() -> str:
    ...
    home = os.environ.get("CRYOZEN_HOME")
    if home:
        return os.path.join(home, "config.yaml")
    return os.path.join(os.path.expanduser("~"), ".cryozen-agent", "config.yaml")
...
# `resetTerminalModes()` is the later cousin. ``CRYOZEN_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("CRYOZEN_TUI_NO_EARLY_DISABLE") == "1":
        return
...
# ``cryozen --version`` is answered before config/logging imports.
...
from cryozen_cli.subcommands.cron import build_cron_parser
from cryozen_cli.subcommands.gateway import build_gateway_parser
from cryozen_cli.subcommands.profile import build_profile_parser
from cryozen_cli.subcommands.model import build_model_parser
from cryozen_cli.subcommands.setup import build_setup_parser

from cryozen_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
from cryozen_cli.subcommands.slack import build_slack_parser
from cryozen_cli.subcommands.login import build_login_parser
from cryozen_cli.subcommands.logout import build_logout_parser
from cryozen_cli.subcommands.auth import build_auth_parser
from cryozen_cli.subcommands.status import build_status_parser
from cryozen_cli.subcommands.pause import build_pause_parser
from cryozen_cli.subcommands.webhook import build_webhook_parser
from cryozen_cli.subcommands.hooks import build_hooks_parser
from cryozen_cli.subcommands.doctor import build_doctor_parser
from cryozen_cli.subcommands.verify import build_verify_parser
from cryozen_cli.subcommands.security import build_security_parser
from cryozen_cli.subcommands.approvals import build_approvals_parser
from cryozen_cli.subcommands.dump import build_dump_parser
from cryozen_cli.subcommands.debug import build_debug_parser
from cryozen_cli.subcommands.backup import build_backup_parser
from cryozen_cli.subcommands.import_cmd import build_import_cmd_parser
from cryozen_cli.subcommands.import_agent import build_import_agent_parser
from cryozen_cli.subcommands.config import build_config_parser
from cryozen_cli.subcommands.skin import build_skin_parser
from cryozen_cli.subcommands.console import build_console_parser
from cryozen_cli.subcommands.update import build_update_parser
from cryozen_cli.subcommands.uninstall import build_uninstall_parser
from cryozen_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from cryozen_cli.subcommands.gui import build_gui_parser
from cryozen_cli.subcommands.logs import build_logs_parser
from cryozen_cli.subcommands.prompt_size import build_prompt_size_parser
from cryozen_cli.subcommands.memory import build_memory_parser
from cryozen_cli.subcommands.acp import build_acp_parser
from cryozen_cli.subcommands.tools import build_tools_parser
from cryozen_cli.subcommands.insights import build_insights_parser
from cryozen_cli.subcommands.usage import build_usage_parser
from cryozen_cli.subcommands.monitoring import build_monitoring_parser
from cryozen_cli.subcommands.skills import build_skills_parser
from cryozen_cli.subcommands.pairing import build_pairing_parser
from cryozen_cli.subcommands.plugins import build_plugins_parser
from cryozen_cli.subcommands.mcp import build_mcp_parser
from cryozen_cli.subcommands.claw import build_claw_parser
from cryozen_cli.subcommands.vault import build_vault_parser
from cryozen_cli.subcommands.moa import build_moa_parser
from cryozen_cli.subcommands.fallback import build_fallback_parser
from cryozen_cli.subcommands.worktree import build_worktree_parser
from cryozen_cli.subcommands.browser import build_browser_parser
from cryozen_cli.subcommands.secrets import build_secrets_parser
from cryozen_cli.subcommands.codex_runtime import build_codex_runtime_parser
from cryozen_cli.subcommands.egress import build_egress_parser
from cryozen_cli.subcommands.migrate import build_migrate_parser
from cryozen_cli.subcommands.checkpoints import build_checkpoints_parser
from cryozen_cli.subcommands.bundles import build_bundles_parser
from cryozen_cli.subcommands.curator import build_curator_parser
from cryozen_cli.subcommands.pets import build_pets_parser
from cryozen_cli.subcommands.journey import build_journey_parser
from cryozen_cli.subcommands.computer_use import build_computer_use_parser
from cryozen_cli.subcommands.sessions import build_sessions_parser
from cryozen_cli.subcommands.completion import build_completion_parser
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'cryozen {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any cryozen module import: many modules
# cache CRYOZEN_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# CRYOZEN_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.cryozen-agent/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors cryozen_cli.profiles._PROFILE_ID_RE


def _inside_mcp_add_args(argv: list, index: int) -> bool:
    """True once argv reaches `cryozen mcp add ... --args <command argv>`.
    ...
    ``--profile``), not to Cryozen' own profile selector.
    """
    try:
...
def _looks_like_cryozen_invocation() -> bool:
    """False when ``sys.argv`` belongs to a test runner rather than a ``cryozen`` run.

    pytest's own ``-p no:xdist`` reaches ``_scan_profile_flag`` through ``sys.argv`` at import
    time; it must stay a silent skip, while a real ``cryozen -p 'Work Bot'`` must fail loudly.
    """
    return "pytest" not in (sys.argv[0] or "")


def _exit_invalid_profile_name(value: str) -> None:
    from cryozen_cli.profiles import _invalid_profile_name_error

    print(f"Error: {_invalid_profile_name_error(value)}", file=sys.stderr)
    print("Run `cryozen profile list` to see your profiles.", file=sys.stderr)
    sys.exit(2)
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`cryozen chat -p
    ...
    ``cryozen`` run — after a subcommand, ``-p`` may belong to that subcommand or a
    plugin (`cryozen kanban ... -p 8080`), and option-looking values (``no:xdist``,
    ``--flag``) are always a silent skip.
    """
    from cryozen_cli._parser import top_level_value_flag_sets
    ...
    while i < len(argv):
        ...
        if arg in {"--profile", "-p"} and i + 1 < len(argv):
            ...
            if re.match(_PROFILE_NAME_RE, value):
                return value, 2, i
            if not saw_subcommand and not _looks_like_option_value(raw) and _looks_like_cryozen_invocation():
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    """Resolve `sudo cryozen -p <name>` against the invoking user's home.
    ...
    if name == "default":
        return None
    from cryozen_constants import named_profile_is_live, sudo_invoker_default_home
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin CRYOZEN_HOME to the profile dir; a bare invocation
    means "the root CRYOZEN_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``cryozen profile use``) would silently redirect the default gateway into
    ...
    CRYOZEN_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    CRYOZEN_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), CRYOZEN_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("CRYOZEN_SUPERVISED_CHILD") or os.environ.get("CRYOZEN_S6_SUPERVISED_CHILD"):
        return True
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set CRYOZEN_HOME before imports."""
    argv = sys.argv[1:]
    profile_name, consume, profile_index = _scan_profile_flag(argv)

    # CRYOZEN_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the cryozen root (systemd hardcodes CRYOZEN_HOME=/root/.cryozen-agent)
    # we must still read active_profile — the user may have run
    # `cryozen profile use` and the gateway should honour it (#22502).
    cryozen_home_env = os.environ.get("CRYOZEN_HOME", "")
    if profile_name is None and cryozen_home_env and Path(cryozen_home_env).parent.name == "profiles":
        return
    ...
    if profile_name is None and cryozen_home_env and os.environ.get("CRYOZEN_UPDATE_POST_SWAP") == "1":
        return

    if profile_name is None and not _under_gateway_supervisor(argv) and not _desktop_ssh_backend(argv):
        try:
            from cryozen_constants import get_default_cryozen_root

            active_path = get_default_cryozen_root() / "active_profile"
    ...
    if profile_name is None:
        return
    try:
        from cryozen_cli.profiles import resolve_profile_env

        cryozen_home = resolve_profile_env(profile_name)
    except FileNotFoundError as exc:
        cryozen_home = _resolve_sudo_user_profile_env(profile_name)
        if not cryozen_home:
        ...
        # A bug in profiles.py must NEVER prevent cryozen from starting
        print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
        return
    os.environ["CRYOZEN_HOME"] = cryozen_home
...
# ``-p``/active_profile re-homed the process after cryozen_bootstrap ran: re-point the temp vars
# at THIS home's scratch dir (a user-set TMPDIR is still left alone).
try:
    from cryozen_constants import export_scratch_tmp_env as _export_scratch_tmp_env

    _export_scratch_tmp_env()
except Exception:
    pass  # an unwritable home leaves the system temp dir in place; never block startup

# Windows launcher self-heal — the ``cryozen`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``cryozen update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m cryozen_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no cryozen module may import before
...
# ``<checkout>\bin``, where ``cryozen update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``cryozen`` stopped
...
if sys.platform == "win32":
    try:
        from cryozen_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.cryozen-agent/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from cryozen_cli.config import get_cryozen_home
from cryozen_cli.env_loader import load_cryozen_dotenv
...
load_cryozen_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets → CRYOZEN_REDACT_SECRETS BEFORE cryozen_logging
...
try:
    ...
    # serves cryozen_logging, cryozen_time and later raw reads: 3-4 config.yaml parses become one.
    # Managed overlay included: administrator-pinned redact_secrets / force_ipv4 win here too.
    from cryozen_cli.config_effective import load_user_config_effective as _load_effective_early

    _cfg_path = get_cryozen_home() / "config.yaml"
    if _cfg_path.exists():
        _early_cfg_raw = _load_effective_early(_cfg_path)
        if "CRYOZEN_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["CRYOZEN_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from cryozen_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from cryozen_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if cryozen_constants not importable yet

import logging
import threading
from datetime import datetime

from cryozen_cli import __version__, __release_date__

from cryozen_cli.model_setup_flows import (
...
from cryozen_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from cryozen_cli.main_platform_setup import (
    cmd_slack,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from cryozen_cli.main_dashboard import (
...
from cryozen_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from cryozen_cli.main_provider_setup import (
...
from cryozen_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from cryozen_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from cryozen_cli.main_desktop import (
    cmd_gui,
)
from cryozen_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
...
from cryozen_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from cryozen_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from cryozen_cli.main_tui_launch import (
    _launch_tui,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_cryozen_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("CRYOZEN_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("CRYOZEN_TERMUX_PREFETCH_UPDATES") == "1"


def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.cryozen-agent/.env assigns a non-empty value to any provider key."""
...
def _auth_store_logged_in(auth_file: Path, registry, strict_profile_scope: bool) -> bool:
    ...
    from cryozen_cli.auth import get_auth_status
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    """Check if at least one inference provider is usable. Never creates one.
    ...
    env and host-wide fallbacks (gh auth) must not make it appear ready.
    Unscoped callers keep the legacy behavior.
    """
    from cryozen_cli.config import get_env_path, get_cryozen_home, load_config
    from cryozen_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    if _auth_store_logged_in(get_cryozen_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from cryozen_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a read-only ``SessionDB`` (lazy import, so test patches on ``cryozen_state``
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``cryozen -c`` from repo A continues repo A's last session rather than the
    ...
    """
    # A finite `cryozen -z`/`chat -q` run is CLI history too: `cryozen -z … --resume latest` chains on it.
    if source == "cli":
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, cryozen_bin
        cli_args: the original CLI arguments (everything after 'cryozen')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    cryozen_bin = container_info["cryozen_bin"]
    ...
    if _probe_container(inspect_cmd, backend).returncode != 0:
        ...
        if not sudo_path:
            print(
                ...
                f"The container may be running under root. Try: sudo cryozen {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if _probe_container(cmd_prefix[:2] + inspect_cmd, backend, via_sudo=True).returncode != 0:
            print(
                ...
                f"  }}];\n"
                f"\n"
                f"Or run: sudo cryozen {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _create_titled_session(title: str) -> Optional[str]:
    ...
    db = None
    try:
        from cryozen_state_ids import new_session_id as mint_session_id
        from cryozen_state_registry import acquire
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'cryozen sessions list' to see available sessions, or "
            ...
            try:
                from cryozen_cli.terminal_breadcrumbs import resolve_breadcrumb_session
            ...
            if _crumb_id:
                ...
                if last_id:
                    ...
                    print(
                        ...
                        "`cryozen`, or list sessions with `cryozen sessions list`.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from cryozen_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: cryozen --resume {_imported_id})")
...
def _resolve_chat_session_args(args, use_tui: bool) -> None:
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'cryozen sessions list' to see available sessions.")
            sys.exit(1)
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        from cryozen_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from cryozen_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'cryozen doctor' for details.\033[0m\n\n")
    except Exception:
        pass
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.cryozen-agent/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from cryozen_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from cryozen_cli.config import get_cryozen_home
            skills_dir = Path(get_cryozen_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import cryozen_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `cryozen setup` (TTY) or exit 1 with guidance."""
    print()
    print(
        "It looks like Cryozen isn't configured yet -- no API keys or providers found."
    )
    print()
    print("  Run:  cryozen setup")
    print()

    from cryozen_cli.setup import (
    ...
    if reply in {"", "y", "yes"}:
        ...
        return
    print()
    print("You can run 'cryozen setup' at any time to configure.")
    sys.exit(1)
...
def cmd_chat(args):
    ...
    from cryozen_cli.stream_json import stream_json_requested
    # Structured stdout is a non-interactive protocol: it overrides CRYOZEN_TUI/display.interface too.
    ...
    # First-run guard: with no usable provider, send the user through setup before chatting.
    ...
    if getattr(args, "yolo", False):
        os.environ["CRYOZEN_YOLO_MODE"] = "1"
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["CRYOZEN_IGNORE_RULES"] = "1"
    # --source: tag session source for filtering (e.g. 'tool' for integrations)
    if getattr(args, "source", None):
        os.environ["CRYOZEN_SESSION_SOURCE"] = args.source
        ...
        os.environ["CRYOZEN_SESSION_SOURCE_EXPLICIT"] = "1"
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older cryozen_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: CryozenCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `cryozen` / `cryozen chat`.
        from cryozen_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from cryozen_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from cryozen_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``cryozen <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "cryozen_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "cryozen_cli.auth", "login_command", doc='Authenticate Cryozen CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "cryozen_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "cryozen_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "cryozen_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "cryozen_cli.cron", "cron_command", forward_return=True, doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "cryozen_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "cryozen_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "cryozen_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "cryozen_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "cryozen_cli.doctor", "run_doctor", forward_return=True, doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "cryozen_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "cryozen_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "cryozen_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "cryozen_cli.backup", "run_import", doc='Restore a Cryozen backup from a zip file.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "cryozen_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "cryozen_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "cryozen_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "cryozen_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "cryozen_cli.mcp_config", "mcp_command", forward_return=True)
cmd_claw = _forward_command("cmd_claw", "cryozen_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "cryozen_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from cryozen_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from cryozen_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from cryozen_cli.auth import AuthError, format_auth_error, resolve_provider
    from cryozen_cli.config import get_compatible_custom_providers, get_env_value
    from cryozen_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'cryozen model' for "
                "available providers, or run 'cryozen doctor' to diagnose config "
    ...
    if not active:
        try:
            ...
            if exc.code == "no_provider_configured":
                ...
                # "run `cryozen model`" from inside `cryozen model` is circular.
                print("No provider is set up yet — pick one below.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``cryozen model``) and the setup wizard
    ...
    effective_provider = config_provider or os.getenv("CRYOZEN_INFERENCE_PROVIDER") or "auto"
    ...
    from cryozen_cli.models import _PROVIDER_LABELS
    ...
    # cryozen_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.cryozen-agent/.env that poisons
...
# ``cryozen_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on cryozen_cli.main; importing update_cmd eagerly would cost every ``cryozen``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    ...
    "cryozen_cli.dashboard_procs": (
        "_detect_concurrent_cryozen_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def cmd_verify(args):
    ...
    from cryozen_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `cryozen security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from cryozen_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `cryozen approvals <subcmd>`."""
    from cryozen_cli.approvals_suggest import approvals_command
...
def cmd_config(args):
    ...
    from cryozen_cli.config import config_command
...
def cmd_backup(args):
    """Back up Cryozen home directory to a zip file."""
    from cryozen_cli import backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `cryozen --version` pre-import fast path.
...
def cmd_uninstall(args):
    """Uninstall Cryozen Agent (or just the Chat GUI with --gui).
    ...
    if getattr(args, "gui_summary", False):
        from cryozen_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from cryozen_cli.uninstall import run_gui_uninstall
    ...
    from cryozen_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from cryozen_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    from cryozen_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Cryozen Agent")
        return True
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Cryozen runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from cryozen_cli.update_inventory import (
    ...
    if getattr(args, "list_venv_holders", False):
        ...
        from cryozen_cli.update_cmd_windows import VENV_HOLDERS_EXIT, list_venv_holders
    ...
    from cryozen_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from cryozen_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update Cryozen Agent: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from cryozen_cli.update_lock import (
        UPDATE_EXIT_CONCURRENT,
        UpdateLock,
        describe_holder,
    )

    # A child spawned off cryozen.exe: the parent still holds the shim (and the venv python)
    ...
    from cryozen_cli.update_handoff import wait_for_shim_parent_exit
    ...
    from cryozen_cli.update_cmd import _cmd_update_impl
    ...
    try:
        ...
        from cryozen_cli.update_receipt import COMMAND_BOUNDARY_STOP_REASON
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``cryozen -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from cryozen_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        ...
        own_home = str(get_cryozen_home())
        if not _find_stale_dashboard_pids(scope_home=own_home):
            print("No cryozen dashboard processes running for this profile.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `cryozen update`;
        ...
        from cryozen_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `cryozen serve` is headless/non-interactive: fail closed on a corrupt
    ...
    if token_file and not headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with cryozen serve")
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    Desktop Electron spawns its backend with CRYOZEN_DESKTOP=1 plus
    CRYOZEN_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    CRYOZEN_SERVE_HEADLESS=1). A shell inheriting those then running
    `cryozen dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    re-sets CRYOZEN_SERVE_HEADLESS itself.
    ...
    CRYOZEN_DASHBOARD_SESSION_TOKEN, which the terminal pane never receives and
    the terminal tool's env policy strips from agent children.
    """
    desktop_owned_child = (
        os.environ.get("CRYOZEN_DESKTOP") == "1"
        and bool(os.environ.get("CRYOZEN_DASHBOARD_SESSION_TOKEN"))
    )
    if (
        not headless_backend
        and not desktop_owned_child
        and _is_electron_packaged_web_dist(os.environ.get("CRYOZEN_WEB_DIST", ""))
    ):
        os.environ.pop("CRYOZEN_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("CRYOZEN_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    # the same logs directory as every other Cryozen surface.
    try:
        from cryozen_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        from cryozen_cli.main_dep_hints import missing_optional_deps_message
    ...
    try:
        ...
        # #61115, #65696.
        from cryozen_cli.config import apply_terminal_config_to_env
    ...
    desktop = os.environ.get("CRYOZEN_DESKTOP") == "1"
    if headless_backend and desktop:
        return True
    try:
        from cryozen_cli.mcp_startup import (
...
def cmd_dashboard(args):
    ...
    from cryozen_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from cryozen_cli import completion
...
def cmd_logs(args):
    """View and filter Cryozen log files."""
    from cryozen_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Cryozen command console."""
    from cryozen_cli.console_engine import run_console_repl
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``cryozen`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``cryozen photon`` failing
    ...
    not import it, so the CLI registration never happens and ``cryozen photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("CRYOZEN_TUI") == "1":
        return True
    # The chat path decides TUI-vs-classic via _resolve_use_tui (--cli/--tui
    # flags, TTY gate, CRYOZEN_TUI env, display.interface config). Bare
    # `cryozen`/`cryozen chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: CRYOZEN_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from cryozen_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from cryozen_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery and _should_background_mcp_startup(args):
        try:
            from cryozen_cli.mcp_startup import start_background_mcp_discovery
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from cryozen_cli.mcp_startup import get_mcp_server_filter
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["CRYOZEN_SAFE_MODE"] = "1"
    os.environ["CRYOZEN_IGNORE_USER_CONFIG"] = "1"
    os.environ["CRYOZEN_IGNORE_RULES"] = "1"
...
def _light_chat_parser():
    ...
    from cryozen_cli._parser import build_top_level_parser
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("CRYOZEN_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from cryozen_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("CRYOZEN_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("CRYOZEN_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["CRYOZEN_DEFER_AGENT_STARTUP"] = "1"
            os.environ["CRYOZEN_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["CRYOZEN_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `cryozen --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``cryozen-agent``) — matching is exact. ``CRYOZEN_AGENT`` is the
    Cryozen-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Cryozen running inside another agent's terminal).
    """
    os.environ.setdefault("AI_AGENT", "cryozen-agent")
    os.environ.setdefault("CRYOZEN_AGENT", "true")
...
def _register_plugin_cli_commands(subparsers) -> None:
    ...
    try:
        ...
        from cryozen_cli.plugins import discover_plugins, get_plugin_manager
...
def _cmd_sessions_lazy(args, **kwargs):
    """``cryozen sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from cryozen_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _build_cli_parser():
    """Build the full ``cryozen`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``cryozen --help`` order; keep it stable. Groups
    live in ``cryozen_cli/subcommands/<group>.py`` with handlers injected so
    ...
    # OUTBOUND egress firewall; ``cryozen proxy`` (gateway group) is the INBOUND one.
    ...
    from cryozen_cli.send_cmd import register_send_subparser
    ...
    from cryozen_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    from cryozen_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from cryozen_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # "desktop" is canonical (Cryozen-Setup.exe tells users to run it, so it
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``cryozen -c model`` — 'model' is the session name)
...
def main():
    """Main entry point for cryozen CLI."""
    _set_process_title()
    ...
    try:
        from cryozen_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``cryozen.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_cryozen_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``cryozen update``, and
    ...
    # substring match is deliberately loose: over-matching (``cryozen skills
    # install update``) only defers recovery one launch; under-matching
    # (``cryozen -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from cryozen_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('cryozen_cli.cli_output', 'line_input'),
}
