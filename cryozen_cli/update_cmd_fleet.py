"""Gateway fleet restart + post-update verification for ``cryozen update``.

Split out of ``cryozen_cli/update_cmd.py``; every name is re-imported there so
``cryozen_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
from cryozen_cli.update_cmd_common import _best_effort
from cryozen_cli.update_inventory import _gateway_service_matches_profile

# Log-record parity with the origin module.
logger = logging.getLogger("cryozen_cli.update_cmd")

# Under CRYOZEN_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "cryozen-gateway*", "cryozen-serve*", "--plain", "--no-legend", "--no-pager"]


def _write_gateway_update_exit_code(ok: bool) -> None:
    from cryozen_cli.update_cmd import get_cryozen_home
    path = get_cryozen_home() / ".update_exit_code"
    with suppress(OSError):
        path.write_text("0" if ok else "1", encoding="utf-8")


def _fleet_restart_pending_marker_path() -> Path:
    """LEGACY per-``CRYOZEN_HOME`` breadcrumb. Read-compat only — nothing writes it any more.

    One host runs one multiplexing gateway, so the pull→restart obligation is host-scoped
    (``cryozen_cli/update_host_obligation.py``). An obligation armed by the old per-profile code
    is still read and cleared here so an in-flight update is discharged after the upgrade.
    """
    from cryozen_cli.update_cmd import get_cryozen_home
    return get_cryozen_home() / _FLEET_RESTART_PENDING_NAME


def _write_legacy_fleet_restart_pending_marker(
    *, expected_sha: str = "", runtimes: list[dict] | None = None
) -> bool:
    """Arm the LEGACY per-``CRYOZEN_HOME`` marker. True when written. Never raises.

    Fallback only: ``$CRYOZEN_HOME`` is writable by construction (the updater already writes its
...
def _write_fleet_restart_pending_marker(*, expected_sha: str = "", runtimes: list[dict] | None = None) -> None:
    ...
    An unwritable host state dir (``CRYOZEN_GATEWAY_LOCK_DIR`` on a read-only mount, a container
    ...
    if runtimes == []:
        ...
        # a no-gateway host would then fail every later ``cryozen update`` (#115311).
        return
    from cryozen_cli.update_cmd import _m
    from cryozen_cli.update_host_obligation import host_obligation_path, write_host_obligation
    ...
    print(
        ...
        "restart gateways with `cryozen gateway restart` if this update is interrupted.",
        file=sys.stderr,
    )
...
def _clear_fleet_restart_pending_marker() -> None:
    ...
    from cryozen_cli.update_cmd import _m
    from cryozen_cli.update_host_obligation import clear_host_obligation
...
def _fleet_restart_obligation_armed() -> bool:
    ...
    from cryozen_cli.update_host_obligation import host_obligation_present
...
def _obligation_fields() -> dict[str, str] | None:
    ...
    """
    from cryozen_cli.update_host_obligation import host_obligation_present, obligation_fields
...
def _current_checkout_sha() -> str | None:
    ...
    from cryozen_cli.update_cmd import _capture_head_sha, _m
    try:
        from cryozen_cli.build_info import get_code_identity
...
def _receipt_looks_unfinished(receipt: dict) -> bool:
    ...
    successful receipt look unfinished, or the next ``cryozen update`` retriggers
...
def _receipt_reports_stale_runtime(receipt: dict, expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from cryozen_cli.update_cmd import _current_checkout_sha
    ...
    from cryozen_cli.update_receipt import row_is_external
...
def _live_fleet_covers_receipt(expected_sha: str | None, receipt: dict, owed: set[tuple[str, str]] | None, *, accept_states: tuple = ("current",)) -> bool:
    ...
    if not expected_sha:
        return False
    from cryozen_cli.update_receipt import collect_fleet_versions, row_is_external
...
def _marker_only_restart_obsolete() -> bool:
    ...
    """
    from cryozen_cli.update_serve_obligations import defer_manual_serve
...
def _pending_fleet_restart_needed(*, receipt: dict | None = None, pending_manual: list[dict] | None = None) -> bool:
    ...
    from cryozen_cli.update_cmd import _current_checkout_sha
    from cryozen_cli.update_receipt import read_latest_receipt
    from cryozen_cli.update_serve_obligations import retain_receipt_manual_serves
...
def _update_owes_fleet_restart(*, receipt: dict | None = None, pending_manual: list[dict] | None = None) -> bool:
    ...
    # A fleet an operator has since restarted onto a moved checkout (``cryozen gateway restart`` —
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print("⚠ A previous `cryozen update` pulled new code but did not restart running gateways.", file=stream)
    print("  Gateways may still be serving pre-update modules (mixed sys.modules).", file=stream)
    if startup:
        print("  Run `cryozen update` or `cryozen gateway restart`.", file=stream)


def _warn_pending_fleet_restart_on_startup() -> None:
    """Cheap CLI-startup hint. Never restarts; never raises."""
    from cryozen_cli.update_receipt import read_latest_receipt
    from cryozen_cli.update_serve_obligations import retain_receipt_manual_serves, warn_pending_manual_serves
...
def _restart_systemd_gateway_units_best_effort(failed: list, listings) -> None:
    """Restart every cryozen-gateway/serve unit ONCE PER LIVE HOST PROCESS.

    One host runs one multiplexing gateway, so leftover per-profile units
    (``cryozen-gateway-<profile>.service``) all point at the SAME live ``MainPID``; restarting
    ...
    """
    from cryozen_cli.update_host_obligation import collapse_units_to_host_processes
    ...
    for unit_key, owner_key in covered.items():
        print(
            ...
            "together with: cryozen gateway migrate"
        )
...
def _live_fleet_current_rows() -> list[dict] | None:
    ...
    if not checkout_sha:
        return None
    try:
        from cryozen_cli.update_receipt import collect_fleet_versions
        fleet = collect_fleet_versions()
    except Exception as exc:
...
def _restart_identity_sha() -> str:
    ...
    installs it exists for: each profile's ``cryozen update`` would re-kill the one shared
...
def _run_pending_fleet_restart() -> bool:
    ...
    ``cryozen update`` must attach to the first one's restart instead of killing the shared
    gateway again (the obligation record carries the proof).

    See #95294.
    """
    from cryozen_cli.update_cmd import _m
    from cryozen_cli.update_host_obligation import host_restart_already_completed, mark_host_restart_completed
    ...
    # Warn if legacy Cryozen gateway unit files are still installed. When both cryozen.service (from a
    # pre-rename install) and the current cryozen-gateway.service are enabled, they SIGTERM-fight for the
    # same bot token (see PR #11909). Flagging here means every `cryozen update` surfaces the issue until the
    # user migrates.
    try:
        from cryozen_cli.gateway import (
    ...
    # A gateway this very update cold-started (or a manual `cryozen gateway restart` seconds
    ...
    try:
        ...
        # --- Systemd services (Linux) --- Discover all cryozen-gateway* units (default + profiles) plus
        # cryozen-serve* units (the Desktop app's backend, #83438).
        if systemd_listings is not None:
            _restart_systemd_gateway_units_best_effort(failed, systemd_listings)
        # --- Launchd services (macOS) --- Restart EVERY ai.cryozen.gateway* LaunchAgent, not only the
...
def _defer_fleet_restart_after_update(*, update_complete: bool, resume_incomplete: bool = False) -> None:
    ...
    ``cryozen update --no-gateway-restart`` (cron running inside the gateway's
    ...
    next normal update (or ``cryozen gateway restart``) catches up.
    ...
    print("  Restart them separately: `cryozen gateway restart` or a daily-restart cron.")
    print("  (fleet restart deferred — marker kept for catch-up)")
    with suppress(Exception):
        from cryozen_cli.update_receipt import record_skip
        record_skip("gateway_restart", "--no-gateway-restart: deferred, marker kept")
    partial = (not update_complete) or resume_incomplete
    with suppress(Exception):
        from cryozen_cli.update_receipt import finalize_update_receipt
...
def _apply_pending_fleet_restart_catchup(*, defer: bool = False) -> None:
    """On an already-up-to-date ``cryozen update``, finish a skipped restart.
    ...
    """
    from cryozen_cli.update_cmd import _run_pending_fleet_restart
    ...
    if defer:
        ...
        print("  Restart separately: `cryozen gateway restart` or next non-cron update.")
        return
    print()
    _warn_pending_fleet_restart()
    print("→ Running the pending fleet restart...")
    if not _run_pending_fleet_restart():
        print("  ⚠ Fleet restart incomplete. Recover with: cryozen gateway restart")
        sys.exit(1)
    if not _pending_fleet_restart_needed():
        return
    ...
    # clears, even after a successful manual `cryozen gateway restart` (#117051).
    fleet = _live_fleet_current_rows()
    if fleet is not None:
        from cryozen_cli.update_receipt import settle_latest_receipt_fleet
        ...
        if settled:
            ...
            return
    print("  ⚠ Fleet restart ran, but gateways are still off the checkout code. Recover with: cryozen gateway restart")
    sys.exit(1)
...
def _is_cryozen_gateway_unit(unit: str) -> bool:
    """Exact base unit or hyphenated profile family only: ``startswith("cryozen-serve")``
    would accept ``cryozen-server.service``."""
    return (
        ...
        unit == "cryozen-gateway.service"
        or unit.startswith("cryozen-gateway-")
        or unit == "cryozen-serve.service"
        or unit.startswith("cryozen-serve-")
    )


def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each cryozen-gateway*/cryozen-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_cryozen_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``cryozen-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``cryozen-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``cryozen-gatewayd`` is never signalled.

    See #83438.
    """
    return svc_name == "cryozen-gateway" or svc_name.startswith("cryozen-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from cryozen_cli.gateway import is_macos
    if not failed_units:
        return
    ...
    print("    cryozen gateway status")
    if any(not name.startswith("ai.cryozen.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.cryozen.") for name in ordered):
...
def _restart_launchd_gateway_after_update(*, supervision_verify: bool = True) -> tuple[list, list]:
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: cryozen gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: cryozen gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    The pull is shared across profiles, so every ``ai.cryozen.gateway*`` LaunchAgent
    ...
    # Units labelled before the profile-name suffix scheme (ai.cryozen.gateway-<hash>) are invisible
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``cryozen_cli.gateway`` no longer importing
    ...
    """
    try:
        from cryozen_cli.gateway import find_gateway_pids
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither CRYOZEN_SPAWN
# nor CRYOZEN_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a cryozen-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``cryozen_cli.gateway`` in the
    ...
    child's ``cryozen-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``cryozen_cli.gateway`` from the freshly
...
def _gateway_home_for_pid(pid: int):
    """CRYOZEN_HOME of the gateway ``pid`` per the fleet inventory, else None (own profile's file)."""
    with suppress(Exception):
        from cryozen_cli.update_receipt import _profile_homes
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to cryozen-gateway* qualifies (idempotent
...
def _repair_unit_without_fatal_exit_park(svc_name: str, scope: str) -> None:
    ...
    if system and not _needs_sudo(scope) and svc_name == get_service_name():
        # The refresh adopts the unit's CRYOZEN_HOME into os.environ (sudo strips it); the rest of the
        # update keeps running for the invoking profile.
        launch_home = os.environ.get("CRYOZEN_HOME")
        try:
            ...
            if launch_home is None:
                ...
                os.environ["CRYOZEN_HOME"] = launch_home
        return
    print(
        ...
        f"    Repair: {'sudo ' if system else ''}cryozen gateway install{' --system' if system else ''}"
    )
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. cryozen-serve has no handler → blunt restart below.
    _main_pid = 0
    ...
    if _manage_cmd is None:
        ...
        print(
            ...
            f"    To let `cryozen update` restart it automatically, allow\n"
    ...
    # budget, restart-policy mismatch). Mirrors `cryozen gateway restart` (`systemd_restart()`).
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    """Restart every active cryozen-gateway*/cryozen-serve* systemd unit (user + system).
    ...
    """
    from cryozen_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
    ...
    def _on_list_timeout(scope: str, exc: subprocess.TimeoutExpired) -> None:
        ...
        print(
            ...
            f"Check the gateway with: cryozen gateway status"
        )
...
class _GatewayRestartOutcome:
    ...
    def record_receipt(self, **extra) -> None:
        ...
        with suppress(Exception):
            from cryozen_cli.update_receipt import record_gateway_restart
...
def _restart_manual_gateways(out: _GatewayRestartOutcome, _drain_budget) -> None:
    ...
    if out.restarted_services or out.killed_pids:
        ...
        if unmapped_count:
            ...
            print("    Restart manually: cryozen gateway run")
            if unmapped_count > 1:
                print("    (or: cryozen -p <profile> gateway run  for each profile)")
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from cryozen_cli.gateway import find_gateway_pids, _get_service_pids
...
def _recover_after_restart_phase_abort(
    ...
    from cryozen_cli.update_abort_recovery import _owed_stale_serve_rows
    from cryozen_cli.update_cmd import (
...
def _gateway_drain_budget() -> float:
    ...
    try:
        from cryozen_cli.gateway import _get_restart_exit_wait_budget
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from cryozen_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/cryozen-serve`` vs ``system/cryozen-serve`` are different
    ...
    try:
        ...
        from cryozen_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        # macOS: EVERY ai.cryozen.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy cryozen.service fights cryozen-gateway.service over the bot token; warn on
    every update until migrated."""
    from cryozen_cli.gateway import (has_legacy_cryozen_units, _find_legacy_cryozen_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_cryozen_units()):
        return
    print()
    print("⚠ Legacy Cryozen gateway unit(s) detected:")
    for name, path, is_sys in _find_legacy_cryozen_units():
        scope = "system" if is_sys else "user"
        print(f"    {path}  ({scope} scope)")
    print()
    print("  These pre-rename units (cryozen.service) fight the current")
    print("  cryozen-gateway.service for the bot token and cause SIGTERM")
    print("  flap loops. Remove them with:")
    print()
    print("    cryozen gateway migrate-legacy")
    print()
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    # units, so a unit-less `cryozen serve` keeps stale sys.modules. Runs AFTER
    ...
    # units, so an sshd-spawned `serve --isolated` or a manual `cryozen serve` (no unit) is left running its
    ...
    print("  cryozen model              # Select provider and model")
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from cryozen_cli.update_receipt import print_fleet_version_matrix
        ...
        if print_fleet_version_matrix(_fleet_snapshot):
            ...
            from cryozen_cli.update_cmd_stale_survivors import signal_stale_fleet_survivors
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from cryozen_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            if report_unaccounted_runtimes(_runtime_outcomes):
                restart.incomplete = True
            with suppress(Exception):
                import cryozen_cli.update_receipt as _ur
    ...
    with _best_effort('Multiplex auto-migration after update failed: %s'):
        from cryozen_cli.gateway_migrate import maybe_auto_migrate_after_update
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``cryozen_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
