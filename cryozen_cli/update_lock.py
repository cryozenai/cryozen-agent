"""Cross-process mutual exclusion for in-flight Cryozen updates.
...
MARKER_NAME = ".cryozen-update-in-progress"

# Set by an orchestrating updater (Tauri `cryozen-setup --update`) to its own pid before
# spawning `cryozen update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "CRYOZEN_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Cryozen is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Cryozen home (never the context-local profile override): the Rust
    updater resolves ``$CRYOZEN_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_cryozen_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``cryozen update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Cryozen update is already running (started {elapsed} ago, "
        f"process {holder.pid}).\n"
        "\n"
        "  Running two at once would corrupt the install. Wait for it to finish\n"
        "  (watch `cryozen logs`), or close the Desktop/dashboard window that\n"
        "  started it, then run `cryozen update` again."
    )
