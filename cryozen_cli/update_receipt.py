...
# ``cryozen update`` is a single-threaded CLI command; a module singleton lets the 7k-line updater
...
class UpdateReceipt:
    """Collects the observable facts of one ``cryozen update`` run."""
    ...
    def gateway_restart_result(
        ...
        if fresh_recovery is not None:
            ...
            # ``cryozen serve`` hosts tui_gateway and is not a gateway profile, so neither the
...
def _receipt_dir() -> Path:
    # ``cryozen_constants`` (stdlib-only), never ``cryozen_cli.config``: the receipt must be
    ...
    return get_cryozen_home() / "logs" / "update_receipts"
...
def finalize_pending_update_receipt(exit_code: Optional[int] = None, stop_reason: str = "") -> Optional[Path]:
    ...
    ``cryozen update`` has many early ``sys.exit`` paths (preflight refusals, venv-holder refusal,
...
def settle_latest_receipt_fleet(fleet: list[dict[str, Any]], *, discharges) -> bool:
    ...
    pre-pull SHAs) keeps ``cryozen update`` exiting 1 and every CLI start warning about mixed
    modules, long after the operator's ``cryozen gateway restart`` fixed the fleet (#117051). The
...
def _profile_homes() -> list[tuple[str, Path]]:
    ...
    from cryozen_cli.profiles import _get_default_cryozen_home, _get_profiles_root, _PROFILE_ID_RE
...
def _code_root_for_path(raw: Any) -> Optional[Path]:
    """Return the Cryozen checkout containing an absolute process path."""
    ...
    with suppress(Exception):
        ...
        for parent in [candidate, *candidate.parents][:_CODE_ROOT_MAX_DEPTH]:
            if (parent / "cryozen_cli" / "main.py").is_file():
...
_FLEET_ROW_IDENTITY_PENDING = (
    ...
    " — re-check with `cryozen gateway status`"
)
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if stale_or_down:
        ...
        print("  Run `cryozen gateway restart` (or `cryozen -p <profile> gateway restart` for a named")
        print("  profile), then `cryozen gateway status` to confirm.")
