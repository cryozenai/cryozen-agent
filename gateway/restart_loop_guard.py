...
Defenses 1-2 (``_CRYOZEN_GATEWAY`` guard on ``cryozen gateway stop|restart`` /
...
persisted to ``<CRYOZEN_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_cryozen_home() / "gateway" / "restart_loop.json"
