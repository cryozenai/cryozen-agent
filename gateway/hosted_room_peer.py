...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    return hmac.new(material, b"cryozen-hosted-room-installation-grant-v1", hashlib.sha256).digest()
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local CRYOZEN_HOME override; the process environment
        # retains the installation root and is the authority here.
        root = os.environ.get("CRYOZEN_HOME") or get_cryozen_home()
...
def derive_room_grant_secret(api_key: str) -> bytes:
    ...
    return hmac.new(api_key.encode("utf-8"), b"cryozen-hosted-room-grant-v1", hashlib.sha256).digest()
...
def catalog_mapping(
    *, installation_id: str, protocol_versions: Iterable[int] = (PROTOCOL_VERSION,),
    ...
    catalog per served profile, so there is no env (``CRYOZEN_PROFILE``) fallback (#116900)."""
    # A Desktop-managed gateway exits with the app: the caller's flag is only an upper bound.
    persistent_process = bool(persistent_process and os.getenv("CRYOZEN_DESKTOP") != "1")
...
def _configured_room_link_url() -> str | None:
    ...
    if (override := os.getenv("CRYOZEN_ROOM_LINK_URL")) is not None:
        return override
