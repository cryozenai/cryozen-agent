...
class GatewayInboundMixin:
    ...
    async def _hm_pre_gateway_dispatch_hook(
        ...
        try:
            from cryozen_cli.lifecycle import ainvoke_hook as _ainvoke_hook
    ...
    async def _hm_report_ignored_dm(self, source: SessionSource) -> None:
        ...
        hint = unauthorized_owner_hint(
            platform_name, source.user_id, source.user_name or "", cryozen_home=display_cryozen_home(),
        )
    ...
    async def _hm_admit_event(
        ...
        # spawning context, which may carry ANOTHER message's CRYOZEN_SESSION_* ContextVars; until
        ...
        if (
            ...
            and not is_internal
            and not getattr(event, "_cryozen_startup_restore_replay", False)
        ):
    ...
    def _hm_estop_turn_allowed(self, event: "MessageEvent", source: SessionSource) -> bool:
        ...
        with suppress(Exception):
            ...
            if _estop_cmd:
                from cryozen_cli.commands import resolve_command as _resolve_estop_cmd
    ...
    def _hm_estop_gate(
        ...
        """Global emergency-stop (`cryozen pause`) notice when this turn must be blocked, else None.
    ...
    def _hm_write_update_response(response_text: str) -> Optional[str]:
        ...
        response_path = _cryozen_home / ".update_response"
        try:
            tmp = response_path.with_suffix(".tmp")
            tmp.write_text(response_text, encoding="utf-8")
            tmp.replace(response_path)
            (_cryozen_home / ".update_prompt.json").unlink(missing_ok=True)
        except OSError as e:
            return str(e)
        return None
    ...
    def _hm_update_prompt_reply(self, event: "MessageEvent", _quick_key: str) -> Optional[str]:
        ...
        if cmd in {"approve", "yes"}:
            ...
            if cmd:
                with suppress(Exception):
                    from cryozen_cli.commands import resolve_command as _resolve_update_cmd
    ...
    def _hm_evict_idle_stale_agent(self, _quick_key: str) -> None:
        ...
        _raw_stale_timeout = _float_env("CRYOZEN_AGENT_TIMEOUT", 1800)
    ...
    async def _hm_busy_slash_or_photo(
        ...
        from cryozen_cli.commands import resolve_command as _resolve_cmd_inner
    ...
    def _hm_busy_telegram_grace_queue(
        ...
        _grace = float(os.getenv("CRYOZEN_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "3.0"))
    ...
    async def _hm_resolve_command(
        ...
        from cryozen_cli.commands import is_gateway_known_command, resolve_command as _resolve_cmd
    ...
    async def _hm_dispatch_quick_and_plugin_commands(
        ...
        if command:
            try:
                from cryozen_cli.plugins import get_plugin_command_handler
                plugin_handler = get_plugin_command_handler(command.replace("_", "-"))
                if plugin_handler:
                    # The agent-turn path binds CRYOZEN_SESSION_* via _set_session_env; this dispatch
    ...
    def _hm_unknown_slash_reply(command: str, source: SessionSource) -> Optional[str]:
        ...
        from cryozen_cli.commands import GATEWAY_KNOWN_COMMANDS
            ...
            if _plat and (_skill_name or extra_keys):
                ...
                if _disabled_extra:
                    return (
                        ...
                        f"Enable them with: `cryozen skills config`"
                    )
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        if not is_internal:
            if await asyncio.to_thread(self._is_telegram_topic_root_lobby, source):
                ...
                return None
            # External-drain new-turn gate: when an orchestrator engaged an external drain (.drain_request.json,
    ...
    async def _dispatch_plugin_message_injection(
        ...
        await adapter.handle_message(MessageEvent(
            ...
            metadata={
                "cryozen_plugin_id": plugin_id, "cryozen_plugin_injection": True,
