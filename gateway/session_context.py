"""Session-scoped context variables for the Cryozen gateway.

Replaces the old ``os.environ``-based ``CRYOZEN_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "CRYOZEN_SESSION_PLATFORM", "CRYOZEN_SESSION_SOURCE", "CRYOZEN_SESSION_CHAT_ID",
    "CRYOZEN_SESSION_CHAT_TYPE", "CRYOZEN_SESSION_CHAT_NAME", "CRYOZEN_SESSION_THREAD_ID",
    "CRYOZEN_SESSION_USER_ID", "CRYOZEN_SESSION_USER_ID_ALT", "CRYOZEN_SESSION_USER_NAME",
    "CRYOZEN_SESSION_SCOPE_ID", "CRYOZEN_SESSION_KEY", "CRYOZEN_SESSION_ID",
    "CRYOZEN_UI_SESSION_ID", "CRYOZEN_SESSION_MESSAGE_ID", "CRYOZEN_SESSION_PROFILE",
    "CRYOZEN_BROWSER_CONTROL_PRINCIPAL", "CRYOZEN_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "CRYOZEN_CRON_SESSION", "CRYOZEN_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("CRYOZEN_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("CRYOZEN_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("CRYOZEN_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("CRYOZEN_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("CRYOZEN_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``CRYOZEN_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``CRYOZEN_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds CRYOZEN_SESSION_PLATFORM, CLI/TUI/
# desktop bind CRYOZEN_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``CRYOZEN_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("CRYOZEN_PLATFORM") or get_session_env("CRYOZEN_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("CRYOZEN_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See cryozenai/cryozen-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``CRYOZEN_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
