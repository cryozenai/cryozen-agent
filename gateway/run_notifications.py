...
# (`cryozen update` re-runs it in the terminal) and only a short tail is quoted in chat.
_UPDATE_FAILED_NOTICE = (
    "❌ Cryozen update failed; the previous version is still running. Run `cryozen update` on the "
...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``cryozen update --gateway`` and its watcher exchange under CRYOZEN_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_cryozen_home / ".update_pending.json",
            claimed=_cryozen_home / ".update_pending.claimed.json", output=_cryozen_home / ".update_output.txt",
            exit_code=_cryozen_home / ".update_exit_code",
            prompt=_cryozen_home / ".update_prompt.json", response=_cryozen_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``cryozen update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Cryozen update finished." if exit_code == 0 else _UPDATE_FAILED_NOTICE
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Cryozen update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
    ...
    async def _send_update_notification(self) -> bool:
        ...
        try:
            ...
            if chat_id:
                ...
                if exit_code == 0:
                    msg = "✅ Cryozen update finished successfully."
                    if output:
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.delivery import resolve_delivery_transport
        from gateway.run import _cryozen_home, _non_conversational_metadata
        notify_path = _cryozen_home / ".restart_notify.json"
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — Cryozen is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if getattr(self, "_session_db_handle_cache", None) is not None:
            ...
            if not error:
                ...
                return
        from cryozen_constants import get_default_cryozen_root, profile_cli_selector
        from cryozen_state import _default_db_path, classify_persistence_error
        cause = classify_persistence_error(error)
        # Copy-pasteable, so name the real store and pin the profile: a bare `cryozen` follows
        # active_profile, which may be a different database (#105887).
        profile_arg = profile_cli_selector()
        if cause == "corrupt":
            db_path = _default_db_path()
            backups_dir = get_default_cryozen_root() / "backups"
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                f"1. Run `cryozen {profile_arg}doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   cryozen {profile_arg}sessions recover --source {db_path} "
                "--inspect-only\n"
                f"   (if it reports recoverable) cryozen {profile_arg}sessions recover "
                ...
                f"Run `cryozen {profile_arg}doctor` for sanitized diagnostics."
            )
            ...
            message = (
                ...
                f"it is repaired: run `cryozen {profile_arg}doctor --fix`, then restart the gateway. Do not run "
                f"recovery tools or restore a backup unless `cryozen {profile_arg}doctor` confirms damage."
            )
        else:
            ...
            if failure.cause not in _SELF_CLEARING_STORAGE_CAUSES:
                action = f"{action} Then `cryozen {profile_arg}gateway restart`."
            message = (
    ...
    async def _inject_watch_notification(
        ...
        if not source:
            # API-server sessions bind the RAW X-Cryozen-Session-Id key, not a structured ``agent:...`` key.
    ...
    def _completion_event_scope(self, evt: dict):
        ...
        (``tools.async_delegation`` → ``get_cryozen_home()/state.db``) resolve from the ambient scope.
        ...
        if get_cryozen_home_override() == str(profile_home):
