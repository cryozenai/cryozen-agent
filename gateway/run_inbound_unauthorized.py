...
def pairing_code_reply(platform_name: str, code: str, profile_arg: str = "") -> str:
    ...
    approve_cmd = f"cryozen {profile_arg}pairing approve {platform_name} {code}"
    return (
...
def unauthorized_owner_hint(
    platform_name: str, user_id: str, user_name: str = "", *, cryozen_home: str,
) -> str:
    ...
    allowlist = (
        f"add the ID to {env_var} in {cryozen_home}/.env and restart the gateway"
    ...
    return (
        ...
        f"{cryozen_home}/config.yaml so unknown senders receive a pairing code you can approve with "
        f"`cryozen pairing approve {platform_name} <code>`."
    )
