...
class GatewayAgentCacheMixin:
    ...
    def _is_intentional_model_switch(self, session_key: str, agent: Any, config_model: str) -> bool:
        ...
        override names that model. Anything else (a fallback onto some other model) is ordinary drift
        and still evicts."""
        override = self._session_model_override(session_key)
        return override is not None and override.get("model") == agent.model
    ...
    def _bind_adapter_run_generation(self, adapter: Any, session_key: str, generation: int | None) -> None:
        ...
        with suppress(Exception):
            ...
            if interrupt_event is not None:
                interrupt_event._cryozen_run_generation = int(generation)
    ...
    async def _interrupt_and_clear_session(
        ...
        if running_agent and running_agent is not _AGENT_PENDING_SENTINEL:
            ...
            try:
                from cryozen_cli.plugins import invoke_hook as _invoke_hook
    ...
    def _ephemeral_change_key(context, redact_pii: bool) -> str:
        ...
        try:
            ...
            home_display = str(display_cryozen_home())
        except Exception:
            home_display = ""
        key_tuple = (
    ...
    def _run_release_in_profile_scope(self, target, args: tuple, session_key: Optional[str]) -> None:
        ...
        root Cryozen dir even when the gateway was launched under a named profile. Its scope is
            ...
            if session_key and store is not None:
                try:
                    ...
                    logger.warning("Could not resolve the owning profile for %s; releasing under the default profile",
                                   session_key, exc_info=True)
            owner_home = Path(owner) if owner else get_default_cryozen_root()
            if current_secret_scope() is None or cryozen_home_key(get_cryozen_home()) != cryozen_home_key(owner_home):
