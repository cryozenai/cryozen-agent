...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``cryozen setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``cryozen gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved CRYOZEN_HOME and overlay the
        ...
        # `cryozen` invocation, so the import happens only when a lookup asks for it.
