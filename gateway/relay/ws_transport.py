...
def _normalize_slack_parent_command(text: str, message_type: MessageType) -> tuple[str, MessageType]:
    """Mirror native Slack ``/cryozen`` routing for authenticated relay text."""
    parent_parts = text.strip().split(maxsplit=1)
    if not parent_parts or parent_parts[0] != "/cryozen":
...
def _event_from_wire(raw: Dict[str, Any]) -> MessageEvent:
    ...
    source = SessionSource(
        ...
        # Multiplex mode: the connector stamps the target Cryozen profile; None on
    ...
    if platform_enum == Platform.SLACK:
        ...
        # gateway command rather than the legacy `cryozen` parent name.
...
class PassthroughForward:
    ...
    # SessionSource (#60586) — the connector stamps it when it resolves the target profile for a
...
class WebSocketRelayTransport:
    ...
    def __init__(
        ...
        *,
        ...
        # Set while a connector-brokered suspend is in flight. See _await_redial_hold.
