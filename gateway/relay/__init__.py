"""Relay/connector support package for the Cryozen gateway.
...
def _env_or_cfg(env_var: str, cfg_key: str) -> str:
    """Env var first (set by the hosting container), then ``gateway.<cfg_key>``; stripped, ``""`` when unset."""
...
def relay_explicitly_disabled() -> bool:
    ...
    try:
        cfg = read_yaml_layers(get_cryozen_home())
...
def relay_instance_id() -> Optional[str]:
    ...
    so inbound routes per-instance rather than tenant-broadcast (the hosting platform
    stamps its instance id). Tenant-scoped like ``relay_endpoint()``; absent -> null."""
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from cryozen_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**Cryozen Agent:**" and shadow the connector's
        # linked-owner fallback, which actually disambiguates. Only a customized
        # name is forwarded.
        if value == "Cryozen Agent":
            value = ""
...
def _post_provision(
    *,
    ...
    routeKeys}`` (the connector validates ``access_token`` against its IdP, derives the
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``cryozen gateway enroll``.
    ...
    With no ``token_url`` configured there is no identity to resolve: raises.
    ...
    if not token_url:
        raise RuntimeError(
            "no relay identity provider configured: set gateway.idp.token_url "
            "(GATEWAY_RELAY_IDP_TOKEN_URL), with client_id + client_secret for the OAuth2 "
            "client_credentials grant or neither for an ambient token endpoint."
        )
...
def self_provision_relay() -> bool:
    ...
    ``~/.cryozen-agent/.env``), so a hosted container re-provisions every boot; the
    connector's rotation window covers a still-connected prior instance. The trigger
    is deliberately NOT ``is_managed()``: "pointed at a connector without a pinned
    secret" is the real signal and self-guards (an
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'cryozen'}"
