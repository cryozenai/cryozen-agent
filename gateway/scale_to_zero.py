...
SOLELY by the hosting orchestrator's env stamp (not config); the idle timeout IS config.yaml. Quiesce
...
SCALE_TO_ZERO_ENV = "CRYOZEN_SCALE_TO_ZERO"  # stamped by the hosting orchestrator
...
# Orchestrator-brokered suspend, stamped where the guest cannot suspend itself. Carries
...
def dashboard_client_heartbeat_path(cryozen_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under CRYOZEN_HOME."""
    if cryozen_home is None:
        from cryozen_constants import get_cryozen_home
        cryozen_home = get_cryozen_home()
    return Path(cryozen_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
...
def brokered_sleep_url(environ: Optional[dict] = None) -> Optional[str]:
    """The orchestrator sleep endpoint to POST, or None when this backend has no broker.
...
# Must EXCEED the broker's own hard request ceiling (route maxDuration = 30s),
...
def request_brokered_suspend(
    ...
    *,
    ...
    """POST the orchestrator sleep URL so it stops this machine on our behalf.
    ...
    ok = 200 <= status < 300
    if ok:
        logger.info("scale-to-zero: machine suspend accepted by the orchestrator (%s)", status)
    else:
