...
from cryozen_cli.config import atomic_config_write, cfg_get
...
_BUSY_MODE_BEHAVIOR = {
    "queue": ("queues for next turn", "Messages will be queued for the next turn while Cryozen is busy."),
    "steer": ("steers into current run (after next tool call)",
              "Messages will be steered into the current run (after the next tool call)."),
    "interrupt": ("interrupts current run", "Messages will interrupt the current run while Cryozen is busy."),
}
...
def _spawn_detached_update(cryozen_cmd, output_path, exit_code_path) -> None:
    """Spawn ``cryozen update --gateway`` detached so it survives the gateway restart it may trigger.
    ...
    this interpreter (not venv\\Scripts\\cryozen.exe — that shim holds its own file open, and the
    update must replace it), redirects both outputs to one file and writes the exit code."""
    import shutil
    import subprocess
    if sys.platform == "win32":
        from cryozen_cli._subprocess_compat import windows_detach_popen_kwargs
        subprocess.Popen(
            [sys.executable, "-c", _WINDOWS_UPDATE_HELPER, str(output_path), str(exit_code_path),
             sys.executable, "-m", "cryozen_cli.main", "update", "--gateway"],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **windows_detach_popen_kwargs())
        return
    cryozen_cmd_str = " ".join(shlex.quote(part) for part in cryozen_cmd)
    update_cmd = (
        f"PYTHONUNBUFFERED=1 {cryozen_cmd_str} update --gateway"
...
class GatewaySlashCommandsMixin(
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """The prefix users can always type to reach Cryozen commands (adapter ``typed_command_prefix``,
    ...
    async def _kanban_auto_subscribe(self, event: MessageEvent, task_id: str, requested_board) -> bool:
        ...
        def _sub():
            from cryozen_cli import kanban_db as _kb
            from cryozen_cli import kanban_db_connect as _kbc
    ...
    async def _handle_platform_command(self, event: MessageEvent) -> str:
        ...
        if action == "pause":
            ...
            return f"✓ {name} paused. Resume with `/platform resume {name}` or `cryozen gateway restart` to reset."
        if not queued:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _cryozen_home
        ...
        async def _write_marker(name: str, build, label: str) -> None:
            try:
                await asyncio.to_thread(atomic_json_write, _cryozen_home / name, build(), indent=None)
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Cryozen Agent version."""
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        # (NOT `cryozen skills diff <name>`, which diffs a bundled skill against its stock version).
        if sub == "diff" and len(out) > 3000:
            pending_id = args[1] if len(args) > 1 else "<id>"
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.cryozen-agent/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Cryozen is working."""
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # get_cryozen_home()/skills, a contextvar override under multiplex.
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return ("No skill bundles installed.\nCreate one on the host with:\n"
                    "  `cryozen bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        protect privacy; ``cryozen debug share`` from the CLI does full uploads."""
        from cryozen_cli.debug import (_GATEWAY_PRIVACY_NOTICE, _best_effort_sweep_expired_pastes,
        ...
        # ``get_cryozen_home()`` and uploads them to a public paste. Losing the contextvar override
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update — spawn ``cryozen update`` detached (``setsid``) so it survives the gateway
        restart it may trigger; marker files let this or the next gateway process notify the user."""
        import json
        from gateway.run import _cryozen_home, _resolve_cryozen_bin
        ...
        if src.platform not in self._UPDATE_ALLOWED_PLATFORMS:
            try:
                ...
                return t("gateway.update.platform_not_messaging")
        if is_managed():
            return f"✗ {format_managed_message('update Cryozen Agent')}"
        if not (Path(__file__).parent.parent.resolve() / '.git').exists():
            return t("gateway.update.not_git_repo")
        cryozen_cmd = _resolve_cryozen_bin()
        if not cryozen_cmd:
            return t("gateway.update.cryozen_cmd_not_found")
        pending_path = _cryozen_home / ".update_pending.json"
        output_path = _cryozen_home / ".update_output.txt"
        exit_code_path = _cryozen_home / ".update_exit_code"
        pending = {
        ...
        try:
            _spawn_detached_update(cryozen_cmd, output_path, exit_code_path)
        except Exception as e:
