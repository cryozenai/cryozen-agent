...
class WhatsAppBehaviorMixin(OwnAccessPolicyMixin):
    ...
    DEFAULT_REPLY_PREFIX: str = "✻ *Cryozen Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/cryozen) is mirrored to CRYOZEN_HOME so npm install works."""
    import shutil
    from cryozen_constants import get_cryozen_home
    install_bridge = Path(__file__).resolve().parents[2] / "scripts" / "whatsapp-bridge"
    cryozen_home_bridge = get_cryozen_home() / "scripts" / "whatsapp-bridge"
    try:
        ...
        pass
    if cryozen_home_bridge.exists():
        return cryozen_home_bridge
    try:
        cryozen_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(install_bridge, cryozen_home_bridge, dirs_exist_ok=False)
        return cryozen_home_bridge
    except Exception:
        return install_bridge
