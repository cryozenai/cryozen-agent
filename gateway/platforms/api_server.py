...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    try:
        from cryozen_cli.profiles import profile_matches_home
...
_STATIC_FEATURE_FLAGS = {
    ...
    "session_continuity_header": "X-Cryozen-Session-Id",
    "session_key_header": "X-Cryozen-Session-Key"}
...
_BROWSER_CONTROL_WS_PROTOCOL = "cryozen-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "cryozen-browser-control-ticket."
...
def _cryozen_version() -> str:
    """Canonical Cryozen version: ``cryozen_cli.__version__`` (dist-info can be stale on
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    from cryozen_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    try:
...
def _request_agent_overrides(
    ...
    The virtual model (``cryozen-agent``) means "gateway default". A bare ``model`` without
    ``provider`` is honored only when ``allow_bare_model`` (generic clients hardcode "gpt-4o";
    OpenAI-compatible handlers pass the ``direct_model_requests`` opt-in, Cryozen-native
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            ...
            with suppress(Exception):
                ...
                db_path = str(get_cryozen_home() / "response_store.db")
        ...
        from cryozen_state_wal import apply_wal_with_fallback
...
_CORS_HEADERS = {
    ...
    "Access-Control-Allow-Headers": "Authorization, Content-Type, Idempotency-Key, X-Cryozen-Session-Id"}
_SECURITY_HEADERS = {
...
def _derive_chat_session_id(system_prompt: Optional[str], first_user_message: str) -> str:
    ...
    turns of an Open WebUI-style conversation), so one Cryozen session/sandbox is reused."""
...
class _ProviderAuthResolutionError(RuntimeError):
    ...
    def user_text(self) -> str:
        ...
        from cryozen_cli.auth import is_rate_limited_auth_error
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    """aiohttp server routing OpenAI-format requests through cryozen-agent's AIAgent."""
    ...
    def _resolve_model_name(explicit: str) -> str:
        """Advertised /v1/models name: explicit override > active profile name > "cryozen-agent"
        (precedence owned by ``cryozen_cli.model_switch.resolve_effective_model``)."""
        from cryozen_cli.model_switch import resolve_effective_model
        profile_name = ""
        with suppress(Exception):
        ...
        return resolve_effective_model(explicit, profile_name, "cryozen-agent")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from cryozen_cli.auth import has_usable_secret
        ...
        if not profile:
            with suppress(Exception):
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_cryozen_home())
            return nullcontext()
        from gateway.run import _profile_runtime_scope
    ...
    def _declared_conversation_session(self, gateway_session_key: Optional[str]) -> Optional[str]:
        """Resolve the live session a client declared with ``X-Cryozen-Session-Key`` (the key
    ...
    def _parse_session_key_header(
        ...
        """Validate ``X-Cryozen-Session-Key`` (per-channel memory scope) -> ``(key_or_None, None)``
        or ``(None, error)``. Requires API-key auth so a client can't guess another scope."""
        raw = request.headers.get("X-Cryozen-Session-Key", "").strip()
        if not raw:
            return None, None
        if not self._api_key:
            logger.warning(
                "X-Cryozen-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping.")
            return None, _error_response(
                "X-Cryozen-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        """SessionDB for the active profile home (the runtime scope redirects ``get_cryozen_home()``
        per profile). Sync, for ``_create_agent``; handlers use ``_ensure_session_db_async``."""
        if self._session_db is not None:
            return self._session_db
        try:
            from cryozen_constants import get_cryozen_home
            return self._open_and_cache_session_db(get_cryozen_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        "cryozen-agent" upstream as a provider model id 400s)."""
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "cryozen_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text not in allowed:
            return "api_server"
        return "cryozen_browser" if text == "browser" else text
    ...
    def _recover_or_record_model(self, model: str, runtime_kwargs: Dict[str, Any], gateway_session_key) -> str:
        ...
        # No model.default but a provider resolved (e.g. `cryozen auth add` without `cryozen model`).
        if not model and runtime_kwargs.get("provider"):
            with suppress(Exception):
                from cryozen_cli.models import get_default_model_for_provider
    ...
    def _select_agent_runtime(
        ...
        # cryozen_cli.model_switch.resolve_effective_model.
        from cryozen_cli.model_switch import resolve_effective_model
        if session_override:
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "cryozen-agent", "version": _cryozen_version()})
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": readiness["status"], "readiness": readiness, "platform": "cryozen-agent",
            "version": _cryozen_version(), "gateway_state": gw_state,
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — cryozen-agent plus configured model_routes aliases (alias + resolved
        ...
        def _model(mid: str, root: str, parent) -> Dict[str, Any]:
            return {"id": mid, "object": "model", "created": now, "owned_by": "cryozen", "permission": [],
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        ...
        try:
            from cryozen_cli.inventory import build_model_options_payload, load_picker_context
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "cryozen.api_server.capabilities", "platform": "cryozen-agent",
            "model": self._model_name,
            "auth": {"type": "bearer", "required": bool(self._api_key)},
            "runtime": {
                "mode": "server_agent", "tool_execution": "server", "split_runtime": False,
                "description": (
                    "The API server creates a server-side Cryozen AIAgent; "
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<CRYOZEN_HOME>/plugin-data/.../artifacts``-style controlled root), so artifacts never escape the
        ...
        try:
            ...
            # Unscoped fallback (tests/manual wiring): controlled root under the Cryozen home.
            try:
                from cryozen_state import get_cryozen_home
                root = Path(get_cryozen_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    async def _handle_toolsets(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            from cryozen_cli.tools_config import (
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Cryozen sessions."""
        db = await self._ensure_session_db_async()
        if db is None:
        ...
        # Exact-title lookup (`cryozen peer dm` -> canonical "Bot Chat"). include_hidden is honored
        ...
        if title_filter and not sessions:
            # A canonical Bot Chat auto-archived by the orphan reaper would make `cryozen peer dm`
            ...
            try:
                ...
                # which would fail `cryozen peer dm` resolution and mint transient sessions — same accident
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Cryozen session row. Existence check, insert and
        ...
        # would persist "cryozen-agent" and later send it to the provider literally.
        ...
        return web.json_response({"object": "cryozen.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "cryozen.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "cryozen.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "cryozen.session", "session": self._session_response(fork)}, status=201)
    ...
    def _session_headers(session_id: str, gateway_session_key: Optional[str]) -> Dict[str, str]:
        """``X-Cryozen-Session-Id`` (+ ``X-Cryozen-Session-Key`` when declared) response headers."""
        headers = {"X-Cryozen-Session-Id": session_id}
        if gateway_session_key:
            headers["X-Cryozen-Session-Key"] = gateway_session_key
        return headers
    ...
    def _effective_turn_runtime(self, runtime_request: Dict[str, Any], result: Any, usage: Any) -> Dict[str, Any]:
        ...
        return self._sanitize_runtime_metadata(
            ...
            # gateway/status readers resolve process-level paths and do NOT follow the CRYOZEN_HOME
    ...
    async def _answer_through_live_bot_chat(self, ctx: Dict[str, Any]) -> Optional["web.Response"]:
        ...
        This is the ``cryozen peer dm`` transport. Running the turn here would make this process a
        ...
        if record["status"] == "settled":
            return web.json_response(
                {"object": "cryozen.session.chat.completion", "session_id": session_id,
        ...
        if record["status"] in ("queued", "claimed"):
            return web.json_response(
                {"object": "cryozen.session.chat.queued", "session_id": session_id,
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        one bounded re-run of a transient failure; ``cryozen peer dm`` is the client)."""
        ...
        return web.json_response(
            {"object": "cryozen.session.chat.completion",
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        runtime = self._sanitize_runtime_metadata(
            ...
            model_lock="accepted")
        return web.json_response(
            {"object": "cryozen.session.model_lock", "session_id": session_id, "runtime": runtime})
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        # Optional transient per-run context (standalone `cryozen cron run` /
    ...
    def _bind_api_server_session(
        *, chat_id: str = "", session_key: str = "", session_id: str = "", profile: str = "",
        ...
        reach ``CRYOZEN_SESSION_PROFILE``: the persistent-Docker container key is derived from it, so an
    ...
    def _turn_runtime_metadata(
        ...
        runtime = dict(getattr(agent, "_cryozen_api_runtime", {}) or {})
