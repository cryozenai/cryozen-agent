...
There are two ways to add a platform to the Cryozen gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.cryozen-agent/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Cryozen code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `cryozen gateway status` or `get_connected_platforms()` until the SDK
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `cryozen_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Cryozen core.
...
```python
"cryozen-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _CRYOZEN_CORE_TOOLS,
    "includes": []
},
```

And add it to the `cryozen-gateway` composite:

```python
"cryozen-gateway": {
    "includes": [..., "cryozen-your-platform"]
}
```
...
## 12. Status Display (`cryozen_cli/status.py`)
...
## 13. Gateway Setup Wizard (`cryozen_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ cryozen_cli/ toolsets.py \
  --include="*.py" -l | sort -u
