...
def _reply_anchor_for_event(event) -> str | None:
    ...
    if (platform == "slack" and isinstance(raw_message, dict)
            and raw_message.get("_cryozen_no_thread_response")):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    ``CRYOZEN_SESSION_PLATFORM`` contextvar before the post-handler auto-TTS block runs.
    ...
    audio_path = os.path.join(
        tempfile.gettempdir(), "cryozen_voice", f"tts_reply_{uuid.uuid4().hex[:12]}.{ext}")
...
def resolve_proxy_url(
    ...
    *platform_env_var* is a per-adapter, per-profile-configurable setting (each proxy URL can
    ...
    process-env read — those are OS/system-level network settings, not a per-profile Cryozen concept."""
...
def _config_section(name: str) -> dict:
    ...
    try:
        from cryozen_cli.config import load_config_readonly as _load_config
...
from cryozen_constants import get_default_cryozen_root, get_cryozen_dir, get_cryozen_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.cryozen-agent/.env manually.")
...
# ``cryozen pairing approve`` command (cryozen_cli/subcommands/pairing.py) that lets the owner fix it.
...
UNAUTHORIZED_ACTION_NOTICE = (
    ...
    "`cryozen pairing approve {platform} <request-id>` on the host (`cryozen pairing list` shows the id).")
...
IMAGE_CACHE_DIR = get_cryozen_dir("cache/images", "image_cache")
...
def _cache_dir_accessors(kind: str, constant_name: str, new_subpath: str, old_name: str):
    ...
    get_cryozen_dir (active profile) unless a test monkeypatched the module constant away from
    its import-time default, and creates the directory; ``cleanup(max_age_hours=24)`` deletes
    older files and returns the count."""
    def get_dir() -> Path:
        d = get_cryozen_dir(new_subpath, old_name)
...
async def _cache_media_from_url(url: str, ext: str, retries: int, *, media_type: str, accept: str,
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; CryozenAgent/1.0)", "Accept": accept}
...
AUDIO_CACHE_DIR = get_cryozen_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_cryozen_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_cryozen_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_cryozen_dir("cache/screenshots", "browser_screenshots")
...
_CRYOZEN_ROOT = get_default_cryozen_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "CRYOZEN_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "CRYOZEN_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "CRYOZEN_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "CRYOZEN_MEDIA_DELIVERY_STRICT"
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    *(_CRYOZEN_HOME / "cache" / d for d in _MEDIA_DELIVERY_CACHE_SUBDIRS))
...
# Credential stores at the CRYOZEN_HOME root, denied per-file so skills/, logs/ and agent-written
...
def _profile_cache_roots() -> List[Path]:
    ...
    roots cover only the active CRYOZEN_HOME). Enumerated at check time so profiles created after
    startup count and are allowlisted BEFORE the ``/root`` denylist (CRYOZEN_HOME symlinked).

    ``CRYOZEN_HOME=/opt/data``) while the model emits a profile-scoped path silently fails delivery.
    ...
    when CRYOZEN_HOME is symlinked under a denied prefix and $HOME is not that prefix). See issue #31733.
    """
...
def _profile_dirs() -> List[Path]:
    ...
    try:
        return [p for p in (_CRYOZEN_ROOT / "profiles").iterdir() if p.is_dir()]
    except OSError:
        return []


def _credential_home_roots() -> List[Path]:
    """Every Cryozen home whose credential stores the denylist must cover: the ACTIVE home
    (the per-turn CRYOZEN_HOME override under ``gateway.multiplex_profiles``), the shared root
    ...
    return list(dict.fromkeys((get_cryozen_home(), _CRYOZEN_ROOT, *_profile_dirs())))


def _kanban_root() -> Path:
    """Kanban is root-shared across profiles by design (``kanban_db.kanban_home``)."""
    return Path(os.environ.get("CRYOZEN_KANBAN_HOME", "").strip() or _CRYOZEN_ROOT).expanduser()
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    override = os.environ.get("CRYOZEN_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    Credential sub-dirs (``~/.ssh``, ``~/.cryozen-agent/.env``) stay blocked (more-specific entries)."""
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Cryozen cache dirs (``/root/.cryozen-agent/...`` in
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted cache dirs (``/root/.cryozen-agent/...``), persistent ``/workspace`` and ``/root``."""
    if not candidate.is_absolute():
        return None
    # In-process gateways (Desktop, `cryozen serve`) may not have bridged terminal.* config into
    ...
    # prefixes). /root/.cryozen-agent/* that missed a cache mount is the container's credential surface —
    # translating it via the home mount would dodge the host denylist.
    if "/root" not in mounted and not candidate.as_posix().startswith("/root/.cryozen-agent"):
        mounts.extend(
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    (``CRYOZEN_MEDIA_DELIVERY_STRICT=1``, public bots where prompt injection must not exfiltrate
    host secrets): MUST be under a Cryozen cache, an operator root (``CRYOZEN_MEDIA_ALLOW_DIRS``),
    ...
    for root in _media_delivery_allowed_roots():
        ...
        if resolved_root is not None and _path_is_within(resolved, resolved_root):
            return str(resolved)
    # Non-strict (default): anything not denylisted (/etc, /proc, ~/.ssh, Cryozen-root secrets).
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?cryozen\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+cryozen[.!?\s]*$", re.IGNORECASE))
...
class BasePlatformAdapter(ABC):
    ...
    # adapters (API server). Propagated to ``CRYOZEN_SESSION_ASYNC_DELIVERY`` so tools never promise
    # a delivery they can't keep.
    supports_async_delivery: bool = True
    # ``send()`` chunks natively via ``truncate_message()`` -> the router skips its truncation.
    splits_long_messages: bool = False
    # Prefix users can always TYPE for Cryozen commands ("!" where the client eats a leading "/").
    ...
    # ``cryozen gateway migrate`` can tell "URL changes" from "this profile would be skipped" as new
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        """Acquire a scoped lock for this adapter; True on success. A live cross-CRYOZEN_HOME
        ...
        remedy = (f" Stop that gateway first (cryozen --profile {owner_profile} gateway stop)."
    ...
    async def _handle_message_while_active(self, event: MessageEvent, session_key: str) -> None:
        ...
        from cryozen_cli.commands import (is_interrupt_then_dispatch, should_bypass_active_session)
    ...
    async def _synthesize_auto_tts(self, text_content: str) -> Tuple[List[str], Optional[str]]:
        ...
        (logged, never raised). Path built platform-aware HERE: CRYOZEN_SESSION_PLATFORM is cleared
        post-handler."""
        paths: List[str] = []
        requested_path = None
        try:
    ...
    def _wants_auto_tts(self, event: MessageEvent, session_key: str, interrupt_event: asyncio.Event,
        ...
        generation = getattr(interrupt_event, "_cryozen_run_generation", None)
        return bool(
    ...
    async def _fire_post_delivery_callback(self, session_key: str, interrupt_event: asyncio.Event) -> None:
        ...
        _post_cb = self.pop_post_delivery_callback(
            session_key, generation=getattr(interrupt_event, "_cryozen_run_generation", None))
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            self._streaming_tts_completed_turns.discard(self._streaming_tts_turn_key(
                session_key, getattr(interrupt_event, "_cryozen_run_generation", None),
                event=event) or "")
    ...
    def build_source(
        ...
        profile is stamped on ``source.profile`` for per-profile CRYOZEN_HOME isolation."""
