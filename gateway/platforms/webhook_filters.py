...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.cryozen-agent to the active profile home."""
    ...
    if raw == "~/.cryozen-agent" or raw.startswith("~/.cryozen-agent/"):
        return cryozen_home / raw[len("~/.cryozen-agent/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else cryozen_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under CRYOZEN_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from cryozen_constants import get_cryozen_home
    scripts_root = (get_cryozen_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.cryozen-agent" or raw_text.startswith("~/.cryozen-agent/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        Non-zero exit, empty/``[SILENT]`` stdout, or a ``[SILENT]``/``__cryozen_ignore__`` flag drops the
        ...
        if not isinstance(transformed, dict):
            ...
            return False, None
        silenced = transformed.get("[SILENT]") is True or transformed.get("__cryozen_ignore__") is True
