...
def _cryozen_extras(completed, is_partial, is_failed, err_msg, finish_reason: str) -> Dict[str, Any]:
    return {
...
class _ResponsesStream:
    ...
    async def emit_status(self, payload: Dict[str, Any]) -> None:
        ...
        ``cryozen.status`` custom event; not a Responses output item."""
        await self.response.write(self._api._sse_frame(payload, event="cryozen.status"))
...
class OpenAICompatRoutesMixin:
    ...
    def _spawn_stream_agent(self, stream_q, **run_kwargs) -> tuple:
        ...
        def _on_status(kind, message=None):
            # Lifecycle/warning status (provider wait, auto-recovery countdown, fallback switch) as a
            # ``cryozen.status`` event, so a client sees why the stream is silent instead of a dead socket.
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # X-Cryozen-Session-Key scopes long-term memory per channel; independent of
        # X-Cryozen-Session-Id (the key persists across transcripts, the id rotates on /new).
        gateway_session_key, key_err = self._parse_session_key_header(request)
        if key_err is not None:
            return key_err
        # X-Cryozen-Session-Id continues an existing session (history from state.db, not the body);
        # requires a configured API key or any client could read history by guessing ids.
        provided_session_id = request.headers.get("X-Cryozen-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Cryozen-Session-Id rejected: "
        ...
        run_kwargs = dict(
            ...
            # #98619: only an explicitly provided X-Cryozen-Session-Id is wake-capable (the
        ...
        if provided_session_id and body.get("cryozen_notification_category") == "diagnostic":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """``cryozen.tool.progress`` status=running; ``_``-prefixed tools stay off the wire."""
        ...
        response_headers = {"X-Cryozen-Session-Id": (provided_session_id or result.get("session_id", session_id))}
        if gateway_session_key:
            response_headers["X-Cryozen-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["cryozen"] = {
                "completed": completed, "partial": is_partial, "failed": is_failed}
            response_headers["X-Cryozen-Completed"] = "false"
            response_headers["X-Cryozen-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)
        # Soft partial (some text, run incomplete): 200 + finish_reason="length"/Cryozen extras.
        response_data = {
    ...
    async def _prepare_sse_response(
        ...
        if session_id:
            sse_headers["X-Cryozen-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Cryozen-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async for delta in _iter_stream_items(stream_q, agent_task, response):
                ...
                if isinstance(delta, tuple) and len(delta) == 2 and delta[0] == "__tool_progress__":
                    ...
                    await response.write(_sse_frame(delta[1], event="cryozen.tool.progress"))
                    ...
                    await response.write(_sse_frame(delta[1], event="cryozen.status"))
                else:
            ...
            if finish_reason != "stop":
                ...
                finish_chunk["cryozen"] = _cryozen_extras(
                    completed, is_partial, is_failed, "" if presentation_muted else err_msg, finish_reason)
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # Session precedence: previous_response_id chain > declared X-Cryozen-Session-Key > fresh
        ...
        response_headers = {"X-Cryozen-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Cryozen-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
