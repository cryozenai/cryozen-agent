...
class PlatformConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        # write them where the docs and ``cryozen config set platforms.webhook.port`` put them:
...
class GatewayConfig:
    ...
    sessions_dir: Path = field(default_factory=lambda: get_cryozen_home() / "sessions")
    ...
    # ``None``: a request the gateway settles at boot, not a verdict. ``cryozen_cli.gateway_multiplex_mode
    ...
    # route); CRYOZEN_ROOM_LINK_URL overrides.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            sessions_dir=Path(data["sessions_dir"]) if "sessions_dir" in data else get_cryozen_home() / "sessions",
...
def load_gateway_config() -> GatewayConfig:
    """Load gateway configuration. Priority: env > ~/.cryozen-agent/config.yaml > legacy gateway.json > defaults."""
    from gateway import config_loader

    _home = get_cryozen_home()
