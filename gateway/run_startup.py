...
class GatewayStartupMixin:
    ...
    def _log_agent_budget() -> None:
        ...
        ``CRYOZEN_MAX_ITERATIONS`` before this runs, so the env slot already carries the config value;
        ...
        from cryozen_cli.config import TURN_LIMIT_UNLIMITED, resolve_turn_limit
        limit = resolve_turn_limit(os.getenv("CRYOZEN_MAX_ITERATIONS"))
        logger.info("Agent budget: max_iterations=%s (agent.max_turns from config.yaml, else the CRYOZEN_MAX_ITERATIONS bridge)",
    ...
    async def _drain_startup_restore_queue(self) -> int:
        ...
        while queue:
            ...
            try:
                ...
                with suppress(Exception):
                    setattr(event, "_cryozen_startup_restore_replay", True)
                await adapter.handle_message(event)
            except Exception:
    ...
    async def _run_boot_probe_in_launch_scope(self, fn):
        ...
        multiplexing (the ``_discover_gateway_mcp_tools`` shape). Boot probes (``check_fn``s) read
        profile-scoped secrets; an unscoped read under multiplex fails
        ...
        if getattr(self.config, "multiplex_profiles", False):
            ...
            try:
                async with _async_profile_runtime_scope(get_cryozen_home()):
    ...
    async def _recover_unclean_sessions(self) -> tuple[int, int]:
        ...
        with _log_suppressed(logging.WARNING, "Exact active-turn recovery on startup failed: %s"):
            agent_timeout = max(1.0, _float_env("CRYOZEN_AGENT_TIMEOUT", 1800))
    ...
    def _start_loop_heartbeat_task(self) -> None:
        ...
        with _log_suppressed(logging.DEBUG, "Failed to start gateway loop heartbeat", exc_info=True):
            ...
            task._cryozen_supervised_watcher = True  # type: ignore[attr-defined]
    ...
    def _open_faulthandler_log(self):
        ...
        from gateway.run import get_cryozen_home
        log_dir = getattr(self.config, "log_dir", None) or os.path.join(str(get_cryozen_home()), "logs")
    ...
    async def _start_log_startup_environment(self) -> None:
        ...
        with suppress(Exception):
            ...
            _redact_raw = os.getenv("CRYOZEN_REDACT_SECRETS", "true")
            if _redact_raw.lower() in {"1", "true", "yes", "on"}:
                ...
                logger.warning(
                    "Secret redaction: DISABLED (CRYOZEN_REDACT_SECRETS=%s). API keys and tokens may appear "
        ...
        with _log_suppressed(logging.DEBUG, "security advisory check failed at gateway startup", exc_info=True):
            from cryozen_cli.security_advisories import detect_compromised, gateway_log_message
            _adv_msg = gateway_log_message(detect_compromised())
            if _adv_msg:
                logger.warning("%s", _adv_msg)
                logger.warning("Run `cryozen doctor` on the gateway host for full remediation steps.")
        ...
        with _log_suppressed(logging.DEBUG, "check_systemd_timing_alignment failed: %s"):
            ...
            if _alignment is not None and _alignment.get("mismatch"):
                logger.warning(
                    ...
                    "mid-drain. Run `cryozen gateway install --force` to regenerate the unit, or shorten "
    ...
    def _register_launch_profile_config_hooks() -> None:
        ...
        with _profile_runtime_scope(get_process_cryozen_home()):
            GatewayStartupMixin._register_config_hooks(
                "shell-hook/webhook registration failed at gateway startup", level=logging.WARNING)
    ...
    def _register_config_hooks(fail_fmt: str, *fail_args, level: int = logging.DEBUG) -> None:
        ...
        Gateway has no TTY, so consent must come from --accept-hooks, CRYOZEN_ACCEPT_HOOKS, or
    ...
    def _recover_secondary_process_checkpoints(self, process_registry) -> int:
        ...
        from cryozen_constants import get_cryozen_home
        launch_home = get_cryozen_home().resolve()
        recovered = 0
    ...
    async def _start_recover_previous_run(self) -> None:
        ...
        _clean_marker = _cryozen_home / ".clean_shutdown"
        ...
        if connected_count != 0:
            if startup_nonretryable_errors:
                ...
                logger.error(
                    ...
                    "then `cryozen gateway restart`): %s. The gateway is DEGRADED — it serves the "
    ...
    async def _start_impl(self) -> bool:
        logger.info("Starting Cryozen Gateway...")
    ...
    async def _handoff_resolve_destination(
        ...
        try:
            new_thread_id = await transport.adapter.create_handoff_thread(
                home_chat_id, f"Cryozen — {cli_title}",
            )
        except Exception as exc:
