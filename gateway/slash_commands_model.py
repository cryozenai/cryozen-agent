...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: cryozen gateway restart"
        ),
    )
...
class GatewayModelCommandsMixin:
    ...
    async def _perform_model_switch(
        ...
        try:
            from cryozen_cli.context_switch_guard import enrich_model_switch_warnings_for_gateway
    ...
    async def _record_model_switch(
        ...
        """
        from cryozen_cli.model_switch import format_model_for_display
    ...
    async def _model_switch_confirmation(
        ...
        from gateway.run import _load_gateway_config
        from cryozen_cli.model_switch import format_model_for_display, resolve_display_context_length_async
    ...
    async def _handle_model_command_locked(self, event: MessageEvent) -> Optional[str]:
        ...
        ctx = _ModelSwitchContext(
            ...
            # the caller's). user_id is critical for the fallback lookup path (cryozen_state.py:1994-2009)
            ...
            source=source,
            config_path=(profile_home or _cryozen_home) / "config.yaml",
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        """Handle /personality — list or set a personality (cryozen_cli.personality owns the state)."""
        from gateway.run import _load_gateway_config
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> Optional[str]:
        ...
        # Labels tell the truth about the route: a Cryozen-internal step (``ultra``) that the wire
