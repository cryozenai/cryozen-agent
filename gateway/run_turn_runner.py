...
class TurnRunner:
    ...
    def _progress_subagent_notice(self, preview, kwargs: dict) -> None:
        """Only terminal failure statuses render (same notice rail as agent notices)."""
        ctx = self._ctx
    ...
    def _progress_onboarding_hint(self, kwargs: dict) -> None:
        ...
        from gateway.run import _cryozen_home, _load_gateway_config
        ctx = self._ctx
        try:
            if (kwargs.get("duration") or 0) >= ctx._LONG_TOOL_THRESHOLD_S and ctx.progress_mode == "all":
                ...
                if gate_on and not is_seen(cfg, TOOL_PROGRESS_FLAG):
                    ...
                    mark_seen(_cryozen_home / "config.yaml", TOOL_PROGRESS_FLAG)
        except Exception as err:
    ...
    async def _task_card_publish(self, st) -> None:
        ...
        if not st.native_failed:
            result = await st.adapter.send_native_task_card_progress(
                chat_id=ctx.source.chat_id, tasks=st.visible_tasks(), title="Cryozen is working",
    ...
    def _skip_context_files(self, platform_key) -> bool:
        ...
        # ``cryozen gateway setup`` writes ``gateway.platforms`` as a LIST of enabled platform names,
    ...
    def _notice_callback_sync(self, notice) -> None:
        """Out-of-band notices fire from the agent's sync worker thread; hop onto the gateway loop."""
    ...
    def run_sync(self):
        ...
        — never os.environ["CRYOZEN_SESSION_KEY"], which would misroute approvals across sessions.
        """
        ...
        # tool worker threads. We deliberately do NOT write os.environ["CRYOZEN_SESSION_KEY"] here:
        ...
        # *subprocess* exports CRYOZEN_SESSION_KEY (from its own --session-key argv, a separate process) — so
        ...
        try:
            ...
            if is_rate_limited_auth_error(exc.__cause__):
                # Quota cap with valid credentials: re-signing in cannot help; name the reset window (#89401).
            ...
            return {
                "final_response": (
                    ...
                    "Use /model to pick a different model. If it keeps failing, run `cryozen doctor` "
                    "on the host."),
                "messages": [], "api_calls": 0, "tools": [],
            }
