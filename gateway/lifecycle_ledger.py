...
``<CRYOZEN_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_cryozen_home() -> Path:
    """CRYOZEN_HOME for process-level identity files (ignore task overrides)."""
    from cryozen_constants import get_cryozen_home, get_process_cryozen_home

    # get_process_cryozen_home expands ``~``/``$VAR`` (python -m gateway.run skips the CLI normalizer).
    return get_process_cryozen_home() if os.environ.get("CRYOZEN_HOME", "").strip() else get_cryozen_home()


def _home_path(home: Optional[Path], *relative: str) -> Path:
    return (_process_cryozen_home() if home is None else home).joinpath(*relative)


def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<CRYOZEN_HOME>/state/gateway.lifecycle.json``."""
...
def _report_unclean_exit(evidence: Dict[str, Any], home: Optional[Path]) -> None:
    ...
    if verdict not in ("ok", "absent"):
        logger.error(
            ...
            "missing until it is repaired. Run `cryozen doctor`.",
            verdict,
        )
