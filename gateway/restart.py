...
# Set by ``cryozen gateway run --external-supervisor``. Unlike systemd's INVOCATION_ID
# and launchd's XPC_SERVICE_NAME, this survives wrappers that replace the child
# environment (e.g. ``sudo env -i``).
EXTERNAL_GATEWAY_SUPERVISOR_ENV = "CRYOZEN_GATEWAY_EXTERNAL_SUPERVISOR"
...
def launchd_service_label(environ: Mapping[str, str] | None = None) -> str | None:
    """Return this process's ``ai.cryozen.*`` launchd job label, or ``None``.
    ...
    if not label.startswith("ai.cryozen"):
        return None
    return label
...
def read_launchd_exit_timeout_s(
    ...
    *,
    ...
    is not launchd-owned (non-darwin, or no ``ai.cryozen`` ``XPC_SERVICE_NAME``), ``launchctl`` is missing
...
def is_gateway_supervisor_process(environ: Mapping[str, str] | None = None) -> bool:
    ...
    ``--external-supervisor`` opt-in. The generalized ``CRYOZEN_SUPERVISED_CHILD`` launcher marker is
    ...
    return bool(env.get("INVOCATION_ID") or env.get("CRYOZEN_S6_SUPERVISED_CHILD") or (xpc_service and xpc_service != "0")
...
def is_supervised_gateway_launch(environ: Mapping[str, str] | None = None) -> bool:
    ...
    Superset of :func:`is_gateway_supervisor_process` that also honours ``CRYOZEN_SUPERVISED_CHILD``,
    the marker every generated launcher exports (systemd unit, launchd plist, s6 run script, Windows
    Scheduled Task — see ``cryozen_cli.main._apply_profile_override``). This is the identity the
    ...
    """
    env = os.environ if environ is None else environ
    if env.get("CRYOZEN_SUPERVISED_CHILD"):
        return True
