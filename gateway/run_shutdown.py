...
# exit (bounded), then spawns ``cryozen gateway restart``.
...
class GatewayShutdownMixin:
    ...
    def _scale_to_zero_has_live_background_work(self) -> bool:
        ...
        PERMANENT supervised watchers (_cryozen_supervised_watcher, incl. the scale-to-zero watcher
        itself) are excluded, else this would be True forever and the gateway could never go dormant.
        """
        if any(
            not t.done() and not getattr(t, "_cryozen_supervised_watcher", False)
    ...
    async def _scale_to_zero_watcher(self, interval: float = 30.0) -> None:
        ...
        sees only INBOUND connections and would freeze mid-job. Without a flaps socket the orchestrator brokers
    ...
    async def _scale_to_zero_self_suspend(self) -> None:
        """Suspend this machine, in-guest where possible and via the orchestrator otherwise (fail-awake).
        ...
        try:
            if self_suspend_available():
                ...
                # No in-guest API (Azure ACA): the orchestrator holds the credential for the stop verb and
                # brokers it for us.
    ...
    def _pause_failed_platform(self, platform, *, reason: str = "") -> None:
        ...
        logger.warning(
            ...
            "resume %s` to retry, or `cryozen gateway restart` to restart the gateway.",
    ...
    async def _notify_interrupted_cron_jobs(self, job_ids) -> int:
        ...
        for job_id in job_ids:
            ...
            msg = (
                f"⚠️ Scheduled job '{job_name}' was cut short because Cryozen is {action}; "
                "no result this run. It will run again on schedule, or run it now with "
                f"`cryozen cron run {job_name}` once Cryozen is back."
            )
    ...
    async def _notify_active_sessions_of_shutdown(self) -> None:
        ...
        msg = (
            "⚠️ Cryozen is shutting down — your current task will be interrupted. "
            "When it is back online, send any message and I'll try to pick up where we left off."
        )
        if self._restart_requested:
            msg = (
                "⚠️ Cryozen is restarting — your current task will be interrupted. "
    ...
    async def _finalize_session_off_loop(
        ...
        """Run cryozen_cli.lifecycle.finalize_session off-loop, bounded; on timeout the worker is left alone.
        ...
        def _call() -> None:
            from cryozen_cli.lifecycle import finalize_session
    ...
    def _stuck_loop_counts_path(self) -> Path:
        ...
        return _cryozen_home / self._STUCK_LOOP_FILE
    ...
    def _restart_watcher_env() -> dict:
        """Watcher env minus ``_CRYOZEN_GATEWAY`` (else the CLI's self-restart guard refuses; gateway stays down)."""
        ...
        watcher_env.pop("_CRYOZEN_GATEWAY", None)
        return watcher_env

    @staticmethod
    def _spawn_windows_restart_watcher(cryozen_cmd: list, current_pid: int, restart_after_s: float) -> None:
        ...
        # it owns one hidden console, inherited by the `cryozen gateway restart` child, so nothing flashes.
        # See #54220, #56747.
        ...
        watcher_argv = [
            ...
            str(current_pid), str(restart_after_s), *cryozen_cmd, "gateway", "restart",
        ]
    ...
    async def _launch_detached_restart_command(self) -> None:
        ...
        if not cryozen_cmd:
            logger.error("Could not locate cryozen binary for detached /restart")
            return
        ...
        if sys.platform == "win32":
            GatewayShutdownMixin._spawn_windows_restart_watcher(cryozen_cmd, current_pid, restart_after_s)
            return
        cmd = " ".join(shlex.quote(part) for part in cryozen_cmd)
        shell_cmd = (
    ...
    def _wedged_chat_agent_count(self) -> int:
        ...
        timeout = _float_env("CRYOZEN_AGENT_TIMEOUT", 1800)
        if timeout <= 0:
            return 0
    ...
    def _describe_active_work(self) -> list:
        ...
        (``cryozen update``, ``cryozen gateway status``) can name it instead of printing a bare count.
    ...
    def _start_systemd_watchdog(self) -> bool:
        ...
        watchdog.ready("Cryozen Gateway running")
        return True
    ...
    def _stop_quiesce_and_close_session_dbs(self, timeout: float, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        def _close_shared() -> None:
            ...
            # (#90837).
            from cryozen_state_registry import close_all
            closed = close_all()
            if closed:
    ...
    async def _stop_persist_exit_state(self, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        from gateway.run import _cryozen_home, _planned_restart_notification_path, _shutdown_gateway_health_export
        from utils import atomic_json_write
        ...
        if not ctx.timed_out:
            with suppress(Exception):
                (_cryozen_home / ".clean_shutdown").touch()
        else:
            logger.info(
