"""Read-only gateway introspection commands: /status, /context, /usage, /agents, /insights.
...
from cryozen_cli.status_report import build_status_fields
...
HISTORY_UNREADABLE = ("⚠️ I can't read this conversation's history right now (your earlier messages "
                      "exist but cannot be loaded). Run `cryozen doctor --fix` on the host, or use /new "
                      "to start fresh.")
...
class GatewayStatusCommandsMixin:
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        def _with_account_blocks(lines: list[str]) -> str:
            # The account block is preceded by a blank divider only when something precedes it.
            if account_lines:
                if lines:
                    lines.append("")
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # Not a bare hop: ``SessionDB()`` resolves ``get_cryozen_home()`` at call time, a
