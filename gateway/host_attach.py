...
def _record_home(record) -> Path:
    ...
    return Path(record.home) if getattr(record, "home", "") else Path(get_default_cryozen_root())
...
def _identity_matches(identity, record, home: Path) -> bool:
    ...
    if not isinstance(identity, dict) or identity.get("pid") != record.pid:
        return False
    reported = identity.get("cryozen_home")
    if not reported:
    ...
    try:
        ...
        return bool(_same_cryozen_home(Path(str(reported)), home))
    except Exception:
...
def attach_message(gateway: HostGateway, profile: str) -> str:
    return (
        ...
        f"`cryozen -p {gateway.profile_label} gateway restart`.")
...
def _unknown_served_message(gateway: HostGateway, profile: str) -> str:
    return (
        ...
        f"   Start anyway:        cryozen gateway run --force")
...
def _refuse_message(gateway: HostGateway, profile: str) -> str:
    return (
        ...
        f"   Fold this profile into it:   cryozen gateway migrate --multiplex\n"
...
def decide(our_home: Path, *, replace: bool = False) -> HostAttachDecision:
    ...
    if attached is not None and attached.standalone:
        ...
        logger.warning(
            ...
            "Fold every profile onto one gateway with: cryozen gateway migrate --multiplex",
