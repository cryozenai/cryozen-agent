...
# Python-driven tool session (e.g. Cryozen terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:cryozenai/cryozen-agent.git"
REPO_URL_HTTPS="https://github.com/cryozenai/cryozen-agent.git"
CRYOZEN_HOME="${CRYOZEN_HOME:-$HOME/.cryozen-agent}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${CRYOZEN_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$CRYOZEN_INSTALL_DIR"
...
#   code at /usr/local/lib/cryozen-agent, command at /usr/local/bin/cryozen,
#   data still at /root/.cryozen-agent (CRYOZEN_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$CRYOZEN_HOME/.no-bundled-skills so future"
            echo "                   'cryozen update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Cryozen.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.cryozen-agent/cryozen-agent"
            echo "                   default (root, Linux): /usr/local/lib/cryozen-agent"
            echo "  --cryozen-home PATH  Data directory (default: ~/.cryozen-agent, or \$CRYOZEN_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Cryozen installs the code under"
            echo "  /usr/local/lib/cryozen-agent and links the command into"
            echo "  /usr/local/bin/cryozen (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CRYOZEN_HOME"
            echo "  (default /root/.cryozen-agent).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CRYOZEN_HOME/cryozen-agent are preserved in-place."
...
print_banner() {
    ...
    echo "│             ✻ Cryozen Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Cryozen.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `cryozen update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Cryozen-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Cryozen Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install cryozen command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `cryozen` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CRYOZEN_HOME/cryozen-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CRYOZEN_HOME/cryozen-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/cryozen-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CRYOZEN_HOME/cryozen-agent — then preserve it)
#
# Always no-op when the user set --dir or $CRYOZEN_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CRYOZEN_HOME.
    if is_termux; then
        INSTALL_DIR="$CRYOZEN_HOME/cryozen-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CRYOZEN_HOME/cryozen-agent/.git" ]; then
            INSTALL_DIR="$CRYOZEN_HOME/cryozen-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/cryozen-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/cryozen-agent"
        ...
        # /usr/local/bin/cryozen wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/cryozen"
        log_info "  Data:    $CRYOZEN_HOME (unchanged)"
...
# Point a Cryozen-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $CRYOZEN_HOME/node/bin — which is NOT on
...
# Cryozen's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Cryozen-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$CRYOZEN_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$CRYOZEN_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$CRYOZEN_HOME/node/etc/npmrc"
}

get_cryozen_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/cryozen" ]; then
        echo "$link_dir/cryozen"
    else
        echo "cryozen"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://cryozenai.github.io/cryozen-agent/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Cryozen owns its own uv at $CRYOZEN_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (cryozen_cli/managed_uv.py) looks in the same
    # place, so install.sh and `cryozen update` stay in sync.
    local _managed_uv="$CRYOZEN_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $CRYOZEN_HOME/bin ..."
    mkdir -p "$CRYOZEN_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "${TMPDIR:-$CRYOZEN_HOME}/cryozen-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "${TMPDIR:-$CRYOZEN_HOME}/cryozen-uv-installer.$$.sh")"
    ...
    # directly into $CRYOZEN_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$CRYOZEN_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Cryozen currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Cryozen requires Python >=3.11,<3.14"
...
# replaced with the Cryozen-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Cryozen-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Cryozen-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Cryozen-managed Node from a previous run over a too-old system one.
    if [ -x "$CRYOZEN_HOME/node/bin/node" ] && [ -x "$CRYOZEN_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$CRYOZEN_HOME/node/bin/node" --version)"; then
        export PATH="$CRYOZEN_HOME/node/bin:$PATH"
        log_success "Node.js $("$CRYOZEN_HOME/node/bin/node" --version) found (Cryozen-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Cryozen-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Cryozen requires Node 22.22+, 24.11+, or 26+) — installing Cryozen-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.cryozen-agent/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.cryozen-agent/node/..."
    ...
    # Place into ~/.cryozen-agent/node/ and symlink binaries into the same bin dir
    # the cryozen command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$CRYOZEN_HOME/node"
    mkdir -p "$CRYOZEN_HOME"
    mv "$extracted_dir" "$CRYOZEN_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$CRYOZEN_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$CRYOZEN_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$CRYOZEN_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$CRYOZEN_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.cryozen-agent/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Cryozen install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Cryozen Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `cryozen update` path
                # (#4735).
                ...
                local stash_name
                stash_name="cryozen-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``cryozen update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Cryozen behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up cryozen command..."

    if [ "$USE_VENV" = true ]; then
        CRYOZEN_BIN="$INSTALL_DIR/venv/bin/python"
        CRYOZEN_ENTRYPOINT="$INSTALL_DIR/cryozen"
    else
        CRYOZEN_BIN="$(which cryozen 2>/dev/null || echo "")"
        if [ -z "$CRYOZEN_BIN" ]; then
            log_warn "cryozen not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$CRYOZEN_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$CRYOZEN_ENTRYPOINT" ]; }; then
        log_warn "Cryozen launcher prerequisites not found"
    ...
    # Create a user-facing shim for the cryozen command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $CRYOZEN_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$CRYOZEN_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/cryozen"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/cryozen" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CRYOZEN_BIN" "$CRYOZEN_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/cryozen" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/cryozen"
    log_success "Installed cryozen launcher → $command_link_display_dir/cryozen"

    # Also expose `cryozen-agent`. The `cryozen-agent` console script declared in
    ...
    rm -f "$command_link_dir/cryozen-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/cryozen-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CRYOZEN_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/cryozen-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CRYOZEN_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/cryozen-agent"
    log_success "Installed cryozen-agent launcher → $command_link_display_dir/cryozen-agent"

    # Also expose `cryozen-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `cryozen-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Cryozen as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/cryozen-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/cryozen-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CRYOZEN_BIN" "$CRYOZEN_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/cryozen-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CRYOZEN_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/cryozen-acp"
    log_success "Installed cryozen-acp launcher → $command_link_display_dir/cryozen-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "cryozen command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v cryozen' \
                >/dev/null 2>&1; then
        ...
        log_info "cryozen not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Cryozen Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Cryozen Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Cryozen Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so cryozen works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.cryozen-agent directory structure (config at top level, code in subdir)
    mkdir -p "$CRYOZEN_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.cryozen-agent/.env (top level, easy to find)
    if [ ! -f "$CRYOZEN_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CRYOZEN_HOME/.env"
            log_success "Created ~/.cryozen-agent/.env from template"
        else
            touch "$CRYOZEN_HOME/.env"
            log_success "Created ~/.cryozen-agent/.env"
        fi
    else
        log_info "~/.cryozen-agent/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$CRYOZEN_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.cryozen-agent/config.yaml (top level, easy to find)
    if [ ! -f "$CRYOZEN_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CRYOZEN_HOME/config.yaml"
            log_success "Created ~/.cryozen-agent/config.yaml from template"
        fi
    else
        log_info "~/.cryozen-agent/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in cryozen_cli/default_soul.py — the
    ...
    if [ ! -f "$CRYOZEN_HOME/SOUL.md" ]; then
        cat > "$CRYOZEN_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.cryozen-agent/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.cryozen-agent/"

    # Seed bundled skills into ~/.cryozen-agent/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `cryozen update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'cryozen update'." \
            > "$CRYOZEN_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $CRYOZEN_HOME/.no-bundled-skills"
        log_info "  Future 'cryozen update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.cryozen-agent/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.cryozen-agent/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CRYOZEN_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$CRYOZEN_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.cryozen-agent/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$CRYOZEN_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Cryozen Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Cryozen will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Cryozen Agent browser tools — explicit browser override."
...
# install — the same closure `cryozen update` installs
# (cryozen_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `cryozen update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `cryozen tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$CRYOZEN_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Cryozen' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$CRYOZEN_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'cryozen tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `cryozen tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `cryozen computer-use install` on their own).
    ...
    # Same upstream installer `cryozen computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Cryozen install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Cryozen'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'cryozen tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: cryozen computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'cryozen setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m cryozen_cli.main setup < /dev/tty
    else
        python -m cryozen_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CRYOZEN_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Cryozen to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CRYOZEN_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'cryozen whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                CRYOZEN_CMD="$(get_cryozen_command_path)"
                $CRYOZEN_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'cryozen whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'cryozen gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $CRYOZEN_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $CRYOZEN_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: cryozen gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: cryozen gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $CRYOZEN_CMD gateway > "$CRYOZEN_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.cryozen-agent/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: cryozen gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: cryozen gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.cryozen-bootstrap-complete, which tells the Cryozen
    ...
    local marker_path="$INSTALL_DIR/.cryozen-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $CRYOZEN_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $CRYOZEN_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $CRYOZEN_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}cryozen${NC}              Start chatting"
    echo -e "   ${GREEN}cryozen setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}cryozen config${NC}       View/edit configuration"
    echo -e "   ${GREEN}cryozen config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}cryozen gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}cryozen update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'cryozen' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'cryozen' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'cryozen' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CRYOZEN_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CRYOZEN_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CRYOZEN_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$CRYOZEN_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Cryozen` rename
...
# _purge_electron_build_cache() used by `cryozen desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Cryozen-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `cryozen desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Cryozen" ]; then
            app="$desktop_dir/release/linux-unpacked/Cryozen"
        elif [ -x "$desktop_dir/release/linux-unpacked/cryozen" ]; then
            app="$desktop_dir/release/linux-unpacked/cryozen"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Cryozen.app" \
            "$desktop_dir/release/mac/Cryozen.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `cryozen desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading CRYOZEN_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if CRYOZEN_HOME="$CRYOZEN_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from cryozen_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `cryozen update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Cryozen-managed Node
            # provisioned during prerequisites/node-deps (at $CRYOZEN_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $CRYOZEN_HOME.
    # $CRYOZEN_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'cryozen update' on this host install.
