...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[cryozen] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[cryozen`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[cryozen`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        CryozenHome = $(if ($paths) { $paths.cryozen_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\cryozen\cryozen-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-cryozen'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-cryozen') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
