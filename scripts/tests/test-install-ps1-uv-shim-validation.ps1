...
# trust any file at $CryozenHome\bin\uv.exe -- including the Chocolatey ShimGen
...
$testRoot = Join-Path $env:TEMP ("cryozen-uv-shim-test-" + [Guid]::NewGuid().ToString('N'))
$CryozenHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
New-Item -ItemType Directory -Force -Path $testRoot | Out-Null
. $installScript -CryozenHome $CryozenHome -InstallDir $InstallDir
...
function Invoke-InstallUvScenario {
    ...
    if (Test-Path $CryozenHome) { Remove-Item -LiteralPath $CryozenHome -Recurse -Force }
    New-Item -ItemType Directory -Force -Path (Join-Path $CryozenHome 'bin') | Out-Null
