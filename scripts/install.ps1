# ============================================================================
# Cryozen Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$CryozenHome = $(if ($env:CRYOZEN_HOME) { $env:CRYOZEN_HOME } else { "$env:LOCALAPPDATA\cryozen" }),
    [string]$InstallDir = $(if ($env:CRYOZEN_HOME) { "$env:CRYOZEN_HOME\cryozen-agent" } else { "$env:LOCALAPPDATA\cryozen\cryozen-agent" }),
    ...
    # builds apps/desktop into a launchable Cryozen.exe.
    #
    # Why opt-in:
    #   * Cryozen-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Cryozen.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Cryozen.exe
    ...
    #     `cryozen desktop` already builds on demand.
...
# CRYOZEN_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[cryozen] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'CryozenInstall.LongPath').Type) {
            Add-Type -Namespace 'CryozenInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [CryozenInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -CryozenHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('CryozenHome')) {
    $CryozenHome = ConvertTo-LongPath $CryozenHome
} else {
    $CryozenHome = ConvertTo-LongPath $(
        if ($env:CRYOZEN_HOME) { $env:CRYOZEN_HOME } else { "$env:LOCALAPPDATA\cryozen" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:CRYOZEN_HOME) { "$env:CRYOZEN_HOME\cryozen-agent" } else { "$env:LOCALAPPDATA\cryozen\cryozen-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: CryozenHome=$CryozenHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:cryozenai/cryozen-agent.git"
$RepoUrlHttps = "https://github.com/cryozenai/cryozen-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Cryozen Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Cryozen.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $CryozenHome)) {
        New-Item -ItemType Directory -Force -Path $CryozenHome | Out-Null
    }
    $envFile = Join-Path $CryozenHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `cryozen update`'s npx
    ...
    $prefixDir = Join-Path $CryozenHome "node"
...
function Install-Uv {
    # Cryozen owns its own uv at $CryozenHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (cryozen_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `cryozen update` stay in sync.
    $managedUv = Join-Path $CryozenHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $CryozenHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $CryozenHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $CryozenHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $CryozenHome "bin"
        ...
        # (cryozen_cli/managed_uv.py looks only at $CryozenHome\bin\uv.exe).
...
# Put the Cryozen-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone cryozen-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Cryozen-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Cryozen-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("cryozen-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and CRYOZEN_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Cryozen belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # cryozen_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".cryozen-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Cryozen-managed interpreter
    ...
    #
    # Under Cryozen-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Cryozen-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Cryozen setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Cryozen uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\cryozen\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Cryozen needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Cryozen copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\cryozen\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``CRYOZEN_GIT_BASH_PATH`` (User scope) so Cryozen can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Cryozen-managed PortableGit install instead..."
    }

    # Download PortableGit into $CryozenHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $CryozenHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Cryozen features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Cryozen needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``CRYOZEN_GIT_BASH_PATH`` (User env scope) so Cryozen can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $CryozenHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $CryozenHome\git\usr\bin\bash.exe
    $candidates += "$CryozenHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$CryozenHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("CRYOZEN_GIT_BASH_PATH", $candidate, "User")
            $env:CRYOZEN_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set CRYOZEN_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Cryozen may not find Git Bash."
    Write-Info "If needed, set CRYOZEN_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Cryozen-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Cryozen requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Cryozen requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Cryozen-managed Node.js installation instead..."

    # Prefer a Cryozen-managed Node from a previous run over a too-old system one.
    $managedNode = "$CryozenHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$CryozenHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$CryozenHome\node"
        Write-Success "Node.js $version found (Cryozen-managed)"
        ...
        Update-ManagedNpm "$CryozenHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Cryozen-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $CryozenHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $CryozenHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\cryozen-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$CryozenHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$CryozenHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$CryozenHome\node") {
                    try {
                        Rename-Item "$CryozenHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Cryozen-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$CryozenHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $CryozenHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\cryozen-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `cryozen
                    # update` path (#4735).
                    ...
                    $stashName = "cryozen-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. cryozen-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``cryozen update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Cryozen behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running cryozen processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/cryozenai/cryozen-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/cryozenai/cryozen-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/cryozenai/cryozen-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\cryozen-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\cryozen-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `cryozen update` (see the notes at the shared clone-path
    ...
    # next `cryozen update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Cryozen-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Cryozen-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running cryozen process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running cryozen processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Cryozen_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (cryozen.exe) plus its child tree.
            & taskkill /F /T /IM cryozen.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM cryozen.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m cryozen_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not cryozen.exe.
            ...
            # Cryozen children re-exec through .cryozen-runtime, so killing only
            ...
            # user (see cryozen_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Cryozen " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `cryozen update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Cryozen (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `cryozen.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'cryozen-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install cryozen-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and cryozen.exe broken with
    ...
    # here, not 30 seconds later when the user runs `cryozen`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Cryozen processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Cryozen processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Cryozen and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register cryozen.exe in dist-info/RECORD but fail to
        ...
        # `cryozen` command while cryozen-agent.exe / cryozen-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m cryozen_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `cryozen dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\cryozen_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `cryozen dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `cryozen dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: cryozen_cli/web_server.py"
        }
    }
...
function Install-CryozenCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the cryozen launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring cryozen.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "cryozen.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the cryozen command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # cryozen_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("cryozen", "cryozen-acp")) {
    ...
    $requiredExe = Join-Path $Destination "cryozen.exe"
    $requiredCmd = Join-Path $Destination "cryozen.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the cryozen command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up cryozen command..."
    
    if ($NoVenv) {
        $cryozenBin = "$InstallDir"
    } else {
        # $CryozenHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `cryozen update`'s autostash
        ...
        # installer staged under cryozen-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-CryozenCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $cryozenBin = "$CryozenHome\bin"
        Install-CryozenCommandLaunchers -Root $InstallDir -Destination $cryozenBin | Out-Null
    }
    ...
    #   cryozen-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The cryozen-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept cryozen via $cryozenBin)"
        }
    }
    
    if ($currentPath -notlike "*$cryozenBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$cryozenBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $cryozenBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set CRYOZEN_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\cryozen instead
    # of the Unix default ~/.cryozen-agent
    $currentCryozenHome = [Environment]::GetEnvironmentVariable("CRYOZEN_HOME", "User")
    if (-not $currentCryozenHome -or $currentCryozenHome -ne $CryozenHome) {
        [Environment]::SetEnvironmentVariable("CRYOZEN_HOME", $CryozenHome, "User")
        Write-Success "Set CRYOZEN_HOME=$CryozenHome"
    }
    $env:CRYOZEN_HOME = $CryozenHome
    
    # Update current session
    $env:Path = "$cryozenBin;$env:Path"
    
    Write-Success "cryozen command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.cryozen-bootstrap-complete which tells the Cryozen
    ...
    # Cryozen-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `cryozen update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".cryozen-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the CRYOZEN_HOME directory structure ($CryozenHome, default %LOCALAPPDATA%\cryozen)
    New-Item -ItemType Directory -Force -Path "$CryozenHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CryozenHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CryozenHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CryozenHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CryozenHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CryozenHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CryozenHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CryozenHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CryozenHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$CryozenHome\.env"
    ...
    # Create config.yaml
    $configPath = "$CryozenHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Cryozen's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$CryozenHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in cryozen_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $CryozenHome"
    
    # Seed bundled skills into $CryozenHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $CryozenHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $CryozenHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$CryozenHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $CryozenHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Cryozen-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'cryozen setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\cryozen-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\cryozen-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\cryozen-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `cryozen tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $CryozenHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Cryozen' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Cryozen' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'cryozen tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `cryozen
...
# paths still lazy-install via install_cua_driver() (cryozen_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `cryozen computer-use install` runs. Bounded
        ...
        # Cryozen' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'cryozen tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: cryozen computer-use install"
            }
        } else {
...
# the final `electron` -> `Cryozen` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Cryozen.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_cryozen_desktop command
    # resolves apps/desktop/release/win-unpacked/Cryozen.exe directly,
    ...
    # enforces the supported Node lines and prepends the Cryozen-managed Node to
    ...
    # The --dir mode produces an unpacked Cryozen.exe in
    ...
    # are NOT trying to work around). The Cryozen icon + product name are
    # stamped onto Cryozen.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\cryozen-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Cryozen.exe",
        "$desktopDir\release\win-arm64-unpacked\Cryozen.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Cryozen.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Cryozen icon + identity are stamped onto Cryozen.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, cryozen-agent#38216).
    ...
    #    Cryozen.exe. We deliberately do NOT point them at `cryozen desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Cryozen.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Cryozen.lnk')
        )
        ...
        # with the Cryozen icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.cryozen-agent/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'cryozen setup'."
        return
    }
    ...
    # Run cryozen setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m cryozen_cli.main setup
    } else {
        python -m cryozen_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $cryozenCmd = "$InstallDir\venv\Scripts\cryozen.exe"
    if (-not (Test-Path $cryozenCmd)) {
        $cryozenCmd = "cryozen"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$CryozenHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'cryozen whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: cryozen gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$CryozenHome\logs\gateway.log"
            Start-Process -FilePath $cryozenCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$CryozenHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: cryozen gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: cryozen gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$CryozenHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$CryozenHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CryozenHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CryozenHome\cryozen-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   cryozen              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   cryozen setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   cryozen config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   cryozen config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   cryozen gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   cryozen update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Cryozen repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.cryozen-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Cryozen-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Cryozen to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://cryozenai.github.io/cryozen-agent/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
