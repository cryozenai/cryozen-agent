...
VERSION_FILE = REPO_ROOT / "cryozen_cli" / "__init__.py"
...
# Explicit email → GitHub login overrides for authors whose commit email is
# not a GitHub noreply address. Empty by default; noreply addresses resolve
# on their own (see resolve_author).
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Cryozen
    ...
    # Keep the bootstrap installer (Cryozen-Setup.dmg CFBundleShortVersionString)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "cryozenai@users.noreply.github.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/cryozenai/cryozen-agent",
    ...
    lines.append(f"# Cryozen Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Cryozen Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Cryozen Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Cryozen Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Cryozen Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Cryozen Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Cryozen Agent v{new_version} ({calver_date})' "
