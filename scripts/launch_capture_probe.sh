...
run_py() {
  ...
  if [ "$with_var" = yes ]; then
    PYTHONPATH="$CAP_DIR${PYTHONPATH:+:$PYTHONPATH}" \
      CRYOZEN_E2E_CAPTURE_LAUNCH="$WORK/spec.json" python3 "$@"
  else
...
r = subprocess.run(["npm", "exec", "--", "electron", "."], cwd=tmp, env={"CRYOZEN_DESKTOP_CWD": tmp, "PATH": "/usr/bin"})
...
assert spec["env"]["CRYOZEN_DESKTOP_CWD"] == tmp, "env= kwarg not captured"
...
run_py yes -c '
import subprocess, tempfile
exe = "/x/apps/desktop/release/linux-unpacked/Cryozen"
...
run_py yes -c '
import subprocess
r = subprocess.run(["C:\\x\\apps\\desktop\\release\\win-unpacked\\Cryozen.exe"], env={})
