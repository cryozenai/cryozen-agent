#!/usr/bin/env bash
# Canonical test runner for cryozen-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.cryozen-agent/...)
#
# Usage:
...
# (CRYOZEN_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.cryozen-agent/cryozen-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.cryozen-agent/cryozen-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${CRYOZEN_PYTHON:-}" ] && [ -x "$CRYOZEN_PYTHON" ] \
    && "$CRYOZEN_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: CRYOZEN_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `cryozen` binary rather
  # than the devShell hook.
  PYTHON="$CRYOZEN_PYTHON"
  echo "▶ no local venv — using Nix dev venv via CRYOZEN_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and CRYOZEN_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.cryozen-agent/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.cryozen-agent"
...
#     CRYOZEN_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# CRYOZEN_RUN_SLOW_PET_TESTS / CRYOZEN_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no CRYOZEN_TEST_* glob) so the "no
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${CRYOZEN_RUN_SLOW_PET_TESTS:+CRYOZEN_RUN_SLOW_PET_TESTS="$CRYOZEN_RUN_SLOW_PET_TESTS"} \
  ${CRYOZEN_E2E_BROWSER:+CRYOZEN_E2E_BROWSER="$CRYOZEN_E2E_BROWSER"} \
