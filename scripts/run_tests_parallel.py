...
Environment:
    CRYOZEN_TEST_WORKERS  Override worker count (default: os.cpu_count())
...
def _runner_scratch_root() -> str:
    ...
    the root's shape: under the Cryozen home conftest relocates the basetemp; under a dot-dir
    ...
    if os.name == "nt" or not os.path.isdir("/var/tmp"):  # no-tmp: ok — probing the disk-backed FHS root
        root = os.path.join(tempfile.gettempdir(), "cryozen-pytest")
    else:
        root = "/var/tmp/cryozen-pytest"  # no-tmp: ok — /var/tmp is disk-backed by FHS, never tmpfs
...
#                        cryozenai/cryozen-agent:test image, via
#                        ``CRYOZEN_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or CRYOZEN_TEST_FILE_TIMEOUT.
#
...
# Set to 0 to disable (env: CRYOZEN_TEST_FILE_RETRIES).
...
def _split_pathspec(value: str) -> List[str]:
    ...
    ``CRYOZEN_TEST_PATHS``) into individual paths.
...
def _effective_file_timeout(
    ...
    but a handful of large-collection files (e.g. ``tests/test_cryozen_state.py``,
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/cryozen-agent/cryozen-agent/tests/acp_adapter/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("CRYOZEN_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $CRYOZEN_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("CRYOZEN_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
        help=(
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("CRYOZEN_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            f"Default: {_DEFAULT_FILE_TIMEOUT_SECONDS}s ({round(_DEFAULT_FILE_TIMEOUT_SECONDS/60)} min), env: CRYOZEN_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--file-retries",
        type=int,
        default=int(
            os.environ.get("CRYOZEN_TEST_FILE_RETRIES", _DEFAULT_FILE_RETRIES)
        ),
        help=(
            ...
            f"Default: {_DEFAULT_FILE_RETRIES}, env: CRYOZEN_TEST_FILE_RETRIES."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: CRYOZEN_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "--generate-slices",
    ...
    # Parse --slice (or CRYOZEN_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("CRYOZEN_TEST_SLICE")
