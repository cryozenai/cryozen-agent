#!/usr/bin/env python3
"""Build the Cryozen Model Catalog — a centralized JSON manifest of curated models.

This script reads the in-repo hardcoded curated list (``OPENROUTER_MODELS``)
and writes it to a JSON manifest that the
Cryozen CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Cryozen release.
...
# Ensure CRYOZEN_HOME is set for imports that touch it at module level.
os.environ.setdefault("CRYOZEN_HOME", os.path.join(os.path.expanduser("~"), ".cryozen-agent"))

from cryozen_cli.models import (  # noqa: E402
    OPENROUTER_MODELS,
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "cryozen-agent repo",
            "docs": "https://cryozenai.github.io/cryozen-agent/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        'The entry labeled "default": true is the model Cryozen '
