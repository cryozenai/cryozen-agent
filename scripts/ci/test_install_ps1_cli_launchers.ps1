# Behavioral test for install.ps1's cryozen launcher staging (PR #92092,
...
#
# The test lifts the real Install-CryozenCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $CryozenHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-CryozenCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-CryozenCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'cryozen-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'cryozen-agent'
    ...
    $missingThrew = $false
    try {
        Install-CryozenCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing cryozen.exe fails the launcher stage'
    ...
    $cryozenV1 = [byte[]](77, 90, 1)
    $cryozenV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'cryozen.exe'), $cryozenV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-CryozenCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $cryozenV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'cryozen.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'cryozen-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'cryozen.exe'), $cryozenV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'cryozen-acp.exe'), $acp)
    Install-CryozenCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $cryozenV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'cryozen.exe'))) `
        'installer refreshes an existing Cryozen launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'cryozen-acp.exe'))) `
    ...
    Install-CryozenCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'cryozen.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'cryozen.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'cryozen.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'cryozen.exe')) -and $cmdBody.Contains('%*')) `
