...
# tree generates llms.txt — the index every LLM (Cryozen included, via the
# cryozen-agent skill) reads to learn what Cryozen can do — and
...
_PY_RELEVANT_CONTRACT_FILES = {
    ...
    # tests/cryozen_cli/test_desktop_slash_registry.py
...
_MCP_CATALOG_FILES = {"cryozen_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Cryozen-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``cryozen serve`` backend, the Docker image)
