...
# (%CRYOZEN_HOME%\cryozen-setup.exe). That binary has no self-update path --
# copy_self_to_cryozen_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `cryozen update` refreshes
...
#     -InstallRoot <path>   repo checkout (CRYOZEN_HOME\cryozen-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Cryozen.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .cryozen-update-in-progress in place (tests)
#
...
# .cryozen-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim CRYOZEN_HOME\.cryozen-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining CRYOZEN_UPDATE_STARTED_AT from the Desktop hand-off.
# cryozen_cli/update_lock.py's ancestry rule lets our
# `cryozen update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Cryozen.exe pid.
try {
    Add-Type -Namespace CryozenHandoff -Name Win32 -MemberDefinition @'
...
$CryozenHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $CryozenHome ".cryozen-update-in-progress"
$LogDir = Join-Path $CryozenHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $CryozenHome ".cryozen-update-result.json"
$script:Ui = $null
$script:UiStage = "Cryozen will open once done."   # until the first gate; matches ui.html
...
# cannot receive events from a detached process. Derived from the web-shell
# spike, reshaped to the quiet update-surface
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale cryozen-update-ui-* leftovers from interrupted
    ...
    # update marker (.cryozen-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "cryozen-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-CryozenStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("cryozen-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [CryozenHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `cryozen debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"cryozen debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [CryozenHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen Cryozen themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Cryozen.exe before writing the replacement,
    ...
    # DO NOT spawn Cryozen.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [CryozenHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [CryozenHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [CryozenHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Cryozen process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [CryozenHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `cryozen update` deliberately runs its build steps with stdout
# inherited (cryozen_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:CRYOZEN_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:CRYOZEN_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:CRYOZEN_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:CRYOZEN_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `cryozen update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (cryozen_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:CRYOZEN_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:CRYOZEN_UPDATE_PROGRESS_LOG
}
...
if (-not ("CryozenUpdateJob" -as [type])) {
...
public static class CryozenUpdateJob {
...
function Invoke-CryozenStep([string]$Exe, [string[]]$CryozenArgs, [string]$Tag) {
    ...
    # afterwards, where `cryozen debug share` picks it up.
    #
    ...
    # .cryozen-update-result.json, clearing .cryozen-update-in-progress,
    ...
    # Cryozen" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($CryozenArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [CryozenUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `cryozen update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [CryozenUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
function Resolve-CryozenVenvDir([string]$Root) {
    # Match cryozen_constants.project_venv_dir(): installer-created venv wins,
...
# the page can be QA'd in any browser; CRYOZEN_SELFTEST_FAIL=1 exercises the
# error state, CRYOZEN_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:CRYOZEN_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:CRYOZEN_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:CRYOZEN_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-CryozenStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Cryozen install to
...
#            update log, the shape of every real `cryozen update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:CRYOZEN_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:CRYOZEN_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:CRYOZEN_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:CRYOZEN_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "cryozen-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "cryozen-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "cryozen-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "cryozen-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "cryozen-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "cryozen-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "cryozen-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "cryozen-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `cryozen update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-CryozenStep $powershell @(
    ...
    $flood = Invoke-CryozenStep $powershell @(
    ...
    $stall = Invoke-CryozenStep $powershell @(
    ...
    try {
        $logstall = Invoke-CryozenStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-CryozenStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-CryozenStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:CRYOZEN_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from CRYOZEN_HOME; pin the process directory to the
    ...
    $VenvDir = Resolve-CryozenVenvDir $InstallRoot
    ...
    if ($SelfTestWorkingDirectory) {
        ...
        $probe = Invoke-CryozenStep $probeExe @("-NoProfile", "-Command", "[Environment]::CurrentDirectory") "cwd"
    ...
    Publish-UiProgress "Waiting for Cryozen to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Cryozen window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Cryozen fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing Cryozen files"
    $shim = Join-Path $VenvDir "Scripts\cryozen.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Cryozen install open ($shim locked after 20s). Nothing was changed. Close other Cryozen windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the cryozen.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\cryozen.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_cryozen_exe must first rename the running cryozen.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that cryozen.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Cryozen window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Cryozen.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m cryozen_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Cryozen installer or `cryozen doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    ...
    $updateArgs = @("-m", "cryozen_cli.main", "update", "--yes") + $gatewayArg + @("--force", "--branch", $Branch)
    ...
    # collides with the "close all Cryozen windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m cryozen_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed cryozen predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-CryozenStep $pythonExe $updateArgs "update"
    Write-HandoffLog "cryozen update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-CryozenUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `cryozen update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "cryozen update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-CryozenStep $pythonExe @("-m", "cryozen_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import cryozen_cli.main; from cryozen_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-CryozenStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated Cryozen runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `cryozen desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `cryozen debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Cryozen is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Cryozen was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Cryozen."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Cryozen"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Cryozen to finish (it could not restart itself)."
