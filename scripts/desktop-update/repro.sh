#!/bin/bash
# repro.sh -- reproduce desktop-update paths against a sandboxed CRYOZEN_HOME.
#
# Nothing here touches your real ~/.cryozen-agent or checkout. Each mode builds (or
# reuses) a disposable install under $TMPDIR and drives the REAL code path --
# the actual installer, the actual orchestrator, the actual `cryozen update`.
#
#   repro.sh shim          shim UI only: success event after 6s
#   repro.sh shim-fail     shim UI only: error event after 6s
#   repro.sh fresh         fresh install into a sandbox CRYOZEN_HOME
...
SANDBOX="${CRYOZEN_UPDATE_REPRO_HOME:-/tmp/cryozen-update-repro}"
SANDBOX_ROOT="$SANDBOX/cryozen-agent"

say() { printf '\n\033[1m== %s ==\033[0m\n' "$1"; }

ensure_sandbox_install() {
  if [ -x "$SANDBOX_ROOT/venv/bin/cryozen" ]; then
  ...
  CRYOZEN_HOME="$SANDBOX" bash "$SANDBOX_ROOT/scripts/install.sh" --non-interactive --skip-setup --cryozen-home "$SANDBOX"
}

case "$MODE" in
  shim)
    CRYOZEN_SELFTEST_HOLD_SECONDS="${CRYOZEN_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  shim-fail)
    CRYOZEN_SELFTEST_FAIL=1 CRYOZEN_SELFTEST_HOLD_SECONDS="${CRYOZEN_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  fresh)
    rm -rf "$SANDBOX"
    ensure_sandbox_install
    say "fresh install OK: $("$SANDBOX_ROOT/venv/bin/cryozen" --version 2>/dev/null || echo '?')"
    ;;
  behind)
    N="${2:-25}"
    ensure_sandbox_install
    ...
    CRYOZEN_HOME="$SANDBOX" bash "$SCRIPT_DIR/posix.sh" \
      --install-root "$SANDBOX_ROOT" --branch main --desktop-pid 0 || true
    say "result file:"
    cat "$SANDBOX/.cryozen-update-result.json" 2>/dev/null || echo "(none written)"
    echo
  ...
  gate)
    ...
    G="$(mktemp -d -t cryozen-gate-test.XXXXXX)"
    UNPACKED="$G/cryozen-agent/apps/desktop/release/linux-unpacked"
    mkdir -p "$UNPACKED"
    touch "$UNPACKED/cryozen" && chmod +x "$UNPACKED/cryozen"
    ...
    decide() { bash "$SCRIPT_DIR/posix.sh" --self-test-gate --install-root "$G/cryozen-agent" "$@" | cut -d: -f1; }

    expect "appimage (not under unpacked)"      skew     "$(decide --relaunch-target /opt/Cryozen/cryozen)"
    expect "sibling-prefix dir not fooled"      skew     "$(decide --relaunch-target "$UNPACKED-evil/cryozen")"
    expect "no chrome-sandbox (namespace)"      relaunch "$(decide --relaunch-target "$UNPACKED/cryozen")"

    touch "$UNPACKED/chrome-sandbox"
    expect "sandbox not root/setuid"            manual   "$(decide --relaunch-target "$UNPACKED/cryozen")"
    expect "opt-out: --sandbox-fallback"        relaunch "$(decide --relaunch-target "$UNPACKED/cryozen" --sandbox-fallback)"
    expect "opt-out: --no-sandbox launch arg"   relaunch "$(decide --relaunch-target "$UNPACKED/cryozen" -- --no-sandbox)"
    expect "opt-out: ELECTRON_DISABLE_SANDBOX"  relaunch "$(ELECTRON_DISABLE_SANDBOX=1 decide --relaunch-target "$UNPACKED/cryozen")"
    ...
    QHOME="$G/qhome"; mkdir -p "$QHOME/cryozen-agent"
    bash "$SCRIPT_DIR/posix.sh" --no-ui --no-marker-cleanup --desktop-pid 0 \
      --install-root "$QHOME/cryozen-agent" --branch 'evil"branch\n$(x)' >/dev/null 2>&1 || true
    if python3 -c "import json,sys; d=json.load(open('$QHOME/.cryozen-update-result.json')); sys.exit(0 if d['branch']=='evil\"branch\\\\n\$(x)' and d['ok']==False else 1)"; then
  ...
  launch)
    ...
    L="$(mktemp -d -t cryozen-launch-test.XXXXXX)"
    fails=0
    expect_msg() { # name python-expr
      if python3 -c "import json,sys; d=json.load(open('$L/.cryozen-update-result.json')); sys.exit(0 if ($2) else 1)"; then
        printf 'ok   %s\n' "$1"
      else
        printf 'FAIL %s -> %s\n' "$1" "$(cat "$L/.cryozen-update-result.json" 2>/dev/null)"; fails=$((fails+1))
      fi
    }
    stub_install() { # creates a fake install whose cryozen update succeeds
      rm -rf "$L"; mkdir -p "$L/cryozen-agent/venv/bin"
      printf '#!/bin/sh\nexit 0\n' > "$L/cryozen-agent/venv/bin/cryozen"
      chmod +x "$L/cryozen-agent/venv/bin/cryozen"
    }

    # 1. linux relaunch target dies instantly -> manual downgrade in result
    stub_install
    UNPACKED="$L/cryozen-agent/apps/desktop/release/linux-unpacked"
    mkdir -p "$UNPACKED"
    printf '#!/bin/sh\nexit 1\n' > "$UNPACKED/cryozen"; chmod +x "$UNPACKED/cryozen"
    if [ "$(uname)" != "Darwin" ]; then
      bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/cryozen-agent" \
        --relaunch-target "$UNPACKED/cryozen" >/dev/null 2>&1 || true
      expect_msg "instant-exit relaunch downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen Cryozen' in d['message']"
    else
      ...
      expect_msg "missing bundle downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen Cryozen' in d['message']"
    fi
    ...
    stub_install
    bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/cryozen-agent" \
      --relaunch-target /opt/Cryozen/cryozen >/dev/null 2>&1 || true
    if [ "$(uname)" != "Darwin" ]; then
