...
distributions it is a RAM-backed tmpfs that fills under Cryozen load. Cryozen resolves scratch
space through one helper (``cryozen_constants.get_scratch_dir()`` → ``CRYOZEN_HOME/cache/scratch``,
which every Cryozen process also exports as ``TMPDIR``/``TMP``/``TEMP``), and prompts + skills
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"cryozen_constants.get_scratch_dir() (or $TMPDIR / tempfile, which Cryozen points there), tell the "
