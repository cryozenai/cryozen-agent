 ...
 *   The artifact published on the website right now -- Cryozen-Setup.exe has
 ...
 * @typedef {InstallMethod | 'cryozen-update' | 'open-app-update' | 'cryozen-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; cryozen-desktop-app-update starts the app
 *   via `cryozen desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://cryozenai.github.io/cryozen-agent/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds Cryozen.exe AND
      ...
      // Website Cryozen-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `cryozen desktop`.
      { method: 'cryozen-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Cryozen-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `cryozen desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'cryozen-desktop-app-update';
}
