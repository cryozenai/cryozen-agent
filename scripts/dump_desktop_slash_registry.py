...
Run after changing any ``desktop=`` value or alias in ``cryozen_cli/commands.py``;
``tests/cryozen_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from cryozen_cli.commands import desktop_surface_registry
