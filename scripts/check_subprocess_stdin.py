...
When Cryozen runs in TUI mode, the gateway child process communicates with
...
# ``get_cryozen_home() / "plugins"`` (user) and ``./.cryozen-agent/plugins/`` (project,
# gated behind ``CRYOZEN_ENABLE_PROJECT_PLUGINS``) — see
# ``cryozen_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``cryozen_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import cryozen_constants (this script
    ...
    # plugins in get_cryozen_home()/plugins/ and project plugins in
    # ./.cryozen-agent/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_cryozen_home() / "plugins"]
    if os.environ.get("CRYOZEN_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".cryozen-agent" / "plugins")
