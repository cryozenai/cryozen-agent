...
Registry and ABC for every inference provider Cryozen knows about.
...
`$CRYOZEN_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `cryozen_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `cryozen_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `cryozen_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `cryozen_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `cryozen_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$CRYOZEN_HOME/plugins/model-providers/` for a private plugin).
...
| `fetch_models(*, api_key)` | Live catalog fetch — default hits `{models_url or base_url}/models` with Bearer auth. Override for no-REST providers (Bedrock), non-Bearer catalogs (Anthropic x-api-key), or public catalogs (OpenRouter). |
