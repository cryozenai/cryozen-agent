...
def _profile_user_agent() -> str:
    """Return a ``cryozen-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from cryozen_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"cryozen-cli/{_ver}"
    except Exception:
        return "cryozen-cli"
...
class ProviderProfile:
    ...
    # ``auth_handler(action, args) -> bool``: ``cryozen auth add|status|logout|refresh <name>`` calls it
    ...
    # Both own their own token endpoints; Cryozen passes no secrets beyond the pooled row itself.
    ...
    # describe how to launch it; cryozen_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.cryozen-agent/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # cryozen-cli UA so the catalog endpoint is reachable.
