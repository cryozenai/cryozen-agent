...
an unlinked SQLite DB/WAL/SHM generation.  ``cryozen_state`` supplies only the
...
_CRYOZEN_EXECUTABLES = frozenset({"cryozen", "cryozen-agent", "cryozen-acp"})
_CRYOZEN_PYTHON_MODULES = frozenset({"acp_adapter", "cryozen_cli.main"})
_CRYOZEN_PYTHON_SCRIPTS = frozenset({"cryozen_cli/main.py", "run_agent.py"})
...
def describe_holder_pid(pid: int) -> str:
    """``PID 123 (cryozen gateway run)`` for operator-facing holder lists; /proc argv first, psutil elsewhere."""
...
def _looks_like_cryozen(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Cryozen execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _CRYOZEN_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _CRYOZEN_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _CRYOZEN_PYTHON_SCRIPTS
    )
...
def _argv_env_home(argv: Sequence[str]) -> Optional[str]:
    """``CRYOZEN_HOME=<path>`` env-style assignment on the argv (``env CRYOZEN_HOME=… cryozen …``)."""
    for token in reversed(list(argv)):
        if isinstance(token, str) and token.startswith("CRYOZEN_HOME="):
            return token[len("CRYOZEN_HOME="):]
    return None
...
def _store_install_layout(this_home: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    string test: an arbitrary ``<X>/profiles/<n>/`` tree is not a Cryozen install, and promoting
    ``<X>`` to "ours" swallows an unrelated instance living under it — the literal two-instance
    shape of #92401. The root store (``~/.cryozen-agent/state.db``) is its own root with no profile name,
    ...
    try:
        ...
        if os.path.basename(this_home) == ".cryozen-agent":
...
def _argv_home_selection(
    ...
    serves; its ``--cryozen-home``/``CRYOZEN_HOME=``/``--profile``/``-p`` selection does. Same
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the CRYOZEN_HOME root, so an absolute-path token
    containing a ``/.cryozen-agent`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Cryozen home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.cryozen-agent deferred
    ...
    2. The process's own ``--cryozen-home``/``CRYOZEN_HOME=``/``--profile``/``-p``
    ...
    3. Path tokens. A token under ``<install root>/profiles/<other>`` is another
       ...
       part of an install layout (a custom ``CRYOZEN_HOME`` is served BY the binary
       under ``~/.cryozen-agent`` — dismissing on it admits maintenance under a live writer).
    """
    ...
    for index, normalized in path_tokens:
        ...
        if index == 0 and not argv0_locates_home:
            continue
        if "/.cryozen-agent" in normalized or normalized.endswith("/.cryozen-agent"):
            other_home_seen = True
...
def foreign_state_db_holders(db_path: Path) -> List[Tuple[int, str]]:
    ...
    # CRYOZEN_HOME would otherwise make every holder invisible and let maintenance proceed.
...
def in_process_state_db_holders(
    ...
    """
    from cryozen_state_registry import other_generations_for_path
...
def held_store_refusal(db_path: Path, *, command: str, force_hint: Optional[str] = "--force") -> Optional[str]:
    ...
    Running ``cryozen sessions optimize-storage`` underneath a fleet of live gateways put every agent into
    ...
    lines = [f"Refusing `cryozen sessions {command}`: another process is using {db_path}."]
    ...
    lines += [
        ...
        f"Stop them first (`cryozen {profile_arg}gateway stop`, quit the Desktop app, pause cron), then re-run.",
    ]
    if force_hint:
