...
def _relay_root() -> Path:
    ...
    every CRYOZEN_HOME, including non-``profiles/`` subdirs of ``~/.cryozen-agent``."""
    from tools.bot_mode_probe import _default_home, _cryozen_root
    return _cryozen_root(Path(_default_home()))
...
def _run_delivery(profile: str, tmp: str, env: dict | None = None, *,
    ...
    try:
        # The relay pins UTF-8 on every platform (#93590): its child is the bootstrapped cryozen_cli
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery,
    ...
    GATEWAY via the one-turn ``cryozen -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "cryozen" else profile
        if resolved not in known:
            return _err(rid, 4092, f"no profile '{profile}' on this gateway")
        # The sender stamped itself with its bare @handle; a relayed "@cryozen" is ANOTHER machine's
        ...
        if _is_authenticated_identity(identity):
            ...
            # cryozen_cli/dashboard_auth/routes.py), so refusing took cross-connection relay offline
        ...
        owner_home = live_home if live_home is not None else Path(_cryozen_home)
        ...
        fd, tmp = tempfile.mkstemp(prefix="cryozen-relay-dm-", suffix=".txt", text=True)
        try:
