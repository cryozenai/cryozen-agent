...
activate with `cryozen --tui` or `CRYOZEN_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `CRYOZEN_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
the launch home, never `~/.cryozen-agent`. The first non-launch home hosted flips
...
**A method that sets only `get_cryozen_home_override()` is half-bound**: config paths resolve to the
...
(`cryozen -p X` workers, `key_cmd` helpers, browser drivers), never `dict(os.environ)`. Grep for
...
themselves come from `cryozen_cli/commands.py` (`cryozen_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds cryozen-ink + tsx --watch)
npm start         # production
npm run build     # full build (cryozen-ink + tsc)
