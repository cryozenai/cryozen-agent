...
def _legacy_group_fence_error(rid, session, params):
    ...
    if hosted or peer:
        ...
        return _err(
            rid, 4122, f"This room is managed by {owner}. Update Cryozen Desktop to continue it.")
    return None
...
def _parse_truncation_params(rid, sid, session, params, history):
    ...
    return None, None, _err(
        ...
        "(update your Cryozen client if a rewind was intended)")
...
def _(rid, params: dict) -> dict:
    from cryozen_cli.input_sanitize import sanitize_user_prompt_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from cryozen_cli.clipboard import has_clipboard_image, save_clipboard_image
...
_PREVIEW_RESTART_RULES = (
    "Restart exactly the app intended for the Preview URL, not Cryozen Desktop itself.",
    ...
    "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Cryozen/Desktop process cwd.",
    ...
    "Before declaring success, verify the Preview URL responds with the intended app, not Cryozen Desktop. If it serves Cryozen/Desktop UI or another unrelated app, stop that process and report failure.",
