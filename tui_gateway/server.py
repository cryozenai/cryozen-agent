...
from cryozen_constants import (
    get_cryozen_home, get_cryozen_home_override, get_process_cryozen_home, profile_name_for_home,
    reset_cryozen_home_override, set_cryozen_home_override)
from cryozen_cli.env_loader import load_cryozen_dotenv
from utils import file_signature, is_truthy_value
from cryozen_state_ids import new_session_id
from tools.environments.local import cryozen_subprocess_env
...
_cryozen_home = _CRYOZEN_HOME_AT_IMPORT = get_cryozen_home()
load_cryozen_dotenv(cryozen_home=_cryozen_home, project_env=Path(__file__).parent.parent / ".env")
...
_CRASH_LOG = os.path.join(_cryozen_home, "logs", "tui_gateway_crash.log")
...
with contextlib.suppress(Exception):
    from cryozen_cli.banner import prefetch_update_check
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, env_float("CRYOZEN_TUI_SLASH_TIMEOUT_S", 45.0))
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    return _ws_orphan_setting("CRYOZEN_TUI_WS_ORPHAN_REAP_GRACE_S", "ws_orphan_reap_grace_s", 20.0)
...
_WS_ORPHAN_ACTIVITY_STALE_S = _ws_orphan_setting("CRYOZEN_TUI_WS_ORPHAN_ACTIVITY_STALE_S", "ws_orphan_activity_stale_s", 600.0)
...
# session.usage = blocking provider account-usage round-trips; complete.* = git ls-files /
...
_rpc_pool_workers = max(2, env_int("CRYOZEN_TUI_RPC_POOL_WORKERS", 8))
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "cryozen_gateway_runtime_session_record", default=None)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar("cryozen_gateway_rpc_method", default="")
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    ~/.local/bin to PATH so slash_worker children resolve Cryozen-managed CLIs under the Desktop's minimal PATH."""
    managed_bin = ""
    with contextlib.suppress(Exception):
        managed_bin = str(Path(get_cryozen_home()) / "bin")
...
class _SlashWorker:
    """Persistent CryozenCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Cryozen agent → needs provider credentials. Tier-1 secrets
        ...
        # `cryozen -p X` would load itself. The launch profile is a profile too: once the process hosts
        ...
        env = _prepend_tool_paths(served_profile_child_env(
            target_home=profile_home or (_cryozen_home if is_multiplex_active() else None),
        ...
        # Prepend the Cryozen venv bin dir and the user-local bin dir to PATH so slash_worker child processes
        # can resolve Cryozen-managed CLIs (browser-use, uvx) even when the parent gateway was launched with
...
_SESSION_TTL_S = max(0.0, env_float("CRYOZEN_TUI_SESSION_TTL_S", float(6 * 3600)))
...
_EXIT_FLUSH_BUDGET_S = max(0.0, env_float("CRYOZEN_TUI_EXIT_FLUSH_BUDGET_S", 5.0))
_INCREMENTAL_FLUSH_INTERVAL_S = max(0.0, env_float("CRYOZEN_TUI_SESSION_FLUSH_INTERVAL_S", _REAPER_SCAN_S))
...
def _launch_state_db_path() -> Path:
    """Launch profile's ``state.db`` at call time: the patched ``_cryozen_home`` when a test changed
    it, else the live process home — resolved through :func:`get_process_cryozen_home`, which honours
    ``CRYOZEN_HOME`` but ignores the context-local override. The desktop multiplex cron ticker sets
    ...
    import time lets a harness that redirects ``CRYOZEN_HOME`` after import be honoured (#112692)."""
    home = _cryozen_home if _cryozen_home != _CRYOZEN_HOME_AT_IMPORT else get_process_cryozen_home()
...
def _profile_db(params: dict | None = None, *, writer: bool = False):
    ...
    cryozen_cli.web_routers.profiles._read_profile_db."""
    ...
    if (profile_home := _profile_home(profile)) is None:
        ...
        try:
            if writer:
                ...
                from cryozen_cli.web_server_sessions import _open_session_db_at_path
...
def _canonical_profile_request(name: str) -> str:
    ...
    installation details — unless a real named profile of that name exists (``cryozen`` is a legal
    id), in which case it wins; other unknown names keep failing closed in ``_profile_home``.
    """
    if name.casefold() in {".cryozen-agent", "cryozen"}:
        from cryozen_cli import profiles as profiles_mod
        # Check the profiles root directly: get_profile_dir rejects "cryozen" as a
...
def _db_unavailable_error(rid, *, code: int):
    from cryozen_state_user_copy import describe_storage_failure, storage_failure_details
...
# backend, so calls carry ``profile`` → open that profile's db and bind its CRYOZEN_HOME (ContextVar
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_cryozen_home).resolve():
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s full runtime scope (CRYOZEN_HOME + secrets + terminal policy) around a
...
def _launch_configured_cwd() -> str | None:
    ...
    # Read the launch file by path. ``_load_cfg`` follows the active CRYOZEN_HOME
    # override, which may belong to a different profile-scoped RPC.
    return _profile_configured_cwd(Path(_cryozen_home))
...
def _bind_build_profile_scopes(profile_home: "str | None") -> "_TurnScopes | None":
    """Bind a session profile's CRYOZEN_HOME / secret / terminal scopes for an agent build. ``None`` is the
    ...
    if profile_home:  # secret/terminal helper failed: keep at least the home + terminal refusal scope
        scopes.home = set_cryozen_home_override(profile_home)
        with contextlib.suppress(Exception):
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # Global-remote: bind the session profile's CRYOZEN_HOME and hand the agent that profile's db —
...
def _active_config_path() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _cryozen_home) / "config.yaml"
...
def _save_cfg(cfg: dict):
    global _cfg_cache, _cfg_sig, _cfg_path
    from cryozen_cli.config import atomic_config_write
...
def _set_session_context(session_key: str, cwd: str | None = None, *, ui_session_id: str = "") -> list:
    with contextlib.suppress(Exception):
        ...
        # Live conversation id for subprocess CRYOZEN_SESSION_ID: an explicitly empty contextvar is authoritative
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ.update(CRYOZEN_GATEWAY_SESSION="1", CRYOZEN_EXEC_ASK="1", CRYOZEN_INTERACTIVE="1")
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps({
    "success": False,
    "error": ("No Cryozen Desktop window answered the tour request. The tour is driven by the desktop app's "
              "renderer, which updates separately from this backend, so an app build older than the tour tool "
              "has nothing listening. Update the Cryozen Desktop app and start a new session. Do not retry tour "
              "in this session.")})
...
def _env_model_seed() -> str:
    """The launch-scoped model seed (``cryozen --tui -m``, hosted provisioning); "" when unset."""
    return (os.environ.get("CRYOZEN_MODEL", "") or os.environ.get("CRYOZEN_INFERENCE_MODEL", "")).strip()
...
def _resolve_model() -> str:
    ...
    with contextlib.suppress(Exception):
        from cryozen_cli.models import get_preferred_silent_default_model
        return get_preferred_silent_default_model()
    return "z-ai/glm-5.2"


def _resolve_session_platform() -> str:
    """``CRYOZEN_DESKTOP=1`` without ``CRYOZEN_DESKTOP_TERMINAL`` → "desktop" (chat panel; the agent then
    suggests TUI-only slash commands), else "tui" (embedded terminal pane or standalone ``cryozen --tui``)."""
    desktop = is_truthy_value(os.environ.get("CRYOZEN_DESKTOP"))
    return "desktop" if desktop and not is_truthy_value(os.environ.get("CRYOZEN_DESKTOP_TERMINAL")) else "tui"
...
def _config_model_target() -> tuple[str, str]:
    """(model, provider) selected by config.yaml — and ONLY config: the CRYOZEN_MODEL launch seed fed into
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider := os.environ.get("CRYOZEN_TUI_PROVIDER", "").strip():
        return model, explicit_provider
    if not (explicit_model := _env_model_seed()):
        return model, None
    with contextlib.suppress(Exception):
        from cryozen_cli.model_switch import resolve_startup_model_route
        from cryozen_cli.models import detect_static_provider_for_model
        full_cfg = _load_cfg()
        cfg = full_cfg.get("model") or {}
        current_provider = ((str(cfg.get("provider") or "").strip().lower() if isinstance(cfg, dict) else "")
                            or os.environ.get("CRYOZEN_INFERENCE_PROVIDER", "").strip().lower() or "auto")
        # Same owner as CryozenCLI/oneshot: ``custom:<name>:<model>`` selects that provider (#73943).
...
from cryozen_state import _BARE_BILLING_PROVIDERS


def _is_routable_provider(provider: str) -> bool:
    with contextlib.suppress(Exception):
        from cryozen_cli.runtime_provider import is_routable_provider
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if provider and not _is_routable_provider(provider):
        healed = None
        try:
            from cryozen_cli.runtime_provider import canonical_custom_identity
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # persist). The full scope, not CRYOZEN_HOME alone: the external memory provider's system_prompt_block()
...
def _load_reasoning_config(model: str = "") -> dict | None:
    """Via the shared chokepoint :func:`cryozen_constants.resolve_reasoning_config` (per-model override >
    global ``agent.reasoning_effort``; YAML False = disabled).

    Closes #21256.
    """
    from cryozen_constants import resolve_reasoning_config
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("CRYOZEN_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    """Toolsets that exist because of the CLIENT (both off ``_CRYOZEN_CORE_TOOLS``; this is the one gate).
    ``platform`` is the SESSION's source, never a process env var: the desktop may drive a URL/cloud
    backend where ``CRYOZEN_DESKTOP`` is unset (AGENTS.md surface rule)."""
...
def _resolve_explicit_toolsets(explicit: list[str], validate_toolset) -> list[str] | None | bool:
    """Resolve a CRYOZEN_TUI_TOOLSETS pin: list, None for "all", False when nothing was valid."""
    ...
    if any(name in {"all", "*"} for name in built_in):
        if ignored := [name for name in explicit if name not in {"all", "*"}]:
            _tui_notice(f"[tui] CRYOZEN_TUI_TOOLSETS=all enables every toolset; ignoring additional entries: {', '.join(ignored)}")
        return None
    if not unresolved:
        return built_in
    ...
    if unknown:
        _tui_notice(f"[tui] ignoring unknown CRYOZEN_TUI_TOOLSETS entries: {', '.join(unknown)}")
    if disabled:
        _tui_notice("[tui] ignoring disabled MCP servers in CRYOZEN_TUI_TOOLSETS "
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    """The agent's toolsets for this session (None = all): an explicit CRYOZEN_TUI_TOOLSETS pin; else the
    ...
    explicit = [item.strip() for item in os.environ.get("CRYOZEN_TUI_TOOLSETS", "").split(",") if item.strip()]
    ...
    if explicit and validate_toolset is not None:
        ...
        fallback_notice = "[tui] no valid CRYOZEN_TUI_TOOLSETS entries; using configured CLI toolsets"
    try:
        from cryozen_cli.config import load_config
        ...
        if fallback_notice is not None:
            _tui_notice("[tui] no valid CRYOZEN_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets")
        return None
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            with contextlib.suppress(Exception):
                from cryozen_cli.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from cryozen_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if provider == "custom" and "provider" not in mirror and agent is not None:
        ...
        with _profile_build_scope(sess.get("profile_home") or _cryozen_home):
    ...
    # Cryozen-internal step (#61634) as a wire level the route does not have.
    ...
    if live_agent:
        ...
        with contextlib.suppress(Exception):
            from cryozen_cli.banner import get_available_skills
    ...
    with contextlib.suppress(Exception):
        from cryozen_cli.banner import get_update_result
        from cryozen_cli.config import recommended_update_command
...
def _resolve_runtime_with_fallback(resolve_kwargs: dict | None = None) -> _RuntimeFallbackResolution:
    ...
    from cryozen_cli.runtime_provider import resolve_runtime_provider
    try:
        ...
        for entry in _load_fallback_model() or []:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from cryozen_cli.fallback_config import effective_runtime_provider, resolve_entry_api_key
                ...
                from cryozen_cli.auth import primary_failure_wording
...
def _rederive_per_model_route(model: str, runtime: dict) -> None:
    ...
    from cryozen_cli.model_switch import model_derived_api_mode
    from cryozen_cli.models import normalize_opencode_base_url
...
def _startup_system_prompt(cfg: dict, task_id: str) -> str:
    """Config ephemeral system prompt + CRYOZEN_TUI_SKILLS preload block. Hard-fails only when EVERY requested
    skill is missing (cli.py parity): a typo'd name must not auto-block the Kanban task."""
    from cryozen_cli.config import resolve_ephemeral_system_prompt_from_config
    ...
    if missing_skills:
        ...
        logger.warning("Unknown skill(s) requested, skipping: %s. Continuing with: %s. "
                       "List available skills with `cryozen skills list`.", missing_display, ", ".join(loaded_skills))
    if skills_prompt:
...
def _make_agent(
    ...
    for _mod in ("cryozen_cli.mcp_startup", "tui_gateway.entry"):
        with contextlib.suppress(Exception):
    ...
    ignore_rules = is_truthy_value(os.environ.get("CRYOZEN_IGNORE_RULES"))
    with _sessions_lock:
        session = _sessions.get(sid)
    agent = AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("CRYOZEN_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("CRYOZEN_TUI_PASS_SESSION_ID")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # Profile-scoped CRYOZEN_HOME (None = launch); SessionBranch copies the parent's (same state.db).
...
def _load_resume_transcript(db, stored_id: str, *, model_history_only: bool = False) -> tuple[list, list, list]:
    ...
    from cryozen_state import SessionResumeTooLargeError
...
def _pet_gen_root():
    ...
    root = get_cryozen_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(1, int(os.environ.get("CRYOZEN_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)))
except (TypeError, ValueError):
...
def _spawn_trees_root():
    root = get_cryozen_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
_CLI_EXEC_BLOCKED = {
    ("setup",): "`cryozen setup` needs a full terminal — run it outside the TUI",
    ("gateway",): "`cryozen gateway` is long-running — run it in another terminal",
    ("sessions", "browse"): "`cryozen sessions browse` is interactive — use /resume here, or run browse in another terminal",
    ("config", "edit"): "`cryozen config edit` needs $EDITOR in a real terminal",
}
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `cryozen` is interactive — use `/cryozen chat -q …` or run `cryozen` in another terminal"
...
def _resolve_name(name: str) -> str:
    with contextlib.suppress(Exception):
        from cryozen_cli.commands import resolve_command
