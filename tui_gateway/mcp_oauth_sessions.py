...
def register_flow(flow, *, httpd=None) -> Dict[str, Any]:
    ...
    rec = {
        ...
        "cryozen_home": flow.cryozen_home,
        "flow": flow,
        "httpd": httpd,
...
def start_flow(
    cryozen_home: str, server_name: str, cfg: dict, *, reconnect_live: bool = False,
    ...
    with _sessions_lock:
        ...
        if any(r["server_name"] == server_name and r["cryozen_home"] == cryozen_home for r in active):
            raise RuntimeError(f"MCP OAuth for '{server_name}' is already in progress")

    session_id = secrets.token_urlsafe(24)
    flow = DashboardOAuthFlow(
        flow_id=session_id, server_name=server_name, profile=None, cryozen_home=cryozen_home,
    ...
    threading.Thread(
        target=run_worker, args=(cryozen_home, server_name, dict(cfg), reconnect_live),
...
def _lookup(
    session_id: str, server_name: str, cryozen_home: Optional[str] = None,
    ...
    if cryozen_home_key(rec["cryozen_home"]) != cryozen_home_key(cryozen_home):
...
def cancel_flow(session_id: str, server_name: str, cryozen_home: str) -> Dict[str, Any]:
    """Cancel only the owning profile's flow and release its callback waiter."""
    rec, err = _lookup(session_id, server_name, cryozen_home)
    if rec is None:
