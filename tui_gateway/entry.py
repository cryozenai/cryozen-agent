...
# Stop a ``utils/``-style package in the launch directory from shadowing Cryozen's own
# top-level modules; ``cryozen_bootstrap``'s name can't collide, so importing it first is safe.
import cryozen_bootstrap
...
# cryozen_cli.mcp_startup (current path). The wait/in-flight/join helpers consult both.
...
def _install_sidecar_publisher() -> None:
    ...
    url = os.environ.get("CRYOZEN_TUI_SIDECAR_URL")
    if not url:
        return
...
# strand the process; ``CRYOZEN_TUI_GATEWAY_SHUTDOWN_GRACE_S`` overrides.
_DEFAULT_SHUTDOWN_GRACE_S = 1.0


def _shutdown_grace_seconds() -> float:
    value = env_float("CRYOZEN_TUI_GATEWAY_SHUTDOWN_GRACE_S", _DEFAULT_SHUTDOWN_GRACE_S)
    return value if value > 0 else _DEFAULT_SHUTDOWN_GRACE_S


def _mcp_startup_call(name: str, *args, default=None, log=None, **kwargs):
    """Call ``cryozen_cli.mcp_startup.<name>`` (lazy import); ``default`` on any failure,
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # Discovery is spawned via the shared owner (ensure_mcp_discovery_started → cryozen_cli.mcp_startup);
    ...
    # CALLER's profile context (agent build binds the session profile's CRYOZEN_HOME first), so a launch
...
def mcp_discovery_in_flight() -> bool:
    ...
    There are two independent discovery-thread owners by surface: the stdio ``cryozen --tui`` path spawns ITS
    thread here (``_mcp_discovery_thread``), while the desktop app + dashboard WebSocket sidecar
    (``tui_gateway/ws.py``) and ``cryozen dashboard`` spawn theirs via
    ``cryozen_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh scheduler imports this
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    ``cryozen_cli.mcp_startup`` thread used by the desktop/dashboard surfaces. See #51587.
    """
    entry_done = True
    thread = _mcp_discovery_thread
...
def _has_configured_mcp_servers() -> bool:
    ...
    from cryozen_cli.mcp_startup import _has_configured_mcp_servers as configured
    return configured()
...
def ensure_mcp_discovery_started() -> None:
    ...
    session profile's CRYOZEN_HOME.

    WebSocket/Desktop entrypoints can accept sessions without running ``main()``, so the agent-build path
    (``server._start_agent_build``) also calls it AFTER binding the session profile's CRYOZEN_HOME override —
    the shared owner in ``cryozen_cli.mcp_startup`` captures the caller's context-local override and
    propagates it into the discovery thread, so discovery reads the SELECTED profile's ``mcp_servers``, not
    the launch profile's. The discovery slot in ``cryozen_cli.mcp_startup`` is keyed by profile home, so
...
def main():
    ...
    # Live-apply skins Cryozen activates mid-conversation.
    server._ensure_skin_watcher()

    # Warm the /model picker's provider-models cache in this idle window (fire-and-forget).
    try:
        from cryozen_cli.model_switch_providers import prewarm_picker_cache_async
