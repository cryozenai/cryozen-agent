...
from cryozen_constants import display_cryozen_home as _display_cryozen_home
...
def _cfg_get_provider(params):
    from cryozen_cli.models import list_available_providers, normalize_provider
...
def _cfg_get_mtime(params):
    cfg_path = _cryozen_home / "config.yaml"
    try:
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_cryozen_home), "display": _display_cryozen_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's CRYOZEN_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    # (``CRYOZEN_CODEX_BASE_URL`` for openai-codex) fails closed and the UI shows onboarding.
...
@method("setup.status")
def _(rid, params: dict) -> dict:
    """Loose provider check; ``profile`` (optional) scopes it to that profile's home. A live probe
    of the config files, so a provider added after boot (the Models page, a picker key,
    ``cryozen setup`` from a shell) counts without a restart. ``ready`` is always true: since one
    host backend serves every profile (#118246), the desktop's setup-profile probe lands here, and
    its kickoff requires ``ready``."""
    try:
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Cryozen provider is configured.", source)
