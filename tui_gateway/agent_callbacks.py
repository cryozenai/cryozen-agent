...
def _agent_cbs(sid: str) -> dict:
    ...
    callbacks = {
        ...
        # Notice spine: AgentNotice → notification.show; recovery → notification.clear.
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from cryozen_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via cryozen_cli.personality (single owner)."""
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    from cryozen_cli.personality import normalize_personality_name, render_personality_prompt
...
def _prompt_text(value) -> str:
    """Normalize config prompt values from YAML for AIAgent (cryozen_cli.personality owns this)."""
    from cryozen_cli.personality import prompt_text
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    ...
    if env_val := os.environ.get("CRYOZEN_TUI_MAX_TURNS"):
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("CRYOZEN_TUI_SKILLS", "")
...
def _load_fallback_model():
    ...
    CryozenCLI/gateway: ``fallback_providers`` first, legacy ``fallback_model`` merged after)."""
...
def _sync_agent_fallback_with_config(sid: str, session: dict) -> None:
    ...
    opened before ``cryozen fallback add`` kept an empty chain forever and a provider-quota 429 ended in
