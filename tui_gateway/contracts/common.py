...
class OpenModel(Result):
    ...
    (the closed set is owned elsewhere: cryozen_state rows, provider inventories)."""
...
class SessionLiveInfo(OpenModel):
    ...
    # equal when verbatim). Lets clients label a clamped Cryozen step ("ultra sends max on this route").
...
class StoredSessionRow(OpenModel):
    """One ``sessions`` row as ``cryozen_state`` lists it (``session.list`` / ``session.info`` rows /
    ``sessions.changed``)."""
...
class MessageReaction(OpenModel):
    """One persisted reaction row (``cryozen_state_messages.set_message_reaction``); ``seen`` is
...
__all__ = [
    "TERMINAL_SUBAGENT_STATUSES", "ConnectorOwner", "EmptyPayload", "EmptyResult", "McpServerStatus",
