...
method("reload.env", params=ReloadEnvParams, result=ReloadEnvResult,
       doc="Re-read ~/.cryozen-agent/.env (CLI /reload parity); built agents keep their pool until /new.")
...
class SkillBrowseItem(OpenModel):
    """``cryozen_cli.skills_hub.browse_skills`` row."""
...
class SkillInspectInfo(OpenModel):
    """``cryozen_cli.skills_hub.inspect_skill``; ``{}`` when the identifier resolves nowhere."""
...
class PluginSettingField(Result):
    ...
    (``cryozen_cli.plugins_settings.plugin_settings_fields``). ``secret`` fields carry no value: ``env``
...
class PluginActivation(Result):
    """What a plugin loaded mid-run does NOW vs later (``cryozen_cli.plugins_activation``). ``activated_now``
...
class PluginsManageResult(Result):
    ...
    (the canonical key written)/``plugin``; ``install`` → ``cryozen_cli.plugins_cmd.dashboard_install_plugin``'s
