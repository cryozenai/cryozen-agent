"""Contracts: config, setup readiness, model inventory, connectors,
image generation and structured session control.

Handlers: ``tui_gateway/methods_config.py`` (``config.get``, ``setup.*``),
``methods_config_set.py`` (``config.set``), ``methods_complete.py``
...
class ConfigProviderRef(OpenModel):
    """``cryozen_cli/models.py::list_available_providers`` row."""
...
class SetupStatusResult(Result):
    """``provider_configured`` is the loose answer; ``ready`` is always true (nothing is minted at
    boot, so there is nothing to wait for). An unknown ``profile`` answers ``ok=False`` + ``error``."""
...
class ModelPricing(Result):
    """``cryozen_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown,
    ``"free"``); the sale fields are set only when a provider reports a discount."""
...
class ModelCapabilities(Result):
    """``cryozen_cli/inventory.py::_apply_capabilities``."""
...
class ModelOptionProvider(OpenModel):
    """One ``cryozen_cli/inventory.py::build_models_payload`` provider row (the union of every field
    the builder sets; ``pricing_pending`` marks the cached-only path)."""
...
class GoalContractSnapshot(Result):
    """``cryozen_cli/goals.py::GoalContract.to_dict``."""
