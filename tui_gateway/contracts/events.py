...
module (the skin engine, the pet store, the goal/loop/heartbeat state files) or they are watcher
signals whose payload is ``{}`` today and may grow. Everything else is closed, so a drifted
emitter fails the suite (``registry.check_payload`` raises under ``CRYOZEN_TEST_ISOLATION``).
"""
...
from .config_setup_control import SessionControlSnapshot
...
class SkinPayload(OpenPayload):
    """``tui_gateway/change_watcher.py::resolve_skin`` — the resolved active skin (``CryozenSkin``).
...
class NotificationShowPayload(Payload):
    """``agent/status_output.py::AgentNotice`` via notice_callback, and ``server._await_agent_ready``'s
