...
def resolve_skin() -> dict:
    try:
        from cryozen_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _watcher_home() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _cryozen_home)
...
def _pairing_roots(home: Path) -> list:
    ...
    if cached is not None and cached[0] == home and cached[1] == dir_mtime and now - cached[2] < _PAIRING_ROOTS_TTL_S:
        return cached[3]
    from cryozen_constants import named_profile_is_live
...
def _ensure_skin_watcher() -> None:
    ...
    threading.Thread(target=_loop, name="cryozen-change-watcher", daemon=True).start()
