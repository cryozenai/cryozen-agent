"""Tests for cryozen_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import cryozen_logging
# Use whatever RotatingFileHandler class cryozen_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. cryozen_logging
...
from cryozen_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    cryozen_logging._logging_initialized = False
    ...
    cryozen_logging._reset_queued_handlers()
    ...
    cryozen_logging._install_session_record_factory()
    yield
    ...
    cryozen_logging._logging_initialized = False
    cryozen_logging.clear_session_context()


@pytest.fixture
def cryozen_home(tmp_path, monkeypatch):
    """Provide an isolated CRYOZEN_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_cryozen_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["CRYOZEN_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, cryozen_home):
        log_dir = cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        assert log_dir == cryozen_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, cryozen_home):
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in cryozen_logging._queued_file_handlers
    ...
    def test_idempotent_no_duplicate_handlers(self, cryozen_home):
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, cryozen_home):
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)

        test_logger = logging.getLogger("test_cryozen_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        cryozen_logging.flush_log_queue()

        agent_log = cryozen_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_profile_routing_follows_context_home(self, cryozen_home, tmp_path):
        ...
        profile_home.mkdir()
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        assert cryozen_logging.enable_profile_log_routing(
            [cryozen_home, profile_home]
        ) is True
        ...
        assert "profile-routed cron record" not in (
            cryozen_home / "logs" / "agent.log"
        ).read_text()

    def test_release_profile_log_handlers_closes_only_deleted_profile(self, cryozen_home, tmp_path):
        ...
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        assert cryozen_logging.enable_profile_log_routing(
            [cryozen_home, deleted_home, other_home]
        ) is True

        logger = logging.getLogger("agent.profile-delete-log-release")
        token = set_cryozen_home_override(deleted_home)
        try:
            logger.warning("deleted profile log handles")
        finally:
            reset_cryozen_home_override(token)
        token = set_cryozen_home_override(other_home)
        try:
        ...
        routers = [
            handler for handler in cryozen_logging._queued_file_handlers
            if isinstance(handler, cryozen_logging._ProfileRoutingFileHandler)
        ]
        ...
        assert cryozen_logging.release_profile_log_handlers(deleted_home) == 2
    ...
    def test_a_second_home_routes_instead_of_stacking_an_unfiltered_handler(self, cryozen_home, tmp_path):
        ...
        profile_home.mkdir()
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        cryozen_logging.setup_logging(cryozen_home=profile_home)

        assert not [h for h in cryozen_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)], (
        ...
        a_log = (cryozen_home / "logs" / "agent.log").read_text()
    ...
    def test_setup_for_an_already_routed_home_adds_no_duplicate_writer(self, cryozen_home, tmp_path):
        ...
        profile_home.mkdir()
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        assert cryozen_logging.enable_profile_log_routing([cryozen_home, profile_home]) is True
        cryozen_logging.setup_logging(cryozen_home=profile_home)
        ...
        assert "once please" not in (cryozen_home / "logs" / "agent.log").read_text()

    def test_a_component_log_added_after_routing_is_routed_too(self, cryozen_home, tmp_path):
        ...
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        cryozen_logging.setup_logging(cryozen_home=profile_home)
        cryozen_logging.setup_logging(cryozen_home=cryozen_home, mode="gateway")
        ...
        a_log = (cryozen_home / "logs" / "gateway.log").read_text()
    ...
    def test_explicit_params_override_config(self, cryozen_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (cryozen_home / "config.yaml").write_text(yaml.dump(config))

        cryozen_logging.setup_logging(cryozen_home=cryozen_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, cryozen_home):
        cryozen_logging.setup_logging(cryozen_home=cryozen_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, cryozen_home):
        cryozen_logging.setup_logging(cryozen_home=cryozen_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, cryozen_home):
        ...
        gw_log = cryozen_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, cryozen_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, cryozen_home):
        cryozen_logging.setup_logging(cryozen_home=cryozen_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, cryozen_home):
        cryozen_logging.setup_logging(cryozen_home=cryozen_home, mode="gui")

        logging.getLogger("cryozen_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        cryozen_logging.flush_log_queue()

        gui_log = cryozen_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, cryozen_home):
        """When session context is set, log lines include [session_id]."""
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        cryozen_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = cryozen_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, cryozen_home):
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        cryozen_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_cryozen_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        cryozen_logging._add_rotating_handler(
            log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in cryozen_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
        ...
        cryozen_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("cryozen_cli.config.is_managed", return_value=True):
                cryozen_logging._add_rotating_handler(
                    log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = cryozen_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert cryozen_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not cryozen_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, cryozen_home):
        level, max_size, backup = cryozen_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, cryozen_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> cryozen_logging._ManagedRotatingFileHandler:
        handler = cryozen_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that cryozen_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        cryozen_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        cryozen_logging.setup_logging(cryozen_home=cryozen_home, mode="gateway")
        gw_path = cryozen_home / "logs" / "gateway.log"
        rotated = cryozen_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = cryozen_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, cryozen_home):
        cryozen_logging.setup_logging(cryozen_home=cryozen_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_cryozen_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in cryozen_logging._queued_file_handlers
        )
