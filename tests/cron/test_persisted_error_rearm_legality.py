...
def cron_store(tmp_path, monkeypatch):
    ...
    (cryozen_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
    monkeypatch.setattr(J, "CRYOZEN_DIR", cryozen_home)
    monkeypatch.setattr(J, "CRON_DIR", cryozen_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", cryozen_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", cryozen_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_cryozen_now", lambda: now):
            due = J.get_due_jobs()
