...
resolve_runtime_provider against a temp CRYOZEN_HOME) and the job-store pin helpers.
"""
...
def _run(job, tmp_path, *, current_provider="openrouter", current_model=None, cron_model=None,
    ...
    with patch("cron.scheduler._cryozen_home", tmp_path), \
         patch("cron.scheduler._get_cryozen_home", return_value=tmp_path), \
         ...
         patch("cryozen_cli.runtime_provider.resolve_runtime_provider", side_effect=_resolve), \
...
class TestUnpinnedJobsFollowTheMainModel:
    ...
    def test_missing_model_guides_to_user_owned_cli(self, tmp_path, monkeypatch):
        ...
        assert "cryozen cron edit pin-test --model <name>" in error
...
class TestPinnedLocksTheMainModel:
    ...
    def _store(monkeypatch, tmp_path, main_model="main-model", main_provider="openrouter"):
        ...
        monkeypatch.setattr(jobs, "get_cryozen_home", lambda: tmp_path, raising=True)
        state = {"jobs": []}
        ...
        monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", resolver)
    ...
    def test_pinned_never_overrides_an_explicit_model(self, monkeypatch, tmp_path):
        ...
        job = jobs.create_job(prompt="do a thing", schedule="every 1 hour", model="my-model",
                              provider="openrouter", pinned=True)
        assert (job["model"], job["provider"]) == ("my-model", "openrouter")
