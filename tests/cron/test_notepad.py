...
use the notepad, and the `cryozen cron notepad` CLI handler.
"""
...
def cron_env(tmp_path, monkeypatch, notepad):
    """Isolated cron environment with temp CRYOZEN_HOME (mirrors test_cron_context_from)."""
...
class TestNotepadProfileIsolation:
    def test_profile_override_routes_writes_to_current_home(self, tmp_path):
        ...
        import_token = set_cryozen_home_override(profile_a)
        try:
            importlib.reload(notepad_mod)
        finally:
            reset_cryozen_home_override(import_token)

        runtime_token = set_cryozen_home_override(profile_b)
        try:
            notepad_mod.set_note("job-1", "cursor", "page=7")
        finally:
            reset_cryozen_home_override(runtime_token)
...
class TestPromptInjection:
    def test_nonempty_notepad_rendered_into_prompt(self, cron_env, notepad):
        ...
        assert f"cryozen cron notepad {job['id']} set" in prompt
...
class TestNotepadCli:
    ...
    def test_cli_set_get_list_delete(self, notepad, capsys):
        from cryozen_cli.cron import cron_notepad
