...
class TestSlackSyntheticThreadCapture:
    def test_synthetic_slack_thread_not_captured(self):
        ...
        env = {
            "CRYOZEN_SESSION_PLATFORM": "slack",
            "CRYOZEN_SESSION_CHAT_ID": "D0BJTDCSR7C",
            "CRYOZEN_SESSION_THREAD_ID": "1755043010.123456",
            "CRYOZEN_SESSION_MESSAGE_ID": "1755043010.123456",
        }
    ...
    def test_genuine_slack_thread_preserved(self):
        ...
        env = {
            "CRYOZEN_SESSION_PLATFORM": "slack",
            "CRYOZEN_SESSION_CHAT_ID": "C0AGENERAL",
            "CRYOZEN_SESSION_THREAD_ID": "1755040000.000100",
            "CRYOZEN_SESSION_MESSAGE_ID": "1755043010.123456",
        }
        with _session_env(env):
            origin = _origin_from_env()
    ...
    def test_non_slack_platform_thread_untouched(self):
        ...
        env = {
            "CRYOZEN_SESSION_PLATFORM": "telegram",
            "CRYOZEN_SESSION_CHAT_ID": "-1003941067111",
            "CRYOZEN_SESSION_THREAD_ID": "2203",
            "CRYOZEN_SESSION_MESSAGE_ID": "2203",
        }
        with _session_env(env):
            origin = _origin_from_env()
        assert origin is not None
...
def _run_at_in(minutes: int) -> str:
    return (cryozen_time.now() + timedelta(minutes=minutes)).isoformat()
...
class TestNearHorizonSlackThreadKept:
    ...
    def test_oneshot_firing_now_keeps_thread(self):
        ...
        frozen = cryozen_time.now()
        with (
            _session_env(_TOP_LEVEL_SLACK),
            patch("tools.cronjob_job_args.cryozen_time") as frozen_clock,
        ):
