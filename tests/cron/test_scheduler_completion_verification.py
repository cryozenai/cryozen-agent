...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cryozen_state, "SessionDB", _RecordingSessionDB)
    monkeypatch.setattr(run_agent, "AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "cryozen_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setenv("CRYOZEN_TEST_RUNTIME_KEY", "unused-placeholder")

    def _fake_runtime(**_kwargs):
        return {
            "api_key": os.environ.get("CRYOZEN_TEST_RUNTIME_KEY", ""),
            "base_url": None,
