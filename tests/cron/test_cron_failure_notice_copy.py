"""User-facing cron failure notices: plain words, the real output path, and the exact `cryozen cron`
...
def test_generic_failure_names_runs_and_pause_commands_and_the_real_output_dir():
    ...
    for cmd in ("cryozen cron runs ab12cd34", "cryozen cron run ab12cd34", "cryozen cron pause ab12cd34"):
        assert f"`{cmd}`" in msg
    assert f"{display_cryozen_home()}/cron/output/ab12cd34/" in msg
...
def test_auth_failure_points_at_login_and_a_retry_command(monkeypatch):
    ...
    assert "/login" in msg and "`cryozen auth add <provider>`" in msg
    assert "cryozen login" not in msg  # that command was removed
    assert "`cryozen cron run ab12cd34`" in msg
...
def test_auth_failure_names_the_pinned_provider_and_the_failing_profile(monkeypatch, tmp_path):
    ...
    _no_chain(monkeypatch)
    profile_home = tmp_path / ".cryozen-agent" / "profiles" / "ops"
    ...
    assert "`cryozen -p ops auth add openai-codex --type oauth`" in msg, msg
    assert "<provider>" not in msg
    unpinned = _summarize_cron_failure_for_delivery(JOB, "Error code: 401 - Unauthorized")
    assert "`cryozen -p ops auth add <provider>`" in unpinned, unpinned
...
def test_rate_and_usage_limit_phrases_still_yield_a_provider_notice(monkeypatch):
    ...
    provider limit turns into a raw generic notice."""
    _no_chain(monkeypatch)
    for text in (
        "Provider rate limit active until 15:00",
        ...
        assert "`cryozen cron run ab12cd34`" in msg or "`cryozen cron edit ab12cd34" in msg, msg
...
def test_cron_cause_gloss_is_the_shared_table():
    ...
    for reason in FAILURE_CAUSE_GLOSS:
        ...
        assert notice is not None and "`cryozen cron" in notice, reason
...
def test_waf_block_names_the_header_fix_not_a_bare_rerun(monkeypatch):
    ...
    never by `cryozen cron run` alone — the action must say so (#53099, #70566)."""
    _no_chain(monkeypatch)
    msg = _summarize_cron_failure_for_delivery(JOB, "Error code: 403 - Sorry, you have been blocked")
    assert "extra_headers" in msg and "`cryozen cron edit ab12cd34 --provider <name>`" in msg, msg
...
def test_transient_provider_failures_never_lead_with_jargon(monkeypatch):
    ...
    for err in ("Request timed out.", "HTTP 429: Too Many Requests"):
        ...
        assert "`cryozen cron run ab12cd34`" in msg
        assert "`cryozen cron runs ab12cd34`" in msg
...
def test_blocked_config_notice_says_it_did_not_run_and_will_self_heal():
    ...
    assert "`cryozen cron doctor`" in text
