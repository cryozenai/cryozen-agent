...
def cryozen_env(tmp_path, monkeypatch):
    ...
    # Reload modules that cache get_cryozen_home() at import time.
    import importlib
...
def test_create_job_no_agent_requires_script(cryozen_env):
...
def test_update_job_roundtrips_no_agent_flag(cryozen_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = cryozen_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(cryozen_env):
...
def test_run_job_no_agent_success_returns_script_stdout(cryozen_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = cryozen_env / "scripts" / "alert.sh"
...
def test_run_job_no_agent_reloads_dotenv_before_script(cryozen_env, monkeypatch):
    ...
    def fake_load(*, cryozen_home=None, project_env=None):
        loaded_homes.append(cryozen_home)
        return []

    monkeypatch.setattr(env_loader, "load_cryozen_dotenv", fake_load)

    script_path = cryozen_env / "scripts" / "probe.sh"
    script_path.write_text('#!/bin/bash\necho "ok"\n')
    ...
    assert success is True
    assert error is None
    assert loaded_homes, "load_cryozen_dotenv was not called on the no_agent path"
    assert str(loaded_homes[0]) == str(cryozen_env)
...
def test_no_agent_script_gets_owning_profiles_declared_secret_never_launch_residue(
    cryozen_env, monkeypatch, tmp_path,
):
    ...
    (cryozen_env / ".env").write_text("LAUNCH_ONLY_TOKEN=launch-secret\n", encoding="utf-8")
    monkeypatch.setenv("LAUNCH_ONLY_TOKEN", "launch-secret")  # what load_cryozen_dotenv() did at startup
    ...
    home_token = set_cryozen_home_override(str(routed))
...
def test_no_agent_script_of_launch_profile_keeps_its_own_env_credential(cryozen_env, monkeypatch):
    ...
    monkeypatch.delenv("JOB_SVC_TOKEN", raising=False)
    (cryozen_env / "scripts" / "probe.sh").write_text(_PRESENCE_PROBE, encoding="utf-8")
...
def test_timed_out_no_agent_script_delivery_is_not_mislabeled_as_provider_failure(
    ...
    (cryozen_env / "scripts" / "slow.py").write_text("import time; time.sleep(999)\n")
    job = create_job(
        prompt=None,
        schedule="every 5m",
        script="slow.py",
...
def test_agent_provider_timeout_delivery_keeps_fallback_guidance(cryozen_env, monkeypatch):
...
def test_run_job_script_path_traversal_still_blocked(cryozen_env):
...
def test_run_job_script_nul_path_fails_cleanly(cryozen_env):
...
def test_run_job_script_nul_rejected_before_any_path_call(cryozen_env, monkeypatch):
...
def test_run_job_script_accepts_pathlike_script_path(cryozen_env):
    ...
    script = cryozen_env / "scripts" / "probe.py"
...
@pytest.mark.parametrize(
    "error",
    [
        "Script timed out after 900s: /home/u/.cryozen-agent/scripts/nightly.sh",
