...
@pytest.mark.parametrize("error", [None, subprocess.TimeoutExpired("cryozen", 1)])
def test_cli_owner_deferral_and_attempt_fence(tmp_path, monkeypatch, error):
    ...
    monkeypatch.setattr(delivery.shutil, "which", lambda _: "/bin/cryozen")
...
def test_delivery_exception_retains_attempt_and_continues_siblings(tmp_path, monkeypatch):
    ...
    def resolve_cli(name, *args, **kwargs):
        # Delivery resolves the CLI (the running install's ``cryozen_cli``) right after discovery.
        nonlocal armed
        armed = True
        return object()
