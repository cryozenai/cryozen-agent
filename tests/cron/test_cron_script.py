...
- Path resolution (absolute, relative to CRYOZEN_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp CRYOZEN_HOME."""
    cryozen_home = tmp_path / ".cryozen-agent"
    cryozen_home.mkdir()
    (cryozen_home / "cron").mkdir()
    (cryozen_home / "cron" / "output").mkdir()
    (cryozen_home / "scripts").mkdir()
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "CRYOZEN_DIR", cryozen_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", cryozen_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", cryozen_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", cryozen_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_missing_script_names_the_profile_folder(self, cron_env):
        ...
        assert "cryozen cron edit" in output


    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Cryozen provider env (SECURITY.md §2.3)."""
        from tools.environments.local_env_policy import _CRYOZEN_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler_script import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_CRYOZEN_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.cryozen-agent/scripts/ must be rejected."""
        from cron.scheduler_script import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("CRYOZEN_SESSION_PLATFORM") is None
        assert os.environ.get("CRYOZEN_SESSION_CHAT_ID") is None
        assert os.environ.get("CRYOZEN_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_CRON_SCRIPT_TIMEOUT", "2")
