...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_cryozen_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "cryozen_constants.get_default_cryozen_root", lambda: root
    )
    token = set_cryozen_home_override(str(grant_home))
