...
class TestPoolReaping:
    def test_pool_of_an_unticked_home_is_reaped(self, tmp_path):
        ...
        try:
            ...
            assert cryozen_home_key(home_b) not in sched._parallel_pools, (
                "an unticked home's ThreadPoolExecutor leaked for the process's lifetime"
            )
            assert cryozen_home_key(home_b) not in sched._parallel_pool_max_workers
            # The home still being ticked keeps its pool.
            assert cryozen_home_key(home_a) in sched._parallel_pools
        finally:
