...
def _run(job, tmp_path):
    ...
    with patch("run_agent.AIAgent") as agent_cls, \
         ...
         patch("cryozen_state_registry.acquire", return_value=MagicMock()), \
         patch("tools.mcp_tool_discovery.discover_mcp_tools", return_value=[]), \
         patch("cryozen_cli.runtime_provider.resolve_runtime_provider", return_value=dict(_RUNTIME)):
...
def test_requested_mcp_server_owned_by_other_profile_blocks_run(tmp_path):
    ...
    from cryozen_constants import cryozen_home_key, reset_cryozen_home_override, set_cryozen_home_override

    set_multiplex_active(True)
    token = set_cryozen_home_override(tmp_path / "other")
    try:
        undo = _register_notion_in_scope(cryozen_home_key())
    finally:
