"""
Tests for timezone support (cryozen_time module + integration points).
...
def _reset_cryozen_time_cache():
    """Reset the cryozen_time module cache."""
    cryozen_time.reset_cache()


# =========================================================================
# cryozen_time.now() — core helper
# =========================================================================

class TestCryozenTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_cryozen_time_cache()

    def teardown_method(self):
        _reset_cryozen_time_cache()
        os.environ.pop("CRYOZEN_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["CRYOZEN_TIMEZONE"] = "Asia/Kolkata"
        result = cryozen_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["CRYOZEN_TIMEZONE"] = "UTC"
        result = cryozen_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["CRYOZEN_TIMEZONE"] = "America/New_York"
        result = cryozen_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["CRYOZEN_TIMEZONE"] = "Europe/London"
        tz = cryozen_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching CRYOZEN_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("CRYOZEN_TIMEZONE", raising=False)

        monkeypatch.setenv("CRYOZEN_HOME", str(first_home))
        assert str(cryozen_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch CRYOZEN_HOME in one process.
        monkeypatch.setenv("CRYOZEN_HOME", str(second_home))
        assert str(cryozen_time.get_timezone()) == "America/New_York"
    ...
    def test_multiplex_prefers_routed_profile_config_over_env(self, tmp_path, monkeypatch):
        """Under the multiplexed gateway CRYOZEN_TIMEZONE holds only the DEFAULT profile's value
        ...
        monkeypatch.setenv("CRYOZEN_TIMEZONE", "America/New_York")

        # Single-profile process: env stays authoritative.
        assert cryozen_time.get_timezone_name() == "America/New_York"

        set_multiplex_active(True)
        try:
            assert cryozen_time.get_timezone_name() == "America/New_York"  # default profile turn
            token = set_cryozen_home_override(str(routed_home))
            try:
                assert cryozen_time.get_timezone_name() == "Asia/Tokyo"
                assert str(cryozen_time.get_timezone()) == "Asia/Tokyo"
            finally:
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_cryozen_home_override(str(homes[key]))
                try:
                    tz = cryozen_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When CRYOZEN_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) CRYOZEN_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("CRYOZEN_TIMEZONE=" + os.environ.get("CRYOZEN_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "CRYOZEN_TIMEZONE=NOT_SET" in result["output"], (
            "CRYOZEN_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When CRYOZEN_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("CRYOZEN_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=cryozen_tz) which shifted
        absolute time when system-local tz != Cryozen tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Cryozen tz
        ...
        # Use a Cryozen timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _cryozen_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["CRYOZEN_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_cryozen_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Cryozen timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["CRYOZEN_TIMEZONE"] = "US/Eastern"
        _reset_cryozen_time_cache()
