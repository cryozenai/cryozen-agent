"""pytest's basetemp must never sit inside the operator's platform-native Cryozen home.

Every per-test sandbox is ``<basetemp>/.../cryozen_test`` and ``get_default_cryozen_root()``
...
def test_basetemp_inside_the_native_home_is_relocated_outside_it(tmp_path, monkeypatch):
    ...
    native.mkdir()
    monkeypatch.setattr(cryozen_constants, "_get_platform_default_cryozen_home", lambda: native)
    ...
    assert relocated.parent.name == "cryozen-pytest" and relocated.parent != Path.home()
    ...
    monkeypatch.setenv("CRYOZEN_HOME", str(relocated / "t0" / "cryozen_test"))
    assert cryozen_constants.get_default_cryozen_root() == relocated / "t0" / "cryozen_test"
...
def test_fallback_root_escapes_a_repo_checked_out_inside_the_native_home(tmp_path, monkeypatch):
    # Default install: repo at ~/.cryozen-agent/cryozen-agent and TEMP under the home (Windows).
    native = tmp_path / "native-home"
    (native / "cryozen-agent").mkdir(parents=True)
    monkeypatch.setattr(cryozen_constants, "_get_platform_default_cryozen_home", lambda: native)
    monkeypatch.setattr(suite_conftest, "PROJECT_ROOT", native / "cryozen-agent")
