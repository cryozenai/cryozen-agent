...
def cryozen_home(tmp_path, monkeypatch):
    ...
    # /etc/cryozen on the dev/CI box can't influence the test.
    monkeypatch.setenv("CRYOZEN_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(cryozen_home, monkeypatch):
...
def test_env_expansion_in_user_config(cryozen_home, monkeypatch):
    from cryozen_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_cryozen_dotenv(cryozen_home=str(home))
