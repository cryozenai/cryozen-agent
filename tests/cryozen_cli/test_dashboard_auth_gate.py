...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 cryozen-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
    ...
    ("cryozen-agent-prod-abc.fly.dev", False, True),
])
def test_should_require_auth_truth_table(host, allow_public, expected):
    from cryozen_cli.web_server import should_require_auth
    assert should_require_auth(host, allow_public) is expected


def test_empty_provider_login_page_shows_supported_auth_paths():
    from cryozen_cli.dashboard_auth import clear_providers
    from cryozen_cli.dashboard_auth.login_page import render_login_html
    ...
    assert (
        'href="https://cryozenai.github.io/cryozen-agent/docs/'
...
def test_trusted_container_proxy_controls_https_detection():
    ...
    from cryozen_cli.dashboard_auth.cookies import detect_https
...
def test_desktop_ssh_backend_serves_session_token_requests_despite_public_url(monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_DASHBOARD_PUBLIC_URL", "https://dashboard.example.test:9443")
    ...
    try:
        ...
        with_token = client.get("/api/profiles", headers={"X-Cryozen-Session-Token": ssh_token})
...
def test_loopback_public_url_fail_closed_message_is_actionable(monkeypatch):
    ...
    assert "dashboard.oauth.self_hosted" in msg
...
def test_should_require_dashboard_auth_truth_table(
    ...
    if public_url is None:
        monkeypatch.delenv("CRYOZEN_DASHBOARD_PUBLIC_URL", raising=False)
        monkeypatch.setattr(
            web_server, "_dashboard_public_hosts", lambda: frozenset()
        )
    else:
        monkeypatch.setenv("CRYOZEN_DASHBOARD_PUBLIC_URL", public_url)
