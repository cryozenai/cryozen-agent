"""Regression tests for the OAuth dispatcher in cryozen_cli.web_server.
...
Cryozen must not mint subscription OAuth tokens from an unattended HTTP
...
import cryozen_cli.web_routers.oauth as _rt_oauth
import cryozen_cli.web_server_oauth as _web_server_oauth

client = TestClient(app)
HEADERS = {"X-Cryozen-Session-Token": _SESSION_TOKEN}
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "cryozen_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "cryozen_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "cryozen_cli.web_server_oauth._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_minimax_start_route_honors_poller_mock_on_owning_module(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "cryozen_cli.auth._minimax_poll_token",
        fail_poll_token,
    ), patch(
        "cryozen_cli.web_server_oauth._minimax_poller",
        fake_poller,
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from cryozen_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_cryozen_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "cryozen auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "cryozen auth" not in detail
    finally:
...
def test_anthropic_card_is_api_key_only_and_has_no_dashboard_login():
    """Anthropic takes an API key only; the dashboard offers no OAuth login for it."""
    from cryozen_cli import web_server as ws
    ...
    assert providers["anthropic"]["cli_command"] == "cryozen auth add anthropic --type api-key"
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `cryozen model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `cryozen model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Cryozen."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``cryozen auth remove
    xai-oauth``.
    """
    from cryozen_cli import auth as auth_mod
    from cryozen_cli import web_server as ws
    ...
    # Prior `cryozen auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to cryozen_cli.auth.get_auth_status (the
    ...
    with patch("cryozen_cli.auth.get_auth_status", return_value=fake_status):
