"""Tests for the `cryozen proxy` subcommand and its upstream adapters."""
...
from cryozen_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic CRYOZEN_HOME."""
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
def test_proxy_appends_done_when_upstream_omits_sentinel():
    """#90848: complete SSE without [DONE] gets one appended."""
    async def run():
        frames = [
