...
calls ``os._exit(0)`` ~5ms after ``CRYOZEN_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            CRYOZEN_PARENT_PID=str(parent.pid),
            CRYOZEN_PARENT_START_MARKER=drifted_marker,
            CRYOZEN_PARENT_NONCE="nonce-95693",
            CRYOZEN_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("CRYOZEN_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "CRYOZEN_BACKEND_READY"), "backend never announced READY"
