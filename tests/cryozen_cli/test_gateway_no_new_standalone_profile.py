...
One gateway per host serves every profile. ``cryozen -p X gateway install|start`` used to refuse only
...
``--force`` and pointed at ``cryozen gateway install`` (default) / ``cryozen gateway migrate --multiplex``;
...
def test_named_profile_install_and_start_refuse_without_force_when_no_multiplexer_runs(quiet_host):
    ...
    for profile in ("coder", "ops", "coder"):
        ...
        for verb in ("install", "start"):
            ...
            assert "cryozen gateway install" in out and "cryozen gateway migrate --multiplex" in out
            assert f"cryozen -p {profile} gateway install --force" in out
