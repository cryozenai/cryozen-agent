"""Dashboard Cryozen Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_cryozen_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from cryozen_cli.console_engine import ConsoleResult, CryozenConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(CryozenConsoleEngine, "execute", slow_execute)
    ...
    with console_client.websocket_connect(_url()) as conn:
        ...
        assert complete["prompt"] == "cryozen> "
...
def test_console_cancel_stops_forked_agent_request_before_reporting(console_client, monkeypatch, blocking_provider, stop):
    ...
    from cryozen_cli.web_routers import chat_ws
    ...
    skill_dir = get_cryozen_home() / "skills" / "console-cancel-probe"
