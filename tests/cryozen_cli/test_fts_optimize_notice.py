...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            cryozen_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(cryozen_constants, "get_cryozen_home", lambda: tmp_path)
    monkeypatch.setattr(cryozen_state, "SessionDB", FakeSessionDB)
    ...
    assert "cryozen sessions optimize-storage" in capsys.readouterr().out
    conn.close()
