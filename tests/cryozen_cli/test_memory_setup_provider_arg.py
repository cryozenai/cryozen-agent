"""Tests for `cryozen memory setup [provider]` routing.
...
``cryozen memory setup honcho``) without the interactive picker — which matters
because the per-provider ``cryozen <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "cryozen memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets CRYOZEN_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"CRYOZEN_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("cryozen_cli.tools_config_cua.shutil.which", side_effect=which_side_effect), \
             patch("cryozen_cli.tools_config_cua.subprocess.run", fake_run):
