...
next command, and every ``cryozen <sub> <cmd>`` it cites is a registered subcommand.
"""
...
from cryozen_cli import gateway as gateway_cli
from cryozen_cli import profiles
from cryozen_cli.update_lock import UpdateHolder, describe_holder
...
def test_systemctl_failure_is_explained_not_tracebacked(monkeypatch, capsys):
    exc = subprocess.CalledProcessError(1, ["systemctl", "--user", "start", "cryozen-gateway"])
    out = _run_gateway_command_raising(monkeypatch, capsys, exc)
    assert not out.lstrip().startswith("Command '")
    assert "cryozen gateway status --deep" in out
    assert "cryozen gateway install --force" in out
...
def test_missing_systemctl_points_at_foreground_run(monkeypatch, capsys):
    ...
    assert "cryozen gateway run" in out
...
def test_existing_gateway_guard_says_bots_are_online_and_names_status(monkeypatch, capsys):
    ...
    for cmd in ("cryozen gateway status", "cryozen gateway restart", "cryozen gateway stop"):
        assert cmd in out
...
def test_unsupported_platform_copy_says_what_is_unsupported(subcommand):
    ...
    assert "cryozen gateway run" in text or subcommand == "uninstall"
...
def test_invalid_profile_name_explains_rule_in_words_with_example():
    ...
    assert "cryozen profile create my-work" in msg
...
def test_existing_profile_error_offers_use_and_list(tmp_path, monkeypatch):
    ...
    assert "cryozen profile use work" in msg
    assert "cryozen profile list" in msg
...
def test_describe_holder_avoids_jargon_and_names_next_steps():
    ...
    assert "cryozen logs" in message
    assert "cryozen update" in message
