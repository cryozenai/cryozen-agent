"""Tests for cryozen_cli.agent_import — ``cryozen import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real Cryozen stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/cryozen_cli/test_profiles.py:
Path.home() and CRYOZEN_HOME are redirected to tmp_path so nothing touches
the real ~/.cryozen-agent.
"""
...
from cryozen_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and CRYOZEN_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, CRYOZEN_HOME -> tmp/.cryozen-agent."""
...
@pytest.fixture()
def cryozen_home(profile_env):
    return profile_env / ".cryozen-agent"
...
def run_import(agent, source, cryozen_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=cryozen_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, cryozen_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, cryozen_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, cryozen_home):
        before = snapshot_tree(cryozen_home)
        report = run_import("claude-code", claude_tree, cryozen_home, execute=False)
        assert snapshot_tree(cryozen_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, cryozen_home):
        preview = run_import("claude-code", claude_tree, cryozen_home, execute=False)
        applied = run_import("claude-code", claude_tree, cryozen_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, cryozen_home):
        return run_import("claude-code", claude_tree, cryozen_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, cryozen_home):
        config = yaml.safe_load((cryozen_home / "config.yaml").read_text(encoding="utf-8"))
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, cryozen_home):
        return run_import("codex", codex_tree, cryozen_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, cryozen_home):
        config = yaml.safe_load((cryozen_home / "config.yaml").read_text(encoding="utf-8"))
    ...
    def test_skill_copied(self, report, cryozen_home):
        assert (cryozen_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, cryozen_home):
        run_import("claude-code", claude_tree, cryozen_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in cryozen_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, cryozen_home):
        run_import("codex", codex_tree, cryozen_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in cryozen_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, cryozen_home):
        report = run_import("claude-code", claude_tree, cryozen_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, cryozen_home):
        run_import("claude-code", claude_tree, cryozen_home, execute=True)
        config = yaml.safe_load((cryozen_home / "config.yaml").read_text(encoding="utf-8"))
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, cryozen_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, cryozen_home, execute=True)
        ...
        assert "still importable" in (
            cryozen_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")


    def test_empty_tree_all_skipped(self, profile_env, cryozen_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, cryozen_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, cryozen_home):
        dest = cryozen_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, cryozen_home):
        run_import("claude-code", claude_tree, cryozen_home, execute=True)
        first = (cryozen_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")
        report = run_import("claude-code", claude_tree, cryozen_home, execute=True)
        assert (cryozen_home / "memories" / "MEMORY.md").read_text(encoding="utf-8") == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, cryozen_home):
        memory = cryozen_home / "memories" / "MEMORY.md"
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, cryozen_home):
        return cryozen_home / "config.yaml"
    ...
    def test_malformed_config_is_left_byte_identical(
            self, claude_tree, cryozen_home, config_path):
    ...
    def test_malformed_config_records_an_error_not_an_import(
        ...
        for kind in CONFIG_WRITING_KINDS:
            ...
            assert "cryozen config edit" in reason
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, cryozen_home,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_config_write_is_atomic_and_leaves_no_temp_files(
        ...
        leftovers = [p.name for p in cryozen_home.glob(".tmp*")]
        assert leftovers == []

    def test_symlinked_config_stays_a_symlink(
            self, claude_tree, cryozen_home, tmp_path, config_path):
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, cryozen_home, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from cryozen_cli import agent_import
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, cryozen_home, capsys):
        """End-to-end through import_agent_command with --dry-run."""
        import types
        from cryozen_cli.agent_import import import_agent_command
        ...
        assert not (cryozen_home / "memories" / "MEMORY.md").exists()
        assert not (cryozen_home / "skills" / "claude-code-imports").exists()
        config_text = (cryozen_home / "config.yaml").read_text(encoding="utf-8") \
            if (cryozen_home / "config.yaml").exists() else ""
...
class TestSyncManifest:
    ...
    def test_import_registers_source_and_unchanged_sync_is_noop(
            self, claude_tree, cryozen_home, capsys):
        from cryozen_cli.agent_import_sync import load_sync_manifest

        self._run_command("claude-code", claude_tree)
        entry = load_sync_manifest(cryozen_home)["agents"]["claude-code"]
    ...
    def test_sync_reimports_changed_source_but_never_clobbers_user_skill(
        ...
        user_skill = cryozen_home / "skills" / "claude-code-imports" / "hand-rolled"
        ...
        imports = cryozen_home / "skills" / "claude-code-imports"
        ...
        # An imported skill the user then EDITED locally is no longer Cryozen-owned: the next sync
    ...
    def test_sync_dry_run_previews_without_writing(self, claude_tree, cryozen_home, capsys):
        from cryozen_cli.agent_import_sync import load_sync_manifest

        self._run_command("claude-code", claude_tree)
        old_digest = load_sync_manifest(cryozen_home)["agents"]["claude-code"]["digest"]
        (claude_tree / "CLAUDE.md").write_text(
            CLAUDE_MD + "\n- Dry sync entry\n", encoding="utf-8")
        before = snapshot_tree(cryozen_home)
        capsys.readouterr()
        ...
        assert snapshot_tree(cryozen_home) == before
        assert load_sync_manifest(cryozen_home)["agents"]["claude-code"]["digest"] == old_digest
