"""Behavior tests for cryozen_cli.inventory.
...
from cryozen_cli.inventory import (
    ConfigContext,
...
def _openrouter_row(model: str = "openai/gpt-5.5") -> dict:
    return {
        "slug": "openrouter",
        "name": "OpenRouter",
        "models": [model],
...
def test_explicit_only_drops_anthropic_row_without_explicit_config():
    """No explicit config (API key, pool entry, model.provider) -> Anthropic stays hidden."""
    rows = [
...
def test_user_defined_rows_carry_alias_set_for_gui_current_match():
    ...
    rows = [
        ...
        _openrouter_row() | {"is_current": False},
    ]
    ...
    assert "aliases" not in by_slug["openrouter"]
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("cryozen_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    with patch("cryozen_cli.models.clear_provider_models_cache") as clear:
