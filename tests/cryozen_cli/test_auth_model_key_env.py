...
pointer — ``model.key_env`` in config.yaml → the env var in ``$CRYOZEN_HOME/.env``
(e.g. ``CRYOZEN_CUSTOM_LMSTUDIO_API_KEY``) — while keeping ``model.provider`` on
...
def cryozen_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CRYOZEN_CUSTOM_LMSTUDIO_API_KEY", raising=False)
    return home
...
def test_model_key_env_pointer_is_honored_for_registry_provider(cryozen_home):
    _write(
        cryozen_home,
        "model:\n  default: some-model\n  provider: lmstudio\n"
        "  base_url: http://127.0.0.1:1234/v1\n"
        "  key_env: CRYOZEN_CUSTOM_LMSTUDIO_API_KEY\n",
        "CRYOZEN_CUSTOM_LMSTUDIO_API_KEY=sk-lm-desktop-saved\n",
    )
    ...
    assert creds["source"] == "CRYOZEN_CUSTOM_LMSTUDIO_API_KEY"


def test_model_key_env_does_not_leak_across_providers(cryozen_home):
    ...
    _write(
        ...
        "model:\n  default: gpt-x\n  provider: openai\n"
        "  key_env: CRYOZEN_CUSTOM_LMSTUDIO_API_KEY\n",
        "CRYOZEN_CUSTOM_LMSTUDIO_API_KEY=sk-lm-should-not-leak\n",
    )
    from cryozen_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER, resolve_api_key_provider_credentials
