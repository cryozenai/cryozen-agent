...
Before the fix, ``cryozen whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``cryozen gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def _env_value(cryozen_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from cryozen_cli.main_platform_setup import cmd_whatsapp
    ...
    monkeypatch.setattr("cryozen_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``cryozen whatsapp`` with an existing paired session and
