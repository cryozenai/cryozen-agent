"""Tests for ``cryozen plugins validate`` (cryozen_cli/plugin_validate.py).
...
from cryozen_cli.plugin_validate_desktop import desktop_surface_hits, is_desktop_surface
...
def _portable_plugin(root: Path, servers: dict, declarations: dict) -> Path:
    ...
    (root / "plugin.json").write_text(json.dumps({
        ...
        "name": "example-plugin",
        "extensions": {"com.cryozen.cryozen": {"servers": declarations}},
    }), encoding="utf-8")
...
def test_requires_cryozen_spec_is_validated(tmp_path):
    manifest = dict(BASE_MANIFEST, requires_cryozen=">=0.21")
    d = _make_plugin(tmp_path, manifest=manifest)

    report = validate_plugin_dir(d)

    assert report.ok, report.failures
    assert ("requires_cryozen", True, "spec '>=0.21' parses") in report.checks
...
class TestRequiresCryozenSpec:
    """A typo'd ``requires_cryozen`` clause must fail admission, not silently gate nothing."""

    def test_typoed_clause_fails_admission(self, tmp_path):
        d = _make_plugin(
            tmp_path, manifest={**BASE_MANIFEST, "requires_cryozen": ">=0.21.1,<0.x"}
        )
        report = validate_plugin_dir(d)
        assert not report.ok
        assert any(
            "requires_cryozen" in f and "does not parse" in f for f in report.failures
        ), report.failures
...
class TestDesktopSurface:
    ...
    def test_sdk_only_plugin_passes(self, tmp_path):
        d = self._desktop_plugin(tmp_path, (
            "import { definePlugin } from '@cryozen/plugin-sdk'\n"
    ...
    def test_prototype_patch_and_chunk_import_fail(self, tmp_path):
        d = self._desktop_plugin(tmp_path, (
            ...
            "const sdk = await import('@cryozen/plugin-sdk')\n"
        ))
        report = validate_plugin_dir(d)
        failed = {name: detail for name, ok, detail in report.checks if not ok}
        assert "desktop surface" in failed
