...
import cryozen_cli.update_cmd_fleet as fleet
import cryozen_cli.update_host_obligation as host_obligation
import cryozen_cli.update_restart_recovery as recovery
...
def two_profiles(tmp_path, monkeypatch):
    """Two profile CRYOZEN_HOMEs behind ONE host state dir — the real multiplex topology."""
...
def no_live_fleet(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.update_receipt.collect_fleet_versions", lambda: [])
...
def test_host_gateway_restarts_once_when_two_profiles_run_the_catch_up(
    ...
    """``cryozen -p coder update`` then ``cryozen -p writer update`` stops the host gateway ONCE."""
    monkeypatch.setattr("cryozen_cli.gateway.find_gateway_pids", lambda **k: [4242])
    monkeypatch.setattr("cryozen_cli.gateway.supports_systemd_services", lambda: False)
    monkeypatch.setattr("cryozen_cli.gateway.is_macos", lambda: False)
    monkeypatch.setattr("cryozen_cli.gateway.is_windows", lambda: False)
    monkeypatch.setattr("cryozen_cli.gateway._wait_for_gateway_exit", lambda **k: True)
    monkeypatch.setattr(fleet, "_restart_macos_launchd_gateways", lambda *a, **k: None)
    kills: list = []
    monkeypatch.setattr("cryozen_cli.gateway.kill_gateway_processes", lambda **k: kills.append(k))
...
def test_leftover_per_profile_units_restart_their_one_host_process_once(monkeypatch, capsys):
    ...
    fleet._restart_systemd_gateway_units_best_effort(
        failed, _listing(["cryozen-gateway.service", "cryozen-gateway-coder.service", "cryozen-gateway-writer.service"]))

    assert restarted == ["cryozen-gateway"]
    assert failed == []
    out = capsys.readouterr().out
    assert "cryozen-gateway-coder" in out and "legacy per-profile unit" in out
...
def test_units_with_distinct_live_pids_are_each_restarted(monkeypatch):
    ...
    pids = {"cryozen-gateway": 1, "cryozen-gateway-coder": 2}
    ...
    fleet._restart_systemd_gateway_units_best_effort(
        [], _listing(["cryozen-gateway.service", "cryozen-gateway-coder.service"]))

    assert sorted(restarted) == ["cryozen-gateway", "cryozen-gateway-coder"]


def _host_record(tmp_path, monkeypatch, profiles: list[str]) -> None:
    lock_dir = tmp_path / "gateway-locks"
    lock_dir.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("CRYOZEN_GATEWAY_LOCK_DIR", str(lock_dir))
...
def test_recovery_keeps_separate_processes_separate(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_GATEWAY_LOCK_DIR", str(tmp_path / "empty-locks"))
    argvs: list[list[str]] = []
...
def test_unwritable_host_state_dir_still_arms_the_obligation(two_profiles, no_live_fleet, monkeypatch, tmp_path):
    ...
    The host record moved out of ``$CRYOZEN_HOME`` (writable by construction) into the host state
...
def test_restart_runs_once_per_host_on_a_non_git_install(two_profiles, monkeypatch, capsys):
    ...
    ``mark_host_restart_completed("")`` can never match, so every profile's ``cryozen update``
