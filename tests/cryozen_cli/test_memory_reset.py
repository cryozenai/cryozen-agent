"""Tests for the `cryozen memory reset` CLI command.
...
- Profile-scoped reset (uses CRYOZEN_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake CRYOZEN_HOME with memory files."""
    cryozen_home = tmp_path / ".cryozen-agent"
    memories = cryozen_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nCryozen repo is at ~/.cryozen-agent/cryozen-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `cryozen memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        cryozen_home = tmp_path / ".cryozen-agent"
        (cryozen_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
