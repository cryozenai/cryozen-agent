...
These guard the contract:
  ...
  * OpenRouter is NEVER merged — it keeps its curated semantics.
...
appear in ``/model`` without a Cryozen release.
"""
...
class TestMergeHelper:
    def test_deepseek_picker_ignores_models_dev_retired_ids(self):
        ...
        ), patch("cryozen_cli.models._PROVIDER_CATALOG_FETCHERS", {}), \
                patch("cryozen_cli.models._profile_live_catalog", return_value=None):
...
class TestProviderModelIdsPreferred:
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("cryozen_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from cryozen_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("cryozen_cli.main_provider_setup._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("cryozen_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("cryozen_cli.config.get_env_value", return_value=""),
