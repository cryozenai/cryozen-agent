...
Cryozen keeps its OWN copy of the Codex OAuth token (per profile + top-level),
separate from the Codex CLI's ``~/.codex/auth.json``. OAuth refresh_tokens are
single-use, so when the Codex CLI (or another Cryozen process) rotates the shared
...
import cryozen_cli.auth as auth
import cryozen_cli.auth_codex as auth_codex
from cryozen_cli.auth import AuthError, _refresh_codex_auth_tokens, resolve_codex_runtime_credentials
...
def test_self_heals_on_stale_refresh_token(monkeypatch):
    ...
    # the recovered token was persisted to the Cryozen auth store
...
def test_self_heals_missing_singleton_access_token_from_codex_cli(tmp_path, monkeypatch):
    """Exact cron failure path: Cryozen auth has refresh_token but missing access_token."""
    ...
    assert resolved["source"] == "cryozen-auth-store"
    stored = json.loads((cryozen_home / "auth.json").read_text())
...
def test_opt_out_never_adopts_codex_cli_login(tmp_path, monkeypatch):
    ...
    family the user did not hand to Cryozen. Both automatic recovery paths must leave it (and Cryozen' own
    auth.json) untouched and surface the real error instead."""
    cryozen_home = tmp_path / "cryozen"
    codex_home = tmp_path / "codex"
    cryozen_home.mkdir()
    codex_home.mkdir()
    (cryozen_home / "config.yaml").write_text("auth:\n  adopt_external_logins: false\n")
    cryozen_auth = {"version": 1, "providers": {"openai-codex": {
        "tokens": {"refresh_token": "stale-refresh"}, "auth_mode": "chatgpt"}}}
    (cryozen_home / "auth.json").write_text(json.dumps(cryozen_auth))
    (codex_home / "auth.json").write_text(json.dumps({
    ...
    assert json.loads((cryozen_home / "auth.json").read_text()) == cryozen_auth
...
def _seed_homes(tmp_path, monkeypatch, cryozen_tokens, cli_tokens):
    cryozen_home, codex_home = tmp_path / "cryozen", tmp_path / "codex"
    cryozen_home.mkdir()
    codex_home.mkdir()
    (cryozen_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {"openai-codex": {
        "tokens": cryozen_tokens, "auth_mode": "chatgpt"}}}))
    (codex_home / "auth.json").write_text(json.dumps({"tokens": cli_tokens}))
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
    monkeypatch.setenv("CODEX_HOME", str(codex_home))
    return cryozen_home / "auth.json"
...
def test_recovery_refuses_codex_cli_login_from_another_workspace(tmp_path, monkeypatch, caplog):
    ...
    Cryozen credential it is supposed to repair — the store stays byte-identical and the log says why."""
    ...
    with caplog.at_level("WARNING", logger="cryozen_cli.auth"), pytest.raises(AuthError) as info:
