...
from cryozen_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    CRYOZEN_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def cryozen_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("cryozen_cli.config") or mod == "cryozen_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, cryozen_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, cryozen_home, capsys, monkeypatch):
    ...
    def test_failed_snapshot_is_loud_and_update_continues(self, cryozen_home, capsys, monkeypatch):
