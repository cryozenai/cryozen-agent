"""Regression tests for #34107 — Docker UID/GID handling in ensure_cryozen_home.

When Cryozen runs in Docker with ``CRYOZEN_UID=1000`` / ``CRYOZEN_GID=911``,
the entrypoint chowns the top-level ``CRYOZEN_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_cryozen_home()`` — especially
...
The fix is a ``_chown_to_cryozen_uid`` helper (``cryozen_constants``, the single home of the
managed/container/CRYOZEN_UID policy) that reads the env vars and applies chown after
...
# ---------------------------------------------------------------------------
# _resolve_cryozen_uid_gid
# ---------------------------------------------------------------------------


class TestResolveCryozenUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from cryozen_constants import _resolve_cryozen_uid_gid
...
class TestChownToCryozenUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_cryozen_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("CRYOZEN_UID", raising=False)
        monkeypatch.delenv("CRYOZEN_GID", raising=False)
        from cryozen_cli import config as cfg
