...
from cryozen_cli import model_setup_flows
...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setitem(
        cli.CLI_CONFIG,
        "model",
        {"default": "tencent/hy3:free", "provider": "openrouter"},
    )
    config = {
    ...
    monkeypatch.setattr("cryozen_cli.config.load_config", lambda: config)
    monkeypatch.setattr("cryozen_cli.runtime_provider.load_config", lambda: config)

    shell = cli.CryozenCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "cryozen_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_cryozen_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in CryozenCLI.__init__")

    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("cryozen_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.CryozenCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("cryozen_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_ensure_runtime_credentials_passes_cli_model_as_target_model(monkeypatch):
    """`cryozen -m mimo-v2.5 --provider opencode-go` must resolve credentials for the model the
    ...
    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    shell = cli.CryozenCLI(model="mimo-v2.5", provider="opencode-go", compact=True, max_turns=1)
...
@pytest.mark.parametrize(("exc_factory", "expected", "absent"), [
    ...
    (lambda: __import__("cryozen_cli.auth", fromlist=["AuthError"]).AuthError(
...
def test_fallback_runtime_labels_quota_outage_and_bad_credentials_distinctly(monkeypatch, tmp_path, exc_factory, expected, absent):
    ...
    monkeypatch.setattr("cryozen_cli.fallback_config.resolve_entry_api_key", lambda entry: "k")
...
def test_ensure_runtime_credentials_records_quota_vs_bad_key(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", _raise_quota)
    ...
    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", _raise_missing)
...
def _seed_stale_custom_model(tmp_path, monkeypatch):
    ...
    config_home = tmp_path / "cryozen"
    config_home.mkdir()
    monkeypatch.setenv("CRYOZEN_HOME", str(config_home))
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex"],
    )

    shell = cli.CryozenCLI(compact=True, max_turns=1)
...
def test_startup_fallback_re_resolves_reasoning_for_the_fallback_model(monkeypatch, reasoning_flag, expected_effort):
    ...
    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    shell = cli.CryozenCLI(compact=True, max_turns=1, reasoning=reasoning_flag)
...
def test_custom_entry_model_swap_re_resolves_reasoning(monkeypatch):
    """`cryozen chat --model <custom-provider-name>`: the runtime's explicit `model` replaces the
    ...
    shell = cli.CryozenCLI(model="my-lan", compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "cryozen_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("cryozen_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("cryozen_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("cryozen_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("cryozen_cli.main_provider_setup._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("cryozen_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    cryozen_main._model_flow_custom({})
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("cryozen_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "cryozen_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("cryozen_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("cryozen_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "cryozen_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "cryozen_cli.main_provider_setup._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("cryozen_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    cryozen_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_auto_provider_name_localhost():
    from cryozen_cli.main_provider_setup import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from cryozen_cli.main_provider_setup import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "cryozen_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("cryozen_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("cryozen_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="CRYOZEN_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "CRYOZEN_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from cryozen_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
