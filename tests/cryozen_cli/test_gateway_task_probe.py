...
from cryozen_cli.gateway import _windows_scheduled_task_state
...
def test_task_query_without_powershell_module_autoload():
    ...
    task_name = "Cryozen_test_'" + uuid.uuid4().hex
    with patch("cryozen_cli.gateway.subprocess.run", side_effect=run_without_modules):
