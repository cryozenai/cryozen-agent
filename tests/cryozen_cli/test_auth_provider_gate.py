...
def test_vertex_adc_counts_as_explicit_when_config_present(tmp_path, monkeypatch):
    ...
    Cryozen at it (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH),
    ...
    # vertex.project_id in config.yaml is a deliberate, Cryozen-scoped signal.
    ...
    # No Cryozen-scoped Vertex config at all → stays hidden.
...
def test_vertex_ambient_google_creds_env_does_not_count_as_explicit(tmp_path, monkeypatch):
    ...
    unrelated GCP work) must NOT mark Vertex explicit — only Cryozen-scoped
...
def test_profile_dotenv_key_counts_as_explicit_when_process_env_lacks_it(tmp_path, monkeypatch):
    ...
    from cryozen_cli.auth import is_provider_explicitly_configured
    from cryozen_cli.web_server_profiles import _config_profile_scope
...
def test_dotenv_key_counts_when_shell_exports_the_var_empty(tmp_path, monkeypatch):
    ...
    (tmp_path / "cryozen" / ".env").write_text("DEEPSEEK_API_KEY=sk-dotenv-only-secret\n")

    from cryozen_cli.auth import is_provider_explicitly_configured, resolve_api_key_provider_credentials
...
def test_bedrock_not_explicit_without_aws_env(tmp_path, monkeypatch, _clean_aws_env):
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "cryozen"))
    (tmp_path / "cryozen").mkdir(parents=True, exist_ok=True)
...
def test_bedrock_ambient_aws_profile_is_not_explicit(tmp_path, monkeypatch, _clean_aws_env):
    ...
    not an explicit Cryozen provider choice — same principle as gh_cli-seeded
