"""The `cryozen profile purge-identity` retry path: dispatched, and honest about failure.

`cryozen profile delete` reports a pending identity settlement when it cannot purge and names this
...
from cryozen_cli import profile_cmd, profiles
...
def test_purge_identity_reports_success(profile_env, monkeypatch, capsys):
    monkeypatch.setattr("cryozen_cli.profile_identity.purge_profile_identity", lambda name: True)
...
def test_purge_identity_exits_nonzero_when_settlement_stays_pending(
    ...
    monkeypatch.setattr("cryozen_cli.profile_identity.purge_profile_identity", lambda name: False)
...
def test_purge_identity_refuses_a_same_name_profile_created_after_the_delete(profile_env, capsys):
    ...
    scope = str(profile_env / ".cryozen-agent" / "sessions")
    db = SessionDB(profile_env / ".cryozen-agent" / "state.db")
    ...
    check = SessionDB(profile_env / ".cryozen-agent" / "state.db")
    try:
