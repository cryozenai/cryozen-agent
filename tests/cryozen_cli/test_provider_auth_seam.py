"""OAuth-shaped model-provider plugins are first-class in `cryozen auth` (#116408).

The seam: ``ProviderProfile.auth_handler``. These tests drive the real ``cryozen auth`` argparse
surface against a plugin discovered from an isolated CRYOZEN_HOME, so they fail if the dispatch is
...
def _rediscover() -> None:
    """Point the next profile lookup at the (new) CRYOZEN_HOME user plugin dir.
    ...
    for mod in [m for m in sys.modules if m.startswith("_cryozen_user_provider")]:
        del sys.modules[mod]


@pytest.fixture
def install_provider(tmp_path, monkeypatch):
    """Write a model-provider plugin into an isolated CRYOZEN_HOME and discover it."""
    installed: list[str] = []

    def _install(name: str = "fake-auth", *, with_handler: bool = True) -> Path:
        """Write (or rewrite) the fixture plugin and re-run discovery."""
        plugin_dir = tmp_path / "cryozen" / "plugins" / "model-providers" / name
...
def _parse_auth_args(argv: list[str]) -> argparse.Namespace:
    """Parse `cryozen auth <argv>` through the real subcommand parser."""
...
def test_oauth_plugin_owns_every_auth_action(tmp_path, install_provider, capsys, argv, expected):
    """An oauth_external plugin registers and `cryozen auth <action>` reaches its handler, args included,
    before any built-in path (nothing printed, nothing written to the pool)."""
    install_provider()

    import cryozen_cli.auth as auth_mod
    from cryozen_cli.auth_commands import auth_command
    ...
    assert not (tmp_path / "cryozen" / "auth.json").exists()
...
def test_oauth_plugin_without_handler_fails_loud_and_builtins_are_untouched(tmp_path, install_provider):
    ...
    pool = json.loads((tmp_path / "cryozen" / "auth.json").read_text(encoding="utf-8"))["credential_pool"]
