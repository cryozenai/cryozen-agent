...
the SAME provider universe as ``cryozen model`` (the CLI/TUI picker).
...
def test_every_cryozen_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `cryozen model` universe."""
    ...
    assert not missing, (
        "providers shown in `cryozen model` but not configurable in the desktop "
