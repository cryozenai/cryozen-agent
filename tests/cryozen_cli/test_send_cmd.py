"""Tests for the ``cryozen send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``cryozen_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from cryozen_cli import send_cmd
...
def whatsapp_bridge(monkeypatch):
    """Route ``cryozen send --to whatsapp:...`` through the real plugin standalone sender into a fake
    ...
    monkeypatch.setattr(send_cmd, "_load_cryozen_env", lambda: None)
...
def test_load_cryozen_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``cryozen send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``cryozen config set``) lives
    ...
    cryozen_home.mkdir()
    (cryozen_home / ".env").write_text("SOME_TOKEN=abc123\n", encoding="utf-8")
    ...
    # Force get_cryozen_home() to re-resolve under the patched env.
    from importlib import reload

    import cryozen_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_cryozen_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_cryozen_env_utf8_bom_preserves_first_key(tmp_path, monkeypatch):
    ...
    `cryozen send` failed to authenticate.
    """
    import os
...
def test_load_cryozen_env_bomless_utf8_still_loads(tmp_path, monkeypatch):
    """BOM-less UTF-8 .env files must keep loading after the utf-8-sig switch."""
    import os

    cryozen_home = tmp_path / ".cryozen-agent"
    cryozen_home.mkdir()
    (cryozen_home / ".env").write_bytes(b"SEND_PLAIN_TOKEN=plain-val\n")
...
def test_load_cryozen_env_latin1_fallback_still_loads(tmp_path, monkeypatch):
...
def test_load_cryozen_env_latin1_fallback_overrides_shell(tmp_path, monkeypatch):
    ...
    (cryozen_home / ".env").write_bytes(b"SEND_OVR_LABEL=caf\xe9-file\n")
...
def test_load_cryozen_env_fallback_read_error_is_swallowed(tmp_path, monkeypatch):
    ...
    (cryozen_home / ".env").write_bytes(b"SEND_ERR_TOKEN=caf\xe9\n")
...
def test_load_cryozen_env_bom_only_env_is_noop(tmp_path, monkeypatch):
    """A .env containing only a BOM must load zero vars without error."""
    import os

    cryozen_home = tmp_path / ".cryozen-agent"
    cryozen_home.mkdir()
    (cryozen_home / ".env").write_bytes(b"\xef\xbb\xbf")
...
def test_help_and_empty_list_hint_name_the_resolved_home(tmp_path, monkeypatch, capsys):
    ...
    hardcoded ``~/.cryozen-agent`` (absent on a Windows install or under a profile home)."""
    import argparse
    import sys
    import types

    home = tmp_path / "AppData" / "Local" / "cryozen"
    ...
    assert "~/.cryozen-agent" not in help_text
    ...
    assert "~/.cryozen-agent" not in out


def test_empty_list_hint_names_default_root_directory_under_profile_home(tmp_path, monkeypatch, capsys):
    """Under ``CRYOZEN_HOME=<root>/profiles/<p>`` the ``--list`` empty state says the default root already holds
