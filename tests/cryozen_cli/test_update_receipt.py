"""Tests for cryozen_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
from cryozen_cli import update_cmd, update_cmd_maint


@pytest.fixture()
def receipt_home(tmp_path, monkeypatch):
    """Isolated CRYOZEN_HOME for receipt writes."""
    home = tmp_path / ".cryozen-agent"
    home.mkdir()
    # ``_receipt_dir`` resolves through ``cryozen_constants.get_cryozen_home`` (env var), not
    # ``cryozen_cli.config`` — patch where production reads.
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["cryozen-gateway"],
        ...
        assert gr["restarted_services"] == ["cryozen-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another cryozen.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "cryozen.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(update_cmd, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            cryozen_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            cryozen_main,
        ...
        monkeypatch.setattr(
            cryozen_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import cryozen_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "cryozen_cli.profiles._get_default_cryozen_home", lambda: home
        )
        monkeypatch.setattr(
            "cryozen_cli.profiles._get_profiles_root",
    ...
    def test_identity_pending_row_gets_restart_aware_copy(self, capsys):
        ...
        assert ok is False
        out = capsys.readouterr().out
        assert "code identity not published yet" in out and "cryozen gateway status" in out
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
