...
from cryozen_cli.config import get_env_value, load_config, save_config
from cryozen_cli import setup as setup_mod
from cryozen_cli.setup import setup_model_provider
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("cryozen_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("cryozen_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "cryozen_cli.main._model_flow_named_custom",
...
def test_modal_setup_prompts_for_direct_modal_credentials(tmp_path, monkeypatch):
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
    ...
    monkeypatch.setattr("cryozen_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setitem(sys.modules, "swe_rex", object())

    from cryozen_cli.setup import setup_terminal_backend

    setup_terminal_backend(config)

    assert config["terminal"]["backend"] == "modal"
    assert "modal_mode" not in config["terminal"]
    assert get_env_value("MODAL_TOKEN_ID") == "token-id"
...
def test_vercel_setup_prefills_project_and_team_from_link_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("cryozen_cli.setup.prompt", fake_prompt)
