...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /cryozen-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``CRYOZEN_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``CRYOZEN_HOME`` value, which
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        async def start_gateway(**kwargs):
            assert "_CRYOZEN_GATEWAY_BREAKAWAY" not in os.environ
            return True
        ...
        diag_path = pathlib.Path(os.environ["CRYOZEN_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            "CRYOZEN_HOME": str(tmp_path),
            "CRYOZEN_GATEWAY_DETACHED": "1",
            "CRYOZEN_GATEWAY_EXIT_DIAG": "1",
            "CRYOZEN_GATEWAY_MAX_STARTS": "0",
    ...
    from cryozen_cli._subprocess_compat import windows_detach_flags_without_breakaway
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["CRYOZEN_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "CRYOZEN_HOME": str(tmp_path),
        "CRYOZEN_GATEWAY_EXIT_DIAG": "0",
        "CRYOZEN_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("CRYOZEN_S6_SUPERVISED_CHILD", raising=False)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("cryozen_constants.is_container", lambda: True)
    monkeypatch.setattr("cryozen_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("cryozen_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_spawn_detached_gateway_timestamps_stderr(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "get_cryozen_home", lambda: tmp_path)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "cryozen-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test CRYOZEN_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="CRYOZEN_HOME=/home/alice/.cryozen-agent"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "remove_legacy_cryozen_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "cryozen-gateway.service"
...
class TestWaitForGatewayExit:
    ...
    def test_kill_gateway_processes_force_uses_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway, "_capture_gateway_argv", lambda pid: ["python", "-m", "cryozen_cli.main", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
    ...
    def test_kill_gateway_processes_force_refuses_recycled_pid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: None if pid == 11 else ["python", "-m", "cryozen_cli.main", "gateway", "run"],
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
...
class TestRestartWaitsForApiServerPort:
    """Regression for #91547: ``cryozen gateway restart`` waited only for the old PID; on macOS the
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    gateway every time Cryozen Desktop opens (``cryozen serve`` calls
...
class TestReapUnsupervisedGatewayOrphansWindows:
    ...
    Scheduled-Task-supervised gateway every time Cryozen Desktop opens
    (``cryozen serve`` calls ``_reap_unsupervised_gateway_orphans`` during
...
class TestReaperCandidateIsSupervisorOwned:
    ...
    def test_windows_scheduled_task_gateway_spared_without_pidfile(self, monkeypatch):
        ...
        gateway_pid = 52615
        bootstrap_pid = 52616   # Task-launched `cryozen gateway run` bootstrap
        ...
        bootstrap = SimpleNamespace(
            pid=bootstrap_pid, parent=lambda: services, name=lambda: "cryozen-gateway.exe"
        )
        gw = SimpleNamespace(
            pid=gateway_pid, parent=lambda: bootstrap, name=lambda: "cryozen-gateway.exe"
        )
        ...
        orphan = SimpleNamespace(pid=orphan_pid, parent=lambda: None, name=lambda: "cryozen-gateway.exe")
    ...
    def test_windows_backstop_fails_open_when_bootstrap_exited(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        stranded = SimpleNamespace(pid=4242, parent=lambda: None, name=lambda: "cryozen-gateway.exe")
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "cryozen_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        """Cryozen_Gateway_* is Running => reaper returns False, kills nothing."""
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        ...
        # hardcoded "CryozenGateway" would leave the guard dormant on every
        # standard install (task name is Cryozen_Gateway / Cryozen_Gateway_<p>).
        import cryozen_cli.gateway_windows as gateway_windows

        monkeypatch.setattr(
            gateway_windows, "get_task_name", lambda: "Cryozen_Gateway_testprof"
        )
        queried = []
        ...
        assert result is False
        assert killed_pids == []
        assert queried == ["Cryozen_Gateway_testprof"]
    ...
    def test_windows_scheduled_task_running_returns_false_off_windows(
        ...
        assert gateway._windows_scheduled_task_supervises("CryozenGateway") is False
        assert gateway._windows_scheduled_task_state("CryozenGateway") is None
    ...
    def test_supervises_ready_and_queued_but_not_disabled(self, monkeypatch):
        ...
        for state, expected in states.items():
            ...
            assert gateway._windows_scheduled_task_supervises("Cryozen_Gateway") is expected, state
...
def test_find_windows_gateway_services_maps_verified_pid_tree(monkeypatch):
    ...
    fake_psutil = SimpleNamespace(
        win_service_iter=lambda: [
            FakeService("CryozenGateway", 100),
...
def test_find_windows_gateway_services_rejects_transitional_ancestor(monkeypatch):
    ...
    class FakeService:
        def as_dict(self):
            return {"name": "CryozenGateway", "pid": 100, "status": "stop_pending"}
...
def test_find_windows_gateway_services_ignores_task_scheduler_ancestor(monkeypatch):
    ...
    Cryozen-owned service (by binary path) stays SCM-supervised."""
    import psutil
    import cryozen_cli.gateway_windows as gateway_windows

    monkeypatch.setattr(gateway_windows, "cryozen_service_roots", lambda: (r"C:\cryozen\cryozen-agent",))
    ...
    owned = run(FakeService("gw", r'"C:\cryozen\cryozen-agent\venv\Scripts\cryozen.exe" gateway run'))
    ...
    # Cryozen's, and never a reason to abort; a Cryozen-NAMED service is settled without asking binpath.
    ...
    named = run(UnreadableConfigService("CryozenGateway", OSError(15100, "MUI file missing")))
    assert [(s.name, s.service_pid, s.gateway_pid) for s in named] == [("CryozenGateway", 2360, 18480)]
...
def test_find_windows_gateway_services_rejects_shared_service_host_pid(monkeypatch):
    ...
    fake_psutil = SimpleNamespace(
        win_service_iter=lambda: [
            FakeService("CryozenGatewayA"),
            FakeService("CryozenGatewayB"),
        ],
        Process=FakeProcess,
    )
...
def test_find_windows_gateway_services_skips_unrelated_service_with_unreadable_config(monkeypatch):
    ...
    loader, WinError 0 from OpenServiceW behind endpoint-security agents) is not Cryozen's and must be
    skipped, not turned into "SCM service enumeration failed" that aborts `cryozen update` (#116173)."""
    monkeypatch.setattr(gateway.sys, "platform", "win32")
    import cryozen_cli.gateway_windows as gateway_windows
