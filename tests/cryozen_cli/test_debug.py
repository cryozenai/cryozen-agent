"""Tests for ``cryozen debug`` CLI command and debug utilities."""
...
@pytest.fixture
def cryozen_home(tmp_path, monkeypatch):
    """Set up an isolated CRYOZEN_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO cryozen_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from cryozen_cli.debug import _upload_paste_rs
        ...
        with patch("cryozen_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "cryozen_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from cryozen_cli.debug import upload_to_pastebin

        with patch("cryozen_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("cryozen_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, cryozen_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = cryozen_home / "logs" / "agent.log"
        from cryozen_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, cryozen_home):
        """When truncation lands on a line boundary, keep the first full line."""
        from cryozen_cli.debug import _capture_log_snapshot
        ...
        (cryozen_home / "logs" / "agent.log").write_text(line * num_lines)
...
class TestMissingLogNote:
    ...
    `cryozen debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, cryozen_home):
        from cryozen_cli.debug import _capture_log_snapshot

        (cryozen_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, cryozen_home):
        from cryozen_cli.debug import _capture_log_snapshot

        (cryozen_home / "logs" / "desktop.log").unlink()

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.full_text is None
        assert "not on this host" in snap.tail_text
        assert "Cryozen Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(cryozen_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, cryozen_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, cryozen_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from cryozen_cli.debug import _capture_log_snapshot

        (cryozen_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, cryozen_home):
        """The uploaded report — what people paste into support — must explain it."""
        from cryozen_cli.debug import collect_debug_report
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of CRYOZEN_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def cryozen_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated CRYOZEN_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".cryozen-agent"
        home.mkdir()
        monkeypatch.setenv("CRYOZEN_HOME", str(home))
        ...
        monkeypatch.delenv("CRYOZEN_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, cryozen_home_with_secret):
        from cryozen_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, cryozen_home_with_secret):
        from cryozen_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, cryozen_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when CRYOZEN_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("CRYOZEN_REDACT_SECRETS", "false")

        from cryozen_cli.debug import _capture_log_snapshot

        assert os.environ.get("CRYOZEN_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, cryozen_home_with_secret
    ):
        from cryozen_cli.debug import _capture_log_snapshot

        log_path = cryozen_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, cryozen_home_with_secret):
        from cryozen_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from cryozen_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, cryozen_home):
        from cryozen_cli.debug import collect_debug_report

        with patch("cryozen_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- cryozen dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- cryozen dump ---" in report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, cryozen_home, capsys):
        ...
        with patch("cryozen_cli.dump.run_dump"), \
             patch("cryozen_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("cryozen_cli.debug.upload_to_pastebin",
    ...
    def test_share_uploads_five_pastes(self, cryozen_home, capsys):
        ...
        with patch("cryozen_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- cryozen dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, cryozen_home_with_secret, capsys
    ):
        ...
        with patch("cryozen_cli.dump.run_dump"), \
             patch("cryozen_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("cryozen_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        from cryozen_cli.debug import run_debug
        ...
        out = capsys.readouterr().out
        assert "cryozen debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, cryozen_home):
        from cryozen_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from cryozen_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from cryozen_cli.debug import delete_paste
        ...
        with patch("cryozen_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.cryozen-agent/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``cryozen debug``
    invocation.
    """


    def test_records_pending_to_json(self, cryozen_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from cryozen_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, cryozen_home):
        """Same URL recorded twice → one entry with the later expire_at."""
        from cryozen_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, cryozen_home):
        from cryozen_cli.debug import (
        ...
        with patch("cryozen_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, cryozen_home):
        from cryozen_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("cryozen_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, cryozen_home):
        ...
        with patch(
            "cryozen_cli.debug.delete_paste",
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, cryozen_home):
        from cryozen_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("cryozen_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from cryozen_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("cryozen_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, cryozen_home, capsys):
        from cryozen_cli.debug import run_debug_share
        ...
        with patch("cryozen_cli.dump.run_dump"), \
             patch("cryozen_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("cryozen_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
    ...
    def test_share_output_warns_on_dpaste_fallback(self, cryozen_home, capsys):
        """With dpaste.com URLs the output must not promise 6-hour auto-delete
        or a working `cryozen debug delete` (#106164)."""
        from cryozen_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``cryozen debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, cryozen_home):
        from cryozen_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (cryozen_home / "logs" / "agent.log").write_text(
        ...
        with patch("cryozen_cli.dump.run_dump"), patch(
            "cryozen_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("cryozen_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, cryozen_home):
        from cryozen_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    ...
    def test_no_redact_omits_banner(self, cryozen_home):
        from cryozen_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, cryozen_home):
        from cryozen_cli.debug import collect_share_bundle
...
class TestDebugSlashCommand:
    ...
    ``CryozenCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("cryozen_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`cryozen debug share` requires explicit consent before uploading.
    ...
    def test_non_interactive_requires_yes(self, cryozen_home, capsys, monkeypatch):
        ...
        with patch("cryozen_cli.dump.run_dump"), \
             patch("cryozen_cli.debug.upload_to_pastebin") as mock_upload:
    ...
    def test_local_never_prompts(self, cryozen_home, capsys, monkeypatch):
