...
Covers:
 - Normal path (tokens already in Cryozen auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Cryozen
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `cryozen auth openai-codex`. The old
...
@pytest.fixture()
def cryozen_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Cryozen auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(cryozen_auth_only_env):
    """openai-codex appears when tokens are already in Cryozen auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Cryozen
    auth store."""
    cryozen_home = tmp_path / ".cryozen-agent"
    cryozen_home.mkdir()
    ...
    (cryozen_home / "auth.json").write_text(
    ...
    for var in [
        ...
        "ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN", "DEEPSEEK_API_KEY",
    ]:
        monkeypatch.delenv(var, raising=False)
...
def test_no_codex_when_no_credentials(tmp_path, monkeypatch):
    ...
    for var in [
        "OPENROUTER_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "DEEPSEEK_API_KEY", "COPILOT_GITHUB_TOKEN",
