...
Regression guard for the bug where `cryozen model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`cryozen_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in CRYOZEN_OVERLAYS
        overlay = CRYOZEN_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `cryozen_cli/main.py` (cmd_model): read
