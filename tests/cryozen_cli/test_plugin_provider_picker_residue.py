...
Residue after picker admission: `cryozen model` had no flow for a plugin profile (selection was a silent
...
def plugin(monkeypatch):
    ...
    import providers
    from cryozen_cli import auth
    from cryozen_cli.auth_plugin_providers import PLUGIN_MIRRORED_PROVIDERS, register_plugin_provider
    ...
    monkeypatch.setattr("cryozen_cli.auth_plugin_providers.PLUGIN_MIRRORED_PROVIDERS", mirrored)
...
def test_cryozen_model_routes_registered_plugin_profiles_to_the_generic_flow(plugin, monkeypatch, tmp_path):
    """Selecting an admitted external-process or OAuth plugin in `cryozen model` persists config.model;
...
def test_oauth_plugin_status_follows_the_credential_pool(plugin):
    ...
    `cryozen auth add` hint. Bundled OAuth providers never reach this builder."""
    from cryozen_cli import auth, models
    ...
    assert "cryozen auth add example-oauth" in signed_out["hint"]
    ...
    for builtin in ("xai-oauth", "openai-codex"):
...
def test_any_external_process_plugin_counts_as_signed_in_when_its_binary_resolves(plugin, monkeypatch, tmp_path):
    ...
    from cryozen_cli import auth
    from cryozen_cli.inventory import _external_process_signed_in, _filter_explicit_provider_rows
