...
def _client():
    ...
    # Keep the state DB under the isolated CRYOZEN_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_cryozen_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        config_text = (cryozen_home / "config.yaml").read_text()
        env_text = (cryozen_home / ".env").read_text()
    ...
    def test_http_oauth_mode_is_persisted_for_existing_auth_flow(self):
        ...
        from cryozen_cli.mcp_config import _get_mcp_servers
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.cryozen-agent/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `cryozen auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `cryozen auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_cryozen_home):
        ...
        (get_cryozen_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_cryozen_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
        (get_cryozen_home() / "profiles" / "work" / "config.yaml").write_text("{}\n")  # identity marker
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_cryozen_action", fake_spawn_action)
    ...
    def test_enable_platform_reuses_inflight_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_cryozen_action", fail_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "cryozen_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("cryozen-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # cryozen-index source advertises availability + featured search.
            idx = _Src("cryozen-index")
        ...
        assert {"official", "github", "cryozen-index"} <= ids
...
class TestOfficialSkillsCatalogEndpoint:
    ...
    def test_lists_full_catalog_with_installed_flags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "cryozen_cli.web_routers.skills._installed_hub_identifiers",
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "cryozen_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/cryozen/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/cryozen/update`` runs ``cryozen update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import cryozen_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/cryozen/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/cryozen/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        import cryozen_cli.debug as dbg
        ...
        monkeypatch.setattr("cryozen_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `cryozen tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_cryozen_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_cryozen_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _CRYOZEN_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_cryozen_action stores
    ...
    _web_server_gateway._spawn_cryozen_action(["gateway", "restart"], "gateway-restart")

    assert "_CRYOZEN_GATEWAY" not in captured["env"]
    assert captured["env"]["CRYOZEN_NONINTERACTIVE"] == "1"
...
def test_named_profile_action_isolates_parent_env_and_loads_target_env(monkeypatch, tmp_path):
    ...
    import cryozen_cli.env_loader as env_loader
    import cryozen_cli.web_server as ws

    user_home = tmp_path / "user"
    default_home = user_home / ".cryozen-agent"
    ...
    for key, value in {
        ...
        "CRYOZEN_ACP_AUTH_METHOD": "default-acp",
    ...
    monkeypatch.setattr(ws.subprocess, "Popen", _fake_popen)
    _web_server_gateway._spawn_cryozen_action(["-p", "verifier", "gateway", "restart"], "gateway-restart")

    child_env = captured["env"]
    assert captured["cmd"][-4:] == ["-p", "verifier", "gateway", "restart"]
    assert child_env["CRYOZEN_HOME"] == str(target_home)
    assert child_env["CRYOZEN_NONINTERACTIVE"] == "1"
    ...
    for leaked_key in (
        ...
        "A2A_AUTH_MINI", "EXTERNAL_PROFILE_AUTH", "CRYOZEN_ACP_AUTH_METHOD",
    ):
    ...
    probe = subprocess.run(
        [
            ...
            "load_cryozen_dotenv(cryozen_home=os.environ['CRYOZEN_HOME']); "
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_cryozen_home
):
    ...
    startup under CRYOZEN_DESKTOP=1 so the stale orphan is cleared first.
    """
    import cryozen_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from cryozen_cli.gateway, so patch it
    # on that module.
    import cryozen_cli.gateway as g
