"""Tests for cryozen_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `cryozen setup` and `cryozen import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("cryozen_constants.is_container", lambda: container)
