...
managers acquire the same path through ``cryozen_state_registry`` from the REPL thread a
...
import cryozen_cli.goals as goals
...
def test_cli_session_store_is_the_registry_handle_goals_reuse(monkeypatch):
    ...
    constructed = []
    real_open = cryozen_state_registry._open_session_db

    def recording_open(path):
        constructed.append(path)
        return real_open(path)

    monkeypatch.setattr(cryozen_state_registry, "_open_session_db", recording_open)

    cli = SimpleNamespace()
    try:
        CryozenCLI._init_session_store(cli)
        ...
        cryozen_state_registry.close_all()
