...
``cryozen update`` on macOS printed ``Update complete!`` and exited 0 while the
``ai.cryozen.gateway`` LaunchAgent sat deregistered for 36 minutes.  The restart
...
from cryozen_cli.update_cmd import _warn_incomplete_gateway_fleet_restart
...
class TestInvokingProfileIsVerifiedLikeItsSiblings:
    ...
    def test_restart_that_leaves_the_old_pid_supervised_is_not_a_restart(
        ...
        update printed ``✓ Restarted ai.cryozen.gateway`` over pre-update code.
...
class TestIncompleteFleetWarningIsPlatformCorrect:
    ...
    def test_linux_recovery_instructions_are_unchanged(self, monkeypatch, capsys):
        ...
        _warn_incomplete_gateway_fleet_restart(["cryozen-gateway.service"])
