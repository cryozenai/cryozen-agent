...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (cryozen_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "cryozen"))
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})

    from cryozen_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "cryozen" / "auth.json").read_text())
...
def test_auth_add_api_key_without_terminal_exits_with_guidance_and_stores_nothing(tmp_path, monkeypatch):
    """No --api-key, no TTY, nothing piped: the prompt hits EOF. The command must exit with a
    message saying how to supply the key (not a traceback) and must not persist an entry."""
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "cryozen"))
    ...
    monkeypatch.setattr(auth_commands, "masked_secret_prompt", _eof)
    ...
    assert "pipe the key on stdin" in str(excinfo.value)
    payload = json.loads((tmp_path / "cryozen" / "auth.json").read_text())
    assert not [e for e in payload.get("credential_pool", {}).get("anthropic", []) if e.get("source") == "manual"]


def test_auth_add_api_key_rejects_value_with_internal_whitespace(tmp_path, monkeypatch):
    """A pasted value with spaces/newlines inside (wrong clipboard contents) is never a key:
    refuse it instead of storing a credential that later fails as a 401."""
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "cryozen"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    ...
    class _Args:
        provider = "anthropic"
        auth_type = "api-key"
        api_key = "cd ~/project && run something else"
        label = None

    with pytest.raises(SystemExit) as excinfo:
        auth_add_command(_Args())

    assert "not an API key" in str(excinfo.value)
    payload = json.loads((tmp_path / "cryozen" / "auth.json").read_text())
    assert not payload.get("credential_pool", {}).get("anthropic")
...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((cryozen_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("cryozen_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from cryozen_cli.auth_commands import auth_list_command
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``cryozen auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``cryozen auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("cryozen_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_auth_add_codex_warns_when_login_is_same_account_as_pooled_entry(tmp_path, monkeypatch, capsys):
    """A second ``cryozen auth add openai-codex`` for the SAME OpenAI account must tell the user
    ...
    assert '"me@example.com"' in err and "cryozen auth remove openai-codex 1" in err
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from cryozen_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from cryozen_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """cryozen auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``cryozen auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "cryozen" / "auth.json").read_text(encoding="utf-8"))
...
def test_clear_provider_auth_removes_provider_pool_entries(tmp_path, monkeypatch):
    ...
    from cryozen_cli.auth import clear_provider_auth
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`cryozen logout --provider openai-codex` must still clear model.provider.
    ...
    from cryozen_cli.auth import logout_command
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from cryozen_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Cryozen reads from has a
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "cryozen_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "cryozen_cli.copilot_auth.get_copilot_api_token",
...
def test_qwen_oauth_login_marks_active_through_moved_owner(monkeypatch):
    ...
    import cryozen_cli.auth_commands as auth_commands
    import cryozen_cli.auth_qwen as auth_qwen
...
def test_auth_add_openrouter_oauth_persists_pkce_key_without_touching_api_key_default(tmp_path, monkeypatch):
    """`cryozen auth add openrouter --type oauth` stores the PKCE-minted key as an ``api_key`` pool row
    ...
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr("cryozen_cli.auth._openrouter_pkce_login", lambda **kw: {"api_key": "sk-or-v1-from-pkce"})
...
def test_openrouter_loopback_callback_binds_nonce_path_and_rejects_forged_redirect(monkeypatch):
    ...
    import cryozen_cli.auth_openrouter as orm
