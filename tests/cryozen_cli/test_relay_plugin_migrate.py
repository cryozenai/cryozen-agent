"""``cryozen update`` / ``cryozen migrate relay``: legacy ``CRYOZEN_NEMO_RELAY_ATIF_*``/``ATOF_*`` vars
...
from cryozen_cli.relay_plugin_migrate import (
...
CRYOZEN_NEMO_RELAY_ATOF_ENABLED=1
CRYOZEN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY={home}/telemetry/atof
CRYOZEN_NEMO_RELAY_ATOF_FILENAME=cryozen-atof.jsonl
CRYOZEN_NEMO_RELAY_ATOF_MODE=append
CRYOZEN_NEMO_RELAY_ATIF_ENABLED=1
CRYOZEN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY={home}/telemetry/atif
CRYOZEN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE=trajectory-{{session_id}}.json
CRYOZEN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
"""
...
def test_legacy_env_becomes_validated_toml_selected_from_env(profile_env):
    ...
    assert sink["type"] == "file" and sink["filename"] == "cryozen-atof.jsonl"
    assert document["components"][0]["config"]["atif"]["filename_template"] == "trajectory-{session_id}.json"
    # Relay itself accepts the file Cryozen will load at runtime.
    ...
    assert "# migrated to relay-plugins.toml: CRYOZEN_NEMO_RELAY_ATOF_ENABLED=1" in (profile_env / ".env").read_text(encoding="utf-8")
