...
``get_cryozen_home()`` printed ``[CRYOZEN_HOME fallback] ... wrong profile`` on every ``cryozen``
command for users of ``cryozen profile use`` (#112319, also reported in #112839).
"""
...
def test_help_text_names_the_profile_config_once_the_process_is_re_homed(sticky_profile_home, monkeypatch):
    from cryozen_cli._parser import _cfg_path

    monkeypatch.setenv("CRYOZEN_HOME", str(sticky_profile_home / "profiles" / "coder"))

    assert _cfg_path() == "~/.cryozen-agent/profiles/coder/config.yaml"
