"""Tests for cryozen backup and import commands."""
...
def _no_real_gateway_service(monkeypatch):
    ...
    assert the wiring."""
    import cryozen_cli.gateway as gateway_mod
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew cryozen_cli.backup's datetime forward instead of sleeping.
    ...
    import cryozen_cli.backup as _backup

    shim = getattr(_backup.datetime, "_cryozen_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _cryozen_test_shim = True
...
def _make_cryozen_tree(root: Path) -> None:
    """Create a realistic ~/.cryozen-agent directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "cryozen_state.db"):
    ...
    # cryozen-agent repo (should be EXCLUDED)
    (root / "cryozen-agent").mkdir(exist_ok=True)
    (root / "cryozen-agent" / "run_agent.py").write_text("# big file\n")
    (root / "cryozen-agent" / ".git").mkdir()
    (root / "cryozen-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_cryozen_agent(self):
        from cryozen_cli.backup import _should_exclude
        assert _should_exclude(Path("cryozen-agent/run_agent.py"))
        assert _should_exclude(Path("cryozen-agent/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from cryozen_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
    ...
    def test_excludes_desktop_emergency_state_db_baks(self):
        ...
        state.db.pre-update-emergency-*.bak files at the CRYOZEN_HOME root —
...
class TestIterBackupFiles:
    def test_manual_and_automatic_paths_share_one_walk(self, tmp_path):
        ...
        ``cryozen-agent`` at ANY depth, silently dropping nested skill dirs
        like ``skills/autonomous-ai-agents/cryozen-agent/`` that the manual
        path preserved. One shared iterator makes that drift impossible;
        this test pins the contract."""
        from cryozen_cli.backup import _iter_backup_files

        root = tmp_path / ".cryozen-agent"
        root.mkdir()
        _make_cryozen_tree(root)

        # The case the old automatic walk got wrong: a nested dir named
        # cryozen-agent holding real skill content.
        nested = root / "skills" / "autonomous-ai-agents" / "cryozen-agent"
        ...
        rel_nested = str(Path("skills/autonomous-ai-agents/cryozen-agent/SKILL.md"))
        assert rel_nested in selected
        assert str(Path("models/big.gguf")) not in selected
        assert not any(s.startswith("cryozen-agent") for s in selected)

    def test_prunes_browser_use_cli_profiles_at_home_roots_only(self, tmp_path):
        """The Browser Use CLI backend writes ``CRYOZEN_HOME/browser_profiles/`` (underscore) — a
    ...
    def test_skipped_dirs_collected_for_summary(self, tmp_path):
        ...
        assert "cryozen-agent" in skipped
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        cryozen_home.mkdir()
        _make_cryozen_tree(cryozen_home)
        ...
        import cryozen_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = cryozen_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, cryozen_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside CRYOZEN_HOME."""
        cryozen_home = tmp_path / ".cryozen-agent"
        cryozen_home.mkdir()
        _make_cryozen_tree(cryozen_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(cryozen_home / "skills" / "outside-link.txt", outside)
        ...
        from cryozen_cli.backup import run_backup
        run_backup(args)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        cryozen_home.mkdir()
        _make_cryozen_tree(cryozen_home)
        with sqlite3.connect(cryozen_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from cryozen_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(cryozen_home=cryozen_home)
        assert snap_id and (cryozen_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Cryozen backup."""
        from cryozen_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_auto_installs_gateway_service(self, tmp_path, monkeypatch):
        ...
        from cryozen_cli.backup import run_import
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Cryozen configuration has been restored." in out
        assert "cryozen gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        cryozen_home = tmp_path / ".cryozen-agent"
        (cryozen_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (cryozen_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (cryozen_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            cryozen_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (cryozen_home / "gateway.pid").write_text("4242")
        (cryozen_home / "processes.json").write_text('{"live": true}')
        ...
        assert (cryozen_home / "gateway.pid").read_text() == "4242"
        assert (cryozen_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (cryozen_home / "cron.pid").exists()
        assert not (cryozen_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        self._make_backup_zip(zip_path, {
            ...
            "auth.json": '{"providers": {"openai-codex": "token"}}',
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (cryozen_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".cryozen-agent"
        src_home.mkdir(parents=True)
        _make_cryozen_tree(src_home)

        monkeypatch.setenv("CRYOZEN_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from cryozen_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".cryozen-agent"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("CRYOZEN_HOME", str(dst_home))
        ...
        # cryozen-agent should NOT be present
        assert not (dst_home / "cryozen-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from cryozen_cli.sizefmt import format_bytes as _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from cryozen_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".cryozen-agent/", "")
            zf.writestr(".cryozen-agent/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_incomplete_archive_is_kept_but_reported_as_failure(self, tmp_path, monkeypatch, capsys):
        ...
        cryozen_home.mkdir()
        _make_cryozen_tree(cryozen_home)
        unreadable = cryozen_home / "skills" / "locked.md"
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        from cryozen_cli.backup import _RUN_BACKUP_PREFIX, run_backup
        from cryozen_cli.main import cmd_backup
    ...
    def test_empty_cryozen_home(self, tmp_path, monkeypatch):
        """Backup handles empty cryozen home (no files to back up)."""
        cryozen_home = tmp_path / ".cryozen-agent"
        cryozen_home.mkdir()
        # Only excluded dirs, no actual files
        (cryozen_home / "__pycache__").mkdir()
        (cryozen_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        cryozen_home = tmp_path / ".cryozen-agent"
        cryozen_home.mkdir()
        (cryozen_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = cryozen_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        cryozen_home = tmp_path / ".cryozen-agent"
        cryozen_home.mkdir()
        (cryozen_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (cryozen_home / "config.yaml").exists()
        assert (cryozen_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`cryozen import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the CRYOZEN_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (cryozen_home / "config.yaml").write_text(original)
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (cryozen_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(cryozen_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside CRYOZEN_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        cryozen_home = dst_home / ".cryozen-agent"
        cryozen_home.mkdir()
    ...
    def test_symlinked_target_keeps_its_symlink(self, tmp_path, monkeypatch):
        ...
        cryozen_home = tmp_path / ".cryozen-agent"
        cryozen_home.mkdir()
        store = cryozen_home / "store"
        store.mkdir()
        real = store / "config.yaml"
        real.write_text("model: original\n")
        link = cryozen_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        cryozen_home = tmp_path / ".cryozen-agent"
        cryozen_home.mkdir()
        target = cryozen_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo cryozen import`` onto a user-owned (or Docker/NAS volume-owned)
        CRYOZEN_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "cryozen_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``cryozen import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from cryozen_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from cryozen_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from cryozen_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def cryozen_home(self, tmp_path):
        """Create a fake CRYOZEN_HOME with critical state files."""
    ...
    def test_state_db_safely_copied(self, cryozen_home):
        from cryozen_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(cryozen_home=cryozen_home)
        db_copy = cryozen_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, cryozen_home, monkeypatch, capsys):
        ...
        snap_id = backup_mod.create_quick_snapshot(cryozen_home=cryozen_home)
        err = capsys.readouterr().out
        assert "SQLite safe copy FAILED" in err or "CRITICAL" in err
        assert "state.db" in err
        # Other small files may still snapshot
        if snap_id:
            manifest = (cryozen_home / "state-snapshots" / snap_id / "manifest.json")
            assert manifest.exists()
            data = json.loads(manifest.read_text(encoding="utf-8"))
            assert "state.db" not in data.get("files", {})
            assert "state.db" in data.get("failed_dbs", [])

    def test_restore_refused_db_is_not_counted(self, cryozen_home, monkeypatch):
        """A refused live-safe restore (holder detected, backup leg failed) must
        not be counted as a restored file — `cryozen import` reports it, and
        /snapshot restore must not claim success for that file either."""
        import cryozen_cli.backup as backup_mod
        from cryozen_cli.backup import create_quick_snapshot, restore_quick_snapshot
        ...
        restore_quick_snapshot(snap_id, cryozen_home=cryozen_home)

        manifest = json.loads(
            (backup_mod._quick_snapshot_root(cryozen_home) / snap_id / "manifest.json").read_text()
        )
    ...
    def test_restore_state_db_live_connection(self, cryozen_home):
        ...
        live_conn = sqlite3.connect(str(cryozen_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, cryozen_home=cryozen_home)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, cryozen_home):
        ...
        (cryozen_home / "platforms" / "pairing").mkdir(parents=True)
        (cryozen_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (cryozen_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (cryozen_home / "pairing").mkdir()
        (cryozen_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (cryozen_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = cryozen_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (cryozen update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, cryozen_home, capsys):
        ...
        """
        import json
        from cryozen_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", cryozen_home=cryozen_home)
        assert first_id is not None
        first_dir = cryozen_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", cryozen_home=cryozen_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = cryozen_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, cryozen_home=cryozen_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, cryozen_home):
        ...
        board_dir = cryozen_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(cryozen_home=cryozen_home)
        copy = (
            cryozen_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, cryozen_home=cryozen_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, cryozen_home, monkeypatch):
        ...
        import cryozen_cli.backup as bk
        from cryozen_cli.backup import create_quick_snapshot

        board = cryozen_home / "kanban" / "boards" / "work"
        ...
        copy = cryozen_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``cryozen update``
    ...
    def test_backup_contents_match_full_backup(self, cryozen_home):
        """Pre-update backup should include the same user data that
        ``cryozen backup`` would, and should exclude the same directories."""
        from cryozen_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(cryozen_home=cryozen_home)
        ...
        # cryozen-agent repo excluded
        assert not any(n.startswith("cryozen-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, cryozen_home):
        """``cryozen update`` in ``full`` mode takes the quick snapshot *before*
        ...
        from cryozen_cli.backup import (
        ...
        snap_id = create_quick_snapshot(label="pre-update", cryozen_home=cryozen_home)
        assert snap_id and (cryozen_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, cryozen_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(cryozen_home=cryozen_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (cryozen_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, cryozen_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside CRYOZEN_HOME."""
        from cryozen_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def cryozen_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".cryozen-agent"
        root.mkdir()
        _make_cryozen_tree(root)
        # Point CRYOZEN_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("CRYOZEN_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve CRYOZEN_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(cryozen_home, value):
        import yaml
        (cryozen_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(cryozen_home):
        d = cryozen_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(cryozen_home):
        d = cryozen_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, cryozen_home, capsys):
        ...
        self._set_mode(cryozen_home, "off")
        from cryozen_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(cryozen_home)
        assert not self._zips(cryozen_home)



    def test_config_full_mode(self, cryozen_home, capsys):
        self._set_mode(cryozen_home, "full")
        from cryozen_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(cryozen_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (cryozen claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``cryozen claw migrate`` runs before mutating ~/.cryozen-agent/."""
    ...
    def test_restorable_with_cryozen_import(self, cryozen_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Cryozen
        backup — `cryozen import` should accept it."""
        from cryozen_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(cryozen_home=cryozen_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, cryozen_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from cryozen_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(cryozen_home=cryozen_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(cryozen_home=cryozen_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`cryozen update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, cryozen_home: Path, label="pre-update"):
        from cryozen_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, cryozen_home=cryozen_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from cryozen_cli.backup import restore_cron_jobs_if_emptied
        cryozen_home = tmp_path / ".cryozen-agent"
        jobs_path = cryozen_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(cryozen_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, cryozen_home=cryozen_home)
        assert result is not None
...
class TestRestoreConfigModelSettingsIfRewritten:
    ...
    def _seed(self, cryozen_home: Path) -> Path:
        cryozen_home.mkdir(parents=True, exist_ok=True)
        cfg = cryozen_home / "config.yaml"
        cfg.write_text(self.USER_CONFIG, encoding="utf-8")
        return cfg

    def test_restores_rewritten_provider_and_dropped_moa(self, tmp_path):
        import yaml
        from cryozen_cli.backup import restore_config_model_settings_if_rewritten

        cryozen_home = tmp_path / ".cryozen-agent"
        cfg = self._seed(cryozen_home)
        snap_id = self._make_snapshot(cryozen_home)
        assert snap_id
        ...
        result = restore_config_model_settings_if_rewritten(
            snap_id, cryozen_home=cryozen_home
        )
        assert result is not None
        assert result["restored"] is True
        assert result["snapshot_id"] == snap_id
    ...
    def test_noop_when_user_never_set_protected_keys(self, tmp_path):
        ...
        cryozen_home = tmp_path / ".cryozen-agent"
        cryozen_home.mkdir(parents=True)
        cfg = cryozen_home / "config.yaml"
...
# location, NOT under CRYOZEN_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, cryozen_home: Path) -> None:
        cryozen_home.mkdir(parents=True, exist_ok=True)
        (cryozen_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (cryozen_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (cryozen_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        cryozen_home = tmp_path / ".cryozen-agent"
        self._make_min_tree(cryozen_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under CRYOZEN_HOME,
        ...
        # External state did NOT leak into CRYOZEN_HOME.
        assert not (cryozen_home / "_external").exists()


# ---------------------------------------------------------------------------
# run_import: CRYOZEN_HOME override handling (issue #99839)
# ---------------------------------------------------------------------------


class TestImportHonorsCryozenHomeOverride:
    """`cryozen import` must restore into the home the command runs under.

    Resolving the target through get_default_cryozen_root() maps a profile
    ...
    def test_import_targets_named_profile_home(self, tmp_path, monkeypatch):
        """CRYOZEN_HOME=<root>/profiles/<name> must restore INTO the profile,
        not into <root> (which would clobber the live root config)."""
        root = tmp_path / "cryozen-root"
        profile = root / "profiles" / "coder"
        profile.mkdir(parents=True)
        # Live root config that must survive untouched.
        (root / "config.yaml").write_text("model:\n  provider: openai\n")

        monkeypatch.setenv("CRYOZEN_HOME", str(profile))
        from cryozen_constants import get_cryozen_home

        assert get_cryozen_home() == profile
    ...
    def test_import_skips_gateway_install_for_non_default_home(
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(sandbox))
        ...
        monkeypatch.setattr(
            backup_mod,
            "_get_platform_default_cryozen_home",
            lambda: native_default,
        )

        calls = []
        monkeypatch.setattr(
            "cryozen_cli.gateway.ensure_gateway_service",
            lambda *a, **kw: calls.append(kw),
        )
        monkeypatch.setattr(
            "cryozen_cli.gateway._is_service_running",
            lambda: False,
        )
    ...
    def test_import_installs_gateway_when_default_home_is_target(
        ...
        native_default = tmp_path / "native-default"
        native_default.mkdir()
        monkeypatch.setenv("CRYOZEN_HOME", str(native_default))
...
def _write_session_db(path: Path, sessions: int, messages_per_session: int) -> None:
    """Create a minimal Cryozen-shaped session database at *path*."""
...
class TestImportLiveSessionDatabase:
    """`cryozen import` must not swap the inode of a database Cryozen holds open.
...
def test_run_backup_prunes_older_default_named_zips_but_not_others(tmp_path, monkeypatch):
    """Hourly `cryozen backup` callers accumulated 150+ zips; --keep bounds the default-named
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    for i in range(4):
        (tmp_path / f"cryozen-backup-2026-01-0{i + 1}-000000.zip").write_bytes(b"old")
    (tmp_path / "my-archive.zip").write_bytes(b"mine")

    backup_mod.run_backup(Namespace(output=None, keep=2))

    kept = sorted(p.name for p in tmp_path.glob("cryozen-backup-*.zip"))
    assert len(kept) == 2 and kept[0] == "cryozen-backup-2026-01-04-000000.zip"
