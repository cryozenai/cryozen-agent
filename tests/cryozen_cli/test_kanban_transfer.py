"""Tests for kanban board export / import (``cryozen_cli.kanban_transfer``).
...
from cryozen_cli import kanban_transfer as kt
from cryozen_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("CRYOZEN_KANBAN_HOME", str(root))
        for var in ("CRYOZEN_KANBAN_DB", "CRYOZEN_KANBAN_WORKSPACES_ROOT",
                    "CRYOZEN_KANBAN_ATTACHMENTS_ROOT", "CRYOZEN_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Cryozen"):
