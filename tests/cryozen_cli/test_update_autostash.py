...
from cryozen_cli import config as cryozen_config
from cryozen_cli import main as cryozen_main
...
def _patch_gateway_discovery():
    ...
    it ``ai.cryozen.gateway`` and the verify step exits 1 (#111866, #110701).
    """
    with patch("cryozen_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("cryozen_cli.gateway.supports_systemd_services", return_value=False), \
         patch("cryozen_cli.update_cmd_fleet._restart_macos_launchd_gateways", lambda *a, **k: None), \
         patch("cryozen_cli.gateway.find_profile_gateway_processes", return_value=[]), \
         patch("cryozen_cli.update_inventory.collect_runtime_inventory", return_value=None), \
         patch("cryozen_cli.update_inventory.report_unaccounted_runtimes", return_value=False), \
         patch.object(cryozen_main, "_fleet_probe_expected_runtimes", lambda *a, **kw: False), \
         patch("cryozen_cli.update_receipt.collect_fleet_versions", return_value=[]):
        yield
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(cryozen_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(cryozen_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(cryozen_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(cryozen_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(cryozen_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(cryozen_config, "check_config_version", lambda **_kwargs: (5, 5))
    monkeypatch.setattr(cryozen_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(cryozen_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(cryozen_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    cryozen_main._refresh_active_memory_provider_dependencies()
...
def _make_update_side_effect(
    ...
    ``refs/cryozen-update-backups/orphan-<branch>-*`` (oldest first) so the
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        cryozen_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        cryozen_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(cryozen_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        cryozen_main.cmd_update(SimpleNamespace())
...
def test_cmd_update_orphan_history_backs_up_before_reset(monkeypatch, tmp_path, capsys):
    ...
    ``refs/cryozen-update-backups/orphan-*`` ref before the reset proceeds."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    assert ref_name.startswith("refs/cryozen-update-backups/orphan-main-")
...
def test_cmd_update_orphan_rescue_refs_pruned_beyond_keep_limit(monkeypatch, tmp_path, capsys):
    ...
    stale_refs = [
        "refs/cryozen-update-backups/orphan-main-"
...
def test_prune_orphan_rescue_refs_leaves_unparseable_names_alone():
    ...
    weird = "refs/cryozen-update-backups/orphan-main-not-a-timestamp-xyz"
    calls = []
    ...
    with mock_patch.object(cryozen_main.subprocess, "run", side_effect=fake_run):
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        cryozen_main, "_discard_stashed_changes",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        cryozen_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "cryozen_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    cryozen_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    cryozen_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``cryozen update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(cryozen_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".cryozen-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by cryozen update (cryozen_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "cryozen-update-autostash")

    assert marker.exists(), (
        ".cryozen-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".cryozen-bootstrap-complete" not in status
...
def test_stash_selector_is_a_bare_index_never_a_brace_selector(tmp_path):
    ...
    import cryozen_cli.update_cmd_stash as stash_mod
...
def test_autostash_survives_intent_to_add_entries(tmp_path):
    ...
    Reported: `cryozen update` aborted with "Entry 'tests/...' not uptodate. Cannot merge." because
    ...
    assert cryozen_main._restore_stashed_changes(["git"], tmp_path, stash_ref, prompt_user=False)
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    import subprocess
    from cryozen_cli import update_cmd
    import cryozen_cli.update_cmd_deps as update_cmd_deps
    ...
    assert "made the Cryozen agent unexecutable" in output
...
def test_restore_allows_preexisting_import_time_failure(monkeypatch, tmp_path):
    ...
    assert cryozen_main._restore_stashed_changes(
        ["git"], tmp_path, stash_ref, prompt_user=False
    )
...
def test_restore_stays_parked_when_untracked_baseline_is_unknown(
    ...
    from cryozen_cli import update_cmd
    import cryozen_cli.update_cmd_stash as update_cmd_stash
...
def test_prune_orphan_rescue_refs_with_real_git_unpins_objects(tmp_path):
    ...
    old_ref = "refs/cryozen-update-backups/orphan-main-20200101-000000-" + snap_sha[:12]
    ...
    remaining = git("for-each-ref", "refs/cryozen-update-backups/").stdout
...
def test_conflicted_restore_records_parked_step_in_receipt(monkeypatch, tmp_path):
    import subprocess
    from cryozen_cli import update_receipt
    ...
    restored = cryozen_main._restore_stashed_changes(["git"], tmp_path, stash_ref, prompt_user=False)
