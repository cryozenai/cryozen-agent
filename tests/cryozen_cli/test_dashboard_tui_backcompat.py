"""Regression test: `cryozen dashboard --tui` must not hard-crash.

Older Cryozen desktop app shells (<= 0.15.x) spawn the backend as::

    cryozen dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Cryozen couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real cryozen_cli.main parser in a subprocess.
