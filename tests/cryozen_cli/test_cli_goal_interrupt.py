...
minimal ``CryozenCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def cryozen_home(tmp_path, monkeypatch):
    """Isolated CRYOZEN_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".cryozen-agent"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("CRYOZEN_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves CRYOZEN_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal CryozenCLI stub with an active goal wired in."""
    from cli import CryozenCLI
    from cryozen_cli.goals import GoalManager

    cli = CryozenCLI.__new__(CryozenCLI)
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, cryozen_home):
        ...
        with patch("cryozen_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, cryozen_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, cryozen_home):
        ...
        src = inspect.getsource(CryozenCLI.chat)
