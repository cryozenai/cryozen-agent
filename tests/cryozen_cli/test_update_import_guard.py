"""Tests for the post-update *import* guard in ``cryozen update``.
...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    # NOTE: patch update_cmd's global, not cryozen_main's. Both modules expose
...
def test_import_guard_does_not_accept_forged_static_marker(monkeypatch, tmp_path):
    ...
    (tmp_path / "consumer.py").write_text(
        "import os, sys\n"
        "sys.stdout.write('__CRYOZEN_IMPORT_HEALTH__[]')\n"
        "sys.stdout.flush()\n"
        "os._exit(7)\n"
    )
    monkeypatch.setattr(update_cmd, "_UPDATE_CRITICAL_MODULES", ("consumer",))
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("cryozen update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``cryozen update`` can run under a different Python than the install's.
...
def test_import_guard_rejects_module_satisfied_only_by_inherited_pythonpath(
    ...
    stale.mkdir()
    (stale / "cryozen_stale_supply.py").write_text("VALUE = 'from the stale tree'\n")

    monkeypatch.setattr(
        update_cmd, "_UPDATE_CRITICAL_MODULES", ("cryozen_stale_supply",)
    )
    monkeypatch.setattr(
        update_cmd_deps, "_UPDATE_CRITICAL_MODULES", ("cryozen_stale_supply",)
    )
    ...
    assert ok is False
    assert module == "cryozen_stale_supply"
    assert error is not None and "cryozen_stale_supply" in error
...
def test_import_guard_accepts_candidate_with_foreign_pythonpath(monkeypatch, tmp_path):
    ...
    stale = tmp_path / "stale"
    stale.mkdir()
    (stale / "cryozen_stale_supply.py").write_text("VALUE = 'stale'\n")
    (tmp_path / "cryozen_stale_supply.py").write_text("VALUE = 'candidate'\n")
...
@pytest.mark.parametrize("modname", ["agents", "agentops", "toolsets_x", "cryozenx"])
...
@pytest.mark.parametrize("modname", ["tools.todo_tool", "agent.context_compressor",
                                     "cryozen_constants", "cryozen_cli.config", "cli"])
...
def test_probe_and_hint_share_one_first_party_definition():
    ...
    `cli` was first-party to the hint but not the probe, and `cryozenx`
    (third-party) matched the probe's loose `startswith("cryozen")`. A user
    ...
    """
    from cryozen_constants import FIRST_PARTY_MODULE_ROOTS, is_first_party_module
    ...
    for lookalike in ("agents", "agentops", "toolsets_x", "cryozenx"):
