...
def cron_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    (home / ".cryozen-agent").mkdir(parents=True)
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("CRYOZEN_HOME", str(home / ".cryozen-agent"))
    for name in list(sys.modules):
        if name.split(".")[0] in ("cron", "gateway", "cryozen_cli", "cryozen_constants"):
            monkeypatch.delitem(sys.modules, name, raising=False)
    return home / ".cryozen-agent"


def _status_block(pids):
    from cryozen_cli.cron import _print_ticker_health
...
def test_update_signals_proven_stale_gateway_survivor(cron_home, tmp_path):
    ...
    from cryozen_cli.update_cmd_fleet import _GatewayRestartOutcome
...
def test_verify_fleet_hands_stale_rows_to_survivor_signalling(monkeypatch):
    ...
    import cryozen_cli.update_cmd_fleet as fleet_mod
    import cryozen_cli.update_cmd_stale_survivors as surv
