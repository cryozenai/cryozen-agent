...
CRYOZEN_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def cryozen_home(monkeypatch, tmp_path):
    """Fresh CRYOZEN_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(cryozen_home):
    _write_env(cryozen_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(cryozen_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = cryozen_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(cryozen_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(cryozen_home, OPENAI_API_KEY=old)
    _write_config(
        cryozen_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = cryozen_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
# load_pool() or a separate `cryozen auth add`.
# ---------------------------------------------------------------------------
...
def test_put_api_env_materializes_credential_pool_entry(cryozen_home):
    ...
    the old cached credential until a separate ``cryozen auth add opencode-go``
...
def test_put_api_env_writes_auth_json_for_provider(cryozen_home):
    """Sentinel for #96058: PUT /api/env must modify auth.json on disk.

    The reported symptom was ``stat -c '%y' ~/.cryozen-agent/auth.json`` returning
...
def test_update_rotates_keyed_providers_mirror(cryozen_home):
    old = "sk-kp-" + "n" * 24
    new = "sk-kp-" + "o" * 24
    _write_env(cryozen_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_delete_scrubs_keyed_providers_mirror(cryozen_home):
    old = "sk-kp-" + "p" * 24
    _write_env(cryozen_home, OPENAI_API_KEY=old)
    _write_config(
        cryozen_home,
...
def test_scrub_never_touches_providers_base_url_alias(cryozen_home):
