"""Tests for the AI_AGENT / CRYOZEN_AGENT harness-attribution env vars.
...
Cryozen-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Cryozen' id in the public agent-harness
registry (``cryozen-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Cryozen process env, and survives the
cross-session leak guard that strips ``CRYOZEN_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__CRYOZEN_CWD_testsession0__"
        env._snapshot_path = "/tmp/cryozen-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'CRYOZEN_AGENT="${CRYOZEN_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT CRYOZEN=$CRYOZEN_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "CRYOZEN_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} CRYOZEN=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", CRYOZEN_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi CRYOZEN=false" in out.stdout
