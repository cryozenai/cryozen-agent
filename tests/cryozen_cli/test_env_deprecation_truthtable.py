...
session restoration / cron / the gateway):
    ...
    the user's ~/.cryozen-agent/.env FILE.
    ...
    * There is NO quiet/non-interactive gating in the scanner itself; the
      gateway calls it before setting CRYOZEN_QUIET (gateway/run.py), so the
      warning fires regardless of CRYOZEN_QUIET.

Exercises the real scanner against a real temp CRYOZEN_HOME .env file.
"""
...
TRUTH_TABLE = [
    ...
    pytest.param(
        "TERMINAL_CWD=.\n", {"CRYOZEN_QUIET": "1"}, None, ("TERMINAL_CWD",),
        id="quiet-mode-no-gating-dotenv-warns",
    ),
    pytest.param(
        "API_KEY=x\n",
        {"CRYOZEN_QUIET": "1", "TERMINAL_CWD": "."},
        None,
        (),
...
def test_deprecated_env_warning_truth_table(
    ...
    if env_file is not None:
        (cryozen_home / ".env").write_bytes(env_file.encode("utf-8"))
    ...
    for key in ("TERMINAL_CWD", "MESSAGING_CWD", "CRYOZEN_QUIET"):
        monkeypatch.delenv(key, raising=False)
    for key, value in proc_env.items():
        monkeypatch.setenv(key, value)

    import cryozen_cli.config as config_module
