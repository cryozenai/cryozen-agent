...
from cryozen_cli.copilot_auth import _try_gh_cli_token
...
from cryozen_cli.auth import PROVIDER_REGISTRY as _REGISTRY
...
_EXTRA_ENV_VARS = (
    ...
    "CRYOZEN_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "CRYOZEN_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.auth._load_auth_store", lambda: {})
...
class TestResolveProvider:
    ...
    def test_alias_chatgpt_every_alias_table(self):
        ...
        parse_model_input) and ``cryozen auth login`` all resolve the ChatGPT alias, not just auth."""
        from cryozen_cli.providers import normalize_provider
        from cryozen_cli.models import parse_model_input
        from cryozen_cli.auth_commands import _normalize_provider
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch, tmp_path):
        from cryozen_cli.copilot_auth import _invalidate_gh_cli_token_cache
        from cryozen_platform.resolver import known_dirs
        ...
        monkeypatch.setattr("cryozen_cli.copilot_auth.subprocess.run", _fake_run)
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("cryozen_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        from cryozen_cli.runtime_provider import resolve_runtime_provider
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("cryozen_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "cryozen_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "cryozen_cli.models.fetch_github_model_catalog",
    ...
    def test_runtime_copilot_acp_uses_process_runtime(self, monkeypatch):
        monkeypatch.setattr("cryozen_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
        monkeypatch.setenv("CRYOZEN_COPILOT_ACP_ARGS", "--acp --stdio --debug")
...
class TestHasAnyProviderConfigured:
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        import yaml
        from cryozen_cli import config as config_module
        cryozen_home = tmp_path / ".cryozen-agent"
        cryozen_home.mkdir()
        config_file = cryozen_home / "config.yaml"
        config_file.write_text(yaml.dump({
            "model": {"default": "anthropic/claude-opus-4.6", "provider": "openrouter"},
        }))
        monkeypatch.setattr(config_module, "get_env_path", lambda: cryozen_home / ".env")
        monkeypatch.setattr(config_module, "get_cryozen_home", lambda: cryozen_home)
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
        # Clear all provider env vars
        for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY",
                     "ANTHROPIC_TOKEN", "OPENAI_BASE_URL"):
            monkeypatch.delenv(var, raising=False)
        from cryozen_cli.main import _has_any_provider_configured
    ...
    def test_config_provider_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        """
        import yaml
        cryozen_home = self._setup_home(monkeypatch, tmp_path)
        (cryozen_home / "config.yaml").write_text(yaml.dump({
            "model": {"default": "anthropic/claude-opus-4.6", "provider": "openrouter"},
        }))
        sweep_calls = []
        ...
        monkeypatch.setattr("cryozen_cli.auth.get_auth_status", _trap)
        from cryozen_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
        assert sweep_calls == [], (
    ...
    def test_auth_json_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("cryozen_cli.auth.get_auth_status", _guarded_status)
        from cryozen_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
...
class TestKimiCodeCredentialAutoDetect:
    ...
    def test_non_kimi_providers_unaffected(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
...
class TestZaiEndpointAutoDetect:
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
    ...
    def test_failed_probe_is_not_repeated_within_ttl(self, monkeypatch):
        ...
        from cryozen_cli import auth_zai_kimi
        monkeypatch.setenv("GLM_API_KEY", "glm-key-that-429s")
        monkeypatch.setattr(auth_zai_kimi, "_zai_probe_failed_until", {})
        calls = []
        monkeypatch.setattr("cryozen_cli.auth.detect_zai_endpoint", lambda *a, **kw: calls.append(1))
        for _ in range(3):
...
class TestZaiParallelProbe:
    ...
    def test_candidate_model_fallback_within_endpoint(self, monkeypatch):
        ...
        from cryozen_cli.auth import ZAI_ENDPOINTS, detect_zai_endpoint
    ...
    def test_priority_order_wins_over_completion_order(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_cli.auth.httpx.post", _slow_first)
        result = detect_zai_endpoint("test-key", timeout=1.0)
        assert result is not None
        assert result["id"] == first[0]

    def test_all_fail_returns_none(self, monkeypatch):
        from cryozen_cli.auth import detect_zai_endpoint

        monkeypatch.setattr("cryozen_cli.auth.httpx.post", self._mock_post({}))
    ...
    def test_early_exit_does_not_wait_for_slow_losers(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_cli.auth.httpx.post", _slow_losers)
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from cryozen_cli.models import _PROVIDER_MODELS as main_models
        from cryozen_cli.models import _PROVIDER_MODELS as models_models
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        from cryozen_cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        from cryozen_cli.models import _fetch_deepinfra_models_by_tag
        import cryozen_cli.models as _m
        from cryozen_cli import models_pricing
...
class TestDeepInfraProviderProfile:
    ...
    def test_profile_registered_with_alias_and_aux(self):
        ...
        from cryozen_cli.auth import PROVIDER_REGISTRY, resolve_provider
