...
def test_identical_hook_failure_warns_once_then_debug(manager, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="cryozen_cli.plugins"):
        for i in range(5):
...
def test_execution_chain_middleware_failure_warns_once(manager, caplog, monkeypatch):
    ...
    from cryozen_cli import middleware as mw

    monkeypatch.setattr("cryozen_cli.plugins._plugin_manager", manager)
