...
def main_mod(monkeypatch):
    import cryozen_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_cryozen_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from cryozen_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """cryozen -z bypasses CryozenCLI, but recall still needs SessionDB."""
    from cryozen_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "cryozen_state_registry", mod("cryozen_state_registry", acquire=fake_acquire))
    monkeypatch.setitem(
        sys.modules,
        "cryozen_cli.config",
        mod("cryozen_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "cryozen_cli.models",
        mod("cryozen_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        mod("cryozen_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["CRYOZEN_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="openrouter/cryozen-test", provider="openrouter", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["CRYOZEN_MODEL"] == "openrouter/cryozen-test"
    assert env["CRYOZEN_INFERENCE_MODEL"] == "openrouter/cryozen-test"
    assert env["CRYOZEN_TUI_PROVIDER"] == "openrouter"
    assert env["CRYOZEN_INFERENCE_PROVIDER"] == "openrouter"
    assert env["CRYOZEN_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["CRYOZEN_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("cryozen-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_cryozen_ink(monkeypatch, main_mod, tmp_path):
    tui_dir = tmp_path / "ui-tui"
    tsx = tui_dir / "node_modules" / ".bin" / "tsx"
    ink_dir = tui_dir / "packages" / "cryozen-ink"
    ...
    monkeypatch.delenv("CRYOZEN_TUI_DIR", raising=False)
