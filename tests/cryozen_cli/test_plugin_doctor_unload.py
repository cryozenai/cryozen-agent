...
Doctor loads a plugin under a temporary ``CRYOZEN_HOME`` through a throwaway
...
def test_doctor_runs_on_unload_during_teardown(tmp_path: Path) -> None:
    ...
    marker = tmp_path / "unloaded.marker"  # outside the temp CRYOZEN_HOME
...
def test_doctor_still_reports_and_cleans_up_for_unload_plugin(tmp_path: Path) -> None:
    ...
    # entries restored, no cryozen_plugins.* module leak.
    import sys
