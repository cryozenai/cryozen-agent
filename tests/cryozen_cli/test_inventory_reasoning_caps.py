...
the upstream honors levels a route doesn't advertise, so publishing it would
...
def test_advertised_efforts_never_reach_the_picker(monkeypatch):
    ...
    It under-reports what the upstream serves, so forwarding it would let the
...
def test_reasoning_mandatory_route_cannot_disable(monkeypatch):
    ...
    The upstream answers a disable on these routes with HTTP 400, so offering
...
def test_catalog_failure_never_breaks_the_picker(monkeypatch):
    ...
    monkeypatch.setattr(models_reasoning_caps, "openrouter_model_reasoning_capabilities", _boom)
    rows = [{"slug": "openrouter", "models": ["deepseek/deepseek-v4-pro"]}]
    inv._apply_capabilities(rows)
