...
cryozen_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # cryozen_time checks an env override first; ensure it's unset so config wins.
    ...
    assert cryozen_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (CRYOZEN_TIMEZONE, CRYOZEN_REDACT_SECRETS, CRYOZEN_MAX_ITERATIONS,
    ...
    # writing CRYOZEN_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
