...
`cryozen` launch re-read the stale base_url and routed the new model at the
old host.
...
canonical ``cryozen_cli.model_switch.persist_model_selection`` shape shared by every
surface.
"""
...
class _StubCLI:
    ...
    def _stage_and_swap_model(self, result, old_model):
        ...
        return _cli_mod.CryozenCLI._stage_and_swap_model(self, result, old_model)
    ...
    def _confirm_and_apply_cli_model_switch(
        ...
        return cli_mod.CryozenCLI._confirm_and_apply_cli_model_switch(
...
def _run_switch(monkeypatch, result, cmd="/model MiniMax-M3 --global"):
    ...
    monkeypatch.setattr("cryozen_cli.model_switch.switch_model", lambda **kw: result)
    monkeypatch.setattr(
        "cryozen_cli.inventory.load_picker_context",
        lambda: (_ for _ in ()).throw(RuntimeError("no picker context in test")),
    )
    cli_mod.CryozenCLI._handle_model_switch(_StubCLI(), cmd)
    return saved
...
def test_session_only_switch_does_not_touch_config(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.model_switch.switch_model", lambda **kw: _make_result())
    monkeypatch.setattr(
        "cryozen_cli.inventory.load_picker_context",
        lambda: (_ for _ in ()).throw(RuntimeError("no picker context in test")),
    )

    cli_mod.CryozenCLI._handle_model_switch(_StubCLI(), "/model MiniMax-M3 --session")
...
def _run_apply(monkeypatch, result, persist_global=True):
    ...
    monkeypatch.setattr("utils.atomic_roundtrip_yaml_update", _fake_save)
    cli_mod.CryozenCLI._apply_model_switch_result(_StubCLI(), result, persist_global)
    return saved
