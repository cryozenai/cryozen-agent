...
cryozen_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `cryozen config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_cryozen_home = get_cryozen_home()`` at import time (module
    singleton), so monkeypatching CRYOZEN_HOME after import doesn't move it.
    ...
    this only matters for tests that swap CRYOZEN_HOME.)
    """
    import cli

    cli._cryozen_home = home
