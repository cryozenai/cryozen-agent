...
from cryozen_cli.main_agent_cmds import cmd_insights
...
def _run_show_insights(command: str):
    ...
    with patch("cryozen_state.SessionDB", return_value=db), \
...
def test_insights_skips_open_when_store_missing(capsys):
    ...
    for run in (lambda: CryozenCLI.__new__(CryozenCLI)._show_insights("/insights 7"),
                lambda: cmd_insights(SimpleNamespace(days=30, source=None))):
        with patch("cryozen_state.SessionDB") as ctor:
            run()
...
def test_show_insights_opens_read_only():
    cli_obj = CryozenCLI.__new__(CryozenCLI)
    db = MagicMock()
    with patch("cryozen_state.SessionDB", return_value=db) as ctor, \
         patch("agent.insights.InsightsEngine", _InsightsEngineStub):
        _InsightsEngineStub.calls = []
