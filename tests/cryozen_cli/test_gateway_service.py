...
import cryozen_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`cryozen gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `cryozen gateway restart` in the background; when a
        ...
        assert "cryozen gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``CRYOZEN_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../cryozen_test`` CRYOZEN_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "cryozen-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir CRYOZEN_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="CRYOZEN_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/cryozen_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="CRYOZEN_HOME=/tmp/cryozen-e2e-41264"\n'
        assert (
            gateway_cli._temp_home_in_service_definition(unit)
            == "/tmp/cryozen-e2e-41264"
        )
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="CRYOZEN_HOME={tmp_path}/cryozen-home"\n'
...
class TestServiceIdentityForForeignHome:
    """A CRYOZEN_HOME that is neither ``~/.cryozen-agent`` nor ``~/.cryozen-agent/profiles/<name>`` must never resolve to
    the default profile's ``cryozen-gateway`` unit (a temp-home harness uninstalled the production gateway)."""
    ...
    def test_foreign_home_gets_its_own_unit(self, machine_home, tmp_path, monkeypatch):
        ...
        foreign.mkdir()
        monkeypatch.setenv("CRYOZEN_HOME", str(foreign))

        default_unit = machine_home / ".config" / "systemd" / "user" / "cryozen-gateway.service"
        assert gateway_cli.get_service_name() != "cryozen-gateway"
    ...
    def test_default_and_named_profile_homes_keep_their_names(self, machine_home, monkeypatch):
        default_home = machine_home / ".cryozen-agent"
        (default_home / "profiles" / "alpha").mkdir(parents=True)

        monkeypatch.setenv("CRYOZEN_HOME", str(default_home))
        assert gateway_cli.get_service_name() == "cryozen-gateway"

        monkeypatch.setenv("CRYOZEN_HOME", str(default_home / "profiles" / "alpha"))
        assert gateway_cli.get_service_name() == "cryozen-gateway-alpha"

    def test_sudo_user_default_home_keeps_bare_service_name(self, machine_home, tmp_path, monkeypatch):
        sudo_home = tmp_path / "alice"
        sudo_default = sudo_home / ".cryozen-agent"
        ...
        # After unit sync, CRYOZEN_HOME points at the invoking user's native home.
        monkeypatch.setenv("CRYOZEN_HOME", str(sudo_default))
        assert gateway_cli.get_service_name() == "cryozen-gateway"


class TestUninstallRefusesForeignUnit:
    """systemd_uninstall must not stop/disable/unlink a unit pinned to another CRYOZEN_HOME."""

    def test_unit_for_other_home_is_left_alone(self, tmp_path, monkeypatch, capsys):
        unit_path = tmp_path / "cryozen-gateway.service"
        unit_path.write_text('[Service]\nEnvironment="CRYOZEN_HOME=/somewhere/else"\n', encoding="utf-8")
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "mine"))
        monkeypatch.setattr(gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path)
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("CRYOZEN_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("CRYOZEN_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_timeout_stop_sec_covers_default_cron_drain_floor(self, monkeypatch):
        ...
        monkeypatch.delenv("CRYOZEN_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("CRYOZEN_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_restart_exit_code_is_also_declared_a_success_status(self):
        ...
        and any OnFailure= alert unit fires on every routine restart (cryozen update,
        cryozen gateway restart, the in-app restart). SuccessExitStatus=75 keeps the same
    ...
    def test_user_unit_carries_ld_library_path_escaped_for_systemd(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gateway_cli, "get_systemd_unit_path", lambda system=False: tmp_path / "cryozen-gateway.service")
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".cryozen-agent" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `cryozen gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import cryozen_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.cryozen.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>CRYOZEN_HOME</key>"
                "<string>/Users/alice/.cryozen</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_cryozen_home", lambda: tmp_path)
...
class TestLaunchdUnsupportedFallbackPolicy:
    ...
    Regression for the recurrence where ``cryozen gateway install --force`` over the LIVE job
    ...
    def test_eio_on_a_supervised_label_does_not_degrade_to_detached(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.cryozen.gateway")
        monkeypatch.setattr(
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        assert ("observe", "ai.cryozen.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.cryozen.gateway"])
            ),
        )
...
class TestSystemUnitCryozenHome:
    """CRYOZEN_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".cryozen-agent" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".cryozen-agent")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_cryozen = target_home / ".cryozen-agent"
        root_home = tmp_path / "root"
        root_cryozen = root_home / ".cryozen-agent"
        managed_bin = target_cryozen / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        root_cryozen.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
        monkeypatch.setenv("CRYOZEN_HOME", str(root_cryozen))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home), 1001),
        )
        monkeypatch.setattr(gateway_cli, "get_cryozen_home", lambda: root_cryozen)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "cryozen_constants.iter_cryozen_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "cryozen_constants.cryozen_managed_node_tree_present",
    ...
    def test_system_unit_orders_after_target_user_manager(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / ".cryozen-agent"))
        monkeypatch.setattr(
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'CRYOZEN_HOME=/home/alice/.cryozen-agent' in unit
        assert '/root/.cryozen-agent' not in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's CRYOZEN_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        cryozen_home = str(gateway_cli.get_cryozen_home().resolve())
        assert f'CRYOZEN_HOME={cryozen_home}' in unit


class TestSystemUnitRefreshSyncsCryozenHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.cryozen-agent."""

    def test_refresh_adopts_unit_cryozen_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_cryozen = root_home / ".cryozen-agent"
        alice_cryozen = alice_home / ".cryozen-agent"
        root_cryozen.mkdir(parents=True)
        alice_cryozen.mkdir(parents=True)
        (root_cryozen / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_cryozen / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's CRYOZEN_HOME + drain timeout).
        monkeypatch.setenv("CRYOZEN_HOME", str(alice_cryozen))
        ...
        # Simulate sudo without inherited CRYOZEN_HOME (falls back to root).
        monkeypatch.setenv("CRYOZEN_HOME", str(root_cryozen))
        ...
        assert os.environ["CRYOZEN_HOME"] == str(alice_cryozen)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned CRYOZEN_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_cryozen_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_cryozen_home_from_systemd_unit",
...
class TestCryozenHomeForTargetUser:
    """Unit tests for _cryozen_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("CRYOZEN_HOME", raising=False)

        result = gateway_cli._cryozen_home_for_target_user("/home/alice")
        assert result == "/home/alice/.cryozen-agent"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "cryozen gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".cryozen-agent" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("CRYOZEN_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_cryozen_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home), 1001),
        ...
        assert f'CRYOZEN_HOME={target_home / ".cryozen-agent" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".cryozen-agent" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRYOZEN_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_cryozen_home", lambda: profile_dir)
    ...
    def test_launchd_osascript_wrapper_round_trips_shell_hostile_paths(self, tmp_path, monkeypatch):
        ...
        profile_dir = tmp_path / 'my "odd" dir \\ here' / ".cryozen-agent"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CRYOZEN_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_cryozen_home", lambda: profile_dir)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".cryozen-agent" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.cryozen.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".cryozen-agent" / "cryozen-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".cryozen-agent" / "cryozen-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".cryozen-agent" / "cryozen-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("CRYOZEN_HOME", str(root_home / ".cryozen-agent"))
        monkeypatch.setattr(gateway_cli, "get_cryozen_home", lambda: root_home / ".cryozen-agent")
        ...
        # WorkingDirectory is anchored at the target user's CRYOZEN_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.cryozen-agent" in unit
        assert "WorkingDirectory=/home/alice/.cryozen-agent/cryozen-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(SystemctlUnavailableError):
            gateway_cli._run_systemctl(["start", "cryozen-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "cryozen-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'cryozen gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyCryozenUnitDetection:
    ...
    The detector must flag ``cryozen.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``cryozen-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Cryozen Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m cryozen_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "cryozen.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "cryozen.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_cryozen_units()
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m cryozen_cli.main gateway run
          - python path/to/cryozen_cli/main.py gateway run
          - cryozen gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m cryozen_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/cryozen/cryozen_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/cryozen gateway run --replace",
            "ExecStart=/venv/bin/python /opt/cryozen/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "cryozen.service" if i == 0 else "cryozen.service"  # same name
            # Test each variant fresh
            (user_dir / "cryozen.service").write_text(
                f"[Unit]\nDescription=Old Cryozen\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_cryozen_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "cryozen.service" in out
        assert "cryozen gateway migrate-legacy" in out


class TestRemoveLegacyCryozenUnits:
    """Tests for remove_legacy_cryozen_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "cryozen.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_cryozen_units(interactive=False)
        ...
        assert any("--user stop cryozen.service" in c for c in cmds_joined)
        assert any("--user disable cryozen.service" in c for c in cmds_joined)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        profile_unit = user_dir / "cryozen-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "cryozen-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `cryozen gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import cryozen_cli.main as cli_main
        ...
        # public, just confirm that `cryozen gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import cryozen_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "cryozen_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_cryozen_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "cryozen_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_cryozen_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_cryozen_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_cryozen_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_cryozen_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_cryozen_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "cryozen-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "cryozen-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "cryozen-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``cryozen gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "cryozen-gateway")
        ...
        assert "sudo systemctl start cryozen-gateway" in out
        assert "sudo cryozen gateway uninstall --system" in out
        assert "cryozen gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``cryozen gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (CRYOZEN_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_user_unit_workingdirectory_is_cryozen_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".cryozen-agent"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_cryozen_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_cryozen_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.cryozen.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.cryozen.gateway"
...
class TestLaunchdUnloadedJobStderrStaysOffTerminal:
    ...
    def fake_launchctl(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.cryozen.gateway")
        monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "gui/501")
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: tmp_path / "ai.cryozen.gateway.plist")
        monkeypatch.setattr(gateway_cli, "_clear_launchd_unsupported_marker", lambda: None)
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        cryozen = tmp_path / "home" / ".cryozen-agent"
        cryozen.mkdir(parents=True)
        (cryozen / "config.yaml").write_text(config_yaml, encoding="utf-8")
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen))
        monkeypatch.delenv("CRYOZEN_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("CRYOZEN_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"CRYOZEN_CRON_DRAIN_TIMEOUT": "200"},
        )
        assert "TimeoutStopSec=240" in unit


class TestUnitAnchoredServiceIdentity:
    """The installed ``cryozen-gateway.service`` owns the bare name: under ``sudo`` the naming basis moves
    mid-command when ``_sync_cryozen_home_from_systemd_unit()`` adopts the unit's CRYOZEN_HOME (#108674).
    ...
    def test_home_not_pinned_by_unit_keeps_its_suffix(self, tmp_path, monkeypatch):
        alice_home = tmp_path / "alice" / ".cryozen-agent"
        alice_home.mkdir(parents=True)
        bob_home = tmp_path / "bob" / ".cryozen-agent"
        bob_home.mkdir(parents=True)
        root_home = tmp_path / "root" / ".cryozen-agent"
        root_home.mkdir(parents=True)
        unit_dir = tmp_path / "systemd"
        unit_dir.mkdir()
        (unit_dir / f"{gateway_cli._SERVICE_BASE}.service").write_text(
            f'[Service]\nEnvironment="CRYOZEN_HOME={alice_home}"\n', encoding="utf-8"
        )
        monkeypatch.setattr(gateway_cli, "_SYSTEM_UNIT_DIR", unit_dir)
        monkeypatch.setattr(cryozen_constants, "_get_platform_default_cryozen_home", lambda: root_home)
        monkeypatch.setenv("CRYOZEN_HOME", str(bob_home))
    ...
    def test_unprivileged_profile_command_ignores_the_system_unit(self, tmp_path, monkeypatch):
        ...
        profile_home = tmp_path / "alice" / ".cryozen-agent" / "profiles" / "kimi"
        profile_home.mkdir(parents=True)
        unit_dir = tmp_path / "systemd"
        unit_dir.mkdir()
        (unit_dir / f"{gateway_cli._SERVICE_BASE}.service").write_text(
            f'[Service]\nEnvironment="CRYOZEN_HOME={profile_home}"\n', encoding="utf-8"
        )
        monkeypatch.setattr(gateway_cli, "_SYSTEM_UNIT_DIR", unit_dir)
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        monkeypatch.setattr(os, "geteuid", lambda: 1000)
        monkeypatch.setenv("CRYOZEN_HOME", str(profile_home))
        assert gateway_cli.get_service_name() == "cryozen-gateway-kimi"
    ...
    def test_real_unit_sync_keeps_the_name_it_validated(self, tmp_path, monkeypatch):
        ...
        before ``_sync_cryozen_home_from_systemd_unit()`` must survive the mutation it performs."""
        alice_home = tmp_path / "alice" / ".cryozen-agent"
        alice_home.mkdir(parents=True)
        root_home = tmp_path / "root" / ".cryozen-agent"
        root_home.mkdir(parents=True)
        ...
        gateway_cli._sync_cryozen_home_from_systemd_unit(system=True)

        assert os.environ["CRYOZEN_HOME"] == str(alice_home)  # the sync really ran
