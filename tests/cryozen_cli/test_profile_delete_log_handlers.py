...
A long-lived multiplex process (the Desktop's ``cryozen serve`` backend) routes records to every
...
def routed_profile(tmp_path, monkeypatch):
    ...
    cryozen_logging.setup_logging(cryozen_home=home, force=True)
    try:
        yield home, profile
    finally:
...
def _log_into(profile: Path, *, adopt: bool) -> None:
    ...
    ``setup_logging(cryozen_home=<profile>)`` adoption path ``agent_init`` takes."""
    logger = logging.getLogger("agent.tests.profile-delete")
    token = set_cryozen_home_override(profile)
    try:
        if adopt:
            cryozen_logging.setup_logging(cryozen_home=profile)
        else:
            assert cryozen_logging.enable_profile_log_routing([Path(profile).parent.parent, profile]) is True
...
def _routers() -> list:
    return [h for h in cryozen_logging._queued_file_handlers
            if isinstance(h, cryozen_logging._ProfileRoutingFileHandler)]
