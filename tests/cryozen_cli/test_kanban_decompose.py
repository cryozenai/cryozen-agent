"""Tests for the decomposer module + `cryozen kanban decompose` CLI surface.
...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("cryozen_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("cryozen_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch("cryozen_cli.profiles.get_active_profile_name", return_value=names[0] if names else "default"),
    ]
