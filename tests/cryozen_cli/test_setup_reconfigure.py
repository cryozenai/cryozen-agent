...
- Bare `cryozen setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `cryozen setup --quick` runs the narrower "fill in missing items" flow.
- `cryozen setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("cryozen_cli.setup.ensure_cryozen_home", {}),
        ("cryozen_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("cryozen_cli.config.is_managed", {"return_value": False}),
        ("cryozen_cli.setup.load_config", {"return_value": {}}),
        ("cryozen_cli.setup.save_config", {}),
        ("cryozen_cli.setup.get_env_value", {"return_value": None}),
        ("cryozen_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("cryozen_cli.setup._print_setup_summary", {}),
        ("cryozen_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("cryozen_cli.setup.load_config", {"return_value": {}}),
        ("cryozen_cli.setup.save_config", {}),
        ("cryozen_cli.auth.get_active_provider", {"return_value": None}),
        ("cryozen_cli.setup.get_env_value", {"return_value": None}),
        ("cryozen_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `cryozen setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="cryozen_cli.setup.prompt_choice",
                quick="cryozen_cli.setup._run_quick_setup",
                model="cryozen_cli.setup.setup_model_provider",
                terminal="cryozen_cli.setup.setup_terminal_backend",
                agent="cryozen_cli.setup.setup_agent_settings",
                gateway="cryozen_cli.setup.setup_gateway",
                tools="cryozen_cli.setup.setup_tools",
            )
            from cryozen_cli.setup import run_setup_wizard
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("cryozen_cli.setup.prompt_choice", {"return_value": 0}),
                first="cryozen_cli.setup_quick._run_first_time_quick_setup",
            )
            from cryozen_cli.setup import run_setup_wizard
            from cryozen_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("cryozen_cli.setup.prompt_choice", {"return_value": 2}),
                blank="cryozen_cli.setup_quick._run_blank_slate_setup",
            )
            from cryozen_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "cryozen_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["cryozen", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
