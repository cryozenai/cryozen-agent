"""Tests for cryozen_cli.stderr_timestamp."""
...
from cryozen_cli import stderr_timestamp
...
_LAUNCHD_ENV = {"PATH": "/usr/bin", "XPC_SERVICE_NAME": "ai.cryozen.gateway-butler"}
...
def test_main_injects_flag_into_stale_gateway_child(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "ai.cryozen.gateway-butler")
    ...
    stale = [sys.executable, "-c", code, "-m", "cryozen_cli.main", "gateway", "run", "--replace"]
...
def test_main_maps_gateway_ex_config_to_clean_stop(tmp_path):
    ...
    log_path = tmp_path / "gateway.error.log"
    gateway_tail = ["-m", "cryozen_cli.main", "gateway", "run"]
...
def test_wrapper_forwards_sigusr1_restart_request_to_child(tmp_path):
    """Regression for #101426: launchd owns the wrapper's PID, so ``cryozen update`` sends its
    ...
    wrapper = subprocess.Popen(
        [sys.executable, "-m", "cryozen_cli.stderr_timestamp", "--error-log", str(log_path), "--",
