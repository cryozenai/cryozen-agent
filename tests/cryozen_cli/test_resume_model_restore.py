...
SessionDB.session_gateway_runtime (cryozen_state.py) — the round trip that
makes `cryozen --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare CryozenCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.CryozenCLI)
...
def test_session_gateway_runtime_falls_back_to_top_level_keys():
    ...
    meta = _row(model_config={"provider": "openrouter", "api_mode": "chat_completions"})
    runtime = SessionDB.session_gateway_runtime(meta)
    assert runtime == {"provider": "openrouter", "api_mode": "chat_completions"}
...
def test_restore_llamacpp_session_keeps_launch_base_url_for_same_provider(monkeypatch):
    """`cryozen --provider llamacpp --base-url X --resume` is user intent for the SAME provider the
...
def test_session_gateway_runtime_explicit_provider_wins_over_billing():
    ...
    meta = _row(model_config={"provider": "openrouter"})
