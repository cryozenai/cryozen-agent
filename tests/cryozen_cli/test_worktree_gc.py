"""Behavior contracts for cryozen_cli.worktree_gc (attended reclaim).
...
def _add_worktree(repo_path, name, branch=None):
    ...
    branch = branch or f"cryozen/{name}"
...
class TestAuditVerdicts:
    ...
    def test_unique_unpushed_commits_keep(self, repo):
        tree, _ = _add_worktree(repo, "cryozen-work")
        (tree / "new.py").write_text("x = 1\n")
        _git(["add", "."], tree)
        _git(["commit", "-m", "unique work"], tree)
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        record = _verdict(records, "cryozen-work")
        assert record.verdict == "keep"
    ...
    def test_without_a_remote_local_trunk_is_the_baseline(self, local_repo):
        ...
        clean_tree, _ = _add_worktree(local_repo, "cryozen-clean")
        squashed_tree, _ = _add_worktree(local_repo, "cryozen-squashed")
        (squashed_tree / "k.py").write_text("k = 1\n")
        _git(["add", "."], squashed_tree)
        _git(["commit", "-m", "squash me"], squashed_tree)
        _git(["merge", "--squash", "cryozen/cryozen-squashed"], local_repo)
        ...
        records = worktree_gc.audit_worktrees(str(local_repo), with_sizes=False)
        assert _verdict(records, "cryozen-clean").verdict == "reap"
        assert _verdict(records, "cryozen-squashed").verdict == "reap"
        ...
        assert "cryozen/cryozen-squashed" not in by_name  # branch -D ran only on merged work
    ...
    def test_patch_equivalent_commits_reap(self, repo):
        ...
        tree, _ = _add_worktree(repo, "cryozen-merged")
        ...
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        assert _verdict(records, "cryozen-merged").verdict == "reap"

    def test_live_locked_tree_keeps(self, repo):
        tree, _ = _add_worktree(repo, "cryozen-live")
        _git(["worktree", "lock", str(tree),
              "--reason", f"cryozen pid={os.getpid()}"], repo)
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        record = _verdict(records, "cryozen-live")
        assert record.verdict == "keep"
...
class TestReclaim:
    def test_reap_removes_tree_and_branch(self, repo):
        tree, branch = _add_worktree(repo, "cryozen-clean")
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        actions = worktree_gc.reclaim_worktrees(str(repo), records=records)
        assert any("removed cryozen-clean" in a for a in actions)
    ...
    def test_untracked_files_archived_under_the_active_profile_home(self, repo, tmp_path, monkeypatch):
        """The archive follows the active Cryozen home (a named profile here), never ~/.cryozen-agent."""
        ...
        assert not (native_home / ".cryozen-agent").exists()

    def test_dry_run_changes_nothing(self, repo):
        tree, _ = _add_worktree(repo, "cryozen-clean")
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
    ...
    def test_frozen_list_ignores_trees_created_after_audit(self, repo):
        ...
        _add_worktree(repo, "cryozen-old")
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        late_tree, _ = _add_worktree(repo, "cryozen-late")
        worktree_gc.reclaim_worktrees(str(repo), records=records)
    ...
    def test_dead_locked_tree_is_unlocked_and_reaped(self, repo):
        tree, _ = _add_worktree(repo, "cryozen-zombie")
        _git(["worktree", "lock", str(tree),
              "--reason", "cryozen pid=999999999"], repo)
        records = worktree_gc.audit_worktrees(str(repo), with_sizes=False)
        assert _verdict(records, "cryozen-zombie").verdict == "reap"
        worktree_gc.reclaim_worktrees(str(repo), records=records)
        assert not tree.exists()
...
class TestBranchGC:
    ...
    def test_checked_out_and_protected_kept(self, repo):
        _tree, branch = _add_worktree(repo, "cryozen-active")
        records = worktree_gc.audit_branches(str(repo))
        by_name = {record.name: record for record in records}
        assert by_name["main"].verdict == "keep"
...
class TestOlderThanGate:
    ...
    def test_aged_reapable_tree_still_reaps(self, repo):
        ...
        tree, _ = _add_worktree(repo, "cryozen-old")
        old = _time.time() - 10 * 86400
        _os.utime(tree, (old, old))
        records = worktree_gc.audit_worktrees(
            str(repo), with_sizes=False, older_than_days=7,
        )
        assert _verdict(records, "cryozen-old").verdict == "reap"
    ...
    def test_older_than_never_widens_eligibility(self, repo):
        ...
        tree, _ = _add_worktree(repo, "cryozen-old-work")
        (tree / "README.md").write_text("edited\n")
        ...
        record = _verdict(records, "cryozen-old-work")
        assert record.verdict == "keep"
        assert "tracked" in record.reason
...
class TestExternalTrees:
    ...
    def test_managed_trees_not_reported_as_external(self, repo):
        _add_worktree(repo, "cryozen-managed")
        external = worktree_gc.audit_external_trees(str(repo))
        assert all("cryozen-managed" not in r.path for r in external)
...
class TestCmdWorktreeJson:
    ...
    def test_list_json_shape(self, repo, capsys):
        ...
        from cryozen_cli.worktree_cmd import cmd_worktree

        _add_worktree(repo, "cryozen-json")
        ...
        assert "cryozen-json" in names
        tree = [t for t in payload["trees"] if t["name"] == "cryozen-json"][0]
    ...
    def test_prune_dry_run_json(self, repo, capsys):
        ...
        _add_worktree(repo, "cryozen-json-prune")
        assert cmd_worktree(self._ns(repo, "prune", dry_run=True)) == 0
        payload = json.loads(capsys.readouterr().out)
        assert payload["dry_run"] is True
        assert any("cryozen-json-prune" in a for a in payload["actions"])
        # dry-run: tree still present
        assert (repo / ".worktrees" / "cryozen-json-prune").exists()
