"""`cryozen proxy` must resolve upstream credentials off the event loop.

``UpstreamAdapter`` is a synchronous contract (``adapters/base.py`` — every
method is a plain ``def``), and adapters implement it with blocking I/O:

  * An OAuth adapter may take ``_auth_store_lock()``, a *cross-process* advisory
    lock with ``AUTH_LOCK_TIMEOUT_SECONDS = 15.0``, read ``auth.json`` from disk,
    and issue a token-refresh POST.
...
The harness mirrors ``tests/cryozen_cli/test_proxy.py``: the proxy and a fake
...
from cryozen_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential

aiohttp = pytest.importorskip("aiohttp")
from aiohttp import web  # noqa: E402

from cryozen_cli.proxy.server import create_app  # noqa: E402
...
def test_get_retry_credential_runs_off_the_event_loop():
    ...
    ``UpstreamAdapter`` contract, and it is the most expensive of them: an OAuth
    adapter may force a token-refresh POST under the 15s cross-process
...
def test_is_authenticated_runs_off_the_event_loop():
    ...
    calls", but an adapter may implement it by reading auth state under the
    15s cross-process ``_auth_store_lock()``. ``/health`` is what
