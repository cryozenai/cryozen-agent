"""Tests for cryozen_cli.gateway_windows."""
...
import cryozen_cli.gateway as gateway
import cryozen_cli.gateway_windows as gateway_windows
import cryozen_cli.setup as setup


_BREAKAWAY_MARKER = "_CRYOZEN_GATEWAY_BREAKAWAY"
...
def test_exec_schtasks_decodes_ansi_output_under_utf8_mode(monkeypatch):
    ...
    xml = r'<Arguments>//B //Nologo "C:\Users\方舟\AppData\Local\cryozen\gateway-service\Cryozen_Gateway.vbs"</Arguments>'
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "Cryozen_Gateway", "/XML"])
...
def test_exec_schtasks_round_trips_non_ascii_task_argument_live(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.locale, "getpreferredencoding", lambda *a, **k: "utf-8")
    task = f"Cryozen_Test_{os.getpid()}"
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    cryozen_home = tmp_path / "cryozen-home"
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda cryozen_home: "")
    monkeypatch.setattr("cryozen_cli.config.get_cryozen_home", lambda: str(cryozen_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "cryozen_cli.main"]
    assert cwd == str(cryozen_home.resolve())
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "cryozen_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_build_gateway_argv",
        lambda home=None: (argv, cwd, {"CRYOZEN_GATEWAY_DETACHED": "1"}),
    )
    monkeypatch.setattr("cryozen_cli.config.get_cryozen_home", lambda: tmp_path)
...
def test_spawn_detached_warns_and_marks_no_breakaway_fallback(
    ...
    monkeypatch.setattr(
        ...
        lambda home=None: (
            argv,
            cwd,
            {"CRYOZEN_GATEWAY_DETACHED": "1", "SECRET_SENTINEL": "do-not-log"},
        ),
    )
    monkeypatch.setattr("cryozen_cli.config.get_cryozen_home", lambda: tmp_path)
    monkeypatch.setattr(gateway_windows.subprocess, "Popen", fake_popen)
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_cryozen_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".cryozen-agent"
        home.mkdir()
        monkeypatch.setattr("cryozen_cli.config.get_cryozen_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".cryozen-agent"
        project = tmp_path / "checkout"
        monkeypatch.setattr("cryozen_cli.config.get_cryozen_home", lambda: missing)
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Cryozen_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Cryozen_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Cryozen_Gateway_alice")
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Cryozen\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Cryozen\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Cryozen_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Cryozen_Gateway_alice")
    ...
    assert "Cryozen_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "cryozen_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("CRYOZEN_HOME", "PYTHONIOENCODING", "CRYOZEN_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
def test_atomic_write_leaves_no_staging_file_when_swap_fails(monkeypatch, tmp_path):
    ...
    startup.mkdir()
    entry, staging = startup / "Cryozen_Gateway.vbs", startup / "Cryozen_Gateway.tmp"
...
def test_uninstall_and_reinstall_sweep_stale_startup_staging_file(monkeypatch, tmp_path):
    ...
    startup = tmp_path / "Startup"
    startup.mkdir()
    entry, staging = startup / "Cryozen_Gateway_alice.vbs", startup / "Cryozen_Gateway_alice.tmp"
    script = tmp_path / "task" / "Cryozen_Gateway_alice.cmd"
    ...
    monkeypatch.setattr(gateway_windows, "_legacy_startup_entry_path", lambda: startup / "Cryozen_Gateway_alice.cmd")
...
def test_status_names_and_uninstall_removes_pre_suffix_launchers(monkeypatch, tmp_path, capsys):
    """#116157: a Scheduled Task ``Cryozen_Gateway`` and a Startup ``Cryozen_Gateway.vbs`` left from before
    ...
    legacy_vbs.write_text(gateway_windows._build_startup_launcher(home / "gateway-service" / "Cryozen_Gateway.cmd"), encoding="utf-8")
    legacy_pair = home / "gateway-service" / "Cryozen_Gateway.cmd"
    legacy_pair.write_text("legacy", encoding="utf-8")
    schtasks_calls = []
    registered = {"Cryozen_Gateway"}
    task_xml = gateway_windows._build_scheduled_task_xml("Cryozen_Gateway", home / "gateway-service" / "Cryozen_Gateway.vbs", None)
    ...
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Cryozen_Gateway_alice")
    monkeypatch.setattr(gateway_windows, "get_task_script_path", lambda: home / "gateway-service" / "Cryozen_Gateway_alice.cmd")
    monkeypatch.setattr(gateway_windows, "get_startup_entry_path", lambda: startup / "Cryozen_Gateway_alice.vbs")
    monkeypatch.setattr(gateway_windows, "_legacy_startup_entry_path", lambda: startup / "Cryozen_Gateway_alice.cmd")
    monkeypatch.setattr(gateway_windows, "_startup_dir", lambda: startup)
    monkeypatch.setattr(gateway_windows, "_cryozen_home", lambda: home)
    monkeypatch.setattr(gateway_windows, "_exec_schtasks", fake_schtasks)
    ...
    assert ["/Delete", "/F", "/TN", "Cryozen_Gateway"] in schtasks_calls
    gateway_windows.status()
...
def test_secondary_profile_leaves_default_profiles_bare_launchers_alone(monkeypatch, tmp_path, capsys):
    """The bare ``Cryozen_Gateway`` task and Startup entry are the LIVE identity of the default ``~/.cryozen-agent``
    ...
    startup.mkdir()
    default_vbs = startup / "Cryozen_Gateway.vbs"
    default_vbs.write_text(gateway_windows._build_startup_launcher(default_home / "gateway-service" / "Cryozen_Gateway.cmd"), encoding="utf-8")
    task_xml = gateway_windows._build_scheduled_task_xml("Cryozen_Gateway", default_home / "gateway-service" / "Cryozen_Gateway.vbs", None)
    schtasks_calls = []

    def fake_schtasks(args):
        schtasks_calls.append(args)
        if args[0] == "/Query" and args[args.index("/TN") + 1] == "Cryozen_Gateway":
            return (0, task_xml, "")
        return (1, "", "ERROR: The system cannot find the file specified.")

    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Cryozen_Gateway_work")
    monkeypatch.setattr(gateway_windows, "get_task_script_path", lambda: home / "gateway-service" / "Cryozen_Gateway_work.cmd")
    monkeypatch.setattr(gateway_windows, "get_startup_entry_path", lambda: startup / "Cryozen_Gateway_work.vbs")
    monkeypatch.setattr(gateway_windows, "_legacy_startup_entry_path", lambda: startup / "Cryozen_Gateway_work.cmd")
    monkeypatch.setattr(gateway_windows, "_startup_dir", lambda: startup)
    monkeypatch.setattr(gateway_windows, "_cryozen_home", lambda: home)
    monkeypatch.setattr(gateway_windows, "_exec_schtasks", fake_schtasks)
    monkeypatch.setattr(gateway_windows, "_gateway_pids", lambda *a, **k: [])
    ...
    assert not any(call[0] == "/Delete" and "Cryozen_Gateway" in call for call in schtasks_calls)
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author">
    <Exec>
      ...
      <Arguments>"C:\\Users\\me\\.cryozen-agent\\gateway-service\\Cryozen_Gateway.vbs"</Arguments>
    </Exec>
  </Actions>
</Task>
"""
...
def test_scheduled_task_drift_names_missing_hardening_leaves(monkeypatch):
    ...
    ``cryozen gateway status`` prints together with the ``cryozen gateway install`` repair hint."""
    launcher = Path(r"C:\Users\me\.cryozen-agent\gateway-service\Cryozen_Gateway.vbs")
    template = gateway_windows._build_scheduled_task_xml("Cryozen_Gateway", launcher, r"PC\me")
...
def test_reconcile_scheduled_task_reregisters_only_on_drift(monkeypatch, tmp_path):
    ...
    assert gateway_windows.reconcile_scheduled_task("Cryozen_Gateway") is True
    ...
    calls.clear()
    assert gateway_windows.reconcile_scheduled_task("Cryozen_Gateway") is False
...
def _arrange_uninstalled_start(monkeypatch):
    ...
    installs, spawns = [], []
    monkeypatch.delenv("CRYOZEN_GATEWAY_INSTALL_START_ON_LOGIN", raising=False)
    monkeypatch.delenv("CRYOZEN_NONINTERACTIVE", raising=False)
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "_print_start_attestation_warning", lambda: None)
...
def test_stdin_interactive_only_when_isatty_and_a_console_answers_get_console_mode():
    """Windows CRT isatty() is True for the NUL device (`cryozen gateway start < NUL`, stdin=DEVNULL), so
...
def test_start_with_nul_stdin_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    ...
    assert "cryozen gateway install" in capsys.readouterr().out


def test_start_without_tty_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    """`cryozen gateway start < /dev/null` must not answer the persistence question with a default Yes
    ...
    assert installs == [] and spawns == [1]
    out = capsys.readouterr().out
    assert "cryozen gateway install" in out and "did not complete" not in out
...
def test_start_on_tty_hands_both_answers_to_install_and_honours_the_env_opt_out(monkeypatch):
    ...
    again). CRYOZEN_GATEWAY_INSTALL_START_ON_LOGIN=0 → no question, no install, a plain start."""
    installs, spawns = _arrange_uninstalled_start(monkeypatch)
    monkeypatch.setattr(setup, "is_interactive_stdin", lambda: True)
    ...
    installs.clear()
    monkeypatch.setenv("CRYOZEN_GATEWAY_INSTALL_START_ON_LOGIN", "0")
...
# resume_pending=True) never fires when `cryozen gateway stop` kills the
...
def test_cryozen_owns_windows_service_requires_name_or_binary_under_a_cryozen_root():
