...
* ``cryozen -p <served> cron status`` names the HOST gateway as the ticker and never tells the
  user to start a second process.
* ``cryozen -p <served> doctor`` reports the single host gateway + its roster, not
  "Per-profile gateways: up/total", and still checks the HOST systemd unit's linger.
* ``cryozen claw``'s destructive-action warning FIRES for a served profile.
...
def served_host(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_PROFILE", "served")
    monkeypatch.setenv("CRYOZEN_GATEWAY_LOCK_DIR", str(locks))
...
def test_cron_status_names_the_host_gateway_for_a_served_profile(served_host, capsys, monkeypatch):
    ...
    assert "cryozen gateway install" not in out
    assert "sudo cryozen gateway install --system" not in out


def test_claw_warning_fires_for_a_served_profile(served_host, capsys, monkeypatch):
    from cryozen_cli import claw
...
def test_doctor_reports_the_single_host_gateway_not_per_profile_slots(served_host, capsys, monkeypatch):
    ...
    class _Mgr:
        def is_running(self, name):
            return name in ("main-cryozen", "gateway-legacy")
    ...
    monkeypatch.setattr("cryozen_cli.service_manager.S6ServiceManager", _Mgr)
    issues: list[str] = []
...
def test_doctor_checks_host_unit_linger_under_a_served_profile(served_host, tmp_path, capsys, monkeypatch, xdg):
    ...
    (unit_dir / "cryozen-gateway.service").write_text("[Unit]\n")  # the HOST unit; no per-profile unit
    ...
    monkeypatch.setattr("cryozen_cli.gateway.is_linux", lambda: True)
    monkeypatch.setattr("cryozen_cli.service_manager.detect_service_manager", lambda: "systemd")
    monkeypatch.setattr("cryozen_cli.gateway.get_systemd_linger_status", lambda *a, **k: (False, "off"))
