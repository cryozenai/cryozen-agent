...
a step that still raises so one bad value cannot wedge ``cryozen config migrate`` /
``cryozen update`` and leave the config unversioned.
"""
...
def test_failing_step_is_skipped_with_warning_and_config_still_migrates(tmp_path, caplog):
    """``migrate_config`` (the ``cryozen config migrate`` / ``cryozen update`` path) keeps going
    ...
    from cryozen_cli import config_migrations
    ...
    with patch.dict(os.environ, {"CRYOZEN_HOME": str(tmp_path)}), \
            ...
            caplog.at_level(logging.WARNING, logger="cryozen_cli.config_migrations"):
