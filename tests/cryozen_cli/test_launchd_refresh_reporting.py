...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.cryozen.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.cryozen.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("cryozen_constants.display_cryozen_home", lambda: "~/.cryozen-work")
    ...
    assert "~/.cryozen-work/logs/launchd-reload.log" in out
