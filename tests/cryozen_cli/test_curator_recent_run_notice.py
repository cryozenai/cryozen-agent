...
The notice prints the most recent curator run summary on `cryozen update`,
...
session start) so users normally never see the rename map. `cryozen update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from cryozen_cli import update_cmd_maint
