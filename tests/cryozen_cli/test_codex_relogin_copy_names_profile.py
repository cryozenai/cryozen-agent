...
Profiles are islands (93889b770da): a bare ``cryozen auth`` / ``cryozen model`` from a named
profile's error text re-signs the ROOT store, which is exactly the loop #114012 measured.
Every relogin hint raised by ``cryozen_cli/auth_codex.py`` and appended by
``format_auth_error`` must carry the ``-p <profile>`` selector under a profile CRYOZEN_HOME.
"""
...
from cryozen_cli.auth import AuthError, _read_codex_tokens, format_auth_error
from cryozen_cli.auth_codex import _codex_refresh_failure_error
...
def test_missing_and_reused_codex_credential_copy_names_the_profile(codex_profile_home):
    ...
    for err in (missing.value, reused):
        text = str(err)
        assert "`cryozen -p codex auth add openai-codex --type oauth`" in text, text
        assert "`cryozen auth`" not in text, text
...
def test_format_auth_error_relogin_suffix_names_the_profile(codex_profile_home):
    ...
    assert "`cryozen -p codex model`" in rendered, rendered
    assert "`cryozen model`" not in rendered, rendered
