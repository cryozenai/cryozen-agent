...
Profile-aware location: ``$CRYOZEN_HOME/logs/dashboard-auth.log``.
...
from cryozen_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $CRYOZEN_HOME and ~ to a tmp dir for the duration of the test."""
...
def test_audit_writes_jsonlines(profile_home):
    audit_log(AuditEvent.LOGIN_START, provider="self-hosted", ip="1.2.3.4")
    audit_log(
    ...
    assert second["provider"] == "self-hosted"
...
def test_audit_redacts_token_like_fields(profile_home):
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="self-hosted", access_token="should-not-appear",
