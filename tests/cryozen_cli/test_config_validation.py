...
class TestTimezoneValidation:
    ...
    schedule on server-local time (cryozen_time._get_zoneinfo falls back with
    ...
    def test_invalid_or_non_string_zone_is_an_error(self):
        [issue] = _tz_issues({"timezone": "Asia/Tokio", "model": {"provider": "openrouter"}})
        assert issue.severity == "error"
        assert "Asia/Tokio" in issue.message
        assert "IANA" in issue.hint and "CRYOZEN_TIMEZONE" in issue.hint
        [issue] = _tz_issues({"timezone": 9, "model": {"provider": "openrouter"}})
    ...
    def test_valid_blank_missing_or_unverifiable_zone_is_silent(self, monkeypatch):
        for cfg in ({"timezone": "Asia/Tokyo"}, {}, {"timezone": ""}, {"timezone": "   "}, {"timezone": None}):
            assert _tz_issues({**cfg, "model": {"provider": "openrouter"}}) == []
        ...
        assert _tz_issues({"timezone": "Asia/Tokio", "model": {"provider": "openrouter"}}) == []
...
class TestUnknownTopLevelKeys:
    ...
    (gateway/run.py, cryozen send) so users can feed skills and external apps
...
class TestQuotedContainerValues:
    ...
    def test_quoted_list_in_container_slot_is_flagged_with_remedy(self):
        ...
        assert "cryozen config set plugins.enabled '[\"a\",\"b\"]'" in flagged["plugins.enabled"].hint
