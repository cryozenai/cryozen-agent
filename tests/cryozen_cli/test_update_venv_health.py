...
Covers three additions to ``cryozen update``:
...
def test_detect_venv_python_prefetches_only_cheap_process_fields(_winp, tmp_path):
    venv_py = str(tmp_path / "venv" / "Scripts" / "python.exe")
    holder = _proc(101, venv_py, "python.exe", [venv_py, "-m", "cryozen_cli.main", "serve"])
...
def test_detect_venv_python_matches_uv_default_dotvenv(_winp, tmp_path):
    ...
    no ``cryozen_cli.main`` in its cmdline, so only that arm can match it — the guard was blind to it."""
    venv_py = str(tmp_path / ".venv" / "Scripts" / "python.exe")
    holder = _proc(104, venv_py, "python.exe", [venv_py, str(tmp_path / "tools" / "cryozen_kernel_runner.py")])
    me = MagicMock()
    me.parents.return_value = []
    fake_psutil = types.SimpleNamespace(
...
def test_detect_venv_python_keeps_external_interpreter_fallback(_winp, tmp_path):
    external = _proc(
        ...
        "python.exe",
        ["python.exe", "-m", "cryozen_cli.main", "serve"],
        str(tmp_path),
    )
    me = MagicMock()
    me.parents.return_value = []
...
def _run_update_until_guard(args):
    ...
    with patch.object(cli_main, "_is_windows", return_value=True), patch.object(
        ...
        cli_main,
        "_detect_venv_python_processes",
        return_value=[(101, "python.exe", "python.exe -m cryozen_cli.main serve")],
    ), patch.object(
...
def test_venv_dependency_set_stale_compares_installed_distribution_with_checkout(tmp_path):
    ...
    with patch.object(cli_main, "PROJECT_ROOT", tmp_path), patch.object(cli_main, "_is_windows", return_value=False), \
        ...
        assert update_cmd._venv_dependency_set_stale() == (True, "installed cryozen-agent 0.20.6, checkout is 0.21.2")
...
def test_current_checkout_with_stale_dependency_set_runs_the_sync(monkeypatch, capsys):
    ...
    monkeypatch.setattr("cryozen_cli.managed_uv.update_managed_uv", lambda **kwargs: None)
    monkeypatch.setattr("cryozen_cli.managed_uv.ensure_uv", lambda **kwargs: "uv")
    ...
    assert run((True, "installed cryozen-agent 0.20.6, checkout is 0.21.2")) is True
    assert calls == ["sync"]
