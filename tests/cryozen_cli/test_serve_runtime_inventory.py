...
A network-bound `cryozen serve --host <ip>` powering a remote Desktop used to
be invisible to the update pipeline: not in the inventory, a dead-end at the
venv-holder guard, and never relaunched after `cryozen update` killed it. The
...
import cryozen_cli.update_inventory as update_inventory
from cryozen_cli import main as cli_main
import cryozen_cli.main_install_repair as main_install_repair
import cryozen_cli.main_dashboard as main_dashboard
...
def _ledger_entry(**over):
    entry = {
        ...
        "spawner_create": None,
        "registered_at": 222.0,
        "argv": "cryozen serve --host 100.94.65.93 --port 9119",
        "host": "100.94.65.93",
        "port": 9119,
        "profile": "",
    }
...
def test_register_self_records_structured_detail(tmp_path, monkeypatch):
    from cryozen_cli import process_identity as pi
...
def test_inventory_includes_manual_serve_from_ledger(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "cryozen_cli.process_identity", fake_pi)
...
def test_serve_relaunch_commands_built_from_structured_identity(monkeypatch):
    ...
    assert ["cryozen", "serve", "--host", "100.94.65.93", "--port", "9119"] in cmds
    assert ["cryozen", "--profile", "work", "serve", "--port", "9200"] in cmds
    assert ["cryozen", "dashboard", "--host", "0.0.0.0", "--port", "9300"] in cmds
...
def test_scan_dashboard_processes_includes_ledger_only_serves(monkeypatch):
    """A profiled serve (`cryozen --profile p serve ...`) matches no scan
    pattern; the ledger row must still surface it."""
    import cryozen_cli.dashboard_procs as dp

    profiled = _ledger_entry(
        pid=8123,
        argv="cryozen --profile work serve --host 100.94.65.93 --port 9119",
        profile="work",
    )
...
def test_inventory_classifies_launchd_job_owned_serve(monkeypatch):
    ...
    jobs = [("gui/501", "ai.cryozen.dashboard",
             ["cryozen", "serve", "--host", "100.94.65.93", "--port", "9119"], None)]
    monkeypatch.setitem(sys.modules, "cryozen_cli.process_identity", fake_pi)
    ...
    assert row.detail["launchd_label"] == "ai.cryozen.dashboard"
...
def test_stale_serve_warning_names_the_launchd_kickstart_command(capsys):
    ...
    from cryozen_cli import update_abort_recovery
