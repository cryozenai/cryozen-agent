...
reset).  Cryozen must detect that and lift the stale local cooldown instead
...
def _write_auth_store(cryozen_home, payload):
...
def test_resolver_recovers_when_probe_confirms_reset(tmp_path, monkeypatch):
    ...
    _write_auth_store(cryozen_home, _pool_only_rate_limited_store())
    ...
    store = json.loads((cryozen_home / "auth.json").read_text())
...
def test_resolver_selects_entry_with_expired_millisecond_reset(tmp_path, monkeypatch):
    ...
    cryozen_home = tmp_path / "cryozen"
    _write_auth_store(cryozen_home, store)
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
...
def test_pool_probe_not_fired_for_non_quota_exhaustion(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path / "cryozen", store)
...
def test_resolver_refreshes_expired_token_before_probe(tmp_path, monkeypatch):
    ...
    now = time.time()
    cryozen_home = tmp_path / "cryozen"
    _write_auth_store(cryozen_home, _expired_jwt_pool_store(now))
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
    ...
    entry = json.loads((cryozen_home / "auth.json").read_text())["credential_pool"]["openai-codex"][0]
...
def test_pool_selection_refreshes_expired_token_before_probe(tmp_path, monkeypatch):
    ...
    disk = json.loads((cryozen_home / "auth.json").read_text())
