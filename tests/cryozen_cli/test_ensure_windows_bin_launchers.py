"""``cryozen`` must survive git operations on the checkout (launcher layout).

The Windows ``cryozen`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``CRYOZEN_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``cryozen-agent\\bin``) was swept by ``cryozen update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``cryozen`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``cryozen update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``cryozen_constants.venv_bin_dir``), so these
...
from cryozen_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: CRYOZEN_HOME/cryozen-agent/venv/Scripts + launchers."""
    home = tmp_path / "cryozen"
    root = home / "cryozen-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under CRYOZEN_HOME gains nothing anywhere."""
    home = tmp_path / "cryozen-home"
    home.mkdir()
    monkeypatch.setenv("CRYOZEN_HOME", str(home))
    root = tmp_path / "src" / "cryozen-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``cryozen -p <name>`` CRYOZEN_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("CRYOZEN_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `cryozen update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "cryozen.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "cryozen.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\cryozen\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/CRYOZEN/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``cryozen update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/cryozen.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/cryozen.exe is not gitignored — cryozen update's autostash "
