"""Tests for cryozen_cli/goals.py — persistent cross-turn goals."""
...
def cryozen_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves CRYOZEN_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from cryozen_cli.goals import _parse_judge_response
...
class TestGoalManager:
    ...
    def test_set_then_status(self, cryozen_home):
    ...
    def test_continuation_prompt_shape(self, cryozen_home):
        ...
        mgr.set("port goal command to cryozen")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to cryozen" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from cryozen_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, cryozen_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from cryozen_cli import goals
        from cryozen_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from cryozen_cli.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, cryozen_home):
        from cryozen_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_does_not_clobber_existing_child_goal(self, cryozen_home):
        from cryozen_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
...
class TestSessionDbCacheAfterProfileDelete:
    """``cryozen profile delete`` force-closes every registry handle under the profile home
    ...
    def test_delete_then_recreate_gets_a_live_store(self, cryozen_home):
        import shutil

        import cryozen_state
        import cryozen_state_registry as registry
        from cryozen_constants import reset_cryozen_home_override, set_cryozen_home_override
        from cryozen_cli.goals import GoalState, _get_session_db, load_goal, save_goal
        ...
        with patch.object(cryozen_state, "DEFAULT_DB_PATH", cryozen_state._IMPORT_DEFAULT_DB_PATH):
            profile = cryozen_home / "profiles" / "p1"
            profile.mkdir(parents=True)
            token = set_cryozen_home_override(profile)
            try:
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, cryozen_home):
        from cryozen_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, cryozen_home):
        import pytest
        from cryozen_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, cryozen_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, cryozen_home):
        from unittest.mock import patch
        from cryozen_cli import goals
...
class TestStatusLineSubgoalCount:
    ...
    def test_status_line_with_subgoals(self, cryozen_home):
        from cryozen_cli.goals import GoalManager
...
class TestWaitBarrier:
    ...
    def test_parked_on_live_pid_does_not_continue_or_judge(self, cryozen_home):
        from cryozen_cli import goals
        from cryozen_cli.goals import GoalManager
    ...
    def test_wait_on_rejects_a_pid_not_alive_on_this_host(self, cryozen_home, monkeypatch):
    ...
    def test_stop_waiting_clears_barrier(self, cryozen_home):
        from cryozen_cli.goals import GoalManager
    ...
    def test_barrier_on_a_process_that_never_exits_expires(self, cryozen_home):
...
class TestJudgeDrivenWait:
    ...
    def test_judge_wait_on_dead_pid_continues_instead_of_parking(self, cryozen_home):
    ...
    def test_judge_wait_on_pid_dying_between_check_and_park_continues(self, cryozen_home):
    ...
    def test_judge_wait_pid_parks_loop(self, cryozen_home):
        from cryozen_cli import goals
        from cryozen_cli.goals import GoalManager
    ...
    def test_time_barrier_clears_after_deadline(self, cryozen_home):
        from cryozen_cli.goals import GoalManager
    ...
    def test_continue_verdict_still_continues_with_background(self, cryozen_home):
...
class TestSessionTriggerBarrier:
    ...
    def test_registry_releases_on_watch_match_while_alive(self, cryozen_home):
    ...
    def test_wait_on_session_validation(self, cryozen_home):
        from cryozen_cli.goals import GoalManager
...
class TestParseContract:
    ...
    def test_inline_fields_parsed(self):
        from cryozen_cli.goals import parse_contract
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from cryozen_cli.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from cryozen_cli.goals import GoalContract
...
class TestGoalManagerContract:
    ...
    def test_set_contract_after_the_fact(self, cryozen_home):
        from cryozen_cli.goals import GoalManager, GoalContract
        ...
        # Survives reload.
        from cryozen_cli.goals import GoalManager as GM2
        assert GM2(session_id="c-after").has_contract()

    def test_persistence_roundtrip(self, cryozen_home):
        from cryozen_cli.goals import GoalManager, GoalContract
...
class TestJudgeWithContract:
    ...
    def test_judge_uses_contract_template(self, cryozen_home):
        from unittest.mock import patch
        from cryozen_cli import goals
        from cryozen_cli.goals import GoalContract
...
class TestDraftContract:
    def test_draft_parses_json(self, cryozen_home):
        from unittest.mock import patch
        from cryozen_cli import goals
    ...
    def test_draft_returns_none_when_no_client(self, cryozen_home):
        from unittest.mock import patch
        from cryozen_cli import goals
...
class TestContractAndBackgroundCompose:
    ...
    def test_judge_prompt_carries_contract_and_background(self, cryozen_home):
        from unittest.mock import patch
        from cryozen_cli import goals
        from cryozen_cli.goals import GoalContract
...
class TestBlockedVerdict:
    ...
    def test_blocked_verdict_pauses_goal_instead_of_done(self, cryozen_home):
        from unittest.mock import patch
        from cryozen_cli.goals import GoalManager
...
def test_goal_session_db_is_the_registry_shared_handle(cryozen_home):
