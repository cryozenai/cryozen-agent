...
``cryozen update`` / ``cryozen dashboard --stop`` fall back to ``_kill_pids_posix`` for a
...
# Chat TUI): `cryozen dashboard --stop` / `cryozen update` must not SIGTERM their own process.
_STOPPER_CHILD = textwrap.dedent(
    """
    import os, pathlib, sys
    from cryozen_cli import dashboard_procs as dp
