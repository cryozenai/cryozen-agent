"""Guards for cryozen_cli._startup_fast — the pre-import version fast path.
...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from cryozen_cli.config import ...`` silently makes `cryozen --version`
...
def test_cli_starts_from_a_deleted_cwd(tmp_path):
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "cryozen_cli.main", "--version"],
            capture_output=True, text=True, timeout=60, env=env,
            cwd=REPO_ROOT, preexec_fn=lambda: os.fchdir(fd))
    finally:
        os.close(fd)
    assert result.returncode == 0, result.stderr
    assert "Cryozen Agent v" in result.stdout
...
def test_startup_fast_import_weight():
    ...
    probe = (
        "import sys, json\n"
        "import cryozen_cli._startup_fast\n"
    ...
    assert not offenders, (
        f"cryozen_cli._startup_fast imported heavy modules: {offenders} — "
...
def test_fast_version_parity_off_termux(tmp_path):
    home = tmp_path / ".cryozen-agent"
    home.mkdir()
    result = _run_version({"CRYOZEN_HOME": str(home), "TERMUX_VERSION": ""})
    assert result.returncode == 0, result.stderr
    out = result.stdout
    for field in ("Cryozen Agent v", "Install directory:", "Python:", "OpenAI SDK:"):
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version(
        {"CRYOZEN_HOME": str(home), "TERMUX_VERSION": "0.118"}
    )
    assert result.returncode == 0, result.stderr
    assert "Cryozen Agent v" in result.stdout
...
def test_literal_tilde_cryozen_home_expands_before_any_reader(tmp_path):
    """A literal ``~`` in CRYOZEN_HOME (fish, or any quoted value) is expanded at process entry.
    ...
    fake_home.mkdir()
    cwd.mkdir()
    env = {**os.environ, "HOME": str(fake_home), "USERPROFILE": str(fake_home), "CRYOZEN_HOME": "~/.x"}
    env.pop("CRYOZEN_DEV", None)
    env["PYTHONPATH"] = str(REPO_ROOT)
    result = subprocess.run(
        [sys.executable, "-m", "cryozen_cli.main", "config", "path"],
    ...
    # Raw-reader observable: the ~30 ``os.environ["CRYOZEN_HOME"]`` readers in cryozen_cli/ never
    # call the resolver, so the entry-point hunk in main.py (not cryozen_constants) must have
    # rewritten the env var by the time the module import finishes.
    probe = subprocess.run(
        [sys.executable, "-c", "import cryozen_cli.main, os; print(os.environ['CRYOZEN_HOME'])"],
        capture_output=True, text=True, timeout=120, cwd=cwd, env=env,
    )
...
def test_normalize_cryozen_home_env_rewrites_tilde_and_leaves_absolute_alone(tmp_path, monkeypatch):
    from cryozen_cli import _startup_fast

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("USERPROFILE", str(tmp_path))
    monkeypatch.setenv("CRYOZEN_HOME", "~/.x")
    _startup_fast.normalize_cryozen_home_env()
    assert os.environ["CRYOZEN_HOME"] == str(tmp_path / ".x")

    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "abs"))
    _startup_fast.normalize_cryozen_home_env()
    assert os.environ["CRYOZEN_HOME"] == str(tmp_path / "abs")

    monkeypatch.delenv("CRYOZEN_HOME")
    _startup_fast.normalize_cryozen_home_env()
    assert "CRYOZEN_HOME" not in os.environ
