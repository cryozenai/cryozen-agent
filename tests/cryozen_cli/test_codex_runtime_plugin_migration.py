...
from cryozen_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_cryozen_tools_mcp_entry,
    _format_toml_value,
...
# ---- end-to-end migrate(, expose_cryozen_tools=False) ----
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from cryozen_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_cryozen_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(cryozen_cfg, codex_home=tmp_path, expose_cryozen_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Cryozen' managed block, re-running
        ...
        migrate({"mcp_servers": {"cryozen-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.cryozen-mcp]" in final
    ...
    def test_summary_reports_migration_count(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_cryozen_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``cryozen codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "cryozen_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_cryozen_tools=False)
...
# ---- Bug C: CRYOZEN_HOME tempdir leak into ~/.codex/config.toml ----


class TestCryozenHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_cryozen_tools_mcp_entry()`` read ``CRYOZEN_HOME``
    ...
    once codex spawned the cryozen-tools MCP subprocess.
    """




    def test_real_cryozen_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate CRYOZEN_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.cryozen-agent"
        monkeypatch.setenv("CRYOZEN_HOME", real_path)
        entry = _build_cryozen_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("CRYOZEN_HOME") == real_path

    def test_unset_cryozen_home_omits_env_key(self, monkeypatch):
        """When CRYOZEN_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever CRYOZEN_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_cryozen_tools_mcp_entry()
        env = entry.get("env", {})
        assert "CRYOZEN_HOME" not in env, (
            f"CRYOZEN_HOME should not be set when env var is unset, got: "
            f"{env.get('CRYOZEN_HOME')!r}"
        )
...
class TestSameNameUserMcpTable:
    """Issue #79023: a Cryozen server whose name the user already declares outside the managed
    ...
    def test_user_table_wins_and_output_stays_valid_toml(self, tmp_path):
        ...
        report = migrate(
            ...
            codex_home=tmp_path, discover_plugins=False, expose_cryozen_tools=False,
            default_permission_profile=None)
    ...
    def test_unloadable_existing_config_explains_plugin_rerun(self, tmp_path, monkeypatch):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=True, expose_cryozen_tools=False)
        assert "re-run `cryozen codex-runtime migrate` to migrate plugins" in (report.plugin_query_error or "")
    ...
    def test_cli_migrate_dry_run_json_reports_without_writing(self, tmp_path, monkeypatch, capsys):
        """`cryozen codex-runtime migrate --dry-run --json` is the supported automation seam:
        drive it through the real ``cryozen`` argparse tree so the subcommand registration in
        cryozen_cli/main.py stays pinned, and honour ``CODEX_HOME`` like every codex sibling."""
        import json

        import cryozen_cli.main as main
