"""Fixtures shared across cryozen_cli tests."""
...
def all_assignees_spawnable(monkeypatch):
    """Pretend every assignee maps to a real Cryozen profile.
...
def _suppress_concurrent_cryozen_gate(request, monkeypatch):
    """Default ``_detect_concurrent_cryozen_instances`` to ``[]`` for every test.

    The Windows update path now refuses to proceed when another
    ``cryozen.exe`` is detected (issue #26670). On a developer's Windows
    machine running the test suite via ``cryozen`` itself, this would
    flag the running agent as a concurrent instance and abort every
    ``cmd_update`` test. Tests that want to exercise the gate explicitly
    re-patch ``_detect_concurrent_cryozen_instances`` with their own
    ...
    if request.node.get_closest_marker("real_concurrent_gate"):
        return
    try:
        from cryozen_cli import main as _cli_main
    except Exception:
        return
    # raising=False: under pytest's per-test spawn isolation, a concurrent
    # xdist worker importing a module that transitively touches cryozen_cli.main
    # can briefly expose a partially-initialized module object here — one where
    # _detect_concurrent_cryozen_instances isn't defined yet. A bare setattr
...
def _inline_post_swap_handoff(request, monkeypatch):
    """Run the post-swap tail in-process instead of re-executing ``cryozen update --post-swap``.
    ...
    if request.node.get_closest_marker("real_post_swap_handoff"):
        return
    try:
        from cryozen_cli import update_cmd, update_receipt
    except Exception:
        return
...
def _discharge_host_update_obligation():
    """Start and end every ``cryozen_cli`` test with NO host update-restart obligation.

    The record is host-scoped on purpose (one multiplexer per host), so it lives in the
    per-OS-USER host state dir — not in the per-test ``CRYOZEN_HOME``. The root conftest pins
    that dir per test only when the caller supplied no ``CRYOZEN_GATEWAY_LOCK_DIR`` (#118097
...
def isolated_update_runtime(monkeypatch, tmp_path, request):
    ...
    from cryozen_cli import gateway, main, update_cmd, update_cmd_fleet
    from cryozen_cli import update_inventory, update_receipt
