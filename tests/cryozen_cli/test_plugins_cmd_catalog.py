"""Catalog-aware ``cryozen plugins`` surface (cryozen_cli/plugins_cmd_catalog.py): a bare catalog name installs
...
from cryozen_cli import plugin_catalog as pc_cat
from cryozen_cli import plugins_cmd as pc
from cryozen_cli import plugins_cmd_catalog as cat
...
def test_catalog_platform_mismatch_refuses_before_install(world, monkeypatch):
    ...
    monkeypatch.setattr("cryozen_platform.host.facts.os_family", lambda: "darwin")
...
def test_owner_repo_hash_subdir_shorthand_resolves_like_the_catalog_spelling():
    from cryozen_cli.plugins_cmd import _resolve_git_url
    assert _resolve_git_url("plastic-labs/honcho#cryozen-plugin-honcho") == (
        "https://github.com/plastic-labs/honcho.git", "cryozen-plugin-honcho")
...
def test_in_tree_sidecar_cannot_forge_catalog_provenance(world, tmp_path):
    ...
    its own .cryozen-catalog.json is not a catalog install and does not mark the real entry installed."""
...
def test_kill_list_covers_update_enable_and_load_of_an_installed_plugin(world, tmp_path, monkeypatch):
    ...
    from cryozen_cli.plugins_discovery import gate_manifest
