...
config passthrough), a silent operator footgun that only manifests in
long-running setups.
"""
...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh CRYOZEN_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_cli_passthrough_")
    os.makedirs(os.path.join(test_home, "profiles", "default"), exist_ok=True)
    monkeypatch.setenv("CRYOZEN_HOME", test_home)
    for mod in list(sys.modules.keys()):
        if mod.startswith("cryozen_cli") or mod.startswith("cryozen_state") or mod == "cryozen_constants":
            del sys.modules[mod]
    yield test_home


def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: cryozen kanban dispatch must pass kanban.max_in_progress from
    ...
    monkeypatch.setattr(
        "cryozen_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.config.load_config", lambda: fake_config)
