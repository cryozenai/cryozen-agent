...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "cryozen_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # CRYOZEN_HOME.  For a standard install (CRYOZEN_HOME unset) that root is
        # the platform-native default (~/.cryozen-agent), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from cryozen_constants import get_default_cryozen_root
        assert env.get("CRYOZEN_HOME") == str(get_default_cryozen_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (CRYOZEN_DESKTOP=1) must NOT
