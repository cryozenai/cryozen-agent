"""Tests for the top-level `./cryozen` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./cryozen` should use `cryozen_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "cryozen"
    called = []

    fake_main_module = types.ModuleType("cryozen_cli.main")

    def fake_main():
        called.append("cryozen_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "cryozen_cli.main", fake_main_module)
    ...
    assert called == ["cryozen_cli.main"]
