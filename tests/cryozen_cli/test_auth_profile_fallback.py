...
When ``CRYOZEN_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.cryozen-agent/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.cryozen-agent            (has its own auth.json fixture)
    * Profile     -> tmp_path/.cryozen-agent/profiles/coder   (active, CRYOZEN_HOME points here)
    ...
    global_root = tmp_path / ".cryozen-agent"
    global_root.mkdir()
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "openai-codex": {"access_token": "codex-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("openai-codex")
    assert state is not None
    assert state["access_token"] == "codex-global"
...
#
# Several runtime helpers call ``_load_provider_state`` directly with a
# profile-loaded auth store rather than going through
# ``get_provider_auth_state``. Without the fallback wired into
# ``_load_provider_state`` itself, those helpers report "not logged in" even
# though the user has a valid global login. These tests pin the per-provider
# shadowing into the helper.
# ---------------------------------------------------------------------------
...
def test_codex_cooldown_clear_writes_to_the_store_that_owns_the_borrowed_pool(profile_env):
    ...
    from cryozen_cli.auth_codex import clear_codex_pool_quota_cooldowns
...
def test_root_write_through_is_visible_to_the_next_fallback_read(profile_env):
    ...
    import os
    from cryozen_cli.auth import _save_auth_store, read_credential_pool
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_cryozen_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (CRYOZEN_HOME != ~/.cryozen-agent, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    cryozen_home = tmp_path / "classic"
    cryozen_home.mkdir()
