...
leaked shell variable, so ``scripts/run_tests.sh``'s ``env -i`` was no help:
  ...
  1. A test drives the ``voice.toggle`` RPC with ``action="tts"``. The
     ...
     ``os.environ["CRYOZEN_VOICE_TTS"] = "1"``.
  ...
  3. Any later test that drives a turn to completion hits the TTS dispatch in
     ``prompt.submit``, which calls ``cryozen_cli.voice.speak_text`` on a
...
Two independent defences in ``tests/conftest.py``, one test each below:
  ...
  * ``_CRYOZEN_BEHAVIORAL_VARS`` now blanks ``CRYOZEN_VOICE`` /
    ``CRYOZEN_VOICE_TTS`` at every test setup, so step 2 cannot cross a test
    boundary.
...
def test_voice_toggle_still_leaks_the_env_var_but_speech_is_stubbed(monkeypatch):
    ...
    assert os.environ.get("CRYOZEN_VOICE_TTS") == "1"
    ...
    # calls it (a late `from cryozen_cli.voice import speak_text`), with the
...
def test_voice_env_does_not_leak_into_the_next_test():
    ...
    one above, in the same process, which left ``CRYOZEN_VOICE_TTS=1`` set.
    Before ``CRYOZEN_VOICE``/``CRYOZEN_VOICE_TTS`` were added to
    ``_CRYOZEN_BEHAVIORAL_VARS``, this assertion failed.
    """
    assert "CRYOZEN_VOICE_TTS" not in os.environ
    assert "CRYOZEN_VOICE" not in os.environ
...
def test_guard_can_be_opted_out_of_explicitly():
    ...
    import cryozen_cli.voice as voice
