...
Covers:
- Defense 1: gateway stop/restart refuse when _CRYOZEN_GATEWAY=1
...
from cryozen_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `python*` carries no cryozen/gateway token yet takes it down — on every platform.
        ...
        "pkill -9 python3",
        "pkill -f python",
        'pkill -f "python -m cryozen_cli.main"',
        "pkill -f cryozen_cli",
        "killall -9 python3.12",
        "sudo pkill -9 python3",
        "pgrep python | xargs kill -9",
        "kill $(pidof python3)",
        # Windows spellings of the cryozen-gateway forms (salvaged from #94379).
        "cryozen.exe gateway restart",
        "cryozen.cmd gateway stop",
        r'"C:\Program Files\cryozen.exe" gateway restart',
        "taskkill /F /IM cryozen-gateway.exe",
        "Stop-Process -Name cryozen-gateway -Force",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        # Unrelated scripts that merely contain "cryozen" cannot match the gateway cmdline.
        "pkill -f 'cryozen-polis/run.sh'",
        "pkill -f my_cryozen_bot.py",
        "pgrep python",
        "cryozen.exe gateway start",
        "my-cryozen.exe gateway restart",
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.cryozen.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.cryozen-agent/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l cryozen-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.cryozen.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.cryozen.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.cryozen.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.cryozen-agent/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.cryozen.gateway-apollo:/a.plist' "
            "'ai.cryozen.gateway-cronus:/c.plist' 'ai.cryozen.gateway-plutus:/p.plist' "
            "'ai.cryozen.gateway:/Users/botuser/Library/LaunchAgents/ai.cryozen.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated cryozen note on the next line"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.cryozen.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.cryozen.gateway',
        "launchctl kick'start' -k gui/501/ai.cryozen.gateway",
        "launchctl kick\\start -k gui/501/ai.cryozen.gateway",
        'launchctl "kickstart" -k gui/501/ai.cryozen.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.cryozen.gateway',
        "launchctl dis\\able gui/501/ai.cryozen.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.cryozen."gateway"',
        # Same class on the systemctl and cryozen-CLI branches.
        'systemctl re"start" cryozen-gateway',
        'cryozen gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.cryozen.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.cryozen.update-checker',
        'systemctl re"start" cryozen-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "cryozen cron list",
        "cryozen update",
        "cryozen config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `cryozen gateway start` is benign — starting a gateway from inside a
        ...
        "cryozen gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway cryozen
        # services must not be falsely blocked (the old `.*cryozen` matched any
        # cryozen token).
        "launchctl unload ai.cryozen.update-checker.plist",
        "launchctl restart ai.cryozen.daemon",
        ...
        "systemctl restart cryozen-meta.service",
        "systemctl restart cryozen-cron-helper",
        ...
        "echo after the cryozen gateway restarted cleanly",
        "the cryozen gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "cryozen skill view gateway-notes && echo cryozen gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `cryozen` is a path component
        # there, not a command.
        "cat '/docs/cryozen gateway restart-notes.md'",
        "less /home/user/notes/cryozen gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "cryozen gateway restart; echo done",
        "cryozen gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;cryozen gateway restart",
        "true && cryozen gateway stop",
        "echo $(cryozen gateway restart)",
        "echo `cryozen gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.cryozen.gateway"])',
        'subprocess.run(["cryozen", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.cryozen.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["cryozen", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: cryozen gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `cryozen -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry CRYOZEN_HOME/CRYOZEN_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("CRYOZEN_PROFILE", "zeus")
        monkeypatch.delenv("CRYOZEN_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "cryozen -p zeus gateway stop",
        "cryozen -p zeus gateway restart",
        "cryozen --profile zeus gateway restart",
        "cryozen --profile zeus gateway stop",
        "cryozen --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "cryozen -v -p zeus gateway restart",
        "cryozen -p zeus -v gateway restart",
        "cryozen --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "cryozen -p venus gateway stop",
        "cryozen -p venus gateway restart",
        "cryozen --profile venus gateway restart",
        "cryozen --profile=venus gateway stop",
        "cryozen -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "cryozen -p zeus gateway start",
        "cryozen -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("cryozen gateway restart")
        assert _contains_gateway_lifecycle_command("cryozen gateway stop")

    def test_cryozen_home_derived_profile(self, monkeypatch):
        # Without CRYOZEN_PROFILE the guard falls back to the CRYOZEN_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("CRYOZEN_PROFILE", raising=False)
        monkeypatch.delenv("CRYOZEN_PROFILE_NAME", raising=False)
        import cryozen_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("cryozen -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("cryozen -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("cryozen -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("cryozen gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_cryozen_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade cryozen then run cryozen gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_cryozen_gateway_once.sh). The script must live under
        # CRYOZEN_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / ".cryozen-agent"))
        scripts_dir = tmp_path / ".cryozen-agent" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_CRYOZEN_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _CRYOZEN_GATEWAY=1.

    Issue #37453: systemctl --user restart cryozen-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart cryozen-gateway",
        "systemctl --user restart cryozen-gateway",
        "systemctl stop cryozen-gateway.service",
        "cryozen gateway restart",
        "cryozen gateway uninstall",
        "launchctl kickstart gui/501/ai.cryozen.gateway",
        "launchctl bootout gui/501/ai.cryozen.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.cryozen.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.cryozen-agent/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.cryozen.gateway.restart-once.plist",
        "pkill -f cryozen.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart cryozen-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.cryozen.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-cryozen label: label-independent detection is the point
        # (#62891 second reproduction used `ai.cryozen.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.cryozen.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _CRYOZEN_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _CRYOZEN_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_CRYOZEN_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="cryozen gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["cryozen gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.cryozen.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.cryozen.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-cryozen systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run cryozen gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("cryozen gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfecryozen gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under CRYOZEN_HOME/scripts (the
    ...
    def test_python_script_with_pathlib_division_not_blocked(self, tmp_path):
        ...
        ".cryozen-agent") must NOT be blocked.
        ...
        script.write_text(
            ...
            'ENV = Path.home() / ".cryozen-agent" / ".env"\n'
            'print("digest ok")\n'
        )
        check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("cryozen gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/cryozen\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 cryozen gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_cryozen_home()` falls back to Path.home(), which raises
        RuntimeError when neither CRYOZEN_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart cryozen-gateway' .",
        "grep -rn 'cryozen gateway restart' ./logs",
        "rg 'cryozen gateway restart' ../archive",
        "grep -c 'systemctl stop cryozen-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM cryozen_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell cryozen gateway restart"',
        'sqlite3 ./db ".system systemctl restart cryozen-gateway"',
        'psql ./x -c "\\! systemctl restart cryozen-gateway"',
        "grep -r 'cryozen gateway restart' . | sh",
        "grep -r 'cryozen gateway restart' ./logs | bash",
        "grep -r 'cryozen gateway restart' . | sudo sh",
        "grep -r 'x' . ; cryozen gateway restart",
        "grep -r 'x' . && systemctl restart cryozen-gateway",
        'grep -r "$(cryozen gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart cryozen-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell cryozen gateway restart"',
        'sqlite3 db ".system systemctl restart cryozen-gateway"',
        'psql -c "\\! systemctl restart cryozen-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    cryozen_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run cryozen gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / ".cryozen-agent"))
        (tmp_path / ".cryozen-agent").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run cryozen gateway restart nightly",
        ))
...
class TestTerminalToolGatewayLifecycleGuardRemote:
    ...
    def test_unscannable_executed_script_names_the_reason(self, monkeypatch, tmp_path):
        ...
        import tools.terminal_tool as tt
        from cryozen_cli.sqlite_safe_read import connect_tracked
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop cryozen-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart cryozen-gateway' /var/log/syslog",
        "rg 'cryozen gateway restart' /home/user/.cryozen-agent/logs/",
        "journalctl -u cryozen-gateway --grep 'systemctl restart cryozen-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'cryozen_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell cryozen gateway restart"',
        'psql -c "\\! systemctl restart cryozen-gateway"',
        "grep 'systemctl restart cryozen-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart cryozen-gateway",
        'grep "$(systemctl restart cryozen-gateway)" f',
        "grep 'restart' log; systemctl restart cryozen-gateway",
        'sqlite3 db "SELECT 1"; cryozen gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "cryozen gateway restart",
        "sudo systemctl stop cryozen-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart cryozen-gateway%'\""
        )
