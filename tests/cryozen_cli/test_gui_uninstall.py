"""Tests for cryozen_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import cryozen_cli.gui_uninstall as gu


def _make_agent(cryozen_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = cryozen_home / "cryozen-agent"
    (agent_root / "cryozen_cli").mkdir(parents=True)
    (agent_root / "cryozen_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(cryozen_home: Path) -> None:
    """Create the source-built GUI artifacts a `cryozen desktop` run produces."""
    desktop = cryozen_home / "cryozen-agent" / "apps" / "desktop"
    ...
    (cryozen_home / "cryozen-agent" / "node_modules").mkdir(parents=True)
    (cryozen_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(cryozen_home: Path) -> None:
    (cryozen_home / "config.yaml").write_text("x: 1\n")
    (cryozen_home / ".env").write_text("KEY=secret\n")
    (cryozen_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(cryozen_home)
    _make_gui_build(cryozen_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(cryozen_home)
    ...
    assert summary["cryozen_home"] == str(cryozen_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `cryozen desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    cryozen_home = tmp_path / ".cryozen-agent"
    _make_agent(cryozen_home)
    icon = lde.icon_path(cryozen_home / "cryozen-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(cryozen_home)
    ...
    assert lde.icon_path(cryozen_home / "cryozen-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (cryozen_home / "cryozen-agent" / "cryozen_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".cryozen-agent")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import cryozen_cli.uninstall as uninstall
