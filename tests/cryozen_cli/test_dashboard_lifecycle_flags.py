"""Tests for ``cryozen dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``cryozen update``
...
from cryozen_cli.main import cmd_dashboard
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("cryozen_cli.dashboard_procs._scan_dashboard_processes", return_value=[]), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "No cryozen dashboard or serve processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        # couldn't see (#81564).
        processes = [
            (12345, "cryozen dashboard --port 9119"),
            (12346, "python -m cryozen_cli.main dashboard --host 0.0.0.0 --port 9120"),
            (12347, "cryozen serve --host 100.94.65.93 --port 9119"),
        ]
        with patch("cryozen_cli.dashboard_procs._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("cryozen_cli.main_dashboard._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        # Status is informational — always exits 0.
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "3 cryozen dashboard/serve process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("cryozen_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda **_: next(scans)), \
             patch("cryozen_cli.dashboard_procs._kill_stale_dashboard_processes",
        ...
        # for the post-`cryozen update` path).
        kwargs = mock_kill.call_args.kwargs
        assert "reason" in kwargs
        assert "stop" in kwargs["reason"].lower()
        assert exc.value.code == 0

    def test_stop_scopes_scan_and_kill_to_the_invoking_cryozen_home(self, tmp_path, monkeypatch, capsys):
        ...
        with patch("cryozen_cli.main._find_stale_dashboard_pids", return_value=[12345]) as scan, \
             patch("cryozen_cli.dashboard_procs._kill_stale_dashboard_processes",
        ...
        with patch("cryozen_cli.main._find_stale_dashboard_pids", return_value=[]), \
             patch("cryozen_cli.dashboard_procs._kill_stale_dashboard_processes") as kill, \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("cryozen_cli.main._find_stale_dashboard_pids", return_value=[12345]), \
             patch("cryozen_cli.dashboard_procs._kill_stale_dashboard_processes",
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``cryozen dashboard --stop`` must not end up ALSO starting
    a new server."""
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("cryozen_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"cryozen_cli.web_server": fake_ws}), \
             pytest.raises(SystemExit):
            cmd_dashboard(_ns(stop=True))
...
class TestArgparseWiring:
    ...
    ``cryozen dashboard --stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from cryozen_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("cryozen_cli.main")
