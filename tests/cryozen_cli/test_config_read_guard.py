...
Canonical owners:
  ...
  * ``cryozen_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._cryozen_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/cryozen/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "cryozen_cli/managed_scope.py",
