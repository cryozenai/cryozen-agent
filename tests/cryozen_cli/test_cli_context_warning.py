...
def _isolate(tmp_path, monkeypatch):
    """Isolate CRYOZEN_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal CryozenCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("cli.get_tool_definitions", return_value=[]), \
         patch("cryozen_cli.banner.build_welcome_banner"):
        from cli import CryozenCLI
        obj = CryozenCLI.__new__(CryozenCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["cryozen-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Cryozen' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Cryozen' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Cryozen' minimum context length."""
