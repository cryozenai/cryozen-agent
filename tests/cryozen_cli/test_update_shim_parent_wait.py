"""A child spawned off the Windows ``cryozen.exe`` shim waits for its parent before touching the venv.
...
from cryozen_cli import main_install_repair, update_handoff, update_lock
...
import psutil
from cryozen_cli import main as cli_main, update_cmd, update_handoff, update_lock
...
Path(os.environ["CRYOZEN_TEST_READY"]).touch()
try:
...
def test_update_child_outwaits_shim_parent_then_owns_the_lock_and_the_resume_token(tmp_path):
    ...
    the child (a fresh ``cryozen update``) must not pause gateways, scan holders or claim the
    ...
    marker = cryozen_home / update_lock.MARKER_NAME
    marker.write_text(f"{parent.pid}\n{int(time.time())}\n", encoding="utf-8")
    env = {
        **os.environ, "CRYOZEN_HOME": str(cryozen_home), "CRYOZEN_TEST_READY": str(ready),
...
def test_shim_holder_is_the_launcher_ancestor_not_the_interpreter_it_ran():
    """``cryozen.exe`` spawns python.exe with the shim as argv[0] and exits only after reaping it,
