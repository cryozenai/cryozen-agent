"""Tests for the Speech-to-Text category in `cryozen tools` (tools_config).

Covers the STT provider picker rows, config writes (stt.provider),
the model picker catalog, config-only checklist exclusion,
...
from cryozen_cli.tools_config import (  # noqa: E402
...
class TestModelPicker:
    ...
    def test_configure_stt_model_defaults_to_current(self):
        ...
        with patch(
            "cryozen_cli.tools_config._prompt_choice", return_value=0
        ) as pc:
...
class TestPostSetup:
    def test_faster_whisper_in_post_setup_ready(self):
        from cryozen_cli.tools_config import _POST_SETUP_READY
