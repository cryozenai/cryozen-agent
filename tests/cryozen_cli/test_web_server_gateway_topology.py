...
from cryozen_cli.web_server_gateway import _collect_profile_gateway_topology, _profile_platform_ports
...
class TestStatusEndpointTopology:
    ...
    def _setup_client(self, monkeypatch, _isolate_cryozen_home):
        try:
    ...
    def test_status_merges_independent_profile_gateway_failures(self, monkeypatch):
        ...
        # them in as <profile>:<platform> so external health monitoring sees them.
        import cryozen_cli.profiles as profiles_mod
    ...
    def test_profile_names_and_mode_public_when_auth_gated(self, monkeypatch):
        ...
        # remote client reads /api/status over the network (a gated bind)
        ...
        try:
            ...
            # alongside cryozen_home / gateway_pid.
            assert "gateways" not in data
            assert "cryozen_home" not in data
