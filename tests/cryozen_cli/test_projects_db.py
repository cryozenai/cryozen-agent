"""Tests for the per-profile Projects store (cryozen_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Cryozen Agent", folders=["/tmp/cryozen"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "cryozen-agent"
    assert proj.name == "Cryozen Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/cryozen"
    assert [f.path for f in proj.folders] == ["/tmp/cryozen"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "cryozen-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' CRYOZEN_HOME.
