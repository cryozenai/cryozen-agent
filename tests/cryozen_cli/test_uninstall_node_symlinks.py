"""Tests for cryozen_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $CRYOZEN_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Cryozen-managed node dir.
"""
...
def _make_cryozen_node(cryozen_home: Path) -> Path:
    """Create a fake $CRYOZEN_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = cryozen_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    cryozen_home = fake_home / ".cryozen-agent"
    _make_cryozen_node(cryozen_home)
    ...
    removed = uninstall.remove_node_symlinks(cryozen_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    cryozen_home = fake_home / ".cryozen-agent"
    node_bin = _make_cryozen_node(cryozen_home)
