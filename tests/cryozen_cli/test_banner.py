...
def test_build_welcome_banner_title_falls_back_when_no_tag():
    ...
    raw = buf.getvalue()
    assert "Cryozen Agent v" in raw, "Version label missing from title"
...
def test_empty_model_shows_no_model_configured(tmp_path, monkeypatch):
    """The banner prints before credentials resolve, so ``model`` is empty on a fresh install:
    the red "no model configured" line tells the user how to fix it."""
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / ".cryozen-agent"))
    (tmp_path / ".cryozen-agent").mkdir()
    with (
        patch.object(model_tools, "check_tool_availability", return_value=([], [])),
