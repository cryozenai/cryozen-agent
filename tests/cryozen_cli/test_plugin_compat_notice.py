"""cryozen_cli.plugin_compat: detect plugins on old import paths, tell the user, disable after the date.

Kept with the compat layer (tests/cryozen_cli/test_compat_manifest_targets.py); deleted with it.
"""
...
from cryozen_cli import plugin_compat as pc

ROOT = Path(__file__).resolve().parent.parent.parent
pytestmark = pytest.mark.skipif(not (ROOT / "compat_manifest.json").exists(), reason="compat layer removed")

MANIFEST = {"tools.web_tools": {"normalize_url_for_request": "tools.url_safety.normalize_url_for_request"},
            "cryozen_cli.kanban_db": {"connect": "cryozen_cli.kanban_db_connect.connect"}}


@pytest.mark.parametrize("src, expect", [
    ("from tools.web_tools import normalize_url_for_request\n", ["tools.web_tools.normalize_url_for_request"]),
    ("import tools.web_tools\nx = tools.web_tools.normalize_url_for_request()\n", ["tools.web_tools.normalize_url_for_request"]),
    ("import tools.web_tools as wt\nwt.normalize_url_for_request()\n", ["tools.web_tools.normalize_url_for_request"]),
    ("from unittest.mock import patch\npatch('cryozen_cli.kanban_db.connect')\n", ["cryozen_cli.kanban_db.connect"]),
...
def test_scan_plugin_walks_dir_and_skips_tests(tmp_path):
    (tmp_path / "__init__.py").write_text("from tools.web_tools import normalize_url_for_request\n")
    (tmp_path / "sub").mkdir(); (tmp_path / "sub" / "m.py").write_text("import cryozen_cli.kanban_db as k\nk.connect()\n")
    (tmp_path / "tests").mkdir(); (tmp_path / "tests" / "t.py").write_text("from tools.web_tools import normalize_url_for_request\n")
...
def test_compat_report_only_external_plugins_with_hits(tmp_path, monkeypatch):
    ...
    bad = tmp_path / "bad"; bad.mkdir(); (bad / "__init__.py").write_text("from tools.web_tools import normalize_url_for_request\n")
    bundled = tmp_path / "bundled"; bundled.mkdir(); (bundled / "__init__.py").write_text("from tools.web_tools import normalize_url_for_request\n")
    report = pc.compat_report([_manifest("good", good), _manifest("bad", bad), _manifest("ours", bundled, "bundled")], force=True)
    assert list(report) == ["bad"] and report["bad"][0].old == "tools.web_tools.normalize_url_for_request"
...
def test_disable_only_after_the_date_and_not_when_allowed(tmp_path, monkeypatch):
    ...
    assert reason and pc.COMPAT_REMOVAL in reason and "cryozen plugins compat" in reason
...
def test_summary_lines_name_plugins_and_the_date():
    ...
    assert "cryozen plugins compat" in tail
...
def test_discovery_refreshes_report_file(tmp_path, monkeypatch):
    ...
    plugin = tmp_path / "plugins" / "oldpaths"; plugin.mkdir(parents=True)
    (plugin / "plugin.yaml").write_text("name: oldpaths\nversion: 0.1\ndescription: t\n")
    (plugin / "__init__.py").write_text("from tools.web_tools import normalize_url_for_request\ndef register(ctx):\n    pass\n")
    from cryozen_cli.plugins_manifest import PluginManifest
    real = PluginManifest(name="oldpaths", version="0.1", description="t", source="user", path=str(plugin))
    mgr = PluginManager(scope_key=str(tmp_path))
    ...
    (plugin / "__init__.py").write_text("from tools.url_safety import normalize_url_for_request\ndef register(ctx):\n    pass\n")
    mgr._refresh_plugin_compat_report([real])
    assert not (tmp_path / "r.json").exists()
...
def test_scan_root_never_falls_back_to_cwd(tmp_path, monkeypatch):
    ...
    (tmp_path / "stray.py").write_text("from tools.web_tools import normalize_url_for_request\n")
    ...
    (pkg / "__init__.py").write_text("from tools.web_tools import normalize_url_for_request\n")
...
def test_allow_override_requires_literal_true_and_notice_reports_it(monkeypatch):
    ...
    report = {"bad": [pc.Hit("x.py", 1, "tools.web_tools.normalize_url_for_request", "tools.url_safety.normalize_url_for_request")]}
