...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``cryozen_session_at`` / ``cryozen_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    Models the real-world bug — a session hits another OIDC provider first,
    which tries to reach its unreachable JWKS and raises ProviderError. The gate must keep trying the
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["cryozen_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
