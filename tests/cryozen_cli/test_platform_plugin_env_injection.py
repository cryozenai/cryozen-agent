...
The Desktop Gateway form and ``cryozen config`` render env fields from ``OPTIONAL_ENV_VARS``; before
...
def test_user_platform_plugins_inject_env_metadata_but_non_platforms_do_not(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(config_mod, "get_cryozen_home", lambda: home)
    for key in keys:
