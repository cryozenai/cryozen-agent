...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CRYOZEN_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CRYOZEN_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CRYOZEN_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from cryozen_cli.providers import CRYOZEN_OVERLAYS
        assert "bedrock" in CRYOZEN_OVERLAYS
