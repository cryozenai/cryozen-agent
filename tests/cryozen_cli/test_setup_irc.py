"""Tests for IRC gateway configuration via `cryozen setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Cryozen install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Cryozen install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#cryozen")
            monkeypatch.setenv("IRC_NICKNAME", "cryozen-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `cryozen setup gateway` experience with IRC present."""
