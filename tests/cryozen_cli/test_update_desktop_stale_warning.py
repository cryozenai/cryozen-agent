...
#88251: ``cryozen update`` treated a failed desktop pack as non-fatal, printed
...
import cryozen_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_constants.with_cryozen_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "cryozen_constants.display_cryozen_home", lambda: str(tmp_path), raising=False
    )
...
def test_lost_desktop_with_surviving_build_stamp_is_rebuilt(desktop_env, monkeypatch):
    ...
    forever; the build stamp under CRYOZEN_HOME survived and is the proof Desktop was installed here."""
    desktop_dir, calls = desktop_env
    monkeypatch.setattr(
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_cryozen_home", lambda: tmp_path)
