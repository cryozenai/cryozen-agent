"""``cryozen gateway migrate``: preflight verdicts, apply/rollback bookkeeping, and the update hook.

Service layer is faked through the module's ``_installed_services`` / ``_service_op`` seams (the same
shape ``cryozen gateway install`` tests use); the default gateway boot is faked by writing the
...
from cryozen_cli import gateway_migrate as gm
...
def fleet(tmp_path, monkeypatch):
    ...
    def _service_op(kind, system, verb, home, *, run_as_user=None):
        ...
        if verb == "start" and name != "default":
            # What the real `cryozen -p <name> gateway run` checks first: is a live multiplexer
        ...
        if verb == "uninstall":
            ...
            runtime.update({"pid": os.getpid(), "cryozen_home": str(root), "gateway_state": "running"})
    ...
    monkeypatch.setattr(status, "_read_process_cmdline", lambda pid: "cryozen gateway run")
    ...
    from cryozen_cli import gateway_migrate_guards as guards
...
def _name(home: Path) -> str:
    return cryozen_constants.profile_name_for_home(home) or "default"
...
def test_update_hook_refuses_to_cross_service_user_or_home_boundary(
    ...
    boundary (other service domain, other UNIX user, CRYOZEN_HOME outside profiles/). It prints the
...
def test_update_hook_still_migrates_same_user_same_scope_profiles_under_the_default_tree(fleet, capsys, monkeypatch):
    ...
    profiles/ (the shape `cryozen profile create` produces) still auto-migrates."""
    fleet.services["default"] = ("systemd", False)
...
def test_auto_multiplex_migration_false_opts_out_of_the_update_hook_but_not_the_explicit_command(fleet, capsys):
    ...
    left alone by ``cryozen update`` (no output, no ops, no flag flip), while the operator typing
    ...
    from cryozen_cli.gateway_migrate_guards import auto_migration_opted_out
    (fleet.root / "config.yaml").write_text(
        "model:\n  default: x\nauto_multiplex_migration: false\n", encoding="utf-8")
...
def test_explicit_migrate_with_no_standalone_secondaries_still_flips_flag_and_restarts_default(fleet, capsys, monkeypatch):
    ...
    def _detached(home):  # no service manager anywhere -> detached start writes the served record
        ...
        (fleet.root / "gateway.pid").write_text(json.dumps({"pid": os.getpid(), "cryozen_home": str(fleet.root)}))
        (fleet.root / "gateway_state.json").write_text(json.dumps({
            "pid": os.getpid(), "cryozen_home": str(fleet.root), "gateway_state": "running",
            "served_profiles": ["default", "coder", "ops"]}))
        return True
...
def test_opt_out_reads_effective_config_managed_false_wins_and_string_false_is_false(fleet, tmp_path, monkeypatch):
    ...
    from cryozen_cli import config as cfg
    ...
    monkeypatch.setenv("CRYOZEN_MANAGED_DIR", str(managed))
...
def test_unknown_default_system_principal_blocks_the_update_hook(fleet, tmp_path, monkeypatch, capsys):
    ...
    from cryozen_cli import gateway as gw
    from cryozen_cli.gateway_migrate_guards import auto_migration_blockers, gateway_identity
    unit_dir = tmp_path / "system"; unit_dir.mkdir()
    monkeypatch.setattr(gw, "_SYSTEM_UNIT_DIR", unit_dir)
    with gm._home_env(fleet.root):
...
def test_windows_task_detection_reads_both_the_task_and_the_startup_fallback(monkeypatch):
    """`cryozen gateway install` falls back to a Startup-folder entry when it cannot register a
