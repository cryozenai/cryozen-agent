...
from cryozen_cli.urllib_security import (
...
def test_cross_host_redirect_drops_arbitrary_credentials_on_wire():
    ...
    assert headers["user-agent"] == "cryozen-test"
    for name in (
        "authorization",
        "cookie",
...
def test_azure_catalog_probe_drops_api_key_and_bearer_on_redirect():
    from cryozen_cli import azure_detect
...
def _reset_https_context_cache():
    ...
    import cryozen_cli.urllib_security as urllib_security
...
def test_cryozen_owned_opener_uses_resolved_https_context(monkeypatch):
    import cryozen_cli.urllib_security as urllib_security
...
def test_resolved_https_context_prefers_configured_ca_bundle(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CRYOZEN_CA_BUNDLE", str(ca_bundle))
...
def test_invalid_ca_bundle_falls_back_to_certifi_on_macos(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CRYOZEN_CA_BUNDLE", str(missing_bundle))
    monkeypatch.setattr(urllib_security.sys, "platform", "darwin")
    monkeypatch.setattr(certifi, "where", lambda: "/certifi/cacert.pem")
    monkeypatch.setattr(ssl, "create_default_context", create_default_context)
...
def test_cryozen_owned_openers_parse_the_ca_bundle_once(monkeypatch, tmp_path):
    """Every Cryozen request builds an opener; the bundle must not be re-parsed each time."""
    import cryozen_cli.urllib_security as urllib_security
