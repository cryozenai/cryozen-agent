...
install``. A locked ``cryozen.exe`` makes the install exit 2, the handler
...
def test_windows_dep_failure_does_not_zip_fallback(monkeypatch):
    monkeypatch.setattr(cryozen_main, "_is_windows", lambda: True)
...
def _porcelain_run(stdout: str, returncode: int = 0):
    def fake_run(cmd, **kwargs):
        ...
        if "ls-tree" in joined:  # tracked root entries = what the ZIP ships
            return subprocess.CompletedProcess(cmd, 0, stdout="cryozen_cli\nscratch\n", stderr="")
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M cryozen_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    monkeypatch.setattr(cryozen_main, "PROJECT_ROOT", fake_root)
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.cryozen-update-staging/")
    assert is_artifact("?? cli.py.cryozen-update-staging")
    assert is_artifact("?? tools.cryozen-update-old/")
    ...
    assert not is_artifact(" M cryozen_cli/update_cmd.py")
    assert not is_artifact("?? notes.cryozen-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.cryozen-update-staging/\n?? cli.py.cryozen-update-old\n"),
    )
    assert (
