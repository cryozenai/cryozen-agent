...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_cryozen_home(), so
        # point CRYOZEN_HOME at the temp dir (the _cryozen_home import-time
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in CRYOZEN_HOME/config.yaml
    ...
    install whose CRYOZEN_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read CRYOZEN_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh CRYOZEN_HOME with NO config.yaml (managed/desktop first launch).
        cryozen_home = tmp_path / ".cryozen-agent"
        cryozen_home.mkdir()
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
