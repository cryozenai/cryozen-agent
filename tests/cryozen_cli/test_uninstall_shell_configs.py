...
This rewrites files Cryozen does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``CRYOZEN_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
...
class TestHappyPath:
    def test_cryozen_path_block_is_removed(self, fake_home: Path):
        rc = fake_home / ".zshrc"
        ...
        assert "# Cryozen Agent" not in text
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/CRYOZEN_HOME patches the fake_home fixture installed.
    ...
    def test_symlinked_shell_config_stays_a_symlink(self, fake_home: Path):
        ...
        assert "# Cryozen Agent" not in real.read_text(encoding="utf-8")
