...
def test_credentials_ready_false_when_no_provider(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", _raise)
...
def test_offer_first_run_setup_routes_into_shared_picker(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.main.select_provider_and_model", _fake_picker)
...
def test_offer_first_run_setup_re_resolves_reasoning_for_picked_model(monkeypatch):
    ...
    monkeypatch.setitem(cli.CLI_CONFIG, "agent", {
        **cli.CLI_CONFIG.get("agent", {}), "reasoning_effort": "medium",
        "reasoning_overrides": {"gpt-5.5": "high"}})
    shell = _make_shell(cli, monkeypatch)
    assert shell.reasoning_config["effort"] == "medium"
    monkeypatch.setattr("cryozen_cli.main.select_provider_and_model", lambda: None)
    monkeypatch.setattr("builtins.input", lambda *a, **k: "y")
...
def test_offer_first_run_setup_declined(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.main.select_provider_and_model", _fail_picker)
...
def test_offer_first_run_setup_picker_cancel_is_graceful(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.main.select_provider_and_model", _cancel_picker)
...
def test_empty_key_error_names_actual_provider(monkeypatch, capsys):
    ...
    assert "cryozen model" in out or "cryozen setup" in out
...
def _bench_oauth_pool(monkeypatch, **entry_fields):
    import time
    from agent.credential_pool import STATUS_EXHAUSTED, CredentialPool, PooledCredential

    benched = PooledCredential(id="e1", provider="openai-codex", auth_type="oauth", access_token="x",
                               refresh_token="r", label="device", source="manual:device_code",
...
def _forbid_wizard(monkeypatch, shell):
    monkeypatch.setattr("cryozen_cli.main.select_provider_and_model",
...
def test_benched_credential_prints_cooldown_instead_of_wizard(monkeypatch, capsys):
    ...
    def _raise(**kwargs):
        raise AuthError("Cryozen is not logged into OpenAI Codex.", provider="openai-codex",
                        code="codex_auth_missing", relogin_required=True)

    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", _raise)
    _bench_oauth_pool(monkeypatch, last_error_code=429, last_error_reason="rate_limited")
    ...
    assert "not logged into OpenAI Codex" in out
    assert "re-authenticate" not in out and "cryozen model" not in out


def test_auth_json_only_login_explains_instead_of_wizard(monkeypatch, capsys, tmp_path):
    """auth.json-only shape: logged into an OAuth provider but no ``model.provider`` (requested "auto"). The
    ...
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **kw: "openai-codex")
    monkeypatch.setitem(rp._OAUTH_RUNTIME_PROVIDERS, "openai-codex",
                        dataclasses.replace(rp._OAUTH_RUNTIME_PROVIDERS["openai-codex"], resolve=_oauth_fail))
    ...
    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", lambda **kw: (_ for _ in ()).throw(
        AuthError("Cryozen is not connected to any AI provider yet.", code="no_provider_configured")))
    shell._maybe_offer_first_run_setup()
    assert offered == [True]
