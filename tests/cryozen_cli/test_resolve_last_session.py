"""Verify `cryozen -c` picks the session the user most recently used."""
...
def test_search_sessions_exposes_last_active_column(tmp_path, monkeypatch):
    ...
    db = cryozen_state.SessionDB(db_path=Path(tmp_path / "state.db"))
    try:
...
def test_resolve_last_session_real_db_prefers_workspace(monkeypatch, tmp_path):
    ...
    real_db = cryozen_state.SessionDB
    ...
    monkeypatch.setattr(
        "cryozen_cli.main.subprocess.run",
        lambda cmd, **kw: __import__("subprocess").CompletedProcess(
            cmd, 0, stdout=str(repo_a), stderr=""
        ),
    )
    monkeypatch.setattr("cryozen_state.SessionDB", lambda **kw: real_db(db_path=state_db, **kw))
    assert _resolve_last_session("cli") == "repo_a"


def test_resolve_last_session_cli_continues_a_oneshot(monkeypatch, tmp_path):
    """`cryozen -z … --resume latest` / `cryozen -c` chain on the previous one-shot: its distinct `oneshot`
    ...
    monkeypatch.setattr("cryozen_cli.main._resolve_workspace_key", lambda: None)
    monkeypatch.setattr("cryozen_state.SessionDB", lambda **kw: real_db(db_path=state_db, **kw))
