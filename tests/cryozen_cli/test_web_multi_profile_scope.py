...
Regression for the cross-profile leak class in ``cryozen dashboard`` / ``cryozen serve``:
``_config_profile_scope`` bound only CRYOZEN_HOME, so ``GET /api/config?profile=B`` expanded B's
``${VAR}`` refs to the DEFAULT profile's plaintext credentials (its ``os.environ``), and console
``send`` for B (``send_cmd._load_cryozen_env``) copied B's ``.env`` into the shared process env with
...
def client(two_homes):
    from cryozen_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
    c = TestClient(app)
...
def test_console_send_for_named_profile_does_not_write_process_env(two_homes, monkeypatch):
    ...
    from cryozen_cli.web_routers.chat_ws import _execute_console_line

    root, b = two_homes
    seen = {}

    def fake_send(args):
        import cryozen_cli.send_cmd as send_cmd
        ...
        seen["home"] = Path(os.environ.get("CRYOZEN_HOME", ""))
        return '{"success": true}'
