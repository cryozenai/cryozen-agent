"""Tests for cryozen_cli.model_normalize — provider-aware model name normalization.
...
class TestDeepseekVSeriesPassThrough:
    ...
    aggregators (OpenRouter via DeepInfra) routes to V3 —
...
class TestColonProviderPrefixIsStrippedLikeSlash:
    """Issue #64787: ``-m openai-codex:gpt-5.6-sol`` (Cryozen's own ``provider:model`` switch syntax)
