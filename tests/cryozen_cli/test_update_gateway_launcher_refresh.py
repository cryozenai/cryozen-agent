...
2. ``cli_main._refresh_windows_gateway_launchers`` — ``cryozen
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "cryozen_cli.main", "gateway", "run"]
    with mock.patch.object(
        gateway_windows, "_stable_gateway_working_dir", return_value=str(tmp_path)
    ), mock.patch("cryozen_cli.config.get_cryozen_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: cryozen update regenerates launchers
# ---------------------------------------------------------------------------


def test_update_launcher_refresh_reregisters_drifted_scheduled_task(monkeypatch):
    """``cryozen update`` must not only rewrite the launcher scripts but also re-register a Scheduled
    ...
    assert reconciled == ["Cryozen_Gateway"]
