"""Regression tests: `cryozen dashboard` validates CRYOZEN_WEB_DIST before serving.

A custom CRYOZEN_WEB_DIST without --skip-build previously skipped BOTH the
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "cryozen_cli.profiles.get_active_profile_name", lambda: "default"
    )
    ...
    monkeypatch.setattr(
        "cryozen_cli.mcp_startup.start_background_mcp_discovery",
        lambda **_k: None,
    )


def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """CRYOZEN_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("CRYOZEN_WEB_DIST", str(empty_dist))
    ...
    builds = []
    monkeypatch.setattr(
        "cryozen_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
    ...
    assert "CRYOZEN_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("CRYOZEN_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "cryozen_cli" / "web_dist"
    ...
    monkeypatch.setattr("cryozen_cli.main_web_build._build_web_ui", fake_build)
...
def test_desktop_child_dashboard_drops_packaged_renderer(main_mod, monkeypatch):
    """#116107: every desktop-spawned process inherits CRYOZEN_DESKTOP=1 with the
    packaged dist; a browser `cryozen dashboard` from it must not keep serving the
    IPC-only desktop renderer."""
    packaged = "/Applications/Cryozen.app/Contents/Resources/app.asar.unpacked/dist"
    monkeypatch.setenv("CRYOZEN_DESKTOP", "1")
    monkeypatch.setenv("CRYOZEN_WEB_DIST", packaged)
    monkeypatch.setenv("CRYOZEN_SERVE_HEADLESS", "1")

    main_mod._dashboard_sanitize_desktop_env(headless_backend=False)

    assert "CRYOZEN_WEB_DIST" not in os.environ
    assert "CRYOZEN_SERVE_HEADLESS" not in os.environ
...
def test_desktop_headless_serve_keeps_packaged_renderer(main_mod, monkeypatch):
    ...
    assert os.environ["CRYOZEN_WEB_DIST"] == packaged
...
def test_desktop_owned_fallback_dashboard_keeps_packaged_renderer(main_mod, monkeypatch):
    ...
    packaged = "/Applications/Cryozen.app/Contents/Resources/app.asar.unpacked/dist"
    monkeypatch.setenv("CRYOZEN_DESKTOP", "1")
    monkeypatch.setenv("CRYOZEN_DASHBOARD_SESSION_TOKEN", "desktop-spawn-token")
    monkeypatch.setenv("CRYOZEN_WEB_DIST", packaged)
