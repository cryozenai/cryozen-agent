...
``cryozen*.exe`` shims aside (``cryozen.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``cryozen`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("cryozen", "cryozen-agent", "cryozen-acp", "cryozen-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "cryozen.exe" in names, "cryozen.exe must be restored after a no-op install"
    assert "cryozen-acp.exe" in names
    assert "cryozen-gateway.exe" in names
    assert (scripts / "cryozen.exe").read_bytes() == b"MZ-old-cryozen"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "cryozen.exe").read_bytes() == b"MZ-new-cryozen"
...
# ---------------------------------------------------------------------------
# cryozen_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "cryozen.exe" in names, "cryozen.exe must be restored after a no-op recovery install"
    assert (scripts / "cryozen.exe").read_bytes() == b"MZ-old-cryozen"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "cryozen.exe").write_bytes(b"MZ-new-cryozen")
