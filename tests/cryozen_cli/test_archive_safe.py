"""Tests for the shared tar.gz writer (``cryozen_cli.archive_safe.make_targz``).

``make_targz`` backs both ``cryozen profile export`` and ``cryozen kanban
...
from cryozen_cli.archive_safe import make_targz
