...
def home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_MULTIPLEX_PROFILES", "0")
    h = tmp_path / "cryozen"
    h.mkdir()
    return h
...
def test_same_file_reloaded_three_times_leaves_path_unchanged(home):
    ...
    seen = []
    for _ in range(3):
        load_cryozen_dotenv(cryozen_home=home, load_external_secrets=False)
...
def test_alternating_project_env_and_none_stays_stable(home, tmp_path):
    ...
    for i in range(6):
        load_cryozen_dotenv(
            cryozen_home=home, project_env=project_env if i % 2 else None, load_external_secrets=False
        )
...
def test_alternating_homes_each_resolve_against_boot_path(tmp_path, monkeypatch):
    ...
    for h in (home_a, home_b, home_a, home_b, home_a):
        load_cryozen_dotenv(cryozen_home=h, load_external_secrets=False)
...
def test_later_layer_in_the_same_load_still_sees_the_earlier_layer(home, tmp_path):
    """Within one load_cryozen_dotenv the project layer builds on the user layer, as before; the
    ...
    for _ in range(3):
        load_cryozen_dotenv(cryozen_home=home, project_env=project_env, load_external_secrets=False)
