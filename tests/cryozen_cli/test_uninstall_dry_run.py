...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_cryozen_home", lambda: cryozen_home)
    monkeypatch.setattr(uninstall, "_is_default_cryozen_home", lambda home: False)
    ...
    assert str(cryozen_home) in output
    assert project_root.exists()
    assert cryozen_home.exists()
