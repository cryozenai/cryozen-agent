"""ensure_cryozen_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".cryozen-agent"
    second = tmp_path / "b" / ".cryozen-agent"

    monkeypatch.setenv("CRYOZEN_HOME", str(first))
    cfg.ensure_cryozen_home()

    # Profile switch: CRYOZEN_HOME moves → the new path is ensured too.
