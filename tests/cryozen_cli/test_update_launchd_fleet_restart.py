"""Regression for #41403 — ``cryozen update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.cryozen.gateway-<profile>`` services kept running pre-update
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed CRYOZEN_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.cryozen.gateway",
            "ai.cryozen.gateway-merit-ops",
            "ai.cryozen.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(cryozen_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        gui_loaded = {"ai.cryozen.gateway-a"}
        ...
        assert _locate_launchd_gateway_service("ai.cryozen.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.cryozen.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.cryozen.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.cryozen.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.cryozen.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.cryozen.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.cryozen.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        # box with a live ai.cryozen.gateway* fleet would leak its PIDs into the assertion.
        monkeypatch.setattr(gw.subprocess, "run", lambda *a, **k: _completed(0, ""))
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.cryozen.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.cryozen.gateway", "ai.cryozen.gateway-a", "ai.cryozen.gateway-b"],
        )
        located = {
            "ai.cryozen.gateway": (f"gui/{UID}", 100),
            "ai.cryozen.gateway-a": (f"gui/{UID}", 200),
            "ai.cryozen.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through cryozen_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.cryozen.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.cryozen.gateway", current, "ai.cryozen.gateway-user-scoped"],
            located={
                "ai.cryozen.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.cryozen.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.cryozen.gateway",
            f"user/{UID}/ai.cryozen.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.cryozen.gateway",
            f"user/{UID}/ai.cryozen.gateway-user-scoped",
        ...
        assert restarted == [
            current,
            "ai.cryozen.gateway",
            "ai.cryozen.gateway-user-scoped",
        ]
        assert failed == []
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.cryozen.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.cryozen.gateway-a"],
            located={"ai.cryozen.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.cryozen.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.cryozen.gateway",
            labels=["ai.cryozen.gateway", "ai.cryozen.gateway-idle"],
            located={
                "ai.cryozen.gateway": (f"gui/{UID}", 100),
                "ai.cryozen.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.cryozen.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.cryozen.gateway",
            labels=["ai.cryozen.gateway", "ai.cryozen.gateway-dormant"],
            located={
                "ai.cryozen.gateway": (f"gui/{UID}", 100),
                "ai.cryozen.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.cryozen.gateway-dormant"]
        assert restarted == ["ai.cryozen.gateway", "ai.cryozen.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.cryozen.gateway",
            labels=["ai.cryozen.gateway", "ai.cryozen.gateway-a"],
            located={
                "ai.cryozen.gateway": (f"gui/{UID}", 100),
                "ai.cryozen.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.cryozen.gateway-a"]
        assert restarted == ["ai.cryozen.gateway", "ai.cryozen.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.cryozen.gateway",
                "ai.cryozen.gateway-bad",
                "ai.cryozen.gateway-good",
            ],
            located={
                "ai.cryozen.gateway": (f"gui/{UID}", 100),
                "ai.cryozen.gateway-bad": (f"gui/{UID}", 200),
                "ai.cryozen.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.cryozen.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.cryozen.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.cryozen.gateway-good"]
        assert restarted == ["ai.cryozen.gateway", "ai.cryozen.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.cryozen.gateway",
                "ai.cryozen.gateway-wedged",
                "ai.cryozen.gateway-after",
            ],
            located={
                "ai.cryozen.gateway": (f"gui/{UID}", 100),
                "ai.cryozen.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.cryozen.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.cryozen.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.cryozen.gateway-after"]
        assert restarted == ["ai.cryozen.gateway", "ai.cryozen.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.cryozen.gateway": (f"gui/{UID}", 100),
                "ai.cryozen.gateway-wedged": (f"gui/{UID}", 200),
                "ai.cryozen.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.cryozen.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.cryozen.gateway",
            labels=["ai.cryozen.gateway", "ai.cryozen.gateway-zombie"],
            located={
                "ai.cryozen.gateway": (f"gui/{UID}", 100),
                "ai.cryozen.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.cryozen.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.cryozen.gateway"]
        assert failed == ["ai.cryozen.gateway-zombie"]


def _write_launchd_plist(agents_dir: Path, label: str, *, argv=(), cryozen_home=None, raw=None):
    ...
    if cryozen_home is not None:
        data["EnvironmentVariables"] = {"CRYOZEN_HOME": str(cryozen_home)}
...
class TestLegacyLaunchdLabelsForInstall:
    ...
    from the plist's pinned CRYOZEN_HOME, fail-closed, so another install's fleet stays untouched
    (the #41403 boundary) and a sandboxed CRYOZEN_HOME never enumerates the account's real units."""
    ...
    def test_only_units_pinned_to_this_installs_homes_are_credited(self, monkeypatch, tmp_path):
        ...
        _write_launchd_plist(agents, "ai.cryozen.gateway-398559f7", argv=[venv_python], cryozen_home=root / "profiles" / "gopod")
        _write_launchd_plist(agents, "ai.cryozen.gateway-1a2b3c4d", argv=[venv_python], cryozen_home=root)
        _write_launchd_plist(agents, "ai.cryozen.gateway-gopod", argv=[venv_python], cryozen_home=root / "profiles" / "gopod")
        # Another install's unit: same venv on disk, but its home lives under a different root.
        _write_launchd_plist(agents, "ai.cryozen.gateway-5db8084b", argv=[venv_python], cryozen_home=tmp_path / "other-root" / "profiles" / "gopod")
        _write_launchd_plist(agents, "ai.cryozen.gateway-nohome", argv=[venv_python])  # no pinned home: unattributable
        _write_launchd_plist(agents, "ai.cryozen.gateway-broken", raw="not a plist at all")
        assert gw.legacy_launchd_labels_for_install(exclude={"ai.cryozen.gateway-gopod", "ai.cryozen.gateway"}) == [
            "ai.cryozen.gateway-1a2b3c4d", "ai.cryozen.gateway-398559f7",
        ]
...
class TestRestartMacosLaunchdGatewaysLegacyUnits:
    def test_legacy_labelled_sibling_is_restarted_like_any_other(self, monkeypatch, tmp_path, capsys):
        ...
        current = "ai.cryozen.gateway"
        legacy = "ai.cryozen.gateway-398559f7"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.cryozen.gateway-gopod"],
            legacy_labels=[legacy],
            located={
                current: (f"gui/{UID}", 100),
                "ai.cryozen.gateway-gopod": (f"gui/{UID}", 200),
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.cryozen.gateway-gopod",
            f"user/{UID}/{legacy}",
        ]
        assert restarted == [current, "ai.cryozen.gateway-gopod", legacy]
        assert failed == []
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.cryozen.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
    ...
    def test_returns_false_when_pid_never_changes(self, monkeypatch):
        ...
        assert not gw._wait_for_launchd_service_pid(
            "ai.cryozen.gateway-x", old_pid=200, timeout=3.0, domain=f"gui/{UID}"
        )
...
class TestIncompleteWarningOnMacos:
    ...
    def test_launchd_labels_get_bootstrap_hint(self, capsys):
        _warn_incomplete_gateway_fleet_restart(["ai.cryozen.gateway-merit-ops"])
        out = capsys.readouterr().out
