...
from cryozen_cli.tool_availability_notices import filter_to_enabled_toolsets, tool_availability_warning_lines
...
def test_terminal_line_names_backend_reason_and_fix_commands():
    ...
    assert "cryozen doctor" in text and "cryozen setup terminal" in text
...
def test_web_collapses_env_dump_into_one_provider_sentence():
    ...
    assert "Web search is off" in text and "cryozen setup tools" in text
...
def test_generic_toolset_lists_vars_and_setup_hint():
    ...
    text = "\n".join(lines)
    assert "rl" in text and "TINKER_API_KEY" in text and "cryozen setup" in text
...
def test_web_notice_cites_setup_tools_not_the_toggle_menu():
    # `cryozen tools` with no subcommand is the enable/disable menu; providers live under `cryozen setup tools`.
    lines = tool_availability_warning_lines([_WEB], terminal_reason=None, terminal_backend="local")
    text = "\n".join(lines)
    assert "cryozen setup tools" in text
    assert "cryozen tools[/]" not in text


class TestFilterToEnabledToolsets:
    _BUNDLE = {"cryozen-cli": ["terminal", "process_manage", "web_search", "read_file"],
    ...
    def test_composite_bundle_keeps_terminal_notice_on_default_install(self):
        kept = filter_to_enabled_toolsets([_TERMINAL, _WEB, _RL], ["cryozen-cli"], self._resolve)
    ...
    def test_resolver_error_is_not_fatal(self):
        ...
        assert filter_to_enabled_toolsets([_TERMINAL], ["cryozen-cli"], boom) == []
