"""Tests for the unified provider catalog (cryozen_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``cryozen model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from cryozen_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
