...
def _enable(cryozen_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<cryozen_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        cryozen_home = Path(os.environ["CRYOZEN_HOME"])  # set by hermetic conftest fixture
        user_plugins = cryozen_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(cryozen_home, "image_gen/openai")
...
class TestForeignHarnessManifestDirs:
    def test_foreign_harness_dirs_skipped_without_warnings(
        ...
        They must be skipped silently; the plugin's real Cryozen manifest
        (``.cryozen-plugin/plugin.yaml``) is still discovered."""
        import os
        cryozen_home = Path(os.environ["CRYOZEN_HOME"])  # set by hermetic conftest fixture
        sp = cryozen_home / "plugins" / "superpowers"
        (sp / ".cryozen-plugin").mkdir(parents=True)
        (sp / ".cryozen-plugin" / "plugin.yaml").write_text(
            yaml.dump(
                {
                    "name": "superpowers",
                    "version": "6.3.0",
        ...
        with caplog.at_level("WARNING", logger="cryozen_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()

        assert "superpowers/.cryozen-plugin" in mgr._plugins
        parse_warnings = [
    ...
    def test_broken_portable_plugin_still_warns(self, tmp_path, monkeypatch, caplog):
        ...
        import os
        cryozen_home = Path(os.environ["CRYOZEN_HOME"])  # set by hermetic conftest fixture
        broken = cryozen_home / "plugins" / "broken-portable"
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        cryozen_home = Path(os.environ["CRYOZEN_HOME"])  # set by hermetic conftest fixture
        _write_plugin(cryozen_home / "plugins", ["p1"])
        _enable(cryozen_home, "p1")
...
class TestBackendGate:
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(cryozen_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(cryozen_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(cryozen_home, "bad-img-plugin")
