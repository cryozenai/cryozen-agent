...
@pytest.fixture
def client(monkeypatch, tmp_path, _isolate_cryozen_home):
    try:
...
def _install(name: str, sidecar: dict | None):
    ...
    installer-owned ``.install-metadata.json`` record (an in-tree ``.cryozen-catalog.json`` is inert)."""
    from cryozen_constants import get_cryozen_home
    from cryozen_cli.plugins_cmd import _read_install_metadata, _write_install_metadata
    d = get_cryozen_home() / "plugins" / name
...
def test_install_endpoint_refuses_removed_plugins_with_no_bypass(client, monkeypatch):
    ...
    for body in ({"identifier": "https://github.com/evil/bad-plugin.git"},
        ...
        assert "removed" in resp.json()["detail"] or "not in the Cryozen plugin catalog" in resp.json()["detail"]
