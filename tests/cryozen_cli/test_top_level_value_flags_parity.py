...
option absent from both sets, so every ``cryozen --reasoning high <cmd> …``
invocation mis-classified ``high`` as the first positional and forced eager
plugin CLI discovery (~500-650ms startup tax).

Both sites now share ``cryozen_cli._parser.top_level_value_flag_sets()``,
...
from cryozen_cli.main import _first_positional_argv, _plugin_cli_discovery_needed
...
def test_first_positional_skips_reasoning_value(monkeypatch):
    monkeypatch.setattr("sys.argv", ["cryozen", "--reasoning", "high", "chat", "hello"])
    assert _first_positional_argv() == "chat"
    assert _plugin_cli_discovery_needed() is False


@pytest.mark.parametrize("argv", [
    ["cryozen", "--reasoning", "ultra", "--provider", "openai", "-z", "prompt here"],
    ["cryozen", "-m", "x", "--reasoning=low", "chat"],
])
