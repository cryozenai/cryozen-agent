"""Tests for CryozenCLI initialization -- catches configuration bugs
...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a CryozenCLI instance with minimal mocking."""
    import importlib
    ...
    clean_env = {"LLM_MODEL": "", "CRYOZEN_MAX_ITERATIONS": ""}
    if env_overrides:
    ...
    try:
        with patch.dict(sys.modules, prompt_toolkit_stubs), \
            ...
            with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), \
                ...
                return _cli_mod.CryozenCLI(**kwargs)
    finally:
...
class TestFallbackChainInit:
    def test_merges_new_and_legacy_fallback_config(self):
        cli = _make_cli(config_overrides={
            ...
            "fallback_model": {"provider": "openrouter", "model": "gpt-5"},
        })
...
class TestPromptToolkitTerminalCompatibility:
    ...
    def test_cpr_gating_posix_suppresses_without_ssh(self, monkeypatch):
        ...
        ``tests/cryozen_cli/test_cpr_local_leak.py`` under ``windows_only``, where it
...
class TestHistoryDisplay:
    ...
    def test_resume_without_target_lists_recent_sessions(self, capsys):
        ...
        cli._session_db.list_sessions_rich.return_value = [
            ...
            },
            {
                ...
                "title": "Checking Running Cryozen Agent",
                "preview": "check running gateways for cryozen agent",
                "last_active": 0,
            },
        ]
        ...
        assert "Checking Running Cryozen Agent" in output
    ...
    def test_sessions_with_target_delegates_to_resume(self):
        ...
        with patch.object(cli, "_handle_resume_command") as mock_resume:
            cli.process_command("/sessions Checking Running Cryozen Agent")

        mock_resume.assert_called_once_with(
            "/resume Checking Running Cryozen Agent"
        )
...
class TestNestedDictModelDefaultPairing:
    ...
    model AND the nested provider, so ``CryozenCLI`` routes the model through
    ...
    def test_nested_dict_default_keeps_provider_paired(self):
        cli = _make_cli(config_overrides={
            "model": {
                "default": {"provider": "openrouter", "model": "nested-default-model"},
                "provider": "auto",
            },
        })
    ...
    def test_provider_prefixed_startup_model_overrides_stale_provider(self):
        cli = _make_cli(
            ...
            model="localbox/deepseek-v4-pro",
        )

        assert cli.model == "deepseek-v4-pro"
        assert cli.requested_provider == "localbox"
...
class TestRootLevelProviderOverride:
    ...
    def test_model_provider_wins_over_root_provider(self, tmp_path, monkeypatch):
        ...
        import cli
        monkeypatch.setattr(cli, "_cryozen_home", cryozen_home)
...
class TestPluginToolsetStartupValidation:
    ...
    def _init_toolsets(monkeypatch, toolsets, *, registry, plugin_keys):
        ...
        stub = object.__new__(_cli_mod.CryozenCLI)
        printed: list[str] = []
        ...
        monkeypatch.setattr(
            "cryozen_cli.plugins.get_plugin_toolset_keys_nowait",
