"""Tests for cryozen_constants module."""
...
from cryozen_platform.host import runtime as host_runtime
from cryozen_platform.host import facts as host_facts
from cryozen_constants import (
    VALID_REASONING_EFFORTS,
    agent_browser_runnable,
    find_cryozen_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_cryozen_root,
    get_cryozen_dir,
    get_cryozen_home,
    get_process_cryozen_home,
    heal_cryozen_managed_node,
    cryozen_managed_node_tree_present,
    iter_cryozen_node_dirs,
    is_container,
    node_tool_runnable,
    ...
    with_cryozen_node_path,
)


class TestGetDefaultCryozenRoot:
    """Tests for get_default_cryozen_root() — Docker/custom deployment awareness."""

    @pytest.mark.linux_only
    def test_no_cryozen_home_returns_native(self, tmp_path, monkeypatch):
        """When CRYOZEN_HOME is not set, returns ~/.cryozen-agent."""
        monkeypatch.delenv("CRYOZEN_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_cryozen_root() == tmp_path / ".cryozen-agent"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (CRYOZEN_HOME=<root>/profiles/<name>),
        ...
        assert get_default_cryozen_root() == docker_root
    ...
    def test_expanded_custom_profile_returns_custom_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv(
            "CRYOZEN_HOME", f"{home_token}/deployment/profiles/research"
        )
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "native-home")

        assert get_default_cryozen_root() == custom_root

    @pytest.mark.windows_only
    def test_no_cryozen_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\cryozen, not ~/.cryozen-agent."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("CRYOZEN_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_cryozen_root() == local_appdata / "cryozen"

    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; CRYOZEN_HOME / home changes invalidate.

        get_default_cryozen_root() resolves CRYOZEN_HOME against the native
        ...
        (native home, CRYOZEN_HOME) compared for free each call.
        """
        # CRYOZEN_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_cryozen_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            cryozen_constants, "_default_cryozen_root_memo", None, raising=False
        )

        first = get_default_cryozen_root()
        ...
        # CRYOZEN_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("CRYOZEN_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_cryozen_root() == docker_root
        assert resolve_calls["n"] > before, (
            "CRYOZEN_HOME change must force a fresh resolution"
        )
...
class TestGetCryozenHome:
    """Tests for get_cryozen_home() platform-aware fallback."""

    @pytest.mark.windows_only
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When CRYOZEN_HOME is unset on Windows, use %LOCALAPPDATA%\\cryozen."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("CRYOZEN_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(cryozen_constants, "_profile_fallback_warned", False)

        assert get_cryozen_home() == local_appdata / "cryozen"


class TestGetProcessCryozenHome:
    """Tests for get_process_cryozen_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_cryozen_home() == home
    ...
    def test_process_and_context_homes_expand_environment_and_user_syntax(
        ...
        for syntax in (home_token, "~"):
            ...
            monkeypatch.setenv("CRYOZEN_HOME", f"{syntax}/process-home")
            assert get_process_cryozen_home() == process_home

            override_home = tmp_path / "override-home"
            token = set_cryozen_home_override(f"{syntax}/override-home")
            try:
                assert get_cryozen_home() == override_home
                assert get_process_cryozen_home() == process_home
            finally:
...
class TestCryozenManagedNode:
    ...
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_cryozen_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(home))
        monkeypatch.setattr(cryozen_constants, "node_tool_runnable", lambda path: True)

        assert find_cryozen_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(cryozen_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(cryozen_constants, "heal_cryozen_managed_node", lambda: False)
        monkeypatch.setattr(
            cryozen_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert cryozen_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Cryozen-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(cryozen_constants, "heal_cryozen_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = cryozen_constants._CRYOZEN_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = cryozen_constants.find_cryozen_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert cryozen_constants.find_cryozen_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert cryozen_constants.find_cryozen_node_executable("node") == str(node)
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from cryozen_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".cryozen-agent"
    ...
    def test_install_tree_skipped(self, monkeypatch):
        ...
        Regression test for #93050: secure_parent_dir() chmod'd /opt/cryozen to
        0700 because it has 3 path parts and passed the ``< 3`` guard, locking
        out UID 10000 (cryozen user) from traversing the install dir.
        """
        install_root = Path(cryozen_constants.__file__).resolve().parent

        # Directly under the install root (e.g. /opt/cryozen/auth.json)
    ...
    def test_install_tree_siblings_still_hardened(self, monkeypatch):
        ...
        # Prefix-named sibling of the install root (/opt/cryozen-data/...).
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import cryozen_cli._subprocess_compat as subprocess_compat
...
class TestGetCryozenDir:
    """Tests for ``get_cryozen_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_cryozen_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_cryozen_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert cryozen_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert cryozen_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert cryozen_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert cryozen_constants.windows_path_to_wsl("/home/alex") is None
        assert cryozen_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(cryozen_constants, "is_wsl", lambda: True)
        assert cryozen_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert cryozen_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert cryozen_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def _install_fake_psutil(self, monkeypatch, procs):
        ...
        monkeypatch.setattr(cryozen_constants.sys, "platform", "win32")
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(cryozen_constants.sys, "platform", "darwin")
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
        assert cryozen_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert cryozen_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(home))
        monkeypatch.setenv(
            "CRYOZEN_NODE_TARGET_MAJOR",
            str(cryozen_constants._CRYOZEN_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(cryozen_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            cryozen_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            cryozen_constants,
        ...
        monkeypatch.setattr(
            cryozen_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(cryozen_constants._CRYOZEN_NODE_TARGET_MAJOR)
        ...
        result = cryozen_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(cryozen_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_cryozen_managed_node() is False
        assert cryozen_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_cryozen_managed_node() is False
        assert cryozen_constants._managed_node_heal_attempted is True
        # The flag is set, so the once-per-process budget is spent.
        assert heal_cryozen_managed_node() is False
        assert calls["n"] == 1

class TestProjectVenvDirOutOfTree:
    """#116148: a checkout with no in-tree venv whose interpreter lives in ``$CRYOZEN_HOME/venvs/<name>``
    ...
    def _running_from(monkeypatch, checkout, venv):
        monkeypatch.setattr(cryozen_constants, "__file__", str(checkout / "cryozen_constants.py"))
    ...
    def test_out_of_tree_install_resolves_the_running_interpreter_venv(self, monkeypatch, tmp_path):
        checkout = tmp_path / "cryozen-agent"
        checkout.mkdir()
        venv = tmp_path / "venvs" / "cryozen"
        cryozen_constants.venv_python_path(venv).parent.mkdir(parents=True)
        cryozen_constants.venv_python_path(venv).write_text("", encoding="utf-8")
        self._running_from(monkeypatch, checkout, venv)

        assert cryozen_constants.project_venv_dir(checkout) == venv
    ...
    def test_foreign_root_and_in_tree_venv_are_unchanged(self, monkeypatch, tmp_path):
        ...
        assert cryozen_constants.project_venv_dir(other) is None
        (checkout / ".venv").mkdir()
        assert cryozen_constants.project_venv_dir(checkout) == checkout / ".venv"
