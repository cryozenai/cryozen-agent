...
Under a gateway (Telegram, Discord, Slack, ...) Cryozen passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Cryozen via Telegram AND Discord
...
class TestPinTransition:
    ...
    def test_identity_signature_reflects_both_session_prefixes(self, tmp_path, monkeypatch):
        ...
        cfg_path = tmp_path / "honcho.json"
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
        base = {"apiKey": "k", "peerName": "Igor", "aiPeer": "cryozen"}
    ...
    def test_identity_signature_reflects_a_repointed_host_workspace(self, tmp_path, monkeypatch):
        """``cryozen honcho peers map`` can repoint a host block's workspace; the cached agent's manager
        ...
        cfg_path.write_text(json.dumps({**base, "hosts": {"cryozen": {"workspace": "old"}}}))
        sig_old = provider.identity_signature()["workspace"]
        cfg_path.write_text(json.dumps({**base, "hosts": {"cryozen": {"workspace": "new"}}}))
...
class TestProfilePeerUniqueness:
    """Each Cryozen profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `cryozen -p partner` pinned to a different person's peer).
