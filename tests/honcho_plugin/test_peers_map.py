"""Tests for ``cryozen honcho peers map``: account discovery, the resolution preview, and alias writes."""
...
def _cfg(root=None, **host):
    return {"apiKey": "***", **(root or {}), "hosts": {"cryozen": host}}
...
def _run_map(monkeypatch, tmp_path, *, answers, cfg, db_rows=(), ws_peers=None, workspaces=None):
    ...
    # One profile per host block: "cryozen" is default, "cryozen.<name>" is profile <name>.
    profiles = [("default" if k == "cryozen" else k.removeprefix("cryozen."), k, v) for k, v in cfg["hosts"].items()]
    ...
    for name, value in {
        ...
        "_peers_map_client": lambda workspace=None: (client, SimpleNamespace(workspace_id="cryozen") if client else None),
...
class TestCmdPeersMap:
    ...
    def test_dash_clears_alias(self, monkeypatch, tmp_path, aliases, expected_host):
        ...
        assert written["cfg"]["hosts"]["cryozen"] == expected_host
    ...
    def test_cleared_host_map_does_not_resurrect_root_aliases(self, monkeypatch, tmp_path, capsys):
        ...
        assert written["cfg"]["hosts"]["cryozen"]["userPeerAliases"] == {}
        assert "root aliases no longer apply to [cryozen]" in capsys.readouterr().out

        path = tmp_path / "honcho.json"
        path.write_text(json.dumps(written["cfg"]))
        assert HonchoClientConfig.from_global_config(host="cryozen", config_path=path).user_peer_aliases == {}
...
class TestWorkspaceSwitch:
    ...
    def test_browse_client_is_built_for_the_browsed_workspace(self, monkeypatch):
        ...
        seen = []
        base = client_mod.HonchoClientConfig(host="cryozen", workspace_id="cryozen", api_key="k")
        ...
        assert own.workspace_id == "cryozen"
        assert browsed.workspace_id == "cosmania-dex"
        assert [c.workspace_id for c in seen] == ["cryozen", "cosmania-dex"]
