...
class TestHonchoSession:
    def _make_session(self):
        return HonchoSession(
            ...
            assistant_peer_id="cryozen-assistant",
...
class TestPeerLookupHelpers:
    ...
    def test_search_context_uses_peer_perspective_message_search(self):
        ...
        honcho_client.search.return_value = [
            SimpleNamespace(content="Robert runs neuralancer", peer_id="cryozen", session_id="s-old", id="m1"),
...
class TestToolsModeInitBehavior:
    ...
    def _make_provider_with_config(self, recall_mode="tools", init_on_session_start=False,
        ...
        with patch("plugins.memory.honcho.client.HonchoClientConfig.from_global_config", return_value=cfg), \
             ...
             patch("cryozen_constants.get_cryozen_home", return_value=MagicMock()):
...
class TestPerSessionMigrateGuard:
    ...
    per-session creates a fresh Honcho session every Cryozen run. Uploading
...
class TestContextTokensForwarded:
    ...
    def _manager(self, context_tokens=4000):
        ...
        session = HonchoSession(key="cli:test", user_peer_id="robert", assistant_peer_id="cryozen",
...
def _provider_with_raw(raw, host="cryozen"):
...
class TestSessionStartInjection:
    @pytest.mark.parametrize("raw, headings", [
        ...
        ({"injection": {"sessionStart": ["summary"]}, "hosts": {"cryozen": {"injection": {"sessionStart": ["peerCard"]}}}},
         ["## User Peer Card"]),
    ...
    def test_desktop_panel_writes_the_pin_the_provider_reads(self, submitted, headings):
        from cryozen_cli.web_routers.memory_providers import _apply_field_values
    ...
    def test_initialize_reads_the_pin(self):
        raw = {"injection": {"sessionStart": ["summary"]}}
        provider = TestDialecticCadenceDefaults._make_provider(cfg_extra={"raw": raw, "host": "cryozen"})
...
class TestInjectionAuditLog:
    ...
    @pytest.mark.parametrize("raw, env", [
        ({}, None),
        ({"logging": True, "hosts": {"cryozen": {"logging": False}}}, None),
