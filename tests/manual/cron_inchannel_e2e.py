...
Uses a throwaway CRYOZEN_HOME so it never touches ~/.cryozen-agent. No real names.
"""
...
def _fresh_home():
    """Point CRYOZEN_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_cryozen_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["CRYOZEN_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER CRYOZEN_HOME is set.
...
# a different get_cryozen_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"CRYOZEN_HOME (throwaway): {HOME}")
