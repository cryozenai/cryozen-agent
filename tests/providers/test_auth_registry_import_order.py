...
def test_tui_import_exposes_auth_registry_to_provider_plugins(tmp_path):
    ...
    plugin_dir = cryozen_home / "plugins" / "model-providers" / "import-order-probe"
    ...
    (plugin_dir / "__init__.py").write_text(
        ...
        "\n"
        "from cryozen_cli.auth import PROVIDER_REGISTRY, ProviderConfig\n"
    ...
    probe = subprocess.run(
        [
            sys.executable,
            "-c",
            "from tools.environments.local import _CRYOZEN_PROVIDER_ENV_BLOCKLIST; "
            ...
            "assert 'IMPORT_ORDER_PROBE_KEY' in _CRYOZEN_PROVIDER_ENV_BLOCKLIST",
        ],
        cwd=REPO_ROOT,
        env=env,
    ...
    assert "partially initialized module 'cryozen_cli.auth'" not in probe.stderr
