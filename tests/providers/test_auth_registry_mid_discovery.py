"""Regression tests for #102123: plugins discovered after ``cryozen_cli.auth`` is
first imported must still reach ``PROVIDER_REGISTRY``.

``cryozen_cli.auth`` mirrors provider-plugin profiles into ``PROVIDER_REGISTRY``
when it is imported.  If a plugin's own imports pull ``cryozen_cli.auth`` in
...
def _run_probe(cryozen_home: Path, code: str) -> subprocess.CompletedProcess:
    env = os.environ.copy()
    env["CRYOZEN_HOME"] = str(cryozen_home)
    env.pop("CRYOZEN_PROFILE", None)
...
def test_plugins_discovered_after_auth_import_resolve(tmp_path):
    ...
    # Sorted first: a plugin whose imports drag cryozen_cli.auth in mid-discovery
    # (any plugin importing agent.credential_pool or similar does this).
    _write_plugin(
        cryozen_home,
        "aaa-early-probe",
        "import cryozen_cli.auth  # noqa: F401 — simulate a core-importing plugin\n"
...
# never imports cryozen_cli.auth on its own).
# ---------------------------------------------------------------------------
...
def test_user_plugin_alias_repoints_and_display_name_follows(_isolated_registries, monkeypatch, tmp_path):
    """A $CRYOZEN_HOME plugin owns the aliases it declares and the display name of a same-name row.
