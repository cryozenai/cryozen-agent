...
#              install landed on OLD with a working `cryozen`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `cryozen`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method cryozen-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  cryozen-update|installer-script|installer-script+desktop|cryozen-desktop-app-update) ;;
  *) echo "error: --update-method must be cryozen-update, installer-script, installer-script+desktop or cryozen-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/cryozen-installer-script-e2e"
LOG_DIR="${CRYOZEN_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled cryozen or a
# developer config, and old installer scripts hardcode $HOME/.cryozen-agent (the
# CRYOZEN_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export CRYOZEN_HOME="$HOME/.cryozen-agent"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/Cryozen" \
    "$release_dir/linux-unpacked/cryozen" \
    "$release_dir/mac-arm64/Cryozen.app" \
    "$release_dir/mac/Cryozen.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local cryozen="$INSTALL_DIR/venv/bin/cryozen"
  [ -x "$cryozen" ] || fail "no cryozen console script at $cryozen"
  "$cryozen" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "cryozen --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "cryozen --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `cryozen desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `cryozen update` itself makes. Probe the
  # INSTALLED cryozen for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `cryozen desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local cryozen="$INSTALL_DIR/venv/bin/cryozen"
  if ! "$cryozen" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "cryozen desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$cryozen" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "cryozen desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "cryozen desktop --build-only ($1) exited $rc; transcript above"
  ok "cryozen desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  cryozen-update)
    ...
    # installed cryozen; older ones read the prompt from stdin, so close it.
    CRYOZEN="$INSTALL_DIR/venv/bin/cryozen"
    if "$CRYOZEN" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$CRYOZEN" update --yes)
    else
      update_cmd=("$CRYOZEN" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "cryozen update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "cryozen update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  cryozen-desktop-app-update)
    # The real user surface: `cryozen desktop` launches the app, the user
    ...
    # suite's own) is configured into CRYOZEN_HOME exactly like the dev:mock
    ...
    step "capturing the cryozen desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      CRYOZEN_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$CRYOZEN" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "cryozen desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "cryozen desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "cryozen desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$CRYOZEN_HOME/logs" "$ildest/cryozen-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/cryozen/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$CRYOZEN_HOME/.cryozen-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$CRYOZEN_HOME" > "$ildest/cryozen-home-ls.txt" 2>/dev/null || true
