...
#   mock_start <workroot>      start, write config into $CRYOZEN_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, CRYOZEN_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$CRYOZEN_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $CRYOZEN_HOME (api $url/v1)"
}
