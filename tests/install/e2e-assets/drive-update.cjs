// drive-update.cjs — launch the INSTALLED Cryozen.exe (real Electron desktop
...
//
//   node <this file> <path-to-Cryozen.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Cryozen.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: CRYOZEN_HOME (isolated install) and
  ...
  // The real backend (`cryozen serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Cryozen... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.cryozenDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Cryozen — this window will close" overlay should appear,
  ...
  // the staged binary), which claims CRYOZEN_HOME/.cryozen-update-in-progress
  ...
  // proved the desktop had exited and `cryozen update` was already running).
  //
  ...
  const cryozenHome = process.env.CRYOZEN_HOME
  const markerPath = cryozenHome ? path.join(cryozenHome, '.cryozen-update-in-progress') : null
  const resultPath = cryozenHome ? path.join(cryozenHome, '.cryozen-update-result.json') : null
