...
# (Cryozen-Setup.dmg from the website) can reach HEAD.
#
...
#              cryozen-desktop-app-update  capture `cryozen desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              cryozen-update              CLI update from the installed venv
...
#     --update-method open-app-update|cryozen-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://github.com/cryozenai/cryozen-agent/releases/latest/download/cryozen-setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|cryozen-desktop-app-update|cryozen-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${CRYOZEN_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/cryozen-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$CRYOZEN_HOME/cryozen-agent"
}
...
phase_stage() {
  ...
  touch "$CRYOZEN_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Cryozen.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Cryozen.app" \
    "/Applications/Cryozen.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Cryozen-Setup.dmg"

  local dmg="$WORK_ROOT/Cryozen-Setup.dmg"
  ...
  # "Install Cryozen" button with native input.
  local rc=0
  ...
  log_group "Cryozen-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$cryozen" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "cryozen --version failed after install"
  ok "cryozen --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Cryozen.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    cryozen-update)
      ...
      # update subcommand only in later releases; ask the installed cryozen.
      local cryozen="$INSTALL_DIR/venv/bin/cryozen"
      local update_cmd=("$cryozen" update)
      ...
      [ "$rc" -eq 0 ] || fail "cryozen update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Cryozen.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Cryozen.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Cryozen.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, cryozen update) need.
      local app app_bin
...
PYEOF
    ...
    cryozen-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "cryozen desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "cryozen desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.cryozen-agent/logs" "$ildest/cryozen-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Cryozen"
  ...
  "$INSTALL_DIR/venv/bin/cryozen" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "cryozen --version failed after update"
  ok "cryozen --version works post-update"
