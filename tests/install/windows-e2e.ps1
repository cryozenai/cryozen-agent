...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Cryozen the way
...
# actually touches:
#
#   INSTALL   - downloads the production Cryozen-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Cryozen.exe must appear.
...
#   installer's `git clone` and `cryozen update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and cryozen-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "cryozen-desktop-app-update", "cryozen-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Cryozen-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:CRYOZEN_E2E_WORKROOT) { $env:CRYOZEN_E2E_WORKROOT } else { Join-Path $env:TEMP "cryozen-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://github.com/cryozenai/cryozen-agent/releases/latest/download/cryozen-setup.exe",
...
$CryozenHome  = Join-Path $WorkRoot "cryozen-home"
$InstallDir  = Join-Path $CryozenHome "cryozen-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if cryozen ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit cryozen thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Cryozen.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Cryozen.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-CryozenRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $CryozenHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "cryozen-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $CryozenHome ".cryozen-update-result.json"
    ...
    Get-ChildItem -LiteralPath $CryozenHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "cryozen-home-ls.txt")
}

function Test-CryozenRuns([string]$Label) {
    Save-InstallSideState $Label
    $cryozenExe = Join-Path $InstallDir "venv\Scripts\cryozen.exe"
    Assert-True (Test-Path -LiteralPath $cryozenExe) "$Label -- venv\Scripts\cryozen.exe exists"
    & $cryozenExe --version 2>&1 | ForEach-Object { Write-Host "    cryozen --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- cryozen --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-CryozenHome", $CryozenHome, "-InstallDir", $InstallDir)
...
function Invoke-CryozenUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $cryozenExe = Join-Path $InstallDir "venv\Scripts\cryozen.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $cryozenExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $cryozenExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "cryozen update transcript" $log
    Assert-True ($updateExit -eq 0) "cryozen update exited $updateExit (expected 0)"
}

function Invoke-CryozenDesktopAppUpdate([string]$TargetSha) {
    # The cryozen-desktop launch surface: `cryozen desktop` runs its whole
    ...
    $prevCap = $env:CRYOZEN_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:CRYOZEN_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $cryozenExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:CRYOZEN_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "cryozen desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "cryozen desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $CryozenHome ".cryozen-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured cryozen desktop spec; update completed"
}
...
function Stop-CryozenAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Cryozen.exe (Electron) -- never cryozen.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Cryozen" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Cryozen.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `cryozen update`/desktop builds use the Cryozen-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $CryozenHome "node\node.exe"),
        (Join-Path $CryozenHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Cryozen-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Cryozen-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Cryozen-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Cryozen-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:CRYOZEN_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $CryozenHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  Cryozen-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Cryozen.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Cryozen-Setup.exe", (Join-Path $CryozenHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Cryozen" -ErrorAction SilentlyContinue)) "Cryozen.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Cryozen-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $CryozenHome "logs\bootstrap-installer.log"
    ...
    Stop-CryozenAppProcesses "post-install"
    ...
    Test-CryozenRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Cryozen.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Cryozen.exe present before update"
    ...
    try {
        ...
        # `cryozen update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .cryozen-update-result.json on every exit;
        #   * older checkouts -> the staged cryozen-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/cryozen/relaunch
        ...
        $updateLog = Join-Path $CryozenHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-CryozenRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Cryozen.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Cryozen.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Cryozen" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Cryozen window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Cryozen" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $CryozenHome "logs\desktop-update-handoff.log"
        ...
        Stop-CryozenAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, cryozen runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-CryozenRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-CryozenRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $CryozenHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "cryozen-desktop-app-update" {
            Invoke-CryozenDesktopAppUpdate $state.current
        }
        "cryozen-update" {
            Invoke-CryozenUpdate
        }
        "installer-script" {
        ...
        "desktop-installer@latest" {
            # A user re-downloading Cryozen-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $CryozenHome "logs\desktop.log"))) {
