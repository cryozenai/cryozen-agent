"""Shared fixtures for the cryozen-agent test suite.
...
2. **Isolated CRYOZEN_HOME.** CRYOZEN_HOME points to a per-test tempdir so
   code reading ``~/.cryozen-agent/*`` via ``get_cryozen_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".cryozen-agent"`` instead of the canonical
   ``get_cryozen_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No CRYOZEN_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox CRYOZEN_HOME before ANY test module is imported ──────────────────
# `cryozen_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_cryozen_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.cryozen-agent/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes CRYOZEN_HOME, but fixtures run
...
# the REAL Cryozen root — capture it BEFORE the sandbox rewires CRYOZEN_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.cryozen-agent (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("CRYOZEN_KANBAN_HOME", "").strip()
_PRE_SANDBOX_CRYOZEN_HOME = os.environ.get("CRYOZEN_HOME", "")


def _cryozen_home_points_at_production(value: str) -> bool:
    """True when a pre-set CRYOZEN_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    CRYOZEN_HOME=~/.cryozen-agent``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``cryozen_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.cryozen-agent`` — the escape vector that landed pytest
    ...
    CRYOZEN_HOME is honored now.
    """
    if not value:
        return True
    try:
        # The platform-default root, not a hardcoded ``~/.cryozen-agent``: Windows installs live under
        # ``%LOCALAPPDATA%\cryozen``, and a dev shell exporting that path used to be honored as
        # "custom", pinning import-time paths (``tui_gateway.server._cryozen_home``) to the live
        # install so the state.db guard tripped on every store-touching test (#112692).
        from cryozen_state_guard import _real_platform_state_root

        resolved = Path(value).expanduser().resolve()
        real_root = _real_platform_state_root() or (Path.home() / ".cryozen-agent").resolve()
    except Exception:
        return True
...
if _cryozen_home_points_at_production(os.environ.get("CRYOZEN_HOME", "")):
    _SESSION_CRYOZEN_HOME = tempfile.mkdtemp(prefix="cryozen-test-home-")
    os.environ["CRYOZEN_HOME"] = _SESSION_CRYOZEN_HOME
    atexit.register(shutil.rmtree, _SESSION_CRYOZEN_HOME, True)
...
# like a real CLI") — which used to disarm cryozen_state's live-DB guard in
# the child at the same moment the child lost the CRYOZEN_HOME redirect.
# CRYOZEN_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# cryozen_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export CRYOZEN_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["CRYOZEN_TEST_ISOLATION"] = os.environ.get("CRYOZEN_HOME", "") or "1"

#: CRYOZEN_HOME as it stood when conftest was imported - i.e. before any test
...
CRYOZEN_HOME_AT_CONFTEST_IMPORT = os.environ.get("CRYOZEN_HOME", "")

# ── Host-rendezvous isolation ───────────────────────────────────────────────
# ``gateway/host_rendezvous.py`` publishes ONE record per role per OS USER, in
# ``$CRYOZEN_GATEWAY_LOCK_DIR`` else ``$XDG_STATE_HOME/cryozen/gateway-locks`` —
# deliberately outside CRYOZEN_HOME, because the host singleton spans profiles.
...
HOST_LOCK_DIR_AT_CONFTEST_IMPORT = os.environ.get("CRYOZEN_GATEWAY_LOCK_DIR", "")
if not HOST_LOCK_DIR_AT_CONFTEST_IMPORT:
    ...
    _LOCK_DIR_PREFIX = "cryozen-test-gateway-locks-"
    ...
    os.environ["CRYOZEN_GATEWAY_LOCK_DIR"] = _SESSION_LOCK_DIR
...
# CRYOZEN_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_CRYOZEN_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``cryozen_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Cryozen agent session inherits them and
    # cryozen_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "CRYOZEN_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "CRYOZEN_INTERACTIVE",
    "CRYOZEN_QUIET",
    "CRYOZEN_TOOL_PROGRESS",
    "CRYOZEN_TOOL_PROGRESS_MODE",
    "CRYOZEN_MAX_ITERATIONS",
    "CRYOZEN_SESSION_PLATFORM",
    "CRYOZEN_SESSION_CHAT_ID",
    "CRYOZEN_SESSION_CHAT_NAME",
    "CRYOZEN_SESSION_CHAT_TYPE",
    "CRYOZEN_SESSION_THREAD_ID",
    "CRYOZEN_SESSION_SOURCE",
    "CRYOZEN_SESSION_KEY",
    "CRYOZEN_GATEWAY_SESSION",
    "CRYOZEN_CRON_SESSION",
    "_CRYOZEN_GATEWAY",
    "CRYOZEN_PLATFORM",
    "CRYOZEN_MODEL",
    "CRYOZEN_INFERENCE_MODEL",
    "CRYOZEN_INFERENCE_PROVIDER",
    "CRYOZEN_TUI_PROVIDER",
    "CRYOZEN_MANAGED",
    "CRYOZEN_MANAGED_DIR",
    "CRYOZEN_DEV",
    "CRYOZEN_CONTAINER",
    "CRYOZEN_EPHEMERAL_SYSTEM_PROMPT",
    "CRYOZEN_TIMEZONE",
    "CRYOZEN_REDACT_SECRETS",
    "CRYOZEN_BACKGROUND_NOTIFICATIONS",
    "CRYOZEN_EXEC_ASK",
    "CRYOZEN_HOME_MODE",
    "CRYOZEN_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.cryozen-agent/kanban.db instead of the per-test CRYOZEN_HOME.
    "CRYOZEN_KANBAN_DB",
    "CRYOZEN_KANBAN_BOARD",
    "CRYOZEN_KANBAN_HOME",
    "CRYOZEN_KANBAN_WORKSPACES_ROOT",
    "CRYOZEN_KANBAN_LOGS_ROOT",
    "CRYOZEN_KANBAN_TASK",
    "CRYOZEN_KANBAN_WORKSPACE",
    "CRYOZEN_KANBAN_RUN_ID",
    "CRYOZEN_KANBAN_CLAIM_LOCK",
    "CRYOZEN_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "CRYOZEN_DELEGATED_CHILD_CONTEXT",
    "CRYOZEN_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "cryozen" host.
    "CRYOZEN_HONCHO_HOST",
    # Dashboard auth gate (PR #30156). When set, the bundled dashboard-auth
    # plugins (basic, self_hosted OIDC) auto-register on plugin discovery,
    ...
    # these, so production tests must not see them either. (The password
    # half of the basic provider is covered by the `_PASSWORD` suffix.)
    "CRYOZEN_DASHBOARD_BASIC_AUTH_USERNAME",
    "CRYOZEN_DASHBOARD_OIDC_ISSUER",
    "CRYOZEN_DASHBOARD_OIDC_CLIENT_ID",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CRYOZEN_HOME to per-test tempdirs so code that
    reads ``~/.cryozen-agent/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral CRYOZEN_* vars that could change test semantics.
    for name in _CRYOZEN_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("CRYOZEN_HONCHO_HOST", "cryozen")

    # 3. Redirect CRYOZEN_HOME to a per-test tempdir. Code that reads
    #    ``~/.cryozen-agent/*`` via ``get_cryozen_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.cryozen-agent/*`` via
    #    ``Path.home() / ".cryozen-agent"`` instead of ``get_cryozen_home()``
    #    is a bug to fix at the callsite.
    fake_cryozen_home = tmp_path / "cryozen_test"
    fake_cryozen_home.mkdir()
    (fake_cryozen_home / "sessions").mkdir()
    (fake_cryozen_home / "cron").mkdir()
    (fake_cryozen_home / "memories").mkdir()
    (fake_cryozen_home / "skills").mkdir()
    monkeypatch.setenv("CRYOZEN_HOME", str(fake_cryozen_home))
    ...
    # write the developer's live ~/.local/state/cryozen/gateway-locks.
    ...
    if not HOST_LOCK_DIR_AT_CONFTEST_IMPORT:
        ...
        monkeypatch.setenv("CRYOZEN_GATEWAY_LOCK_DIR", str(tmp_path / "gateway-locks"))
    ...
    # cryozen_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("CRYOZEN_TEST_ISOLATION", str(fake_cryozen_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("CRYOZEN_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. cryozen_state computes ``DEFAULT_DB_PATH = get_cryozen_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from cryozen_state import ...``) that
    ...
    cryozen_state_mod = sys.modules.get("cryozen_state")
    if cryozen_state_mod is not None and hasattr(cryozen_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            cryozen_state_mod, "DEFAULT_DB_PATH", fake_cryozen_home / "state.db"
        )
    ...
    monkeypatch.setenv("CRYOZEN_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.cryozen-agent/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import cryozen_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path CRYOZEN_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_cryozen_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from cryozen_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
def _close_leaked_session_dbs():
    ...
    Root cause of OOM incident 20260816: ~40 files under tests/cryozen_cli/
    ...
    ``pytest tests/cryozen_cli/`` accumulated 16-25 GB RSS and had to be
    OOM-killed three times in one day.

    Rather than editing every test file, ``SessionDB.__init__`` registers each
    instance in ``cryozen_state_guard._test_instance_registry`` (a WeakSet,
    populated only when the ``CRYOZEN_TEST_ISOLATION`` marker is set — i.e.
    ...
    Instances opened through ``cryozen_state_registry.acquire()`` are skipped:
    ...
    if wait is not None:
        wait()
    try:
        from cryozen_state_guard import _test_instance_registry as registry
    except Exception:
        return
    if not registry:
        return
...
# invocation), kanban writes silently pollute the real ~/.cryozen-agent. This autouse
...
# allow-list because test-level fixtures legitimately move CRYOZEN_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session CRYOZEN_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``CRYOZEN_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Cryozen root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_CRYOZEN_HOME and not _cryozen_home_points_at_production(
        _PRE_SANDBOX_CRYOZEN_HOME
    ):
        # CRYOZEN_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_cryozen_root().resolve()
    # No pre-existing CRYOZEN_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".cryozen-agent").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.cryozen-agent`` captured at import time. Hermetic tests that legitimately
    move CRYOZEN_HOME to sibling tempdirs are unaffected.

    Only patches when ``cryozen_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("cryozen_cli.kanban_db")
    _kdbc = sys.modules.get("cryozen_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import cryozen_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.cryozen-agent. See #69283."
        )
...
# ``cryozen_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Cryozen root. This fixture wires the
# test-side knobs:
...
#     installs where CRYOZEN_HOME is not ~/.cryozen-agent) into the guard's
...
# so subprocess children that import cryozen_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("cryozen_state")
    ...
    if _PRE_SANDBOX_CRYOZEN_HOME and not _cryozen_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_CRYOZEN_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Cryozen home override redirects every later
    # ``get_cryozen_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from cryozen_constants import get_cryozen_home_override, set_cryozen_home_override

        if get_cryozen_home_override() is not None:
            set_cryozen_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal cryozen config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``cryozen-gateway`` process
...
# correlated with a ``tests/cryozen_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> cryozen-gateway``
...
def _wal_is_usable() -> bool:
    """True when Cryozen will actually put a database into WAL mode here.

    Cryozen refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Cryozen can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Cryozen managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``cryozen_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_cryozen_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_cryozen_home`` fixture
    redirects ``CRYOZEN_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.cryozen-agent/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``cryozen_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["CRYOZEN_VOICE_TTS"] = "1"``. The
...
#      ``cryozen_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``cryozen_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``cryozen_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``cryozen_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Cryozen runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.cryozen-agent``.
#
...
def _relocate_basetemp_outside_operator_home(config) -> None:
    """Move pytest's basetemp out of the operator's platform-native Cryozen home.

    Every per-test sandbox is ``<basetemp>/.../cryozen_test``. ``get_default_cryozen_root()``
    prefers the platform-native home whenever ``CRYOZEN_HOME`` sits *under* it, so a basetemp
    inside ``~/.cryozen-agent`` (or ``%LOCALAPPDATA%\\cryozen``, where ``TEMP`` commonly lives on
    ...
    """
    from cryozen_constants import _get_platform_default_cryozen_home

    native = _get_platform_default_cryozen_home().resolve()
    ...
    # Cryozen home). The repo is no escape either: the default install checks it
    # out *inside* the home (~/.cryozen-agent/cryozen-agent). The relocated basetemp goes
    # into ONE prunable root outside the home, never loose into the operator's
    # $HOME (123 ``cryozen-pytest-basetemp-*`` dirs piled up there in a day, one per
    ...
    assert not safe.resolve().is_relative_to(native), (
        f"pytest basetemp {safe} still resolves inside the operator's Cryozen home {native}; "
    ...
    config._cryozen_relocated_basetemp = safe
...
def _pytest_disk_temp_root(native: Path) -> Path:
    ...
    if os.name != "nt" and os.path.isdir("/var/tmp"):  # no-tmp: ok — disk-backed FHS root
        root = Path("/var/tmp/cryozen-pytest")  # no-tmp: ok — /var/tmp is disk-backed by FHS, never tmpfs
    else:
        root = native.parent / "cryozen-pytest"
...
def _remove_relocated_basetemp(config) -> None:
    safe = getattr(config, "_cryozen_relocated_basetemp", None)
    if safe is not None:
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Cryozen falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Cryozen uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart cryozen-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting cryozen/python patterns are also blocked.
    """
    ...
    def _matches_cryozen_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _CRYOZEN_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_cryozen_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch cryozen-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m cryozen_cli.main".
                if (
                    "cryozen" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live cryozen-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting cryozen/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `cryozen update` (or the
        # equivalent `python -m cryozen_cli.main update`).  These commands
        ...
        if "update" in low and (
            # cryozen update / cryozen update --gateway / setsid bash -c ... cryozen update
            ("cryozen" in low and "update" in low.split())
            or
            # python -m cryozen_cli.main update --gateway
            ("cryozen_cli" in low and "update" in low.split())
            or
            # venv/bin/cryozen update  (absolute path variant used in tests)
            (".venv/bin/cryozen" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`cryozen update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m cryozen_cli.main
        # gateway run|start|restart``). ``_spawn_cryozen_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp CRYOZEN_HOME, resolves the DEVELOPER's
        # ``cryozen-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … cryozen gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "cryozen gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (cryozen_cli.web_server_gateway."
                "_spawn_cryozen_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_CRYOZEN_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import cryozen_cli.voice as _voice
    except Exception:
