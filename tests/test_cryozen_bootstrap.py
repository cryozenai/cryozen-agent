"""Tests for cryozen_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Cryozen entry point
(cryozen, cryozen-agent, cryozen-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Cryozen entry point imports cryozen_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported cryozen_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("cryozen_bootstrap", None)
    import cryozen_bootstrap  # noqa: WPS433
    return cryozen_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Cryozen CLI
...
class TestEntryPointsImportBootstrap:
    """Every Cryozen entry point must import cryozen_bootstrap as its
    ...
    # Entry points that invoke Cryozen as a process.  Each one must
    # import cryozen_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "cryozen_cli/main.py",   # cryozen CLI (console_script)
        "run_agent.py",          # cryozen-agent (console_script)
        "acp_adapter/entry.py",  # cryozen-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import cryozen_bootstrap' and that
        ...
        half-finished ``cryozen update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``cryozen_bootstrap`` as a top-level module) leaves cryozen
        ...
        """
        # Resolve relative to the cryozen-agent repo root.  Tests live
        # at tests/test_cryozen_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # cryozen start even when cryozen_bootstrap hasn't been
        ...
        assert first_import_name == "cryozen_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'cryozen_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import cryozen_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Cryozen's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("CRYOZEN_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["CRYOZEN_PYTHON_SRC_ROOT"] = env
            elif "CRYOZEN_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["CRYOZEN_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/cryozen")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CRYOZEN_PYTHON_SRC_ROOT", None)
            else:
                os.environ["CRYOZEN_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/cryozen", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Cryozen root.  The guard must relocate Cryozen to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/cryozen"])
        assert result[0] == "/opt/cryozen"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Cryozen root.
        assert result.index("/opt/cryozen") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["CRYOZEN_PYTHON_SRC_ROOT"] = "/env/cryozen"
            hb.harden_import_path()
            assert sys.path[0] == "/env/cryozen"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CRYOZEN_PYTHON_SRC_ROOT", None)
...
class TestHappyEyeballsSocketConnect:
    ...
    def test_import_routes_http_client_and_urllib3_connects_through_the_racer(self):
        ...
        assert getattr(urllib3_connection.create_connection, "_cryozen_happy_eyeballs", False)
    ...
    def test_racer_bug_raises_instead_of_falling_back_to_the_serial_walk(self, monkeypatch):
        ...
        for racer in (socket.create_connection, urllib3_connection.create_connection):
            assert getattr(racer, "_cryozen_happy_eyeballs", False)
