...
work on a file several Cryozen processes share (gateway service, the Desktop
app's `cryozen serve` backend, CLI sessions, the TUI slash worker):

* `cryozen_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `cryozen_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
