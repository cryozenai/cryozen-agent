"""POSIX advisory locks must survive Cryozen' own database inspection.
...
Cryozen used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `cryozen sessions
...
out across Cryozen' inspection calls.
"""
...
from cryozen_cli.sqlite_safe_read import (
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_cryozen_fail_close", False):
    ...
    conn._cryozen_fail_close = False
    conn.close()
    assert not has_live_connection(db)
    assert read_header_bytes_preopen(db, length=16) is not None
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import cryozen_cli.sqlite_safe_read as ssr
...
def test_repair_connections_are_tracked_for_byte_probe_safety(tmp_path, clean_registry, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
    from cryozen_state import SessionDB
    from cryozen_state_repair import _connect_repair_durable, _repair_conn
...
def test_byte_probe_never_cancels_the_repair_exclusion(tmp_path, clean_registry):
    """A live repair's EXCLUSIVE lock must survive Cryozen' own inspection (#63386).
    ...
    """
    import cryozen_state_repair as repair
