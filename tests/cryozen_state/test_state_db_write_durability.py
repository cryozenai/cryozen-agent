...
The defect: cryozen_state already knows macOS `fsync()` does not guarantee
...
from cryozen_state import repair_state_db_schema
from cryozen_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in cryozen_state_repair; cryozen_state only
    # re-imports them.
    import cryozen_state_repair

    source = Path(cryozen_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(cryozen_state_repair.__file__))
