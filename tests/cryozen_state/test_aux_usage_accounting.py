...
class TestRecordAuxiliaryUsage:
    ...
    def test_main_loop_and_aux_rows_coexist(self, db):
        ...
        db.record_auxiliary_usage(
            ...
            billing_provider="openrouter", input_tokens=40, output_tokens=8,
        )
        rows = _usage_rows(db, "s1")
...
class TestAnalyticsAuxRows:
    def test_aux_usage_rows_and_merge(self, db):
        from cryozen_cli.web_server_profiles import _aux_task_summary, _aux_usage_rows, _merge_aux_into_by_model
        ...
        db.record_auxiliary_usage(
            ...
            billing_provider="openrouter", input_tokens=200, output_tokens=20,
        )
...
class TestInsightsAuxTotals:
    def test_overview_totals_include_aux_usage(self, db):
        """`cryozen insights` overview must count aux tokens, not just the
