...
import cryozen_state_schema
from cryozen_state import SessionDB
from cryozen_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from cryozen_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m cryozen_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "cryozen-agent.service"),
            ("grep", "cryozen-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m cryozen_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/cryozen-agent/.venv/bin/cryozen gateway"),
            ("python3", "/opt/cryozen-agent/tools/check_state.py"),
            ("cryozen-monitor", "gateway"),
            ("cryozenctl", "serve"),
            ("python3", "worker.py", "cryozen_cli.main"),
            ("python3", "-m", "other.module", "cryozen_cli.main"),
            ("python3", "-c", "cryozen_cli.main"),
            ("python3", "-Icprint('cryozen_cli.main')", "cryozen_cli/main.py"),
        ),
    )
    def test_uninspectable_non_cryozen_process_is_not_a_holder(self, argv):
        assert not cryozen_state_holders._looks_like_cryozen(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/cryozen", "gateway"),
            ("/usr/local/bin/cryozen-agent", "serve"),
            ("/usr/local/bin/cryozen-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "cryozen_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "cryozen_cli.main", "gateway"),
            ("/usr/bin/python3", "-mcryozen_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "cryozen_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "cryozen_cli.main"),
            (
                "/opt/cryozen-agent/.venv/bin/python",
                "/opt/cryozen-agent/cryozen_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "cryozen_cli/main.py", "gateway"),
            ("python3", "/opt/cryozen-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_cryozen_process_remains_a_holder(self, argv):
        assert cryozen_state_holders._looks_like_cryozen(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(cryozen_state_holders.os, "stat", _stat)
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Cryozen process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Cryozen
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00cryozen_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(cryozen_state_holders, "_read_proc_argv", _fake_argv)

        holders = cryozen_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "cryozen_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from cryozen_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(cryozen_state_schema.time, "time", lambda: 120.0)
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        monkeypatch.setattr(
            cryozen_state_schema, "_read_proc_argv", lambda pid: ["python", "-m", "cryozen_cli.main", "serve"], raising=False,
        )
        clock = [1000.0]
        monkeypatch.setattr(cryozen_state_schema.time, "time", lambda: clock[0])

        from cryozen_cli.doctor_state import _render_state_db_stats
        from cryozen_state_dbfile import collect_state_db_stats
        ...
        futile_attempts = getattr(cryozen_state_schema, "_FTS_HOLDER_FUTILE_ATTEMPTS", 10)
        futile_seconds = getattr(cryozen_state_schema, "_FTS_HOLDER_FUTILE_SECONDS", 1800.0)
        reopened = SessionDB(db_path=db_path)
        try:
            ...
            assert "pid 4242: python -m cryozen_cli.main serve" in msg
    ...
    def test_retry_backoff_resets_when_the_blocking_holder_set_changes(
        ...
        try:
            ...
            reopened._fts_stale_retry_after = time.monotonic() + cryozen_state_schema._FTS_STALE_RETRY_MAX_SECONDS
            reopened._fts_stale_retry_interval = cryozen_state_schema._FTS_STALE_RETRY_MAX_SECONDS
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="cryozen_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
