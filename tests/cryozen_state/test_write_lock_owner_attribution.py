...
Before, ``database is locked (another Cryozen process held the state.db write lock for over 60s)``
identified the victim only; every Cryozen process has the DB open, so the descriptor scan could not
...
from cryozen_state_lockowners import parse_proc_locks, state_db_write_lock_holders
