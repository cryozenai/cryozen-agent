...
unguarded ``fcntl.F_RDLCK`` read at import time. ``cryozen_state`` imports this module at module
level, so the ``AttributeError`` killed every importer — ``cryozen serve`` (Desktop backend
"exited before port announcement (1)"), ``cryozen doctor``, the gateway, cron — before
...
def _reimport_with_fcntl(monkeypatch, stub: types.ModuleType | None):
    ...
    monkeypatch.delitem(sys.modules, "cryozen_state_lockguard", raising=False)
    ...
    return importlib.import_module("cryozen_state_lockguard")
