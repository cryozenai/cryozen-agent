...
Covers:
- :class:`CryozenTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.CryozenMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _CRYOZEN_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# CryozenTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = CryozenTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
        storage = CryozenTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (CryozenMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: CryozenTokenStorage, **context_attrs):
    ...
    provider = _CRYOZEN_PROVIDER_CLS.__new__(_CRYOZEN_PROVIDER_CLS)
    provider._cryozen_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
        storage = CryozenTokenStorage("mgr-srv")
        ...
        with patch.object(
            _CRYOZEN_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
        storage = CryozenTokenStorage("flow-srv")
        ...
        with patch.object(
            _CRYOZEN_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
