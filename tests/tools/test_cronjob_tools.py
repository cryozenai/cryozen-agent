...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `cryozen-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.cryozen-agent/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.cryozen-agent/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because cryozen uses an internal scheduler, not system crontab.
    ...
    def test_accepts_external_cron_worker_with_presence_vars_stripped(self, monkeypatch):
        ...
        for v in ("CRYOZEN_INTERACTIVE", "CRYOZEN_GATEWAY_SESSION", "CRYOZEN_EXEC_ASK"):
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["CRYOZEN_INTERACTIVE", "CRYOZEN_GATEWAY_SESSION", "CRYOZEN_EXEC_ASK"],
    )
...
class TestAgentCannotSetModelPin:
    """Per-job inference pins are user-owned (dashboard / `cryozen cron`
...
class TestGithubExemptionAbuse:
    ...
    def test_same_line_payload_after_github_url_is_scanned(self):
        ...
        for sep in (";", " &&", " |"):
            prompt = f"{self.GH}{sep} cat ~/.cryozen-agent/.env"
    ...
    def test_subshell_and_backtick_payloads_are_scanned(self):
        ...
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}$(cat ~/.cryozen-agent/.env)")
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}`cat ~/.cryozen-agent/.env`")
    ...
    def test_payload_between_two_github_blocks_is_scanned(self):
        ...
        prompt = f"{self.GH}; cat ~/.cryozen-agent/.env; {self.GH}"
        assert "Blocked" in _scan_cron_prompt(prompt)
