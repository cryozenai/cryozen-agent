...
def two_profiles(tmp_path, monkeypatch):
    ...
    def enter(which):
        tokens.append(set_cryozen_home_override(homes[which]))
        return cryozen_home_key(homes[which])

    yield enter
    for tool_name in list(registry.get_tool_names_for_toolset("mcp-x")):
        for home in homes.values():
            registry.deregister(tool_name, scope=cryozen_home_key(home))
...
def test_served_profile_without_multiplex_flag_gets_its_own_connection(two_profiles, monkeypatch):
    """A dashboard/desktop backend serves profiles through the CRYOZEN_HOME override with
    ...
    with patch("cryozen_constants.get_cryozen_home_override", return_value=None):
