"""Cross-session CRYOZEN_SESSION_ID leak via the shared bash snapshot.
...
``CRYOZEN_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $CRYOZEN_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (CRYOZEN_SESSION_* / UI /
...
run stamps per command (CRYOZEN_DELEGATED_CHILD_CONTEXT, CRYOZEN_CRON_SESSION), or
...
def test_export_snippet_shape():
    snippet = _export_dump_excluding_session_vars('"$__cryozen_snap_tmp"')
    ...
    assert "${!CRYOZEN_SESSION_*}" in snippet
    assert "${!CRYOZEN_CRON_AUTO_DELIVER_*}" in snippet
    assert "${!CRYOZEN_BROWSER_CONTROL_*}" in snippet
    assert "CRYOZEN_UI_SESSION_ID" in snippet
    assert "grep -vE" not in snippet
    assert '"$__cryozen_snap_tmp"' in snippet
    ...
    assert snippet.rstrip().endswith('> "$__cryozen_snap_tmp"')
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$CRYOZEN_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "CRYOZEN_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
...
def test_export_dump_drops_every_bridged_var_and_the_delegation_marker():
    ...
    exports = "; ".join([f'export {n}="x"' for n in scoped] + ['export CRYOZEN_HOME="/h"', 'export MYVAR="keep"'])
    out = subprocess.run(
    ...
    assert 'declare -x CRYOZEN_HOME="/h"' in out
