...
2. ``local_delivery_command`` hardcoded ``"cryozen"``, relying on PATH —
   ...
   basename so resolved absolute paths (and ``cryozen.exe``) still take
...
def test_local_delivery_resolves_sibling_cryozen(tmp_path, monkeypatch):
    bin_dir = tmp_path / "venv" / "bin"
    bin_dir.mkdir(parents=True)
    sibling = bin_dir / "cryozen"
    sibling.touch()
    sibling.chmod(0o755)
...
def test_local_delivery_uses_shutil_which_when_no_sibling(tmp_path, monkeypatch):
    ...
    which_hit = str(tmp_path / "usr-local-bin" / "cryozen")
    monkeypatch.setattr(
        bot_relay.shutil, "which", lambda name: which_hit if name == "cryozen" else None
    )
...
def test_delivery_lock_recognizes_resolved_cli_paths(tmp_path, monkeypatch):
    ...
    now that argv[0] may be a resolved absolute path (or cryozen.exe)."""
    acquired = []
    ...
    with bot_mode_dm._delivery_lock(
        [str(tmp_path / "venv" / "bin" / "cryozen"), "-p", "ops", "chat"],
        stdin_file=False,
    ):
        pass
    with bot_mode_dm._delivery_lock(["cryozen", "-p", "ops", "chat"], stdin_file=False):
        pass
    with bot_mode_dm._delivery_lock(
        ["C:\\venv\\Scripts\\cryozen.exe", "-p", "ops", "chat"], stdin_file=False
    ):
        pass
