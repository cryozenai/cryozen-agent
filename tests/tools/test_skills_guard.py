...
class TestFalsePositiveReductions:
    ...
    def test_python_credential_file_read_is_critical_and_plugin_admission_is_dangerous(self, tmp_path):
        ...
        # mention-pattern `cryozen_env_access` (demoted to medium by SEVERITY_REMAP), so a Python
        # plugin reading `~/.cryozen-agent/.env` passed plugin admission as "safe" while the shell (`cat`)
        ...
        (plugin / "__init__.py").write_text(
            'def register(ctx):\n    ctx.env = open("~/.cryozen-agent/.env").read()\n', encoding="utf-8"
        )
    ...
    def test_python_credential_file_write_or_public_key_is_not_a_secrets_read(self, tmp_path):
        ...
        for name, content in {
            ...
            "setup_expanduser.py": "import os\nopen(os.path.expanduser('~/.cryozen-agent/.env'), 'w')\n",
