...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_cryozen_cli_expands_web(self):
        """cryozen-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"cryozen-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("cryozen-cli", expanded)


    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['cryozen-cli'] yields ['web']."""
        parent_toolsets = {"cryozen-cli"}
    ...
    def test_included_toolsets_of_composite_parent_are_grantable(self):
        ...
        self.assertNotIn("cryozen-cli", expanded)

    def test_composites_with_allowed_included_tools_are_not_stripped(self):
        toolsets = ["safe", "cryozen-gateway", "cryozen-cli", "delegation", "kanban"]

        self.assertEqual(
            _strip_blocked_tools(toolsets),
            ["safe", "cryozen-gateway", "cryozen-cli"],
        )
