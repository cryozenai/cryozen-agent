...
class TestDeregisterAuthorization:
    ...
    def test_plugin_cannot_deregister_unowned_tool_without_opt_in(self):
        reg = self._reg()
        reg.register_plugin_override_policy("cryozen_plugins.evil", False)
        with patch.object(ToolRegistry, "_caller_module", return_value="cryozen_plugins.evil"):
            import pytest
    ...
    def test_plugin_root_module_can_deregister_submodule_handler(self):
        ...
        cryozen_plugins.pkg (root cleanup code) must be allowed to deregister a
        ...
        handler = eval("lambda *a, **k: 'sub'", {"__name__": "cryozen_plugins.pkg.handlers"})
        reg.register(
            name="sub_tool", toolset="pkg-ts",
            schema={"name": "sub_tool", "description": "", "parameters": {"type": "object", "properties": {}}},
            handler=handler,
        )
        # Caller is the plugin root (cryozen_plugins.pkg), handler is in a
        # submodule (cryozen_plugins.pkg.handlers) — must be allowed.
        with patch.object(ToolRegistry, "_caller_module", return_value="cryozen_plugins.pkg"):
    ...
    def test_opted_in_plugin_submodule_can_deregister(self):
        ...
        root (``cryozen_plugins.allowed``).  If the caller is a submodule
        (``cryozen_plugins.allowed.cleanup``), the old code looked up
        ...
        reg.register_plugin_override_policy("cryozen_plugins.allowed", True)
        with patch.object(ToolRegistry, "_caller_module", return_value="cryozen_plugins.allowed.cleanup"):
            reg.deregister("protected")
        assert reg._tools.get("protected") is None


    def test_core_code_deregister_always_allowed(self):
        """Non-plugin callers (core Cryozen code) are never gated."""
        reg = self._reg()
    ...
    def test_full_bypass_blocked(self):
        ...
        with pytest.raises(PermissionError):
            evil_handler = eval("lambda *a, **k: 'hijacked'", {"__name__": "cryozen_plugins.evil"})
