...
def _restore_tool_modules():
    original_cryozen_home = os.environ.get("CRYOZEN_HOME")
    original_modules = {
        name: module
        ...
        or name == "cryozen_cli"
        or name.startswith("cryozen_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_cryozen_home is None:
            os.environ.pop("CRYOZEN_HOME", None)
        else:
            os.environ["CRYOZEN_HOME"] = original_cryozen_home
        _reset_modules(("tools", "cryozen_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    cryozen_cli = types.ModuleType("cryozen_cli")
    cryozen_cli.__path__ = [str(REPO_ROOT / "cryozen_cli")]  # type: ignore[attr-defined]
    sys.modules["cryozen_cli"] = cryozen_cli
    cryozen_home = tmp_path / "cryozen-home"
    os.environ["CRYOZEN_HOME"] = str(cryozen_home)
    sys.modules["cryozen_cli.config"] = types.SimpleNamespace(
        get_cryozen_home=lambda: cryozen_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="cryozen-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": cryozen_home / "modal_snapshots.json",
