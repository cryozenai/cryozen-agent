...
was gated on ``[ -f "$CRYOZEN_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'CRYOZEN_HOME="{home}"\n'
        # In tests we run unprivileged; as_cryozen is a passthrough then.
        'as_cryozen() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Cryozen loads $CRYOZEN_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/cryozen/.env.example -> $CRYOZEN_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $CRYOZEN_HOME/.env from it on first boot"
    )
