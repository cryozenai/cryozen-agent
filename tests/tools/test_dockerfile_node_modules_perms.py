...
Older images made /opt/cryozen/ui-tui and /opt/cryozen/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV CRYOZEN_TUI_DIR=/opt/cryozen/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R cryozen:cryozen /opt/cryozen/ui-tui" not in text
    assert "chown -R cryozen:cryozen /opt/cryozen/node_modules" not in text
