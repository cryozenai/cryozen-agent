...
def _entry(name, *, platforms=(), requires_env=()):
    return SimpleNamespace(
        ...
        description=f"Drives {name}. More text.", requires_cryozen="", platforms=list(platforms),
...
def test_only_a_setup_profile_session_selects_the_tool(tmp_path, monkeypatch):
    import model_tools
    from cryozen_cli.profiles import write_profile_meta
    from cryozen_constants import reset_cryozen_home_override, set_cryozen_home_override

    homes = {}
    for name, role in (("default", None), ("cryozen-setup", "setup")):
        homes[name] = tmp_path / name
        homes[name].mkdir()
        if role:
            write_profile_meta(homes[name], role=role)
    for name, expected in (("default", False), ("cryozen-setup", True)):
        token = set_cryozen_home_override(str(homes[name]))
        try:
...
def test_an_approved_row_installs_into_default_and_lists_the_live_tools(tmp_path):
    from cryozen_cli.profiles import get_profile_dir, write_profile_meta
    from cryozen_constants import reset_cryozen_home_override, set_cryozen_home_override

    setup_home = tmp_path / "cryozen-setup"
    setup_home.mkdir()
    ...
    token = set_cryozen_home_override(str(setup_home))  # the call comes from the setup chat
    try:
