...
def fake_cryozen(tmp_path, monkeypatch):
    """Build a two-profile Cryozen layout and point CRYOZEN_HOME at
    the cryozen-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_cryozen):
        from tools.file_tools import write_file_tool
        target = fake_cryozen["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_cryozen):
        ...
        target = fake_cryozen["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_cryozen_path_unaffected(self, fake_cryozen, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_cryozen):
    ...
    def test_cross_profile_patch_bypass(self, fake_cryozen):
        from tools.file_tools import patch_tool
        target = fake_cryozen["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_cryozen):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_cryozen, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_cryozen["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up CRYOZEN_HOME (set in
        ...
        assert "not found in active profile 'cryozen-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.cryozen-agent/profiles/<name>/."""
        # Don't set CRYOZEN_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_cryozen_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_cryozen_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_cryozen):
        """When active profile is 'cryozen-security', the prompt warns
        ...
        assert "Active Cryozen profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.cryozen-agent/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Cryozen profile: default" in src
        assert "Active Cryozen profile: {active_profile}" in src
