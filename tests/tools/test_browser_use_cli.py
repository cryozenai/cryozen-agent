...
* Mode detection — ``browser.backend: browser-use`` in config (set via the
  ``cryozen tools`` picker); off by default.
...
class TestModeDetection:
    def test_default_on_when_cli_available(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_cli.config.read_raw_config", lambda: {})
    ...
    def test_explicit_off_wins_over_default(self, monkeypatch):
        monkeypatch.setattr(
            "cryozen_cli.config.read_raw_config",
    ...
    def test_config_read_failure_uses_default(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_cli.config.read_raw_config", boom)
        monkeypatch.setattr(bu_cli, "_find_cli", lambda: None)
        assert bu_cli.is_browser_use_cli_mode() is False
...
class TestSubprocessEnvironment:
    ...
    def test_subprocess_env_strips_parent_python_import_paths(self, monkeypatch):
        ...
        own Python — inherited PYTHONPATH/PYTHONHOME pointing at Cryozen's
        ...
        browser_tool = ModuleType("tools.browser_tool")
        browser_tool._build_browser_env = lambda: {
            "PYTHONPATH": "/cryozen:/cryozen/venv/lib/site-packages",
            "PYTHONHOME": "/cryozen/venv",
            "KEEP_ME": "yes",
        }
        monkeypatch.setitem(sys.modules, "tools.browser_tool", browser_tool)
...
class TestToolSurfaceSwap:
    ...
    def test_browser_exec_in_browser_toolsets(self):
        from toolsets import TOOLSETS, _CRYOZEN_CORE_TOOLS

        assert "browser_exec" in _CRYOZEN_CORE_TOOLS
...
class TestVaultSupervisorAttach:
    def test_exec_attaches_supervisor_to_the_browser_it_drives(self, tmp_path, monkeypatch, _fake_supervisor_registry):
        ...
        monkeypatch.setattr("cryozen_cli.config.read_raw_config", lambda: {"browser": {"backend": "browser-use"}})
...
class TestLegacyCloudMigration:
    ...
    def test_direct_api_config_migrates(self, monkeypatch):
        monkeypatch.setattr("cryozen_cli.config.read_raw_config", lambda: self._LEGACY)
    ...
    def test_camofox_user_does_not_migrate(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "cryozen_cli.config.read_raw_config", lambda: {"browser": {}}
        )
        monkeypatch.setenv("BROWSER_USE_API_KEY", "bu-key")
    ...
    def test_picker_highlights_cli_row_for_migrated_config(self, monkeypatch):
        from cryozen_cli.tools_config import TOOL_CATEGORIES, _is_provider_active
...
class TestOwnTabPreamble:
    ...
    def test_named_shared_browser_gets_preamble(self, tmp_path, monkeypatch):
        ...
        assert result["success"] is True
        assert "_cryozen_ensure_own_tab" in result["output"]
        # model code still present, after the preamble
        assert result["output"].index("_cryozen_ensure_own_tab") < result["output"].index("print('payload')")
    ...
    def test_named_packaged_chromium_skips_preamble(self, tmp_path, monkeypatch):
        ...
        assert result["success"] is True
        assert "_cryozen_ensure_own_tab" not in result["output"]
    ...
    def test_sentinel_never_reaches_subprocess_env(self, tmp_path, monkeypatch):
        ...
        cli = _fake_cli(tmp_path, 'cat > /dev/null\necho "sentinel:${_CRYOZEN_BU_PRIVATE_BROWSER:-unset}"\n')
        monkeypatch.setattr(bu_cli, "_find_cli", lambda: [cli])
        result = json.loads(bu_cli.browser_exec("print(1)", session="r7k2"))
...
class TestProviderPickerIntegration:
    """The `cryozen tools` Browser Automation picker row (browser_backend
    ...
    def test_picker_row_names_stay_unique(self):
        ...
        from cryozen_cli.tools_config import TOOL_CATEGORIES, _plugin_browser_providers
    ...
    def test_selecting_cli_row_writes_backend_and_keeps_cloud_provider(self):
        from cryozen_cli.tools_config import _write_provider_config
    ...
    def test_selecting_provider_row_keeps_cli_mode(self):
        ...
        (local/Browserbase/Firecrawl) keeps the driver choice."""
        from cryozen_cli.tools_config import _write_provider_config
    ...
    def test_provider_row_stays_active_alongside_cli_mode(self, monkeypatch):
        from cryozen_cli.tools_config import _is_provider_active
...
class TestBrowserUseSlashCommand:
    ...
    def _run(self, cmd, config, monkeypatch):
        import cryozen_cli.config as hc
...
class TestBrowserSlashDispatch:
    ...
    def _run(self, cmd, monkeypatch):
        ...
        import cryozen_cli.cli_commands_mixin as mod
    ...
    def test_connect_keeps_url_case_and_defaults_to_status(self, monkeypatch):
        from cryozen_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
...
class TestFindCliManagedBin:
    """MANAGED-FIRST: _find_cli probes $CRYOZEN_HOME/bin before PATH and
    ~/.local/bin, so the Cryozen-installed copy always wins."""
    ...
    def test_managed_bin_precedes_user_local_bin(self, tmp_path, monkeypatch):
        """MANAGED-FIRST: Cryozen' managed copy wins over a user-level side
...
class TestInstallCli:
    def test_path_install_does_not_short_circuit(self, tmp_path, monkeypatch):
        ...
        otherwise resolution stays pinned to a binary Cryozen can't update."""
        ...
        fake = _types.ModuleType("cryozen_cli.managed_uv")
        fake.ensure_uv = lambda **kw: None
        monkeypatch.setitem(_sys.modules, "cryozen_cli.managed_uv", fake)
...
class TestDefaultDowngradeNotice:
    ...
    def test_notice_when_default_and_cli_missing(self, tmp_path, monkeypatch):
        ...
        assert "cryozen tools" in notice
...
class TestLightpandaBackendResolution:
    """browser.engine: lightpanda in Browser Use mode — Cryozen spawns
...
class TestLightpandaStatusLine:
    ...
    def test_status_reports_lightpanda_in_use(self, monkeypatch):
        out = self._status(monkeypatch, used=True, reason="Browser Use mode: Cryozen spawns `lightpanda serve` per session")
