...
try:
    ...
    pytest.skip("cryozen-agent tools not importable (missing deps)", allow_module_level=True)
...
class TestDockerHostBindApproval:
    ...
    def _isolate_approval_state(monkeypatch):
        ...
        hermetic CRYOZEN_HOME fixture runs), so on a dev machine whose real
        ...
        CRYOZEN_YOLO_MODE off the environment when the module is imported at
        collection time, before conftest's per-test env blanking runs. A test
        run launched from a --yolo Cryozen session (or any shell exporting
        CRYOZEN_YOLO_MODE=1) freezes True and every guard auto-approves.
