...
def worker_home(tmp_path, monkeypatch):
    ...
    for var in ("CRYOZEN_KANBAN_DB", "CRYOZEN_KANBAN_WORKSPACES_ROOT", "CRYOZEN_KANBAN_HOME", "CRYOZEN_KANBAN_BOARD"):
...
def test_seed_then_inject_new_comment(worker_home, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_PROFILE", "worker-bot")
    agent = FakeAgent()
...
def test_delegated_child_in_worker_process_neither_receives_nor_consumes_notes(worker_home, monkeypatch):
    """A delegate_task child inherits the worker's ``CRYOZEN_KANBAN_TASK``; operator notes
