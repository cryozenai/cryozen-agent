...
def test_real_process_restart_restores_owned_completion_once(tmp_path):
    ...
    env = {**os.environ, "CRYOZEN_HOME": str(tmp_path), "PYTHONPATH": repo}
    producer = r'''
import time
from tools import async_delegation as ad
r = ad.dispatch_async_delegation(
...
def test_one_child_unit_keeps_its_finished_child_when_the_owner_dies(tmp_path):
    ...
    env = {**os.environ, "CRYOZEN_HOME": str(tmp_path), "PYTHONPATH": repo, "REPRO_MARKER": str(marker)}
    producer = r'''
import os, sys, time
from unittest.mock import MagicMock
import tools.delegate_tool as dt
...
def test_connect_creates_state_db_0o600_under_permissive_umask(tmp_path, monkeypatch):
    """``_connect`` shares state.db with cryozen_state.SessionDB -- a fresh
    CRYOZEN_HOME must land the file (and its WAL sidecar, if created) at 0o600
