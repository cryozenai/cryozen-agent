...
def test_output_path_rejects_cryozen_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    cryozen_home = tmp_path / "cryozen-home"
    cryozen_home.mkdir()
    monkeypatch.setattr(file_safety, "_cryozen_home_path", lambda: cryozen_home)
    monkeypatch.setattr(file_safety, "_cryozen_root_path", lambda: cryozen_home)

    target = cryozen_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    cryozen_home = tmp_path / "cryozen-home"
    token_dir = cryozen_home / "mcp-tokens"
