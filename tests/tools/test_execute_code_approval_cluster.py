...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad CRYOZEN_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: CRYOZEN_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_cryozen_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "CRYOZEN_HOME": "/h", "CRYOZEN_PROFILE": "p",
        "CRYOZEN_CONFIG": "/c.yaml", "CRYOZEN_ENV": "/e",
        "CRYOZEN_DELEGATED_CHILD_CONTEXT": "1",
        # other CRYOZEN_* → dropped (broad prefix removed)
        "CRYOZEN_BASE_URL": "https://x", "CRYOZEN_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "CRYOZEN_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "CRYOZEN_BASE_URL", "CRYOZEN_INTERACTIVE", "CRYOZEN_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped CRYOZEN_* vars."""
    import logging
