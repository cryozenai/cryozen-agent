...
def _set_xai_oauth_unavailable(monkeypatch):
    ...
    import cryozen_cli.auth_xai as auth_xai
...
def test_prefer_api_key_falls_back_to_oauth_without_explicit_key(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "cryozen_cli.config.get_env_value", lambda name, default=None: default
    )
    _install_fake_oauth_pool(monkeypatch, "oauth-token-x1")
...
def test_prefer_api_key_honors_cryozen_xai_base_url_with_validation(monkeypatch):
    """The preferred-key path reads the same override pair as the OAuth
    branch (CRYOZEN_XAI_BASE_URL first, then XAI_BASE_URL) behind the same
    ...
    monkeypatch.setattr(
        ...
        lambda name, default=None: {
            ...
            "CRYOZEN_XAI_BASE_URL": "https://staging.x.ai/v1",
...
# ---------------------------------------------------------------------------
# #116155: `cryozen auth remove xai` suppresses env:XAI_API_KEY, but a stale value
