...
def test_local_terminal_requirements(monkeypatch, caplog):
    """Local backend uses Cryozen' own LocalEnvironment wrapper."""
...
def test_modal_backend_without_credentials_logs_clear_error(monkeypatch, caplog, tmp_path):
    _clear_terminal_env(monkeypatch)
    ...
    assert ok is False
    assert any("no Modal credentials/config were found" in record.getMessage() for record in caplog.records)
