...
and the shared slash-command handlers (cryozen_cli/write_approval_commands.py).
...
@pytest.fixture
def cryozen_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="cryozen_wa_test_")
    home = os.path.join(d, ".cryozen-agent")
    os.makedirs(home)
    monkeypatch.setenv("CRYOZEN_HOME", home)
    yield home
...
def test_default_gate_is_off(cryozen_home):
...
def test_invalid_subsystem_is_off(cryozen_home):
    from tools import write_approval as wa
    assert wa.write_approval_enabled("bogus") is False


def test_list_pending_skips_non_dict_record(cryozen_home):
...
def test_memory_gate_off_allows_write(cryozen_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(cryozen_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(cryozen_home, monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.config.load_config", _boom)
...
def test_handle_approve_all(cryozen_home):
...
def test_handle_approve_surfaces_overwritten_entry(cryozen_home):
...
def test_handle_approval_on(cryozen_home):
    from cryozen_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(cryozen_home):
    from cryozen_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(cryozen_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(cryozen_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(cryozen_home):
