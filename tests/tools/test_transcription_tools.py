...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("CRYOZEN_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("CRYOZEN_LOCAL_STT_LANGUAGE", raising=False)
...
class TestTranscribeLocalCommand:
    def test_command_provider_uses_sanitized_child_env(self, monkeypatch):
        """Salvage of #56332: command STT must not inherit Cryozen secrets."""
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("CRYOZEN_LOCAL_STT_LANGUAGE", "en")
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.cryozen_xai_user_agent = MagicMock(return_value="cryozen-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("CRYOZEN_LOCAL_STT_LANGUAGE", "fr")
...
class TestExplicitOpenaiSelectionError:
    """A stored openai selection with no credentials must not be reported as generic setup guidance.

    When ``_resolve_openai_audio_client_config()`` raises its selection-specific ValueError (the
    ``cryozen tools`` remediation naming the stored selection), the old boolean probe flattened it
    into False — the log said "no API key" and the transcription result returned the all-provider
    install hint, pointing operators at unrelated setup.
    """

    def _no_openai_credentials(self, monkeypatch, *, selected="openai"):
        monkeypatch.delenv("VOICE_TOOLS_OPENAI_KEY", raising=False)
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            "tools.tool_backend_helpers.read_selection", lambda section: selected,
        )

    def test_get_provider_openai_none_names_the_selection(self, monkeypatch, caplog):
        ...
        assert "stt is configured to use openai" in warning
        assert "no API key available" not in warning

    def test_dispatch_returns_selection_specific_error(self, monkeypatch):
        """The final transcription result carries the selection error and its cryozen tools
        remediation instead of the all-provider install hint."""
        self._no_openai_credentials(monkeypatch)
        monkeypatch.setattr(
            "tools.transcription_tools._load_stt_config", lambda: {}
        )
        ...
        assert result["success"] is False
        assert "stt is configured to use openai" in result["error"]
        assert "cryozen tools" in result["error"]
    ...
    def test_auto_detect_none_keeps_generic_hint(self, monkeypatch):
        ...
        self._no_openai_credentials(monkeypatch, selected=None)
        monkeypatch.setattr(
            "tools.transcription_tools._load_stt_config", lambda: {}
        )
        with patch("tools.transcription_tools._HAS_OPENAI", True), \
