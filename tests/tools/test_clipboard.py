...
Coverage:
  cryozen_cli/clipboard.py  — platform-specific image extraction (macOS, WSL, Wayland, X11)
...
from cryozen_cli.clipboard import (
...
class TestSaveClipboardImage:
    def test_creates_parent_dirs(self, tmp_path):
        ...
        with patch("cryozen_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("cryozen_cli.clipboard._linux_save", return_value=False):
...
class TestClipboardChildStdin:
    def test_probe_uses_devnull_stdin(self):
        with patch("cryozen_cli.clipboard.subprocess.run") as mock_run:
    ...
    def test_pipe_to_file_uses_devnull_stdin(self, tmp_path):
        ...
        with patch("cryozen_cli.clipboard.subprocess.run", side_effect=fake_run):
...
class TestMacosClipboardFileUrl:
    ...
    Other apps still paste the image; Cryozen must treat a local image file-url
    ...
    def test_only_copied_image_files_are_clipboard_images(self, tmp_path, name, expected):
        ...
        with patch("cryozen_cli.clipboard.subprocess.run", side_effect=self._furl_run(src)):
            assert _macos_has_image() is expected
...
class TestLinuxSave:
    ...
    def test_wsl_tried_first(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("cryozen_cli.clipboard._is_wsl", return_value=True):
            with patch("cryozen_cli.clipboard._wsl_save", return_value=True) as m:
                assert _linux_save(dest) is True
                m.assert_called_once_with(dest)

    def test_wayland_fails_falls_through_to_xclip(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("cryozen_cli.clipboard._is_wsl", return_value=False):
            with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                with patch("cryozen_cli.clipboard._wayland_save", return_value=False):
                    with patch("cryozen_cli.clipboard._xclip_save", return_value=True) as m:
                        assert _linux_save(dest) is True
                        m.assert_called_once_with(dest)
...
class TestWindowsHasImage:
    def setup_method(self):
        import cryozen_cli.clipboard as cb
        cb._ps_exe = False  # reset cache

    def test_clipboard_has_image(self):
        with patch("cryozen_cli.clipboard._get_ps_exe", return_value="powershell"):
            with patch("cryozen_cli.clipboard.subprocess.run") as mock_run:
...
class TestConvertToPng:
    ...
    def test_imagemagick_failure_preserves_original(self, tmp_path, failure):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch("cryozen_cli.clipboard.subprocess.run", side_effect=side_effect):
...
class TestHasClipboardImage:
    ...
    def test_macos_dispatch(self):
        ...
        with patch("cryozen_cli.clipboard._macos_has_image", return_value=True) as m:
    ...
    def test_wsl_falls_through_to_wayland_when_windows_path_empty(self):
        ...
        """
        with patch("cryozen_cli.clipboard._is_wsl", return_value=True):
            with patch("cryozen_cli.clipboard._wsl_has_image", return_value=False) as wsl:
                with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                    with patch("cryozen_cli.clipboard._wayland_has_image", return_value=True) as wl:
                        assert has_clipboard_image() is True
...
class TestPreprocessImagesWithVision:
    ...
    @pytest.fixture
    def cli(self):
        """Minimal CryozenCLI with mocked internals."""
        with patch("cli.load_cli_config") as mock_cfg:
            ...
            with patch.dict("os.environ", {"OPENROUTER_API_KEY": "test-key"}):
                with patch("cli.CLI_CONFIG", mock_cfg.return_value):
                    ...
                    cli_obj = CryozenCLI.__new__(CryozenCLI)
...
class TestTryAttachClipboardImage:
    ...
    def test_image_found_attaches(self, cli):
        with patch("cryozen_cli.clipboard.save_clipboard_image", return_value=True):
    ...
    def test_image_path_follows_naming_convention(self, cli):
        ...
        assert path.parent == Path(os.environ["CRYOZEN_HOME"]) / "images"
