...
def gateway_session(monkeypatch):
    ...
    for k in ("CRYOZEN_CRON_SESSION", "CRYOZEN_YOLO_MODE", "CRYOZEN_INTERACTIVE"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("CRYOZEN_GATEWAY_SESSION", "1")
    monkeypatch.setenv("CRYOZEN_SESSION_KEY", SESSION_KEY)
    # ``--yolo`` is frozen at import from CRYOZEN_YOLO_MODE; a host shell running yolo must not
...
def cli_session(monkeypatch):
    mod._session_approved.clear()
    for k in ("CRYOZEN_CRON_SESSION", "CRYOZEN_YOLO_MODE", "CRYOZEN_GATEWAY_SESSION", "CRYOZEN_EXEC_ASK"):
        monkeypatch.delenv(k, raising=False)
