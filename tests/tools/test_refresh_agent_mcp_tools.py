...
def test_preserve_prefix_keeps_the_bridge_tools_byte_identical(monkeypatch):
    ...
    deferred-tool count and the embedded listing. Either can move between turns (a late
    MCP server, a ``check_fn`` flap), and a moved byte in the tool array re-prefills
    ...
    built["function"]["description"] = "Search 21 additional tools. Listing as built."
    ...
    # manage_connections flapped out (check_fn flap); a late server grew the count.
