...
def _is_cryozen_cli(argv) -> bool:
    """Match the delivery CLI by basename — local_delivery_command may
    resolve the venv-relative cryozen next to the interpreter (#93590)."""
    name = str(argv[0]).rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
    return name in ("cryozen", "cryozen.exe")
...
def _transport_calls(calls):
    ...
    return [argv for argv in calls if argv and _is_cryozen_cli(argv)]
...
def test_deliver_retries_same_argv_on_transient_failure(home, monkeypatch):
    ...
    def _fake_run(argv, **kwargs):
        ...
        if not _is_cryozen_cli(list(argv)):
            return _Proc(0)
...
def test_deliver_failure_carries_typed_reason(home, monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.quiet_single_query.run_reported_turn",
        lambda argv, **k: _Proc(1, stderr="502 server error - overloaded")
        if _is_cryozen_cli(list(argv))
        else _Proc(0),
    )
    out = _deliver({"profile": "ops", "message": "ping"})
    assert "error" in out
...
# `cryozen … -Q` prints the turn's final_response (the provider prose) on STDOUT and the session
...
def test_deliver_retry_reads_the_stream_the_cli_writes_and_resumes_the_persisted_row(home, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        if _is_cryozen_cli(list(argv)) else _Proc(0),
    )
    out = _deliver({"profile": "ops", "message": "ping"})
    assert out["error"]["data"]["reason"] == bfr.PROVIDER_SERVER_ERROR
...
def test_run_local_turn_retry_reads_the_stream_the_cli_writes_and_resumes_the_persisted_row(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(bot_mode_dm.subprocess, "run", _fake_run)
    rc = bot_mode_dm._run_local_turn(["cryozen", "-p", "ops", "chat"], str(dm), env={"CRYOZEN_HOME": str(tmp_path)})
    assert rc == 0
    assert envs[0] == {"CRYOZEN_HOME": str(tmp_path)}
    assert envs[1] == {"CRYOZEN_HOME": str(tmp_path), RESUME_UNANSWERED_TURN_ENV: "1"}
    assert "the reply text" in capsys.readouterr().out
