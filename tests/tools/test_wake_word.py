"""Tests for tools.wake_word — the "Hey Cryozen" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey cryozen"}) == "hey cryozen"
    assert ww.wake_phrase({}) == "hey cryozen"
...
def test_looks_like_path():
    ...
    assert _looks_like_path("models/hey_cryozen.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "cryozen_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey cryozen"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey cryozen"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_cryozen": object()}

        def predict(self, frame):
            return {"hey_cryozen": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_cryozen.onnx"}}
    )
    assert calls["download"] == [["/models/hey_cryozen.onnx"]]
    assert eng._labels == ["hey_cryozen"]


def test_bundled_hey_cryozen_model_ships_on_disk():
    # The "hey cryozen" wake word works out of the box only if the model is
