...
``cryozen_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``CRYOZEN_`` prefix was dropped
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            out[k] = v
            continue
        if k in _CRYOZEN_CHILD_ALLOWED:
            out[k] = v
    return out
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # CRYOZEN_* handling (#27303): only the operational allowlist passes;
        # every other CRYOZEN_* is dropped (the broad prefix was removed).
        "CRYOZEN_HOME": "/home/alice/.cryozen-agent",        # allowlisted → kept
        "CRYOZEN_PROFILE": "default",                 # allowlisted → kept
        "CRYOZEN_INTERACTIVE": "1",                   # not allowlisted → dropped
        "CRYOZEN_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "CRYOZEN_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``cryozen_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``cryozen_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        import tempfile, ast
        stub = generate_cryozen_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_cryozen_tools_module(["terminal"], transport="uds")
...
# let the child import cryozen_tools, a user script that printed non-ASCII
...
def _configured_timezone_child_env():
    return code_execution_env._build_child_env(
        rpc_endpoint="socket",
        rpc_token="token",
        tmpdir="/tmp/cryozen-code-execution-test",
...
def test_windows_child_keeps_os_local_timezone_when_timezone_is_configured(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_time.get_timezone_name", lambda: "America/Los_Angeles")
