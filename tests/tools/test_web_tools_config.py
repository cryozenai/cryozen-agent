...
class TestBackendSelection:
    ...
    ``cryozen tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_xai_only_env_end_to_end_toolset_gate(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))  # isolate auth.json / credential pool
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from cryozen_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.cryozen-agent/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.cryozen-agent/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_keenable_search_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Keenable-Title"] == "cryozen-agent"
    ...
    def test_tavily_request_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Client-Name"] == "cryozen-agent"
