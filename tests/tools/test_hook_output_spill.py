...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("cryozen_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="cryozen-spill-test-")
    ...
    def test_default_directory_uses_cryozen_home(self):
        """When no directory override, spill under CRYOZEN_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="cryozen-home-")
        try:
            with patch.dict(os.environ, {"CRYOZEN_HOME": test_home}):
                # Also patch get_cryozen_home to the env var to mirror production.
            ...
            # ~/.cryozen-agent/hook_outputs depending on get_cryozen_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.cryozen-agent/hook_outputs/sess")),
            ]
