...
def _assert_cryozen_identity_headers(headers):
    """Both Perplexity endpoints carry the Cryozen identity headers (same set as Kimi/OpenCode)
    ...
    assert headers["X-Pplx-Integration"] == "cryozen-agent"
...
def test_search_dispatch_maps_search_api_shape():
    ...
    _assert_cryozen_identity_headers(post.call_args.kwargs["headers"])
