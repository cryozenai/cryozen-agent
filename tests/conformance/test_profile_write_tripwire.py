...
--------------------
When a non-default profile is active (``CRYOZEN_HOME`` points at
...
class ProfileTripwire:
    ...
    Attributes:
        root:     the default profile's home (``<tmp>/cryozen``).
...
def profile_tripwire(tmp_path, monkeypatch):
    ...
    1. A temp cryozen root acting as the *default* profile, seeded with the
       ...
       detectable.
    2. ``CRYOZEN_HOME`` (env var AND the context-local override) pointed at
       the testprof home — the exact activation shape ``--profile`` uses.
    3. ``cryozen_state.DEFAULT_DB_PATH`` restored to its import-time snapshot.
       ...
       makes resolution flow through ``get_cryozen_home()`` — the production
