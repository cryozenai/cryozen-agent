...
$CRYOZEN_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $CRYOZEN_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"cryozen-restart-{safe}"
    volume = f"cryozen-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "cryozen", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "cryozen", "profile", "create", "ghost").check_returncode()
