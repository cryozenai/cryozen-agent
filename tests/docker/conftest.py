...
Override the image with ``CRYOZEN_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CRYOZEN_TEST_IMAGE", "cryozen-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CRYOZEN_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("CRYOZEN_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged cryozen user
# ---------------------------------------------------------------------------
#
# Background: every Cryozen runtime path inside the container drops to UID
# 10000 (the ``cryozen`` user) via ``s6-setuidgid cryozen``. ``docker exec``
...
# fails (PermissionError swallowed) for cryozen, so a test that ran as root
...
# helpers below run every probe under ``-u cryozen`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: cryozen).
