"""Runtime smoke test for Docker $CRYOZEN_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the cryozen user after container boot.
...
registered later runs its log service as the dropped cryozen user and
...
def test_logs_gateways_seeded_and_cryozen_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by cryozen after boot."""
    ...
    assert "logs=cryozen" in r.stdout, (
        f"logs/ not owned by cryozen: {r.stdout}"
    )
    assert "gateways=cryozen" in r.stdout, (
        f"logs/gateways/ not owned by cryozen: {r.stdout}"
    )
