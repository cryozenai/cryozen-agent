...
(``tsserver/request`` notifications) that Cryozen's generic client does not
run, so it never publishes diagnostics.  Cryozen pins the self-hosting 2.x line
...
def _stage(tmp_path, monkeypatch, vue_version: str, *, js_sdk: bool):
    """Fake ``<CRYOZEN_HOME>/lsp`` tree: a staged launcher, the Vue package, optionally a JS TypeScript SDK."""
