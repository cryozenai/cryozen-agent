...
def test_kanban_guidance_requires_worker_task_at_agent_init(monkeypatch, task_id, expected):
    ...
    if task_id is None:
        ...
        monkeypatch.setenv("CRYOZEN_KANBAN_TASK", task_id)
    monkeypatch.setattr("cryozen_cli.plugins.discover_plugins", lambda: None)
    monkeypatch.setattr(
        model_tools,
...
def test_kanban_guidance_fallback_requires_owned_worker_task(monkeypatch, task_id, owner, expected):
    ...
    is not identity, and an inherited CRYOZEN_KANBAN_TASK is not ownership (#112486)."""
...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_cryozen_home``,
    ``get_default_cryozen_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``CRYOZEN_HOME`` and no resolver mocks.
    """

    def test_real_cryozen_home_under_profiles_renders_correct_paths(
        ...
        assert get_default_cryozen_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Cryozen profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """CRYOZEN_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".cryozen-agent"
        ...
        assert "Active Cryozen profile: default." in prompt
...
def test_coding_prompt_orders_shared_context_before_workspace(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "CRYOZEN_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "CRYOZEN_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_cryozen_home", lambda: Path("/cryozen"))

    # Production renders this as str(get_cryozen_home()) + "/profiles/<name>/",
    # and str(Path("/cryozen")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/cryozen".
    _home_str = str(Path("/cryozen"))
    expected_profile = (
    ...
    with (
        ...
        patch("cryozen_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under CRYOZEN_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from cryozen_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("cryozen_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from cryozen_time import now as cryozen_now
        sid = cryozen_now().strftime("%Y%m%d_%H%M%S_fresh")
