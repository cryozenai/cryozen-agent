"""load_cryozen_dotenv() reloads must not clobber values an external secret source resolved (#74265).

The gateway module import, per-turn reloads and cron fires all call ``load_cryozen_dotenv()`` again;
...
def test_source_secret_survives_second_load_cryozen_dotenv(tmp_path, monkeypatch):
    ...
    env_loader.load_cryozen_dotenv(cryozen_home=home)
    assert os.environ["GLM_API_KEY"] == "vault-value", (
        "second load_cryozen_dotenv() wrote the .env placeholder back over the source-resolved value"
    )
