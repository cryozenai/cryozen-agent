...
def _make_agent():
    cryozen_home = Path(tempfile.mkdtemp(prefix="cryozen-test-home-"))
    (cryozen_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._cryozen_home", cryozen_home),
