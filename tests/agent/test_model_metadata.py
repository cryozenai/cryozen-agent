...
class TestDefaultContextLengths:
    ...
    def test_muse_spark_resolves_1m_without_network(self, model, provider, base_url):
        ...
        live /models probe are unavailable (fresh CRYOZEN_HOME, offline)."""
        with patch("agent.model_metadata.get_cached_context_length", return_value=None), \
             patch("agent.model_metadata._query_ollama_api_show", return_value=None), \
             patch("agent.model_metadata.fetch_endpoint_model_metadata", return_value={}), \
...
# =========================================================================
# Remote endpoint /v1/models context-window resolution
# =========================================================================

class TestEndpointContextResolution:
    """A remote endpoint's /v1/models listing resolves context windows by name."""
    ...
    def test_empty_model_never_fuzzy_matches_endpoint_catalog(self, mock_fetch):
        ...
        an aggregator) and poisoned the resolved context length."""
        import agent.model_metadata as mm
        ...
        assert mm._resolve_endpoint_context_length(
            "grok-4.6", "https://relay.example.com/v1"
        ) == 500_000
...
class TestGetModelContextLength:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("openai-codex", None),  # CRYOZEN_CODEX_BASE_URL / model.base_url proxy per #115902
        ],
    )
...
class TestMoAContextLength:
    ...
    def test_moa_resolves_from_aggregator(self, tmp_path, monkeypatch):
        home = str(tmp_path / ".cryozen-agent")
