...
A custom provider entry may name a header that carries Cryozen' conversation id so a
...
def _providers(with_header: bool):
    ...
    return patch("cryozen_cli.config.get_compatible_custom_providers", return_value=[entry])
