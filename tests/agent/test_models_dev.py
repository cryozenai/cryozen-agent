...
class TestProviderMapping:
    def test_all_mapped_providers_are_strings(self):
        for cryozen_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
            assert isinstance(cryozen_id, str)
    ...
    def test_unmapped_provider_not_in_dict(self):
        assert "not-a-real-provider" not in PROVIDER_TO_MODELS_DEV
...
class TestMirrorUrlOverride:
    ...
    def test_mirror_url_used_when_configured(self, mock_get):
        ...
        with patch.object(md, "_disk_cache_age_seconds", return_value=None), \
             ...
             patch.object(md, "_load_etag", return_value=""), \
             patch.object(md, "_save_etag"), \
             patch("cryozen_cli.config.load_config_readonly", return_value=fake_config):
            fetch_models_dev()
...
class TestCatalogProviderAlias:
    ...
    def test_mistyped_alias_warns_once_and_keeps_the_configured_slug(self, caplog):
        """``catalog_provider: deepsek`` is neither a Cryozen provider id nor a models.dev id: warn
...
class TestModelOverrides:
    ...
    def test_provider_key_accepts_either_id_space(self):
        """Override keyed by Cryozen id resolves for models.dev id and back."""
        overrides = {
            "copilot": {
        ...
        with self._setup_overrides(overrides):
            # Caller passes the models.dev id; config keyed by Cryozen id.
        ...
        with self._setup_overrides(overrides):
            # Caller passes the Cryozen id; config keyed by models.dev id.
