...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated CRYOZEN_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="cryozen_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".cryozen-agent"))
    prev_home = os.environ.get("CRYOZEN_HOME")
    os.environ["CRYOZEN_HOME"] = os.path.join(test_home, ".cryozen-agent")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("cryozen_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into cryozen_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["CRYOZEN_HOME"] = prev_home
