...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CRYOZEN_HOME under tmp_path.

    ``CRYOZEN_ALLOW_PRIVATE_URLS`` opts the loopback test server into private-IP
    ...
    monkeypatch.setenv("CRYOZEN_ALLOW_PRIVATE_URLS", "1")
    from tools import url_safety
    url_safety._reset_allow_private_cache()
    (tmp_path / ".cryozen-agent").mkdir()

    # Force the constants/image cache helpers to re-read CRYOZEN_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("cryozen_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_cryozen_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under CRYOZEN_HOME — gateway cleanup
