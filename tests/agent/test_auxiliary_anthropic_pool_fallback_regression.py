...
Root cause (observed 2026-07-05): the pooled Anthropic entry was unusable, so
`_select_pool_entry("anthropic")` returned `(True, None)` — pool exists, no
selectable entry. The old `_try_anthropic` hard-failed on that branch
(`return None, None`), even though a perfectly valid `ANTHROPIC_API_KEY` was
available. This wedged
...
class TestAnthropicPoolExhaustedFallsBackToEnv:
    def test_pool_present_no_entry_falls_back_to_resolve_token(self, monkeypatch):
        ...
        monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-api03-standalone-key")
        with patch(
        ...
        assert mock_build.call_args.args[0] == "sk-ant-api03-standalone-key"
