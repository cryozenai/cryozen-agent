...
class TestParseConfigStringList:
    """#86661: `cryozen config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `cryozen config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    cryozen_home = tmp_path / "da\\ta"
    subprocess_home = cryozen_home / "home"
    subprocess_home.mkdir(parents=True)
    (cryozen_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
    monkeypatch.setenv("HOME", str(cryozen_home))
