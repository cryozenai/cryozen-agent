...
def _auth_fields(provider: str, token: str) -> dict:
    """Anthropic pools hold API keys only; Codex rows are refreshable OAuth grants."""
    if provider == "anthropic":
        return {"auth_type": "api_key"}
    return {"auth_type": "oauth", "refresh_token": f"rt-{token}", "expires_at": time.time() + 3600}


def _seed(provider: str, token: str, *, model_cooldown: str | None = None) -> None:
    row = {"id": "entry-1", "label": "pooled", "priority": 1, "source": "manual", "access_token": token,
           **_auth_fields(provider, token)}
    if model_cooldown:
...
def _seed_two(provider: str, first_token: str, second_token: str) -> None:
    ...
    for idx, token in ((1, first_token), (2, second_token)):
        rows.append({"id": f"entry-{idx}", "label": f"pooled-{idx}", "priority": idx, "source": "manual",
                     "access_token": token, **_auth_fields(provider, token)})
