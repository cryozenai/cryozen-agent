...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Cryozen Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    attachments = cryozen_home / "attachments"
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.cryozen-agent.
    assert "/root/.cryozen-agent/attachments/archive.zip" in result.message
...
def test_oversized_text_reference_maps_host_attachment_to_container_path(
    ...
    assert "/root/.cryozen-agent/attachments/large.txt" in attached_context
    assert "too large to inline safely" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert result.expanded
    assert str(payload) in result.message
    assert "/root/.cryozen-agent/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.cryozen-agent/auth.json` and read the
    ...
    oauth = cryozen_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = cryozen_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.cryozen-agent/auth.json and @file:.cryozen-agent/.anthropic_oauth.json "
        "and @file:.cryozen-agent/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
...
async def test_canonical_guard_fails_closed_when_lookup_raises(tmp_path: Path, monkeypatch):
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.cryozen-agent/auth.json",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Cryozen/composer-images/a.png",
...
async def test_side_thread_expansion_guards_the_served_profile_home(tmp_path: Path, monkeypatch):
    ...
    token = set_cryozen_home_override(served_home)
    try:
        result = preprocess_context_references(
    ...
    assert any("internal Cryozen path" in w for w in result.warnings)


@pytest.mark.asyncio
async def test_composer_paste_outside_workspace_is_attached_but_sibling_dir_is_not(tmp_path, monkeypatch):
    """Desktop saves a large paste under <CRYOZEN_HOME>/composer-pastes and attaches it
    ...
    cryozen_home = tmp_path / ".cryozen-agent"
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
    paste = cryozen_home / "composer-pastes" / "pasted_content_1.txt"
    paste.parent.mkdir(parents=True)
    paste.write_text("PASTED-BODY-MARKER\n", encoding="utf-8")
    lookalike = cryozen_home / "my-composer-pastes-backup" / "secret.txt"
