...
def out_of_turn():
    ...
    from agent import conversation_context

    tokens = (
        aux._RUNTIME_MAIN_CONTEXT.set(None),
        conversation_context.set_conversation_context(None),
        conversation_context.set_affinity_scope(None),
    )
    try:
        yield
    finally:
        aux._RUNTIME_MAIN_CONTEXT.reset(tokens[0])
        conversation_context.reset_conversation_context(tokens[1])
        conversation_context.reset_affinity_scope(tokens[2])
