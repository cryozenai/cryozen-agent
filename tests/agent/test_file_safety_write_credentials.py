"""Secret stores under CRYOZEN_HOME are write-denied; control files stay writable (#110464).
...
@pytest.fixture()
def cryozen_layout(tmp_path, monkeypatch):
    """Profile CRYOZEN_HOME plus a distinct global root, both patched."""
    root = tmp_path / "cryozen_root"
...
def test_read_denied_secret_stores_are_write_denied_on_profile_and_root(cryozen_layout):
    root, profile = cryozen_layout
...
def test_control_files_and_lookalikes_outside_home_stay_writable(cryozen_layout, tmp_path):
    root, profile = cryozen_layout
    for base in (profile, root):
...
class TestProfileHomeProcessHome:
    """With the process HOME pinned to ``{CRYOZEN_HOME}/home`` (TERMINAL_HOME_MODE=profile,
    ...
    def profile_home_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_cryozen_home_path", lambda: profile)
        monkeypatch.setattr(fs, "_cryozen_root_path", lambda: profile.parent)
        return profile
