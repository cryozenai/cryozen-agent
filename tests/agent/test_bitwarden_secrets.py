...
binary download are exercised manually by `cryozen secrets bitwarden
...
def cryozen_home(tmp_path, monkeypatch):
    """Point Cryozen at an isolated home directory."""
    home = tmp_path / ".cryozen-agent"
    home.mkdir()
    monkeypatch.setenv("CRYOZEN_HOME", str(home))
    # Some modules cache get_cryozen_home; clear if needed.
    import cryozen_constants
    if hasattr(cryozen_constants, "_CRYOZEN_HOME_CACHE"):
        cryozen_constants._CRYOZEN_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(cryozen_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from cryozen_cli.env_loader import _apply_external_secret_sources
