...
manipulation. Cryozen keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point CRYOZEN_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_cryozen_home /
    get_default_cryozen_root) instead of monkeypatching private helpers —
    ...
    CRYOZEN_HOME=<root>/profiles/<name> makes get_default_cryozen_root()
