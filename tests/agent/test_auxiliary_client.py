...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "cryozen_cli.moa_config.resolve_moa_preset",
            lambda cfg, name: preset,
        )
        monkeypatch.setattr("cryozen_cli.config.load_config", lambda: {"moa": {}})
        monkeypatch.setattr("cryozen_cli.config.load_config_readonly", lambda: {"moa": {}})
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp CRYOZEN_HOME
...
class TestNormalizeAuxProvider:
    ...
    def test_covers_every_alias_the_main_path_resolves(self):
        """Every alias cryozen_cli.auth resolves also resolves in aux — drift becomes a red test (#115006)."""
        from cryozen_cli.auth import _PROVIDER_ALIASES as auth_table
...
class TestReadCodexAccessToken:
    def test_valid_auth_store(self, tmp_path, monkeypatch):
        ...
        (cryozen_home / "auth.json").write_text(json.dumps({
            "version": 1,
            "providers": {
                "openai-codex": {
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``cryozen auth status`` already reports these as logged in; compression
        ...
        from cryozen_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
        monkeypatch.delenv("CRYOZEN_XAI_BASE_URL", raising=False)
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
        monkeypatch.setenv("CRYOZEN_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestAnthropicAuxClient:
    """Auxiliary Anthropic client construction (API keys only)."""

    def test_api_key_builds_anthropic_aux_client(self, monkeypatch):
        """Regular API keys (sk-ant-api-*) build a native Anthropic auxiliary client."""
    ...
    def test_pool_entry_takes_priority_over_legacy_resolution(self):
        ...
        assert mock_build.call_args.args[0] == "sk-ant-api03-pooled"
...
class TestBuildCodexClient:
    ...
    def test_profile_codex_base_url_overrides_pool_endpoint(self, monkeypatch):
        ...
        with (
            ...
            monkeypatch.setenv("CRYOZEN_CODEX_BASE_URL", "http://127.0.0.1:8787/v1")
            mock_openai.return_value = MagicMock()
            from agent.auxiliary_client import _build_codex_client
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``cryozen model``, no per-task override is
...
class TestExplicitProviderRouting:
    ...
    def test_explicit_anthropic_api_key(self, monkeypatch):
        """provider='anthropic' + regular API key builds a native Anthropic client."""
...
class TestOpenRouterPaidLaneGuard:
    ...
    def test_free_only_allows_builtin_default_model(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("cryozen_cli.config.load_config_readonly", return_value={"auxiliary": {"free_only": True}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
            client, model = _try_openrouter()
    ...
    def test_free_only_skips_paid_configured_model(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("cryozen_cli.config.load_config_readonly",
    ...
    def test_paid_lane_warns_once(self, monkeypatch, caplog):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("cryozen_cli.config.load_config_readonly", return_value=_paid_cfg), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("cryozen_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
...
class TestIsModelNotFoundError:
    ...
    def test_openrouter_catalog_404(self):
        """The exact incident error: a recommended model dropped from the
        OpenRouter catalog."""
        exc = Exception(
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_custom_endpoint", return_value=(mock_client, "local-model")), \
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Cryozen auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from cryozen_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("cryozen_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Cryozen owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    ...
    def test_generic_reasoning_fallback_clamps_ultra_for_auxiliary_and_moa_calls(self, task, monkeypatch):
        """The OpenAI-compatible fallback must never put Cryozen-only ``ultra`` on the wire."""
    ...
    def test_profile_projection_receives_wire_clamped_effort(self, monkeypatch):
        ...
        an OpenRouter slot 400'd otherwise (#112010)."""
        import agent.auxiliary_client as aux
    ...
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("cryozen_cli.config.load_config", return_value=config), patch("cryozen_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("cryozen_cli.config.load_config", return_value=config), patch("cryozen_cli.config.load_config_readonly", return_value=config), \
             caplog.at_level(logging.WARNING, logger="agent.auxiliary_client"):
    ...
    def test_bare_custom_auth_error_does_not_fall_back_to_env_base_url(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        with patch("cryozen_cli.runtime_provider.resolve_runtime_provider",
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Cryozen reasoning.

    MoA slot reasoning is carried through call_llm as a Cryozen
...
class TestCodexAuxiliaryAdapterReservedToolAliases:
    ...
    reversed on the parsed tool_calls before Cryozen dispatch (#114260)."""
    ...
    def test_wire_tools_match_main_transport_aliases_and_strict(self, base_url, aliased, monkeypatch):
        ...
        # Deterministic xAI branch: a non-xAI web backend keeps client dispatch under ``cryozen_web_search``.
        ...
        assert {t["name"] for t in resp_kwargs["tools"]} == {
            f"cryozen_{n}" if n in aliased else n
        ...
        assert history_names == ["cryozen_search_files" if "search_files" in aliased else "search_files"]
        assert resp_kwargs.get("_wire_aliases", {}) == {f"cryozen_{n}": n for n in aliased}
    ...
    def test_create_maps_aliases_back_and_never_sends_alias_map(self):
            ...
            def create(self, **kwargs):
                ...
                return SimpleNamespace(
                    ...
                    output=[SimpleNamespace(type="function_call", call_id="c9", id="fc_9",
                                            name="cryozen_search_files", arguments='{"pattern": "x"}')],
                )
        ...
        assert "cryozen_search_files" in {t["name"] for t in wire_tools}
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/cryozenai/cryozen-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/cryozenai/cryozen-agent/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "CryozenAgent"
...
class TestAuxUnhealthyCache:
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        with patch("agent.auxiliary_client._read_main_provider", return_value="auto"), \
             ...
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(fallback_client, "k-model")):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(fallback_client, "n-model", "api-key")), \
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("cryozen_cli.config.load_config", return_value=fake_config), patch("cryozen_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("cryozen_cli.config.load_config", return_value={"model": {}}), patch("cryozen_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
...
class TestFastModelTier:
    ...
    def test_catalog_match_prefers_rolling_alias_over_pinned_id(self):
        ...
        with patch("cryozen_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
    ...
    def test_catalog_fetch_is_authenticated(self):
        ...
        with patch(
            "cryozen_cli.auth.resolve_api_key_provider_credentials",
            return_value={"api_key": "sk-test", "base_url": "https://api.example.com/v1"},
        ), patch(
            "cryozen_cli.models_pricing.fetch_models_with_pricing", return_value={}
        ) as fetch:
    ...
    def test_fast_tier_is_opt_in(self):
        ...
        with patch.object(ac, "_fast_model_from_catalog") as spy:
            ac._get_aux_model_for_provider("openrouter")
