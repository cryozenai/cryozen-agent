...
def test_reference_messages_drops_system_but_renders_tools_as_text():
    ...
    messages = [
        {"role": "system", "content": "huge cryozen system prompt"},
    ...
    assert all("huge cryozen system prompt" not in m["content"] for m in view)
...
def test_references_run_in_parallel(monkeypatch):
    ...
    Both successful references must be in flight at the same time: each one
    waits on a two-party barrier that only opens when the other arrives, so a
    serial dispatch breaks the barrier and surfaces as a failed reference.
    ...
    def concurrent_call_llm(**kwargs):
        if kwargs["model"] == "boom":
            raise RuntimeError("kaboom")
        both_in_flight.wait()
        return _response(f"resp-{kwargs['provider']}")

    monkeypatch.setattr(moa_loop, "call_llm", concurrent_call_llm)
...
def test_slot_runtime_anthropic_oauth_routes_through_provider_branch(monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.runtime_provider.resolve_runtime_provider", fake_resolve
    )
...
def test_reference_messages_flattens_cache_decorated_content():
    ...
    plain = [
        {"role": "system", "content": "cryozen system prompt"},
        {"role": "user", "content": "Can we get codex usage resets into cryozen?"},
    ]
