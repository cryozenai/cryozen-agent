...
Regression: main provider ``xai-oauth`` with an expired token, a local custom endpoint still
configured from an earlier setup, no ``fallback_providers``. Every compression, title and
memory-flush call for the Grok conversation fell through to the built-in discovery chain and was
sent to the other endpoint while the chat visibly stayed on Grok. The discovery chain is
...
def custom_is_the_only_working_provider():
    """Main provider unusable; the custom endpoint would win the discovery chain; no fallback policy."""
    ...
    with patch.object(aux, "_try_main_provider_route", return_value=None), \
         ...
         patch.object(aux, "_try_custom_endpoint", return_value=(MagicMock(name="custom"), "custom-model")):
        yield
...
def test_quarantined_fallback_hands_over_to_the_next_configured_entry():
    ...
    with patch.object(aux, "_try_configured_fallback_chain", return_value=(healthy, "m2", "fallback_chain[1](custom:backup)")), \
    ...
    assert client is healthy and label == "fallback_chain[1](custom:backup)"
...
def test_no_selected_main_provider_still_discovers(custom_is_the_only_working_provider):
    ...
    assert client is not None and (model, label) == ("custom-model", "local/custom")
