...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"cryozen-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "cryozen-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "cryozen-agent-dev"
...
# `cryozen curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `cryozen curator pin <umbrella>` in the rename block so
