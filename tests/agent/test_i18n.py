...
@pytest.mark.parametrize("value", ["zh", "zh-CN", "zh-TW", "zh-hant", "zh-Hans", "chinese"])
def test_unshipped_chinese_tags_resolve_to_english(value: str):
    """No Chinese catalog ships, so every Chinese tag or alias resolves to the English fallback."""
    assert i18n._normalize_lang(value) == i18n.DEFAULT_LANGUAGE


def test_default_when_nothing_set(monkeypatch):
    """With no env var and no config override, falls back to English."""
    monkeypatch.delenv("CRYOZEN_LANGUAGE", raising=False)
...
def test_language_is_per_profile_under_multiplex(monkeypatch, tmp_path):
    """CRYOZEN_LANGUAGE in the DEFAULT profile's environ must not leak into a secondary profile's
    ...
    monkeypatch.setenv("CRYOZEN_LANGUAGE", "ja")  # default profile's .env, bridged into environ
    i18n.reset_language_cache()
    ...
    try:
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(prof_b))
...
def test_t_missing_key_in_non_english_falls_back_to_english(tmp_path, monkeypatch):
    ...
    i18n.reset_language_cache()
    try:
        assert i18n.t("foo", lang="ja") == "English Foo"
    finally:
...
def test_locales_dir_env_override_ignored_when_missing(tmp_path, monkeypatch):
    """A bogus CRYOZEN_BUNDLED_LOCALES falls through to source/wheel resolution
    instead of returning a path that doesn't exist."""
    monkeypatch.setenv("CRYOZEN_BUNDLED_LOCALES", str(tmp_path / "does-not-exist"))
