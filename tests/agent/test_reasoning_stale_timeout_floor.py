...
thinking phase.  Cryozen's default cloud-stream stale detector
(``CRYOZEN_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``CRYOZEN_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_non_reasoning_model_keeps_default(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("CRYOZEN_API_CALL_STALE_TIMEOUT", raising=False)
...
def test_explicit_provider_stale_timeout_wins_over_context_tier_and_reasoning_floor(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
    monkeypatch.delenv("CRYOZEN_STREAM_STALE_TIMEOUT", raising=False)
