...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_unmatched_api_key_hint_rotates_without_benching_innocent_key(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "cryozen" / "auth.json").read_text())
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "cryozen" / "auth.json").read_text()
...
def test_load_pool_collapses_duplicate_env_rows_to_active_key(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "cryozen" / "auth.json").read_text())
...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "cryozen_cli.env_loader.get_secret_source",
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from cryozen_cli.auth import write_credential_pool
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://cryozen/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://cryozen/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "cryozen" / "auth.json").read_text())["credential_pool"]["openai-codex"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.cryozen-agent/.env when
    ...
    # User edited ~/.cryozen-agent/.env with the fresh key
    (cryozen_home / ".env").write_text(
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.cryozen-agent/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (cryozen_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_custom_endpoint_pool_seeds_from_config(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "cryozen" / "config.yaml"
    import yaml
...
def test_load_pool_skips_exchange_for_suppressed_copilot(tmp_path, monkeypatch):
    ...
    removed with ``cryozen auth remove copilot gh_cli``.  The gate must run
...
def test_load_pool_skips_resolve_when_all_copilot_sources_suppressed(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "cryozen"))
    from cryozen_cli.copilot_auth import COPILOT_ENV_VARS
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    monkeypatch.setattr("cryozen_cli.copilot_auth.resolve_copilot_token", _boom)
...
def test_load_pool_copilot_exchange_only_when_selected_and_warns_once(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr("cryozen_cli.copilot_auth.resolve_copilot_token", lambda: ("gho_raw_initial", "gh auth token"))
    exchanges = []
    ...
    monkeypatch.setattr("cryozen_cli.copilot_auth.get_copilot_api_token", degraded_exchange)
    ...
    with caplog.at_level(logging.WARNING, logger="agent.credential_pool"):
        ...
        (tmp_path / "cryozen" / "config.yaml").write_text("model:\n  provider: deepseek\n  default: deepseek-chat\n", encoding="utf-8")
        pool = load_pool("copilot")
        load_pool("copilot")
        ...
        (tmp_path / "cryozen" / "config.yaml").write_text(
            "model:\n  provider: deepseek\n  default: deepseek-chat\nauxiliary:\n  approval:\n    provider: copilot\n", encoding="utf-8")
        from cryozen_cli import config as _cfg
        ...
        monkeypatch.setattr("cryozen_cli.copilot_auth.resolve_copilot_token", lambda: ("gho_raw_rotated", "gh auth token"))
        load_pool("copilot")
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
def test_persist_preserves_concurrent_disk_only_entry(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool
    from cryozen_cli.auth import read_credential_pool, write_credential_pool
    ...
    final = json.loads((tmp_path / "cryozen" / "auth.json").read_text())
...
def _disk_entry(tmp_path) -> dict:
    ...
    store = json.loads((tmp_path / "cryozen" / "auth.json").read_text())
...
def test_reset_statuses_clears_a_cooldown_that_is_still_binding(tmp_path, monkeypatch):
    ...
    the cooldown was copied straight back. ``cryozen auth reset`` printed "Reset
...
def test_reset_statuses_clears_the_classified_failure_reason(tmp_path, monkeypatch):
    ...
    status and no error code but still classified ``billing``. ``cryozen auth
...
def test_live_pool_flush_does_not_resurrect_a_cooldown_reset_by_another_process(tmp_path, monkeypatch):
    """A running session's next ordinary flush must not undo ``cryozen auth reset`` (#89415).
    ...
    assert load_pool("deepseek").reset_statuses() == 1  # process B: `cryozen auth reset deepseek`
