...
class TestChatCompletionsBasic:
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """Cryozen-internal ``_``-prefixed markers must never reach the wire.
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    """Profile extra_body (e.g. aggregator attribution tags) must not reach a native
    ...
    def _tagging_profile(self):
        ...
        class _TaggingProfile(ProviderProfile):
            def build_extra_body(self, *, session_id=None, **context):
                return {"tags": ["product=test"]}

        return _TaggingProfile(name="tagging-relay", base_url="https://relay.example/v1")
    ...
    def test_tags_preserved_on_relay_endpoint(self, transport):
        kw = transport.build_kwargs(
...
class TestPromptCacheKeyCapability:
    ...
    def test_stale_profile_without_supports_prompt_cache_key_does_not_crash(self, transport):
        ...
        Regression: a provider profile object had no attribute
