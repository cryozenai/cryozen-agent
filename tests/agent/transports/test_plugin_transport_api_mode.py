...
Both tests drive a REAL plugin discovered from an isolated CRYOZEN_HOME rather than the gates in
...
def install_dialect_plugin(tmp_path, monkeypatch):
    """Write a model-provider plugin (optionally registering its transport) under a temp CRYOZEN_HOME."""
...
def test_registered_plugin_dialect_reaches_every_gate(install_dialect_plugin):
    ...
    from cryozen_cli.runtime_provider import _parse_api_mode, resolve_runtime_provider
