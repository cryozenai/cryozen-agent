...
def test_manifest_matches_what_the_prompt_actually_loads(project, tmp_path_factory):
    """Every context type present at once; the ladder picks .cryozen.md, the chain lists both AGENTS files,
    CLAUDE.md/.cursorrules/.cursor/rules/*.mdc are shadowed, an empty file never wins, SOUL.md rides along."""
    (project / ".cryozen.md").write_text("cryozen rules")
    ...
    assert statuses == {
        ".cryozen.md": "loaded", "../AGENTS.md": "shadowed", "AGENTS.override.md": "empty", "AGENTS.md": "shadowed",
    ...
    # Same walk, other winner: drop .cryozen.md and the whole AGENTS chain loads while the rest stays shadowed.
    (project / ".cryozen.md").unlink()
...
def test_truncated_and_suppressed_statuses_follow_the_builder(project, monkeypatch, tmp_path_factory):
    ...
    # Install-tree guard: a fallback cwd (cwd=None) inside the Cryozen tree lists the file but never loads it.
