...
path via ``cryozen chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp CRYOZEN_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point CRYOZEN_HOME at a temp dir and turn moa.save_traces on."""
    cryozen_home = tmp_path / ".cryozen-agent"
    cryozen_home.mkdir()
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))

    # save_moa_turn reads config via cryozen_cli.config.load_config; stub it to
    ...
    return cryozen_home / "moa-traces"
