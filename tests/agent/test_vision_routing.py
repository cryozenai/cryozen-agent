...
def isolated_home(monkeypatch):
    """Temp CRYOZEN_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="cryozen_test_31179_")
    cryozen_home = os.path.join(test_home, ".cryozen-agent")
...
def _fresh_modules():
    """Drop cached cryozen modules so each test reloads against current env."""
    _drop_reload_targets()
