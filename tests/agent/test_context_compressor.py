...
class TestSummarizeToolResultClarify:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # cryozen_cli/oneshot.py no-user callback
    ...
    def test_live_oneshot_producer_is_recognized_as_sentinel(self):
        ...
        from cryozen_cli.oneshot import _oneshot_clarify_callback
...
class TestAuthFailureAborts:
    ...
    Real incident: an OAuth token pointed at a stale staging inference URL
    ...
    def test_missing_provider_api_key_is_terminal_access_failure(self):
        ...
        oauth_err = RuntimeError(
            ...
            "Run `cryozen auth add minimax-oauth` to sign in, or switch to a different provider "
            "with `cryozen model`."
        )
    ...
    def test_missing_provider_api_key_preserves_original_messages(self):
        ...
        err = RuntimeError(
            ...
            "to a different provider with cryozen model."
        )
        with patch(
...
class TestTruncateToolCallArgsJson:
    ...
    def test_shrunken_args_remain_valid_json(self):
        ...
        original = _json.dumps({
            "path": "~/.cryozen-agent/skills/shopping/browser-setup-notes.md",
            "content": content,
        })
        assert len(original) > 500
        shrunk = shrink(original)
        parsed = _json.loads(shrunk)  # must not raise
        assert parsed["path"] == "~/.cryozen-agent/skills/shopping/browser-setup-notes.md"
