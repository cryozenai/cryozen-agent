"""Regression tests for the codex_app_server → Cryozen UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_cryozen_tools_mcp_server_emits_bare_tool_name(self):
        ...
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "cryozen-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "cryozen agent docs"
        assert calls[0].args[3] == {"query": "cryozen agent docs"}
