...
class CopilotACPClientSafetyTests(unittest.TestCase):
    ...
    def test_read_text_file_redacts_sensitive_content(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # agent.redact snapshots CRYOZEN_REDACT_SECRETS at import time into
    ...
    def test_write_text_file_respects_safe_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"CRYOZEN_WRITE_SAFE_ROOT": str(safe_root)}, clear=False):
                response = self._dispatch(
                    {
                        "jsonrpc": "2.0",
                        "id": 5,
        ...
        self.assertIn("CRYOZEN_WRITE_SAFE_ROOT", str(response["error"]))
...
def test_run_prompt_preserves_real_home_when_profile_home_available(monkeypatch, tmp_path):
    ...
    (cryozen_home / "home").mkdir(parents=True)
    real_home = tmp_path / "real-home"
    real_home.mkdir()

    monkeypatch.setenv("HOME", str(real_home))
    monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
    # Hermeticity: an ambient CRYOZEN_REAL_HOME (exported by Cryozen' own
    ...
    assert captured["kwargs"]["env"]["CRYOZEN_REAL_HOME"] == str(real_home)
...
# `session/set_model` call. Without it, picking gpt-5.6-terra in Cryozen
