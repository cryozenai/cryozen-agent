...
def cryozen_home(monkeypatch):
    """Isolate CRYOZEN_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``CRYOZEN_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="cryozen-reload-skills-")
    monkeypatch.setenv("CRYOZEN_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "CRYOZEN_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_detects_removed_skill_carries_description(self, cryozen_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(cryozen_home / "skills", "demo", "soon to be gone")
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, cryozen_home):
