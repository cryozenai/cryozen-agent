...
The pool quarantines a dead ``openai-codex`` / ``xai-oauth`` / ``anthropic`` refresh
token — for the user this is the moment the login is lost, and a debug-only line made it look like
"I signed in once and Cryozen keeps failing" (#113023). Two invariants: the WARNING carries the
``cryozen auth add <provider>`` hint, and a row the quarantine does not drop (an independent
...
@pytest.mark.parametrize(
    ...
    [
        ("openai-codex", "_is_terminal_codex_oauth_refresh_error", "_sync_entry_from_auth_store",
         "_clear_terminal_tokens_state", "cryozen auth add openai-codex"),
    ],
)
...
def test_anthropic_transient_refresh_failure_stays_exhausted(monkeypatch, caplog):
    ...
    entry = _entry("anthropic", source="manual:cryozen_pkce")
    pool._entries = [entry]
