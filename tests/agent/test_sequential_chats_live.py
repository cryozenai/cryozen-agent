...
AlexKucera reported on Discord (2026-04-16) that after ``cryozen update`` pulled
...
Opt-in — not part of default CI:
    CRYOZEN_LIVE_TESTS=1 pytest tests/agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.cryozen-agent/.env).
"""
...
# Load ~/.cryozen-agent/.env so live runs pick up OPENROUTER_API_KEY without
# needing the runner to shell-source it first. Silent if the file is absent.
def _load_user_env() -> None:
    env_file = Path.home() / ".cryozen-agent" / ".env"
