...
def test_persistence_failure_default_copy_is_actionable_and_profile_aware(monkeypatch):
    monkeypatch.setenv("CRYOZEN_HOME", "/srv/cryozen-profile")
    text = TurnExplainersMixin._format_turn_completion_explanation("session_persistence_failed", "replaced")
    assert "cryozen gateway stop" in text and "cryozen doctor" in text
    assert "~/.cryozen-agent" not in text and "/srv/cryozen-profile" in text
    assert "manifest" not in text  # the runbook stays in logger.error at cryozen_state
