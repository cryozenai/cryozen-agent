...
def _ensure_redaction_enabled(monkeypatch):
    """Ensure CRYOZEN_REDACT_SECRETS is not disabled by prior test imports."""
...
class TestTerminalOutputRedaction:
    ...
    def test_command_reads_secret_file_detection(self):
        ...
        # With paths
        assert _command_reads_secret_file("cat ~/.cryozen-agent/.env")
    ...
    def test_cat_env_file_masks_opaque_token(self):
        ...
        out = (
            ...
            "DEEPSEEK_API_KEY=xyz789opaqueKey\n"
            "DEBUG=true\n"
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("cat ~/.cryozen-agent/config.yaml", "api_key: cryozenConfigSecret123", "cryozenConfigSecret123"),
            (
                "head ~/.cryozen-agent/profiles/work/config.yaml",
            ...
            (
                "cat $HOME/.cryozen-agent/config.yaml",
            ...
            (
                "grep 'foo|bar' ~/.cryozen-agent/config.yaml",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "cat ~/.cryozen-agent/config.example.yaml",
            "cat ~/.cryozen-agent/config.template.yaml",
            "cat ~/.bashrc.example",
...
class TestCryozenHomePathClassification:
    """``_is_secret_file_arg`` must see the RESOLVED Cryozen home: a managed Windows home
    (``%LOCALAPPDATA%\\cryozen``) has no ``.cryozen-agent`` segment and a resolved path never spells
    ``$CRYOZEN_HOME``, so the literal test alone classified its ``config.yaml`` as ordinary YAML."""
    ...
    def test_resolved_home_config_is_secret_bearing_but_project_config_is_not(self, tmp_path, monkeypatch):
        ...
        home = tmp_path / "cryozen"  # no ".cryozen-agent" segment
