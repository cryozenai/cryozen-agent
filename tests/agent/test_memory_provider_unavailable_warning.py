"""Regression tests for cryozenai/cryozen-agent#2765.
...
service that didn't inherit ``~/.cryozen-agent/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "cryozen memory status" in msg
