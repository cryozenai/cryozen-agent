...
from cryozen_cli.auth_plugin_providers import is_refreshable_oauth_provider
...
def test_plugin_metadata_survives_load_save_load():
    ...
    assert PooledCredential.from_dict("example-oauth", {"access_token": "t", "scope": "s1"}).scope == "s1"
...
def test_pool_refresh_dispatches_to_profile_hook(plugin_profiles, monkeypatch):
    ...
    assert is_refreshable_oauth_provider("openai-codex") is True  # built-ins unchanged
...
def test_plugin_refresh_outcome(plugin_profiles, caplog, hook, status, tokens, extra_key):
    ...
    if status == STATUS_DEAD:
        assert "cryozen auth add example-oauth" in caplog.text
    else:
        assert "cryozen auth add" not in caplog.text


def test_plugin_refresh_adopts_peer_rotation_without_spending_token(plugin_profiles):
    """Two Cryozen processes share one auth.json: the second refresh adopts the first's rotated pair
