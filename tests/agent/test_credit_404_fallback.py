"""Regression for #115702: a paid model behind an empty credit balance can answer HTTP 404 with a
structured billing code. The code must classify as billing (fallback chain armed, no retry burn)
and the resulting switch must be a WARNING naming the failing profile and remedy.
"""
...
def test_404_insufficient_credits_code_is_billing_with_fallback():
    ...
    err = _StatusError(
        "Not Found", 404, {"error": {"code": "insufficient_credits", "message": "Not Found"}},
    )
    verdict = classify_api_error(err, provider="openrouter", model="z-ai/glm-5.2")
...
def test_billing_fallback_warning_names_failing_profile_and_remedy(tmp_path, caplog):
    ...
    for name in ("alpha", "beta"):
        token = set_cryozen_home_override(tmp_path / ".cryozen-agent" / "profiles" / name)
        try:
            caplog.clear()
            with caplog.at_level(logging.INFO, logger="agent.chat_completion_helpers"):
                _log_fallback_activated(None, FailoverReason.billing, "z-ai/glm-5.2", "openrouter", "free/model", "openrouter")
        finally:
            reset_cryozen_home_override(token)
        warnings = [r for r in caplog.records if r.levelno == logging.WARNING]
        assert len(warnings) == 1
        seen[name] = warnings[0].getMessage()
    assert "Profile alpha:" in seen["alpha"] and "cryozen -p alpha model" in seen["alpha"]
