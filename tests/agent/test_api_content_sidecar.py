...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("cryozen_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated CRYOZEN_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="cryozen_api_content_")
...
class TestSessionRowExistsBeforePreflightCompaction:
    ...
    def test_in_place_compaction_multimodal_context_part_survives_reload(self, tmp_path):
        ...
        try:
            ...
            with patch("cryozen_cli.plugins.invoke_hook", return_value=[{"context": "PLUGIN-CTX"}]):
                ctx = _build(
