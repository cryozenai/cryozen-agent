...
def test_oneshot_run_gets_distinct_source(monkeypatch, inherited):
    """A finite `cryozen chat -q` / `cryozen -z` run is tagged `oneshot`, whether launched from a plain shell
    or spawned inside a TUI/Desktop session (whose transport label it inherits but is not) (#112550)."""
    monkeypatch.setenv("CRYOZEN_SESSION_SOURCE", inherited)
...
def test_oneshot_keeps_explicit_source_flag(monkeypatch, explicit):
    """`cryozen chat -q --source tui` is a documented flag, not an inherited transport label: main.py
    marks it CRYOZEN_SESSION_SOURCE_EXPLICIT=1 and the one-shot drop must not override it."""
    monkeypatch.setenv("CRYOZEN_SESSION_SOURCE", explicit)
    monkeypatch.setenv("CRYOZEN_SESSION_SOURCE_EXPLICIT", "1")
    monkeypatch.setenv("CRYOZEN_SINGLE_QUERY_SESSION", "1")
