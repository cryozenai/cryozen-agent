...
class TestAuxiliaryConfigBridge:
    ...
    def test_approval_bridged(self, monkeypatch):
        config = {
            "auxiliary": {
                "approval": {"provider": "openrouter", "model": "gemini-2.5-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
...
class TestDefaultConfigShape:
    """Verify the DEFAULT_CONFIG in cryozen_cli/config.py has correct auxiliary structure."""
...
class TestCLIDefaultsHaveAuxiliaryKeys:
    ...
    def test_cli_defaults_can_merge_auxiliary(self):
        ...
        # load_cli_config lives in cryozen_cli/cli_config_load.py (re-exported by
