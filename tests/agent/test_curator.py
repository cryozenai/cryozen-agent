...
def curator_env(tmp_path, monkeypatch):
    """Isolated CRYOZEN_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # save_state() resolves the state path from CRYOZEN_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.cryozen-agent once monkeypatch
    ...
    # thread here, while CRYOZEN_HOME is still pinned to this test's tmp home
...
def test_bundled_skills_are_off_limits_unless_opted_in(curator_env, monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.config.load_config", lambda: {"curator": {}})
    assert usage._prune_builtins_enabled() is False
    assert DEFAULT_CONFIG["curator"]["prune_builtins"] is False
    monkeypatch.setattr("cryozen_cli.config.load_config", lambda: {"curator": {"prune_builtins": True}})
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from cryozen_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `cryozen model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from cryozen_cli.main_provider_setup import _AUX_TASKS
    from cryozen_cli.web_server_config import _AUX_TASK_SLOTS
    ...
    # 2. cryozen_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. cryozen_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_receives_configured_reasoning(curator_env, monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.config.load_config_readonly", lambda: cfg)
    monkeypatch.setattr(
        "cryozen_cli.runtime_provider.resolve_runtime_provider",
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``cryozen curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``cryozen curator rollback`` restores a
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.cryozen-agent/skills/<umbrella>/references/ && mv ...`` its
