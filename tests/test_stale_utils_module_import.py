...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.cryozen-agent/cryozen-agent/utils.py)
...
2. ``cryozen update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``cryozen_cli/main.py`` (the ``cryozen update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``cryozen_constants`` after the
