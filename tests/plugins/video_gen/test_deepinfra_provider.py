...
def _mock_url_download(captured: dict, raise_exc: Exception | None = None):
    ...
    def _fake_save_url_video(url, *, prefix="video", **kw):
        ...
        if raise_exc:
            raise raise_exc
        return Path(f"/home/x/.cryozen-agent/cache/videos/{prefix}_test.mp4")
...
def test_generate_uses_env_only_proxy_http_client(monkeypatch):
    """The SDK client is built on Cryozen' env-only-proxy httpx client: a macOS system proxy (seen by
