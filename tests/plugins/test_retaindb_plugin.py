...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure CRYOZEN_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", cryozen_home=str(tmp_path / ".cryozen-agent"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(cryozen_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", cryozen_home=str(cryozen_home))
        return p
