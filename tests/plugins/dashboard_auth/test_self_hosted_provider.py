...
_ISSUER = "https://auth.example.com/application/o/cryozen"
_CLIENT_ID = "cryozen-dashboard"
...
class TestDiscovery:
    ...
    def test_same_origin_redirect_allowed(self):
        ...
        resp = self._mock_get(
            ...
            url="https://auth.example.com/.well-known/openid-configuration/application/o/cryozen",
        )
        with patch(
            "plugins.dashboard_auth.self_hosted.httpx.get", return_value=resp
        ):
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://cryozen.example/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://cryozen.example/auth/callback"
    ...
    def test_state_in_cookie_matches_url(self, provider):
        ...
        parsed = urllib.parse.urlparse(result.redirect_url)
        params = dict(urllib.parse.parse_qsl(parsed.query))
        pkce = result.cookie_payload["cryozen_session_pkce"]
...
class TestPluginRegister:
    ...
    def clear_env(self, monkeypatch):
        for var in (
            ...
            "CRYOZEN_DASHBOARD_OIDC_SCOPES",
            "CRYOZEN_DASHBOARD_OIDC_CLIENT_SECRET",
        ):
    ...
    def test_skips_when_unconfigured(self, patch_config):
        ...
        assert "CRYOZEN_DASHBOARD_OIDC_ISSUER" in oidc_plugin.LAST_SKIP_REASON
        assert "self_hosted" in oidc_plugin.LAST_SKIP_REASON


    def test_registers_from_env(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv("CRYOZEN_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("CRYOZEN_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_cli.config.load_config", _broken)
        ctx = MagicMock()
    ...
    def test_secret_from_env(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("CRYOZEN_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        monkeypatch.setenv("CRYOZEN_DASHBOARD_OIDC_CLIENT_SECRET", "env-secret")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_empty_env_secret_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_DASHBOARD_OIDC_CLIENT_SECRET", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
