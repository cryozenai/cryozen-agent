...
HTTP 503 ``{"detail": "Auth provider unreachable"}`` while the IdP was
perfectly healthy: the provider's JWT verification folded *every*
...
Real ``SelfHostedOIDCProvider`` JWKS path,
...
from cryozen_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def _oidc(base_url: str) -> SelfHostedOIDCProvider:
    provider = SelfHostedOIDCProvider(issuer=base_url, client_id="cryozen")
    provider._get_discovery = lambda: {"jwks_uri": f"{base_url}/jwks", "issuer": base_url}
    return provider
...
def test_opaque_bearer_with_healthy_idp_is_not_unreachable(empty_jwks_server):
    provider = _oidc(empty_jwks_server)
    assert provider.verify_session(access_token=OPAQUE_PEER_KEY) is None


def test_foreign_kid_jwt_with_healthy_idp_is_not_unreachable(empty_jwks_server):
    provider = _oidc(empty_jwks_server)
    assert provider.verify_session(access_token=FOREIGN_KID_JWT) is None


def test_real_jwt_with_unreachable_idp_still_raises_provider_error():
    provider = _oidc("http://127.0.0.1:9")  # discard port: connection refused
    with pytest.raises(ProviderError):
        provider.verify_session(access_token=FOREIGN_KID_JWT)


def test_opaque_bearer_with_unreachable_idp_is_still_just_not_ours():
    """No network call is even needed to know an opaque string is not our JWT."""
    provider = _oidc("http://127.0.0.1:9")
    assert provider.verify_session(access_token=OPAQUE_PEER_KEY) is None
...
@pytest.fixture
def _gated_oidc(empty_jwks_server):
    clear_providers()
    ...
    register_provider(_oidc(empty_jwks_server))
...
def test_gated_api_rejects_opaque_bearer_with_401_not_503(_gated_oidc):
    r = _gated_oidc.get("/api/auth/me", headers={"Authorization": f"Bearer {OPAQUE_PEER_KEY}"})
    assert r.status_code != 503, r.text
    assert r.status_code == 401
    assert "unreachable" not in r.text.lower()


def test_gated_api_rejects_opaque_cookie_with_401_not_503(_gated_oidc):
    _gated_oidc.cookies.set(SESSION_AT_COOKIE, OPAQUE_PEER_KEY)
    r = _gated_oidc.get("/api/auth/me")
    assert r.status_code != 503, r.text
    assert "unreachable" not in r.text.lower()
