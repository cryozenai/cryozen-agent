...
CRYOZEN_HOME override (``cryozen_constants.set_cryozen_home_override``).
...
def homes(tmp_path, monkeypatch):
    """Profile A (launch home, ``CRYOZEN_HOME``) and profile B with different config/.env values."""
...
def test_memory_provider_skill_prune_only_touches_the_active_home(homes, monkeypatch):
    ...
    from cryozen_cli.plugins import _reset_plugin_managers_for_tests, get_plugin_manager
    ...
    try:
        for home in (a, b):
            with scoped(home):
                ...
                provider.initialize(session_id=f"s-{home.name}", cryozen_home=str(home))
