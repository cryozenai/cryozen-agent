...
OPENVIKING_* / HINDSIGHT_BANK_ID / CRYOZEN_HONCHO_HOST, the provider must fall back to its own default
...
_DEFAULT_ENV = {
    ...
    "CRYOZEN_HONCHO_HOST": "host-default", "HONCHO_BASE_URL": "https://honcho.default",
...
def test_secondary_profile_memory_identity_never_inherits_default_environ(secondary_profile):
    ...
    assert (cfg["agent_id"], cfg["host"], cfg["mode"]) == ("cryozen", "", "platform")
    ...
    provider.initialize("s1", cryozen_home=str(secondary_profile))
    assert provider._client.project == "cryozen-b"
    ...
    assert (hcfg["banks"]["cryozen"]["bankId"], hcfg["mode"]) == ("cryozen", "cloud")
