...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "cryozen_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "cryozen_cli.web_server_chat", stub)
    monkeypatch.setattr(cryozen_cli, "web_server_chat", stub, raising=False)
...
def test_dashboard_board_project_binding_is_exposed_in_ui():
    ...
    assert "cryozen-kanban-board-project-unbind" in bundle
...
# tests/cryozen_cli/test_kanban_db.py. The OUTER fallback here is not, which
...
def test_diag_severity_tokens_route_through_host_theme_tokens():
    """The three ``--cryozen-diag-*`` rungs must resolve through the host's
    ...
    block = css[css.index("--cryozen-diag-warning"):]
    ...
    declared = {
        ...
        for name, token, fallback in re.findall(
            r"--cryozen-diag-(warning|error|critical)\s*:\s*var\(\s*(--color-[\w-]+)\s*,\s*(#[0-9a-fA-F]{6})\s*\)\s*;",
            block,
        )
    }
    assert declared == {
