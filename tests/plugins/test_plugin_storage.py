...
The contract under test: data lives under ``<cryozen home>/plugin-data/<name>/``
...
def test_data_dir_lives_outside_the_install_tree(cryozen_home):
    root = plugin_data_dir("my-plugin")

    assert root == cryozen_home / "plugin-data" / "my-plugin"
    assert root.is_dir()
    # The invariant that motivated the module: data must not live under the
    # install tree that `cryozen plugins remove` deletes.
    assert (cryozen_home / "plugins") not in root.parents


def test_data_dir_is_stable_across_calls(cryozen_home):
    assert plugin_data_dir("p") == plugin_data_dir("p")


@pytest.mark.parametrize("bad", ["", ".", "..", "../escape", "a/b", "a\\b", "x" * 65])
def test_hostile_names_are_rejected(cryozen_home, bad):
    with pytest.raises(ValueError):
        plugin_data_dir(bad)


def test_plugin_db_journal_mode_is_the_shared_fallback_verdict(cryozen_home):
    ...
    assert (cryozen_home / "plugin-data" / "board" / "data.db").exists()


def test_plugin_db_rejects_path_shaped_filenames(cryozen_home):
    with pytest.raises(ValueError):
