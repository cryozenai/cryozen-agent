"""Plugin data paths follow the active profile's CRYOZEN_HOME, including the ContextVar override.

Several plugins carried a ``~/.cryozen-agent`` fallback (guarding an ImportError of ``cryozen_constants``
...
def _openviking_log(home):
    ...
    return ov.get_cryozen_home() / ov._OPENVIKING_SERVER_LOG_RELATIVE_PATH
