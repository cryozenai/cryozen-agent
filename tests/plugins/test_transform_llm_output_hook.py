...
from cryozen_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(cryozen_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <cryozen_home>/plugins/<name> and opt it in."""
    plugin_dir = cryozen_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "cryozen_empty"))
