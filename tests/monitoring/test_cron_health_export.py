...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("cryozen.gateway.up"), _M("cryozen.gateway.active_agents"),
        _M("cryozen.gateway.busy"), _M("cryozen.gateway.drainable"),
        _M("cryozen.gateway.restart_requested"),
        _M("cryozen.platform.up"), _M("cryozen.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("cryozen.cron.scheduler.heartbeat_age_seconds"),
        _M("cryozen.cron.scheduler.last_success_age_seconds"),
        _M("cryozen.cron.scheduler.catch_up_occurrences"),
        _M("cryozen.cron.jobs.enabled"), _M("cryozen.cron.jobs.running"),
        _M("cryozen.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Cryozen Agent-owned Relay transport health" in text
