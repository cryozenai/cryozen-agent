...
def test_shim_removes_only_its_owned_profile(tmp_path, outcome):
    ...
    install = tmp_path / "cryozen-agent"
    install.mkdir()
    env = {**os.environ, "HOME": str(tmp_path), "CRYOZEN_HOME": str(tmp_path),
           "XDG_CONFIG_HOME": str(config), "TMPDIR": str(tmp_path),
           "PATH": f"{bin_dir}:/usr/bin:/bin", "CRYOZEN_SELFTEST_HOLD_SECONDS": "1",
           "CRYOZEN_UPDATE_SHIM_GRACE_SECONDS": "1", "CRYOZEN_SELFTEST_FAIL": "1" if outcome == "error" else ""}
    ...
    collision = tmp_path / f"cryozen-update-ui-{process.pid}"
    collision.mkdir()
