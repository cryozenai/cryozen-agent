...
`scripts/desktop-update/windows.ps1` drives `cryozen update` for the in-app
Desktop updater. It used to invoke the update through the venv's
`venv\\Scripts\\cryozen.exe` console-script launcher. On Windows that launcher is
a real process that keeps `cryozen.exe` mapped as its running image and spawns
`python.exe` as a child. The update ends in `uv pip install -e .`, which rewrites
the console-script shims -- including the `cryozen.exe` the launcher still has
...
Start Menu shortcut points at a `Cryozen.exe` that no longer exists.

Driving the update as `python.exe -m cryozen_cli.main update` puts the inherited
...
hand-off. The invariant it guards is that every `Invoke-CryozenStep` call site
(the update, its retry, and the desktop rebuild) drives `$pythonExe`, never the
`$cryozenExe` shim. `cryozen.exe` may still be *named* in the file for the
...
def _handoff_source() -> str:
    ...
    fixture runs a synthetic PowerShell step through ``Invoke-CryozenStep`` to
...
def test_invoke_cryozen_step_calls_drive_python_not_the_shim() -> None:
    source = _handoff_source()

    invocations = re.findall(r"Invoke-CryozenStep\s+(\$\w+)", source)
    assert invocations, (
        "Expected at least one Invoke-CryozenStep call in "
    ...
    assert not offenders, (
        "Every Invoke-CryozenStep call in scripts/desktop-update/windows.ps1 "
        "must drive $pythonExe, not the cryozen.exe shim. Driving the update "
        "through the shim keeps cryozen.exe mapped as a running image, so uv's "
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "cryozen_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m cryozen_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "cryozen_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m cryozen_cli.main desktop ...` for the same reason."
    )


def test_update_no_longer_invokes_the_cryozen_exe_shim() -> None:
    source = _read()

    assert "Invoke-CryozenStep $cryozenExe" not in source, (
        "scripts/desktop-update/windows.ps1 still invokes the update through "
        "the cryozen.exe shim (`Invoke-CryozenStep $cryozenExe`). That is the "
...
def test_handoff_resolves_uv_default_dotvenv_before_building_python_and_shim_paths() -> None:
    ...
    assert "function Resolve-CryozenVenvDir" in source
    assert 'Join-Path $Root ".venv"' in source
    assert "$VenvDir = Resolve-CryozenVenvDir $InstallRoot" in source
    assert 'Join-Path $VenvDir "Scripts\\python.exe"' in source
    assert 'Join-Path $VenvDir "Scripts\\cryozen.exe"' in source


def test_desktop_relaunch_waits_for_an_in_place_rebuild() -> None:
    source = _read()
    relaunch = re.search(
        r"function Start-DesktopRelaunch \{(?P<body>.*?)\n\}\n\nfunction Invoke-CryozenStep",
        source,
        re.DOTALL,
    )
