...
def progress(tmp_path):
    ...
    status = tmp_path / "cryozen-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Cryozen to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `cryozen update`, and reports the stage that was on screen
...
n="$(cat "$CRYOZEN_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$CRYOZEN_TEST_CALLS"
for f in "$TMPDIR"/cryozen-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$CRYOZEN_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$CRYOZEN_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "cryozen-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    cryozen = install_root / "venv" / "bin" / "cryozen"
    cryozen.write_text(FAKE_CRYOZEN)
    cryozen.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "CRYOZEN_TEST_CAPTURE": str(capture),
        "CRYOZEN_TEST_CALLS": str(calls),
        "CRYOZEN_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".cryozen-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`cryozen update` is the longest wait in the hand-off and the one the
