"""Windows installer regression for Cryozen children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected Cryozen process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    runtime_dir = install_dir / ".cryozen-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real Cryozen launchers
    ...
    _write_cmd(
        ...
        '/C:"/IM cryozen.exe" >nul\n'
        "if not errorlevel 1 exit /b 0\n"
        '"%SystemRoot%\\System32\\taskkill.exe" %*\n',
    )
    uv = cryozen_home / "bin" / "uv.exe"
    ...
    wrapper.write_text(
        ...
        f"-InstallDir '{install_dir}' -CryozenHome '{cryozen_home}'\n"
        "exit $LASTEXITCODE\n",
        encoding="utf-8",
    )
