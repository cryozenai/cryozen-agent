...
The ``cryozen update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m cryozen_cli.main gateway run`` — image name
    ``pythonw``, not ``cryozen.exe`` — so the ``taskkill /IM cryozen.exe`` guard
    ...
    # The cryozen.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM cryozen.exe' in text
