...
def local_route(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("cryozen_cli.banner.get_available_skills", lambda: {})
...
def test_session_info_recovers_identity_from_the_owning_profile(tmp_path, monkeypatch):
    ...
    assert get_cryozen_home() == launch
    ...
    with server._profile_build_scope(secondary):
        ...
        assert get_cryozen_home() == secondary
    assert get_cryozen_home() == launch
