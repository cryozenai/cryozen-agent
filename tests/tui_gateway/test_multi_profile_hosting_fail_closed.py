...
Regression for the silent cross-profile secret leak class: ``cryozen serve`` hosted many profile
...
only CRYOZEN_HOME. And the launch-profile asymmetry: a default-member hosted-room turn in a
...
def two_homes(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text(
        f"A_ONLY_TOKEN={A_VAL}\nCRYOZEN_API_KEY={A_API_KEY}\nCRYOZEN_BASE_URL={A_BASE_URL}\n",
        encoding="utf-8")
    (b / ".env").write_text(
        f"B_ONLY_TOKEN={B_VAL}\nCRYOZEN_API_KEY={B_API_KEY}\nCRYOZEN_BASE_URL={B_BASE_URL}\n",
        encoding="utf-8")
    for home in (root, b):
    ...
    monkeypatch.setenv("CRYOZEN_API_KEY", A_API_KEY)
    monkeypatch.setenv("CRYOZEN_BASE_URL", A_BASE_URL)
    monkeypatch.setenv("INJECTED_TOKEN", ENV_VAL)  # systemd / op run credential injection
    monkeypatch.setattr(server, "_cryozen_home", root)
...
def test_rpc_scope_reaches_llm_oneshot_and_model_options(two_homes, monkeypatch):
    ...
    def fake_oneshot(**kwargs):
        seen["oneshot"] = (Path(os.environ.get("CRYOZEN_HOME", "")), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        from cryozen_constants import get_cryozen_home
        seen["oneshot_home"] = Path(get_cryozen_home())
        return "t"
    ...
    def build_payload(ctx, **kwargs):
        from cryozen_constants import get_cryozen_home
        seen["options"] = (Path(get_cryozen_home()), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        return {"providers": []}

    monkeypatch.setattr("cryozen_cli.inventory.build_model_options_payload", build_payload)
...
def test_manual_compress_routes_bind_the_sessions_full_runtime_scope(two_homes, monkeypatch, route):
    ...
    def observe_scope():
        seen.append((Path(get_cryozen_home()), get_secret("A_ONLY_TOKEN"), get_secret("B_ONLY_TOKEN")))
...
def test_launch_profile_agent_build_is_scoped_once_multiplexing(two_homes, monkeypatch):
    ...
    try:
        ...
        assert Path(get_cryozen_home()) == root
    finally:
...
class _MemoryManager:
    ...
    def build_system_prompt(self):
        from agent.secret_scope import get_secret
        from cryozen_constants import get_cryozen_home
        return f"{get_cryozen_home()}|{get_secret('MEM_PROVIDER_KEY')}"
