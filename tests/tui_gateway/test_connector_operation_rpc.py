...
- ``connectors.operation.status`` reads the live operation with the session ownership checks
  (4000 / 4001); ``deadline_at`` is the server's, never recomputed
...
def _open_op():
    operation = ConnectionOperation([Target("linear", "mcp", "install"), Target("notion", "mcp", "install")],
                                    session_key=SID)
    live.open(operation)
    return operation
...
def test_operation_status_returns_the_live_snapshot_with_the_server_deadline(owned):
    owner, _, _ = owned
    operation = _open_op()
    operation.transition("linear", TargetState.initiated, Actor.backend_watcher, connect_url="https://l/linear")
    ...
    assert by["linear"]["state"] == "initiated" and by["linear"]["connect_url"] == "https://l/linear"
...
def test_panel_connect_refuses_to_rerun_a_waiting_target(owned):
    """Try again re-runs only a failed target; a waiting target keeps its link (the card reopens
    it), so the RPC refuses rather than start a second attempt."""
    owner, _, _ = owned
    operation = _open_op()
    operation.transition("linear", TargetState.initiated, Actor.backend_watcher, connect_url="https://l/linear/1")

    refused = _connect_rpc(owner, connectors=["linear"])
    assert refused["error"]["code"] == 4002, refused
    assert operation.target("linear").state == TargetState.initiated
    assert operation.target("linear").connect_url == "https://l/linear/1"
