...
"No LLM provider configured. Run `cryozen model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real CRYOZEN home.
...
# provider (e.g. OpenRouter) but who switched THIS session to a self-hosted model
...
# found" (the b200/cryozen-ultra-sft report). The stored MODEL NAME is the one
...
ULTRA_CONFIG = {
    ...
    "model": {"default": "some-openrouter-model", "provider": "openrouter"},
    "providers": {
        "cryozen-ultra": {
            "api": ULTRA_URL,
            "api_key": "sk-ultra",
            "models": ["cryozen-ultra-sft"],
        }
    },
}
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("cryozen-ultra-sft")
            == "custom:cryozen-ultra"
        )
...
# what left room bots stuck on a stale provider (e.g. "out of credits"
...
#
# Regression: GH #89497 (room bots hang then report "out of credits").
...
class TestRoomPlumbingRuntimeOverrides:
    def test_marked_row_returns_no_overrides(self):
        ...
        row = {
            ...
            "model_config": json.dumps(
                {"model": "openai/gpt-5.6-luna-pro", "provider": "openrouter", "room_plumbing": True}
            ),
        }
    ...
    def test_marked_row_dict_model_config(self):
        ...
        row = {
            "model": "openai/gpt-5.6-luna-pro",
            "model_config": {"model": "openai/gpt-5.6-luna-pro", "provider": "openrouter", "room_plumbing": True},
        }
        assert _stored_session_runtime_overrides(row) == {}
    ...
    def test_legacy_group_title_shape_still_skipped(self):
        ...
        row = {
            ...
            "model": "openai/gpt-5.6-luna-pro",
            "billing_provider": "openrouter",
            "model_config": json.dumps({"model": "openai/gpt-5.6-luna-pro", "provider": "openrouter"}),
        }
        assert _stored_session_runtime_overrides(row) == {}
...
class TestFollowProfileConfigRuntimeOverrides:
    def test_composer_pick_on_bot_chat_survives_resume_until_profile_model_changes(self, monkeypatch, tmp_path):
        ...
        db.create_session(stored, "desktop", model="profile/default",
                          model_config={"model": "profile/default", "provider": "openrouter", "follow_profile_config": True})
        db.append_message(stored, "user", "hi")
        db.append_message(stored, "assistant", "hello")

        class _FakeAgent:
            model, provider, base_url, api_key, api_mode = "profile/default", "openrouter", "", "", ""
            _session_db = db
        ...
        monkeypatch.setattr(server, "_cryozen_home", str(launch))
        ...
        try:
            with (
                patch("cryozen_cli.model_switch.parse_model_flags", return_value=("glm-5.1", None, False, False, None)),
                patch("cryozen_cli.model_switch.resolve_persist_behavior", return_value=False),
                patch("cryozen_cli.model_switch.switch_model", return_value=result),
            ...
            assert row["composer_override_profile"] == {"model": "profile/default", "provider": "openrouter"}
            ...
            assert record["composer_override_profile"] == {"model": "profile/default", "provider": "openrouter"}
    ...
    def test_profile_model_change_supersedes_composer_override_on_resume_and_live(self, monkeypatch):
        ...
        monkeypatch.setattr(server, "_config_model_target", lambda: ("profile/new-default", "openrouter"))
        row = {
            "title": "Bot Chat",
            "model": "openai/gpt-5.6-luna-pro",
            "model_config": json.dumps(
                {
                    ...
                    "composer_override_profile": {"model": "profile/old-default", "provider": "openrouter"},
                }
            ),
        }
        ...
        session = {
            "agent": types.SimpleNamespace(model="openai/gpt-5.6-luna-pro", provider="openrouter"),
            "model_override": {"model": "openai/gpt-5.6-luna-pro", "provider": "openrouter"},
            "composer_override_profile": {"model": "profile/old-default", "provider": "openrouter"},
            "config_model_seen": ("profile/old-default", "openrouter"),
        }
        ...
        apply_switch.assert_called_once_with(
            "sid", session, "profile/new-default --provider openrouter",
    ...
    def test_ensure_db_row_persists_contract_marker(self, monkeypatch):
        ...
        session = {
            ...
            "follow_profile_config": True,
            "composer_override_profile": {"model": "profile/default", "provider": "openrouter"},
        }
        ...
        assert captured["model_config"].get("composer_override_profile") == {"model": "profile/default", "provider": "openrouter"}
...
# VeniceAI/empero endpoint under a model that should run on OpenRouter). The sibling
...
class TestRuntimeModelConfigDropsStaleKeys:
    ...
    def test_falsy_model_drops_stale_existing_model(self):
        ...
        agent = _agent_like(model="", provider="openrouter")
        existing = {"model": "meituan/longcat-2.0:free", "provider": "openrouter"}
    ...
    def test_truthy_provider_overwrites_stale_existing(self):
        ...
        config = _runtime_model_config(_agent_like(provider="openrouter"), existing)
    ...
    def test_existing_none_returns_only_agent_identity(self):
        ...
        config = _runtime_model_config(_agent_like(provider="openrouter"), None)

        assert config == {"model": "deepseek/deepseek-v4-flash-0731", "provider": "openrouter"}
