...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="cryozen/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is CRYOZEN_HOME (config/state) must not spawn a
    # phantom project; it lands in the Home bucket. A real repo alongside it
    # still groups normally.
    resolve = _resolver(
        {
            "/home/me/.cryozen-agent": ("/home/me/.cryozen-agent", "/home/me/.cryozen-agent"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.cryozen-agent", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.cryozen-agent"
...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.cryozen-agent")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.cryozen-agent"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/cryozen-agent": ("/www/cryozen-agent", "/www/cryozen-agent")})
    sessions = [
        _session("/www/cryozen-agent", branch="main"),
        _session("/www/cryozen-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/cryozen-agent"]
    assert _lane_ids(project) == ["/www/cryozen-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/cryozen-agent"
