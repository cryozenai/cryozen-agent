...
empty ``.env``, which is all a crashed ``cryozen profile create`` leaves behind. Both made a host
...
def two_profile_host(tmp_path, monkeypatch):
    """Fake HOME so ``profiles/`` never resolves to the live install (see cryozen-agent-dev)."""
    home = tmp_path / "fakehome" / ".cryozen-agent"
    ...
    from cryozen_cli.profiles import _get_profiles_root
