...
def test_record_repos_persists_and_shows_zero_session_repo(tmp_path):
    ...
    # Repo-first: a scanned repo with no cryozen sessions still surfaces.
...
def test_scan_time_is_not_treated_as_session_activity(tmp_path):
    ...
    in Cryozen outranked the ones they actually work in.
    """
...
def test_terminal_session_persists_its_launch_cwd():
    ...
    The user cd'd into that directory before running cryozen. Dropping it left
...
def test_desktop_launch_cwd_is_marked_as_context_artifact():
    assert server._context_cwd_is_launch_artifact(
        {"source": "desktop", "cwd": "/opt/cryozen"}
    ) is True
...
def test_desktop_agent_rebuild_preserves_workspace_provenance(
    ...
    session = {
        "agent": object(),
        "attached_images": [],
        "cwd": "/picked/repo" if explicit_cwd else "/opt/cryozen",
        "edit_snapshots": {},
...
def _bind_profiles(monkeypatch, tmp_path: Path, homes: dict[str, Path]) -> None:
    ...
    """
    monkeypatch.setattr(
        "cryozen_cli.profiles.get_profile_dir",
...
def _serving_launch_profile(launch_home: Path):
    ...
    Both the ambient override AND ``server._cryozen_home`` point at it: once the process
    ...
    token = set_cryozen_home_override(launch_home)
    prev_db, prev_error, prev_home = server._db, server._db_error, server._cryozen_home
    server._cryozen_home = launch_home
    ...
    try:
        ...
        server._db.close()
        server._db, server._db_error, server._cryozen_home = prev_db, prev_error, prev_home
        reset_cryozen_home_override(token)
...
def test_projects_without_a_profile_stay_on_the_launch_home(monkeypatch, tmp_path):
    ...
    assert not (Path(os.environ["CRYOZEN_HOME"]) / "projects.db").exists()
