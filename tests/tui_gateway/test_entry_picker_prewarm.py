...
guard is covered in ``tests/cryozen_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``cryozen_cli.model_switch_providers.prewarm_picker_cache_async``
...
import cryozen_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from cryozen_cli.model_switch,
