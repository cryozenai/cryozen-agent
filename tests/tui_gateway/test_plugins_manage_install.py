...
def test_plugins_manage_install_success():
    # The real ``dashboard_install_plugin`` ok payload, key for key. Under CRYOZEN_TEST_ISOLATION the
    ...
    with patch(
        "cryozen_cli.plugins_cmd.dashboard_install_plugin",
...
def test_plugins_manage_update_requires_catalog_sidecar(tmp_path, monkeypatch):
    ...
    import cryozen_cli.plugins_cmd as plugins_cmd
...
def test_plugins_manage_list_reports_desktop_half(tmp_path):
    ...
    with patch("cryozen_cli.plugins_cmd._discover_all_plugins", return_value=rows), \
         ...
         patch("cryozen_cli.plugins_cmd_catalog.catalog_pins", return_value={}):
...
def test_plugins_manage_list_reports_declared_server_snapshot(tmp_path):
    ...
    with patch("cryozen_cli.plugins_cmd._discover_all_plugins", return_value=rows), \
         patch("cryozen_cli.plugins_cmd._get_enabled_set", return_value=set()), \
         patch("cryozen_cli.plugins_cmd._get_disabled_set", return_value=set()), \
         patch("cryozen_cli.plugins_cmd._is_portable_plugin_dir", return_value=True), \
         patch("cryozen_cli.agent_plugins.load_agent_plugin", return_value=package), \
         patch("cryozen_platform.declaration.lookup", return_value=object()), \
