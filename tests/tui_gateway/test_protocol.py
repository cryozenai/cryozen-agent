...
def server():
    ...
    # e.g. cryozen_cli.active_sessions would bind the mocked get_cryozen_home
    ...
    with patch.dict("sys.modules", {
        "cryozen_constants": MagicMock(get_cryozen_home=MagicMock(return_value="/tmp/cryozen_test")),
        "cryozen_cli.env_loader": MagicMock(),
        "cryozen_cli.banner": MagicMock(),
        "cryozen_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `CRYOZEN_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("CRYOZEN_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("CRYOZEN_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_session_resume_rejects_runaway_transcript_before_history_load(
    ...
    assert "cryozen sessions export" in response["error"]["message"]
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/cryozen-agent-dev": {"name": "cryozen-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "cryozen-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient CRYOZEN_HOME
    ...
    # The gateway's own CRYOZEN_HOME (the test-isolation tempdir, no
...
def test_slash_exec_routes_a_secondary_only_bundle_to_dispatch(server, tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CRYOZEN_BUNDLES_DIR", raising=False)
    profile_b = tmp_path / "profile_b"
    external_b = tmp_path / "external_b"
    for name in ("one", "two"):
        d = external_b / name
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `cryozen config set` does)
    ...
    import cryozen_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_cryozen_home", lambda: tmp_path)
    ...
    # Activate midnight, as `cryozen config set display.skin midnight` would.
