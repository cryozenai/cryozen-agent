...
def cryozen_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves CRYOZEN_HOME.
...
def server(cryozen_home, monkeypatch):
    ...
    # Pin config resolution to the isolated CRYOZEN_HOME. Sibling test
    ...
    # module-level ``_cryozen_home = get_cryozen_home()`` snapshot freezes
...
def test_moa_bare_returns_usage(server, session, cryozen_home):
    _write_moa_config(cryozen_home, """
moa:
...
def test_goal_draft_uses_session_profile_without_blocking_rpc_reader(
    ...
    def draft(objective):
        observed.append(get_cryozen_home())
        started.set()
