...
from cryozen_constants import reset_cryozen_home_override, set_cryozen_home_override
from cryozen_cli.active_sessions import active_session_registry_snapshot
from cryozen_cli.browser_connect import ChromeDebugLaunch
...
def test_session_slot_is_claimed_on_first_turn_not_on_create(monkeypatch, tmp_path):
    ...
    token = set_cryozen_home_override(home)
...
def test_compute_host_turn_end_updates_metadata_mirror(monkeypatch):
    ...
    import cryozen_cli.banner as _banner
...
def test_profile_scoped_mcp_discovery_uses_target_home(monkeypatch, tmp_path):
    """MCP discovery must start under the selected profile's CRYOZEN_HOME."""
    from cryozen_cli import mcp_startup
    ...
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "default"))
    token = set_cryozen_home_override(str(profile_home))
    ...
    monkeypatch.setattr(
        ...
        lambda: seen.append(str(get_cryozen_home())),
    )
...
def test_completion_cwd_prefers_profile_over_stale_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_cryozen_home", tmp_path / "launch-home")
...
def test_completion_cwd_prefers_launch_config_over_stale_env(monkeypatch, tmp_path):
    ...
    configured.mkdir()
    stale = tmp_path / "cryozen-agent"
    stale.mkdir()
    launch_home = _write_profile_cfg(tmp_path / "launch-home", str(configured))

    monkeypatch.setenv("TERMINAL_CWD", str(stale))
    monkeypatch.setattr(server, "_cryozen_home", launch_home)
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["CRYOZEN_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("CRYOZEN_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_VOICE", "1")
...
def test_prompt_submit_typed_stop_phrase_ends_voice_chat(monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_VOICE", "1")
    monkeypatch.setenv("CRYOZEN_VOICE_TTS", "1")
    ...
    assert os.environ["CRYOZEN_VOICE"] == "0"
    assert os.environ["CRYOZEN_VOICE_TTS"] == "0"
...
def test_wake_owner_is_sticky_and_routes_detection_to_first_transport(monkeypatch):
    ...
    try:
        ...
        assert emitted == [(
            "wake.detected",
            "first-session",
            {"phrase": "hey cryozen", "profile": None, "start_new_session": True},
            first,
        )]
...
def test_wake_toggle_persists_enabled_flag_only_on_explicit_gesture(monkeypatch):
    ...
    config = {"enabled": False, "phrase": "hey cryozen", "surface": "auto",
              "start_new_session": True}
    persisted = []
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_VOICE", "1")
    # setenv (not delenv) — the handler writes CRYOZEN_VOICE_TTS directly, and
    ...
    monkeypatch.setenv("CRYOZEN_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("CRYOZEN_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("CRYOZEN_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "cryozen_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("CRYOZEN_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("CRYOZEN_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "cryozen_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import cryozen_cli.config as config_mod
    ...
    from cryozen_cli.tools_config import _RECENTLY_SHIPPED_TOOLSETS
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("CRYOZEN_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "cryozen_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("CRYOZEN_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("CRYOZEN_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("CRYOZEN_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("CRYOZEN_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "cryozen_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown CRYOZEN_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def _two_repo_project_skill_sessions(tmp_path, monkeypatch) -> tuple[Path, Path]:
    ...
    def repo(name: str, skill: str) -> Path:
        ...
        (r / ".cryozen-agent" / "skills" / skill).mkdir(parents=True)
        (r / ".cryozen-agent" / "skills" / skill / "SKILL.md").write_text(
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("cryozen_state_registry.acquire", lambda db_path=None: profile_db)
    monkeypatch.setattr(server, "_get_db", lambda: launch_db)
    monkeypatch.setattr(server, "_enable_gateway_prompts", lambda: None)
    monkeypatch.setattr(
        server,
...
def test_stored_session_runtime_overrides_skips_bare_billing_provider(monkeypatch):
    ...
    import cryozen_cli.runtime_provider as rp

    monkeypatch.setattr(rp, "load_config", lambda: cfg)
    monkeypatch.setattr("cryozen_cli.config.load_config", lambda: cfg)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("CRYOZEN_MODEL", raising=False)
    monkeypatch.setenv("CRYOZEN_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("CRYOZEN_MODEL", raising=False)
    monkeypatch.delenv("CRYOZEN_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " openrouter/cryozen-test "}}
    )

    assert server._resolve_model() == "openrouter/cryozen-test"
...
def test_config_sync_switches_unpinned_session(monkeypatch):
    _patch_config_model(monkeypatch, "new/model", provider="openrouter")
    session = _sync_test_session(config_model_seen=("old/model", "openrouter"))
    calls = []
    monkeypatch.setattr(
        server,
        "_apply_model_switch",
    ...
    assert session["config_model_seen"] == ("new/model", "openrouter")
...
def test_config_sync_switches_when_only_provider_differs(monkeypatch):
    _patch_config_model(monkeypatch, "old/model", provider="openrouter")
    session = _sync_test_session(config_model_seen=("old/model", ""))
    calls = []
    monkeypatch.setattr(
        server,
    ...
    assert calls == ["old/model --provider openrouter"]
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set CRYOZEN_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("CRYOZEN_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("CRYOZEN_MODEL", raising=False)
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `cryozen --tui -m <model>` sets CRYOZEN_MODEL/CRYOZEN_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("CRYOZEN_MODEL", "one-shot/model")
    monkeypatch.setenv("CRYOZEN_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("CRYOZEN_MODEL", "seed/model")
    monkeypatch.setenv("CRYOZEN_INFERENCE_MODEL", "seed/model")
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "openrouter"}})

    assert server._config_model_target() == ("", "openrouter")
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "cryozen_cli.model_switch.resolve_persist_behavior",
        lambda *a: pytest.fail("persist_override must bypass resolve_persist_behavior"),
    )
    monkeypatch.setattr(
        "cryozen_cli.model_switch.persist_model_selection",
        lambda _r: pytest.fail("persist_override=False must not persist"),
    )
    monkeypatch.setattr(
        "cryozen_cli.model_cost_guard.expensive_model_warning",
        lambda *a, **k: None,
    )
    session = {"agent": None}

    out = server._apply_model_switch(
        "sid", session, "new/model --provider openrouter", persist_override=False
    )
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("CRYOZEN_MODEL", "openrouter/cryozen-test")
    monkeypatch.setenv("CRYOZEN_TUI_PROVIDER", "openrouter")
    monkeypatch.delenv("CRYOZEN_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("openrouter/cryozen-test", "openrouter")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("CRYOZEN_MODEL", "openrouter/cryozen-test")
    monkeypatch.delenv("CRYOZEN_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("CRYOZEN_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "cryozen_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("openrouter/cryozen-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("CRYOZEN_MODEL", "sonnet")
    monkeypatch.delenv("CRYOZEN_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("CRYOZEN_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "cryozen_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with CryozenCLI / gateway: fallback_providers stays first and keeps
...
def test_make_agent_passes_configured_fallback_chain(monkeypatch):
    ...
    monkeypatch.delenv("CRYOZEN_INFERENCE_MODEL", raising=False)
    monkeypatch.delenv("CRYOZEN_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("CRYOZEN_DESKTOP", raising=False)
    monkeypatch.delenv("CRYOZEN_DESKTOP_TERMINAL", raising=False)
    monkeypatch.setattr(
        server,
        "_load_cfg",
        lambda: {
    ...
    monkeypatch.setattr(
        "cryozen_cli.runtime_provider.resolve_runtime_provider",
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "cryozen_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "cryozen_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_superseded_by_resume_is_recoverable_end_reason():
    from cryozen_state_common import _RECOVERABLE_END_REASONS
...
def test_ensure_session_db_row_records_a_terminal_workspace(monkeypatch, tmp_path):
    ...
    The user cd'd there before running cryozen. Leaving it null stranded the row
...
def test_ensure_session_db_row_stamps_profile_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("cryozen_state_registry.acquire", _ProfileDB)
    monkeypatch.setattr(server, "_resolve_model", lambda: "test-model")
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_cryozen_home", tmp_path)
...
def test_config_get_approval_mode_uses_smart_default_when_key_is_missing(
    ...
    monkeypatch.setattr(server, "_cryozen_home", tmp_path)
    # Point the canonical resolver (load_config → env CRYOZEN_HOME) at the
    # temp home too, so the smart default is asserted against THIS config
    # rather than whatever the developer's real ~/.cryozen-agent happens to hold.
...
def test_config_get_approval_mode_fails_safe_to_manual_for_invalid_explicit_value(
    ...
    # tools.approval, which reads via cryozen_cli.config.load_config —
    # that path resolves CRYOZEN_HOME from the environment, not
    # server._cryozen_home.
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_get_approval_mode_normalizes_yaml_off(tmp_path, monkeypatch):
    ...
    # load_config, which resolves CRYOZEN_HOME from the environment.
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_set_approval_mode_persists_three_way_value_and_emits_live_status(
    ...
    monkeypatch.setattr(server, "_cryozen_home", tmp_path)
    # config.set writes via server._cryozen_home, but the post-write
    # session.info emit resolves the effective mode through the canonical
    # tools.approval resolver (load_config → env CRYOZEN_HOME).
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
    emitted = []
    monkeypatch.setattr(server, "_emit", lambda *args: emitted.append(args))
...
def test_config_set_fast_updates_live_agent_session_scoped(monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("CRYOZEN_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("CRYOZEN_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("CRYOZEN_EXEC_ASK", raising=False)
    monkeypatch.delenv("CRYOZEN_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("CRYOZEN_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["CRYOZEN_GATEWAY_SESSION"] == "1"
    assert server.os.environ["CRYOZEN_EXEC_ASK"] == "1"
    assert server.os.environ["CRYOZEN_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("cryozen_cli.main._has_any_provider_configured", lambda **_kw: False)
...
def test_setup_status_is_a_live_probe_that_is_always_ready(monkeypatch):
    """A provider configured after boot counts on the next poll, and ``ready`` is always true so the
    desktop's setup-profile kickoff never waits on anything."""
    configured = {"value": False}
    monkeypatch.setattr("cryozen_cli.main._has_any_provider_configured", lambda **_kw: configured["value"])
    ...
    assert first["result"] == {"provider_configured": False, "ready": True}
    assert second["result"] == {"provider_configured": True, "ready": True}
...
def test_invalid_params_and_unknown_method_name_the_version_skew_fix():
    ...
    assert "cryozen update" in resp["error"]["message"]
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("cryozen_cli.main._has_any_provider_configured", lambda **_kw: True)
...
def test_setup_runtime_check_honors_requested_provider(monkeypatch):
    ...
    scoped = server.handle_request(
        {"id": "1", "method": "setup.runtime_check", "params": {"provider": "openrouter"}}
    )
    assert scoped["result"]["ok"] is True
    assert scoped["result"]["provider"] == "openrouter"
...
def test_setup_runtime_check_agrees_with_session_fallback_chain(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", fake_resolve)
...
def test_setup_runtime_check_scopes_launch_profile_in_multiplex_backend(monkeypatch, tmp_path):
    ...
    launch_home = tmp_path / ".cryozen-agent"
    launch_home.mkdir()
    monkeypatch.setenv("CRYOZEN_CODEX_BASE_URL", "https://codex.launch.test/v1")
    monkeypatch.setattr(server, "_cryozen_home", launch_home)
    ...
    def resolve_codex(requested=None, **_kwargs):
        ...
        return {
            ...
            "base_url": secret_scope.get_secret("CRYOZEN_CODEX_BASE_URL"),
            "source": "credential-pool",
        }

    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", resolve_codex)
...
def test_setup_readiness_scopes_to_requested_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("cryozen_cli.profiles.profile_exists", lambda name: name == "bot")
    monkeypatch.setattr(server, "_profile_home", lambda profile: bot_home if profile == "bot" else None)
    seen = {}

    def fake_resolve(requested=None, **kwargs):
        seen["home"] = Path(str(get_cryozen_home())).resolve()
    ...
    try:
        ...
        assert status["result"] == {"provider_configured": False, "ready": True, "profile": "bot"}
    ...
    assert status["result"] == {"provider_configured": True, "ready": True, "profile": "bot"}
    ...
    assert Path(str(get_cryozen_home())).resolve() != bot_home.resolve()
...
def test_setup_readiness_unknown_profile_never_answers_for_launch_profile(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.profiles.profile_exists", lambda name: False)
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "cryozen_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=_Agent())
    monkeypatch.setattr("cryozen_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_recovers_failed_profile_resume_after_build_completes(
    ...
    monkeypatch.setenv("CRYOZEN_HOME", str(launch_home))
    monkeypatch.delenv("CRYOZEN_MODEL", raising=False)
    monkeypatch.delenv("CRYOZEN_INFERENCE_MODEL", raising=False)
    ...
    monkeypatch.setattr("cryozen_cli.model_switch.switch_model", fake_switch_model)
    monkeypatch.setattr(
        "cryozen_cli.model_selection_guards.combined_selection_warning",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr("cryozen_state_registry.acquire", FakeDb)
    monkeypatch.setattr(server, "_make_agent", fake_make_agent)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing CRYOZEN_INFERENCE_PROVIDER on a switch leaked the new
    ...
    try:
        ...
        assert os.environ["CRYOZEN_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "CRYOZEN_TUI_PROVIDER" not in os.environ
        assert "CRYOZEN_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write CRYOZEN_MODEL / CRYOZEN_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("CRYOZEN_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("CRYOZEN_MODEL", raising=False)
    monkeypatch.delenv("CRYOZEN_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["CRYOZEN_TUI_PROVIDER"] == "openai-codex"
        assert "CRYOZEN_MODEL" not in os.environ
        assert "CRYOZEN_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("CRYOZEN_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("CRYOZEN_MODEL", "old/model")
    monkeypatch.setattr(
        "cryozen_cli.model_switch.switch_model",
    ...
    try:
        ...
        assert os.environ["CRYOZEN_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["CRYOZEN_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_config_set_model_session_switch_clears_pending_once_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setattr("cryozen_cli.model_switch.switch_model", lambda **_kwargs: result)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda *args, **kwargs: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_personality_preserves_history_and_returns_info(monkeypatch):
    ...
    monkeypatch.setattr(server, "_emit", lambda *args: emits.append(args))
    # Persistence now flows through the single owner (cryozen_cli.personality),
    # never _write_config_key / agent.system_prompt.
    import cryozen_cli.personality as personality_mod
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        # attachments dir.
        assert not (workspace / ".cryozen-agent" / "desktop-attachments").exists()
...
def test_commands_catalog_includes_plugin_commands(monkeypatch):
    monkeypatch.setattr(
        "cryozen_cli.plugins.get_plugin_commands",
        lambda: {
            "lcm": {
...
def test_plugin_slash_command_runs_under_the_session_env(monkeypatch):
    ...
    # thread with no CRYOZEN_SESSION_* binding, so a handler reading get_session_env() saw "" (or the
    ...
    def handler(arg):
        seen["key"] = get_session_env("CRYOZEN_SESSION_KEY")
        return f"ok:{arg}"

    monkeypatch.setattr("cryozen_cli.plugins.get_plugin_command_handler",
    ...
    assert get_session_env("CRYOZEN_SESSION_KEY") in ("", None)
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Cryozen TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("cryozen_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "cryozen_cli.commands_completion.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("cryozen_state")
    ...
    monkeypatch.setitem(sys.modules, "cryozen_state", fake_mod)
    fake_registry = types.ModuleType("cryozen_state_registry")
    fake_registry.acquire = _broken_shared
    monkeypatch.setitem(sys.modules, "cryozen_state_registry", fake_registry)
...
def test_prompt_submit_fails_loudly_when_store_unavailable(monkeypatch):
    ...
    assert "not saved" in msg and "cryozen doctor --fix" in msg
...
def test_session_list_returns_clean_error_when_state_db_is_unavailable(monkeypatch):
    ...
    assert "cryozen doctor --fix" in resp["error"]["message"]
    assert resp["error"]["data"]["code"] == "storage_unavailable"
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_cryozen_home
    # fixture pins CRYOZEN_HOME to a temp dir; the handler should append
    # /sessions to it.
...
def test_session_list_honors_params_profile_opens_profile_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr(
        "cryozen_cli.web_server_sessions._open_session_db_at_path",
...
def test_handoff_request_uses_session_profile_home(monkeypatch, tmp_path):
    ...
    assert get_cryozen_home() != profile_home
...
def test_session_delete_honors_params_profile_sessions_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: None)
    monkeypatch.setattr("cryozen_state_registry.acquire", ProfileDB)
...
def test_session_branch_installs_parent_profile_secret_scope(monkeypatch, tmp_path):
    ...
    session.branch already binds the parent's CRYOZEN_HOME and state.db, but the
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `cryozen model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `cryozen model` and the classic CLI /model picker.
    ...
    for providers whose /models endpoint returns image/video
    ...
    with patch(
        ...
        # guarding against.
        with patch("cryozen_cli.models.provider_model_ids") as live_fetch:
            resp = server._methods["model.options"](99, {"session_id": ""})

    assert "result" in resp, resp
    providers = resp["result"]["providers"]
    openrouter = next((p for p in providers if p.get("slug") == "openrouter"), None)
    assert openrouter is not None
    assert openrouter["models"] == [
        "moonshotai/kimi-k2.5",
        "anthropic/claude-opus-4.7",
    ]
    assert openrouter["total_models"] == 30
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from cryozen_cli.inventory import ConfigContext

    calls = []

    monkeypatch.setattr(server, "_resolve_model", lambda: "")
    monkeypatch.setattr(
        "cryozen_cli.inventory.load_picker_context",
    ...
    monkeypatch.setattr(
        "cryozen_cli.inventory.build_models_payload",
...
def test_model_options_preserves_canonical_custom_row_after_agent_init(monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.runtime_provider.canonical_custom_identity",
        canonical,
    )
    monkeypatch.setattr(
        "cryozen_cli.model_switch.list_authenticated_providers",
    ...
    monkeypatch.setattr(
        "cryozen_cli.auth.is_provider_explicitly_configured",
        lambda _slug: False,
    )
    monkeypatch.setattr("cryozen_cli.inventory._apply_pricing", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("cryozen_cli.inventory._apply_capabilities", lambda *_args, **_kwargs: None)
...
def test_model_save_key_uses_credential_lifecycle_and_picker_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.auth.PROVIDER_REGISTRY",
        {
    ...
    monkeypatch.setattr("cryozen_cli.config.is_managed", lambda: False)
    save_credential = Mock()
    monkeypatch.setattr(
        "cryozen_cli.credential_lifecycle.save_provider_env_credential",
        save_credential,
    )
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``cryozen --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_verification_status_returns_recorded_evidence(tmp_path, monkeypatch):
    monkeypatch.setenv("CRYOZEN_VERIFY_ON_STOP", "1")  # ledger is inert when the guard is off
    ...
    token = set_cryozen_home_override(profile_home)
    project = tmp_path / "project"
    project.mkdir()
    (project / ".git").mkdir()
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"cryozen_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool_lifecycle": fake}):
        ...
        with (
            patch(
                "cryozen_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("cryozen_cli.browser_connect.local_port_in_use", return_value=False),
            patch("cryozen_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
                "cryozen_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_squatted_port_launches_on_alternate(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool_lifecycle": fake}):
        with (
            patch("cryozen_cli.browser_connect.launch_chrome_debug", side_effect=_launch),
            patch("cryozen_cli.browser_connect.local_port_in_use", return_value=True),
            patch("cryozen_cli.browser_connect.find_free_debug_port", return_value=9223),
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_cryozen_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.cryozen-agent/.env
    ...
    with patch.dict(sys.modules, {"cryozen_cli.config": fake}):
...
def test_session_save_writes_under_cryozen_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Cryozen profile
    ...
    Regression: the gateway handler wrote ``cryozen_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Cryozen.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("cryozen_conversation_*.json"))
    ...
    assert payload["model"] == "cryozen-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Cryozen."
...
def _periodic_trim_calls(monkeypatch):
    ...
    import cryozen_cli.mem_trim as mem_trim
...
class TestResolveRuntimeWithFallback:
    ...
    def test_quota_auth_error_is_logged_as_rate_limited_not_auth_failed(self, monkeypatch, caplog):
        ...
        from cryozen_cli.auth import CODEX_RATE_LIMITED_CODE, AuthError
...
def test_tts_stream_begin_and_stop_lifecycle(monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_VOICE_TTS", "1")
    monkeypatch.setenv("CRYOZEN_VOICE", "0")  # no barge-in monitor (no mic)
    playback_stops: list = []
...
def test_full_duplex_stop_phrase_mid_generation_ends_voice_chat(monkeypatch, tmp_path):
    ...
    assert os.environ.get("CRYOZEN_VOICE") == "0"  # voice chat ended
...
def test_build_persist_message_quotes_paths_containing_spaces(tmp_path):
    ...
    img_dir = tmp_path / "Application Support" / "Cryozen" / "composer-images"
...
def test_prompt_submit_releases_old_history_before_heap_trim(monkeypatch, tmp_path):
    ...
    try:
        ...
        monkeypatch.setattr(server, "render_message", lambda _t, _c: "")
        monkeypatch.setattr(server, "_emit", lambda *a: None)
        monkeypatch.setattr(server, "set_cryozen_home_override", lambda _home: object())
        monkeypatch.setattr(
            server,
        ...
        monkeypatch.setattr("cryozen_cli.mem_trim.trim_memory", _inspect_trim_frame)
...
# silently rewrote ~/.cryozen-agent/config.yaml top-to-bottom — top-level keys
...
def test_prompt_submit_truncation_archives_instead_of_deleting(monkeypatch):
    ...
    tests/cryozen_state/test_replace_messages_archive_siblings.py.
    """
...
def test_persist_live_session_system_prompt_uses_profile_home(monkeypatch, tmp_path):
    ...
    CRYOZEN_HOME to the session's profile before rebuilding the system
    ...
    # to the profile home, not the root ~/.cryozen-agent.
...
def test_persist_live_session_system_prompt_restores_pre_existing_override(tmp_path):
    """reset_cryozen_home_override() restores the previous ContextVar state,
    ...
    class FakeAgent:
        ...
        def _build_system_prompt(self, system_message=None):
            from cryozen_constants import get_cryozen_home
            built_homes.append(str(get_cryozen_home()))
            return "inner prompt"
    ...
    outer_token = set_cryozen_home_override(outer_home)
    try:
        server._persist_live_session_system_prompt(session)
        ...
        # to None.
        assert get_cryozen_home_override() == str(outer_home)
    finally:
        reset_cryozen_home_override(outer_token)
    assert get_cryozen_home_override() is None
