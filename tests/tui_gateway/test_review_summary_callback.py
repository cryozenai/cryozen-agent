    ...
    with patch.dict(
        "sys.modules",
        {
            "cryozen_constants": MagicMock(
                get_cryozen_home=MagicMock(return_value="/tmp/cryozen_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'cryozen-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'cryozen-release' patched"
    }
