...
def test_the_cas_writer_round_trips_through_the_listing(home):
    ...
    before = _row()["ui_meta_revisions"]["cryozen-bots"]

    envelope = srv._methods["profiles.configure"](2, {
        "name": "bob",
        "ui_meta": {"cryozen-bots": {"title": "Bobby"}},
        "ui_meta_expected_revisions": {"cryozen-bots": before},
    })
    assert envelope["result"]["applied"]["ui_meta"] is True

    row = _row()
    assert row["ui_meta"]["cryozen-bots"]["title"] == "Bobby"
    assert row["ui_meta_revisions"]["cryozen-bots"] == before + 1
