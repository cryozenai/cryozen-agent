...
Drives a fake process profile (registered like a ``$CRYOZEN_HOME/plugins/<name>`` model-provider plugin
would) through ``list_available_providers``, ``provider_model_ids``, the TUI/Desktop ``model.options``
RPC, the ``cryozen model`` setup picker and a session-scoped ``config.set model`` switch.
"""
import shutil
import sys
...
def picker_env(monkeypatch, tmp_path):
    ...
    import cryozen_cli.inventory as inventory
...
def test_process_provider_reaches_every_shared_picker(picker_env, monkeypatch):
    ...
    from cryozen_cli.main_provider_setup import _build_provider_picker_rows
    from cryozen_cli.models import _PROVIDER_LABELS, list_available_providers, provider_model_ids
    ...
    from cryozen_cli import main, model_setup_flows
