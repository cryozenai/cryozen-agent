...
``X-Cryozen-Session-Key``.  Every conversation-affinity hint Cryozen sends is
derived from that physical id — ``prompt_cache_key`` on both OpenAI-wire
transports, the OpenRouter sticky ``session_id``, and xAI's
``x-grok-conv-id`` — so all of them re-keyed on every single reply.
...
def _headers(session_key=None):
    ...
    if session_key:
        h["X-Cryozen-Session-Key"] = session_key
    return h
