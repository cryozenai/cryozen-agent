...
shared by the runtime self-provision path and the `cryozen gateway enroll` CLI.
...
Three modes:
  ...
  No token_url configured: raises (there is no default identity provider).
...
def test_no_token_url_raises_without_any_request(monkeypatch):
    """No IdP configured: a clear error, and no network call of any kind."""
    def fail_urlopen(req, timeout=None):
        raise AssertionError("no request may be made without a configured token_url")

    monkeypatch.setattr("urllib.request.urlopen", fail_urlopen)
    with pytest.raises(RuntimeError, match="gateway.idp.token_url"):
        relay._resolve_relay_identity_token()
