...
managed). The real gate is "relay_url set + no pinned secret + a resolvable
identity token".
"""
...
def _arm(monkeypatch, *, url="wss://connector.example/relay", token="idp-token"):
    """Arm the real trigger: a relay URL + a resolvable identity token.

    Note there is intentionally no `managed` knob — self-provision does not
    consult is_managed(). A test that wants the "no identity" branch
    makes the identity resolver raise instead.
    """
...
def test_provisions_and_sets_env_in_process(monkeypatch):
    ...
    assert captured["access_token"] == "idp-token"
...
def test_no_identity_token_is_non_fatal(monkeypatch):
    """A box with a relay URL but no resolvable identity skips quietly."""
...
def test_relay_display_name_suppresses_stock_brand(monkeypatch):
    """The default 'Cryozen Agent' brand is identical on every install — forwarding
    ...
    class _Skin:
        def get_branding(self, key, fallback=""):
            return "Cryozen Agent" if key == "agent_name" else fallback

    monkeypatch.setattr("cryozen_cli.skin_engine.get_active_skin", lambda: _Skin())
