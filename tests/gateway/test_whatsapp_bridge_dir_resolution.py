...
(/opt/cryozen/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable CRYOZEN_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_cryozen_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable CRYOZEN_HOME."""
    ...
    monkeypatch.setattr(
        "cryozen_constants.get_cryozen_home", lambda: cryozen_home
    )
    ...
    expected = cryozen_home / "scripts" / "whatsapp-bridge"
