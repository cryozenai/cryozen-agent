...
The adapter is constructed and connected inside ``_profile_runtime_scope`` (CRYOZEN_HOME override +
...
def _observe(routed_home: Path) -> tuple:
    try:
        ...
        secret = "<unscoped>"
    return ("routed" if Path(get_cryozen_home()) == routed_home else "launch", secret)
...
def routed_scope(tmp_path):
    ...
    home_token = set_cryozen_home_override(str(routed_home))
