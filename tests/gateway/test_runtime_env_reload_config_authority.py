...
CRYOZEN_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env CRYOZEN_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``cryozen config get terminal.backend`` still says local.
    """
    cryozen_home = tmp_path / ".cryozen-agent"
    cryozen_home.mkdir()
    (cryozen_home / "config.yaml").write_text(
    ...
    (cryozen_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
