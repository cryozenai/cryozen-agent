...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    cryozen_home.mkdir()
    monkeypatch.setattr(base, "_CRYOZEN_HOME", cryozen_home)
    monkeypatch.setattr(base, "_CRYOZEN_ROOT", cryozen_home)
    path = cryozen_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestSecretCaptureGuidance:
    def test_gateway_secret_capture_message_points_to_local_setup(self):
        ...
        assert "~/.cryozen-agent/.env" in message
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.cryozen-agent/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("CRYOZEN_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("CRYOZEN_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("CRYOZEN_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("CRYOZEN_KANBAN_HOME", str(tmp_path / "cryozen"))
        monkeypatch.setenv("CRYOZEN_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "cryozen" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("CRYOZEN_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("CRYOZEN_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("CRYOZEN_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.cryozen-agent/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        cryozen_dir = fake_home / ".cryozen-agent"
        (cryozen_dir / "mcp-tokens").mkdir(parents=True)
        secret = cryozen_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._CRYOZEN_HOME",
            cryozen_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._CRYOZEN_ROOT",
            cryozen_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the CRYOZEN_HOME root (google_token.json)
        ...
        fake_home = tmp_path / "home"
        cryozen_dir = fake_home / ".cryozen-agent"
        cryozen_dir.mkdir(parents=True)
        token = cryozen_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._CRYOZEN_HOME", cryozen_dir)
        monkeypatch.setattr("gateway.platforms.base._CRYOZEN_ROOT", cryozen_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_cryozen_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.cryozen-agent
        ...
        cryozen_dir = fake_home / ".cryozen-agent"
        cryozen_dir.mkdir(parents=True)
        artifact = cryozen_dir / "adhoc_report.pdf"
    ...
    def test_denylist_blocks_session_stores_but_not_neighbouring_artifacts(self, tmp_path, monkeypatch):
        ...
        pasted into a chat; ``MEDIA:~/.cryozen-agent/state.db`` must not exfiltrate them (#41071). Named
        ...
        monkeypatch.setattr("gateway.platforms.base._CRYOZEN_HOME", cryozen_dir)
        monkeypatch.setattr("gateway.platforms.base._CRYOZEN_ROOT", cryozen_dir)
        board = cryozen_dir / "kanban" / "boards" / "team-a"
        ...
        for rel in denied + allowed:
            path = cryozen_dir / rel
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(b"SQLite format 3\x00")
        assert [rel for rel in denied if BasePlatformAdapter.validate_media_delivery_path(str(cryozen_dir / rel))] == []
        assert [rel for rel in allowed if not BasePlatformAdapter.validate_media_delivery_path(str(cryozen_dir / rel))] == []
    ...
    def test_denylist_covers_every_profile_home_not_just_the_launch_home(self, tmp_path, monkeypatch):
        ...
        turn is the launch (default) profile's or the secondary's own (CRYOZEN_HOME override), while
        ...
        self._patch_roots(monkeypatch)
        fake_home = tmp_path / "home"
        cryozen_root = fake_home / ".cryozen-agent"
        profile_b = cryozen_root / "profiles" / "beta"
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._CRYOZEN_HOME", cryozen_root)
        monkeypatch.setattr("gateway.platforms.base._CRYOZEN_ROOT", cryozen_root)
        ...
        for scope in (None, profile_b):  # default profile's turn, then beta's own turn
            token = set_cryozen_home_override(scope)
            try:
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting CRYOZEN_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose CRYOZEN_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.cryozen-agent) emits
        ...
        cryozen_root = denied_root / ".cryozen-agent"
        prof_cache = cryozen_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._CRYOZEN_ROOT", cryozen_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.cryozen-agent/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        cryozen_home = tmp_path / ".cryozen-agent"
        cache = cryozen_home / "cache" / "images"
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.cryozen-agent/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_off_and_installed_range_never_touch_process_env(self):
        env = {"CRYOZEN_HUMAN_DELAY_MODE": "custom", "CRYOZEN_HUMAN_DELAY_MIN_MS": "10",
               "CRYOZEN_HUMAN_DELAY_MAX_MS": "20"}
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"CRYOZEN_MEDIA_DELIVERY_STRICT": "1",
                                     "CRYOZEN_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Cryozen
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../cryozen/cache/video/abc.mp4``
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.cryozen-agent exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".cryozen-agent"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.cryozen-agent/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
