...
@pytest.mark.parametrize(
    ...
    [
        ("darwin", "ai.cryozen.gateway", 60.0 - LAUNCHD_STOP_CLEANUP_RESERVE_S),
        ...
        ("linux", "ai.cryozen.gateway", 180.0),
    ],
)
def test_capped_drain_fits_inside_launchd_budget_minus_reserve(monkeypatch, platform, label, expected):
    ...
    # End to end through the reader: only ai.cryozen jobs yield a budget.
