...
def profile_env(tmp_path, monkeypatch):
    """Isolated CRYOZEN_HOME mirroring tests/cryozen_cli/test_profiles.py."""
...
def _record(pid=424242, start=111222333, home=None, argv=None):
    return {
        ...
        "argv": argv or ["python", "-m", "cryozen_cli.main", "gateway", "run"],
        "start_time": start,
        "cryozen_home": home,
    }
...
class TestRecordAuthority:
    def test_valid_bound_same_home_record_allows(self, profile_env):
        ...
        with (
            patch(
                ...
                return_value=_record(home=str(profile_env / ".cryozen-agent")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".cryozen-agent" / "gateway.pid",
            ),
            patch(
            ...
            patch(
                "gateway.status._get_process_cryozen_home",
                return_value=profile_env / ".cryozen-agent",
            ),
        ):
        ...
        with (
            patch(
                ...
                return_value=_record(
                    home="/home/other/.cryozen-agent/profiles/timothy"
                ),
            ),
            patch(
                "gateway.status._get_pid_path",
            ...
            patch(
                "gateway.status._get_process_cryozen_home",
                return_value=profile_env / ".cryozen-agent" / "profiles" / "tim",
            ),
        ):
    ...
    def test_legacy_record_without_home_refuses(self, profile_env):
        """A pre-cryozen_home-stamping record cannot prove ownership."""
        from gateway.run import _replace_target_belongs_to_other_profile

        legacy = _record(home=None)
        legacy.pop("cryozen_home")
    ...
    def test_unbound_record_wrong_pid_refuses(self, profile_env):
        ...
        with (
            patch(
                "gateway.status._read_pid_record",
                return_value=_record(pid=999999, home=str(profile_env / ".cryozen-agent")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".cryozen-agent" / "gateway.pid",
    ...
    def test_unbound_record_stale_start_time_refuses(self, profile_env):
        ...
        with (
            patch(
                "gateway.status._read_pid_record",
                return_value=_record(start=1, home=str(profile_env / ".cryozen-agent")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".cryozen-agent" / "gateway.pid",
...
class TestArgvConsistencyCheck:
    ...
    def test_prefix_collision_is_not_a_conflict(self, profile_env):
        ...
        tim_home = Path("/home/x/.cryozen-agent/profiles/tim")
        # Foreign target advertising timothy — NOT ours.
        assert (
            conflict("python -m cryozen_cli.main --profile timothy gateway run", tim_home)
            is True
        )
        # Our own exact name stays consistent.
        assert (
            conflict("python -m cryozen_cli.main --profile tim gateway run", tim_home)
            is False
        )
        assert (
            conflict("python -m cryozen_cli.main -p tim gateway run", tim_home)
            is False
        )

    def test_explicit_home_flag_exact_compare(self, profile_env):
        """CRYOZEN_HOME= on the argv compares path-exactly, not by prefix."""
        from gateway.run import (
            _looks_like_profile_conflict_from_cmdline as conflict,
        )
        ...
        assert (
            conflict(
                "python -m cryozen_cli.main CRYOZEN_HOME=/home/x/.cryozen-agent/profiles/timothy gateway run",
                tim_home,
            )
            is True
        )
        assert (
            conflict(
                "python -m cryozen_cli.main --cryozen-home /home/x/.cryozen-agent/profiles/tim/ gateway run",
                tim_home,
            )
    ...
    def test_default_home_refuses_any_named_profile_flag(self):
        ...
        root = Path("/home/x/.cryozen-agent")
    ...
    def test_consistency_contradiction_refuses_even_with_agreeing_record(
        ...
        with (
            ...
            patch(
                ...
                return_value="python -m cryozen_cli.main --profile other-profile gateway run",
            ),
            patch(
                "gateway.status._get_process_cryozen_home",
                return_value=profile_env / ".cryozen-agent",
