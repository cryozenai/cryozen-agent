...
closure the PR changed, against a real temp ``CRYOZEN_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_cryozen_home() / config.yaml`` — point it here.
    monkeypatch.setattr("cryozen_constants.get_cryozen_home", lambda: cryozen_home)
    monkeypatch.setattr("cryozen_cli.config.get_cryozen_home", lambda: cryozen_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_cryozen_home", default_home)
    _stub_picker_dependencies(monkeypatch)
...
def _make_store_runner(adapter, sessions_dir, monkeypatch):
    ...
    monkeypatch.setattr(cryozen_state, "SessionDB", _no_sqlite)
