...
class TestGatewayConnectionErrorReply:
    ...
    def test_auth_and_rate_limit_preserved(self):
        ...
        assert "sign-in" in auth_reply.lower() and "/model" in auth_reply
    ...
    def test_every_reply_names_a_slash_command_and_no_jargon(self):
        ...
        'gateway logs' are operator words (the log pointer is the `cryozen logs` command)."""
...
class TestQuotaExhaustedIsNotAnAuthFailure:
    ...
    def test_quota_envelope_with_auth_preamble_names_reset_window(self):
        ...
        assert "Sign-in to the AI model service failed" not in reply and "resets in ~33h" in reply
        ...
        assert "Sign-in to the AI model service failed" not in _gateway_provider_error_reply("request 05:14:15,401 failed")
        for text in ("HTTP 401: Unauthorized", "returned 401.", "HTTP 401, re-authenticate", "Error code: 401 - token rejected"):
            assert "Sign-in to the AI model service failed" in _gateway_provider_error_reply(text), text
    ...
    def test_turn_runner_resolution_quota_failure_names_reset_window_not_login(self, monkeypatch):
        ...
        from cryozen_cli.auth_constants import CODEX_RATE_LIMITED_CODE
        ...
        assert "Sign-in to the AI model service failed" not in reply and "resets in ~33h" in reply and result["api_calls"] == 0
