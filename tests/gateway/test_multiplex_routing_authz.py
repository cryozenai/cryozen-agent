...
Every test builds a bare ``GatewayRunner`` with stub adapters against a temp ``CRYOZEN_HOME`` and
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("cryozen_cli.profiles.profiles_to_serve", return_value=served), \
            patch("cryozen_cli.profiles.get_profile_dir", side_effect=lambda n: home / "profiles" / n), \
