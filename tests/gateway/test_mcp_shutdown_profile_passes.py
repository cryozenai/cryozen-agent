...
context: a caller sitting inside a served profile's scope would otherwise hand its CRYOZEN_HOME
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "launch"))
...
async def test_teardown_thread_does_not_inherit_the_callers_home_override(tmp_path, monkeypatch):
    ...
    from cryozen_constants import (
        get_cryozen_home_override, reset_cryozen_home_override, set_cryozen_home_override)
    ...
    monkeypatch.setattr(
        ...
        lambda **_kw: seen.append(get_cryozen_home_override()))

    token = set_cryozen_home_override(str(poison))
    try:
