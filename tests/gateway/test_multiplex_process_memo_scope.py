...
(CRYOZEN_HOME override), not the launch home the module constants were frozen from."""
...
def two_homes(tmp_path, monkeypatch):
    """Launch home A (CRYOZEN_HOME) and a routed profile B with different config everywhere."""
...
def test_max_turns_and_fallback_chain_follow_routed_profile(two_homes, monkeypatch):
    ...
    monkeypatch.setattr(gateway_run, "_cryozen_home", a)
...
async def test_gateway_hooks_fire_per_routed_profile(two_homes):
    ...
    try:
        ...
        reset_cryozen_home_override(token)
    assert ctx_a.get("seen") == ["cryozen_hook_hook-a"]
    assert ctx_b.get("seen") == ["cryozen_hook_hook-b"]
