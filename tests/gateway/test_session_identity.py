...
(no patched predicates) against a temp ``CRYOZEN_HOME`` with two served profiles.
"""
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("cryozen_cli.profiles.profiles_to_serve", return_value=served), \
            patch("cryozen_cli.profiles.get_profile_dir", side_effect=lambda n: home if n == "default" else home / "profiles" / n), \
            patch("cryozen_cli.profiles.profile_exists", return_value=True):
        yield rig
...
def test_unresolved_under_multiplex_raises_and_never_means_default(mux, tmp_path, monkeypatch):
    ...
    solo_home.mkdir()
    monkeypatch.setenv("CRYOZEN_HOME", str(solo_home))
