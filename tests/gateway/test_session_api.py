    ...
    with patch.object(adapter, "_resolve_route", return_value=None):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                ...
                json={"provider": "openrouter", "model": "x-ai/grok-4.5", "require_model_lock": True})
...
async def test_run_agent_binds_api_session_context_for_tool_env(adapter, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_SESSION_ID", "stale-session")
    observed = {}
    ...
    class FakeAgent:
        ...
        def run_conversation(self, user_message, conversation_history, task_id):
            ...
            observed["context_session_id"] = get_session_env("CRYOZEN_SESSION_ID")
            observed["context_platform"] = get_session_env("CRYOZEN_SESSION_PLATFORM")
            observed["context_session_key"] = get_session_env("CRYOZEN_SESSION_KEY")
            observed["child_session_id"] = _make_run_env({}).get("CRYOZEN_SESSION_ID")
...
async def test_session_chat_treats_pre_existing_poisoned_row_as_no_model(session_db):
    ...
    virtual model alias (e.g. "cryozen-agent") persisted literally as its
...
async def test_session_model_lock_endpoint_then_chat_reuses_persisted_lock_and_provider_credentials(
    ...
    # The live resolver would answer for openrouter from the ambient env; route through the
    # gateway's (patched) per-provider resolver like an unconfigured provider does.
    monkeypatch.setattr(
        "gateway.platforms.api_server._resolve_request_runtime_agent_kwargs",
        lambda *_a, **_k: (_ for _ in ()).throw(RuntimeError("no ambient credentials")),
    )
    ...
    assert payload["runtime"]["provider"] == "openrouter"
...
async def test_session_model_lock_endpoint_then_chat_stream_reuses_persisted_lock(
        ...
        return (
            {
                ...
                "runtime": {
                    "provider": "openrouter",
                    "model": "x-ai/grok-4.5",
                    "requested": {"provider": "openrouter", "model": "x-ai/grok-4.5"},
    ...
    assert captured["route"] == {"provider": "openrouter", "model": "x-ai/grok-4.5"}
    assert captured["requested_runtime"]["provider"] == "openrouter"
...
async def test_confirmed_runtime_lock_rejects_actual_runtime_mismatch(adapter, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="confirmed model lock runtime mismatch"):
        await adapter._run_agent(
            ...
            conversation_history=[],
            session_id="mismatch-session",
            route={"provider": "openrouter", "model": "x-ai/grok-4.5"},
            requested_runtime={"provider": "openrouter", "model": "x-ai/grok-4.5"},
            route_source="session_model_lock",
...
def test_confirmed_runtime_lock_rejects_provider_only_mismatch(adapter):
    ...
    with pytest.raises(RuntimeError, match="confirmed model lock runtime mismatch"):
        adapter._turn_runtime_metadata(
            FakeAgent(),
            route={"provider": "openrouter", "model": "some-model"},
            requested_runtime={"provider": "openrouter", "model": "some-model"},
            route_source="session_model_lock",
            confirmed_runtime_lock=True,
        )
...
async def test_unconfirmed_request_does_not_replace_confirmed_session_lock(adapter, session_db):
    ...
    assert config["browser_model_lock"]["provider"] == "openrouter"
    ...
    with patch.object(adapter, "_create_agent", side_effect=fake_create_agent):
        async with TestClient(TestServer(app)) as cli:
            ...
            responses = await (await cli.post(
                "/v1/responses", json={"model": "cryozen-agent", "input": "go", "stream": True})).text()
