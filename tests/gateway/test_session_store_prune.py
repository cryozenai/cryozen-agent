...
def test_session_store_default_db_uses_runtime_cryozen_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime CRYOZEN_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    cryozen_state before a fixture redirected CRYOZEN_HOME used to pin every
    default SessionDB() at the developer's real ~/.cryozen-agent/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_cryozen_home"
    fake_home.mkdir()
    monkeypatch.setenv("CRYOZEN_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "cryozen sessions list" in raw["_README"]
