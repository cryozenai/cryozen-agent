...
class TestFormatSessionInfo:
    ...
    def test_moa_preset_names_the_billed_aggregator(self, runner, tmp_path):
        ...
        moa_cfg = {"moa": {"presets": {"review": {
            ...
            "aggregator": {"provider": "openrouter", "model": "claude-opus-4.8"},
        }}}}
        with p1, p2, p3, patch("cryozen_cli.config.load_config", return_value=moa_cfg):
            info = runner._format_session_info()
        assert "Acting model (billed for the run): openrouter:claude-opus-4.8" in info
...
class TestResetNoticeSessionInfo:
    ...
    def test_multiplex_uses_profile_config(self, runner, tmp_path):
        ...
        with patch("gateway.run._cryozen_home", base), \
