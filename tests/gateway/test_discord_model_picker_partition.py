...
provider whose list exceeds 25 entries (e.g. an OpenRouter catalog with free
variants appended), not silently truncate the tail at 25 options.
...
across up to 3 select menus. This is what makes ``:free`` picks appended after
the curated list appear on Discord; they previously fell off the 25-option cliff.
"""
...
def test_long_model_lists_render_across_partitioned_selects():
    # 37 models: 32 curated + 5 free variants appended at the tail
    ...
    models = [
        ...
        "sakana/fugu-ultra",
        # --- free variants (the tail that was dropped) ---
        "tencent/hy3:free",
