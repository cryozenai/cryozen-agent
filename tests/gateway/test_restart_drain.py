...
async def test_restart_command_while_busy_requests_drain_without_interrupt(monkeypatch):
    ...
    monkeypatch.delenv("CRYOZEN_GATEWAY_EXTERNAL_SUPERVISOR", raising=False)
...
def test_load_busy_text_mode_follows_input_mode_and_honors_legacy(tmp_path, monkeypatch):
    monkeypatch.setattr(gateway_run, "_cryozen_home", tmp_path)
    monkeypatch.delenv("CRYOZEN_GATEWAY_BUSY_TEXT_MODE", raising=False)
    monkeypatch.delenv("CRYOZEN_GATEWAY_BUSY_INPUT_MODE", raising=False)
    ...
    monkeypatch.setenv("CRYOZEN_GATEWAY_BUSY_TEXT_MODE", "queue")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "queue"

    # Bogus legacy value is ignored → falls through to busy_input_mode (interrupt).
    monkeypatch.setenv("CRYOZEN_GATEWAY_BUSY_TEXT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "interrupt"
...
async def test_windows_detached_restart_scrubs_gateway_marker(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway_run, "_resolve_cryozen_bin", lambda: ["cryozen"])
    ...
    assert cmd[-3:] == ["cryozen", "gateway", "restart"]
    assert kwargs["env"].get("_CRYOZEN_GATEWAY") is None
...
async def test_drain_suppress_skips_home_channel_keeps_session_ping(tmp_path, monkeypatch):
    ...
    # Orchestrator auto-update drain: marker present with suppress_notification=True.
    dc.write_drain_request(principal="orchestrator", suppress_notification=True)
...
async def test_request_restart_skips_wait_when_only_wedged_turns(monkeypatch):
    ...
    Regression: a WhatsApp turn wedged for 30+ min pinned `cryozen update`
    ...
    """
    monkeypatch.delenv("CRYOZEN_AGENT_TIMEOUT", raising=False)
    runner, _adapter = make_restart_runner()
    runner.stop = AsyncMock()
...
def test_wedged_agent_count_disabled_timeout_counts_nothing(monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_AGENT_TIMEOUT", "0")
    runner, _adapter = make_restart_runner()
...
async def test_request_restart_skips_wait_for_cron_run_past_inflight_allowance(monkeypatch, tmp_path):
    ...
    #115469 Defect B: a no-agent job whose delivery hung pinned ``cryozen update`` in "draining" for the
