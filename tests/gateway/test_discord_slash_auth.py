...
def adapter():
    ...
    a._client = SimpleNamespace(user=SimpleNamespace(id=99999, name="CryozenBot"), guilds=[])
    return a
...
def _capture_skill_registration(adapter, monkeypatch, entries):
    ...
    import cryozen_cli.commands_platforms as _hc
    monkeypatch.setattr(
...
async def test_skill_handler_rejects_before_dispatch_for_unauthorized(
    ...
    assert "cryozen pairing approve discord" in (
