...
def _mention_entity(text, mention="@cryozen_bot"):
...
def test_observed_group_context_uses_shared_source_and_prompt_for_later_mentions():
    async def _run():
        ...
        text = "@cryozen_bot what did Alice say?"
        msg = _group_message(
            text,
            from_user_id=222,
...
def test_observed_group_context_preserves_slash_command_text_for_dispatch():
    ...
    event = MessageEvent(
        ...
        raw_message=_group_message(
            "/new@cryozen_bot",
            entities=[_bot_command_entity("/new@cryozen_bot", "/new@cryozen_bot")],
        ),
    )

    attributed = adapter._apply_telegram_group_observe_attribution(event)

    assert attributed.text == "/new@cryozen_bot"
...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @cryozen_bot", entities=[_mention_entity("hi @cryozen_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@cryozen_bot",
            entities=[_bot_command_entity("/status@cryozen_bot", "/status@cryozen_bot")],
        ),
        is_command=True,
    ) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="cryozen_bot")

    assert adapter._should_process_message(_group_message("email ops@cryozen_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@cryozen_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @cryozen_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @cryozen_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @cryozen_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @cryozen_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @cryozen_bot")])
    ) is False
...
def test_config_bridges_telegram_group_settings(monkeypatch, tmp_path):
    ...
    # from a developer's real ~/.cryozen-agent/.env, defeating the env-over-YAML bridge
...
class _IdentityBot:
    ...
    def __init__(self, bot_id=999, cached="cryozen_bot", server=None):
        self.id = bot_id
        self._cached = cached
...
def test_bot_never_adopts_another_accounts_username():
    ...
    adapter = _make_adapter(require_mention=True)
    adapter._bot = _IdentityBot(cached="cryozen_bot")
    ...
    assert adapter._current_bot_username() == "cryozen_bot"
...
def test_sibling_bot_wake_word_message_is_observed_not_dropped():
    ...
    async def _run():
        ...
        store = _FakeSessionStore()
        adapter._session_store = store
        msg = _bot_sender_message("cryozen, can you take this one?")
        ...
        assert message["observed"] is True
        assert message["content"].endswith("cryozen, can you take this one?")
...
def test_sibling_bot_explicit_mention_still_dispatches_and_is_not_observed():
    ...
    human = _group_message("cryozen, hello")
