...
class TestApiServerModelsUnderProfile:
    def test_resolve_model_name_follows_active_profile(self, monkeypatch):
        ...
        adapter = _make_adapter(multiplex=True)
        adapter._model_name = "cryozen-agent"
        monkeypatch.setattr(
...
class TestApiServerSessionProfileBinding:
    """CRYOZEN_SESSION_PROFILE must be bound per /p/<profile>/ request.
    ...
    so every API-server turn bound CRYOZEN_SESSION_PROFILE="" and the
    ...
    def test_profile_is_bound_into_session_vars(self):
        ...
        try:
            assert get_session_env("CRYOZEN_SESSION_PROFILE") == "nm-media"
        finally:
