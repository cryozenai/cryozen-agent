...
def test_short_home_binds_in_home(tmp_path: Path):
    ...
    try:
        short_home = short_root / ".cryozen-agent"
        short_home.mkdir()
...
def test_long_home_uses_pointer_fallback(tmp_path: Path):
    deep = tmp_path / ("x" * 120) / ".cryozen-agent"
...
def test_windows_pipe_name_is_stable_and_home_scoped(tmp_path: Path):
    ...
    assert a.startswith(r"\\.\pipe\cryozen-gateway-")
    assert a != b
...
def test_long_home_end_to_end_via_pointer(tmp_path: Path):
    deep = tmp_path / ("p" * 120) / ".cryozen-agent"
    deep.mkdir(parents=True)
...
def test_default_identify_payload_shape(home: Path, monkeypatch):
    ...
    for key in ("cryozen_home", "supervisor", "kind", "start_time"):
        assert key in ident
...
def test_collect_fleet_versions_prefers_socket(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.profiles._get_profiles_root", lambda: tmp_path / "no-profiles"
    )
    # stale state file that would report a WRONG pid — socket must win
    (home / "gateway_state.json").write_text(
        json.dumps({"pid": 1, "code_sha": "stalefile", "kind": "cryozen-gateway"})
    )
    monkeypatch.setattr(
...
def test_runtime_inventory_dedupes_same_pid_across_homes(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cryozen_cli.profiles._get_profiles_root", lambda: profiles_root
    )
    monkeypatch.setattr(
        "cryozen_cli.gateway._get_service_pids", lambda all_profiles=False: set()
    )
    monkeypatch.setattr(
        "cryozen_cli.gateway.find_profile_gateway_processes", lambda: []
    )
    monkeypatch.setattr(
        "gateway.control_socket.identify_gateway",
