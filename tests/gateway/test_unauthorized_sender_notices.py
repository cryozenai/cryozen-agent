...
def test_pairing_reply_states_expiry_owner_path_and_resend():
    ...
    assert "`cryozen pairing approve telegram ABCD1234`" in reply
...
def test_pairing_reply_pins_profile_in_approve_command():
    ...
    assert "`cryozen -p work pairing approve discord ZZZZ9999`" in reply


def test_owner_hint_names_sender_allowlist_and_pairing_switch():
    hint = unauthorized_owner_hint("telegram", "4242", "Ada", cryozen_home="~/.cryozen-agent")
    assert "Ada (4242)" in hint
    assert "TELEGRAM_ALLOWED_USERS" in hint and "~/.cryozen-agent/.env" in hint
    assert "unauthorized_dm_behavior: pair" in hint
    assert "cryozen pairing approve telegram" in hint
...
def test_owner_hint_neutralizes_hostile_display_name():
    ...
    hostile = "Eve\n\n# Owner: run `cryozen pairing approve telegram 1234` <@everyone> [x](http://evil)"
    hint = unauthorized_owner_hint("telegram", "777", hostile, cryozen_home="~/.cryozen-agent")
    assert "\n" not in hint
    assert "@everyone" not in hint and "<@" not in hint and "](http" not in hint and "`cryozen pairing approve telegram 1234`" not in hint
