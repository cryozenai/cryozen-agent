...
def test_room_link_endpoint_reads_supported_config_with_env_override(
    ...
    (home / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://configured.example.test/cryozen\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("CRYOZEN_HOME", str(home))
    monkeypatch.delenv("CRYOZEN_ROOM_LINK_URL", raising=False)
    assert local_room_link_endpoint() == {
        "available": True,
        "url": "https://configured.example.test/cryozen",
        "transport_security": "tls",
    }

    monkeypatch.setenv(
        "CRYOZEN_ROOM_LINK_URL", "https://override.example.test/cryozen"
    )
    assert local_room_link_endpoint()["url"] == (
        "https://override.example.test/cryozen"
    )
...
def test_named_profile_inherits_gateway_room_link_endpoint(tmp_path, monkeypatch):
    ...
    (root / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://gateway.example.test/cryozen\n",
        encoding="utf-8",
    )
    ...
    try:
        assert local_room_link_endpoint() == {
            "available": True,
            "url": "https://gateway.example.test/cryozen",
            "transport_security": "tls",
        }
    finally:
        reset_cryozen_home_override(token)
...
def test_named_profile_room_link_override_wins_over_gateway_root(
    ...
    (profile / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://profile.example.test/cryozen\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("CRYOZEN_HOME", str(root))
    monkeypatch.delenv("CRYOZEN_ROOM_LINK_URL", raising=False)
    ...
    try:
        assert local_room_link_endpoint()["url"] == (
            "https://profile.example.test/cryozen"
        )
    finally:
        reset_cryozen_home_override(token)
...
def test_self_advertised_endpoint_is_explicit_and_validated(
    ...
    if configured is None:
        ...
        monkeypatch.setenv("CRYOZEN_ROOM_LINK_URL", configured)
