...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    def test_connect_websocket_sets_channel_ua_tag_and_uses_owned_executor(self):
        ...
        https://github.com/cryozenai/cryozen-agent/issues/50656
        https://github.com/cryozenai/cryozen-agent/issues/78318
        """
        from gateway.config import PlatformConfig
        from plugins.platforms.feishu.adapter import FeishuAdapter
...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_group_message_matches_bot_name_when_only_name_available(self):
        ...
        adapter2._bot_name = "Cryozen Bot"
    ...
    @patch.dict(
        os.environ,
        {
            "CRYOZEN_FEISHU_TEXT_BATCH_MAX_MESSAGES": "2",
        },
        clear=True,
    )
    ...
    @patch.dict(
        os.environ,
        {
            "CRYOZEN_FEISHU_TEXT_BATCH_MAX_MESSAGES": "8",
            "CRYOZEN_FEISHU_TEXT_BATCH_MAX_CHARS": "4000",
        },
        clear=False,
    )
    ...
    def test_download_remote_document_reads_response_before_httpx_client_closes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict(os.environ, {"CRYOZEN_HOME": tmp}, clear=False):
                adapter = FeishuAdapter(PlatformConfig())
    ...
    def test_dedup_state_persists_across_adapter_restart(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"CRYOZEN_HOME": temp_home}, clear=False):
    ...
    def test_send_splits_fenced_code_blocks_into_separate_post_rows(self):
        ...
        content = (
            "确认已入库 ✓\n"
            "文件路径：`/root/.cryozen-agent/profiles/agent_cto/cron/jobs.json`\n"
            "**解码后的内容：**\n"
            "```json\n"
            '{"cron": "list"}\n'
            "```\n"
...
class TestHydrateBotIdentity(unittest.TestCase):
    ...
    def test_hydration_populates_open_id_from_bot_info(self):
        ...
        payload = json.dumps(
            {
                "code": 0,
                "bot": {
                    "bot_name": "Cryozen Bot",
                    "open_id": "ou_cryozen_hydrated",
                },
            }
        ).encode("utf-8")
        ...
        self.assertEqual(adapter._bot_open_id, "ou_cryozen_hydrated")
        self.assertEqual(adapter._bot_name, "Cryozen Bot")

    @patch.dict(
        os.environ,
        {
            "FEISHU_BOT_OPEN_ID": "ou_env",
            "FEISHU_BOT_NAME": "Env Cryozen",
        },
        clear=True,
    )
    def test_hydration_refreshes_env_values_when_bot_info_available(self):
        ...
        self.assertEqual(adapter._bot_name, "Hydrated Cryozen")
...
class TestWebhookSecurity(unittest.TestCase):
    ...
    def test_webhook_request_rejects_oversized_chunked_body_while_reading(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            token = set_cryozen_home_override(tmpdir)
            try:
                adapter = FeishuAdapter(PlatformConfig())
            finally:
...
class TestDedupTTL(unittest.TestCase):
    ...
    def test_load_tolerates_malformed_timestamp_values(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"CRYOZEN_HOME": temp_home}, clear=True):
                adapter = FeishuAdapter(PlatformConfig())
    ...
    def test_concurrent_dedup_persists_land_in_order(self):
        ...
        # The class-level env wipe drops the per-test CRYOZEN_HOME, so the adapter resolves
        # its dedup store under the operator's real ~/.cryozen-agent and every id a live gateway
        ...
        with tempfile.TemporaryDirectory() as scratch:
            with patch("plugins.platforms.feishu.adapter.get_cryozen_home", return_value=Path(scratch)):
                adapter = FeishuAdapter(PlatformConfig())
            writes = []
            calls = [0]
...
class TestFeishuMentionMap(unittest.TestCase):
    ...
    def test_build_mentions_map_marks_self_by_open_id(self):
        ...
        self.assertEqual(ref.name, "Cryozen")
    ...
    def test_build_mentions_map_name_match_does_not_override_mismatching_open_id(self):
        ...
        result = _build_mentions_map(
            ...
            _FeishuBotIdentity(open_id="ou_bot", name="Cryozen Bot"),
        )
    ...
    def test_build_mentions_map_falls_back_to_name_when_bot_open_id_not_hydrated(self):
        ...
        result = _build_mentions_map(
            [bot_mention],
            _FeishuBotIdentity(open_id="", name="Cryozen Bot"),
        )
...
class TestFeishuMentionHint(unittest.TestCase):
    ...
    def test_hint_filters_self_mentions(self):
        ...
        refs = [
            FeishuMentionRef(name="Cryozen", open_id="ou_bot", is_self=True),
            FeishuMentionRef(name="Alice", open_id="ou_alice"),
        ]
        self.assertEqual(
            _build_mention_hint(refs),
...
class TestFeishuStripLeadingSelf(unittest.TestCase):
    def _make_refs(self, *, self_name="Cryozen", other_name=None):
    ...
    def test_stops_at_first_non_self_token(self):
        ...
        result = _strip_edge_self_mentions(
            "@Cryozen @Alice make a group", self._make_refs(other_name="Alice")
        )
    ...
    def test_strips_trailing_self_with_terminal_punct(self):
        ...
        result = _strip_edge_self_mentions("look up docs @Cryozen.", self._make_refs())
    ...
    def test_preserves_trailing_self_before_non_terminal_char(self):
        ...
        result = _strip_edge_self_mentions(
            "please don't @Cryozen anymore", self._make_refs()
        )
        self.assertEqual(result, "please don't @Cryozen anymore")
...
class TestFeishuNormalizeText(unittest.TestCase):
    ...
    def test_renders_self_mention_with_name(self):
        ...
        refs = {"@_user_1": FeishuMentionRef(name="Cryozen", open_id="ou_bot", is_self=True)}
        self.assertEqual(
            _normalize_feishu_text("stop pinging @_user_1 please", refs),
            "stop pinging @Cryozen please",
        )
    ...
    def test_post_message_marks_self_via_mentions_map_lookup(self):
        ...
        raw = json.dumps({
            "en_us": {
                "content": [
                    [
                        {"tag": "at", "user_id": "@_user_1", "user_name": "Cryozen"},
...
class TestFeishuFetchMessageText(unittest.TestCase):
    ...
    def test_fetch_message_text_marks_is_self_via_string_id_shape(self):
        ...
        self.assertEqual(result, "@Cryozen hi")
...
class TestFeishuMentionEndToEnd(unittest.TestCase):
    ...
    def test_scenario_post_bot_plus_alice_filters_self_from_hint(self):
        ...
        self.assertNotIn("Cryozen (open_id=", event.text)
        # Body: leading @Cryozen stripped, Alice preserved, trailing text intact.
        self.assertIn("@Alice review the spec with Alice", event.text)
        self.assertNotIn("@Cryozen @Alice", event.text)
