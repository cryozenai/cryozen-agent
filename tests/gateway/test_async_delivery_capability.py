...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("CRYOZEN_SESSION_PLATFORM") == "api_server"
        finally:
...
class TestDeclareStatelessChannel:
    """``cryozen -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``CRYOZEN_SESSION_*`` routing keys, so an async delegation's
