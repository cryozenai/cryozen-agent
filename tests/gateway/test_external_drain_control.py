...
Task 2.2/2.3. Two layers:
  * drain_control.py — the presence-based marker contract (write/clear/read,
    CRYOZEN_HOME-scoped, never-raises).
...
Q-B, exercises a real `cryozen gateway run`); these lock the unit contract.
"""
...
class TestMarkerContract:
    ...
    def test_write_then_present(self, home):
        payload = dc.write_drain_request(principal="orchestrator")
        assert dc.drain_requested() is True
        assert payload["action"] == "drain"
        assert payload["principal"] == "orchestrator"
        body = dc.read_drain_request()
        assert body is not None and body["principal"] == "orchestrator"


class TestSuppressNotification:
    """The generic suppress_notification flag on the drain marker.

    Gates ONLY the gateway's home-channel shutdown broadcast (orchestrator auto-update
    ...
    def test_flag_round_trips_true(self, home):
        payload = dc.write_drain_request(principal="orchestrator", suppress_notification=True)
...
class TestInstantiationEpoch:
    ...
    def test_marker_from_prior_instantiation_reads_as_absent(self, home, monkeypatch):
        ...
        # container/VM instantiation survives on the durable CRYOZEN_HOME volume
        ...
        dc.write_drain_request(principal="orchestrator")  # stamps "epoch-OLD"
...
class TestMarkerMaxAge:
    def test_fresh_marker_honoured(self, home):
        dc.write_drain_request(principal="orchestrator")
        assert dc.drain_requested() is True
    ...
    def test_rewrite_refreshes_the_clock(self, home):
        ...
        dc.write_drain_request(principal="orchestrator")  # …keep-alive re-write
        assert dc.drain_requested() is True
