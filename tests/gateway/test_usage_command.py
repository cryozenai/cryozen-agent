...
class TestUsageAccountSection:
    ...
    async def test_usage_command_uses_persisted_provider_when_agent_not_running(self, monkeypatch):
        ...
        async def _fake_to_thread(fn, *args, **kwargs):
            # /usage dispatches the account fetch (fetch_account_usage, called with the
            # provider positionally) through to_thread — record every call rather than
            # last-wins so we can pick out the account fetch.
