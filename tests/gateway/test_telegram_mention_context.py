...
def test_sole_addressee_text_stays_clean_and_prompt_is_session_stable(trigger):
    ...
    async def run():
        ...
        if trigger == "dm":
            ...
            msg = _group_message("/new@cryozen_bot", entities=[SimpleNamespace(type="bot_command", offset=0, length=16)])
        elif trigger == "text_mention":
            msg = _group_message("Cryozen hello", entities=[SimpleNamespace(type="text_mention", offset=0, length=7, user=SimpleNamespace(id=999))])
            ...
            msg = _group_message("@cryozen_bot", reply_to_bot=True, entities=[SimpleNamespace(type="code", offset=0, length=12)])
        else:
        ...
        assert len(events) == 2
        first, second = events
        expected_text = {"command": "/new", "mention": "😀 2", "code": "@cryozen_bot"}.get(trigger, msg.text)
        assert first.text == expected_text
        if trigger == "dm":
            assert not first.channel_prompt
        else:
            assert "@cryozen_bot" in first.channel_prompt
