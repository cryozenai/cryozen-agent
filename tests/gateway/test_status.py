...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "cryozen-gateway"
    ...
    def test_get_running_pid_cached_invalidates_when_pid_file_changes(self, tmp_path, monkeypatch):
        ...
        def _write_record(pid: int, start_time: int) -> None:
            record = {
                ...
                "argv": ["python", "-m", "cryozen_cli.main", "gateway"],
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level CRYOZEN_HOME.
        ...
        the process-level CRYOZEN_HOME, not the profile's directory.  See #56986.
        """
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(process_home))
...
class TestScopedGatewayPidQuery:
    ...
    def _write_scoped_profile(self, tmp_path):
        ...
        record = {
            "pid": 4242,
            "kind": "cryozen-gateway",
            "argv": ["python", "-m", "cryozen_cli.main", "gateway", "--profile", "wiki"],
            "start_time": 123,
            "cryozen_home": str(profile_dir.resolve()),
        }
    ...
    def test_scoped_query_reports_live_foreign_profile_pid(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "default-home"))
        ...
        monkeypatch.setattr(
            ...
            lambda pid: "python -m cryozen_cli.main gateway --profile wiki",
        )
...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``cryozen gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "cryozen gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "cryozen -p coder gateway run --replace",
            "/opt/cryozen/.venv/bin/cryozen --profile coder gateway run --replace",
            "cryozen_home=/opt/data/profiles/coder cryozen gateway run --replace",
        ):
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        """A Windows argv renders CRYOZEN_HOME with backslashes while the
        ...
        cmdline = r"cryozen_home=c:\opt\data\profiles\coder cryozen gateway run --replace"
    ...
    def test_command_line_belongs_to_profile_rejects_sibling_homes(self):
        """A substring test let ``CRYOZEN_HOME=/root/profiles/ops2`` satisfy the ``ops`` profile's
        ...
        for cmdline in (
            "CRYOZEN_HOME=/fixture/profiles/ops2 cryozen gateway run",
            "CRYOZEN_HOME=/fixture/profiles/ops-backup cryozen gateway run",
            "CRYOZEN_HOME=/fixture/profiles/ops/2 cryozen gateway run",
        ):
            assert not status._command_line_belongs_to_profile(cmdline, home), cmdline
        default_home = Path("/opt/cryozen-data")
        assert not status._command_line_belongs_to_profile("CRYOZEN_HOME=/opt/cryozen-data2 cryozen gateway run", default_home)
        assert status._command_line_belongs_to_profile("CRYOZEN_HOME=/opt/cryozen-data cryozen gateway run", default_home)
        assert status._command_line_belongs_to_profile("cryozen gateway run", default_home)
    ...
    def test_command_line_belongs_to_profile_matches_own_home_spellings_only(self):
        ...
        (systemd ``Environment=``, ``sh -c`` wrappers) are the same home; ``FOO=cryozen_home=/x``
        ...
        assert status._command_line_belongs_to_profile(
            'cryozen_home="/opt/data/profiles/coder with space" cryozen gateway run', home)
        ...
        assert status._command_line_belongs_to_profile(
            "CRYOZEN_HOME=/opt/data/profiles/coder with space cryozen gateway run", home)
        assert status._command_line_belongs_to_profile(
            r"CRYOZEN_HOME=C:\Users\John Doe\.cryozen-agent cryozen gateway run", Path(r"C:\Users\John Doe\.cryozen-agent"))
        assert not status._command_line_belongs_to_profile(
            "CRYOZEN_HOME=/opt/data/profiles/coder with spaces cryozen gateway run", home)
        home = Path("/fixture/profiles/ops")
        assert status._command_line_belongs_to_profile("CRYOZEN_HOME=/fixture/profiles/ops/ cryozen gateway run", home)
        assert status._command_line_belongs_to_profile("CRYOZEN_HOME=/opt/cryozen-data/ cryozen gateway run", Path("/opt/cryozen-data"))
        assert not status._command_line_belongs_to_profile("FOO=cryozen_home=/fixture/profiles/ops cryozen gateway run", home)
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CRYOZEN_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "cryozen-gateway",
            "argv": ["/Users/user/.cryozen-agent/cryozen-agent/cryozen_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_self_reacquires_when_disk_start_time_null(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "cryozen-gateway",
            "argv": ["cryozen_cli/main.py", "--profile", "milena", "gateway", "run", "--replace"],
    ...
    def test_acquire_scoped_lock_self_reacquires_when_start_times_differ(
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 111,
            "kind": "cryozen-gateway",
            "argv": ["cryozen_cli/main.py", "gateway", "run", "--replace"],
            "scope": "discord-bot-token",
        }))
    ...
    def test_acquire_scoped_lock_stamps_profile_label(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "profiles" / "lead-gen-outreach"))
    ...
    def test_acquire_scoped_lock_omits_profile_when_not_inferable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "some-custom-dir"))
...
class TestScopedLockOwnerLabel:
    ...
    def test_profile_label_for_root_home_is_default(self, monkeypatch):
        monkeypatch.setenv("CRYOZEN_HOME", "/opt/data")
    ...
    def test_owner_label_prefers_explicit_profile_field(self):
        record = {"profile": "coder", "cryozen_home": "/opt/data/profiles/other"}
    ...
    def test_owner_label_ignores_invalid_profile_and_uses_home(self):
        ...
        # cryozen_home fallback — attribution degrades, never corrupts.
        record = {
            "profile": "bad; rm -rf /",
            "cryozen_home": "/opt/data/profiles/zerocool",
        }
        assert status.scoped_lock_owner_label(record) == "zerocool"

    def test_owner_label_falls_back_to_cryozen_home(self):
        # Locks written before the profile field existed still attribute.
        record = {"pid": 559, "cryozen_home": "/opt/data/profiles/zerocool"}
        assert status.scoped_lock_owner_label(record) == "zerocool"
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_cryozen_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's CRYOZEN_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
        ...
        payload = json.loads((tmp_path / ".gateway-takeover.json").read_text())
        assert payload["replacer_cryozen_home"] == str(tmp_path)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        CRYOZEN_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.cryozen-agent flap
        ...
        marker_path.write_text(json.dumps({
            ...
            "replacer_pid": 99999,
            "replacer_cryozen_home": str(tmp_path / "profiles" / "other"),
    ...
    def test_consume_accepts_legacy_marker_without_cryozen_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Cryozen versions have no
        ``replacer_cryozen_home`` field; an absent field is treated as same-home so
...
class TestScopedLockTakeover:
    ...
    def _owner_record(target_home: Path, *, pid: int = 4242, start_time: int = 123):
        ...
        record = {
            "pid": pid,
            "kind": "cryozen-gateway",
            "argv": ["python", "-m", "cryozen_cli.main", "gateway", "run"],
            "start_time": start_time,
            "cryozen_home": str(target_home),
        }
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        replacer_home.mkdir()
        monkeypatch.setenv("CRYOZEN_HOME", str(replacer_home))
        ...
        monkeypatch.setattr(
            ...
            lambda _pid: "python -m cryozen_cli.main gateway run",
        )
        calls = []
        ...
        def terminate(pid, *, force=False):
            ...
            assert payload["target_cryozen_home"] == str(target_home)
            assert payload["replacer_cryozen_home"] == str(replacer_home)
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``cryozen gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00cryozen_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "cryozen_cli/main.py" in result
        assert calls == ["proc"]
...
class TestGatewayBusyDerivation:
    ...
    def test_degraded_gateway_with_work_is_busy_and_drainable(self):
        # Serving with a parked platform (#91547): in-flight turns must not look idle to an orchestrator.
        assert status.derive_gateway_busy(
...
class TestResolveGatewayLiveness:
    ...
    def test_probe_exception_degrades_instead_of_raising(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CRYOZEN_GATEWAY_LOCK_DIR", str(tmp_path))
    ...
    def test_profile_dir_scopes_every_read_to_that_profile(self, tmp_path):
        ...
        ignore the CRYOZEN_HOME contextvar override (#56986), so the profile
...
def test_strict_gateway_identity_rejects_reused_pid(tmp_path, monkeypatch):
    pid_path = tmp_path / "gateway.pid"
    lock_path = tmp_path / "gateway.lock"
    record = {"pid": 123, "start_time": 10.0, "kind": "cryozen-gateway"}
...
def test_retained_gateway_state_keeps_watchdog_degraded_like_startup_failed():
    ...
    ``cryozen gateway stop`` records the intent. The startup-time ``degraded`` (retryable platforms, no
