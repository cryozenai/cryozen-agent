...
``[diff "x"] command=/textconv=`` driver. Cryozen gathers workspace context by
...
context-gathering git path Cryozen runs neutralizes every sink. They use a real
...
def test_web_git_diff_is_safe(malicious_repo):
    from cryozen_cli import web_git
    repo, marker = malicious_repo
