...
def test_required_frontmatter_fields(p):
    ...
    cryozen = (fm.get("metadata") or {}).get("cryozen") or {}
    if not (cryozen.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.cryozen.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    cryozen = (fm.get("metadata") or {}).get("cryozen") or {}
    dangling = [
        rs for rs in (cryozen.get("related_skills") or []) if rs not in _all_names()
    ]
