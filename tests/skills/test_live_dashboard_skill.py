...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["cryozen"]["related_skills"]:
        hits = (
...
def test_tools_wired_and_home_not_hardcoded():
    ...
    no-change ticks stay silent, and the Cryozen home path is resolved, never assumed."""
    _, body = _frontmatter_and_body()
    ...
    assert "~/.cryozen-agent" not in body
...
def test_frontmatter_blueprint_is_a_valid_installed_blueprint():
    ...
    assert "[SILENT]" in job["prompt"] and "~/.cryozen-agent" not in job["prompt"]