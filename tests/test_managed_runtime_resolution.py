"""Guard: Cryozen-owned subprocesses must not resolve managed runtimes by bare PATH.

Cryozen installs runtimes for itself — ``uv`` at ``$CRYOZEN_HOME/bin/uv``, Node at
``$CRYOZEN_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Cryozen's own
...
* when a system copy also exists, the one Cryozen does not own wins — which is
...
``iter_cryozen_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Cryozen provisions into CRYOZEN_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Cryozen-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("cryozen_cli/update_cmd_deps.py", "uv"): (
    ...
    ("cryozen_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (cryozen_agent-<version>/) gets created and deleted concurrently, and
...
def _resolution_sites() -> set[tuple[str, str, str]]:
    ...
    for path in _source_files():
        rel = path.relative_to(REPO_ROOT).as_posix()
        if rel == "cryozen_platform" or rel.startswith("cryozen_platform/"):
            continue
        try:
            source = path.read_text(encoding="utf-8")
        except (OSError, UnicodeDecodeError):
...
def test_bare_which_and_known_path_tables_are_allowlisted():
    ...
    assert not unlisted, (
        ...
        + "\nuse a cryozen_platform resolver or add a justified allowlist row"
    )


def test_resolution_allowlist_has_no_stale_rows():
    """Remove bootstrap rows as their call sites move to cryozen_platform."""
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Cryozen-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$CRYOZEN_HOME/bin (uv) and $CRYOZEN_HOME/node are not on an "
        ...
        "  node/npm -> cryozen_constants.find_node_executable()\n"
        "  PATH env -> cryozen_constants.iter_cryozen_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(cryozen_constants, helper))


def test_managed_uv_helpers_exist():
    from cryozen_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
