"""Scratch dir contract: TMPDIR/TMP/TEMP follow CRYOZEN_HOME/cache/scratch unless the user set them."""
...
from cryozen_constants import apply_scratch_tmp_env, get_scratch_dir, prune_scratch_dir


def test_scratch_env_follows_home_and_respects_user_tmpdir(tmp_path):
    """Unset temp vars → scratch of env CRYOZEN_HOME; a Cryozen-exported value re-derives for a routed
    home; a user/OS-set value (macOS ``/var/folders``, ``%TEMP%``) is never touched."""
    home_a, home_b = tmp_path / "a", tmp_path / "b"
    env = {"CRYOZEN_HOME": str(home_a)}
    assert apply_scratch_tmp_env(env) is True
    scratch_a = str(home_a / "cache" / "scratch")
    assert env["TMPDIR"] == env["TMP"] == env["TEMP"] == env["CRYOZEN_SCRATCH_DIR"] == scratch_a
    assert os.path.isdir(scratch_a)

    env["CRYOZEN_HOME"] = str(home_b)  # child served under another profile's home
    assert apply_scratch_tmp_env(env) is True
    assert env["TMPDIR"] == str(home_b / "cache" / "scratch")

    user_env = {"CRYOZEN_HOME": str(home_a), "TMPDIR": "/var/folders/zz"}
    assert apply_scratch_tmp_env(user_env) is False
    assert user_env["TMPDIR"] == "/var/folders/zz" and "CRYOZEN_SCRATCH_DIR" not in user_env
...
def test_bootstrap_import_exports_scratch_to_process_and_children(tmp_path):
    """``import cryozen_bootstrap`` alone makes ``tempfile`` (this process AND a child) land in scratch."""
    env = {k: v for k, v in os.environ.items() if k not in ("TMPDIR", "TMP", "TEMP", "CRYOZEN_SCRATCH_DIR")}
    env["CRYOZEN_HOME"] = str(tmp_path)
    code = ("import tempfile, os, subprocess, sys; import cryozen_bootstrap; "
...
class TestScratchDirPermissionPolicy:
    ...
    an explicit CRYOZEN_HOME_MODE and a managed/shared home win (#117347)."""
    ...
    def _isolate_env(self, monkeypatch, tmp_path):
        ...
        for var in ("CRYOZEN_HOME_MODE", "CRYOZEN_MANAGED", "CRYOZEN_CONTAINER",
                    "CRYOZEN_SKIP_CHMOD", "CRYOZEN_UID", "CRYOZEN_GID"):
    ...
    def test_explicit_home_mode_retained_with_setgid(self, tmp_path, monkeypatch):
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setenv("CRYOZEN_HOME_MODE", "2770")
        scratch = get_scratch_dir(tmp_path, prune=False)
    ...
    def test_managed_env_leaves_preexisting_mode_untouched(self, tmp_path, monkeypatch):
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setenv("CRYOZEN_MANAGED", "nixos")
    ...
    def test_effective_home_marker_does_not_govern_another_homes_scratch(self, tmp_path, monkeypatch):
        ...
        # ``cryozen doctor`` have already resolved theirs, so the marker lookup must not fall back
        # to ``get_cryozen_home()``: for a sticky profile with CRYOZEN_HOME unset that lookup warns
    ...
    def test_canonical_container_signal_without_env_override_keeps_operator_mode(self, tmp_path, monkeypatch):
        # Podman/containerd/K8s runtimes often don't export CRYOZEN_CONTAINER; the canonical
        # _detect_container breadth (not the narrower legacy signal set) must skip the chmod.
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setattr("cryozen_constants._detect_container", lambda: True)
        pre = tmp_path / "cache" / "scratch"
        pre.mkdir(parents=True)
        os.chmod(pre, 0o750)
        scratch = get_scratch_dir(tmp_path, prune=False)
    ...
    def test_container_keeps_operator_mode_but_honors_explicit(self, tmp_path, monkeypatch):
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setenv("CRYOZEN_CONTAINER", "1")
        pre = tmp_path / "cache" / "scratch"
        pre.mkdir(parents=True)
        os.chmod(pre, 0o750)
    ...
    def test_cryozen_uid_gid_applied_to_scratch(self, tmp_path, monkeypatch):
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setenv("CRYOZEN_UID", "1000")
        monkeypatch.setenv("CRYOZEN_GID", "911")
...
def test_config_and_constants_share_one_policy_implementation(tmp_path, monkeypatch):
    """cryozen_constants is the single home of managed / container / CRYOZEN_UID policy: config
    ...
    assert config.get_managed_system is cryozen_constants.get_managed_system
    assert config._chown_to_cryozen_uid is cryozen_constants._chown_to_cryozen_uid
    assert not hasattr(config, "_is_container")

    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / "home"))
    for var in ("CRYOZEN_MANAGED", "CRYOZEN_CONTAINER", "CRYOZEN_SKIP_CHMOD"):
        monkeypatch.delenv(var, raising=False)
    monkeypatch.setattr("cryozen_constants._detect_container", lambda: True)
