...
- ``cryozen verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``cryozen verify --json`` when the workspace
...
def _ledger_on(monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_VERIFY_ON_STOP", "1")
...
def cryozen_home(tmp_path, monkeypatch):
    monkeypatch.setenv("CRYOZEN_HOME", str(tmp_path / ".cryozen-home"))
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        cryozen_dir = project / ".cryozen-agent"
        cryozen_dir.mkdir()
        (cryozen_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(cryozen_home):
    project = _workspace(cryozen_home)
    ...
    assert status["evidence"]["canonical_command"] == "cryozen verify"


def test_record_verify_run_records_failure(cryozen_home):
    project = _workspace(cryozen_home)
...
def test_cli_passing_run_writes_ledger_evidence(cryozen_home, capsys):
    project = _workspace(cryozen_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(cryozen_home, capsys):
    project = _workspace(cryozen_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(cryozen_home, capsys):
...
def test_cli_run_uses_cryozen_session_id_env(cryozen_home, capsys, monkeypatch):
    monkeypatch.setenv("CRYOZEN_SESSION_ID", "sess-42")
    project = _workspace(cryozen_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> cryozen verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(cryozen_home, capsys):
    project = _workspace(cryozen_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `cryozen verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_cryozen_verify_when_recipe_has_start(cryozen_home):
    project = _workspace(cryozen_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "cryozen verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_cryozen_verify_when_manifest_exists(cryozen_home):
    # No start script, but a saved .cryozen-agent/environment.json qualifies.
    project = _workspace(
        cryozen_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(cryozen_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(cryozen_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "cryozen verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(cryozen_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(cryozen_home, capsys):
    project = _workspace(cryozen_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(cryozen_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(cryozen_home, capsys):
    project = _workspace(cryozen_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
