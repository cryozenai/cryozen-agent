...
class TestCreateSession:
    ...
    def test_register_task_cwd_translates_windows_drive_for_wsl_tools(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_platform.host.runtime._wsl_detected", True)
        monkeypatch.setattr(
    ...
    def test_make_agent_prefers_passed_toolsets_over_config_servers(self, monkeypatch):
        ...
        monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", lambda **_kw: {})
        monkeypatch.setattr("cryozen_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
        monkeypatch.setattr("acp_adapter.session._register_task_cwd", lambda task_id, cwd: None)
        manager = SessionManager(db=None)

        manager._make_agent(session_id="fresh", cwd=".")
        manager._make_agent(
            session_id="rebuilt", cwd=".", enabled_toolsets=["cryozen-acp", "mcp-acp-server"], disabled_toolsets=["browser"],
        )

        assert (seen[0]["enabled_toolsets"], seen[0]["disabled_toolsets"]) == (["cryozen-acp", "mcp-cfg-server"], None)
        assert (seen[1]["enabled_toolsets"], seen[1]["disabled_toolsets"]) == (["cryozen-acp", "mcp-acp-server"], ["browser"])
    ...
    def test_make_agent_surfaces_the_provider_resolution_failure(self, monkeypatch):
        ...
        def _no_creds(**_kw):
            raise RuntimeError("No Codex credentials stored. Run `cryozen auth add openai-codex`")

        class BareFails:
            def __init__(self, **kwargs):
                raise RuntimeError("No LLM provider configured. Run `cryozen setup`")

        class BareWorks:
            def __init__(self, **kwargs):
                self.kwargs = kwargs

        monkeypatch.setattr("cryozen_cli.config.load_config", lambda: {"model": {"default": "m", "provider": "openai-codex"}})
        monkeypatch.setattr("cryozen_cli.runtime_provider.resolve_runtime_provider", _no_creds)
        monkeypatch.setattr("cryozen_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
        monkeypatch.setattr("acp_adapter.session._register_task_cwd", lambda task_id, cwd: None)
        manager = SessionManager(db=None)
...
class TestSymlinkAliasNormalization:
    ...
    def test_missing_path_keeps_lexical_normalization(self):
        ...
        assert acp_session._normalize_cwd_for_compare(
            "/nonexistent-cryozen-test/x/../y"
        ) == "/nonexistent-cryozen-test/y"
