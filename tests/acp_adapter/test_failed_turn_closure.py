...
Driven through the real ``CryozenACPAgent.prompt()`` with a real ``AIAgent`` and ``SessionDB``
...
def acp(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CRYOZEN_DISABLE_PLUGINS", "1")
    monkeypatch.setenv("NO_PROXY", "127.0.0.1,localhost")
    provider = _LoopbackProvider()

    import acp_adapter.session as acp_session
    import cryozen_cli.config as cli_config
    import cryozen_cli.mcp_startup as mcp_startup
    import cryozen_cli.runtime_provider as runtime_provider
    ...
    from acp_adapter.server import CryozenACPAgent, TextContentBlock
    from acp_adapter.session import SessionManager
    from cryozen_state import SessionDB

    db_path = tmp_path / "state.db"
    db = SessionDB(db_path)
    server = CryozenACPAgent(session_manager=SessionManager(db=db))
    conn = _RecordingConn()
...
def test_acp_refusal_closes_the_turn_and_is_not_replayed_into_the_next_prompt(acp):
    ...
    Invariant: the durable tail after a failed turn is a Cryozen-authored assistant row (never
