...
COMMON_CRYOZEN_TOOLS = ["read_file", "search_files", "terminal", "patch", "write_file", "process"]


class TestToolKindMap:
    def test_all_cryozen_tools_have_kind(self):
        """Every common cryozen tool should appear in TOOL_KIND_MAP."""
        for tool in COMMON_CRYOZEN_TOOLS:
...
class TestBuildToolTitle:
    ...
    def test_execute_code_title_includes_first_code_line(self):
        ...
        assert "from cryozen_tools import terminal" in title
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("web_search", {"query": "cryozen agent acp"}),
            ("execute_code", {"code": "\nfrom cryozen_tools import terminal\nprint('done')"}),
