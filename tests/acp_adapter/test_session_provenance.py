...
def test_root_session_no_compression(db):
    ...
    assert prov["currentCryozenSessionId"] == "root1"
    assert prov["rootCryozenSessionId"] == "root1"
    assert prov["parentCryozenSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_cryozen_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentCryozenSessionId"] == "old"
    assert prov["rootCryozenSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousCryozenSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootCryozenSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"cryozen"}
    assert "sessionProvenance" in meta["cryozen"]
    assert meta["cryozen"]["sessionProvenance"]["currentCryozenSessionId"] == "root1"
