"""Tests for acp_adapter.server — CryozenACPAgent ACP server."""
...
def agent(mock_manager):
    """CryozenACPAgent backed by a mock session manager."""
    return CryozenACPAgent(session_manager=mock_manager)
...
class TestSessionOps:
    ...
    async def test_new_session_returns_authenticated_cross_provider_model_state(self):
        ...
        acp_agent = CryozenACPAgent(session_manager=manager)
        ...
        with (
            patch("cryozen_cli.inventory.load_picker_context", return_value=picker_context),
            patch("cryozen_cli.inventory.build_models_payload", return_value=payload) as build_payload,
        ):
...
class TestPrompt:
    ...
    async def test_prompt_binds_session_id_into_subprocess_env(self, agent, mock_manager):
        ...
        leaving the ``CRYOZEN_SESSION_ID`` ContextVar bound to the explicit ""
        ...
        empty ``CRYOZEN_SESSION_ID`` instead of the session's own id.
        """
        ...
        def _run(*args, **kwargs):
            ...
            captured["child"] = _make_run_env({}).get("CRYOZEN_SESSION_ID")
    ...
    async def test_prompt_fails_tool_calls_left_open_before_responding(self, agent, mock_manager, executor_raises):
        ...
        with patch.object(CryozenACPAgent, "_run_agent_turn", side_effect=_turn):
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["cryozen-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["cryozen-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["cryozen-acp", "mcp-srv"]
