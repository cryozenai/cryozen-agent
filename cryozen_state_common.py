"""Shared constants and helpers for the SessionDB family of modules.  Lives outside cryozen_state so
...
def routed_sessions_setting(key: str, env_var: str) -> Any:
    ...
    carrier CLI/cron children read). Under a multiplexer a routed turn runs with a CRYOZEN_HOME override
...
# 'new_session' (cryozen_cli/cli_session_mixin.py) without a reset child row.  A compression rotation must
...
# optimized via ``cryozen sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-Cryozen-Session-Key / build_session_key)
...
# and must not serve reads until `cryozen sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `cryozen db optimize`. This is
...
# claims progress under SQLite transaction authority).  Mirrors `cryozen_state_repair._cross_process_repair_lock`:
...
# `<db>.repair.lock` (offline schema surgery, minutes in VACUUM).  Lives here: mixins cannot import cryozen_state.

# ── Cross-process full-FTS-rebuild admission (single authority) ────────────── Several independent Cryozen
# processes routinely share one state.db (gateway service, the Desktop app's `cryozen serve` backend,
...
# authority and is intentionally multi-process. Semantics mirror `cryozen_state_repair._cross_process_repair_lock`
...
# indeterminate liveness still defers. It lives here (not cryozen_state) because the search/schema mixins
# cannot import cryozen_state (cycle). The lock file is `<db>.fts_rebuild.lock`, distinct from
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-Cryozen)"
    age = ""
