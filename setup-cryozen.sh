...
# 5. Symlinks the 'cryozen' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}✻ Cryozen Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "${TMPDIR:-${CRYOZEN_HOME:-$HOME/.cryozen-agent}}/cryozen-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "${TMPDIR:-${CRYOZEN_HOME:-$HOME/.cryozen-agent}}/cryozen-uv-installer.$$.sh")"
...
if is_termux; then
    # Cryozen currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Cryozen requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink cryozen into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up cryozen command..."

CRYOZEN_BIN="$SCRIPT_DIR/venv/bin/cryozen"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CRYOZEN_BIN" "$COMMAND_LINK_DIR/cryozen"
echo -e "${GREEN}✓${NC} Symlinked cryozen → $COMMAND_LINK_DISPLAY_DIR/cryozen"
...
# ============================================================================
# Seed bundled skills into ~/.cryozen-agent/skills/
# ============================================================================

CRYOZEN_SKILLS_DIR="${CRYOZEN_HOME:-$HOME/.cryozen-agent}/skills"
mkdir -p "$CRYOZEN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.cryozen-agent/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CRYOZEN_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  cryozen status        # Check configuration"
if is_termux; then
    echo "  cryozen gateway       # Run gateway in foreground"
else
    echo "  cryozen gateway install # Install gateway service (messaging + cron)"
fi
echo "  cryozen cron list     # View scheduled jobs"
echo "  cryozen doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m cryozen_cli.main setup
fi
