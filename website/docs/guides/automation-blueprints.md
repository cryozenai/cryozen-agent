...
Copy-paste blueprints for common automation patterns. Each blueprint uses Cryozen's built-in [cron scheduler](../user-guide/features/cron.md) for time-based triggers and [webhook platform](../user-guide/messaging/webhooks.md) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`cryozen webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `cryozen webhook subscribe`) |
...
```bash
cryozen cron create "0 2 * * *" \
  "You are a project manager triaging the cryozenai/cryozen-agent GitHub repo.

1. Run: gh issue list --repo cryozenai/cryozen-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
cryozen webhook subscribe github-pr-review \
  --events "pull_request" \
...
cryozen cron create "0 9 * * 1" \
  "Scan the cryozenai/cryozen-agent repo for documentation drift.

1. Run: gh pr list --repo cryozenai/cryozen-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (cryozen_cli/commands.py, cryozen_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (cryozen_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
cryozen cron create "0 6 * * *" \
  "Run a dependency security audit on the cryozen-agent project.

1. cd ~/.cryozen-agent/cryozen-agent && source .venv/bin/activate
...
```bash
cryozen webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
cryozen webhook subscribe alert-triage \
...
```python title="~/.cryozen-agent/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Cryozen-Monitor/1.0"})
...
cryozen cron create "every 30m" \
  ...
  --script ~/.cryozen-agent/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
cryozen cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Cryozen Agent development." \
...
```bash
cryozen webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
cryozen webhook subscribe stripe-payments \
...
```bash
cryozen cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the cryozen-agent codebase.
