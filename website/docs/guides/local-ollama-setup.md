---
sidebar_position: 9
title: "Run Cryozen Locally with Ollama — Zero API Cost"
description: "Step-by-step guide to running Cryozen Agent entirely on your own machine with Ollama and open-weight models like Gemma 4, no cloud API keys or paid subscriptions needed"
---

# Run Cryozen Locally with Ollama — Zero API Cost

:::tip Desktop users: there's a one-click path
On the Cryozen desktop app, **Settings → Providers → Local Models** installs
...
You'll set up Cryozen Agent running entirely on your own hardware, using [Ollama](https://ollama.com) as the model backend. No API keys, no subscriptions, no data leaving your machine. Once configured, Cryozen works exactly like it does with OpenRouter or Anthropic — terminal commands, file editing, web browsing, delegation — but the model runs locally.

By the end, you'll have:

- Ollama serving one or more open-weight models
- Cryozen connected to Ollama as a custom endpoint
...
```bash
# ~/.cryozen-agent/.env
CRYOZEN_API_TIMEOUT=1800   # 30 minutes — generous for slow local models
```
:::
...
Cryozen is an **agentic** assistant — it edits files, runs commands, and browses the web through tool calls. Models without tool-call support can only chat; they can't take actions. For the full Cryozen experience, use a model that supports tools (like `gemma4:31b`).
:::
...
You can pull several models and switch between them inside Cryozen with `/model`. Ollama loads the active model into memory on demand and unloads idle ones automatically.
:::
...
Alternatively, edit `~/.cryozen-agent/config.yaml` directly:
...
Cryozen will use the terminal tool, file operations, and your local model — no cloud calls.
...
By default, Ollama uses a 2048-token context. Cryozen requires at least 64,000 tokens for agentic work with tools:

```bash
# Create a Modelfile that extends context
cat > ~/.cryozen-agent/cache/scratch/Modelfile << 'EOF'
FROM gemma4:31b
PARAMETER num_ctx 64000
EOF

ollama create gemma4-64k -f ~/.cryozen-agent/cache/scratch/Modelfile
```

Then update your Cryozen config to use `gemma4-64k` as the model name.
...
Once Cryozen works locally in the CLI, you can expose it as a Telegram or Discord bot — still running entirely on your hardware.

### Telegram

1. Create a bot via [@BotFather](https://t.me/BotFather) and get the token
2. Add to your `~/.cryozen-agent/config.yaml`:
...
Cryozen sends a fixed payload on every API call — the system prompt plus the tool schemas for all enabled tools — before any of your conversation content. On CPU-only or low-VRAM setups, processing that prompt (the *prefill* phase) dominates the first turn: the model can sit silent for minutes while it works through the prompt, then generate at its normal pace. This is expected behaviour, not a hang. The [Mac local-LLM guide](./local-llm-on-mac.md#timeouts) documents the same effect — during prefill on large contexts, local models may produce no output for minutes while processing the prompt — and Cryozen automatically raises its stream read timeout from 120s to 1800s for local endpoints (`CRYOZEN_STREAM_READ_TIMEOUT`).
...
- **Widen the API timeout** — set `CRYOZEN_API_TIMEOUT=1800` in `~/.cryozen-agent/.env` (see [What You Need](#what-you-need)).
- **Measure and trim the fixed prompt** — run `cryozen prompt-size` for a byte breakdown of the system prompt and tool schemas, then disable unused toolsets with `cryozen tools` and uninstall skills you don't need with `cryozen skills`.
...
- **Terminal commands** — Cryozen wraps the command, runs it, reads output regardless of model
...
- **Long context windows** — cloud models offer 100K–1M tokens; local runtimes often default below Cryozen's 64K minimum unless you configure them
