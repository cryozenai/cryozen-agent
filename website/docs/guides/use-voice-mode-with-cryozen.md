---
sidebar_position: 8
title: "Use Voice Mode with Cryozen"
description: "A practical guide to setting up and using Cryozen voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Cryozen
...
:::tip
Edge TTS is the default text-to-speech backend and needs no API key, so voice output works as soon as you have a model provider configured.
See [Text-to-Speech](../user-guide/features/tts.md) for the other backends.
:::
...
- you want Cryozen sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Cryozen.
...
## Step 1: make sure normal Cryozen works first
...
Cryozen supports both local and cloud speech stacks.
...
### If you use `cryozen setup`

If you choose NeuTTS in the setup wizard, Cryozen checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Cryozen transcribes and responds
...
- asking Cryozen to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Cryozen loop.

## Tuning CLI behavior

### Silence threshold

If Cryozen starts/stops too aggressively, tune:
...
Cryozen stays a normal chat bot, but can speak replies.
...
- you want Cryozen to function like a portable research or ops assistant
...
Cryozen joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Cryozen detects speech boundaries
- transcripts are posted in the associated text channel
- Cryozen responds in text and audio
...
1. get text Cryozen working
2. run `cryozen setup tts` to enable voice support
