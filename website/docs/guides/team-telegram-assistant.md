...
This tutorial walks you through setting up a Telegram bot powered by Cryozen Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Cryozen Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](../getting-started/installation.md) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.cryozen-agent/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Cryozen itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Cryozen Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_cryozen_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Cryozen Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.cryozen-agent/.env`:
...
```
[Gateway] Starting Cryozen Gateway...
...
# View live logs
journalctl --user -u cryozen-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `cryozen gateway install` to update the plist.
:::
...
```bash
cryozen gateway stop && cryozen gateway start
```
...
cryozen pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.cryozen-agent/.env`:
...
Control how much detail the bot shows when using tools. In `~/.cryozen-agent/config.yaml`:
...
Customize how the bot communicates by editing `~/.cryozen-agent/SOUL.md`:

For a full guide, see [Use SOUL.md with Cryozen](./use-soul-with-cryozen.md).
...
```bash
# From the CLI
cryozen cron list          # View all scheduled jobs
cryozen cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u cryozen-gateway` (Linux) or `~/.cryozen-agent/logs/gateway.log` (macOS) |
| Cron job output | `~/.cryozen-agent/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.cryozen-agent/cron/jobs.json` |
| Pairing data | `~/.cryozen-agent/pairing/` |
| Session history | `~/.cryozen-agent/sessions/` |
