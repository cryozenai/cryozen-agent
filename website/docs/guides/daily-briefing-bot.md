...
:::tip
This recipe uses web search, summarization, and optional TTS.
Configure a model provider with `cryozen model` (see [AI Providers](../integrations/providers.md)) and a web search backend with `cryozen tools` (see [Web Search](../user-guide/features/web-search.md)).
:::
...
2. **Cryozen spins up** a fresh agent session with your prompt
...
- **Gateway running** — the gateway daemon handles cron execution:
  ...
  sudo cryozen gateway install --system   # Linux servers: boot-time system service
  # or
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.cryozen-agent/cron/output/` and you can read them anytime.
:::
...
Cryozen will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Cryozen Agent
```
...
Before creating cron jobs, ensure Cryozen has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Cryozen what you want:
...
Cryozen will create the cron job for you using the unified `cronjob_manage` tool.
...
For faster briefings, tell Cryozen to delegate each topic to a sub-agent:
...
Cryozen will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
