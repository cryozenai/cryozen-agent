...
description: "Diagnose and fix common Cryozen cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`cryozen gateway` for foreground, or `cryozen gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `cryozen cron tick`.
...
```bash
date
cryozen cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.cryozen-agent/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.cryozen-agent/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.cryozen-agent/.env` |
...
| `local` | Write access to `~/.cryozen-agent/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `cryozen cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `cryozen skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `cryozen skills list`.
...
2. `~/.cryozen-agent/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `cryozen cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Cryozen config directory). Verify:
```bash
ls ~/.cryozen-agent/scripts/your-script.py   # Must exist
cryozen cron edit <job_id> --script ~/.cryozen-agent/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `cryozen skills install <skill-name>`.
...
Jobs are stored in `~/.cryozen-agent/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.cryozen-agent/cron/jobs.json
chmod 600 ~/.cryozen-agent/cron/jobs.json   # Your user should own it
```
...
```bash
cryozen cron list                    # Show all jobs, states, next_run times
cryozen cron run <job_id>            # Schedule for next tick (for testing)
cryozen cron edit <job_id>           # Fix configuration issues
cryozen logs                         # View recent Cryozen logs
cryozen skills list                  # Verify installed skills
```
...
1. Run the job with `cryozen cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.cryozen-agent/logs/agent.log` for scheduler messages and `~/.cryozen-agent/logs/errors.log` for warnings
3. Open an issue at [github.com/cryozenai/cryozen-agent](https://github.com/cryozenai/cryozen-agent) with:
