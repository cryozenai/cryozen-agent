...
```bash
cryozen teams-pipeline validate
```
...
```bash
cryozen teams-pipeline token-health
cryozen teams-pipeline token-health --force-refresh
```
...
```bash
cryozen teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
cryozen teams-pipeline maintain-subscriptions
cryozen teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Cryozen cron (recommended if you already run the Cryozen gateway)

Cryozen ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.cryozen-agent/scripts/`. First create the script:

```bash
mkdir -p ~/.cryozen-agent/scripts
cat > ~/.cryozen-agent/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec cryozen teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.cryozen-agent/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
cryozen cron create "0 */12 * * *" \
...
```bash
cryozen cron list
cryozen cron status        # scheduler status
```

#### Option 2: systemd timer (recommended for Linux production deployments)

Create `/etc/systemd/system/cryozen-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Cryozen Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=cryozen
EnvironmentFile=/etc/cryozen/env
ExecStart=/usr/local/bin/cryozen teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/cryozen-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Cryozen Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now cryozen-teams-pipeline-maintain.timer
systemctl list-timers cryozen-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/cryozen teams-pipeline maintain-subscriptions >> /var/log/cryozen/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.cryozen-agent/.env` at the top of a wrapper script that crontab calls.
...
```bash
cryozen teams-pipeline subscriptions   # should show expirationDateTime advanced
cryozen teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
cryozen teams-pipeline list
cryozen teams-pipeline list --status failed
cryozen teams-pipeline show <job-id>
```

### Replay a stored job

```bash
cryozen teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
cryozen teams-pipeline fetch --meeting-id <meeting-id>
cryozen teams-pipeline fetch --join-web-url "<join-url>"
cryozen teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `cryozen teams-pipeline maintain-subscriptions --dry-run`
- inspect `cryozen teams-pipeline list --status failed`
...
- run `cryozen teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `cryozen teams-pipeline run`
...
- [ ] `cryozen teams-pipeline validate` returns an OK snapshot
- [ ] `cryozen teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Cryozen cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
