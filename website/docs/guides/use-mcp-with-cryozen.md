---
sidebar_position: 6
title: "Use MCP with Cryozen"
description: "A practical guide to connecting MCP servers to Cryozen Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Cryozen

This guide shows how to actually use MCP with Cryozen Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Cryozen tool
- you want Cryozen to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Cryozen to internal APIs, databases, or company systems without modifying Cryozen core

Do not use MCP when:
- a built-in Cryozen tool already solves the job well
...
- Cryozen remains the agent
- MCP servers contribute tools
- Cryozen discovers those tools at startup or reload time
...
If you installed Cryozen with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Cryozen banner/status should show MCP integration when configured
- ask Cryozen what tools it has available
- use `/reload-mcp` after config changes
- check logs if the server failed to connect
- run `cryozen mcp test <server>` from a shell — it connects, lists the
...
## WSL2: bridge Cryozen in WSL to Windows Chrome

This is the practical setup when:

- Cryozen runs inside WSL2
...
In this setup, Cryozen does **not** connect to Chrome directly. Instead:

- Cryozen runs in WSL
- Cryozen starts a local stdio MCP server
...
```text
Cryozen (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Cryozen stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Cryozen core browser transport
...
```bash
cryozen mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
cryozen mcp test chrome-devtools-win
```

Then start a fresh Cryozen session or run:
...
Once loaded, Cryozen can use the MCP-prefixed browser tools directly. For example:

```text
Call the MCP tool mcp_chrome_devtools_win_list_pages and list the open browser tabs.
```

### When `/browser connect` is the wrong tool

If Cryozen runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Cryozen from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Cryozen from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Cryozen:
...
2. Cryozen-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Cryozen will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Cryozen to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Cryozen to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Cryozen remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
cryozen mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
cryozen mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `cryozen mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Cryozen to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Cryozen to merge, publish, deploy, or spawn runtimes.
...
Now Cryozen can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Cryozen core.
...
Because Cryozen now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
