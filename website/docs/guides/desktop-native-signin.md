...
description: "How the Cryozen Desktop app signs in to a gated gateway using your system browser and PKCE — no embedded webview, no session cookies"
---

# Desktop Native Sign-In (RFC 8252)

When the Cryozen Desktop app connects to a **gated gateway** (a hosted or
...
For Cryozen specifically, native sign-in means:
...
```
Desktop app                Gateway (/auth/native/*)          OIDC provider (IDP)
...
desktop app* and an OAuth client *to the upstream identity provider* (your
OIDC provider). This is required because the upstream `client_id` and permitted
redirect URIs are bound to the gateway's own origin — a desktop app can't be a
direct client of the identity provider. The desktop still gets the full RFC 8252
...
dashboard-auth subsystem. OAuth providers (e.g. the bundled **self-hosted** OIDC provider, which works with Authentik, Keycloak, Zitadel, Auth0, Okta, and other conformant identity providers)
