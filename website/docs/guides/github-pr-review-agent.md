...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Cryozen Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Cryozen instantly when PRs are opened or updated.
:::
...
- **Cryozen Agent installed** — see the [Installation guide](../getting-started/installation.md)
- **Gateway running** for cron jobs:
  ```bash
  cryozen gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.cryozen-agent/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Cryozen can access GitHub. Start a chat:
...
```
Run: gh pr list --repo cryozenai/cryozen-agent --state open --limit 3
```
...
Still in the chat, ask Cryozen to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo cryozenai/cryozen-agent
```
...
A skill gives Cryozen consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.cryozen-agent/skills/code-review
```

Create `~/.cryozen-agent/skills/code-review/SKILL.md`:
...
Verify it loaded — start `cryozen` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Cryozen your team's standards:
...
2. Teach Cryozen your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
cryozen gateway status    # Is the gateway running?
cryozen cron list         # Is the job enabled?
```
