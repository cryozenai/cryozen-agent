...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.cryozen-agent/.env` at the end.
...
4. Fill in:
   - **Name:** `Cryozen Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `cryozen-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the Cryozen app
New-CsApplicationAccessPolicy `
  -Identity "Cryozen-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Cryozen meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Cryozen-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.cryozen-agent/.env`:
...
Cryozen ships a Graph auth smoke-test. From your Cryozen install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.cryozen-agent/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `cryozen gateway restart`.
