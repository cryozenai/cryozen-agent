...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `cryozen send`."
---

# Pipe Script Output to Messaging Platforms

`cryozen send` is a small, scriptable CLI that pushes a message to any
messaging platform Cryozen is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | cryozen send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `cryozen
...
cryozen send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
cryozen send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
cryozen send --list

# Filter by platform
cryozen send --list telegram
```
...
Any platform Cryozen ships adapters for works as a target:
...
`cryozen send` resolves the message body in this order:

1. **Positional argument** — `cryozen send --to telegram "hi"`
2. **`--file PATH`** — `cryozen send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | cryozen send --to telegram`

When stdin is a TTY (no pipe), Cryozen does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  cryozen send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `cryozen send` reuses your Cryozen config, the same script works on
any host where Cryozen is installed — no need to export bot tokens into
...
`cryozen send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  cryozen send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | cryozen send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.cryozen-agent/bin/cryozen send \
...
```bash
./train.py --epochs 200 && \
  cryozen send --to telegram "training done" || \
  cryozen send --to telegram "training failed (exit $?)"
```
...
cryozen send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(cryozen send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `cryozen send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `cryozen send` calls
the platform's REST endpoint directly using credentials from
`~/.cryozen-agent/.env` and `~/.cryozen-agent/config.yaml` (or the equivalent files under
your resolved Cryozen home — `%LOCALAPPDATA%\cryozen` on Windows, or the profile
directory when `CRYOZEN_HOME` / `-p` is set). It's a standalone subprocess
...
`Looked in: C:\Users\me\AppData\Local\cryozen\.env (no DISCORD_BOT_TOKEN),
C:\Users\me\AppData\Local\cryozen\config.yaml (no platforms.discord block),
...
`.env` so `cryozen send` can use it. When your shell is scoped to a profile home
(`CRYOZEN_HOME=<root>/profiles/<name>`) but the connected gateway runs from the
default root, the error says so — the gateway never read the profile's `.env`,
and `cryozen send --list` points at the root's `channel_directory.json`.
...
pointing at the gateway; start it with `cryozen gateway start` and retry.
...
# Machine-readable
cryozen send --list --json
```

The listing is built from `~/.cryozen-agent/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`cryozen gateway
...
| Approach | Multi-platform | Reuses Cryozen creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `cryozen send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`cryozen send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `cryozen send`.
...
- [Gateway Internals](../developer-guide/gateway-internals.md) —
  the delivery router that `cryozen send` shares with cron delivery.
