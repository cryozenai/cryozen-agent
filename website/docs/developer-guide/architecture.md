...
sidebar_position: 1
title: "Architecture"
description: "Cryozen Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Cryozen Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ cryozen_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # CryozenCLI facade — mixins in cryozen_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── cryozen_state.py           # SQLite session/state database facade (+ cryozen_state_*.py siblings)
├── cryozen_constants.py       # CRYOZEN_HOME, profile-aware paths
...
│
├── cryozen_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `cryozen` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # cryozen skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # cryozen tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # cryozen gateway start/stop
│
...
```text
User input → CryozenCLI.process_input()
...
Three discovery sources: `~/.cryozen-agent/plugins/` (user), `.cryozen-agent/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `cryozen plugins` or `config.yaml`.
...
Exposes Cryozen as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`cryozen -p <name>`) gets its own CRYOZEN_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
