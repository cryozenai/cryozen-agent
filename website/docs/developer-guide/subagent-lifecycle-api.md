...
Plugins can launch and supervise fresh Cryozen child sessions without importing
...
active agent turn fails closed with `No active Cryozen parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Cryozen can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Cryozen's existing unsafe-tool block remains enforced.
