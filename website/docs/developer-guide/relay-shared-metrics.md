...
Cryozen includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Cryozen and does not require a Cryozen observability plugin. Cryozen remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Cryozen execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `cryozen-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Cryozen `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `CRYOZEN_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `CRYOZEN_NEMO_RELAY_ATOF_*` and `CRYOZEN_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Cryozen does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Cryozen requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Cryozen
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Cryozen installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Cryozen process so configured interceptors can
...
Collection remains off unless Cryozen policy enables it:
...
opt-in. Set `CRYOZEN_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Cryozen does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Cryozen reports the error and does not invoke
...
retain the same `session_id` and add `cryozen.session.segment` plus
`cryozen.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Cryozen
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Cryozen process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Cryozen logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Cryozen
process and share the same policy boundary.

Run profiles in separate Cryozen processes when they require different trust
...
Cryozen core owns one Relay host and one isolated Relay session scope per Cryozen
...
marks do not require Cryozen plugin registration. Shared-metrics marks must
...
```text
Cryozen turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Cryozen shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Cryozen sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Cryozen used for the logical call, such as
`anthropic/claude-sonnet-4.6` through `openrouter`. These identifiers are
...
New calls use `cryozen.model_route.count`. The previous
`cryozen.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `cryozen.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Cryozen
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Cryozen processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `cryozen.task_run`, parented to
the owning Cryozen session. The start counter contains only bounded execution
...
provider attempts for the same Cryozen API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Cryozen tool-call
...
cancellations. Active task ownership follows the task ID if Cryozen rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`cryozen.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Cryozen
...
tool or plugin names. Cryozen does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `cryozen.tool_approval` marks and
...
Successful skill mutations emit `cryozen.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `cryozen.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Cryozen derives reuse and patch-generation
...
```text
$CRYOZEN_HOME/telemetry/shared_metrics/metrics.sqlite3
$CRYOZEN_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Cryozen version,
...
schema field name, its current scope is one `CRYOZEN_HOME`, so it is more
...
Deleting `$CRYOZEN_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `CRYOZEN_HOME`. To reset it, stop Cryozen
processes and remove `$CRYOZEN_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Cryozen CLI turn against the deterministic local model server:
...
`$CRYOZEN_HOME/telemetry/shared_metrics/outbox/` to the configured telemetry
ingest endpoint.
The ingest contract validates only the envelope (`schema_version` plus a UUID
`package_id`) and stores the body verbatim.
...
telemetry:
  shared_metrics:
    ...
    send: false      # NEW: transmit to the configured ingest endpoint
```
...
Removing `$CRYOZEN_HOME/telemetry/shared_metrics` still resets local identity,
...
- **Remote:** retention of sent packages is set by the operator of the ingest
  endpoint.
...
| Delete already-sent data | Not self-service; requires the operator of the ingest endpoint |
