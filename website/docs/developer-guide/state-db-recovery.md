...
description: "How Cryozen recovers state.db when the FTS index or the file itself is corrupt"
---
...
`cryozen doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `cryozen doctor --fix` while the gateway
is still up. Next steps:

```bash
cryozen gateway stop
CRYOZEN_HOME="$HOME/.cryozen-agent" cryozen sessions recover --source "$HOME/.cryozen-agent/state.db" --inspect-only
# if recoverable:
CRYOZEN_HOME="$HOME/.cryozen-agent" cryozen sessions recover --source "$HOME/.cryozen-agent/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
cryozen gateway stop
CRYOZEN_HOME="$HOME/.cryozen-agent" cryozen sessions repair --check-only
CRYOZEN_HOME="$HOME/.cryozen-agent" cryozen sessions repair
```
...
```bash
CRYOZEN_HOME="$HOME/.cryozen-agent" cryozen sessions repair --check-only
sqlite3 "$HOME/.cryozen-agent/state.db" \
