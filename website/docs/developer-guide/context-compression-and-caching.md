...
Cryozen Agent uses a dual compression system and Anthropic prompt caching to
...
- **Failed probes use the current table without persisting it.** Failures have a
  five-minute in-memory cooldown scoped to Cryozen home, endpoint, model, and
  region. Expiry or explicit cache invalidation permits another attempt.

The cache remains at `context_length_cache.yaml` under the active Cryozen home.
...
Configure via `cryozen plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](./context-engine-plugin.md).

## Dual Compression System

Cryozen has two separate compression layers that operate independently:
...
This is **not an exact-count-only policy**, nor a guarantee that token counts
are never estimated. The following policies remain unchanged:
...
value is kept per `model@host` in `~/.cryozen-agent/cache/image_token_costs.json` and
...
capped request every 30 seconds. JSON-decode, closed-stream and
...
- **Repeated stall → deterministic fallback.** A first stall keeps the
  ...
  re-entering the same silent stream every turn.
...
- **Provider-proven overflow** — when the provider itself rejects the request
  ...
  the next failure would extend the ladder. If that attempt fails,
...
compression:
  ...
  codex_app_server_auto: native  # native|cryozen|off for Codex app-server thread compaction
...
auxiliary:
  compression:
    ...
    provider: auto           # Provider: "auto", "openrouter", "anthropic", "main", etc.
...
| `codex_app_server_auto` | `native` | `native`, `cryozen`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
- Keys may be **provider-scoped** as `"<provider>:<substring>"` (e.g.
  ...
  elsewhere (OpenRouter, direct OpenAI) keeps the global `threshold`.
...
excluded because they already unlock the wider window), Cryozen raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$CRYOZEN_HOME`
...
```bash
cryozen config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
cryozen config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Cryozen keeps the
...
`gpt-5.6-terra-900k`, `gpt-5.6-luna-900k`, `gpt-5.4-900k`). These are Cryozen-side aliases: the suffix is stripped before
...
thread context, so Cryozen's auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Cryozen
  records the resulting compaction events (compression counters, session
  events). Set `cryozen` to let Cryozen's compression threshold initiate
  app-server compaction, or `off` to disable Cryozen-initiated automatic
  compaction entirely (codex may still compact natively).

Cryozen's local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Cryozen's summary compressor.
...
encrypted `compaction` output item. Cryozen captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Cryozen
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Cryozen can avoid; user-facing docs for `/model`, fallback providers, and
