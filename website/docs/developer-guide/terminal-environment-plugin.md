...
Cryozen runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.cryozen-agent/plugins/`, registering a backend the user selects
...
| `cryozen setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `cryozen status` / `cryozen doctor` | `doctor_checks()` |
...
```python title="~/.cryozen-agent/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.cryozen-agent"    # where synced cache files land, or None
...
```yaml title="~/.cryozen-agent/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
cryozen plugins enable acmebox
cryozen config set terminal.backend acmebox
```
