...
| Variable | Meaning |
|----------|---------|
| `CRYOZEN_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `CRYOZEN_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `CRYOZEN_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Cryozen itself — they're private to these helpers. The variables Cryozen *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `cryozen --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_cryozen_root)" || { echo "htui: not in a Cryozen checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$CRYOZEN_MAIN_CHECKOUT/.venv/bin/python" -m cryozen_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `CRYOZEN_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_cryozen_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `CRYOZEN_TUI_DIR` are mutually exclusive
`CRYOZEN_TUI_DIR` points Cryozen at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `cryozen --tui --dev` exits with an error. Run `unset CRYOZEN_TUI_DIR` before `htui`.
:::
...
hgui() (
  ...
  root="$(_cryozen_root)" || { print -u2 'hgui: not in a Cryozen checkout'; return 1; }
  deps="${CRYOZEN_GUI_DEPS_CHECKOUT:-$CRYOZEN_MAIN_CHECKOUT}"
  desktop="$root/apps/desktop"

  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _cryozen_link_deps "$desktop" "$deps/apps/desktop" || return 1
    _cryozen_link_deps "$root" "$deps" || return 1
  else
  ...
  export CRYOZEN_DESKTOP_CRYOZEN_ROOT="$root"
  export CRYOZEN_DESKTOP_PYTHON="$CRYOZEN_MAIN_CHECKOUT/.venv/bin/python"
  export CRYOZEN_DESKTOP_CWD="$root"
  export CRYOZEN_DESKTOP_DEV_SERVER="http://127.0.0.1:$vite_port"
  export CRYOZEN_DESKTOP_CDP_PORT="$cdp_port"
  export CRYOZEN_DESKTOP_USER_DATA_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/cryozen-hgui/slot-$slot"
  # A userData override would otherwise also relocate the agent's home.
  export CRYOZEN_HOME="${CRYOZEN_HOME:-$HOME/.cryozen-agent}"
...
For example, after setting `CRYOZEN_MAIN_CHECKOUT` and sourcing the helpers:

```bash
# Terminal 1: main checkout
cd "$CRYOZEN_MAIN_CHECKOUT"
HGUI_SLOT=0 hgui

# Terminal 2: an existing worktree
cd /path/to/cryozen-worktree
HGUI_SLOT=1 hgui
```
...
|----------|----------------|
| `HGUI_SLOT` | Helper-only slot number, `0`–`9`; not a Cryozen setting. |
| `CRYOZEN_DESKTOP_CRYOZEN_ROOT` | Runs the backend from this worktree, not the packaged/PATH runtime. |
| `CRYOZEN_DESKTOP_PYTHON` | Reuses the main checkout's Python environment. Adjust for an installation that uses `venv` rather than `.venv`. |
| `CRYOZEN_DESKTOP_CWD` | Roots new desktop work in the worktree. |
| `CRYOZEN_DESKTOP_DEV_SERVER` | Points Electron at this instance's Vite server. |
| `CRYOZEN_DESKTOP_CDP_PORT` | Gives each instance its own renderer debugging port. |
| `CRYOZEN_DESKTOP_USER_DATA_DIR` | Separates Electron's single-instance lock, browser storage, and desktop preferences. |
| `CRYOZEN_HOME` | Explicitly preserves the agent home despite the Electron user-data override. |
...
The default `CRYOZEN_HOME` is shared: sessions, configuration, credentials, and profiles remain the same. Avoid editing the same conversation from both instances. For destructive tests or incompatible database migrations, pass a separate temporary `CRYOZEN_HOME` and configure that sandbox independently.
:::

Quit the app normally or press Ctrl-C in its launching terminal. `concurrently -k` manages its own child commands, and Electron owns its backend shutdown. Do not add a global `killport`, `pkill electron`, or a sweep of all `serve`/`dashboard --port 0` processes: those can terminate another instance. Remove the old `_cryozen_gui_cleanup` trap if replacing an earlier version of this helper.
...
```bash
# The enclosing worktree, verified as a real Cryozen checkout.
_cryozen_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/cryozen_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
- [TUI](../user-guide/tui.md) — `cryozen --tui --dev` and the `CRYOZEN_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/cryozenai/cryozen-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `CRYOZEN_*` variable Cryozen reads
