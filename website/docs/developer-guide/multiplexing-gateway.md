...
by `cryozen_cli/gateway_multiplex_mode.py::resolve_multiplex_mode`, which runs
the `cryozen gateway migrate` preflight and keeps the gateway standalone when a
...
- At startup, `GatewayRunner.__init__` calls
  ...
  `get_secret()`.
- The dashboard/Desktop backend (`cryozen serve`) has no such flag, so
  `cryozen_cli/web_server.py::start_server` calls
...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_cryozen_home_override(home)          config / state.db / skills /
...
default profile's scope so global gateway settings resolve
consistently.
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`CRYOZEN_HOME`, `CRYOZEN_PROFILE`, proxy settings,
...
reload helper: `cryozen_cli.env_loader.load_cryozen_dotenv` skips the
...
snapshot. The unscoped startup load is unchanged.
...
`config.yaml` resolve through `get_secret` when a scope is installed, and `.env` writes made under a scope (`save_env_value`, e.g. a
`/pair` grant mirror) update the installed scope mapping instead of the
process environment.

## The CRYOZEN_HOME override

`cryozen_constants.py` holds a context-local override consulted by
`get_cryozen_home()` before the `CRYOZEN_HOME` env var. Everything that resolves
...
automatically. `get_process_cryozen_home()` exists for the few machine-level
assets that must not follow the override. `cryozen_home_key()` gives
per-home registries a stable scope key. A one-shot warning fires if
...
`profiles_to_serve(multiplex, profile_allowlist)` in `cryozen_cli/profiles.py`
...
cron ticking, `/p/<profile>/` HTTP admission, route eligibility,
...
`SessionStore` binds no database handle at construction. Session
DB handles are resolved at call time through the active CRYOZEN_HOME override —
...
CRYOZEN_HOME override. Asset writes are atomic, type- and size-capped.
...
- Fail-closed: unscoped `get_secret()` under multiplexing raises; a routed
  ...
  enters the default profile's scope rather than an undefined one.
...
| --- | --- |
| MCP discovery and tool registration | Process-global; the first profile to build an agent wins the discovery slot. Full per-profile MCP registries are future work. |
| Terminal / sandbox env (`TERMINAL_*`) | Global by allowlist; tools read it from the process environment. |
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `cryozen_home_key()`. |
...
- `agent/secret_scope.py`, `cryozen_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `cryozen_cli/profiles.py`
