...
Cryozen gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Cryozen's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `cryozen.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `cryozen.platform.up/degraded` with bounded `error_code` attributes |
...
`cryozen.gateway.active_agents`, `cryozen.gateway.background_work`, and
`cryozen.gateway.background_delegations` are complementary. `active_agents`
...
```bash
cryozen monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'cryozen-agent[otlp]'`),
...
a plain `cryozen gateway run`: the exporter lives in the gateway process, so
...
`cryozen.gateway.up`, `cryozen.platform.up`, and `cryozen.platform.degraded`.
...
absent_over_time(cryozen_gateway_up[5m])

# Locally owned bridge is explicitly down.
cryozen_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
cryozen_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
cryozen_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
cryozen_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(cryozen_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `cryozen.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
cryozen.status = failed|unknown
cryozen.delivery_outcome = failed|not_configured
cryozen.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `cryozen.job_key`;
...
Cryozen Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log ~/.cryozen-agent/cache/scratch/cryozen_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log ~/.cryozen-agent/cache/scratch/cryozen_otel_capture.jsonl --wait 8
...
`cryozen.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
cryozen monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
  --log ~/.cryozen-agent/cache/scratch/cap.jsonl --wait 8          # drive the real exporter
```
...
The `cryozen monitoring` CLI intentionally exposes `status` only. This first
release covers only Cryozen Agent-owned service-health and operational-diagnostic
signals, including Cryozen Agent-owned Relay transport health. The relay
connector's authoritative shared connector/platform state is explicitly out of scope, as
