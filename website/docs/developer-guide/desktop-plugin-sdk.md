...
title: "Desktop Plugin SDK (@cryozen/plugin-sdk)"
description: "Extend the native Cryozen Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Cryozen Desktop](../user-guide/desktop.md) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `CryozenPlugin`.
It imports one module — `@cryozen/plugin-sdk` — and gets everything: the app's
...
`$CRYOZEN_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Cryozen. This page is the **native
desktop app** (`cryozen desktop`) SDK — the `@cryozen/plugin-sdk` module and
`$CRYOZEN_HOME/desktop-plugins/`. The **web dashboard** (`cryozen dashboard`) has
its own, unrelated plugin system on `window.__CRYOZEN_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](../user-guide/features/extending-the-dashboard.md). Python
CLI/gateway plugins are documented at [Build a Cryozen Plugin](./plugins/index.md).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$CRYOZEN_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$CRYOZEN_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `CryozenPlugin` contract, appear in **Capabilities → Plugins**,
...
toggle and contributes nothing while disabled.

## Quick start — your first plugin

Create `$CRYOZEN_HOME/desktop-plugins/hello/plugin.js` (that's `~/.cryozen-agent/...`
...
```javascript
// ~/.cryozen-agent/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@cryozen/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Cryozen's }),
      jsx('div', {
...
The only importable specifiers are `@cryozen/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `CryozenPlugin`:

```ts
interface CryozenPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `cryozen.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@cryozen/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@cryozen/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@cryozen/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@cryozen/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@cryozen/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@cryozen/plugin-sdk'
...
`setAccentOverride(hex)` and clear it in `ctx.onDispose`.
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@cryozen/plugin-sdk'
...
`window.cryozen.send('get-price eth')` (or a declarative
`<button data-cryozen-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, WORKSPACE_PAGE_HEADER_AREA } from '@cryozen/plugin-sdk'
...
for requests; `targetProfile` is the backend Cryozen profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Cryozen
...
ctx.os.notify({
  ...
  // Body click → focus Cryozen + navigate. Same vocabulary as OS deep links:
  activate: 'cryozen://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'cryozen://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `cryozen://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `cryozen://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@cryozen/plugin-sdk'
...
```javascript
import { queryClient } from '@cryozen/plugin-sdk'
...
copies that half into `$CRYOZEN_HOME/desktop-plugins/<id>/` beside a
`.cryozen-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.cryozen-agent/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`cryozen plugins update`, or
...
a package into `~/.cryozen-agent/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.cryozen-agent/plugins` is not reachable as a filesystem —
...
the `cryozen://` scheme — a plain anchor on your website or README:
...
`cryozen-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Cryozen plugin and declare it in a
`manifest.json`:

```
~/.cryozen-agent/plugins/<id>/
└── dashboard/
    ├── manifest.json      # { "name": "<id>", "api": "plugin_api.py" }
...
cryozen-agent codebase directly (`cryozen_state`, `cryozen_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.cryozen-agent/`)
...
(`cryozen.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `CryozenPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@cryozen/plugin-sdk` alias.
2. It's still lint-fenced to `@cryozen/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
`ctx.rest`, storage, `navigate`) and the `window.cryozenDesktop` native bridge
...
the loader's import allowlist (`@cryozen/plugin-sdk` and `react*` only; a static
...
- **Only three specifiers resolve:** `@cryozen/plugin-sdk`, `react`,
...
| Plugin contract | `CryozenPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `CryozenOpenTarget`, `Contribution` |
...
### Agents: the `cryozen-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`cryozen-desktop-plugins`** skill — it carries the same contract as this page in
...
`$CRYOZEN_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `cryozen logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@cryozen/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.cryozen-agent/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.cryozen-agent/logs/errors.log` for
