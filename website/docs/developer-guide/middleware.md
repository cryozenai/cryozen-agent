...
Cryozen middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Cryozen's
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Cryozen calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Cryozen retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Cryozen guardrails,
...
- `telemetry_schema_version`: currently `cryozen.observer.v1`
- `middleware_schema_version`: currently `cryozen.middleware.v1`
...
Cryozen stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Cryozen runs
them as a nested chain in registration order. Middleware failures are fail-open:
Cryozen logs a warning and continues with the next middleware or the base
...
For each provider request, Cryozen applies middleware in this order:
...
For each tool call, Cryozen applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Cryozen pre-execution path against the effective arguments:
...
For isolated local testing, use one `CRYOZEN_HOME` for plugin enablement and the
agent run:

```bash
export CRYOZEN_HOME=$HOME/.cryozen-agent/cache/scratch/cryozen-middleware-test
mkdir -p "$CRYOZEN_HOME"
cryozen plugins enable <plugin-name>
cryozen chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run cryozen plugins enable <plugin-name>
uv run cryozen chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["cryozen_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", str(Path.home() / ".cryozen-agent" / "cache" / "scratch" / "cryozen-middleware-demo"))
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Cryozen expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Cryozen treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Cryozen preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Cryozen does not convert downstream
