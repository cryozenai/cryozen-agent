...
description: "Wire contract between the Cryozen gateway relay adapter and external connectors (experimental)"
---
...
This document is the formal interface between the **Cryozen gateway** (Python,
`gateway/relay/`) and the **connector** (a separate Node/TypeScript service).
The connector implementer's first action is to
read this file.
...
connector stamps it when it resolves the target profile for a multi-profile
gateway interaction; omitting it (single-profile gateways) preserves legacy routing to
the default `agent:main` session namespace, mirroring the `profile` field the
`inbound` frame's `SessionSource` already carries. The body is
...
1. The owner triggers a link from the `cryozen` CLI. The
   ...
   the instance's own per-gateway secret, and **never** from the request body).
...
> drain, the actual machine suspend, and any suspended-health model. The
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `cryozen gateway enroll --wake-url <url>` (or
  `GATEWAY_RELAY_WAKE_URL` / `gateway.relay_wake_url`). Provisioned: stamped into
...
- **The poke.** When a buffered-only (going-idle) destination receives its FIRST
  ...
  instance's registered `wakeUrl`, **directly** (not through any control plane).
  It carries no tenant data and no inbound: it only says "you
...
1. **Register a `wakeUrl` before the instance can ever be suspended.** A
   ...
   wake target is registered (self-hosted: `--wake-url`; provisioned: stamped) as a
...
the inbound (`scope_id`, `user_id`) and, on a multiplexed gateway, the Cryozen
...
Additive within `contract_version` 1, but note the deploy order: an older
connector that ignores `content` would *set* "is typing…" on a clear frame, so
deploy the connector first.
...
**per-gateway secret** (`cryozen gateway enroll` → connector `/relay/enroll`, or
...
**management plane** the agent (or an operator tool) uses to declare who-sees-what
...
full design and invariants live in the connector's own documentation; this
section is the gateway-facing summary.
...
auth** as the WS upgrade: the gateway's own per-gateway secret bearer (§6.1
