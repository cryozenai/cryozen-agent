...
sidebar_position: 15
title: "CLI Internals"
description: "How cryozen_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `cryozen_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `cryozen_cli/AGENTS.md`; user-facing behaviour
...
The bug class behind many fleet-update failures:
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `cryozen_cli.update_cmd._cryozen_holder_subcommand` (top-level subcommand of any Cryozen argv). Never
...
- Never blanket-exclude ancestors from process scans: when `/update` runs as the gateway's child, a
  gateway ancestor must stay visible to the pause machinery. Exclude interactive ancestry,
  carve out gateway-shaped ancestors.
- Match on FULL cmdlines; truncate only at display time.
- Before adding any new scan heuristic, remember that the gateway control socket replaces scans as
...
Built-in skins (`_BUILTIN_SKINS` in `cryozen_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.cryozen-agent/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

Cryozen supports profiles — fully isolated instances, each with its own `CRYOZEN_HOME` (config, API
keys, memory, sessions, skills, gateway). For single-profile commands (`cryozen -p x <cmd>`),
`_apply_profile_override()` in `cryozen_cli/main.py` sets `CRYOZEN_HOME` before any module imports, so
every `get_cryozen_home()` reference scopes to the active profile. The multiplex gateway and the
Desktop/dashboard `serve` backend serve several profiles from one process instead: the active
profile is a contextvar override bound per activity, `os.environ["CRYOZEN_HOME"]` stays the launch
...
`Path.home() / ".cryozen-agent" / "profiles"`, not `get_cryozen_home() / "profiles"`) so
`cryozen -p coder profile list` sees all profiles regardless of which one is active — intentional.
