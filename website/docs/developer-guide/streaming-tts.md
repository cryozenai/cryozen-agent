...
Cryozen can stream TTS audio as it arrives from the provider, instead of waiting
...
| openai      | chunked HTTP (`with_streaming_response`, `pcm`) | yes | `tts.openai.api_key` → env |
