...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Cryozen Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Cryozen tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Cryozen core, use the plugin route instead:

- [Plugins](../user-guide/features/plugins.md)
- [Build a Cryozen Plugin](./plugins/index.md)
...
2. **`toolsets.py`** — add tool name to `_CRYOZEN_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `cryozen_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `cryozen_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `cryozen chat -q "Use the weather tool for London"`
