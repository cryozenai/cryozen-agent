...
The ACP adapter wraps Cryozen's synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
cryozen acp / cryozen-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.cryozen-agent/.env
  -> configure stderr logging
  -> construct CryozenACPAgent
...
- `allow_once` -> Cryozen `once`
- `allow_always` -> Cryozen `always`
- reject options -> Cryozen `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Cryozen tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["cryozen-acp"])
...
retries, interrupt before any reply) is closed by the core loop with a Cryozen-authored
...
Instead it reuses Cryozen's runtime resolver:

- `acp_adapter/auth.py`
- `cryozen_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Cryozen provider/credentials. It also always advertises a terminal setup auth method (`cryozen-setup`, args `--setup`) so first-run ACP clients can open Cryozen's interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.cryozen-agent/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `cryozen-acp` toolset definition
- `cryozen_cli/main.py` — `cryozen acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `cryozen-acp` script
