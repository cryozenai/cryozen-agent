...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Cryozen Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Cryozen Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Cryozen core? Start with [Build a Cryozen Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Cryozen itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Cryozen instance (throwaway CRYOZEN_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m cryozen_cli.main
scripts/dev-sandbox.sh --persistent python -m cryozen_cli.main desktop  # state survives restarts, but lives in the worktree :)
```

### Manual clone fallback

Use this only if you intentionally do not want Cryozen's managed install layout
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.cryozen-agent/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.cryozen-agent` — use `get_cryozen_home()` from `cryozen_constants` for code paths and `display_cryozen_home()` for user-facing messages. See [AGENTS.md](https://github.com/cryozenai/cryozen-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Cryozen can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Cryozen to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
- **Questions and design proposals**: [GitHub Discussions](https://github.com/cryozenai/cryozen-agent/discussions)
- **Skills Hub**: publish specialized skills for other users

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/cryozenai/cryozen-agent/blob/main/LICENSE).
