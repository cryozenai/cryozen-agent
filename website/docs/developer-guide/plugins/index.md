...
title: "Build a Cryozen Plugin"
description: "Step-by-step guide to building a complete Cryozen plugin with tools, hooks, data files, and skills"
---

# Build a Cryozen Plugin

This guide walks through building a complete Cryozen plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Cryozen has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](../../user-guide/features/tts.md#voice-message-transcription-stt) — set `CRYOZEN_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](../../user-guide/features/hooks.md#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.cryozen-agent/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](../../user-guide/features/skills.md) — `cryozen skills tap add <repo>` · [Publishing a tap](../../user-guide/features/skills.md#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `cryozenai/cryozen-agent`. Users install them into `~/.cryozen-agent/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in Cryozen GitHub Discussions. See [CONTRIBUTING.md](https://github.com/cryozenai/cryozen-agent/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

Cryozen can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Cryozen already owns. It does not replace native `plugin.yaml` plus
...
```bash
cryozen plugins install owner/repository --no-enable
cryozen plugins list
cryozen plugins enable <plugin-name>
```
...
Cryozen validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Cryozen.
...
Portable `streamable-http` entries are routed through Cryozen's existing native
...
same full-trust posture as other installed Cryozen plugins.
...
records it as Published. Cryozen keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Cryozen does not expose a
...
continue to work after a normal Cryozen upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Cryozen inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Cryozen releases can therefore load a plugin whose manifest contains
...
Cryozen enforces this contract with frozen external-plugin fixtures discovered
from an isolated `CRYOZEN_HOME`. Those tests load and invoke the plugin through
...
Cryozen's internals were split into `<stem>_<topic>` sibling modules in Sep 2026. **Internal
...
- **Check your plugin:** `cryozen plugins compat /path/to/your/plugin` lists every `file:line` with the
  old path and the new one, and exits 1 while any remain. `COMPAT_MANIFEST.md` in the repo is the full map.
- **What users see:** a notice under the CLI banner, in `cryozen doctor` and after `cryozen update`, and a
  one-time Desktop dialog naming the plugin. Each resolution through an old path also emits a
  `CryozenPluginCompatWarning` once per process.
- **From 2026-09-14:** plugins that still import old paths are **not loaded** (the reason shows in
  `cryozen plugins list`). Users can force-load with `plugins.allow_deprecated_imports: true` until the
...
```bash
mkdir -p ~/.cryozen-agent/plugins/calculator
cd ~/.cryozen-agent/plugins/calculator
```

### Validate with Plugin Doctor

`cryozen plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Cryozen itself. It reports invalid hook names, callbacks that do
...
```bash
cryozen plugins doctor . --ci
```

Doctor uses a temporary `CRYOZEN_HOME`, restores plugin registration state after
...
This tells Cryozen: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `cryozen_cli/plugin_capabilities.py` for the
...
`cryozen_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`cryozen_agent.plugins` entry point:

```toml
[project.entry-points."cryozen_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."cryozen_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Cryozen reads these from installed metadata without importing your code, so
`cryozen plugins capabilities` and the consent flow stay accurate for pip
installs.

### Manifest v2 reference

`plugin.yaml` also supports an additive **v2 schema**. Every field is
...
this Cryozen understands still loads with a warning.
...
| `python_dependencies` | list of str | PEP 508 requirements (e.g. `"requests>=2.0,<3"`). Installed into Cryozen's venv on `cryozen plugins install` / `enable` and **re-applied after every `cryozen update`** (see [Python dependencies](#python-dependencies)). A `pyproject.toml` beside `plugin.yaml` with `[project].dependencies` is the equivalent, preferred form. |
| `python_runtime` | str | `external` — the plugin manages its own interpreter/venv (sidecar pattern); Cryozen installs nothing and leaves any `pyproject.toml` alone. |
...
requires_plugins:
  - id: other-plugin
    version_range: ">=1.0,<2"
python_dependencies:
  - "somepkg>=1.0,<2"     # installed on install/enable, re-applied after cryozen update
config_schema:
...
When both exist the `pyproject.toml` wins. What Cryozen does with them:

- **Install / enable** — the declared packages are installed into Cryozen's venv with
  `uv pip install` (pip fallback) under a **constraints file built from Cryozen's own pinned
  dependencies**, so a plugin can never move a core package (httpx, pydantic, …) off the version
  Cryozen was tested with. Environment markers (`; sys_platform == "win32"`) are honoured.
...
- **`cryozen update` re-applies them** — the update's `uv sync` rebuilds the venv from Cryozen's lock
  and strips anything else. Afterwards Cryozen walks every profile's enabled plugins and reinstalls
  ...
  message** naming it, and memory providers are kept over everything else, because a Cryozen that
  boots without memory looks like data loss.
- **`cryozen plugins update`** re-runs the install for whatever the new revision declares.
- **`--no-deps`** on `cryozen plugins install` skips all of this for one plugin (no conflict gate,
...
- **Opt out with `python_runtime: external`** — plugins that keep a heavy runtime (torch, native
  ...
  `plugin.yaml`; Cryozen then installs nothing and the plugin never joins the shared resolution.
- **Nothing to load is an error** — `cryozen plugins validate` (and the catalog CI) fail a
...
`CRYOZEN_HOME/plugins/` survives `cryozen update` and Desktop updates: the updater only rebuilds the
...
4. **Accept `**kwargs`:** Cryozen injects context keywords (`task_id`, `session_id`, `user_task`,
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `cryozen my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Cryozen continues fine
...
Use plugin-relative config keys for user-visible behavior. Cryozen resolves them
...
`<CRYOZEN_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `CRYOZEN_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
CRYOZEN_PLUGINS_DEBUG=1 cryozen plugins list
```
...
The same logs are always written to `~/.cryozen-agent/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
cryozen logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `cryozen plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.cryozen-agent/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
```
~/.cryozen-agent/plugins/calculator/
...
tree, and `cryozen plugins update` / `remove` git-pull or delete it — your
...
# <cryozen home>/plugin-data/<name>/ — created on first use
...
```
~/.cryozen-agent/plugins/my-plugin/
├── __init__.py
├── plugin.yaml
└── skills/
    ├── my-workflow/
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.cryozen-agent/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.cryozen-agent/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `cryozen plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Cryozen into installing arbitrary packages — only specs Cryozen itself ships are eligible |
...
auditable in `~/.cryozen-agent/logs/agent.log`. Plugins load after built-in
...
does not ship with Cryozen core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `cryozen plugins list`
...
loader, the plugin is disabled and Cryozen continues. The gate exists
...
You normally never edit this key by hand. `cryozen plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Cryozen injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: cryozen-agent"}
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$CRYOZEN_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $CRYOZEN_HOME/hook_outputs
```
...
- **The system prompt is Cryozen's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `cryozen_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Cryozen sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`cryozen.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Cryozen still loads on an older one.

### Register CLI commands

Plugins can add their own `cryozen <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for cryozen my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: cryozen my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for cryozen my-plugin."""
...
After registration, users can run `cryozen my-plugin status`, `cryozen my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `cryozen name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `cryozen chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `CRYOZEN_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `cryozen <subcommand>` (e.g. `cryozen kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "cryozen kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Cryozen from a hook.
...
A plugin can load while the gateway (or the TUI/Desktop server) is already running: `cryozen plugins
...
(`cryozen_cli/plugins_activation.py`):
...
Install surfaces report exactly this split: `cryozen plugins install/enable` prints it after nudging the running
...
Cryozen has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.cryozen-agent/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `cryozen gateway status` without SDK instantiation.
...
Cryozen also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](../../user-guide/features/plugins.md#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Cryozen without any Python plugin. Declare them in `~/.cryozen-agent/config.yaml`:
...
Cryozen connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](../../user-guide/features/mcp.md).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.cryozen-agent/hooks/<name>/`. Unlike plugins there is no `plugins.enabled` step: the gateway imports every valid hook directory at startup, so placing the files **is** the opt-in ([trust model](../../user-guide/features/hooks.md#gateway-hook-trust)):

```yaml
# ~/.cryozen-agent/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.cryozen-agent/hooks/long-task-alert/handler.py
...
cryozen skills search my-workflow --source myorg/skills-repo
cryozen skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install cryozen-plugin-calculator
# Plugin auto-discovered on next cryozen startup
```
