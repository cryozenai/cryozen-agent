...
# Cryozen Observer Hooks

Cryozen observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Cryozen's planner, model providers, memory, tool registry,
...
Cryozen also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "cryozen.observer.v1"
```

Hook callbacks are fail-open. Cryozen catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Cryozen only builds
...
The native NeMo Relay SDK integration maps Cryozen session, turn, LLM, and tool
