...
Cryozen Agent uses a SQLite database (`~/.cryozen-agent/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `cryozen_state.py` (facade) plus the `cryozen_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)

## Cryozen home and profile isolation

`get_cryozen_home()` is the authoritative filesystem resolver for state and
configuration. It uses a context-local override first, then the `CRYOZEN_HOME`
environment variable, and finally the platform default (`~/.cryozen-agent` on macOS
and Linux; `%LOCALAPPDATA%/cryozen` on Windows). Consequently, the default
database is always `get_cryozen_home() / "state.db"`, not a path that callers
should hard-code as `~/.cryozen-agent/state.db`.

Named profiles are isolated directories: a profile named `coder`, for example,
uses `<default Cryozen root>/profiles/coder/` and therefore has its own
...
profile must retain or pass that profile's `CRYOZEN_HOME`; falling back to the
...
of Cryozen. An explicit `--profile`/`-p` resolves that profile and writes the
resolved directory to `CRYOZEN_HOME`. Without an explicit selector, a
profile-specific `CRYOZEN_HOME` is preserved; otherwise the bootstrap can use
the default root's active-profile selection. `HOME` only determines the
platform default used when no context override or `CRYOZEN_HOME` is available.
Changing `HOME` is not a safe way to select a named profile. In particular, a
subprocess that drops `CRYOZEN_HOME` can fall back to the default profile even
when another profile is active, so subprocess spawners should pass
`CRYOZEN_HOME` explicitly.

Use `display_cryozen_home()` only for user-facing text. It formats the resolved
home relative to the user's home directory when possible (for example,
`~/.cryozen-agent/profiles/coder`); it does not provide a separate resolution rule.

### Test isolation guard

Tests must use a temporary `CRYOZEN_HOME` or an explicit temporary database
path. The live-system guard raises before a test-context process opens a
production `state.db` under the real default Cryozen root or a real named
profile, preventing fixture data or SQLite side effects from reaching a live
installation.

`CRYOZEN_STATE_DB_GUARD_BYPASS=1` is a test-only escape hatch for a spawned
...
either bypass in normal Cryozen commands, development shells, or application
...
Each named profile stores its transcript in its own `$CRYOZEN_HOME/state.db`,
including when one `cryozen serve` process serves several profiles. In-session
...
splice boundary, Cryozen excludes only that leading item when it exactly matches
...
```
~/.cryozen-agent/state.db (SQLite, WAL mode)
...
`cryozen sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `cryozen_state_common.py` (applied by `cryozen_state_schema.py`) for the full current column list
...
an authenticated `cryozen serve` (OAuth or the basic username/password provider)
...
- `reasoning_details` is always kept in history; on the chat-completions wire it is replayed only to OpenRouter (every other chat-completions route gets a copy without it, since strict schemas reject the field)
...
indexed columns — see `SCHEMA_SQL` in `cryozen_state_common.py` for the exact SQL.
...
Multiple cryozen processes (gateway + CLI sessions + worktree agents) share one
...
`session_persistence_failed:locked` and, on Linux, `cryozen_state_lockowners`
logs a WARNING naming the process that held the lock at that moment
(`PID 594094 (cryozen --worktree --yolo) holds WAL write lock on state.db-shm`),
...
because every Cryozen process has the DB open. Look for that line in
`~/.cryozen-agent/logs/errors.log` next to the `database is locked` failure.
...
db = SessionDB()                           # Default: ~/.cryozen-agent/state.db
db = SessionDB(db_path=Path("~/.cryozen-agent/cache/scratch/test.db").expanduser())  # Custom path
```
...
Default path: `get_cryozen_home() / "state.db"` — `~/.cryozen-agent/state.db` for the
default profile, `~/.cryozen-agent/profiles/<name>/state.db` for a named profile, or
wherever `CRYOZEN_HOME` points (see [Cryozen home and profile isolation](#cryozen-home-and-profile-isolation)).
