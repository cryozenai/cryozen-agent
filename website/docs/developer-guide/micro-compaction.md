...
has to be thrown away or summarized. Cryozen has always done this in one batch:
...
Cryozen folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Cryozen's normal session flush is
...
With `micro_compact` unset or `false` Cryozen behaves exactly as it always has:
...
Defaults to `$CRYOZEN_HOME/logs/agent.log`. It reports passes, outcome mix, net
