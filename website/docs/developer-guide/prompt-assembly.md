...
description: "How Cryozen builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Cryozen deliberately separates:
...
2. **context** — caller-supplied `system_message`, project context files (`.cryozen.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`), then the worktree-dependent git workspace snapshot, operator instructions and platform hints
...
the first host-info paragraph after the rendered `# Cryozen runtime environment` boundary, with a
...
the per-turn user-message channel (`agent/surface_switch.py`), so the cached prefix survives. Legacy prompts retain their original
...
```
# Layer 1: Agent Identity (from ~/.cryozen-agent/SOUL.md)
You are Cryozen, an AI assistant created by Cryozen.
...
The platform hint (Layer 10 above) is the per-surface guidance Cryozen
...
`SOUL.md` lives at `~/.cryozen-agent/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:

```python
# From agent/prompt_builder.py (simplified)
def load_soul_md() -> Optional[str]:
    soul_path = get_cryozen_home() / "SOUL.md"
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_cryozen_md(cwd_path)       # 1. .cryozen.md / CRYOZEN.md (walks to git root)
    ...
    # SOUL.md from CRYOZEN_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.cryozen.md`, `CRYOZEN.md` | CWD up to git root | Cryozen-native project config |
| 2 | `AGENTS.md` | CWD only | Common agent instruction file |
| 3 | `CLAUDE.md` | CWD only | Claude Code compatibility |
| 4 | `.cursorrules`, `.cursor/rules/*.mdc` | CWD only | Cursor compatibility |

All context files are:
- **Security scanned** — checked for prompt injection patterns (invisible unicode, "ignore previous instructions", credential exfiltration attempts). A hit replaces a project file with a `[BLOCKED: …]` marker; the user's own `SOUL.md` in `CRYOZEN_HOME` is warned about and loaded anyway (it is human-approved on write, so it is the same trust class as `config.yaml`)
...
- **YAML frontmatter stripped** — `.cryozen.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Cryozen concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.cryozen.md` / `CRYOZEN.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Cryozen already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.cryozen-agent/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.cryozen-agent/MEMORY.md` and `~/.cryozen-agent/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.cryozen.md`, `CRYOZEN.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Cryozen.
- Ephemeral overlays such as `CRYOZEN_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Cryozen assembles prompts for everyone, change Python and treat it as a code contribution
