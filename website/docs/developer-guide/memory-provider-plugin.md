...
description: "How to build a memory provider plugin for Cryozen Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Cryozen Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](./context-engine-plugin.md), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `cryozen plugins`.
:::

## Installation Layouts

Cryozen discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Cryozen. Closed to new providers — see [CONTRIBUTING](https://github.com/cryozenai/cryozen-agent/blob/main/CONTRIBUTING.md). |
| User | `$CRYOZEN_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.cryozen-agent/plugins/<name>/` | Opt-in via `CRYOZEN_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `cryozen_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
Cryozen, in `$CRYOZEN_HOME/plugins/<name>/` when installed by a user, or in
`./.cryozen-agent/plugins/<name>/` for a project-local one:
...
`cryozen_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."cryozen_agent.memory_providers"]
...
package layout. No copy under `$CRYOZEN_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Cryozen reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `cryozen <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          cryozen_home (str): Active CRYOZEN_HOME path. Use for storage.
        """
...
| Keyword | Meaning |
|---|---|
| `cryozen_home` | Active profile's storage directory. |
...
| `session_title_source` | Stored title provenance, when available: `derived`, `llm`, or `user`. Automatic sources must not be mistaken for explicit identity overrides; missing provenance retains a provider's legacy behavior. Shared constants live in `cryozen_state_common.py`. |
...
| `agent_workspace`, `agent_context` | Runtime agent scope. `agent_workspace` is `cryozen`; `agent_context` is `cron` for scheduler runs, `subagent` for `delegate_task` children, else `primary` — skip automatic writes for the non-primary values. |
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `cryozen memory setup` | **Yes** |
| `save_config(values, cryozen_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
None of the providers bundled with Cryozen advertise checkpoint API v2 — the
...
The gate binds to every compaction authority, not just the Cryozen
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Cryozen
...
Every setup surface Cryozen gives a bundled provider is driven by files in the provider's
...
| `cryozen memory setup` wizard | `get_config_schema()` declares the fields the wizard prompts for, `save_config(config, cryozen_home)` persists them, `post_setup(cryozen_home, config)` runs afterwards for anything interactive (OAuth, first sync); `get_status_config()` feeds `cryozen memory status` |
| `cryozen <provider> …` subcommands | `cli.py` with `register_cli(subparser)` ([Adding CLI Commands](#adding-cli-commands)) |
| Python dependencies | `pyproject.toml` `[project] dependencies` (or `python_dependencies` in `plugin.yaml`); installed under Cryozen's own pins at install time and re-applied across `cryozen update` |

Your provider's name, `memory.<name>` config section, data directory and tool names are the
contract with existing users. A provider that moves out of core keeps all four; Cryozen then
installs the catalog plugin automatically for anyone whose `memory.provider` still names it
(on `cryozen update`, and once at agent start when `security.allow_lazy_installs` is on).

## Config Schema

`get_config_schema()` returns a list of field descriptors used by `cryozen memory setup`:
...
Every field in `get_config_schema()` is prompted during `cryozen memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$CRYOZEN_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, cryozen_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(cryozen_home) / "my-provider.json"
...
**`sync_turn()` MUST be non-blocking.** If your backend has latency (API calls, LLM processing), run the work in a daemon thread — spawned with `agent.memory_provider.spawn_context_thread`, never a bare `threading.Thread`. Profile isolation (the active `CRYOZEN_HOME`, the per-turn secret scope) lives in `contextvars`, and a plain thread starts with an empty context: under multiplexed profiles it would silently write into the *default* profile's store, and `get_secret()` fails closed there.
...
The same applies to prefetch and writer threads. Small JSON config sidecars (`$CRYOZEN_HOME/<provider>.json`) are read with `utils.read_json_or_empty` and written with `utils.atomic_json_write`; anything under `config.yaml` goes through `cryozen_cli.config.save_config(..., merge_existing=True)`.
...
the `messages` parameter; Cryozen will continue calling them with the legacy
signature.
...
All storage paths **must** use the `cryozen_home` kwarg from `initialize()`, not hardcoded `~/.cryozen-agent`:

```python
# CORRECT — profile-scoped
from cryozen_constants import get_cryozen_home
data_dir = get_cryozen_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.cryozen-agent/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `cryozen my-provider status`, `cryozen my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `cryozen <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `cryozen --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: cryozen my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the cryozen my-provider argparse tree.
