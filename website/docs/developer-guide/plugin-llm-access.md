...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Cryozen already has applies. The plugin never sees a
...
For the async surface, `acomplete()` and `acomplete_structured()` take the same arguments and can be fanned out with `asyncio.gather()`.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Cryozen provider id (e.g. "openrouter")
...
using Cryozen built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Cryozen provider
        # (must be one Cryozen already knows about — same names as
        # `cryozen model` and config.yaml model.provider).
        ...
        # plugin sends — Cryozen does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.cryozen-agent/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Cryozen's usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Cryozen subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/cryozenai/cryozen-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/cryozenai/cryozen-agent/blob/main/tests/agent/test_plugin_llm.py)
