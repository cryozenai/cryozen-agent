...
description: "Authoritative reference for Cryozen terminal commands and command families"
---
...
```bash
cryozen [global-options] <command> [subcommand/options]
```
...
| `--profile <name>`, `-p <name>` | Select which Cryozen profile to use for this invocation. Overrides the sticky default set by `cryozen profile use`. |
...
| Command | Purpose |
|---------|---------|
| `cryozen chat` | Interactive or one-shot chat with the agent. |
| `cryozen model` | Interactively choose the default provider and model. |
| `cryozen moa` | Configure named Mixture of Agents presets selectable from the model picker. |
| `cryozen fallback` | Manage fallback providers tried when the primary model errors. |
| `cryozen gateway` | Run or manage the messaging gateway service. |
| `cryozen proxy` | Local OpenAI-compatible proxy that attaches OAuth provider credentials. See [Subscription Proxy](../user-guide/features/subscription-proxy.md). |
| `cryozen egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
| `cryozen lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
| `cryozen setup` | Interactive setup wizard for all or part of the configuration. |
...
| `cryozen secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.cryozen-agent/.env`. |
| `cryozen migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
| `cryozen codex-runtime` | Noninteractive counterpart of `/codex-runtime`: `migrate [--dry-run] [--json]` regenerates the Cryozen-managed block in `~/.codex/config.toml` for the selected profile. See [Codex app-server runtime](../user-guide/features/codex-app-server-runtime.md#running-the-migration-from-a-script). |
| `cryozen status` | Show agent, auth, and platform status. |
| `cryozen usage` | Show the configured account's rate-limit windows (the `/usage` block) without a session; `--json` for scripts. |
| `cryozen cron` | Inspect and tick the cron scheduler. |
| `cryozen pause` / `cryozen resume` | Global emergency stop: no new cron fires (built-in ticker, cron provider triggers, misfire catch-up), kanban dispatch or gateway turns start until resumed; in-flight work is never killed. |
| `cryozen kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `cryozen project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
| `cryozen webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `cryozen hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
| `cryozen doctor` | Diagnose config and dependency issues. |
| `cryozen security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
| `cryozen approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
| `cryozen dump` | Copy-pasteable setup summary for support/debugging. |
| `cryozen prompt-size` | Show a byte breakdown of the system prompt + tool schemas (skills index, memory, profile). Runs offline. |
| `cryozen debug` | Debug tools — upload logs and system info for support. |
| `cryozen backup` | Back up Cryozen home directory to a zip file. |
| `cryozen checkpoints` | Inspect / prune / clear `~/.cryozen-agent/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `cryozen import` | Restore a Cryozen backup from a zip file. |
| `cryozen logs` | View, tail, and filter agent/gateway/error log files. |
| `cryozen config` | Show, edit, migrate, and query configuration files. |
| `cryozen skin` | List, switch, and tweak display skins. |
| `cryozen console` | Open the safe Cryozen command console. |
| `cryozen pairing` | Approve or revoke messaging pairing codes. |
| `cryozen skills` | Browse, install, publish, audit, and configure skills. |
| `cryozen bundles` | Group several skills under a single `/<name>` slash command. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
| `cryozen curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `cryozen journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
| `cryozen memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `cryozen honcho`) register automatically when their provider is active. |
| `cryozen acp` | Run Cryozen as an ACP server for editor integration. |
| `cryozen mcp` | Manage MCP server configurations and run Cryozen as an MCP server. |
| `cryozen plugins` | Manage Cryozen Agent plugins (install, enable, disable, remove). |
| `cryozen tools` | Configure enabled tools per platform. |
| `cryozen computer-use` | Install or check the Computer Use (cua-driver) backend (macOS/Windows/Linux). |
| `cryozen pets` | Browse, install, and select [petdex](../user-guide/features/pets.md) animated pets shown across the CLI, TUI, and desktop app. Subcommands: `list`, `install`, `select`, `show`, `off`, `scale`, `remove`, `doctor`. |
| `cryozen sessions` | Browse, export, prune, rename, and delete sessions. |
| `cryozen insights` | Show token/cost/activity analytics. |
| `cryozen claw` | OpenClaw migration helpers. |
| `cryozen import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `cryozen dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `cryozen serve` | Start the Cryozen backend server (headless; powers the desktop app and remote backends). |
| `cryozen desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `cryozen profile` | Manage profiles — multiple isolated Cryozen instances. |
| `cryozen completion` | Print shell completion scripts (bash/zsh/fish). |
| `cryozen --version` | Show version information. |
| `cryozen update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `CRYOZEN_HOME` snapshot. |
| `cryozen uninstall` | Remove Cryozen from the system. |

## `cryozen chat`

```bash
cryozen chat [options]
```
...
| `--ignore-user-config` | Ignore `~/.cryozen-agent/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, persistent memory, and preloaded skills. Combine with `--ignore-user-config` for a fully isolated run. |
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Cryozen itself. |
...
```bash
cryozen
cryozen chat -q "Summarize the latest PRs"          # seeds an interactive session
cryozen chat --oneshot -q "Summarize the latest PRs"  # answer and exit
cryozen chat --provider openrouter --model anthropic/claude-sonnet-4.6
cryozen chat --toolsets web,terminal,skills
cryozen chat --quiet -q "Return only JSON"
cryozen chat -q "Inspect this repository" --format stream-json
cryozen chat --worktree -q "Review this repo and open a PR"
cryozen chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
cryozen chat --safe-mode -q "Is this bug mine or Cryozen's?"
```
...
```bash
cryozen chat -q "Summarize this repository" --format stream-json
```
...
(`CRYOZEN_KANBAN_TASK` set) whose turn failed only because the provider was
...
### `cryozen -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `cryozen -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
cryozen -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(cryozen -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.cryozen-agent/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `CRYOZEN_INFERENCE_MODEL` | Override the model for this run |
...
```bash
cryozen -z "…" --provider openrouter --model openai/gpt-5.5
# or:
CRYOZEN_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 cryozen -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `cryozen chat --oneshot -q` instead; `-z` is explicitly for "I only want the final answer".
...
`cryozen -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, the `completed` / `failed` / `partial` / `interrupted` flags and `turn_exit_reason` (why `completed` is false, e.g. `max_iterations_reached(3/3)`). Those top-level counters cover the **main agent loop** only. Auxiliary LLM calls made on the same run (title generation, vision, context compression, `web_extract`, background review, …) are reported separately under `auxiliary` — the same totals plus a per-task `by_task` map — and `total_including_auxiliary` (`estimated_cost_usd`, `total_tokens`, `api_calls`) is the grand total to bill on. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
cryozen -z "summarize this repo" --usage-file ~/.cryozen-agent/cache/scratch/usage.json
jq .total_including_auxiliary.estimated_cost_usd ~/.cryozen-agent/cache/scratch/usage.json
jq .auxiliary.by_task ~/.cryozen-agent/cache/scratch/usage.json      # what did title generation / vision cost?
```

## `cryozen model`

Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Cryozen chat session.
...
:::warning cryozen model vs /model — know the difference
**`cryozen model`** (run from your terminal, outside any Cryozen session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Cryozen chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Cryozen session first (`Ctrl+C` or `/quit`), then run `cryozen model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `cryozen model` from the terminal.
:::
...
| `migrate` | Fold per-profile standalone gateways onto the one host gateway (`--multiplex`, the only mode — `cryozen update` runs it automatically unless a real boundary blocks it). Re-running it converges a half-migrated host; a manifest on disk is the resume record, never a rollback (there is no `--standalone`). Runs a preflight (duplicate bot tokens, secondary port-binders without a `/p/<profile>/` ingress) and changes nothing when blocked. Flags: `--dry-run`, `-y`/`--yes`. See [Migrating from per-profile gateways](../user-guide/multi-profile-gateways.md#migrating-from-per-profile-gateways). |
| `migrate-legacy` | Remove legacy `cryozen.service` units left over from pre-rename installs. Profile units (`cryozen-gateway-<profile>.service`) and unrelated services are never touched. Flags: `--dry-run`, `-y`/`--yes`. |
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Cryozen Relay](../user-guide/messaging/relay.md). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `CRYOZEN_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `cryozen update`. |
| `--no-supervise` | On `run`: inside the s6-overlay Docker image, opt out of auto-supervision and use pre-s6 foreground semantics — gateway runs as the container's main process with no auto-restart. No-op outside the s6 image. Equivalent to setting `CRYOZEN_GATEWAY_NO_SUPERVISE=1`. |
...
`cryozen gateway restart`, or service-restart update exits with status `75`
...
`cryozen gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `cryozen gateway run` instead of `cryozen gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s cryozen 'cryozen gateway run'`. See [WSL FAQ](./faq.md#wsl-gateway-keeps-disconnecting-or-cryozen-gateway-start-fails) for details.
:::

## `cryozen lsp`

```bash
cryozen lsp <subcommand>
```
...
```bash
cryozen setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure]
```
...
| `--reconfigure` | Backwards-compat alias — bare `cryozen setup` on an existing install now does this by default. |
...
```bash
cryozen slack manifest              # print manifest to stdout
cryozen slack manifest --write      # write to ~/.cryozen-agent/slack-manifest.json
cryozen slack manifest --long-description-file AGENTS.md --write
cryozen slack manifest --slashes-only  # just the features.slash_commands array
```
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$CRYOZEN_HOME/slack-manifest.json`. |
| `--name NAME` | `Cryozen` | Bot display name in Slack. |
...
Run `cryozen slack manifest --write` again after `cryozen update` to pick
up any new commands.
...
```bash
cryozen send --to <target> "message text"
cryozen send --to <target> --file <path>
echo "message" | cryozen send --to <target>
cryozen send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.cryozen-agent/.env` + `~/.cryozen-agent/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `cryozen send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `cryozen send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
```bash
cryozen send --to telegram "MEDIA:~/.cryozen-agent/cache/scratch/screenshot.png"
cryozen send --to telegram "Build chart for today MEDIA:~/.cryozen-agent/cache/scratch/chart.png"   # with caption
cryozen send --to discord:#ops "MEDIA:~/.cryozen-agent/cache/scratch/report.pdf"
```
...
```bash
cryozen send --to telegram "[[as_document]] MEDIA:~/.cryozen-agent/cache/scratch/screenshot.png"
```

Examples:

```bash
cryozen send --to telegram "deploy finished"
echo "RAM 92%" | cryozen send --to telegram:-1001234567890
cryozen send --to discord:#ops --file ~/.cryozen-agent/cache/scratch/report.md
cryozen send --to slack:#eng --subject "[CI]" --file build.log
cryozen send --list                  # all platforms
cryozen send --list telegram         # filter by platform
```
...
```bash
cryozen peer add <name> --url http://host:port --key <API_SERVER_KEY>
cryozen peer list
cryozen peer dm <peer>[/<agent>] "message"
cryozen peer run <peer>[/<agent>] --idempotency-key <key> "message"
cryozen peer status <peer>[/<agent>] <run_id>
cryozen peer stop <peer>[/<agent>] <run_id>
cryozen peer remove <name>
```

Bot-to-bot DMs across machines. Register another Cryozen gateway (any machine
running the `api_server` platform) as a *peer*, then message its agents:
`cryozen peer dm` resolves the remote agent's canonical **Bot Chat** session
over the peer's API server, runs one agent turn there, and prints the reply
on stdout — the cross-machine twin of the local
`cryozen -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
| Subcommand | Description |
|--------|-------------|
| `add <name> --url <URL> [--key <KEY>] [--note TEXT]` | Register or update a peer. The URL goes to `config.yaml` (`bot_peers`); the key is stored as `CRYOZEN_PEER_<NAME>_KEY` in `~/.cryozen-agent/.env`. |
...
includes the peer roster and the `cryozen peer dm` pattern, so agents discover
...
```bash
cryozen secrets bitwarden <subcommand>
cryozen secrets bw <subcommand>          # short alias
```

Pull API keys from an external secret manager at process startup instead of storing them in `~/.cryozen-agent/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `cryozen claw migrate` (one-shot import of OpenClaw configuration into Cryozen) — `cryozen migrate` is the top-level config-rewrite command.


## `cryozen codex-runtime`

```bash
cryozen codex-runtime migrate [--dry-run] [--json]
```

Runs the `~/.codex/config.toml` migration that `/codex-runtime codex_app_server` triggers, without a chat session: Cryozen's `mcp_servers` (plus installed codex plugins and the `default_permissions` default) are projected into the managed block for the selected profile (`cryozen -p <name> codex-runtime migrate`). User text outside the block is kept verbatim; a user-owned `[mcp_servers.<name>]` with the same name as a Cryozen server is preserved and the Cryozen projection for that name skipped (reported as `preserved_user_servers`). The result is validated as TOML before an atomic write; exit code is 1 when the report contains errors.
...
```bash
cryozen security <subcommand>
```

On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Cryozen venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.cryozen-agent/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Cryozen Python venv. |
| `--skip-plugins` | off | Skip scanning plugin requirements files. |
| `--skip-mcp` | off | Skip scanning pinned MCP servers in `config.yaml`. |


## `cryozen login` / `cryozen logout` *(Deprecated)*

:::caution
`cryozen login` has been removed. Use `cryozen auth` to manage OAuth credentials, `cryozen model` to select a provider, or `cryozen setup` for full interactive setup.
:::
...
```bash
cryozen auth                                              # Interactive wizard
cryozen auth list                                         # Show all pools
...
cryozen auth priority openrouter backup-key 0             # Move a credential to the front of fill_first order
cryozen auth reset openrouter                             # Clear cooldowns
cryozen auth reset openrouter 2                           # Clear the cooldown on one credential
cryozen auth refresh openai-codex work                    # Refresh one OAuth credential and clear its cooldown
cryozen auth status anthropic                             # Show auth status for a provider
cryozen auth logout anthropic                             # Log out and clear stored auth state
cryozen auth spotify                                      # Authenticate Cryozen with Spotify via PKCE
```
...
```bash
cryozen usage                          # configured model provider, human-readable block
cryozen usage --provider openai-codex  # a specific provider
cryozen usage --json                   # one JSON document on stdout
```
...
```bash
cryozen cron <list|create|edit|pause|resume|run|remove|status|runs|incidents|doctor|tick>
```
...
(the default) uses the built-in in-process ticker. To use another trigger, name
a custom provider under `plugins/cron/<name>/` or `$CRYOZEN_HOME/plugins/<name>/`. An unknown or unavailable provider falls back to
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.cryozen-agent/kanban/current`). |
...
cryozen kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
cryozen kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
cryozen kanban boards switch atm10-server
cryozen kanban list                  # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
cryozen kanban boards rm atm10-server
cryozen kanban boards rm atm10-server --delete
```

Board resolution order (highest precedence first): `--board <slug>` flag → `CRYOZEN_KANBAN_BOARD` env var → `~/.cryozen-agent/kanban/current` file → `default`.
...
```bash
cryozen egress install                  # download the pinned iron-proxy binary
cryozen egress install --force          # re-download even if already installed

cryozen egress setup                    # interactive wizard: CA, mappings, config
cryozen egress setup --tunnel-port N    # override the tunnel listener port (default 9090)
cryozen egress setup --from-bitwarden   # use Bitwarden Secrets Manager as credential source
cryozen egress setup --no-bitwarden     # explicitly switch back to env-based credentials
cryozen egress setup --rotate-tokens    # mint fresh proxy tokens (default preserves existing)

cryozen egress start                    # spawn the managed proxy daemon
cryozen egress stop                     # SIGTERM (then SIGKILL after 5s grace)
cryozen egress restart                  # stop (if running) then start — needed for secret changes
cryozen egress reload                   # hot-reload the ruleset in-place (no restart, no dropped
                                       #   connections) via the loopback management API

cryozen egress status                   # binary + config + pid + listening + mappings
cryozen egress status --show-tokens     # print proxy tokens in full (default: redacted)

cryozen egress disable                  # flip proxy.enabled = false (does not stop a running proxy)
cryozen egress config                   # print the path to proxy.yaml for inspection
```
...
cryozen egress setup && cryozen egress start
cryozen config set terminal.backend docker   # if not already

# Switching credential source after the fact
cryozen egress setup --from-bitwarden       # env → bitwarden
cryozen egress setup --no-bitwarden         # bitwarden → env
# (just `setup` without either flag preserves the existing mode)

# Rotating all tokens (e.g. after a suspected token leak)
cryozen egress setup --rotate-tokens    # setup offers to restart the running daemon for you
# (running sandboxes still hold old tokens; restart them too)

# Adding a new upstream
# Edit ~/.cryozen-agent/config.yaml proxy.extra_allowed_hosts: [api.example.com]
cryozen egress setup
cryozen egress restart                  # one-command apply (stop + start)
```

### Diagnostic shortcuts

```bash
cryozen egress status                     # current state in one view
cat ~/.cryozen-agent/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.cryozen-agent/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.cryozen-agent/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
```bash
cryozen project <create|list|show|add-folder|remove-folder|rename|set-primary|use|archive|restore|bind-board>
```
...
```bash
cryozen webhook <subscribe|list|remove|test>
```
...
### `cryozen webhook subscribe`

```bash
cryozen webhook subscribe <name> [options]
```
...
| `--script` | Filter/transform script under `~/.cryozen-agent/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |
...
Subscriptions persist to `~/.cryozen-agent/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart. Re-running `subscribe` for an existing name keeps its secret and profile binding unless you pass `--secret` / `--route-profile`.
...
Exit status: `0` when the report lists no unresolved problems, `1` when at least one remains (including problems `--fix` could not repair), so a health gate or CI step can trust `cryozen doctor` as a check.
...
| Section | Details |
|---------|---------|
| **Header** | Cryozen version, release date, git commit hash |
| **Environment** | OS, Python version, OpenAI SDK version |
| **Identity** | Active profile name, CRYOZEN_HOME path |
...
- Asking for help in GitHub Discussions — share it in a code block
- Comparing your setup to someone else's
- Quick sanity check when something isn't working

:::tip
`cryozen dump` is specifically designed for sharing. For interactive diagnostics, use `cryozen doctor`. For a visual overview, use `cryozen status`.
:::

## `cryozen debug`

```bash
cryozen debug share [options]
```
...
The report includes system info (OS, Python version, Cryozen version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.

Uploads use public paste services tried in order: paste.rs, dpaste.com.

### Examples

```bash
cryozen debug share              # Upload debug report, print URL
cryozen debug share --lines 500  # Include more log lines
cryozen debug share --expire 30  # Keep paste for 30 days
cryozen debug share --local      # Print report to terminal (no upload)
```

## `cryozen backup`

```bash
cryozen backup [options]
```

Create a zip archive of your Cryozen configuration, skills, sessions, and data. The backup excludes the cryozen-agent codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.

| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/cryozen-backup-<timestamp>.zip`). |
| `-q`, `--quick` | Quick snapshot: only critical state files (config.yaml, state.db, .env, auth, cron jobs). Much faster than a full backup. |
| `-l`, `--label <name>` | Label for the snapshot (only used with `--quick`). |
| `-k`, `--keep <N>` | After a full backup, delete older `cryozen-backup-*.zip` files in the output directory beyond the newest N (default 3; `0` keeps everything). Custom-named zips are never touched. |

The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Cryozen is running (WAL-mode safe).
...
- `models/`, `runtimes/`, `node/` at the root of `~/.cryozen-agent` (and of each `profiles/<name>/`) — regenerable runtime downloads, often tens of GB. Deeper directories with the same names (a skill's `models/`) are kept.
- Browser profiles: `browser-profile/` (the real-profile snapshot — copied Cookies / Login Data), `browser-profiles/` (live CDP profiles) at any depth, and `browser_profiles/` (the Browser Use CLI backend's Chromium user-data dir, with its own Login Data / Cookies) at the root of `~/.cryozen-agent` and of each `profiles/<name>/`. Credential stores that must never enter an archive; all are regenerated on the next launch.
...
- The `cryozen-agent` code itself (this is a user-data backup, not a repo snapshot).

### Examples

```bash
cryozen backup                           # Full backup to ~/cryozen-backup-*.zip
cryozen backup -o ~/backups/cryozen.zip   # Full backup to specific path
cryozen backup --quick                   # Quick state-only snapshot
cryozen backup --quick --label "pre-upgrade"  # Quick snapshot with label
```

## `cryozen checkpoints`

```bash
cryozen checkpoints [COMMAND]
```

Inspect and manage the shadow git store at `~/.cryozen-agent/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `cryozen checkpoints` is equivalent. |
...
```bash
cryozen checkpoints                                  # status overview
cryozen checkpoints prune --retention-days 3         # aggressive cleanup
cryozen checkpoints prune --max-size-mb 200          # tighten size cap once
cryozen checkpoints clear-legacy -f                  # drop v1 archive dirs
cryozen checkpoints clear -f                         # wipe everything
```
...
```bash
cryozen import <zipfile> [options]
```

Restore a previously created Cryozen backup into your Cryozen home directory. All files in the archive overwrite existing files in your Cryozen home; `--force` only skips the confirmation prompt that fires when the target already has a Cryozen installation.
...
```
  ⚠ Session data replaced by older backup contents:
    ...
    Recover from a newer backup or snapshot: cryozen snapshot list
```

### Examples
```bash
cryozen import ~/cryozen-backup-20260423.zip           # Prompts before overwriting existing config
cryozen import ~/cryozen-backup-20260423.zip --force   # Overwrite without prompting
```

## `cryozen logs`

```bash
cryozen logs [log_name] [options]
```

View, tail, and filter Cryozen log files. All logs are stored in `~/.cryozen-agent/logs/` (or `<profile>/logs/` for non-default profiles).
...
cryozen logs gateway -n 100

# Show only warnings and errors from the last hour
cryozen logs --level WARNING --since 1h

# Filter by a specific session
cryozen logs --session abc123

# Follow errors.log, starting from 30 minutes ago
cryozen logs errors --since 30m -f
...
cryozen logs --level WARNING --since 2h --session tg-12345
```
...
Cryozen uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `cryozen logs list` subcommand shows all log files including rotated ones.


## `cryozen prompt-size`

```bash
cryozen prompt-size [--platform <name>] [--json]
```
...
- **Prompt tiers** — stable / context / volatile, matching how Cryozen layers
...
cryozen prompt-size --platform telegram

# Machine-readable output for scripts
cryozen prompt-size --json
```

:::tip
The skills index and tool schemas scale with how many skills and tools you have
enabled. To shrink the prompt, disable unused toolsets (`cryozen tools`) or
uninstall skills you don't need (`cryozen skills`). Context files (AGENTS.md,
...
`cryozen config set/get/unset` use `.` as the nesting separator, but many real
...
- **Existing keys just work.** When navigating an existing mapping, an
  ...
  splitting. `cryozen config set providers.p.models.grok-4.6.supports_vision true`
...
- **Creating a new dotted key requires escaping.** Escape literal dots with a
  backslash: `cryozen config set 'providers.p.models.grok-4\.7.context_length' 128000`
...
```bash
cryozen pairing <list|approve|revoke|clear-pending>
```
...
```bash
cryozen skills browse
cryozen skills browse --source official
cryozen skills search react --source skills-sh
cryozen skills search https://mintlify.com/docs --source well-known
cryozen skills inspect official/security/1password
cryozen skills inspect skills-sh/vercel-labs/json-render/json-render-react
cryozen skills install official/migration/openclaw-migration
cryozen skills install skills-sh/anthropics/skills/pdf --force
cryozen skills install https://sharethis.chat/SKILL.md                     # Direct URL (+ referenced support files)
cryozen skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
cryozen skills check
cryozen skills update
cryozen skills config
cryozen skills reset google-workspace
cryozen skills reset google-workspace --restore --yes
cryozen skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
cryozen skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
cryozen skills opt-in --sync            # undo: remove marker and re-seed now
```
...
- `--source well-known` lets you point Cryozen at a site exposing `/.well-known/skills/index.json`.
...
```bash
cryozen bundles <subcommand>
```

Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.cryozen-agent/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.cryozen-agent/skill-bundles/` and report added/removed bundles |

Examples:

```bash
cryozen bundles create backend-dev \
  --skill github-code-review \
  --skill test-driven-development \
  --skill github-pr-workflow \
  -d "Backend feature work"

cryozen bundles list
cryozen bundles show backend-dev
cryozen bundles delete backend-dev
```
...
| `backup` | Take a manual tar.gz snapshot of `~/.cryozen-agent/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.cryozen-agent/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `cryozen update`. Use `cryozen curator run --dry-run` to preview before that happens.
...
```bash
cryozen moa list
cryozen moa configure [name]
cryozen moa delete <name>
```

`cryozen moa configure` reuses Cryozen's provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.

## `cryozen fallback`

```bash
cryozen fallback <subcommand>
```
...
| `add` | Pick a provider + model (same picker as `cryozen model`) and append to the chain |
...
```bash
cryozen hooks <subcommand>
```

Inspect shell-script hooks declared in `~/.cryozen-agent/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.cryozen-agent/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `cryozen <provider>` command for provider-specific management (e.g. `cryozen honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `cryozen --help` to see what's currently wired in.
:::
...
Starts Cryozen as an ACP (Agent Client Protocol) stdio server for editor integration.
...
```bash
cd ~/.cryozen-agent/cryozen-agent && uv pip install -e '.[acp]'
```
...
```bash
cryozen mcp <subcommand>
```

Manage MCP (Model Context Protocol) server configurations and run Cryozen as an MCP server.

| Subcommand | Description |
|------------|-------------|
| *(none)* or `picker` | Interactive catalog picker — browse the curated MCP catalog and install/enable/disable. |
| `catalog` | List the curated MCP catalog (plain text, scriptable). |
| `install <name>` | Install a catalog entry (e.g. `cryozen mcp install deepwiki`). |
| `serve [-v\|--verbose]` | Run Cryozen as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Cryozen](../guides/use-mcp-with-cryozen.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-cryozen-as-an-mcp-server).

## `cryozen plugins`

```bash
cryozen plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `cryozen plugins` with no subcommand opens a composite interactive screen with two sections:
...
| `install <identifier> [--force] [--ref COMMIT_SHA] [--allow-removed]` | Install a plugin from the Cryozen plugin catalog (bare entry name), a Git URL, or `owner/repo` shorthand. Catalog names resolve to the entry's repo at its pinned 40-hex commit SHA, show the declared capability summary, and record catalog provenance on the installer's `.install-metadata.json` record (a `.cryozen-catalog.json` copy is written inside the plugin dir for convenience, but is never trusted). Raw URLs are flagged as custom (unreviewed) sources; `--ref` (full 40-character commit SHA) pins them, and on a catalog entry records the checked-out SHA rather than the reviewed pin. `--allow-removed` (DANGEROUS) bypasses the removed-plugin blocklist at install AND exempts that install from the update/enable/load-time kill-list checks. |
| `search [term] [--json]` | Search the Cryozen plugin catalog (matches entry names, descriptions, and declared tools; omit `term` to list everything). The catalog is curated in-repo (`plugin-catalog/`), refreshed from the live repo with a 6-hour cache, and falls back to the in-tree copy offline. Cataloged ≠ audited — admission reviews the entry, not the code. |
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Cryozen Plugin](../developer-guide/plugins/index.md).

## `cryozen tools`

```bash
cryozen tools [--summary]
```
...
## `cryozen computer-use`

```bash
cryozen computer-use <subcommand>
```
...
`cryozen computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`cryozen tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

If cua-driver is already present, Cryozen checks its version and runtime
...
installer. Cryozen never replaces a custom binary selected through
`CRYOZEN_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`cryozen computer-use status` reports when repair is required.

The built-in `computer_use` toolset is the recommended Cryozen integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects Cryozen and links Cua's
skill pack into the Cryozen skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode Cryozen passes Cua's canonical
...
`cryozen update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next Cryozen update.

## `cryozen pets`

```bash
cryozen pets <list|install|select|show|off|scale|remove|doctor>
```

[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Cryozen shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
```bash
cryozen insights [--days N] [--source platform]
```
...
```bash
cryozen claw migrate [options]
```

Migrate your OpenClaw setup to Cryozen. Reads from `~/.openclaw` (or a custom path) and writes to `~/.cryozen-agent`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Cryozen files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.cryozen-agent/` (by default a single restore-point archive is written to `~/.cryozen-agent/backups/pre-migration-*.zip` before apply; restorable with `cryozen import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Cryozen equivalents or **archived** for manual review.
...
cryozen claw migrate --dry-run

# Full migration (all compatible settings, no secrets)
cryozen claw migrate --preset full

# Full migration including API keys
cryozen claw migrate --preset full --migrate-secrets

# Migrate user data only (no secrets), overwrite conflicts
cryozen claw migrate --preset user-data --overwrite

# Migrate from a custom OpenClaw path
cryozen claw migrate --source /home/user/old-openclaw
```

## `cryozen import-agent`

```bash
cryozen import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Cryozen. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.cryozen-agent/skills/`. Always previews before applying; API keys and credentials are never imported.
...
cryozen dashboard --port 8080 --no-open
...
```bash
cryozen profile <subcommand>
```

Manage profiles — multiple isolated Cryozen instances, each with its own config, sessions, skills, and home directory.
...
```bash
cryozen profile list
cryozen profile create work --clone
cryozen profile create work --clone --sync-imports   # also carry over the import-agent sync manifest
cryozen profile use work
cryozen profile alias work --name h-work
cryozen profile export work -o work-backup.tar.gz
cryozen profile import work-backup.tar.gz --name restored
cryozen profile install github.com/user/my-distro --alias
cryozen profile update work
cryozen -p work chat -q "Hello from work profile"
```
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Cryozen commands, subcommands, and profile names.

Examples:

```bash
# Bash
cryozen completion bash >> ~/.bashrc

# Zsh
cryozen completion zsh >> ~/.zshrc

# Fish
cryozen completion fish > ~/.config/fish/completions/cryozen.fish
```

## `cryozen update`

```bash
cryozen update [--gateway] [--check] [--plan] [--no-backup] [--backup] [--yes]
```

Pulls the latest `cryozen-agent` code and reinstalls dependencies in the managed venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install. Use `--check` to see whether your checkout is behind `origin/main` without installing.

`cryozen update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Cryozen may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--backup` | Force a **full** pre-update backup for this run: the quick state snapshot plus a complete zip of `CRYOZEN_HOME` (config, auth, sessions, skills, pairing data). The default mode is `quick` — a lightweight state snapshot only. Set the permanent mode via `updates.pre_update_backup: quick | full | off` in `config.yaml`. |
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `cryozen config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, Cryozen attempts to restart all running gateway profiles automatically so they pick up the new code. Use `cryozen gateway restart` when you want to restart a gateway without applying an update.
...
- **Update receipts + fleet version check.** Every run writes a machine-readable receipt to `~/.cryozen-agent/logs/update_receipts/` (pre-update fleet plan, steps, skips with reasons, restart outcome; `latest.json` points at the newest). After the restart phase the updater verifies each live gateway's running code against the updated checkout and prints a per-profile version matrix; a gateway still on pre-update code fails the update (exit 1) with the exact restart command.
...
- **npm lockfile churn.** Before stashing or switching branches, Cryozen makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `cryozen update`.
- **Pairing data snapshot.** Even when `--backup` is off, `cryozen update` takes a lightweight snapshot of `~/.cryozen-agent/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `cryozen backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `cryozen.service` warning.** If Cryozen detects a pre-rename `cryozen.service` systemd unit (instead of the current `cryozen-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| Command | Description |
|---------|-------------|
| `cryozen --version` | Print version information. |
| `cryozen update` | Pull latest changes and reinstall dependencies. |

| `cryozen uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Cryozen, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
