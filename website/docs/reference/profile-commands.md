...
This page covers all commands related to [Cryozen profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `cryozen profile` without a subcommand shows help.
...
```bash
cryozen profile use <name>
```

Sets `<name>` as the active profile. All subsequent `cryozen` commands (without `-p`) will use this profile.
...
cryozen profile use default
```

## `cryozen profile create`

```bash
cryozen profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `cryozen profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `cryozen update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.cryozen-agent`), use `cryozen skills opt-out` / `cryozen skills opt-in`. |
...
cryozen profile create work2 --clone-from work

# Clone everything from a specific profile
cryozen profile create work2-backup --clone-from work --clone-all
```

## `cryozen profile describe`

```bash
cryozen profile describe [<name>] [options]
```
...
cryozen profile describe researcher

# Set it explicitly
cryozen profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
cryozen profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
cryozen profile describe --all --auto
```

## `cryozen profile delete`

```bash
cryozen profile delete <name> [options]
```
...
```bash
cryozen profile delete mybot
cryozen profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.cryozen-agent`) cannot be deleted — use `cryozen uninstall` to remove everything.
:::

## `cryozen profile show`

```bash
cryozen profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

The skills count here (and in `cryozen profile list`) is counted on the spot. The Desktop and dashboard profile lists are polled every few seconds, so they show the last known count instead and refresh it in the background — a freshly started backend may briefly show `0` skills for a profile until the first background count lands, and a skill you just installed appears in those lists within about a minute.

This shows the profile's Cryozen home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ cryozen profile show work
Profile: work
Path:    ~/.cryozen-agent/profiles/work
...
## `cryozen profile alias`

```bash
cryozen profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Cryozen installation.
...
```bash
cryozen profile alias work
# Creates/updates ~/.local/bin/work

cryozen profile alias work --name mywork
# Creates ~/.local/bin/mywork

cryozen profile alias work --remove
# Removes the wrapper script
```

## `cryozen profile rename`

```bash
cryozen profile rename <old-name> <new-name>
```
...
```bash
cryozen profile rename mybot assistant
# ~/.cryozen-agent/profiles/mybot → ~/.cryozen-agent/profiles/assistant
...
after the rename; `cryozen profile migrate-identity` retries that step too if it was reported as
failed.

## `cryozen profile migrate-identity`

```bash
cryozen profile migrate-identity <old-name> <new-name>
```

Retries the identity migration of a rename that already completed. Run it if `cryozen profile
...
#     cryozen profile migrate-identity mybot assistant

cryozen profile migrate-identity mybot assistant
# ✓ Session/routing identity migrated: mybot → assistant
```

## `cryozen profile purge-identity`

```bash
cryozen profile purge-identity <name>
```

Retries the identity purge of a delete that already completed. Run it if `cryozen profile delete`
...
is decided by `cryozen profile delete`, which removes the profile's own `profiles/<name>/`, its
...
#   Restart the gateway, then run:
#     cryozen profile purge-identity mybot

cryozen profile purge-identity mybot
# ✓ Session/routing identity purged: mybot
```

## `cryozen profile export`

```bash
cryozen profile export <name> [options]
```
...
```bash
cryozen profile export work
# Creates work.tar.gz in the current directory

cryozen profile export work -o ./work-2026-03-29.tar.gz
```
...
## `cryozen profile import`

```bash
cryozen profile import <archive> [options]
```
...
```bash
cryozen profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

cryozen profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `cryozen -p telemetry`). |
...
cryozen profile install github.com/your-org/telemetry-distribution --alias

# Install from a full HTTPS git URL
cryozen profile install https://github.com/your-org/telemetry-distribution.git

# Install from SSH
cryozen profile install git@github.com:your-org/telemetry-distribution.git

# Install from a local directory during development
cryozen profile install ./telemetry/
```

### `cryozen profile update`

```bash
cryozen profile update <name> [--force-config] [--yes]
```
...
### `cryozen profile info`

```bash
cryozen profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Cryozen version, author, env var requirements, the source URL/path, and
...
`cryozen profile list` also shows the distribution name and version in a
`Distribution` column, and `cryozen profile show <name>` / `delete <name>`
...
cryozen profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
cryozen profile install https://github.com/your-org/internal-assistant.git
```
...
`cryozen_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Cryozen version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Cryozen can clone from.
3. Tell recipients to run `cryozen profile install <your-repo-url>`.
...
## `cryozen -p` / `cryozen --profile`

```bash
cryozen -p <name> <command> [options]
cryozen --profile <name> <command> [options]
```

Global flag to run any Cryozen command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
cryozen -p work chat -q "Check the server status"
cryozen --profile dev gateway start
cryozen -p personal skills list
cryozen -p work config edit
```

## `cryozen completion`

```bash
cryozen completion <shell>
```
...
- `cryozen profile <TAB>` — subcommands (list, use, create, etc.)
- `cryozen profile use <TAB>` — profile names
- `cryozen -p <TAB>` — profile names
