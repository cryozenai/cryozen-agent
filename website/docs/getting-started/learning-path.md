...
sidebar_position: 3
title: 'Learning Path'
description: 'Choose your learning path through the Cryozen Agent documentation based on your experience level and goals.'
---

# Learning Path

Cryozen Agent can do a lot — CLI assistant, Telegram/Discord bot, task automation, batch data generation, and more. This page helps you figure out where to start and what to read based on your experience level and what you're trying to accomplish.

:::tip Start Here
If you haven't installed Cryozen Agent yet, begin with the [Installation guide](./installation.md) and then run through the [Quickstart](./quickstart.md). Everything below assumes you have a working installation.
:::

:::tip First-time provider setup
Run `cryozen setup`, choose **Quick Setup**, and paste an API key for the provider you want.
An [OpenRouter](https://openrouter.ai) key is the simplest single-key path; see [AI Providers](../integrations/providers.md) for every option.
:::
...
- **Just browsing?** Check the [Key Features](#key-features-at-a-glance) table for a quick overview of everything Cryozen Agent can do.
...
| **Advanced** | Build custom tools, create skills, generate training data, contribute to the project | [Architecture](../developer-guide/architecture.md) → [Adding Tools](../developer-guide/adding-tools.md) → [Creating Skills](../developer-guide/creating-skills.md) → [Contributing](../developer-guide/contributing.md) | ~4–6 hours |
...
Use Cryozen Agent as an interactive terminal assistant for writing, reviewing, and running code.
...
:::tip
Pass files directly into your conversation with context files. Cryozen Agent can read, edit, and run code in your projects.
:::

### "I want a Telegram/Discord bot"

Deploy Cryozen Agent as a bot on your favorite messaging platform.
...
7. [Use Voice Mode with Cryozen](../guides/use-voice-mode-with-cryozen.md)
...
:::tip
Cron jobs let Cryozen Agent run tasks on a schedule — daily summaries, periodic checks, automated reports — without you being present.
:::
...
Extend Cryozen Agent with your own tools and reusable skill packages.

1. [Plugins](../user-guide/features/plugins.md)
2. [Build a Cryozen Plugin](../developer-guide/plugins/index.md)
...
page is for built-in Cryozen core development, not the usual user/custom-tool path.
:::

### "I want to generate training data"

Run the agent over many prompts in parallel and export the resulting tool-calling trajectories for evaluation or fine-tuning.
...
:::tip
Batch runs work best when you already understand how Cryozen Agent handles conversations and tool calls.
Run through the Beginner path first if you're new.
:::

### "I want to use it as a Python library"

Integrate Cryozen Agent into your own Python applications programmatically.
...
| **Code Execution** | Run Python scripts that call Cryozen tools programmatically | [Code Execution](../user-guide/features/code-execution.md) |
