...
description: "Run Cryozen Agent directly on an Android phone with Termux"
---

# Cryozen on Android with Termux
...
Cryozen Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Cryozen from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Cryozen now ships a Termux-aware installer path:
...
- links `cryozen` into `$PREFIX/bin` so it stays on your Termux PATH
...
Cryozen requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `cryozen` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/cryozen" "$PREFIX/bin/cryozen"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `cryozen` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.cryozen-agent/.env`.
...
`uvloop` bundles libuv, whose `./configure` does not run on Android. Cryozen keeps it out of the core install and of the `termux` / `termux-all` extras — `uvicorn` falls back to the stdlib asyncio loop, which the dashboard and gateway run fine on. If you see this error, you are installing an extra that includes `uvloop` (for example `[all]`): install `.[termux]` or `.[termux-all]` instead.

### `cryozen doctor` says ripgrep or Node is missing
