...
If you only run one Cryozen agent, you don't need this page — see
...
[Connecting Desktop to Many Cryozen Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Cryozen agents that should all
...
(`ai.cryozen.gateway-<name>.plist`), a systemd user service
(`cryozen-gateway-<name>.service`), a systemd **system** service when installed with
`sudo cryozen gateway install --system` (runs as the invoking user via `User=`), a
Windows Scheduled Task, or an s6/Docker service — and the Desktop app spawns its own
per-profile `cryozen serve` backend. This guide adds the patterns for managing them
collectively.
...
cryozen profile create coder
cryozen profile create personal-bot
cryozen profile create research
...
- `cryozen -p <name> gateway run` while it is live **attaches** instead of
...
- `cryozen gateway start --all` / `restart --all` mean *the one host
  ...
  `cryozen gateway migrate --multiplex` one-liner.
- `cryozen gateway run --replace` takes the host role over, whichever profile
  launched the running process; `cryozen gateway run --force` starts a separate
...
[`cryozen gateway migrate --multiplex`](#migrating-from-per-profile-gateways) and
...
`cryozen gateway migrate --multiplex` one-liner. Nothing is changed on disk.
...
- `gateway.multiplex_profiles: false` is **retired**. It used to keep
  ...
  the gateway logs a warning pointing at `cryozen gateway migrate --multiplex`.
...
Other processes (`cryozen -p <name> gateway start`, the dashboard, `cryozen gateway
...
UNIX users, or a `CRYOZEN_HOME` outside `<default home>/profiles/` — every
...
```bash
cryozen config set gateway.multiplex_profiles true
cryozen gateway restart
```

Equivalently, in the default profile's `~/.cryozen-agent/config.yaml`:
...
You do **not** run `cryozen gateway start` for the secondary profiles — the
...
`cryozen -p coder gateway install` (or `start`, `run`, and
the service step of `cryozen -p coder setup`) refuses with exit 78 whether or not
...
❌ Profile 'coder' does not get a gateway of its own.
  ...
  Or fold an existing per-profile fleet onto one host gateway:
    ...
    cryozen gateway migrate --multiplex

  A separate per-profile gateway (for a fleet split across UNIX users or a
  CRYOZEN_HOME outside profiles/) needs --force:  cryozen -p coder gateway install --force
  ...
  it will be removed once they are — plan to fold this profile with `cryozen gateway migrate --multiplex`.)
```
...
PID and served set, and the pointer is `cryozen -p default gateway restart`.
...
by this gateway`. `cryozen -p coder gateway install|start|run` works without
...
multiplexer or `cryozen serve`. Point webhook clients at the standalone
...
`cryozen -p coder gateway status` prints `standalone by config
(gateway.standalone: true)` before the profile's own gateway state, and
`cryozen gateway status` (default) lists it as `standalone by config: coder`
after the served set. `cryozen gateway migrate --multiplex` leaves the profile
...
`CRYOZEN_HOME` outside `profiles/`): it installs a real per-profile service, and
...
With a multiplexer running, a named-profile `cryozen gateway run`, `start`,
...
loop. `cryozen -p coder gateway stop` refuses the same way (exit 78) when coder has no
gateway of its own — there is nothing to stop but the multiplexer, which
`cryozen gateway stop` on the default profile takes down for every served profile.
...
The setup flows follow the same rule: `cryozen -p coder setup gateway`, `cryozen -p coder setup`,
`cryozen -p coder gateway setup` and `cryozen -p coder import` configure the profile's bots but
...
- **WhatsApp (bridge) and Relay are shared ingress owned by the default profile.**
  ...
  is not being served; `cryozen gateway status --profile work` shows
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.cryozen-agent/profiles/coder/.env`; the default listener key is rejected. Under
...
- Webhook routes without `profile` remain default-profile routes and are not
  ...
  way: `cryozen webhook subscribe <name> --route-profile coder` writes
  `profile: coder` into the default gateway's `webhook_subscriptions.json` and
  prints the `/p/coder/webhooks/<name>` URL (`cryozen webhook ls` shows the
...
A standalone `cryozen -p coder gateway run` binds coder's Twilio, LINE, Teams,
...
```
$ cryozen -p coder gateway status
✓ Gateway is running via the default-profile multiplexer
  Manage it from the default profile: cryozen gateway status
...
`cryozen gateway status` and `cryozen status` on the default profile list the same
...
There is a single process-level PID and lock (the multiplexer, under the default home). `cryozen status` on the default profile reports the multiplexer and lists the profiles it serves (`Serves: coder, research`). `cryozen -p coder status` and `cryozen -p coder gateway status` report "running via the default-profile multiplexer" instead of "stopped". The dashboard's `/api/status?profile=coder` / Channels page report the multiplexer as coder's running gateway, with coder's own adapters as its platforms. The single `gateway_state.json` lives under the default home: secondary adapters appear there as `<profile>:<platform>` entries beside `served_profiles`; no per-profile gateway status file is written.

`cryozen -p coder cron status` names the single host gateway and the profiles it serves — `Scheduler host: the host gateway (PID 4211) serving profiles default, coder` — then checks coder's own ticker heartbeat and last successful tick. A missing or stale heartbeat produces a warning rather than an unconditional running verdict. `cron list` and `cron create` also warn when a served profile has no fresh heartbeat. `cron status` adds tick-failure details that those lightweight checks do not read.

When no gateway owns the host role, `cron status` tells you to start the **one** host gateway (`cryozen --profile default gateway install` / `gateway run`) and to make sure it serves this profile. Installing a per-profile service is shown only under `LEGACY (pre-multiplex topology, not recommended)`: it would start a second gateway process on the host. `cryozen doctor` follows the same rule — under s6 it reports `Host gateway: the host gateway (PID 4211) serving profiles default, coder` instead of a per-profile slot count, flags any still-supervised per-profile slot as LEGACY, and checks the host systemd unit's linger even when you run doctor from a served profile. The `state.db` holder lines name the shared host process too, so "3 process(es) holding the DB open" says which gateway and which profiles stopping it would affect.
...
`HINDSIGHT_BANK_ID`, `CRYOZEN_HONCHO_HOST`) are read from the routed profile's
...
`GATEWAY_PROXY_URL`, Firecrawl / Browserbase /
...
`CRYOZEN_LANGUAGE` and `display.language`, and `hooks.outbound[].secret_env` are
...
`trust_recent_files*`) and the `auth.json` OAuth credentials used for auxiliary calls are all
...
`CRYOZEN_HOME`, without the default profile's secrets in their environment, and
...
| Dashboard actions (`cryozen -p <name> …` spawned by the Desktop/dashboard) | A scrubbed child env pinned to that profile's `CRYOZEN_HOME` | The child loads its own `.env`; the dashboard profile's tokens and ports are not inherited |
...
`cryozen profile delete <name>`, or move the
...
multiplexer started as `cryozen -p <name> gateway run` always ticks its own
profile's cron store as well.

The served set is **live**. A profile created while the multiplexer is running
(`cryozen profile create`, the dashboard, Desktop or the TUI) is served at once:
...
default profile's `gateway_state.json` is updated, and `cryozen -p <name> gateway
...
adapters the same way, and `cryozen profile rename` unroutes the old name before
...
`~/.local/bin/cryozen-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: cryozen-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    cryozen gateway "$action"
  else
    cryozen -p "$profile" gateway "$action"
  fi
}
...
```bash
cryozen-gateways start      # start every configured profile
cryozen-gateways stop       # stop every configured profile
cryozen-gateways restart    # restart all
cryozen-gateways status     # status across all
cryozen-gateways list       # delegates to `cryozen gateway list`
```

:::tip
The `default` profile is targeted with `cryozen gateway <action>` (no `-p`),
not `cryozen -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `cryozen -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.cryozen.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/cryozen-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.cryozen.gateway.plist` /
`cryozen-gateway.service`.
...
```bash
# Default profile
tail -f ~/.cryozen-agent/logs/gateway.log
tail -f ~/.cryozen-agent/logs/gateway.error.log

# Named profile
tail -f ~/.cryozen-agent/profiles/<name>/logs/gateway.log
tail -f ~/.cryozen-agent/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.cryozen-agent/logs/gateway.log ~/.cryozen-agent/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
cryozen logs -f                  # follow default profile
cryozen -p coder logs -f         # follow one profile
cryozen logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
cryozen profile list             # profiles + model + gateway state
cryozen-gateways status          # full status across every profile
launchctl list | grep cryozen    # macOS — PIDs and labels
systemctl --user list-units 'cryozen-gateway-*'   # Linux — units
```
...
```
~/.cryozen-agent/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.cryozen-agent/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
cryozen config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.cryozen-agent/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=cryozen --why="gateways running" \
  sleep infinity &
...
`cryozen-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.cryozen-agent/.env ~/.cryozen-agent/profiles/*/.env
```
...
never double-binds a running fleet). `cryozen update` folds them for you unless a
...
```bash
cryozen gateway migrate --multiplex --dry-run   # print the plan and any blockers; changes nothing
cryozen gateway migrate --multiplex             # apply (asks for confirmation on a TTY; -y skips)
```
...
keeps `cryozen -p <name> gateway install --force` as its path — or opts out of
the host gateway with `gateway.standalone: true` (see
[No new per-profile gateways](#no-new-per-profile-gateways)), which
`cryozen gateway migrate --multiplex` respects.

### What `cryozen update` does
...
service) and `gateway.multiplex_profiles` is off, `cryozen update` runs the same
preflight:

- **Nothing blocks it** → the migration runs automatically (the same code path
  as `cryozen gateway migrate --multiplex --yes`) and prints what it did. This
...
install that is already multiplexing is left alone. `cryozen update` also does
nothing when no secondary profile runs its own gateway — it never flips modes
on an install where nothing was running.

### Boundaries `cryozen update` never crosses on its own

The unattended hook only folds profiles that share **one UNIX user, one service
domain and one `profiles/` tree** — the shape `cryozen profile create` produces.
...
| `CRYOZEN_HOME` outside `<default home>/profiles/` | a unit pinning `CRYOZEN_HOME=/opt/cryozen/profiles/emma` |

In that case `cryozen update` prints the boundary it found plus
`cryozen gateway migrate --multiplex`, and changes nothing — no unit is removed
...
findings appear as **notices** in `cryozen gateway migrate --multiplex --dry-run`
...
```bash
cryozen config set gateway.auto_multiplex_migration false
```

`cryozen update` then leaves per-profile gateways exactly as they are, with no
...
other setting, so a value pinned in the managed scope (`/etc/cryozen/config.yaml`)
wins over the profile's own file. It governs the **automatic** path only:
`cryozen gateway migrate --multiplex` is an explicit request and still migrates
(and is the supported way to opt back in). Absent or `true` keeps the default
behaviour described above.

The explicit command is different: `cryozen gateway migrate --multiplex` with
...
`cryozen profile create --clone` leaves the source's bot tokens and allowlists
...
Older clones that still carry them are flagged by `cryozen profile list`.
:::
...
1. Stops each secondary profile's standalone gateway and uninstalls its
   ...
   recorded in `~/.cryozen-agent/gateway_migration.json` for rollback.
...
| A secondary profile enables a port-binding platform that has **no** `/p/<profile>/` ingress on the default listener | The multiplexer skips that whole profile (see [rule 2](#2-http-inbound-platforms-are-reached-via-a-pprofile-url-prefix)) | Disable the platform in that profile (`platforms.<name>.enabled: false`), or run the profile standalone: set `gateway.standalone: true` in its own `config.yaml` and wait for the host to rescan (at most 30 seconds), or send its `rescan-profiles` control verb. Use `cryozen -p <name> gateway install --force` only where a boundary blocks the fold. |
...
above). `cryozen profile create` confirms this when the live multiplexer picked the
profile up; it prints the `cryozen gateway restart` reminder only when it could not
...
window, the next `cryozen gateway migrate --multiplex` sees the flag on, the
...
`cryozen update` pulls the latest code once and syncs new bundled skills into
every profile:
...
boundary (different UNIX user, `CRYOZEN_HOME` outside `profiles/`) and the
...
You ran `cryozen gateway start` after a previous `cryozen gateway stop`. The
...
```bash
ps -ef | grep "cryozen_cli.*-p <profile>"
cat ~/.cryozen-agent/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.cryozen.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.cryozen.gateway-<profile>.plist

# Linux
systemctl --user restart cryozen-gateway-<profile>.service
```

### Health check

```bash
cryozen doctor                  # default profile
cryozen -p <profile> doctor     # one profile
```
