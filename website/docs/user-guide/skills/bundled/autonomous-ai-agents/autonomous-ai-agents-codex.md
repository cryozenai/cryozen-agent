...
| Related skills | [`claude-code`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code.md), [`cryozen-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-cryozen-agent.md) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Cryozen terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Cryozen itself, `model.provider: openai-codex` uses Cryozen-managed Codex
OAuth from `~/.cryozen-agent/auth.json` after `cryozen auth add openai-codex`. For the
...
## Cryozen Gateway Caveat

When invoking the Codex CLI from a Cryozen gateway/service context (for example,
...
terminal(command="codex --sandbox workspace-write exec 'Fix issue #78: <description>. Commit when done.'", workdir="~/.cryozen-agent/cache/scratch/issue-78", background=true, pty=true)
terminal(command="codex --sandbox workspace-write exec 'Fix issue #99: <description>. Commit when done.'", workdir="~/.cryozen-agent/cache/scratch/issue-99", background=true, pty=true)
