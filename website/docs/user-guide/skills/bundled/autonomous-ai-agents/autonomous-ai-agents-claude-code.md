...
| Related skills | [`codex`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-codex.md), [`cryozen-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-cryozen-agent.md), [`opencode`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-opencode.md) |
...
# Claude Code — Cryozen Orchestration Guide

Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) via the Cryozen terminal. Claude Code v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.
...
Cryozen interacts with Claude Code in two fundamentally different ways. Choose based on the task.
...
```
# Start a task
terminal(command="claude -p 'Start refactoring the database layer' --output-format json --max-turns 10 > ~/.cryozen-agent/cache/scratch/session.json", workdir="/project", timeout=180)

# Resume with session ID
terminal(command="claude -p 'Continue and add connection pooling' --resume $(cat ~/.cryozen-agent/cache/scratch/session.json | python -c 'import json,sys; print(json.load(sys.stdin)[\"session_id\"])') --max-turns 5", workdir="/project", timeout=120)
...
```json
{
  "hooks": {
    ...
    "Stop": [{
      "hooks": [{"type": "command", "command": "echo 'Claude finished a response' >> ~/.cryozen-agent/cache/scratch/claude-activity.log"}]
    }]
  }
}
```
...
1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Using `pty=true` alone in Cryozen terminal works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
