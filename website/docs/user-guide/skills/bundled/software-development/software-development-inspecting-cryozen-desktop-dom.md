---
title: "Inspecting Cryozen Desktop Dom — Read the live Cryozen desktop DOM/CSS over CDP"
sidebar_label: "Inspecting Cryozen Desktop Dom"
description: "Read the live Cryozen desktop DOM/CSS over CDP"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Inspecting Cryozen Desktop Dom

Read the live Cryozen desktop DOM/CSS over CDP.
...
| Path | `skills/software-development/inspecting-cryozen-desktop-dom` |
| Version | `1.0.0` |
| Author | Cryozen |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `desktop`, `electron`, `cdp`, `dom`, `ui-verification`, `self-inspection` |
| Related skills | [`node-inspect-debugger`](../../bundled/software-development/software-development-node-inspect-debugger.md), [`dogfood`](../../bundled/software-development/software-development-dogfood.md) |
...
# Inspecting the live Cryozen desktop DOM
...
`debugging-cryozen-desktop`), or anything where the real question is "does this
look right".
...
- **no `CRYOZEN_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`CRYOZEN_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${CRYOZEN_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
CRYOZEN_HOME=$HOME/.cryozen-agent/cache/scratch/cdp-probe-home \
CRYOZEN_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
CRYOZEN_DESKTOP_CDP_PORT=9333 \
  npx electron . --user-data-dir=$HOME/.cryozen-agent/cache/scratch/cdp-probe-userdata
```

The separate `--user-data-dir` dodges Electron's single-instance lock, so it
cannot collide with a running `hgui`; the separate `CRYOZEN_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `CRYOZEN_HOME` baked in, if you
...
- **A throwaway `CRYOZEN_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `cryozen:api` and may exit on its own. The renderer still mounts and the DOM is
