...
| Related skills | [`python-debugpy`](../../bundled/software-development/software-development-python-debugpy.md) |
...
Driver script (save as `~/.cryozen-agent/cache/scratch/cdp-debug.js`):
...
```bash
node ~/.cryozen-agent/cache/scratch/cdp-debug.js
```

Cryozen-specific note: `chrome-remote-interface` is NOT in `ui-tui/package.json`. Install it to a throwaway location if you don't want to dirty the project:

```bash
mkdir -p ~/.cryozen-agent/cache/scratch/cdp-tools && cd ~/.cryozen-agent/cache/scratch/cdp-tools && npm i chrome-remote-interface
NODE_PATH=~/.cryozen-agent/cache/scratch/cdp-tools/node_modules node ~/.cryozen-agent/cache/scratch/cdp-debug.js
```

## Debugging Cryozen ui-tui
...
### Debugging a running `cryozen --tui`
...
require('fs').writeFileSync('~/.cryozen-agent/cache/scratch/cpu.cpuprofile', JSON.stringify(profile));
// Open ~/.cryozen-agent/cache/scratch/cpu.cpuprofile in Chrome DevTools → Performance tab
```
...
require('fs').writeFileSync('~/.cryozen-agent/cache/scratch/heap.heapsnapshot', chunks.join(''));
```
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In Cryozen, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
