...
| Related skills | [`node-inspect-debugger`](../../bundled/software-development/software-development-node-inspect-debugger.md) |
...
- A long-running process (cryozen gateway, tui_gateway) misbehaves and you can't restart it
...
The cryozen test runner and pytest both support this:
...
For long-lived processes: Cryozen gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source <cryozen-agent-repo>/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Cryozen you have two practical options:

**Option 1: `debugpy`'s own CLI REPL** — not an official feature, but a tiny DAP client script:

```python
# ~/.cryozen-agent/cache/scratch/dap_client.py
...
```json
{
  "name": "Attach to Cryozen",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<cryozen-agent-repo>" }
  ]
}
```
...
## Debugging Cryozen-specific Processes

### Tests
See Recipe 3. The wrapper captures subprocess output, so run pytest directly for interactive pdb.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `cryozen` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `cryozen --tui`)
...
debugpy.wait_for_client()
```
Start `cryozen --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Cryozen subagents, debug one process at a time.
