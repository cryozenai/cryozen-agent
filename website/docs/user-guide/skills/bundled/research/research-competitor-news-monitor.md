...
| Related skills | [`rss-feeds`](../../optional/research/research-rss-feeds.md), [`reddit-reading`](../../optional/social-media/social-media-reddit-reading.md) |
...
Use `rss-feeds` (optional) or `blogwatcher` (optional, stateful) for feeds, `reddit-reading` for community discussion, and `web_search`/`web_extract` for pages. Write the watch contract (watchlist, categories, materiality threshold, last cutoff) to a state file under `~/.cryozen-agent/competitor-watches/<watch-slug>.json`, then create the job:

```
cronjob(action="create",
        schedule="every monday 9am",
        prompt="Load the competitor-news-monitor skill and run the tick for the watch contract at ~/.cryozen-agent/competitor-watches/<watch-slug>.json.",
