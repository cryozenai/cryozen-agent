...
Ledger location: `$CRYOZEN_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `CRYOZEN_CITATION_LEDGER`.

## How to Run

```bash
S=~/.cryozen-agent/skills/research/grounded-citations/scripts/sources.py
...
`cryozen skills install official/social-media/reddit-reading` or
`cryozen skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `CRYOZEN_CITATION_LEDGER`) if their
