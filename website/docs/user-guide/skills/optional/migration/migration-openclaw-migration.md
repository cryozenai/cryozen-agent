---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Cryozen"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Cryozen"
---
...
Import an OpenClaw setup (memories, skills) into Cryozen.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Cryozen |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Cryozen`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Cryozen Migration

Use this skill when a user wants to move their OpenClaw setup into Cryozen Agent with minimal manual cleanup.
...
```bash
cryozen claw migrate              # Full interactive migration
...
cryozen claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `cryozen setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_cryozen.py` to:

- import `SOUL.md` into the Cryozen home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Cryozen memory entries
- merge OpenClaw command approval patterns into Cryozen `command_allowlist`
- migrate Cryozen-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Cryozen working-directory configuration
- copy OpenClaw skills into `~/.cryozen-agent/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Cryozen workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.cryozen-agent/tts/`
- archive non-secret docs that do not have a direct Cryozen destination
...
- `scripts/openclaw_to_cryozen.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.cryozen-agent/skills/migration/openclaw-migration/scripts/openclaw_to_cryozen.py`

Do not guess a shorter path like `~/.cryozen-agent/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.cryozen-agent/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Cryozen-compatible secrets, currently:
...
Cryozen CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Cryozen. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Cryozen workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Cryozen skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.cryozen-agent/skills/migration/openclaw-migration/scripts/openclaw_to_cryozen.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.cryozen-agent/skills/migration/openclaw-migration/scripts/openclaw_to_cryozen.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.cryozen-agent/skills/migration/openclaw-migration/scripts/openclaw_to_cryozen.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.cryozen-agent/skills/migration/openclaw-migration/scripts/openclaw_to_cryozen.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.cryozen-agent/skills/migration/openclaw-migration/scripts/openclaw_to_cryozen.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.cryozen-agent/skills/migration/openclaw-migration/scripts/openclaw_to_cryozen.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Cryozen unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Cryozen targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Cryozen destination. Unsupported auth blobs must still be reported as skipped.
...
- Cryozen persona state imported
- Cryozen memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.cryozen-agent/skills/openclaw-imports/`
