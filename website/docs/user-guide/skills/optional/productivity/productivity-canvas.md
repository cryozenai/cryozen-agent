...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Cryozen Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${CRYOZEN_HOME:-~/.cryozen-agent}/.env`:
...
```bash
CANVAS="python $CRYOZEN_HOME/skills/productivity/canvas/scripts/canvas_api.py"
