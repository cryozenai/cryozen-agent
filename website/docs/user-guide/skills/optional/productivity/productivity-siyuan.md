...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/productivity/siyuan` |
...
3. Store it in `${CRYOZEN_HOME:-~/.cryozen-agent}/.env`:
   ```
...
```yaml
# In ~/.cryozen-agent/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
