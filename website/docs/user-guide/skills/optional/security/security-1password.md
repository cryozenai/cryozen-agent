...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/security/1password` |
...
- `tmux` available for stable authenticated sessions during Cryozen terminal calls (desktop app flow only)
...
### Service Account (recommended for Cryozen)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${CRYOZEN_HOME:-~/.cryozen-agent}/.env` (the skill will prompt for this on first load).
...
## Cryozen Execution Pattern (desktop app flow)

Cryozen terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-${CRYOZEN_HOME:-$HOME/.cryozen-agent}/cache/scratch}/cryozen-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/cryozen-op.sock"
