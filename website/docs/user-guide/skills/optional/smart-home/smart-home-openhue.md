...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/smart-home/openhue` |
...
curl -sL "https://github.com/openhue/openhue-cli/releases/latest/download/openhue_Linux_x86_64.tar.gz" \
  | tar -xz -C ~/.cryozen-agent/cache/scratch openhue \
  && install -m 0755 ~/.cryozen-agent/cache/scratch/openhue ~/.local/bin/openhue
...
- Bridge must be on the same local network as the machine running Cryozen
