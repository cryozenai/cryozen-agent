...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Cryozen core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Cryozen native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Cryozen native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Cryozen native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
```bash
parallel-cli search "latest React 19 changes" --json -o ~/.cryozen-agent/cache/scratch/react-19-search.json
```
...
## Recommended Cryozen usage patterns
...
- For large result sets, save JSON to `~/.cryozen-agent/cache/scratch/*.json` (the Cryozen scratch dir) instead of stuffing everything into context.
- Do not silently choose Parallel when Cryozen native tools are already sufficient.
