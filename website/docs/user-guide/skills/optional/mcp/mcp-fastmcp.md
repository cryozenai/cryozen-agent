...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/mcp/fastmcp` |
...
| Related skills | [`cryozen-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-cryozen-agent.md), [`mcporter`](../../optional/mcp/mcp-mcporter.md) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Cryozen or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Cryozen. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.cryozen-agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.cryozen-agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Cryozen integration, either:

- configure the server in `~/.cryozen-agent/config.yaml` using the `native-mcp` skill, or
...
### Cryozen cannot see the deployed server

The server-building part may be correct while the Cryozen config is not. Load the `native-mcp` skill and configure the server in `~/.cryozen-agent/config.yaml`, then restart Cryozen.
