...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/creative/ascii-art` |
...
```bash
python -m pyfiglet "CRYOZEN" -f slant | boxes -d stone
# Or without pyfiglet installed:
curl -s "https://asciified.thelicato.io/api/v2/ascii?text=CRYOZEN&font=Slant" | boxes -d stone
```
...
```bash
curl -s 'https://ascii.co.uk/art/cat' -o ~/.cryozen-agent/cache/scratch/ascii_art.html
```
...
```python
import os, re, html
with open(os.path.expanduser('~/.cryozen-agent/cache/scratch/ascii_art.html')) as f:
    text = f.read()
