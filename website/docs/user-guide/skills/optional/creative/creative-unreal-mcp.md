...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Cryozen MCP catalog. The
...
Two halves, in this order: the editor side must be up before Cryozen connects.
...
### One-time, Cryozen side

    cryozen mcp install unreal-engine
...
`~/.cryozen-agent/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Cryozen — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Cryozen connects
...
2. Start the Cryozen session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Cryozen
they appear as:

| Cryozen tool | Purpose |
|---|---|
...
then happens at `cryozen mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Cryozen runs batched calls
...
- **Start order matters.** Editor + server up first, then the Cryozen
...
- **Timeouts on long operations.** Cryozen' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.cryozen-agent/config.yaml` for
