...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/creative/pretext` |
...
| Related skills | [`p5js`](../../bundled/creative/creative-p5js.md), [`excalidraw`](../../optional/creative/creative-excalidraw.md) |
...
This skill exists so Cryozen can make **cool demos** with it — the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for the community demo corpus.
...
2. **Start from a template**:
   ...
   - `write_file` to a new `.html` in `~/.cryozen-agent/cache/scratch/` or the user's workspace.
