---
title: "Honcho — Configure and troubleshoot Honcho memory for Cryozen"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Cryozen"
---
...
Configure and troubleshoot Honcho memory for Cryozen.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Cryozen

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Cryozen profile its own peer identity while sharing a unified view of the user.
...
```bash
cryozen honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Cryozen profile's AI peer
...
Honcho models conversations as interactions between **peers**. Cryozen creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Cryozen instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Cryozen run |
| `global` | Single session across all directories |

Manual override: `cryozen honcho map my-project-name`
...
Each Cryozen profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block cryozen.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `cryozen.coder` host block in `honcho.json`
...
```bash
cryozen honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Cryozen when Honcho memory is active.
...
Config file: `$CRYOZEN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `cryozen honcho setup`. Ensure `memory.provider: honcho` is in `~/.cryozen-agent/config.yaml`.

### Memory not persisting across sessions
Check `cryozen honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `cryozen profile create <name> --clone`. For existing profiles: `cryozen honcho sync`.
...
| Command | Description |
|---------|-------------|
| `cryozen honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `cryozen honcho status` | Show resolved config, connection test, peer info for active profile |
| `cryozen honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `cryozen honcho disable` | Disable Honcho for the active profile |
| `cryozen honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `cryozen honcho peers` | Show peer identities across all profiles |
| `cryozen honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `cryozen honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `cryozen honcho sessions` | List known directory-to-session-name mappings |
| `cryozen honcho map <name>` | Map current working directory to a Honcho session name |
| `cryozen honcho identity` | Seed AI peer identity or show both peer representations |
| `cryozen honcho sync` | Create host blocks for all Cryozen profiles that don't have one yet |
| `cryozen honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Cryozen + Honcho |
| `cryozen memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `cryozen memory status` | Show active memory provider and config |
| `cryozen memory off` | Disable external memory provider |
