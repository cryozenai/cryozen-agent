...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/autonomous-ai-agents/agent-merge-conflict-arbiter` |
...
- Both sides' intent sources: kanban completion summaries (`terminal` running
  `cryozen kanban show <task-id>`), PR bodies, or at minimum each branch's
  commit messages.
...
- Collect each side's intent: `cryozen kanban show <task-id>` for completion
