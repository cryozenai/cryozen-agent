...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/autonomous-ai-agents/openhands` |
...
| Related skills | [`claude-code`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code.md), [`codex`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-codex.md), [`opencode`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-opencode.md), [`cryozen-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-cryozen-agent.md) |
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Cryozen.

## When to Use

- User wants a coding task delegated to OpenHands specifically.
- User wants a coding agent that can run on a non-Anthropic / non-OpenAI provider (DeepSeek, Qwen, Ollama, vLLM, etc.) — sibling skills `claude-code` and `codex` are tied to one vendor.
- Multi-step file edits + shell commands inside a workspace.

For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Cryozen-native subagents, use `delegate_task`.
...
terminal(
  ...
  workdir="~/.cryozen-agent/cache/scratch",
  timeout=120
)
```
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `cryozen-agent` (Cryozen subagents via `delegate_task`).
