...
| | |
|---|---|
| Source | Optional — install with `cryozen skills install official/autonomous-ai-agents/dynamic-workflow` |
...
for serial chains. For a refactor or fix campaign on cryozen-agent itself, load
`cryozen-agent` (the dev workflow) alongside; this skill owns the fan-out shape.
...
10. Live QA is its own wave: one squad per surface, isolated `CRYOZEN_HOME`,
