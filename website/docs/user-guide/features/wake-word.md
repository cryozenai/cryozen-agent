...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Cryozen's wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Cryozen")

The wake word turns Cryozen into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Cryozen listens in the background for a spoken
trigger phrase. Say it, and Cryozen starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Cryozen backend (for example a
...
Cryozen supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey cryozen"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey cryozen"** — a model for it ships with Cryozen, so
...
```bash
cd ~/.cryozen-agent/cryozen-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `cryozen` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.cryozen-agent/config.yaml`,
...
wake_word:
  ...
  phrase: "hey cryozen"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_cryozen         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey cryozen" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey cryozen" utterances are missed.
...
default) to let Cryozen choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Cryozen installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Cryozen surfaces, and `surface` picks which one
...
| `cli` | Only the classic `cryozen` CLI. |
| `tui` | Only `cryozen --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Cryozen surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Cryozen does not silently fail over to another open surface.
...
"Hey Cryozen" works out of the box — the bundled openWakeWord model
(`model: hey_cryozen`) is the default. To wake on something else, the easiest
...
command (`cryozen -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.cryozen-agent/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("cryozen" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Cryozen" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey cryozen"
  porcupine:
    keyword: ~/.cryozen-agent/wakewords/hey_cryozen.ppn
```

Set your access key in `~/.cryozen-agent/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `cryozen tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `cryozen-agent[wake]`).
...
but the phrase never fires. Cryozen detects this (`/wake status` shows
"mic delivers only silence"; the desktop's folded voice menu carries the same
hint on its trigger).
Fix: System Settings → Privacy & Security → Microphone → enable the Cryozen
backend (it may appear as your terminal, `python`, or Cryozen), then toggle the
...
```bash
cryozen config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
cryozen config set wake_word.input_device null
```
