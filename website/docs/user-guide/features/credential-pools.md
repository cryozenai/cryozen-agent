...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Cryozen automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Cryozen auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
cryozen auth add openrouter --api-key sk-or-v1-your-second-key
...
`cryozen auth remove <provider> <target>` when a label is ambiguous, and `priority=` is
the order the pool tries credentials in under the `fill_first` strategy.

## Interactive Management

Run `cryozen auth` with no subcommand for an interactive wizard:
...
Each `cryozen auth add openai-codex` login becomes its own pool entry, but only **different** OpenAI accounts rotate independently: two logins of the same account share one token family upstream, so OpenAI revokes the older one and the second entry adds no quota. Cryozen warns at add time (`warning: this login is the same OpenAI account as openai-codex credential #N`) — log into a different account, or keep just one.

## CLI Commands

| Command | Description |
|---------|-------------|
| `cryozen auth` | Interactive pool management wizard |
| `cryozen auth list` | Show all pools and credentials |
| `cryozen auth list <provider>` | Show a specific provider's pool |
| `cryozen auth add <provider>` | Add a credential (prompts for type and key) |
| `cryozen auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `cryozen auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `cryozen auth add openai-codex --browser` | Codex only: sign in with the browser authorization-code + PKCE flow on `localhost:1455` instead of the default device code (for orgs that disable device-code grants); falls back to device code when the port is busy. Default for every Codex login via `auth.codex_login_flow: browser` |
| `cryozen auth add <provider> --priority 0` | Add a credential and place it first in the `fill_first` order |
| `cryozen auth priority <provider> <target> <n>` | Move a credential to priority `n` (0 = tried first); the rest are renumbered |
| `cryozen auth remove <provider> <index>` | Remove credential by 1-based index |
| `cryozen auth reset <provider>` | Clear all cooldowns/exhaustion status (applies to running sessions too: a live gateway or chat picks the reset up on its next request instead of writing its stale cooldown back) |
| `cryozen auth reset <provider> <target>` | Clear the cooldown on one credential by index, id, or label |
| `cryozen auth refresh <provider> [target]` | Refresh one OAuth credential's tokens and return it to rotation (proves the grant is alive; the next request re-checks quota) |
...
Refresh-only lookups and peeks do not count. Status reads (`cryozen doctor`, the `/model`
...
Configure via `cryozen auth` → "Set rotation strategy" or in `config.yaml`:
...
| Strategy | Behavior |
|----------|----------|
| `fill_first` (default) | Use the first healthy key until it's exhausted, then move to the next; order is each credential's `priority` (`cryozen auth priority` changes it) |
...
```bash
cryozen auth list openai-codex                 # find the index, id or label
cryozen auth priority openai-codex 1 99        # 1-based index, entry id, or exact label; large n = last
cryozen auth priority openai-codex work-seat 0 # ...and back to the front later
```

`cryozen auth priority <provider> <target> <priority>` reorders one credential and renumbers the
...
`cryozen auth reset` to clear; it is not refreshed on a timer, and it is still used once every
...
| **400 Codex model entitlement** (`The '<model>' model is not supported when using Codex with a ChatGPT account.`) | Bench this key for the rejected model only and rotate to the next key; other models keep using the key. Other 400s never rotate | Until `cryozen auth reset` (per model; an entitlement is a plan property, not a window) |
...
**Every Codex login in an always-on home is dead: sign in again, do not wait for adoption.** Cryozen
...
without a Codex credential until you run `cryozen auth add openai-codex` in *that* home
(`cryozen -p <profile> auth add openai-codex` for a named profile), which offers the Codex CLI import
interactively. Profiles that should share one login can point at the same `CRYOZEN_HOME` instead of
...
the remaining cooldown (or the `cryozen auth add <provider>` re-login for a dead one) — the first-run
...
When you set up a custom endpoint via `cryozen model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via cryozen model:
cryozen auth list
# Shows:
...
cryozen auth add Together.ai --api-key sk-together-second-key
```
...
Cryozen automatically discovers credentials from multiple sources and seeds the pool on startup:
...
Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `cryozen auth add`) are never auto-pruned.
...
Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Cryozen can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Cryozen-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`cryozen_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`cryozen_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`agent/turn_api_error.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.cryozen-agent/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Cryozen's credential store, so its token remains persistable.

An `env:` row is re-hydrated from the environment on every load, and the variable name does not have to be one Cryozen declares for the provider: numbered siblings (`OPENROUTER_API_KEY_2`, see [Auto-Discovery](#auto-discovery)) appear here automatically, and a hand-written row pointing at any other variable is filled the same way, without the secret ever being written to `auth.json`.
