...
Type `@` followed by a reference to inject content directly into your message. Cryozen expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Cryozen env: `$CRYOZEN_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CRYOZEN_HOME/skills/.hub/`
