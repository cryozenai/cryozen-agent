...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Cryozen CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Cryozen CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.cryozen-agent/skins/mytheme.yaml
```

Or set the default skin in `~/.cryozen-agent/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Cryozen - ice blue and kawaii | `Cryozen Agent` | Ice-blue borders, frost-white text, kawaii faces in spinners. The CRYOZEN-AGENT banner with the ice-crystal hero. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Cryozen Agent` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Cryozen Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Cryozen Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Cryozen Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|--------------------------|
| `banner_border` | Panel border around the startup banner | `#388BFD` (blue) |
| `banner_title` | Title text color in the banner | `#E0F7FF` (frost white) |
| `banner_accent` | Section headers in the banner (Available Tools, etc.) | `#7DD3FC` (ice blue) |
| `banner_dim` | Muted text in the banner (separators, secondary labels) | `#5B7A99` (slate blue) |
| `banner_text` | Body text in the banner (tool names, skill names) | `#E0F7FF` (frost white) |
| `ui_accent` | General UI accent color (highlights, active elements) | `#7DD3FC` |
| `ui_label` | UI labels and tags | `#60A5FA` (sky blue) |
...
| `prompt` | Interactive prompt text color | `#E0F7FF` |
| `input_rule` | Horizontal rule above the input area | `#388BFD` |
| `response_border` | Border around the agent's response box (ANSI escape) | `#7DD3FC` |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Cryozen Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Cryozen Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ✻` |
| `response_label` | Label on the response box header | ` ✻ Cryozen ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default CRYOZEN-AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default ice-crystal art) | `""` |

## Custom skins

Create YAML files under `~/.cryozen-agent/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.cryozen-agent/skins/mytheme.yaml
...
- Built-in skins load from `cryozen_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.cryozen-agent/skins/` take precedence over built-in skins with the same name.
