...
When Cryozen Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
2. **The gateway scans agent responses for file paths.** Any absolute path
   (`~/.cryozen-agent/cache/scratch/...`) or home-relative path (`~/...`) ending in a supported
...
persona in `~/.cryozen-agent/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.cryozen-agent/config.yaml` (switchable per session
via `/personality`).

The mechanic the agent has to use is simple: render the file to an
absolute path (e.g. `~/.cryozen-agent/cache/scratch/q3-revenue.png`) and mention that path as
...
If you use Cryozen's kanban multi-agent workflow, workers can attach
...
kanban_complete(
    ...
    artifacts=[
        "~/.cryozen-agent/cache/scratch/q3-revenue.png",
        "~/.cryozen-agent/cache/scratch/q3-report.pdf",
    ],
)
```
...
Install MCP servers via `~/.cryozen-agent/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Cryozen Agent's deliverable
