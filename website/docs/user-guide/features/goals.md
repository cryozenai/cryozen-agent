...
description: "Set a standing goal and let Cryozen keep working across turns until it's done. Our take on the Ralph loop."
---

# Persistent Goals (`/goal`)

`/goal` gives Cryozen a standing objective that survives across turns. After every turn a lightweight judge model checks whether the goal is satisfied by the assistant's last response. If not, Cryozen automatically feeds a continuation prompt back into the same session and keeps working — until the goal is achieved, you pause or clear it, or the turn budget runs out.

It's our take on the **Ralph loop**, directly inspired by [Codex CLI 0.128.0's `/goal`](https://github.com/openai/codex). The core idea — keep a goal alive across turns and don't stop until it's achieved — is theirs. The implementation here is independent and adapted to Cryozen's architecture.

## When to use it

Use `/goal` for tasks where you want Cryozen to iterate on its own without you re-prompting every turn:
...
`/goal` and [Kanban](./kanban) both keep Cryozen working without you re-prompting, so it's tempting to assume one flows into the other. It doesn't — the boundary is sharp:
...
| Many independent tasks, with dependencies, handoffs, or multiple profiles | [Kanban](./kanban) — `cryozen kanban create …` |
| One card on the board that should keep iterating until its acceptance criteria are met | A kanban card with `--goal` |

:::note
If you want work on the board, put it there yourself (`cryozen kanban create …`) — `/goal` won't do it for you. The reverse is also true: pausing, resuming, or clearing a goal in this chat never creates, claims, or moves a kanban card.
:::

## Quick start

```
/goal Fix every failing test in tests/cryozen_cli/ and make sure scripts/run_tests.sh passes for that directory
```

What you'll see:

1. **Goal accepted** — `⊙ Goal set (20-turn budget): <your goal>`
2. **Turn 1 runs** — Cryozen starts working as if you'd sent the goal as a normal message.
3. **Judge runs** — after the turn, the judge model decides `done`, `continue`, or `blocked`.
4. **Loop fires if needed** — if `continue`, you'll see `↻ Continuing toward goal (1/20): <judge's reason>` and Cryozen takes the next step automatically.
...
`/goal draft <text>` both creates the goal and starts its first turn, including when drafting is unavailable and Cryozen falls back to a free-form goal. `draft` is a whole-word subcommand: `/goal drafting docs` keeps `drafting docs` as the literal objective without calling the draft model.
...
| `stop_when` | The condition under which Cryozen should stop and ask for input. |
...
**1. Let Cryozen draft it** (recommended — adapted from Codex's "let the agent draft the goal" tip):

```
/goal draft Migrate the auth service from session cookies to JWT
```

Cryozen expands your one-liner into a full contract via the `goal_judge` auxiliary model, sets it, and shows you the result so you can review or tighten any field. If the aux model is unavailable, it falls back to a plain free-form goal — drafting never blocks setting a goal.
...
/goal gate add scripts/run_tests.sh tests/cryozen_cli/test_goals.py
```
...
| Command | What it does |
|---|---|
| `/goal wait <pid> [reason]` | Manually park the loop until the process with that PID exits. The PID must be a live process on the Cryozen host; a remote or already-exited PID is rejected (and a judge `wait_on_pid` naming one continues instead of parking). |
...
After every turn, Cryozen calls an auxiliary model with:
...
If the judge errors (network blip, malformed response, unavailable aux client), Cryozen treats the verdict as `continue` — a broken judge never wedges progress. The **turn budget** is the real backstop.

### Turn budget

Default is 20 continuation turns (`goals.max_turns` in `config.yaml`). When the budget is hit, Cryozen auto-pauses and tells you exactly how to proceed:
...
The continuation prompt is a plain user-role message appended to history. It does **not** mutate the system prompt, swap toolsets, or touch the conversation in any way that invalidates Cryozen's prompt cache. Running a 20-turn goal costs the same cache-wise as 20 turns of normal conversation.
...
```yaml
goals:
  # Max continuation turns before Cryozen auto-pauses and asks you to
...
```
You: /goal Create four files ~/.cryozen-agent/cache/scratch/note_{1..4}.txt, one per turn, each containing its number as text

  ⊙ Goal set (20-turn budget): Create four files ~/.cryozen-agent/cache/scratch/note_{1..4}.txt, one per turn, each containing its number as text

Cryozen: Creating ~/.cryozen-agent/cache/scratch/note_1.txt now.
  💻 echo "1" > ~/.cryozen-agent/cache/scratch/note_1.txt   (0.1s)
  I've created ~/.cryozen-agent/cache/scratch/note_1.txt with the content "1". I'll continue with the remaining files on the next turn as you specified.

  ↻ Continuing toward goal (1/20): Only 1 of 4 files has been created; 3 files remain.

Cryozen: [Continuing toward your standing goal]
  💻 echo "2" > ~/.cryozen-agent/cache/scratch/note_2.txt   (0.1s)
  Created ~/.cryozen-agent/cache/scratch/note_2.txt. Two more to go.

  ↻ Continuing toward goal (2/20): 2 of 4 files created; 2 remain.

Cryozen: [Continuing toward your standing goal]
  💻 echo "3" > ~/.cryozen-agent/cache/scratch/note_3.txt   (0.1s)
  Created ~/.cryozen-agent/cache/scratch/note_3.txt.

  ↻ Continuing toward goal (3/20): 3 of 4 files created; 1 remains.

Cryozen: [Continuing toward your standing goal]
  💻 echo "4" > ~/.cryozen-agent/cache/scratch/note_4.txt   (0.1s)
  All four files have been created: ~/.cryozen-agent/cache/scratch/note_1.txt through ~/.cryozen-agent/cache/scratch/note_4.txt, each containing its number.
...
`/goal` is Cryozen's take on the **Ralph loop** pattern. The user-facing design — keep a goal alive across turns, don't stop until it's achieved, with create/pause/resume/clear controls — was popularised and shipped in [Codex CLI 0.128.0](https://github.com/openai/codex). Our implementation is independent (central `CommandDef` registry, `SessionDB.state_meta` persistence, auxiliary-client judge, adapter-FIFO continuation on the gateway side) but the idea is theirs. Credit where credit's due.
