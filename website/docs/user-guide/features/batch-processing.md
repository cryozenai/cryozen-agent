...
Batch processing lets you run the Cryozen agent across hundreds or thousands of prompts in parallel, generating structured trajectory data. This is primarily used for **training data generation** — producing ShareGPT-format trajectories with tool usage statistics that can be used for fine-tuning or evaluation.
...
Batch runs spin up many concurrent agent sessions, each making model calls and tool calls.
To keep cost per trajectory stable, pin `--model` to a specific model, and consider a [credential pool](./credential-pools.md) so several API keys share the load instead of one key hitting its rate limit.
OpenRouter is a convenient single key when a run mixes models from several vendors.
:::
