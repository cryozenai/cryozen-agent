...
**Tool Search** is Cryozen's opt-in progressive-disclosure layer for that
...
Cryozen keeps its working-set core tools (`terminal`, `read_file`, `write_file`,
`patch`, `search_files`, `todo`, `memory`, `browser_*`, `web_search`,
`web_extract`, `clarify`, `execute_code`, `delegate_task`, and the rest of
`_CRYOZEN_CORE_TOOLS`) loaded directly by default. Cold, event-triggered built-ins
...
one. Multi-entry batches of local tools are rejected.
...
When the model invokes `tool_call`, Cryozen **unwraps the bridge** and
...
in `cryozen_cli/config_defaults.py`. It is the single source of truth for the
...
`tool_call` takes `calls` as an array of `{name, arguments}` entries; local
deferred tools stay one entry per `tool_call`. A multi-entry batch that names
a local tool is rejected with a correction that restates the valid shape using the
...
- **No provider-native validation for deferred schemas.** `tool_describe`
  ...
  only the generic `tool_call.arguments` object. Cryozen therefore coerces and
  validates the underlying arguments locally before dispatch; the concrete
  tool or MCP server remains responsible for schemas Cryozen cannot safely
...
- **No JS sandbox.** Cryozen uses the simpler "structured tools" mode
