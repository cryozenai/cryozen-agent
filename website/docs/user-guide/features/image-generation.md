...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `cryozen tools`.
...
Cryozen Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `cryozen tools` and persists in `config.yaml`.
...
Navigate to **🎨 Image Generation**, pick your backend (for example FAL.ai, OpenAI, xAI, Krea, or OpenRouter), then the picker shows all supported models in a column-aligned table — arrow keys to navigate, Enter to select:
...
```yaml
image_gen:
  provider: fal                 # or openai, xai, krea, openrouter, ...
  model: fal-ai/flux-2/klein/9b
  max_parallel_requests: 4      # concurrent images in one tool-call batch
```

`image_gen.provider` is the single selection key: a vendor name (`fal`, `openai`, `xai`, `krea`, ...) goes direct with your own key. The runtime always follows this stored selection: `provider: fal` without `FAL_KEY` errors with `image_gen is configured to use fal (set via cryozen tools), but FAL_KEY is not set. Run 'cryozen tools' to change it.` rather than silently rerouting. Change providers via `cryozen tools`, not by adding/removing keys.

`max_parallel_requests` defaults to `4`. Cryozen clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no Cryozen
...
The `fal-ai/gpt-image-1.5` and `fal-ai/gpt-image-2` request quality is pinned to `medium` (~$0.034–$0.06/image at 1024×1024). The `low` / `high` tiers are not exposed as a user-facing option so that per-image cost stays predictable; the cost spread between tiers is 3 to 22 times. If you want a cheaper option, pick Klein 9B or Z-Image Turbo; if you want higher quality, use Nano Banana Pro or Recraft V4 Pro.
...
only for now; responses are saved to `$CRYOZEN_HOME/cache/images/`.

## FAL: GPT Image 2.5

Select **GPT Image 2.5 Flare** or **GPT Image 2.5 Sunburst** under
`cryozen tools` → Image Generation → FAL.ai. The model IDs are:
...
```bash
cryozen config set image_gen.provider fal
cryozen config set image_gen.model openai/gpt-image-2.5/flare/text-to-image
```
...
Both accept up to 16 source images. Cryozen pins quality to `medium`, matching its
...
pages. Direct FAL requires a funded `FAL_KEY`.
...
Select them through `cryozen tools` → Image Generation → OpenAI, or set:

```bash
cryozen config set image_gen.provider openai
cryozen config set image_gen.openai.model gpt-image-2.5-flare
```
...
`OPENAI_API_KEY` is required. Requests go through Cryozen's own HTTP client, which
...
Cryozen posts straight to the Codex backend's native
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials, according to the stored `image_gen.provider` selection.
...
- **Requires credentials** for the active backend (FAL `FAL_KEY`, `OPENAI_API_KEY`, xAI OAuth, `KREA_API_KEY`)
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Cryozen materializes them to the local cache so delivery still works after expiry
