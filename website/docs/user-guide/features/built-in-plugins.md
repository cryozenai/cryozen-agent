...
description: "Plugins shipped with Cryozen Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Cryozen ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.cryozen-agent/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](./plugins.md) page for the general plugin system, and [Build a Cryozen Plugin](../../developer-guide/plugins/index.md) to write your own.
...
2. **User** — `~/.cryozen-agent/plugins/<name>/`
3. **Project** — `./.cryozen-agent/plugins/<name>/` (requires `CRYOZEN_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `cryozen_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `cryozen memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `cryozen plugins list` and the interactive `cryozen plugins` UI), but none load until you explicitly enable them:

```bash
cryozen plugins enable disk-cleanup
```

Or via `~/.cryozen-agent/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Cryozen. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
cryozen plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `cryozen plugins enable <name>`.
...
| `cryozen-achievements` | dashboard tab | Steam-style collectible badges generated from your real Cryozen session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `cryozen memory` and `cryozen plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CRYOZEN_HOME` or `/tmp/cryozen-*`, track it silently as `test` / `temp` / `cron-output`. | <!-- no-tmp: ok — documents the plugin's scope guard -->
...
| empty dirs under CRYOZEN_HOME | always | Never |
...
**State** — everything lives at `$CRYOZEN_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `CRYOZEN_HOME` or `/tmp/cryozen-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end. User project trees (`workspace/`, `projects/`, `plans/`, `home/`) are never tracked or swept at all: a `test_*.py` or `tmp_*` file inside your project is source code, not scratch. `kanban/` (task attachments and workspaces) is never tracked either, and a tracked *directory* under a protected top level such as `cache/` is never removed — only the files inside it age out.

**Enabling:** `cryozen plugins enable disk-cleanup` (or check the box in `cryozen plugins`).

**Disabling again:** `cryozen plugins disable disk-cleanup`.
...
**Enabling:** `cryozen plugins enable security-guidance` (or check the box in `cryozen plugins`).

**Disabling again:** `cryozen plugins disable security-guidance`.
...
Traces Cryozen turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Cryozen's canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `cryozen logs`.
...
```bash
cryozen tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Cryozen and the next turn ships a trace.
...
Then put the credentials in `~/.cryozen-agent/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Cryozen turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `CRYOZEN_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Cryozen session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `cryozen chat` session lives under one Langfuse session.

**Verify:**

```bash
cryozen plugins list                 # observability/langfuse should show "enabled"
cryozen chat -q "hello"              # check the Langfuse UI for a "Cryozen turn" trace
```
...
| `CRYOZEN_LANGFUSE_RELEASE` | — | Release/version tag |
| `CRYOZEN_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `CRYOZEN_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `CRYOZEN_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Cryozen-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Cryozen-prefixed wins when both are set.
...
**Disabling:** `cryozen plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled Cryozen plugin. Do not run `cryozen plugins enable observability/nemo_relay`; Cryozen core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `CRYOZEN_NEMO_RELAY_PLUGINS_TOML` to that file before starting Cryozen. The policy is process-wide for every profile hosted by that Cryozen process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `CRYOZEN_NEMO_RELAY_ATOF_*` and `CRYOZEN_NEMO_RELAY_ATIF_*` settings no longer activate exporters — a `.env` that still carries them (and no `CRYOZEN_NEMO_RELAY_PLUGINS_TOML`) exports **nothing**, and the gateway logs one warning saying so. `cryozen doctor` reports these stale settings when no replacement `plugins.toml` is selected.

**Automatic migration.** `cryozen update` (and `cryozen migrate relay`, or `cryozen migrate relay --all-profiles` for every profile home) converts the legacy variables into `<cryozen home>/relay-plugins.toml`, sets `CRYOZEN_NEMO_RELAY_PLUGINS_TOML` in that profile's `.env`, and comments the legacy lines out (nothing is deleted). Under a multiplexed gateway every profile home gets its own file. The generated file is validated through Relay before it is written; this is the shape it produces (note the `type = "file"` sink discriminator — a sink without it is rejected):
...
type = "file"
output_directory = "/home/you/.cryozen-agent/telemetry/nemo-relay/atof"
filename = "cryozen-atof.jsonl"
mode = "append"

[components.config.atif]
enabled = true
agent_name = "Cryozen Agent"
model_name = "unknown"
output_directory = "/home/you/.cryozen-agent/telemetry/nemo-relay/atif"
...
Then add `CRYOZEN_NEMO_RELAY_PLUGINS_TOML=/home/you/.cryozen-agent/relay-plugins.toml` to `.env` and restart the gateway.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `cryozen.session.segment` plus `cryozen.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.cryozen-agent/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
cryozen plugins enable google_meet
cryozen meet setup   # preflight: playwright, chromium, auth file
cryozen meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `cryozen plugins disable google_meet`. Any saved transcripts stay in `~/.cryozen-agent/workspace/meetings/` until you remove them.
...
- Scans your entire `~/.cryozen-agent/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$CRYOZEN_HOME/plugins/cryozen-achievements/state.json`
...
| Secret | Hidden until Cryozen detects the first related signal in your history |

**API** — routes mount under `/api/plugins/cryozen-achievements/`:
...
**State files** — live under `$CRYOZEN_HOME/plugins/cryozen-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Cryozen updates. |
...
**Enabling:** Nothing to enable — `cryozen-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `cryozen dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/cryozen-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.cryozen-agent/plugins/cryozen-achievements/` that ships no dashboard. The plugin's state files under `$CRYOZEN_HOME/plugins/cryozen-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Cryozen plugin — see [Build a Cryozen Plugin](../../developer-guide/plugins/index.md). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.cryozen-agent/plugins/<name>/`
- Manifest source is reported as `bundled` in `cryozen plugins list`
