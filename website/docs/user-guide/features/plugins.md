...
title: "Plugins"
description: "Extend Cryozen with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Cryozen has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](../../developer-guide/adding-tools.md) page is for built-in Cryozen
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Cryozen Plugin](../../developer-guide/plugins/index.md)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.cryozen-agent/plugins/` with a `plugin.yaml` and Python code:
...
Start Cryozen — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.cryozen-agent/plugins/hello-world/plugin.yaml`**
...
**`~/.cryozen-agent/plugins/hello-world/__init__.py`**

```python
"""Minimal Cryozen plugin — registers a tool and a hook."""
...
Drop both files into `~/.cryozen-agent/plugins/hello-world/`, restart Cryozen, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Cryozen does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.cryozen-agent/plugins/` are disabled by default. Enable them only for trusted repositories by setting `CRYOZEN_ENABLE_PROJECT_PLUGINS=true` before starting Cryozen.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `cryozen <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `cryozen plugins install` |
| Distribute via pip | `[project.entry-points."cryozen_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Cryozen — see [Built-in Plugins](./built-in-plugins.md) |
| User | `~/.cryozen-agent/plugins/` | Personal plugins |
| Project | `.cryozen-agent/plugins/` | Project-specific plugins (requires `CRYOZEN_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `cryozen_agent.plugins` entry_points | Distributed packages |
| Nix | `services.cryozen-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](../../getting-started/nix-setup.md#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Cryozen also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.cryozen-agent/plugins/model-providers/<name>/` override bundled model providers of the same name (last-writer-wins in `register_provider()`), so you can replace a built-in provider profile without any repo edits. Memory providers resolve the other way round: for `~/.cryozen-agent/plugins/memory/<name>/` the **bundled** provider wins on a name collision (bundled, then user, then project, then entry points; first seen wins), so a user memory provider needs its own unique name.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `cryozen plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.cryozen-agent/config.yaml`. This stops third-party code from running without your explicit consent.

:::note `plugins.enabled` governs plugins only
[Gateway event hooks](./hooks.md#gateway-event-hooks) under `~/.cryozen-agent/hooks/<name>/` are not plugins and are **not** gated by `plugins.enabled` or `plugins.disabled`. That directory is trusted by placement: any subdirectory holding a valid `HOOK.yaml` + `handler.py` is imported by the gateway at startup, and placing the files there is the opt-in. See the [gateway hook trust model](./hooks.md#gateway-hook-trust).
:::
...
```bash
cryozen plugins                    # interactive toggle (space to check/uncheck)
cryozen plugins enable <name>      # add to allow-list
cryozen plugins disable <name>     # remove from allow-list + add to disabled
```

After `cryozen plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
cryozen plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Cryozen checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `cryozen plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`cryozen plugins install <source> --force --ref <new-commit>`. The
profile-local install metadata contains no config values, environment values,
secrets, or capability grants.

The same pin is available in Cryozen Desktop: **Skills → Plugins → Install from
...
can confirm everyone is running the same commit. `cryozen plugins list` prints
...
`cryozen plugins install` clones non-interactively (it never prompts for a
username or password), so a private repo needs a credential Cryozen can find on
its own. Every clone, pinned `--ref` fetch and `cryozen plugins update` pull is
...
refuses anonymous access does Cryozen look for a credential. For an `https://`
...
before. The same resolution applies to `cryozen plugins update`, catalog MCP
installs from git, and profile distributions fetched from a git URL.

`cryozen doctor` sends a configured `GITHUB_TOKEN`/`GH_TOKEN` to `api.github.com`
...
Several categories of plugin bypass `plugins.enabled` — they're part of Cryozen's built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.cryozen-agent/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.cryozen-agent/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Cryozen tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Cryozen runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Cryozen never
materializes the prompt on another surface.

Cryozen still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Cryozen that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.cryozen-agent/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 27 lifecycle events currently accepted by `cryozen_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](./hooks.md#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Cryozen has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.cryozen-agent/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Cryozen also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Cryozen Plugin](../../developer-guide/plugins/index.md) · [Adding Tools](../../developer-guide/adding-tools.md) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](./hooks.md) · [Build a Cryozen Plugin](../../developer-guide/plugins/index.md) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Cryozen Plugin](../../developer-guide/plugins/index.md) · [Extending the CLI](../../developer-guide/extending-the-cli.md) |
| A **subcommand** for `cryozen <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](../../developer-guide/extending-the-cli.md) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `CRYOZEN_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](./tts.md#stt-custom-command-providers) · [Python plugin guide](./tts.md#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Cryozen auto-discovers the server's tools and registers them alongside built-ins. | [MCP](./mcp.md) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `cryozen skills tap add <repo>` | [Skills Hub](./skills.md#skills-hub) · [Publishing a custom tap](./skills.md#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.cryozen-agent/hooks/<name>/` | [Event Hooks](./hooks.md#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `cryozen plugins install` needed. See the **[Nix Setup guide](../../getting-started/nix-setup.md#plugins)** for full details.
...
```bash
cryozen plugins                               # unified interactive UI
cryozen plugins list                          # table: enabled / disabled / not enabled (bundled backends,
                                             # platforms and the live memory.provider count as enabled)
cryozen plugins search <term>                 # search the Cryozen plugin catalog
cryozen plugins install <name>                # install a catalog entry (repo @ reviewed pinned SHA)
cryozen plugins install user/repo             # install from Git, then prompt Enable? [y/N]
cryozen plugins install user/repo --enable    # install AND enable (no prompt)
cryozen plugins install user/repo --no-enable # install but leave disabled (no prompt)
cryozen plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
cryozen plugins remove my-plugin              # uninstall; also drops it from plugins.enabled/disabled/entries
                                             # and resets memory.provider when it was the live provider
cryozen plugins enable my-plugin              # add to allow-list
cryozen plugins disable my-plugin             # remove from allow-list + add to disabled (bundled platforms:
                                             # either spelling works, e.g. photon-platform or platforms/photon)
cryozen plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Cryozen Desktop registers the `cryozen://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
cryozen://plugin/install?catalog=NAME               # catalog entry, installs the reviewed pin
cryozen://plugin/install?repo=owner/repo            # any git repo
cryozen://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
cryozen://plugin/install?repo=owner/repo&force=1    # replace an existing install
cryozen://plugin/install?catalog=<name>             # reviewed catalog entry at its pinned commit
```

The `catalog=<name>` form is what the **Open in Cryozen Desktop** button on
...
For a `repo=` link, clicking one opens Cryozen and shows a **confirmation dialog** — the repo id,
...
Plugins → Install from Git**. Legacy `cryozen://plugin-agent/…` and
`cryozen://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `cryozen-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="cryozen://plugin/install?repo=owner/repo&enable=1">Install in Cryozen</a>
```

MCP servers have the equivalent link form — see
[Add to Cryozen link](../../reference/mcp-config-reference.md#add-to-cryozen-link).
...
When a plugin declares capabilities, `cryozen plugins install` (and
`cryozen plugins enable`) shows the list with one-line risk descriptions and
...
granted, `cryozen plugins update` surfaces the additions and asks again. New
...
`cryozen plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
cryozen plugins capabilities             # all plugins with declared/granted capabilities
cryozen plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Cryozen has not reviewed the plugin's code. Only install
...
### Discovering plugins — the Cryozen plugin catalog

`cryozen plugins search <term>` searches the **Cryozen plugin catalog** — the
curated, SHA-pinned catalog maintained in the cryozen-agent repository
(`plugin-catalog/`). Matching covers entry names, descriptions, and declared
tools:

```bash
cryozen plugins search telegram    # search the catalog
cryozen plugins browse             # browse every entry
cryozen plugins info <name>        # full details for one entry
```
...
recorded so `cryozen plugins update` can re-pin when the catalog moves:

```bash
cryozen plugins install <catalog-name>
```
...
A **plugin pack** is a declarative, shareable YAML file (`cryozen-pack.yaml`)
...
config:                                  # optional, non-secret seeds only
  cryozen-media-studio:
    default_model: flux-3
skills: []                               # declared list only (not auto-installed yet)
```

```bash
cryozen plugins pack show ./cryozen-pack.yaml     # dry-run review
cryozen plugins pack install ./cryozen-pack.yaml  # review → confirm → install
cryozen plugins pack export > cryozen-pack.yaml   # snapshot the current install
cryozen plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `cryozen plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `cryozen plugins install`.
...
provenance (installed via `cryozen plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`cryozen skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `cryozen plugins install` and `cryozen plugins update` runs a static
...
On `cryozen plugins update`, a dangerous verdict on the updated tree
...
- **Documentation prose** (`README.md`, `AGENTS.md`, `docs/**/*.md`, `.txt`,
  ...
  **own** install directory (`rm -rf "$HOME/.cryozen-agent/plugins/<name>"`) is a
...
Running `cryozen plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `cryozen plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Cryozen reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Cryozen rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Cryozen can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Cryozen drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Cryozen checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Cryozen's existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
