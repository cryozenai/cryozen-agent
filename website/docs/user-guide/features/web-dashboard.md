...
# Cryozen Web Dashboard

The web dashboard is a browser-based UI for managing your Cryozen Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
```bash
# Custom port
cryozen dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
cryozen dashboard --host 0.0.0.0

# Start without opening browser
cryozen dashboard --no-open
```
...
child with the selected profile's `CRYOZEN_HOME`, so the conversation runs
...
processes (manage them via `cryozen -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `cryozen-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.cryozen-agent/cryozen-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.cryozen-agent/cryozen-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `cryozen dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `cryozen dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.cryozen-agent` is nearly full
...
- Node.js (same requirement as `cryozen --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.cryozen-agent/cryozen-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.cryozen-agent/.env
ExecStart=/path/to/venv/bin/python -m cryozen_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.cryozen-agent/.env` containing:

```bash
CRYOZEN_DASHBOARD_BASIC_AUTH_USERNAME=admin
CRYOZEN_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
CRYOZEN_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `cryozen config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Cryozen instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `cryozen profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.cryozen-agent/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `cryozen skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.

### MCP

Manage [MCP](./mcp) servers without the CLI. The same `mcp_servers`
block in `config.yaml` that `cryozen mcp` reads from.
...
**Catalog:** browse the reviewed MCP servers (the bundled `optional-mcps/`
...
`cryozen mcp catalog` / `cryozen mcp install` use.
...
Connect Cryozen to any messaging platform from the browser — full parity with
`cryozen setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.cryozen-agent/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page. On a system-scope install (`cryozen gateway install --system`) the dashboard runs the restart under `sudo -n`, so the dashboard user needs passwordless sudo; without it the request fails immediately instead of reporting a restart that the CLI then refuses.

### System

A consolidated administration panel for installation-wide operations:

- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Cryozen versions, CPU core count + utilization, memory, disk usage of the Cryozen home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Cryozen version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `cryozen update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `cryozen curator`.
...
This re-reads `~/.cryozen-agent/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`CRYOZEN_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.cryozen-agent`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/mcp/catalog` | Browse the reviewed MCP catalog |
...
| `GET /api/cryozen/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 24h cache (the check goes through the GitHub API, never `git fetch`) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Cryozen Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page.
Two interactive providers ship in the box:

- **[Username/password](#usernamepassword-provider-no-oauth-idp)**: the simplest way to put auth on a self-hosted, on-prem, or homelab dashboard. No external identity provider. **Use it only on a trusted network or behind a VPN, not for public-internet exposure.**
- **[Self-hosted OIDC](#self-hosted-oidc-provider)**: bring your own identity provider via standard OpenID Connect (Authentik, Keycloak, Zitadel, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). This is the provider for any dashboard reachable over the public internet, and the recommended path for an internet-facing [remote Cryozen Desktop connection](#connecting-cryozen-desktop-to-a-remote-backend).

You can also register your own provider from a plugin (see [Custom providers](#custom-providers)).
...
|-------|-----------|----------|
| `cryozen dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `cryozen dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OIDC |
...
Since the June 2026 hardening, `--insecure` no longer bypasses dashboard authentication: a non-loopback bind always requires an auth provider (the username/password provider or OIDC). If you want an auth-free dashboard, bind to `127.0.0.1` and reach it over an SSH tunnel or Tailscale.
:::

### Fail-closed semantics

If the gate would engage but **no** `DashboardAuthProvider` is registered (no bundled provider configured, no custom plugin), `cryozen dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `cryozen dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Cryozen doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds), or cancel and configure an [OIDC provider](#self-hosted-oidc-provider). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
The username/password provider is intended for self-hosted / on-prem / homelab dashboards on a **trusted network**, or reachable only over a **VPN**. It protects a single shared credential with no external identity provider, MFA, or per-user accounts behind it, so it is **not suitable for exposing a dashboard directly to the public internet**. For an internet-facing dashboard, use a [self-hosted OIDC](#self-hosted-oidc-provider) or [custom OAuth](#custom-providers) provider instead.
:::

#### Configuration

Like the other bundled providers, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback operators are unaffected.
...
|---------|-----------|-------|
| `CRYOZEN_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `CRYOZEN_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `CRYOZEN_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `CRYOZEN_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `CRYOZEN_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.cryozen-agent/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
CRYOZEN_DASHBOARD_BASIC_AUTH_USERNAME=admin
CRYOZEN_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
...
```bash
cryozen dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** with no per-IDP code. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like the username/password provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback dashboards.
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/cryozen/   # required
      client_id: cryozen-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `CRYOZEN_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `CRYOZEN_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `CRYOZEN_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-cryozen.json` — it defines a `cryozen` realm, a **public PKCE client** (`cryozen-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "cryozen-dashboard",
      "name": "Cryozen Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-cryozen.json:/opt/keycloak/data/import/realm-cryozen.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/cryozen/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/cryozen`). The admin console is at
...
```bash
export CRYOZEN_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/cryozen"
export CRYOZEN_DASHBOARD_OIDC_CLIENT_ID="cryozen-dashboard"
export CRYOZEN_DASHBOARD_PUBLIC_URL="http://localhost:9119"
cryozen dashboard --host 0.0.0.0 --port 9119 --no-open
```

`CRYOZEN_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → cryozen-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `CRYOZEN_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/cryozen"
  trusted_proxies:
    - "172.20.0.5"
```
...
first; without one, Cryozen fails closed at startup. This prevents the local SPA
...
Cryozen always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
cryozen dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `CRYOZEN_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `CRYOZEN_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
OAuth providers (the bundled self-hosted OIDC provider and any custom OAuth plugin) use the authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a sign-in button per registered provider → `/auth/login?provider=<name>`.
3. Server stashes PKCE state in a short-lived cookie and redirects the user to the identity provider's authorization endpoint.
4. User authenticates with the identity provider and lands at `/auth/callback?code=…&state=…`.
5. Server exchanges the code at the provider's token endpoint, verifies the returned token against the provider's JWKS, and sets the `cryozen_session_at` cookie (plus `cryozen_session_rt` when the provider issues a refresh token).
6. User is redirected to `/` (or to the original deep-link path via the `next=` query parameter).

The session cookie lives for the access token's TTL.
When the provider issued a refresh token, the dashboard refreshes the session transparently; otherwise the SPA's fetch wrapper detects the 401 envelope on expiry and full-page-navigates back to `/login` to re-run the flow.

### Cookies set

| Name | Lifetime | Notes |
|------|----------|-------|
| `cryozen_session_at` | Access token TTL | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `cryozen_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `cryozen_session_rt` | Long-lived | Rotating refresh token; written only when the provider issues one |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$CRYOZEN_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers

To plug in another OAuth provider (e.g. an IdP that does not speak standard OIDC), create a plugin that registers a `DashboardAuthProvider`:

```python
# ~/.cryozen-agent/plugins/dashboard-auth-myidp/__init__.py
from cryozen_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): the hosting orchestrator provisions a per-agent secret via `CRYOZEN_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
Cryozen Desktop can drive a Cryozen backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateways → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Cryozen Desktop](../desktop.md) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine (a VPS, a public host, anything internet-facing), the recommended provider is [self-hosted OIDC](#self-hosted-oidc-provider) against an identity provider you run. The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
```bash
# 1. Set the dashboard login credentials in ~/.cryozen-agent/.env (secrets file, 0600).
cat >> ~/.cryozen-agent/.env <<'EOF'
CRYOZEN_DASHBOARD_BASIC_AUTH_USERNAME=admin
CRYOZEN_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
cryozen dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `CRYOZEN_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.cryozen-agent/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.cryozen-agent/.env`, so the credentials are in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/cryozen` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `CRYOZEN_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `CRYOZEN_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `CRYOZEN_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |
...
- **Signed out on every restart** — set `CRYOZEN_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `cryozen_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `cryozen update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `cryozen dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Cryozen Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Cryozen Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
| **Cryozen Blue** (`cryozen-blue`) | Light mode with vivid blue accents on a cream canvas |
