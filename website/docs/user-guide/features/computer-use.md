...
Cryozen Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Cryozen integration. It
...
**Fresh installs already have the driver.** The Cryozen installer
...
- **`cryozen tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
cryozen computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `cryozen computer-use
status` to verify the install.

Already have cua-driver? Cryozen reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `cryozen update`, and the first
`computer_use` call of a session, Cryozen checks the local version and
...
selected with `CRYOZEN_CUA_DRIVER_CMD` stays
under your control, so Cryozen reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Cryozen autodetection is a
planned cua-driver follow-up, so currently point Cryozen at that directory or
...
interface. The built-in toolset provides Cryozen actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `cryozen computer-use doctor` (CuaDriver, `com.trycua.driver`, in every permission mode — the driver daemon always launches through `CuaDriver.app`). |
...
```
cryozen -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.cryozen-agent/config.yaml`.

## Permission modes and logged-in browser profiles

Cryozen maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Cryozen session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Cryozen approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Cryozen risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.cryozen-agent/cua-manifest.yaml
```
...
for the format). Cryozen launches a private runtime with
...
identity instead of resetting with every Cryozen build), and Cryozen verifies
...
resetting or closing the Cryozen session, cancellation cleanup, or process exit
closes that transport session. Cryozen also stops private runtimes that it
launched for bounded or unrestricted access. One Cryozen
...
## `cryozen computer-use doctor` — your first triage stop

`cryozen computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ cryozen computer-use doctor
...
`cryozen computer-use status` prints the same dead-daemon line and exits 1.
...
Cryozen run declares a public cua-driver **session name** (something like
`cryozen-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Cryozen disables it automatically where it is a known failure
...
Cryozen keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Cryozen-side `computer_use` workflow and action vocabulary. For
...
The command links the pack into `~/.cryozen-agent/skills/cua-driver` (Cryozen is one
...
These are **platform deep dives, not duplicates of the Cryozen skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Cryozen
...
image capture also saves a bounded, shareable copy under Cryozen's image cache
...
Cryozen applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.cryozen-agent/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Cryozen applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Cryozen agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Cryozen agent itself at High integrity (launch from an
  elevated terminal); otherwise target non-elevated windows.
- **Windows: `cryozen computer-use doctor` fails with "Access is denied"
  while the tool works.** A cua-driver installed under
  `C:\Program Files\WindowsApps` cannot be executed by the Cryozen venv
  ...
  `CRYOZEN_CUA_DRIVER_CMD` to a copy outside `WindowsApps`.
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Cryozen warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Cryozen from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Cryozen passes the
...
```
CRYOZEN_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
CRYOZEN_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Cryozen disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Cryozen sets
...
When it's on, `cryozen computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Cryozen at a binary you built from source instead
of the published release. Cryozen resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`CRYOZEN_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Cryozen testing (see notes).
...
### Option B — point Cryozen straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`CRYOZEN_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
CRYOZEN_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
CRYOZEN_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Cryozen is using your build

- `cryozen computer-use status` prints the resolved binary path and
  version.
- `cryozen computer-use doctor` confirms the binary is reachable and
...
- **Cryozen spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Cryozen YOLO, Cryozen instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Cryozen
  change needed either way.
- **Local builds skip the version check.** Cryozen warns when the
...
**First action when anything's off: run `cryozen computer-use doctor`.**
...
Run `cryozen computer-use install` to fetch the cua-driver binary, or
run `cryozen tools` and enable the Computer Use toolset.

**Clicks seem to have no effect** — Capture and verify. A modal you
didn't see may be blocking input. Dismiss it with `escape` or the close
button.

**macOS: System Settings shows CuaDriver ON, but `cryozen computer-use
...
cryozen computer-use permissions grant
```
...
Wayland without an XWayland bridge. `cryozen computer-use doctor` will
...
- **Cryozen-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Cryozen `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Cryozen autodetection is a
  planned follow-up; currently point Cryozen at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Cryozen among them)
