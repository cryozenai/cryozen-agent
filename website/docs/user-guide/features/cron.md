...
Schedule tasks to run automatically with natural language or cron expressions. Cryozen exposes cron management through a single `cronjob_manage` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Cryozen itself through the `cronjob_manage` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the main agent model from `cryozen model`.

- **Per-job pin** — a job that carries its own model. Set it to a specific model via the dashboard, `cryozen cron create/edit --model … --provider …`, or by editing `~/.cryozen-agent/cron/jobs.json`; or **lock in the current main model** with `cryozen cron create/edit --pin` (the agent's `cronjob_manage` tool can do this too with `pinned=true`, but only when you ask it to). `--unpin` (`pinned=false`) releases the lock. The agent cannot point a job at a *different* model — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`cryozen config set cron.model <name>`) and switching your chat model with `cryozen model` or `/model` never touches your cron fleet.
- **Main agent model** — when neither of the above is set, a job runs on whatever `cryozen model` / `/model` is set to **at the moment it fires**. Change your main model and every unpinned job follows on its next run.
...
For unattended runs, prefer a provider whose credentials do not expire mid-schedule: an API key, or an OAuth login that refreshes automatically. See [AI Providers](../../integrations/providers.md).
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `cryozen cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob_manage` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Cryozen disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
cryozen cron create "every 2h" "Check server status"
cryozen cron create "every 1h" "Summarize new feed items" --skill blogwatcher
cryozen cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Cryozen normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Cryozen will use the unified `cronjob_manage` tool internally.
...
A missing-credential verdict names the profile and `CRYOZEN_HOME` the scheduler
read, e.g. `provider credential missing: No Codex credentials stored … [profile
'default', CRYOZEN_HOME /opt/data]`. When an interactive session with "the same"
credential works, compare that path with the shell's `CRYOZEN_HOME`: a gateway
started without the shell's environment (Docker `HOME` vs `CRYOZEN_HOME`, a
...
Or: `cryozen config set cron.preflight false`

## Moving unpinned jobs to a new global default

An unpinned job follows the main agent model, so `cryozen model` moves your cron fleet with it.
When you want a job to *stay* on a model:

```bash
cryozen cron edit <job_id> --pin                                   # lock the current main model onto one job
cryozen cron edit <job_id> --provider <provider> --model <model>   # pin an explicit model
cryozen cron edit <job_id> --unpin                                 # follow the main model again
cryozen config set cron.model <model>                              # every unpinned job, without touching chat
```

`cryozen cron list` and the `cronjob_manage` tool report `pinned` per job.

## Skill-backed cron jobs

A cron job can load one or more skills before it runs the prompt. Each skill loads exactly as it does from `/skill-name` in a chat session, including the `[Skill config ...]` block with its resolved `metadata.cryozen.config` values from `config.yaml`.
...
cryozen cron create "every 1d at 09:00" \
...
```bash
cryozen cron edit <job_id> --schedule "every 4h"
cryozen cron edit <job_id> --prompt "Use the revised task"
cryozen cron edit <job_id> --skill blogwatcher --skill maps
cryozen cron edit <job_id> --add-skill maps
cryozen cron edit <job_id> --remove-skill blogwatcher
cryozen cron edit <job_id> --clear-skills
```
...
```bash
cryozen cron list
cryozen cron pause <job_id_or_name>
cryozen cron resume <job_id_or_name>
cryozen cron run <job_id_or_name>
cryozen cron remove <job_id_or_name>
cryozen cron edit <job_id_or_name> [...flags]
cryozen cron status
cryozen cron tick
```
...
### Pausing everything: `cryozen pause`

`cryozen pause [--reason ...]` is the global emergency stop (`cryozen resume` lifts it). While it is engaged no scheduled cron fire starts, whichever door it arrives through: the built-in ticker skips its dispatch instead of force-firing everything that was held back. Runs already in flight are never killed, and nothing is lost: due work catches up on the first tick or sweep after `cryozen resume`. Explicit manual runs (`cryozen cron run`, the dashboard's Trigger button) are an operator override and still execute while paused.
...
```bash
cryozen cron create "every 1h" "Post the digest" --paused --paused-reason "Awaiting review"
cryozen cron resume <job_id>
```
...
```bash
cryozen gateway install     # Install as a user service
sudo cryozen gateway install --system   # Linux: boot-time system service for servers
cryozen gateway             # Or run in foreground

cryozen cron list
cryozen cron status
```

`cryozen cron status` reports whether the scheduler is alive (gateway process, ticker heartbeat, last successful tick) and the soonest scheduled run across your active jobs, ordered by actual instant even when jobs store different UTC offsets. A `next_run_at` that is already more than 15 minutes in the past is never shown as an upcoming "Next run": `cron status` prints `⚠ Next run <time> is OVERDUE — passed 7h ago but the job has not fired`, `cron list` and the in-chat `/cron list` label the row `Overdue:`, the dashboard and the Desktop cron panel (including a Bot's Routines card) show `Overdue since`, and when the scheduler has stopped ticking `status` (with the gateway down) and the dashboard Cron page also say when it last ticked. That is the signature of a scheduler that stopped ticking — restart the gateway (`cryozen gateway restart`) so the next tick picks the overdue job up, or run it right away with `cryozen cron run <id>`.

For a named profile served by the default-profile multiplexer, `cryozen cron status` names that scheduler host and reports the named profile's own heartbeat health. Missing or stale heartbeats point to `cryozen --profile default gateway restart`. `cron list` and `cron create` also warn when that heartbeat is missing or stale; `cron status` additionally checks the last successful tick and reports tick errors.

### Gateway scheduler behavior

On each tick Cryozen:

1. loads jobs from `~/.cryozen-agent/cron/jobs.json`
...
A file lock at `~/.cryozen-agent/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
The worker is the gateway's own interpreter running `python -m cron.scheduler`, with the gateway's checkout pinned on its `PYTHONPATH` (plus any entries the gateway itself was started with), so it imports the same Cryozen tree the gateway runs — regardless of the venv's editable-install mapping, the unit's `WorkingDirectory`, or `PYTHONSAFEPATH` on the host. A worker that dies before acknowledging the handoff records its own stderr tail in the job's last error and in the execution ledger, so the failing import (or whatever killed it) is named instead of a bare exit code.

### Execution history

Cryozen records each claimed cron attempt in the profile-local
`~/.cryozen-agent/cron/executions.db` before executor or provider dispatch. Attempts
...
`cryozen cron run` / `/cron run`, so a one-shot invocation with no scheduler
running heals the ledger too — Cryozen marks an abandoned attempt `unknown` only
...
Inspect recent attempts with `cryozen cron runs [job-id] --limit 20` (alias:
...
that the retained ledger records as completed, Cryozen skips that replay and
...
in a row and suggesting you fix, pause (`cryozen cron pause <job>`), or remove
it. Any successful run resets the streak, and `cryozen cron list` shows the
...
(the run is still recorded — `cryozen cron runs` and the failure streak see it,
...
```bash
cryozen cron incidents                 # list incidents (newest activity first)
cryozen cron incidents --state alerted # filter: detected | alerted | resolved | closed
cryozen cron incidents ack <id>        # acknowledge — silence this signature for good
```
...
### Fleet health check: `cryozen cron doctor`

`cryozen cron doctor` is a read-only health check over every active job. It prints grouped, per-job issues and exits `1` while any finding stands, including historical late or catch-up dispatches (`0` when no findings remain).

A successful catch-up does not clear the lateness warning; the next on-time dispatch does. A watchdog such as `cryozen cron doctor || alert` can therefore keep alerting for a full schedule interval after the host wakes, even if the catch-up succeeds.
...
- script missing, not a file, or resolving outside `CRYOZEN_HOME/scripts`,
...
`cryozen cron incidents` (durable failure records) and `cryozen cron runs`
...
| `"local"` | Save to local files only (`~/.cryozen-agent/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `cryozen cron list` shows it in yellow as
`delivery_failed: <reason>`, `cryozen cron doctor` reports it as a delivery
...
- `bot-chat:<profile>` targets another profile **on the same machine**. Names are validated against `cryozen profile list` when the job is created; profiles on other gateways or machines can never be targeted, so same-named profiles across machines are unambiguous.
...
in `cryozen cron list`:

```
⚠ Delivery UNVERIFIED: adapter acked slack:C0123456 without message_id/raw_response
```

and in `cryozen cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.cryozen-agent/cron/output/`), but no message is sent to the delivery target.
...
The run is then recorded as failed (`last_status`, failure streak, `cryozen cron runs` and `cryozen cron incidents`
all reflect it) and the failure notice is delivered like any other failed run. The full response is still saved
under `~/.cryozen-agent/cron/output/` for triage. The marker is strict: mentioning or quoting `[CRON_FAILURE]` anywhere
...
Or set the `CRYOZEN_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
Or set the `CRYOZEN_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$CRYOZEN_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other Cryozen-managed secrets are **not** inherited by cron scripts.
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.cryozen-agent/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.cryozen-agent/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.cryozen-agent/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.cryozen-agent/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Cryozen reads Job 1's most recent output from `~/.cryozen-agent/cron/output/{job1_id}/*.md`
...
From the CLI: `cryozen cron create "every 6h" "Scan for news" --continuity`, and `cryozen cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
This is separate from `last_delivery_error` (delivery), which can correctly be empty when the agent itself failed.

For a connection failure, inspect the run document under `cron/output/<job_id>/` in the active
Cryozen home. Its `## Error` section includes the chained traceback, with credential patterns
...
`cryozen cron list` as `⚠ late` / `⚠ catch-up after missed fire`.
...
Or run `cryozen config set cron.catch_up_missed false`. With this opt-out, a recurring
...
This does not change one-shot expiry or resume behavior. There is no per-job override.
...
Runtimes that can't receive detached results (one-shot `cryozen -z`, `cryozen
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `cryozen tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `cryozen tools` cron-platform config wins; otherwise Cryozen falls back to the built-in defaults. If the cron-platform toolset config cannot be read at all (for example a malformed `platform_toolsets` block in `config.yaml`), the run fails with a recorded error instead of quietly running with every tool — check `cryozen cron list` / `cryozen cron doctor`. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.
...
If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Cryozen should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.cryozen-agent/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.cryozen-agent/scripts/.feed-changed.last"
...
```bash
#!/bin/bash
# ~/.cryozen-agent/scripts/flag-ready.sh
if test -f ~/.cryozen-agent/cache/scratch/new-data-ready; then
  rm -f ~/.cryozen-agent/cache/scratch/new-data-ready
  echo '{"wakeAgent": true}'
else
  echo '{"wakeAgent": false}'
fi
...
```python
#!/usr/bin/env python
# ~/.cryozen-agent/scripts/new-rows.py
import json, sqlite3
...
:::tip
Cryozen's own `~/.cryozen-agent/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

The `script` + `wakeAgent` gate covers SQL, file, and command triggers without a separate trigger mechanism, and a gate that decides not to wake the agent costs nothing.
...
Jobs are stored in `~/.cryozen-agent/cron/jobs.json`. Output from job runs is saved to `~/.cryozen-agent/cron/output/{job_id}/{timestamp}.md`.

Job definitions are plain JSON on disk: they survive `cryozen update`, gateway restarts, and machine reboots. A job that was mid-run during a restart is marked `unknown` in the execution ledger — it is not automatically retried, but the job's next scheduled tick fires normally. See [Execution history](#execution-history) for details.

:::tip
Ask the agent to manage jobs through the `cronjob_manage` tool, `cryozen cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `CRYOZEN_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Cryozen resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
