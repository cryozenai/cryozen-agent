...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Cryozen's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Cryozen instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
cryozen memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `cryozen` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Automatically generated Cryozen titles (`derived` or `llm`), including lineage titles for Desktop branches, are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions without a manual mapping.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Cryozen syncs them back at session init.
...
The `cryozen honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `cryozen memory setup honcho` (or run `cryozen memory setup` and pick it from the list); the `cryozen honcho` subcommand then appears on the next invocation.

```bash
cryozen memory setup honcho    # Configure Honcho directly (works before activation)
cryozen honcho status          # Connection status, config, and key settings
cryozen honcho setup           # Redirects to `cryozen memory setup` (post-activation alias)
cryozen honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
cryozen honcho peer            # Show or update peer names + dialectic reasoning level
cryozen honcho mode            # Show or set recall mode (hybrid/context/tools)
cryozen honcho tokens          # Show or set token budget for context and dialectic
cryozen honcho identity        # Seed or show the AI peer's Honcho identity
cryozen honcho sync            # Sync Honcho config to all existing profiles
cryozen honcho peers           # Show peer identities across all profiles
cryozen honcho sessions        # List known Honcho session mappings
cryozen honcho map             # Map current directory to a Honcho session name
cryozen honcho enable          # Enable Honcho for the active profile
cryozen honcho disable         # Disable Honcho for the active profile
cryozen honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `cryozen honcho`

If you previously used the standalone `cryozen honcho setup`:
...
No re-login or re-setup needed. Run `cryozen memory setup` and select "honcho" — the wizard detects your existing config.
