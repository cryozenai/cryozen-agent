...
`x_search` is read-only public X discovery: current discussion, reactions, claims, profiles, threads, and synthesized answers with citations.
It cannot post, reply, like, send DMs, upload media, or delete anything, and an `x_search` answer is never evidence that an X account changed state.
Any state-changing X action needs a direct X API integration, and must be confirmed by that API's response.
...
|------------|--------|-------|
| **SuperGrok / X Premium+ OAuth** | Browser login at `accounts.x.ai`, refreshed automatically | `cryozen auth add xai-oauth` — see [xAI Grok OAuth (SuperGrok / X Premium+)](../../guides/xai-grok-oauth.md) |
| **`XAI_API_KEY`** (preferred) | Paid xAI API key | Set in `~/.cryozen-agent/.env` |
...
Auto-enables when xAI credentials (OAuth token or `XAI_API_KEY`) are present. Disable explicitly via `cryozen tools` → Search → x_search if you don't want this.
...
If the next user request is "reply to the best one" or "like that post", `x_search` cannot do it: it remains a discovery tool, and a write needs a direct X API integration.

## Troubleshooting

### "No xAI credentials available"

The tool surfaces this when both auth paths fail. Either set `XAI_API_KEY` in `~/.cryozen-agent/.env` or run `cryozen auth add xai-oauth` and complete the browser login. Then restart your session so the agent re-reads the tool registry.
...
1. **Toolset not enabled.** Run `cryozen tools` and confirm `🐦 X (Twitter) Search` is checked.
2. **No xAI credentials.** The check_fn returns False, so the schema stays hidden. Run `cryozen auth status` to confirm xai-oauth login state, and check that `XAI_API_KEY` is set (if you're using the API-key path).
...
- **xAI index gap.** Some active accounts intermittently fail to surface in `x_search` even when they post regularly. Retry after a few minutes, or read the account's timeline through the X API directly when you need an exact handle's posts.
