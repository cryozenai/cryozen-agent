...
Cryozen runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Cryozen
...
1. Cryozen captures a baseline of current diagnostics for the file.
...
…). Cryozen auto-detects the binary on PATH or in
`<CRYOZEN_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Cryozen at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<CRYOZEN_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`cryozen lsp status` reports `installed` once `pwsh` is found; if the
...
Cryozen launches it as `laravel-lsp lsp` (stdio). There is no
auto-install recipe; `cryozen lsp status` shows `manual-only` until the
binary is found.
...
tree — Cryozen installs `typescript@6` (the last JavaScript-based line;
TypeScript 7 is the Go port and ships no `tsserver.js`) together with
the server when you run `cryozen lsp install typescript` /
`cryozen lsp install vue-language-server` or auto-install fires on first use.
...
Neovim setup) that Cryozen's generic client does not run, so it never
publishes diagnostics. If an earlier Cryozen installed 3.x, the log shows a
one-time `vue-language-server: ... 3.x` warning; delete
`<CRYOZEN_HOME>/lsp/node_modules/@vue` and `<CRYOZEN_HOME>/lsp/bin/vue-language-server*`,
then run `cryozen lsp install vue-language-server` (the recipe co-installs the
TypeScript SDK).

## CLI

```
cryozen lsp status          # service state + per-server install status
cryozen lsp list            # registry, optionally --installed-only
cryozen lsp install <id>    # eagerly install one server
cryozen lsp install-all     # try every server with a known recipe
cryozen lsp restart         # tear down running clients
cryozen lsp which <id>      # print resolved binary path
```

`cryozen lsp status` is the best starting point — it shows which
...
lsp:
  ...
  # rest of the process (until `cryozen lsp restart`); N = retried after
  ...
  #   auto    — install via npm/pip/go install into <CRYOZEN_HOME>/lsp/bin
  #   manual  — only use binaries already on PATH
  install_strategy: auto

  # Node package manager for the npm-based servers: npm (default), pnpm
  # or yarn. Installs still land in <CRYOZEN_HOME>/lsp/node_modules; a
  ...
  # in <CRYOZEN_HOME>/lsp/.yarnrc.yml. pnpm 11 blocks git-hosted transitive
  ...
  # <CRYOZEN_HOME>/lsp/.npmrc if your policy allows it.
  package_manager: npm
...
built-ins, so they can also take over an extension Cryozen already
handles.
...
give an absolute path) and `cryozen lsp status` lists it as
`installed`. A malformed entry is logged and skipped without
affecting the other servers.

## Installation locations

When `install_strategy: auto`, Cryozen installs binaries into
`<CRYOZEN_HOME>/lsp/bin/`. NPM packages land in
`<CRYOZEN_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Cryozen-owned and is
...
default the pair stays broken until `cryozen lsp restart` or process
...
are not idle: removing a Cryozen-managed worktree (`cryozen -w` session
...
exists on disk (deleted outside Cryozen). The sweep is part of the idle
...
**single process** per Cryozen process: the first Python project spawns
...
**`cryozen lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CRYOZEN_HOME>/lsp/bin/`. Run
`cryozen lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `cryozen lsp status`**
...
When `cryozen lsp status` shows a `Backend warnings` section, install
...
`~/.cryozen-agent/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.cryozen-agent/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `cryozen lsp restart` to clear the set;
