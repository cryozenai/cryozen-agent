...
Cryozen Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
cryozen memory setup      # interactive picker + configuration
cryozen memory status     # check what's active
cryozen memory off        # disable external provider
```

You can also select the active memory provider via `cryozen plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.cryozen-agent/config.yaml`:
...
When a memory provider is active, Cryozen automatically:
...
**Setup Wizard:**
```bash
cryozen memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `cryozen honcho setup` command still works (it now redirects to `cryozen memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$CRYOZEN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$CRYOZEN_HOME/honcho.json` > `~/.cryozen-agent/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/cryozenai/cryozen-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev).
...
:::tip Migrating from `cryozen honcho`
If you previously used `cryozen honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Cryozen profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Cryozen profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Cryozen profile. Host key `cryozen` → default; `cryozen.<profile>` for others. |
...
```bash
cryozen profile create coder --clone
```

`--clone` creates a `cryozen.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Cryozen profile, creates host blocks for any profile without one, inherits settings from the default `cryozen` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `cryozen memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/cryozenai/cryozen-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev).
...
cryozen memory setup    # select "openviking"
# Or manually:
cryozen config set memory.provider openviking
```

`cryozen memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.cryozen-agent/.env`, and for named profiles use
`~/.cryozen-agent/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Cryozen sends no peer ID and writes
...
Set `memory.openviking.agent: cryozen` to restore the old peer-scoped writes.
...
Cryozen sends `User-Agent: openviking-memory-cryozen/<version>` on OpenViking
...
**Setup (Platform):**
```bash
cryozen memory setup    # select "mem0" → "Platform"
# Or manually:
cryozen config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.cryozen-agent/.env
```

**Setup (OSS):**
```bash
cryozen memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
cryozen memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
cryozen memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
cryozen memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
cryozen memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.cryozen-agent/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.cryozen-agent/.env
```
...
**Config:** `$CRYOZEN_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.cryozen-agent/.env`.
...
| `user_id` | `cryozen-user` | User identifier |
| `agent_id` | `cryozen` | Agent identifier |
...
**Switching modes:** Re-run `cryozen memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
cryozen memory setup    # select "hindsight"
# Or manually:
cryozen config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.cryozen-agent/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p cryozen ui start`

**Config:** `$CRYOZEN_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `cryozen` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Cryozen Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/cryozenai/cryozen-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
cryozen memory setup    # select "holographic"
# Or manually:
cryozen config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.cryozen-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$CRYOZEN_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
cryozen memory setup    # select "retaindb"
# Or manually:
cryozen config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.cryozen-agent/.env
```
...
# Then configure Cryozen
cryozen memory setup    # select "byterover"
# Or manually:
cryozen config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$CRYOZEN_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](https://app.supermemory.ai), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
cryozen memory setup    # select "supermemory"
# Or manually:
cryozen config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.cryozen-agent/.env
```
...
Before running `cryozen memory setup`, set `base_url` in
`$CRYOZEN_HOME/supermemory.json`:
...
Then run `cryozen memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$CRYOZEN_HOME/supermemory.json`
...
| `container_tag` | `cryozen` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `cryozen-{identity}` → `cryozen-coder`) to isolate memories per Cryozen profile
...
| **Requires** | `pip install cryozen-memori` + `cryozen-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install cryozen-memori
cryozen-memori install
cryozen config set memory.provider memori
cryozen memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `cryozen-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](../profiles.md):

- **Local storage providers** (Holographic, ByteRover) use `$CRYOZEN_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$CRYOZEN_HOME/` so each profile has its own credentials
- **Cloud providers** (RetainDB) auto-derive profile-scoped project names
- **Env var providers** (OpenViking) are configured via each profile's `.env` file

## Providers Moving to the Plugin Catalog

Memory providers are moving out of the Cryozen tree into their maintainers' own repositories,
...
When a provider you have configured stops shipping with Cryozen, `cryozen update` installs its
...
`false`, in which case it prints the `cryozen plugins install <name>` one-liner instead).
