...
Cryozen can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<CRYOZEN_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Cryozen renders the sprite at full fidelity when your
...
cryozen pets list
cryozen pets list cat

# Install a pet and make it active in one step
cryozen pets install boba --select
...
## `cryozen pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `cryozen pets list [query] [--limit N]` |
| List installed pets | `cryozen pets list --installed` |
| Install a pet | `cryozen pets install <slug> [--select] [--force]` |
| Set the active pet | `cryozen pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `cryozen pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `cryozen pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `cryozen pets off` |
| Remove an installed pet | `cryozen pets remove <slug>` |
| Diagnose setup | `cryozen pets doctor` |

`cryozen pets show` flags:
...
Beyond installing pre-made pets from the gallery, Cryozen can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `cryozen pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Cryozen state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](./image-generation.md), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **OpenAI** (`gpt-image-2`, API key or Codex sign-in), **OpenRouter**, and **Krea**. OpenRouter runs a quality-first model chain by default.

- Resolution order prefers OpenAI, then OpenRouter, then Krea.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `cryozen tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `CRYOZEN_PET_IMAGE_PROVIDER` env var (e.g. `CRYOZEN_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Cryozen is
...
Run `cryozen pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Cryozen uses its own
  profile-scoped `<CRYOZEN_HOME>/pets/` instead — install through `cryozen pets`.

## See also

- The [`cryozen-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-cryozen-agent.md)
