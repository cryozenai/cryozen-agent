...
description: "Connect Cryozen Agent to external tool servers via MCP — and control exactly which MCP tools Cryozen loads"
---

# MCP (Model Context Protocol)

MCP lets Cryozen Agent connect to external tool servers so the agent can use tools that live outside Cryozen itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Cryozen to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

:::tip Coming from Claude Code?
The `mcpServers` block in your `~/.claude.json` maps to `mcp_servers` in Cryozen's `config.yaml` — and `cryozen import-agent claude-code` migrates it (along with skills and instructions) automatically. See [Import from Other Agents](../import-from-other-agents.md).
:::

## What MCP gives you

- Access to external tool ecosystems without writing a native Cryozen tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Cryozen to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.cryozen-agent/config.yaml`:
...
4. Ask Cryozen to use the MCP-backed capability.
...
Cryozen will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for reviewed MCPs

Cryozen ships a curated catalog of MCP servers that the Cryozen maintainers
have reviewed and merged. They're disabled by default — install only what you actually
want.
...
(`cryozen --tui`, a callout above the composer) and the classic CLI (a panel):
...
3. **Authorization.** For an OAuth entry the card shows the authorization link.
   Cryozen never opens the browser by itself: click **Open in browser** on the
...
4. **Save.** Cryozen saves the server configuration, the tokens and your setup
   ...
   tokens; Cryozen asks you to authorize again only when they no longer work.
5. **Tools.** Cryozen then lists the server's tools and registers them. The
...
```bash
cryozen mcp                   # interactive picker (default)
cryozen mcp catalog           # plain-text list, scriptable
cryozen mcp install deepwiki  # install a catalog entry by name
```
...
`optional-mcps/` in the cryozen-agent repo — presence in that directory means
maintainer approval. There is no community submission tier; entries are added by
merging a PR.
...
- **API key** — Cryozen prompts at install time and writes the value to
  `~/.cryozen-agent/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Cryozen points you at
  `cryozen auth <provider>` if you haven't authenticated already.
...
3. Run `cryozen mcp install n8n-official` and enter that URL when prompted.
4. Complete browser OAuth. If needed, run `cryozen mcp login n8n-official`
   or use **Authorize** on the configured server in Desktop or the dashboard.
5. Review tools with `cryozen mcp configure n8n-official`, then start a new
   session or use `/reload-mcp`.

The Cryozen backend must be able to reach the URL. n8n controls permissions
...
After credentials are configured, Cryozen probes the MCP server to list every
...
written (if not). Re-run `cryozen mcp configure <name>` once the server is
reachable to refine.
...
the cryozen-agent repo, so the maintainers have reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/cryozenai/cryozen-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Cryozen
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Cryozen`) for that entry instead of silently hiding it. Run `cryozen update`
to install the latest Cryozen when you see that.
...
from environment variables (which include everything in `~/.cryozen-agent/.env`).
...
to register, then run `cryozen mcp login <name>` and restart (or
...
pre-registered with a pinned `redirect_port`, Cryozen keeps the registered
...
same machine as the Cryozen process. For a remote host, use `cryozen mcp login`
over SSH port-forwarding.

### Updating tool selection later

```bash
cryozen mcp configure linear
```
...
MCPs are never auto-updated. Re-run `cryozen mcp install <name>` to refresh
after a Cryozen update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/cryozenai/cryozen-agent/tree/main/optional-mcps).
...
rejected), and Cryozen's bundled `github/*` skills driving the `gh` CLI are a
...
HTTP MCP servers are remote endpoints Cryozen connects to directly.
...
- you do not want Cryozen spawning a local subprocess for that integration
...
Most hosted MCP servers (Cloudflare, Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Cryozen handles discovery, client identification, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.

Cryozen identifies itself with a [Client ID Metadata Document](../../reference/mcp-config-reference.md#client-identification-cimd-and-dcr) on servers that support one, and falls back to Dynamic Client Registration on those that don't. Both are automatic; there is nothing to configure.

:::tip Figma remote MCP
Figma's hosted endpoint (`https://mcp.figma.com/mcp`) allowlists Dynamic Client Registration by **exact `client_name`** — bare `"Cryozen Agent"` 403s, while `"Claude Code"` and `"Codex"` succeed. Cryozen auto-sets `oauth.client_name: "Claude Code"` for `mcp.figma.com` so install/login works without a special trick:
...
Or: `cryozen mcp install figma`, then `cryozen mcp login figma`.
:::
...
**Google-hosted servers (Gmail, Calendar).** Google only issues a refresh token when the authorization request carries `access_type=offline`, which MCP discovery never advertises. Cryozen adds it (plus `prompt=consent`, so a repeat login is re-granted one) whenever the discovered authorization server is `accounts.google.com`, so the connection persists across restarts and works from `cryozen gateway`. Other issuers' requests are untouched.

**Remote / headless hosts.** When Cryozen runs on a different machine than your browser, the loopback callback can't reach your laptop. Ways to complete the flow:

- **Cryozen Desktop (automatic):** when you run the OAuth sign-in from the Desktop app's MCP setup UI against a remote backend, Desktop hosts the callback listener on *your* machine and relays the authorization back to the gateway automatically — no tunnel, paste, or proxy needed. Requires both the Desktop app and the backend to be up to date.
- **Paste-back (no setup):** on an interactive terminal Cryozen prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **Device-code login (no callback at all):** if the server's authorization server advertises a device authorization endpoint, run `cryozen mcp login <server> --flow device` on the machine running Cryozen. It prints a verification URL and a short code; open the URL on any device, enter the code, and Cryozen polls for approval. No browser is launched on the host and no callback listener is needed. Set `oauth.flow: device` on the server to make `login` and `reauth` use it by default. Details: [Device-code login](../../reference/mcp-config-reference.md#device-code-login-rfc-8628).
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches Cryozen on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where Cryozen expects them.
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `cryozen mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `cryozen mcp login` can list the tools and look like it worked, but every real tool call later times out. `cryozen mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `cryozen mcp login googledrive` — with the pre-registered client, Cryozen skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.cryozen-agent/config.yaml` from inside a running Cryozen session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `cryozen mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

**Need longer than 5 minutes to approve?** Set `oauth.timeout` on the server entry (seconds). `cryozen mcp login`, the dashboard and Desktop re-auth all wait `oauth.timeout` + 15 s (or the entry's `connect_timeout`, whichever is longer); a login that still runs out of time reports `Connecting to MCP server '<name>' timed out after Ns` naming both knobs instead of a blank failure line.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Cryozen passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
- `value_from: profile` sends the active Cryozen profile name, resolved once at connect time — useful when multiple profiles on one machine talk to the same server and it needs to tell them apart.
...
Cryozen reads MCP config from `~/.cryozen-agent/config.yaml` under `mcp_servers`.
...
| `cwd` | string | Working directory for the stdio server process. Default: the session working directory when one is pinned (ACP/gateway sessions, `terminal.cwd`), else the Cryozen process directory |
...
| `enabled` | bool | If `false`, Cryozen skips the server entirely |
...
For well-known MCP servers, `cryozen mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
cryozen mcp add codex --preset codex
```
...
You can pick any local name (`cryozen mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Cryozen registers MCP tools

Cryozen prefixes MCP tools so they do not collide with built-in names:
...
- Cryozen only registers resource utilities if the MCP session actually supports resource operations
- Cryozen only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Cryozen, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Cryozen skips the server completely and does not even attempt a connection.
...
You can also separately disable Cryozen-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Cryozen does not create an empty runtime MCP toolset for that server.
...
Cryozen discovers MCP servers at startup and registers their tools into the normal tool registry.
...
MCP servers can notify Cryozen when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Cryozen receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
A running messaging gateway (`cryozen gateway run`) also watches `config.yaml` on its own: within about a minute of you removing an `mcp_servers` entry or setting `enabled: false`, that server's connection is torn down; a newly added entry is connected. A server whose first connect failed (an unreachable host, or an OAuth server on a headless box that had no token yet) is retried automatically on its connect cooldown schedule (30 s, doubling up to 10 min) once you fix the cause. No restart or `/reload-mcp` needed for the edit to take effect.

**Expired OAuth tokens in the background.** The gateway, `/reload-mcp`, and the periodic self-probe of a parked server never open a browser — nobody is there to complete the flow. When a refresh token dies, the server parks with a warning in `gateway.log` and you re-authorize once with `cryozen mcp login <server>` (or the Desktop/dashboard *Authorize* button); the parked server picks the new token up on its next probe.
...
For stdio servers, Cryozen does not blindly pass your full shell environment.
...
cd ~/.cryozen-agent/cryozen-agent && uv pip install -e ".[mcp]"

node --version
npx --version
```

Then verify your config and restart Cryozen.
...
`cryozen mcp test <name>` reports what the server actually answered. When the MCP SDK can only say
`Server returned an error response` (a 4xx/5xx whose body is not a JSON-RPC error), Cryozen appends
...
endpoint. `cryozen logs --level debug` additionally shows the exact endpoint each connect attempt used.
...
Because Cryozen now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Cryozen may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Cryozen via the `sampling/createMessage` protocol. This allows an MCP server to ask Cryozen to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Cryozen as an MCP server

In addition to connecting **to** MCP servers, Cryozen can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Cryozen's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Cryozen
- You want a single MCP server that bridges to all of Cryozen's connected messaging platforms at once
- You already have a running Cryozen gateway with connected platforms
...
Add Cryozen to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Cryozen in a specific location:

```json
{
  "mcpServers": {
    "cryozen": {
      "command": "/home/user/.cryozen-agent/cryozen-agent/venv/bin/cryozen",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Cryozen-specific channel browser:
...
The MCP server includes a live event bridge that polls Cryozen's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
cryozen mcp serve              # Normal mode
cryozen mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Cryozen's session store — `~/.cryozen-agent/state.db` is the primary source, with `sessions.json` kept only as a legacy fallback. A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `cryozen send` CLI.
...
- The embedded `cryozen mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Cryozen, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
...
- [Use MCP with Cryozen](../../guides/use-mcp-with-cryozen.md)
