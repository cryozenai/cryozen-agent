...
Cryozen can optionally hand `openai/*`, `openai-codex/*` and [named custom provider](#prerequisites) turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — Cryozen becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).

This is **opt-in only**. Default Cryozen behavior is unchanged unless you flip the flag. Cryozen never auto-routes you onto this runtime.

:::tip
Not using OpenAI Codex? Run `cryozen model` to pick another provider such as OpenRouter, Anthropic, or Google Gemini.
...
- **Native Codex plugins** — Linear, GitHub, Gmail, Calendar, Canva, etc. — installed via `codex plugin` are auto-migrated and active in your Cryozen session.
- **Cryozen's richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into Cryozen for tools it doesn't have built in.
- **Memory and skill nudges keep working** — Codex's events are projected into Cryozen's message shape so the self-improvement loop sees a normal-looking transcript.
- **Your Cryozen persona rides along** — the composed system prompt (SOUL.md, MEMORY.md/USER.md, per-channel `system_prompt` overrides) is sent to the codex thread once as developer instructions when the thread starts, and Codex's built-in personality is disabled so it cannot compete with yours.
...
These ship with `codex app-server` itself — no Cryozen involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of Cryozen's `todo` tool, but managed entirely inside codex's runtime.
- **`view_image`** — load a local image file into the conversation so the model can see it.
- **`web_search`** — codex has its own built-in web search when configured. Cryozen also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, Cryozen queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. Cryozen tool callback (MCP server, registered in `~/.codex/config.toml`)

Cryozen registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
- **`image_generate`** — image generation through Cryozen's image_gen plugin chain.
- **`skill_view` / `skills_list`** — read from Cryozen's skill library.
- **`text_to_speech`** — TTS through Cryozen's configured provider.

When the model wants one of these, codex spawns the `cryozen_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Cryozen's default runtime), and the result is returned to codex like any other MCP response.

### What's NOT available on this runtime

These four Cryozen tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:

- **`delegate_task`** — spawn subagents
- **`memory`** — Cryozen's persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — Cryozen's todo store (codex's `update_plan` is the in-runtime equivalent)
...
**One thing to be aware of:** each continuation prompt is a fresh codex turn, which means codex re-evaluates command approval policy from scratch. If you're doing a long-running goal with lots of writes, expect more approval prompts than you'd see on a single in-session task. Set `default_permissions = ":workspace"` (which Cryozen does automatically when you enable the runtime) so simple workspace writes don't require prompting.

### Kanban (multi-agent worktree dispatch)

**Works on this runtime, with one subtle dependency.** The kanban dispatcher spawns each worker as a separate `cryozen chat -q` subprocess that reads the user's config — which means if `model.openai_runtime: codex_app_server` is set globally, workers also come up on the codex runtime.
...
- The Cryozen tool callback for browser_*, vision, image_gen, skills, TTS

What also works because the MCP callback exposes them:
- **`kanban_complete` / `kanban_request_review` / `kanban_request_changes` / `kanban_block` / `kanban_comment` / `kanban_heartbeat`** — the worker handoff tools. These read `CRYOZEN_KANBAN_TASK` from env (set by the dispatcher), gate access correctly, and write to the per-board SQLite DB pinned by `CRYOZEN_KANBAN_DB`. Without these in the callback, a worker on this runtime could do its task but couldn't report back, hanging until the dispatcher's timeout.
...
The kanban tools are gated by `CRYOZEN_KANBAN_TASK` env var the dispatcher sets — that var is propagated to the codex subprocess (codex inherits env) and from there to the spawned `cryozen-tools` MCP server subprocess. So the tools see the right task id and gate correctly. For Codex app-server workers, Cryozen also passes narrow app-server sandbox overrides when `CRYOZEN_KANBAN_TASK` is present: keep `workspace-write` sandboxing, add the **board DB directory plus every Kanban path the dispatcher pinned** as extra writable roots (`CRYOZEN_KANBAN_WORKSPACES_ROOT`, `CRYOZEN_KANBAN_WORKSPACE`, legacy `CRYOZEN_KANBAN_ROOT` — deduplicated, DB-dir first), and keep network disabled by default. This avoids the brittle `:danger-no-sandbox` workaround while letting `kanban_complete` / `kanban_block` update the board DB **and** letting workers write reports/artifacts under workspace mounts that live outside the DB directory (e.g. `/media/.../kanban-workspaces/...` on a separate drive).
...
|  | Cryozen default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Cryozen's own `cryozen auth add openai-codex` writes to `~/.cryozen-agent/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.

   <a id="named-custom-providers"></a>**Or: a named custom provider.** A `providers.<name>` entry in Cryozen config can use this runtime when the **same name** is defined as a Codex provider. Cryozen config:
   ...
   Cryozen sends only `model` and `modelProvider = "my-gateway"` on `thread/start`; codex resolves `base_url` and reads the key from `env_key` in its own environment. **Cryozen never forwards the API key**, so `MY_GATEWAY_API_KEY` must be present in the process environment Cryozen runs in — `~/.cryozen-agent/.env` is loaded at startup and provider credentials are inherited by the codex subprocess. Auxiliary calls (titles, compression, memory review) still use Cryozen's own `providers.my-gateway` entry.

   Caveats: the name after `custom:` is the `providers:` config key and must match the `[model_providers.<name>]` table name exactly — if it does not exist on the codex side, codex reports an unknown provider rather than silently using the Cryozen endpoint. Anonymous `provider: custom` (a bare `base_url`) is not eligible: it has no stable name to hand to codex, so it stays on Cryozen's standard runtime.

3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, Cryozen auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ```bash
   codex plugin marketplace add openai-curated
   # then via codex's TUI, install Linear / GitHub / Gmail / etc.
   ```
   Cryozen will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.

## Enabling

In a Cryozen session:
...
- Migrates user MCP servers from `~/.cryozen-agent/config.yaml` to `~/.codex/config.toml`.
...
- **Registers Cryozen's own tools as an MCP server** so the codex subprocess can call back for tools codex doesn't ship with.
...
You can also set it manually in `~/.cryozen-agent/config.yaml`:
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= Cryozen runtime)
```

If the Cryozen process cannot resolve `codex` from `PATH` — typical for gateway services,
...
`model.codex_bin` is used everywhere Cryozen spawns codex: the `/codex-runtime` availability
...
Cryozen's background self-improvement fires on counter thresholds:
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default Cryozen runtime.
...
One detail: the review fork itself needs to call Cryozen's agent-loop tools (`memory`, `skill_manage`), which require Cryozen's own dispatch. So when the parent agent is on `codex_app_server`, the review fork is **downgraded to `codex_responses`** — same OAuth credentials, same `openai-codex` provider, but talks to OpenAI's Responses API directly so Cryozen owns the loop and the agent-loop tools work. This is invisible to the user.

Net effect: enable the codex runtime and your memory + skill nudges keep firing exactly as they would otherwise.

## How approvals work

Codex requests approval before executing commands or applying patches. These get translated into Cryozen's standard "Dangerous Command" prompt:
...
For `apply_patch` (file edit) approvals, Cryozen shows a summary of what changed (`1 add, 1 update: src/new.py, src/old.py`) when codex provides the data via the corresponding `fileChange` item.

## Permission profiles

Codex has three built-in permission profiles:
- `:read-only` — no writes; every shell command requires approval
- `:workspace` — writes within the current workspace allowed without prompts (Cryozen's default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside Cryozen's managed block:

```toml
default_permissions = ":read-only"
```

(Cryozen will preserve your override on re-migration as long as it lives outside the `# managed by cryozen-agent` markers.)

## Auxiliary tasks and ChatGPT subscription token cost

When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because Cryozen's auxiliary client uses the main provider/model when no per-task override is set.
...
To route specific aux tasks to a cheaper / different model, set explicit overrides in `~/.cryozen-agent/config.yaml`:
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and Cryozen downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — Cryozen's own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.

## Editing `~/.codex/config.toml` safely

Cryozen wraps everything it manages between two marker comments:

```toml
# managed by cryozen-agent — `cryozen codex-runtime migrate` regenerates this section
default_permissions = ":workspace"
[mcp_servers.filesystem]
...
[plugins."github@openai-curated"]
...
# end cryozen-agent managed section
```

Anything **outside** that block is yours. Re-running migration (via `/codex-runtime codex_app_server`, whenever you toggle the runtime on, or `cryozen codex-runtime migrate`) replaces the managed block in place but preserves user content above and below it verbatim. This means you can:

- Add your own MCP servers Cryozen doesn't know about
...
**Same-name servers.** If your own `[mcp_servers.<name>]` table (outside the block) uses the same name as a server in Cryozen's `mcp_servers`, your table wins: Cryozen skips its projection for that name instead of emitting a second `[mcp_servers.<name>]` header (which is invalid TOML and would stop codex from starting). The migration report lists such names under "Kept N user-owned MCP server(s)". To let Cryozen manage the server, delete your table and re-run the migration. The rendered file is parsed as TOML before it replaces `config.toml`; an unparsable result is reported and the existing file is left untouched.

### Running the migration from a script

```bash
cryozen codex-runtime migrate            # rewrite the managed block for the active profile
cryozen codex-runtime migrate --dry-run  # report only, no write
cryozen codex-runtime migrate --json     # machine-readable report (migrated, preserved_user_servers, errors, …)
cryozen -p work codex-runtime migrate    # a named profile's mcp_servers
```
...
By default, Cryozen points the codex subprocess at `~/.codex/` regardless of which Cryozen profile is active. This means `cryozen -p work` and `cryozen -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.

If you want per-profile Codex isolation (separate auth, separate installed plugins, separate config), set `CODEX_HOME` explicitly per profile. The cleanest way is to point at a directory under your `CRYOZEN_HOME`:

```bash
# Inside the work profile, you might wrap cryozen:
CODEX_HOME=~/.cryozen-agent/profiles/work/codex cryozen chat
```

You'll need to re-run `codex login` once with that `CODEX_HOME` set so the OAuth tokens land in the profile-scoped location. After that, `cryozen -p work` will operate on isolated Codex state.
...
Cryozen does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
Cryozen's `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.

What translates:

| Cryozen (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
What's not migrated:
- Cryozen-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).

## Native Codex plugin migration

Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, Cryozen writes a `[plugins."<name>@openai-curated"]` block enabling it in your Cryozen session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Cryozen's codex runtime, Cryozen activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; Cryozen doesn't touch credentials.

## Cryozen tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, Cryozen registers itself as an MCP server in `~/.codex/config.toml`:

```toml
[mcp_servers.cryozen-tools]
command = "/path/to/python"
args = ["-m", "agent.transports.cryozen_tools_mcp_server"]
env = { CRYOZEN_HOME = "/your/.cryozen-agent", PYTHONPATH = "...", CRYOZEN_QUIET = "1" }
startup_timeout_sec = 30.0
tool_timeout_sec = 600.0
```

When the model calls `web_search` (or another exposed Cryozen tool), codex spawns the `cryozen_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Cryozen runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Cryozen Agent 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via Cryozen UI
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `cryozen-tools` callback)
...
- Cryozen web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)

Known limitations:

- **Cryozen auth and codex auth are separate sessions.** You need both `codex login` AND `cryozen auth add openai-codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Cryozen's `_import_codex_cli_tokens` — Cryozen won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
- **No inline patch preview in approval prompts when codex doesn't track the changeset.** Codex's `fileChange` approval params don't always carry the changeset. Cryozen caches the data from the corresponding `item/started` notification when possible, but if approval arrives before the item has streamed, the prompt falls back to whatever `reason` codex provides.
- **`fallback_providers` fail over only on quota and rate-limit failures.** When a codex app-server turn fails with a billing / usage-limit / rate-limit error, Cryozen switches to the configured [fallback provider](./fallback-providers.md) and retries the same turn on it; auth failures (`codex login` expired), turn timeouts and unknown-model errors do not fail over on this runtime and surface as the turn's error instead.
- **Prior Cryozen history is seeded only into a thread codex starts from scratch.** A codex thread that codex hands back via `thread/resume` already holds the conversation. When no resumable thread exists — the session ran on another provider before `/model` switched to openai-codex, codex could not resume the stored thread, or the running thread was retired — the new thread's `developerInstructions` carry Cryozen's system prompt followed by the session's prior turns (user and assistant text, tool names, tool-result previews; the most recent ~32K characters). When the composed prompt changes mid-session (for example `/personality` in the TUI or Desktop), the next turn retires the running thread and starts a new one carrying the updated prompt plus that same history seed.
- **The codex thread itself does survive a restart.** After each committed turn Cryozen stores the codex thread id on the session row (`codex_thread_id` in the session's `model_config`, `cryozen sessions` / `state.db`). The next agent built for that same Cryozen session — a later `/api/sessions/{id}/chat` request, or the first turn after the API server or gateway restarts — issues `thread/resume` for the stored id before `turn/start`, so the model keeps its own memory of the earlier turns (that is why no history seed is sent on resume). When codex cannot hand the thread back (its rollout was deleted, `CODEX_HOME` changed, the previous app-server was killed while still writing it), Cryozen fails closed: it drops the stored id, starts a fresh thread and shows one line — `Codex thread could not be resumed; starting a new one.` — on the status rail of the surface you are on (CLI, TUI/Desktop, messaging gateway). A `/new` session never resumes an older thread.
...
If you find a bug, [open an issue](https://github.com/cryozenai/cryozen-agent/issues) with the output of `cryozen logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.

## Architecture

```
                ┌─── Cryozen shell (CLI / TUI / gateway) ───┐
        ...
        └──────────────────────────────────┘           │
                                                        ...
                                                        ▼
        ┌──────────────────────────────────────────────────────────┐
        │  cryozen_tools_mcp_server.py (subprocess on demand)        │
        │   web_search, web_extract, browser_*, vision_analyze,    │
        │   image_generate, skill_view, skills_list, text_to_speech│
        └──────────────────────────────────────────────────────────┘
```

For protocol details, see the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
