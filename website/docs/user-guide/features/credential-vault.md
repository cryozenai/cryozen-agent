...
fills the password through Cryozen. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Cryozen generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `cryozen vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Cryozen
...
in, Cryozen picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Cryozen hands the master password to the manager's
...
Prefer not to use a detected manager? `cryozen vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `cryozen vault add`), bound to the checkout site,
...
- **CLI**: `cryozen vault list`, `cryozen vault add`, `cryozen vault rm <handle>`,
  `cryozen vault sources`.

Items live encrypted under `~/.cryozen-agent/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `cryozen chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Cryozen: not in
