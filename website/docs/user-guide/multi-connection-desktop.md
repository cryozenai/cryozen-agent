...
# Connecting Desktop to Many Cryozen Instances

Register every Cryozen backend you own — the local runtime, remote gateways on
your LAN or VPS, and SSH hosts — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Cryozen gateway…"**) deep-links straight to
...
list of Cryozen gateways. Its intro says it plainly: *"Manage this device and
every Cryozen gateway it can reach through remote or SSH connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Cryozen runtime managed by this app." | automatic |
| **Remote gateway** | "A Cryozen gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Cryozen install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  entry; **remote** entries are deduplicated on the normalized URL (trimmed,
  trailing slashes stripped, lowercased); **SSH** entries are deduplicated on
  the normalized `user@host:port` plus remote profile.
...
3. Pick the kind: **Local**, **Remote gateway**, or **SSH**.
...
4. Fill the fields:
   ...
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `cryozen serve` backend,
     ...
     - **Authentication** — choose **Session token** or **OAuth**:
       - **Session token** — paste the dashboard session token from the
         ...
         token."*
       - **OAuth** — sign in through the gateway's own identity provider in
         your system browser; no token to paste. See
         [Desktop Native Sign-In](../guides/desktop-native-signin.md).
   - *SSH only:*
     ...
     - **Cryozen path (optional)** — full path to the `cryozen` executable on
       the remote (for example `/opt/cryozen/bin/cryozen`). Leave blank to
       auto-detect. Set it when the remote's non-interactive shell does not
       have `cryozen` on its `PATH` and **Test** reports *"Cryozen is not
...
Gateways are machines; profiles are isolated Cryozen agents
...
- With several gateways, the sidebar shows one named gateway selector. Its device,
  network, or terminal icon identifies the connection type; profile
...
- With several gateways the profile rail is a **fleet rail**: every registered
  ...
  kind glyph (device, network, terminal) — the same glyph the gateway
...
connection is registered) dispatches `cryozen update` to every eligible
...
panel, ⌘K **Update Cryozen**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `cryozen serve` is running on the remote host, the
...
- **"Update Cryozen Desktop to chat with agents on other connections"** — the
