...
`~/.cryozen-agent/config.yaml`, `~/.cryozen-agent/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/cryozen`:

```text
/etc/cryozen/
├── config.yaml     # managed config layer (wins over ~/.cryozen-agent/config.yaml)
└── .env            # managed env layer (wins over ~/.cryozen-agent/.env + shell)
```
...
The location can be relocated with the `CRYOZEN_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `CRYOZEN_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Cryozen.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export CRYOZEN_MANAGED_DIR=/opt/org/cryozen-policy
```

:::warning
A user who can set `CRYOZEN_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `cryozen doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/cryozen/config.yaml` (managed) | `/etc/cryozen/.env` (managed) |
| 2 | `~/.cryozen-agent/config.yaml` (user) | `~/.cryozen-agent/.env` (user) |
...
```bash
cryozen config        # shows a header naming the managed source + the pinned keys
cryozen doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Cryozen refuses and names the source:

```bash
$ cryozen config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/cryozen/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `cryozen config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/cryozen

# Pin some config values for every user on this machine
sudo tee /etc/cryozen/config.yaml >/dev/null <<'YAML'
model:
  provider: openrouter
security:
  redact_secrets: true
...
sudo tee /etc/cryozen/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/cryozen
sudo chmod 0644 /etc/cryozen/config.yaml /etc/cryozen/.env
```

Changes take effect on the next Cryozen start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`cryozen doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Cryozen as `root`), managed scope is advisory.
