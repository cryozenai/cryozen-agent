...
2. At startup, after `.env` loads, Cryozen runs the helper ONCE via `/bin/sh -c` and parses its stdout as a dotenv blob.
...
secrets:
  command:
    enabled: true
    command: "cat /run/user/1000/cryozen-secrets.env"
    # or any vault CLI that dumps KEY=VALUE lines:
    # command: "pass show cryozen/env"
    # command: "secret-tool lookup service cryozen-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach Cryozen's output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (Cryozen discards its stderr on purpose) |
