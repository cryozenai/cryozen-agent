...
title: "Git Worktrees"
description: "Run multiple Cryozen agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Cryozen Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Cryozen so each session has a clean, isolated working directory.

## Why Use Worktrees with Cryozen?

Cryozen treats the **current working directory** as the project root:

- CLI: the directory where you run `cryozen` or `cryozen chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.cryozen-agent/config.yaml`
...
Cryozen creates `.worktrees/my-experiment/` inside the repo (branch
`cryozen/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `cryozen-<id>`
...
git worktree add ../repo-feature feature/cryozen-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/cryozen-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Cryozen there:

```bash
cd ../repo-feature

# Start Cryozen in the worktree
cryozen
```
...
git worktree add ../repo-experiment-a feature/cryozen-a
git worktree add ../repo-experiment-b feature/cryozen-b
```
...
Each Cryozen process:

- Works on its own branch (`feature/cryozen-a` vs `feature/cryozen-b`).
...
- Cryozen checkpoint data under `~/.cryozen-agent/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Cryozen experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/cryozen-checkpoints-docs`, `feature/cryozen-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Cryozen from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `cryozen -w` (Automatic Worktree Mode)

Cryozen has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `cryozen/cryozen-<hash>`).
...
```bash
cryozen -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `cryozen -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Cryozen session its own clean checkout.
