...
sidebar_position: 30
title: "Cryozen Relay"
description: "Connect Cryozen to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Cryozen Relay (Connector)
...
Cryozen Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves an identity token from your identity provider, configured with
   `gateway.idp.token_url`; this proves which tenant you own. With
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.cryozen-agent/.env`:
...
`cryozen gateway enroll` refuses to run in managed/hosted installs — there the
...
`cryozen gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.cryozen-agent/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
| `gateway.idp.token_url` | `config.yaml` | Identity provider that enrollment/provisioning authenticates against: OAuth2 client-credentials when `gateway.idp.client_id`/`client_secret` are also set; otherwise an ambient token endpoint (plain GET returning the token, raw or `{"access_token": …}`). |
...
token. Check the `gateway.idp.*` settings (token URL, client id and secret)
and retry.
...
startup. Restart it (`cryozen gateway restart`).
