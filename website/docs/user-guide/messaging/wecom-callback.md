...
Connect Cryozen to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Cryozen supports two WeCom integration modes:
...
> Run `cryozen gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Cryozen decrypts the message, queues it for the agent
...
(Use `cryozen gateway start` only after `cryozen gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Cryozen and the token the
...
in `~/.cryozen-agent/.env` values around `=` will also break signature checks. After
fixing, restart `cryozen gateway run`.
...
Check `cryozen gateway run` logs for the bound host/port. If the adapter bound to
