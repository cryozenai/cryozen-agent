...
sidebar_position: 12
title: "Google Chat"
description: "Set up Cryozen Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Cryozen Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Cryozen
...
> Run `cryozen gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Cryozen can read (e.g.,
`~/.cryozen-agent/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `cryozen-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `cryozen-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a cryozen restart)
...
- Principal: `cryozen-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Cryozen calls
...
- **App name**: whatever you want users to see ("Cryozen" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/cryozen-chat-events`.
...
`ADDED_TO_SPACE` event that Cryozen uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Cryozen

Add the Google Chat section to `~/.cryozen-agent/.env`:

```bash
# Required
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/cryozen-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.cryozen-agent/google-chat-sa.json
...
On Docker / hosted images `/opt/cryozen/.venv` is read-only. That installer
routes through `tools.lazy_deps` into `CRYOZEN_LAZY_INSTALL_TARGET`
...
Send "hola" in the test DM. The bot posts a "Cryozen is thinking…" marker, then
...
`~/.cryozen-agent/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Cryozen is thinking…").
...
Thread support: when a user replies inside a thread, Cryozen detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Cryozen session.
...
3. Download the JSON. Move it onto the host that runs Cryozen.
4. Register the client with Cryozen (run under the profile you want it scoped to):
...
cryozen -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Cryozen home (e.g.
`~/.cryozen-agent/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.cryozen-agent/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.cryozen-agent/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Cryozen isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `cryozen gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Cryozen will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.cryozen-agent/google_chat_user_tokens/<sanitized_email>.json` (filesystem
