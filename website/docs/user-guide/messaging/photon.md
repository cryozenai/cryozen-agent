...
Connect Cryozen to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Cryozen — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Cryozen runs it in a
...
cryozen photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Cryozen Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.cryozen-agent/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.cryozen-agent/.env`
...
(device token, dashboard project id) lives in `~/.cryozen-agent/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Cryozen
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Cryozen replies with a pairing code. Approve it with:

```bash
cryozen pairing approve photon <CODE>
```

Use `cryozen pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.cryozen-agent/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.cryozen-agent/.env`):
...
By default Cryozen responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Cryozen` and
`@Cryozen agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Cryozen will reply.

## Status & troubleshooting

```bash
cryozen photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Cryozen uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run cryozen photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `cryozen photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `cryozen photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `cryozen photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Cryozen sends images, voice
  ...
  media.
- **Native polls are supported.** Cryozen sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Cryozen, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Cryozen-sent messages
...
- **Message effects are supported.** Cryozen sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `cryozen send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<cryozen-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Cryozen is set
...
| `PHOTON_MENTION_PATTERNS` | Cryozen wake words  | JSON list / comma / newline regex patterns for group mentions |
