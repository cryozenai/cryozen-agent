...
sidebar_position: 1
title: "Telegram"
description: "Set up Cryozen Agent as a Telegram bot"
---

# Telegram Setup

Cryozen Agent integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.

## Quick setup (dashboard and desktop app)

The **Messaging → Telegram** page in the [dashboard](../features/web-dashboard.md) and the [desktop app](../desktop.md) has a **Create with QR** button. Scan the code (or open the link) in Telegram; Cryozen creates the bot for you, detects your Telegram user ID, writes `TELEGRAM_BOT_TOKEN` and `TELEGRAM_ALLOWED_USERS` into your profile's `.env`, and restarts the gateway. If you prefer to create the bot yourself, follow the manual steps below.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "Cryozen Agent") — this can be anything
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_cryozen_bot`)
...
Enable `status_indicator` and Cryozen sets that short description to **Online**
...
and the next boot discards them before Cryozen ever sees them — silently, no
log, no retry.
...
Cryozen suppresses repeated Telegram `update_id` values before message batching,
...
`priority_mode` controls how your list combines with Cryozen's built-in priority list:

- `prepend`: put your commands first, then Cryozen defaults
- `append`: keep Cryozen defaults first, then your commands
...
Telegram allows up to 100 BotCommands, but large command payloads can fail. Cryozen defaults to 60 for reliability and clamps configured values to `1..100`; use `/commands` for the full command list.

### Inline command picker: search every command (no cap)

The `/` menu is capped, but Telegram's **inline mode** is not. Once enabled, type `@yourbotname` followed by a search term in any chat to get a live, searchable picker over **every** Cryozen command and installed skill — results are computed per keystroke and paginated, so nothing is ever trimmed:
...
Cryozen Agent uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
## Step 5: Configure Cryozen

### Option A: Interactive Setup (Recommended)

```bash
cryozen gateway setup
```
...
Add the following to `~/.cryozen-agent/.env`:
...
By default, Cryozen connects to Telegram using **long polling** — the gateway makes outbound requests to Telegram's servers to fetch new updates. This works well for local and always-on deployments.
...
If the fallback IP discovery path is unhealthy on your host, set `CRYOZEN_TELEGRAM_DISABLE_FALLBACK_IPS=true` to keep cold connect on the plain `api.telegram.org` path. You can also bound DNS-over-HTTPS fallback discovery with `CRYOZEN_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT` in seconds; the default is `5`.
...
You can also set it manually in `~/.cryozen-agent/.env`:
...
Voice messages you send on Telegram are automatically transcribed by Cryozen's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running Cryozen — no API key required
...
If you'd rather have the **agent itself** handle audio — for diarization, a custom transcription tool, or just archiving the recording — set `stt.enabled: false` in `~/.cryozen-agent/config.yaml`:
...
With STT disabled, the gateway still downloads the voice/audio attachment into Cryozen's audio cache, but **does not transcribe it**. The agent receives the message with a marker like:

```
[The user sent a voice message: /home/<user>/.cryozen-agent/cache/audio/<hash>.ogg]
```
...
Telegram's **public** Bot API caps `getFile` downloads at **20 MB**, so any voice note, audio file, video, or document larger than that is silently rejected by Cryozen with a "too large" reply. The documented way around this is to run a **local** [telegram-bot-api](https://github.com/tdlib/telegram-bot-api) daemon — the same server software Telegram uses, but running on your network. A local server raises the file ceiling to **2 GB** and Cryozen auto-lifts its own internal cap when it sees a custom `base_url` configured.
...
### Step 4: Point Cryozen at the local server

Add the URLs under `platforms.telegram.extra` in `~/.cryozen-agent/config.yaml`:
  ...
  telegram:
    extra:
      ...
      local_mode: true        # see Step 5 below — only set this if the bot's data
                              # directory is readable by the Cryozen process
```
...
grep -E "Using custom Telegram base_url|Using Telegram local_mode" ~/.cryozen-agent/logs/gateway.log | tail
```
...
2. **With `--local`** (set via `TELEGRAM_LOCAL=1` above): files are written to the server's filesystem and the `getFile` response returns an **absolute path** instead of an HTTP URL. The 20MB ceiling is lifted. Cryozen must then read the bytes **from disk**, not over HTTP.

To make the disk-read path work, set `local_mode: true` in the config above **and** make sure the Cryozen process can read the path the server returns. Two scenarios:

- **Same machine** — telegram-bot-api and Cryozen run on the same host. Bind-mount the data volume to a directory that Cryozen can read (e.g., `/var/lib/telegram-bot-api`), and make sure the file ownership matches. The container drops privileges to its internal `telegram-bot-api` user (uid varies by image); the simplest fix is to add `user: "<UID>:<GID>"` to the compose service so files are owned by a uid Cryozen already runs as.
- **Different machines** — the bot server runs on one host (e.g., a NAS, a separate VM) and Cryozen on another. The server's data directory must be shared with the Cryozen machine at the **same absolute path** the server reports (typically `/var/lib/telegram-bot-api`). NFS works well for this; CIFS/SMB with `uid=` mount remapping is friendlier if you don't want to deal with uid mismatches at the filesystem level.

If `local_mode: true` is set but Cryozen can't `stat` the returned file path (permissions or wrong mount), python-telegram-bot silently falls back to an HTTP `getFile` against the local server — which in `--local` mode responds with `404 Not Found`. The symptom shows up in `gateway.log` as:
...
If you see that, the cap-lift is working but the file-share isn't. Verify `ls -la /var/lib/telegram-bot-api/<TOKEN>/voice/` from the Cryozen host as the user the gateway runs as, and confirm a single file is `cat`-able without a permission error.
...
```bash
tail -f ~/.cryozen-agent/logs/gateway.log | grep -iE "telegram|cache"
```

You should see a `[Telegram] Cached user voice at /home/<user>/.cryozen-agent/cache/audio/...` line and **no** "too large" rejection. Combined with `stt.enabled: false` (above), the path to the original audio file then lands in the agent's inbound message for downstream processing.

## Group Chat Usage

Cryozen Agent works in Telegram group chats with a few considerations:
...
- In groups with multiple Cryozen bots, `telegram.exclusive_bot_mentions` keeps routing deterministic. When a message explicitly mentions one or more Telegram bot usernames, only the mentioned bot profiles process it; other Cryozen bots ignore it before reply and wake-word fallbacks run. This is enabled by default.
- Renaming the bot's `@username` in BotFather is picked up automatically — Cryozen follows the new handle for mention routing without a gateway restart. Collectible (Fragment) usernames that don't end in `bot` are supported too.
- Use `telegram.ignored_threads` to keep Cryozen silent in specific Telegram forum topics, even when the group would otherwise allow free responses or mention-triggered replies
- If `telegram.require_mention` is left unset or false, Cryozen keeps the previous open-group behavior and responds to normal group messages it can see

### Multiple Cryozen bots in one group

If you run several Cryozen profiles in the same Telegram group, create one Telegram bot token per profile and start one gateway per profile. Do not reuse the same bot token in multiple running gateways; Telegram will reject concurrent polling for the same token.
...
With this setup, a group message like `@research_bot @ops_bot summarize this` is processed by `research_bot` and `ops_bot` only. Other Cryozen bots in the group stay silent, even if the message is a reply to one of their earlier messages or would otherwise match a shared wake word.

Two Cryozen bots that answer each other's quote-replies can still loop forever with `TELEGRAM_ALLOW_BOTS=all`, because a reply to the bot always passes the `require_mention` gate. Setting `telegram.bots_require_mention: true` (env `TELEGRAM_BOTS_REQUIRE_MENTION`) closes that path: a message from another bot only triggers a response when it explicitly `@mentions` this bot, while human replies keep working unchanged.
...
Group conversation text and media captions keep every mention when the message names other participants too (`@research_bot , @ops_bot are you both listening?` reaches `research_bot` verbatim); when this bot is the only one addressed, its own handle is still stripped so short answers such as `@cryozen_bot 2` keep working. Group turns also carry the bot's own Telegram username in the per-channel context so the model can tell which retained mentions are for it. Slash commands still use the normal command-trigger cleanup.
...
cryozen gateway status
cryozen gateway stop

# named profiles
cryozen -p research gateway start
cryozen -p research gateway status
cryozen -p research gateway stop
```

For a small fixed fleet, use a shell loop or script that calls `cryozen gateway <action>` for the default profile and `cryozen -p <profile> gateway <action>` for each named profile. This is more reliable than assuming a single process-level command controls every named profile on every service manager.
...
1. **Telegram delivery:** turn off BotFather privacy mode, promote the bot to
   admin, or mention the bot directly. Cryozen cannot respond to group messages
...
3. **Cryozen authorization:** make sure the sender is listed in
...
5. **Multi-bot routing:** if a group contains several bots, make sure each
   Cryozen profile uses a unique bot token and keep `exclusive_bot_mentions`
...
Add this to `~/.cryozen-agent/config.yaml`:
...
Telegram Bot API 9.4 (February 2026) introduced **Private Chat Topics** — bots can create forum-style topic threads directly in 1-on-1 DM chats, no supergroup needed. This lets you run multiple isolated workspaces within your existing DM with Cryozen.
...
There is no "Topics" toggle in the DM chat itself — a bot DM is not a group, so the group-forum toggle described in some older guides does not apply here. Without Threaded Mode, Cryozen will log `The chat is not a forum` on startup and skip topic creation. See [Prerequisites](#prerequisites) below for the same steps with more detail.
:::

Add topics under `platforms.telegram.extra.dm_topics` in `~/.cryozen-agent/config.yaml`:
...
1. On gateway startup, Cryozen calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
...
A ChatGPT-style multi-session DM — one bot, many parallel conversations. Unlike the operator-curated `extra.dm_topics` above, this mode is **user-driven**: no config, no pre-declared topic names. The end user flips it on with `/topic`, then taps the Telegram **+** button to create as many topics as they want, each one a fully independent Cryozen session.
...
| Topic names | Chosen by operator | Chosen by user; auto-renamed to match Cryozen session title |
...
When the user first runs `/topic`, Cryozen calls `getMe` to verify both flags. If either is off, Cryozen sends a screenshot of the BotFather Threads Settings page and explains what to toggle — no activation happens until prerequisites are met.
...
4. Cryozen responds inside that topic — the topic is now a standalone session
...
When Cryozen generates a session title for a topic (via the auto-title pipeline, after the first exchange), the Telegram topic itself is renamed to match — e.g. "New Topic" becomes "Database migration plan". The rename is best-effort: failures are logged but don't break the session.
...
When this flag is on, Cryozen still generates an internal session title (used by `cryozen sessions`, the TUI, etc.) but never edits the Telegram topic name. Useful when you organise topics by hand under BotFather Threaded Mode and don't want every first reply to overwrite the title.

### `/new` inside a topic

Resets the current topic's session (new session ID, fresh history) without touching other topics. Cryozen replies with a reminder that for parallel work, creating another topic (via **All Messages**) is usually what you want.
...
This binds the current topic to an existing Cryozen session instead of starting fresh. Useful for continuing a conversation that started before topic mode was enabled. Restrictions:

- The target session must belong to the same Telegram user
- The target session must not already be bound to another topic

Cryozen confirms with the session title and replays the last assistant message for context.

To discover session IDs, send `/topic` (no argument) in the root DM — Cryozen lists the user's unlinked Telegram sessions.
...
Send `/topic off` in the root DM. Cryozen flips the row off for **this profile's** namespace, clears that profile's `(thread_id → session_id)` bindings for the chat, and the root DM reverts to a normal Cryozen chat. Existing topics in Telegram aren't deleted — they just stop being gated as independent sessions. Re-run `/topic` later to turn it back on.
...
```bash
sqlite3 ~/.cryozen-agent/state.db \
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.cryozen-agent/config.yaml`:
...
1. When a message arrives in a mapped group topic, Cryozen looks up the `chat_id` and `thread_id` in `group_topics` config
...
| Topic creation | Cryozen creates topics via API if `thread_id` is missing | Admin creates topics in Telegram UI |
...
- **Bot API 9.4 (Feb 2026):** Private Chat Topics — bots can create forum topics in 1-on-1 DM chats via `createForumTopic`. Cryozen uses this for two distinct features: operator-curated [Private Chat Topics](#private-chat-topics-bot-api-94) (config-driven, fixed topic list) and user-driven [Multi-session DM mode](#multi-session-dm-mode-topic) (activated by `/topic`, unlimited user-created topics).
- **Privacy policy:** Telegram now requires bots to have a privacy policy. Set one via BotFather with `/setprivacy_policy`, or Telegram may auto-generate a placeholder. This is particularly important if your bot is public-facing.
- **Bot API 9.5 (Mar 2026): Native streaming via `sendMessageDraft`.** Cryozen supports Telegram's native streaming-draft API as an opt-in transport for private chats. The default remains the legacy `editMessageText` path because draft previews can visibly collapse and re-render on some Telegram clients.

### Streaming transport (`gateway.streaming.transport`)

When streaming is enabled (`gateway.streaming.enabled: true`), Cryozen picks one of four transports:
...
**MarkdownV2 fallback.** When the rich path is unavailable for a message, Cryozen converts markdown to MarkdownV2. Since MarkdownV2 has no native table syntax, pipe tables are normalized:
...
### Migrating from chat IDs in `TELEGRAM_GROUP_ALLOWED_USERS`
...
When you send `/model` with no arguments in a Telegram chat, Cryozen shows an interactive inline keyboard for switching models:
...
Or in `~/.cryozen-agent/config.yaml`:
...
Or add it to `~/.cryozen-agent/.env`:

```bash
HTTPS_PROXY=http://proxy.example.com:8080
```

The proxy applies to both the primary transport and all fallback IP transports. No additional Cryozen configuration is needed — if the environment variable is set, it's used automatically.

:::note
This covers the custom fallback transport layer that Cryozen uses for Telegram connections. The standard `httpx` client used elsewhere already respects proxy env vars natively.
:::
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `cryozen gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.cryozen-agent/.env`. |
...
Configure the response timeout via `agent.clarify_timeout` in `~/.cryozen-agent/config.yaml` (default `3600` seconds). If you don't respond within the timeout, the agent unblocks with a sentinel message and adapts rather than hanging.

If Telegram cannot render the button card (the Bot API rejects it, or the send fails after its 15-second acknowledgement window), Cryozen re-asks the same question as a plain numbered-list message and your typed reply (a number or the option text) is taken as the answer. When even that cannot be delivered, the agent is released at once with `[clarify prompt could not be delivered]` instead of waiting out the timeout and mistaking the silence for you not answering.
...
Configure in `~/.cryozen-agent/config.yaml`:
...
```bash
CRYOZEN_TELEGRAM_NOTIFICATIONS=all
```
