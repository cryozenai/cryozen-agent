...
When running Cryozen inside Docker, the default `network_mode: host` gives the
...
The Cryozen [SECURITY.md](https://github.com/cryozenai/cryozen-agent/blob/main/SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ cryozen-agent │   │ cryozen-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   CRYOZEN_UID=$(id -u) CRYOZEN_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=cryozen,cryozen-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://cryozen-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](https://github.com/cryozenai/cryozen-agent/blob/main/SECURITY.md) — Cryozen trust model and vulnerability reporting
- [Docker](../docker.md) — running Cryozen in a container
- [Egress proxy](iron-proxy.md) — credential-injection firewall for the sandbox
- [docker-compose.yml](https://github.com/cryozenai/cryozen-agent/blob/main/docker-compose.yml) — default compose configuration
