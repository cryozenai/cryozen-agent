...
Cryozen Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Cryozen checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.cryozen-agent/config.yaml`:
...
| `timeout` | `300` | Seconds Cryozen waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `CRYOZEN_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `cryozen --yolo` or `cryozen chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `CRYOZEN_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `CRYOZEN_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Cryozen shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
`CRYOZEN_SUPERVISED_CHILD` marker they export.
...
the job uses KeepAlive or controls Cryozen.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Cryozen refuses to run them **regardless** of:
...
Configure the timeout in `~/.cryozen-agent/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.cryozen-agent/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.cryozen-agent/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` cryozen/gateway | Self-termination prevention |
...
The `CRYOZEN_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.cryozen-agent/config.yaml`:
...
unattended ones: a cron job, `cryozen chat -q` run or webhook session under
...
re-save it with `cryozen config edit`. Other malformed values are ignored with
a warning; they never become per-character approvals. Loading does not rewrite
your configuration file.

:::tip
Use `cryozen config edit` to review or remove patterns from your permanent allowlist.
:::

:::caution
The list is read when Cryozen starts. A pattern you remove while a session is
already running stays approved in that session until it next writes the file
(the next time you answer `always` to a prompt) or you restart Cryozen. If you
removed it for safety reasons, restart.
:::

### Mining Approval History (`cryozen approvals suggest`)
...
```bash
cryozen approvals suggest            # dry run — prints a numbered proposal
cryozen approvals suggest --apply 1,3  # merge picks into command_allowlist
cryozen approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.cryozen-agent/state.db`) for
...
Before `write_file` or `patch` touches disk, Cryozen checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `CRYOZEN_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Cryozen secret stores | `.env`, `.anthropic_oauth.json`, `auth/google_oauth.json`, Bitwarden cache (`cache/bws_cache.json`, `cache/bws_cache.enc.json`), `vault/`, `browser-profile/`, `mcp-tokens/`, `pairing/` under CRYOZEN_HOME (active profile and global root). Control files (`auth.json`, `config.yaml`, `webhook_subscriptions.json`) are read-denied but stay writable. |
...
Sensitive paths inside the safe root are still blocked — pointing `CRYOZEN_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

The `~` in the OS-credential rows means *every* home a write can land in, not just the process `HOME`: the OS user's real home, the profile home (`{CRYOZEN_HOME}/home` under `TERMINAL_HOME_MODE=profile`, containers and spawned workers, where the process `HOME` is pinned), and named accounts (`~root/.ssh/authorized_keys`). An absolute path to the real home's `~/.aws/credentials` is denied even when the agent process runs with `HOME` pointed elsewhere.

Safe-root violations return `Write denied: '…' is outside CRYOZEN_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`
...
### CRYOZEN_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/cryozenai/cryozen-agent) (`CRYOZEN_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.cryozen-agent/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.cryozen-agent/cron/jobs.json`, profile skills, or other Cryozen state outside that prefix

To allow both a workspace and Cryozen home:

```bash
export CRYOZEN_WRITE_SAFE_ROOT=/path/to/project:/home/you/.cryozen-agent
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [CRYOZEN_WRITE_SAFE_ROOT](../reference/environment-variables.md#cryozen_write_safe_root).

### Cron and other Cryozen state

Do not ask the agent to `patch` `~/.cryozen-agent/cron/jobs.json` directly. Use the `cronjob_manage` tool, [`cryozen cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Cryozen control files when write safety blocks direct edits.
...
When running the messaging gateway, Cryozen controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.cryozen-agent/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.cryozen-agent/.env to allow open access,
...
For more flexible authorization, Cryozen includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `cryozen pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.cryozen-agent/config.yaml`:
...
cryozen pairing list

# Approve a pairing code
cryozen pairing approve telegram ABC12DEF

# Revoke a user's access
cryozen pairing revoke telegram 123456789

# Clear all pending codes
cryozen pairing clear-pending
```

:::tip Docker users: run pairing commands as the `cryozen` user
The official Docker image runs the gateway as the unprivileged `cryozen` user
...
cannot read them — the approval is silently ignored.

Always pass `-u cryozen`:

```bash
docker exec -u cryozen cryozen-agent cryozen pairing approve telegram ABC12DEF
```
...
**Storage:** Pairing data is stored in `~/.cryozen-agent/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Cryozen applies strict security hardening to every container.
...
Container resources are configurable in `~/.cryozen-agent/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.cryozen-agent/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `CRYOZEN_HOME`. Skills declare these in frontmatter:
...
When loaded, Cryozen checks if these files exist in the active profile's `CRYOZEN_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Cryozen infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Cryozen infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Cryozen integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Cryozen under WSL.
...
Your own `SOUL.md` in `CRYOZEN_HOME` is treated differently: it is a file you wrote (file-tool writes to it
need your approval, and project checkouts never supply it), so a scanner hit there **does not block the
file**. Cryozen logs a warning naming the matched pattern, loads the file as usual, and `/context` lists it as
...
the file. The exception does not extend to a `SOUL.md` shipped by a profile distribution: `cryozen profile
install <git-url>` and `cryozen profile update` copy a third party's `SOUL.md` into the profile home without
...
4. **Store secrets securely** — keep API keys in `~/.cryozen-agent/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.cryozen-agent/logs/` for unauthorized access attempts
10. **Keep updated** — run `cryozen update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.cryozen-agent/.env`:
...
TERMINAL_SSH_USER=cryozen
TERMINAL_SSH_KEY=~/.ssh/cryozen_agent_key
```
...
Most third-party code Cryozen can run is gated by an explicit allow-list: general plugins need `plugins.enabled`, shell hooks need a first-use approval (or `hooks_auto_accept`), MCP servers are listed in config. One surface is deliberately different:

| Extension point | Loaded from | Loaded when | Opt-in |
|-----------------|-------------|-------------|--------|
| [Gateway event hooks](./features/hooks.md#gateway-event-hooks) | `<profile home>/hooks/<name>/` (`HOOK.yaml` + `handler.py`) | Gateway startup (`HookRegistry.discover_and_load()`), per served profile | **Placing the directory.** No `plugins.enabled` entry, no prompt; `CRYOZEN_SAFE_MODE` does not skip it. |

The gateway imports every valid hook directory in-process, with the gateway's own privileges. This is the documented contract (since `3988c3c245f`), not an oversight: the profile home is operator-owned configuration, and anyone who can write into it can already run code as you through `config.yaml` shell hooks or by editing `plugins.enabled`, so a separate consent gate for `hooks/` would add friction without moving the trust boundary. Treat the contents of `~/.cryozen-agent/hooks/` like the contents of `config.yaml` — review a `handler.py` before you place it, and include `ls ~/.cryozen-agent/hooks/` whenever you audit the rest of the profile home (the directory is not on the [protected-paths denylist](#file-write-safety), so it is ordinary writable state). Full details: [gateway hook trust model](./features/hooks.md#gateway-hook-trust).

## Supply-chain advisory checking

Cryozen ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `cryozen_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `cryozen doctor` for the full remediation.
- **`cryozen doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
cryozen doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Cryozen installs these **lazily** on first use rather than eagerly under `cryozen-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `cryozen tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `cryozen tools`.
