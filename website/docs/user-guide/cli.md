...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Cryozen Agent terminal interface — commands, keybindings, personalities, and more"
---
...
Cryozen Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
Run `cryozen setup` once to pick a model provider and paste its API key, then start chatting with `cryozen chat`.
See [AI Providers](../integrations/providers.md) for every supported provider.
:::

:::tip
Cryozen also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `cryozen --tui` — see the [TUI](tui.md) guide.
:::
...
cryozen chat --query-file prompt.txt
cryozen chat --query-file - < prompt.txt

# With a specific model
cryozen chat --model "anthropic/claude-sonnet-4"

# With a specific provider
cryozen chat --provider openrouter  # Force OpenRouter
cryozen chat --provider anthropic   # Force the native Anthropic API
...
cryozen chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
cryozen --continue             # Resume the most recent CLI session (-c)
cryozen --resume <session_id>  # Resume a specific session by ID (-r)
cryozen --resume latest        # Resume the most recent session (same as -c)
cryozen --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
cryozen chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
cryozen -w                         # Interactive mode in worktree
cryozen -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`cryozen -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
cryozen worktree list              # audit: age, size, verdict, reason per tree
cryozen worktree list --json       # machine-readable audit (trees, external trees, branches)
cryozen worktree prune             # remove safe trees + delete merged branches
cryozen worktree prune --dry-run   # show the plan without changing anything
cryozen worktree prune --older-than 7   # only reap trees idle for 7+ days
cryozen worktree prune --trees-only     # leave local branches alone
cryozen worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running cryozen session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.cryozen-agent/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `cryozen -w` for days — no longer accumulate merged scratch trees
...
The `cryozen plugins` commands manage native Cryozen plugins and portable Agent
...
cryozen plugins disable <plugin-name>
cryozen plugins update <plugin-name>
cryozen plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Cryozen currently
...
<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/cli-layout.svg')} alt="Stylized preview of the Cryozen CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Cryozen CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `CRYOZEN_YOLO_MODE` is on (either `cryozen --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Cryozen to background (Unix only). Run `fg` in the shell to resume. |
...
- **Runs on your machine, in the session working directory.** With the default local terminal backend `!pwd` matches what the agent would see. A remote or sandboxed `terminal.backend` (`ssh`, `docker`, …) is **not** used for `!` commands — they always run on the host where Cryozen itself runs, so `!hostname` names your machine while the agent's `terminal` tool names the backend. Ask the agent (or open a shell on the target) to run something *inside* the backend. Path completion in the composer, by contrast, does follow the configured backend and lists the target's filesystem.
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.cryozen-agent/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Cryozen supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Cryozen replies |
...
```bash
cryozen -s cryozen-agent-dev,github-auth
cryozen chat -s github-pr-workflow -s github-auth
```

Cryozen loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
`--ignore-rules` (equivalently `CRYOZEN_IGNORE_RULES=1`) skips auto-load together with AGENTS.md, SOUL.md, `.cursorrules` and memory injection; explicit `-s` skills still load. The setting is profile-scoped: each profile's `config.yaml` controls its own list.

## Skill Slash Commands

Every installed skill in `~/.cryozen-agent/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.cryozen-agent/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Cryozen..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Cryozen also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Cryozen — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Cryozen is working, Cryozen prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Cryozen to the background — just like any terminal process. The shell prints a confirmation:

```
Cryozen Agent has been suspended. Run `fg` to bring Cryozen Agent back.
```
...
Resume this session with:
  cryozen --resume 20260225_143052_a1b2c3
...
```bash
cryozen --continue                          # Resume the most recent CLI session
cryozen -c                                  # Short form
cryozen -c "my project"                     # Resume a named session (latest in lineage)
cryozen --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
cryozen --resume "refactoring auth"         # Resume by title
cryozen --resume latest                     # Resume the most recent session (same as -c)
cryozen --resume latest --in ./my-project   # Latest session for ./my-project's workspace
cryozen -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `cryozen sessions rename <id> <title>` from the command line. Use `cryozen sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Cryozen's SQLite state database under `~/.cryozen-agent/state.db`. The database keeps:
...
Cryozen immediately confirms the task and gives you back the prompt:
