...
description: "What to do when Cryozen says another process holds an old copy of the session database's write-ahead log, and what the files beside state.db are"
---

# Session storage recovery

Cryozen keeps every conversation in one SQLite file per profile, `state.db`, with two
sidecar files SQLite manages itself: `state.db-wal` (the write-ahead log) and `state.db-shm`.
Several Cryozen processes can share that file safely — the gateway, the Desktop app, the
...
> another Cryozen process still holds an old copy of the session database's write-ahead log,
> so Cryozen stopped writing to keep the file safe …
...
1. **Quit every Cryozen process on that profile.** Desktop app, gateway, dashboard, cron:

   ```bash
   cryozen gateway stop          # add -p <profile> for a named profile
   ```

   then quit the Desktop app from its menu and stop any dashboard (`cryozen dashboard --stop`)
...
2. **Ask doctor who is still holding the log.**
   ...
   ```bash
   cryozen doctor                # add -p <profile> for a named profile
   ```
...
3. **Start Cryozen again** (one process first — the gateway or the Desktop app) and send your
   message once more. Your conversation resumes from where it stopped.

## Do not

- **Do not run `cryozen doctor --fix` while the processes are running.** Doctor refuses the
...
- **Do not copy `state.db` alone.** The three files are one image. Use a snapshot
  (`cryozen backup`) or `cryozen sessions recover`, never `cp state.db somewhere/`.
...
`cryozen sessions optimize`, `cryozen sessions optimize-storage` and `cryozen sessions prune`
...
```text
Refusing `cryozen sessions optimize-storage`: another process is using ~/.cryozen-agent/state.db.
  PID 41230 (cryozen gateway run): state.db, state.db-shm, state.db-wal
  PID 41355 (cryozen serve --profile work): state.db-wal
...
Stop them first (`cryozen gateway stop`, quit the Desktop app, pause cron), then re-run.
...
| `state.db.retired-wal-<timestamp>-<pid>/` | A capture Cryozen made of the log copy a process was still holding when it refused to write, plus a `manifest.json` describing it. Forensic evidence, not a backup you restore blindly. | Keep it. If conversations from just before the incident are missing after recovery, attach the directory to a bug report; a maintainer can tell from `manifest.json` whether those frames belong on top of the current file. |
| `state.db.pre-update-emergency-<timestamp>.bak` | A snapshot the Desktop updater takes before it touches the store. | Keep it until you have used the updated app for a while. Restore only with every Cryozen process stopped: `cryozen sessions recover --source <file> --inspect-only` first. |
| `state.db.corrupt.<timestamp>.bak`, `*.malformed-backup` | Copies of a file Cryozen found damaged before it repaired or quarantined it. | Do not restore them over `state.db` — they are the same damage. Keep for a report; safe to delete once you are back to normal. |
| `state-snapshots/` | Quick snapshots `cryozen update` and `cryozen backup` take. | Restore with every Cryozen process stopped; see [`cryozen backup`](../reference/cli-commands.md#cryozen-backup). |

## When the three steps do not work

If every Cryozen process is stopped, `cryozen doctor` no longer lists a holder, and the
...
```bash
cryozen sessions recover --source ~/.cryozen-agent/state.db --inspect-only
```
