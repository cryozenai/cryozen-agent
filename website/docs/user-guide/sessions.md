...
Cryozen Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.cryozen-agent/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Cryozen stores session history so it can resume conversations, but it does not
...
Cryozen explicitly injects for that turn.
...
For example, if a user sends an image and asks Cryozen to make a meme from it,
Cryozen may inspect that image once with vision and run an image-processing
...
`cryozen sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `cryozen sessions optimize` merges FTS5 index segments and
VACUUMs the database without touching any session data. Both `optimize` and `prune` refuse
while another Cryozen process (gateway, Desktop, dashboard, cron) holds `state.db` — stop it
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`cryozen` or `cryozen chat`) |
| `oneshot` | Finite non-interactive runs: `cryozen chat --oneshot -q`, `-Q`, `cryozen -z`, and `-q` on non-TTY stdio. Hidden from the TUI, Desktop and dashboard session pickers (like `kanban` and `tool`), even when launched from inside a TUI or Desktop session — the run inherits that transport's environment but is not that conversation. Still counts as CLI history: `cryozen -c` / `--resume latest` continue the last one-shot, and `cryozen sessions list` shows it. An explicit `--source <tag>` always wins (`cryozen chat -q --source tui` is stored as `tui`). |
...
cryozen -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
cryozen -c "my project"   # → resumes "my project #3"
```
...
cryozen --resume 20250305_091523_a1b2c3d4
cryozen -r 20250305_091523_a1b2c3d4

# Resume by title
cryozen --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
cryozen --resume latest

# Or with the chat subcommand
cryozen chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `cryozen sessions list`.
...
cryozen --resume latest --in ./my-project

# Works with the TUI too
cryozen --tui --resume latest --in ./my-project
```
...
```bash
cryozen --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`cryozen sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
cryozen sessions list --workspace my-project
cryozen sessions list --workspace ~/code/cryozen-agent
```

### Conversation Recap on Resume

When you resume a session, Cryozen displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Cryozen session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.cryozen-agent/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `cryozen -r "<title>"` from the shell) and pick up where the platform left off.
...
Cryozen automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `cryozen sessions list` or `cryozen sessions browse`.
...
```bash
cryozen sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Cryozen creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`cryozen -c "my project"`), it automatically picks the most recent session in the lineage.
...
Cryozen provides a full set of session management commands via `cryozen sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
cryozen sessions list

# Filter by platform
cryozen sessions list --source telegram

# Show more sessions
cryozen sessions list --limit 50
```
...
`cryozen sessions export` is one surface for every export format, selected with `--format`:
...
cryozen sessions export backup.jsonl

# Export sessions from a specific platform
cryozen sessions export telegram-history.jsonl --source telegram

# Export a single session
cryozen sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Point at a directory (existing, or ending in /) and the file is named for you:
# ~/exports/cryozen_session_20250305_091523_a1b2c3d4.jsonl
cryozen sessions export ~/exports/ --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
cryozen sessions export backup.jsonl --redact
```

Exported files contain one JSON object per line with full session metadata and all messages.

Each record also carries a `timings` block derived from the message timestamps, so a reader of an export attached to a bug report can tell a single long model gap from many small tool round-trips without reconstructing it by hand. It holds only ids, roles, counts and durations — `wall_clock_ms`, `largest_gap_ms`, `role_counts`, `tool_calls_emitted` and per-message `intervals` — never prompt text, tool arguments or results, so it survives `--redact` unchanged. Cryozen does not persist a model/tool stopwatch, so `complete` is always `false`; when a session has no timestamped messages, `available` is `false` and `unavailable_reason` says why. The block is rebuilt on every export and ignored (and not counted toward size limits) on import.
...
cryozen sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
cryozen sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
cryozen sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
cryozen sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `cryozen-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
cryozen sessions export --format trace

# One session to a local trace file
cryozen sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
cryozen sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.cryozen-agent/session-exports`).

```bash
# Export one session to Markdown
cryozen sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
cryozen sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
cryozen sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
cryozen sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
cryozen sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
cryozen sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
cryozen sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
cryozen sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
cryozen sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
cryozen sessions pin 20250305_091523_a1b2c3d4
cryozen sessions pin 20250305 20250306

# Remove the pin
cryozen sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
cryozen sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
cryozen sessions pinned --json > pinned-sessions.json
```
...
cryozen sessions prune

# Custom age threshold — bare numbers are days
cryozen sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
cryozen sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
cryozen sessions prune --newer-than 5h

# Explicit window with absolute timestamps
cryozen sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
cryozen sessions prune --source telegram
cryozen sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
cryozen sessions prune --newer-than 5h --title "smoke test"   # title substring
cryozen sessions prune --older-than 30 --max-messages 3        # tiny sessions
cryozen sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
cryozen sessions prune --model gpt-5 --older-than 1w           # by model (substring)
cryozen sessions prune --provider openrouter --older-than 60   # by billing provider
cryozen sessions prune --branch feature/old-experiment         # by git branch
cryozen sessions prune --user 12345678 --chat-type group       # by messaging origin
cryozen sessions prune --max-tokens 500 --older-than 7         # by token usage
cryozen sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
cryozen sessions prune --newer-than 5h --dry-run

# Skip confirmation
cryozen sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `cryozen sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `cryozen sessions prune` keeps the 90-day cutoff. Every
...
`cryozen sessions archive` takes the same filters as `prune` but soft-hides
...
cryozen sessions archive --newer-than 5h

# Archive by title substring, preview first
cryozen sessions archive --title "dry run" --dry-run
cryozen sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `cryozen sessions archive` refuses to
...
`cryozen sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
cryozen sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`cryozen insights`](../reference/cli-commands.md#cryozen-insights).
...
`cryozen sessions repair-routing` finds message-bearing session rows with no
...
cryozen sessions repair-routing
...
cryozen sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
cryozen sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.cryozen-agent/state.db ~/.cryozen-agent/state.db.bak`).
...
profile. Current versions put new state in the right place; `cryozen sessions
repair-profiles` settles what is already crossed.

```bash
# Report only — every store is scanned, nothing is written
cryozen sessions repair-profiles

# Perform the repairs (stop the gateway first; a snapshot of every store is taken)
cryozen sessions repair-profiles --apply

# Machine-readable report
cryozen sessions repair-profiles --json
```
...
`cryozen profile migrate-identity <old> <new>`), and `agent:main:…` rows inside
...
`database.journal_mode: delete` only applies to databases Cryozen creates. An
...
commits, and a downgrade underneath them destroys those commits — so Cryozen
...
cryozen sessions set-journal-mode delete     # WAL -> rollback journal
cryozen sessions set-journal-mode wal        # back to WAL
cryozen sessions set-journal-mode delete --db ~/.cryozen-agent/kanban.db   # another Cryozen store
```
...
outright unless you pass `--force` after stopping every Cryozen process
...
Started a conversation in another agent CLI? You can pull it into Cryozen and
continue it here. Cryozen reads Claude Code's session logs
...
cryozen sessions import

# Limit to one tool, or point at a specific file
cryozen sessions import --from claude
cryozen sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
cryozen --resume @claude
cryozen --resume @codex
```

`cryozen sessions import` creates a new Cryozen session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `cryozen --resume <id>` command.
`--resume @claude` / `--resume @codex` show the same picker and drop you
straight into the imported conversation.

**Cryozen Desktop** has the same importer in the command palette (**Import
session**). It lists the logs on the machine the
connected backend runs on — not the computer running the app — shows a
read-only preview, and **Continue in Cryozen** copies the conversation into the
...
When Cryozen cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Cryozen uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Cryozen in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.cryozen-agent/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.cryozen-agent/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.cryozen-agent/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.cryozen-agent/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.cryozen-agent/sessions/sessions.json` is a **legacy mirror** of it,
...
`cryozen sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.cryozen-agent/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `cryozen sessions list`, the cause is
`state.db` not receiving them — run `cryozen sessions repair` and watch for a
...
`*.jsonl` files in `~/.cryozen-agent/sessions/`. They are no longer written or
read by Cryozen. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Auto-pruning (**on by default**): when `sessions.auto_prune` is `true`, ended sessions inactive for `sessions.retention_days` (default 90) are pruned at CLI/gateway/cron startup
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Cryozen process in the same `CRYOZEN_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever, turn it off in `~/.cryozen-agent/config.yaml`:
...
`cryozen sessions recover` synthesizes for orphaned messages) whose last
...
`in its tip segment`). `cryozen sessions export` still works for such sessions.
...
cryozen sessions delete <session_id>
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `cryozen sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::
