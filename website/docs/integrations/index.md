...
Cryozen Agent connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what Cryozen can do and where it can run.

:::tip Start here
If you only have time to set up one integration, set up an inference provider in [AI Providers](./providers.md).
An [OpenRouter](https://openrouter.ai) key is the simplest single-key path to hundreds of models.
:::

## AI Providers & Routing

Cryozen supports multiple AI inference providers out of the box. Use `cryozen model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](./providers.md)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Cryozen auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](../user-guide/features/mcp.md)** — Connect Cryozen to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native Cryozen tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support eight backend providers, configured via `config.yaml` or `cryozen tools`:
...
Cryozen includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](../user-guide/features/acp.md)** — Use Cryozen Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. Cryozen runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](../user-guide/features/api-server.md)** — Expose Cryozen as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use Cryozen as a backend with its full toolset.
...
Cryozen runs as a gateway bot on 27+ messaging platforms, all configured through the same `gateway` subsystem:
...
| **Slack** | [api.slack.com/apps?new_app=1](https://api.slack.com/apps?new_app=1) | The **Create New App** dialog — pick *From an app manifest* and paste the manifest `cryozen slack manifest --agent-view` generates |
...
- **[Buzz](./buzz.md)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns Cryozen as a managed ACP runtime, the `buzz-acp` relay bridge hosts a Cryozen identity server-side, or the native gateway platform joins Buzz channels with full Cryozen memory/skills/approvals/cron. The overview page compares all three.
...
- **[Plugin System](../user-guide/features/plugins.md)** — Extend Cryozen with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.cryozen-agent/plugins/`, project-local `.cryozen-agent/plugins/`, and pip-installed entry points.
- **[Build a Plugin](../developer-guide/plugins/index.md)** — Step-by-step guide for creating Cryozen plugins with tools, hooks, and CLI commands.
