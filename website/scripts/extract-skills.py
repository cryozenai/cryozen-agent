...
2. The unified Cryozen Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                cryozen_meta = metadata.get("cryozen", {})
                if isinstance(cryozen_meta, dict):
                    tags = cryozen_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
