...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/cryozenai/cryozen-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    cryozen_meta = (fm.get("metadata") or {}).get("cryozen") or {}
    tags = cryozen_meta.get("tags") or []
    related = cryozen_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `cryozen skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def prune_stale_pages(written: set[Path]) -> int:
    """Delete generated pages this run did not write.
