...
LIVE_URL = "https://cryozenai.github.io/cryozen-agent/docs/api/plugin-stars.json"
...
def _http_json(url: str, headers: dict[str, str], timeout: float = 15.0):
    req = urllib.request.Request(url, headers={"User-Agent": "cryozen-agent-docs", **headers})
...
def _graphql(query: str, token: str) -> dict:
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "cryozen-agent-docs", "Authorization": f"Bearer {token}",
