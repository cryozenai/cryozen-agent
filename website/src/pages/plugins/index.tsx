...
// Static JSON under website/static/api/ (same pattern as the Skills Hub);
// resolved against the site baseUrl with useBaseUrl() in the component.
const PLUGINS_PATH = "/api/plugins.json";
const META_PATH = "/api/plugins-meta.json";
      ...
      <div className={styles.cardInner}>
        ...
        {onPick ? (
          ...
          <a
            ...
            title="Opens the Install Plugin dialog in Cryozen Desktop at the reviewed version. No app? Use the install command below."
            onClick={(e) => e.stopPropagation()}
          >
            Open in Cryozen Desktop
          </a>
        )}

        {
          <div className={styles.cardDetail}>
            ...
            {plugin.requiresCryozen && (
              <div className={styles.metaRow}>
                <span className={styles.metaLabel}>Requires</span>
                <span className={styles.metaValue}>
                  <code>cryozen {plugin.requiresCryozen}</code>
                </span>
              </div>
            )}
            <div className={styles.metaRow}>
...
export default function PluginCatalogPage() {
  ...
  // (Cryozen desktop's Capabilities > Plugins tab) as a one-click catalog
  // picker. Site chrome is hidden via CSS and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'cryozen-plugin-pick', name, repo, sha, subdir, tier,
  //     installCmd }
  ...
  const pickPlugin = useCallback((plugin: CatalogPlugin) => {
    ...
    window.parent.postMessage(
      {
        type: "cryozen-plugin-pick",
        name: plugin.name,
        repo: plugin.repo,
        sha: plugin.sha,
        subdir: plugin.subdir || "",
        tier: plugin.tier,
        installCmd: plugin.installCommand || `cryozen plugins install ${plugin.name}`,
      },
      "*"
    );
  }, []);
  ...
  const pluginsUrl = useBaseUrl(PLUGINS_PATH);
  ...
  return (
    <Layout
      ...
      description="Give Cryozen new powers: reviewed plugins you can install in one click"
    >
      <div className={`${styles.page} ${pickerMode ? styles.pickerMode : ""}`}>
        <header className={styles.hero}>
          <div className={styles.heroGlow} />
          <div className={styles.heroContent}>
            <p className={styles.heroEyebrow}>Cryozen Agent</p>
            ...
            <p className={styles.heroSub}>
              Give Cryozen new powers. Memory, voice, messaging, browsing, Desktop panes and more,
          ...
          {!data && !loadError ? (
            ...
            <div className={styles.empty}>
              ...
              <p className={styles.emptyDesc}>
                The plugin catalog is a curated, human-reviewed list of Cryozen
