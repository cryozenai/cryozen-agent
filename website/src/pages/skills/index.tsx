        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            <div className={styles.installHint}>
              <code>{skill.installCmd || `cryozen skills install ${skill.name}`}</code>
              <CopyButton
                text={skill.installCmd || `cryozen skills install ${skill.name}`}
              />
            </div>
            {onPick ? (
              <button
...
// Static JSON under website/static/api/; resolved against the site baseUrl
// with useBaseUrl() in the component.
...
export default function SkillsDashboard() {
  ...
  // app (Cryozen desktop's Bot Mode agent editor) as a skill PICKER. Site
  // chrome is hidden via a CSS class and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'cryozen-skill-pick', name, identifier, installCmd, source }
  ...
  const pickSkill = useCallback(
    (skill: Skill) => {
      ...
      window.parent.postMessage(
        {
          type: "cryozen-skill-pick",
          name: skill.name,
          identifier: skill.identifier || skill.name,
          installCmd: skill.installCmd || `cryozen skills install ${skill.name}`,
          source: skill.source,
        },
        "*"
      );
  ...
  return (
    <Layout
      title="Skills Hub"
      description="Browse all skills and plugins available for Cryozen Agent"
    >
