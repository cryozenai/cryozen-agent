# nix/web.nix — Cryozen Web Dashboard (Vite/React) frontend build
{ cryozenNpmLib, ... }:
cryozenNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @cryozen/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../cryozen_cli/web_dist for the
