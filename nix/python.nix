...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Cryozen derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            cryozen-agent = prev.cryozen-agent.overrideAttrs (_old: {
              CRYOZEN_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $CRYOZEN_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$CRYOZEN_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        cryozen-agent = prev.cryozen-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "cryozen-agent-env" {
    cryozen-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "cryozen-agent-editable-env" {
    cryozen-agent = dependency-groups;
  };
}
