# nix/nixosModules.nix — the NixOS module for cryozen-agent
#
...
#
# Container mode: cryozen runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CRYOZEN_HOME/.env
# and read by cryozen at startup — no container recreation needed for env changes.
#
# Tool resolution: the cryozen wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.cryozen-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      cryozen-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      cryozenHome = "${cfg.stateDir}/.cryozen-agent";
      ...
      # CRYOZEN_HOME via addToSystemPackages, so they can save settings through the
      ...
      # cryozen user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "cryozen-container-entrypoint" ''
        set -eu

        CRYOZEN_UID="''${CRYOZEN_UID:?CRYOZEN_UID must be set}"
        CRYOZEN_GID="''${CRYOZEN_GID:?CRYOZEN_GID must be set}"

        # ── Group: ensure a group with GID=$CRYOZEN_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$CRYOZEN_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="cryozen"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$CRYOZEN_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$CRYOZEN_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$CRYOZEN_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$CRYOZEN_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="cryozen"
          TARGET_HOME="/home/cryozen"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$CRYOZEN_UID" -g "$CRYOZEN_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$CRYOZEN_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$CRYOZEN_UID:$CRYOZEN_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure CRYOZEN_HOME is owned by the target user.
        ...
        if [ -n "''${CRYOZEN_HOME:-}" ] && [ -d "$CRYOZEN_HOME" ]; then
          find "$CRYOZEN_HOME" \! -user "$CRYOZEN_UID" -exec chown "$CRYOZEN_UID:$CRYOZEN_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/cryozen-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/cryozen-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/cryozen ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/cryozen
          chmod 0440 /etc/sudoers.d/cryozen
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$CRYOZEN_UID" --regid="$CRYOZEN_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $CRYOZEN_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "cryozen-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        cryozen_bin=${containerDataDir}/current-package/bin/cryozen
      '';

      # Default: /var/lib/cryozen/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the cryozen group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit cryozenHome; };
    ...
    in
    {
      options.services.cryozen-agent =
        common.sharedOptions {
          defaultPackage = cryozen-agent;
          defaultPackageText = lib.literalExpression "cryozen-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/cryozen";
              description = "State directory. Contains .cryozen-agent/ subdir (CRYOZEN_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the cryozen CLI to environment.systemPackages and export
                CRYOZEN_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.cryozen-agent symlink to the service
                  stateDir. These users are automatically added to the cryozen group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.cryozen-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the cryozen CLI to system PATH and export CRYOZEN_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.cryozen-agent/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.CRYOZEN_HOME = cryozenHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.cryozen-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.cryozen-agent.workingDirectory;
                optionPath = "services.cryozen-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.cryozen-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the cryozen user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.cryozen-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed cryozen
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure cryozen is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${cryozenHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${cryozenHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."cryozen-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cryozenHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${cryozenHome} ${cfg.workingDirectory}
                  ...
                  find ${cryozenHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${cryozenHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${cryozenHome}/$_subdir"
                    chmod 2770 "${cryozenHome}/$_subdir"
                    find "${cryozenHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg cryozenHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${cryozenHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${cryozenHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${cryozenHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.cryozen-agent";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cryozenHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "cryozen-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.cryozen-agent -> stateDir/.cryozen-agent for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.cryozen-agent = {
              description = "Cryozen Agent Gateway";
              ...
              # $CRYOZEN_HOME/.env by the activation script. load_cryozen_dotenv()
              ...
              preStart = lib.mkIf lingerEnabled ''
                ...
                if [ ! -S "/run/user/$(id -u)/bus" ]; then
                  echo "cryozen-agent: no user bus at /run/user/$(id -u)/bus after 10s;" \
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.cryozen-agent = {
              description = "Cryozen Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
                "network-online.target"
              ]
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  CRYOZEN_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  CRYOZEN_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env CRYOZEN_UID="$CRYOZEN_UID" \
                    --env CRYOZEN_GID="$CRYOZEN_GID" \
                    --env CRYOZEN_HOME=${containerDataDir}/.cryozen-agent \
                    --env CRYOZEN_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/cryozen gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
