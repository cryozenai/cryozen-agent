...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      cryozen-agent = self'.packages.default;
      cryozenVenv = cryozen-agent.cryozenVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "cryozen-check";
                homeDirectory = "/home/cryozen-check";
                stateVersion = "24.11";
              };
            }
            { services.cryozen-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.cryozen-agent = programs;
              services.cryozen-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.cryozen-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.cryozen-agent);
      ...
      configKeys = pkgs.runCommand "cryozen-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${cryozenVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "cryozen-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "cryozen-build-package" { } ''
          echo "PASS: package built at ${cryozen-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "cryozen-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.CRYOZEN_TEST = "1";
                environmentFiles = [ "/run/secrets/cryozen-env" ];
                cryozenHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "cryozen" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "cryozen" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.cryozenAgentSetup.data;

            failures =
              lib.optional (names != [
                "cryozen-agent"
                "cryozen-backend"
              ]) "expected cryozen-agent + cryozen-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/cryozen gateway" (argvOf "cryozen-agent")
              ) "gateway process does not run `cryozen gateway`: ${argvOf "cryozen-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/cryozen serve" (argvOf "cryozen-backend")
              ) "backend process does not run `cryozen serve`: ${argvOf "cryozen-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "cryozen-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/cryozen-check/.cryozen-agent" (envOf n)) names
              ) "gateway and backend must share one CRYOZEN_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.CRYOZEN_HOME or null != "/home/cryozen-check/.cryozen-agent"
              ) "programs.cryozen-agent.enable must export CRYOZEN_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "cryozen-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/cryozen-check/.cryozen-agent/SOUL.md" activation
              ) "cryozenHomeFiles must install into CRYOZEN_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads CRYOZEN_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "CRYOZEN_MANAGED=home-manager" (envOf n)) names
              ) "processes must report CRYOZEN_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "cryozen-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "cryozen-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/cryozen-check";
            ...
            cases = [
              ...
              {
                name = "cryozenHomeFiles needs no directory";
                ok = accepts { cryozenHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "cryozen-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one CRYOZEN_HOME ───────────────
        # `programs.enable` exports CRYOZEN_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.cryozen-agent
        # while the services use the CRYOZEN_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/cryozen-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                cryozenHome = "/home/cryozen-check/.cryozen-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "cryozen-desktop") cfg.home.packages;
            ...
            # second Cryozen runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "cryozen-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.cryozen-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.cryozen-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one cryozen-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "CRYOZEN_HOME" != "/home/cryozen-check/.cryozen-work"
              ) "the launcher must carry CRYOZEN_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "CRYOZEN_HOME")})"
              ++ lib.optional (
                setValue "CRYOZEN_MANAGED" != "home-manager"
              ) "the launcher must report CRYOZEN_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "CRYOZEN_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "CRYOZEN_DESKTOP_CRYOZEN" != "${lib.head agentPackages}/bin/cryozen"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "CRYOZEN_DESKTOP_CRYOZEN")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "CRYOZEN_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "CRYOZEN_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "CRYOZEN_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "CRYOZEN_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "CRYOZEN_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "cryozen-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares CRYOZEN_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set CRYOZEN_HOME" wrapper
              ) "the launcher must carry CRYOZEN_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "CRYOZEN_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "CRYOZEN_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? cryozen-backend || cfg.launchd.agents ? cryozen-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "cryozen-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `cryozen`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.cryozen-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.cryozen-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "cryozen-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "cryozen-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "cryozen" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."cryozen-agent-setup".text;

            failures =
              lib.optional (names != [
                "cryozen-agent"
                "cryozen-backend"
              ]) "expected cryozen-agent + cryozen-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/cryozen gateway" (execOf "cryozen-agent")
              ) "gateway unit does not run `cryozen gateway`: ${execOf "cryozen-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/cryozen dashboard" (execOf "cryozen-backend")
              ) "backend unit does not run `cryozen dashboard`: ${execOf "cryozen-backend"}"
              ++ lib.optional (
                units.cryozen-agent.environment.CRYOZEN_HOME != units.cryozen-backend.environment.CRYOZEN_HOME
              ) "gateway and backend must share one CRYOZEN_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/cryozen/.cryozen-agent/SOUL.md" activation
              ) "cryozenHomeFiles must install into CRYOZEN_HOME";
          ...
          in
          pkgs.runCommand "cryozen-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs cryozen, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.cryozen-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/cryozen serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec cryozen directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "cryozen-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "cryozen-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec cryozen, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/cryozen dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "cryozen-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        cron-worker-user-scope =
          let
            ...
            managed = configOf { };
            gateway = managed.systemd.services.cryozen-agent;
            ...
            unmanaged = (configOf { createUser = false; }).systemd.services.cryozen-agent;
          ...
          in
          pkgs.runCommand "cryozen-cron-worker-user-scope" { } (
            if failures != [ ] then
        ...
        # $CRYOZEN_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                CRYOZEN_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "cryozen-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'CRYOZEN_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--cryozen-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "cryozen-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "cryozen-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${cryozen-agent}/bin/cryozen || (echo "FAIL: cryozen binary missing"; exit 1)
          test -x ${cryozen-agent}/bin/cryozen-agent || (echo "FAIL: cryozen-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${cryozen-agent}/bin/cryozen --version 2>&1 | grep -qi "cryozen" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "cryozen-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in cryozen cryozen-agent cryozen-acp; do
            test -x ${cryozen-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "cryozen-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking cryozen --help ==="
          ${cryozen-agent}/bin/cryozen --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${cryozen-agent}/bin/cryozen --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "cryozen-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${cryozen-agent}/share/cryozen-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${cryozen-agent}/share/cryozen-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "CRYOZEN_BUNDLED_SKILLS" ${cryozen-agent}/bin/cryozen || \
            (echo "FAIL: CRYOZEN_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: CRYOZEN_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${cryozen-agent}/share/cryozen-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${cryozen-agent}/share/cryozen-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "CRYOZEN_OPTIONAL_SKILLS" ${cryozen-agent}/bin/cryozen || \
            (echo "FAIL: CRYOZEN_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, CRYOZEN_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "cryozen-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${cryozen-agent}/share/cryozen-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${cryozen-agent}/share/cryozen-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "CRYOZEN_BUNDLED_PLUGINS" ${cryozen-agent}/bin/cryozen || \
            (echo "FAIL: CRYOZEN_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: CRYOZEN_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "cryozen-bundled-locales" { } ''
          set -e
          ...
          grep -q "CRYOZEN_BUNDLED_LOCALES" ${cryozen-agent}/bin/cryozen || \
            (echo "FAIL: CRYOZEN_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: CRYOZEN_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + CRYOZEN_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && CRYOZEN_BUNDLED_LOCALES=${cryozen-agent}/share/cryozen-agent/locales \
            ${cryozenVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with CRYOZEN_BUNDLED_LOCALES set"; exit 1)
        ...
        # CRYOZEN_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "cryozen-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${cryozen-agent}/share/cryozen-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${cryozen-agent}/share/cryozen-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "CRYOZEN_OPTIONAL_MCPS" ${cryozen-agent}/bin/cryozen || \
            (echo "FAIL: CRYOZEN_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: CRYOZEN_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${cryozen-agent}/bin/cryozen mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: cryozen mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "cryozen-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${cryozen-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${cryozen-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "CRYOZEN_TUI_DIR" ${cryozen-agent}/bin/cryozen || \
            (echo "FAIL: CRYOZEN_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: CRYOZEN_TUI_DIR set in wrapper"
        ...
        # Verify CRYOZEN_NODE is set in wrapper and points to Node 26+
        # (Cryozen pins its toolchain to Node 26 everywhere)
        cryozen-node = pkgs.runCommand "cryozen-node-version" { } ''
          set -e
          echo "=== Checking CRYOZEN_NODE in wrapper ==="
          grep -q "CRYOZEN_NODE" ${cryozen-agent}/bin/cryozen || \
            (echo "FAIL: CRYOZEN_NODE not set in wrapper"; exit 1)
          echo "PASS: CRYOZEN_NODE present in wrapper"

          CRYOZEN_NODE=$(sed -n "s/^export CRYOZEN_NODE='\(.*\)'/\1/p" ${cryozen-agent}/bin/cryozen)
          test -x "$CRYOZEN_NODE" || (echo "FAIL: CRYOZEN_NODE=$CRYOZEN_NODE not executable"; exit 1)
          echo "PASS: CRYOZEN_NODE executable at $CRYOZEN_NODE"

          NODE_MAJOR=$("$CRYOZEN_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Cryozen requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All CRYOZEN_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify CRYOZEN_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "cryozen-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(CRYOZEN_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking CRYOZEN_MANAGED guards ==="
          check_blocked "config set" ${cryozen-agent}/bin/cryozen config set model foo
          check_blocked "config edit" ${cryozen-agent}/bin/cryozen config edit
        ...
        extra-python-packages = let
          ...
          cryozenWithExtra = cryozen-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "cryozen-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${cryozenWithExtra}/bin/cryozen || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${cryozenWithExtra}/bin/cryozen || \
          ...
          if grep -q "PYTHONPATH" ${cryozen-agent}/bin/cryozen; then
        ...
        extra-dependency-groups = let
          cryozenWithGroups = cryozen-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "cryozen-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${cryozenWithGroups}"
          echo "venv: ${cryozenWithGroups.cryozenVenv}"
        ...
        messaging-variant = pkgs.runCommand "cryozen-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.cryozenVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "cryozen-config-roundtrip" {
          ...
          merge_and_load() {
            local cryozen_home="$1"
            export CRYOZEN_HOME="$cryozen_home"
            ${configMergeScript} ${nixSettings} "$cryozen_home/config.yaml"
            ${cryozenVenv}/bin/python3 -c '
import json, sys
