# nix/homeManagerModules.nix — the Home Manager module for cryozen-agent
#
# This module is the user-level equivalent of nixosModules.default. Cryozen is
...
#
# `services.cryozen-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.cryozen-agent.enable and
#                                        home.sessionVariables
#   added     programs.cryozen-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.cryozen-agent")  -> cryozenHome, set directly
#
# To use the module:
#   imports = [ cryozen-agent.homeManagerModules.default ];
#   programs.cryozen-agent = {
#     enable = true;          # the cryozen CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.cryozen-agent;
      ...
      # not reach it, and the application would open ~/.cryozen-agent while the
      # services use cryozenHome. Thus the launcher carries the value itself.
      #
      # CRYOZEN_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        CRYOZEN_HOME = cfg.cryozenHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) CRYOZEN_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        CRYOZEN_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("CRYOZEN_DESKTOP_REMOTE_URL is set
      # but CRYOZEN_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          CRYOZEN_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export CRYOZEN_DESKTOP_REMOTE_TOKEN
        else
          echo "cryozen-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "cryozen-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.cryozen-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Cryozen needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.cryozen-agent` stays the authority for the state and the
      # configuration. This module reads cryozenHome and the backend address
      # from it, and never the reverse.
      options.programs.cryozen-agent = {
        enable = lib.mkEnableOption ''
          the Cryozen Agent command line application.

          This adds `cryozen` to home.packages, and exports CRYOZEN_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.cryozen-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.cryozen-agent.package";
          description = ''
            The cryozen-agent package to install.

            The default follows `services.cryozen-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Cryozen Desktop application (Electron).

            This adds `cryozen-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Cryozen
            runtime that `package` gives, and reads the CRYOZEN_HOME of
            `services.cryozen-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Cryozen runtime with
            the usual distribution. This module gives it the Nix package
            instead, with CRYOZEN_DESKTOP_CRYOZEN. It installs no second copy
            of Cryozen, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.cryozenDesktop;
            defaultText = lib.literalExpression "config.programs.cryozen-agent.package.cryozenDesktop";
            description = ''
              The cryozen-desktop package to use.

              The default follows `package`, and thus also
              `services.cryozen-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Cryozen runtime, and this module cannot make
      ...
      options.services.cryozen-agent =
        ...
        // {
          cryozenHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.cryozen-agent";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.cryozen-agent"'';
            description = ''
              The value of CRYOZEN_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.cryozen-agent` to it.
              This module sets CRYOZEN_HOME directly. Thus an existing
              ~/.cryozen-agent continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.cryozen-work";
          };

          # `installPackage` moved to `programs.cryozen-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `cryozen` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.cryozen-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.CRYOZEN_HOME = cfg.cryozenHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # CRYOZEN_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put CRYOZEN_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `cryozen` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.cryozen-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.cryozenAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) cryozenHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.cryozen-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.cryozen-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.cryozen-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "cryozen-agent";
              };
            })

            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.cryozen-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "cryozen-backend";
              };
            })
          ]
        ))
