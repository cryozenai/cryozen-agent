...
#
# `services.cryozen-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  cryozenHome, programs.cryozen-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "cryozen-config-attrs";
    description = "Cryozen YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $CRYOZEN_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Cryozen Agent";
      ...
      package = mkOption {
        ...
        description = "The cryozen-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Cryozen configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `cryozen config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $CRYOZEN_HOME/.env. Cryozen reads that file at each start, with
          load_cryozen_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."cryozen/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Cryozen refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Cryozen reads SOUL.md and memories/ from CRYOZEN_HOME, so put those
          files in `cryozenHomeFiles`.
      ...
      cryozenHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into CRYOZEN_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Cryozen reads SOUL.md and the memory files from CRYOZEN_HOME and not
          from the working directory. Declare those files here, or Cryozen
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the cryozen plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Cryozen discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          cryozen_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as cryozen (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in cryozen-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `cryozen gateway`.";
      };
      ...
      # `cryozen serve` and `cryozen dashboard` are the same entry point,
      # cryozen_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Cryozen Desktop
      ...
      # controls an external gateway, with `cryozen gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Cryozen
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            CRYOZEN_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Cryozen Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."cryozen/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `cryozen` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.cryozen-agent.installPackage was removed. Cryozen now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.cryozen-agent.enable = ${lib.boolToString (value != false)};  # the cryozen CLI, and CRYOZEN_HOME for your shells
        programs.cryozen-agent.desktop.enable = true;  # the desktop application

      `services.cryozen-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "cryozen-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "cryozen-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "cryozen-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "cryozen-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "cryozen-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # CRYOZEN_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.cryozenHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and Cryozen make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${cryozenHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Cryozen
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${cryozenHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${cryozenHome}/config.yaml
            ${run}chmod ${modes.config} ${cryozenHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "cryozen-managed" managedSystem} ${cryozenHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${cryozenHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${cryozenHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${cryozenHome}/auth.json"
        else
          ''
            if [ ! -e ${cryozenHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${cryozenHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree cryozenHome cfg.cryozenHomeFiles}
      ...
      ${run}find ${cryozenHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "cryozen-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${cryozenHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/cryozen"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps cryozen as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "cryozen-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "cryozen-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "cryozen-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          CRYOZEN_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export CRYOZEN_DASHBOARD_SESSION_TOKEN

          if [ -z "$CRYOZEN_DASHBOARD_SESSION_TOKEN" ]; then
            echo "cryozen-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "cryozen-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "cryozen-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "cryozen-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "cryozen-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "cryozen-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "cryozen-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Cryozen Agent web dashboard and desktop backend"
    else
      "Cryozen Agent backend for Cryozen Desktop";

  # The environment that each Cryozen process needs, from either module.
  #
  # managedSystem gives the value of CRYOZEN_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Cryozen an identity and a memory, use
          ${optionPath}.cryozenHomeFiles instead. Those files go to
          CRYOZEN_HOME. Cryozen reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of CRYOZEN_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
