...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      cryozenNpmLib = self'.packages.default.passthru.cryozenNpmLib;
      ...
      cryozenAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "cryozen" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../cryozen} $out/bin/cryozen
          '')
        ...
        shellHook = ''
          ${cryozenAgentDevShellHook}
          ${cryozenNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export CRYOZEN_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Cryozen Agent dev shell in $CRYOZEN_PYTHON_SRC_ROOT"
          echo "Ready. Run 'cryozen' or 'sandbox cryozen' to start."
        '';
      };
    };
}
