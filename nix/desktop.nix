# nix/desktop.nix — Cryozen Desktop (Electron) app build + wrapper
#
# `cryozenAgent` is the fully-built `.#default` package — it ships the
# `cryozen` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `CRYOZEN_DESKTOP_CRYOZEN` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Cryozen CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # CRYOZEN_HOME and CRYOZEN_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "cryozen-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "cryozen-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = cryozenNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "cryozen-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe cryozenNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/cryozen-desktop $out/bin
    cp -r ${renderer}/* $out/share/cryozen-desktop/
    ...
    substituteInPlace $out/share/cryozen-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/cryozen-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # CRYOZEN_DESKTOP_CRYOZEN to the absolute path of the nix-built `cryozen`
    # binary so the desktop's resolver step 4 ("existing Cryozen CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/cryozen-desktop \
      --add-flags "$out/share/cryozen-desktop" \
      --set CRYOZEN_DESKTOP_CRYOZEN "${lib.getExe cryozenAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/cryozen.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../cryozen_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/cryozen-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/cryozen.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/cryozen.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Cryozen Agent";
    homepage = "https://github.com/cryozenai/cryozen-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "cryozen-desktop";
  };
}
