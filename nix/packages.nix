# nix/packages.nix — Cryozen Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./cryozen-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.cryozenTui;
        web = full.cryozenWeb;
        desktop = full.cryozenDesktop;

        update-npm-lockfile = full.cryozenNpmLib.updateNpmLockfile;
      };
    };
}
