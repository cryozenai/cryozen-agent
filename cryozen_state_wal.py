"""SQLite journal-mode and PRAGMA policy for state.db (split from cryozen_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``cryozen_state_wal.<name>``.
"""
...
from cryozen_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable
...
# line would repeat per connection). Tests clear these via ``cryozen_state_wal.<name>``.
...
def apply_wal_with_fallback(conn: sqlite3.Connection, *, db_label: str = "state.db", require_wal: bool = False) -> str:
    ...
    and ``cryozen_cli.kanban_db_connect.connect``. WAL-incompatible filesystems either raise ``OperationalError`` ("locking protocol" / "disk I/O error") or —
...
def _wal_reset_repair_hint() -> str:
    """Repair hint matching what ``cryozen update`` can actually do for this install type.

    See #75153.
    """
    try:
        from cryozen_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
        method = detect_install_method(get_project_root())
        cmd = recommended_update_command_for_method(method)
        if method in {"git", "unknown"}:
            return f"Cryozen-managed installs can repair the embedded runtime with `{cmd}`"
        return f"update the container image with `{cmd}`" if method == "docker" else cmd  # else nix/nixos
    except Exception:
        return "install a Python build bundled with SQLite 3.51.3+ (or backports 3.50.7 / 3.44.6) and restart Cryozen"
...
_ONCE_LOGS = {
    "wal_reset_bug": (_wal_reset_bug_warned_lock, "_wal_reset_bug_warned_paths", logging.WARNING,
        ...
        "%s. See `cryozen doctor`. This warning fires once per process per database."),
    ...
    "delete_overridden": (_delete_overridden_warned_lock, "_delete_overridden_warned_paths", logging.ERROR,
        ...
        "downgrade under open connections can corrupt the DB). To apply journal_mode=DELETE, stop every Cryozen "
        "process using this database and run `cryozen sessions set-journal-mode delete` (add `--db PATH` for a "
    ...
    "cross_vm_fs_existing_wal": (_cross_vm_existing_wal_warned_lock, "_cross_vm_existing_wal_warned_paths", logging.ERROR,
        ...
        "concurrent writers can silently corrupt the database. Cryozen does not live-downgrade an on-disk WAL database. "
        "Fix one of two ways: stop every Cryozen process using this database and run `cryozen sessions "
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    try:
        from cryozen_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
