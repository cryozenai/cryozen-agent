...
timeout. Config is env-only (``CRYOZEN_STARTUP_WATCHDOG=0``,
``CRYOZEN_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "CRYOZEN_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "CRYOZEN_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (cryozen_cli.main / cryozen_cli.gateway /
...
def _process_cryozen_home() -> Path:
    """CRYOZEN_HOME for diagnostic files — stdlib-only replica of the cryozen_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<CRYOZEN_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_cryozen_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``CRYOZEN_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``CRYOZEN_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
