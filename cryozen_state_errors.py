...
Shared by cryozen_state and its mixins; string predicates match wrapped RPC
...
STORAGE_RECOVERY_DOCS_URL = "https://cryozenai.github.io/cryozen-agent/docs/user-guide/session-storage-recovery"
...
_DELETED_WAL_GENERATION_MSG = (
    "FATAL: session storage stopped writing because another Cryozen process still holds a deleted "
    "state.db-wal or state.db-shm inode (an old copy of the write-ahead log). Nothing is lost: quit "
    "every Cryozen process on this profile (Desktop app, gateway, dashboard, cron), run `cryozen doctor` "
    "(it names the processes still holding the log), then start Cryozen again. Do not delete the WAL "
    "yourself and do not run `cryozen doctor --fix` while they are running. "
...
_STATE_DB_CORRUPT_MSG = (
    ...
    "Stop the gateway, then run `cryozen sessions recover --source <state.db> "
