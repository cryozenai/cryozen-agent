...
The design is **Cryozen-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
cryozen skills install official/security/unbroker
```

Then start a new Cryozen session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Cryozen session:
...
- **PII handling.** Dossiers live under the Cryozen home directory (`0600`, optionally
