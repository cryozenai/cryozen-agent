...
All per-subject data lives under PDD_DATA_DIR (default: $CRYOZEN_HOME/unbroker),
which is the same trust boundary Cryozen uses for .env and OAuth tokens.
"""
...
def cryozen_home() -> Path:
    return Path(os.environ.get("CRYOZEN_HOME") or (Path.home() / ".cryozen-agent"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else cryozen_home() / "unbroker"
