...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CRYOZEN_HOME", os.path.expanduser("~/.cryozen-agent")), "skills/security/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CRYOZEN_HOME", Path.home() / ".cryozen-agent")) / "skills" / "security" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Cryozen config paths
# ═══════════════════════════════════════════════════════════════════

CRYOZEN_HOME = Path(os.getenv("CRYOZEN_HOME", Path.home() / ".cryozen-agent"))
CONFIG_PATH = CRYOZEN_HOME / "config.yaml"
PREFILL_PATH = CRYOZEN_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Cryozen config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.cryozen-agent/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Cryozen for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Cryozen for changes to take effect.")
