...
Usage:
  ...
  <server_name> matches the files in $CRYOZEN_HOME/mcp-tokens/<server>.json etc.
...
def _cryozen_home():
    # Prefer Cryozen' own resolver (profile-safe); fall back to env then ~/.cryozen-agent.
    try:
        from cryozen_constants import get_cryozen_home
        return str(get_cryozen_home())
    except Exception:
        return os.environ.get("CRYOZEN_HOME") or os.path.expanduser("~/.cryozen-agent")


def _tokens_dir():
    return os.path.join(_cryozen_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "cryozen-diag", "version": "1.0"}},
        },
        headers={
