#!/usr/bin/env python3
"""Canvas LMS API CLI for Cryozen Agent.
...
def _check_config():
    ...
    if missing:
        cryozen_env = os.path.join(
            os.environ.get("CRYOZEN_HOME", os.path.expanduser("~/.cryozen-agent")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {cryozen_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Cryozen Agent"
    )
