...
### 1. Start order: editor first, Cryozen second

Cryozen probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Cryozen session.
...
restart the editor, restart the Cryozen session.
...
server fails to bind (Output Log) or Cryozen' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.cryozen-agent/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Cryozen

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Cryozen' connection
lives in `~/.cryozen-agent/config.yaml` via `cryozen mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Cryozen — just don't
mistake it for the Cryozen setup step.
...
explicitly warns against overlapping calls. Cryozen executes same-turn tool
...
### 9. Timeouts: Cryozen gives up before Unreal does

Cryozen' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Cryozen runs elsewhere (SSH backend,
