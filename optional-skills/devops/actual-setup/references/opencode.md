...
is typically only in `~/.cryozen-agent/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Cryozen,
...
`cryozen-custom-providers` skill, pitfall 2). Cryozen hits this because it forwards
