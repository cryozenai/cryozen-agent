...
Usage (via cron with --no-agent):
    ...
    cryozen cron create cryozen-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CRYOZEN_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name cryozen-issues --repo cryozenai/cryozen-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Cryozen .env file
(``${CRYOZEN_HOME:-~/.cryozen-agent}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Cryozen-Watcher/1.0",
    }
