...
Usage (via cron with --no-agent):
    ...
    cryozen cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CRYOZEN_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Cryozen-Watcher/1.0"})
