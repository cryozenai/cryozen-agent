...
- OpenViking server running and reachable from Cryozen

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Cryozen can identify older servers that expose the legacy status-only health
...
connection values into Cryozen, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.cryozen-agent/.env`; for a named profile use
...
OpenViking's server config is separate from Cryozen:
...
Cryozen-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Cryozen lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Cryozen sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Cryozen also sends `User-Agent: openviking-memory-cryozen/<version>` on
OpenViking requests. This standard harness identifier contains the Cryozen
...
does not ask for a peer ID. Without a configured peer, Cryozen sends neither
...
order remains environment, linked OpenViking config, then Cryozen YAML. To use
...
on the old implicit `cryozen` peer now use user memory for new writes. Without
...
Set `agent: cryozen` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `cryozen-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Cryozen
...
Use the same OpenViking profile and credentials as Cryozen for manual recovery.
...
Cryozen built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Cryozen action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Cryozen
...
file URIs, such as
`viking://user/default/peers/cryozen/memories/preferences/mem_abc123.md` (any
