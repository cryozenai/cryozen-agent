...
```bash
cryozen memory setup honcho   # configure Honcho directly (works on a fresh install)
cryozen memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
cryozen config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.cryozen-agent/.env
```

> `cryozen honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `cryozen memory setup honcho`.
...
Set `"recallSync": true` in `$CRYOZEN_HOME/honcho.json` (at the root or under
`hosts.cryozen`), or enable **Current-query recall** in the memory settings or
`cryozen honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `cryozen model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CRYOZEN_HOME/honcho.json` | Profile-local (isolated Cryozen instances) |
| 2 | `~/.cryozen-agent/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Cryozen profile: `cryozen` (default) or `cryozen_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `cryozen honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 2 | `per-session` strategy | Cryozen session ID (`20260415_a3f2b1`) |
| 3 | Manual map (`sessions` config) | `"myproject-main"` |
| 4 | Explicit `/title` command (non-automatic title) | `"refactor-auth"` |
| 5 | `per-repo` strategy | Git root directory name (`cryozen-agent`) |
...
Automatically generated Cryozen titles (`derived` or `llm`) are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions.
...
If `sessionPeerPrefix` is `true`, the user peer name is prepended: `alice-cryozen-agent`.
...
- **`per-session`** — Cryozen session ID (timestamp + hex). Every `cryozen` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Cryozen profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`cryozen`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Cryozen profile: `cryozen` (default) or `cryozen_<profile>` (e.g. `cryozen -p coder` -> host key `cryozen_coder`). Older `cryozen.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `CRYOZEN_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `cryozen-agent`) |
...
|---------|-------------|
| `cryozen memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `cryozen honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `cryozen memory setup`) |
| `cryozen honcho status` | Show resolved config for active profile |
| `cryozen honcho enable` / `disable` | Toggle Honcho for active profile |
| `cryozen honcho mode <mode>` | Change recall or observation mode |
| `cryozen honcho peer --user <name>` | Update user peer name |
| `cryozen honcho peer --ai <name>` | Update AI peer name |
| `cryozen honcho tokens --context <N>` | Set context token budget |
| `cryozen honcho tokens --dialectic <N>` | Set dialectic max chars |
| `cryozen honcho map <name>` | Map current directory to a session name |
| `cryozen honcho sync` | Create host blocks for all Cryozen profiles |
