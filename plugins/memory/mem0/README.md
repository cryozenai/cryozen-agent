...
```bash
cryozen memory setup    # select "mem0"
```
...
Behavioral settings live in `$CRYOZEN_HOME/mem0.json` (set them via `cryozen memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.cryozen-agent/.env`.
...
| `user_id` | `cryozen-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Cryozen session and call `mem0_search` — it connects to your server.
...
```bash
cryozen memory setup mem0 --mode oss \
...
```bash
cryozen memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$CRYOZEN_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.cryozen-agent/mem0_qdrant"}}
  }
}
```
...
```bash
cryozen memory setup mem0 --mode platform --api-key sk-...
```
...
ls -la ~/.cryozen-agent/mem0_qdrant
...
- Check `user_id` matches between sessions (`$CRYOZEN_HOME/mem0.json`).
