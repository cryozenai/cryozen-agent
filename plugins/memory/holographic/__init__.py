"""cryozen-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $CRYOZEN_HOME/config.yaml under plugins.cryozen-memory-store: db_path ($CRYOZEN_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from cryozen_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "cryozen-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, cryozen_home):
        """Write config to config.yaml under plugins.cryozen-memory-store."""
        # The canonical writer: config lock, managed-mode refusal, default stripping, atomic replace.
        # ``merge_existing`` keeps every other section; *cryozen_home* is the active profile already.
        from cryozen_cli.config import save_config
        save_config({"plugins": {"cryozen-memory-store": dict(values)}}, merge_existing=True)

    def get_config_schema(self):
        from cryozen_constants import display_cryozen_home
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_cryozen_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _cryozen_home = str(get_cryozen_home())
        db_path = self._config.get("db_path", _cryozen_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $CRYOZEN_HOME so paths resolve to the active profile
            db_path = db_path.replace("$CRYOZEN_HOME", _cryozen_home).replace("${CRYOZEN_HOME}", _cryozen_home)
