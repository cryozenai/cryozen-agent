...
``$CRYOZEN_HOME/plugins/<name>/``, project ``./.cryozen-agent/plugins/<name>/`` (opt-in via
CRYOZEN_ENABLE_PROJECT_PLUGINS), then ``cryozen_agent.memory_providers`` entry points.
...
ENTRY_POINTS_GROUP = "cryozen_agent.memory_providers"
# Per Cryozen home (plugin managers are per home too): pruning under one multiplexed profile must
...
def _registered_skills_for_active_home() -> dict[str, Path]:
    ...
    return _REGISTERED_MEMORY_PROVIDER_SKILLS.setdefault(cryozen_home_key(), {})

# Synthetic parent package so user-installed providers don't collide with bundled ones.
_USER_NAMESPACE = "_cryozen_user_memory"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.cryozen-agent/plugins/`` or None. Gated on CRYOZEN_ENABLE_PROJECT_PLUGINS like the
    PluginManager scan: a repo you merely ``cd`` into must not offer a memory backend."""
    try:
        from cryozen_cli.plugins import _env_enabled

        if not _env_enabled("CRYOZEN_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".cryozen-agent" / "plugins"
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``cryozen <provider>`` commands."""
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from cryozen_cli.plugins import resolve_module_origin
...
def load_memory_provider(name: str, *, register_skills: Optional[bool] = None) -> Optional["MemoryProvider"]:
    ...
    if provider_dir is not None and _explicitly_disabled(name, provider_dir):
        # The Plugins hub / `cryozen plugins disable` park a user-installed provider in
        # ``plugins.disabled``; the loader must honour it or "disabled" is a lie in the UI.
        logger.warning("Memory provider '%s' is disabled via plugins.disabled; run `cryozen plugins enable %s` "
...
def import_memory_provider_module(name: Optional[str] = None) -> bool:
    ...
    instead of a fresh native extension load. Exists so ``cryozen acp`` can pay the heavy
...
class _ProviderCollector:
    ...
    def _plugin_context(self):
        ...
        if self._context is None:
            from cryozen_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
    ...
    try:
        ...
        if cli_mod is None:
            if not _is_bundled(plugin_dir):
                # cli.py imports as _cryozen_user_memory.<name>.cli, usually before the
