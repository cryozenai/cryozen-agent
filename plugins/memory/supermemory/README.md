...
- Hosted: API key from [app.supermemory.ai](https://app.supermemory.ai)
...
Before running `cryozen memory setup`, add the local endpoint to
...
Then run `cryozen memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$CRYOZEN_HOME/supermemory.json`
...
| `container_tag` | `cryozen` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `cryozen-{identity}` → `cryozen-coder`). |
...
`https://api.supermemory.ai`. Cryozen resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: cryozen`, and document writes stamp
`metadata.sm_source: cryozen`. This is a **functional routing key, not telemetry**:
it groups Cryozen-written memories into a dedicated "Cryozen" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Cryozen profile:

```json
{
  "container_tag": "cryozen-{identity}"
}
```

For a profile named `coder`, this resolves to `cryozen-coder`. The default profile resolves to `cryozen-default`. Without `{identity}`, all profiles share the same container.
