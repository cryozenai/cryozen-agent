...
# --- Client construction (keyed / self-hosted vs keyless) ---------------------
def _wt():
...
def check_firecrawl_api_key() -> bool:
    """True when direct Firecrawl (keyed, self-hosted, or explicitly selected keyless) is usable."""
    return _get_direct_firecrawl_config() is not None


def _get_firecrawl_client() -> Any:
    """Get or create the cached Firecrawl client. A stored ``web`` selection (or a per-capability key
    naming firecrawl) with no credentials unlocks keyless cloud mode; a stored selection that still
    resolves to nothing gets the selection-naming error. Raises ValueError when unusable."""
    wt = _wt()
    ...
    if resolved is None:
        ...
        if selected is not None or _is_explicit_firecrawl_selection():
            # A per-capability key naming ANOTHER vendor is not a firecrawl selection: the other
            # capability's ladder reached firecrawl on its own (#113017).
            logger.error("Firecrawl client initialization failed: direct Firecrawl selected but "
                         "FIRECRAWL_API_KEY/FIRECRAWL_API_URL is not set.")
        ...
        raise ValueError(
            "Web tools are not configured. Set FIRECRAWL_API_KEY for cloud Firecrawl or set "
...
# --- Response shape normalization (SDK / keyless REST differ) ------------------
...
def _extract_web_search_results(response: Any) -> List[Dict[str, Any]]:
    """Search results across SDK/keyless response shapes."""
