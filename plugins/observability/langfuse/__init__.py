"""langfuse — Cryozen plugin tracing conversations, LLM calls and tool usage to Langfuse.

Activated via ``plugins.enabled``; hooks are inert without the ``langfuse`` SDK
and credentials. Env: CRYOZEN_LANGFUSE_PUBLIC_KEY / SECRET_KEY (required),
...
# Under a multiplexed profile override, one settled client (or _INIT_FAILED) per Cryozen home: the
...
def _debug(message: str) -> None:
    if _env("CRYOZEN_LANGFUSE_DEBUG").lower() in {"1", "true", "yes", "on"}:
...
def _capture_mode() -> str:
    ...
    value = _env("CRYOZEN_LANGFUSE_CAPTURE").lower()
    ...
    if not _warned_invalid_capture:
        ...
        logger.warning(
            "Langfuse plugin: invalid CRYOZEN_LANGFUSE_CAPTURE=%r, falling back "
            "to %r (valid: %s)",
...
def _settled_client() -> Any:
    ...
    if get_cryozen_home_override() is None:
        return _LANGFUSE_CLIENT
    return _LANGFUSE_CLIENT_BY_HOME.get(cryozen_home_key())
...
def _settle_client() -> Any:
    ...
    if get_cryozen_home_override() is None:
        ...
        _LANGFUSE_CLIENT_BY_HOME[cryozen_home_key()] = settled
...
def _build_client() -> Optional[Langfuse]:
    ...
    if Langfuse is None:
        logger.warning(
            ...
            "tracing is disabled. Run `cryozen tools` and configure Langfuse "
            "Observability to reinstall it."
        )
        return None

    public_key, secret_key = (_secret(f"CRYOZEN_LANGFUSE_{n}") or _secret(f"LANGFUSE_{n}") for n in ("PUBLIC_KEY", "SECRET_KEY"))
    ...
    placeholder_issues = [issue for issue in (
        _validate_langfuse_key("CRYOZEN_LANGFUSE_PUBLIC_KEY", public_key),
        _validate_langfuse_key("CRYOZEN_LANGFUSE_SECRET_KEY", secret_key),
    ) if issue]
    if placeholder_issues:
        logger.warning(
            "Langfuse plugin: credentials look like placeholders, traces will "
            "NOT be emitted (%s). Set real Langfuse keys (pk-lf-... / sk-lf-...) "
            "or unset CRYOZEN_LANGFUSE_PUBLIC_KEY / CRYOZEN_LANGFUSE_SECRET_KEY to "
    ...
    for key, name, default in (("base_url", "BASE_URL", "https://cloud.langfuse.com"), ("environment", "ENV", ""),
                               ("release", "RELEASE", "")):
        value = _secret(f"CRYOZEN_LANGFUSE_{name}") or _secret(f"LANGFUSE_{name}") or default
        if value:
            kwargs[key] = value
    sample_rate = _secret("CRYOZEN_LANGFUSE_SAMPLE_RATE")
    if sample_rate:
        try:
            kwargs["sample_rate"] = float(sample_rate)
        except ValueError:
            logger.warning("Invalid CRYOZEN_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _resolve_max_depth(configured_depth: str) -> int:
    """Parse ``CRYOZEN_LANGFUSE_MAX_DEPTH``; an invalid value warns ONCE per distinct value.
    ...
    try:
        ...
        logger.warning("Invalid CRYOZEN_LANGFUSE_MAX_DEPTH=%r; use a non-negative integer. Falling back to 4.", configured_depth)
        return 4
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("CRYOZEN_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if max_depth is None:
        max_depth = _resolve_max_depth(_env("CRYOZEN_LANGFUSE_MAX_DEPTH", "4") or "4")
    if depth > max_depth:
        return "<max-depth>"
...
def _canonical_usage_and_cost(canonical: Any, *, provider: str, model: str,
    ...
    """Translate canonical Cryozen usage into Langfuse usage and cost maps."""
    ...
    # need an explicit total (Cryozen estimate also includes request pricing).
...
def _start_root_trace(task_key: str, *, task_id: str, session_id: str, platform: str, provider: str, model: str,
    ...
    metadata = {
        "source": "cryozen", "task_id": task_id, "turn_id": turn_id, "api_request_id": api_request_id,
    ...
    def open_root():
        ctx = client.start_as_current_observation(trace_context=trace_ctx, name="Cryozen turn", as_type="chain",
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(session_id=session_id or task_key, trace_name="Cryozen turn",
                                      tags=["cryozen", "langfuse"]):
...
def register(ctx) -> None:
    # Both hook-name variants so the plugin works across Cryozen versions:
