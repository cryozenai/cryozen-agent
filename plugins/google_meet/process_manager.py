...
One active meeting at a time, recorded in ``$CRYOZEN_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_cryozen_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Cryozen Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "CRYOZEN_MEET_URL": url, "CRYOZEN_MEET_OUT_DIR": str(out),
           "CRYOZEN_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "CRYOZEN_MEET_HEADED"),
        (auth_state, "CRYOZEN_MEET_AUTH_STATE"),
        (duration, "CRYOZEN_MEET_DURATION"),
        (mode, "CRYOZEN_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "CRYOZEN_MEET_REALTIME_MODEL"),
        (realtime_voice, "CRYOZEN_MEET_REALTIME_VOICE"),
        (realtime_instructions, "CRYOZEN_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("CRYOZEN_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["CRYOZEN_MEET_REALTIME_KEY"] = realtime_api_key
