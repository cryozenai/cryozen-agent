...
Let the cryozen agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where cryozen runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `cryozen meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `cryozen meet` CLI |
...
| `cli.py` | `cryozen meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CRYOZEN_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `cryozen meet node {run,list,approve,remove,status,ping}` |
...
cryozen meet install                                      # pip + Chromium
cryozen meet setup                                        # preflight
cryozen meet auth                                         # optional
cryozen meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
cryozen meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.cryozen-agent/.env
cryozen meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
cryozen meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
cryozen meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.cryozen-agent/.env
cryozen meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, cryozen will **not** switch your system audio input automatically — the
...
admission the bot unmutes itself if Meet seated it muted; `status.json` / `cryozen meet
...
cryozen plugins enable google_meet
cryozen meet node run --display-name my-mac --host 0.0.0.0 --port 18789
