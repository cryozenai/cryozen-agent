...
the ``token_auth`` middleware seam): the hosting orchestrator provisions a per-agent unique
secret (``CRYOZEN_DASHBOARD_DRAIN_SECRET``, env-only — it is a credential); an inbound bearer
...
from cryozen_cli.dashboard_auth import DashboardAuthProvider, Session, TokenPrincipal
...
def _settings() -> dict:
    ...
    secret = os.environ.get("CRYOZEN_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        raise SkipRegistration(
            "CRYOZEN_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent >=256-bit secret "
            "(e.g. `python -c \"import secrets; print(secrets.token_urlsafe(32))\"`) to enable "
            "orchestrator-driven drain coordination; leave it unset to disable the drain endpoint.")
    ...
    if reason is not None:
        raise SkipRegistration(
            f"CRYOZEN_DASHBOARD_DRAIN_SECRET rejected — {reason}. The drain endpoint stays disabled (fail-closed).",
            level="warning")
...
def register(ctx) -> None:
    ...
    reason) when ``CRYOZEN_DASHBOARD_DRAIN_SECRET`` is unset or fails the entropy gate. On
    ...
    # doesn't bounce the orchestrator's bearer call.
    try:
        from cryozen_cli.dashboard_auth.token_auth import register_token_route
