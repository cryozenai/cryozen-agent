...
Speaks the OpenAI-style ``/chat/completions`` image protocol (``modalities:
["image","text"]``, references as ``image_url`` parts, output in
``choices[0].message.images[].image_url.url``); ``(base_url, api_key)`` come from
:func:`cryozen_cli.runtime_provider.resolve_runtime_provider`. OpenRouter also
...
_ATTRIBUTION_HEADERS = {
    ...
    "HTTP-Referer": "https://github.com/cryozenai/cryozen-agent",
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    """One instance per OpenRouter-compatible backend; instances differ only in the runtime
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        releases selectable); backends without the Image API = chat-completions only. Offline:
        default chain + snapshot."""
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return fail(
                ...
                f"Configure {self._display} in `cryozen tools` → Image Generation.",
                "missing_api_key")
