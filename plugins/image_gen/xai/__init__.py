...
from tools.xai_http import (
    build_xai_storage_options, cryozen_xai_user_agent, maybe_mark_xai_storage_notice_seen,
...
def _fetch_live_models(creds: Optional[Dict[str, Any]] = None) -> Dict[str, Dict[str, Any]]:
    ...
    response = requests.get(
        ...
        headers={"Authorization": f"Bearer {api_key}", "User-Agent": cryozen_xai_user_agent()},
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_factory(provider_name, aspect_ratio)(
                "No xAI credentials found. Configure xAI OAuth in `cryozen model` or set XAI_API_KEY.",
                "missing_api_key")
        ...
        storage_options = build_xai_storage_options(
            "image_gen", filename_prefix="cryozen-xai-image", extension="png")
