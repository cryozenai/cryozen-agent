...
# Cryozen' 3 abstract ratios → Krea's enum (1:1, 4:3, 3:2, 16:9, 2.35:1, 4:5, 2:3, 9:16).
...
_USER_AGENT = "Cryozen-Agent/1.0 (krea-image-gen)"
...
def _headers(auth_token: str, *, json_body: bool) -> Dict[str, str]:
...
def _poll_krea_job(
    ...
    headers = _headers(auth_token, json_body=False)
...
def _enhance_image(
    base_url: str, auth_token: str, image_url: str, prompt: str
) -> Optional[str]:
    ...
    try:
        resp = requests.post(
            f"{base_url}{_ENHANCE_PATH}", headers=_headers(auth_token, json_body=True),
...
def _submit_job(
    base_url: str, auth_token: str, model_path: str, payload: Dict[str, Any], fail: ErrorFn,
    ...
    submit_body, failure = post_json(
        ...
        headers=_headers(auth_token, json_body=True),
        ...
        **kwargs: Any,
        ...
        if not auth_token:
            return error_factory("krea", aspect)(
                "KREA_API_KEY not set. Run `cryozen tools` → Image Generation → Krea to configure, "
                "or get a key at https://www.krea.ai/settings/api-tokens.",
                "auth_required")
        ...
        # 1. Submit job.
        job_id, err = _submit_job(base_url, auth_token, meta["path"], payload, fail)
        if err is not None:
            return err
        ...
        if _upscale_requested(kwargs.get("upscale"), meta):
            enhanced_url = _enhance_image(base_url, auth_token, result_image_url, prompt)
            if enhanced_url:
