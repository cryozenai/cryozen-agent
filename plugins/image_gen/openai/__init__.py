...
def _build_client(openai: Any, base_url: str, api_key: str) -> Any:
    """``openai.OpenAI`` on Cryozen' env-only-proxy httpx client, so a local/custom endpoint never
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_factory("openai", aspect)(
                ...
                "Run `cryozen tools` → Image Generation → OpenAI to configure, or `cryozen setup` "
                "to add the key.",
                "auth_required")
        ...
        # gpt-image bills per text/image token; the tier id is a Cryozen label, the API model prices.
