"""Per-plugin persistent storage: ``<cryozen home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<cryozen home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `cryozen plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<cryozen home>/plugin-data/<name>/``; resolves ``get_cryozen_home()`` on
    ...
    root = get_cryozen_home() / "plugin-data" / _validate_name(name)
