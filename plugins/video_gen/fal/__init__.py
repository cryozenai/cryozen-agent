...
# Family catalog. Capability flags gate which keys reach the payload — keys a family doesn't advertise are never sent (endpoints
# reject unknown keys). Enums default to None (endpoint decides), flags to False. ``durations`` is always a
...
def _build_payload(family: Dict[str, Any], *, prompt: str, image_url: Optional[str], duration: Optional[int], aspect_ratio: str,
    ...
    payload: Dict[str, Any] = {key: value for ok, key, value in (
        ...
        # Newer endpoints declare no `seed` — gate on the family.
...
def _fal_video_available() -> bool:
    """True when FAL_KEY is configured; raises the selection-naming ValueError when `cryozen tools`
    stored a video_gen provider but FAL_KEY is missing."""
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    """Submit with the direct FAL_KEY credentials; ``.get()`` blocks."""
    client = _load_fal_client()
    return client.submit(endpoint, arguments=arguments, headers={"x-idempotency-key": str(uuid.uuid4())})
...
_NO_BACKEND_MSG = "No FAL backend available. Set FAL_KEY (run `cryozen tools` → Video Generation → FAL to configure)."
_MODALITY_MISSING_MSG = {
    "image": "FAL family {fid} has no image-to-video endpoint. Pick a family with image-to-video support via `cryozen tools` → Video Generation.",
...
class FALVideoGenProvider(VideoGenProvider):
    ...
    def generate(
        ...
        upscaled_url = _upscale_video(url) if upscale else None
