"""Email platform adapter for the Cryozen gateway: users talk to Cryozen by sending email; IMAP (polled)
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _cryozen_version = "0"
        imap.xatom("ID", f'("name" "cryozen-agent" "version" "{_cryozen_version}" '
                         '"vendor" "Cryozen" "support-email" "cryozenai@users.noreply.github.com")')
...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing := [name for name, value in required if not value]:
            message = f"Not configured — missing {', '.join(missing)}. Set it via `cryozen gateway setup` (env) or platforms.email in config.yaml."
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        for key, value in (("From", address), ("To", chat_id), ("Subject", "Cryozen Agent"), ("Date", formatdate(localtime=True))):
            msg[key] = value
