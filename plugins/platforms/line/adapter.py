...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, CRYOZEN_HOME).
        ...
        try:
            ...
            cryozen_home = Path(get_cryozen_home()).resolve()
        except Exception:
            cryozen_home = Path.home().joinpath(".cryozen-agent").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), cryozen_home)):  # no-tmp: ok — macOS /private/tmp alias in the allowed-roots check, not a write target
...
def is_connected(config) -> bool:
    """Surface in ``cryozen status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """``env_enablement_fn``: seed ``PlatformConfig.extra`` from env-only setups so ``cryozen status`` sees them."""
    if not _env_credentials_present():
        return None
...
def interactive_setup() -> None:
    """``cryozen setup line`` wizard (writes ``~/.cryozen-agent/.env``); CLI helpers are lazy-imported."""
    from cryozen_cli.config import get_env_value, save_env_value
    from cryozen_cli.cli_output import print_header, print_info, prompt
