...
// `message.content.text` before Cryozen can see it. We rewrite the two inbound
...
const MARKER = "Cryozen patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Cryozen to patch; keep the legacy v8 path below for older pinned installs.
    if (
