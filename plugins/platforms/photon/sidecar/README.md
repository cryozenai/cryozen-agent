...
Small Node helper that bridges Cryozen Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Cryozen is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Cryozen-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Cryozen can receive inbound messages
...
The Cryozen plugin's `cryozen photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Cryozen is Python, so replies go through this sidecar
