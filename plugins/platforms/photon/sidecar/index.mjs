// Cryozen Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Cryozen): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Cryozen -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Cryozen-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `cryozen photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Cryozen can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Cryozen'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Cryozen allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Cryozen, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-cryozen-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Cryozen supplied them so a known-good
