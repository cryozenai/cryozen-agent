...
``(id, projectSecret)``. Storage: runtime SDK creds -> ``~/.cryozen-agent/.env``; management
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a cryozen dependency
...
# published CLI client until Cryozen gets its own client_id.
...
DEFAULT_PROJECT_NAME = "Cryozen Agent"
DEFAULT_POLL_INTERVAL = 5  # RFC 8628 polling defaults; Photon's `interval` / `expires_in` win
DEFAULT_POLL_TIMEOUT = 1800
E164_RE = re.compile(r"^\+[1-9]\d{6,14}$")


# -- auth.json helpers (shares the file with the rest of cryozen-agent) ------------

def _auth_json_path() -> Path:
    """The active profile's ``auth.json`` (shared with the rest of cryozen-agent)."""
    return get_cryozen_home() / "auth.json"
...
def _store_pool_record(key: str, record: Dict[str, Any]) -> None:
    ...
    from cryozen_cli.auth import _auth_store_lock
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.cryozen-agent/.env`` (secret never bound to a printable local
    in a caller — CodeQL clean flow)."""
    try:
        from cryozen_cli.config import save_env_value
    except ImportError:
        logger.warning("photon: cryozen_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    rows = [
        ...
        "──────────────────────",
        "  device token        : " + (
            "✓ stored" if load_photon_token() else "✗ missing (run `cryozen photon setup`)"),
        "  project id          : " + (sid if sid else "✗ missing"),
        "  project secret      : " + ("✓ stored" if sec else "✗ missing"),
        "  my number           : " + (phone if phone else "✗ missing (run `cryozen photon setup --phone ...`)"),
        "  assigned number     : " + (assigned if assigned else "✗ missing (run `cryozen photon setup`)")]
    emit("\n".join(rows))
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_token() -> str:
        return (
            ...
            else "✗ missing (run `cryozen photon setup`)"
        )
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `cryozen photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `cryozen photon setup`)"
