# A2A — Agent-to-Agent protocol for Cryozen
...
peer (another Cryozen, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
cryozen gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Cryozen serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.cryozen-agent/a2a_audit.jsonl`.
- Conversations persist to `~/.cryozen-agent/a2a_conversations/` — they survive context
