...
def _connect_error_detail(exc: BaseException) -> str:
    ...
    if (sys.platform == "darwin" and is_supervised_gateway_launch()
        ...
        return (
            ...
            "Run `cryozen gateway install` to regenerate the launchd job, then `cryozen gateway restart`. "
            "https://github.com/cryozenai/cryozen-agent/issues/71206"
        )
    return text
...
class HomeAssistantAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        payload = {"title": "Cryozen Agent", "message": content[:self.MAX_MESSAGE_LENGTH]}
...
# cryozen_cli/gateway.py, and the _send_email dispatch in tools/send_message_tool.py. EMAIL_*
