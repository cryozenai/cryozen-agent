...
Auto-tracks and cleans up ephemeral files created during Cryozen Agent
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CRYOZEN_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
<!-- no-tmp: ok — documents the legacy scratch scope this plugin cleans up -->
- `is_safe_path()` rejects anything outside `CRYOZEN_HOME` or `/tmp/cryozen-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CRYOZEN_HOME/disk-cleanup/` is itself excluded
- `$CRYOZEN_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
