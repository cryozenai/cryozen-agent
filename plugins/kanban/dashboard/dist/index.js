/**
 * Cryozen Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.cryozen-agent/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__CRYOZEN_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__CRYOZEN_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "cryozen-kanban-dot-triage",
    todo: "cryozen-kanban-dot-todo",
    ready: "cryozen-kanban-dot-ready",
    running: "cryozen-kanban-dot-running",
    blocked: "cryozen-kanban-dot-blocked",
    review: "cryozen-kanban-dot-review",
    done: "cryozen-kanban-dot-done",
    archived: "cryozen-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-cryozen-task";
  ...
  const DOCS_URL = "https://cryozenai.github.io/cryozen-agent/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://cryozenai.github.io/cryozen-agent/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "cryozen.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="cryozen-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])cryozen-kanban-md-code\1/i.test(attrs)) {
      return ' class="cryozen-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "cryozen-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "cryozen-kanban-md",
  ...
  // standard `drop` event and our `cryozen-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      function startDrag() {
        ...
        proxy.classList.add("cryozen-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("cryozen-kanban-column--drop");
          if (target) target.classList.add("cryozen-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (!dragging) return;
        if (lastTarget) {
          lastTarget.classList.remove("cryozen-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("cryozen-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("cryozen-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__CRYOZEN_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "cryozen-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "cryozen-kanban-attention",
        "cryozen-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "cryozen-kanban-attention-bar" },
        h("span", { className: "cryozen-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "cryozen-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "cryozen-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "cryozen-kanban-attention-row",
                  "cryozen-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "cryozen-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "cryozen-kanban-attention-row-id" }, task.id),
                h("span", { className: "cryozen-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "cryozen-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "cryozen-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "cryozen-kanban-diag-action-btn",
      action.suggested ? "cryozen-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " cryozen-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".cryozen-kanban-drawer-comment-row input, .cryozen-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "cryozen-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("cryozen-kanban-diag", sevClass) },
      h("div", { className: "cryozen-kanban-diag-header" },
        h("span", { className: "cryozen-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "cryozen-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "cryozen-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "cryozen-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "cryozen-kanban-diag-data-row" },
                  h("span", { className: "cryozen-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "cryozen-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "cryozen-kanban-diag-data-row" },
                h("span", { className: "cryozen-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "cryozen-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "cryozen-kanban-diag-reassign-row" },
            h("span", { className: "cryozen-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "cryozen-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "cryozen-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "cryozen-kanban-diag-msg",
              msg.ok ? "cryozen-kanban-diag-msg--ok" : "cryozen-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "cryozen-kanban-section" },
      h("div", { className: "cryozen-kanban-section-head-row" },
        h("span", { className: "cryozen-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "cryozen-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "cryozen-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "cryozen-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "cryozen-kanban-docs-link",
      title: "Open Cryozen Kanban docs in a new tab",
      "aria-label": "Cryozen Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `cryozen kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "cryozen-kanban-msg-ok" : "cryozen-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardProjectBadge(props) {
    ...
    return h(Badge, {
      variant: "outline",
      className: "cryozen-kanban-board-project text-xs font-normal gap-1",
      ...
      h("button", {
        type: "button",
        className: "cryozen-kanban-board-project-unbind",
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "cryozen-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "cryozen-kanban-boardswitcher" },
      h("div", { className: "cryozen-kanban-boardswitcher-inner" },
  ...
  // Live (non-archived) Cryozen projects available for board scoping,
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "cryozen-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "cryozen-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "cryozen-kanban-dialog-title" },
        ...
        h("div", { className: "cryozen-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Cryozen profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "cryozen-kanban-bulk" },
      h("span", { className: "cryozen-kanban-bulk-count" },
      ...
      h("div", { className: "cryozen-kanban-bulk-priority",
      ...
      h("div", { className: "cryozen-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Cryozen profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "cryozen-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("cryozen-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("cryozen-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "cryozen-kanban-trash",
        dragOver ? "cryozen-kanban-trash--drop" : "",
        props.draggingTaskId ? "cryozen-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "cryozen-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "cryozen-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".cryozen-kanban-card")) return true;
      if (target.closest && target.closest(".cryozen-kanban-column-add")) return true;
      if (target.closest && target.closest(".cryozen-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("cryozen-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("cryozen-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".cryozen-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "cryozen-kanban-columns",
        isScrollable ? "cryozen-kanban-columns--scrollable" : "",
        isPanning ? "cryozen-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("cryozen-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("cryozen-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "cryozen-kanban-column",
        dragOver ? "cryozen-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "cryozen-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "cryozen-kanban-col-check",
        ...
        h("span", { className: cn("cryozen-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "cryozen-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "cryozen-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "cryozen-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "cryozen-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "cryozen-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "cryozen-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "cryozen-kanban-lane" },
                  h("div", { className: "cryozen-kanban-lane-head" },
                    h("span", { className: "cryozen-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "cryozen-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "cryozen-kanban-card--stale-red";
    if (age >= tier.amber) return "cryozen-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".cryozen-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "cryozen-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "cryozen-kanban-card",
        props.selected ? "cryozen-kanban-card--selected" : "",
        props.failed ? "cryozen-kanban-card--failed" : "",
        props.draggingSource ? "cryozen-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "cryozen-kanban-card-content" },
          h("div", { className: "cryozen-kanban-card-row" },
            h("label", {
              className: "cryozen-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "cryozen-kanban-card-check",
            ...
            h("span", { className: "cryozen-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or cryozen kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "cryozen-kanban-warning-badge",
                    "cryozen-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "cryozen-kanban-needs-assignee",
          ...
          h("div", { className: "cryozen-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "cryozen-kanban-card-row cryozen-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "cryozen-kanban-assignee",
                            title: `Assigned to Cryozen profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "cryozen-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "cryozen-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "cryozen-kanban-ago",
  ...
  // centered modal reusing the cryozen-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "cryozen-kanban-dialog cryozen-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Cryozen profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Cryozen profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "cryozen-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "cryozen-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "cryozen-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "cryozen-kanban-drawer-close",
        ...
        data ? h("div", { className: "cryozen-kanban-drawer-comment-foot" },
          h("div", {
            className: "cryozen-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "cryozen-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "cryozen-kanban-section" },
      h("div", { className: "cryozen-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "cryozen-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "cryozen-kanban-drawer-body" },
      h("div", { className: "cryozen-kanban-drawer-title" },
        h("span", { className: cn("cryozen-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "cryozen-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "cryozen-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "cryozen-kanban-section" },
            h("div", { className: "cryozen-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "cryozen-kanban-section" },
            h("div", { className: "cryozen-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "cryozen-kanban-done-no-result cryozen-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "cryozen-kanban-section" },
            h("div", { className: "cryozen-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "cryozen-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "cryozen-kanban-section" },
        h("div", { className: "cryozen-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "cryozen-kanban-comment" },
            h("div", { className: "cryozen-kanban-comment-head" },
              h("span", { className: "cryozen-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "cryozen-kanban-diag-action-btn",
      ...
      h("div", { className: "cryozen-kanban-section" },
        h("div", { className: "cryozen-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "cryozen-kanban-comment" },
                h("div", { className: "cryozen-kanban-comment-head" },
                  h("span", { className: "cryozen-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "cryozen-kanban-comment-ago" },
      ...
      h("div", { className: "cryozen-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "cryozen-kanban-event",
              isDiag ? "cryozen-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "cryozen-kanban-event-header" },
                  h("span", { className: "cryozen-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "cryozen-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "cryozen-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "cryozen-kanban-event-header-plain" },
                  h("span", { className: "cryozen-kanban-event-kind" }, e.kind),
                  h("span", { className: "cryozen-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "cryozen-kanban-event-phantom-row" },
                  h("span", { className: "cryozen-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "cryozen-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "cryozen-kanban-event-payload" },
    ...
    return h("div", { className: "cryozen-kanban-section" },
      h("div", { className: "cryozen-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "cryozen-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `cryozen-kanban-run--${r.outcome || r.status || "ended"}`
          : "cryozen-kanban-run--active";
        return h("div", { key: r.id, className: cn("cryozen-kanban-run", outcomeClass) },
          h("div", { className: "cryozen-kanban-run-head" },
            h("span", { className: "cryozen-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "cryozen-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "cryozen-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "cryozen-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "cryozen-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "cryozen-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "cryozen-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "cryozen-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "cryozen-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "cryozen-kanban-pre cryozen-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "cryozen-kanban-meta-row" },
      h("span", { className: "cryozen-kanban-meta-label" }, props.label),
      h("span", { className: "cryozen-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "cryozen-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "cryozen-kanban-meta-row" },
        h("span", { className: "cryozen-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "cryozen-kanban-meta-value cryozen-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "cryozen-kanban-meta-row" },
        h("span", { className: "cryozen-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "cryozen-kanban-meta-value cryozen-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "cryozen-kanban-meta-row" },
        h("span", { className: "cryozen-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "cryozen-kanban-meta-value cryozen-kanban-editable",
    ...
    return h("div", { className: "cryozen-kanban-meta-row" },
      h("span", { className: "cryozen-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "cryozen-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "cryozen-kanban-section" },
      h("div", { className: "cryozen-kanban-section-head-row" },
        h("span", { className: "cryozen-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "cryozen-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "cryozen-kanban-section" },
      h("div", { className: "cryozen-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "cryozen-kanban-deps-row" },
        h("span", { className: "cryozen-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "cryozen-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "cryozen-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "cryozen-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "cryozen-kanban-dep-chip-x",
      ...
      h("div", { className: "cryozen-kanban-deps-row" },
        h("span", { className: "cryozen-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "cryozen-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "cryozen-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "cryozen-kanban-msg-ok"
          : "cryozen-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "cryozen-kanban-section" },
      ...
      h("div", { className: "cryozen-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "cryozen-kanban-home-sub cryozen-kanban-home-sub--on"
              : "cryozen-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__CRYOZEN_PLUGINS__ && typeof window.__CRYOZEN_PLUGINS__.register === "function") {
    window.__CRYOZEN_PLUGINS__.register("kanban", KanbanPage);
  }
})();
