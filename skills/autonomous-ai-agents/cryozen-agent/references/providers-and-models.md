...
Set via `cryozen model` (picker) or `cryozen setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://cryozenai.github.io/cryozen-agent/docs/integrations/providers
...
| openai-codex | OAuth | `cryozen auth add openai-codex` |
| qwen-oauth | OAuth | `cryozen auth add qwen-oauth` |
| minimax-oauth | OAuth | `cryozen auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`cryozen auth`).
Fallback chain when the primary fails: `cryozen fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`cryozen_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   cryozen config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
