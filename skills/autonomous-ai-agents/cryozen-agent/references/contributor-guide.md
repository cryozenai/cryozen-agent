...
For occasional contributors and PR authors. Full developer docs: https://cryozenai.github.io/cryozen-agent/docs/developer-guide/
...
├── cli.py                # Interactive CLI (CryozenCLI)
├── cryozen_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── cryozen_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.cryozen-agent/config.yaml` (settings), `~/.cryozen-agent/.env` (API keys) — both under `$CRYOZEN_HOME` when it is set.
...
`_CRYOZEN_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_cryozen_home()` for paths,
never hardcode `~/.cryozen-agent`. For custom/local-only tools, write a plugin in
`~/.cryozen-agent/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `cryozen_cli/commands.py`
...
- Tests auto-redirect `CRYOZEN_HOME` to temp dirs — never touch real `~/.cryozen-agent/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/cryozen_cli/test_auth_toctou_file_modes.py`)
...
(`docker, singularity, modal, daytona, ssh, vercel_sandbox`), host info is
...
- Use `get_cryozen_home()` from `cryozen_constants` for all paths (profile-safe)
