...
Author widget apps for the Cryozen TUI (`cryozen --tui`): glanceable ambient
...
- The TUI must be in use (`cryozen --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.cryozen-agent/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.cryozen-agent/logs/tui_gateway_crash.log` if a widget never appears.
