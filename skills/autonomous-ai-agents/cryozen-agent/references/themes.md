# Themes / Skins — Author a Cryozen Color Theme

Author a Cryozen **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`cryozen_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.cryozen-agent/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Cryozen itself.
...
- Write access to the Cryozen home dir — `~/.cryozen-agent` by default, or `$CRYOZEN_HOME`
  / the active profile's dir. Skins live in `<cryozen-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `cryozen config set`).
...
3. `write_file` it to `<cryozen-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `cryozen config set display.skin default` (or they can `/skin default`).
...
```
cryozen skin set <key> <hex>      # e.g. cryozen skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `cryozen skin set background "#08201f"` changes
only the background; `cryozen skin use <name>` / `cryozen skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.cryozen-agent`** when a profile is active — resolve the real home
  from `$CRYOZEN_HOME` first, falling back to `~/.cryozen-agent`.
...
- **Name collisions**: a skin named like a desktop built-in (`mono`, `slate`,
  `cyberpunk`, `midnight`, `ember`) won't override that built-in on the
  GUI. Pick a fresh name.
- **Never hand-edit `config.yaml` to activate.** Use `cryozen config set
...
- **You apply it, not the user.** `cryozen config set display.skin <name>` is
...
- `read_file` the written `<cryozen-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `cryozen config get display.skin` and confirm it reports `<name>`.
