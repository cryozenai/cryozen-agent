...
Registry of record: `cryozen_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Cryozen config+state snapshots (CLI)
...
/update                  Update Cryozen to latest
/version (/v)            Show version
/debug [local]           Upload debug report, get shareable links
```
