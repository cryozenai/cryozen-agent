...
Edit with `cryozen config edit` or `cryozen config set section.key value`.
Full reference: https://cryozenai.github.io/cryozen-agent/docs/user-guide/configuration
...
`cryozen config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `cryozen tools` (interactive) or `cryozen tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_CRYOZEN_CORE_TOOLS` is the default bundle most platforms inherit).
