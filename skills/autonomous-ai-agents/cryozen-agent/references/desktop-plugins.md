...
Write plugins for the Cryozen desktop app: statusbar items, layout panes,
...
system (`~/.cryozen-agent/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$CRYOZEN_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$CRYOZEN_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.cryozen-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Cryozen desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$CRYOZEN_HOME/desktop-plugins/` (usually
  `~/.cryozen-agent/desktop-plugins/`).

## How to Run

1. Create `$CRYOZEN_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.cryozen-agent/...` by default, or `~/.cryozen-agent/profiles/<profile>/...` under a
...
The ONLY import surface is `@cryozen/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Cryozen
  ...
  (`cryozen://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.cryozen-agent/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@cryozen/plugin-sdk`, `react`, and
