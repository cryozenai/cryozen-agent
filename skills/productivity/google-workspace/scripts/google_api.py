#!/usr/bin/env python3
"""Google Workspace API CLI for Cryozen Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Cryozen-facing JSON contract and falls back to the Python client
...
from _cryozen_home import get_cryozen_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CRYOZEN_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Cryozen Agent")
