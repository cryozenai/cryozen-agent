...
- **`optional-skills/`** — heavier/niche skills shipped but NOT active; installed via
  `cryozen skills install official/<category>/<skill>` (adapter `tools/skills_hub_official.py`
...
`[linux, macos]`, ...), `metadata.cryozen.tags`, `metadata.cryozen.category`,
`metadata.cryozen.related_skills`, `metadata.cryozen.config` (config.yaml settings the skill needs —
stored under `skills.config.<key>`, prompted during setup, injected at load). Top-level `tags:` /
`category:` are accepted and mirrored from `metadata.cryozen.*` by the loader.
...
2. **Prose references native Cryozen tools or the MCP servers the skill expects, in backticks**
...
in the `cryozen-agent-dev` skill.

## Curator (skill lifecycle)

Background maintenance that tracks usage on agent-created skills and auto-archives stale ones;
archives go to `~/.cryozen-agent/skills/.archive/` and are restorable. Core `agent/curator.py` (review
loop, auto-transitions, LLM review prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots);
CLI `cryozen_cli/curator.py` → `cryozen curator status|run|pause|resume|pin|unpin|archive|restore|
prune|backup|rollback`; telemetry `tools/skill_usage.py` owns `~/.cryozen-agent/skills/.usage.json`
