...
- Give it a name like "cryozen-agent"
- Select scopes:
...
- Give it a title like "cryozen-agent-<machine-name>"
...
uv run python "${CRYOZEN_HOME:-$HOME/.cryozen-agent}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _cryozen_env="${CRYOZEN_HOME:-$HOME/.cryozen-agent}/.env"; [ -f "$_cryozen_env" ] && grep -q "^GITHUB_TOKEN=" "$_cryozen_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_cryozen_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${CRYOZEN_HOME:-$HOME/.cryozen-agent}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
