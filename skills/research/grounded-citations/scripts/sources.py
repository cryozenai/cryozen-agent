...
Ledger path resolution (first wins):
  --ledger PATH
  $CRYOZEN_CITATION_LEDGER
  $CRYOZEN_HOME/cache/citations/ledger.json
"""
...
from _cryozen_home import get_cryozen_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("CRYOZEN_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_cryozen_home() / "cache" / "citations" / "ledger.json"
