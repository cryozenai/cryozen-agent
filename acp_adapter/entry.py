"""CLI entry point for the cryozen-agent ACP adapter.

Loads ``~/.cryozen-agent/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: cryozen acp / cryozen-acp
"""

# IMPORTANT: cryozen_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See cryozen_bootstrap.py for full rationale.
try:
    import cryozen_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``cryozen update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Cryozen modules.
    cryozen_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from CRYOZEN_HOME (default ``~/.cryozen-agent``)."""
    from cryozen_cli.env_loader import load_cryozen_dotenv

    cryozen_home = get_cryozen_home()
    loaded = load_cryozen_dotenv(cryozen_home=cryozen_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", cryozen_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="cryozen-acp", description="Run Cryozen Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Cryozen version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Cryozen provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.cryozen-agent/node/ "
...
def _print_version() -> None:
    from cryozen_cli import __version__ as cryozen_version

    print(cryozen_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import CryozenACPAgent  # noqa: F401

    print("Cryozen ACP check OK")


def _run_setup() -> None:
    from cryozen_cli.main import main as cryozen_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "cryozen", "model"]
        cryozen_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting cryozen-agent ACP adapter")
    ...
    import acp
    from .server import CryozenACPAgent

    # Windows: import the configured memory provider (and numpy) on the main thread before
    # the MCP-discovery and ACP stdin-reader threads start (cryozen_cli's ~150 ms
    ...
    if os.environ.get("CRYOZEN_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
