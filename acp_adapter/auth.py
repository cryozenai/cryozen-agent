"""ACP auth helpers — detect and advertise Cryozen authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "cryozen-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Cryozen runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Cryozen.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Cryozen credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Cryozen using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Cryozen provider", type="terminal", args=["--setup"],
        description=("Open Cryozen' interactive model/provider setup in a terminal. "
                     "Use this when Cryozen has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Cryozen can resolve any runtime provider credentials."""
