...
export const $cryozenHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.cryozen-agent/logs/bootstrap-installer.log')
    $cryozenHome.set('~/.cryozen-agent')
  ...
  try {
    const [logPath, cryozenHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_cryozen_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $cryozenHome.set(cryozenHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Cryozen" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Cryozen-Setup.exe --update);
...
export async function launchCryozenDesktop(): Promise<void> {
  ...
  await invoke('launch_cryozen_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Cryozen repository' },
