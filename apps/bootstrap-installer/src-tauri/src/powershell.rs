...
/// read side does not see EOF until the last of them closes it. `cryozen update`
...
/// stranded with it.
///
/// Same bound `Invoke-CryozenStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `cryozen_home_override` propagates to the child as $CRYOZEN_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, cryozen_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = cryozen_home_override {
        cmd.env("CRYOZEN_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, cryozen_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = cryozen_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_cryozen_home() {
    ...
    /// child. `cryozen update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
