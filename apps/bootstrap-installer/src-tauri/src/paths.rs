...
//!
//! Mirrors `cryozen_constants.get_cryozen_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\cryozen
//!   macOS:   ~/.cryozen-agent
//!   Linux:   ~/.cryozen-agent  (override via $CRYOZEN_HOME)
//!
//! NOTE (macOS): Python's get_cryozen_home(), scripts/install.sh, and the
//! Electron desktop's resolveCryozenHome() ALL use ~/.cryozen-agent on macOS — there
...
/// Returns the canonical Cryozen home directory, respecting $CRYOZEN_HOME if set.
pub fn cryozen_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("CRYOZEN_HOME") {
    ...
    {
        // %LOCALAPPDATA%\cryozen — matches scripts/install.ps1's $CryozenHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("cryozen");
        }
    }

    // macOS + Linux + fallback: ~/.cryozen-agent (matches Python get_cryozen_home(),
    // install.sh, and the Electron desktop's resolveCryozenHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".cryozen-agent");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".cryozen-agent")
}

pub fn log_dir() -> PathBuf {
    cryozen_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    cryozen_home().join("bootstrap-cache")
}
...
/// CRYOZEN_HOME so it survives repo checkout deletion (unlike anything under
/// cryozen-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\cryozen\cryozen-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_cryozen`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under CRYOZEN_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves CRYOZEN_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    cryozen_home().join(".cryozen-update-in-progress")
}
...
pub fn copy_self_to_cryozen_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to CRYOZEN_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_CRYOZEN_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".cryozen-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under CRYOZEN_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[cryozen-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("CRYOZEN_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_cryozen_home() -> String {
    cryozen_home().to_string_lossy().into_owned()
}
