...
/** One persisted reaction row (``cryozen_state_messages.set_message_reaction``); ``seen`` is stamped once announced. */
...
/** ``provider_configured`` is the loose answer; ``ready`` is always true (nothing is minted at boot, so there is nothing to wait for). An unknown ``profile`` answers ``ok=False`` + ``error``. */
...
/** One ``cryozen_cli/inventory.py::build_models_payload`` provider row (the union of every field the builder sets; ``pricing_pending`` marks the cached-only path). */
...
/** ``cryozen_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown, ``"free"``); the sale fields are set only when a provider reports a discount. */
...
export type ConnectionTargetKind = 'mcp' | 'plugin' | 'skill'
export type ConnectionTargetAction = 'authorize' | 'enable' | 'install'
/** ``tools/connectors/contract.py::TargetState``. */
export type ConnectionTargetState = 'pending' | 'initiated' | 'connected' | 'skipped' | 'failed' | 'not_connected'
...
/** ``cryozen_cli/foreign_sessions_browser.py::list_foreign_sessions`` — ``id`` is an opaque handle, never a path. */
...
export type ToolLabelKind = 'mcp' | 'tool'
...
/** One ``config_schema`` key of a plugin manifest, rendered by the Plugins hub (``cryozen_cli.plugins_settings.plugin_settings_fields``). ``secret`` fields carry no value: ``env`` names the ``.env`` variable and ``has_value`` whether it is set. */
...
export type ConnectionActor = 'user' | 'backend_watcher'
...
/** ``tui_gateway/change_watcher.py::resolve_skin`` — the resolved active skin (``CryozenSkin``). ``{}`` when the skin engine failed to load. Colour maps are token → colour string. */
...
/** ``agent/status_output.py::AgentNotice`` via notice_callback, and ``server._await_agent_ready``'s slow-build notice. ``level``: info | warn | error | success; ``kind``: sticky | ttl | agent. */
...
export type ConnectorErrorReason = 'INVALID_PARAMS' | 'NOT_OWNER' | 'UNSUPPORTED_RUNTIME' | 'CONNECTOR_REQUEST_FAILED' | 'UNKNOWN_TARGET' | 'LINK_STILL_VALID' | 'REISSUE_REFUSED' | 'UNKNOWN_OPERATION' | 'INVALID_ANSWER'
...
export interface RpcMethods {
  ...
  /** Run ``cryozen <argv>`` non-interactively and capture its output; ``blocked`` explains a refusal. */
  ...
  /** Run failed targets of the session's open connection operation again (Try again). */
  'connectors.connect': { params: ConnectorsConnectParams; result: ConnectorsConnectResult }
  /** The current snapshot of one open session operation. */
  'connectors.operation.status': { params: ConnectionOperationParams; result: ConnectionOperationStatus }
  /** The browser leg came back (cryozen://connections/done): observe the targets now, not at the next tick. */
  ...
  /** Re-read ~/.cryozen-agent/.env (CLI /reload parity); built agents keep their pool until /new. */
  ...
  /** Export the transcript to ~/.cryozen-agent/sessions/saved (classic /save). */
  ...
  /** Token / context / cost counters for the session (+ provider account lines when available). */
