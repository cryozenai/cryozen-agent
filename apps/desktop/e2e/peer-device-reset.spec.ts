 ...
 * Requires the built desktop and a repo venv (or CRYOZEN_DESKTOP_PYTHON).
 */
...
function pythonBinary(): string {
  if (process.env.CRYOZEN_DESKTOP_PYTHON) {
    return process.env.CRYOZEN_DESKTOP_PYTHON
  }
  ...
  if (!candidate) {
    throw new Error('Create the repo Python venv or set CRYOZEN_DESKTOP_PYTHON before running this spec')
  }
...
function isolatedEnv(sandbox: Sandbox): Record<string, string> {
  ...
  for (const key of ['CRYOZEN_DESKTOP_USER_DATA_DIR', 'CRYOZEN_DESKTOP_IGNORE_EXISTING', 'CRYOZEN_DESKTOP_CRYOZEN_ROOT', 'CRYOZEN_DESKTOP_APP_NAME', 'CRYOZEN_DESKTOP_SKIP_QUIT_CONFIRM']) {
    env[key] = defaults[key]
  }
  ...
  return {
    ...
    CRYOZEN_DESKTOP_PYTHON: pythonBinary(),
...
const peerTest = test.extend<{ gateways: { app: ElectronApplication; source: Page } }>({
  ...
  gateways: async ({}, runTest, info) => {
    ...
    try {
      const remoteProfileHome = path.join(remote.cryozenHome, 'profiles', REMOTE_PROFILE)
      fs.mkdirSync(remoteProfileHome, { recursive: true })

      for (const cryozenHome of [local.cryozenHome, remote.cryozenHome, remoteProfileHome]) {
        writeMockProviderConfig(cryozenHome, mock.url)
        writeEnvFile(cryozenHome)
      }

      const port = await freePort()
      const remoteUrl = `http://127.0.0.1:${port}`
      child = spawn(pythonBinary(), ['-m', 'cryozen_cli.main', 'serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'], {
        cwd: REPO_ROOT,
        detached: process.platform !== 'win32',
        env: { ...isolatedEnv(remote), CRYOZEN_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN },
      ...
      await expect.poll(async () => {
        ...
        try {
          return (await fetch(`${remoteUrl}/api/status`, {
            headers: { 'X-Cryozen-Session-Token': REMOTE_TOKEN },
