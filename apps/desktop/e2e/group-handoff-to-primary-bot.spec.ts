...
// #100406: in a Bot Mode group room a teammate's `@cryozen` handoff must give
// the primary profile (internal name `default`) its turn, exactly like
// `@cryozen → @code-farmer` already does. The live roster stamps the primary
// row's handle as the bare profile id ("default"), and the mention parser let
// that stamped handle shadow the `@cryozen` alias — so the room settled with
// Cryozen never driven. The mock inference server scripts each member's line
...
/** The plugin's persisted room log (`cryozen.plugin.cryozen-bots.group-chats`). */
async function roomLog(page: Page, group: string): Promise<RoomLogEntry[]> {
  return page.evaluate(name => {
    const raw = window.localStorage.getItem('cryozen.plugin.cryozen-bots.group-chats')
...
test.afterEach(async ({}, info) => {
  ...
  await info.attach('room-log', {
    body: JSON.stringify(await roomLog(fixture.page, 'Cryozen, Code Farmer'), null, 2),
  ...
  await info.attach('runtime-source', {
    body: JSON.stringify(
      await fixture.app.evaluate(() => ({
        cwd: process.cwd(),
        argv: process.argv,
        root: process.env.CRYOZEN_DESKTOP_CRYOZEN_ROOT,
        home: process.env.CRYOZEN_HOME
      })),
      null,
      2
    ),
  ...
  await info.attach('desktop-log', {
    body: readFileSync(join(fixture.sandbox.cryozenHome, 'logs/desktop.log')),
...
test('a teammate handing off with @cryozen drives the primary profile', async () => {
  test.setTimeout(420_000)
  const page = fixture!.page
  const group = 'Cryozen, Code Farmer'
  ...
  for (const title of ['Cryozen', 'Code Farmer']) {
  ...
  // to @cryozen; Cryozen' scripted reply is "B". Neither script token carries
  // a literal `@`, so the user send itself never mentions Cryozen.
  await composer.fill(
    '@code-farmer Please reply with one line only. ' +
      'E2E_SAY(code-farmer)[{at}cryozen Please reply with the letter B.] E2E_SAY(cryozen)[B]'
  )
  await composer.press('Enter')

  // Code Farmer's handoff line lands first (the reverse direction is not in
  // question); then the room must NOT settle without Cryozen' turn.
  await expect
    .poll(
      async () =>
        (await roomLog(page, group)).some(e => e.from?.name === 'code-farmer' && /@cryozen/.test(e.text || '')),
      {
        timeout: 180_000
      }
    )
  ...
  await expect
    .poll(
      ...
      {
        timeout: 180_000,
        message: 'the primary profile (default / @cryozen) never took its turn after being @mentioned by a teammate'
      }
    )
    .toBe(true)
  ...
  await composer.fill(
    '@cryozen Begin the reverse handoff. E2E_SAY(cryozen)[{at}code-farmer Reply with D.] E2E_SAY(code-farmer)[D]'
  )
  await composer.press('Enter')
  await expect
