...
const MODE_KEY = 'cryozen.desktop.interfaceMode.v1'
...
const ARRANGEMENT_KEYS = [
  'cryozen.desktop.floatingPanes.v1',
  'cryozen.desktop.layoutTree.v2',
  'cryozen.desktop.layoutPreset.active',
  'cryozen.desktop.paneStates.v1',
  'cryozen.desktop.dismissedPanes.v1',
  'cryozen.desktop.paneShare.v1',
  'cryozen.desktop.hiddenStripTabs.v1',
  'cryozen.desktop.userPlacedPanes.v1',
  'cryozen.desktop.panesFlipped',
  'cryozen.desktop.collapsedTreeSides.v1'
]
...
async function preferenceSnapshot(page: MockBackendFixture['page']): Promise<Record<string, string>> {
  return page.evaluate(
    ({ modeKey, layoutKeys }) => {
      ...
      arrangement.add('cryozen.desktop.layoutModeScopes.v1')
      const out: Record<string, string> = {}
      for (let i = 0; i < window.localStorage.length; i += 1) {
        const key = window.localStorage.key(i)
        if (key && key.startsWith('cryozen.desktop.') && key !== modeKey && !arrangement.has(key)) {
