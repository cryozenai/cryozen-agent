 ...
 * host load; the desktop still reads them through the real `cryozen serve`.
 */
function seedArchivedRows(cryozenHome: string, hiddenId: string, controlId: string): void {
  execFileSync(
    RUNTIME_PYTHON,
    [
      '-c',
    ...
    { cwd: REPO_ROOT, env: { ...process.env, CRYOZEN_HOME: cryozenHome }, stdio: 'pipe' },
  )
  expect(fs.existsSync(path.join(cryozenHome, 'state.db'))).toBe(true)
}
...
async function setupSeededMockBackend(): Promise<MockBackendFixture & { controlId: string; hiddenId: string }> {
  ...
  seedArchivedRows(sandbox.cryozenHome, hiddenId, controlId)
