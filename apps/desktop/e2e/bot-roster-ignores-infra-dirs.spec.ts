...
test.beforeAll(async () => {
  ...
  for (const stray of ['sessions', 'logs']) {
    fs.mkdirSync(path.join(sandbox.cryozenHome, 'profiles', stray, 'cron'), { recursive: true })
  }

  // A deleted profile: identity file present, tombstone in profiles/.deleted/<name>.
  const ghost = path.join(sandbox.cryozenHome, 'profiles', 'ghost')
  fs.mkdirSync(ghost, { recursive: true })
  fs.writeFileSync(path.join(ghost, 'profile.yaml'), 'ui_meta:\n  cryozen-bots: {}\n')
  fs.mkdirSync(path.join(sandbox.cryozenHome, 'profiles', '.deleted'), { recursive: true })
  fs.writeFileSync(path.join(sandbox.cryozenHome, 'profiles', '.deleted', 'ghost'), '')
