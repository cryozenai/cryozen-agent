/**
 * Shared E2E fixtures for the Cryozen desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → cryozen serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `cryozen desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `cryozen-e2e-${prefix}-${Math.random()}`))
  const cryozenHome = path.join(root, 'cryozen-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(cryozenHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(cryozenHome, 'config.yaml')
 ...
 */
export function writeEnvFile(cryozenHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(cryozenHome, '.env')
 ...
 */
function writeEmptyConfig(cryozenHome: string): void {
  const configPath = path.join(cryozenHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - CRYOZEN_HOME → sandbox cryozen-home (isolated config/sessions)
 *  - CRYOZEN_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - CRYOZEN_DESKTOP_IGNORE_EXISTING=1 → don't pick up `cryozen` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - CRYOZEN_DESKTOP_CRYOZEN_ROOT → REPO_ROOT (dev checkout resolution)
 *  - CRYOZEN_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    CRYOZEN_HOME: sandbox.cryozenHome,
    CRYOZEN_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    CRYOZEN_DESKTOP_IGNORE_EXISTING: '1',
    CRYOZEN_DESKTOP_CRYOZEN_ROOT: REPO_ROOT,
    CRYOZEN_DESKTOP_APP_NAME: `CryozenE2E-${Date.now()}`,
    ...
    CRYOZEN_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated CRYOZEN_HOME + userData
 * @param env      - the process environment (already has CRYOZEN_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.cryozenHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via CRYOZEN_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`cryozen serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.cryozenHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { CRYOZEN_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Cryozen backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Cryozen.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Cryozen.app', 'Contents', 'MacOS', 'Cryozen')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'cryozen')
}
 ...
 * progress without spawning a real Cryozen backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * CRYOZEN_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    CRYOZEN_DESKTOP_BOOT_FAKE: '1',
    CRYOZEN_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + cryozen-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).CRYOZEN_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).CRYOZEN_DESKTOP_CRYOZEN
  delete (env as Record<string, string | undefined>).CRYOZEN_DESKTOP_CRYOZEN_ROOT
