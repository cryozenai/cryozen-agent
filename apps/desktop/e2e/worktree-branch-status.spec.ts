...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Cryozen E2E'], { cwd: repo })
...
function configureRepoCwd(cryozenHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(cryozenHome, mockUrl)
  fs.appendFileSync(path.join(cryozenHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.cryozenHome, mock.url, repo)
