...
/**
 * E2E smoke tests for the packaged Cryozen desktop app.
 *
 ...
 * (credential stripping, isolated CRYOZEN_HOME + userData, unique app name).
 *
...
test('window opens with the Cryozen title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      cryozenDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).cryozenDesktop?.hud?.open({ sessionId: null })
  )
