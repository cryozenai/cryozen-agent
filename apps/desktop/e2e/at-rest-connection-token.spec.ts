 ...
 * dir (macOS `~/Library/Application Support/Cryozen/connection.json`, Windows
 * `AppData\Roaming\Cryozen\connection.json`). Anything that can read the file
 ...
 *      or in CRYOZEN_HOME (logs included).
 ...
 * `X-Cryozen-Session-Token` header it receives — a dropped or mangled token
 ...
 */
const SENTINEL_TOKEN = 'cryozen-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'CryozenE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-Cryozen-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote Cryozen gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-cryozen-session-token']
 ...
 * backend, so we skip spawning `cryozen serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startCryozen() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    CRYOZEN_DESKTOP_APP_NAME: STABLE_APP_NAME,
    CRYOZEN_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { cryozenDesktop?: Record<string, unknown> }).cryozenDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture Cryozen should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { cryozenDesktop: any }).cryozenDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('with keychain encryption opted IN, a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // CRYOZEN_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.cryozenHome, needles),
      'the gateway session token leaked into a CRYOZEN_HOME file (logs included)',
    ).toEqual([])
