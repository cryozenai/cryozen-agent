...
// record rather than any bot's profile meta (cryozen-agent#89813, #105544).
...
async function storedRoom(page: Page): Promise<{ pinned?: boolean; sectionId?: null | string }> {
  return page.evaluate(
    room => JSON.parse(localStorage.getItem('cryozen.plugin.cryozen-bots.group-chats') || '{}')[room] || {},
    ROOM
  )
}
...
test.beforeAll(async () => {
  ...
  for (const name of ['alpha', 'beta']) {
    await seedBot(sandbox.cryozenHome, mock.url, name)
  }
