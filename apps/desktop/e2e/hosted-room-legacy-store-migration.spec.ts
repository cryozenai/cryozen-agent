...
// Hosted Group Chat rooms live in the gateway, and the Desktop's own `cryozen serve` backend runs
...
const PYTHON = process.env.CRYOZEN_DESKTOP_PYTHON ?? path.join(REPO_ROOT, 'venv', 'bin', 'python')

let fixture: MockBackendFixture | null = null
let rpc: Rpc | null = null

function seedProfile(cryozenHome: string, mockUrl: string, name: string): void {
...
function seedLegacyRoom(cryozenHome: string): void {
  const script = `
import sys
...
`
  const out = execFileSync(PYTHON, ['-c', script, cryozenHome], {
    cwd: REPO_ROOT,
...
/** Locate the Electron-spawned `cryozen serve` backend for this sandbox: its port and session token. */
function findBackend(cryozenHome: string): { port: number; token: string; pid: number } {
  ...
  for (const pidText of pids) {
    ...
    if (vars.get('CRYOZEN_HOME') !== cryozenHome) {
      continue
    }
    const token = vars.get('CRYOZEN_DASHBOARD_SESSION_TOKEN') ?? ''
    // The backend inherits Electron's remote-debugging socket fd too; the serve port is the listener
    // that only the cryozen process holds.
  ...
  throw new Error(`no cryozen serve backend found for ${cryozenHome} (pids: ${pids.join(',')})`)
}
...
test.beforeAll(async () => {
  ...
  writeMockProviderConfig(sandbox.cryozenHome, mock.url)
  writeEnvFile(sandbox.cryozenHome)
  seedProfile(sandbox.cryozenHome, mock.url, 'sentinel')
  seedProfile(sandbox.cryozenHome, mock.url, 'friday')
  seedLegacyRoom(sandbox.cryozenHome)
  expect(fs.existsSync(path.join(sandbox.cryozenHome, 'shared-state.db'))).toBe(false)
  ...
  const backend = findBackend(sandbox.cryozenHome)
...
test('rooms from before the shared-state.db split are still reachable from the Desktop backend (#109775)', async () => {
  ...
  expect(fs.existsSync(path.join(fixture!.sandbox.cryozenHome, 'shared-state.db'))).toBe(true)
  expect(fs.existsSync(path.join(fixture!.sandbox.cryozenHome, 'state.db'))).toBe(true)
})
