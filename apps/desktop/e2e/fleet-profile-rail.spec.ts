 ...
 * second gateway, "Homelab", is a REAL second `cryozen serve` this spec spawns
 * with its own CRYOZEN_HOME, profiles and session token, registered in the v2
...
function findCryozenBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'cryozen')
  ...
  throw new Error('cryozen binary not found: create the repo venv (uv sync) or put cryozen on PATH')
}
...
/**
 * Spawn a second, fully real `cryozen serve` as the remote gateway. Its
 * session token is pinned through CRYOZEN_DASHBOARD_SESSION_TOKEN so the
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        CRYOZEN_HOME: home,
        CRYOZEN_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    seedProfiles(sandbox.cryozenHome, ['research'])
