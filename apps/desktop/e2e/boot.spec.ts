 ...
 * packaged binary) with a real `cryozen serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → cryozen serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Cryozen title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Cryozen')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { cryozenDesktop?: Record<string, unknown> }).cryozenDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → cryozen serve is spawned → WS connects → config
