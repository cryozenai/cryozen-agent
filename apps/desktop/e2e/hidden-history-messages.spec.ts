...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.cryozenHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.cryozenHome)
  const builder = await RealSessionBuilder.start(sandbox.cryozenHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.cryozenHome, mock.url)
  fs.appendFileSync(path.join(sandbox.cryozenHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
