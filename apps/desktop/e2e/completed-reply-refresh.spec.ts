...
test('a delayed history read cannot remove the latest completed reply', async () => {
  ...
  try {
    ...
    await app.evaluate(({ ipcMain }, sessionId) => {
      ...
      const original = handlers.get('cryozen:api')!
      ...
      ipcMain.removeHandler('cryozen:api')
      ipcMain.handle('cryozen:api', async (event: unknown, request: { path: string }) => {
    ...
    await page.evaluate(async sessionId => {
      await (window as any).cryozenDesktop.api({
