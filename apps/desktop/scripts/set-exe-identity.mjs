#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Cryozen icon + version metadata onto the
// built Cryozen.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Cryozen.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `cryozen desktop`, the installer's
...
// path (which rebuilds via `cryozen desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterExtract closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Cryozen.exe>
//
...
// Stamp the Cryozen icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(
  ...
  const options = {
    ...
    'version-string': {
      ProductName: 'Cryozen',
      FileDescription: 'Cryozen',
      CompanyName: 'Cryozen',
      LegalCopyright: 'Copyright (c) 2026 Cryozen'
    }
  }
  ...
  console.log('[set-exe-identity] done — Cryozen icon + identity stamped')
}
