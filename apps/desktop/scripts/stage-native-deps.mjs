...
// user name, i.e. a default %LOCALAPPDATA%\cryozen home): a recursive
  ...
  if (halfInstalledDir) {
    lines.push(
      `[stage-native-deps] ${halfInstalledDir} exists but is not a loadable package — an ` +
        ...
        'To restore read_window_below: close every Cryozen window and gateway so the extract is not ' +
        'interrupted again, then run `cryozen desktop --force-build` — it removes the stale dir before npm.'
    )
  }
  return lines.join('\n')
}
