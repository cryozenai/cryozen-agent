...
const UPSTREAM_URL = 'https://github.com/cryozenai/cryozen-agent.git'
...
const LEGACY_STORAGE_KEY = 'cryozen.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'cryozen.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.cryozenDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `cryozen-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const cryozenHome = join(sandbox, 'cryozen-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(cryozenHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(cryozenHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        CRYOZEN_DESKTOP_APP_NAME: `CryozenShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        CRYOZEN_DESKTOP_CRYOZEN_ROOT: target.targetRoot,
        CRYOZEN_DESKTOP_IGNORE_EXISTING: '1',
        CRYOZEN_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'cryozen-short-session-ab-'))
  const prepared = []
