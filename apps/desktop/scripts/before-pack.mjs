 ...
 * product name (`Cryozen`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Cryozen'
 *
 * This is a hard failure with no obvious cause for the user — `cryozen desktop`
 ...
 * (the mac-unpacked Cryozen.app bundle) and Windows (win-unpacked), so we
 ...
 * Cryozen.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `cryozen desktop --build-only` (cryozen_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Cryozen.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Cryozen'}.exe`
