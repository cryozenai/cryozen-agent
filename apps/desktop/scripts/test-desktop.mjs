...
// no longer bundles the Cryozen Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Cryozen.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Cryozen'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Cryozen.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Cryozen'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default CRYOZEN_HOME for non-sandboxed runs -- matches main.ts's
// resolveCryozenHome(). On Windows it's %LOCALAPPDATA%\cryozen; elsewhere
// it's ~/.cryozen-agent. The fresh-install sandbox launchFresh() sets its own
// CRYOZEN_HOME and never touches this.
const DEFAULT_CRYOZEN_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'cryozen')
  }
  return path.join(os.homedir(), '.cryozen-agent')
})()
const VENV_ROOT = path.join(DEFAULT_CRYOZEN_HOME, 'cryozen-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'cryozen-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.CRYOZEN_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Cryozen-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Cryozen-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Cryozen-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Cryozen-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CRYOZEN_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CRYOZEN_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const cryozenHome = path.join(sandbox, 'cryozen-home')
  ...
  env.CRYOZEN_DESKTOP_CWD = cwd
  env.CRYOZEN_DESKTOP_IGNORE_EXISTING = '1'
  env.CRYOZEN_DESKTOP_TEST_MODE = 'fresh-install'
  env.CRYOZEN_DESKTOP_USER_DATA_DIR = userDataDir
  env.CRYOZEN_HOME = cryozenHome
  delete env.CRYOZEN_DESKTOP_CRYOZEN
  delete env.CRYOZEN_DESKTOP_CRYOZEN_ROOT
  ...
  console.log(`  CRYOZEN_HOME: ${cryozenHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(cryozenHome, 'cryozen-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Cryozen Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of cryozen_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'cryozen-agent', 'cryozen_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Cryozen
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CRYOZEN_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  CRYOZEN_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
