...
How to build Cryozen Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Cryozen surface can also
...
- A **connection/mode apply** (local ↔ remote ↔ SSH) is the soft re-home:
...
- A **runtime home change** (switching the underlying `CRYOZEN_HOME` profile) is
...
Desktop lives at the seams: versions, profiles, local vs remote vs SSH,
...
previews) and the preview pane's `<webview>` (`persist:cryozen-preview`). Neither
...
- `main.ts` installs `electron/preview-guest-preload-entry.ts` via
  `will-attach-webview`, keyed on the `persist:cryozen-preview` partition only.
...
- `PreviewPane` admits `http:`/`https:` only (`src/lib/preview-external.ts`)
  and hands the URL to the existing `cryozen:openExternal` IPC, which applies
...
its shape. "Plugin" means several unrelated things across Cryozen — do not assume
...
process might be a remote gateway this app merely connected to. See
