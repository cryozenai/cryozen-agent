...
<p align="center">
  <a href="https://github.com/cryozenai/cryozen-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
  <a href="https://cryozenai.github.io/cryozen-agent/docs/"><img src="https://img.shields.io/badge/docs-cryozen.github.io-ffd700-FFD700?style=for-the-badge" alt="Documentation"></a>
  <a href="https://github.com/cryozenai/cryozen-agent/discussions"><img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="Discord"></a>
...
**The native desktop app for [Cryozen Agent](../../README.md) — the self-improving AI agent from [Cryozen](https://github.com/cryozenai).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Cryozen surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Cryozen and hear it back.</td></tr>
...
### Install with Cryozen (recommended)

Already have the Cryozen CLI? Just run:

```bash
cryozen desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Cryozen gateway or install Cryozen locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Cryozen Desktop website.](https://cryozenai.github.io/cryozen-agent/).
...
left and right Command keys together in any app. Cryozen captures that app's
frontmost window and attaches the image to the last-active Cryozen composer,
...
```bash
# throwaway CRYOZEN_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Cryozen Agent runtime into `CRYOZEN_HOME`
(`~/.cryozen-agent`, or `%LOCALAPPDATA%\cryozen` on Windows), using the same layout as a
CLI install.
...
- **Cryozen Agent** runs as a headless `cryozen serve` process and exposes the
...
1. `CRYOZEN_DESKTOP_CRYOZEN_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `CRYOZEN_DESKTOP_CRYOZEN`, or `cryozen` on `PATH`
5. a system Python that can import the Cryozen runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Cryozen
...
screen offers **Connect to existing Cryozen** before starting the local installer.
...
terminal commands, and file operations run against the remote Cryozen host, not
...
transport- or Cryozen-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Cryozen-Session-Token`.
...
Boot logs land in `CRYOZEN_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\cryozen\cryozen-agent\.cryozen-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\cryozen\cryozen-agent\venv"
```

> The default Cryozen home on Windows is `%LOCALAPPDATA%\cryozen`. Set the `CRYOZEN_HOME` env var if you've relocated it.

---

## Community

- 💬 [Discord](https://github.com/cryozenai/cryozen-agent/discussions)
- 📖 [Documentation](https://cryozenai.github.io/cryozen-agent/docs/)
- 🐛 [Issues](https://github.com/cryozenai/cryozen-agent/issues)
...
Built by [Cryozen](https://github.com/cryozenai).
