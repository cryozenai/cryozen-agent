...
const CRYOZEN_REF_TYPES = WIRE_REFERENCE_KINDS
type CryozenRefType = (typeof CRYOZEN_REF_TYPES)[number]
...
const CRYOZEN_DIRECTIVE_RE = referenceRe()
...
export const cryozenDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CRYOZEN_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...cryozenDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Cryozen directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.cryozenDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.cryozenDesktop?.readFileDataUrl(id)
