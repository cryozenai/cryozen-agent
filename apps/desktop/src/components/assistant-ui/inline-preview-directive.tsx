 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.cryozen.send(prompt)` (or declarative
 * `data-cryozen-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'cryozen-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'cryozen-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `cryozen.send(prompt)`.
 ...
 *  wires `data-cryozen-send` so declarative HTML works with zero script:
 *  `<button data-cryozen-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.cryozen={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-cryozen-send]"):null;' +
    'if(el)send(el.getAttribute("data-cryozen-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `cryozen serve` — renders live.)
