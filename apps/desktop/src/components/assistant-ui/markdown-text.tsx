...
import { paragraphPlainText, TranscriptDirectiveLeaf, useResolvedDirective } from './transcript-directive'
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.cryozenDesktop && text && normalizeExternalUrl(text) === target) {
...
function MarkdownParagraph({
  ...
  const directive = useResolvedDirective(scratchpad ? null : plain)

  // A paragraph that is one claimed directive renders as the card alone.
  if (directive) {
    return <TranscriptDirectiveLeaf streaming={streaming} text={directive} />
  }
