...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Cryozen Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Cryozen docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Cryozen Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Cryozen docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://cryozenai.github.io/cryozen-agent/docs' },
    ...
    expect(view.status).toBe('error')
    expect(view.title).toBe('Failed to open cryozenai.github.io/cryozen-agent/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://cryozenai.github.io/cryozen-agent/docs' },
        result: { success: true, url: 'https://cryozenai.github.io/cryozen-agent/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened cryozenai.github.io/cryozen-agent/docs')
  })
})
...
describe('buildToolView title actions', () => {
  ...
  it('renders compact terminal titles for session 20260624_231846_bdbd1e commands', () => {
    const rows = [
      [
        'cd /Users/testuser/www/sample-site && pnpm run lint 2>&1 | tail -20; echo "lint_exit=${PIPESTATUS[0]}"',
        'Ran pnpm run lint'
      ],
      [
        'cd /Users/testuser/www/sample-site && pnpm run build 2>&1 | tail -20; echo "build_exit=${PIPESTATUS[0]}"',
        'Ran pnpm run build'
      ],
      [
      ...
      [
        'cd /Users/testuser/www/sample-site && COREPACK_ENABLE_DOWNLOAD_PROMPT=0 corepack pnpm@10.20.0 --version 2>&1 | tail -3',
        'Ran COREPACK_ENABLE_DOWNLOAD_PROMPT=0 corepack pnpm@10.20.0 --version'
      ],
      [
        'cd /Users/testuser/www/sample-site && COREPACK_ENABLE_DOWNLOAD_PROMPT=0 corepack use pnpm@10.20.0 2>&1 | tail -10; echo "exit=$?"',
  ...
  it('uses inherited backend context for live terminal rows', () => {
    const view = buildToolView(
      part({
        args: {
          command: 'cd /Users/testuser/www/sample-site && pnpm run lint 2>&1 | tail -20',
