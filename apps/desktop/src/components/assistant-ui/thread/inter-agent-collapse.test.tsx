...
const DELIVERY = 'Message from 🤖 Cryozen (@cryozen): please check the build'
...
describe('inter-agent collapse gate', () => {
  it('keeps the report after a relayed answer to this bot\u2019s own dispatch expanded', async () => {
    // Bot Mode round trip seen from the DISPATCHING bot: it messaged @cryozen,
    // the answer came back as an inbound "Message from" row, and the next
    // assistant message is its report to the human — not a reply to cryozen.
    render(
      <Harness
        messages={[
          user('u1', 'ask cryozen for the list'),
          dispatch('a0', '@Cryozen'),
          user('u2', DELIVERY),
          assistant('a1', 'here is the list cryozen sent', false)
        ]}
      />
    )

    await screen.findByText('here is the list cryozen sent')
    ...
    render(
      <Harness
        messages={[
          ...
          user('u4', 'Message from 🤖 Cryozen (@cryozen): unsolicited: build broke'),
