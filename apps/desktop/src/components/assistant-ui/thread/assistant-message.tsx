...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's CRYOZEN_HOME/logs. On a
  ...
  // An expired/revoked OAuth grant (HTTP 401 on openai-codex / xai-oauth / ...):
  ...
  // CRYOZEN_HOME/logs, and its parent is the Cryozen data folder itself (what
  ...
  const openLocalDir = useCallback(async (resolve: (logsRoot: string) => string, failedMessage: string) => {
    try {
      const root = await window.cryozenDesktop?.logsRoot?.()
      ...
      const result = await window.cryozenDesktop?.openDir?.(resolve(root))
  ...
  const openCryozenFolder = useCallback(
    () => openLocalDir(root => root.replace(/[\\/]+logs[\\/]*$/, ''), copy.errorOpenCryozenFolderFailed),
    [copy.errorOpenCryozenFolderFailed, openLocalDir]
  )
  ...
  const localFolders = Boolean(window.cryozenDesktop?.logsRoot)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {plan.openCryozenFolder && localFolders && (
        <button className="aui-error-action" onClick={() => void openCryozenFolder()} type="button">
          {copy.errorOpenCryozenFolder}
        </button>
      )}
      {plan.retry && (
