...
describe('agent message detection', () => {
  it('matches the Bot Mode delivery prefix with sender and body', () => {
    const m = AGENT_MESSAGE_RE.exec('Message from 🤖 Cryozen: hello there')

    expect(m?.[1]?.trim()).toBe('Cryozen')
  ...
  it('recognizes a relayed sender re-stamped @handle@connection, keeping the bare handle for the avatar', () => {
    // The receiving gateway re-stamps a relayed "@cryozen" as "@cryozen@<connection>"
    // (#103731) — still an agent notice, and the avatar resolves by profile handle.
    const m = AGENT_MESSAGE_RE.exec('Message from 🤖 cryozen (@cryozen@Cloud-1): status?')

    expect(m?.[1]?.trim()).toBe('cryozen')
    expect(m?.[2]).toBe('cryozen')
  ...
  it('does not match prose that merely contains the phrase', () => {
    expect(AGENT_MESSAGE_RE.test('I got a Message from 🤖 Cryozen: earlier')).toBe(false)
