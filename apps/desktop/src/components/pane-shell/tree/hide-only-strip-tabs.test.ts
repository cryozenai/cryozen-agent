...
function sessionsBotsTree() {
  ...
  registerPane('cryozen-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'cryozen-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('labels the zone menu rows from the pane string localizer, not the register-time title', () => {
    ...
    expect(hideOnlyZoneTabs('g-side').map(tab => tab.title)).toEqual(['Сеансы', 'cryozen-bots:pane'])
  })

  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    sessionsBotsTree()

    expect(setStripTabHidden('cryozen-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('cryozen-bots:pane')
    expect($hiddenStripTabs.get()).toContain('cryozen-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('cryozen-bots:pane')

    expect(setStripTabHidden('cryozen-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('cryozen-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('cryozen-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('cryozen-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('cryozen.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('cryozen-bots:pane')
    ...
    revealTreePane('cryozen-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('cryozen-bots:pane')
    expect(window.localStorage.getItem('cryozen.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'cryozen-bots:pane', title: 'cryozen-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'cryozen-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
