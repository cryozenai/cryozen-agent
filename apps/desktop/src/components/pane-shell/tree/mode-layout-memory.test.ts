...
it('restores independently customized modes through real pane bindings and reloads', async () => {
  ...
  for (const deck of decks) {
    for (const simpleId of ['sidebar-left', 'sidebar-right']) {
      ...
      const saved = window.localStorage.getItem('cryozen.desktop.layoutTree.v2')
      ...
      expect(window.localStorage.getItem('cryozen.desktop.layoutTree.v2')).toBe(saved)
...
it.each(['advanced', 'simple'] as const)(
  ...
  async initialMode => {
    ...
    window.localStorage.setItem('cryozen.desktop.layoutTree.v2', raw)
    window.localStorage.setItem('cryozen.desktop.paneStates.v1', JSON.stringify(legacyPanes))
    window.localStorage.setItem('cryozen.desktop.layoutPreset.active', 'custom')
    window.localStorage.setItem('cryozen.desktop.hiddenStripTabs.v1', '["bots"]')

    if (initialMode === 'simple') {
      window.localStorage.setItem('cryozen.desktop.interfaceMode.v1', 'simple')
    }
