...
it('resolves once and stops ticking by default', () => {
  const { container } = render(<DecodeText prefix={1} text="CRYOZEN" />)

  act(() => vi.advanceTimersByTime(settleTicks('CRYOZEN') * TICK_MS))
  expect(container.textContent).toBe('CRYOZEN')
...
it('keeps replaying only when the caller asks for loop', () => {
  const { container } = render(<DecodeText loop prefix={1} text="CRYOZEN" />)
