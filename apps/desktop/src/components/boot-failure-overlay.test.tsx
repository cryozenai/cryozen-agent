...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Cryozen gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.cryozenDesktop
  Object.defineProperty(window, 'cryozenDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'cryozenDesktop', { configurable: true, value: original })
}
...
describe('BootFailureOverlay', () => {
  it('keeps keyboard focus inside the recovery surface', () => {
    ...
    const recoverySurface = screen.getByRole('dialog', { name: /Cryozen couldn't start/i })
