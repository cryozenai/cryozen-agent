...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.cryozenDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.cryozenDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.cryozenDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }

  // Clear this gateway's stale auth first, then re-establish it through the
  // connection's native/embedded OAuth login flow.
  ...
  const openLogs = () => void window.cryozenDesktop?.revealLogs().catch(() => undefined)
  ...
  if (view === 'connect') {
    return (
      <BootFailureModal title={copy.gatewaySettings}>
        <div className="flex max-h-[86vh] w-full max-w-[46rem] flex-col overflow-hidden rounded-xl border border-(--stroke-brand) bg-(--ui-chat-bubble-background) shadow-brand">
  ...
  return (
    <BootFailureModal>
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--stroke-brand) bg-(--ui-chat-bubble-background) shadow-brand">
