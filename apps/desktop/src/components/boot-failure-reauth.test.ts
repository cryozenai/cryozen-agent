...
describe('isRemoteReauthError', () => {
  ...
  it('ignores non-auth boot errors and nullish', () => {
    expect(isRemoteReauthError('Cryozen background process exited during startup.')).toBe(false)
...
describe('shouldApplyPostBootProgressError', () => {
  it('applies only confirmed reauth after a healthy boot — not transient ticket blips', () => {
    ...
    expect(
      shouldApplyPostBootProgressError(
        'Could not reach the remote Cryozen gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    ).toBe(false)
...
describe('deriveProviderShape', () => {
  ...
  it('OAuth shape when the provider is a redirect IDP', () => {
    expect(deriveProviderShape([{ name: 'acme', displayName: 'Cryozen', supportsPassword: false }])).toEqual({
      isPassword: false,
      providerLabel: 'Cryozen'
    })
  })
  ...
  it('mixed deployment keeps generic OAuth copy (not every provider is password)', () => {
    const shape = deriveProviderShape([
      ...
      { name: 'acme', displayName: 'Cryozen', supportsPassword: false }
    ])

    expect(shape.isPassword).toBe(false)
    expect(shape.providerLabel).toBe('Username & Password / Cryozen')
  })
...
describe('signInLabel', () => {
  ...
  it('OAuth gateway names the provider', () => {
    expect(signInLabel({ url: 'x', isPassword: false, providerLabel: 'Cryozen' })).toBe('Sign in with Cryozen')
  })
