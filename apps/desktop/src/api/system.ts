...
import { capabilityScoped, cryozenApi, type OwnerScope, ownerScoped, type ProfileScope, profileScoped } from './client'
...
export function getMemoryProviderConfig(provider: string, profile?: null | string): Promise<MemoryProviderConfig> {
  return cryozenApi<MemoryProviderConfig>({
...
export function startMemoryProviderOAuth(
  ...
  return cryozenApi<MemoryProviderOAuthStatus>({
    ...profileScoped(profile),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `cryozen memory` / `cryozen curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return cryozenApi<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return cryozenApi<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return cryozenApi<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return cryozenApi<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
export function updateCryozen(): Promise<ActionResponse> {
  return cryozenApi<ActionResponse>({
    ...profileScoped(),
    path: '/api/cryozen/update',
    method: 'POST'
  })
}
...
export function checkCryozenUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return cryozenApi<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/cryozen/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200, profile?: ProfileScope): Promise<ActionStatusResponse> {
  return window.cryozenDesktop.api<ActionStatusResponse>({
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return cryozenApi<AudioTranscriptionResponse>({
...
export function speakText(text: string, owner?: OwnerScope): Promise<AudioSpeakResponse> {
  return cryozenApi<AudioSpeakResponse>({
    ...ownerScoped(owner),
...
export function setTtsLease(lease: string, active: boolean): Promise<AudioTtsLeaseResponse> {
  return cryozenApi<AudioTtsLeaseResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(profile?: null | string): Promise<ElevenLabsVoicesResponse> {
  return cryozenApi<ElevenLabsVoicesResponse>({
...
export function getGhAuthStatus(refresh = false): Promise<{ available: boolean; authenticated: boolean }> {
  return cryozenApi<{ available: boolean; authenticated: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `cryozen doctor` / `cryozen security
// audit` / `cryozen backup` / `cryozen debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return cryozenApi<ActionResponse>({ ...profileScoped(), path: '/api/ops/doctor', method: 'POST', body: {} })
}
...
export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return cryozenApi<ActionResponse & { archive?: string }>({
    ...profileScoped(),
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return cryozenApi<DebugShareResponse>({
    ...profileScoped(),
