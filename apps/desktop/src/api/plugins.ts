import { reconnectBackoffDelayMs } from '@cryozen/shared'

import type { CryozenConnection } from '@/global'
import { RECONNECT_ATTEMPT_TIMEOUT_MS, withTimeout } from '@/lib/with-timeout'

import { cryozenApi, getApiRequestConnection, getApiRequestProfile, profileScoped } from './client'
 ...
 *
 *  Exported for tests. */
export async function activeConnection(): Promise<CryozenConnection> {
  ...
  return withTimeout(
    window.cryozenDesktop.getConnection(profile),
    RECONNECT_ATTEMPT_TIMEOUT_MS,
    `Timed out connecting to profile "${profile}"`
  )
}

/** Options for a plugin REST call — mirrors the app's own `cryozenDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see CryozenApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.cryozenDesktop?.api) {
    throw new Error('Cryozen desktop bridge unavailable')
  }
