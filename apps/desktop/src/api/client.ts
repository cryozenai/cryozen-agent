import { JsonRpcGatewayClient } from '@cryozen/shared'

import type { CryozenApiRequest } from '@/global'
...
// "Timed out connecting to Cryozen backend" that hangs the UI (#48504).
//
...
export const GATEWAY_NOT_CONNECTED_MESSAGE = 'Cryozen gateway is not connected'

export class CryozenGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Cryozen gateway connection closed',
      connectErrorMessage: 'Could not connect to Cryozen gateway',
      ...
      onUnhandledRequest: request =>
        console.warn(`[gateway] Cryozen Desktop has no server-request registry for ${request.method} (${request.id})`),
 ...
 *  the ambient tag `cryozenApi` spreads underneath (as capabilityScoped does). */
...
// process to route them to that host (cryozen:api's registry branch). Null
...
export function cryozenApi<T>(request: CryozenApiRequest): Promise<T> {
  return window.cryozenDesktop.api<T>({ ...connectionScoped(), ...request })
}
