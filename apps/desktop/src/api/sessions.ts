 ...
 * under multiplex-only there is no per-profile backend whose CRYOZEN_HOME could
...
export async function listSessions(
  ...
  const result = await cryozenApi<PaginatedSessions>({
    ...profileScoped(),
    path:
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await cryozenApi<SidebarSessionsResponse>({
      ...profileScoped(),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return cryozenApi<SessionSearchResponse>({
...
export function getSessionMessages(
  ...
  return cryozenApi<SessionMessagesResponse>({
    ...sessionScope,
...
export function renameSession(
  ...
  return cryozenApi<{ ok: boolean; title: string }>({
    ...(owner ? { profile: owner } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
