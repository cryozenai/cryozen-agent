...
import { capabilityScoped, cryozenApi, type ProfileScope, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'
...
export function getStatus(): Promise<StatusResponse> {
  return cryozenApi<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return cryozenApi<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getCryozenConfig(profile?: string): Promise<CryozenConfig> {
  return cryozenApi<CryozenConfig>({
...
): Promise<CryozenConfigRecord> {
  const origin = capabilityScoped(profile ?? undefined)

  const record = await window.cryozenDesktop.api<CryozenConfigRecord>({ ...origin, ...request })
...
export function getCryozenConfigRecord(
  profile?: ProfileScope,
...
export function getCryozenConfigDefaults(): Promise<CryozenConfigRecord> {
...
export function getCryozenConfigSchema(profile?: null | string): Promise<ConfigSchemaResponse> {
  return cryozenApi<ConfigSchemaResponse>({
    ...profileScoped(profile),
    path: '/api/config/schema'
  })
}

export function saveCryozenConfig(
  config: CryozenConfigRecord,
  profile?: ProfileScope,
  { preserveLanguage = false }: { preserveLanguage?: boolean } = {}
): Promise<{ ok: boolean }> {
  return window.cryozenDesktop.api<{ ok: boolean }>({
...
/** Capability-scoped counterpart of saveCryozenConfig — writes the config of
 *  the profile/connection the Capabilities scope selector points at (possibly
 *  on another registered gateway), mirroring getCryozenConfigRecord. */
export function saveCryozenConfigRecord(config: CryozenConfigRecord, profile?: ProfileScope): Promise<{ ok: boolean }> {
  return window.cryozenDesktop.api<{ ok: boolean }>({
    ...resolveConfigWriteScope(config, profile),
    path: '/api/config',
    method: 'PUT',
...
export function getEnvVars(profile?: null | string): Promise<Record<string, EnvVarInfo>> {
  return cryozenApi<Record<string, EnvVarInfo>>({
    ...profileScoped(profile),
    path: '/api/env'
  })
}
...
export function revealEnvVar(key: string, profile?: ProfileScope): Promise<{ key: string; value: string }> {
  return window.cryozenDesktop.api<{ key: string; value: string }>({
    ...capabilityScoped(profile),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function getCustomEndpoints(profile?: null | string): Promise<CustomEndpointsResponse> {
  return cryozenApi<CustomEndpointsResponse>({
    ...profileScoped(profile),
...
export function validateCustomEndpoint(
  ...
  return cryozenApi<CustomEndpointValidationResponse>({
    ...profileScoped(profile),
...
export function activateCustomEndpoint(
  ...
  return cryozenApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(profile),
...
export function listOAuthProviders(profile?: null | string): Promise<OAuthProvidersResponse> {
  return cryozenApi<OAuthProvidersResponse>({
    ...profileScoped(profile),
...
export function disconnectOAuthProvider(
  ...
  return cryozenApi<{ ok: boolean; provider: string }>({
    ...profileScoped(profile),
...
export function startOAuthLogin(providerId: string, profile?: ProfileScope): Promise<OAuthStartResponse> {
  return window.cryozenDesktop.api<OAuthStartResponse>({
    ...capabilityScoped(profile),
...
export function submitOAuthCode(
  ...
  return cryozenApi<OAuthSubmitResponse>({
    ...profileScoped(profile),
...
export function pollOAuthSession(
  ...
  return window.cryozenDesktop.api<OAuthPollResponse>({
    ...capabilityScoped(profile),
