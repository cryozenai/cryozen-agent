import type { McpCatalogResponse, McpServerSummary } from '@/types/cryozen'
...
export function testMcpServer(name: string, profile?: ProfileScope): Promise<McpTestResult> {
  return window.cryozenDesktop.api<McpTestResult>({
 ...
 *  `saveCryozenConfig`, this REPLACES rather than deep-merges, so deletes,
...
// `cryozen mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveCryozenConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return cryozenApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(
  ...
  return window.cryozenDesktop.api<McpServerSummary>({
    ...capabilityScoped(profile),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: ProfileScope, detectApps = false): Promise<McpCatalogResponse> {
  return window.cryozenDesktop.api<McpCatalogResponse>({
    ...capabilityScoped(profile),
...
export function installMcpCatalogEntry(
  ...
  return window.cryozenDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>(
    {
      ...capabilityScoped(profile),
