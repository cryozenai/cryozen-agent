...
import { getCryozenConfig } from '@/cryozen'
...
import { useCryozenConfig } from './use-cryozen-config'

vi.mock('@/cryozen', () => ({
  getCryozenConfig: vi.fn(),
  getCryozenConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'cryozen.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getCryozenConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getCryozenConfig>>)

describe('useCryozenConfig refreshCryozenConfig', () => {
  beforeEach(() => {
  ...
  it('mirrors display.show_reasoning and resets a missing key to the default', async () => {
    ...
    const { result } = renderHook(() => useCryozenConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshCryozenConfig()
    })
  ...
  // `agent.reasoning_effort: high` to Cryozen' built-in medium.
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useCryozenConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getCryozenConfig>>>()
    vi.mocked(getCryozenConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useCryozenConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshCryozenConfig(true)
    })
    expect(getCryozenConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getCryozenConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getCryozenConfig>>>()
    vi.mocked(getCryozenConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useCryozenConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshCryozenConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getCryozenConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getCryozenConfig>>>()
    vi.mocked(getCryozenConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshCryozenConfig(true)
      refreshC = result.current.refreshCryozenConfig(true)
    })
