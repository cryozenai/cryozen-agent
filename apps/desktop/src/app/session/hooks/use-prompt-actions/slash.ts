import { skillInvocationText } from '@cryozen/shared'
import { parseCommandDispatch, parseSlashCommand } from '@cryozen/shared'
import { type MutableRefObject, useCallback, useRef } from 'react'

import { prepareDefaultNewSession } from '@/app/session/new-session-route'
import { getProfiles } from '@/cryozen'
...
const renderWakeStatus = (status: WakeStatusResponse): string => {
  const lines = [
    ...
    `Phrase: "${status.phrase?.trim() || 'hey cryozen'}"`,
  ...
  return useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        // lease. Sending it through slash.exec creates a separate CryozenCLI in
