...
export async function readImageForRemoteAttach(
  ...
  const dataUrl = await window.cryozenDesktop?.readFileDataUrl(filePath)
...
export async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.cryozenDesktop?.readFileDataUrlForAttach ?? window.cryozenDesktop?.readFileDataUrl
 ...
 * - `session.usage`:    { calls, input, output, total, account_lines? }
...
export function renderRpcResult(response: unknown, name: string): string {
  ...
  if ('total' in r || 'input' in r || 'output' in r || 'calls' in r) {
    ...
    // Provider account limits (e.g. Codex quota windows) — same order as CLI /usage.
    if (Array.isArray(r.account_lines)) {
      for (const line of r.account_lines) {
