...
export function handleStatusEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'notification.show') {
    // Driver-agnostic agent notice. The Ink TUI renders these in its status
    // bar; the desktop renders them as toasts. The notice key doubles as the
    // toast id, so an escalating notice replaces in place instead of
    // stacking. Shown regardless of which session is focused.
  ...
  if (event.type === 'notification.clear') {
    // Key-matched dismissal. notify() keys the toast by the notice key, so this maps
  ...
  if (event.type === 'error') {
    const errorMessage = payload?.message || 'Cryozen reported an error'
