import type { BillingBlock } from '@cryozen/shared'
...
import { clearBillingBlock, runBillingRecovery, setBillingBlock } from '@/store/billing-block'
 ...
 * billing-specific toast — never the generic "Cryozen error" — with a CTA to
 * the provider's billing page when one is known.
 */
function surfaceBillingBlock(sessionId: string, raw: unknown): void {
  ...
  notify({
    ...
    durationMs: 0,
    ...(block.billing_url
      ? { action: { label: translateNow('billingBlock.addCredits'), onClick: () => runBillingRecovery(block) } }
      : {})
  })
}
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Cryozen reported an error',
