...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshCryozenConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshCryozenConfig — two REST calls (config + defaults) per event, per
