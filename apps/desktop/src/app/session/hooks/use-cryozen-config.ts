...
import { getCryozenConfig, getCryozenConfigDefaults } from '@/cryozen'
...
interface CryozenConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useCryozenConfig({ activeSessionIdRef }: CryozenConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshCryozenConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getCryozenConfig(), getCryozenConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Cryozen' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshCryozenConfig, sttEnabled, voiceMaxRecordingSeconds }
}
