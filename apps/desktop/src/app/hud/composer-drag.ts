...
function setWorkspaceTransfer(transferring: boolean): void {
  window.cryozenDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.cryozenDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.cryozenDesktop?.hud?.beginMove?.()
}
 ...
 * (see cryozen:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.cryozenDesktop?.hud?.endMove?.()
      }
