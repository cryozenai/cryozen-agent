...
import { useCryozenConfigRecord } from '../hooks/use-config-record'
...
import { useCryozenConfig } from '../session/hooks/use-cryozen-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // "Restart Cryozen" from a toast: recycle the local backend the user is
  ...
  useEffect(() => {
    ...
    if (backendRestartRequest > 0) {
      ...
      void window.cryozenDesktop
        ...
        .catch(err => notifyError(err, translateNow('notifications.errors.restartCryozenFailed')))
    }
  ...
  const requestGateway = useMemo<AmbientGatewayRequest>(
    () =>
  ...
  const { refreshCryozenConfig, sttEnabled, voiceMaxRecordingSeconds } = useCryozenConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('cryozen:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('cryozen:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshCryozenConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshCryozenConfig])
  ...
  const configRecord = useCryozenConfigRecord()
