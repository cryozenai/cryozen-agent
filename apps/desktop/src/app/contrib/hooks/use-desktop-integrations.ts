...
import { pathFromCryozenDeepLink, resolveCryozenOpenPath } from '@/lib/cryozen-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Cryozen" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.cryozenDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.cryozenDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.cryozenDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.cryozenDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // cryozen://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.cryozenDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future cryozenDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveCryozenOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.cryozenDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (cryozen://index-network/intent/1, cryozen://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromCryozenDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.cryozenDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.cryozenDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.cryozenDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.cryozenDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
