...
import { getCryozenConfigRecord, saveMcpServers } from '@/cryozen'
...
import { setCryozenConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `cryozen://mcp/install` deep links. The payload is
 ...
 * confirming lets Cryozen spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getCryozenConfigRecord())
      ...
      setCryozenConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
