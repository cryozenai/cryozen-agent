...
import { CryozenGateway } from '@/cryozen'
import type { CryozenConnection, CryozenWindowState, DesktopBootProgress } from '@/global'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.cryozenDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: CryozenGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    let pendingWindowState: CryozenWindowState | null = null

    const publish = (next: CryozenConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Cryozen...". Once the
    ...
    // The other way a cold boot concludes. Main keeps startCryozen() available
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Cryozen…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Cryozen backend'
        )
        ...
        // as an opaque "Could not connect to Cryozen gateway". resolveGatewayWsUrl
        ...
        if (!manual || (isActivePrimary() && gatewayActivationEpoch() === manual.activationEpoch)) {
          reconcileBusyStatesOnReconnect()
          await callbacksRef.current.refreshCryozenConfig().catch(() => undefined)
    ...
    async function getWindowBackend(startup = false): Promise<CryozenConnection> {
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshCryozenConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const onBootProgress = (payload: DesktopBootProgress) => {
      ...
      // Soft switch / post-boot startCryozen re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new CryozenGateway()
    ...
    configureGatewayRegistry({
      onServerRequest: request => {
        if (!callbacksRef.current.handleServerRequest(request)) {
          request.fail(JSON_RPC_METHOD_NOT_FOUND, `Cryozen Desktop cannot answer ${request.method}`)
        }
      },
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen({ explicit: true }))) {
          throw new Error('Cryozen gateway is not connected')
        }
    ...
    const offExit = desktop.onBackendExit(() => {
      ...
      notify({
        ...
        durationMs: 0,
        action: {
          label: translateNow('boot.errors.restartCryozen'),
    ...
    async function boot() {
      ...
      try {
        ...
        // round-trip must not hang "Starting Cryozen…" forever. Initial boot
        ...
        const conn = await withTimeout(
          ...
          BACKEND_BOOT_WAIT_TIMEOUT_MS,
          'Timed out connecting to Cryozen backend'
        )
        ...
        // reaches the recovery affordance instead of hanging "Starting Cryozen…".
        const wsUrl = await withTimeout(
          resolveDesktopGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshCryozenConfig(),
          ...
          // retry it), not brick boot behind the "Cryozen couldn't start"
