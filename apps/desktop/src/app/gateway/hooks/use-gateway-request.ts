import { isGatewayReauthRequired } from '@cryozen/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as CryozenGateway | null
  const gatewayRef = useRef<CryozenGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.cryozenDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<CryozenGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as CryozenGateway | null
      }),
    []
  )
