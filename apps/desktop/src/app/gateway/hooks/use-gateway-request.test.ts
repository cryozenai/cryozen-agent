...
vi.mock('@/cryozen', async importOriginal => {
  const actual = await importOriginal<typeof CryozenModule>()

  class FakeCryozenGateway {
  ...
  return { ...actual, CryozenGateway: FakeCryozenGateway }
})

import type * as CryozenModule from '@/cryozen'
import type { CryozenGateway } from '@/cryozen'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as CryozenGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as CryozenGateway, 'default')
  $gateway.set(primary as unknown as CryozenGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.cryozenDesktop, 'getGatewayWsUrlFor')
  ...
  it.each(['token', 'oauth'] as const)(
    ...
    async authMode => {
      ...
      Object.defineProperty(window, 'cryozenDesktop', { configurable: true, value: desktop })

      const primary = makePrimaryGateway()
      primary.request.mockRejectedValueOnce(new Error('connection closed')).mockResolvedValueOnce({ recovered: true })
      setPrimaryGateway(primary as unknown as CryozenGateway, connection.profile)
      $gateway.set(primary as unknown as CryozenGateway)
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as CryozenGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { cryozenDesktop: unknown }).cryozenDesktop = { getConnection }
    $gateway.set(dropped)
