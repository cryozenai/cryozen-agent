import { compactNumber } from '@cryozen/shared'
...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updateCryozen } from '@/cryozen'
import type { ActionStatusResponse, AnalyticsResponse, SessionInfo, StatusResponse } from '@/cryozen'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `cryozen logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateCryozen()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.cryozenActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
      ...
      <div className="min-h-0 flex-1">
        {loading ? (
          ...
          <EmptyPanel
            ...
            description={cc.usageEmpty(period)}
          />
        )}
      </div>
    )
