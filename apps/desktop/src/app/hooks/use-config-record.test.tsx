...
import { bindConfigReadOrigin, getCryozenConfigRecord } from '@/cryozen'
import { queryClient } from '@/lib/query-client'

import { CRYOZEN_CONFIG_KEY, useCryozenConfigRecord } from './use-config-record'
...
it('updates the write origin when a refetch replaces the displayed record', async () => {
  ...
  vi.mocked(getCryozenConfigRecord).mockResolvedValueOnce(first).mockResolvedValueOnce(second)

  const { result } = renderHook(() => useCryozenConfigRecord(), { wrapper })
  ...
  await queryClient.invalidateQueries({ queryKey: CRYOZEN_CONFIG_KEY })
