...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__cryozenTourEngine) {
    ${driverIife}
    w.__cryozenTourHolder = {};
    w.__cryozenTourCollect = (${collectTourTargets.toString()});
    w.__cryozenTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__cryozen-tour-style')) {
    var style = document.createElement('style');
    style.id = '__cryozen-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__cryozenTourEngine(
    w.driver.js.driver,
    w.__cryozenTourHolder,
    ${JSON.stringify(action)},
    w.__cryozenTourCollect,
    document
  ));
})()`
}
