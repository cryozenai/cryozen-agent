        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.cryozenDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.cryozenDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Cryozen to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.cryozenDesktop?.watchPreviewFile ||
      !window.cryozenDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.cryozenDesktop.onPreviewFileChanged(payload => {
    ...
    void window.cryozenDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.cryozenDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.cryozenDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:cryozen-preview')
    ...
    // `cryozen:openExternal` channel, never a popup side effect.
    const onGuestExternal = (event: Event) => {
      ...
      if (admitPreviewExternalUrl(url)) {
        void window.cryozenDesktop?.openExternal?.(url)
      }
    }
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.cryozenDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.cryozenDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.cryozenDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.cryozenDesktop?.openExternal(currentUrl)
                : undefined
            }
