...
const preamble = () => `  var w = window;
  ...
  w.__cryozenActHolder = w.__cryozenActHolder || {};
  w.__cryozenAct = ${actEngineSource()};
  w.__cryozenWatch_fn = (${watchInPage.toString()});
  w.__cryozenWatchTag = ${WATCH_TAG};
  var holder = w.__cryozenActHolder;
  var act = function (a) { return w.__cryozenAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__cryozenWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__cryozenHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'CRYOZEN-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
