...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/cryozen'
...
const SIDEBAR_NAV: SidebarNavItem[] = [
  ...
  // Artifacts and Scheduled jobs are outputs of running Cryozen the developer
...
// highlight markers (sqlite snippet() delimiters — see cryozen_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('cryozen:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('cryozen:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask Cryozen to
