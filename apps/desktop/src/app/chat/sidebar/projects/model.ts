...
import type { SessionInfo } from '@/cryozen'
...
// zero Cryozen sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, CryozenGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, CryozenGitWorktree[]>>({})
