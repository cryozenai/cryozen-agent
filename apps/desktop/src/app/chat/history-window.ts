...
import { capabilityScoped, cryozenApi, type ProfileScope } from '@/api/client'
import {
  cachedTimelineIndex,
  previousPromptRowId,
  timelineIndexKey
...
import type { SessionMessagesResponse } from '@/types/cryozen'
...
export async function fetchHistoryWindow(
  ...
  const response = await cryozenApi<HistoryWindowResponse>({
    ...route,
