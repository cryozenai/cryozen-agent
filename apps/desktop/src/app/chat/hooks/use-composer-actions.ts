...
export const CRYOZEN_PATHS_MIME = 'application/x-cryozen-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Cryozen sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.cryozenDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(CRYOZEN_PATHS_MIME)
...
export function useComposerActions({
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob, isCurrent: () => boolean = () => true) => {
      ...
      try {
        ...
        const savedPath = await window.cryozenDesktop?.saveImageBuffer(data, blobExtension(blob), name)
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.cryozenDesktop?.saveClipboardImage()
   ...
   * Cryozen-managed composer-pastes file via the main process, then attached
  ...
  const attachPastedText = useCallback(
    async (text: string) => {
      const save = window.cryozenDesktop?.savePastedText
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.cryozenDesktop?.getPathForFile ? window.cryozenDesktop.getPathForFile(file) : ''
