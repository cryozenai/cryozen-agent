...
describe('PluginsTab', () => {
  ...
  it('renders a unified package as ONE row with a Desktop switch and an Agent switch', () => {
    $pluginRecords.set({
      media: { id: 'media', name: 'Media Studio', kind: 'disk', status: 'loaded', packageName: 'cryozen-media-studio' }
    })
    $agentPlugins.set([
      {
        description: '',
        key: 'cryozen-media-studio',
        name: 'cryozen-media-studio',
        source: 'git',
        status: 'disabled',
        version: '1'
      }
  ...
  it('offers "Install here" for a desktop half whose agent half is not in the selected profile', async () => {
    $pluginRecords.set({
      media: {
        ...
        kind: 'disk',
        status: 'loaded',
        packageName: 'cryozen-media-studio',
        packageOrigin: { repo: 'https://github.com/example-org/media-studio.git', sha: 'abc' }
      }
    })
    ...
    await waitFor(() => {
      expect($pluginInstallRequest.get()).toMatchObject({
        legacyHint: 'agent',
        profile: 'workbot',
        repo: 'https://github.com/example-org/media-studio.git',
        sha: 'abc'
      })
    })
  })
  ...
  it('opens the dual-target install modal from a catalog pick message', async () => {
    ...
    window.dispatchEvent(
      new MessageEvent('message', {
        ...
        origin: 'https://cryozenai.github.io/cryozen-agent'
      })
    )
