...
import { isToolsStatusPhase, ToolsStatus, ToolsWash } from './tools-status'
import { ToolsSummary, useShowAllTools } from './tools-summary'
import type { QuickAction, ToolRowModel, ToolsEditorCounts, ToolsFilter } from './types'
...
export function ToolsList(props: ToolsListProps) {
  ...
  if (isToolsStatusPhase(phase)) {
    return <ToolsStatus connectorName={props.connectorName} phase={phase} />
  }
...
function ToolsListBody(props: ToolsListProps) {
  ...
  const { connectorName, editor, listKey, preview = false, summaryTitle, tools } = props
    ...
    <div className="flex min-h-0 flex-1 flex-col" data-slot="tools-list">
      {all.open ? (
        <ToolsFilterBar
          ...
          quickActions={preview ? EMPTY_QUICK_ACTIONS : chrome.quickActions}
          tiny={chrome.tiny}
          total={tools.length}
        />
      ) : (
