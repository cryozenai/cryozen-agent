...
import { getProfiles, type ProfileScope, profileScopeKey } from '@/cryozen'
...
export function useCapabilityScope({
  ...
  const registryBridge = window.cryozenDesktop?.connections
  const rosterBridge = window.cryozenDesktop?.getAgentRoster
  ...
  const options: ScopeOption[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Cryozen (default)' : p.name,
      value: p.name
    }))
