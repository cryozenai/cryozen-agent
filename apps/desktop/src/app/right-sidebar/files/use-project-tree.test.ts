...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { cryozenDesktop: { readDir: typeof readDir } }).cryozenDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): CryozenReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { cryozenDesktop: unknown }).cryozenDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: CryozenReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<CryozenReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: CryozenReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: CryozenReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: CryozenReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<CryozenReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.cryozenDesktop is missing', async () => {
    delete (window as unknown as { cryozenDesktop?: unknown }).cryozenDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: CryozenReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: CryozenReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: CryozenReadDirResult) => void) | undefined
  ...
  it('drops a child listing that was read before the preference flipped', async () => {
    const gitRoot = vi.fn(async () => '/p')
    const readFileDataUrl = vi.fn(async () => `data:text/plain;base64,${btoa('*.log\n')}`)
    let releaseChild: ((value: CryozenReadDirResult) => void) | undefined
    ...
    readDir.mockImplementation(async path => {
      ...
      if (path === '/p/src') {
        return new Promise<CryozenReadDirResult>(resolve => {
