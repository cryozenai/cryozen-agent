...
import { getCryozenConfigSchema, getElevenLabsVoices, saveCryozenConfig } from '@/cryozen'
...
import type { ConfigFieldSchema, CryozenConfigRecord } from '@/types/cryozen'

import { cryozenConfigCacheWriter, useCryozenConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<CryozenConfigRecord | null>(null)
  ...
  const {
    ...
    writeScope
  } = useCryozenConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => cryozenConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['cryozen-config-schema'] : ['cryozen-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getCryozenConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `cryozen config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<CryozenConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveCryozenConfig(patch, writeScope ?? scopeProfile)
  ...
  const applyConfig = (next: CryozenConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: CryozenConfigRecord) => {
