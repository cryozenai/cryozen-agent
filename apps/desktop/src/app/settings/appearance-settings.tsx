...
function ResumeLastSessionSetting() {
  ...
  const configQuery = useCryozenConfigRecord()
  ...
  const update = (on: boolean) => {
    ...
    void saveCryozenConfig(setNested({}, 'display.resume_last_session', on), writeScope)
      .then(result => {
        if (!result.ok) {
...
function MarketplaceThemeResults({
  ...
  const search = useQuery({
    ...
    queryFn: () => window.cryozenDesktop?.themes?.searchMarketplace(debounced) ?? Promise.resolve([]),
