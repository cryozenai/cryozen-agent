...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `cryozen auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'acme' ? 'Acme' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
describe('ProvidersSettings', () => {
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    expect(startManualProviderOAuth).toHaveBeenCalledWith('acme', 'default')
    expect(disconnectOAuthProvider).not.toHaveBeenCalled()
  })
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          cli_command: 'cryozen auth add qwen-oauth',
          disconnect_hint: "Use `cryozen auth add qwen-oauth` or that provider's CLI to remove it.",
