...
const getCryozenConfigSchema = vi.fn()
...
vi.mock('@/cryozen', async () => ({
  ...
  getCryozenConfigRecord: (profile?: string) => getCryozenConfigRecord(profile),
  getCryozenConfigSchema: () => getCryozenConfigSchema(),
  saveCryozenConfig: (config: unknown, profile?: string) => saveCryozenConfig(config, profile),
...
beforeEach(() => {
  ...
  getCryozenConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('renders and saves the Codex compression auto-raise setting', async () => {
    ...
    getCryozenConfigSchema.mockResolvedValue({
      fields: {
    ...
    try {
      ...
      await vi.waitFor(() =>
        expect(saveCryozenConfig).toHaveBeenCalledWith({ compression: { codex_gpt55_autoraise: false } }, 'default')
      )
    } finally {
      vi.useRealTimers()
    }
  ...
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getCryozenConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await vi.waitFor(() => expect(saveCryozenConfig).toHaveBeenCalledTimes(1))
      expect(saveCryozenConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await vi.waitFor(() => expect(saveCryozenConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveCryozenConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('threads the "Applies to" request scope into both the config read and the autosave write', async () => {
    ...
    getCryozenConfigRecord.mockResolvedValue({ checkpoints: { enabled: false } })
    ...
    try {
      ...
      await vi.waitFor(() => expect(getCryozenConfigRecord).toHaveBeenCalledWith('nash'))

      ;(await screen.findByRole('switch')).click()
      await vi.advanceTimersByTimeAsync(700)

      await vi.waitFor(() => expect(saveCryozenConfig).toHaveBeenCalledWith({ checkpoints: { enabled: true } }, 'nash'))
    } finally {
      vi.useRealTimers()
    }
  })
