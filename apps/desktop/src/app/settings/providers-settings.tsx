...
import { disconnectOAuthProvider, listOAuthProviders } from '@/cryozen'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/cryozen'
...
//      in cryozen_cli/provider_catalog.py) — the SAME provider identity
//      `cryozen model` uses. This is authoritative: a provider tagged by the
...
function OAuthPicker({
  ...
  const isConnected = (p: OAuthProvider) => Boolean(p.status?.logged_in)
  ...
  return (
    <section className="mb-5 grid gap-2">
      ...
      {/* The no-account path, matching onboarding. Behind the
...
function ConnectedProviderRow({
  ...
  // Cryozen can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // Fall back to a static "remove it elsewhere" hint when we offer no button.
