...
import type { ToolsetConfig } from '@/types/cryozen'
...
const saveCryozenConfigRecord = vi.fn()
const getElevenLabsVoices = vi.fn()

vi.mock('@/cryozen', () => ({
  // useCryozenConfigRecord (via VoiceProviderFields) reads these from the barrel.
  ...
  saveCryozenConfigRecord: (config: unknown, profile?: unknown) => saveCryozenConfigRecord(config, profile),
...
beforeEach(() => {
  ...
  getCryozenConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  saveCryozenConfig.mockResolvedValue({ ok: true })
  saveCryozenConfigRecord.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveCryozenConfigRecord).toHaveBeenCalled(), { timeout: 3000 })

    const saved = saveCryozenConfigRecord.mock.calls.at(-1)?.[0] as Record<
      string,
    ...
    expect(saveCryozenConfigRecord.mock.calls.at(-1)?.[1]).toBeUndefined()
  })
  ...
  it('autosaves the inline voice fields into the profile the panel is scoped to', async () => {
    ...
    const [saved, forwarded] = saveCryozenConfigRecord.mock.calls.at(-1) as [
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a signed-out cloud row and a
