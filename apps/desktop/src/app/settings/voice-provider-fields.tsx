...
import {
  getCryozenConfigSchema,
  getElevenLabsVoices,
  type ProfileScope,
  profileScopeKey,
  saveCryozenConfigRecord
} from '@/cryozen'
...
export function VoiceProviderFields({
  ...
  const { data: loadedConfig, writeScope } = useCryozenConfigRecord(profile)
  ...
  const writeConfigCache = useMemo(() => cryozenConfigCacheWriter(profile), [scopeKey])

  const { data: schemaResponse } = useQuery({
    queryKey: ['cryozen-config-schema'],
    queryFn: () => getCryozenConfigSchema(),
    staleTime: 5 * 60 * 1000
  })
  ...
  const [baseline, setBaseline] = useState<CryozenConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void saveCryozenConfigRecord(diffConfig(baseline ?? {}, config), writeScope ?? profile)
        .then(() => {
          setBaseline(config)
