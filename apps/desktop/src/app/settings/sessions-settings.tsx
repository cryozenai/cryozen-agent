...
import type { CryozenConfigRecord, SessionInfo } from '@/types/cryozen'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    if (!window.cryozenDesktop) {
      return
    }

    let alive = true

    void getCryozenConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        ...
        await saveCryozenConfig({ sessions: { auto_archive: autoArchive, auto_archive_days: archiveDays } }, writeScope)
      } catch (err) {
...
// / Program Files), which buried any files Cryozen wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.cryozenDesktop` is
    ...
    const settings = window.cryozenDesktop?.settings
