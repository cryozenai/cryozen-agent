...
import { saveCryozenConfig } from '@/cryozen'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { CryozenConfigRecord } from '@/types/cryozen'

import { setCryozenConfigCache, useCryozenConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: CryozenConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig, dataUpdatedAt, writeScope } = useCryozenConfigRecord()
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setCryozenConfigCache, so
    ...
    const timeout = window.setTimeout(() => {
      ...
      void saveCryozenConfig(setNested({}, 'terminal.font_family', value), writeScope)
        .then(result => {
          if (!result.ok) {
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="cryozen-terminal-font-families"
          ...
          <datalist id="cryozen-terminal-font-families">
