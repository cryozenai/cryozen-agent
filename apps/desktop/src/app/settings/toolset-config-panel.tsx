 ...
 * server-computed `status` (keys ∧ post-setup install state). Older backends don't send `status` — fall back to the legacy
 ...
 * log inline — the GUI equivalent of the install step `cryozen tools` runs
...
/**
 * Backend model catalog — the GUI counterpart of the model picker `cryozen
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      await selectToolsetProvider(toolset, provider.name, undefined, profile)
