...
import { type ProfileScope, saveCryozenConfigRecord } from '@/cryozen'
 ...
 * the default browser's profile (cookies/logins) into a Cryozen-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config, writeScope } = useCryozenConfigRecord(profile)
  const setConfig = cryozenConfigCacheWriter(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        ...
        await saveCryozenConfigRecord({ browser: { use_real_profile: on } }, writeScope ?? profile)
