...
function editorFromConnection(conn: DesktopRegistryConnection): EditorState {
  return {
    ...
    remoteCryozenPath: conn.remoteCryozenPath || '',
...
export function findDuplicateConnection(
  ...
  if (editor.kind === 'remote') {
    ...
    return (
      connections.find(c => c.kind === 'remote' && c.id !== editor.id && normalizeGatewayUrl(c.url || '') === key) ??
      null
    )
  }
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.cryozenDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.cryozenDesktop.oauthLoginConnectionConfig(editorUrl)
    ...
    async (allowPlainTextToken = false) => {
      ...
      try {
        ...
        if (editor.kind === 'remote') {
          ...
          payload.remoteCryozenPath = editor.remoteCryozenPath.trim()
        }
  ...
  // Fan out `cryozen update` to every eligible source; per-connection results
          ...
          {editor.kind === 'ssh' && (
            <>
              ...
              <ListRow
                action={
                  <Input
                    onChange={e => setEditor({ ...editor, remoteCryozenPath: e.target.value })}
                    placeholder={t.settings.gateway.sshCryozenPathPlaceholder}
                    value={editor.remoteCryozenPath}
                  />
                }
                description={t.settings.gateway.sshCryozenPathDesc}
                title={t.settings.gateway.sshCryozenPathTitle}
              />
            </>
          )}
