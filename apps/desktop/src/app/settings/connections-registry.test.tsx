...
describe('ConnectionsRegistrySection', () => {
  it('refreshes a cached roster immediately after a successful connection test', async () => {
    ...
    Object.assign(window.cryozenDesktop!, { getAgentRoster })
  ...
  it('saves a custom remote Cryozen path for SSH connections', async () => {
    render(<ConnectionsRegistrySection />)
    ...
    fireEvent.change(screen.getByPlaceholderText('auto-detect'), {
      target: { value: '/opt/cryozen/bin/cryozen' }
    })
    ...
    expect(save.mock.calls[0][0]).toMatchObject({
      ...
      kind: 'ssh',
      label: 'Build host',
      remoteCryozenPath: '/opt/cryozen/bin/cryozen'
    })
  })

  it('clears a saved remote Cryozen path back to auto-detect', async () => {
    ...
    expect(pathInput.value).toBe('/opt/cryozen/bin/cryozen')
    fireEvent.change(pathInput, { target: { value: '   ' } })
    fireEvent.click(screen.getByText('Save connection').closest('button')!)

    await waitFor(() => expect(save).toHaveBeenCalledTimes(1))
    expect(save.mock.calls[0][0]).toMatchObject({ id: 'build-host', remoteCryozenPath: '' })
  })
