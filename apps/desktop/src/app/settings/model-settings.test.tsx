...
const getCryozenConfigRecord = vi.fn()
const saveCryozenConfig = vi.fn()
...
vi.mock('@/cryozen', async () => ({
  ...
  getCryozenConfigRecord: () => getCryozenConfigRecord(),
  saveCryozenConfig: (config: unknown) => saveCryozenConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'acme', model: 'example-model' })
    ...
    providers: [
      {
        ...
        authenticated: true,
        capabilities: { 'example-model': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'acme', model: 'example-model' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'acme', model: 'example-model', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'acme', model: 'example-model', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getCryozenConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveCryozenConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  it('loads the current main model and lists configured providers only', async () => {
    ...
    // "Acme" shows in both the trigger and the open list.
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'acme', model: 'example-model' })
    ...
    await waitFor(() => expect(screen.getAllByRole('combobox')[0].textContent).toContain('Acme'))
  ...
  it('writes the profile default speed (service_tier) as a sparse patch, never the cached snapshot', async () => {
    ...
    getCryozenConfigRecord.mockResolvedValue({
      agent: { reasoning_effort: 'medium', service_tier: 'normal' },
      auxiliary: { curator: { provider: 'auto', model: '', reasoning_effort: 'high' } }
    })
    await renderModelSettings()
    await waitFor(() => expect(getCryozenConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() => expect(saveCryozenConfig).toHaveBeenCalledWith({ agent: { service_tier: 'fast' } }))
  })
      ...
      providers: [
        {
          ...
          models: ['example-model'],
          authenticated: true,
          capabilities: { 'example-model': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('edits auxiliary reasoning effort below the selected model and applies it with the assignment', async () => {
    getAuxiliaryModels.mockResolvedValueOnce({
      main: { provider: 'acme', model: 'example-model' },
      tasks: [{ task: 'vision', provider: 'acme', model: 'example-model', base_url: '', reasoning_effort: null }]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'acme', model: 'example-model' }]
    })
...
describe('ModelSettings MoA preset editor', () => {
  const moaConfig = () => ({
    ...
    presets: {
      default: {
        reference_models: [
          { provider: 'acme', model: 'example-model' },
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('acme · example-model')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'acme', model: 'example-model', enabled: false }),
...
describe('ModelSettings code-skew 503', () => {
  const skewError = new Error(
    'Error invoking remote method \'cryozen:api\': Error: 503: {"detail":"Restart required: This process is running code from 08b4875f4a but the checkout on disk is now 48d2528066. The model picker would risk a stale-module crash — restart the Desktop-owned backend to load the new code (use Restart backend in Cryozen Desktop, or quit and reopen the app)"}'
  )
  ...
  it('unwraps the stale-backend 503 instead of dumping IPC JSON', async () => {
    ...
    expect(screen.queryByText(/cryozen:api/)).toBeNull()
  ...
  it('recycles the Desktop-owned backend and reloads the catalog', async () => {
    ...
    ;(window as unknown as { cryozenDesktop: { recycleBackend: typeof recycleBackend } }).cryozenDesktop = {
      recycleBackend
    }
