...
import { DEFAULT_REASONING_EFFORT, isReasoningEffort, REASONING_EFFORT_VALUES } from '@cryozen/shared'
...
import { cryozenConfigCacheWriter, invalidateCryozenConfig, useCryozenConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `cryozen model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in cryozen_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile, subpage }: ModelSettingsProps) {
  ...
  const { data: config, writeScope } = useCryozenConfigRecord(scopeProfile)
  const setConfig = useMemo(() => cryozenConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateCryozenConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveCryozenConfig(setNested({}, key, value), writeScope ?? scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `cryozen model` curation). Best-effort — fall through to the refreshed
  ...
  const recycleStaleBackend = useCallback(async () => {
    ...
    try {
      await window.cryozenDesktop?.recycleBackend?.(scopeProfile)
          ...
          {needsSetup && !setupIsApiKey && selectedProviderRow && (
            <p className="mt-2 text-xs text-muted-foreground">
              {selectedProviderRow?.auth_type === 'api_key'
                ...
                : `${selectedProviderRow?.name} signs in through your browser — Cryozen runs the flow for you.`}
            </p>
          )}
