...
import { getCryozenConfigRecord, listAllProfileSessions } from '@/cryozen'
...
// Cryozen session ids: <YYYYMMDD>_<HHMMSS>_<6 hex>. Used to offer a direct
...
function CommandPaletteBody({ onExited }: { onExited: () => void }) {
  ...
  const configQuery = useQuery({
    ...
    queryFn: () => getCryozenConfigRecord()
  })
        ...
        : []),
      {
        ...
        items: [
          ...
          },
          {
            ...
            icon: Download,
            id: 'cc-update-cryozen',
            keywords: ['update', 'upgrade', 'cryozen', 'version', 'system', 'restart'],
            label: cc.updateCryozen,
  ...
  const searchGroups = useMemo<PaletteGroup[]>(() => {
    ...
    // Apply a theme directly from the root search (e.g. "cryozen" → Cryozen). Live
