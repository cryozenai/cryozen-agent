...
import { host, LruCache } from '@cryozen/plugin-sdk'
...
// to (local, remote URL, SSH, docker) must be able to find
...
// cryozen_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['cryozen-bots']?.title || profile?.display_name || ''),
