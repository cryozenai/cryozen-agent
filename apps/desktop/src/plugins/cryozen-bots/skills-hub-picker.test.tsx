 ...
 * `{type:'cryozen-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
import type * as CryozenSdk from '@cryozen/plugin-sdk'
import { cleanup, fireEvent, render, screen, waitFor } from '@testing-library/react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'

const HUB_ORIGIN = 'https://cryozenai.github.io/cryozen-agent'
...
vi.mock('@cryozen/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof CryozenSdk>()
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'acme/web-research', name: 'Web Research', type: 'cryozen-skill-pick' },
      {
        source: frame.contentWindow
      }
    )

    expect(installCalls()).toEqual([['skills.manage', { action: 'install', query: 'acme/web-research' }]])
  })
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'cryozen-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'acme/web-research', type: 'oauth-callback' }, { source: frame.contentWindow })
    postPick({ identifier: 'acme/web-research', type: 'cryozen-skill-pick' }, { source: frame.contentWindow })
