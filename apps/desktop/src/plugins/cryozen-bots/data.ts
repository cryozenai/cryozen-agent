...
import { atom, host, queryClient, useQuery, useValue } from '@cryozen/plugin-sdk'
 ...
 *  protocol into the system prompt itself (cryozen-agent bot_mode_probe).
...
async function fetchRosterSnapshot(activeConnectionId: null | string | undefined): Promise<RosterSnapshot> {
  ...
  // Multi-source desktops (cryozen-agent #86875) also expose the union
...
function mergeMultiSourceRoster(
  ...
  for (const agent of agents) {
    ...
    profiles.push({
      ...
      // them a remote default titled "CoS Bot" only ever tags as @cryozen(-device)
 ...
 *  callable alias is 'cryozen' — the mention middleware resolves it back to
...
export function botHandle(name: string, bot?: Partial<RosterRow> | null): string {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'cryozen' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`cryozen profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Cryozen"
 *  can never hijack the primary profile's @cryozen alias. */
export function mentionNameForms(value: null | string | undefined): string[] {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'cryozen'].includes(form)
  )
}
...
export function botFriendlyNames(bot: Partial<RosterRow> | null | undefined): Array<null | string | undefined> {
  ...
  return [bot?.ui_meta?.['cryozen-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Cryozen Desktop to open another Bot chat.'
    })
...
export function resolveRosterMentions(
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)(?:@([a-z0-9][a-z0-9_-]*))?/gi)) {
    ...
    if (token === 'cryozen') {
      token = byForm.has('cryozen') ? 'cryozen' : token
    }
