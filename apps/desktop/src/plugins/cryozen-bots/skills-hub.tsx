...
import { Button, host, Input } from '@cryozen/plugin-sdk'
...
// https://cryozenai.github.io/cryozen-agent/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'cryozen-skill-pick', ...} to us (cryozen-agent#86243). We validate
...
export function HubSkillsSection({ bot, onInstalled }: HubSkillsSectionProps) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== 'cryozen-skill-pick' || !data.name) {
        return
      }
