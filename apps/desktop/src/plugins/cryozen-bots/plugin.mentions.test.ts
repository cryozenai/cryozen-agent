...
vi.mock('@cryozen/plugin-sdk', async () => {
...
async function contributions({
  ...
  // Bots pane does.
  if (profiles) {
    cache.set(JSON.stringify(['cryozen-bots', 'roster', cacheKeyConnection]), {
      key: ['cryozen-bots', 'roster', cacheKeyConnection],
      value: { profiles }
    })
  }
...
describe('@-mention completions', () => {
  ...
  it('surfaces default as @cryozen and prefix-filters on the handle', async () => {
    const { provide } = await contributions({
      focused: 'researcher',
      profiles: [
        { name: 'default' },
    ...
    expect(provide('').map(item => item.insert)).toEqual(expect.arrayContaining(['@cryozen', '@writer-homelab']))
   ...
   *  picker would surface as a retargeted local @cryozen. */
  const REMOTE_DEFAULTS: Array<Record<string, unknown>> = [
    {
      ...
      remoteSource: true,
      sourceScoped: true,
      ui_meta: { 'cryozen-bots': { title: 'CoS Bot' } }
    },
    {
      connectionId: 'wsl',
      connectionLabel: 'WSL',
  ...
  it.each([
    ...
    'lists titled remote defaults under their slug, qualified on collision, and keeps @cryozen local ($label)',
    async ({ profiles }) => {
      ...
      // the picker pins each to its connection; the local default stays @cryozen.
      expect(inserts).toEqual(expect.arrayContaining(['@cryozen', '@cos-bot@vps', '@cos-bot@wsl']))
      expect(inserts).not.toContain('@cos-bot')
      expect(inserts.filter(insert => insert === '@cryozen')).toHaveLength(1)
      ...
      // A bare @cryozen is this machine's default in either roster order.
      const local = await handler({ text: '@cryozen summarize' })
      expect(local.text).toMatch(/@cryozen = agent profile "default"/)
...
describe('the mention middleware', () => {
  it('identifies a remote @name-device mention without delivering anything', async () => {
    ...
    expect(result.text).not.toMatch(/cryozen -p '?default/)
  ...
  it('annotates the resolvable handle for a local row whose UI alias differs', async () => {
    ...
    // The local resolver only knows bare profile names / 'cryozen'.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
    ...
    expect(result.text).toMatch(/message_agent target: "cryozen"/)
  ...
  it('teaches no shellout and forbids forwarding the user’s text verbatim', async () => {
    ...
    // a `cryozen -p …` handoff, giving the model a second send path and a way
    // to relay the raw draft.
    const { handler } = await contributions({ focused: 'research', profiles: [{ name: 'research' }, { name: 'ops' }] })
    const result = await handler({ text: 'ask @ops to summarize' })

    expect(result.text).not.toMatch(/cryozen -p/)
  ...
  it('keeps a poisoned bot title inert prose', async () => {
    ...
    // the invariant that matters is that no cryozen command is ever emitted.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
        { name: 'ops' },
    ...
    expect(result.text).not.toMatch(/`cryozen/)
  })
    ...
    hostMock.agents.mockResolvedValueOnce({
      agents: [
        ...
        },
        {
          ...
          handle: 'default-vps',
          profile: 'default',
          profileMetadata: { ui_meta: { 'cryozen-bots': { title: 'CoS Bot' } } }
        }
      ],
      sources: [
