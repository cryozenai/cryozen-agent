/**
 * Cryozen Bot Mode — a "one chat per agent" roster for the Cryozen desktop.
 *
 * Left pane "Bots": one row per Cryozen profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Cryozen cron jobs) scoped to the
 ...
 * bot-initiated sends use `cryozen -p <bot> chat --in ~ -c "Bot Chat"`.
 */
...
import type { ChatEmptyProps, PluginContext } from '@cryozen/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @cryozen.
    ctx.register({
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.cryozenDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.cryozenDesktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is Cryozen' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['cryozen-bots']?.title || bot.title || ''
            ).trim()

            // message_agent only resolves canonical identities: the relay
            // matches a roster row's handle/profile (± @connection-id), the
            // local path a bare profile name or 'cryozen'. botHandle() prefers
