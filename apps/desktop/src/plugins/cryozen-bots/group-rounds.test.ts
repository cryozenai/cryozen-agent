...
describe('routing', () => {
  ...
  it('resolves @cryozen to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@cryozen take a look', members)
  ...
  it('stamps a local member reply with its connection label and keeps (you) for that member only', async () => {
    ...
    room.rounds.sendToGroupChat('Core', [local], '@cryozen status?')
    await settle(room, 'Core')
...
describe('turn prompt', () => {
  it('addresses the default profile as @cryozen', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @cryozen,/)
    ...
    expect(peer).toMatch(/group chat with @cryozen/)
  })

  // #89720: a renamed primary is @bobby to the roster, autocomplete and the
  // mention resolver; introducing it to itself as @cryozen made it treat
