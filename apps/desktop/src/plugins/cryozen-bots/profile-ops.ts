...
import { forgetSessionUnread, host, queryClient } from '@cryozen/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['cryozen-bots']
...
/** Permanently delete a bot's Cryozen profile, then remove plugin-local state
 ...
 * directory (cryozen-agent#52279). That is the "can't delete a bot" error. */
