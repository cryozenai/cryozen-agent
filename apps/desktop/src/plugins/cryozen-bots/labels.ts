...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Cryozen". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `cryozen profile rename
  ...
  // never generic "Cryozen" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Cryozen (the agent it is)
...
/** The profile id the backend accepts (`cryozen_cli.profiles._PROFILE_ID_RE`
