...
import { atom, host } from '@cryozen/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'cryozen-bots-groups'
...
/** The v3 envelope stored under the default profile's `cryozen-bots-groups`
 ...
 *  clients (Cryozen Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Cryozen"
 *  (community report, Aug 21 2026: renamed default still read "Cryozen is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Cryozen (matching displayName and the
 *  @cryozen handle) so the main agent never loses its name in rooms.
 *
 ...
 *  failing `default`s are never one anonymous "Cryozen" — judged against the
...
export function groupSpeakerLabel(name?: null | string, group?: null | string) {
  ...
  if (boundary !== -1) {
    ...
    const label = title || (profile.toLowerCase() === 'default' ? 'Cryozen' : profile)
  ...
  // title, then the local row's display_name, then default → Cryozen.
  ...
  return isDefault ? 'Cryozen' : trimmed
}
