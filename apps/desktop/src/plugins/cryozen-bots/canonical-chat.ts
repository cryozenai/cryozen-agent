...
import * as sdk from '@cryozen/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['cryozen-bots'].chat) were
 ...
 *  click path's tile-staleness probe (cryozen-agent#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Cryozen Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (cryozen-agent#89617) — clicking Retry
  ...
  // until an app restart (cryozen-agent#93604). A resume is cheap and
...
export function notifyBotOpenFailure(error: unknown, bot: RosterRow, step: BotOpenStep, botName?: string) {
  ...
  if (botModeGatewayNeedsUpdate(error)) {
    const connectionLabel = bot.connectionLabel || bot.connectionId || 'Cryozen'
    host.notify?.({
      kind: 'error',
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update Cryozen Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['cryozen-bots']
