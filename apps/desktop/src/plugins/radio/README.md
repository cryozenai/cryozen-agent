...
Bundled Cryozen Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@cryozen/plugin-sdk`, `react`, and
...
`$CRYOZEN_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use Cryozen theme
...
Stream availability, regional restrictions, and metadata vary. Cryozen is not
