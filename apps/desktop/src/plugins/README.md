...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `CryozenPlugin` and
...
as standalone runtime plugins instead.

User- and agent-authored plugins load at runtime from
`$CRYOZEN_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`cryozen-desktop-plugins` skill.
