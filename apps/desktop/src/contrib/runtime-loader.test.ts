...
import type * as CryozenModule from '@/cryozen'
import type { CryozenReadDirResult } from '@/global'
...
// getStatus would supply the connected backend's cryozen_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ cryozen_home: '/remote/box/.cryozen-agent' }))

vi.mock('@/cryozen', async importActual => ({
  ...(await importActual<typeof CryozenModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<CryozenReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { cryozenDesktop: unknown }).cryozenDesktop = {
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { cryozenDesktop?: unknown }).cryozenDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend cryozen_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.cryozen-agent/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.cryozen-agent/desktop-plugins')
    ...
    expect(readDir).not.toHaveBeenCalledWith('/local/.cryozen-agent/plugins')
    // The remote backend's cryozen_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.cryozen-agent/desktop-plugins')
  })
  ...
  it('treats a folder without plugin.js as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.cryozen-agent/desktop-plugins') {
        return {
          entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.cryozen-agent/desktop-plugins/my-feature' }]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.cryozen-agent/desktop-plugins/my-feature')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.cryozen-agent/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.cryozen-agent/desktop-plugins/odd' }] }
      }
  ...
  it('loads a unified desktop half (app-root copy + package marker) OPT-IN and tags it with its package', async () => {
    ...
    const root = '/local/.cryozen-agent/desktop-plugins'
    ...
    readDir.mockImplementation(async dir => {
      ...
      if (dir === `${root}/uni`) {
        return {
          entries: [
            { isDirectory: false, name: '.cryozen-package.json', path: `${root}/uni/.cryozen-package.json` },
    ...
    readFileText.mockImplementation(async file =>
      file.endsWith('.cryozen-package.json')
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches the Electron-resolved app root, never the backend cryozen_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.cryozen-agent/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.cryozen-agent/desktop-plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.cryozen-agent/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.cryozen-agent/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.cryozenDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.cryozen-agent/desktop-plugins')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.cryozen-agent/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.cryozen-agent/desktop-plugins/huge/plugin.js'
      })
  ...
  it('disposes runtime host event subscriptions before a hot reload (#112366)', async () => {
    ...
    try {
      const source = `
        import { host } from '@cryozen/plugin-sdk'
        export default {
...
describe('uninstallDiskPlugin (Plugins hub trash button)', () => {
  ...
  const seedStandalone = async () => {
    ...
    ;(window.cryozenDesktop as unknown as { removeDesktopPlugin: unknown }).removeDesktopPlugin = removeDesktopPlugin
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'cryozen-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "cryozen-bots", name: "Bot Mode", register() {} }',
        'cryozen-bots',
        { file: '/local/.cryozen-agent/desktop-plugins/cryozen-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['cryozen-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['cryozen-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.cryozen-agent/desktop-plugins/cryozen-bots/plugin.js'
      })
...
describe('specifier scanning is limited to code (strings/comments never load-block)', () => {
  ...
  it('never rewrites a mapped specifier quoted inside a string', async () => {
    ...
    try {
      ...
      const id = await loadRuntimePlugin(
        `const doc = "from '@cryozen/plugin-sdk'"
export default { id: 'quoted-spec', register: () => { globalThis.__captured = doc } }`,
        'quoted-spec'
      )

      expect(id).toBe('quoted-spec')
      expect((globalThis as unknown as { __captured?: string }).__captured).toBe("from '@cryozen/plugin-sdk'")
    } finally {
...
describe('loader hardening: hangs, leaks, duplicate ids, stale incarnations', () => {
  ...
  it('ctx.setInterval / ctx.addEventListener registrations die with the plugin on unload', async () => {
    ...
    try {
      await loadRuntimePlugin(
        `export default {
          ...
          register(ctx) {
            ...
            ctx.addEventListener(window, 'cryozen-probe', () => { globalThis.__scopedEvents++ })
          }
        }`,
        'scoped-lifetime'
      )
      ...
      window.dispatchEvent(new Event('cryozen-probe'))
