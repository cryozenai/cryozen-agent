 ...
 * `CryozenPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/cryozen'
...
export type { PluginRestOptions } from '@/cryozen'
 ...
 *  under `cryozen.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.cryozenDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Cryozen — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `cryozen://index-network/intent/1`),
...
export interface CryozenPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `cryozen.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.cryozenDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.cryozenDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.cryozenDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.cryozenDesktop
