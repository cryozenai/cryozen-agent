import type { GatewayWsUrlResult } from '@cryozen/shared'
import type { TranslucencyState } from '@cryozen/shared/translucency'
...
import type { CryozenNotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    cryozenDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`cryozen --tui
      ...
      // over cryozen:window-control; Electron/OS chrome owns them elsewhere.
      windowControls: {
        custom: boolean
        minimize: () => void
        toggleMaximize: () => void
      ...
      profile: {
        ...
        // backend under the new CRYOZEN_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: CryozenApiRequest) => Promise<T>
      notify: (payload: CryozenNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<CryozenReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<CryozenReadFileTextResult>
      selectPaths: (options?: CryozenSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<CryozenPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<CryozenPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<CryozenPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: CryozenActiveWork) => void
      setTitleBarTheme?: (payload: CryozenTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<CryozenReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<CRYOZEN_HOME>/desktop-plugins),
      ...
      /** LOCAL `<CRYOZEN_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<CryozenGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<CryozenGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<CryozenGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<CryozenRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: CryozenReviewScope, baseRef?: null | string) => Promise<CryozenReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: CryozenReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<CryozenReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<CryozenRepoPullRequests>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: CryozenTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<CryozenTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: CryozenWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: CryozenPreviewFileChanged) => void) => () => void
...
export interface CryozenTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface CryozenTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `cryozen update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  cryozenRoot?: string
...
export interface CryozenConnection {
  baseUrl: string
  customWindowControls?: boolean
  darwinMajor?: number
  isFullscreen: boolean
  isMaximized?: boolean
  // The live, RESOLVED connection mode. Only ever 'local' or 'remote'.
  mode?: 'local' | 'remote'
  authMode?: 'oauth' | 'token'
  remoteHost?: string
  remoteIdentity?: string
  remoteKind?: 'ssh' | 'url'
  remoteCryozenVersion?: string
...
export interface CryozenTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface CryozenActiveWork {
  count: number
  titles: string[]
}

export interface CryozenWindowState {
  customWindowControls?: boolean
  darwinMajor?: number
  isFullscreen: boolean
  isMaximized?: boolean
...
export interface CryozenApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface CryozenPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface CryozenReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface CryozenPreviewWatch {
  id: string
  path: string
}
...
export interface CryozenGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface CryozenGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface CryozenGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface CryozenRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface CryozenRepoStatus {
  branch: null | string
...
// turn began.
export type CryozenReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface CryozenReviewFile {
  path: string
  added: number
  removed: number
...
export interface CryozenReviewList {
  files: CryozenReviewFile[]
...
export interface CryozenReviewPr {
  url: string
  state: string
  number: number
}
...
export interface CryozenBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface CryozenRepoPullRequests {
  ghReady: boolean
  prs: CryozenBranchPullRequest[]
}
...
export interface CryozenReviewShipInfo {
  ghReady: boolean
  pr: CryozenReviewPr | null
}

export interface CryozenReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface CryozenReadDirResult {
  entries: CryozenReadDirEntry[]
  error?: string
}

export interface CryozenPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface CryozenSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
