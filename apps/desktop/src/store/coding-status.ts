...
import type { CryozenGitWorktree, CryozenRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: CryozenGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, CryozenRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, CryozenGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<CryozenRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<CryozenGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<CryozenRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<CryozenGitWorktree[]>>()
const $noRepoStatus = atom<CryozenRepoStatus | null>(null)
const $noWorktrees = atom<CryozenGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<CryozenRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<CryozenGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: CryozenRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: CryozenGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<CryozenRepoStatus | null>
  seq: number
  target: string
}
