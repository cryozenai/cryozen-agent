...
describe('requestPluginCatalogInstallFromDeepLink', () => {
  ...
  it('opens the reviewed/pinned catalog dialog exactly like an in-app pick', async () => {
    const lookup = lookupFor({
      ...
      entry: {
        name: 'aihubmix',
        repo: 'https://github.com/example-org/provider-aihubmix',
        sha: 'b'.repeat(40),
        subdir: 'aihubmix'
      }
    })
    ...
    expect($pluginInstallRequest.get()).toEqual({
      ...
      profile: null,
      repo: 'https://github.com/example-org/provider-aihubmix#aihubmix',
      sha: 'b'.repeat(40)
    })
  ...
  it.each([
    ['unknown', 'not in the Cryozen plugin catalog'],
    ['unavailable', 'Could not load the Cryozen plugin catalog'],
  ...
  it('resolves against the live catalog feed by default and rejects unknown names', async () => {
    ...
    expect($pluginInstallRequest.get()).toBeNull()
    expect($notifications.get()[0]?.message).toContain('\u201Cweather-evil\u201D is not in the Cryozen plugin catalog')
