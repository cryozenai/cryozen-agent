...
import { CryozenGateway, setApiRequestConnection } from '@/cryozen'
...
function dialProfile(
  desktop: NonNullable<typeof window.cryozenDesktop>,
  profile: string,
...
const isOpen = (gateway: CryozenGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: CryozenConnection) => void
  /**
...
interface Secondary {
  ...
  connection: CryozenConnection | null
  gateway: CryozenGateway
  /**
...
interface GatewayRegistryState {
  ...
  primaryGateway: CryozenGateway | null
   ...
   *  `cryozen serve --profile <primary>` child and can never stand in for a
  ...
  $gateway: ReturnType<typeof atom<CryozenGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('cryozen.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<CryozenGateway | null>(null),
...
export function setPrimaryGateway(gateway: CryozenGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<CryozenConnection, 'connectionId' | 'mode'> | null): void {
...
async function isAttachedSharedRemote(
  ...
  // A local primary is one `cryozen serve --profile <primary>` child; every other
  ...
  try {
    ...
    // (early return above): it is one `cryozen serve --profile <primary>` child
...
export function activeGateway(): CryozenGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the cryozen module (null
...
function publishActiveConnection(connection: CryozenConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new CryozenGateway()
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: CryozenGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Cryozen gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.cryozenDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Cryozen Desktop.')
  }
...
function publishTurnLease(scope: string, activeTurn: boolean): void {
  void window.cryozenDesktop?.touchBackend?.(scope, { activeTurn }).catch(() => undefined)
}
...
export async function ensureActiveGatewayOpen({
  explicit = false
}: { explicit?: boolean } = {}): Promise<CryozenGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Cryozen gateway is not connected" on the Sessions `+` action (#88880).
