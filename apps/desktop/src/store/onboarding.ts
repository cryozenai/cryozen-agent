...
import type { OAuthProvider, OAuthStartResponse } from '@/types/cryozen'
...
const CONFIGURED_CACHE_KEY = 'cryozen-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'cryozen-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Cryozen is ready', message: `${provider} connected.` })
}
...
async function fetchProviderDefaultModel(
  ...
  // `cryozen model` does. Fall back to the first curated model if the
  // endpoint can't resolve one.
  ...
  try {
    ...
    if (recommended.model && models.map(String).includes(recommended.model)) {
      ...
      // Recommended model isn't in the curated options list; trust it anyway.
      defaultModel = recommended.model
    }
  } catch {
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'Cryozen could not save the selected model.')
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Cryozen still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Cryozen still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Cryozen Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
async function openSignInUrl(url: string) {
  if (window.cryozenDesktop?.openExternal) {
    try {
      await window.cryozenDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Cryozen still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Cryozen still cannot reach ${resolvedUrl}.` }
    }
