...
describe('migrateTilesForProfile', () => {
  ...
  it('moves tabs, bot tiles, cached tails, remembered ids and owner hints to the new profile name', async () => {
    ...
    storage.writeJson('cryozen.desktop.sessionTiles.v2', {
      webdesign_bhp: [
    ...
    storage.writeJson('cryozen.desktop.sessionOwnerHints.v1', [
    ...
    const tiles = storage.readJson<Record<string, Tile[]>>('cryozen.desktop.sessionTiles.v2')
