 ...
 * `@cryozen/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-cryozen-glass]`
 * block in styles.css).
 */
...
} from '@cryozen/shared/translucency'
 ...
 * sets `cryozenDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.cryozenDesktop?.glassSupported === 'boolean'
    ? window.cryozenDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.cryozenDesktop?.translucencySupported === 'boolean'
    ? window.cryozenDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-cryozen-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-cryozen-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-cryozen-glass-on', glassLive)
  root.toggleAttribute('data-cryozen-glass', glassOn)
  root.toggleAttribute('data-cryozen-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-cryozen-glass-scope', scope)
  } else {
    root.removeAttribute('data-cryozen-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.cryozenDesktop?.setTranslucency?.(state)
  })
