...
import { addMcpServer, getMcpCatalog, listMcpServers, removeMcpServer } from '@/cryozen'
...
import type { McpCatalogEntry } from '@/types/cryozen'

/**
 * The MCP draft provider — the suggestion bus's founding member (PR #85036).
 *
 * Matches the draft against the curated MCP catalog's `suggest`
 * metadata (`GET /api/mcp/catalog` — the same reviewed manifests behind
 * `cryozen mcp catalog`), by whole-word keyword and pasted-link host suffix,
