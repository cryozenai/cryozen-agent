 ...
 *  Working in `~/www/cryozen-agent`, the draft says "cryozen-agent" constantly
 *  — a "Use skill: cryozen-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`cryozen-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/cryozen-agent-dev`, `plugin:skill`). */
