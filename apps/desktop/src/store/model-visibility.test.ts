import type { ModelOptionProvider } from '@cryozen/shared'
...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    ...
    const visible = effectiveVisibleKeys(stored, [
      provider('acme', ['example-model-70b', 'example-model-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('acme', 'example-model-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('acme', 'example-model-8b'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    // Simulates: user hid all "acme" models, then toggles one back on.
    const stored = new Set([emptyProviderSentinelKey('acme'), modelVisibilityKey('ollama', 'qwen3:latest')])

    // After toggle: sentinel removed, one model added.
    const afterToggle = new Set(stored)
    afterToggle.delete(emptyProviderSentinelKey('acme'))
    afterToggle.add(modelVisibilityKey('acme', 'example-model-70b'))
    ...
    expect(visible.has(modelVisibilityKey('acme', 'example-model-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('acme', 'example-model-8b'))).toBe(false)
  })
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('acme')])
    const providers = [provider('acme', ['cryozen-x', 'cryozen-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('acme'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('acme', 'cryozen-x'))).toBe(false)
...
describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('acme', ['cryozen-x', 'cryozen-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    ...
    let stored: Set<string> | null = null
    stored = apply(stored, 'acme', 'cryozen-x')
    stored = apply(stored, 'acme', 'cryozen-y')
    ...
    expect(visible.has(modelVisibilityKey('acme', 'cryozen-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('acme', 'cryozen-y'))).toBe(false)
  })
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('acme', 'cryozen-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('acme', 'cryozen-x'))).toBe(true)
