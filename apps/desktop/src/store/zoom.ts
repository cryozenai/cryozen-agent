...
export function setZoomPercent(percent: number): void {
  window.cryozenDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.cryozenDesktop?.zoom) {
  void window.cryozenDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.cryozenDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
