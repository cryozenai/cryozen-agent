 ...
 *
 * The archive is the CLI's own `cryozen profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/cryozen'
...
import type { ProfileDesktopOverlay } from '@/types/cryozen'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Cryozen profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.cryozenDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.cryozenDesktop?.selectPaths?.({
