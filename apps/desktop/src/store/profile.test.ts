...
const { getProfiles } = await import('@/cryozen')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/cryozen/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<CryozenConnection> = {}): CryozenConnection =>
  ({ baseUrl: 'https://box.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as CryozenConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { cryozenDesktop: { getConnection } })
