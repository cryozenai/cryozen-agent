...
const agentConn = (over: Partial<CryozenConnection> = {}): CryozenConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as CryozenConnection

const localConn = (over: Partial<CryozenConnection> = {}): CryozenConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as CryozenConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<CryozenConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<CryozenConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { cryozenDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<CryozenConnection>(() => undefined))
