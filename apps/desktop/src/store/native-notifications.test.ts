...
beforeEach(() => {
  notify.mockClear()
  desktopWindow.cryozenDesktop = { notify } as unknown as Window['cryozenDesktop']
...
describe('dispatchNativeNotification session context', () => {
  it('names the session on blocking-prompt titles only, falling back to the id tail without a row', () => {
    ...
    try {
      ...
      dispatchNativeNotification({ kind: 'turnDone', sessionId: 'named-chat', title: 'Cryozen finished' })
      expect(notify).toHaveBeenCalledWith(expect.objectContaining({ title: 'Cryozen finished' }))
    } finally {
      setSessions([])
    }
  })
...
describe('sendTestNativeNotification', () => {
  it('fires regardless of focus or active session', () => {
    setWindowState({ focused: true, hidden: false })
    setActiveSessionId('on-screen')
    sendTestNativeNotification('Cryozen', 'works')
    expect(notify).toHaveBeenCalledTimes(1)
  })
})
