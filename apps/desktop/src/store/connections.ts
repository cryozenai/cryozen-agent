...
const LAST_PROFILE_STORAGE_KEY = 'cryozen.desktop.lastProfileByConnection'
...
async function rememberConnection(connectionId: string): Promise<void> {
  const setLastUsed = window.cryozenDesktop?.connections?.setLastUsed
...
export async function selectConnection(connectionId: string, options: SelectConnectionOptions = {}): Promise<void> {
  ...
  // The primary local descriptor (startCryozen) historically publishes without
  ...
  try {
    ...
    if (targetConnection.authMode === 'oauth' && targetConnection.kind === 'remote') {
