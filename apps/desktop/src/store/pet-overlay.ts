 ...
 * app's bounds and stays visible while Cryozen is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'cryozen.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'cryozen.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.cryozenDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.cryozenDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.cryozenDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.cryozenDesktop?.petOverlay?.close()
}
