import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@cryozen/shared'
import { atom, batch, computed } from 'nanostores'

import { cryozenApi, getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/cryozen'
...
import type { ProfileInfo } from '@/types/cryozen'
...
// /api/profiles/active `current`). "default" is the root ~/.cryozen-agent. This is the
...
function profileListSource(connection: CryozenConnection | null): null | string {
  if (!connection) {
    return null
  }
...
const PROFILE_ORDER_STORAGE_KEY = 'cryozen.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'cryozen.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await cryozenApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new CRYOZEN_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.cryozenDesktop.profile.set(name)
}
...
export function prewarmProfileBackend(name: string, connectionId: null | string = null): void {
  ...
  // tunnel and spawns `cryozen -p <profile> serve --isolated` on the remote
...
async function resolveConnectionForProfile(profile: string): Promise<CryozenConnection | null> {
  const getConnection = window.cryozenDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<CryozenConnection | null> {
  const getConnectionFor = window.cryozenDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'cryozen.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.cryozenDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.cryozenDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.cryozen-agent) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.cryozenDesktop?.touchBackend?.(target).catch(() => undefined)
}
