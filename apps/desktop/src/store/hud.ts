 ...
 * composer with the reply scrolling above it, so Cryozen can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.cryozenDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.cryozenDesktop?.hud
...
export function resetHudLayout(): void {
  void window.cryozenDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.cryozenDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.cryozenDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
