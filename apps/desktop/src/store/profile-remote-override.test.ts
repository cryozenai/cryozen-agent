...
describe('remoteHostLabel', () => {
  it('keeps a non-default port and drops default ones', () => {
    expect(remoteHostLabel('https://cryozen.example.com:8443/x')).toBe('cryozen.example.com:8443')
    expect(remoteHostLabel('https://cryozen.example.com:443')).toBe('cryozen.example.com')
    expect(remoteHostLabel('http://cryozen.example.com:80')).toBe('cryozen.example.com')
  })
...
describe('refreshProfileRemoteOverrides', () => {
  it('publishes only profiles whose scope resolves to a remote override', async () => {
    getConnectionConfig.mockImplementation(async (profile: string) => {
      ...
      if (profile === 'lab') {
        return { mode: 'remote', remoteUrl: 'https://lab.example.com:8443' }
      }

      return { mode: 'local', remoteUrl: '' }
    })

    await refreshProfileRemoteOverrides(['work', 'lab', 'home'])

    expect($profileRemoteOverrides.get()).toEqual({
      work: { host: 'work.example.com', url: 'https://work.example.com' },
      lab: { host: 'lab.example.com:8443', url: 'https://lab.example.com:8443' }
    })
  })
