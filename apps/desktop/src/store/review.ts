...
import type { CryozenReviewFile, CryozenReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Cryozen' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'cryozen.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'cryozen.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'cryozen.desktop.reviewTreeMode'
const SELECTED_KEY = 'cryozen.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<CryozenReviewFile[]>([])
...
export const $reviewShipInfo = atom<CryozenReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['cryozenDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: CryozenReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly CryozenReviewFile[], path: string): CryozenReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.cryozenDesktop?.openExternal?.(existing.url)
