 ...
 * (`cryozen:data-url-read-max:*`). The default and the clamp are shared with
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.cryozenDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.cryozenDesktop?.dataUrlReadMax) {
