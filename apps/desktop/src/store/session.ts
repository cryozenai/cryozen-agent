import type { ConnectionState } from '@cryozen/shared'
...
import type { SessionInfo, UsageStats } from '@/types/cryozen'
...
const COMPOSER_MODEL_KEY = 'cryozen.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'cryozen.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'cryozen.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'cryozen.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'cryozen.desktop.composer.fast'
...
function composerScopeForConnection(connection: CryozenConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'cryozen.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'cryozen.desktop.lastRoute'
...
function workspaceCwdKey(connection: CryozenConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.cryozenDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.cryozenDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<CryozenConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'cryozen.desktop.sessionOwnerHints.v1'
 ...
 * CryozenConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<CryozenConnection | null>) => {
 ...
 *  weaker when the route clamps a Cryozen-internal step such as `ultra`. Never
...
// until config loads, and re-seeded on every profile switch by useCryozenConfig.
