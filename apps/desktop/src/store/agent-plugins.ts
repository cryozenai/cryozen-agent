...
/**
 * Feature store for backend (agent) plugins — the native Cryozen plugins plus
 ...
 * primitives `cryozen plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Cryozen. */
  portable?: boolean
...
const MANAGEABLE_BUNDLED_KEYS = new Set(['disk-cleanup'])
 ...
 *  CRYOZEN_HOME. Always refetches (it's a cheap local disk scan on the
...
export async function installAgentPlugin(
  ...
  opts: {
    ...
    ref?: string
    /** Target profile's CRYOZEN_HOME (null/undefined = backend launch profile). */
    profile?: string | null
  }
 ...
 *  deletes `<CRYOZEN_HOME>/plugins/<name>` and its install metadata). Drops the
