...
import { checkCryozenUpdate, getActionStatus, updateCryozen } from '@/cryozen'
import type {
...
import type { BackendUpdateCheckResponse } from '@/types/cryozen'
...
const UPDATE_TOAST_SNOOZE_KEY = 'cryozen:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'cryozen:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'cryozen:install-method-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateCryozen'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.cryozenDesktop?.getVersion?.()
...
export async function checkBackendUpdates({
  ...
  try {
    const status = mapBackendCheck(await checkCryozenUpdate(force))
...
export async function checkUpdates({ force = false }: UpdateCheckOptions = {}): Promise<DesktopUpdateStatus | null> {
  const bridge = window.cryozenDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `cryozen update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'cryozen update',
        command: result.command ?? 'cryozen update'
      })
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== cryozen-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updateCryozen()
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'cryozen update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkCryozenUpdate(true)
...
const FOCUS_RECHECK_KEY = 'cryozen.updates.last-passive-check'
