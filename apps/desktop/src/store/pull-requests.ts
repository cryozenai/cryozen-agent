...
import { scanSessionPullRequests, type SessionInfo } from '@/cryozen'
import type { CryozenBranchPullRequest } from '@/global'
...
export const $pullRequestsByBranch = atom<Record<string, CryozenBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'cryozen.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('cryozen.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: CryozenBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
