...
import { cryozenApi, type CryozenGateway, getCryozenConfig } from '@/cryozen'
import type { CryozenGitBaseBranch, CryozenGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/cryozen'
...
const PROJECT_SCOPE_KEY = 'cryozen.desktop.projectScope'
...
async function activeProjectsContext(profile = projectProfile()): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== normalizeProfileKey($activeGatewayProfile.get())) {
    throw new Error('Active Cryozen profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await cryozenApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<CryozenGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<CryozenGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Cryozen
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getCryozenConfig(context.profile))
...
// repo, returning where Cryozen should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<CryozenGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<CryozenGitBaseBranch[]> {
  const git = desktopGit()
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.cryozenDesktop?.writeClipboard?.(path)
  }
}
