 ...
 *
 * Local models ship on main behind `--local` (either `cryozen desktop --local`
 * or the flag on Cryozen.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.cryozenDesktop?.localModelsEnabled === true
)
