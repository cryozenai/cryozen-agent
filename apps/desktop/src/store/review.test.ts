...
// reaches the gateway); everything else routes through window.cryozenDesktop.git,
...
function file(path: string, over: Partial<CryozenReviewFile> = {}): CryozenReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as CryozenReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.cryozenDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as CryozenReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.cryozenDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: CryozenReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as CryozenReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { cryozenDesktop?: unknown }).cryozenDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as CryozenReviewShipInfo)
