...
// (#73044). Exercises the REAL cryozen + store/gateway + store/profile chain:
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/cryozen')
...
const conn = (over: Partial<CryozenConnection> = {}): CryozenConnection =>
  ({
    authMode: 'oauth',
