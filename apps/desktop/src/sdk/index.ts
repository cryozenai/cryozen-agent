/**
 * @cryozen/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__CRYOZEN_PLUGIN_SDK__` and maps the import to it, so a
...
import { cryozenApi, type CryozenGateway, deleteProfile, getLogs, getStatus } from '@/cryozen'
...
import type { PaginatedSessions, UsageStats } from '@/types/cryozen'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Cryozen profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.cryozenDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.cryozenDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and CryozenGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (cryozen-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.cryozenDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Cryozen Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.cryozenDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Cryozen Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Cryozen Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (cryozen-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Cryozen Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return cryozenApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return cryozenApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}, timeoutMs?: number): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Cryozen gateway unavailable')
    }
   ...
   *  components that take a `CryozenGateway` prop directly (e.g. `ConnectorsTab`),
   *  which need the instance, not just a JSON-RPC door. Re-read per use — the
   *  active instance changes on a profile swap. */
  getGateway: (): CryozenGateway | null => $gateway.get()
}
 ...
 *  "CRYOZEN AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { CryozenGateway } from '@/cryozen'
...
export type { CryozenOpenTarget } from '@/lib/cryozen-open-target'
...
/** Compact labels for the reasoning levels exported from @cryozen/shared, so a
...
export type { StatusResponse } from '@/types/cryozen'
/** Public SDK name for the shared gateway wire event; kept stable for plugins. */
export type { GatewayEvent as RpcEvent } from '@cryozen/shared'
/** THE compact-number formatter — every user-facing count/token figure goes
 *  through here (1230 → "1.2k", 1_500_000 → "1.5M"). Don't hand-roll `/1000`. */
export { compactNumber } from '@cryozen/shared'
/** Cryozen' reasoning levels, so a plugin surfacing a thinking depth uses the
...
export { contrastRatio } from '@cryozen/shared/color'
