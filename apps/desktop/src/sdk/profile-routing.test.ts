...
vi.mock('@/cryozen', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), cryozenApi: vi.fn() }))
...
const { deleteProfile, cryozenApi } = await import('@/cryozen')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its CRYOZEN_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { cryozenDesktop: unknown }).cryozenDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(cryozenApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(cryozenApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(cryozenApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
