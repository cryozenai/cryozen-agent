...
describe('requestTheme', () => {
  ...
  it('activates a theme contributed through the registry', () => {
    const aurora: DesktopTheme = { ...midnightTheme, description: 'Aurora', label: 'Aurora', name: 'aurora' }
    const dispose = registry.register({ area: THEMES_AREA, data: aurora, id: 'aurora' })

    renderProbe()

    let accepted = false
    act(() => {
      accepted = requestTheme('aurora')
    })

    expect(accepted).toBe(true)
    expect(ctx.themeName).toBe('aurora')
