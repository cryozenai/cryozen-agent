import { contrastRatio } from '@cryozen/shared/color'
import { describe, expect, it } from 'vitest'

import { hexToOklch, withHue } from './color'
import { cryozenTheme, githubTheme } from './presets'
...
const seedAt = (hue: number) => withHue(cryozenTheme.colors.primary, hue)

const BRAND_BLUE = '#0053FD'

describe('themeHue', () => {
  it('reads the accent hue that ships', () => {
    // Brand blue. Both palettes sit at this hue — light seeds `#0053fd` and dark
    // `#4a84fe`, the same blue at two lightnesses, which is what lets one pick
    // serve both appearances.
    expect(themeHue(cryozenTheme)).toBe(263)
    expect(Math.round(hexToOklch(cryozenTheme.darkColors!.primary)!.h)).toBe(263)
  })
...
describe('the shipped cryozen accents', () => {
  const cases = [
    { appearance: 'light', colors: cryozenTheme.colors, seed: '#0053fd' },
    { appearance: 'dark', colors: cryozenTheme.darkColors!, seed: '#4a84fe' }
  ] as const
  ...
  it('is one blue at two lightnesses, not two blues', () => {
    const light = hexToOklch(cryozenTheme.colors.primary)!
    const dark = hexToOklch(cryozenTheme.darkColors!.primary)!
  ...
  it('leaves GitHub’s neutrals in place — only the accent family is forked', () => {
    for (const key of ['background', 'foreground', 'card', 'border', 'sidebarBackground'] as const) {
      expect(cryozenTheme.colors[key]).toBe(githubTheme.colors[key])
      expect(cryozenTheme.darkColors![key]).toBe(githubTheme.darkColors![key])
    }
  })
})
...
describe('retintTheme', () => {
  ...
  it('is an identity at the theme’s own accent', () => {
    const same = retintTheme(cryozenTheme, cryozenTheme.colors.primary)

    expect(same.colors).toEqual(cryozenTheme.colors)
    expect(same.darkColors).toEqual(cryozenTheme.darkColors)
  })

  it('moves every accent-family slot, in both modes', () => {
    const rose = retintTheme(cryozenTheme, seedAt(350))

    for (const mode of ['colors', 'darkColors'] as const) {
      const before = cryozenTheme[mode]!
  ...
  it('keeps the four seed slots locked together', () => {
    const teal = retintTheme(cryozenTheme, seedAt(195)).colors
  ...
  it('leaves the chrome alone', () => {
    ...
    const violet = retintTheme(cryozenTheme, seedAt(285))

    for (const key of ['background', 'foreground', 'card', 'border', 'muted', 'mutedForeground'] as const) {
      expect(violet.colors[key], key).toBe(cryozenTheme.colors[key])
      expect(violet.darkColors![key], `dark ${key}`).toBe(cryozenTheme.darkColors![key])
    }
  })

  it('holds perceived lightness and chroma while only the hue moves', () => {
    const base = hexToOklch(cryozenTheme.colors.primary)!

    for (const hue of HUES) {
      const seed = hexToOklch(retintTheme(cryozenTheme, seedAt(hue)).colors.primary)!
  ...
  it('keeps the accent readable on the sidebar at every hue', () => {
    for (const hue of HUES) {
      const t = retintTheme(cryozenTheme, seedAt(hue))
  ...
  it('re-picks the foreground that sits on the accent', () => {
    for (const hue of HUES) {
      const c = retintTheme(cryozenTheme, seedAt(hue)).colors
  ...
  it('accepts any hex form and ignores junk', () => {
    expect(retintTheme(cryozenTheme, '#0053FD').colors.primary).toBe(retintTheme(cryozenTheme, '0053fd').colors.primary)
    // A half-typed hex from a text input must not blow up the theme.
    expect(retintTheme(cryozenTheme, '#00').colors).toEqual(cryozenTheme.colors)
    expect(retintTheme(cryozenTheme, 'nonsense').colors).toEqual(cryozenTheme.colors)
  })

  // The real motivating case: brand blue is legible on GitHub's light sidebar
  ...
  describe('a seed that only works in one mode', () => {
    const blue = retintTheme(cryozenTheme, BRAND_BLUE)

    it('keeps the picked color where it already passes', () => {
      expect(blue.colors.primary.toLowerCase()).toBe(BRAND_BLUE.toLowerCase())
    })

    it('lightens it for the mode where it does not', () => {
      const dark = blue.darkColors!.primary

      expect(dark.toLowerCase()).not.toBe(BRAND_BLUE.toLowerCase())
    ...
    it('adapts by lightness, holding the hue — so it still reads as the brand', () => {
      const picked = hexToOklch(BRAND_BLUE)!
  ...
  it('does not brand a slot that never tracked the accent', () => {
    ...
    const neutralRing = {
      ...cryozenTheme,
      colors: { ...cryozenTheme.colors, ring: '#9a9a9a' },
  ...
  describe('a theme whose accent slots are shades of each other', () => {
    const shaded = {
      ...cryozenTheme,
      colors: { ...cryozenTheme.colors, primary: '#ddd6ff', ring: '#8b80e8', midground: '#8b80e8' },
      darkColors: undefined
    }
