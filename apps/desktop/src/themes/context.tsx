...
import { ensureContrast, mix, parseColor } from '@cryozen/shared/color'
...
import { BUILTIN_THEME_LIST, cryozenTheme, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, migrateSkinName } from './presets'
...
const SKIN_KEY = 'cryozen-desktop-theme-v2'
const MODE_KEY = 'cryozen-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'cryozen-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'cryozen-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'cryozen-desktop-active-profile-v1'
...
// Legacy and retired names migrate first (`migrateSkinName`); anything that
// still resolves to nothing falls back to DEFAULT_SKIN_NAME.
const normalizeSkin = (name: string | null): string => {
  const migrated = name ? migrateSkinName(name) : null

  return migrated && resolveTheme(migrated) ? migrated : DEFAULT_SKIN_NAME
}
...
const storedSkin = (profile: string): string => {
  const stored = skinPref.stored(profile)

  return stored ? migrateSkinName(stored) : DEFAULT_SKIN_NAME
}
...
// mix / ensureContrast live in @cryozen/shared/color (shared with the TUI);
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? cryozenTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark', chatFontFamily = $chatFontFamily.get()) {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...cryozenTheme.typography, ...theme.typography }
  ...
  root.dataset.cryozenTheme = skinName
  root.dataset.cryozenMode = rendered
  ...
  window.cryozenDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('cryozen-boot-background', chromeBg)
    window.localStorage.setItem('cryozen-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.cryozenThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.cryozenDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Cryozen authoring/activating a skin from a
