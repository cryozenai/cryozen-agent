...
// The live-authoring loop: Cryozen writes/edits one skin file and every surface
...
describe('ThemeProvider ← backend skin sync', () => {
  ...
  it('paints a persisted backend skin once the connect-time seed makes it resolvable', () => {
    window.localStorage.setItem('cryozen-desktop-theme-v2', 'bloomberg')
...
describe('ThemeProvider highlight preview', () => {
  ...
  // Existing installs may have persisted a retired skin id; it must resolve to its replacement
  // instead of dropping to an unregistered theme.
  it.each([
    ['default', 'cryozen'],
    ['gold', 'cryozen']
  ])('migrates a persisted legacy skin %s to %s', (legacy, current) => {
    window.localStorage.setItem('cryozen-desktop-theme-v2', legacy)
    renderProbe()

    expect(ctx.themeName).toBe(current)
    expect(skinPref.resolve('default')).toBe(current)
    expect(cssVar('--theme-foreground')).toBe(getBaseColors(current, 'light').foreground)
  })
...
// here as a reference implementation; the shared @cryozen/shared/color ladder
