/**
 * Cryozen skin → DesktopTheme converter.
 *
 * A "skin" is the CLI/TUI theme unit: a YAML file in `$CRYOZEN_HOME/skins/` (or a
 * built-in) resolved by `cryozen_cli/skin_engine.py` and pushed to every surface
 ...
 * skin Cryozen authors from a prompt lights up all three surfaces from one file.
 *
...
import { ensureContrast, mix } from '@cryozen/shared/color'
import type { CryozenSkin, SkinColors } from '@cryozen/shared/skin'
 ...
 */
export function skinToDesktopTheme(skin: CryozenSkin): DesktopTheme | null {
