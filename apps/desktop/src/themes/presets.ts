 ...
 *
 * The palette-bearing skins (cryozen, catppuccin, everforest, solarized) are forks
 ...
 *
 *   cryozen    ← github.github-vscode-theme   (Light Default / Dark Default)
 ...
 * re-derived. `cryozen-alt` is first-party — do not re-derive it from GitHub.
 */

import { THEME_PRESET_PALETTES } from '@cryozen/shared'
 ...
 *
 * `cryozen` is a fork of this with its own accent, so shipping both keeps the
 * original available on its own terms instead of only existing as the thing
 * cryozen diverged from. Everything but the accent family is identical between
 * them; separate presets are what let cryozen's accent move without silently
...
/**
 * Cryozen - the canonical Cryozen desktop identity: GitHub's chrome carrying
 * the brand blue. Forked from github.github-vscode-theme (Light Default / Dark Default),
 ...
 */
export const cryozenTheme: DesktopTheme = {
  name: 'cryozen',
  label: 'Cryozen',
  description: 'Neutral chrome, brand blue accent',
  ...THEME_PRESET_PALETTES.cryozen,
  typography: {
    fontSans: SYSTEM_SANS,
    fontMono: SYSTEM_MONO,
    fontUrl: 'https://fonts.googleapis.com/css2?family=Courier+Prime:wght@400;700&display=swap'
...
/**
 * Cryozen Alt - the hand-authored palette from before the GitHub fork. Light
 * is glass neutrals with brand blue; dark is cream on mission-blue.
 */
export const cryozenAltTheme: DesktopTheme = {
  name: 'cryozen-alt',
  label: 'Cryozen Alt',
  description: 'Glass neutrals, cream on mission-blue',
  ...THEME_PRESET_PALETTES['cryozen-alt'],
  typography: {
    fontSans: SYSTEM_SANS,
    fontMono: SYSTEM_MONO,
    fontUrl: 'https://fonts.googleapis.com/css2?family=Courier+Prime:wght@400;700&display=swap'
...
export const BUILTIN_THEMES: Record<string, DesktopTheme> = {
  cryozen: cryozenTheme,
  github: githubTheme,
  catppuccin: catppuccinTheme,
  everforest: everforestTheme,
  solarized: solarizedTheme,
  'cryozen-alt': cryozenAltTheme,
...
export const DEFAULT_SKIN_NAME = 'cryozen'

/**
 * Retired skin ids mapped to the id
 * that replaces them. The single place legacy names are translated: persisted
 * picks, backend `display.skin` values and `/skin` arguments all pass through
 * `migrateSkinName`.
 */
const LEGACY_SKIN_NAMES: Readonly<Record<string, string>> = {
  default: 'cryozen',
  gold: 'cryozen'
}

/** The current id for a possibly-legacy skin name; unknown names pass through. */
export const migrateSkinName = (name: string): string => LEGACY_SKIN_NAMES[name] ?? name
