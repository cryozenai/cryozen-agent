/**
 * Live skin sync from the Cryozen backend.
 *
 * The backend resolves the active skin (built-in or `$CRYOZEN_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Cryozen
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { CryozenSkin } from '@cryozen/shared/skin'
...
const BACKEND_THEMES_KEY = 'cryozen-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: CryozenSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (cryozen), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → cryozen). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as CryozenSkin)
