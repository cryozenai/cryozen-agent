...
import { migrateSkinName } from './presets'

// Retired skin names land on their replacement so old muscle memory works.
...
export function useSkinCommand() {
  ...
  return useCallback(
    (rawArg: string) => {
      ...
      const targetName = ALIASES[normalized] || migrateSkinName(normalized)
