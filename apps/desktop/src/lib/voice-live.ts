...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to Cryozen.
 *
 ...
 * that into an ordinary Cryozen turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Cryozen keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Cryozen) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await cryozenApi<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Cryozen; late results for an
  ...
  async start(history: LiveHistoryMessage[]): Promise<void> {
    ...
    const response = await cryozenApi<{
      ok: boolean
      session?: { id: string }
  ...
  /** Quiet progress for the live model ("Cryozen is running the tests…"). */
