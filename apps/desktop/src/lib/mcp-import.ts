...
const emptyCryozenFlags = (): CryozenAddFlags => ({
  args: [],
  auth: null,
  command: null,
  env: {},
...
function readCryozenToken(flags: CryozenAddFlags, tokens: string[], index: number): number {
...
function cryozenEntry(flags: CryozenAddFlags): McpImportEntry | null {
...
function fromCryozenAdd(tokens: string[]): McpImportEntry | null {
  const flags = emptyCryozenFlags()

  for (let i = 3; i < tokens.length; i++) {
    i = readCryozenToken(flags, tokens, i)
...
function parseLine(line: string): McpImportEntry[] | null {
  ...
  if (tokens[0] === 'cryozen' && tokens[1] === 'mcp' && tokens[2] === 'add') {
    const entry = fromCryozenAdd(tokens)
