import { GatewayReauthRequiredError, isGatewayReauthRequired, resolveGatewayWsUrl } from '@cryozen/shared'
...
describe('desktop connection scope', () => {
  ...
  function aliasConnection(authMode: (typeof authModes)[number]) {
    ...
    } as CryozenConnection
  }

  function registeredConnection(authMode: (typeof authModes)[number]) {
    return { ...aliasConnection(authMode), profile: 'remote-profile', registryScoped: true } as CryozenConnection
  }
  ...
  it('a registry-scoped flag without a connectionId still takes the legacy mint', async () => {
    const desktop = fakeDesktop()
    const scopedWithoutId = { ...registeredConnection('token'), connectionId: undefined } as CryozenConnection
