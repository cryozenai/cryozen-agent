...
export interface ErrorSurface {
  ...
  /** Auth layer only: display name of the failing provider ("OpenAI Codex"). */
   ...
   *  older backends. */
  apiKeyEnv?: string
  /** The backend's own plain sentence for this failure, when it sent one. */
  message?: string
...
export interface ErrorRecoveryPlan {
  ...
  editMessage: boolean
  /** Reveal the Cryozen data folder so the user can free space (disk_full). */
  openCryozenFolder: boolean
...
const CODE_PLANS: Partial<Record<ErrorCodeKey, Partial<ErrorRecoveryPlan>>> = {
  ...
  disk_full: { openCryozenFolder: true, retry: true },
