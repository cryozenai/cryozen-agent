...
/** Why a known Cryozen command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Cryozen profile', surface: action('profile') },
  {
    name: '/skin',
 ...
 * `cryozen_cli/commands.py::desktop_surface_registry` by
...
function isKnownCryozenSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Cryozen' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownCryozenSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Cryozen and were never asked for. It is
