...
const UPDATE_BACKEND = 'Update the Cryozen backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.cryozenDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Cryozen Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.cryozenDesktop.openExternal(authUrl)
    let pollFailures = 0
