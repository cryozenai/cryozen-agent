...
describe('reasoning blocks in streamed markdown', () => {
  it('does not fuse the words a stripped block separated', () => {
    expect(preprocessMarkdown('no<thinking>hmm</thinking> Cryozen Desktop')).toBe('no Cryozen Desktop')
    ...
    expect(preprocessMarkdown('no<thinking>a</thinking><think>b</think>Cryozen')).toBe('no Cryozen')
  })
