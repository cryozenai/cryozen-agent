 ...
 * `cryozen://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `cryozen://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `cryozen://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type CryozenOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const CRYOZEN_PROTOCOL = 'cryozen:'
...
export function normalizeCryozenOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('cryozen://') || trimmed.startsWith(`${CRYOZEN_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // cryozen://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // cryozen://index-network/intent/1 → /index-network/intent/1
...
export function resolveCryozenOpenPath(target: CryozenOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeCryozenOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeCryozenOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeCryozenOpenString(target.path)
...
export function pathFromCryozenDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  ...
  if (kind === 'open') {
    return resolveCryozenOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveCryozenOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `cryozen://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromCryozenDeepLink('open', name, params)
}
