...
//
// When someone is in a hands-free "Hey Cryozen" voice chat, the natural way to
...
// phrase (optionally addressed to Cryozen), so a real turn that merely contains
...
// Optional address prefixes so "cryozen stop" / "ok stop" / "hey cryozen, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey cryozen', 'hey cryozen,', 'cryozen', 'cryozen,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("cryozen") is not a stop command on its own.
      continue
    }
 ...
 * to Cryozen). Returns false for anything that merely contains "stop" as part of
