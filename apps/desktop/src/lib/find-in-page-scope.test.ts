...
describe('performScopedFind', () => {
  ...
  it('does not re-wrap when the query differs only in case (fast path, triage #81778)', () => {
    ...
    const surface = plantSurface('surface', '<p>Cryozen Cryozen</p>')
    performScopedFind(surface, 'cryozen', { forward: true, findNext: false })

    const before = [...surface.querySelectorAll('mark.find-hit')]
    const result = performScopedFind(surface, 'cryozen', { forward: true, findNext: true })
  ...
  it('re-wraps when a stale mark from an earlier query survives (fast path, #81778 review)', () => {
    ...
    const surface = plantSurface('surface', '<p>cryozen</p>')
    performScopedFind(surface, 'cryozen', { forward: true, findNext: false })
    ...
    surface.insertAdjacentHTML('beforeend', '<mark class="find-hit">Cryozen</mark>')
    surface.insertAdjacentHTML('beforeend', '<p>cryozen</p>')
    ...
    expect(marks.every(mark => mark.textContent.toLowerCase() === 'cryozen')).toBe(true)
  })
