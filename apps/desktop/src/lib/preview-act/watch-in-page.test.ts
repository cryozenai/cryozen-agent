...
beforeEach(() => {
  ...
  document.querySelector('cryozen-watch')?.remove()
  delete (window as unknown as Record<string, unknown>).__cryozenWatch
})

/** The overlay lives in a closed shadow root, so a test can only see the host. */
const host = () => document.querySelector('cryozen-watch')
...
const drawn = () =>
  (window as unknown as { __cryozenWatch: { parts: Record<string, HTMLElement> & { shadow: ShadowRoot } } })
    .__cryozenWatch.parts
...
describe('watchInPage', () => {
  it('mounts one host on documentElement and reuses it across stages', () => {
    ...
    expect(document.querySelectorAll('cryozen-watch')).toHaveLength(1)
  ...
  it('rebuilds its chrome when the injected source changes underneath it', () => {
    ...
    const stamp = (value: number) => {
      ;(window as unknown as Record<string, unknown>).__cryozenWatchTag = value
    }
