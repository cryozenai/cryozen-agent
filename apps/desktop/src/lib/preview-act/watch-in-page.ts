...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __cryozenWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__cryozenWatch || (win.__cryozenWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('cryozen-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('cryozen-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __cryozenWatchTag?: number }).__cryozenWatchTag
