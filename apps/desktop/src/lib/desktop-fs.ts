import { cryozenApi } from '@/api/client'
import type {
  CryozenConnection,
  CryozenReadDirResult,
  CryozenReadFileTextResult,
  CryozenSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: CryozenConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: CryozenConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<CryozenReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<CryozenReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<CryozenReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<CryozenReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.cryozenDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: CryozenSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
