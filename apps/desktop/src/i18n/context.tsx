import { applyDocumentLocale, isRecord } from '@cryozen/shared/i18n'
import { createContext, type ReactNode, useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react'

import { type CryozenConfigRecord, getCryozenConfigRecord, retainConfigReadOrigin, saveCryozenConfig } from '@/cryozen'
...
export interface I18nConfigClient {
  getConfig: () => Promise<CryozenConfigRecord>
  saveConfig: (config: CryozenConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.cryozenDesktop?.api) {
    ...
    return getCryozenConfigRecord(undefined, { includeDefaults: false })
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.cryozenDesktop?.api) {
    ...
    // No explicit scope: saveCryozenConfig resolves the record's captured read
    // origin itself (resolveConfigWriteScope), and withConfigDisplayLanguage
    // retains that origin onto the derived record.
    return saveCryozenConfig(config, undefined, { preserveLanguage: true })
  }
}

export function getConfigDisplayLanguage(config: CryozenConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: CryozenConfigRecord, locale: Locale): CryozenConfigRecord {
    ...
    const loadLocale = () => {
      ...
      return configClient
        ...
        .then(async config => {
          ...
          const machineProfile = await window.cryozenDesktop?.getMachineProfile?.().catch(() => null)
