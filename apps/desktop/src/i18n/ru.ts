...
export const ru = defineLocale({
  sessionImport: {
    ...
    subtitle: 'Перенесите разговор в Cryozen и продолжите с того места, где остановились.',
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в Cryozen.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в Cryozen. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в Cryozen',
    continue: 'Продолжить в Cryozen',
  ...
  boot: {
    ready: 'Cryozen Desktop готов',
    desktopBootFailedWithMessage: message => `Не удалось запустить приложение: ${message}`,
    steps: {
      connectingGateway: 'Подключение к шлюзу',
      loadingSettings: 'Загрузка настроек Cryozen',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду Cryozen…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingCryozenDesktop: 'Запуск Cryozen Desktop…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс Cryozen завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс Cryozen завершился при запуске.',
    ...
    failure: {
      title: 'Cryozen не удалось запустить',
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд Cryozen старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    installMethodUnsupportedTitle: 'Неподдерживаемый способ установки',
    updateCryozen: 'Обновить Cryozen',
    ...
    errors: {
      ...
      methodNotAllowed:
        'Бэкенд приложения отклонил запрос (405 Method Not Allowed). Попробуйте перезапустить Cryozen Desktop.',
    ...
    native: {
      ...
      inputBody: 'Cryozen ожидает ваш ответ.',
      turnDoneTitle: 'Cryozen завершил',
      turnDoneBody: '',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям Cryozen по умолчанию?',
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока Cryozen в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'Cryozen задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока Cryozen был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока Cryozen был в фоне.'
        }
      },
    ...
    appearance: {
      ...
      colorModeDesc: 'Выберите фиксированный режим или позвольте Cryozen следовать настройкам системы.',
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Cryozen сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия Cryozen — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из Cryozen и откройте снова, затем вернитесь сюда.',
        on: 'Вкл',
        off: 'Выкл',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого Cryozen останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые Cryozen может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как Cryozen обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда Cryozen обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Рантайм Cryozen обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
      ...
      automaticUpdatesDesc: 'Cryozen автоматически проверяет обновления в фоне и сообщает, когда они готовы.',
    ...
    config: {
      ...
      minimizeToTrayDesc:
        'Сворачивание окон или закрытие главного окна скрывает их в системном трее (строке меню macOS), оставляя Cryozen работать. Для выхода выберите «Выйти из Cryozen» в меню трея или нажмите Cmd+Q. По умолчанию выключено; действует только на этом устройстве.',
      ...
      loading: 'Загрузка конфигурации Cryozen…',
    ...
    hudModifier: {
      ...
      permission:
        'Разрешите Cryozen в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем повторите попытку. Жест не записывает нажатия клавиш и не снимает экран.',
      unavailable:
        'Вспомогательная программа жеста HUD не запустилась или неожиданно остановилась. Повторите попытку или перезапустите Cryozen. Обычное сочетание HUD по-прежнему работает внутри Cryozen.',
      missingHelper:
        'В этой установке Cryozen отсутствует вспомогательная программа жеста HUD. Обновите или переустановите Cryozen и повторите попытку.',
      unsupportedSession:
    ...
    screenshot: {
      ...
      enabledDesc:
        'Нажмите обе клавиши Command одновременно в любом приложении, чтобы снять его переднее окно и прикрепить снимок к текущему черновику Cryozen. Автоматической отправки нет. По умолчанию выключено; действует только на этом Mac. Окно может содержать конфиденциальные данные — проверьте вложение перед отправкой.',
      ...
      inputPermission:
        'Разрешение на мониторинг ввода позволяет Cryozen распознавать обе клавиши Command, когда активно другое приложение. Разрешите Cryozen доступ в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем вернитесь сюда и повторите попытку.',
      screenPermission:
        'Разрешение на запись экрана позволяет Cryozen снимать переднее окно приложения при использовании этого сочетания. Разрешите Cryozen доступ в Системных настройках → Конфиденциальность и безопасность → Запись экрана, затем вернитесь сюда и повторите попытку. Перезапустите Cryozen, если macOS попросит.',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая Cryozen.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами Cryozen, до которых можно дотянуться через удалённые, SSH или Cloud-соединения.',
      stagedNote:
      ...
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм Cryozen, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз Cryozen, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindSshDesc: 'Установка Cryozen, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения хранятся зашифрованными. Заголовки, которыми управляет Cryozen (Authorization, Cookie, Host…), игнорируются.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом Cryozen в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      envOverrideTitle: 'Переменные окружения управляют этой сессией приложения.',
      envOverrideDesc:
        'Сбросьте CRYOZEN_DESKTOP_REMOTE_URL и CRYOZEN_DESKTOP_REMOTE_TOKEN, чтобы использовать сохранённую настройку ниже.',
      modeTitle: 'Режим подключения',
      localTitle: 'Локальный шлюз',
      localDesc: 'Запускает приватный бэкенд Cryozen на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду Cryozen.',
      remoteAuthHint:
      ...
      remoteUrlDesc: 'Базовый URL удалённого бэкенда дашборда. Поддерживаются префиксы пути, например /cryozen.',
      ...
      restartingMessage: 'Cryozen Desktop переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · Cryozen ${version}` : ''}`,
      ...
      sshDesc:
        'Cryozen запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshCryozenPathTitle: 'Путь к Cryozen (необязательно)',
      sshCryozenPathDesc: 'Полный путь к бинарнику cryozen на удалённой машине. Пусто = автоопределение.',
      sshCryozenPathPlaceholder: 'автоопределение',
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — Cryozen найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — Cryozen запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrNotInstalled:
        'Cryozen не установлен на удалённой машине. Установите его там (curl -fsSL https://cryozenai.github.io/cryozen-agent/install.sh | sh) или задайте путь к Cryozen.',
      sshErrPlatform:
        'Неподдерживаемая удалённая платформа. SSH-режим Cryozen Desktop поддерживает удалённые хосты Linux, macOS и Windows.',
      sshErrTimeout: 'SSH-соединение истекло. Хост может быть недоступен или «спит».',
      sshErrUpdateRequired: 'Перед подключением через SSH обновите Cryozen на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в Cryozen. Проверьте конфигурацию ниже — она пришла из ссылки, а не из Cryozen.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. Cryozen проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте Cryozen на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
  ...
  skills: {
    ...
    skillArchivedMessage: 'Восстановить через cryozen curator restore.',
    hub: {
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как Cryozen создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите Cryozen, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите Cryozen, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — Cryozen уведомит, когда будет готово.',
    ...
    nav: {
      ...
      settings: { title: 'Настройки', detail: 'Настройка Cryozen desktop' },
    ...
    cryozenActiveSessions: (version, count) => `Cryozen ${version} · Активные сеансы: ${count}`,
    ...
    usageEmpty: period => `Нет использования за последние ${period} ${RU_NOUN(period, 'день', 'дня', 'дней')}.`,
    retry: 'Повторить',
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ID пользователя бота', placeholder: '@cryozen:example.org' },
  ...
  profiles: {
    ...
    defaultDescription: 'Используется при запуске Cryozen и для новых чатов. Профили существующих сессий не меняются.',
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды Cryozen: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  modelAssignment: {
    saveFailed: 'Cryozen не сохранил это изменение модели.'
  },
  ...
  cron: {
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. Cryozen выполнит его и доставит результаты в выбранное вами место.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд Cryozen, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из Cryozen. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд Cryozen, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск Cryozen...',
    placeholderReconnecting: 'Переподключение к Cryozen…',
    placeholderFollowUp: 'Отправить продолжение',
    newSessionPlaceholders: [
      'Что будем делать?',
      'Дайте Cryozen задачу',
      'О чём вы думаете?',
      'Опишите, что нужно',
      'Что обсудим?',
    ...
    attachUrlDesc: 'Cryozen загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить Cryozen открыть PR',
  ...
  updates: {
    stages: {
      ...
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем Cryozen…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем Cryozen…',
    ...
    unsupportedMessage: 'Эта версия Cryozen не может обновлять себя из приложения.',
    connectionRetry: 'Проверьте соединение и попробуйте снова.',
    gitUnusable: 'Cryozen не удалось запустить Git на этом компьютере, поэтому проверить обновления не получилось.',
    ...
    availableBody: 'Новая версия Cryozen готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда Cryozen готова к установке.',
    ...
    manualBody:
      'Cryozen установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'Cryozen подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение Cryozen (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор Cryozen возьмёт процесс на себя в своём окне и автоматически перезапустит Cryozen, когда закончит. Пожалуйста, не открывайте Cryozen вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. Cryozen переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем Cryozen откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить Cryozen?',
    blockerBody:
      'Cryozen нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить Cryozen',
    foreignBlockerBody:
      'Cryozen не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'Cryozen может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
  ...
  install: {
    ...
    oneTimeTitle: 'Cryozen требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка Cryozen Desktop',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу Cryozen или установите Cryozen локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему Cryozen',
    ...
    installLocalTitle: 'Установить Cryozen локально',
    installLocalDesc: 'Скачайте Cryozen, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите Cryozen Desktop и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему Cryozen',
    remoteSetupDesc: 'Введите URL вашего шлюза. Cryozen Desktop определит, нужен токен или вход через браузер.',
    remoteUrlTitle: 'URL шлюза',
    remoteUrlDesc: 'Используйте базовый URL шлюза Cryozen, включая https:// для удалённых.',
    remoteUrlPlaceholder: 'https://gateway.example.com/cryozen',
    probing: 'Определяем аутентификацию шлюза...',
    probeError: 'Не удалось подключиться к этому шлюзу Cryozen.',
    ...
    settingUpTitle: 'Настройка Cryozen Agent',
    finishingTitle: 'Завершаем',
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция Cryozen CLI или desktop. Остановите все работающие инстанции Cryozen и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик Cryozen скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    headerTitle: 'Настроим для вас Cryozen Agent',
    headerDesc: 'Подключите провайдера модели, чтобы начать общение. Большинство вариантов — в один клик.',
    preparingInstall: 'Cryozen завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем Cryozen…',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description:
          'Укажите Cryozen локальный или self-hosted OpenAI-совместимый endpoint (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — Cryozen подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте Cryozen там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте Cryozen там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} — ${size}. Cryozen покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'Cryozen перезапускается...',
      askRestart: 'Попросить Cryozen перезапустить сервер',
      lookingRestart: taskId => `Cryozen ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'Cryozen работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      blankPageBody: 'Введите адрес выше, чтобы просматривать, или попросите Cryozen открыть страницу.',
      finishedRestarting: message =>
        `Cryozen завершил перезапуск сервера предпросмотра${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Cryozen не смог перезапустить сервер.',
      stillWorking:
        'Cryozen всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  interfaceMode: {
    ...
    hint: 'Меняет то, что показано, а не то, что умеет Cryozen.',
    sessionNote:
      'Задано простым режимом. Изменение здесь действует до конца сеанса; переключитесь в расширенный, чтобы сделать его своим.',
    simple: {
      label: 'Простой',
      description: 'Для общения с Cryozen. Боковая панель и чат; без терминала, файлов и панелей diff.'
    },
    advanced: {
      label: 'Расширенный',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Cryozen загружает ответ',
    ...
    approval: {
      gatewayDisconnected: 'Шлюз Cryozen не подключён',
      ...
      alwaysDescription: pattern =>
        `Это добавит паттерн «${pattern}» в ваш постоянный список разрешений (~/.cryozen-agent/config.yaml). Cryozen больше не будет спрашивать о подобных командах — ни в этом сеансе, ни в будущем.`,
  ...
  prompts: {
    ...
    secretDesc: 'Cryozen нужны учётные данные, чтобы продолжить.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите Cryozen Desktop, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите Cryozen Desktop, чтобы сохранять изображения',
    ...
    handoff: {
      ...
      timedOut: 'Превышено время ожидания шлюза. Выполняется ли `cryozen gateway`?'
    }
  },
  errors: {
