...
import type { CryozenConfigRecord } from '@/cryozen'

import { type I18nConfigClient, I18nProvider, useI18n } from './context'
import type { Locale } from './types'

function LanguageProbe({ target = 'ja' }: { target?: Locale }) {
...
describe('I18nProvider', () => {
  ...
  it('normalizes an initial locale alias and switches translations', async () => {
    render(
      <I18nProvider configClient={null} initialLocale="ja-JP">
  ...
  it('loads the initial locale from display.language config', async () => {
    const configClient: I18nConfigClient = {
      getConfig: vi.fn().mockResolvedValue({ display: { language: 'ja_jp' } }),
      saveConfig: vi.fn()
    }
  ...
  it('lands a configured Chinese language on English without rewriting it', async () => {
    const configClient: I18nConfigClient = {
  ...
  it('reads latest config before saving language and preserves unrelated values', async () => {
    ...
    const latestConfig: CryozenConfigRecord = {
    ...
    expect(saveConfig).toHaveBeenCalledWith({
      display: { language: 'ja', skin: 'slate' },
      terminal: { cwd: '/new' }
    })
  })
  ...
  it('retries a transient config failure and applies the persisted locale', async () => {
    const getConfig = vi
      ...
      .mockResolvedValueOnce({ display: { language: 'ja-JP' } })
    ...
    await waitFor(() => expect(screen.getByTestId('locale').textContent).toBe('ja'), { timeout: 5_000 })
