import type { ConnectionRequestPayload, ToolLabel } from '@cryozen/shared'
...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Cryozen doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in cryozen_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `cryozen model` uses — instead of
 ...
 * choices as `cryozen model`'s custom-provider setup (#93622). */
...
export interface CryozenConfig {
  agent?: {
...
export type CryozenConfigRecord = Record<string, unknown>
...
export interface ProfileInfo {
  ...
  display_name?: string
  /** Bot Mode title (profile.yaml ui_meta['cryozen-bots'].title) — the name the
...
// non-desktop) Cryozen simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (cryozen.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors cryozen_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Cryozen, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Cryozen but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `cryozen tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ post-setup install
   *  state). Optional for older backends. */
 ...
 *  not Cryozen); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** cryozen-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** cryozen-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/cryozen/update/check` — the backend's own update state.
...
/** One curated MCP catalog entry from `GET /api/mcp/catalog`. */
