...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-brand` + a `--stroke-brand` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-brand`,
...
```
shadow-brand           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-brand) /* currentColor hairline, theme-adaptive */
```
...
normal behavior. The tray's **Show Cryozen** restores hidden windows;
**Quit Cryozen** keeps the ordinary active-work confirmation and teardown.
...
| `--stroke-brand` | the overlay hairline (pairs with `shadow-brand`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph - the
  ice-crystal mark on its own dark rounded tile, identical in light/dark.
...
- Every user-facing string goes through `useI18n()` (`src/i18n/context.tsx`).
  No literals in JSX.
- **Update all locales together** — `en`, `ja`, `ar`, `ru`. A string change
...
- [ ] Reuse a primitive (`Button`, `SearchField`, `SegmentedControl`,
      ...
      forking one?
- [ ] Tokens (`--ui-*`, `shadow-brand`, `--stroke-brand`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-brand` + `border-(--stroke-brand)`, no hard border?
