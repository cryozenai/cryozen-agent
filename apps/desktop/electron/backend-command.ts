// Backend subcommand routing for the desktop-managed Cryozen process.
//
// The desktop app launches its own headless backend via `cryozen serve` — it
...
// app hasn't updated yet, or an older `cryozen` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Cryozen profile to pin via `--profile`.
 */
 ...
 * `-m cryozen_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `cryozen_cli/subcommands/dashboard.py` source registers
