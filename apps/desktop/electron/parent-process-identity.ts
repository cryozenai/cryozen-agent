export type ParentWatchdogEnv = {
  CRYOZEN_PARENT_PID: string
  CRYOZEN_PARENT_START_MARKER?: string
  CRYOZEN_PARENT_NONCE?: string
  /** Spawn tag consumed by cryozen_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { CRYOZEN_PARENT_PID: String(pid) }
  env.CRYOZEN_SPAWN = spawnTag(pid, startMarker)
  ...
  env.CRYOZEN_PARENT_START_MARKER = startMarker
  env.CRYOZEN_PARENT_NONCE = nonce

  return env
}

/** Build the `CRYOZEN_SPAWN` tag mirrored by cryozen_cli/process_identity.py.
