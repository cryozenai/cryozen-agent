 ...
 *
 * The Python side (cryozen_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes CRYOZEN_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(cryozenHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(cryozenHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(cryozenHome: string, userData: string): PendingNotice | null {
  const report = readReport(cryozenHome)
  ...
  const copy = report.in_effect
    ? {
        ...
        message: `These plugins were built for an older Cryozen and were turned off: ${nameList}. Cryozen works normally without them.`,
        detail: `Look for an updated version of each plugin or ask its author.\n\n${list}`
      }
    : {
        title: 'Some plugins need an update',
        message: `These plugins were built for an older Cryozen and will stop working on ${report.removal_date}: ${nameList}.`,
