...
// orphan (ppid=1) — a hung remote CLI (e.g. a wedged `cryozen --version`)
...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(CRYOZEN_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Cryozen-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.cryozen-agent/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'cryozen-desktop-ssh')
  }

  return path.join(os.homedir(), '.cryozen-agent', 'desktop-ssh')
}
