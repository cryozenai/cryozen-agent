...
export function createMinimizeToTray(options: Options) {
  ...
  const broadcast = () => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send('cryozen:minimize-to-tray:changed', status())
      }
    }
  }
  ...
  const apply = async (on: boolean) => {
    ...
    if (!on) {
      ...
      try {
        ...
        tray.setToolTip('Cryozen')
        tray.setContextMenu(
          Menu.buildFromTemplate([
            { label: 'Show Cryozen', click: restore },
            { type: 'separator' },
            // Do not bypass the ordinary active-work confirmation or teardown.
            { label: 'Quit Cryozen', click: () => app.quit() }
          ])
        )
  ...
  ipcMain.handle('cryozen:minimize-to-tray:get', status)
  ipcMain.handle('cryozen:minimize-to-tray:set', (_event, on) => setEnabled(on === true))
