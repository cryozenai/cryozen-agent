 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Cryozen Agent by spawning
...
// Relative, not `@cryozen/shared/ansi`: the electron bundle is built by esbuild
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.cryozen-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(cryozenHome) {
  return path.join(cryozenHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.cryozen-agent/cryozen-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(cryozenHome) {
  if (!cryozenHome) {
    return null
  }

  const candidate = path.join(cryozenHome, 'cryozen-agent', 'scripts', installScriptName())
...
function cachedScriptPath(cryozenHome, commit) {
  return path.join(bootstrapCacheDir(cryozenHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/cryozenai/cryozen-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(cryozenHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(cryozenHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, cryozenHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass CRYOZEN_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          CRYOZEN_HOME: cryozenHome || process.env.CRYOZEN_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, cryozenHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, cryozenHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--cryozen-home', cryozenHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, cryozenHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, cryozenHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(cryozenHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, cryozenHome, emit })
    abortSignal?.throwIfAborted()
