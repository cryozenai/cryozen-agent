...
// that never go through the `cryozen desktop` launcher.
...
describe('renderer heap flags', () => {
  ...
  it('merges with a js-flags switch already on argv instead of overwriting it', () => {
    ...
    const planned = planLaunchSwitches(cfg, ['/app/cryozen', '--js-flags=--expose-gc', '--disable-gpu'])
    ...
    expect(planLaunchSwitches(cfg, ['/app/cryozen', '--js-flags', '--max-old-space-size=4096'])).toEqual([
      { name: 'disable-gpu' },
