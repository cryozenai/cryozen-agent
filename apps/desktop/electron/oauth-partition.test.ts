...
// evict each other's `cryozen_session*` cookies when they ride the single
// shared `persist:cryozen-remote-oauth` partition — and, worse, gateway A's
...
describe('resolveOauthPartition (#92183 per-connection cookie jars)', () => {
  ...
  it('falls back to the legacy partition for unmatched and malformed inputs', () => {
    ...
    expect(resolveOauthPartition('https://unregistered.example.com/api/status', { registry: reg })).toBe(
      LEGACY_OAUTH_PARTITION
    )
