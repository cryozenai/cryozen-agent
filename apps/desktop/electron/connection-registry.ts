 ...
 * apps/shared/src/backend-scope.ts (`@cryozen/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteCryozenPath || '').trim() !== String(activeSsh.remoteCryozenPath || '').trim() ||
...
/** Turn `ls ~/.cryozen-agent/profiles` output into roster names. Always includes
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteCryozenPath: input.remoteCryozenPath,
  ...
  if (kind === 'remote') {
    ...
    // transport-/Cryozen-managed names; an empty result stores nothing.
 ...
 * `remoteCryozenPath`/`remoteProfile`. Only fields the payload explicitly
 ...
 *
 * Remote entries are matched by normalized URL so re-applying never
 * duplicates a gateway. Existing identity and user-chosen label win. Switching
 * to local keeps registered remotes available while moving primary/last-used
 * back to This device.
 */
