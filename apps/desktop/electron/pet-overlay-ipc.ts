...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('cryozen:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('cryozen:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('cryozen:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('cryozen:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('cryozen:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('cryozen:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('cryozen:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('cryozen:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('cryozen:pet-overlay:control', payload)
  })
}
