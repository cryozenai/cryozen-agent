// Attach to the host's running Cryozen backend (multiplex-only, Desktop half).
//
...
export function spawnLedgerPath(cryozenHomeRoot: string, join: (...parts: string[]) => string): string {
  return join(cryozenHomeRoot, SPAWN_LEDGER_FILENAME)
}
...
export async function attachToHostBackend(
  ...
  if (decision.action === 'spawn') {
    if (decision.reason === 'isolated') {
      deps.log('[attach] CRYOZEN_DESKTOP_ISOLATED_BACKEND is set; spawning a dedicated backend')
    }
  ...
  for (const record of ordered) {
    ...
    if (attached) {
      deps.log(
        `[attach] attached to the running Cryozen backend on ${attached.baseUrl} ` +
