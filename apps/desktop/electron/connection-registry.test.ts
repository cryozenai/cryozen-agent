...
test('primary SSH reuse rejects a descriptor with a different remote Cryozen path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
    remote: { mode: 'ssh', host: 'build-host', remoteCryozenPath: '/srv/cryozen', user: 'alice' },
    profiles: {}
  })
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    primary: 'cryozen-vps',
    launchMode: 'primary',
    lastUsed: 'cryozen-vps',
    connections: [
      { id: LOCAL_CONNECTION_ID, kind: 'local', label: 'This device' },
      { id: 'cryozen-vps', kind: 'ssh', label: 'Cryozen VPS', host: 'cryozen-vps' }
    ]
  })

  const descriptor = {
    connectionId: 'cryozen-vps',
    mode: 'remote' as const,
    remoteKind: 'ssh' as const,
    ssh: { host: 'cryozen-vps' }
  }

  assert.equal(registrySourceOwnsPrimaryBackend(registry, 'cryozen-vps', descriptor), true)
...
test('resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile', () => {
  ...
  const registry: ConnectionRegistry = {
    ...
    connections: [
      ...
      { ...base, id: 'ssh-path', label: 'SSH path', remoteCryozenPath: '/opt/cryozen' },
  ...
  assert.equal(resolve({ ...base, remoteCryozenPath: '/opt/cryozen' }), 'ssh-path')
...
// The electron and @cryozen/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@cryozen/shared'))) as {
...
test('roster: source profile metadata follows the connection-qualified row', () => {
  ...
  const vpsMeta = {
    display_name: 'Emma',
    ui_meta: { 'cryozen-bots': { title: 'Emma', shape: 'blobatar::sun', color: '#8b5cf6' } },
    has_avatar: true
  }
...
test('merge preserves fields the editor does not carry (ssh extras)', () => {
  ...
  assert.equal(labelOnly.remoteCryozenPath, '/opt/cryozen')
...
test('duplicate gateway URLs are rejected', () => {
  let registry = emptyRegistry()
  registry = upsertConnection(
    registry,
    normalizeConnectionInput({ kind: 'remote', label: 'Homelab', url: 'http://10.0.0.5:9119' }, registry)
...
test('Apply remote preserves an existing URL identity and label without duplicates', () => {
  ...
  assert.equal(matches[0].id, 'cryozen-alex')
  assert.equal(matches[0].label, 'Existing gateway')
  assert.equal(matches[0].authMode, 'oauth')
  assert.equal(applied.primary, 'cryozen-alex')
  assert.equal(applied.lastUsed, 'cryozen-alex')
})
