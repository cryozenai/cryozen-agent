...
// spawns `cryozen serve`; runtimes older than serve only have `dashboard`, so
...
// (covers a bare `cryozen` resolved from PATH with no known source root). Result
...
export function createBackendServeSupportResolver(cryozenHome: string, rememberLog: (message: string) => void) {
    ...
    const pending = (async () => {
      ...
      if (backend.root) {
        try {
          const src = await fs.promises.readFile(
            path.join(backend.root, 'cryozen_cli', 'subcommands', 'dashboard.py'),
            'utf8'
          )
      ...
      if (supported === null) {
        try {
          ...
          // `cryozen --version` (~10.5s measured cold), and a false negative here
          ...
          await execProbe(backend.command, [...prefix, 'serve', '--help'], {
            ...
            env: { ...process.env, CRYOZEN_HOME: cryozenHome, ...(backend.env || {}) },
            timeout: PROBE_TIMEOUT_MS,
            stdio: 'ignore',
            // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
            // (see resolveCryozenBackend step 4); execFileSync of a .cmd without
