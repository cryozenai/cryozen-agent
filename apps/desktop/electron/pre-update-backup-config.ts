...
export interface CryozenConfigRuntime {
  command?: string
  args?: string[]
...
export async function readPreUpdateBackupEnabled(
  runtime: CryozenConfigRuntime | Promise<CryozenConfigRuntime>,
  cryozenHome: string,
  run = execFileAsync
): Promise<boolean> {
  try {
    ...
    const result = await run(resolvedRuntime.command, resolvedRuntime.args, {
      encoding: 'utf8',
      env: { ...process.env, ...resolvedRuntime.env, CRYOZEN_HOME: cryozenHome },
