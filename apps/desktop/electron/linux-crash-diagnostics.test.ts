...
test('on linux, fatal Chromium output lands in a file under the Cryozen logs dir', () => {
  const plan = linuxCrashDiagnostics('/home/u/.cryozen-agent/logs', 'linux')
  ...
  assert.ok(logFile)
  assert.equal(path.dirname(logFile), '/home/u/.cryozen-agent/logs')
...
test('other platforms get no Chromium logging switches and no crash reporter', () => {
  assert.equal(linuxCrashDiagnostics('/Users/u/.cryozen-agent/logs', 'darwin'), null)
  assert.equal(linuxCrashDiagnostics('C:\\Users\\u\\.cryozen-agent\\logs', 'win32'), null)
})
...
test('a logs dir that cannot be created degrades to no logging, never a dead shell', () => {
  ...
  // Read-only or invalid CRYOZEN_HOME/logs: mkdir throws before app readiness.
...
test('the Chromium log is bounded before Chromium appends to it', () => {
  ...
  assert.deepEqual(reclaimed, [path.join('/home/u/.cryozen-agent/logs', CHROMIUM_LOG_FILENAME)])
})
