 ...
 *   - the browser sees only a minimal "return to Cryozen" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Cryozen.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('cryozen:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('cryozen:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('cryozen:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
