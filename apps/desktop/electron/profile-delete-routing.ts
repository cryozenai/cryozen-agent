// Profile-delete routing logic for the `cryozen:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_cryozen_home() and
...
/** Parse a `cryozen:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its CRYOZEN_HOME.
 *
...
/**
 * Route the next `cryozen:api` request away from the primary/window backend
 ...
 * ensure_cryozen_home() recreates the directory the delete just removed.
 */
