 ...
 * Opt in with CRYOZEN_TEST_REAL_SERVE=1; see pool-retirement-live-fixture/README.md.
 */
...
function isolatedEnv(root: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    HOME: root,
    USERPROFILE: root,
    CRYOZEN_HOME: join(root, '.cryozen-agent'),
    ...
    TZ: 'UTC',
    LANG: 'C.UTF-8',
    CRYOZEN_DESKTOP_CDP_PORT: 'off',
    CRYOZEN_DESKTOP_USER_DATA_DIR: join(root, 'user-data')
  }
}
...
test.skipIf(process.env.CRYOZEN_TEST_REAL_SERVE !== '1' || process.platform === 'win32')(
  'native retirement preserves backend-only cron and hands both waiters capacity only after real child exit',
  async () => {
    const python = process.env.CRYOZEN_TEST_PYTHON
    assert.ok(python && existsSync(python), 'Set CRYOZEN_TEST_PYTHON to an installed Cryozen Python environment')
    const electron = process.env.CRYOZEN_TEST_ELECTRON || (require('electron') as string)
    assert.ok(existsSync(electron), 'CRYOZEN_TEST_ELECTRON must name a real native Electron executable')
    const root = mkdtempSync(join(tmpdir(), 'cryozen-pool-retirement-live-'))
    const resultPath = join(root, 'result.json')
    let child: ChildProcess | undefined
    let output = ''

    try {
      mkdirSync(join(root, '.cryozen-agent'))
      ...
      await build({
        ...
        platform: 'browser',
        format: 'iife',
        alias: { '@': join(desktop, 'src'), '@cryozen/shared': join(repo, 'apps/shared/src') },
