...
export function registerNativeNotifications({
  ...
  ipcMain.handle('cryozen:notify', async (event, payload: CryozenNotification) => {
    ...
    notification.on('click', () => {
      ...
      if (focusSessionId) {
        window.webContents.send('cryozen:focus-session', focusSessionId)
      }

      if (payload?.activate || payload?.notifyId) {
        window.webContents.send('cryozen:notification-activate', {
    ...
    notification.on('action', (actionEvent, index) => {
      ...
      if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
        ...
        window.webContents.send('cryozen:notification-action', { sessionId: payload.sessionId, actionId: action.id })
