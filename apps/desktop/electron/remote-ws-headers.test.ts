...
function expectRequestHeaders(
  ...
  applyRemoteRequestHeaders({ url, requestHeaders: { Origin: 'app://cryozen' } }, callback, store.headersFor)

  expect(callback).toHaveBeenCalledOnce()
  expect(callback).toHaveBeenCalledWith(expected ? { requestHeaders: { Origin: 'app://cryozen', ...expected } } : {})
}
...
describe('OAuth login and registry extra headers', () => {
  ...
  it('injects extra headers on an OAuth partition session, not only defaultSession', () => {
    ...
    const callback = vi.fn()
    listeners[0]({ url: 'https://gateway.example/login', requestHeaders: { Origin: 'app://cryozen' } }, callback)

    expect(listeners).toHaveLength(1)
    expect(callback).toHaveBeenCalledWith({
      requestHeaders: { Origin: 'app://cryozen', ...accessHeaders }
    })
  })
})
