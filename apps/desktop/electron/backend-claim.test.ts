...
test('output tail interleaves stdout and stderr attached from spawn time', () => {
  ...
  child.stderr.emit('data', Buffer.from("ModuleNotFoundError: No module named 'cryozen_cli'\n"))
...
test('exit line carries the buffered tail next to the exit code, preferring the signal', () => {
  ...
  assert.equal(
    formatBackendExitLine('Ignoring stale Cryozen backend exit', 1, null, tail),
    'Ignoring stale Cryozen backend exit (1)\nRecent backend output:\nTraceback (most recent call last):'
  )
  assert.equal(
    formatBackendExitLine('Cryozen backend exited', null, 'SIGTERM', tail),
    'Cryozen backend exited (SIGTERM)\nRecent backend output:\nTraceback (most recent call last):'
  )
})

test('exit line stays byte-identical to the legacy shape when the tail is empty or missing', () => {
  assert.equal(
    formatBackendExitLine('Cryozen backend exited', 0, null, createBackendOutputTail(64)),
    'Cryozen backend exited (0)'
  )
  assert.equal(formatBackendExitLine('Cryozen backend exited', 1, null, null), 'Cryozen backend exited (1)')
})
