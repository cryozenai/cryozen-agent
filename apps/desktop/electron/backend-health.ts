...
export interface CryozenReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForCryozenReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Cryozen gateway uses OAuth, but you are not signed in. ' +
 ...
 * `isReauthRequired` so startCryozen latches; a bare `needsOauthLogin` (the
...
export async function waitForCryozenReady(baseUrl: string, options: CryozenReadyOptions): Promise<void> {
  ...
  throw new Error(`Cryozen backend did not become ready: ${detail}`)
}
