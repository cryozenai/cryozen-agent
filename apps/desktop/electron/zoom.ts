...
export const ZOOM_STORAGE_KEY = 'cryozen:desktop:zoomLevel'
 ...
 * but forgot to emit 'cryozen:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('cryozen:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `cryozen:zoom:changed`, which never fired —
