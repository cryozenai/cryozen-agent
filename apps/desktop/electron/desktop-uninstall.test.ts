...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'cryozen_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'cryozen_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'cryozen_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Cryozen.app/Contents/MacOS/Cryozen', 'darwin'),
    '/Applications/Cryozen.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Cryozen.app/Contents/MacOS/Cryozen', 'darwin'),
    '/Users/x/Applications/Cryozen.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Cryozen\\Cryozen.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Cryozen'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\cryozen-desktop\\Cryozen.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\cryozen-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Cryozen.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_CryozenXXXX/cryozen', 'linux', { APPIMAGE: '/home/x/Apps/Cryozen.AppImage' }),
    '/home/x/Apps/Cryozen.AppImage'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Cryozen.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Cryozen.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.cryozen-agent/cryozen-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.cryozen-agent/cryozen-agent',
    uninstallArgs: ['-m', 'cryozen_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/cryozen/linux-unpacked',
    cryozenHome: '/home/x/.cryozen-agent'
  })
  ...
  assert.match(script, /'-m' 'cryozen_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/cryozen\/linux-unpacked'/)
  assert.match(script, /export CRYOZEN_HOME='\/home\/x\/\.cryozen-agent'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.cryozen-agent/cryozen-agent',
    agentRoot: '/home/x/.cryozen-agent/cryozen-agent',
    uninstallArgs: ['-m', 'cryozen_cli.uninstall', '--mode', 'full'],
    appPath: null,
    cryozenHome: '/home/x/.cryozen-agent'
  })

  // System python + source on PYTHONPATH so import cryozen_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.cryozen-agent\/cryozen-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'cryozen_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'cryozen_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    cryozenHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'cryozen_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    pythonPath: 'C:\\cryozen',
    agentRoot: 'C:\\cryozen',
    uninstallArgs: ['-m', 'cryozen_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Cryozen',
    cryozenHome: 'C:\\Users\\x\\AppData\\Local\\cryozen'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import cryozen_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\cryozen;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "cryozen_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Cryozen" >nul 2>&1/)
