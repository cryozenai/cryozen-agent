// The standalone desktop-plugin root (`<CRYOZEN_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.cryozen-package.json` marker
...
export const PACKAGE_MARKER = '.cryozen-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.cryozen-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every cryozen home the app knows about locally: the default plus each profile. */
export async function localHomes(cryozenHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(cryozenHome, 'profiles'))

  return [cryozenHome, ...profiles.map(name => path.join(cryozenHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(cryozenHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(cryozenHome, 'profiles'))) {
    const scopedRoot = path.join(cryozenHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(cryozenHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(cryozenHome)) {
