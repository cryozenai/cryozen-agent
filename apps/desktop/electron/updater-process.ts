 ...
 *
 * The staged Tauri `cryozen-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `cryozen update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `cryozen update`, swaps/relaunches
 * the app, and writes .cryozen-update-result.json. With the app gone before
 * the update starts, the CRYOZEN_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `cryozen update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into CRYOZEN_HOME on *every* platform
 * (`cryozen-setup.exe` on Windows, `cryozen-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_cryozen_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `cryozen update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(cryozenHome, 'cryozen-setup.exe')
 ...
 *
 * `copy_self_to_cryozen_home` deliberately no-ops during `--update`
 ...
 * "Another Cryozen update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `cryozen update`, which
 ...
 * the installer is written to CRYOZEN_HOME at install/repair time, making mtime
 ...
 * The lead sentence is plain: nothing changed and Cryozen keeps running. The raw
...
export function describeUpdaterHandoffFailure(outcome: Pick<UpdaterHandoffOutcome, 'message'>): string {
  const lead =
    "The updater couldn't start, so nothing was changed and Cryozen keeps running as before. " +
