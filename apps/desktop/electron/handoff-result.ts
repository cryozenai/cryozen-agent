 ...
 * console. It writes CRYOZEN_HOME/.cryozen-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Cryozen within 30 minutes.
...
export function handoffResultPath(cryozenHome: string): string {
  return path.join(cryozenHome, '.cryozen-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(cryozenHome)
  let raw: string
