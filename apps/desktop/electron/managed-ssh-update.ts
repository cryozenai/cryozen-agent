 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `cryozen update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.cryozenHome, 'Cryozen home')
  const cryozenPath = validateRemoteValue(target.cryozenPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(cryozenPath)

  const updateCommand =
    `env CRYOZEN_HOME=${homeWord} ` +
    `CRYOZEN_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'CRYOZEN_UPDATE_ORIGIN_PROFILE=default ' +
    `CRYOZEN_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `CRYOZEN_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:CRYOZEN_HOME=${psLiteral(home)}`,
    `$env:CRYOZEN_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:CRYOZEN_UPDATE_ORIGIN_PROFILE="default"',
    `$env:CRYOZEN_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:CRYOZEN_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:CRYOZEN_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:CRYOZEN_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:CRYOZEN_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(cryozenPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.cryozen-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Cryozen updated and every managed SSH profile is ready.'
        : restoreOk
