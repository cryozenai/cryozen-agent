...
export function registerHudIpc({
  ...
  ipcMain.on('cryozen:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('cryozen:hud:windowing', event => {
  ...
  ipcMain.on('cryozen:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('cryozen:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('cryozen:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('cryozen:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('cryozen:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('cryozen:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('cryozen:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('cryozen:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('cryozen:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('cryozen:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('cryozen:hud:close', async () => {
    closeHudWindow()
