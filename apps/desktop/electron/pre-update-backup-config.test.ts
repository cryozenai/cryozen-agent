...
describe('readPreUpdateBackupEnabled', () => {
  it.each([
    ...
    const runtime = {
      ...
      args: ['-m', 'cryozen_cli.main', 'config', 'get', 'updates.pre_update_backup', '--json'],
      env: { PYTHONPATH: '/runtime/cryozen' }
    }
    ...
    expect(run).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          CRYOZEN_HOME: '/profiles/active',
          PYTHONPATH: '/runtime/cryozen'
        })
      })
    )
  })
  ...
  it.each([
    ...
    await expect(
      readPreUpdateBackupEnabled({ command: '/runtime/cryozen', args: ['config', 'get'] }, '/profiles/active', run)
    ).resolves.toBe(true)
  })
})
