...
test('launches the unpacked helper without prompting and delivers only validated capture messages', () => {
  ...
  const monitor = new CommandScreenshotMonitor({
    platform: 'darwin',
    appPath: '/Applications/Cryozen.app/Contents/Resources/app.asar',
    spawn
  })
  ...
  assert.deepEqual(spawn.mock.calls[0], [
    '/Applications/Cryozen.app/Contents/Resources/app.asar.unpacked/dist/native/command-screenshot-monitor',
    [],
...
test.skipIf(process.platform !== 'darwin')(
  ...
  () => {
    const dir = mkdtempSync(resolve(tmpdir(), 'cryozen-command-monitor-test-'))
...
test.skipIf(process.platform !== 'darwin')(
  ...
  async () => {
    const dir = mkdtempSync(resolve(tmpdir(), 'cryozen-command-monitor-build-'))
