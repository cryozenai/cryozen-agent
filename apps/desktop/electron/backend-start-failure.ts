 ...
 * startCryozen() re-throw the cached error without re-attempting the connect —
...
export interface BackendStartFailureContext {
   ...
   * SSH) primary backend rather than spawning a local child.
   */
...
/**
 * Whether a startCryozen() failure should latch into `backendStartFailure`.
 ...
 * Every subsequent `getConnection`/`api` call re-runs `startCryozen`, re-emits
