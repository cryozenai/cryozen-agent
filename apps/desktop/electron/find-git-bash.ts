 ...
 *   1. CRYOZEN_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\cryozen\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect CRYOZEN_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.CRYOZEN_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'cryozen', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'cryozen', 'git', 'usr', 'bin', 'bash.exe'))
  }
