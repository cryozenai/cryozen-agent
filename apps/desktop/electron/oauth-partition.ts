 ...
 * (`persist:cryozen-remote-oauth`) — the jar was keyed on the auth *mode*, not
 ...
 * setup: one box, two dashboards) fought over the same `cryozen_session*`
 ...
 *     by the upgrade.
 *   - Token-auth remotes and anything unmatched or malformed
...
export const LEGACY_OAUTH_PARTITION = 'persist:cryozen-remote-oauth'
...
// `Partitions/cryozen-remote-oauth%3Aconn%3A<id>` folder is left on disk,
...
export function resolveOauthPartition(requestUrl: unknown, opts: ResolveOauthPartitionOptions = {}): string {
  ...
  for (const entry of registry.connections) {
    ...
    // jar. Token entries never use cookies; the primary (and the v1 remote it migrated from) keeps the
