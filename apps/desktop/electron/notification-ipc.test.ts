...
it('returns native clicks and approval actions to the emitting window, not the primary', async () => {
  ...
  expect(source.webContents.send).toHaveBeenCalledWith('cryozen:focus-session', payload.focusSessionId)
  host.shown[0].emit('action', { actionIndex: 1 }, undefined)
  expect(source.webContents.send).toHaveBeenCalledWith('cryozen:notification-action', {
  ...
  expect(primary.webContents.send).not.toHaveBeenCalled()
  host.shown[0].emit('click')
  expect(primary.webContents.send).toHaveBeenCalledWith('cryozen:focus-session', payload.focusSessionId)
})
