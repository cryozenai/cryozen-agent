// Launching the Cryozen TUI in the user's OWN terminal emulator.
//
...
// `cryozen --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Cryozen runtime is often a venv Python invoked
//     as `python -m cryozen_cli.main`, not a `cryozen` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Cryozen
