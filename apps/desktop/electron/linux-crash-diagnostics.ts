...
// launcher (.desktop entry, Omarchy's cryozen-desktop wrapper) discards stderr.
...
export interface CrashDiagnosticsHost {
  /** Create the logs directory. May throw (read-only or invalid CRYOZEN_HOME). */
...
// a read-only or invalid CRYOZEN_HOME/logs must degrade to "no crash log", never
