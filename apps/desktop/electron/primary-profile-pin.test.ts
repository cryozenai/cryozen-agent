...
test('a live primary keeps answering for its booted profile after the preference moves', () => {
  ...
  // startCryozen() boots the primary as the preference at that moment.
  ...
  // cryozen:profile:remember rewrites active-profile.json without re-homing.
  preference = 'claude'
  ...
  // spawned for the same CRYOZEN_HOME.
  assert.equal(
    pin.resolve(() => preference),
    'default'
  )
