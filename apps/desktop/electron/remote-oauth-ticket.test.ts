...
describe('resolveRemoteOauthTicket', () => {
  it('reserves a larger bounded roster dial budget only for OAuth remote sources', () => {
    ...
    expect(rosterSourceEnumerationTimeoutMs({ kind: 'remote', authMode: 'oauth' })).toBeGreaterThan(defaultBudget)
    expect(rosterSourceEnumerationTimeoutMs({ kind: 'remote', authMode: 'token' })).toBe(defaultBudget)
