 ...
 *
 * `cryozenHome` must be the CRYOZEN_HOME root, not Electron's userData dir: the
 * backend admits `@file:` attachments outside the chat's cwd only from
 * `<CRYOZEN_HOME>/composer-pastes` (agent/context_references.py::_resolve_path),
 * and on Linux/macOS userData is a different tree (#117149).
 */
export async function writeComposerPaste(cryozenHome: string, text: string): Promise<string> {
  const dir = path.join(cryozenHome, COMPOSER_PASTES_DIRNAME)
