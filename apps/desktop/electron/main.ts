...
import { buildDesktopBackendEnv, cryozenManagedNodePathEntries, normalizeCryozenHomeRoot } from './backend-env'
import { createBackendExitRecoveryLatch } from './backend-exit-recovery'
import { isReauthRequiredError, waitForCryozenReady } from './backend-health'
import { backendCommandMatches, createBackendOwnership, createBackendShutdownCoordinator } from './backend-ownership'
import { canImportCryozenCli, PROBE_TIMEOUT_MS, shouldTrustCryozenOverride, verifyCryozenCli } from './backend-probes'
...
import { isCryozenOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvCryozenCommand
} from './windows-cryozen-path'
import {
...
const USER_DATA_OVERRIDE = process.env.CRYOZEN_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.CRYOZEN_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.CRYOZEN_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with CRYOZEN_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[cryozen] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[cryozen] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. CRYOZEN_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[cryozen] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[cryozen] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via CRYOZEN_DESKTOP_PASSWORD_STORE, bridged by the
// `cryozen desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[cryozen] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[cryozen] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `cryozen desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Cryozen sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[cryozen] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[cryozen] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[cryozen] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[cryozen] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[cryozen] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('cryozen:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Cryozen burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[cryozen] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[cryozen] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[cryozen] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[cryozen] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// CRYOZEN_HOME — the user-facing root for everything Cryozen-related. Mirrors
// scripts/install.ps1's $CryozenHome and scripts/install.sh's $CRYOZEN_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\cryozen (matches install.ps1)
//   macOS / Linux: ~/.cryozen-agent (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.cryozen-agent directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\cryozen yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// CRYOZEN_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// CRYOZEN_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.cryozen-agent / %LOCALAPPDATA%\cryozen.
function resolveCryozenHome() {
  if (process.env.CRYOZEN_HOME) {
    return normalizeCryozenHomeRoot(process.env.CRYOZEN_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'cryozen-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a CRYOZEN_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\cryozen and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('CRYOZEN_HOME')

    if (fromRegistry) {
      return normalizeCryozenHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'cryozen')
    const legacy = path.join(app.getPath('home'), '.cryozen-agent')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.cryozen-agent setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.cryozen-agent')
}

const CRYOZEN_HOME = resolveCryozenHome()
...
// the `cryozen desktop` launcher's argv, so a packaged app opened from its
...
// processes only from the browser's pre-launch command line.
{
  ...
  try {
    desktopLaunchYaml = fs.readFileSync(path.join(CRYOZEN_HOME, 'config.yaml'), 'utf8')
  } catch {
  ...
  for (const planned of planLaunchSwitches(readDesktopLaunchConfig(desktopLaunchYaml), process.argv.slice(1))) {
    ...
    console.log(
      `[cryozen] desktop launch switch from config.yaml: --${planned.name}${planned.value === undefined ? '' : `=${planned.value}`}`
    )
  }
}

function pathWithCryozenManagedNode(...entries) {
  const managed = cryozenManagedNodePathEntries(CRYOZEN_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_CRYOZEN_ROOT — the canonical mutable Cryozen install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_CRYOZEN_ROOT = path.join(CRYOZEN_HOME, 'cryozen-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_CRYOZEN_ROOT, 'venv')
...
// boots, going straight to `resolveCryozenBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_CRYOZEN_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_CRYOZEN_ROOT, '.cryozen-bootstrap-complete')
...
// active-profile.json records which Cryozen profile the desktop launches its
// local backend as. When set, startCryozen() passes `cryozen --profile <name>
// dashboard …`, which deterministically pins CRYOZEN_HOME (see
// _apply_profile_override in cryozen_cli/main.py) and bypasses the sticky
// ~/.cryozen-agent/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors cryozen_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = DESKTOP_PROFILE_NAME_RE
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// cryozenDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under CRYOZEN_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by cryozen_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(CRYOZEN_HOME, 'logs', 'desktop.log')
...
const BOOT_FAKE_MODE = process.env.CRYOZEN_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.CRYOZEN_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.CRYOZEN_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.CRYOZEN_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.CRYOZEN_DESKTOP_APP_NAME || 'Cryozen'
...
// 'cryozen:native-theme' ('dark' | 'light' | 'system'); we pin
...
// cryozen:window-control IPC channel. See titleBarOverlayOptions.
...
// electron-builder derives from the build `appId` (com.cryozen.cryozen) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.cryozen.cryozen')
}

// Seed the native About panel with the live Cryozen version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `cryozen update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveCryozenVersion(),
  copyright: 'Copyright © 2026 Cryozen'
})
...
// counts startCryozen() calls that have not settled; `primaryRecoverySuppressed`
// is set by every intentional invalidate of the slot and cleared by the next
// startCryozen(), so the dying child's stale exit never respawns behind a
...
// startCryozen(); this pool only holds EXTRA profile
...
// The legacy CRYOZEN_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.CRYOZEN_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.CRYOZEN_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  ...
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Cryozen backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `cryozen:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.CRYOZEN_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startCryozen() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning cryozen serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Cryozen backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  cryozenLog.push(...lines)

  if (cryozenLog.length > 300) {
    cryozenLog.splice(0, cryozenLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-cryozen-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('cryozen:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the cryozen:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('cryozen:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_CRYOZEN_ROOT
  })
}
...
// The Tauri updater writes CRYOZEN_HOME/.cryozen-update-in-progress for the whole
...
// (`force_kill_other_cryozen`, taskkill /IM cryozen.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(CRYOZEN_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--cryozen-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `cryozen update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Cryozen will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(CRYOZEN_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      const handoffLogPath = path.join(CRYOZEN_HOME, 'logs', 'desktop-update-handoff.log')
      ...
      // and the same reveal primitive as 'cryozen:logs:reveal'.
      void dialog
        .showMessageBox({
          type: 'error',
          title: 'Cryozen update',
          message: "Cryozen couldn't finish updating",
          detail:
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Cryozen to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `cryozen` — must not shadow `cryozen.cmd`/`cryozen.exe`.
...
async function unwrapWindowsVenvCryozenCommand(command, backendArgs) {
  return resolveVenvCryozenCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `cryozen serve`; runtimes older than serve only have `dashboard`. We
...
const backendSupportsServe = createBackendServeSupportResolver(CRYOZEN_HOME, rememberLog)
...
function isCryozenSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'cryozen_cli', 'main.py'))
}

async function findPythonForRoot(root) {
  const override = process.env.CRYOZEN_DESKTOP_PYTHON
...
async function findSystemPython() {
  ...
  // CPython but several Cryozen deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Resolves CRYOZEN_GIT_BASH_PATH
...
// normal CRYOZEN_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\cryozen\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'cryozen', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'cryozen', 'git', 'bin', 'git.exe'))
  }
...
function recentCryozenLog() {
  return cryozenLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's cryozen root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_CRYOZEN_ROOT.
// CRYOZEN_DESKTOP_CRYOZEN_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.CRYOZEN_DESKTOP_CRYOZEN_ROOT && path.resolve(process.env.CRYOZEN_DESKTOP_CRYOZEN_ROOT),
    !IS_PACKAGED && isCryozenSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isCryozenSourceRoot(ACTIVE_CRYOZEN_ROOT) ? ACTIVE_CRYOZEN_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_CRYOZEN_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('cryozen:updates:progress', merged)
  }
}
...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  if (!directoryExists(gitDir)) {
    return {
      ...
      message:
        "This copy of Cryozen can't update itself from inside the app. Download the latest version from the Cryozen website, " +
        `or reinstall Cryozen to enable in-app updates. Details: ${updateRoot} has no version-control metadata.`,
      cryozenRoot: updateRoot,
      branch
    }
  }
        ...
        headers: githubApiHeaders(
          {
            ...
            'User-Agent': 'cryozen-desktop-update-check'
          },
          token
        ),
        timeout: 10_000
...
// Windows that binary owns ALL repo mutation — running `cryozen update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(CRYOZEN_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `cryozen update` can write
// fresh entry points. On Windows this is the file the running backend
// `cryozen.exe` holds open; on POSIX it's never mandatory-locked.
function venvCryozenShimPath(updateRoot) {
  const venvDir = resolveVenvDir(updateRoot)

  return IS_WINDOWS ? path.join(venvDir, 'Scripts', 'cryozen.exe') : path.join(venvDir, 'bin', 'cryozen')
}
...
// Kill only Cryozen-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `cryozen`,
...
function killCryozenOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isCryozenOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Cryozen-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `cryozen.exe` that
// spawned its own grandchildren (a `cryozen` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Cryozen backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Cryozen backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.cryozenBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Cryozen backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.cryozenBackendIdentity
...
// `cryozenProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `cryozen.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a cryozen
// REPL, a pty terminal, the gateway) keep `cryozen.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const cryozenProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\cryozen.exe, but the
  // backend is `python.exe -m cryozen_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (cryozenProcess && Number.isInteger(cryozenProcess.pid)) {
    initialPids.push(cryozenProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(cryozenProcess, {
  ...
  // `cryozen gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvCryozenShimPath(updateRoot), CRYOZEN_HOME)

  // Reap Cryozen-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killCryozenOwnedVenvDaemons(updateRoot)

  const shim = venvCryozenShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentCryozenProcess = backendConnectionState.getProcess()

        if (currentCryozenProcess && Number.isInteger(currentCryozenProcess.pid)) {
          stragglers.push(currentCryozenProcess.pid)
        }
...
// `cryozen update`). Instead we spawn the staged Cryozen-Setup binary with
// --update and quit, so it can run `cryozen update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `cryozen update` → relaunch),
      ...
      // in: the CRYOZEN_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `cryozen update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `cryozen desktop`, never the Tauri installer that self-copies
      // cryozen-setup.exe into CRYOZEN_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `cryozen` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `cryozen update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `cryozen update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, cryozenRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(CRYOZEN_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Cryozen — this window will close and the updater will open. Don’t reopen Cryozen yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    await preflightStateDb(CRYOZEN_HOME, rememberLog)
    ...
    // cryozen.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running cryozen, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Cryozen install open ' +
        '(a second Cryozen window or a terminal running cryozen?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvCryozenShimPath(updateRoot))
      }
    ...
    // Cryozen processes running from this venv.  The updater normally refuses
    ...
    // `cryozen update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `cryozen update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          CRYOZEN_HOME,
          CRYOZEN_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithCryozenManagedNode(venvBin)
        },
      ...
      // The `cryozen update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(CRYOZEN_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Cryozen update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(CRYOZEN_HOME, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(resolveVenvDir(updateRoot), IS_WINDOWS ? 'Scripts' : 'bin')
  const venvCryozen = path.join(venvBin, IS_WINDOWS ? 'cryozen.exe' : 'cryozen')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Cryozen launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the cryozen
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.cryozen-bootstrap-complete')),
      hasVenvCryozen: fileExists(venvCryozen),
...
// Python-level pre-update snapshot inside `cryozen update`.
async function preflightStateDb(cryozenHome, rememberLog) {
  const stateDbPath = path.join(cryozenHome, 'state.db')
    ...
    if (stat.size > 100) {
      ...
      if (
        !(await readPreUpdateBackupEnabled(
          resolveCryozenBackend(['config', 'get', 'updates.pre_update_backup', '--json']),
          cryozenHome
        ))
      ) {
      ...
      const emergencyPath = path.join(cryozenHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(cryozenHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(cryozenHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `cryozen update`, swaps/relaunches the app bundle, and writes
// .cryozen-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'cryozen update', percent: null })

    return { ok: true, manual: true, command: 'cryozen update', cryozenRoot: updateRoot }
  }
  ...
  // A remote-served Desktop owns no local messaging gateway: `cryozen update
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      CRYOZEN_HOME,
      CRYOZEN_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithCryozenManagedNode(path.join(resolveVenvDir(updateRoot), 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Cryozen — this window will close. Don’t reopen Cryozen yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_CRYOZEN_ROOT actually
...
async function isActiveRuntimeUsable() {
  ...
  return (
    isCryozenSourceRoot(ACTIVE_CRYOZEN_ROOT) &&
    ...
    (await canImportCryozenCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_CRYOZEN_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    }))
  )
}
...
async function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `cryozen
...
function resolveWebDist() {
  const override = process.env.CRYOZEN_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set CRYOZEN_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndexWithMissing(): { index: string; missing: string[] } {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: cryozen desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: cryozen desktop --force-build`
  )
...
function resolveCryozenCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Cryozen.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.CRYOZEN_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveCryozenCwd(), sanitized: Boolean(trimmed) }
  }
...
async function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'cryozen_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_CRYOZEN_ROOT, the
...
async function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Cryozen at ${ACTIVE_CRYOZEN_ROOT}`,
    command,
    args: ['-m', 'cryozen_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      cryozenHome: CRYOZEN_HOME,
      pythonPathEntries: [ACTIVE_CRYOZEN_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_CRYOZEN_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
async function resolveCryozenBackend(backendArgs) {
  ...
  //    installed `cryozen` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isCryozenSourceRoot.)
  if (!IS_PACKAGED && isCryozenSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = await createPythonBackend(SOURCE_REPO_ROOT, `Cryozen source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_CRYOZEN_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\cryozen\\cryozen-agent (Windows) or ~/.cryozen-agent/cryozen-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Cryozen runtime at ${ACTIVE_CRYOZEN_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `cryozen` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    CRYOZEN_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.CRYOZEN_DESKTOP_IGNORE_EXISTING !== '1') {
    let cryozenCommand = null
    const cryozenOverride = process.env.CRYOZEN_DESKTOP_CRYOZEN

    if (cryozenOverride) {
      const resolvedOverride = findOnPath(cryozenOverride)

      if (resolvedOverride) {
        cryozenCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(cryozenOverride, { isWsl: IS_WSL })) {
        cryozenCommand = cryozenOverride
      } else {
        rememberLog(`Ignoring Windows Cryozen override under WSL: ${cryozenOverride}`)
      }
    } else {
      cryozenCommand = findOnPath('cryozen')
    }

    if (cryozenCommand) {
      if (looksLikeDesktopAppBinary(cryozenCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Cryozen CLI: ${cryozenCommand}`)
        cryozenCommand = null
      }
    }

    if (cryozenCommand) {
      const unwrapped = await unwrapWindowsVenvCryozenCommand(cryozenCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `cryozen` shim
      ...
      const shellForProbe = isCommandScript(cryozenCommand)

      // CRYOZEN_DESKTOP_CRYOZEN is an explicit deployment override (used by
      ...
      if (
        shouldTrustCryozenOverride(cryozenOverride) ||
        (await verifyCryozenCli(cryozenCommand, { shell: shellForProbe }))
      ) {
        ...
        // unwrapWindowsVenvCryozenCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Cryozen CLI at ${cryozenCommand}`,
          command: cryozenCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Cryozen CLI at ${cryozenCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed cryozen_cli module via system Python.
  ...
  if (python) {
    ...
    // having cryozen_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\cryozen\cryozen-agent\venv.
    if (await canImportCryozenCli(python)) {
      return {
        kind: 'python',
        label: `installed cryozen_cli module via ${python}`,
        command: python,
        args: ['-m', 'cryozen_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: cryozen_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveCryozenBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Cryozen Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_CRYOZEN_ROOT,
...
async function runEnsureRuntime(backend: any, assertStillOwned: () => void): Promise<any> {
  ...
  // backend.kind === 'bootstrap-needed' means resolveCryozenBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Cryozen install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Cryozen recovery was handed off to Cryozen Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      cryozenHome: CRYOZEN_HOME,
      logRoot: path.join(CRYOZEN_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Cryozen install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      ...
      // Latch the failure so subsequent startCryozen() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // cryozen:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(await resolveCryozenBackend(backend.args), assertStillOwned)
  }
  ...
  // %LOCALAPPDATA%\cryozen\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `cryozen` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "Cryozen needs a helper called Git for Windows, which isn't installed. " +
        'Choose Repair install to add it automatically, or install it yourself from git-scm.com and reopen Cryozen.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isCryozenSourceRoot()
    // plus an importable cryozen_cli before it hands back the active runtime.
  ...
  backend.label = `Cryozen at ${ACTIVE_CRYOZEN_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Cryozen runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----cryozen-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Cryozen backend URL protocol: ${parsed.protocol}`))
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Cryozen backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Cryozen-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Cryozen-Session-Token`` header — used by the auth-mode probe before
...
const URL_EFFECTIVE_MARK = 'cryozen-url-effective:'
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('cryozen:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveCryozenCwd()
  ...
  // (e.g. "AppData/Local/cryozen/attachments/foo.xlsx" on Windows, or
  // ".cryozen-agent/attachments/foo.xlsx" elsewhere) rather than relative to the
  ...
  if (!fileExists(resolved) && !directoryExists(resolved)) {
    for (const candidate of homeRelativeAttachmentCandidates(raw, app.getPath('home'), CRYOZEN_HOME)) {
      if (fileExists(candidate)) {
        resolved = candidate
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('cryozen:preview-file-changed', payload)
}
...
// waitForCryozenReady how to read a 401 — rejected session vs gated route.
...
async function waitForCryozen(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForCryozenReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('cryozen:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('cryozen:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('cryozen:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('cryozen:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Cryozen...".
function sendPowerResume() {
  ...
  webContents.send('cryozen:power-resume')
}
...
// 'cryozen:power-battery' push below.
ipcMain.handle('cryozen:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('cryozen:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(CRYOZEN_HOME, app.getPath('userData'))
  } catch (err) {
  ...
  try {
    ...
    // 'Open Plugins' rides the existing deep-link channel (cryozen://open/…),
    ...
    if (response === 0) {
      handleDeepLink(`${CRYOZEN_PROTOCOL}://open/capabilities?tab=plugins`)
    }
  } finally {
    try {
...
function sendOpenUpdatesRequested() {
  ...
  // before the window is up) queue the request; 'cryozen:deep-link-ready' flushes it.
  ...
  webContents.send('cryozen:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('cryozen:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('cryozen:context-menu-spellcheck', {
...
//
// Hosted Cryozen gateways gate the dashboard behind an OAuth provider (e.g.
// Cryozen) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``cryozen_session_at``),
...
//   * The gateway issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token. Both are set as HttpOnly cookies (``cryozen_session_at`` ~15 min,
//     ``cryozen_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// jar above). The primary / v1 remote flows keep the legacy
...
// "not signed in", which on the initial boot path (startCryozen → the renderer's
// single-shot boot() with no retry) surfaced as the "Cryozen couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Cryozen
...
// cookie jar rather than try to read the HttpOnly value. The window loads the
// public ``/login`` interstitial that renders the provider chooser.
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Cryozen gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Cryozen backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in cryozen_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every cryozen:connections:* call.
 ...
 * editor doesn't carry — ssh `remoteCryozenPath`/`remoteProfile` —
...
async function saveRegistryConnection(input: any = {}) {
  ...
  // entries authenticate via cookies/native tokens instead.
  ...
  onDefaultChanged: route => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.webContents.isDestroyed()) {
        win.webContents.send('cryozen:profile:default:changed', route)
      }
    }
  }
})
...
// contains "cryozen", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isCryozenProcess(pid) {
  try {
  ...
  try {
    ...
    return cmdline.includes('cryozen')
  } catch {
...
//   1. Legacy ~/.cryozen-agent/active_profile (explicit CLI choice via cryozen profile use)
//   2. Running gateway (gateway.pid with verified liveness + cryozen identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(CRYOZEN_HOME, 'active_profile'),
    cryozenHome: CRYOZEN_HOME,
    profilesRoot: path.join(CRYOZEN_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.CRYOZEN_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.CRYOZEN_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    sshRemoteCryozenPath: (ssh || savedSsh)?.remoteCryozenPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by CRYOZEN_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // 'remote' persists a remote-shaped block; anything else collapses to local.
  ...
  // Leaving SSH for remote starts from an empty block so SSH fields never leak
  // into a URL connection. (remote↔local toggles preserve a real remote URL.)
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteCryozenPath: input.sshRemoteCryozenPath ?? existingBlock.remoteCryozenPath,
...
async function buildRemoteConnection(
  ...
  if (!token) {
    throw new Error(
      'Remote Cryozen gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.CRYOZEN_DESKTOP_REMOTE_TOKEN,
      url: process.env.CRYOZEN_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
 ...
 * backend (the address is already true), a non-loopback host, or a url
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      cryozenPath: result.cryozenPath,
      cryozenHome: result.cryozenHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { cryozenPath: result.cryozenPath, cryozenHome: result.cryozenHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteCryozenPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      platform,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteCryozenPath: sshConfig.remoteCryozenPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForCryozen: (baseUrl, token) => waitForCryozen(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        cryozenVersion: result.cryozenVersion || '',
        ...
        // per-profile pools also use bare keys. Only startCryozen' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.cryozenVersion || 'cryozen (version unknown)'} at ${result.cryozenPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteCryozenVersion: result.cryozenVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (CRYOZEN_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
// Remote, or the env override): a SINGLE remote backend serves every
// profile via ?profile=.
// Distinct from per-profile overrides — here there's one host for all.
function globalRemoteActive() {
  if (process.env.CRYOZEN_DESKTOP_REMOTE_URL) {
    return true
  }
...
// True when the PRIMARY profile's backend resolves to a remote host —
...
// env → global) so a startCryozen() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Cryozen
  // gateway and reports:
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Cryozen" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteCryozenPath: input.sshRemoteCryozenPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteCryozenPath || '')
          let cryozenPath
          let cryozenVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            cryozenPath = runtime.cryozenPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.cryozenPath])
            cryozenVersion = inspection.version
            supported = inspection.supported
          } else {
            cryozenPath = await remoteLifecycle.locateCryozen(ssh, sshConfig.remoteCryozenPath || '')
            cryozenVersion = await remoteLifecycle.probeCryozenVersion(ssh, cryozenPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, cryozenPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Cryozen on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteCryozenPath: cryozenPath,
            remoteCryozenVersion: cryozenVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startCryozen())
  ...
  // connect to Cryozen gateway". Mirror the renderer's connect here so the test
...
function resetCryozenConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  // The next startCryozen() re-reads active-profile.json for its launch profile.
  primaryProfilePin.clear()
  const cryozenProcess = invalidatePrimaryConnection()
  stopBackendChild(cryozenProcess)
...
// startCryozen() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetCryozenConnection() invalidates it.
  const cryozenProcess = backendConnectionState.getProcess()
  const dying = cryozenProcess && !cryozenProcess.killed ? cryozenProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('cryozen:connection:applied')
}
...
// scoped to that connection. Without this, a removed remote source keeps
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('cryozen:connections:changed', payload)
    }
  }
}
...
// startCryozen() and cleared when the primary is torn down; while a primary is
...
async function ensureBackend(
  ...
  if (route.backend === 'primary') {
    const connection = await startCryozen()
  ...
  // spawning, so two children never share one profile's CRYOZEN_HOME.
...
async function ensureRegistryBackend(
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteCryozenPath: source.remoteCryozenPath,
  ...
  // backend beyond the SSH-fingerprint path above (url remotes have no
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. cryozen:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: cryozenLog.slice(-80),
      ...getWindowState()
    }
  }

  // remote: one gateway host serves every profile of that source,
  ...
  await waitForCryozen(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl

  // Remote backends live on another host too — disable the WSL path
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.cryozenPath || !state?.cryozenHome) {
  ...
  return {
    ...
    cryozenPath: state.cryozenPath,
    cryozenHome: state.cryozenHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteCryozenPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.cryozenPath ? platform : await probeWindowsRemote(ssh, config.remoteCryozenPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          cryozenPath: runtime.cryozenPath,
          cryozenHome: runtime.cryozenHome,
          pythonPath: runtime.python
        }
      }
    }

    const cryozenPath = await remoteLifecycle.locateCryozen(ssh, config.remoteCryozenPath || '')
    const cryozenHome = await remoteLifecycle.probeRemoteCryozenHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, cryozenPath, cryozenHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { cryozenPath: state.cryozenPath, cryozenHome: state.cryozenHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForCryozen(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// descriptor entries (remote registry sources, per-profile remote
...
// local-spawn portion of startCryozen() but without the boot-progress UI,
...
async function runPoolBackendStart(
  ...
  if (remote) {
    await waitForCryozen(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // Everything below starts a LOCAL `cryozen serve` child. Multiplex-only says
  ...
  // --profile wins over the inherited CRYOZEN_HOME env (see _apply_profile_override
  // step 3 in cryozen_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']

  const backend = await ensureRuntime(await resolveCryozenBackend(backendArgs), () =>
    assertPoolEntryStillOwned(poolKey, entry)
  )
  ...
  assertPoolEntryStillOwned(poolKey, entry)
  const cryozenCwd = resolveCryozenCwd()
  ...
  rememberLog(`Starting Cryozen backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawnOwnedBackend(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: cryozenCwd,
        CRYOZEN_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        CRYOZEN_WEB_DIST: webDist,
        ...(readyFile ? { CRYOZEN_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Cryozen backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Cryozen backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(formatBackendExitLine(`Cryozen backend for profile "${profile}" exited`, code, signal, outputTail))
    releaseBackendChild(child)

    if (!ready) {
      rejectStart?.(
        new Error(
          `Cryozen backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForCryozen(baseUrl, token), startFailed])
  assertPoolEntryStillOwned(poolKey, entry, { releaseSlot: false })
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Cryozen backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Cryozen backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
function broadcastPoolBackendRetiring(poolKey: string) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('cryozen:pool:retiring', { poolKey })
    }
  }
}
...
// backend whose ensure_cryozen_home() recreates the deleted profile directory.
//
...
const ISOLATED_BACKEND = process.env.CRYOZEN_DESKTOP_ISOLATED_BACKEND === '1'
...
function startAttachedBackendMonitor(attached: AttachedBackend) {
  ...
  attachedBackendMonitor = setInterval(() => {
    void waitForCryozen(attached.baseUrl, attached.token, undefined, 'token', {}).catch(() => {
      stopAttachedBackendMonitor()
      rememberLog(`[attach] attached backend on ${attached.baseUrl} (pid ${attached.pid}) is gone; recovering`)
      invalidatePrimaryConnection()
      scheduleUnexpectedPrimaryRecovery({ error: 'The Cryozen backend this app attached to exited.', ready: true })
    })
...
function attachToRunningHostBackend(): Promise<AttachedBackend | null> {
  const options = { isolated: ISOLATED_BACKEND, ledgerPath: spawnLedgerPath(CRYOZEN_HOME, path.join) }
...
function hostBackendAttachDeps() {
  return {
    ...
    waitForReady: (baseUrl: string, token: string) => waitForCryozen(baseUrl, token, undefined, 'token', {})
  }
}

function hostSpawnGatePath() {
  return path.join(CRYOZEN_HOME, 'desktop-backend-spawn.json')
}
...
function startCryozen({ supervisorRecovery = false }: { supervisorRecovery?: boolean } = {}) {
  primaryRecoverySuppressed = false
  primaryStartsInFlight += 1

  const start = localBackendLifecycle.start(() => runCryozenStart({ supervisorRecovery }))
...
function reportPrimaryRecoveryCrashLoop(code: number | null, signal: string | null): boolean {
  ...
  const message =
    'Cryozen backend keeps crashing right after it restarts; not restarting it again. Relaunch Cryozen Desktop.'
...
function runPrimaryRecoverySpawn(code: number | null, signal: string | null) {
  startCryozen({ supervisorRecovery: true }).catch(respawnError => {
    ...
    // releaseStart (startCryozen) already ran: same-promise reaction order, so
    // hasPendingStart is false here. See the ordering contract in startCryozen.
 ...
 * retry, so both the per-request short-circuit in runCryozenStart and the
...
async function runCryozenStart({ supervisorRecovery = false }: { supervisorRecovery?: boolean } = {}) {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Cryozen Desktop is already running in another window.')
  }
  ...
  // process, every subsequent startCryozen() call re-throws the same error
  ...
  // latched (ensureBackend -> startCryozen), so a log line here would flood the
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Cryozen backend', 8)
  ...
  // later cryozen:profile:remember must not retarget requests mid-life.
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote Cryozen backend at ${remote.baseUrl}`, 24)
      await waitForCryozen(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Cryozen backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, cryozenLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.cryozen-agent/active_profile file) and
    // resolves CRYOZEN_HOME the same way `cryozen -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Cryozen runtime', 28)

        return resolveCryozenBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    if (setup.kind === 'attached') {
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Attached to the running Cryozen backend',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.spawn', `Starting Cryozen backend via ${backend.label}`, 84)
    rememberLog(`Starting Cryozen backend via ${backend.label}`)
    ...
    const cryozenProcess = spawnOwnedBackend(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: cryozenCwd,
        env: {
          ...process.env,
          // Explicitly pin CRYOZEN_HOME for the child so Python's get_cryozen_home()
          // resolves to the SAME location our resolveCryozenHome() picked. Without
          // this pin, Python falls back to ~/.cryozen-agent on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\cryozen, which differs from C:\Users\<u>\.cryozen-agent.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets CRYOZEN_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(cryozenProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(cryozenProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, cryozenProcess)

    if (!processOwner) {
      stopBackendChild(cryozenProcess)
      await waitForBackendExit(cryozenProcess)
      releaseBackendChild(cryozenProcess)
      throw new Error('Cryozen backend start was superseded by a newer connection attempt.')
    }

    cryozenProcess.stdout.on('data', rememberLog)
    cryozenProcess.stderr.on('data', rememberLog)
    ...
    cryozenProcess.once('error', error => {
      releaseBackendChild(cryozenProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Cryozen backend error: ${error.message}`)
        scheduleUnexpectedPrimaryRecovery({ error: error.message, ready: backendReady })
        rejectBackendStart?.(new Error('Cryozen backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Cryozen backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Cryozen backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    cryozenProcess.once('exit', (code, signal) => {
      releaseBackendChild(cryozenProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(formatBackendExitLine('Ignoring stale Cryozen backend exit', code, signal, primaryOutputTail))
      ...
      rememberLog(formatBackendExitLine('Cryozen backend exited', code, signal, primaryOutputTail))
      ...
      if (!backendReady) {
        const message = `Cryozen backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Cryozen backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentCryozenLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Cryozen backend to launch', 86)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Cryozen backend to become ready', 90)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    await Promise.race([waitForCryozen(baseUrl, token), backendStartFailed])
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    backendReady = true
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => cryozenProcess.exitCode === null && !cryozenProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Cryozen backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Cryozen backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed CRYOZEN_HOME/.plugin-compat-report.json.
    ...
    // getConnection()/startCryozen() caller arriving during the exit wait would
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `cryozen:openExternal`, not here. See window-open-policy.ts.
 ...
 * always carries the `persist:cryozen-preview` partition, so the partition is
 ...
 * audited `cryozen:openExternal` channel. Popup requests themselves stay
...
function installPreviewGuestPreload() {
  app.on('web-contents-created', (_event, contents) => {
    ...
    contents.on('will-attach-webview', (_attachEvent, webPreferences, params) => {
      if (params.partition !== 'persist:cryozen-preview') {
        return
      }
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('cryozen:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Cryozen is
...
// pushes pet state over IPC (cryozen:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via cryozen:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Cryozen app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Cryozen icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see cryozen:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Cryozen in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('cryozen:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Cryozen can be driven
...
// left the user with NO Cryozen window after the second toggle (#88513).
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('cryozen:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('cryozen:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[cryozen] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('cryozen:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `cryozen:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, cryozen:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('cryozen:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('cryozen:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('cryozen:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'cryozen desktop --force-build',
      ...
      // reclaim, an external kill). Cryozen never does this itself and never reloads it
      ...
      onRendererTerminated: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          title: 'Cryozen desktop UI was terminated',
          errorDescription:
  ...
  const startup = defaultRoute ? connectDesktopProfileRoute(defaultRoute) : startCryozen()
...
ipcMain.handle('cryozen:connection', async (event, profile, extra) => {
...
ipcMain.handle('cryozen:connection:for', async (_event, payload) => {
...
ipcMain.on('cryozen:connection:active-route', (event, route) => recordWindowConnectionRoute(event.sender, route))
...
// "Starting Cryozen…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('cryozen:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetCryozenConnection({ soft: true }),
...
ipcMain.handle('cryozen:backend:touch', async (_event, profile, options) => {
...
ipcMain.handle('cryozen:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('cryozen:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('cryozen:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('cryozen:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('cryozen:window:openInstance', async (event, options) => {
...
ipcMain.handle('cryozen:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('cryozen:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = await resolveCryozenBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Cryozen is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `cryozen-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, CRYOZEN_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('cryozen:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('cryozen:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('cryozen:zoom:get', event => {
...
ipcMain.on('cryozen:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('cryozen:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('cryozen:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startCryozen() call restarts the
...
ipcMain.handle('cryozen:bootstrap:repair', async () => {
  // Forceful repair: force the next startCryozen() through the full installer
  ...
  // "bypass active runtime" path inside startCryozen() and fall through
  ...
  resetCryozenConnection()

  return { ok: true }
})
ipcMain.handle('cryozen:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('cryozen:bootstrap:cancel', async () => {
...
ipcMain.handle('cryozen:boot-progress:get', async () => bootProgressState)
ipcMain.handle('cryozen:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('cryozen:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('cryozen:plugin-profile-routes', async (_event, rawProfileNames) => {
  ...
  // A local enumeration can fail while remote sources succeed. Preserve
...
ipcMain.handle('cryozen:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('cryozen:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('cryozen:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('cryozen:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('cryozen:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('cryozen:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('cryozen:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('cryozen:connections:remove', async (_event, id) => {
  const key = String(id || '')
  ...
  // removed connection keep their WebSocket open (remote have no local
...
ipcMain.handle('cryozen:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('cryozen:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('cryozen:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('cryozen:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteCryozenPath: entry.remoteCryozenPath
    })
  ...
  // Remote/local probe built DIRECTLY from the registry entry. Routing
  ...
  if (entry.kind === 'local') {
    const local = await startCryozen()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteCryozenPath: connection.remoteCryozenPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteCryozenProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root CRYOZEN_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('cryozen:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// cryozen:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('cryozen:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('cryozen:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `cryozen update` to every registered connection at once. Each
// dispatch reports independently so one dead LAN box can't wedge the batch. Local reuses the
...
ipcMain.handle('cryozen:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/cryozen/update', {}, { timeoutMs: 15_000 })
...
// analogue of the cryozen:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('cryozen:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('cryozen:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startCryozen() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('cryozen:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
ipcMain.handle('cryozen:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('cryozen:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('cryozen:profile:default:get', async () => desktopProfilePreferences.getDefault())
ipcMain.handle('cryozen:profile:default:set', async (_event, route) => desktopProfilePreferences.setDefault(route))
ipcMain.handle('cryozen:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of cryozen:profile:set: records the profile the
...
ipcMain.handle('cryozen:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('cryozen:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new CRYOZEN_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('cryozen:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('cryozen:requestMicrophoneAccess', async () => {
...
ipcMain.handle('cryozen:window:readBelow', async event => {
...
// backend per pooled (connection, profile) scope; remote sources are one
...
async function handleCryozenApiRequest(request) {
  ...
  // remote host serves every profile via ?profile=, so scope the path.
  ...
  // backend calls ensure_cryozen_home() which recreates the profile directory,
  ...
  // CRYOZEN_HOME is then the scope, so a destructive call can never fall
...
ipcMain.handle('cryozen:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its CRYOZEN_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleCryozenApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleCryozenApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('cryozen:ambient:claim', (_event, key) => ownsAmbientCue(String(key ?? '')))
...
ipcMain.handle('cryozen:data-url-read-max:get', () => ({
...
ipcMain.handle('cryozen:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('cryozen:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('cryozen:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('cryozen:readFileText', async (_event, filePath) => {
...
// `cryozen:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('cryozen:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('cryozen:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('cryozen:writeClipboard', (_event, text) => {
...
ipcMain.handle('cryozen:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('cryozen:readClipboard', () => clipboard.readText())

ipcMain.handle('cryozen:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('cryozen:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('cryozen:context-menu:edit', (event, command) => {
...
ipcMain.handle('cryozen:context-menu:copy-image', event => {
...
ipcMain.handle('cryozen:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('cryozen:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('cryozen:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('cryozen:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('cryozen:savePastedText', async (_event, payload) => {
  ...
  return writeComposerPaste(CRYOZEN_HOME, text)
})

ipcMain.handle('cryozen:saveClipboardImage', async () => {
...
ipcMain.handle('cryozen:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('cryozen:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('cryozen:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('cryozen:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('cryozen:active-work', (event, payload) => {
...
ipcMain.on('cryozen:titlebar-theme', (_event, payload) => {
...
ipcMain.on('cryozen:native-theme', (_event, mode) => {
...
ipcMain.on('cryozen:translucency:support', event => {
...
ipcMain.on('cryozen:keep-awake', (_event, on) => {
...
ipcMain.handle('cryozen:quick-entry:settings:get', async () => {
...
ipcMain.handle('cryozen:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('cryozen:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('cryozen:quick-entry:submit', {
...
ipcMain.on('cryozen:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('cryozen:quick-entry:state', payload)
  }
})

ipcMain.on('cryozen:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('cryozen:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('cryozen:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `cryozen:window:openSession`).
...
ipcMain.handle('cryozen:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('cryozen:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('cryozen:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('cryozen:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveCryozenCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('cryozen:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveCryozenCwd()
}))

ipcMain.handle('cryozen:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('cryozen:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('cryozen:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('cryozen:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('cryozen:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('cryozen:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('cryozen:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: cryozenLog.slice(-200) }))
...
ipcMain.on('cryozen:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (cryozen:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('cryozen:updates:check', async (_event, opts) =>
...
ipcMain.handle('cryozen:updates:apply', async (_event, payload) =>
...
ipcMain.handle('cryozen:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('cryozen:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Cryozen version (the one `release.py` bumps in
// cryozen_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Cryozen version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveCryozenVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'cryozen_cli', '__init__.py')
...
// Renderer-bundle skew: `cryozen update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Cryozen version and push it into the native About panel
// just before showing it, so an in-place `cryozen update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveCryozenVersion()} — app build out of date, update the desktop app`
        : resolveCryozenVersion(),
      copyright: 'Copyright © 2026 Cryozen'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('cryozen:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveCryozenVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    cryozenRoot: resolveUpdateRoot(),
...
// The About page's "Restart Cryozen" button (shown when bundleSwapPending):
...
ipcMain.handle('cryozen:app:relaunch', async () => {
...
ipcMain.handle('cryozen:machine:profile', async () => {
...
// the actual removal via `cryozen uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (cryozen_cli/uninstall.py + cryozen_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_CRYOZEN_ROOT
  ...
  const fallback = () => ({
    cryozen_home: CRYOZEN_HOME,
    agent_installed: isCryozenSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'cryozen_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, CRYOZEN_HOME, NO_COLOR: '1' },
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Cryozen agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import cryozen_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_CRYOZEN_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_CRYOZEN_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `cryozen-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `cryozen-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('cryozen:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('cryozen:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('cryozen:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('cryozen:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// cryozen:// deep links (e.g. cryozen://blueprint/morning-brief?time=08:00,
// cryozen://mcp/install?name=NAME&config=B64 — the vendor "Add to Cryozen"
// button, or cryozen://plugin/install?repo=owner/repo). Dev
// (`CRYOZEN_DESKTOP_DEV_SERVER`) registers cryozen-dev:// instead — bare
// Electron or a stale OS handler often owns cryozen:// on dev machines.
...
// ---------------------------------------------------------------------------
const CRYOZEN_PROTOCOL = DEV_SERVER ? 'cryozen-dev' : 'cryozen'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['cryozen-dev', 'cryozen'] : ['cryozen']
...
function handleDeepLink(url) {
  ...
  // cryozen://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('cryozen:deep-link', payload)
...
ipcMain.handle('cryozen:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${CRYOZEN_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(CRYOZEN_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(CRYOZEN_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${CRYOZEN_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `cryozen://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startCryozen), whose
...
app.whenReady().then(() => {
  ...
  // startCryozen() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching cryozen:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Cryozen never keeps another app's chord hostage.
