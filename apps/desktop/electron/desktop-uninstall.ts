 ...
 * the three user-facing uninstall modes to the `cryozen uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m cryozen_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `cryozen_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'cryozen_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Cryozen.app/Contents/MacOS/Cryozen  → …/Cryozen.app
 *   Windows: …\Cryozen\Cryozen.exe                 → …\Cryozen  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Cryozen.app/Contents/MacOS/Cryozen → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Cryozen.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Cryozen.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Cryozen$/i.test(dir) || /[\\/]cryozen-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/cryozen
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import cryozen_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, cryozenHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export CRYOZEN_HOME=${q(cryozenHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import cryozen_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Cryozen install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "CRYOZEN_HOME=${String(cryozenHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
