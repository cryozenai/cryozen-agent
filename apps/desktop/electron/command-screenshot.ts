...
export function installCommandScreenshot({ rendererUrl }: { rendererUrl: string }): () => void {
  ...
  const publish = () => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send('cryozen:screenshot:status', status())
      }
    }
  }
  ...
  const start = (requestPermission = false) => {
    ...
    monitor.start(
      window => {
        ...
        // Retain the last Cryozen chat window when another application takes focus.
        ...
        if (requestId) {
          recipient.webContents.send('cryozen:screenshot:request', requestId)
        }
      },
      result => {
  ...
  const handle = (name: string, callback: (event: IpcMainInvokeEvent, value: unknown) => unknown) => {
    const channel = `cryozen:screenshot:${name}`
  ...
  ipcMain.on('cryozen:screenshot:subscribe', onSubscribe)
  ...
  const dispose = () => {
    ...
    ipcMain.removeListener('cryozen:screenshot:subscribe', onSubscribe)
