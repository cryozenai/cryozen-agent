...
test('compare payload maps to the behind count and a newest-first commit list; malformed = null', () => {
  ...
  assert.equal(githubRepoSlug('git@github.com:Someone/cryozen-agent.git'), 'someone/cryozen-agent')
  assert.equal(githubRepoSlug('https://gitlab.example/x/y.git'), null)
  assert.equal(
    branchTipApiUrl('cryozenai/cryozen-agent', 'bb/gui'),
    'https://api.github.com/repos/cryozenai/cryozen-agent/commits/bb%2Fgui'
  )
})
