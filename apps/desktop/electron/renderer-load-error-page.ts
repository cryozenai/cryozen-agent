 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `cryozen update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `cryozen desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = escapeHtml(details.title || 'Cryozen couldn\u2019t start the desktop UI')
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>cryozen desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>cryozen desktop --force-build</code>, then restart the app.</p>
