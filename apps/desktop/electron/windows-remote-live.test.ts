...
//   CRYOZEN_WIN_SSH_HOST   ssh alias/host of the Windows box
//   CRYOZEN_WIN_SSH_USER   remote user
//   CRYOZEN_WIN_SSH_CRYOZEN absolute path to the remote cryozen.exe under test
const liveHost = process.env.CRYOZEN_WIN_SSH_HOST || ''
const liveUser = process.env.CRYOZEN_WIN_SSH_USER || ''
const configuredCryozen = process.env.CRYOZEN_WIN_SSH_CRYOZEN || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Cryozen-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredCryozen)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredCryozen.replace('cryozen.exe', 'python.exe')}' -m cryozen_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredCryozen.replace('cryozen.exe', 'python.exe')
        const terminate = `& '${python}' -m cryozen_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.cryozenPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m cryozen_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m cryozen_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
