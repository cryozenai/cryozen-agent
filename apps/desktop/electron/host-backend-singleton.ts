// Multiplex-only, Desktop half: ONE `cryozen serve` per HOST serves every
...
// other producer of `cryozen serve` children: the per-profile backend pool.
...
//   * `CRYOZEN_DESKTOP_ISOLATED_BACKEND=1` — the escape hatch that gives this
//     app a private backend instead of the host's.
//   * Remote / SSH backends — a DIFFERENT host. Its ownership proof
...
export interface HostBackendCollapseOptions {
  /** `CRYOZEN_DESKTOP_ISOLATED_BACKEND=1`: this app wants its own backend. */
  isolated?: boolean
   ...
   * own `CRYOZEN_HOME` is then the only scope there is, so the collapse must
...
export class SecondLocalBackendError extends Error {
  ...
  constructor(poolKey: string) {
    super(
      `Refusing to start a second local Cryozen backend for "${poolKey}": one backend serves every profile on this host. ` +
        'Set CRYOZEN_DESKTOP_ISOLATED_BACKEND=1 for a private backend.'
    )
 ...
 * the one place a local `cryozen serve` child is actually started, so a future
