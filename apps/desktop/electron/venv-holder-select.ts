 ...
 *
 * The pre-update handoff kills Cryozen-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `cryozen`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Cryozen-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Cryozen and holds venv shims mapped).
 */
export function isCryozenOwnedVenvDaemon(
