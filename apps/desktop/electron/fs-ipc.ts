...
// hardening, CRYOZEN_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('cryozen:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('cryozen:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))
  ...
  ipcMain.handle('cryozen:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('cryozen:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<CRYOZEN_HOME>/desktop-plugins`,
  // resolved from the main-process CRYOZEN_HOME (see resolveCryozenHome) — NOT from
  // the connected backend. A remote backend reports its own `cryozen_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(cryozenHome, 'profiles', profile) : cryozenHome
  ...
  async function desktopPluginsRoot(): Promise<string> {
    const root = await ensureDir(path.join(cryozenHome, DESKTOP_PLUGINS_DIR))
    await migrateProfileScopedDesktopPlugins(cryozenHome, root)
    await reconcileUnifiedDesktopHalves(cryozenHome, root)

    return root
  }

  ipcMain.handle('cryozen:fs:desktopPluginsRoot', async () => desktopPluginsRoot())
  ...
  ipcMain.handle('cryozen:fs:reconcileDesktopPlugins', async () => {
    const root = await ensureDir(path.join(cryozenHome, DESKTOP_PLUGINS_DIR))

    return reconcileUnifiedDesktopHalves(cryozenHome, root)
  })

  // The LOCAL logs root (`<CRYOZEN_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where CRYOZEN_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('cryozen:fs:logsRoot', async () => localPluginsRoot('logs'))

  ipcMain.handle('cryozen:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('cryozen:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('cryozen:plugin:removeDesktop', async (_event, payload) =>
    removeDesktopPlugin(path.join(cryozenHome, DESKTOP_PLUGINS_DIR), payload?.name)
  )

  // Rename a file/folder in place. The renderer passes the existing path + a new
  // base name; the destination is resolved in the SAME parent dir so a rename can
  // never move the item elsewhere or traverse out. Rejects on a name collision.
  ipcMain.handle('cryozen:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('cryozen:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('cryozen:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
