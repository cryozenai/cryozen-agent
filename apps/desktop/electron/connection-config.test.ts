...
test('normalizeRemoteHeaders keeps safe proxy headers and drops transport/auth headers', () => {
  assert.deepEqual(
    normalizeRemoteHeaders({
      ...
      'X-Cryozen-Session-Token': { encoding: 'plain', value: 'token' },
...
test('remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin', () => {
  assert.equal(
    remoteRequestMatchesBaseUrl(
      'wss://cryozen.example.com/gateway/api/ws?ticket=abc',
      'https://cryozen.example.com/gateway'
    ),
    true
  )
  assert.equal(remoteRequestMatchesBaseUrl('ws://cryozen.example.com/api/ws', 'http://cryozen.example.com'), true)
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://cryozen.example.com/other/api/ws', 'https://cryozen.example.com/gateway'),
    false
  )
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://other.example.com/gateway/api/ws', 'https://cryozen.example.com/gateway'),
    false
  )
})
...
test('modeIsRemoteLike is true for remote only', () => {
...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/cryozen  ', token: { value: 'sek' } }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/cryozen',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('localProfileEntry preserves inactive SSH drafts but drops remote state', () => {
  const ssh = { mode: 'ssh', host: 'box', user: 'alice', remoteCryozenPath: '/cryozen' }
...
const ROUTES = [
  ...
  },
  {
    ...
    // profile-scope has only the backend PROCESS's CRYOZEN_HOME left as a
  ...
  {
    name: 'CRYOZEN_DESKTOP_ISOLATED_BACKEND keeps a local profile on its own pooled backend',
    profile: 'coder',
    opts: {
      primaryProfile: 'default',
      globalRemote: false,
...
// --- translateSelfProfileQuery (registry SSH-scoped cryozen:api contract) ---
...
test('resolveProfileApiRequest keeps an unscopable mutating route on a process-scoped backend', () => {
  ...
  // nothing left but the backend process's own CRYOZEN_HOME, so it must NOT fall
...
test('resolveProfileApiRequest routes action-status polls with the action-spawning routes', () => {
  ...
  // install): _spawn_cryozen_action registers the dynamic action name only in
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/cryozen/'), 'https://gw.example.com/cryozen')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/cryozen?x=1#frag'), 'https://gw.example.com/cryozen')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/cryozen'), 'https://host/cryozen')
})
...
test('normalizeRemoteBaseUrl auto-prepends http:// for scheme-less host:port input', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('gw.example.com/cryozen/'), 'http://gw.example.com/cryozen')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/cryozen', 't'), 'wss://host/cryozen/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/cryozen', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/cryozen/api/ws?ticket=tkt-9')
...
test('authModeFromStatus returns oauth when auth_required is true', () => {
  assert.equal(authModeFromStatus({ auth_required: true, auth_providers: ['github'] }), 'oauth')
})
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'cryozen_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-cryozen_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-cryozen_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'cryozen_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'cryozen_session_rt', value: 'x' }]), false)
...
test('AT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(AT_COOKIE_VARIANTS, [
    '__Host-cryozen_session_at',
    '__Secure-cryozen_session_at',
    'cryozen_session_at'
  ])
})
...
test('RT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(RT_COOKIE_VARIANTS, [
    '__Host-cryozen_session_rt',
    '__Secure-cryozen_session_rt',
    'cryozen_session_rt'
  ])
})
...
test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'cryozen_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-cryozen_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-cryozen_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'cryozen_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-cryozen_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-cryozen_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'cryozen_session_at', value: 'a' },
      { name: 'cryozen_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'cryozen_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'cryozen_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'cryozen_session_at', value: '' },
      { name: 'cryozen_session_rt', value: '' }
    ]),
    false
  )
})
...
test('gateway WS URL IPC result serializes success and the auth-vs-transport matrix', async () => {
  ...
  for (const error of [
    ...
    new Error('Timed out connecting to Cryozen backend after 8000ms'),
...
test('OAuth ticket-mint 401 stays on the reauth path', () => {
...
test('FIX #95701: a confirmed 401/403 ticket rejection is tagged isReauthRequired so startCryozen latches it', () => {
  for (const statusCode of [401, 403]) {
