...
// registry, and the cryozen:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // CRYOZEN_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.CRYOZEN_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Cryozen' TUI into
    ...
    env.TERM_PROGRAM = 'Cryozen'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a cryozen/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from CRYOZEN_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.CRYOZEN_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `cryozen:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('cryozen:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('cryozen:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('cryozen:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('cryozen:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('cryozen:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('cryozen:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
