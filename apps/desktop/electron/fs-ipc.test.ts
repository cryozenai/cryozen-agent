/**
 * `cryozen:fs:reveal` answers what it did (#115167). `shell.showItemInFolder`
...
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-fs-ipc-'))
...
const reveal = (target: string) => electron.handlers.get('cryozen:fs:reveal')!({}, target)

afterEach(() => {
  electron.showItemInFolder.mockClear()
})

describe('cryozen:fs:reveal', () => {
  ...
  it('reports false for a path that is not on this computer, and only shows one that is', async () => {
    ...
    await expect(reveal('/home/cryozen/.cryozen-agent/attachments/report.zip')).resolves.toBe(false)
    expect(electron.showItemInFolder).not.toHaveBeenCalled()
