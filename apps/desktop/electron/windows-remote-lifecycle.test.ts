...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    cryozenHome: 'C:\\Users\\andre\\.cryozen-agent',
    python: 'C:\\Users\\andre\\.cryozen-agent\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.cryozen-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      cryozenPath: 'C:\\Cryozen\\cryozen.exe',
      cryozenHome: 'C:\\Users\\andre\\.cryozen-agent',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command cryozen.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          cryozenHome: 'C:\\Users\\alice\\.cryozen-agent',
          cryozenPath: 'C:\\Cryozen\\cryozen.exe',
          python: 'C:\\Cryozen\\python.exe'
        })
      }

      if (script.includes('.cryozen-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('cryozen_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.cryozen-agent\\profiles\\research')
  assert.match(script, /\.cryozen-update-in-progress/)
...
test('Windows probe validates Cryozen and Python topology before selection', async () => {
  let script = ''
  await probeWindowsRemote(
    sshWith(async command => {
      script = Buffer.from(command.split(' ').at(-1) || '', 'base64').toString('utf16le')
  ...
  const fallbackJoin = script.indexOf('Join-Path $cryozenHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Cryozen is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Cryozen is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Cryozen's\\python.exe" }, 'inspect', [
    'C:\\x y\\cryozen.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'cryozen_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Cryozen''s/)
  assert.match(script, /C:\\x y\\cryozen\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { cryozenPath: lock.cryozenPath, cryozenHome: lock.cryozenHome }
