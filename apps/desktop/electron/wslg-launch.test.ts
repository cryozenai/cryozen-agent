...
describe('WSLg launch arguments', () => {
  it('preserves the invocation and restarts only once with native Wayland', () => {
    const args = ['.', '--inspect=9229', 'cryozen://session/example']
    ...
    expect(args).toEqual(['.', '--inspect=9229', 'cryozen://session/example'])
  })
  ...
  it('respects explicit backends and the existing config-bridged X11 hint', () => {
    ...
    expect(wslgLaunchArgs([], { ...env, CRYOZEN_DESKTOP_DISABLE_GPU: 'true' }, 'linux')).toEqual([
