/**
 * Shared HTTP transport policy for the Electron main process's Cryozen REST
 ...
 * isGatewayAuthRejection (401/403 → reauth, never retried),
 * ensureNativeAccessToken's dead-refresh-token
...
function htmlResponseError(url: string, statusCode: unknown, location?: unknown) {
  ...
  return new Error(
    `Expected JSON from ${url} but got HTML (status ${statusCode}). The endpoint is likely missing on the Cryozen backend.`
  )
}
