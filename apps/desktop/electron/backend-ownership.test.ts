...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'cryozen serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/cryozen serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m cryozen_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/cryozen --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Cryozen Runtime\\cryozen.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('cryozen chat --query serve'), false)
