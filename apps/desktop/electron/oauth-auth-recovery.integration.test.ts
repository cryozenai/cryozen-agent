...
async function fixture(beforeRefreshResponse?: () => Promise<void>) {
  const home = mkdtempSync(join(tmpdir(), 'cryozen-auth-recovery-'))
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.url === '/auth/native/refresh') {
      state.refreshes++
      expect(JSON.parse(body)).toMatchObject({ refresh_token: 'old-rt', provider: 'github' })
...
test('HTTP outage, dead-refresh and cookie coexistence reach the correct ticket/readiness/media verdicts', async () => {
  ...
  try {
    ...
    try {
      await waitForCryozenReady(f.baseUrl, {
    ...
    const mediaRequest = { url: 'cryozen-media://remote/%2Ftmp%2Fclip.mp4', headers: new Headers(), method: 'GET' }
