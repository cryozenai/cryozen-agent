// Unit tests for the pure Windows `cryozen` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvCryozenCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `cryozen` shim shadows
//      the real cryozen.cmd/cryozen.exe.
...
//   3. resolveVenvCryozenCommand() — must probe the venv python via
//      canImportCryozenCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvCryozen: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvCryozen: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvCryozen: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvCryozen: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvCryozen: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvCryozen: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvCryozenCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportCryozenCli: async () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    cryozenHome: '/fake/cryozen-home',
...
test('resolveVenvCryozenCommand: returns null off Windows', async () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(await resolveVenvCryozenCommand('/root/venv/Scripts/cryozen.exe', [], deps), null)
})

test('resolveVenvCryozenCommand: returns null for a .cmd/.bat script command', async () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(await resolveVenvCryozenCommand('/root/venv/Scripts/cryozen.cmd', [], deps), null)
})

test('resolveVenvCryozenCommand: returns null when the basename is not cryozen/cryozen.exe', async () => {
  const deps = makeDeps()

  assert.equal(await resolveVenvCryozenCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvCryozenCommand: returns null when the parent dir is not Scripts', async () => {
  const deps = makeDeps()

  assert.equal(await resolveVenvCryozenCommand('/root/venv/bin/cryozen.exe', [], deps), null)
})

test('resolveVenvCryozenCommand: returns null when the venv python does not exist on disk', async () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(await resolveVenvCryozenCommand('/root/venv/Scripts/cryozen.exe', [], deps), null)
})

test('resolveVenvCryozenCommand: probes the venv python before trusting it (returns null on failed probe)', async () => {
  let probed = false

  const deps = makeDeps({
    canImportCryozenCli: async (python: string) => {
      probed = true
  ...
  const result = await resolveVenvCryozenCommand('/root/venv/Scripts/cryozen.exe', ['serve'], deps)
...
test('resolveVenvCryozenCommand: returns the resolved python backend descriptor when the probe passes', async () => {
  const deps = makeDeps()
  const result = await resolveVenvCryozenCommand('/root/venv/Scripts/cryozen.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'cryozen_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvCryozenCommand: is case-insensitive on cryozen.exe and the Scripts dir name', async () => {
  const deps = makeDeps()

  assert.ok(await resolveVenvCryozenCommand('/root/venv/Scripts/CRYOZEN.EXE', [], deps))
  assert.ok(await resolveVenvCryozenCommand('/root/venv/SCRIPTS/cryozen.exe', [], deps))
})
