...
export function markerPath(cryozenHome) {
  return path.join(cryozenHome, '.cryozen-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(cryozenHome)
  let raw
 ...
 *
 * The Tauri-based cryozen-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(cryozenHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(cryozenHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Cryozen to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(cryozenHome, opts)
