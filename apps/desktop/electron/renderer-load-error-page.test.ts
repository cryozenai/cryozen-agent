...
test('error page names the failure and carries a Reload button', () => {
  ...
  assert.match(html, /Cryozen couldn.t start the desktop UI/)
  ...
  assert.match(html, /cryozen desktop --force-build/)
...
test('error page reload button targets the real renderer URL when provided', () => {
  const html = buildRendererLoadErrorPage({
    ...
    reloadUrl: 'file:///C:/Cryozen%20Agent/dist/index.html'
  })
  ...
  assert.match(html, /location\.replace\("file:\/\/\/C:\/Cryozen%20Agent\/dist\/index\.html"\)/)
...
test('a custom title overrides the default heading', () => {
  ...
  assert.match(html, /Cryozen desktop UI was terminated/)
  assert.doesNotMatch(html, /Cryozen couldn.t start the desktop UI/)
})
