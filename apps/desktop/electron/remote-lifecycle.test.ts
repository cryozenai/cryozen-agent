...
function ownedLock(over: any = {}) {
  return {
    ...
    port: 40000,
    profile: '',
    cryozenPath: '~/.local/bin/cryozen',
    cryozenHome: '~/.cryozen-agent',
...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.cryozen-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Cryozen', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('CRYOZEN_HOME')) {
          return '/home/alice/.cryozen-agent\n'
        }

        if (command.includes('.cryozen-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.cryozen-agent/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.cryozen-update-in-progress/)
})
...
test('listRemoteCryozenProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  ...
  assert.deepEqual(await listRemoteCryozenProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteCryozenProfiles rejects a hostile CRYOZEN_HOME', async () => {
  const ssh = fakeSsh([[/CRYOZEN_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteCryozenProfiles(ssh),
    (err: any) => {
...
test('locateCryozen prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/cryozen/, 'OK']])
  assert.equal(await locateCryozen(ssh, '/opt/cryozen'), '/opt/cryozen')
})

test('locateCryozen throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong cryozen" bug.
  const ssh = fakeSsh([
    [/command -v cryozen/, '/home/u/.local/bin/cryozen\n'],
    [/\[ -x .*\.local\/bin\/cryozen/, 'OK']
  ])

  await assert.rejects(
    () => locateCryozen(ssh, '/bad/path/cryozen'),
    (err: any) => {
      assert.equal(err.kind, 'cryozen-not-found')
      assert.match(err.message, /\/bad\/path\/cryozen/)
...
test('locateCryozen falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v cryozen/, '/home/u/.local/bin/cryozen\n'],
    [/\[ -x .*\.local\/bin\/cryozen/, 'OK']
  ])

  assert.equal(await locateCryozen(ssh, ''), '/home/u/.local/bin/cryozen')
})

test('locateCryozen preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$CRYOZEN_BIN" "$CRYOZEN_ENTRYPOINT" "$@",
  // where $CRYOZEN_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.cryozen-agent/cryozen-agent/venv/bin/python\n']
  ])

  assert.equal(await locateCryozen(ssh, ''), '/home/u/.local/bin/cryozen')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateCryozen must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateCryozen returns an explicit remoteCryozenPath unchanged', async () => {
  // The override half of #74411: an explicit remoteCryozenPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/cryozen changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/cryozen/, 'OK'],
    [/python3 -c/, '/home/u/.cryozen-agent/cryozen-agent/venv/bin/python\n']
  ])

  assert.equal(await locateCryozen(ssh, '~/.local/bin/cryozen'), '~/.local/bin/cryozen')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteCryozenPath must never be rewritten')
})

test('locateCryozen falls back to ~/.local/bin/cryozen when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateCryozen(ssh, ''), '~/.local/bin/cryozen')
})

test('locateCryozen tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/cryozen/, 'OK']])
  assert.equal(await locateCryozen(ssh, ''), '~/.cryozen-agent/cryozen-agent/venv/bin/cryozen')
})

test('locateCryozen throws a cryozen-not-found error with an install hint', async () => {
...
test('locateCryozen uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v cryozen/, '/x/cryozen'],
    [/\[ -x/, 'OK']
  ])

  await locateCryozen(ssh, '')
  assert.ok(
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/cryozen'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/cryozen serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/cryozen'), true)
  assert.equal(
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'cryozen wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'cryozen')
    const launcher = path.join(temp, 'cryozen launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/cryozen-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/cryozen', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('CRYOZEN_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/cryozen', 'work', {
    cryozenHome: '~/.cryozen-agent',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.cryozen-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"cryozen-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /cryozen-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'cryozen-update-mutex-'))
  const cryozenPath = path.join(directory, 'cryozen')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *cryozen-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(cryozenPath, '', {
      cryozenHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { cryozenPath: '/x/cryozen', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('CRYOZEN_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('CRYOZEN_DASHBOARD_READY port=8765')?.[1], '8765')
  // The remote log is `>> log 2>&1`, so a stderr chunk without a newline can be spliced onto the sentinel.
  assert.equal(READY_RE.exec('INFO  Started server process [4711]CRYOZEN_BACKEND_READY port=65238')?.[1], '65238')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { cryozenPath: '/x/cryozen', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nCRYOZEN_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'CRYOZEN_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'CRYOZEN_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.cryozen-agent\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Cryozen Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'CRYOZEN_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile cryozenPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ cryozenPath: '/old/stale/cryozen', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteCryozenPath: '/new/cryozen', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'CRYOZEN_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes cryozenHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/CRYOZEN_HOME/, '/home/alice/.cryozen-agent\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'CRYOZEN_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"cryozenHome":"\/home\/alice\/\.cryozen-agent"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'CRYOZEN_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'CRYOZEN_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/cryozen'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.cryozen-agent/cryozen-agent/venv/bin/cryozen'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/cryozen'))
  assert.doesNotThrow(() => validateRemotePath('~/.cryozen-agent/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/cryozen'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('cryozen'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/cryozen'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/cryozen\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/cryozen\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/cryozen\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/cryozen', '/usr/`id`/cryozen', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.cryozen-agent/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/cryozen')
  assert.ok(result.includes('/usr/local/bin/cryozen'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/cryozen')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('CRYOZEN_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/cryozen', 'work', {
    tokenFilePath: `~/.cryozen-agent/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.cryozen-agent\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/cryozen', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Cryozen', () => {
  const cmd = buildSpawnCommand('/x/cryozen', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env CRYOZEN_DESKTOP=1/)
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { cryozenPath: '/x/cryozen', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.cryozen-agent/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.cryozen-agent/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/cryozen serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/cryozen serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/cryozen'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/cryozen'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/cryozen'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/cryozen'), false)
})
...
test.skipIf(process.platform === 'win32')(
  ...
  async t => {
    ...
    const dir = await mkdtemp(path.join(os.tmpdir(), 'cryozen-zsh-probe-'))

    try {
      const cryozen = path.join(dir, 'cryozen')
      await writeFile(cryozen, '#!/bin/sh\necho "--ssh-session-token-file --ssh-owner-nonce"\n', { mode: 0o700 })

      const ssh = { exec: async (command: string) => (await exec(command, { shell: zsh })).stdout }

      assert.equal(await remoteSupportsSshOwnership(ssh, cryozen), true)
    } finally {
...
test('probes run under the remote watchdog so a hung CLI cannot orphan (#110478)', async () => {
  ...
  const versionSsh = fakeSsh([
    [
      ...
      (cmd: string) => {
        ...
        return 'Cryozen Agent v0.18.2 (abc123)\n'
      }
    ]
  ])

  assert.equal(await probeCryozenVersion(versionSsh, '/x/cryozen'), 'Cryozen Agent v0.18.2 (abc123)')
  ...
  assert.equal(await remoteSupportsSshOwnership(helpSsh, '/x/cryozen'), true)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'cryozen-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.cryozen-agent/.cryozen-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.cryozen-agent/desktop-ssh/${OWNERSHIP_ID}`
