...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'cryozen-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Cryozen Test'], { cwd: dir })
