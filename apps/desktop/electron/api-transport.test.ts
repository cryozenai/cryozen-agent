/**
 * Unit + live-transport tests for the Electron main process's Cryozen REST
...
describe('htmlResponseError', () => {
  ...
  it('does not blame credentials when the redirect only fixes the scheme or a trailing slash', () => {
    for (const [url, location] of [
      ...
      ['https://gateway.example.com/cryozen/api/health', 'https://gateway.example.com/cryozen/api/health/']
    ]) {
