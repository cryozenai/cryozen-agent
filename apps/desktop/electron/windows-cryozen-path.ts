/**
 * windows-cryozen-path.ts
 *
 * Pure, dependency-injected pieces of Windows `cryozen` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvCryozenCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `cryozen` shim shadowed the real
 *      cryozen.cmd/cryozen.exe; the shim then failed the --version probe and
 ...
 *      and Cryozen launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvCryozenCommand() — unwrapWindowsVenvCryozenCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `cryozen`) must
 * not shadow `cryozen.cmd`/`cryozen.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Cryozen launcher that drives `cryozen update`.
...
export interface ResolveVenvCryozenCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportCryozenCli: (python: string, opts?: { env?: Record<string, string> }) => Promise<boolean>
...
/**
 * If `command` is a Windows venv `cryozen`/`cryozen.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/cryozen(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m cryozen_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportCryozenCli(). A
 * venv whose update died mid-`pip install` still has python.exe + cryozen.exe
 ...
 *
 * Returns null when `command` is not a venv cryozen shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export async function resolveVenvCryozenCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvCryozenCommandDeps
): Promise<{
  label: string
  command: string
  args: string[]
  ...
  if (!/^cryozen(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    !(await canImportCryozenCli(python, {
      env: {
    ...
    rememberLog?.(
      `Ignoring venv Cryozen at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Cryozen Python at ${python}`,
    command: python,
