...
export function resolveDesktopRemoteRoute({
  ...
  if (override) {
    ...
    const route = { ...profileConfig, kind: 'remote' } as StoredRoute
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'CRYOZEN_DESKTOP_REMOTE_URL is set but CRYOZEN_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Cryozen backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `cryozen serve` the desktop never
  ...
  const route = { ...config.remote, kind: 'remote' } as StoredRoute
