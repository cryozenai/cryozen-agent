...
async function setup() {
  electron.directory = await mkdtemp(path.join(os.tmpdir(), 'cryozen-screenshot-'))
...
const call = (channel: string, event: unknown, ...args: unknown[]) =>
  electron.handlers.get(`cryozen:screenshot:${channel}`)(event, ...args)
...
describe.skipIf(process.platform !== 'darwin')('Command screenshot native bridge', () => {
  it('persists opt-in, routes to the last focused subscribed window while backgrounded, and revokes on disable', async () => {
    ...
    ipcMain.emit('cryozen:screenshot:subscribe', first.event, true)
    ipcMain.emit('cryozen:screenshot:subscribe', second.event, true)
    ...
    const requests = second.wc.send.mock.calls.filter(([channel]) => channel === 'cryozen:screenshot:request')
    expect(requests).toHaveLength(1)
    expect(first.wc.send.mock.calls.some(([channel]) => channel === 'cryozen:screenshot:request')).toBe(false)
    ...
    for (let i = 0; i < 3; i += 1) {
      ipcMain.emit('cryozen:screenshot:subscribe', second.event, false)
      ipcMain.emit('cryozen:screenshot:subscribe', second.event, true)
    }
