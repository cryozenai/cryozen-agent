...
const READY_RE = /^CRYOZEN_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitCryozenPath = '') {
  const explicit = psLiteral(explicitCryozenPath)
  ...
  const script = [
    ...
    '$cryozenHome=$env:CRYOZEN_HOME',
    'if(-not $cryozenHome){$cryozenHome=Join-Path $env:LOCALAPPDATA "cryozen"}',
    'Assert-NoReparse $cryozenHome $true',
    '$candidate=[IO.Path]::Combine($cryozenHome, "cryozen-agent\\venv\\Scripts\\cryozen.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "cryozen-agent\\.venv\\Scripts\\cryozen.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $cryozenHome "cryozen-agent\\venv\\Scripts\\cryozen.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "cryozen-agent\\.venv\\Scripts\\cryozen.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command cryozen.exe -ErrorAction SilentlyContinue',
    ...
    '$cryozen=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$cryozen=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $cryozen){throw "Cryozen is not installed on the remote Windows host."}',
    'Assert-NoReparse $cryozen $false',
    'if($explicit -and $cryozen -ne $explicit){throw "The configured Cryozen path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($cryozen), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;cryozenHome=$cryozenHome;cryozenPath=$cryozen;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(cryozenHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class CryozenMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$cryozenHome=${psLiteral(cryozenHome)}`,
    '$installRoot=$cryozenHome',
    '$parent=Split-Path -Parent $cryozenHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".cryozen-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[CryozenMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, cryozenHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(cryozenHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitCryozenPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitCryozenPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'cryozen_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'cryozen_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'cryozen_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$cryozenHome=${psLiteral(runtime.cryozenHome)}`,
    '$installRoot=$cryozenHome',
    '$parent=Split-Path -Parent $cryozenHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".cryozen-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};cryozenPath=${psLiteral(reservation.cryozenPath)};cryozenHome=${psLiteral(reservation.cryozenHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.cryozenPath === 'string' &&
    typeof lock.cryozenHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.cryozenPath === runtime.cryozenPath &&
    lock.cryozenHome === runtime.cryozenHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteCryozenPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.cryozenHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.cryozenPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Cryozen on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.cryozenPath = inspection.path
  const cryozenVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located cryozen at ${runtime.cryozenPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, cryozenPath: runtime.cryozenPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForCryozen(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Cryozen SSH"', 'powershell.exe -NoLogo')
