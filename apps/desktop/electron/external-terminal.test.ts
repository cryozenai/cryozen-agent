...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  const env = terminalScriptEnv(
    ...
    '/home/b/.cryozen-agent'
  )

  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', CRYOZEN_HOME: '/home/b/.cryozen-agent' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'cryozen_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.cryozen-agent/cryozen-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.cryozen-agent/cryozen-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.cryozen-agent/cryozen-agent'`,
      `exec '/home/b/.cryozen-agent/cryozen-agent/venv/bin/python' '-m' 'cryozen_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\cryozen\\venv\\Scripts\\cryozen.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\cryozen\\venv\\Scripts\\cryozen.exe" "--tui" "--resume" "sess"',
    ''
  ])
})
