...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.cryozen-agent`). Default's state.db and gateway.pid live here. */
  cryozenHome: string
  /** Named-profile root (`~/.cryozen-agent/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isCryozenProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `cryozenHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, cryozenHome: string, profilesRoot: string): string {
  return name === 'default' ? `${cryozenHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, cryozenHome: string, profilesRoot: string): string {
  return name === 'default' ? `${cryozenHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveCryozenHome(profilesRoot: string, cryozenHome?: string): string {
  if (cryozenHome) {
    return cryozenHome
  }

  // Tests that predate cryozenHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live cryozen process.
 ...
 *
 * `cryozenHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isCryozenProcess'> & { cryozenHome?: string }
): string[] {
  const cryozenHome = resolveCryozenHome(profilesRoot, deps.cryozenHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, cryozenHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.cryozenHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$CRYOZEN_HOME`, not `$CRYOZEN_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `cryozen --profile default` and is worse than writing nothing
