 ...
 *     `X-Cryozen-Session-Token` header; WS uses `?token=`.
...
// cryozen_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - cryozen_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - cryozen_session_rt: the OAuth refresh token. Long-lived (24h rotating,
...
const AT_COOKIE_VARIANTS = ['__Host-cryozen_session_at', '__Secure-cryozen_session_at', 'cryozen_session_at']
const RT_COOKIE_VARIANTS = ['__Host-cryozen_session_rt', '__Secure-cryozen_session_rt', 'cryozen_session_rt']

// Keep this aligned with cryozen_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['cryozen', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startCryozen latches
  ...
  // statusCode (the fetch layer attaches err.statusCode). Downstream
  // classification (isServerSideHttpError) and the renderer overlay depend on
  // it surviving the ticket-error wrapper. Auth
...
/** Which Cryozen profile the remote SSH dashboard should actually run as.
...
// True for connection modes that resolve to a REMOTE (URL) backend.
// Centralized so every resolution site asks the same question.
...
function normalizeSshConfig(entry) {
  ...
  const remoteCryozenPath = String(entry.remoteCryozenPath || '').trim()

  if (remoteCryozenPath) {
    out.remoteCryozenPath = remoteCryozenPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Cryozen installation. Preserve an explicit mapping
  // when it is a valid Cryozen profile identifier; otherwise fall back to the
 ...
 * override only with a remote `mode` and a non-empty
...
export interface ProfileRouteOptions {
  ...
  ownEntry?: boolean
  /** `CRYOZEN_DESKTOP_ISOLATED_BACKEND=1`: opt out of the host singleton. */
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_cryozen_action` registers the (often dynamic, e.g.
 ...
 * `CRYOZEN_HOME` — so it keeps a pooled, profile-scoped backend even though every
 ...
 *     (multiplex-only: one `cryozen serve` per HOST). The two ways out are
 *     `CRYOZEN_DESKTOP_ISOLATED_BACKEND=1`, which gives this app a private
 *     backend, and a MUTATING request the server cannot scope at all — that
 *     one keeps a pooled backend whose CRYOZEN_HOME does the scoping, so a
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process CRYOZEN_HOME can belong to a different
    ...
    // The same holds for the LOCAL host backend: with one `cryozen serve` per
  ...
  //    a `cryozen serve` child of its own — UNLESS this request mutates state
  //    the server cannot scope, in which case the pooled backend's own
  //    CRYOZEN_HOME is the only scope left and it keeps one.
 ...
 * `cryozen:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://cryozen.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `cryozen:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a cryozen session ACCESS-token cookie
