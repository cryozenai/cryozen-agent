 ...
 *
 * So we delegate to `cryozen gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `cryozen update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(cryozenCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(cryozenCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
...
function runGatewayLifecycleCommand(cryozenCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(cryozenCliPath, args)
  }

  if (!existsSync(cryozenCliPath)) {
    return false
  }

  try {
    exec(cryozenCliPath, args, {
