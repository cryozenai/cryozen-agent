...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/cryozen-runner-test',
    sourceRepoRoot: null,
    cryozenHome: '/tmp/cryozen-runner-test',
    logRoot: '/tmp/cryozen-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'cryozen-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'cryozen-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/cryozen-agent', '--cryozen-home', '/tmp/cryozen', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/cryozen',
      cryozenHome: '/tmp/home'
    }),
    ['--dir', '/tmp/cryozen', '--cryozen-home', '/tmp/home', '--branch', 'main']
  )
})
...
test('installer log lines reach the emitter without escape sequences; \\r redraws keep the last frame', () => {
  ...
  assert.equal(cleanInstallerLogLine('\u001b]0;cryozen\u0007Installing Cryozen'), 'Installing Cryozen')
