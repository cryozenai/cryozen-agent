...
/**
 * Cryozen-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\cryozen\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$CRYOZEN_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_cryozen_node_dirs()` in cryozen_constants.py, which the Electron main
 * process cannot import.
 */
function cryozenManagedNodePathEntries(
  cryozenHome,
  ...
  const root = pathModule.join(cryozenHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const cryozenNodeDirs = cryozenManagedNodePathEntries(cryozenHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([cryozenNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeCryozenHomeRoot(
  cryozenHome,
  ...
  // would pin it under cwd and the Python backend inherits that absolute path via CRYOZEN_HOME.
  let raw = String(cryozenHome)
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). cryozen_bootstrap sets this inside the child too, but only
