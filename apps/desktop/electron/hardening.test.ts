...
function withTempDir(run: (dir: string) => void) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-secret-file-'))
...
test('attachment data URL helper reads bytes above the preview default without changing that limit', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-desktop-large-attachment-'))
...
test('resolveRequestedPathForIpc resolves relative paths from the trimmed base directory', () => {
  const baseDir = path.join(os.tmpdir(), 'cryozen-desktop-base')
...
test('resolveReadableFileForIpc validates existence type size and sensitivity', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-desktop-hardening-'))
...
test('resolveReadableFileForIpc blocks common sensitive files', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-desktop-sensitive-'))
...
test('resolveReadableFileForIpc blocks symlinks whose realpath is sensitive', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-desktop-realpath-'))
...
test('resolveDirectoryForIpc accepts directories and rejects invalid directory targets', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-desktop-dir-'))
...
test('resolveDirectoryForIpc accepts directory symlinks or junctions', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cryozen-desktop-dir-link-'))
...
// (see windows-cryozen-resolution.test.ts). These pin the propagation the PR
...
test('connection-config save and apply IPC handlers route payloads through coerceDesktopConnectionConfig', () => {
  ...
  for (const channel of ['cryozen:connection-config:save', 'cryozen:connection-config:apply']) {
...
test('homeRelativeAttachmentCandidates tries the home dir and the CRYOZEN_HOME attachments dir', () => {
  const candidates = homeRelativeAttachmentCandidates(
    'AppData/Local/cryozen/attachments/foo.xlsx',
    '/Users/alice',
    '/Users/alice/AppData/Local/cryozen'
  )

  assert.deepEqual(candidates, [
    path.join('/Users/alice', 'AppData/Local/cryozen/attachments/foo.xlsx'),
    path.join('/Users/alice/AppData/Local/cryozen', 'attachments', 'foo.xlsx')
  ])
})
...
test('homeRelativeAttachmentCandidates normalizes Windows backslashes before joining', () => {
  ...
  assert.equal(candidates[0], path.join('/Users/alice', 'AppData/Local/cryozen/attachments/foo.xlsx'))
})

test('homeRelativeAttachmentCandidates returns nothing for an absolute path', () => {
  assert.deepEqual(
    homeRelativeAttachmentCandidates('/already/absolute/foo.xlsx', '/Users/alice', '/Users/alice/.cryozen-agent'),
    []
  )
})
...
test('homeRelativeAttachmentCandidates returns nothing for empty input', () => {
  assert.deepEqual(homeRelativeAttachmentCandidates('', '/Users/alice', '/Users/alice/.cryozen-agent'), [])
  assert.deepEqual(homeRelativeAttachmentCandidates('   ', '/Users/alice', '/Users/alice/.cryozen-agent'), [])
})
...
test('homeRelativeAttachmentCandidates second candidate falls back to basename only', () => {
  ...
  const candidates = homeRelativeAttachmentCandidates('foo.xlsx', '/Users/alice', '/Users/alice/.cryozen-agent')

  assert.deepEqual(candidates, [
    path.join('/Users/alice', 'foo.xlsx'),
    path.join('/Users/alice/.cryozen-agent', 'attachments', 'foo.xlsx')
  ])
})
