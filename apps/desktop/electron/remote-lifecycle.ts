 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Cryozen dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Cryozen install on the remote (login-shell probe),
 ...
 *     `CRYOZEN_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForCryozen in.
 *
 * The minted CRYOZEN_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.cryozen-agent/desktop-ssh'
...
// Resolve the remote cryozen executable. An EXPLICIT path is honored strictly
...
async function locateCryozen(ssh, remoteCryozenPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The cryozen binary or wrapper script
    ...
    //     the Cryozen version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/cryozenai/cryozen-agent/issues/74411
    return candidate
  }
  ...
  if (remoteCryozenPath) {
    if (await isExecutable(remoteCryozenPath)) {
      return resolveLauncher(remoteCryozenPath)
    }

    const err: any = new Error(
      `The Cryozen path you set is not an executable on the remote host: "${remoteCryozenPath}". ` +
        'Check the path (it must be the full path to the `cryozen` binary on the remote, e.g. ' +
        '~/cryozen-agent/.venv/bin/cryozen), or clear it to auto-detect.'
    )

    err.kind = 'cryozen-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v cryozen')}`)).trim()
  ...
  candidates.push('~/.local/bin/cryozen')
  candidates.push('/usr/local/bin/cryozen')
  candidates.push('~/.cryozen-agent/cryozen-agent/venv/bin/cryozen')
  ...
  const err: any = new Error(
    'Cryozen is not installed on the remote host (could not find a `cryozen` executable). ' +
      'Install it on the remote with:  curl -fsSL https://cryozenai.github.io/cryozen-agent/install.sh | sh  ' +
      '— or set the Cryozen path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<cryozen> --version`,
// e.g. "Cryozen Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH cryozen a
// connection uses, so a stale/unexpected install is visible.
async function probeCryozenVersion(ssh, cryozenPath) {
  try {
    // Watchdogged: a hung remote CLI must die remotely instead of orphaning
    // when the local ssh child is SIGKILLed (#110478).
    const out = (await ssh.exec(withRemoteTimeout(`${expandRemotePath(cryozenPath)} --version 2>&1`))).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Cryozen Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The CRYOZEN_HOME the remote dashboard will use (explicit env wins, else
// ~/.cryozen-agent). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteCryozenHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${CRYOZEN_HOME:-$HOME/.cryozen-agent}"')).trim().split('\n').pop()

    return out || '~/.cryozen-agent'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Cryozen home.')
...
marker=home/'.cryozen-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Cryozen checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, cryozenHome) {
  const home = assertSafeRemoteHome(cryozenHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Cryozen install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Cryozen update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Cryozen update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteCryozenProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteCryozenHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Cryozen profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
async function readRemoteInstallId(ssh) {
  // The stable backend identity the roster collapses on (`cryozen_cli/install_identity.py`:
  ...
  const root = remoteInstallRoot(assertSafeRemoteHome(await probeRemoteCryozenHome(ssh)))
  ...
  try {
    ...
    const error: any = new Error('Could not read the remote Cryozen install id.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Cryozen home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'cryozenPath', 'cryozenHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !cryozenPath) {
    return false
  }
  ...
  const script =
    ...
    `pid=${Number(pid)}\n` +
    `expected=os.path.expanduser(${shq(cryozenPath)})\n` +
    // The installer-facing launcher is intentionally preserved for invocation
    // (#74411), but it may `exec python <install-dir>/cryozen`, leaving neither
    // launcher nor CRYOZEN_HOME-derived entrypoint in argv. The ownership-scoped
    // token path + random nonce + exact profile below are the alternative proof.
    `cryozen_home=os.path.expanduser(${shq(cryozenHome)}) if ${shq(cryozenHome)} else ""\n` +
    'expected_entries={expected}\n' +
    'if cryozen_home:\n' +
    ' expected_entries.add(os.path.join(cryozen_home,"cryozen-agent","venv","bin","cryozen"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.cryozenPath)
  const expectedHome = lock.cryozenHome ? expandRemotePath(lock.cryozenHome) : "''"
  ...
  const executableMatch = lock.cryozenHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.cryozenPath === expected.cryozenPath &&
    lock.cryozenHome === expected.cryozenHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.cryozenPath)})
cryozen_home=os.path.expanduser(${py(lock.cryozenHome)})
expected_entries={expected_path,os.path.join(cryozen_home,"cryozen-agent","venv","bin","cryozen")}
...
// execing Cryozen. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"cryozen-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(cryozenPath, profile, opts: any = {}) {
  const cryozen = expandRemotePath(cryozenPath)
  ...
  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.cryozenHome || '~/.cryozen-agent')}/.cryozen-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env CRYOZEN_DESKTOP=1 ${cryozen} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} cryozen-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, cryozenPath) {
  const cryozen = expandRemotePath(cryozenPath)
  ...
  const out = await ssh.exec(
    `help="$( ${withRemoteTimeout(`${cryozen} serve --help 2>&1`)} )"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { cryozenPath, profile, token, ownershipId, cryozenHome = '~/.cryozen-agent', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, cryozenPath))) {
    const err: any = new Error(
      'The remote Cryozen install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Cryozen on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(cryozenPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        cryozenHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForCryozen, a token-gated
...
async function connect(deps) {
  ...
  const cryozenHome = await probeRemoteCryozenHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, cryozenHome)
  const cryozenPath = await locateCryozen(ssh, remoteCryozenPath)
  log(`located cryozen at ${cryozenPath}`)
  const cryozenVersion = await probeCryozenVersion(ssh, cryozenPath)

  if (cryozenVersion) {
    log(`remote cryozen version: ${cryozenVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Cryozen Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.cryozenPath === cryozenPath &&
      lock.cryozenHome === cryozenHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    cryozenHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, cryozenHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForCryozen(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
