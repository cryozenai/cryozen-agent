...
// Relative, not `@cryozen/shared`: the electron bundle is built by esbuild with
...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so Cryozen Desktop cannot save remote gateway tokens. ' +
        ...
        'or set CRYOZEN_DESKTOP_REMOTE_URL and CRYOZEN_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set CRYOZEN_DESKTOP_REMOTE_URL and CRYOZEN_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
...
// CRYOZEN_DESKTOP_REMOTE_* env vars.
 ...
 * "AppData/Local/cryozen/attachments/foo.xlsx" on Windows, or
 * ".cryozen-agent/attachments/foo.xlsx" elsewhere) rather than relative to cwd
 * (#115609). Pure and DI-testable: takes `home`/`cryozenHome` as arguments
 ...
 */
function homeRelativeAttachmentCandidates(raw, home, cryozenHome) {
  ...
  return [path.join(home, normalized), path.join(cryozenHome, 'attachments', path.basename(normalized))]
}
