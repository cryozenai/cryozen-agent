"""Centralized logging setup for Cryozen Agent.
...
# in Cryozen (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
from cryozen_constants import get_config_path, get_cryozen_home, mkdir_under_cryozen_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_cryozen_session_injector", False):
        return
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.cryozen_home = str(get_cryozen_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.cryozen_home = ""  # type: ignore[attr-defined]
        return record
    _session_record_factory._cryozen_session_injector = True  # type: ignore[attr-defined]
...
# Logger name prefixes per component; used by _ComponentFilter and ``cryozen logs --component``.
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "cryozen_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("cryozen_cli", "cli"),
    "cron": ("cron",),
    "gui": ("cryozen_cli.web_server", "cryozen_cli.pty_bridge", "cryozen_cli.desktop", "tui_gateway", "uvicorn"),
}
...
def setup_logging(
    *,
    cryozen_home: Optional[Path] = None,
    ...
    """Configure the Cryozen logging subsystem; returns the ``logs/`` directory.
    ...
    home = cryozen_home or get_cryozen_home()
    log_dir = mkdir_under_cryozen_home(home / "logs")
    # A second Cryozen home in a process that already logs for another one — a dashboard or
    # ``cryozen serve`` backend building agents for several profiles, a multiplexed gateway —
...
def setup_verbose_logging() -> None:
    ...
    if any(getattr(h, "_cryozen_verbose", False) for h in root.handlers):
        return
    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._cryozen_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from cryozen_cli.config import is_managed
        ...
        if _is_unavailable_log_stream(exc):
            ...
            if not self._unavailable_reported:
                ...
                _quietly(lambda: print(
                    f"cryozen_logging: {self.baseFilename} unavailable ({exc}); "
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Cryozen home.
    ...
    def __init__(self, existing: RotatingFileHandler, profile_homes: Sequence[Path]) -> None:
        ...
        self._cryozen_routed_log_path = resolved
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "cryozen_home", "")
        try:
...
def _register_queued_handler(handler: logging.Handler) -> None:
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._cryozen_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="cryozen-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_cryozen_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_cryozen_routed_log_path", None) == resolved or (
...
def _read_logging_config():
    ...
    try:
        ...
        # can pin logging.*) so this reuses cryozen_cli.main's early parse (one config.yaml parse
        # per process); fall back to a direct parse for bare cryozen_logging consumers.
        try:
