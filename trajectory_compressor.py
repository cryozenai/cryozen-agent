...
from cryozen_constants import OPENROUTER_BASE_URL, get_cryozen_home
from agent.retry_utils import jittered_backoff
from cryozen_cli.env_loader import load_cryozen_dotenv

# Load .env from CRYOZEN_HOME first, then project root as a dev fallback.
load_cryozen_dotenv(cryozen_home=get_cryozen_home(), project_env=Path(__file__).parent / ".env")
...
_PROVIDER_HOSTS: Tuple[Tuple[str, str], ...] = (
    ("openrouter.ai", "openrouter"), ("z.ai", "zai"), ("moonshot.ai", "kimi-coding"),
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: cryozen setup")
