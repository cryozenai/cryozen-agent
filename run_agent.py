...
try:
    ...
    pass  # partial `cryozen update` — only skips the Windows UTF-8 stdio setup
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    """cwd to stamp on a new session row (``cryozen -c`` / ``--resume``), or None.
...
# Sources that label the human conversation an interactive UI transport hosts. A finite ``cryozen chat -q`` /
# one-shot child spawned from such a session inherits CRYOZEN_SESSION_SOURCE (the terminal tool bridges the
# session env into child processes) but is NOT that conversation: labelling it ``tui``/``desktop`` lists it
# in the TUI/WebUI pickers as a resumable chat and lets ``cryozen -c`` in the TUI continue it (#112550).
# Automation sources (kanban, tool, cron, a2a, ...) are inherited on purpose.
_UI_TRANSPORT_SOURCES = frozenset({"tui", "desktop"})

# Finite non-interactive CLI runs (``cryozen chat -q``/``--oneshot``, ``cryozen -z``) get their own source so human
# pickers hide them without title/cwd heuristics; ``cryozen -c`` still treats them as CLI history.
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    ...
    source = str(get_session_env("CRYOZEN_SESSION_SOURCE", "") or "").strip()
    single_query = get_session_env("CRYOZEN_SINGLE_QUERY_SESSION", "") == "1"
    explicit = get_session_env("CRYOZEN_SESSION_SOURCE_EXPLICIT", "") == "1"
...
_cryozen_home = get_cryozen_home()  # read by agent_init via _ra()._cryozen_home
_loaded_env_paths = load_cryozen_dotenv(cryozen_home=_cryozen_home, project_env=Path(__file__).parent / '.env')
...
from agent.rate_limit_headers import RateLimitHeadersMixin
...
class AIAgent(
    ...
    InterruptControlMixin, TurnExplainersMixin, ActivityTrackingMixin, RateLimitHeadersMixin,
    SessionPersistenceMixin, CompressionFacadeMixin, TurnFacadeMixin, VisionMessagePrepMixin, ReasoningParamsMixin,
):
    """AI Agent with tool calling capabilities."""

    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[cryozen-agent: tool call arguments were corrupted in this session and "
    ...
    def _session_row_model_config(self) -> Any:
        ...
        /yolo toggle for ``cryozen --resume``.
        """
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from cryozen_cli.models_local import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        ``CRYOZEN_API_TIMEOUT`` > 1800s."""
        cfg = get_provider_request_timeout(self.provider, self.model)
        return cfg if cfg is not None else env_float("CRYOZEN_API_TIMEOUT", 1800.0)

    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        """Base non-stream stale timeout: per-model ``stale_timeout_seconds`` > provider-wide >
        ``CRYOZEN_API_CALL_STALE_TIMEOUT`` > reasoning floor > 90s.
        ...
        if cfg is not None:
            return cfg, False
        env_timeout = os.getenv("CRYOZEN_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        return (get_provider_stale_timeout(self.provider, self.model) is not None
                or os.getenv("CRYOZEN_API_CALL_STALE_TIMEOUT") is not None)
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        return (
            ...
            "See cryozen-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(model: str, *, provider: Optional[str] = None) -> bool:
        ...
        if normalized_provider == "custom" or is_actual_route(provider):
            return False
